...
TL;DR: nuCarla is introduced as a large-scale BEV perception dataset in CARLA simulator, addressing the lack of interactive datasets for closed-loop E2E autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack interactive conditions and standardized formats hindering closed-loop E2E model development.

Method: nuCarla mimics nuScenes format, ensures balanced class distribution, and supports closed-loop simulation with high-performance BEV backbones.

Result: nuCarla achieves state-of-the-art detection results and accelerates E2E autonomous driving research.

Conclusion: nuCarla facilitates reliable, safety-aware autonomous driving research by providing scalable, interactive datasets and models.
...
TL;DR: A framework is proposed to address overconfidence in Open Set Recognition (OSR) by mitigating inter-class overlap, combining uncertainty estimation and unknown detection for better performance.


<details>
  <summary>Details</summary>
Motivation: Overconfidence in OSR, caused by inter-class overlap, leads to misclassification of unknown samples as known classes, undermining model reliability.

Method: The framework includes a perturbation-based uncertainty estimation module and a two-stage unknown detection module to enhance discrimination between known and unknown classes.

Result: Experiments on three public datasets demonstrate superior performance compared to existing OSR methods.

Conclusion: The proposed framework effectively reduces overconfidence and improves OSR performance by leveraging uncertainty estimation and distinct classifiers.
...
TL;DR: FSVOD tackles novel object detection in videos with limited labeled data, addressing occlusion and appearance challenges. The proposed method uses selective feature propagation for temporal consistency, improving accuracy without complex proposals.


<details>
  <summary>Details</summary>
Motivation: Traditional detection methods need extensive labeled data and struggle with occlusion/appearance variations in videos. FSVOD aims to detect novel objects efficiently in few-shot settings.

Method: Uses object-aware temporal modeling with filtering to propagate high-confidence features, reducing noise and enhancing consistency. Combines few-shot trained detection/classification heads.

Result: Achieves AP improvements: 3.7% (FSVOD-500), 5.3% (FSYTV-40), 4.3% (VidOR), 4.5% (VidVRD) in 5-shot. Shows gains in 1/3/10-shot settings.

Conclusion: The approach efficiently detects novel objects in videos with limited data, improving temporal consistency and accuracy without relying on expensive proposals.
...
TL;DR: The paper introduces FusionFM, a novel multi-modal image fusion method that uses flow matching for efficient probabilistic transport and leverages pseudo-labels from existing models for supervision. It also includes a Fusion Refiner and mechanisms for multi-task learning.


<details>
  <summary>Details</summary>
Motivation: Existing fusion methods are task-specific and costly, while generative models are slow. The paper aims to improve efficiency and scalability.

Method: Uses flow matching for direct probabilistic transport, pseudo-labels for supervision, a Fusion Refiner for enhancement, and multi-task learning mechanisms.

Result: Achieves competitive performance across tasks with improved sampling efficiency and lightweight design.

Conclusion: FusionFM provides an efficient, scalable, and lightweight solution for multi-modal image fusion.
...
TL;DR: A two-stage trajectory-free crash detection framework uses diffusion-based road segment map generation to identify crashes by comparing monitored maps with model-generated ones, improving real-time crash detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-time crash detection is crucial for proactive safety management and traffic efficiency, but traditional methods rely on trajectory acquisition and vehicle tracking, which have limitations.

Method: The proposed framework includes Mapfusion, a diffusion-based model that generates future road segment maps by learning motion patterns from non-crash data, and compares these with monitored maps for crash detection.

Result: Mapfusion effectively generates realistic road segment maps and accurately detects crashes in real-world experiments, demonstrating robustness across various sampling intervals.

Conclusion: The two-stage framework provides a reliable and efficient solution for real-time crash detection without relying on trajectory data, enhancing traffic safety management.
...
TL;DR: The paper introduces ADATG, a novel adaptive two-grid foundation model training strategy with Hilbert encoding for seismic data, addressing the limitations of existing ViTs by efficiently capturing high- and low-frequency features.


<details>
  <summary>Details</summary>
Motivation: Existing vision transformers (ViTs) fail to effectively process seismic data due to its unique high- and low-frequency characteristics, necessitating specialized techniques.

Method: Proposes ADATG, using spectrum decomposition and hierarchical Hilbert encoding, alongside an adaptive training strategy emphasizing coarse-to-fine feature refinement.

Result: Extensive experiments confirm the method's effectiveness in capturing seismic data features, enhancing foundation model pretraining.

Conclusion: The research underscores the importance of tailored encoding and training strategies for seismic data, improving visual seismic foundation models.
...
TL;DR: The paper proposes a method for passive dementia screening using facial temporal microdynamics from unscripted videos, outperforming traditional speech-based approaches with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing dementia screening methods rely on speech or scripted interviews, limiting scalability and accessibility. The study aims to develop a language-free, scalable approach using natural facial behaviors.

Method: The study analyzes temporal facial kinematics (e.g., blink dynamics, mouth/jaw motions, gaze variability) from videos. Signals are stabilized, converted into microdynamic time series, and summarized into clip-level statistics for screening.

Result: On the YT DemTalk dataset, the method achieved high performance (AUROC: 0.953, AP: 0.961, F1: 0.851, accuracy: 0.857), with gaze lability and mouth/jaw dynamics being the most informative cues.

Conclusion: Facial microdynamics alone can effectively screen for dementia without relying on speech or text, offering a scalable, language-free alternative to traditional methods.
...
TL;DR: The paper highlights the limitations of Chain-of-Thought (CoT) prompting in simulating real-world dynamics and introduces Chain-of-Frames (CoF) reasoning for visual sequences. It proposes Gen-ViRe, a benchmark assessing CoF reasoning across six cognitive dimensions to evaluate video models as reasoners.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fail to evaluate CoF reasoning, limiting understanding of model capabilities in multi-step planning and other cognitive tasks. Gen-ViRe aims to fill this gap by providing a systematic framework grounded in cognitive science.

Method: Gen-ViRe decomposes CoF reasoning into six cognitive dimensions and 24 subtasks, using multi-source data curation, prompting protocols, and hybrid VLM-assisted evaluation.

Result: Experiments on state-of-the-art systems show discrepancies between visual quality and reasoning depth, establishing baselines for future improvements.

Conclusion: Gen-ViRe offers the first quantitative assessment of CoF reasoning, providing diagnostic tools to advance video models as genuine world simulators.
...
TL;DR: The paper introduces ranking-based losses (RSPose) to address key issues in heatmap-based human pose estimation, outperforming traditional losses and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: To solve problems like MSE loss inefficiency, heatmap imbalance, and metric-loss misalignment in human pose estimation.

Method: Proposes ranking-based losses for one-dimensional and two-dimensional heatmaps, tested on COCO, CrowdPose, and MPII datasets.

Result: RSPose achieves 79.9 mAP with ViTPose-H and improves SimCC Resnet-50 by 1.5 AP.

Conclusion: Ranking-based losses align better with evaluation metrics, enhancing localization and accuracy in pose estimation.
...
TL;DR: The paper introduces Collision Sound Source Segmentation (CS3), a task to segment objects causing collision sounds in video frames using audio cues. It proposes a weakly-supervised method leveraging foundation models and egocentric cues, achieving significant performance improvements on new benchmarks.


<details>
  <summary>Details</summary>
Motivation: Humans' multisensory perception inspires the task of identifying objects responsible for collision sounds in videos, addressing challenges like cluttered scenes and brief interactions.

Method: A weakly-supervised approach using CLIP and SAM2 foundation models, enhanced with egocentric cues like objects in hands, to segment sound-producing objects.

Result: The method outperforms baselines by 3× and 4.7× in mIoU on EPIC-CS3 and Ego4D-CS3 benchmarks.

Conclusion: The proposed approach effectively tackles CS3, demonstrating the potential of integrating audio and visual cues for object segmentation in dynamic, cluttered scenes.
...
TL;DR: The paper proposes Geometric Representation Regression (GRR), a geometrically-grounded alternative to Absolute Pose Regression (APR), by predicting disentangled 3D representations to improve camera pose estimation.


<details>
  <summary>Details</summary>
Motivation: Current APR models memorize training views instead of understanding 3D scene geometry, limiting their generalizability.

Method: GRR predicts two geometric representations—ray bundle directions and a pointmap—to estimate camera rotation and translation separately. A differentiable solver recovers the final pose.

Result: GRR achieves state-of-the-art performance on 7-Scenes and Cambridge Landmarks datasets.

Conclusion: Disentangling geometric representations introduces robustness and outperforms traditional APR methods.
...
TL;DR: The paper introduces a new multi-sequence MRI dataset for bladder cancer recurrence prediction and proposes H-CNN-ViT, a hierarchical model combining global and local features, achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: Bladder cancer recurrence monitoring is challenging due to post-surgical alterations in MRI scans and lack of dedicated datasets.

Method: Proposes H-CNN-ViT, a hierarchical gated attention multi-branch model combining ViT and CNN features, processing each modality independently.

Result: Achieves an AUC of 78.6%, outperforming existing models.

Conclusion: The dataset and H-CNN-ViT model offer a benchmark and improved solution for bladder cancer recurrence prediction.
...
TL;DR: QwenCLIP replaces CLIP's text encoder with an LLM-based embedding module to handle long-form radiology reports, improving medical image-text alignment.


<details>
  <summary>Details</summary>
Motivation: CLIP's text encoder is limited to 77 tokens, restricting its ability to process detailed radiology reports. Existing domain-specific encoders like PubMedBERT also face input length constraints.

Method: QwenCLIP replaces CLIP's text encoder with an LLM-based embedding module (e.g., Qwen3-Embedding) and uses learnable prompts for better cross-modal alignment.

Result: The framework captures richer medical semantics from long-form text, enhancing image-text alignment and performance on radiology benchmarks.

Conclusion: QwenCLIP effectively addresses CLIP's limitations for medical applications by leveraging LLMs' extended context and semantic depth.
...
TL;DR: This paper introduces a hybrid model combining EfficientNet and VGG19 with custom components for lumbar spine degeneration classification, outperforming baseline models with high precision and accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional transfer learning methods lack detailed anatomical feature representation and redundancy handling in medical images.

Method: The model integrates EfficientNet and VGG19, adding a Pseudo-Newton Boosting layer and Sparsity-Induced Feature Reduction Layer for improved feature selection.

Result: Achieves precision of 0.9, recall of 0.861, F1 score of 0.88, loss of 0.18, and accuracy of 88.1%, surpassing EfficientNet baseline.

Conclusion: The novel architecture enhances feature representation and redundancy removal, advancing automated diagnostic tools for medical images.
...
TL;DR: The paper evaluates VLMs for AD decision-making, identifies limitations, and proposes a new approach using VLMs as semantic enhancers rather than direct decision-makers, improving performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from inconsistent performance and reliance on handcrafted prompts in current VLM-based AD systems, which hinder robustness and generalization.

Method: The method involves leveraging VLMs for semantic scene enrichment, introducing a multi-modal architecture fusing visual and linguistic features, and adding a post-hoc refinement module.

Result: Experiments on AD benchmarks show state-of-the-art performance, demonstrating improved decision-making accuracy and interpretability.

Conclusion: The approach offers a promising direction for integrating VLMs into AD systems by enhancing reliability and interpretability through semantic enrichment.
...
TL;DR: The paper explores power law scaling in deep neural networks for medical anatomical segmentation, showing larger datasets improve performance. It introduces a novel augmentation method using deformation to enhance efficiency and convergence.


<details>
  <summary>Details</summary>
Motivation: To understand the scaling relationship with data size in medical anatomical segmentation and improve performance without additional data.

Method: Analyzes scaling laws across tasks and modalities, evaluates deformation-guided augmentation strategies, and proposes a scalable augmentation approach using diffeomorphic mappings.

Result: Both registered and generated deformation-based augmentations enhance efficiency, with the proposed method outperforming standard scaling trends.

Conclusion: The study advances segmentation scalability understanding and reduces annotation and computational costs in medical imaging.
...
TL;DR: Uni-Hema is a unified multi-task model for digital hematopathology, integrating detection, classification, segmentation, morphology prediction, and reasoning across multiple diseases. It outperforms single-task models and provides interpretable insights.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack unified, multi-task, multi-modal reasoning for diverse hematopathological conditions, necessitating a comprehensive solution.

Method: Uni-Hema combines 46 public datasets (>700K images, 21K QA pairs) using Hema-Former, a multimodal module linking visual-textual representations for various tasks (e.g., detection, classification).

Result: Uni-Hema matches or surpasses single-task models across hematological tasks, offering interpretable, cell-level insights.

Conclusion: Uni-Hema sets a new benchmark for multi-task, multi-modal hematopathology analysis, with code availability fostering wider adoption.
...
TL;DR: The paper introduces a weakly supervised pipeline for detecting ephemeral gullies using Vision Language Models (VLMs) and a teacher-student model, reducing manual labeling efforts. A novel dataset is also provided.


<details>
  <summary>Details</summary>
Motivation: Ephemeral gullies are hard to detect automatically due to their short cycles and lack of labeled data. The paper aims to address these challenges.

Method: Uses VLMs and a teacher-student model: the teacher learns from noisy VLM labels, and the student is trained weakly using teacher-generated labels and a noise-aware loss function.

Result: The approach outperforms VLMs and the label model, demonstrating its effectiveness.

Conclusion: The proposed method successfully reduces manual labeling efforts and improves detection of ephemeral gullies, supported by a publicly available dataset.
...
TL;DR: SAE-MCVT is a scalable real-time Multi-Camera Vehicle Tracking framework addressing gaps in accuracy, real-time performance, and scalability for city-scale ITS applications.


<details>
  <summary>Details</summary>
Motivation: Existing MCVT studies focus on accuracy but neglect real-time performance and scalability, crucial for real-world city-scale deployment.

Method: Proposes SAE-MCVT with edge devices processing RTSP streams (detection, tracking, geo-mapping, feature extraction) and a central workstation handling cross-camera association using spatial-temporal relations.

Result: Achieves real-time operation on 2K 15 FPS streams with an IDF1 score of 61.2 on RoundaboutHD dataset.

Conclusion: SAE-MCVT is the first scalable real-time MCVT framework suitable for city-scale ITS deployment.
...
TL;DR: The paper evaluates multi-modal LLMs' understanding of road safety concepts using a dataset of traffic signs and norms, finding gaps in safety reasoning compared to humans.


<details>
  <summary>Details</summary>
Motivation: To assess how well multi-modal LLMs comprehend road safety norms, a critical aspect for autonomous vehicles.

Method: Curated a dataset of traffic signs and road-safety norms from school textbooks and evaluated LLMs in a zero-shot setting.

Result: Models showed weaknesses in safety reasoning, highlighting discrepancies between human learning and model interpretation.

Conclusion: Identifies performance gaps in LLMs' understanding of road safety, suggesting areas for future research.
...
TL;DR: CoT prompting shows promise but RL-based fine-tuning degrades performance in Visual Grounding. CuRPO, a novel training strategy, uses CoT length and gIoU rewards to structure data progressively, outperforming existing methods with up to +12.52 mAP improvements.


<details>
  <summary>Details</summary>
Motivation: Addressing the performance degradation in Visual Grounding tasks due to lengthy or complex CoT reasoning and varying data complexities despite increased dataset sizes.

Method: Proposes CuRPO, leveraging CoT length and gIoU rewards as complexity indicators to progressively structure training data from simpler to more challenging examples.

Result: CuRPO outperforms existing methods, achieving up to +12.52 mAP on RefCOCO and demonstrating efficiency and robustness in few-shot learning scenarios.

Conclusion: CuRPO effectively addresses the challenges of CoT reasoning in Visual Grounding, showing superior performance and robustness across multiple datasets.
...
TL;DR: A cluster-based frame selection method is proposed to prevent information leakage in video datasets by grouping similar frames before partitioning.


<details>
  <summary>Details</summary>
Motivation: To address information leakage and improve the reliability of dataset splits in video-derived frames datasets.

Method: Cluster visually similar frames before splitting into training, validation, and test sets.

Result: Produces more representative, balanced, and reliable dataset partitions.

Conclusion: The proposed strategy enhances dataset quality by mitigating information leakage through clustering.
...
TL;DR: Pretraining vision transformers (ViTs) on procedurally-generated data devoid of visual or semantic content improves data efficiency and downstream performance.


<details>
  <summary>Details</summary>
Motivation: To explore inductive biases beneficial across modalities by pretraining ViTs on procedurally-generated data.

Method: Generate data using simple algorithms like formal grammars, pretrain ViTs on this abstract data, then follow with standard image-based training.

Result: Pretraining with procedural data (just 1% of the budget) improves ImageNet-1k accuracy by 1.7% and matches the effect of 28% more ImageNet data.

Conclusion: This approach offers a promising strategy for data-efficient and domain-agnostic pretraining.
...
### [24] [Single Tensor Cell Segmentation using Scalar Field Representations](https://arxiv.org/abs/2511.13947)
*Kevin I. Ruiz Vargas,Gabriel G. Galdino,Tsang Ing Ren,Alexandre L. Cunha*

Main category: cs.CV

TL;DR: The paper proposes a method for cell image segmentation using scalar fields parameterized by a trained network, simplifying training and reducing resource usage while achieving competitive results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a robust and efficient method for cell image segmentation by learning continuous scalar fields, which can handle outliers and produce sharp boundaries.

Method: The method involves parameterizing scalar fields using a trained network, solving Poisson PDEs or diffusion equations, and applying the watershed method for segmentation.

Result: The approach simplifies implementation, reduces training/inference times, and achieves competitive segmentation results on public datasets.

Conclusion: The novel approach is simple, geometrically insightful, and effective for cell segmentation, making it suitable for edge computing.
...
TL;DR: EchoAgent is a framework using LLM-controlled vision tools for structured, interpretable automation in echocardiographic video analysis, achieving accurate results grounded in clinical guidelines.


<details>
  <summary>Details</summary>
Motivation: Current deep learning models lack support for video-level reasoning and guideline-based measurement analysis in echocardiographic interpretation, prompting the need for EchoAgent.

Method: EchoAgent employs specialized vision tools under LLM control for temporal localization, spatial measurement, and clinical interpretation, including a feasibility prediction model for autonomous tool selection.

Result: EchoAgent delivers accurate, interpretable outcomes grounded in visual evidence and clinical guidelines, despite the complexity of spatiotemporal video analysis.

Conclusion: EchoAgent demonstrates the feasibility of guideline-aligned reasoning in echocardiographic analysis, paving the way for trustworthy AI in cardiac ultrasound.
...
TL;DR: CrossTrainer introduces a transferable video representation for skill assessment by identifying universal skill-attributes like balance and control, enabling actionable feedback and proficiency ratings across sports.


<details>
  <summary>Details</summary>
Motivation: Addressing the high cost and scarcity of expert-level supervision for skill assessment in various sports, CrossTrainer aims to generalize skill evaluation across domains.

Method: The approach identifies sport-agnostic skill-attributes, trains a multimodal language model to provide feedback and proficiency levels, and validates it in cross-sport and intra-sport settings.

Result: CrossTrainer achieves up to 60% improvement over state-of-the-art methods in both transfer and in-domain scenarios.

Conclusion: By focusing on shared human skill behaviors, CrossTrainer offers a scalable and generalized solution for skill assessment, enhancing multimodal language models.
...
### [27] [CD-DPE: Dual-Prompt Expert Network based on Convolutional Dictionary Feature Decoupling for Multi-Contrast MRI Super-Resolution](https://arxiv.org/abs/2511.14014)
*Xianming Gu,Lihui Wang,Ying Cao,Zeyu Deng,Yingfeng Ou,Guodong Hu,Yi Chen*

Main category: cs.CV

TL;DR: CD-DPE, a dual-prompt expert network with convolutional dictionary feature decoupling, enhances multi-contrast MRI super-resolution by effectively utilizing reference image features and improving reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Multi-contrast MRI super-resolution faces challenges due to contrast disparities between modalities, leading to suboptimal feature integration. Addressing this can improve clinical diagnosis and decision-making.

Method: Proposes CD-DPE: an iterative convolutional dictionary feature decoupling module (CD-FDM) separates features, and a dual-prompt feature fusion expert module (DP-FFEM) integrates them using frequency and adaptive routing prompts.

Result: CD-DPE outperforms state-of-the-art methods in reconstructing fine details and shows strong generalization on unseen datasets.

Conclusion: The CD-DPE strategy effectively addresses contrast disparities in multi-contrast MRI super-resolution, enhancing reconstruction quality and generalization.
...
### [28] [RISE: Single Static Radar-based Indoor Scene Understanding](https://arxiv.org/abs/2511.14019)
*Kaichen Zhou,Laura Dodds,Sayed Saad Afzal,Fadel Adib*

Main category: cs.CV

TL;DR: RISE introduces a benchmark and system for indoor scene understanding using single-static-radar, leveraging multipath reflections for layout reconstruction and object detection.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of optical sensors (privacy risks, occlusions) and mmWave radar (low spatial resolution) in indoor environments.

Method: Uses Bi-Angular Multipath Enhancement to model multipath reflections and a Hierarchical Diffusion framework for transforming radar responses.

Result: Achieves 60% reduction in Chamfer Distance (16 cm) for layout reconstruction and 58% IoU for object detection.

Conclusion: RISE sets a new standard for privacy-preserving, geometry-aware indoor scene understanding with a single static radar.
...
### [29] [MRI Plane Orientation Detection using a Context-Aware 2.5D Model](https://arxiv.org/abs/2511.14021)
*SangHyuk Kim,Daniel Haehn,Sumientra Rampersad*

Main category: cs.CV

TL;DR: This study develops a 2.5D context-aware classifier to identify anatomical planes in MRI slices, improving accuracy over traditional 2D methods and enhancing diagnostic tasks like brain tumor detection.


<details>
  <summary>Details</summary>
Motivation: Automated systems struggle with identifying anatomical planes in MRI slices, leading to complications in analysis and diagnostic accuracy when metadata is missing.

Method: A 2.5D model is trained using multi-slice information to avoid ambiguity and improve feature learning. It is evaluated against a 2D reference model.

Result: The 2.5D model achieves 99.49% accuracy (vs. 98.74% for 2D), reducing errors by 60%. It also boosts brain tumor detection accuracy from 97.0% to 98.0%.

Conclusion: The 2.5D approach significantly improves plane orientation detection and diagnostic tasks, demonstrating its utility in medical imaging analysis.
...
### [30] [LINGUAL: Language-INtegrated GUidance in Active Learning for Medical Image Segmentation](https://arxiv.org/abs/2511.14028)
*Md Shazid Islam,Shreyangshu Bera,Sudipta Paul,Amit K. Roy-Chowdhury*

Main category: cs.CV

TL;DR: LINGUAL is a framework that uses natural language instructions to automate segmentation tasks in medical images, reducing annotation effort by ~80% compared to traditional active learning methods.


<details>
  <summary>Details</summary>
Motivation: Active learning in medical image segmentation is labor-intensive due to blurry boundaries and varying ROI sizes. Language guidance offers a less demanding alternative.

Method: LINGUAL translates natural language instructions into executable programs via in-context learning, automating segmentation sub-tasks without human intervention.

Result: LINGUAL achieves performance comparable or superior to active learning baselines in active domain adaptation (ADA), while cutting annotation time by ~80%.

Conclusion: LINGUAL effectively reduces cognitive and annotation burdens in medical image segmentation by leveraging language guidance.
...
### [31] [Training-free Detection of AI-generated images via Cropping Robustness](https://arxiv.org/abs/2511.14030)
*Sungik Choi,Hankook Lee,Moontae Lee*

Main category: cs.CV

TL;DR: WaRPAD is a training-free AI-generated image detection method using self-supervised models, leveraging sensitivity to perturbations and patch-based scoring for robust detection across diverse datasets and generative models.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of vision-generative models necessitates reliable detection methods without needing tailored training or prior data knowledge.

Method: WaRPAD uses self-supervised models pre-trained with augmentations like RandomResizedCrop. It defines a base score quantifying sensitivity to perturbations via Haar wavelet decomposition, then averages scores from rescaled image patches.

Result: WaRPAD shows competitive performance and robustness across diverse datasets, 23 generative models, and test-time corruptions.

Conclusion: WaRPAD is a versatile, training-free solution for AI-generated image detection applicable across self-supervised models.
...
### [32] [FashionMAC: Deformation-Free Fashion Image Generation with Fine-Grained Model Appearance Customization](https://arxiv.org/abs/2511.14031)
*Rong Zhang,Jinxiao Li,Jingnan Wang,Zhiwen Zuo,Jianfeng Dong,Wei Li,Chi Wang,Weiwei Xu,Xun Wang*

Main category: cs.CV

TL;DR: FashionMAC is a diffusion-based framework for garment-centric fashion image generation that avoids garment deformation and preserves details while offering fine-grained controllability over model appearance.


<details>
  <summary>Details</summary>
Motivation: The study addresses challenges in preserving garment details and achieving fine-grained appearance control in fashion image generation, which existing methods fail to resolve due to deformation-induced distortions and lack of specialized mechanisms.

Method: The proposed FashionMAC framework avoids garment deformation by directly outpainting garments from dressed persons. It introduces a region-adaptive decoupled attention (RADA) mechanism and chained mask injection for fine-grained controllability.

Result: Experiments show FashionMAC outperforms state-of-the-art methods in preserving garment details and achieving high controllability over synthesized human models.

Conclusion: FashionMAC successfully eliminates deformation-related distortions and enhances controllability, making it a superior solution for high-quality fashion image generation.
...
### [33] [Flood-LDM: Generalizable Latent Diffusion Models for rapid and accurate zero-shot High-Resolution Flood Mapping](https://arxiv.org/abs/2511.14033)
...
TL;DR: A novel latent diffusion model approach achieves high-accuracy flood map super-resolution quickly, enhancing real-time flood risk management and generalizability.


<details>
  <summary>Details</summary>
Motivation: Traditional flood prediction models are computationally intensive and lack generalizability, while existing machine learning solutions struggle with unseen areas.

Method: Utilizes latent diffusion models for super-resolution of coarse-grid flood maps, incorporating physics-informed inputs.

Result: Significantly reduces computational time and maintains accuracy, with improved generalizability and interpretability.

Conclusion: The proposed approach enables efficient, scalable real-time flood risk management with better adaptability to new regions.
...
### [34] [Saliency-Guided Deep Learning for Bridge Defect Detection in Drone Imagery](https://arxiv.org/abs/2511.14040)
*Loucif Hebbache,Dariush Amirkhani,Mohand Saïd Allili,Jean-François Lapointe*

Main category: cs.CV

TL;DR: Proposes a two-stage framework using saliency and YOLOX for anomaly detection in concrete bridges via drone imagery, showing high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of anomaly detection in concrete bridge structures using drone imagery for automated inspection.

Method: Combines saliency-based defect region proposals with a YOLOX detector on brightness-augmented images.

Result: Demonstrates high accuracy and computational efficiency on standard datasets.

Conclusion: The framework is suitable for self-powered inspection systems, offering practical implementation potential.
...
### [35] [Semantic Context Matters: Improving Conditioning for Autoregressive Models](https://arxiv.org/abs/2511.14063)
*Dongyang Jin,Ryan Xu,Jianhao Zeng,Rui Lan,Yancheng Bai,Lei Sun,Xiangxiang Chu*

Main category: cs.CV

TL;DR: SCAR, a Semantic-Context-driven method for autoregressive models, improves image editing by introducing Compressed Semantic Prefilling and Semantic Alignment Guidance.


<details>
  <summary>Details</summary>
Motivation: Extending autoregressive models to general image editing faces challenges like weak conditioning and visual artifacts.

Method: SCAR uses Compressed Semantic Prefilling and Semantic Alignment Guidance to enhance semantic alignment and visual fidelity.

Result: SCAR outperforms prior AR-based methods in visual fidelity and semantic alignment.

Conclusion: SCAR offers a flexible and efficient solution for image editing with autoregressive models.
...
### [36] [CORE: Compact Object-centric REpresentations as a New Paradigm for Token Merging in LVLMs](https://arxiv.org/abs/2511.14072)
*Jingyu Lei,Gaoang Wang,Der-Horng Lee*

Main category: cs.CV

TL;DR: CORE introduces object-centric token compression for LVLMs, improving efficiency and performance by leveraging semantic segmentation and centroid-guided sorting.


<details>
  <summary>Details</summary>
Motivation: Addressing the computational and memory inefficiencies in LVLMs due to quadratic visual token growth, and overcoming suboptimal merges in existing methods.

Method: Uses semantic segmentation masks as priors to merge tokens into compact object-centric representations, aided by centroid-guided sorting for spatial coherence.

Result: Achieves state-of-the-art performance on six benchmarks, maintains 97.4% baseline performance with just 2.2% tokens, and shows high efficiency in adaptive settings.

Conclusion: Object-centric representations excel in efficient LVLM processing, offering superior compression and performance retention.
...
### [37] [Zero-Training Task-Specific Model Synthesis for Few-Shot Medical Image Classification](https://arxiv.org/abs/2511.14082)
*Yao Qin,Yangyang Yan,YuanChao Yang,Jinhua Pang,Huanyong Bi,Yuan Liu,HaiHua Wang*

Main category: cs.CV

TL;DR: A novel zero-training approach, ZS-TMS, uses pre-trained generative models to synthesize task-specific classifier parameters with minimal input (1-shot + text), outperforming existing few-shot methods.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of limited annotated medical data, especially for rare diseases, by eliminating the need for large datasets or training.

Method: Leverages Semantic-Guided Parameter Synthesizer (SGPS) to generate classifier weights directly from minimal task information (e.g., one image + text).

Result: Achieves state-of-the-art performance on few-shot benchmarks (ISIC 2018 and rare disease datasets), excelling in 1-shot and 5-shot scenarios.

Conclusion: Enables rapid deployment of AI diagnostic tools for rare diseases, overcoming data scarcity barriers.
...
### [38] [Automated glenoid bone loss measurement and segmentation in CT scans for pre-operative planning in shoulder instability](https://arxiv.org/abs/2511.14083)
*Zhonghao Liu,Hanxue Gu,Qihang Li,Michael Fox,Jay M. Levin,Maciej A. Mazurowski,Brian C. Lau*

Main category: cs.CV

TL;DR: A fully automated deep learning pipeline was developed for measuring glenoid bone loss on 3D CT scans, outperforming manual methods in accuracy and consistency.


<details>
  <summary>Details</summary>
Motivation: Current methods for measuring glenoid bone loss are time-consuming and prone to variability, necessitating a more reliable automated solution.

Method: The pipeline uses a U-Net for glenoid segmentation, a network for landmark detection, and PCA/circle fitting for bone loss measurement.

Result: The automated method showed strong agreement with consensus readings and better consistency than surgeons (ICC 0.84 vs 0.78), with high recall for severity classification.

Conclusion: The pipeline is a reliable, efficient tool for preoperative planning and screening patients with glenoid bone loss.
...
### [39] [Error-Driven Scene Editing for 3D Grounding in Large Language Models](https://arxiv.org/abs/2511.14086)
*Yue Zhang,Zun Wang,Han Lin,Jialu Li,Jianing Yang,Yonatan Bitton,Idan Szpektor,Mohit Bansal*

Main category: cs.CV

TL;DR: The paper proposes DEER-3D, an error-driven framework using targeted 3D scene editing to improve spatial grounding in 3D-LLMs by generating precise counterfactuals and iterative fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To address the limitation of 3D-LLMs in grounding language to visual and spatial elements due to scarce 3D resources and inherent biases.

Method: Introduces DEER-3D, a framework that diagnoses predicate-level errors (e.g., attribute or spatial relation) and performs minimal, predicate-aligned 3D scene edits for targeted counterfactual supervision.

Result: Demonstrates consistent improvements in grounding accuracy across multiple benchmarks for 3D grounding and scene understanding tasks.
...
### [40] [GCA-ResUNet:Image segmentation in medical images using grouped coordinate attention](https://arxiv.org/abs/2511.14087)
*Jun Ding,Shang Gao*

Main category: cs.CV

TL;DR: GCA-ResUNet integrates Grouped Coordinate Attention into ResNet-50 for medical image segmentation, balancing global context capture with efficiency, outperforming state-of-the-art methods on Synapse and ACDC datasets.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of U-Net and transformer-based methods in capturing long-range dependencies without excessive computation or large datasets, GCA-ResUNet aims for efficient global modeling.

Method: Integrates Grouped Coordinate Attention (GCA) into ResNet-50 residual blocks, enhancing feature representation and boundary delineation with minimal computational overhead.

Result: Achieves Dice scores of 86.11% (Synapse) and 92.64% (ACDC), surpassing baselines while maintaining fast inference and computational efficiency.

Conclusion: GCA-ResUNet effectively combines global modeling with efficiency, offering a practical solution for high-accuracy medical image segmentation.
...
### [41] [BCE3S: Binary Cross-Entropy Based Tripartite Synergistic Learning for Long-tailed Recognition](https://arxiv.org/abs/2511.14097)
*Weijia Fan,Qiufu Li,Jiajun Wen,Xiaoyang Peng*

Main category: cs.CV

TL;DR: BCE3S, a BCE-based tripartite synergistic learning method, improves intra-class compactness, inter-class separability, and classifier balance in long-tailed recognition tasks, outperforming CE-based methods on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of CE-based methods in LTR, which struggle with feature properties and amplify imbalance effects due to coupled classifier vectors.

Method: Proposes BCE3S with three components: joint learning (optimizes features/classifiers), contrastive learning (enhances intra-class compactness), and uniform learning (balances classifier separability).

Result: Achieves higher feature compactness/separability and balanced classifier separability, outperforming CE-based methods on CIFAR10-LT, CIFAR100-LT, ImageNet-LT, and iNaturalist2018.

Conclusion: BCE3S effectively improves LTR performance by decoupling classifier metrics and enhancing feature properties, setting new SOTA results.
...
### [42] [FAPE-IR: Frequency-Aware Planning and Execution Framework for All-in-One Image Restoration](https://arxiv.org/abs/2511.14099)
*Jingren Liu,Shuning Xu,Qirui Yang,Yun Wang,Xiangyu Chen,Zhong Ji*

Main category: cs.CV

TL;DR: FAPE-IR is a unified framework for image restoration that uses a frequency-aware planner and executor to handle multiple degradations, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with real-world scenarios due to task-specific designs. FAPE-IR aims for adaptability and interpretability.

Method: Uses a frozen MLLM planner to generate frequency-aware restoration plans, guiding a LoRA-MoE executor. Includes adversarial training and frequency regularization.

Result: State-of-the-art performance across seven tasks and strong zero-shot generalization.

Conclusion: FAPE-IR provides a unified, interpretable solution for diverse image restoration challenges.
...
### [43] [Text-Driven Reasoning Video Editing via Reinforcement Learning on Digital Twin Representations](https://arxiv.org/abs/2511.14100)
*Yiqing Shen,Chenjia Li,Mathias Unberath*

Main category: cs.CV

TL;DR: The paper introduces RIVER, a reasoning-based implicit video editor that interprets vague user queries through multi-hop reasoning to modify videos, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing video editing tools require explicit descriptions, which limits usability when users provide vague or semantic queries.

Method: RIVER uses digital twin representations and a large language model for reasoning, followed by diffusion-based editing, trained via reinforcement learning.

Result: RIVER excels on RVEBenchmark (519 queries) and outperforms six baselines on VegGIE and FiVE benchmarks.

Conclusion: RIVER advances text-driven video editing by enabling implicit query interpretation and achieves state-of-the-art results.
...
### [44] [RTS-Mono: A Real-Time Self-Supervised Monocular Depth Estimation Method for Real-World Deployment](https://arxiv.org/abs/2511.14107)
*Zeyu Cheng,Tongfei Liu,Tao Lei,Xiang Hua,Yi Zhang,Chengkai Tang*

Main category: cs.CV

TL;DR: The paper proposes RTS-Mono, a lightweight and efficient self-supervised monocular depth estimation method for real-time applications in autonomous driving and robotics.


<details>
  <summary>Details</summary>
Motivation: Existing monocular depth estimation models are computationally heavy or sacrifice performance for efficiency, hindering real-world deployment.

Method: RTS-Mono uses a lightweight encoder-decoder architecture with a Lite-Encoder and a multi-scale sparse fusion decoder to minimize redundancy and improve speed.

Result: RTS-Mono achieves state-of-the-art performance on the KITTI dataset with low parameters (3 M) and high efficiency (49 FPS on Jetson Orin).

Conclusion: RTS-Mono balances performance and efficiency, making it suitable for real-world deployment in resource-constrained environments.
...
### [45] [$A^2$GC: $A$symmetric $A$ggregation with Geometric Constraints for Locally Aggregated Descriptors](https://arxiv.org/abs/2511.14109)
*Zhenyu Li,Tianyi Shang*

Main category: cs.CV

TL;DR: The paper proposes an asymmetric aggregation method for Visual Place Recognition (VPR) with geometric constraints ($A^2$GC-VPR), outperforming existing methods by addressing distributional discrepancies and enhancing spatial awareness.


<details>
  <summary>Details</summary>
Motivation: Current optimal transport-based VPR methods symmetrically treat feature and cluster distributions, limiting effectiveness when discrepancies exist. The paper aims to improve matching accuracy by addressing this asymmetry and incorporating geometric constraints.

Method: Proposes $A^2$GC-VPR, using row-column normalization averaging with separate marginal calibration for asymmetric matching. Introduces learnable coordinate embeddings to fuse geometric constraints with feature similarities, promoting spatial awareness.

Result: Demonstrates superior performance on MSLS, NordLand, and Pittsburgh datasets, confirming improved matching accuracy and robustness compared to state-of-the-art methods.

Conclusion: The asymmetric aggregation method with geometric constraints ($A^2$GC-VPR) effectively addresses distributional discrepancies and enhances spatial awareness, leading to better performance in visual place recognition.
...
### [46] [CascadedViT: Cascaded Chunk-FeedForward and Cascaded Group Attention Vision Transformer](https://arxiv.org/abs/2511.14111)
*Srivathsan Sivakumar,Faisal Z. Qureshi*

Main category: cs.CV

TL;DR: CViT is a lightweight ViT variant with CCFFN, improving efficiency and reducing energy use without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the high computational and energy demands of ViTs for resource-constrained platforms.

Method: Proposes CViT with CCFFN, splitting input features for better efficiency.

Result: CViT-XL achieves 75.5% Top-1 accuracy, reduces FLOPs by 15%, and lowers energy use by 3.3%. CViT-L outperforms EfficientViT-M2 in accuracy.

Conclusion: CViT offers superior efficiency and energy savings, making it ideal for devices like phones and drones.
...
### [47] [Coffee: Controllable Diffusion Fine-tuning](https://arxiv.org/abs/2511.14113)
*Ziyao Zeng,Jingcheng Ni,Ruyi Liu,Alex Wong*

Main category: cs.CV

TL;DR: Coffee prevents undesired concept learning in text-to-image diffusion models by using language regularization.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of preventing diffusion models from learning and entangling undesired concepts during fine-tuning.

Method: Uses textual descriptions to specify undesired concepts, preventing their alignment with user prompts.

Result: Coffee effectively mitigates undesired concept learning and outperforms existing methods.

Conclusion: Coffee provides a flexible, training-free solution for controlled fine-tuning of diffusion models.
...
### [48] [Multi-view Phase-aware Pedestrian-Vehicle Incident Reasoning Framework with Vision-Language Models](https://arxiv.org/abs/2511.14120)
*Hao Zhen,Yunxiang Yang,Jidong J. Yang*

Main category: cs.CV

TL;DR: MP-PVIR is a multi-view framework analyzing pedestrian-vehicle incidents through structured phases, specialized VLMs, and report generation, improving traffic safety insights.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of detailed analysis in pedestrian-vehicle incidents by integrating temporal structuring and multi-view reasoning.

Method: Four-stage framework: multi-view video acquisition, phase segmentation, phase-specific reasoning, and hierarchical synthesis using TG-VLM and PhaVR-VLM.

Result: Achieves mIoU of 0.4881 for segmentation and captioning score of 33.063, with 64.70% accuracy on QA. Generates actionable reports.

Conclusion: MP-PVIR effectively transforms video data into insights, enhancing AI-driven traffic safety analytics.
...
### [49] [Attention Via Convolutional Nearest Neighbors](https://arxiv.org/abs/2511.14137)
*Mingi Kang,Jeová Farias Sales Rocha Neto*

Main category: cs.CV

TL;DR: The paper unifies convolution and self-attention under a k-nearest neighbor framework, introducing ConvNN as a drop-in replacement that balances local and global receptive fields, validated on CIFAR datasets.


<details>
  <summary>Details</summary>
Motivation: To bridge the perceived gap between convolutional and attention-based architectures in computer vision by showing they are special cases of neighbor selection and aggregation.

Method: Proposes Convolutional Nearest Neighbors (ConvNN), a framework unifying convolution (spatial proximity) and attention (feature similarity) into a single k-nearest neighbor approach.

Result: Demonstrates improved accuracy on CIFAR-10 and CIFAR-100 tasks using ConvNN in VGG and ViT architectures, outperforming standard attention variants.

Conclusion: ConvNN dissolves the distinction between convolution and attention, offering regularization benefits and enabling more principled vision architecture design.
...
### [50] [iGaussian: Real-Time Camera Pose Estimation via Feed-Forward 3D Gaussian Splatting Inversion](https://arxiv.org/abs/2511.14149)
*Hao Wang,Linqing Zhao,Xiuwei Xu,Jiwen Lu,Haibin Yan*

Main category: cs.CV

TL;DR: iGaussian introduces a two-stage feed-forward framework for real-time camera pose estimation using direct 3D Gaussian inversion, eliminating the need for iterative render-compare-refine loops.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on computationally heavy iterative processes for pose estimation, limiting real-time performance in robotics.

Method: The framework regresses a coarse 6DoF pose using a Gaussian Scene Prior-based Pose Regression Network, then refines it via feature matching and multi-model fusion.

Result: Experiments show reduced median rotation errors (0.2°) and a 10x speedup (2.87 FPS) compared to optimization-based approaches.

Conclusion: iGaussian outperforms prior methods in accuracy and speed, enabling real-time camera pose estimation for robotics applications.
...
### [51] [Wave-Former: Through-Occlusion 3D Reconstruction via Wireless Shape Completion](https://arxiv.org/abs/2511.14152)
*Laura Dodds,Maisy Lam,Waleed Akbar,Yibo Cheng,Fadel Adib*

Main category: cs.CV

TL;DR: Wave-Former is a novel method using mmWave signals for high-accuracy 3D shape reconstruction of occluded objects, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: The method aims to enable 3D reconstruction of hidden objects for applications in robotics, AR, and logistics, overcoming limitations of past mmWave techniques.

Method: Wave-Former uses a three-stage pipeline: proposing geometric surfaces, applying a transformer-based shape completion model for mmWave signals, and entropy-guided surface selection.

Result: It achieves 72% recall (up from 54%) while maintaining 85% precision, showing strong generalization from synthetic to real-world data.

Conclusion: Wave-Former advances mmWave-based 3D reconstruction by combining physics-awareness with deep learning, achieving superior performance.
...
TL;DR: LVLMs struggle with abstract sketches. This paper introduces a new dataset and O3SLM, an LVLM trained on it, achieving state-of-the-art performance in sketch tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing LVLMs' limitations in interpreting hand-drawn sketches due to lack of suitable datasets.

Method: Introducing a large-scale dataset of image-sketch-instruction triplets and training O3SLM on it.

Result: O3SLM outperforms existing LVLMs in tasks like object localization, counting, SBIR, and VQA.

Conclusion: The new dataset and O3SLM significantly improve LVLMs' sketch comprehension.
...
TL;DR: RiSe addresses performance drops in cross-domain Multimodal Face Anti-Spoofing by tackling modal representation and synergy invariant risks, achieving superior results.


<details>
  <summary>Details</summary>
Motivation: Performance degradation in unseen domains for multimodal FAS due to overlooked risks: modal representation invariance and synergy invariance.

Method: Proposes RiSe with AsyIRM for representation risk and MMSD for synergy risk, focusing on invariant learning and disentanglement.

Result: RiSe achieves state-of-the-art cross-domain performance, validated by theory and experiments.

Conclusion: RiSe effectively mitigates cross-domain challenges in multimodal FAS, offering robust generalization.
...
TL;DR: MVI-Bench is introduced as the first benchmark to evaluate LVLM robustness against misleading visual inputs, focusing on three hierarchical levels and introducing a novel metric, MVI-Sensitivity. Empirical results highlight vulnerabilities and provide actionable insights.


<details>
  <summary>Details</summary>
Motivation: Existing robustness benchmarks overlook misleading visual inputs in LVLMs, prompting the need for MVI-Bench to address this gap.

Method: MVI-Bench evaluates LVLMs using three hierarchical levels of misleading visual inputs (Visual Concept, Attribute, Relationship), curating six categories and 1,248 annotated VQA instances. A novel metric, MVI-Sensitivity, is introduced for granular evaluation.

Result: Testing 18 LVLMs reveals significant vulnerabilities to misleading visual inputs. In-depth analyses offer insights for improving robustness.

Conclusion: MVI-Bench fills a critical gap in LVLM evaluation, providing tools and insights to develop more reliable and robust models.
...
TL;DR: The paper proposes an object-level token merging strategy for Adaptive Token compression in Multimodal Large Language Models (MLLMs) to reduce computational burden and align with human vision cognition.


<details>
  <summary>Details</summary>
Motivation: Patch-level tokenization in MLLMs causes quadratic growth in image tokens, leading to computational inefficiency and misalignment with human vision.

Method: Introduces an object-level token merging strategy for adaptive token compression, aligning with human vision cognition.

Result: Experiments show the method uses only 10% of tokens while achieving ~96% of vanilla model performance.

Conclusion: The proposed method effectively balances token compression and performance, outperforming relevant works.
...
TL;DR: DoGCLR introduces a game-theory-based contrastive learning framework for skeleton-based action recognition, addressing motion loss and suboptimal negative samples via dynamic sample balancing and entropy-driven memory management, achieving superior accuracy on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from uniform processing of skeleton regions and inefficient negative sample storage, leading to motion information loss and poor contrastive learning performance.

Method: Proposes DoGCLR, which uses a Dominance Game to balance positive/negative samples, a spatio-temporal dual weight mechanism for key motion localization, and an entropy-driven strategy for memory bank optimization.

Result: Achieves 81.1%/89.4% accuracy on NTU RGB+D 60 X-Sub/X-View and 71.2%/75.5% on NTU RGB+D 120 X-Sub/X-Set, outperforming state-of-the-art methods by up to 2.7%. Also shows robust performance on PKU-MMD.

Conclusion: DoGCLR significantly improves skeleton-based action recognition by dynamically optimizing contrastive signals and motion representation, demonstrating strong generalizability and robustness.
...
TL;DR: UniSER is a versatile model designed to address diverse image degradations caused by soft effects like lens flare, haze, shadows, and reflections, outperforming specialized and generalist models through curated training data and tailored diffusion transformer training.


<details>
  <summary>Details</summary>
Motivation: Existing solutions for image degradation from soft effects are either specialized (lacking scalability) or generalist (requiring detailed prompts and often failing). UniSER aims to bridge this gap by leveraging shared characteristics of these degradations.

Method: UniSER uses a massive 3.8M-pair dataset with novel, physically-plausible data and fine-tunes a Diffusion Transformer to learn robust restoration priors, integrating fine-grained mask and strength controls.

Result: UniSER outperforms both specialist and generalist models, achieving high-fidelity restoration in real-world scenarios.

Conclusion: The UniSER model successfully unifies restoration tasks for diverse soft-effect degradations, demonstrating scalability and robustness beyond existing approaches.
...
TL;DR: GloTok introduces a global relational approach for image tokenization, achieving uniform semantic distributions and high-quality image generation without direct pre-trained model access during training.


<details>
  <summary>Details</summary>
Motivation: Existing locally supervised tokenization methods limit semantic uniformity, impacting generation quality. GloTok addresses this by leveraging global relational information.

Method: GloTok uses codebook-wise histogram relation learning for uniform semantic distribution and a residual learning module to minimize reconstruction errors.

Result: State-of-the-art reconstruction and generation performance on ImageNet-1k benchmark.

Conclusion: GloTok's global perspective improves semantic uniformity and image generation quality, outperforming existing methods.
...
TL;DR: The paper introduces the first autonomous-driving benchmark dataset collected entirely by AVs, offering rich, naturalistic data for safety evaluation and behavior analysis.


<details>
  <summary>Details</summary>
Motivation: Existing datasets, collected via human-driving or unidentified modes, are insufficient for evaluating AVs' real behavioral safety in autonomous mode.

Method: The dataset includes synchronized camera images, GNSS/IMU data, vehicle trajectories, and detailed annotations. Key frames are segmented with scenario-level attributes.

Result: An end-to-end motion planning model achieves an ADE of 1.4 m on the dataset, demonstrating its utility for AV safety evaluation.

Conclusion: The dataset provides a sustainable foundation for AV research, with weekly expansions enhancing its depth and relevance.
...
TL;DR: UMEG-Net addresses few-shot Precise Event Spotting (PES) by integrating human skeletons and sport-specific keypoints into a unified graph, using GCN and temporal shift modules for robust performance with limited data.


<details>
  <summary>Details</summary>
Motivation: Traditional PES methods rely heavily on large labeled datasets and struggle with few-shot conditions due to their limited input types (pixel- or pose-based). UMEG-Net aims to overcome these challenges by leveraging multimodal inputs.

Method: UMEG-Net combines human skeletons and sport-specific object keypoints in a unified graph, using advanced GCN and multi-scale temporal shift modules for spatio-temporal extraction. Multimodal distillation transfers knowledge from keypoint graphs to visual representations.

Result: UMEG-Net achieves robust performance with limited labeled data and outperforms baseline models in few-shot settings.

Conclusion: UMEG-Net provides a scalable and effective solution for few-shot PES, demonstrating the benefits of integrating multimodal inputs and knowledge distillation.
...
TL;DR: The paper proposes a curriculum learning strategy with fractal softmax for hierarchical semantic learning to address challenges in 3D aortic segmentation, improving accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Aortic pathologies require precise 3D anatomical analysis for minimally invasive repairs, but existing methods struggle with hierarchical relationships and class imbalance.

Method: The approach uses curriculum learning inspired by human cognition, decomposing complex structures progressively and employing a fractal softmax for hierarchical learning.

Result: The method achieves an 11.65% higher Dice score on validation and 5.6% higher on test, with a fivefold acceleration in inference.

Conclusion: The framework enhances segmentation accuracy and efficiency, making it suitable for real-time clinical applications.
...
TL;DR: DetGain is an online data curation method tailored for object detection, dynamically selecting informative samples to improve accuracy efficiently.


<details>
  <summary>Details</summary>
Motivation: High-quality curated datasets outperform larger unfiltered ones, but existing online sampling strategies fall short for object detection due to its complexity.

Method: DetGain estimates each image's impact on dataset-level AP using prediction quality, models global score distributions, and selects samples based on teacher-student contribution gaps.

Result: Experiments on COCO show consistent accuracy improvements across detectors, robustness with low-quality data, and synergy with knowledge distillation.

Conclusion: DetGain is a versatile, minimally intrusive strategy for enhancing data efficiency in object detection.
...
TL;DR: MCFormer is a novel transformer-based network for maritime vessel re-identification, addressing unique challenges like intra-identity variations and local part missing through multi-scale correlation modeling.


<details>
  <summary>Details</summary>
Motivation: Existing vessel Re-ID methods, adapted from pedestrian-focused algorithms, fail to handle unique vessel image problems such as intra-identity variations and local part missing, leading to outlier samples.

Method: Proposes MCFormer with Global Correlation Module (GCM) for global feature aggregation and Local Correlation Module (LCM) for local feature alignment, integrating multi-scale correlations.

Result: MCFormer achieves state-of-the-art performance on three benchmarks.

Conclusion: MCFormer effectively addresses vessel Re-ID challenges by modeling multi-scale correlations, outperforming existing methods.
...
TL;DR: InstantViR is an amortized inference framework for ultra-fast video reconstruction using a pre-trained video diffusion prior, achieving high-quality results at 35 FPS, 100x faster than iterative methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for high perceptual quality and low latency in video streaming, telepresence, and AR/VR, overcoming limitations of existing diffusion-based methods.

Method: Distills a bidirectional video diffusion model into a causal autoregressive student for single-pass inference, replaces VAEs with LeanVAE for efficiency.

Result: Matches or surpasses diffusion-based baselines in quality while running at over 35 FPS, achieving up to 100x speedups.

Conclusion: Demonstrates that diffusion-based video reconstruction can be practical for real-time, interactive, and streaming applications.
...
TL;DR: Orion is a visual agent framework that processes and generates any modality, excelling in visual AI tasks through multi-tool orchestration and symbolic execution.


<details>
  <summary>Details</summary>
Motivation: To advance visual intelligence by moving beyond descriptive outputs of traditional vision-language models toward active, tool-driven workflows.

Method: Utilizes an agentic framework with specialized tools like object detection, OCR, and geometric analysis for multi-step visual tasks.

Result: Achieves state-of-the-art results on benchmarks like MMMU, MMBench, DocVQA, and MMLongBench.

Conclusion: Orion bridges neural perception and symbolic execution, enabling autonomous visual reasoning and production-grade visual intelligence.
...
TL;DR: Proposes Measurement-Constrained Sampling (MCS) for diverse blind face restoration (BFR) by aligning reconstructions with textual prompts using diffusion models, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing BFR methods produce deterministic results, failing to capture the one-to-many nature of plausible high-quality reconstructions from low-quality inputs.

Method: Formulates BFR as a measurement-constrained generative task using text-to-image diffusion models, incorporating Forward and Reverse Measurement constraints.

Result: MCS generates prompt-aligned diverse reconstructions and outperforms current BFR methods.

Conclusion: The MCS approach enhances diversity and quality in BFR by leveraging textual prompts and controlled degradations.
...
TL;DR: The paper proposes an autoregressive diffusion model for speech-driven 3D facial animation to handle varying audio lengths and reduce latency, achieving real-time synthesis.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing methods that poorly handle long audio sequences and suffer from latency.

Method: Uses an autoregressive diffusion model with dynamic conditions from past frames and audio inputs.

Result: Achieves real-time synthesis with high-quality facial animations.

Conclusion: The approach is effective and efficient, demonstrated through a real-time interactive demo.
...
TL;DR: APS introduces explicit learning mechanisms for 3D human motion prediction, outperforming existing methods with significant improvements.


<details>
  <summary>Details</summary>
Motivation: Overcome passive learning traps in current approaches by introducing active perceptual strategies for better motion prediction.

Method: Uses quotient space representations and auxiliary learning objectives for explicit spatio-temporal modeling. Includes data and network perception modules.

Result: Achieves state-of-the-art performance with 16.3%, 13.9%, and 10.1% improvements on H3.6M, CMU Mocap, and 3DPW datasets.

Conclusion: APS effectively enhances active perceptual learning in motion prediction, setting new benchmarks.
...
TL;DR: WeSTAR is a weakly supervised framework that enhances zero-shot generalization of foundation models in monocular depth estimation (MDE) through self-training, hierarchical normalization, and ordinal annotations.


<details>
  <summary>Details</summary>
Motivation: To improve the performance of foundation models in MDE when downstream task data is available, addressing robustness in diverse domains.

Method: Uses weakly supervised self-training with regularization, incorporating dense self-supervision, semantically-aware hierarchical normalization, pairwise ordinal depth annotations, and weight regularization.

Result: Achieves state-of-the-art performance on realistic and corrupted out-of-distribution datasets, improving generalization across benchmarks.

Conclusion: WeSTAR effectively enhances MDE foundation models' robustness and generalization capabilities through parameter-efficient adaptation.
...
TL;DR: Proposes a GNSS-free collaborative perception framework using LiDAR localization with Pose Generator with Confidence (PGC) and Pose-Aware Spatio-Temporal Alignment Transformer (PASTAT), validated on V2VLoc and V2V4Real datasets.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of accurate multi-agent pose alignment in GNSS-denied environments, where traditional methods fail.

Method: Introduces PGC for pose and confidence estimation and PASTAT for confidence-aware spatial alignment with temporal context. Uses V2VLoc dataset.

Result: Achieves state-of-the-art performance in GNSS-denied conditions on V2VLoc and shows effectiveness on V2V4Real.

Conclusion: Proposed framework effectively enables collaborative perception without GNSS, validated by extensive experiments.
...
TL;DR: ManipBench introduces a large-scale benchmark for detecting AI-edited images, addressing limitations of existing benchmarks, while ManipShield, an MLLM-based model, achieves top performance in detection, localization, and explanation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack diversity, generative-model coverage, and interpretability, limiting the effectiveness of manipulation detection methods.

Method: ManipBench includes 450K manipulated images from 25 models across 12 categories, with 100K annotated for interpretability. ManipShield uses contrastive LoRA fine-tuning and task-specific decoders for unified detection.

Result: ManipShield outperforms existing methods, showing strong generalization to unseen models. ManipBench provides comprehensive data for evaluation.

Conclusion: ManipBench and ManipShield improve detection capabilities for AI-edited images, offering diverse data and interpretable results.
...
TL;DR: The paper introduces GSLR, a Gaussian Splatting-based Low-rank tensor Representation framework, to address t-SVD's limitations in capturing local high-frequency information in multi-dimensional images. It outperforms existing methods in image recovery.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the coarse approximation and fixed basis atom limitations of t-SVD methods, which fail to capture spatial local high-frequency information effectively.

Method: The proposed GSLR framework uses tailored 2D and 1D Gaussian splatting to generate latent tensors and transform matrices, respectively, enabling precise representation of high-frequency details.

Result: Experiments show GSLR consistently outperforms state-of-the-art methods in multi-dimensional image recovery, especially in capturing local high-frequency information.

Conclusion: GSLR provides a more accurate and continuous representation of multi-dimensional images, addressing t-SVD's shortcomings and demonstrating superior performance in practical applications.
...
TL;DR: VLM3D leverages large vision-language models (VLMs) to improve text-to-3D generation by addressing semantic alignment and spatial understanding issues.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-3D models struggle with fine-grained semantic details and geometric inconsistencies, motivating the need for a solution.

Method: VLM3D uses VLMs as differentiable critics, employing a dual-query signal (Yes or No log-odds) to evaluate semantic fidelity and geometric coherence.

Result: VLM3D outperforms benchmarks in optimization-based pipelines and corrects spatial errors in feed-forward models.

Conclusion: VLM3D provides a generalizable way to integrate VLMs' language-grounded understanding into diverse 3D generative pipelines.
...
TL;DR: The paper proposes Intermediate Domain Proxies (IDP) to address challenges in Cross-Domain Few-Shot Learning (CDFSL) by leveraging source domain embeddings as a codebook for target domain feature reconstruction and domain alignment.


<details>
  <summary>Details</summary>
Motivation: CDFSL faces challenges like semantic disjoint, large domain discrepancy, and data scarcity. The paper aims to overcome these by constructing and utilizing intermediate domains.

Method: The authors introduce IDP, using source feature embeddings as a codebook to reconstruct target domain features. They explore visual and semantic attributes in proxies and develop a fast domain alignment method.

Result: The proposed model outperforms state-of-the-art models on 8 CDFSL benchmarks.

Conclusion: IDP and collaborative learning effectively address CDFSL challenges, achieving superior performance.
...
TL;DR: NeuralSSD is a neural Galerkin method-based solver for high-quality 3D surface reconstruction from point clouds, balancing reliability through a novel energy equation and leveraging a 3D convolutional network for superior optimization.


<details>
  <summary>Details</summary>
Motivation: Existing implicit surface reconstruction methods lack explicit mechanisms for tight data fitting, prompting the need for NeuralSSD to enhance accuracy and robustness.

Method: NeuralSSD uses a novel energy equation to balance point cloud reliability and introduces a 3D convolutional network to learn spatial information for optimization.

Result: Evaluated on ShapeNet and Matterport datasets, NeuralSSD achieves state-of-the-art accuracy and generalizability in surface reconstruction.

Conclusion: NeuralSSD provides a highly accurate and stable solution for 3D implicit surface reconstruction, outperforming existing methods.
...
TL;DR: NeuralBoneReg is a self-supervised, modality-agnostic framework for bone surface registration in CAOS, outperforming existing methods across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Accurate cross-registration between preoperative and intraoperative data is challenging due to modality heterogeneity, necessitating a robust and automatic solution.

Method: NeuralBoneReg uses a self-supervised approach with two modules: an implicit neural UDF for preoperative bone modeling and an MLP-based registration module for alignment.

Result: NeuralBoneReg achieved mean RRE/RTE of 1.68°/1.86 mm, 1.88°/1.89 mm, and 3.79°/2.45 mm on three datasets, demonstrating strong generalizability.

Conclusion: NeuralBoneReg provides accurate, robust cross-modal alignment for CAOS without requiring inter-subject training data.
...
TL;DR: Gen3d is a novel method for generating high-quality 3D scenes from a single image, overcoming limitations of dense multi-view dependencies. It uses Gaussian splatting optimization and shows strong generalization and high-fidelity results.


<details>
  <summary>Details</summary>
Motivation: Current neural 3D reconstruction methods rely on dense multi-view captures, limiting their applicability. High-quality 3D scene generation is crucial for embodied AI and world models.

Method: Gen3d creates an initial point cloud from an RGBD image, expands its world model, and optimizes a Gaussian splatting representation to finalize the 3D scene.

Result: Experiments show Gen3d excels in generalization, world model generation, and synthesizing high-fidelity novel views.

Conclusion: Gen3d advances single-image 3D scene generation with superior performance and broad applicability.
...
TL;DR: SAM-Fed is a federated semi-supervised learning framework addressing challenges like pseudo-label reliability and resource constraints by leveraging a high-capacity segmentation model and dual knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces data privacy and annotation cost challenges, with existing federated solutions struggling due to unreliable pseudo-labels and client device limitations.

Method: SAM-Fed uses a high-capacity segmentation foundation model to guide lightweight clients, combining dual knowledge distillation and an adaptive agreement mechanism for improved supervision.

Result: SAM-Fed outperforms state-of-the-art FSSL methods in skin lesion and polyp segmentation across homogeneous and heterogeneous settings.

Conclusion: SAM-Fed effectively addresses federated semi-supervised learning challenges, offering reliable pseudo-labels and robust performance despite client resource constraints.
...
TL;DR: Diff-NAF is proposed to improve CT reconstruction under ultra-sparse-view conditions by combining Neural Attenuation Fields with a dual-branch diffusion model, iteratively refining projections for better results.


<details>
  <summary>Details</summary>
Motivation: Multi-source stationary CT faces challenges in ultra-sparse-view settings, leading to degraded reconstruction quality, which traditional methods fail to address effectively.

Method: Diff-NAF uses Neural Attenuation Fields and a dual-branch diffusion model to iteratively refine projections, leveraging Angle-Prior Guided Projection Synthesis and Diffusion-driven Reuse Projection Refinement.

Result: Experiments on simulated and real data show Diff-NAF outperforms existing methods in ultra-sparse-view CT reconstruction.

Conclusion: Diff-NAF effectively enhances projection completeness and reconstruction fidelity, offering superior performance in challenging ultra-sparse-view conditions.
...
TL;DR: Dental3R is a pose-free, graph-guided pipeline for robust 3D intraoral reconstruction from sparse photographs, overcoming challenges like large view baselines and inconsistent illumination. It uses a Geometry-Aware Pairing Strategy (GAPS) and wavelet-regularized training to improve stability and detail preservation.


<details>
  <summary>Details</summary>
Motivation: Conventional intraoral scanning is inaccessible for remote tele-orthodontics, and sparse smartphone imagery poses challenges like unstable geometry estimation and over-smoothed reconstructions.

Method: Proposes Dental3R: (1) constructs GAPS for high-value image pairs, (2) trains 3DGS with wavelet-regularized objectives to preserve details.

Result: Validated on 950 clinical cases and 195 video-based cases, showing superior novel view synthesis for dental occlusion.

Conclusion: Dental3R effectively handles sparse, unposed inputs and outperforms state-of-the-art methods in preserving diagnostic details.
...
TL;DR: LSP-YOLO is a lightweight single-stage network for sitting posture recognition on embedded edge devices, achieving high accuracy and real-time performance with minimal computational cost.


<details>
  <summary>Details</summary>
Motivation: Addressing health problems from poor sitting posture, the paper aims to overcome the limitations of existing methods (high intrusiveness, intensive computation, poor real-time performance) for posture recognition.

Method: LSP-YOLO integrates PConv and SimAM into a lightweight module (Light-C3k2) to reduce computational cost while maintaining feature extraction. It maps keypoints directly to posture classes using pointwise convolution and employs intermediate supervision for efficient fusion of pose estimation and classification.

Result: The smallest model (LSP-YOLO-n) achieved 94.2% accuracy and 251 FPS on PC with a 1.9 MB model size. Real-time, high-accuracy inference was demonstrated on the SV830C + GC030A platform.

Conclusion: LSP-YOLO offers an efficient, lightweight, and deployable solution for sitting posture recognition, suitable for smart classrooms, rehabilitation, and human-computer interaction.
...
TL;DR: The paper introduces StepsNet, a generalized residual architecture addressing shortcut degradation and limited width issues in deep neural networks, demonstrating superior performance across tasks.


<details>
  <summary>Details</summary>
Motivation: Existing residual networks struggle to realize theoretical benefits of depth due to shortcut degradation and depth-width trade-offs, prompting the need for advanced designs.

Method: StepsNet separates features along the channel dimension and progressively learns via blocks with increasing width, addressing shortcut degradation and width limitations.

Result: StepsNet outperforms residual models in tasks like image classification, object detection, semantic segmentation, and language modeling.

Conclusion: StepsNet effectively bridges the gap between theoretical potential and practical performance of deep networks, offering a versatile improvement over residual architectures.
...
TL;DR: ArchMap is a training-free, knowledge-guided framework for robust structured dental understanding in 3D intraoral scans, addressing challenges like variability and incomplete geometry.


<details>
  <summary>Details</summary>
Motivation: Existing deep-learning methods rely on modality-specific training and large datasets, limiting generalization and clinical deployment. ArchMap aims to overcome these issues.

Method: ArchMap uses geometry-aware arch-flattening to standardize scans and a Dental Knowledge Base (DKB) for hierarchical reasoning.

Result: Tested on 1060 cases, ArchMap outperforms supervised baselines in accuracy and stability for tasks like tooth counting and clinical condition identification.

Conclusion: ArchMap shows that geometric normalization and ontology-guided reasoning provide a scalable solution for structured 3D intraoral scan analysis.
...
TL;DR: The paper proposes DentalSCR, a contour-guided framework for accurate 3D-2D alignment between intraoral scans and lateral cephalometric radiographs, addressing challenges like projective magnification and low-contrast dental crowns.


<details>
  <summary>Details</summary>
Motivation: Conventional intensity-driven registration methods fail under real clinical conditions due to issues like geometric distortion and acquisition-dependent variation, necessitating a more stable and accurate approach.

Method: DentalSCR constructs a U-Midline Dental Axis (UMDA) for stable initialization and standardized projection geometry, generates radiograph-like projections, and optimizes registration using symmetric bidirectional Chamfer distance in a coarse-to-fine manner.

Result: Evaluation on 34 clinical cases shows reduced landmark error, tighter dispersion on the lower jaw, and improved Chamfer and Hausdorff distances, outperforming conventional methods.

Conclusion: DentalSCR robustly handles real-world cephalograms, providing high-fidelity, clinically inspectable 3D-2D alignment.
...
TL;DR: ARC-Chapter is a large-scale video chaptering model trained on extensive bilingual annotations, outperforming previous methods significantly and showing strong transferability.


<details>
  <summary>Details</summary>
Motivation: Addressing the inefficiency of existing small-scale, coarse annotations in structuring long videos like lectures and podcasts.

Method: Developed ARC-Chapter with large-scale bilingual annotations (English-Chinese), integrating ASR transcripts, scene texts, and visual captions into hierarchical chapters.

Result: Achieved a 14.0% F1 score and 11.3% SODA score improvement over previous methods, with excellent downstream task performance.

Conclusion: ARC-Chapter sets a new benchmark for video chaptering, demonstrating scalability and transferability.
...
TL;DR: Image-Based Gaussian Splatting improves 3DGS by using source images for fine details and view-dependent effects, enhancing rendering quality without extra storage.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of 3DGS in capturing spatially varying color and view-dependent effects like specular highlights.

Method: Combines base color from standard 3DGS rendering with a learned residual from neighboring training images.

Result: Outperforms prior Gaussian Splatting methods in rendering quality on standard NVS benchmarks.

Conclusion: The proposed method offers an efficient solution for high-quality novel view synthesis without increasing storage overhead.
...
TL;DR: Bapp is a mobile app validated for real-time eyelid movement analysis, showing high accuracy (98.3%) compared to manual annotations by specialists.


<details>
  <summary>Details</summary>
Motivation: Existing tools for assessing eyelid movements are complex, costly, and limited in clinical use. Bapp aims to provide a portable, accessible solution.

Method: Bapp uses Flutter and Google ML Kit for on-device analysis. Validation involved 45 patient videos annotated by ophthalmology specialists.

Result: Bapp achieved 98.4% precision, 96.9% recall, and 98.3% accuracy, proving its reliability for monitoring eyelid movements.

Conclusion: Bapp is a reliable, objective tool for ocular health monitoring, offering a viable alternative to traditional manual methods.
...
### [88] [Blur-Robust Detection via Feature Restoration: An End-to-End Framework for Prior-Guided Infrared UAV Target Detection](https://arxiv.org/abs/2511.14371)
*Xiaolin Wang,Houzhang Fang,Qingshan Li,Lu Wang,Yi Chang,Luxin Yan*

Main category: cs.CV

TL;DR: The paper introduces JFD3, a Joint Feature-Domain Deblurring and Detection framework for improving infrared UAV target detection under motion blur conditions.


<details>
  <summary>Details</summary>
Motivation: Infrared UAV target images suffer from motion blur, reducing contrast between target and background. Existing methods prioritize visual quality over task-relevant feature enhancement.

Method: JFD3 uses a dual-branch architecture with shared weights, guided by clear branch features and a frequency structure module, plus self-supervised loss.

Result: JFD3 achieves superior detection performance on the IRBlurUAV benchmark while maintaining real-time efficiency.

Conclusion: JFD3 effectively enhances discriminative feature representation for blurred infrared UAV images, improving detection performance.
...
### [89] [A Quantitative Method for Shoulder Presentation Evaluation in Biometric Identity Documents](https://arxiv.org/abs/2511.14376)
*Alfonso Pedro Ridao*

Main category: cs.CV

TL;DR: The paper introduces a Shoulder Presentation Evaluation (SPE) algorithm to quantitatively assess shoulder pose compliance in biometric documents, showing strong correlation with human judgments.


<details>
  <summary>Details</summary>
Motivation: Address the lack of quantitative methods for evaluating shoulder pose compliance in biometric standards.

Method: Quantifies shoulder yaw and roll using 3D coordinates from pose estimation frameworks.

Result: SPE scores correlated strongly (r≈0.80) with human labels and effectively identified non-compliant samples.

Conclusion: The SPE algorithm is a lightweight, effective tool for automated compliance checking.
...
### [90] [Cheating Stereo Matching in Full-scale: Physical Adversarial Attack against Binocular Depth Estimation in Autonomous Driving](https://arxiv.org/abs/2511.14386)
*Kangqiao Zhao,Shuo Huai,Xurui Song,Jun Luo*

Main category: cs.CV

TL;DR: The paper introduces a novel 3D physical adversarial attack targeting stereo-based binocular depth estimation in autonomous driving, ensuring visual consistency and effectiveness across viewpoints.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored vulnerability of stereo-based depth estimation to physical adversarial examples (PAEs) compared to monocular perception attacks.

Method: Proposes a 3D PAE with global camouflage texture, a stereo matching rendering module for alignment, and a fine-grained merging attack for stealth.

Result: Demonstrates success in fooling stereo models into producing erroneous depth information.

Conclusion: The study highlights the effectiveness of 3D PAEs in attacking stereo vision systems, advancing adversarial research in autonomous driving.
...
### [91] [Enhancing LLM-based Autonomous Driving with Modular Traffic Light and Sign Recognition](https://arxiv.org/abs/2511.14391)
*Fabian Schmidt,Noushiq Mohammed Kayilan Abdul Nazar,Markus Enzweiler,Abhinav Valada*

Main category: cs.CV

TL;DR: TLS-Assist enhances LLM-based autonomous driving by adding explicit traffic light and sign recognition, improving safety and performance.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based driving agents lack mechanisms to enforce traffic rules and reliably detect safety-critical objects like traffic lights and signs.

Method: Introduces TLS-Assist, a modular redundancy layer that converts detections into structured natural language messages for LLM input.

Result: TLS-Assist improves driving performance by up to 14% over LMDrive and 7% over BEVDriver, reducing traffic infractions.

Conclusion: The framework is effective, plug-and-play, and model-agnostic, with publicly released code and models.
...
### [92] [BEDLAM2.0: Synthetic Humans and Cameras in Motion](https://arxiv.org/abs/2511.14394)
*Joachim Tesch,Giorgio Becherini,Prerana Achar,Anastasios Yiannakidis,Muhammed Kocabas,Priyanka Patel,Michael J. Black*

Main category: cs.CV

TL;DR: BEDLAM2.0 is an enhanced dataset for training 3D human motion estimators, offering more diversity and realism than BEDLAM, particularly benefiting methods that work in world coordinates.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of estimating 3D human motion in world coordinates from video, especially when both human and camera motion are present. Existing datasets lack the necessary richness, limiting progress.

Method: The authors introduce BEDLAM2.0, an improved version of the BEDLAM dataset, featuring more diverse and realistic cameras, body shapes, motions, clothing, hair, 3D environments, and added shoes.

Result: BEDLAM2.0 significantly outperforms BEDLAM in training state-of-the-art methods, particularly for world-coordinate human motion estimation, improving accuracy.

Conclusion: BEDLAM2.0 serves as a superior resource for training 3D human pose and motion regressors, providing rendered videos, ground truth data, and 3D assets for research.
...
### [93] [Stage Aware Diagnosis of Diabetic Retinopathy via Ordinal Regression](https://arxiv.org/abs/2511.14398)
*Saksham Kumar,D Sridhar Aditya,T Likhil Kumar,Thulasi Bikku,Srinivasarao Thota,Chandan Kumar*

Main category: cs.CV

TL;DR: The paper presents a state-of-the-art Ordinal Regression-based method for detecting Diabetic Retinopathy (DR) using preprocessing and achieves a high QWK score of 0.8992 on the APTOS dataset.


<details>
  <summary>Details</summary>
Motivation: Diabetic Retinopathy is a leading cause of preventable blindness, emphasizing the need for timely detection and intervention to prevent irreversible damage.

Method: The framework uses preprocessing techniques (Green Channel Extraction, Noise Masking, CLAHE) and Ordinal Regression on the APTOS-2019 dataset for DR classification.

Result: The model achieved a Quadratic Weighted Kappa (QWK) score of 0.8992, setting a new benchmark on the APTOS dataset.

Conclusion: The Ordinal Regression approach proves highly effective for DR detection, offering a promising solution for timely screening and intervention.
...
### [94] [Language as an Anchor: Preserving Relative Visual Geometry for Domain Incremental Learning](https://arxiv.org/abs/2511.14401)
*Shuyi Geng,Tao Zhou,Yi Zhou*

Main category: cs.CV

TL;DR: LAVA addresses Domain Incremental Learning challenges by replacing direct feature alignment with text-driven relative alignment, preserving consistent geometry across domains.


<details>
  <summary>Details</summary>
Motivation: To overcome inter-domain interference and knowledge fragmentation in DIL by leveraging semantic similarities anchored by language.

Method: Uses LAVA, a framework with text-based reference anchors to guide visual representations and maintain relative geometry.

Result: LAVA outperforms state-of-the-art methods on standard DIL benchmarks.

Conclusion: LAVA effectively bridges domains, enabling robust knowledge reuse and reducing forgetting.
...
### [95] [Cranio-ID: Graph-Based Craniofacial Identification via Automatic Landmark Annotation in 2D Multi-View X-rays](https://arxiv.org/abs/2511.14411)
*Ravi Shankar Prasad,Nandani Sharma,Dinesh Singh*

Main category: cs.CV

TL;DR: The paper introduces Cranio-ID, a novel framework for automating craniometric landmark annotation and cross-modal matching in forensic applications, improving reliability and accuracy over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional landmark localization methods are time-consuming and require expertise, while current automatic methods lack reliability due to insufficient validation.

Method: Cranio-ID uses trained YOLO-pose models for automatic landmark annotation on 2D skulls, followed by cross-modal matching via graph representations and semantic correspondence using cross-attention and optimal transport.

Result: The framework shows significant improvements in reliability and accuracy, validated on S2F and CUHK datasets, and performs effectively in cross-domain skull-to-face and sketch-to-face matching.

Conclusion: Cranio-ID offers a reliable and accurate solution for craniometric landmark annotation and cross-modal matching, addressing limitations of traditional and current automated methods.
...
### [96] [Learning to See Through a Baby's Eyes: Early Visual Diets Enable Robust Visual Intelligence in Humans and Machines](https://arxiv.org/abs/2511.14440)
*Yusen Cai,Bhargava Satya Nunna,Qing Lin,Mengmi Zhang*

Main category: cs.CV

TL;DR: The paper explores how simulating infant vision (CATDiet) trains SSL models, showing improved robustness and biological alignment in object recognition. CombDiet, initialized with CATDiet, outperforms standard SSL.


<details>
  <summary>Details</summary>
Motivation: To understand the ecological benefits of staged visual development in infants and apply it to improve machine learning models.

Method: Train SSL models on object-centric videos under simulated infant vision constraints (CATDiet: grayscale-to-color, blur-to-sharp, preserved temporal continuity). Evaluate using benchmarks across ten datasets.

Result: CATDiet variants show enhanced robustness in object recognition and biological alignment. CombDiet (CATDiet + standard training) outperforms standard SSL.

Conclusion: Early infant visual progression provides a framework for robust machine visual intelligence. All resources will be publicly released.
...
TL;DR: AVI is a training-free framework for video understanding that mimics human-like reasoning with a three-phase process, structured knowledge base, and open-source model ensemble.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack iterative refinement and agent-based methods are costly or require extensive training. AVI aims to provide flexible, human-like video comprehension without these drawbacks.

Method: AVI uses Retrieve-Perceive-Review reasoning, structured entity graphs, and integrates lightweight CV models with VLMs and LLMs.

Result: AVI achieves competitive performance on multiple benchmarks (LVBench, VideoMME-Long, LongVideoBench, Charades-STA) with superior interpretability.

Conclusion: AVI offers a scalable, interpretable, and cost-effective solution for complex video understanding tasks.
...
### [98] [DIR-TIR: Dialog-Iterative Refinement for Text-to-Image Retrieval](https://arxiv.org/abs/2511.14449)
*Zongwei Zhen,Biqing Zeng*

Main category: cs.CV

TL;DR: DIR-TIR framework improves interactive text-to-image retrieval by refining queries and images via dialog and image modules, outperforming single-query methods.


<details>
  <summary>Details</summary>
Motivation: To enhance controllability and fault tolerance in text-to-image retrieval through interactive, multi-turn dialogues.

Method: Uses Dialog Refiner Module to gather precise descriptions and Image Refiner Module to reduce visual-semantic gaps.

Result: Outperforms baselines, achieving higher retrieval precision and better interactive experience across datasets.

Conclusion: DIR-TIR's synergistic module integration significantly improves target image hit accuracy.
...
### [99] [CompEvent: Complex-valued Event-RGB Fusion for Low-light Video Enhancement and Deblurring](https://arxiv.org/abs/2511.14469)
*Mingchen Zhong,Xin Lu,Dong Li,Senyan Xu,Ruixuan Jiang,Xueyang Fu,Baocai Yin*

Main category: cs.CV

TL;DR: CompEvent is a complex neural network framework for low-light video deblurring, integrating event data and RGB frames holistically using complex-valued processing and GRUs for superior performance.


<details>
  <summary>Details</summary>
Motivation: Low-light video deblurring is critical for applications like nighttime surveillance and autonomous driving, but existing methods struggle with combined low-light and motion blur degradations. Event cameras promise better sensitivity and resolution, but staged fusion strategies limit effectiveness.

Method: CompEvent uses: 1) Complex Temporal Alignment GRU for iterative alignment and fusion of video and event streams via complex-valued convolutions; 2) Complex Space-Frequency Learning module for deep fusion in spatial and frequency domains.

Result: CompEvent outperforms state-of-the-art methods in low-light video deblurring by enabling full-process spatiotemporal fusion and maximizing complementary learning between modalities.

Conclusion: CompEvent's holistic approach, leveraging complex-valued neural networks, significantly enhances low-light video deblurring, demonstrating superior performance over existing methods.
...
### [100] [Learning Subglacial Bed Topography from Sparse Radar with Physics-Guided Residuals](https://arxiv.org/abs/2511.14473)
*Bayu Adhi Tama,Jianwu Wang,Vandana Janeja,Mostafa Cham*

Main category: cs.CV

TL;DR: A physics-guided residual learning framework improves subglacial bed topography prediction by combining DeepLabV3+ with lightweight physics terms, outperforming other models.


<details>
  <summary>Details</summary>
Motivation: Accurate subglacial bed topography is crucial for ice sheet modeling, but radar observations are sparse and uneven. Better methods are needed.

Method: Proposes a DeepLabV3+ decoder over ResNet-50, trained with physics and data terms like mass conservation, flow-aligned total variation, Laplacian damping, and non-negativity of thickness.

Result: Outperforms U-Net, Attention U-Net, FPN, and CNN in test-core accuracy and structural fidelity for Greenland sub-regions.

Conclusion: The residual-over-prior design with physics terms yields coherent, plausible beds, suitable for operational mapping under domain shift.
...
### [101] [2D Gaussians Spatial Transport for Point-supervised Density Regression](https://arxiv.org/abs/2511.14477)
*Miao Shang,Xiaopeng Hong*

Main category: cs.CV

TL;DR: Gaussian Spatial Transport (GST) uses Gaussian splatting to map pixel-annotation correspondence and Bayesian probability for transport plans, improving efficiency in tasks like crowd counting and landmark detection.


<details>
  <summary>Details</summary>
Motivation: The paper addresses inefficiencies in conventional optimal transport schemes by proposing GST to eliminate iterative transport plan computations during training.

Method: GST leverages Gaussian splatting to estimate pixel-annotation correspondence and derives a transport plan using Bayesian probability. A loss function integrates the transport plan into standard network optimization.

Result: Experiments on crowd counting and landmark detection show GST's effectiveness, outperforming traditional methods in efficiency.

Conclusion: GST provides a novel, efficient framework for pixel-annotation transport, validated by strong performance in computer vision tasks.
...
### [102] [Segmentation-Aware Latent Diffusion for Satellite Image Super-Resolution: Enabling Smallholder Farm Boundary Delineation](https://arxiv.org/abs/2511.14481)
...
TL;DR: SEED-SR introduces a segmentation-aware latent space approach for farm boundary delineation, bypassing explicit super-resolution and achieving significant improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: Accurate delineation of smallholder farms is challenging due to low revisit frequencies of high-resolution satellite images, necessitating innovative solutions combining high and low-resolution imagery.

Method: SEED-SR uses conditional latent diffusion models and geo-spatial foundation models to perform super-resolution in a segmentation-aware latent space, avoiding pixel-space SR.

Result: Achieves a 20x scale factor and shows 25.5% and 12.9% relative improvement in instance and semantic segmentation metrics, respectively.

Conclusion: SEED-SR effectively addresses limitations of current Ref-SR methods, offering a robust solution for large-scale farm boundary segmentation.
...
### [103] [Enhancing End-to-End Autonomous Driving with Risk Semantic Distillaion from VLM](https://arxiv.org/abs/2511.14499)
*Jack Qin,Zhitao Wang,Yinan Zheng,Keyu Chen,Yang Zhou,Yuanxin Zhong,Siyuan Cheng*

Main category: cs.CV

TL;DR: RSD enhances End-to-End AD systems by using Vision-Language Models to distill risk attention into BEV features, improving generalization and performance in complex driving scenarios.


<details>
  <summary>Details</summary>
Motivation: Current AD systems struggle with generalization and consistency when integrating Vision-Language Models, while end-to-end solutions face high computational demands. RSD aims to overcome these challenges by leveraging risk attention.

Method: Introduces Risk Semantic Distillation (RSD) and RiskHead, a plug-in module that distills risk estimates from VLMs into BEV features, creating interpretable risk-attention maps.

Result: Experiments on Bench2Drive show improved perception and planning due to enhanced BEV representations, better handling of complex and unpredictable conditions.

Conclusion: RSD effectively addresses generalization issues in AD systems by enriching BEV features with risk attention, aligning with human-like driving behavior for safer navigation.
...
### [104] [DeCo-VAE: Learning Compact Latents for Video Reconstruction via Decoupled Representation](https://arxiv.org/abs/2511.14530)
*Xiangchen Yin,Jiahui Yuan,Zhangchi Hu,Wenzhang Sun,Jie Chen,Xiaozhen Qiao,Hao Li,Xiaoyan Sun*

Main category: cs.CV

TL;DR: DeCo-VAE improves video VAEs by decoupling video content into keyframe, motion, and residual components, achieving compact latent representation and superior reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing video VAEs overlook frame content similarity, leading to redundant latent modeling. DeCo-VAE aims to address this by explicitly decoupling video components.

Method: DeCo-VAE decomposes video into keyframe, motion, and residual components. It uses dedicated encoders for each component and a shared 3D decoder for reconstruction. A decoupled adaptation strategy ensures stable training.

Result: DeCo-VAE achieves superior video reconstruction performance, as demonstrated by extensive quantitative and qualitative experiments.

Conclusion: Explicit decoupling of video components and dedicated latent representation lead to compact modeling and improved performance in video VAEs.
...
TL;DR: The paper introduces PAMM, a decoder-based framework leveraging state space models for multi-task dense prediction, enhancing task interconnectivity and global integration of task priors.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing methods in exploring task-level interactions for multi-task dense prediction, particularly focusing on improving interconnectivity and leveraging task-specific properties.

Method: Proposes Parameter Aware Mamba Model (PAMM) with dual state space parameter experts and Multi-Directional Hilbert Scanning for feature sequence construction.

Result: Demonstrates effectiveness on NYUD-v2 and PASCAL-Context benchmarks.

Conclusion: PAMM successfully enhances multi-task dense prediction through scalable state space models and innovative feature sequence methods.
...
### [106] [ForensicFlow: A Tri-Modal Adaptive Network for Robust Deepfake Detection](https://arxiv.org/abs/2511.14554)
*Mohammad Romani*

Main category: cs.CV

TL;DR: ForensicFlow is a tri-modal forensic framework combining RGB, texture, and frequency analysis for robust Deepfake detection, outperforming single-stream methods.


<details>
  <summary>Details</summary>
Motivation: Deepfakes threaten information integrity, and existing CNNs lack multi-scale artifact detection, requiring a more robust solution.

Method: ForensicFlow uses RGB (ConvNeXt-tiny), texture (Swin Transformer-tiny), and frequency (CNN + SE) branches with attention-based fusion.

Result: Achieves AUC 0.9752, F1-Score 0.9408, and accuracy 0.9208 on Celeb-DF (v2), surpassing single-stream baselines.

Conclusion: The tri-modal approach enhances detection resilience against subtle forgeries through comprehensive feature fusion.
...
### [107] [D-PerceptCT: Deep Perceptual Enhancement for Low-Dose CT Images](https://arxiv.org/abs/2511.14518)
*Taifour Yousra Nabila,Azeddine Beghdadi,Marie Luong,Zuheng Ming,Habib Zaidi,Faouzi Alaya Cheikh*

Main category: cs.CV

TL;DR: The paper introduces D-PerceptCT, an HVS-inspired architecture for enhancing LDCT images by preserving perceptually relevant features, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: LDCT images suffer from quality deterioration due to low radiation doses, and existing methods often lose critical details. The paper aims to enhance LDCT images by focusing on perceptually important features.

Method: D-PerceptCT includes a Visual Dual-path Extractor (ViDex) for semantic-awareness and a Global-Local State-Space block for long-range and multiscale features. A novel Deep Perceptual Relevancy Loss Function (DPRLF) is also proposed.

Result: Experiments on the Mayo2016 dataset show D-PerceptCT outperforms SOTA methods in preserving structural and textural details.

Conclusion: D-PerceptCT effectively enhances LDCT images by leveraging HVS principles, offering better diagnostic utility.
...
### [108] [A Generative Data Framework with Authentic Supervision for Underwater Image Restoration and Enhancement](https://arxiv.org/abs/2511.14521)
*Yufeng Tian,Yifan Chen,Zhe Sun,Libang Chen,Mingyu Dou,Jijun Lu,Ye Zheng,Xuelong Li*

Main category: cs.CV

TL;DR: The paper proposes using synthetic datasets from in-air natural images to overcome limitations in underwater image restoration caused by scarce high-quality paired data, achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of high-quality paired datasets in underwater image restoration, which limits model performance.

Method: Uses unpaired image-to-image translation to create synthetic datasets with precise labels for underwater degradation types.

Result: Models trained on synthetic data outperform or match those trained on existing benchmarks.

Conclusion: The synthetic dataset provides a reliable solution for underwater image restoration and is publicly available.
...
TL;DR: Neural SDFs struggle with multiple shapes/scenes due to limited latent space and loss of details. A new method combines generalization and overfitting strategies for high-fidelity results, using efficient sampling to improve training.


<details>
  <summary>Details</summary>
Motivation: Current neural SDFs face challenges in representing multiple shapes/scenes with high-fidelity details due to latent space constraints.

Method: Proposes a common space representation combining generalization and overfitting strategies, with a novel sampling technique to enhance training.

Result: Improved high-fidelity geometry details and compact latent codes, validated by benchmarks.

Conclusion: The method outperforms existing approaches in representation ability and compactness.
...
TL;DR: This paper presents a method for modeling hand-object interactions without prior object knowledge using dynamic 3D Gaussian Splatting, addressing challenges like occlusion, blur, and motion representation through novel optimizable parameters and interaction-aware fields.


<details>
  <summary>Details</summary>
Motivation: The study aims to accurately model the geometry and appearance of hand-object interactions dynamically, overcoming issues like mutual occlusion, edge blur, and complex motion without relying on object priors.

Method: The approach uses dynamic 3D Gaussian Splatting with optimizable parameters for clearer structural representation, integrates hand information into object deformation fields, and employs a progressive optimization strategy with explicit regularizations for stability.

Result: Experiments demonstrate superior performance over existing dynamic 3D-GS-based methods, achieving state-of-the-art results in reconstructing dynamic hand-object interactions.

Conclusion: The proposed method effectively addresses key challenges in hand-object interaction modeling, offering improved accuracy and realism through innovative techniques.
...
### [111] [Deep Learning-Based Regional White Matter Hyperintensity Mapping as a Robust Biomarker for Alzheimer's Disease](https://arxiv.org/abs/2511.14588)
*Julia Machnio,Mads Nielsen,Mostafa Mehdipour Ghazi*

Main category: cs.CV

TL;DR: A deep learning framework is proposed for robust segmentation and localization of white matter hyperintensities (WMH), showing better disease classification accuracy when regional WMH volumes are combined with brain atrophy metrics.


<details>
  <summary>Details</summary>
Motivation: Current automated WMH segmentation methods often overlook spatial distribution, limiting diagnostic value. The study aims to improve early diagnosis by focusing on regional WMH quantification.

Method: A deep learning framework evaluates WMH segmentation and localization across datasets, including an ADNI cohort, and integrates regional WMH volumes with brain structure metrics.

Result: Regional WMH volumes outperform global lesion burden for disease classification (AUC up to 0.97). Anterior white matter tracts show localized vulnerability in AD.

Conclusion: Regional WMH quantification enhances diagnostic accuracy, especially when combined with atrophy markers, offering potential improvements in early diagnosis and stratification for neurodegenerative disorders.
...
TL;DR: CCSD framework improves brain tumor segmentation in MRI by handling missing modalities via self-distillation strategies, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of missing MRI modalities in clinical settings, which hampers deep learning models' performance and generalizability.

Method: Introduces CCSD with shared-specific encoder-decoder architecture and two self-distillation strategies: hierarchical modality self-distillation and progressive modality combination distillation.

Result: Achieves state-of-the-art performance on public benchmarks in various missing-modality scenarios, demonstrating strong generalization and stability.

Conclusion: CCSD effectively tackles missing modality challenges, enhancing robustness and accuracy in brain tumor segmentation from MRI.
...
### [113] [Explaining Digital Pathology Models via Clustering Activations](https://arxiv.org/abs/2511.14558)
*Adam Bajger,Jan Obdržálek,Vojtěch Kůr,Rudolf Nenutil,Petr Holub,Vít Musil,Tomáš Brázdil*

Main category: cs.CV

TL;DR: A clustering-based explainability technique for digital pathology models provides global behavior insight and fine-grained details, enhancing understanding and clinical adoption confidence.


<details>
  <summary>Details</summary>
Motivation: To improve explainability in digital pathology models beyond single-slide saliency methods like GradCAM, offering broader and more detailed insights.

Method: Uses clustering to analyze and visualize the global behavior of convolutional neural networks in digital pathology.

Result: Demonstrated effectiveness on a prostate cancer detection model, showing practical utility.

Conclusion: The technique enhances model transparency and confidence, aiding faster clinical adoption.
...
### [114] [MRI Embeddings Complement Clinical Predictors for Cognitive Decline Modeling in Alzheimer's Disease Cohorts](https://arxiv.org/abs/2511.14601)
*Nathaniel Putera,Daniel Vilet Rodríguez,Noah Videcrantz,Julia Machnio,Mostafa Mehdipour Ghazi*

Main category: cs.CV

TL;DR: The paper evaluates tabular and MRI-based predictors for modeling Alzheimer's disease progression, introducing a 3D Vision Transformer (ViT) for MRI embeddings and highlighting complementary strengths of modalities.


<details>
  <summary>Details</summary>
Motivation: To improve early stratification and personalized management of Alzheimer's by capturing subtle brain changes missed by traditional tabular predictors.

Method: Used Dynamic Time Warping clustering for trajectory-aware labeling, trained a 3D ViT on MRI data, and evaluated embeddings with machine learning and deep learning classifiers.

Result: Clinical features excelled in predicting severe progression (AUC ~0.70), while ViT MRI embeddings were best for identifying stable individuals (AUC 0.71). All methods struggled with moderate cases.

Conclusion: Combining clinical and MRI-based features could enhance Alzheimer's progression modeling, leveraging strengths of each modality.
...
TL;DR: OmniZip is a training-free framework for compressing audio-video tokens in OmniLLMs, achieving faster inference and memory efficiency without performance loss.


<details>
  <summary>Details</summary>
Motivation: Existing token compression methods lack support for multimodal audio-video token compression, creating computational bottlenecks in OmniLLMs.

Method: OmniZip identifies salient audio tokens, computes audio retention scores, and dynamically prunes video tokens using cross-modal similarity. It employs an interleaved spatio-temporal scheme for compression.

Result: OmniZip achieves 3.42X inference speedup and 1.4X memory reduction over competitors while maintaining performance.

Conclusion: OmniZip effectively bridges the gap in multimodal token compression, offering significant computational advantages without training.
...
TL;DR: XAttn-BMD is a multimodal deep learning framework predicting femoral neck BMD using hip X-rays and clinical data, outperforming baselines with its cross-attention mechanism and custom loss.


<details>
  <summary>Details</summary>
Motivation: Poor bone health and osteoporosis risks necessitate accurate BMD prediction from accessible data like X-rays and metadata.

Method: Uses bidirectional cross-attention to integrate image and metadata features, with a Weighted Smooth L1 loss for imbalance handling.

Result: Reduces MSE by 16.7%, MAE by 6.03%, and increases R2 by 16.4% over baselines, validating cross-attention and loss effectiveness.

Conclusion: XAttn-BMD effectively estimates BMD, showing potential for real-world screening applications.
...
TL;DR: HoloTea is a 3D-aware flow-matching framework that improves gene expression imputation from histology by leveraging adjacent sections and a novel prior, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To enable a holistic understanding of tissue organization and disease by addressing scalability and generative capabilities gaps in existing 3D spatial transcriptomics methods.

Method: Uses a ControlNet conditioned on neighboring slides' morphological features, a ZINB-spatial prior for data counts, and a global attention block for scalability.

Result: Consistently outperforms 2D and 3D baselines in accuracy and generalization across diverse spatial transcriptomics datasets.

Conclusion: HoloTea advances 3D virtual tissue modeling, promising faster biomarker discovery and deeper disease insights.
...
### [118] [Improving segmentation of retinal arteries and veins using cardiac signal in doppler holograms](https://arxiv.org/abs/2511.14654)
*Marius Dubosc,Yann Fischer,Zacharie Auray,Nicolas Boutry,Edwin Carlinet,Michael Atlan,Thierry Geraud*

Main category: cs.CV

TL;DR: A new method improves artery-vein segmentation in Doppler holography by leveraging temporal dynamics with standard U-Nets, matching complex models' performance.


<details>
  <summary>Details</summary>
Motivation: Traditional segmentation methods miss temporal data richness in Doppler holography, limiting accurate retinal blood flow assessment.

Method: Integrates pulse analysis-derived features into standard U-Nets to exploit temporal dynamics for segmentation.

Result: Achieves performance comparable to more complex models, enhancing retinal hemodynamics exploration.

Conclusion: Time-resolved preprocessing can optimize deep learning for Doppler holography, advancing retinal hemodynamics analysis.
...
TL;DR: The paper proposes BioST-GCN, a vision-based fall prediction model combining pose and biomechanical data, outperforming baselines but highlighting a significant simulation-reality gap.


<details>
  <summary>Details</summary>
Motivation: Address the scarcity of fall data and improve fall prediction for older adults using non-invasive, vision-based systems.

Method: Developed BioST-GCN, a dual-stream model integrating pose and biomechanical information with cross-attention fusion.

Result: Achieved higher F1-scores on simulated datasets but showed poor zero-shot generalization due to simulation biases.

Conclusion: Emphasizes the need for personalized strategies and real-world data to bridge the simulation-reality gap for elderly fall prediction.
...
TL;DR: Optimizing Gaussian Splatting for scene geometry struggles with overfitting in sparse-view scenarios. The paper proposes 
et{}, using Stereo Geometry-Texture Alignment and Pseudo-Feature Enhanced Geometry Consistency to improve reconstruction and view synthesis.


<details>
  <summary>Details</summary>
Motivation: Sparse input views lead to overfitting in Gaussian Splatting, degrading reconstruction quality and novel view synthesis. Existing methods using flattened Gaussians worsen overfitting.

Method: Introduces Stereo Geometry-Texture Alignment to align rendering and geometry, and Pseudo-Feature Enhanced Geometry Consistency for multi-view consistency, reducing overfitting.

Result: Achieves state-of-the-art performance on DTU, BlendedMVS, and Mip-NeRF360 datasets, improving surface reconstruction and view synthesis.

Conclusion: 
et{} effectively addresses overfitting in sparse-view scenarios, enhancing both geometry reconstruction and novel view rendering quality.
...
TL;DR: SLAM-AGS is a multitask pretraining framework addressing unreliable labels and low witness rates in computational cytology by combining weakly supervised similarity and self-supervised contrastive objectives, with adaptive gradient surgery for stability.


<details>
  <summary>Details</summary>
Motivation: Address unreliable instance-level labels and extremely low witness rates in computational cytology.

Method: Jointly optimizes weakly supervised similarity on slide-negative patches and self-supervised contrastive objectives on slide-positive patches, using Adaptive Gradient Surgery for stable learning. Integrates pretrained encoder into an attention-based Multiple Instance Learning aggregator.

Result: Improves bag-level F1-Score and Top 400 positive cell retrieval, especially at low witness rates (0.5%-10%).

Conclusion: SLAM-AGS resolves gradient interference for stable pretraining and better downstream task performance, with open-source implementation.
...
TL;DR: RepAir, a three-stage framework combining nnU-Net-based segmentation with topology correction, outperforms existing methods in 3D airway segmentation, producing more complete and anatomically consistent results.


<details>
  <summary>Details</summary>
Motivation: Manual airway segmentation from CT scans is impractical, and current automated methods often yield disconnected components, hindering biomarker extraction.

Method: RepAir uses an nnU-Net-based network for initial segmentation, a skeleton-based algorithm to identify discontinuities, and a 1D convolutional classifier to validate reconnections.

Result: RepAir outperforms Bronchinet and NaviAirway on voxel-level and topological metrics across healthy and pathological datasets (ATM'22 and AeroPath).

Conclusion: The RepAir framework enhances airway segmentation accuracy and anatomical consistency, making it reliable for quantitative lung analysis.
...
TL;DR: The paper evaluates watermarking techniques for LLM-generated texts, finding they preserve semantics but alter writing style and are vulnerable to adversarial attacks like back translation.


<details>
  <summary>Details</summary>
Motivation: To address concerns about watermarking negatively impacting text quality and being vulnerable to attacks, hindering its adoption by LLM creators.

Method: Evaluated robustness of watermarking techniques against paraphrasing and back translation attacks, and assessed their impact on text quality and writing style using linguistic metrics.

Result: Watermarking techniques preserve semantics but deviate from unwatermarked texts' writing style and are vulnerable to adversarial attacks, particularly back translation.

Conclusion: Adoption of watermarking faces challenges due to its susceptibility to attacks and impact on writing style, despite preserving semantic quality.
...
TL;DR: RT (Refine Thought) enhances text embedding models' semantic reasoning by multi-pass forwarding, showing improvements in benchmarks like BRIGHT and PJBenchmark1 while maintaining general performance.


<details>
  <summary>Details</summary>
Motivation: To improve the semantic reasoning ability of text embedding models without compromising their general-purpose understanding.

Method: RT runs multiple forward passes of the embedding model to refine the final semantic representation.

Result: Significant improvements in semantic reasoning tasks (BRIGHT, PJBenchmark1) while consistent performance on general-purpose tasks (C-MTEB).

Conclusion: RT effectively activates pretrained semantic reasoning abilities in decoder-only models, serving as a test-time inference method.
...
### [125] [Enhancing Agentic Autonomous Scientific Discovery with Vision-Language Model Capabilities](https://arxiv.org/abs/2511.14631)
*Kahaan Gandhi,Boris Bolliet,Inigo Zubeldia*

Main category: cs.CL

TL;DR: Multi-agent systems using vision-language models (VLMs) enhance autonomous scientific discovery by evaluating and correcting errors dynamically. Case studies show significant improvements over traditional methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve autonomous scientific discovery by leveraging VLMs to dynamically evaluate and correct errors in real-time, reducing reliance on human intervention.

Method: The method involves using VLMs as judges to evaluate plots against dynamically generated rubrics, enabling agents to self-correct and steer exploratory data analysis.

Result: VLM-augmented systems achieve pass at 1 scores of 0.7-0.8 on a benchmark, outperforming code-only (0.2-0.3) and code-and-text baselines (0.4-0.5). Case studies confirm adaptability and error recovery.

Conclusion: VLMs significantly enhance autonomous scientific discovery by enabling real-time error correction and adaptability, outperforming traditional methods and improving interpretability.
...
TL;DR: The paper compares two training-free approaches for segment-level error correction in MT, with QE-informed Retranslation outperforming a QE-based substring replacement method.


<details>
  <summary>Details</summary>
Motivation: To address overcorrection in APE systems and improve MT quality without retraining.

Method: Two training-free approaches: (1) selecting the highest-quality translation from LLM candidates, and (2) using QE explanations to instruct LLMs for substring replacements.

Result: The first approach outperformed the second, achieving a Delta COMET score of 0.0201 vs. -0.0108.

Conclusion: QE-informed Retranslation is more effective for segment-level error correction in MT systems.
...
TL;DR: The paper introduces GM-Extract, a benchmark dataset to evaluate LLMs' retrieval performance of control variables, revealing challenges like the 'lost-in-the-middle' phenomenon. It proposes two metrics for spatial and semantic retrieval, tests models on multi-document tasks, and analyzes mitigation methods' nuanced effectiveness.


<details>
  <summary>Details</summary>
Motivation: To address the 'lost-in-the-middle' phenomenon in LLMs, which hampers long-range context utilization in retrieval-based applications, and to provide a real-world evaluation benchmark.

Method: Develops GM-Extract dataset, uses Document Metric and Variable Extraction Metric for evaluation, tests 7-8B parameter models on multi-document tasks, and surveys mitigation methods (black-box and white-box).

Result: Performance varies with context window representation; mitigation methods show nuanced efficacy, sometimes improving performance and other times harming it.

Conclusion: The study offers insights into LLM retrieval challenges and mitigation strategies, emphasizing practical utility and the need for tailored approaches.
...
TL;DR: LLMs interpret superlatives in e-commerce queries via structured hints, improving search performance and enabling efficient ranking integration.


<details>
  <summary>Details</summary>
Motivation: Understand latent intent behind superlative queries to enhance search relevance and performance.

Method: Decompose queries into attribute-value hints, integrate them into ranking pipelines, and transfer interpretations to lightweight models.

Result: Improved search by 10.9 points in MAP and ranking by 5.9 points in MRR over baselines.

Conclusion: LLMs can effectively interpret and transfer superlative semantics for practical deployment in retrieval systems.
...
TL;DR: MoRA-RAG is a knowledge-grounded LLM framework that structures post-disaster reconnaissance reports for multi-hazard reasoning, outperforming existing methods with high accuracy and reduced hallucinations.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of unstructured narratives in post-disaster reports and unreliable outputs from LLMs lacking domain grounding.

Method: Introduces Mixture-of-Retrieval Agentic RAG (MoRA-RAG), integrating dynamic query routing, agentic chunking, and verification loops.

Result: Achieves 94.5% accuracy, surpassing zero-shot LLMs by 30% and state-of-the-art RAG systems by 10%, with reduced hallucinations.

Conclusion: MoRA-RAG transforms disaster documentation into actionable intelligence, enhancing hazard resilience.
...
TL;DR: HiEAG introduces a Hierarchical Evidence-Augmented Generation framework for multimodal misinformation detection, emphasizing external consistency via retrieval, reranking, and rewriting with MLLMs, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing OOC misinformation detection methods lack focus on external consistency between image-text pairs and external evidence, limiting their effectiveness.

Method: HiEAG decomposes external consistency checking into retrieval, reranking (using AESP), and rewriting (using AEGP) within a multimodal large language model framework, enhancing task adaptation and judgment explanation.

Result: HiEAG achieves superior accuracy across benchmark datasets compared to previous state-of-the-art methods, demonstrating its effectiveness.

Conclusion: The HiEAG framework advances multimodal misinformation detection by leveraging external consistency and MLLMs, setting a new benchmark for performance.
...
TL;DR: The paper addresses class imbalance in multi-label sentiment classification by creating a balanced dataset using GoEmotions, Sentiment140, and GPT-4 mini annotated texts. It proposes an enhanced model combining FastText, CNN, BiLSTM, and attention mechanisms, showing improved performance metrics.


<details>
  <summary>Details</summary>
Motivation: Existing datasets like GoEmotions suffer from class imbalance, negatively impacting model performance for underrepresented emotions.

Method: Constructed a balanced dataset using GoEmotions, Sentiment140 (labeled via RoBERTa-base-GoEmotions), and GPT-4 mini annotations. Developed a model with FastText embeddings, CNN, BiLSTM, attention, and mixed precision training.
...
Conclusion: The balanced dataset and enhanced model effectively address class imbalance, improving multi-label sentiment classification performance.
...
TL;DR: A novel attack method called Stealth Fine-Tuning bypasses safety alignment in RVLMs using segment-level interference and self-generated outputs, achieving high attack success rates with minimal data and computational cost.


<details>
  <summary>Details</summary>
Motivation: To expose vulnerabilities in RVLMs' safety alignment by introducing a lightweight attack method that preserves reasoning abilities.

Method: Stealth Fine-Tuning uses segment-level interference and turn-based weighted loss to fine-tune models with minimal data and compute.

Result: The method outperforms IDEATOR by 38.52% ASR with only 499 samples and 3 hours on a single A100 while maintaining reasoning ability.

Conclusion: Stealth Fine-Tuning is a low-cost, effective way to bypass RVLM safety defenses, highlighting alignment vulnerabilities.
...
TL;DR: A framework generates synthetic discharge summaries to address the imbalance in ICD coding from clinical text, improving rare code prediction without sacrificing performance on common codes.


<details>
  <summary>Details</summary>
Motivation: The extreme long-tail distribution of ICD codes in datasets like MIMIC-III results in low macro-F1 scores, necessitating a solution to improve representation of rare and zero-shot codes.

Method: Leverages real-world co-occurrence patterns, ICD descriptions, synonyms, taxonomy, and similar notes to create realistic synthetic summaries, expanding training data to cover 7,902 ICD codes.

Result: Fine-tuning PLM-ICD and GKI-ICD on extended datasets yields modest macro-F1 improvements while maintaining strong micro-F1, outperforming prior SOTA.

Conclusion: Carefully crafted synthetic data enhances equity in long-tail ICD code prediction, though gains are marginal relative to computational cost.
...
TL;DR: HyperABSA introduces a dynamic hypergraph framework for Aspect-Based Sentiment Analysis, improving robustness and performance over traditional graph-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based approaches for ABSA are limited by redundancy, parameter overhead, and error propagation due to modeling only pairwise dependencies.

Method: HyperABSA uses sample-specific hierarchical clustering to construct hyperedges, introducing an acceleration-fallback cutoff for adaptive granularity.

Result: Experiments on Lap14, Rest14, and MAMS benchmarks show consistent improvements, especially with RoBERTa backbones.

Conclusion: Dynamic hypergraph construction proves efficient and powerful for ABSA, with potential applications in other short-text NLP tasks.
...
TL;DR: Combining Transformer-based relation extraction with knowledge graph matching improves traceable answers for multiple-choice questions, achieving ~70% accuracy with significant category influence.


<details>
  <summary>Details</summary>
Motivation: The study aims to dynamically generate knowledge graphs (KGs) from text using Transformer-based relation extraction (RE) and apply them to answer multiple-choice questions (MCQs) traceably.

Method: The method dynamically creates KGs from input sentences using RE, verifies their truthfulness against factual KGs, and answers fill-in-the-blank MCQs while ensuring traceability.

Result: The approach achieves ~70% accuracy in answering questions and highlights the significant impact of question categories on performance.

Conclusion: Dynamic KG generation and verification enhance MCQ answering with traceability, though accuracy varies by question category.
...
TL;DR: The paper proposes a selective weak-to-strong generalization (W2SG) framework to improve model alignment by avoiding unnecessary weak supervision and refining weak labels.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with robustness due to reliance on weak supervision, which can harm models. The aim is to enhance alignment for superhuman models.

Method: A binary classifier P(IK) identifies questions a strong model can answer, using self-generated labels. Weak labels are refined via graph smoothing.

Result: Experiments on three benchmarks show consistent outperformance over baselines. P(IK) generalizes across tasks and difficulties.

Conclusion: Selective W2SG improves alignment, aiding superalignment efforts by reducing reliance on weak supervision.
...
TL;DR: The paper introduces a symbolic localization framework to trace hallucinations in LLMs, revealing that symbolic triggers cause critical instability early in processing.


<details>
  <summary>Details</summary>
Motivation: To understand and localize the origins of hallucinations in LLMs, particularly focusing on symbolic triggers like modifiers and negation.

Method: Proposes a symbolic localization framework using symbolic linguistic knowledge, tested on five models with HaluEval and TruthfulQA datasets.

Result: Hallucination stems from early-layer instability (layers 2-4) with symbolic triggers, especially negation. Larger models still show high hallucination rates (78.3%-83.7%).

Conclusion: Hallucination is rooted in symbolic linguistic processing failures, not general generation issues, highlighting the importance of symbolic knowledge for understanding LLM hallucinations.
...
TL;DR: DeepEL integrates Large Language Models (LLMs) throughout all stages of Entity Linking (EL), introducing a self-validation mechanism for better performance, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Prior EL methods use LLMs in isolated stages, lacking full integration and optimal disambiguation. DeepEL aims to address these gaps for enhanced accuracy and robustness.

Method: DeepEL incorporates LLMs comprehensively in EL, adding a self-validation mechanism leveraging global context to improve entity disambiguation and cohesion.

Result: DeepEL achieves a 2.6% average F1 score improvement and a 4% gain on out-of-domain datasets across ten benchmarks.

Conclusion: DeepEL's deep LLM integration and self-validation effectively advance EL performance, highlighting LLMs' potential in fully integrated frameworks.
...
TL;DR: The paper introduces ArbESC+, a multi-system approach for Arabic grammatical error correction, outperforming single models by combining various systems and achieving high F0.5 scores on QALB datasets.


<details>
  <summary>Details</summary>
Motivation: Arabic's complex structure makes grammatical error correction challenging. Existing models often work independently, missing benefits from system integration.

Method: The ArbESC+ framework combines multiple models (e.g., AraT5, ByT5) to generate corrections, represented as numerical features, and uses a classifier to select optimal corrections with support techniques for filtering and reliability estimation.

Result: ArbESC+ achieved F0.5 scores of 82.63% (QALB-14), 84.64% (QALB-15 L1), and 65.55% (QALB-15 L2), outperforming single models.

Conclusion: The study demonstrates the effectiveness of multi-system integration for Arabic GEC, advancing tools for Arabic text processing and benefiting researchers and users.
...
TL;DR: This paper introduces a method to improve large language models' performance in the music-entertainment domain by constructing a specialized corpus and refining training objectives.


<details>
  <summary>Details</summary>
Motivation: Current language models struggle in specialized domains like music due to inadequate corpus scale and alignment with training objectives.

Method: A domain-first pipeline constructs a large music-related corpus (40B tokens), uses classifier-based filtering, multi-stage cleaning, and integrates metadata. Training employs token-level soft scoring for quality control.

Result: The approach enhances music-domain pretraining and alignment, supported by the MusicSimpleQA benchmark for factuality assessment.

Conclusion: This work provides a scalable framework and evaluation tool for developing specialized domain LLMs in music.
...
TL;DR: Authentic-Dubber enhances automatic movie dubbing by simulating director-actor interactions, using multimodal references and LLMs for emotional depth, ensuring better lip-sync and expressiveness.


<details>
  <summary>Details</summary>
Motivation: Existing dubbing models lack authentic director-actor collaboration, missing nuanced emotional cues crucial for realistic dubbing.

Method: Proposes Retrieve-Augmented Director-Actor Interaction Learning with a multimodal Reference Footage library, Emotion-Similarity-based Retrieval, and Progressive Graph-based speech generation.

Result: Improves emotional expressiveness and lip-sync, validated on V2C Animation benchmark.

Conclusion: Authentic-Dubber replicates real dubbing workflows effectively, offering superior performance.
...
TL;DR: AfriSpeech-MultiBench is the first evaluation suite for African English accents, benchmarking ASR and LLM-based systems across diverse domains, revealing performance variations and practical insights.


<details>
  <summary>Details</summary>
Motivation: Address the lack of publicly available application-specific model evaluations for Africa's linguistic diversity.

Method: Develop AfriSpeech-MultiBench, benchmarking open, closed, unimodal ASR, and multimodal LLM-based systems using spontaneous and non-spontaneous speech from African accented English datasets.

Result: Open-source ASR excels in spontaneous speech but degrades in noisy dialogue; multimodal LLMs are accent-robust but struggle with domain-specific entities; fine-tuned models achieve competitive accuracy.

Conclusion: The benchmark aids practitioners in selecting voice technologies for African use-cases, promoting inclusive applications for underserved communities.
...
TL;DR: The paper introduces an entropy-guided training framework to compress reasoning outputs by addressing the entropy conflict, achieving shorter reasoning chains without losing accuracy.


<details>
  <summary>Details</summary>
Motivation: Large reasoning models face practical bottlenecks due to long chain-of-thought outputs, which incur high computation costs and poor deployability. Existing compression methods overlook the entropy conflict during training.

Method: The authors propose an entropy-guided training framework that dynamically balances compression and exploration. It encourages concise reasoning steps as entropy decreases and reinforces exploration as entropy rises.
...
Conclusion: The entropy-guided approach effectively resolves the entropy conflict, enabling efficient reasoning without compromising performance.
...
TL;DR: The paper explores how verbalized probability distributions enhance confidence estimation in LLMs by encouraging thorough reasoning.


<details>
  <summary>Details</summary>
Motivation: Understanding model reliability is crucial, especially with LLMs' strong generation capabilities. Current methods lack exploration of how reasoning strategies impact confidence estimation.

Method: The study uses verbalized probability distributions to prompt LLMs to consider all answer candidates and carefully assign confidence scores, leveraging chain-of-thought reasoning.

Result: This approach outperforms others across models and tasks, even post-reinforcement learning, with reasoning patterns aligning with human expectations.

Conclusion: Verbalized probability distributions improve confidence estimation by fostering deeper reasoning, applicable broadly regardless of answer space knowledge.
...
TL;DR: AraLingBench is a human-annotated benchmark for evaluating Arabic linguistic competence in LLMs, revealing gaps in deeper grammatical reasoning despite surface-level proficiency.


<details>
  <summary>Details</summary>
Motivation: To assess the true linguistic mastery of Arabic LLMs beyond surface-level performance and highlight gaps in grammatical and syntactic understanding.

Method: AraLingBench consists of 150 expert-designed multiple-choice questions spanning five linguistic categories (grammar, morphology, spelling, reading comprehension, syntax). It evaluates 35 Arabic and bilingual LLMs.

Result: Current models show strong surface-level proficiency but struggle with deeper grammatical and syntactic reasoning, relying on memorization or pattern recognition rather than genuine comprehension.

Conclusion: AraLingBench serves as a diagnostic tool for improving Arabic LLMs by isolating and measuring fundamental linguistic skills, emphasizing the need for authentic linguistic mastery.
...
TL;DR: The paper introduces ConInstruct, a benchmark to evaluate LLMs' ability to handle conflicting instructions, revealing strong detection capabilities but poor user notification practices.


<details>
  <summary>Details</summary>
Motivation: Existing works overlook LLMs' performance under conflicting instructions, prompting the need for a dedicated evaluation benchmark.

Method: ConInstruct is developed as a benchmark to test LLMs' conflict detection and resolution abilities, with experiments conducted on proprietary and open-source models.

Result: Proprietary LLMs excel in conflict detection; DeepSeek-R1 and Claude-4.5-Sonnet lead in performance. However, LLMs rarely inform users about conflicts.

Conclusion: The study identifies a significant gap in LLMs' ability to communicate conflicting constraints, calling for improvements in instruction-following models.
...
TL;DR: ATLAS is a high-difficulty, cross-disciplinary benchmark suite designed to evaluate LLMs' scientific reasoning and integration across fields, addressing limitations of existing benchmarks like narrow focus and data contamination.


<details>
  <summary>Details</summary>
Motivation: The saturation of LLMs on traditional benchmarks and the inadequacy of high-difficulty benchmarks necessitate a more robust, interdisciplinary evaluation tool.

Method: ATLAS includes 800 original problems across seven scientific fields, featuring high originality, contamination resistance, cross-disciplinary focus, high-fidelity answers, and rigorous quality control. Automated evaluation uses LLM judges.
...
Conclusion: ATLAS aims to be a long-term, community-driven platform for assessing progress toward Artificial General Intelligence.
...
TL;DR: The paper addresses label length bias in LLMs, proposing NCC to normalize and calibrate predictions, improving performance and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing calibration methods overlook biases from multi-token class labels, leading to inconsistent treatment of labels of different lengths.

Method: Proposes normalized contextual calibration (NCC), which normalizes and calibrates predictions at the full-label level.

Result: NCC achieves up to 10% F1 improvement, extends bias mitigation to tasks like multiple-choice QA, and enhances reliability with fewer examples.

Conclusion: Mitigating full-label biases improves LLM performance and robustness, especially in real-world applications with multi-token labels.
...
TL;DR: EduHarm benchmarks educational LLMs' safety vulnerabilities, proposing TSSF to mitigate jailbreak and fine-tuning attacks via attention realignment, layer-wise judgment, and dual routing.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of focus on safety in educational LLMs, the paper introduces EduHarm to systematically evaluate and improve their robustness against attacks.

Method: TSSF employs safety-aware attention realignment, layer-wise safety judgment, and defense-driven dual routing to protect against attacks while maintaining utility.

Result: TSSF effectively defends against eight jailbreak strategies and three fine-tuning datasets, balancing safety and utility.

Conclusion: The proposed framework enhances educational LLM safety without over-refusal, offering robust defense and preserving benign functionality.
...
TL;DR: MedBench v4 is a comprehensive evaluation framework for medical LLMs, multimodal models, and agents, revealing gaps in safety and multimodal reasoning while showing agents improve performance.


<details>
  <summary>Details</summary>
Motivation: To address the need for evaluation frameworks reflecting real clinical workflows and safety constraints for medical AI models.

Method: A nationwide, cloud-based benchmarking infrastructure with over 700,000 expert-curated tasks across specialties, evaluated using LLM-as-a-judge calibrated to human ratings.

Result: Base LLMs scored 54.1/100, multimodal models 47.5/100, and agents 79.8/100, with Claude Sonnet 4.5-based agents achieving the highest scores. Safety and ethics scores remained low.

Conclusion: Agentic orchestration enhances clinical readiness, but gaps in safety and multimodal reasoning persist, making MedBench v4 a practical tool for auditing medical AI.
...
TL;DR: Tell Me is a mental well-being system using large language models for personalized support, synthetic dialogue generation for therapy research, and AI-generated self-care plans, aiming to complement professional therapy.


<details>
  <summary>Details</summary>
Motivation: The system addresses barriers to mental health support and the shortage of confidential therapeutic data by leveraging AI to provide accessible, personalized assistance and facilitate research.

Method: It integrates a retrieval-augmented generation assistant, synthetic client-therapist dialogue generator, and CrewAI-based planner for self-care plans and guided meditation.

Result: The system was evaluated through curated scenarios using LLM-based judgments and human studies, demonstrating its potential to support well-being.

Conclusion: Tell Me showcases interdisciplinary collaboration opportunities to advance responsible AI-human interaction in mental health, complementing existing care.
...
TL;DR: The paper explores Reinforcement Learning (RL) for Large Language Model (LLM) Agents, introducing Agent-R1, a flexible training framework, and validating its effectiveness on Multihop QA tasks.


<details>
  <summary>Details</summary>
Motivation: RL is underutilized for LLM Agents, lacks tailored approaches, and needs adaptable frameworks for diverse tasks.

Method: Extends MDP framework for LLM Agents and develops Agent-R1, a modular RL training framework.

Result: Initial validation shows effectiveness on Multihop QA benchmarks.

Conclusion: Agent-R1 advances RL for LLM Agents, offering modularity and adaptability for future applications.
...
TL;DR: The paper introduces LiveRAG, a benchmark dataset for evaluating Retrieval Augmented Generation (RAG) systems, featuring 895 synthetic Q&A pairs with ground-truth answers, difficulty scores, and supporting claims.


<details>
  <summary>Details</summary>
Motivation: The rise of RAG in generative AI creates a need for systematic evaluation tools to assess effectiveness and robustness.

Method: LiveRAG is derived from SIGIR'2025 LiveRAG Challenge data, augmented with ground-truth answers, supporting claims, and IRT-derived difficulty/discriminability scores.

Result: The benchmark offers diverse questions, varying difficulty, and effectively differentiates system capabilities.

Conclusion: LiveRAG aids RAG research by enabling systematic evaluation and fostering robust Q&A system development.
...
TL;DR: Exploring methods to align and evaluate document-level claim sets for fact-checking, highlighting current limitations and the need for advanced evaluation techniques.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of evaluating extracted claims in fact-checking, especially for informal and context-rich languages like Czech and Slovak.

Method: Aligning claim sets from models and humans, computing similarity scores, and identifying optimal alignment and evaluation methods.

Result: Experiments reveal limitations of current evaluation methods, emphasizing the need for advanced techniques capturing semantic similarity and claim properties.

Conclusion: Highlights gaps in current claim extraction evaluation and advocates for more sophisticated methods to assess key claim attributes.
...
TL;DR: The study uses EHR data to track AKI patients' progression to CKD, identifying 15 distinct post-AKI states with varying CKD risks and factors influencing progression.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of identifying AKI patients at high risk of developing CKD by leveraging EHR data for dynamic tracking and risk assessment.

Method: Employed clustering of patient vectors from medical codes and creatinine measurements, multi-state modeling for transition probabilities, and survival analysis to identify CKD risk factors.

Result: Out of 20,699 AKI patients, 17% developed CKD, with 15 distinct post-AKI states identified. Most patients remained stable or transitioned once. Risk factors varied across states.

Conclusion: The findings support a data-driven approach for early CKD detection in AKI patients, aiding in targeted interventions and decision-support tools.
...
TL;DR: The paper compares how humans and LLMs detect political bias in news, finding RoBERTa aligns best with human labels, while GPT performs well in zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: To explore how LLMs align with human judgment in detecting political bias in news media, given the complexity of linguistic and contextual cues.

Method: Constructed a manually annotated dataset of news articles and evaluated annotation consistency, bias polarity, and inter-model agreement using LLMs like GPT, BERT, RoBERTa, and FLAN.

Result: RoBERTa showed the highest alignment with human labels among transformer models, while GPT had strong zero-shot agreement. A fine-tuned RoBERTa model achieved the highest accuracy.

Conclusion: Hybrid frameworks combining human interpretability and model scalability are needed for effective automated media bias detection due to systematic differences in perception.
...
TL;DR: RareSeek R1, a novel LLM-based diagnostic tool for rare diseases, combines domain-specific clinical data with advanced reasoning techniques to achieve high accuracy and robustness, matching physician performance and improving diagnostic speed.


<details>
  <summary>Details</summary>
Motivation: Rare diseases are challenging to diagnose due to prolonged timelines and noisy data. General LLMs lack real-world EHRs and domain expertise, leading to diagnostic inefficiencies.

Method: Developed via staged instruction tuning, chain of thought learning, and graph-grounded retrieval using a specialized clinical corpus and clinician-validated reasoning set.

Result: Achieves state-of-the-art accuracy, generalization, and stability in multicenter EHRs and benchmarks. Augmented retrieval resolves ambiguity, aligning candidates to mechanisms. Human studies show physician-level performance.

Conclusion: RareSeek R1 advances a narrative-first, knowledge-integrated reasoning paradigm, shortening diagnostic delays and providing auditable, clinically translatable support.
...
TL;DR: The paper explores comparative illusions (CIs) in language processing, using Bayesian inference to explain why people misjudge nonsensical sentences. It extends prior research by modeling illusion strength and predicting fine gradations in CI effects.


<details>
  <summary>Details</summary>
Motivation: To investigate why people systematically misjudge nonsensical comparisons (CIs) and test the noisy-channel theory of sentence comprehension.

Method: Used a quantitative model of posterior probability derived from statistical language models and human behavioral data to predict CI strength.

Result: The model successfully explained fine variations in CI effects and an unexplained effect related to pronoun vs. noun phrase subjects.

Conclusion: The findings support noisy-channel inference as a unified theory for diverse language processing phenomena, including CIs.
...
TL;DR: Larger LLMs can systematically jailbreak smaller ones, with harm likelihood and severity correlating with attacker-to-target size ratio. Attacker-side behavioral diversity impacts adversarial outcomes more than target susceptibility.


<details>
  <summary>Details</summary>
Motivation: To understand how vulnerabilities scale when LLMs interact adversarially and whether larger models can exploit smaller ones despite alignment safeguards.

Method: Simulated over 6,000 multi-turn adversarial interactions using standardized tasks from JailbreakBench. Evaluated harm and refusal scores via three independent LLM judges. Analyzed correlations between harm scores and model size ratios.

Result: Strong correlation between harm score and logarithm of size ratio (Pearson r = 0.51, Spearman rho = 0.52). Attacker-side diversity contributes more to outcomes than target susceptibility. Attacker refusal frequency negatively correlates with harm (rho = -0.93).

Conclusion: Size asymmetry influences adversarial robustness, suggesting the need for further controlled studies on inter-model alignment and safety.
...
### [160] [Fair-GNE : Generalized Nash Equilibrium-Seeking Fairness in Multiagent Healthcare Automation](https://arxiv.org/abs/2511.14135)
*Promise Ekpo,Saesha Agarwal,Felix Grimm,Lekan Molu,Angelique Taylor*

Main category: cs.LG

TL;DR: Fair-GNE introduces a game-theoretic framework for fair workload allocation among agents in healthcare settings, outperforming baselines with adaptive fairness enforcement.


<details>
  <summary>Details</summary>
Motivation: Existing MARL approaches lack certifiable fairness in workload allocation among agents, especially in collaborative healthcare settings with shared resources.

Method: Fair-GNE models MARL as a constrained generalized Nash equilibrium-seeking game to ensure equitable collective equilibrium.

Result: Fair-GNE improves workload balance significantly (0.89 vs. 0.33 JFI) while maintaining 86% task success, showing adaptive fairness enforcement works.

Conclusion: Fair-GNE offers a principled and effective solution for fairness in multi-agent healthcare systems, validated through rigorous simulation.
...
### [161] [Extended Physics Informed Neural Network for Hyperbolic Two-Phase Flow in Porous Media](https://arxiv.org/abs/2511.13734)
*Saif Ur Rehman,Wajid Yousuf*

Main category: cs.LG

TL;DR: The paper introduces the Extended Physics-Informed Neural Network (XPINN) framework to solve nonlinear hyperbolic PDEs, specifically the Buckley-Leverett equation, by dynamically decomposing the domain and using localized subnetworks to capture steep gradients and discontinuities.


<details>
  <summary>Details</summary>
Motivation: Standard PINNs struggle with capturing steep gradients and discontinuities in nonlinear hyperbolic PDEs. This study aims to address these limitations.

Method: The XPINN framework dynamically decomposes the computational domain into evolving regions, employs subnetworks for distinct behaviors, and enforces flux continuity using the Rankine-Hugoniot jump condition.

Result: XPINN accurately captures discontinuous fronts and compound wave interactions, achieving better stability, faster convergence, and enhanced resolution than standard PINNs.

Conclusion: XPINN is an effective and scalable tool for solving challenging hyperbolic PDEs in multiphase flow problems.
...
### [162] [Blurred Encoding for Trajectory Representation Learning](https://arxiv.org/abs/2511.13741)
*Silin Zhou,Yao Chen,Shuo Shang,Lisi Chen,Bingsheng He,Ryosuke Shibasaki*

Main category: cs.LG

TL;DR: BLUE is a novel trajectory representation learning method that balances fine-grained spatial-temporal details with high-level travel semantics by using hierarchical patches and a pyramid encoder-decoder structure.


<details>
  <summary>Details</summary>
Motivation: Existing TRL methods lose fine-grained spatial-temporal details when grouping GPS points into grid cells or road segments. BLUE addresses this limitation.

Method: BLUE gradually reduces GPS precision to create hierarchical patches, uses Transformers for embeddings at each level, and employs an encoder-decoder model with pooling and up-resolution mechanisms.

Result: BLUE outperforms 8 SOTA baselines by an average of 30.90% in accuracy across 3 downstream tasks.

Conclusion: BLUE effectively captures both fine-grained details and high-level patterns, proving superior to existing TRL methods.
...
### [163] [SCALEX: Scalable Concept and Latent Exploration for Diffusion Models](https://arxiv.org/abs/2511.13750)
*E. Zhixuan Zeng,Yuhao Chen,Alexander Wong*

Main category: cs.LG

TL;DR: SCALEX is a scalable, automated framework for exploring biases in diffusion models using natural language prompts without retraining or labeling.


<details>
  <summary>Details</summary>
Motivation: Existing methods for analyzing biases in diffusion models are limited by narrow focus or manual interpretation, hindering scalability and discovery of subtle patterns.

Method: SCALEX extracts meaningful directions from H-space using natural language prompts, enabling zero-shot interpretation and systematic comparison across concepts.

Result: SCALEX detects gender bias in profession prompts, ranks semantic alignment, and reveals clustered conceptual structure autonomously.

Conclusion: SCALEX advances bias analysis in diffusion models by making it more scalable, interpretable, and extensible compared to prior approaches.
...
### [164] [Robustness of LLM-enabled vehicle trajectory prediction under data security threats](https://arxiv.org/abs/2511.13753)
*Feilong Wang,Fuqiang Liu*

Main category: cs.LG

TL;DR: The study explores vulnerabilities in LLM-based vehicle trajectory prediction models, revealing that minor perturbations can disrupt outputs, highlighting a need for robustness in LLM-driven transportation systems.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored robustness of LLM-based prediction models in safety-critical driving systems amidst growing concerns about LLM trustworthiness.

Method: Systematic vulnerability analysis using a one-feature differential evolution attack to perturb kinematic features in LLM input prompts under black-box conditions.

Result: Minor, plausible perturbations significantly disrupt model outputs, showing susceptibility to adversarial manipulation and revealing an accuracy-robustness trade-off.

Conclusion: The study highlights adversarial vulnerabilities in LLM-driven vehicle models and emphasizes the necessity for robustness-focused design in future intelligent transportation systems.
...
### [165] [Adaptive Redundancy Regulation for Balanced Multimodal Information Refinement](https://arxiv.org/abs/2511.13755)
*Zhe Yang,Wenrui Li,Hongtao Chen,Penghong Wang,Ruiqin Xiong,Xiaopeng Fan*

Main category: cs.LG

TL;DR: RedReg introduces an adaptive redundancy regulation method to balance multimodal training by dynamically adjusting gradients based on cross-modal semantics and redundancy, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing modality bias and redundant information accumulation in joint multimodal training to improve performance.

Method: Proposes Adaptive Redundancy Regulation (RedReg) with a redundancy phase monitor and co-information gating mechanism to dynamically suppress gradients based on redundancy and cross-modal semantics.

Result: Superior performance in most scenarios compared to existing methods, validated by ablation experiments.

Conclusion: RedReg effectively balances multimodal optimization by adaptive gradient suppression, preserving modality-specific information when needed.
...
### [166] [Multi-Horizon Time Series Forecasting of non-parametric CDFs with Deep Lattice Networks](https://arxiv.org/abs/2511.13756)
*Niklas Erdmann,Lars Bentsen,Roy Stenbro,Heine Nygard Riise,Narada Dilp Warakagoda,Paal E. Engelstad*

Main category: cs.LG

TL;DR: The paper introduces a nonparametric approach for probabilistic forecasting using deep lattice networks (DLN) to model implicit CDFs, preventing quantile crossovers and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: To address limitations in point predictions and parametric CDF modeling by advancing probabilistic forecasting with monotonic networks.

Method: Adapts DLNs with LSTM embeddings for simultaneous/implicit quantile regression, ensuring monotonicity to prevent quantile crossovers.

Result: The adapted DLN outperforms unconstrained methods and scalable monotonic networks in solar irradiance forecasting.

Conclusion: The DLN adaptation effectively bridges probabilistic forecasting and monotonic networks, encouraging further research crossover.
...
### [167] [VitalBench: A Rigorous Multi-Center Benchmark for Long-Term Vital Sign Prediction in Intraoperative Care](https://arxiv.org/abs/2511.13757)
*Xiuding Cai,Xueyao Wang,Sen Wang,Yaoyao Zhu,Jiao Chen,Yu Yao*

Main category: cs.LG

TL;DR: VitalBench is introduced as a standardized benchmark for intraoperative vital sign prediction, addressing challenges like incomplete data and lack of cross-center validation. It includes data from 4,000+ surgeries and offers three evaluation tracks.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges in deep learning models for medical time-series forecasting, such as incomplete data and limited cross-center validation, VitalBench aims to provide a unified platform for robust model development.

Method: VitalBench incorporates data from two medical centers, offering three evaluation tracks (complete data, incomplete data, cross-center generalization) and uses masked loss techniques for unbiased evaluation.

Result: The framework minimizes preprocessing reliance and ensures consistency in data handling, enabling researchers to focus on architectural innovation.

Conclusion: VitalBench advances predictive models for intraoperative vital signs, ensuring accuracy, robustness, and adaptability across diverse clinical settings.
...
### [168] [ChemFixer: Correcting Invalid Molecules to Unlock Previously Unseen Chemical Space](https://arxiv.org/abs/2511.13758)
*Jun-Hyoung Park,Ho-Jun Song,Seong-Whan Lee*

Main category: cs.LG

TL;DR: ChemFixer corrects invalid molecules generated by deep learning models, preserving their properties and improving usability in drug discovery.


<details>
  <summary>Details</summary>
Motivation: Deep learning models often generate invalid molecules, limiting practical applications in drug discovery.

Method: ChemFixer uses a transformer architecture, pre-trained with masking and fine-tuned on valid/invalid molecular pairs.

Result: It improved molecular validity and preserved properties, expanding usable chemical space and aiding in tasks like DTI prediction.

Conclusion: ChemFixer is a practical tool for enhancing molecular validity and expanding chemical space in drug discovery.
...
### [169] [Multi-Agent VLMs Guided Self-Training with PNU Loss for Low-Resource Offensive Content Detection](https://arxiv.org/abs/2511.13759)
*Han Wang,Deyi Ji,Junyu Lu,Lanyun Zhu,Hailong Zhang,Haiyang Wu,Liqun Liu,Peng Shu,Roy Ka-Wei Lee*

Main category: cs.LG

TL;DR: The paper proposes a self-training framework using MA-VLMs and PNU loss to improve offensive content detection with scarce labeled data.


<details>
  <summary>Details</summary>
Motivation: High-quality labeled data for offensive content detection is scarce due to low prevalence and high annotation costs.

Method: A lightweight classifier iteratively assigns pseudo-labels with MA-VLMs, creating Agreed-Unknown and Disagreed-Unknown sets. MA-VLMs simulate moderator-user perspectives, and PNU loss optimizes the classifier.

Result: The framework outperforms baselines under limited supervision and nears large-scale model performance.

Conclusion: The self-training method effectively leverages unlabeled data and mitigates noise, enhancing detection accuracy.
...
### [170] [MoETTA: Test-Time Adaptation Under Mixed Distribution Shifts with MoE-LayerNorm](https://arxiv.org/abs/2511.13760)
*Xiao Fan,Jingyan Jiang,Zhaoru Chen,Fanding Huang,Xiao Chen,Qinting Jiang,Bowen Zhang,Xing Tang,Zhi Wang*

Main category: cs.LG

TL;DR: MoETTA introduces an entropy-based TTA framework using Mixture-of-Experts (MoE) to handle mixed distribution shifts by enabling diverse gradient directions, outperforming existing methods on new benchmarks.


<details>
  <summary>Details</summary>
Motivation: Real-world deployments often face mixed distribution shifts, which existing TTA methods struggle with due to reliance on unified adaptation paths.

Method: MoETTA integrates MoE architecture with structurally decoupled experts for flexible parameter updates and introduces new benchmarks (potpourri and potpourri+) for realistic testing.

Result: MoETTA consistently outperforms baselines across mixed distribution shifts, achieving SOTA performance.

Conclusion: Modeling multiple adaptation directions via expert-level diversity is effective for handling heterogeneous shifts.
...
### [171] [Gene Incremental Learning for Single-Cell Transcriptomics](https://arxiv.org/abs/2511.13762)
*Jiaxin Qi,Yan Cui,Jianqiang Huang,Gaogang Xie*

Main category: cs.LG

TL;DR: Study addresses the lack of incremental learning frameworks for tokens (specifically genes) by adapting class incremental learning methods to mitigate forgetting in gene incremental learning.


<details>
  <summary>Details</summary>
Motivation: Tokens, like genes, grow dynamically but lack incremental learning frameworks due to their holistic nature.

Method: Adapted class incremental learning methods for genes in single-cell transcriptomics, designing a pipeline and evaluations.

Result: Demonstrated framework soundness and method effectiveness through experiments, providing a benchmark.

Conclusion: Established a benchmark for gene incremental learning, showing success in mitigating forgetting.
...
### [172] [Library Liberation: Competitive Performance Matmul Through Compiler-composed Nanokernels](https://arxiv.org/abs/2511.13764)
*Arun Thangamani,Md Asghar Ahmad Shahid,Adam Siemieniuk,Rolf Morel,Renato Golin,Alexander Heinecke*

Main category: cs.LG

TL;DR: The paper introduces an MLIR-based compilation scheme to auto-generate scalable, high-performance microkernels, reducing reliance on low-level libraries and hardware expertise.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between high-level AI/ML operations and efficient hardware utilization, simplifying code generation for practitioners.

Method: The method uses MLIR dialects to compose nanokernels from low-level IR constructs with optimal register utilization, targeting CPU instructions.

Result: Results show production-quality nanokernels competitive with state-of-the-art libraries.

Conclusion: The approach effectively automates microkernel generation, reducing complexity and improving scalability.
...
### [173] [PROF: An LLM-based Reward Code Preference Optimization Framework for Offline Imitation Learning](https://arxiv.org/abs/2511.13765)
*Shengjie Sun,Jiafei Lyu,Runze Liu,Mengbei Yan,Bo Liu,Deheng Ye,Xiu Li*

Main category: cs.LG

TL;DR: PROF is a novel offline IL framework that uses LLMs to generate and refine reward functions from natural language and expert trajectories, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing offline IL methods oversimplify reward estimation by assuming trajectory-expert similarity correlates with reward, prompting the need for a more sophisticated approach.

Method: PROF leverages LLMs to generate reward functions and employs Reward Preference Ranking (RPR) to assess and refine them without environment interactions.

Result: Empirical results on D4RL show PROF matches or surpasses strong baselines across datasets and domains.

Conclusion: PROF's automated reward function generation and refinement significantly enhance offline IL performance.
...
### [174] [Credal Ensemble Distillation for Uncertainty Quantification](https://arxiv.org/abs/2511.13766)
*Kaizheng Wang,Fabio Cuzzolin,David Moens,Hans Hallez*

Main category: cs.LG

TL;DR: CED compresses deep ensembles into CREDIT for efficient uncertainty quantification in classification tasks.


<details>
  <summary>Details</summary>
Motivation: Deep ensembles are costly during inference, limiting their practicality despite their effectiveness in uncertainty quantification.

Method: Credal ensemble distillation (CED) compresses deep ensembles into CREDIT, which predicts class-wise probability intervals.

Result: CED performs comparably or better than baselines in uncertainty estimation while reducing inference overhead.

Conclusion: CED offers an efficient alternative to deep ensembles for uncertainty quantification in classification tasks.
...
### [175] [Dynamic Temperature Scheduler for Knowledge Distillation](https://arxiv.org/abs/2511.13767)
*Sibgat Ul Islam,Jawad Ibn Ahad,Fuad Rahman,Mohammad Ruhul Amin,Nabeel Mohammed,Shafin Rahman*

Main category: cs.LG

TL;DR: Dynamic Temperature Scheduler (DTS) adjusts temperature dynamically during Knowledge Distillation (KD), outperforming fixed-temperature methods.


<details>
  <summary>Details</summary>
Motivation: Traditional KD uses fixed temperature, which is suboptimal due to mismatched logit magnitudes and changing student needs.

Method: DTS adjusts temperature based on the cross-entropy loss gap between teacher and student.

Result: DTS consistently outperforms static-temperature baselines across vision and NLP tasks.

Conclusion: DTS is the first adaptive temperature scheduling method for KD, seamlessly integrating with existing frameworks.
...
### [176] [Compiling to linear neurons](https://arxiv.org/abs/2511.13769)
*Joey Velez-Ginorio,Nada Amin,Konrad Kording,Steve Zdancewic*

Main category: cs.LG

TL;DR: Cajal is a programming language designed to directly program neural networks by compiling discrete algorithms into differentiable forms compatible with gradient-based learning, improving learning efficiency and debuggability.


<details>
  <summary>Details</summary>
Motivation: Current indirect methods of programming neural networks lack discrete structure, limiting the integration of useful algorithms due to non-differentiability. Cajal aims to bridge this gap.

Method: Cajal, a typed, higher-order linear programming language, compiles programs into linear neurons. These neurons are linked with other networks to combine discrete algorithms with gradient-based learning.

Result: Experiments show that networks using Cajal-linked neurons learn faster, are more data-efficient, and easier to debug compared to traditional methods.

Conclusion: Linear programming languages like Cajal enable direct programming of neural networks, fostering synergy between learning and discrete algorithmic structures.
...
### [177] [Self-Attention as Distributional Projection: A Unified Interpretation of Transformer Architecture](https://arxiv.org/abs/2511.13780)
*Nihal Mehta*

Main category: cs.LG

TL;DR: The paper connects self-attention in Transformers to distributional semantics, showing it emerges from projecting co-occurrence statistics into context, with query-key-value mechanisms and positional encodings as natural refinements.


<details>
  <summary>Details</summary>
Motivation: To provide a mathematical foundation for self-attention in Transformers by linking it to distributional semantics and corpus-level co-occurrence statistics.

Method: Analyzes self-attention by projecting co-occurrence matrix statistics into sequence context, deriving query-key-value mechanisms and positional encodings as refinements.

Result: Demonstrates that Transformer architecture's algebraic form stems naturally from projection principles, not arbitrary design.

Conclusion: The paper establishes a principled mathematical basis for self-attention, showing its design aligns with distributional semantics principles.
...
### [178] [Exploring Transferability of Self-Supervised Learning by Task Conflict Calibration](https://arxiv.org/abs/2511.13787)
*Huijie Guo,Jingyao Wang,Peizheng Guo,Xingchen Shen,Changwen Zheng,Wenwen Qiang*

Main category: cs.LG

TL;DR: The paper investigates SSL representation transferability, proposes TC² to mitigate task conflict, and improves SSL model performance across tasks.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand and enhance the transferability of SSL representations by addressing task conflict and leveraging meta-learning insights.

Method: The authors use TC², which splits batches into SSL tasks, employs factor and weight extraction networks, and integrates optimization frameworks to enhance transferability.

Result: Experiments show TC² consistently boosts SSL model transferability on multiple downstream tasks.

Conclusion: TC² effectively addresses task conflict and improves SSL representation transferability, validated by empirical results.
...
TL;DR: A novel differentiable approach integrates feature importance estimation into model training via ScoresActivation, offering faster, faithful explanations aligned with SHAP and ground truth.


<details>
  <summary>Details</summary>
Motivation: Enhance model interpretability by integrating explainability into training, addressing the disconnect of post hoc methods.

Method: Uses ScoresActivation function within training to rank features by predictive contribution.

Result: Faster (150x) than SHAP, aligns with ground truth, improves accuracy (11.24%-29.33%), robust to irrelevant features.

Conclusion: Bridges accuracy and interpretability, scalable for inherently explainable ML.
...
TL;DR: DAS accelerates RL post-training rollouts by 50% using adaptive speculative decoding, balancing draft budgets for long trajectories without altering model outputs.


<details>
  <summary>Details</summary>
Motivation: The efficiency of RL post-training for LLMs is limited by the long-tail distribution of rollout lengths, where long generations dominate time. Historical rollouts reveal stable patterns, offering an opportunity to improve efficiency.

Method: DAS introduces an adaptive drafter using a suffix tree from recent rollouts and a length-aware speculation policy to allocate draft budgets aggressively for long trajectories.

Result: Experiments on math and code tasks show DAS reduces rollout time by up to 50% while maintaining identical training curves.

Conclusion: Distribution-aware speculative decoding (DAS) significantly accelerates RL post-training without compromising learning quality.
...
TL;DR: The paper introduces AnaCP, a method combining analytic classifiers with incremental feature adaptation for class-incremental learning, addressing limitations of pre-trained models and avoiding catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Traditional CIL methods struggle with catastrophic forgetting and suboptimal feature adaptation when using pre-trained models.

Method: AnaCP uses analytic contrastive projection to enable incremental feature adaptation without gradient-based training, preserving efficiency.

Result: AnaCP outperforms existing baselines and matches joint training accuracy, considered the upper bound for CIL.

Conclusion: AnaCP effectively balances feature adaptation and efficiency, achieving state-of-the-art performance in CIL.
...
TL;DR: Proposes using tractable probabilistic models (TPMs) like sum-product networks (SPNs) for long-term power utility investment planning, enabling robust scenario analysis and risk assessment.


<details>
  <summary>Details</summary>
Motivation: Traditional scenario-based methods limit insight into volatility and hinder robust decision-making in uncertain long-term energy forecasts.

Method: Utilizes TPMs, specifically SPNs, for scalable inference of scenario likelihoods, marginals, and conditional probabilities.

Result: Demonstrates computational and reliability advantages over traditional models in a power system planning case study.

Conclusion: TPMs offer a robust framework for embedding chance-constrained optimization into investment planning, improving reliability and decision-making.
...
TL;DR: MargNet combines strengths of statistical models and neural networks for differentially private tabular data synthesis, excelling in densely correlated datasets.


<details>
  <summary>Details</summary>
Motivation: Existing NN-based methods are underestimated for complex datasets; MargNet addresses limitations by integrating successful statistical model designs.

Method: MargNet uses adaptive marginal selection and trains neural networks to match selected marginals, improving synthesis accuracy and speed.

Result: MargNet matches utility of top statistical methods on sparse datasets (7× speedup) and reduces fidelity error by 26% on dense datasets.

Conclusion: MargNet outperforms existing methods, especially for densely correlated datasets, and combines the best of statistical and NN-based approaches.
...
TL;DR: A novel method uses weather maps as tokens in transformer sequences for renewable energy forecasting, reducing RMSE by 60% for wind and 20% for solar compared to current forecasts.


<details>
  <summary>Details</summary>
Motivation: Accurate renewable energy forecasting is crucial for reducing fossil fuel dependence and decarbonizing the grid. Current methods lack effective integration of spatial weather context with temporal dynamics.

Method: Hourly weather maps are encoded as spatial tokens using a lightweight CNN, then processed by a transformer to capture temporal dynamics over a 45-hour forecast horizon.

Result: The approach reduces RMSE by approximately 60% for wind and 20% for solar compared to ENTSO-E operational forecasts, despite input initialization disadvantages.

Conclusion: The introduced method significantly improves renewable energy forecasting accuracy, demonstrating potential for grid decarbonization.
...
TL;DR: The paper introduces CWCN, a novel GNN framework using complex-weighted edges to address oversmoothing and heterophilic graph challenges, proving high expressiveness and competitive performance.


<details>
  <summary>Details</summary>
Motivation: Existing GNNs face limitations like oversmoothing and poor performance on heterophilic graphs, prompting the need for a more expressive solution.

Method: Proposes CWCN, which assigns complex weights to edges for diffusion in the complex domain, and incorporates learnable matrices and nonlinear activations.

Result: CWCN achieves competitive performance on benchmarks without extra hyperparameters, demonstrating enhanced GNN expressiveness.

Conclusion: Complex-weighted diffusion offers a principled way to improve GNNs, balancing theoretical grounding and practical effectiveness.
...
TL;DR: Investigates how varying bootstrap rates (BR) affects Random Forest performance, showing optimal BR depends on dataset characteristics like global relationships and local variance.


<details>
  <summary>Details</summary>
Motivation: To systematically explore the impact of different bootstrap rates (BR) on Random Forest regression performance, as the default BR=1.0 may not always be optimal.

Method: Analyzed 39 regression datasets and 16 RF configurations with BR ranging from 0.2 to 5.0, using repeated two-fold cross-validation and mean squared error. Synthetic datasets with controlled noise levels were also tested.

Result: Tuning BR led to significant improvements: BR≤1.0 worked best for 24 datasets, BR>1.0 for 15, and BR=1.0 was optimal in only 4 cases. Dataset characteristics influenced preferred BR.

Conclusion: BR is a critical hyperparameter for RF regression; tuning it based on dataset properties (global relationships vs. local variance) can enhance model performance.
...
TL;DR: The paper analyzes the scalability of a Wasserstein-distance approach for training SNNs to reconstruct multidimensional random field models, proving generalization error bounds and addressing dimensionality challenges.


<details>
  <summary>Details</summary>
Motivation: The study aims to tackle the curse of dimensionality in learning multidimensional random field models and improve the robustness of SNN training.

Method: The authors use a Wasserstein-distance approach for training SNNs, proving generalization error bounds and validating results through numerical experiments.

Result: Results show that the convergence rate of generalization error may not explicitly depend on dimensionality, and the improved approach trains SNNs effectively for uncertainty quantification.

Conclusion: The Wasserstein-distance approach proves effective for training SNNs in multidimensional tasks, offering robustness and alleviating dimensionality challenges.
...
TL;DR: PCA Whitening improves SAE performance by simplifying the optimization landscape, enhancing interpretability metrics despite slight reconstruction trade-offs.


<details>
  <summary>Details</summary>
Motivation: To address the challenges in SAE training due to input data correlations and improve feature interpretability.

Method: Apply PCA Whitening to input activations and evaluate ReLU and Top-K SAEs across various architectures and sparsity levels.

Result: Whitening consistently boosts interpretability (e.g., probing accuracy, feature disentanglement) with minor reconstruction quality drops.

Conclusion: Whitening should be a default SAE preprocessing step when prioritizing interpretability over perfect reconstruction.
...
TL;DR: A framework detects SQL errors in LLM outputs by estimating uncertainty at the AST node level, improving accuracy and enabling targeted repairs.


<details>
  <summary>Details</summary>
Motivation: To enhance error detection in LLM-generated SQL by focusing on individual nodes in the AST, addressing limitations of aggregate confidence measures.

Method: Uses a two-stage approach: semantically aware labeling for node correctness and supervised classification with schema-aware features to predict node-level errors.

Result: Outperforms token log-probabilities with a 27.44% AUC improvement, showing robustness in cross-database evaluations.

Conclusion: Node-centric uncertainty estimation is effective, interpretable, and supports practical applications like targeted repair and human review.
...
TL;DR: The paper establishes lower bounds on the running time for differentially private optimization of Lipschitz convex losses, showing dimension-dependent penalties and limitations of gradient quantization.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the computational efficiency trade-offs in differentially private optimization, particularly focusing on the impact of dimensionality and privacy constraints.

Method: The authors analyze settings with non-smooth and smooth losses, using private proxy oracles and minibatches. They derive lower bounds on oracle queries and compare them with upper bounds from DP-SGD.

Result: Lower bounds reveal dimension-dependent runtime penalties for private optimizers. Results are tight for certain conditions (e.g., $d > \tildeΩ(1/α^4)$), and limitations of gradient quantization techniques are demonstrated.

Conclusion: Differentially private optimization incurs significant computational costs due to dimensionality, and gradient quantization has fundamental limitations in this context.
...
TL;DR: The paper introduces a novel method using probabilistic circuits to improve Bayesian structure learning by enabling fast and exact marginalization, outperforming current dynamic programming methods.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in inferring the graphical structure of Bayesian networks (BNs) from data, particularly due to the need for marginalizing over probability distributions, which current methods restrict.

Method: The authors propose a method leveraging tractable probabilistic circuits, avoiding restrictions imposed by dynamic programming. A new learning routine trains these circuits on the original distribution and marginal queries.

Result: Empirical results demonstrate that the method improves Bayesian structure learners' performance by enabling faster and more accurate marginalization.

Conclusion: Using probabilistic circuits offers a promising alternative to traditional dynamic programming methods for Bayesian structure learning, enhancing efficiency and accuracy.
...
TL;DR: The paper explores how adversarial inputs can manipulate probabilistic certification frameworks to falsely guarantee robustness, even with small, imperceptible perturbations.


<details>
  <summary>Details</summary>
Motivation: To understand the limits of certified defenses by demonstrating how adversaries can exploit certification processes to generate misleading robustness guarantees.

Method: Crafting region-focused adversarial examples to create imperceptible perturbations that spoof certificates and produce large robustness radii for incorrect classes.

Result: Successful bypassing of state-of-the-art certified defenses like Densepure on ImageNet, showing the vulnerability of current certification methods.

Conclusion: Highlights the need for deeper understanding and improvement of robustness certification frameworks to address their exploitable weaknesses.
...
TL;DR: Fine-tuning on insecure code can cause models to generate harmful responses beyond the original task, termed emergent misalignment (EMA). Refusal unlearning in narrow domains (Cybersecurity/Safety) can propagate EMA to unrelated domains, with Safety having a larger impact. Cross-entropy loss helps restore alignment.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand and mitigate emergent misalignment (EMA) triggered by fine-tuning or refusal unlearning in specific domains, highlighting risks of harmful cross-domain generalization.

Method: Refusal unlearning was performed on Cybersecurity and Safety concepts, evaluating EMA across seven RAI domains. Cross-entropy loss was applied to restore alignment.

Result: Safety unlearning caused higher EMA impact (e.g., lower refusal scores in Bias). Cross-entropy loss largely restored alignment while maintaining low refusal rates.

Conclusion: EMA arises from concept entanglement; interventions like cross-entropy loss can mitigate its effects while preserving domain-specific alignment.
...
TL;DR: Radial Compensation (RC) decouples curvature from model parameters in generative models on manifolds, improving stability and performance across various applications.


<details>
  <summary>Details</summary>
Motivation: Generative models on curved spaces face challenges with stiff Jacobians, distorted geodesic distances, and entangled curvature parameters, leading to poor test likelihoods and high gradient variance.

Method: RC selects a base density in the tangent space so the likelihood depends only on geodesic distance, decoupling curvature from parameters. It introduces the Balanced-Exponential (bExp) chart family for balancing volume distortion and geodesic error.

Result: RC stabilizes generative models, improves test likelihoods, and prevents radius blow-ups in high-dimensional flows. It works well across densities, VAEs, flows on images/graphs, and protein models.

Conclusion: RC-bExp is a robust default for likelihood-trained generative models on manifolds, offering curvature-invariant Fisher information and reduced gradient variance.
...
TL;DR: A machine learning-based multimodal framework integrates wearable sensors for motion recognition and stress estimation in archery, achieving high accuracy and offering real-time feedback potential.


<details>
  <summary>Details</summary>
Motivation: Traditional motion analysis systems are costly and intrusive, limiting their use in natural training environments for precision sports like archery.

Method: The framework uses a wrist-worn device with accelerometer and PPG sensors to collect motion and physiological data. It employs SmoothDiff features with an LSTM model for motion recognition (96.8% accuracy) and HRV features with an MLP classifier for stress estimation (80% accuracy).

Result: High accuracy was achieved: 96.8% for motion recognition and 80% for stress estimation, demonstrating effective integration of motion and physiological sensing.

Conclusion: The framework provides meaningful insights into athletes' technical and mental states, laying groundwork for real-time feedback systems in precision sports.
...
TL;DR: CafeMed is a novel medication recommendation framework that integrates dynamic causal reasoning and cross-modal attention to address limitations of static causal relationships and independent feature modeling in existing systems.


<details>
  <summary>Details</summary>
Motivation: Existing medication recommendation systems fail to model synergistic effects of medical entities and adapt static causal relationships to patient-specific contexts.

Method: CafeMed uses a Causal Weight Generator (CWG) for dynamic causal effects and a Channel Harmonized Attention Refinement Module (CHARM) to capture interdependencies between diagnoses and procedures.

Result: CafeMed outperforms baselines on MIMIC-III and MIMIC-IV datasets, achieving higher accuracy and lower drug-drug interaction rates.

Conclusion: Dynamic causal relationships and cross-modal synergies lead to more clinically-aligned and personalized medication recommendations.
...
TL;DR: CFG-EC improves Classifier-Free Guidance by correcting noise prediction errors, ensuring better prompt alignment and generation quality.


<details>
  <summary>Details</summary>
Motivation: The inconsistency between training and sampling noise estimates in CFG reduces generation fidelity, prompting the need for correction.

Method: CFG-EC refines unconditional noise predictions by making them orthogonal to conditional errors, reducing interference.

Result: CFG-EC outperforms CFG and CFG++, especially in low guidance sampling, with higher prompt alignment.

Conclusion: CFG-EC reliably enhances guidance trajectories for high-fidelity image generation.
...
TL;DR: Meta-SimGNN integrates graph neural networks with meta-learning to improve WiFi localization by addressing variations in device configurations and environments.


<details>
  <summary>Details</summary>
Motivation: Existing studies overlook device configuration changes (e.g., bandwidth, APs, antennas) impacting CSI dimensionality, compromising neural network usability.

Method: Proposes CSI graph construction with APs as nodes, amplitude-phase fusion for reliable CSI images, and similarity-guided meta-learning for rapid adaptation.

Result: Outperforms baselines in localization generalization and accuracy across diverse scenarios using commodity WiFi devices.

Conclusion: Meta-SimGNN effectively addresses device configuration challenges, enhancing practicality and robustness in deep learning-based localization.
...
TL;DR: The paper introduces a novel observational framework for DP auditing that avoids modifying training datasets by leveraging inherent data randomness, extending auditing to protected attributes like labels.


<details>
  <summary>Details</summary>
Motivation: Existing DP auditing methods are resource-intensive due to dataset modifications, limiting scalability for large systems.

Method: The proposed observational auditing framework evaluates privacy guarantees without altering datasets, using inherent randomness and extending to protected attributes.

Result: Experiments on Criteo and CIFAR-10 show the method effectively audits label privacy, offering scalability.

Conclusion: This work enables practical, large-scale privacy auditing without dataset modifications, addressing key gaps.
...
TL;DR: MoE-SpeQ uses speculative execution and expert offloading to hide I/O bottlenecks in Mixture-of-Experts models, achieving significant speedups on memory-constrained devices.


<details>
  <summary>Details</summary>
Motivation: Addressing the I/O bottleneck caused by offloading experts to host memory in MoE models, which cripples performance due to synchronous transfers on the critical path.

Method: Introduces MoE-SpeQ, leveraging speculative execution with a draft model to predict and prefetch experts, dynamically tuned by an Amortization Roofline Model.

Result: Achieves up to 2.34x speedup over state-of-the-art offloading frameworks for Phi-MoE models on memory-constrained devices.

Conclusion: MoE-SpeQ provides a principled approach to manage data-dependent memory access, making MoE inference more feasible on commodity hardware.
...
TL;DR: Soft-label training treats annotation distributions as ground truth, preserving epistemic uncertainty better than hard-label training in ambiguous tasks.


<details>
  <summary>Details</summary>
Motivation: To address the epistemic misalignment caused by collapsing diverse human judgments into single labels, which forces models to express false confidence on ambiguous data.

Method: Proposes soft-label training, where annotation distributions are treated as ground truth, and compares it empirically to hard-label training across vision and NLP tasks.

Result: Soft-label training achieves 32% lower KL divergence from human annotations, 61% stronger correlation between model and annotation entropy, while maintaining accuracy.

Conclusion: Annotation distributions should be seen as faithful representations of epistemic uncertainty rather than noisy signals to be aggregated away.
...
TL;DR: The paper proposes Synthetic Survival Control (SSC) to estimate counterfactual hazard trajectories in observational survival data, addressing challenges like censoring and non-random treatment.


<details>
  <summary>Details</summary>
Motivation: Answering 'when-if' questions about how interventions affect event timing is crucial in real-world settings with heterogeneous treatments and confounding.

Method: SSC estimates counterfactual hazard trajectories using weighted combinations of observed trajectories from other units in panel data, backed by a low-rank causal survival framework.

Result: SSC shows improved survival estimates in a cancer treatment dataset, with lower post-intervention hazard trajectories for novel therapies.

Conclusion: SSC provides a general, interpretable tool for counterfactual survival inference in observational data across medicine, economics, and public policy.
...
### [203] [A Comprehensive Study of Implicit and Explicit Biases in Large Language Models](https://arxiv.org/abs/2511.14153)
*Fatima Kazi,Alex Young,Yash Inani,Setareh Rafatirad*

Main category: cs.LG

TL;DR: The study identifies and mitigates biases in Large Language Models (LLMs) using benchmarks and proposes a Bias-Identification Framework, achieving up to 20% performance gains.


<details>
  <summary>Details</summary>
Motivation: LLMs inherit biases from training data, leading to unfair outputs and perpetuation of harmful stereotypes. Addressing these biases is crucial for fairness in AI.

Method: Used benchmarks like StereoSet and CrowSPairs to evaluate biases in models (BERT, GPT 3.5). Proposed an automated Bias-Identification Framework for detecting explicit and implicit biases. Employed Bag-of-Words analysis and fine-tuning with prompting and data augmentation.

Result: Fine-tuned models struggled with gender biases but excelled in racial bias detection. LLMs relied heavily on keywords. Performance improved by up to 20% after enhancement strategies.

Conclusion: The study highlights LLMs' limitations in bias detection and proposes effective strategies for improvement, achieving significant gains in performance.
...
### [204] [Certified Signed Graph Unlearning](https://arxiv.org/abs/2511.14168)
*Junpeng Zhao,Lin Li,Kaixi Hu,Kaize Shi,Jingling Yuan*

Main category: cs.LG

TL;DR: CSGU is a novel method for unlearning signed graph data in SGNNs, preserving privacy and sociological principles while outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing unlearning methods for GNNs fail to handle the unique properties of signed graphs, leading to loss of sign information and degraded performance in SGNNs.

Method: CSGU uses a three-stage approach: identifying minimal influenced neighborhoods, quantifying node importance for privacy budget allocation, and performing importance-weighted parameter updates.

Result: CSGU achieves better utility preservation and unlearning effectiveness compared to existing methods.

Conclusion: CSGU effectively addresses the challenges of signed graph unlearning, providing certified privacy guarantees while maintaining model utility.
...
### [205] [Bridging the Gap Between Bayesian Deep Learning and Ensemble Weather Forecasts](https://arxiv.org/abs/2511.14218)
*Xinlei Xiong,Wenbo Hu,Shuxun Zhou,Kaifeng Bi,Lingxi Xie,Ying Liu,Richang Hong,Qi Tian*

Main category: cs.LG

TL;DR: A hybrid Bayesian Deep Learning framework bridges traditional ensemble prediction and modern BDL for weather forecasting, improving accuracy, uncertainty quantification, and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional ensemble prediction is computationally intensive, while Bayesian Deep Learning offers an alternative. The paper aims to unify these approaches for better weather forecasting.

Method: A hybrid Bayesian Deep Learning framework decomposes uncertainty into epistemic and aleatoric components using variational inference and physics-informed stochastic perturbations.

Result: The method improves forecast accuracy, uncertainty calibration, and computational efficiency on the ERA5 dataset.

Conclusion: The proposed framework successfully connects BDL and EPS, offering superior performance and commits to open-sourcing the code.
...
### [206] [EBind: a practical approach to space binding](https://arxiv.org/abs/2511.14229)
*Jim Broadbent,Felix Cohen,Frederik Hvilshøj,Eric Landau,Eren Sasoglu*

Main category: cs.LG

TL;DR: EBind simplifies embedding space binding with a single encoder per modality and high-quality data, enabling efficient training on a single GPU. A 1.8B-parameter model outperforms larger models using curated multimodal data.


<details>
  <summary>Details</summary>
Motivation: To address the complexity and inefficiency of binding embedding spaces in contrastive models, EBind offers a parameter-efficient and data-centric approach.

Method: EBind uses a single encoder per modality and leverages three data sources: automated multimodal quintuples, human-annotated triples, and pre-existing captioned data.

Result: A 1.8B-parameter model achieves superior performance compared to models 4-17x larger, validated by 13 evaluations. A new audio-PC benchmark is introduced.

Conclusion: EBind's simplicity, data quality, and efficiency make it effective for embedding space binding, with plans to open-source resources.
...
### [207] [Object-Centric World Models for Causality-Aware Reinforcement Learning](https://arxiv.org/abs/2511.14262)
*Yosuke Nishimoto,Takashi Matsubara*

Main category: cs.LG

TL;DR: STICA is a framework using object-centric Transformers as a world model for efficient reinforcement learning. It decomposes environments into tokens for better dynamics prediction and causality-guided decisions.


<details>
  <summary>Details</summary>
Motivation: Humans decompose environments into discrete objects for efficient decision-making, inspiring STICA's object-centric approach to tackle high-dimensional, non-stationary environments with rich interactions.

Method: STICA employs object-centric Transformers to represent observations as tokens (including actions and rewards), predicting token-level dynamics. Causality-aware policy and value networks estimate token-level cause-effect relations for decision-making.

Result: Experiments show STICA outperforms state-of-the-art agents in sample efficiency and final performance on object-rich benchmarks.

Conclusion: STICA successfully integrates object-centric world modeling and causality-aware reinforcement learning for superior performance in complex environments.
...
### [208] [Algebraformer: A Neural Approach to Linear Systems](https://arxiv.org/abs/2511.14263)
*Pietro Sittoni,Francesco Tudisco*

Main category: cs.LG

TL;DR: Algebraformer is a Transformer-based model for solving ill-conditioned linear systems efficiently, reducing computational overhead compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing numerical methods for ill-conditioned linear systems often need tuning or expertise, motivating an end-to-end learned solution.

Method: Proposes Algebraformer, leveraging a novel encoding scheme for efficient representation of matrices and vectors, with O(n²) memory complexity.

Result: Demonstrates competitive accuracy on tasks like interpolation and Newton method acceleration, with lower computational overhead.

Conclusion: Shows that neural architectures can effectively reduce complexity in scientific computing pipelines.
...
### [209] [Unified Multimodal Vessel Trajectory Prediction with Explainable Navigation Intention](https://arxiv.org/abs/2511.14265)
*Rui Zhang,Chao Li,Kezhong Liu,Chen Wang,Bolong Zheng,Hongbo Jiang*

Main category: cs.LG

TL;DR: A unified vessel trajectory prediction framework incorporating explainable navigation intentions is proposed, improving scenario applicability and explainability compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Short-term prediction of vessel behavior changes in complex maritime environments is crucial but limited by scenario applicability and explainability in current MTP methods.

Method: The framework classifies navigation intentions into sustained and transient types, models them with intention trees and CVAE, and uses a non-local attention mechanism for global consistency.

Result: Tests on real AIS datasets show broad scenario applicability and significant improvements in ADE and FDE, along with enhanced explainability.

Conclusion: The proposed method advances vessel MTP by addressing key limitations and explicitly revealing navigational intentions.
...
### [210] [Comparing Task-Agnostic Embedding Models for Tabular Data](https://arxiv.org/abs/2511.14276)
*Frederik Hoppe,Lars Kleinemeier,Astrid Franz,Udo Göbel*

Main category: cs.LG

TL;DR: Simple TableVectorizer features outperform tabular foundation models in speed and performance for task-agnostic embeddings.


<details>
  <summary>Details</summary>
Motivation: To evaluate transferable, task-agnostic embeddings from tabular foundation models against classical feature engineering methods.

Method: Systematic comparison of TabPFN, TabICL, and TableVectorizer across tasks like outlier detection and supervised learning.

Result: TableVectorizer features match or exceed foundation models' performance and are significantly faster (up to 1000x).

Conclusion: Classical feature engineering remains competitive for task-agnostic tabular representations, offering efficiency advantages.
...
### [211] [Weight Variance Amplifier Improves Accuracy in High-Sparsity One-Shot Pruning](https://arxiv.org/abs/2511.14282)
*Vincent-Daniel Yun,Junhyuk Jo,Sunwoo Lee*

Main category: cs.LG

TL;DR: VAR, a Variance Amplifying Regularizer, enhances pruning robustness in neural networks by increasing parameter variance during training, outperforming existing methods like SAM and CrAM without extra computational costs.


<details>
  <summary>Details</summary>
Motivation: To address the significant accuracy drop in aggressively pruned neural networks, VAR leverages higher parameter variance for improved pruning robustness, avoiding additional computational burdens.

Method: Introduces VAR, a regularizer that increases parameter variance during training, enhancing pruning robustness by favoring flatter regions in the parameter space.

Result: Empirical results show VAR's superior pruning robustness, and theoretical analysis confirms its convergence behavior.

Conclusion: VAR effectively mitigates pruning-induced accuracy drops, offering a computationally efficient alternative to existing pruning-robust optimizers.
...
### [212] [H-LDM: Hierarchical Latent Diffusion Models for Controllable and Interpretable PCG Synthesis from Clinical Metadata](https://arxiv.org/abs/2511.14312)
*Chenyang Xu,Siming Li,Hao Wang*

Main category: cs.LG

TL;DR: H-LDM introduces a hierarchical latent diffusion model for generating accurate PCG signals, addressing data scarcity in cardiovascular diagnostics with clinically valid synthetic data.


<details>
  <summary>Details</summary>
Motivation: The scarcity of labeled pathological PCG data limits AI systems' diagnostic capabilities, prompting the need for a solution.

Method: H-LDM uses a multi-scale VAE for disentangled latent space learning, a hierarchical text-to-biosignal pipeline, and an interpretable diffusion process guided by Medical Attention.

Result: Achieved Fréchet Audio Distance of 9.7, 92% attribute disentanglement, 87.1% clinical validity, and improved rare disease classification by 11.3%.

Conclusion: H-LDM bridges data scarcity in cardiac diagnostics with interpretable synthetic data, enhancing AI diagnostic accuracy.
...
### [213] [Intervention Efficiency and Perturbation Validation Framework: Capacity-Aware and Robust Clinical Model Selection under the Rashomon Effect](https://arxiv.org/abs/2511.14317)
*Yuwen Zhang,Viet Tran,Paul Weng*

Main category: cs.LG

TL;DR: The paper addresses challenges in clinical machine learning due to the Rashomon Effect, where multiple models perform similarly. It proposes two tools—Intervention Efficiency (IE) and Perturbation Validation Framework (PVF)—for robust model selection.


<details>
  <summary>Details</summary>
Motivation: The Rashomon Effect in clinical ML leads to uncertainty in model selection. Small, imbalanced datasets and noisy features worsen this, making traditional validation unreliable.

Method: The paper introduces IE, a metric linking predictive performance to clinical utility, and PVF, assessing model stability under data perturbations.

Result: Tests on synthetic and real-world datasets show IE and PVF help select models that generalize better and align with capacity constraints.

Conclusion: The proposed tools offer a robust solution for model assessment and selection in clinical settings, addressing the Rashomon Effect.
...
### [214] [Learning with Statistical Equality Constraints](https://arxiv.org/abs/2511.14320)
*Aneesh Barthakur,Luiz F. O. Chamon*

Main category: cs.LG

TL;DR: The paper addresses the challenges of handling multiple requirements in machine learning, especially equality constraints, by proposing a generalization theory and a practical algorithm for equality-constrained statistical learning.


<details>
  <summary>Details</summary>
Motivation: Machine learning applications face growing complexity beyond accuracy, requiring effective methods to handle multiple constraints, particularly equality constraints like fairness and boundary value problems. Existing methods rely on hyperparameter tuning or constrained optimization but lack guarantees for equality constraints.

Method: The authors derive a generalization theory for equality-constrained statistical learning and propose an algorithm that solves a sequence of unconstrained empirical learning problems.

Result: The approach effectively addresses problems involving equality constraints, demonstrated in fair learning, interpolating classifiers, and boundary value problems.

Conclusion: The work provides a theoretical foundation and practical solution for equality-constrained learning, enabling new formulations and improved handling of complex requirements.
...
### [215] [Enforcing hidden physics in physics-informed neural networks](https://arxiv.org/abs/2511.14348)
*Nanxi Chen,Sifan Wang,Rujin Ma,Airong Chen,Chuanjie Cui*

Main category: cs.LG

TL;DR: The paper introduces an irreversibility-regularized strategy for PINNs to address the neglect of the Second Law of Thermodynamics, improving solution accuracy with minimal framework modifications.


<details>
  <summary>Details</summary>
Motivation: Conventional PINNs often neglect the hidden irreversibility from the Second Law of Thermodynamics, leading to unphysical solutions or training failures.

Method: A simple, generalized irreversibility-regularized strategy enforces physical laws as soft constraints during PINN training.

Result: The regularization scheme reduces predictive errors significantly (>10x) across various benchmarks, maintaining minimal changes to existing PINN frameworks.

Conclusion: The proposed framework is broadly applicable to PDE-governed systems and holds significant potential for the scientific machine learning community.
...
### [216] [Watch Out for the Lifespan: Evaluating Backdoor Attacks Against Federated Model Adaptation](https://arxiv.org/abs/2511.14406)
*Bastien Vuillod,Pierre-Alain Moellic,Jean-Max Dutertre*

Main category: cs.LG

TL;DR: The paper analyzes how LoRA affects backdoor attacks in Federated Learning, finding that lower LoRA ranks extend backdoor persistence, and highlights evaluation issues for FL security.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of LoRA on backdoor attacks in FL and improve the robustness and fairness of security evaluations.

Method: Analyzes the influence of LoRA on state-of-the-art backdoor attacks, focusing on backdoor lifespan and persistence under different LoRA ranks.

Result: Lower LoRA ranks lead to longer backdoor persistence post-attack, revealing evaluation flaws in current FL security assessments.

Conclusion: The study improves understanding of FL vulnerabilities and advances fairer evaluation methods for backdoor attacks.
...
### [217] [Toward Robust and Harmonious Adaptation for Cross-modal Retrieval](https://arxiv.org/abs/2511.14416)
*Haobin Li,Mouxing Yang,Xi Peng*

Main category: cs.LG

TL;DR: The paper addresses the query shift problem in Cross-Modal Retrieval (CMR), introducing REST, a method for online and harmonious adaptation.


<details>
  <summary>Details</summary>
Motivation: Existing CMR methods assume full target-domain data availability, but real-world scenarios face online and diverse query shifts, undermining model performance.

Method: REST refines retrieval results for online shift robustness and uses gradient decoupling to prevent forgetting general knowledge.

Result: Experiments on 20 benchmarks across three CMR tasks show REST's effectiveness against query shift.

Conclusion: REST successfully tackles the challenges of online and diverse query shifts in CMR, preserving model performance.
...
### [218] [FlowRoI A Fast Optical Flow Driven Region of Interest Extraction Framework for High-Throughput Image Compression in Immune Cell Migration Analysis](https://arxiv.org/abs/2511.14419)
*Xiaowei Xu,Justin Sonneck,Hongxiao Wang,Roman Burkard,Hendrik Wohrle,Anton Grabmasier,Matthias Gunzer,Jianxu Chen*

Main category: cs.LG

TL;DR: FlowRoI is an efficient optical-flow-based framework for compressing high-throughput imaging data in immune cell migration studies, offering faster processing and better compression rates than standard JPEG2000.


<details>
  <summary>Details</summary>
Motivation: Handling the exponential data growth from high-throughput imaging platforms like ComplexEye necessitates efficient compression methods to alleviate storage and transmission burdens.

Method: FlowRoI uses optical flow to identify migrating cell regions (RoIs) and jointly encodes raw images and RoI masks with JPEG2000 for RoI-aware compression.

Result: FlowRoI processes ~30 fps on a laptop, outperforms JPEG2000 in PSNR for cellular regions, and achieves 2.0-2.2x higher compression rates at matched PSNR.

Conclusion: FlowRoI provides a computationally efficient and effective solution for compressing high-throughput immune cell migration data, enhancing storage and transmission efficiency.
...
### [219] [MiAD: Mirage Atom Diffusion for De Novo Crystal Generation](https://arxiv.org/abs/2511.14426)
*Andrey Okhotin,Maksim Nakhodnov,Nikita Kazeev,Andrey E Ustyuzhanin,Dmitry Vetrov*

Main category: cs.LG

TL;DR: Mirage Atom Diffusion (MiAD) enhances diffusion models by allowing atom count changes during crystal generation, achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models lack the ability to alter atom counts during crystal generation, limiting variability.

Method: Introduces 'mirage infusion,' enabling transitions between existent and non-existent atom states.

Result: MiAD improves model quality by up to 2.5x and achieves an 8.2% S.U.N. rate on the MP-20 dataset.

Conclusion: MiAD outperforms state-of-the-art approaches in de novo crystal generation.
...
### [220] [Hybrid Modeling of Photoplethysmography for Non-invasive Monitoring of Cardiovascular Parameters](https://arxiv.org/abs/2511.14452)
...
TL;DR: A hybrid model using hemodynamic simulations and unlabeled clinical data estimates cardiovascular biomarkers from PPG signals, outperforming supervised baselines in monitoring cardiac output and stroke volume changes.


<details>
  <summary>Details</summary>
Motivation: Non-invasive prediction of cardiac biomarkers like stroke volume and cardiac output from PPG signals is challenging due to invasive measurement requirements and scarce annotated data.

Method: The approach combines a conditional variational autoencoder trained on paired PPG-APW data with a conditional density estimator trained on labeled simulated APW segments.

Result: The hybrid model successfully detects fluctuations in cardiac output and stroke volume and outperforms supervised baselines in monitoring temporal changes.

Conclusion: The proposed hybrid method offers a promising non-invasive solution for continuous cardiovascular monitoring using PPG signals.
...
### [221] [Nonparametric estimation of conditional probability distributions using a generative approach based on conditional push-forward neural networks](https://arxiv.org/abs/2511.14455)
*Nicola Rares Franco,Lorenzo Tedesco*

Main category: cs.LG

TL;DR: CPFN introduces a generative framework for conditional distribution estimation using stochastic maps, enabling efficient sampling and Monte Carlo-based statistics.


<details>
  <summary>Details</summary>
Motivation: To model conditional distributions without directly estimating densities, avoiding complexity like invertibility or adversarial training.

Method: Learns a stochastic map $φ(x,u)$ aligning $φ(x,U)$ with $Y|X=x$ via KL-based training.

Result: Competes with or outperforms state-of-the-art methods (kernel estimators, trees, deep learning) while being lightweight.

Conclusion: CPFN offers an efficient, consistent, and easy-to-train alternative for conditional distribution estimation.
...
### [222] [nnterp: A Standardized Interface for Mechanistic Interpretability of Transformers](https://arxiv.org/abs/2511.14465)
*Clément Dumas*

Main category: cs.LG

TL;DR: nnterp is a lightweight wrapper around NNsight that standardizes transformer analysis tools while preserving exact HuggingFace behavior, enabling compatibility across diverse architectures.


<details>
  <summary>Details</summary>
Motivation: Current tools for mechanistic interpretability research face a tradeoff between standardization (TransformerLens) and exact behavior preservation (NNsight/HuggingFace). nnterp aims to bridge this gap.

Method: nnterp provides a unified interface by wrapping NNsight, automating module renaming, and including validation tests. It supports common interpretability methods and direct attention probability access.

Result: nnterp allows researchers to write intervention code once and deploy it across 50+ model variants spanning 16 architecture families, ensuring correctness and usability.

Conclusion: nnterp successfully bridges the gap between correctness and usability in mechanistic interpretability tooling, offering standardized yet exact analysis tools.
...
### [223] [Notes on Kernel Methods in Machine Learning](https://arxiv.org/abs/2511.14485)
...
TL;DR: The paper offers a comprehensive introduction to kernel methods in machine learning, covering Hilbert spaces, RKHS, and their applications in statistical estimation and probability representation.


<details>
  <summary>Details</summary>
Motivation: To provide a foundational understanding of kernel methods and their geometric underpinnings for advanced topics in machine learning.

Method: Develops theory from Hilbert spaces to RKHS and Hilbert-Schmidt operators, revisiting classical concepts via Hilbert space geometry. Introduces kernel density estimation, MMD, and kernel embeddings.

Result: A structured exposition linking classical statistical concepts to kernel methods, serving as groundwork for advanced areas like Gaussian processes and kernel Bayesian inference.

Conclusion: The paper establishes a solid theoretical foundation for kernel methods, bridging classical statistics and modern machine learning techniques.
...
### [224] [Towards Stable and Structured Time Series Generation with Perturbation-Aware Flow Matching](https://arxiv.org/abs/2511.14488)
*Jintao Zhang,Mingyue Cheng,Zirui Liu,Xianquan Wang,Yitong Zhou,Qi Liu*

Main category: cs.LG

TL;DR: PAFM introduces a perturbation-aware flow matching framework for stable and structurally consistent time series generation, outperforming baselines by modeling localized disturbances and trajectory deviations.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of generating structurally consistent time series due to temporal heterogeneity caused by localized perturbations.

Method: Incorporates perturbation-guided training, a dual-path velocity field, and a mixture-of-experts decoder with flow routing to refine modeling of perturbed behavior.

Result: PAFM consistently outperforms strong baselines in unconditional and conditional generation tasks.

Conclusion: PAFM effectively enhances the structural coherence of generated time series by capturing abrupt transitions and localized disturbances.
...
### [225] [CLO: Efficient LLM Inference System with CPU-Light KVCache Offloading via Algorithm-System Co-Design](https://arxiv.org/abs/2511.14510)
*Jiawei Yi,Ping Gong,Youhui Bai,Jiaqi Ruan,Shengnan Wang,Pengcheng Wang,Haibo Wang,Weiguang Wang,Xia Zhu,Feng Wu,Cheng Li*

Main category: cs.LG

TL;DR: CLO addresses CPU bottlenecks in LLM inference by introducing a CPU-light KVCache offloading system with algorithm-system co-design, improving throughput and reducing overhead.


<details>
  <summary>Details</summary>
Motivation: The scalability limits of inference systems in million-token LLMs, particularly due to KVCache dominating memory usage and data transfer overhead, motivate this work. Existing solutions overlook CPU bottlenecks, leading to inefficiencies.

Method: CLO employs (1) coarse-grained head-wise approximate on-GPU caching, (2) data prefetching with on-GPU persistent caching, (3) a zero-copy transfer engine, and GPU-centric synchronization to minimize CPU overhead and maximize PCIe bandwidth.

Result: Experimental results show CLO achieves comparable accuracy to state-of-the-art systems while improving decoding throughput by 9.3%-66.6%.

Conclusion: Algorithm-system co-design is crucial for efficient memory-constrained LLM inference, as demonstrated by CLO's performance and open-source availability.
...
### [226] [Full Atom Peptide Design via Riemannian Euclidean Bayesian Flow Networks](https://arxiv.org/abs/2511.14516)
*Hao Qian,Shikui Tu,Lei Xu*

Main category: cs.LG

TL;DR: PepBFN, a Bayesian flow network for peptide design, addresses challenges in current models by modeling continuous parameter distributions and multimodal side-chain states, improving performance in computational peptide design.


<details>
  <summary>Details</summary>
Motivation: Current diffusion and flow matching models struggle with categorical sampling of residue types and unimodal assumptions for side-chain torsion angles, leading to suboptimal performance.

Method: PepBFN introduces continuous parameter distributions for discrete residue types, Gaussian mixture flows for multimodal side-chain states, and Riemannian flows for residue orientations on SO(3), refining these via Bayesian updates.

Result: Experiments show PepBFN's effectiveness in side chain packing, reverse folding, and binder design tasks.

Conclusion: PepBFN offers a promising approach for peptide design by addressing key limitations of existing models.
...
### [227] [MissHDD: Hybrid Deterministic Diffusion for Hetrogeneous Incomplete Data Imputation](https://arxiv.org/abs/2511.14543)
*Youran Zhou,Mohamed Reda Bouadjenek,Sunil Aryal*

Main category: cs.LG

TL;DR: The paper addresses challenges in imputing incomplete heterogeneous tabular data using diffusion models by proposing a hybrid deterministic diffusion framework.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based imputation models struggle with mixed-type data due to assumptions of homogeneity, leading to issues like conditional inconsistency and information collapse.

Method: The framework splits features into two channels: a continuous DDIM-based channel for numerical variables and a discrete latent-path diffusion channel for categorical/discrete features, trained under a unified objective.

Result: Experiments show superior imputation accuracy, stability, and robustness across various missing data mechanisms compared to existing methods.

Conclusion: The study highlights the need for structure-aware diffusion processes to effectively handle incomplete tabular data.
...
### [228] [Mind the Gaps: Measuring Visual Artifacts in Dimensionality Reduction](https://arxiv.org/abs/2511.14544)
*Jaume Ros,Alessio Arleo,Fernando Paulovich*

Main category: cs.LG

TL;DR: The paper introduces the Warping Index (WI), a new metric to assess the quality of dimensionality reduction (DR) projections by focusing on preserving empty regions between points to avoid visual distortions.


<details>
  <summary>Details</summary>
Motivation: DR techniques often introduce distortions in projections, which can mislead users. Existing Projection Quality Metrics (PQMs) neglect visual distortions, outliers, or artifacts.

Method: The authors propose the Warping Index (WI), which measures projection quality by emphasizing the correct preservation of empty regions between points.

Result: WI provides a better assessment of DR projection quality by addressing visual distortions that other PQMs overlook.

Conclusion: The Warping Index offers a more faithful evaluation of DR projections, aiding accurate visual analysis by accounting for empty regions and reducing misleading artifacts.
...
### [229] [Task Addition and Weight Disentanglement in Closed-Vocabulary Models](https://arxiv.org/abs/2511.14569)
*Adam Hazimeh,Alessandro Favero,Pascal Frossard*

Main category: cs.LG

TL;DR: Task arithmetic, previously applied to open-vocabulary models, is now explored for closed-vocabulary image classification models, showing weight disentanglement enables effective multi-task editing.


<details>
  <summary>Details</summary>
Motivation: Extend task arithmetic's applicability to closed-vocabulary models, which lack language supervision, to enhance multi-task efficiency.

Method: Study task addition in closed-vocabulary models, focusing on pre-training schemes and weight disentanglement.

Result: Pre-trained closed-vocabulary models, including vision transformers, achieve high task addition performance; linear probing is a competitive baseline.

Conclusion: Task arithmetic is viable for broader model classes, improving multi-task deployment efficiency.
...
### [230] [ReflexGrad: Three-Way Synergistic Architecture for Zero-Shot Generalization in LLM Agents](https://arxiv.org/abs/2511.14584)
*Ankush Kadu,Ashwanth Krishnan*

Main category: cs.LG

TL;DR: ReflexGrad integrates hierarchical task decomposition, history-aware reflection, and gradient-based optimization for zero-shot generalization in reinforcement learning, achieving 67% success on first exposure.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of enabling agents to learn and generalize across tasks without task-specific training, leveraging complementary mechanisms.

Method: ReflexGrad combines LLM-based hierarchical task decomposition, history-aware causal reflection, and gradient-based optimization for systematic improvement.

Result: Achieves a 67% zero-shot success rate on Trial 0 in ALFWorld tasks, demonstrating robust generalization without prior task experience.

Conclusion: Synergistic integration of complementary mechanisms enables effective zero-shot generalization, approaching few-shot baselines.
...
### [231] [Expert-Guided POMDP Learning for Data-Efficient Modeling in Healthcare](https://arxiv.org/abs/2511.14619)
*Marco Locatelli,Arjen Hommersom,Roberto Clemens Cerioli,Daniela Besozzi,Fabio Stella*

Main category: cs.LG

TL;DR: The paper introduces Fuzzy MAP EM, a novel algorithm combining expert knowledge with EM to improve POMDP parameter estimation in data-limited settings.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of learning POMDP parameters from limited data by leveraging expert knowledge.

Method: Integrates fuzzy pseudo-counts from expert-defined models into EM, reformulating it as MAP estimation.

Result: Outperforms standard EM in synthetic medical simulations under low-data and high-noise conditions; validated in a Myasthenia Gravis case study.

Conclusion: Fuzzy MAP EM is effective for data-efficient POMDP modeling in healthcare.
...
### [232] [Failure to Mix: Large language models struggle to answer according to desired probability distributions](https://arxiv.org/abs/2511.14630)
*Ivy Yuqian Yang,David Yu Zhang*

Main category: cs.LG

TL;DR: LLMs struggle to follow probabilistic distributions in outputs, favoring the highest probability option almost exclusively.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs handle probabilistic exploration, contrasting with deterministic benchmarks.

Method: Systematic experiments testing LLMs' ability to produce outputs matching simple probabilistic distributions.

Result: Modern LLMs consistently fail to follow specified distributions, favoring the most probable output overwhelmingly.

Conclusion: LLMs exhibit step-function behavior in probabilistic tasks, prioritizing the highest probability choice despite instructions.
...
### [233] [Adapformer: Adaptive Channel Management for Multivariate Time Series Forecasting](https://arxiv.org/abs/2511.14632)
*Yuchen Luo,Xinyu Li,Liuhua Peng,Mingming Gong*

Main category: cs.LG

TL;DR: Introducing Adapformer, a Transformer-based MTSF model combining CI and CD strategies for better accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing MTSF models struggle with balancing channel interactions; CI misses dependencies, CD risks overfitting.

Method: Adapformer uses ACE (Adaptive Channel Enhancer) and ACF (Adaptive Channel Forecaster) stages for optimized forecasting.

Result: Adapformer outperforms existing models in predictive accuracy and computational efficiency.

Conclusion: Adapformer sets a new benchmark in MTSF by effectively managing channel dependencies.
...
### [234] [Collaborative QA using Interacting LLMs. Impact of Network Structure, Node Capability and Distributed Data](https://arxiv.org/abs/2511.14098)
*Adit Jain,Vikram Krishnamurthy,Yiming Zhang*

Main category: cs.AI

TL;DR: The paper models how interacting LLMs collaboratively answer questions, addressing hallucination spread in networks by combining mean-field dynamics and randomized utility models. It analyzes fixed points and experiments with 100 LLMs.


<details>
  <summary>Details</summary>
Motivation: To understand and mitigate hallucination spread in networks of LLMs when answering questions collaboratively, especially when direct evidence is lacking.

Method: Combines mean-field dynamics from network science and randomized utility models from economics to create a generative model. Uses latent states (truthful/hallucinating) and analyzes fixed points under incentives.

Result: Proposes conditions for fixed-point existence/uniqueness and studies behavior under incentives. Experiments with 100 LLMs explore data heterogeneity, node capability, and network structure.

Conclusion: The framework effectively models hallucination dynamics in LLM networks, with insights into controlling spread via incentives and network design.
...
TL;DR: The paper highlights the limitations of advanced VLMs in spatial reasoning, introduces SpatiaLite benchmark, and proposes an Imagery Driven Framework (IDF) to improve spatial reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Despite advanced reasoning capabilities in VLMs, spatial reasoning remains a challenge. The study aims to understand and improve VLMs' spatial reasoning mechanisms.

Method: Introduces SpatiaLite, a synthetic benchmark to measure spatial reasoning accuracy and efficiency, and proposes IDF for data synthesis and training.

Result: Key findings: VLMs rely on linguistic representations, show inefficiency in spatial reasoning, and IDF can enhance spatial reasoning by building internal world models.

Conclusion: The study identifies VLMs' spatial reasoning limits, proposes solutions, and sets directions for future advancements.
...
### [236] [DataSage: Multi-agent Collaboration for Insight Discovery with External Knowledge Retrieval, Multi-role Debating, and Multi-path Reasoning](https://arxiv.org/abs/2511.14299)
*Xiaochuan Liu,Yuanfeng Song,Xiaoming Yin,Xing Chen*

Main category: cs.AI

TL;DR: DataSage is a multi-agent framework designed to improve automated data insight discovery by addressing limitations like insufficient domain knowledge, shallow analysis, and error-prone code generation.


<details>
  <summary>Details</summary>
Motivation: Organizations need actionable insights for decision-making, but current LLM-driven agents lack depth and accuracy.

Method: DataSage uses external knowledge retrieval, multi-role debating, and multi-path reasoning to enhance analysis.

Result: Experiments show DataSage outperforms existing agents across all difficulty levels.

Conclusion: DataSage provides an effective solution for automated insight discovery.
...
### [237] [KANGURA: Kolmogorov-Arnold Network-Based Geometry-Aware Learning with Unified Representation Attention for 3D Modeling of Complex Structures](https://arxiv.org/abs/2511.13798)
*Mohammad Reza Shafie,Morteza Hajiabadi,Hamed Khosravi,Mobina Noori,Imtiaz Ahmed*

Main category: cs.AI

TL;DR: KANGURA, a geometry-aware 3D machine learning model, outperforms SOTA models in optimizing microbial fuel cell anode structures, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing models fail to capture complex geometric dependencies in microbial fuel cell anode structures, limiting performance optimization.

Method: KANGURA uses Kolmogorov-Arnold Network-based geometry-aware learning with unified representation attention to decompose prediction tasks and refine spatial understanding.

Result: KANGURA achieves 92.7% accuracy on ModelNet40 and 97% accuracy in real-world MFC anode structure optimization.

Conclusion: KANGURA is a powerful framework for 3D geometric modeling, enabling advanced optimization in manufacturing and engineering.
...
### [238] [Causal computations in Semi Markovian Structural Causal Models using divide and conquer](https://arxiv.org/abs/2511.13852)
*Anna Rodum Bjøru,Rafael Cabañas,Helge Langseth,Antonio Salmerón*

Main category: cs.AI

TL;DR: Extending Bjøru et al.'s divide-and-conquer algorithm to semi-Markovian SCMs to handle confounding, proposing new strategies tested theoretically and computationally.


<details>
  <summary>Details</summary>
Motivation: Current methods assume Markovian SCMs (exogenous variables affect only one endogenous variable), limiting their ability to model confounding. Extending to semi-Markovian SCMs addresses this gap.

Method: Develop alternative strategies for decomposing semi-Markovian SCMs, illustrated via a minimal example, and evaluate them theoretically and computationally.

Result: Proposed strategies enable bounding counterfactual probabilities in semi-Markovian SCMs, overcoming limitations of Markovian models.

Conclusion: The extension to semi-Markovian SCMs broadens applicability and improves handling of confounding, validated through theoretical and computational analysis.
...
### [239] [Jailbreaking Large Vision Language Models in Intelligent Transportation Systems](https://arxiv.org/abs/2511.13892)
*Badhan Chandra Das,Md Tasnim Jawad,Md Jueal Mia,M. Hadi Amini,Yanzhao Wu*

Main category: cs.AI

TL;DR: This paper analyzes the vulnerabilities of Large Vision Language Models (LVLMs) in Intelligent Transportation Systems (ITS) to jailbreaking attacks, introduces a novel attack method using image typography and multi-turn prompting, and proposes a defense technique.


<details>
  <summary>Details</summary>
Motivation: LVLMs are widely used but susceptible to jailbreaking attacks, particularly in critical systems like ITS, raising concerns about their security.

Method: Constructed a harmful query dataset, developed an attack exploiting typography and multi-turn prompts, and proposed a multi-layered defense.

Result: Demonstrated severe security risks in LVLMs under attack and validated defense effectiveness via GPT-4 and manual checks.

Conclusion: The study highlights security flaws in LVLMs and suggests urgent improvements to mitigate jailbreaking risks in ITS.
...
### [240] [CORGI: Efficient Pattern Matching With Quadratic Guarantees](https://arxiv.org/abs/2511.13942)
*Daniel Weitekamp*

Main category: cs.AI

TL;DR: CORGI is a new algorithm for rule-based systems that ensures quadratic time/space guarantees for single matches and avoids memory issues by streaming matches iteratively, outperforming traditional RETE approaches.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiencies and memory overflows in rule-based systems caused by exponential time/space requirements in worst-case matching scenarios, especially in AI and database applications.

Method: CORGI eliminates traditional β-memory by building/maintaining a grounded relation graph forward and generating matches iteratively backward, avoiding full conflict sets.

Result: CORGI outperforms RETE implementations (SOAR, OPS5) in combinatorial matching tasks, offering better efficiency and scalability.

Conclusion: CORGI provides a practical solution for real-time rule-based systems by ensuring predictable performance and avoiding memory bottlenecks.
...
### [241] [Scene Graph-Guided Generative AI Framework for Synthesizing and Evaluating Industrial Hazard Scenarios](https://arxiv.org/abs/2511.13970)
*Sanjay Acharjee,Abir Khan Ratul,Diego Patino,Md Nazmus Sakib*

Main category: cs.AI

TL;DR: This paper introduces a generative AI framework that synthesizes realistic images of workplace hazards using scene graphs derived from OSHA reports, validated by a novel VQA Graph Score.


<details>
  <summary>Details</summary>
Motivation: Acquiring realistic images of hazardous workplace conditions is challenging, so the study aims to generate synthetic yet accurate hazard scenes.

Method: OSHA reports are analyzed by GPT-4o to create scene graphs, which guide a text-to-image diffusion model. A VQA framework evaluates realism.

Result: The proposed VQA Graph Score outperforms CLIP and BLIP metrics in validating the synthetic images' realism and semantic accuracy.

Conclusion: The framework successfully generates photorealistic hazard images, validated by a superior evaluation method.
...
### [242] [Artificial Intelligence Agents in Music Analysis: An Integrative Perspective Based on Two Use Cases](https://arxiv.org/abs/2511.13987)
*Antonio Manuel Martínez-Heredia,Dolores Godrid Rodríguez,Andrés Ortiz García*

Main category: cs.AI

TL;DR: This paper reviews AI advancements in music analysis and education, highlighting experimental validation of deep learning and multi-agent systems, and their pedagogical benefits and challenges.


<details>
  <summary>Details</summary>
Motivation: To explore AI's role in enhancing music analysis and education, addressing historical evolution and current methodologies.

Method: Integrative review and experimental validation using generative AI platforms and multi-agent systems for music analysis.

Result: AI improves musical pattern recognition, composition, and educational feedback, but transparency and bias remain challenges.

Conclusion: Provides a unified framework combining technical, pedagogical, and ethical considerations for AI in musicology and education.
...
### [243] [ALEX:A Light Editing-knowledge Extractor](https://arxiv.org/abs/2511.14018)
*Minghu Wang,Shuliang Zhao,Yuanyuan Zhao,Hongxia Xu*

Main category: cs.AI

TL;DR: ALEX is a lightweight knowledge editing framework for LLMs that improves accuracy and efficiency in multi-hop answers through hierarchical memory and advanced retrieval modules.


<details>
  <summary>Details</summary>
Motivation: The static knowledge in LLMs hampers adaptability to evolving information, and existing methods lack scalability and retrieval efficiency for complex questions.

Method: ALEX uses a hierarchical memory architecture for semantic clustering of edits, an IQS module for query-fact bridging, and a DEA engine for efficient retrieval.

Result: ALEX enhances multi-hop answer accuracy (MultiHop-ACC) and reasoning reliability (HopWise-ACC) while reducing search space by over 80%.

Conclusion: ALEX offers a scalable, efficient, and accurate solution for knowledge editing in LLMs.
...
### [244] [Syn-STARTS: Synthesized START Triage Scenario Generation Framework for Scalable LLM Evaluation](https://arxiv.org/abs/2511.14023)
*Chiharu Hagiwara,Naoki Nonaka,Yuhta Hashimoto,Ryu Uchimido,Jun Seita*

Main category: cs.AI

TL;DR: Syn-STARTS uses LLMs to generate synthetic triage cases indistinguishable from real manual datasets, showing potential for AI development in mass casualty incidents.


<details>
  <summary>Details</summary>
Motivation: AI can optimize triage decisions in MCIs but lacks large-scale real-world data due to event rarity, necessitating synthetic alternatives.

Method: Developed Syn-STARTS, an LLM-based framework to generate synthetic triage cases, and compared it with manual datasets like TRIAGE.

Result: Generated cases matched manual dataset quality, and LLM accuracy remained stable across START triage categories (green, yellow, red, black).

Conclusion: Synthetic data like Syn-STARTS can enable high-performance AI models for critical medical scenarios where real data is scarce.
...
### [245] [Making Evidence Actionable in Adaptive Learning](https://arxiv.org/abs/2511.14052)
*Amirreza Mehrabi,Jason W. Morphew,Breejha Quezada,N. Sanjay Rebello*

Main category: cs.AI

TL;DR: The paper proposes an instructor-governed feedback loop for adaptive learning, using safeguards like adequacy, attention, and diversity to convert assessment data into effective micro-interventions. It formalizes intervention assignment as a binary integer program and tests its efficacy in simulations and a physics course with 1,204 students.


<details>
  <summary>Details</summary>
Motivation: Current adaptive learning systems often fail to deliver timely and aligned interventions despite precise diagnostics. This study aims to bridge this gap by introducing safeguards and formalizing intervention strategies.

Method: The adaptive algorithm uses safeguards (adequacy, attention, diversity) and formalizes intervention assignment as a binary integer program. It employs greedy selection for low-richness settings, gradient-based relaxation for rich repositories, and a hybrid approach for intermediate cases.

Result: Both solvers achieved full skill coverage for most learners within bounded time. The gradient-based method reduced redundancy by 12% compared to greedy, while greedy was more computationally efficient in scarce settings.

Conclusion: The proposed controller effectively closes the diagnostic-pedagogical loop, providing equitable and scalable personalization while ensuring load-awareness and adaptability.
...
TL;DR: APD-agents: A multi-agent LLM-driven framework automating mobile app page layout design, improving efficiency and consistency without extensive training.


<details>
  <summary>Details</summary>
Motivation: Manual layout design is time-consuming and requires expertise, demanding automation to streamline collaboration and enhance design quality.

Method: APD-agents employs OrchestratorAgent, SemanticParserAgent, PrimaryLayoutAgent, TemplateRetrievalAgent, and RecursiveComponentAgent to dynamically parse user descriptions, generate layouts, and refine details.

Result: Achieves state-of-the-art performance on the RICO dataset, demonstrating effective automation and collaboration.

Conclusion: The framework successfully automates mobile page design, reducing manual effort and improving consistency.
...
TL;DR: The paper introduces R3, a dual-process framework combining LLMs' reasoning with VLN-specific expertise to improve navigation performance while addressing computational and spatial comprehension challenges.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based approaches for VLN lack precise spatial understanding and suffer from high computational costs, creating a gap compared to expert performance.

Method: Proposes R3, integrating Runner (lightweight transformer), Ruminator (multimodal LLM with CoT prompting), and Regulator (mode switcher) for efficient and accurate navigation.

Result: R3 outperforms state-of-the-art methods by 3.28% (SPL) and 3.30% (RGSPL) on the REVERIE benchmark.

Conclusion: R3 effectively bridges the gap between LLMs and domain experts in VLN, demonstrating superior performance and efficiency.
...
TL;DR: Existing AI benchmarks focus on accuracy but neglect enterprise needs like cost-efficiency and reliability. CLEAR framework addresses these gaps, proving cost-aware optimization is crucial.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fail to address enterprise-critical factors like cost-efficiency, reliability, and stability, limiting their practical utility.

Method: Analyzed 12 benchmarks, evaluated state-of-the-art agents, and introduced CLEAR framework for holistic enterprise-focused assessment.

Result: Optimizing solely for accuracy leads to 4.4-10.8x higher costs versus cost-aware alternatives. CLEAR better predicts production success (ρ=0.83 vs. ρ=0.41).

Conclusion: CLEAR provides a more comprehensive and practical evaluation framework for enterprise AI deployment, balancing accuracy with cost and reliability.
...
TL;DR: HFL-FlowLLM is a novel framework using large language models for network traffic flow classification in heterogeneous federated learning, improving F1 scores by 13% and reducing costs by 87%.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges like distributed data, privacy concerns, high costs, and poor generalization in traditional and federated learning methods for modern communication networks.

Method: Proposes HFL-FlowLLM, integrating large language models into heterogeneous federated learning for traffic flow classification.

Result: Achieves a 13% higher average F1 score and reduces training costs by 87% compared to existing methods.

Conclusion: HFL-FlowLLM shows strong potential for enhancing security and QoS in modern networks.
...
TL;DR: The paper tests LLMs' ability to handle chronological tasks in finance, revealing performance declines with complexity but improvements with explicit reasoning budgets.


<details>
  <summary>Details</summary>
Motivation: To evaluate LLMs' understanding of chronology, crucial for finance applications where look-ahead bias is a concern.

Method: Tasks include chronological ordering, conditional sorting, and anachronism detection, tested across GPT-4.1, Claude-3.7 Sonnet, and GPT-5 under varying reasoning-effort settings.

Result: Performance drops with sequence length; GPT-5 with high reasoning effort excels, while low effort mirrors earlier models' limitations.

Conclusion: Explicit reasoning budgets improve chronological task performance, highlighting current LLM limits and scenarios where reasoning aids accuracy.
...
TL;DR: The paper introduces a two-stage method to reduce hallucination errors in the Whisper ASR model by enhancing encoder robustness and using multi-objective knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: Whisper's performance suffers from hallucination errors, especially in noisy conditions. Existing solutions focus on preprocessing or post-processing, leaving model modifications unexplored.

Method: A two-stage approach: (1) Adaptive Layer Attention (ALA) groups encoder layers into blocks for robust encoding; (2) multi-objective knowledge distillation aligns student-teacher distributions.

Result: Experiments show reduced hallucinations and word error rates in noisy conditions, without degrading clean speech performance.

Conclusion: ALA and KD provide a principled way to enhance Whisper's reliability in noisy real-world scenarios.
...
TL;DR: DevPiolt, an LLM-based model, enhances IoT device operation recommendations by incorporating domain knowledge, aligning with user preferences, and controlling low-quality suggestions, leading to significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing models fail to handle the complexity and user preferences in IoT device operation recommendations, prompting the need for a more robust solution.

Method: Continual pre-training and multi-task fine-tuning equip the LLM with domain knowledge, followed by direct preference optimization and confidence-based exposure control.

Result: DevPiolt outperforms baselines by 69.5% on average and boosts online metrics like device coverage (21.6%) and acceptance rates (29.1%).

Conclusion: DevPiolt effectively improves IoT operation recommendations and has successfully been deployed in real-world applications.
...
TL;DR: The paper introduces a novel time-series forecasting framework for Airbnb market trends using regional representations and LLM embeddings, achieving a 48% reduction in error compared to baselines.


<details>
  <summary>Details</summary>
Motivation: To address housing affordability issues caused by Airbnb's disruption of local markets by providing accurate regional forecasts for policymakers.

Method: Sliding-window approach integrating listing features and contextual factors (urban accessibility, human mobility) into LLM-based embeddings, fed into advanced time-series models (RNN, LSTM, Transformer).

Result: Reduces RMSE and MAE by ~48% compared to traditional models, validated on Seoul's Airbnb dataset.

Conclusion: The framework improves forecasting accuracy and supports data-driven urban policy decisions, detecting oversupplied regions.
...
TL;DR: PathMind is a novel framework enhancing LLM-based knowledge graph reasoning by selectively guiding LLMs with prioritized reasoning paths, reducing noise and retrieval demands.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based KGR methods suffer from indiscriminate path extraction and high retrieval demands, leading to irrelevant noise and inefficiency.

Method: PathMind introduces a 'Retrieve-Prioritize-Reason' paradigm: retrieves subgraphs, prioritizes paths using a semantic-aware function, and employs dual-phase training for accurate responses.

Result: PathMind outperforms baselines, especially in complex reasoning tasks, by efficiently identifying essential paths and reducing input tokens.

Conclusion: PathMind improves faithfulness and interpretability in KGR by addressing key limitations of current LLM-based methods.
...
### [255] [When Words Change the Model: Sensitivity of LLMs for Constraint Programming Modelling](https://arxiv.org/abs/2511.14334)
*Alessio Pellegrino,Jacopo Mauro*

Main category: cs.AI

TL;DR: LLMs can generate executable models for standard optimization problems but struggle with rephrased or perturbed versions, indicating shallow understanding.


<details>
  <summary>Details</summary>
Motivation: To assess whether LLMs genuinely reason or rely on data contamination when generating optimization models.

Method: Rephrased and perturbed CSPLib problems to test LLMs' robustness against linguistic variations.

Result: LLMs perform well on original problems but poorly on modified ones, showing sensitivity to wording.

Conclusion: LLMs lack deep reasoning capabilities and depend heavily on familiar problem structures.
...
### [256] [Operationalizing Pluralistic Values in Large Language Model Alignment Reveals Trade-offs in Safety, Inclusivity, and Model Behavior](https://arxiv.org/abs/2511.14476)
*Dalia Ali,Dora Zhao,Allison Koenecke,Orestis Papakyriakopoulos*

Main category: cs.AI

TL;DR: The study explores how incorporating diverse human values affects LLM alignment, revealing systematic demographic differences in ratings and varying impacts of technical design choices.


<details>
  <summary>Details</summary>
Motivation: The research addresses the oversight of human social diversity in LLM alignment decisions, aiming to understand how pluralistic values influence model behavior and fairness.

Method: Alignment data was collected from US and German participants (N = 1,095) who rated LLM responses across five dimensions. Multiple models were fine-tuned using group-specific preferences, varying rating scales, disagreement handling, and optimization techniques.

Result: Demographic effects were observed: males rated responses as less toxic, while conservatives and Black participants rated them as more emotionally aware. Technical choices (e.g., preserving disagreement, 5-point scales, DPO) significantly impacted alignment outcomes.

Conclusion: The findings highlight the need to balance expert-driven and user-driven signals in alignment to ensure both safety and fair representation.
...
### [257] [A Neuro-Symbolic Framework for Reasoning under Perceptual Uncertainty: Bridging Continuous Perception and Discrete Symbolic Planning](https://arxiv.org/abs/2511.14533)
*Jiahao Wu,Shengwen Yu*

Main category: cs.AI

TL;DR: A neuro-symbolic framework bridges perceptual signals and symbolic reasoning by modeling uncertainty, achieving high success rates in robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of connecting continuous perceptual signals with discrete symbolic reasoning under uncertainty in AI systems.

Method: Combines transformer-based perception, GNN relational reasoning, and an uncertainty-aware symbolic planner to extract probabilistic symbolic states from visual input.

Result: Achieves 94%/90%/88% success rates on robotic manipulation benchmarks, outperforming POMDP baselines by 10–14 points with fast planning (15ms).

Conclusion: The framework provides a general-purpose solution for uncertainty-aware reasoning from perception to symbolic planning, validated by empirical and theoretical guarantees.
...
### [258] [Rate-Distortion Guided Knowledge Graph Construction from Lecture Notes Using Gromov-Wasserstein Optimal Transport](https://arxiv.org/abs/2511.14595)
*Yuan An,Ruhma Hashmi,Michelle Rogers,Jane Greenberg,Brian K. Smith*

Main category: cs.AI

TL;DR: A framework for constructing and refining task-oriented knowledge graphs (KGs) grounded in rate-distortion theory and optimal transport geometry improves MCQ generation from unstructured educational materials.


<details>
  <summary>Details</summary>
Motivation: Converting unstructured educational materials like lecture notes into KGs that capture pedagogical content is challenging but essential for AI-powered learning assistants.

Method: The framework models lecture content as a metric-measure space, aligns candidate KGs using Fused Gromov-Wasserstein couplings, and refines KGs via operators to minimize rate-distortion Lagrangian.

Result: MCQs generated from refined KGs outperform those from raw notes across fifteen quality criteria.

Conclusion: The study provides a principled foundation for optimizing KGs in AI-assisted education.
...
### [259] [AutoTool: Efficient Tool Selection for Large Language Model Agents](https://arxiv.org/abs/2511.14650)
*Jingyi Jia,Qinbin Li*

Main category: cs.AI

TL;DR: AutoTool is a graph-based framework that reduces the high inference cost of tool selection in LLM agents by leveraging tool usage inertia and historical trajectories.


<details>
  <summary>Details</summary>
Motivation: High inference costs in current LLM agent frameworks, especially for tool selection, hinder scalability and efficiency.

Method: AutoTool constructs a directed graph from historical tool usage patterns to model tool selection inertia and integrates parameter-level information.

Result: AutoTool reduces inference costs by up to 30% while maintaining competitive task completion rates.

Conclusion: Integrating statistical structure into LLM agent design improves efficiency without sacrificing performance.
