...
TL;DR: Gaussian See, Gaussian Do introduces a novel method for semantic 3D motion transfer from multiview video, achieving rig-free, cross-category motion transfer with high fidelity and consistency.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of semantic 3D motion transfer between objects without requiring rigs, focusing on meaningful correspondence and cross-category applicability.

Method: The approach uses implicit motion transfer, extracting motion embeddings from source videos via condition inversion, applying them to static target shapes, and supervising dynamic 3D Gaussian Splatting reconstruction. It includes anchor-based view-aware embeddings and a 4D reconstruction pipeline.

Result: The method outperforms adapted baselines in motion fidelity and structural consistency, establishing the first benchmark for semantic 3D motion transfer.

Conclusion: Gaussian See, Gaussian Do successfully achieves rig-free, cross-category motion transfer with high fidelity, offering a robust framework for future work in this area.
...
TL;DR: Benchmarking CNNs, transformers, and Mamba architectures for dental caries segmentation reveals CNN-based DoubleU-Net outperforms others despite the trend toward complex models, highlighting the need for architecture-task alignment.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of dental caries is crucial for early diagnosis, but automated methods face challenges like low contrast and limited data.

Method: Twelve state-of-the-art architectures were trained on the DC1000 dataset under identical conditions to compare performance.

Result: CNN-based DoubleU-Net achieved the highest scores (Dice: 0.7345, mIoU: 0.5978, precision: 0.8145), outperforming transformer and Mamba variants.

Conclusion: Architecture-task alignment is more critical than model complexity for domain-specific medical image segmentation.
...
TL;DR: BR-DF introduces geometric encoding for CAD models using distance functions, ensuring a 100% success rate in generating watertight B-Rep models.


<details>
  <summary>Details</summary>
Motivation: To improve CAD model representation and generation reliability by leveraging volumetric distance functions.

Method: Uses BR-DF encoding with SDF for mesh geometry and UDFs for topology, combined with Marching Cubes conversion and a latent diffusion model for generation.

Result: Achieves performance comparable to SOTA methods with a 100% success rate in producing B-Rep models.

Conclusion: BR-DF is a reliable and effective approach for CAD model generation.
...
TL;DR: GeoSceneGraph synthesizes indoor 3D scenes from text by leveraging graph structure and geometric symmetries without predefined relationships, achieving performance comparable to methods using ground-truth annotations.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing methods (ignoring scene graph structure or requiring inconvenient user inputs) for resource-constrained devices.

Method: Uses equivariant graph neural networks (EGNNs) conditioned on text features, avoiding predefined relationship classes.

Result: Achieves comparable performance to methods relying on ground-truth annotations.

Conclusion: GeoSceneGraph offers a practical solution for text-to-3D scene synthesis with improved coherence and realism.
...
TL;DR: An unsupervised bidirectional MRI synthesizer uses physics-inspired methods to transform between High-Field and Ultra-Low Field images, improving contrast significantly.


<details>
  <summary>Details</summary>
Motivation: To address the lack of models leveraging physics for MRI synthesis between High-Field and Ultra-Low Field images.

Method: Forward model simulates HF to ULF transformation using SNR values; INR network synthesizes HF images without observed data.

Result: WM-GM contrast improved by 52% in synthetic ULF-like images and 37% in 64mT images.

Conclusion: The method is robust and effective for MRI synthesis tasks.
...
TL;DR: I-Mix2Mix improves multi-view image editing by distilling a 2D diffusion model into a multi-view diffusion model, enhancing consistency and quality.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with artifacts and incoherence in multi-view editing from sparse inputs.

Method: Proposes InstructMix2Mix (I-Mix2Mix), integrating a 2D diffusion model into a multi-view diffusion model with novel adaptations like incremental updates and modified attention.

Result: Achieves better multi-view consistency and high per-frame edit quality.

Conclusion: I-Mix2Mix effectively addresses challenges in multi-view editing, outperforming existing methods.
...
TL;DR: SkinR1 addresses limitations in VLMs for dermatology by combining textbook-based reasoning with RL, improving diagnostic accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: The paper aims to overcome trustworthiness and clinical utility issues in VLMs for dermatology caused by data heterogeneity, lack of grounded rationales, and limited scalability.

Method: SkinR1 uses a textbook-based reasoning generator for expert-level supervision, supervised fine-tuning for grounded reasoning, and RL for transferring reasoning to sparse data.

Result: SkinR1 shows superior diagnostic accuracy across multiple dermatology datasets, with ablation studies confirming the value of supervised fine-tuning.

Conclusion: The proposed SkinR1 framework successfully integrates expert reasoning with RL, enhancing VLMs' reliability and scalability in dermatological diagnosis.
...
TL;DR: FarSLIP enhances CLIP's fine-grained alignment in remote sensing (RS) by introducing a multi-granularity dataset and a novel patch-to-patch distillation method.


<details>
  <summary>Details</summary>
Motivation: Current RS-specific CLIP variants lack fine-grained spatial awareness due to underutilized object-level supervision and ineffective region-text alignment methods.

Method: FarSLIP uses patch-to-patch distillation for local-global visual alignment and CLS token-based region-category alignment to avoid semantic coherence degradation.

Result: FarSLIP achieves state-of-the-art performance in RS open-vocabulary segmentation, zero-shot classification, and image-text retrieval.

Conclusion: FarSLIP addresses CLIP's spatial limitations in RS by leveraging multi-granularity data and refined alignment strategies, improving fine-grained vision-language tasks.
...
TL;DR: nnMIL is a multiple-instance learning framework that improves slide-level predictions in computational pathology by leveraging patch-level foundation models with random sampling and scalable training.


<details>
  <summary>Details</summary>
Motivation: Current methods for aggregating patch-level features in whole-slide images lack generalizability and reliability, hindering clinical utility.

Method: nnMIL uses random sampling at patch and feature levels, enabling scalable training, task-aware strategies, and ensemble predictions with uncertainty estimation.

Result: nnMIL outperforms existing methods across 40,000 WSIs and 35 tasks, showing strong generalization and robustness in external cohorts.

Conclusion: nnMIL provides a practical solution for reliable clinical predictions using pathology foundation models, enhancing AI deployment in real-world settings.
...
TL;DR: X-WIN, a novel CXR world model, leverages CT-derived volumetric knowledge to predict 2D projections, improving disease diagnosis and representation learning.


<details>
  <summary>Details</summary>
Motivation: Chest X-rays (CXRs) are limited by structural superposition, hindering 3D anatomy representation and disease diagnosis. The paper aims to overcome this by integrating knowledge from chest CT scans.

Method: X-WIN learns to predict 2D projections of CT scans in latent space. It uses affinity-guided contrastive loss for cross-projection correlation and incorporates real CXRs via masked image modeling and domain classification.

Result: X-WIN outperforms existing models in downstream tasks and can reconstruct 3D CT volumes from 2D projections.

Conclusion: X-WIN advances CXR analysis by bridging the gap between 2D and 3D imaging, enhancing diagnostic accuracy and adaptability.
...
TL;DR: Content-Promoted Scene Layers (CPSL) is a compact 2.5D video representation that enhances 2D content with volumetric benefits, reducing capture and rendering costs while maintaining perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Existing volumetric video representations are costly and limit scalability for real-time communication and on-demand video.

Method: CPSL decomposes frames into geometry-consistent layers with soft alpha bands and an edge-depth cache, enabling parallax-corrected novel-view synthesis via depth-weighted warping and alpha compositing. It maintains temporal coherence with motion-guided propagation.

Result: CPSL outperforms layer-based and neural-field baselines in perceptual quality and boundary fidelity, reducing storage and rendering costs significantly.

Conclusion: CPSL provides a practical solution for scalable 2.5D immersive media from conventional 2D video.
...
TL;DR: This paper introduces a benchmark for long-term periodic human activities, proposing a lightweight, training-free baseline that outperforms existing methods and shows deployment advantages.


<details>
  <summary>Details</summary>
Motivation: The study addresses the gap in research on long-term periodic workflows with low-contrast patterns, which are common in real-world applications like manufacturing and sports.

Method: The authors introduce a multimodal benchmark with 580 sequences and propose a training-free baseline model for periodic workflow detection.

Result: Experiments reveal that their baseline outperforms unsupervised and zero-shot LLM-based methods, demonstrating practical deployment benefits.

Conclusion: The work provides a valuable benchmark and an effective baseline for long-term periodic workflow analysis, reducing reliance on annotations and retraining.
...
TL;DR: A low-cost, general-purpose method using an LED Clock achieves millisecond-level synchronization for multi-view video streams in diverse setups.


<details>
  <summary>Details</summary>
Motivation: Synchronizing heterogeneous cameras is challenging without hardware sync, limiting dynamic-scene applications.

Method: Uses a custom LED Clock with red/IR LEDs to visually decode exposure windows for alignment.

Result: Achieves 1.34 ms RMSE error, outperforming light, audio, and timecode methods.

Conclusion: Enables advanced vision tasks in unconstrained environments, tested successfully in large-scale surgical recordings.
...
TL;DR: Novel DL methods reduce energy consumption in laser cutting by dynamically adjusting pump power based on material and smoke levels, achieving 20%-50% energy savings.


<details>
  <summary>Details</summary>
Motivation: Addresses high energy use and environmental impact in laser cutting due to lack of adaptive control in CO2 laser suction pumps.

Method: Uses closed-loop configurations, material classification via CNN and VGG16, and smoke level detection with DL models for adaptive pump control.

Result: Experimentally proven 20%-50% reduction in pump energy consumption.

Conclusion: Contributes to sustainable manufacturing by significantly lowering energy use in laser cutting processes.
...
TL;DR: The paper introduces EGSA, a fusion mechanism leveraging edge information to improve transparent object perception in depth estimation and segmentation, alongside a progressive training strategy.


<details>
  <summary>Details</summary>
Motivation: Transparent objects challenge depth estimation and segmentation due to their confounding properties, and existing multi-task frameworks suffer from negative cross-task interactions.

Method: Proposes Edge-Guided Spatial Attention (EGSA) for feature fusion and a multi-modal progressive training strategy transitioning from RGB-derived edges to depth-derived edges.

Result: EGSA improves depth accuracy over state-of-the-art MODEST on Syn-TODD and ClearPose benchmarks, especially in transparent regions, while maintaining competitive segmentation performance.

Conclusion: Edge-guided fusion and progressive training enhance transparent object perception, demonstrating robustness without requiring ground-truth depth during training.
...
TL;DR: The paper proposes a feature-based knowledge distillation (KD) framework that excludes logit-based losses, leveraging latent representation geometry to identify effective teacher layers for distillation.


<details>
  <summary>Details</summary>
Motivation: To improve knowledge distillation by focusing solely on feature-based losses, avoiding reliance on logit-based losses like cross entropy.

Method: Introduces a feature KD framework using feature-based losses exclusively, with a knowledge quality metric to identify effective teacher layers.

Result: Achieves state-of-the-art performance, boosting top-1 accuracy by up to 15% on three image classification datasets with diverse student-teacher pairs.

Conclusion: The proposed feature-based KD framework outperforms standard approaches, validated by experiments across different architectures.
...
TL;DR: Kandinsky 5.0 is a cutting-edge framework for high-res image and short video synthesis, featuring models like Kandinsky 5.0 Image Lite, Video Lite, and Video Pro. It highlights data curation, training optimizations, and quality-enhancement techniques to achieve top performance.


<details>
  <summary>Details</summary>
Motivation: The paper aims to advance generative AI by introducing high-quality, efficient models for image and video synthesis, making them accessible to the research community.

Method: The framework uses multi-stage training with pre-training, self-supervised fine-tuning, and RL-based post-training. It includes novel optimizations in architecture, training, and inference.

Result: Kandinsky 5.0 achieves state-of-the-art performance and high generation speeds, validated by human evaluation.

Conclusion: The release of Kandinsky 5.0 and its open-source components aims to boost research and accessibility in high-quality generative models.
...
TL;DR: FinCriticalED is a benchmark for evaluating OCR and vision language models on financial documents, focusing on factual correctness over text similarity.


<details>
  <summary>Details</summary>
Motivation: Financial documents are visually dense and require precise OCR to avoid critical errors like sign inversion or shifted dates, which traditional metrics fail to capture.

Method: The benchmark includes 500 image-HTML pairs with expert-annotated facts, uses LLM-as-Judge for evaluation, and benchmarks various OCR and vision language models.

Result: Proprietary models perform best but still make errors in complex numerical and temporal contexts.

Conclusion: FinCriticalED advances factual precision in financial document understanding and other precision-critical domains.
...
TL;DR: The paper proposes CKDA, a method for Visible-Infrared Lifelong Re-IDentification (VI-LReID), addressing mutual interference between modality-specific and common knowledge by disentangling and aligning them using MCP and MSP modules, along with CKA for feature alignment.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenge of collaborative forgetting caused by conflicting modality-specific and common knowledge in VI-LReID.

Method: CKDA employs Modality-Common Prompting (MCP) and Modality-Specific Prompting (MSP) modules for knowledge disentanglement, and Cross-modal Knowledge Alignment (CKA) for balanced alignment.

Result: Extensive experiments on four datasets confirm CKDA's superiority over state-of-the-art methods.

Conclusion: CKDA effectively addresses mutual interference in VI-LReID, achieving better performance through explicit knowledge disentanglement and alignment.
...
TL;DR: A new hologram representation using 2D Gaussian primitives reduces parameter search space and VRAM usage while improving fidelity and speed.


<details>
  <summary>Details</summary>
Motivation: To simplify hologram representation and enhance scalability in computer-generated holography by reducing storage and search complexity.

Method: Proposes structured complex-valued 2D Gaussian primitives, a differentiable rasterizer, and GPU-optimized light propagation for end-to-end training.

Result: Achieves 2.5x lower VRAM usage, 50% faster optimization, and higher-fidelity reconstructions compared to existing methods.

Conclusion: The new representation and conversion procedure improve hologram estimation efficiency and quality for next-generation systems.
...
TL;DR: The study explores whether computer vision (CV) models like ResNet-50 exhibit developmental alignment with children's learning trajectories in geometry and number concepts, finding partial alignment.


<details>
  <summary>Details</summary>
Motivation: To extend prior findings of cognitive alignment between CV models and human mathematical thinking by investigating if these models also mimic developmental progressions observed in children.

Method: A case study of the ResNet-50 model was conducted, analyzing its performance improvements across training stages and comparing them to developmental trajectories in geometry and number concepts.

Result: Developmental alignment was found for certain geometry concepts (e.g., Euclidean Geometry) and the emergence of a human-like mental number line, but not for others (e.g., Chiral Figures).

Conclusion: The findings highlight the potential of CV models to study human mathematical development and suggest future research directions involving diverse model architectures and benchmarks.
...
TL;DR: UniHOI unifies HOI detection and generation via a shared token space, improving performance in both tasks with innovative attention and learning methods.


<details>
  <summary>Details</summary>
Motivation: Traditional separation of HOI detection and generation limits interaction understanding; UniHOI aims to bridge this gap for better generalization and knowledge sharing.

Method: Introduces a symmetric interaction-aware attention module and a unified semi-supervised learning paradigm to enable bidirectional mapping between images and interaction semantics.

Result: Achieves state-of-the-art results: 4.9% accuracy boost in HOI detection and 42.0% improvement in generation metrics.

Conclusion: UniHOI successfully integrates HOI tasks, demonstrating enhanced performance and generalization capabilities.
...
TL;DR: The paper introduces DLRRF, a fusion model addressing spectral and spatial variability in HSI-MSI fusion by modeling degradation and using low-rank/residual components.


<details>
  <summary>Details</summary>
Motivation: Existing methods worsen fusion performance due to ill-posedness caused by inter-image variability.

Method: DLRRF models spectral variability as degradation, decomposes HSI into low-rank/residual components, and uses dimensionality reduction and implicit regularization.

Result: DLRRF outperforms existing methods in handling inter-image variability during fusion.

Conclusion: DLRRF effectively improves HSI-MSI fusion by addressing variability and leveraging spatial-spectral correlations.
...
TL;DR: CellGenNet is a knowledge distillation framework for robust nuclei segmentation in WSIs, using a student-teacher model with pseudo-labels and hybrid loss to address class imbalance and improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate nuclei segmentation in WSIs is difficult due to staining variability and tissue differences, requiring methods that work under limited supervision.

Method: CellGenNet uses a student-teacher architecture with sparse annotations, pseudo-labels, hybrid loss (binary cross-entropy + Tversky), consistency regularization, and dropout.

Result: CellGenNet outperforms supervised and semi-supervised baselines in segmentation accuracy and generalization across diverse cancer tissue WSIs.

Conclusion: The framework supports scalable and reproducible histopathology analysis by improving segmentation robustness under limited supervision.
...
TL;DR: ProPL introduces a universal semi-supervised framework for ultrasound image segmentation, outperforming existing methods with its flexible task adaptation and uncertainty-driven pseudo-label calibration.


<details>
  <summary>Details</summary>
Motivation: Existing ultrasound segmentation methods are specialized and lack versatility, limiting clinical utility. ProPL aims to address this gap by handling multiple organs/tasks using labeled and unlabeled data.

Method: ProPL uses a shared vision encoder with prompt-guided dual decoders, enabling task adaptation and self-training via uncertainty-driven pseudo-label calibration.

Result: ProPL outperforms state-of-the-art methods across various metrics on a new comprehensive ultrasound dataset spanning 5 organs and 8 tasks.

Conclusion: ProPL sets a new benchmark for universal ultrasound image segmentation, offering practical utility in clinical settings.
...
TL;DR: The paper evaluates Multimodal Large Language Models (MLLMs) on vertically written Japanese text, revealing their poor performance compared to horizontal text. It introduces synthetic and real-world datasets, showing that fine-tuning improves vertical text handling.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs lack robust support for vertically written Japanese text, a critical feature for document understanding tasks in Japanese. Limited research exists on this specific challenge.

Method: The study generates a synthetic Japanese OCR dataset (horizontal and vertical text) and a real-world dataset for evaluation. MLLMs are fine-tuned and tested using these datasets.

Result: Existing MLLMs perform worse on vertically written Japanese text. Fine-tuning with the synthetic dataset improves their vertical text handling.

Conclusion: The study highlights the need for better vertical text support in MLLMs and demonstrates that fine-tuning with targeted datasets can address this gap.
...
TL;DR: The paper explores whether video models can reason via video generation, introduces VR-Bench for evaluation, and demonstrates superior spatial reasoning abilities of video models.


<details>
  <summary>Details</summary>
Motivation: To extend video models' capabilities from generation to reasoning, leveraging spatial layouts and temporal continuity for tasks like spatial planning.

Method: Introduces VR-Bench, a benchmark with procedurally generated maze-solving videos, and uses supervised fine-tuning (SFT) to evaluate video models.

Result: Video models outperform leading vision-language models in spatial perception and reasoning, generalize well, and show improved reliability with diverse sampling during inference.

Conclusion: Reasoning via video is scalable and effective for spatial reasoning tasks, highlighting video models' unique potential.
...
TL;DR: BokehFlow is a depth-free framework for controllable bokeh rendering using flow matching and cross-attention, outperforming existing methods in quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing bokeh rendering methods rely on depth maps or lack controllability and efficiency.

Method: Uses flow matching and cross-attention to synthesize bokeh effects from all-in-focus images without depth inputs.

Result: Achieves photorealistic bokeh effects with precise semantic control via text prompts.

Conclusion: BokehFlow excels in rendering quality and efficiency compared to depth-dependent or generative approaches.
...
TL;DR: MambaTrack3D is a novel tracking framework for dynamic outdoor environments with high temporal variation (HTV) in LiDAR point clouds. It leverages Mamba-based inter-frame propagation and grouped feature enhancement to improve efficiency and accuracy, outperforming existing trackers.


<details>
  <summary>Details</summary>
Motivation: Existing memory-based trackers struggle with computational complexity, temporal redundancy, and underutilized geometric priors in HTV environments. This paper aims to address these limitations.

Method: Introduces MambaTrack3D with a Mamba-based Inter-frame Propagation (MIP) module for efficient inter-frame modeling and a Grouped Feature Enhancement Module (GFEM) to separate foreground/background semantics.

Result: Outperforms HTV-oriented and normal-scenario trackers on KITTI-HTV and nuScenes-HTV benchmarks, achieving up to 6.5% success and 9.5% precision improvements over HVTrack. Also competitive on standard KITTI.

Conclusion: MambaTrack3D achieves superior accuracy-efficiency trade-offs, demonstrating robust performance across HTV and conventional tracking scenarios.
...
TL;DR: TiCAL addresses inter-modal emotion conflicts in MER by using pseudo unimodal labels and hyperbolic feature embedding, improving recognition accuracy by 2.6%.


<details>
  <summary>Details</summary>
Motivation: Existing MER approaches overlook inter-modal emotion conflicts, leading to inconsistent emotional tendencies across modalities.

Method: TiCAL introduces dynamic consistency assessment via pseudo unimodal labels and typicality estimation, embedding features in hyperbolic space for fine-grained distinctions.

Result: TiCAL achieves ~2.6% higher accuracy than state-of-the-art DMD on datasets like CMU-MOSEI and MER2023.

Conclusion: TiCAL effectively mitigates inter-modal conflicts and enhances MER performance through consistency-aware learning and hyperbolic embeddings.
...
TL;DR: The paper introduces JCDM, a diffusion model using multi-view priors to improve pose-guided human image generation by addressing incomplete textures and lack of cross-view interaction.


<details>
  <summary>Details</summary>
Motivation: The limitations of single-view references and missing cross-view interaction in pose-guided human image generation drive the need for a multi-view approach.

Method: JCDM employs an appearance prior module (APM) for holistic identity inference and a joint conditional injection (JCI) mechanism to fuse multi-view cues and align identity, color, and texture.

Result: The model achieves state-of-the-art fidelity and cross-view consistency, supporting variable reference views with minimal backbone modifications.

Conclusion: JCDM effectively enhances pose-guided image generation by leveraging multi-view priors and shared conditioning.
...
TL;DR: The paper explores how visual token redundancy varies in discrete diffusion-based multimodal LLMs (dMLLMs) and its impact on efficiency and response quality. It identifies modality-specific pruning strategies and highlights differences between from-scratch and AR-to-diffusion dMLLMs.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of dMLLMs during inference, particularly due to full-sequence attention, by leveraging visual token redundancy and pruning strategies tailored to different dMLLM architectures and tasks.

Method: The study analyzes visual token redundancy evolution across dMLLM architectures and tasks, examines pruning effects on responses and efficiency, and identifies effective acceleration methods like layer-skipping and progressive pruning.

Result: Visual redundancy emerges in from-scratch dMLLMs during long-answer tasks. Pruning causes information loss, but from-scratch dMLLMs can recover lost information later. Layer-skipping benefits AR-to-diffusion dMLLMs, while progressive pruning suits from-scratch dMLLMs.

Conclusion: The findings provide insights into optimizing dMLLM efficiency through modality-specific pruning and acceleration techniques, enhancing their applicability in multimodal tasks.
...
TL;DR: The paper introduces Gaussian Blending to address visual artifacts in 3DGS-based novel view synthesis, improving rendering quality at unseen sampling rates without compromising speed or memory.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS methods suffer from blurring and staircase artifacts when rendering views at unseen sampling rates, attributed to alpha blending's limitations.

Method: Proposes Gaussian Blending, replacing scalar alpha/transmittance with spatially varying distributions, enabling background splats to contribute naturally.

Result: Outperforms existing models in capturing fine details at various sampling rates, maintaining real-time speed and no extra memory cost.

Conclusion: Gaussian Blending effectively eliminates artifacts and enhances rendering quality seamlessly within existing frameworks.
...
TL;DR: The paper presents a pipeline for generating automated surgical feedback by learning a surgical action ontology from trainer-to-trainee transcripts. It improves feedback fidelity using Instrument-Action-Target (IAT) triplets and context-aware video modeling.


<details>
  <summary>Details</summary>
Motivation: Automating high-quality, trainer-style surgical feedback is essential for scalable, consistent, and timely trainee skill improvement.

Method: The method involves mining IAT triplets from feedback text, fine-tuning a video-to-IAT model with surgical context, and using IAT representations to guide GPT-4o for feedback generation.

Result: The pipeline improves AUC scores for video-to-IAT recognition and feedback fidelity (rated 1-5, GPT-4o scores 2.44 with IAT conditioning vs. 2.17 without). Word error rate drops by 15-31%, and ROUGE scores increase by 9-64%.

Conclusion: Grounding feedback generation in explicit IAT structure enhances fidelity and provides clinician-verifiable rationales, making it suitable for auditable surgical training.
...
TL;DR: An event-triggered system detects elders' physical and mental states using GMM background modeling for motion detection and SVM for image analysis, tested in home scenarios with intuitive social media communication.


<details>
  <summary>Details</summary>
Motivation: To monitor elders' well-being by detecting events like watch dog, danger notice, and photo link, addressing their limited technical experience.

Method: Uses GMM background modeling for motion detection in watch dog and danger notice events, SVM for image analysis, and intuitive social media interaction.

Result: Tested in home scenarios with 5 families, successfully detecting and recording life activities and events.

Conclusion: The system effectively monitors elders' states and facilitates communication with relatives through intuitive design.
...
TL;DR: The paper explores using Computer-Use Agents (CUA) to judge automatic GUI designs created by coding-oriented language models (Coder), introducing AUI-Gym as a benchmark and proposing a collaborative Coder-CUA framework.


<details>
  <summary>Details</summary>
Motivation: GUIs are designed for humans, forcing CUAs to adopt inefficient behaviors. Advances in Coder models inspire exploring CUAs as judges for automatic GUI design.

Method: Introduces AUI-Gym with 1560 synthesized tasks, a verifier for reliability, and a Coder-CUA collaboration framework where Coder designs GUIs and CUA evaluates them.

Result: Success is measured by task solvability and CUA navigation rates, with feedback visualized via a CUA Dashboard.

Conclusion: The framework shifts GUI design toward agent-native efficiency, enabling active CUA participation in digital environments.
...
TL;DR: The paper proposes an Unbiased Semantic Decoding (USD) strategy integrated with SAM to improve few-shot segmentation by leveraging CLIP's semantic alignment and enhancing SAM features.


<details>
  <summary>Details</summary>
Motivation: Current SAM-based few-shot segmentation relies heavily on explicit prompts from support sets, limiting generalization and causing bias for unknown classes. The aim is to activate SAM's potential more effectively.

Method: The USD strategy extracts target information from both support and query sets, using CLIP semantics for consistency. It includes global (image-level) and local (pixel-level) feature enhancements, plus a visual-text prompt generator.

Result: The approach enhances SAM's semantic discrimination without retraining foundation models, focusing on target regions via enriched prompts.

Conclusion: USD improves few-shot segmentation by reducing bias and leveraging CLIP's semantics, offering a promising direction for SAM integration.
...
TL;DR: WaveFuse-AL is a novel active learning framework that combines multiple acquisition strategies with cyclical adaptation to improve medical imaging tasks efficiently.


<details>
  <summary>Details</summary>
Motivation: Active learning reduces annotation costs, but single strategies often perform inconsistently. WaveFuse-AL addresses this by dynamically integrating multiple strategies.

Method: WaveFuse-AL fuses BALD, BADGE, Entropy, and CoreSet strategies using cyclical temporal priors and performance-driven adaptation.

Result: Outperforms single-strategy and alternating-strategy baselines on three medical imaging benchmarks, improving ten out of twelve metrics.

Conclusion: WaveFuse-AL maximizes annotation budget utility and achieves significant performance gains in medical imaging tasks.
...
TL;DR: DCL-SE is a novel framework using dynamic curriculum learning and spatiotemporal encoding for improved neuroimaging analysis, outperforming existing methods in accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in spatiotemporal fidelity and adaptability of large-scale models for high-dimensional neuroimaging diagnostics.

Method: Uses Approximate Rank Pooling (ARP) for encoding 3D brain data into 2D representations and dynamic curriculum learning with Dynamic Group Mechanism (DGM) for progressive training.

Result: DCL-SE consistently outperforms existing methods in accuracy, robustness, and interpretability across six datasets.

Conclusion: Compact, task-specific architectures like DCL-SE are crucial despite the dominance of large-scale pretrained networks.
...
TL;DR: SceneEdited is a city-scale dataset designed for HD map maintenance, featuring over 800 scenes with synthesized changes and tools for 3D map updating research.


<details>
  <summary>Details</summary>
Motivation: Current HD maps often become outdated due to urban changes, creating a gap between detecting changes and updating 3D maps, especially with 2D image-based methods.

Method: The dataset includes calibrated RGB images, LiDAR scans, and change masks, along with baseline methods using structure-from-motion pipelines for updating outdated maps.

Result: SceneEdited provides over 800 scenes covering 73 km of urban area, with 23,000 synthesized changes and tools for scalability and trackability.

Conclusion: The dataset and toolkit establish a standardized benchmark for advancing research in 3D map updating.
...
TL;DR: The paper proposes a method to mitigate catastrophic forgetting in multimodal continual instruction tuning by approximating missing gradients using geometric properties and integrating them with real gradients.


<details>
  <summary>Details</summary>
Motivation: Catastrophic forgetting hinders continual learning; addressing it by treating it as a missing gradients problem.

Method: Utilizes geometric properties of parameter space to approximate missing gradients, integrates them with replay buffer gradients, and regulates with Bernoulli sampling.

Result: Achieves state-of-the-art performance without model expansion, effectively reducing catastrophic forgetting.

Conclusion: The approach successfully balances stability and plasticity, maintaining a compact model architecture.
...
TL;DR: SEC-Depth introduces a self-evolution contrastive learning framework to improve self-supervised depth estimation robustness under adverse weather conditions like rain and fog.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised depth estimation methods perform poorly in adverse weather due to reduced visibility.

Method: The proposed SEC-Depth uses intermediate training parameters to build latency models and employs a self-evolution contrastive loss (SECL) to adaptively adjust learning objectives.

Result: SEC-Depth integrates well with baseline models and significantly boosts robustness in zero-shot evaluations.

Conclusion: SEC-Depth effectively mitigates performance degradation in adverse weather without requiring manual intervention.
...
TL;DR: The paper introduces MMCM, a novel metric for evaluating Human Motion Prediction (HMP) that addresses coverage and validity of predicted motions by clustering motion modes and validating them against real data.


<details>
  <summary>Details</summary>
Motivation: Existing HMP metrics fail to assess both the diversity (coverage) and kinematic validity of predicted motions, leading to misleading evaluations.

Method: The authors propose MMCM, which clusters motion spaces into modes to evaluate coverage and uses dataset-derived valid modes to assess kinematic validity.

Result: Experiments show MMCM effectively defines motion modes and accurately scores multimodal predictions.

Conclusion: MMCM provides a robust metric for evaluating probabilistic HMP by addressing coverage and validity, validated through experiments.
...
TL;DR: The paper introduces instruction-guided lesion segmentation (ILS) and MIMIC-ILS, a dataset for CXR lesion segmentation, alongside ROSALIA, a model achieving high accuracy in segmentation and textual explanations.


<details>
  <summary>Details</summary>
Motivation: Current CXR lesion segmentation models are limited by few labels and reliance on expert-level text, hindering practical use.

Method: Developed ILS paradigm and MIMIC-ILS dataset via automated pipeline; fine-tuned ROSALIA vision-language model on MIMIC-ILS.

Result: ROSALIA achieves high segmentation and textual accuracy, demonstrating the effectiveness of the pipeline and dataset.

Conclusion: ILS and MIMIC-ILS address practical barriers, showcasing promise for pixel-level CXR lesion segmentation.
...
TL;DR: BrainRotViT combines ViT and CNNs for accurate brain age estimation, outperforming traditional methods and showing associations with neurodegenerative diseases.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations like manual feature engineering and computational costs in traditional brain age estimation methods.

Method: A hybrid BrainRotViT architecture uses a pretrained ViT encoder for slice-level features and a residual CNN regressor for brain age prediction, incorporating subject sex.

Result: Achieves MAE of 3.34 years ($r=0.98$, $R^2=0.95$) and generalizes well across cohorts. Brain age gaps link to Alzheimer's, cognitive impairment, and autism.

Conclusion: BrainRotViT offers an efficient, interpretable framework bridging CNNs and transformers, advancing aging and neurodegeneration research.
...
TL;DR: The paper introduces Insert In Style, a zero-shot generative framework for inserting real-world objects into stylized domains without per-subject finetuning or text prompts. It achieves high fidelity and practicality through multi-stage training and specialized masked-attention architecture.


<details>
  <summary>Details</summary>
Motivation: Existing methods for object composition in stylized domains lack generative fidelity or require impractical finetuning. This work addresses the gap by proposing a practical, high-fidelity zero-shot solution.

Method: A unified framework with multi-stage training for disentangling identity, style, and composition, and a masked-attention architecture enforcing this disentanglement. Trained on a new 100k-sample dataset.

Result: State-of-the-art performance outperforming existing methods on identity and style metrics, validated by user studies.

Conclusion: Insert In Style offers a novel, zero-shot solution bridging practicality and high fidelity for stylized object composition, validated by benchmarks and user studies.
...
TL;DR: The paper tackles bias in cross-modal retrieval for recipes and food images by using causal theory to refine representation learning, achieving top performance on the Recipe1M dataset.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat recipes as text describing dishes, causing bias due to overlooked factors like cooking variations.

Method: Proposes causal intervention to adjust for bias, introducing a debiasing neural module (multi-label ingredient classifier).

Result: Achieves MedR=1 on Recipe1M across varying test sizes (1K-50K) and sets new SOTA retrieval performances.

Conclusion: Causal theory effectively addresses retrieval bias, improving accuracy and relevance in food-to-recipe search.
...
TL;DR: Proposes PCMDE, a new metric combining LLMs and VLMs to evaluate semantic and structural accuracy beyond current metrics.


<details>
  <summary>Details</summary>
Motivation: Existing metrics (BLEU, CIDEr, etc.) fail in domain-specific contexts, needing better semantic and structural evaluation.

Method: Three-stage architecture: feature extraction with VLMs, adaptive fusion, and physics-guided reasoning via LLMs.

Result: PCMDE improves evaluation by enforcing constraints like alignment, position, and consistency.

Conclusion: PCMDE addresses gaps in current metrics for domain-specific and context-dependent scenarios.
...
TL;DR: SkinGPT-R1 is a dermatology-focused vision language model that enhances diagnostic reasoning with explicit, verifiable steps, outperforming other models on dermatology benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve dermatologic diagnostics by making the chain of thought reasoning explicit and verifiable, addressing gaps in clinical alignment and reasoning quality.

Method: The authors developed DermCoT, a corpus combining 10,000 filtered and 3,000 certified dermatologic cases, and defined DermEval and DermBench for evaluation. SkinGPT-R1 leverages this for training and validation.

Result: SkinGPT-R1 scores 4.031/5 on DermBench, ranking 1st and improving Vision-R1 by 41%. It shows stable accuracy gains on dermatology benchmarks and benefits from DermCoT supervision and visual distillation.

Conclusion: SkinGPT-R1 demonstrates superior performance in dermatologic reasoning and diagnostics, validated by clinician-aligned benchmarks and ablation studies, highlighting the value of structured reasoning and domain-specific training.
...
TL;DR: SplitFlux disentangles image content and style by fine-tuning single dream blocks via LoRA, outperforming existing methods in content preservation and stylization.


<details>
  <summary>Details</summary>
Motivation: Existing SDXL-based methods struggle with high-quality results, and Flux lacks effective content-style separation.

Method: SplitFlux uses Rank-Constrained Adaptation and Visual-Gated LoRA to fine-tune single dream blocks for disentanglement.

Result: SplitFlux achieves superior content preservation and stylization quality across diverse scenarios.

Conclusion: SplitFlux advances content-style disentanglement, enabling seamless re-embedding into new contexts.
...
TL;DR: GQN uses graph queries and attention to improve 3D radar object detection, outperforming prior methods by up to 53% mAP with lower overhead.


<details>
  <summary>Details</summary>
Motivation: Traditional grid and sequence-based methods struggle with sparse, irregular radar reflections, requiring a better approach for 360-degree automotive perception.

Method: GQN models radar objects as graphs, using graph queries for dynamic BEV attention, EdgeFocus for relational reasoning, and DeepContext Pooling for contextual aggregation.

Result: GQN achieves up to +53% relative mAP on NuScenes, surpassing prior radar methods by 8.2%, with 80% lower graph construction overhead.

Conclusion: GQN effectively addresses radar detection challenges, offering significant performance improvements and efficiency gains.
...
TL;DR: LEO, a framework for 3D scene graph prediction, enhances relational reasoning by transforming the scene graph into a line graph for edge-centric relational reasoning, improving relation prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing object-centric graph neural networks restrict relation representations to pairwise object context, missing high-order dependencies crucial for accurate relation prediction.

Method: LEO predicts object pair links, converts the scene graph to a line graph for edge-centric reasoning, and integrates enriched relation features into the original graph.

Result: Experiments on the 3DSSG dataset show consistent improvements over baselines, validating LEO's effectiveness.

Conclusion: LEO's edge-to-object reasoning paradigm successfully captures high-order dependencies, enhancing 3D scene graph prediction.
...
TL;DR: The paper introduces 'Xsyn', a one-stage X-ray security image synthesis pipeline using text-to-image generation, eliminating extra labor costs while improving dataset quality.


<details>
  <summary>Details</summary>
Motivation: Current X-ray security image synthesis methods are inefficient due to their two-stage, labor-intensive pipeline, which involves foreground extraction and compositing. The authors aim to create a more efficient, one-stage solution.

Method: Xsyn employs Cross-Attention Refinement (CAR) to refine annotations using diffusion models and Background Occlusion Modeling (BOM) to enhance image complexity in latent space.

Result: Xsyn outperforms previous methods with a 1.2% mAP improvement, demonstrating its effectiveness in generating high-quality synthetic images for prohibited item detection.

Conclusion: Xsyn is the first method to achieve labor-free, high-quality X-ray security image synthesis, significantly benefiting prohibited item detection across datasets and detectors.
...
TL;DR: IPTQ-ViT introduces a novel PTQ framework for fully integer-only vision transformers, eliminating retraining needs and improving accuracy over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing QAT and PTQ methods either require costly retraining or fail to achieve fully integer-only inference, limiting their practicality.

Method: IPTQ-ViT uses polynomial-based GELU and bit-shifting-based Softmax approximations, along with a unified metric for selecting optimal functions per layer.

Result: IPTQ-ViT improves top-1 accuracy by up to 6.44%p (avg. 1.78%p) for image classification and 1.0 mAP for object detection, outperforming prior PTQ and matching QAT methods.

Conclusion: IPTQ-ViT offers an efficient, retraining-free solution for fully integer-only vision transformers, balancing accuracy and computational cost.
...
TL;DR: Text2Loc++ is a neural network for localizing 3D point clouds using natural language descriptions, featuring a coarse-to-fine pipeline, new dataset, and innovative training methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of aligning diverse natural language descriptions with 3D point cloud submaps for accurate localization in urban environments.

Method: Text2Loc++ uses a pretrained language model and HTM for semantics, attention-based point cloud encoding, MIT for robustness, MHCL for embedding enhancement, and PMC/CCAT for fine localization.

Result: The model outperforms existing methods by up to 15% on KITTI360Pose and generalizes well to a new dataset with complex linguistic expressions.

Conclusion: Text2Loc++ demonstrates superior performance and robustness in 3D point cloud localization using natural language, with plans to release the code and dataset publicly.
...
TL;DR: Uni-Adapter enhances 3D Vision-Language Foundation Models' performance in noisy or shifted data scenarios using dynamic prototype learning and test-time adaptation, achieving significant improvements on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D VLFMs struggle with noisy, incomplete, or distribution-shifted data in practical settings, limiting their real-world applicability.

Method: Uni-Adapter employs dynamic prototypes stored in a 3D cache, updated for intra-class variability, and uses graph-based label smoothing and entropy-weighted aggregation to refine predictions.

Result: Uni-Adapter improves ModelNet-40C by 10.55%, ScanObjectNN-C by 8.26%, and ShapeNet-C by 4.49% over baseline models.

Conclusion: The proposed Uni-Adapter effectively addresses distribution shifts and enhances 3D VLFMs' robustness without retraining, demonstrating state-of-the-art performance.
...
TL;DR: A multimodal Transformer model integrates radar, RGB, IR, and audio data for UAV detection, achieving high accuracy (0.9812) and real-time efficiency (41.11 FPS).


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of single-modality approaches in UAV detection and aerial object recognition for robust surveillance and security.

Method: Designs a Transformer model fusing radar, RGB, IR, and audio data, leveraging self-attention for discriminative representations.

Result: Achieved 0.9812 accuracy, 0.9873 recall, and 41.11 FPS, with high precision distinguishing drones from other objects.

Conclusion: The multimodal Transformer model offers accurate, real-time UAV detection, validating its efficacy for complex airspace monitoring.
...
TL;DR: FIA-Flow is a black-box framework for high-fidelity image reconstruction from intermediate features in Split DNNs, addressing privacy risks.


<details>
  <summary>Details</summary>
Motivation: To assess the true extent of privacy leakage in Split DNNs by improving reconstruction quality of Feature Inversion Attacks (FIA).

Method: Introduces Latent Feature Space Alignment Module (LFSAM) and Deterministic Inversion Flow Matching (DIFM) for semantic alignment and distribution correction.

Result: Achieves high-fidelity reconstruction across various models, revealing greater privacy threats.

Conclusion: FIA-Flow demonstrates severe privacy risks in Split DNNs, surpassing prior FIA methods.
...
TL;DR: The paper proposes a fingerprint liveness detection algorithm using adaptive thresholding and wavelet transforms to resist distortions and improve accuracy compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Fingerprint spoofing threatens biometric systems, necessitating robust detection techniques resistant to distortions like noise or missing pixels.

Method: The algorithm uses anisotropic diffusion, wavelet transforms, adaptive thresholding, and SVM classification to detect fake fingerprints while resisting distortions.

Result: The method outperforms others by ~8% and 5% accuracy in scenarios with 90% missing pixels and 70x70 block missing, respectively.

Conclusion: The novel approach effectively addresses fingerprint forgery detection challenges, offering improved resistance and accuracy against distortions.
...
TL;DR: The paper introduces a lightweight, model-agnostic framework for UAV navigation that enhances open-set detection by distinguishing ID targets, OOD objects, and background clutter using a fusion of confidence estimates and MLP-based classification.


<details>
  <summary>Details</summary>
Motivation: Robust UAV navigation requires distinguishing between known and unknown objects during flight, but existing methods struggle with open-set detection, often confusing OOD objects with background.

Method: The proposed framework uses a fusion scheme with multiple confidence estimates and a compact MLP to classify ID, OOD, and background regions without compromising speed.

Result: The method outperforms threshold-based baselines by 2.7% AUROC in binary classification and improves open-set mAP by up to 9 points (18% relative gain).

Conclusion: The framework enables reliable three-way classification critical for UAV safety, balancing performance and throughput while surpassing existing techniques.
...
TL;DR: ZOMG is a zero-shot, open-vocabulary framework for segmenting motion sequences into semantic sub-actions without annotations or fine-tuning, outperforming prior methods by +8.7% mAP.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on dense supervision with predefined action classes, which are impractical for open-vocabulary, real-world settings.

Method: ZOMG uses language semantic partition (decomposing instructions via LLMs) and soft masking optimization (learning temporal masks) without altering the pretrained encoder.

Result: Outperforms prior methods by +8.7% mAP on HumanML3D, with improvements in downstream retrieval.

Conclusion: ZOMG establishes a new paradigm for annotation-free motion understanding.
...
### [62] [HV-Attack: Hierarchical Visual Attack for Multimodal Retrieval Augmented Generation](https://arxiv.org/abs/2511.15435)
*Linyin Luo,Yujuan Ding,Yunshan Ma,Wenqi Fan,Hanjiang Lai*

Main category: cs.CV

TL;DR: The paper introduces a hierarchical visual attack method targeting MRAG systems by adding imperceptible perturbations to images, disrupting retrieval and generation without manipulating other components.


<details>
  <summary>Details</summary>
Motivation: Existing research has shown MRAG systems are vulnerable to knowledge poisoning attacks. This study explores visual attacks solely through image perturbations, addressing gaps in understanding their impact.

Method: Proposes a Hierarchical Visual Attack that misaligns multimodal query and augmented knowledge inputs in MRAG. Uses a two-stage strategy to disrupt cross-modal and semantic alignments.

Result: Experiments on OK-VQA and InfoSeek datasets show significant performance drops in retrieval and generation using CLIP-based retrievers and LMMs like BLIP-2 and LLaVA.

Conclusion: The study demonstrates the effectiveness of visual perturbations in attacking MRAG systems, highlighting vulnerabilities in multimodal retrieval and generation chains.
...
TL;DR: AutoPrune leverages LLMs to self-design optimal pruning algorithms, overcoming expert reliance and outlier value issues.


<details>
  <summary>Details</summary>
Motivation: The massive size of LLMs hinders real-world deployment, and existing pruning methods are labor-intensive and expert-dependent. Additionally, uniform sparsity causes outlier value issues under high pruning ratios.

Method: AutoPrune uses LLMs to design pruning algorithms automatically, employing Graph-driven Chain-of-Thought (GCoT) to optimize prompts and Skew-aware Dynamic Sparsity Allocation (SDSA) to address outlier values.

Result: AutoPrune outperforms state-of-the-art methods on mainstream LLM benchmarks.

Conclusion: AutoPrune proves LLMs can self-prune, eliminating expert reliance and mitigating outlier issues, enhancing performance and interpretability.
...
TL;DR: ShelfOcc introduces a vision-only method for 3D occupancy estimation, using native 3D supervision from video to overcome limitations of 2D-based methods, outperforming previous weakly/shelf-supervised approaches by up to 34%.


<details>
  <summary>Details</summary>
Motivation: To address geometric inconsistencies and depth bleeding in 2D projection-based supervision for occupancy estimation without relying on LiDAR or manual annotations.

Method: Generates metrically consistent semantic voxel labels from video, filters and accumulates static geometry across frames, handles dynamic content, and propagates semantics into stable voxel representation.

Result: Substantially outperforms previous weakly/shelf-supervised methods on the Occ3D-nuScenes benchmark (up to 34% relative improvement).

Conclusion: High-quality 3D supervision is crucial for robust occupancy learning, offering a data-driven alternative to architectural innovation for LiDAR-free 3D scene understanding.
...
### [65] [RS-CA-HSICT: A Residual and Spatial Channel Augmented CNN Transformer Framework for Monkeypox Detection](https://arxiv.org/abs/2511.15476)
*Rashid Iqbal,Saddam Hussain Khan*

Main category: cs.CV

TL;DR: The paper introduces RS-CA-HSICT, a hybrid CNN-Transformer framework for MPox detection, combining residual and spatial learning with channel augmentation to improve accuracy and feature extraction. It achieves 98.30% accuracy and 98.13% F1-score.


<details>
  <summary>Details</summary>
Motivation: To enhance MPox detection by integrating CNN and Transformer strengths for improved feature space, lesion detail, and long-range dependencies.

Method: Uses HSICT blocks, residual CNN, spatial CNN, and channel augmentation (CA) for feature refinement. Introduces customized ICT blocks for global and local feature learning, and a fusion-attention mechanism.

Result: Achieves 98.30% accuracy and 98.13% F1-score on Kaggle and diverse MPox datasets, outperforming existing methods.

Conclusion: RS-CA-HSICT effectively combines CNN and Transformer capabilities for superior MPox detection with high accuracy and robustness.
...
### [66] [Controlling False Positives in Image Segmentation via Conformal Prediction](https://arxiv.org/abs/2511.15406)
*Luca Mossina,Corentin Friedrich*

Main category: cs.CV

TL;DR: The paper introduces a post-hoc framework for semantic segmentation that ensures statistical guarantees on false-positive predictions using conformal prediction.


<details>
  <summary>Details</summary>
Motivation: Deep models for semantic segmentation lack explicit statistical guarantees on errors, which is critical for clinical decision-making. Over-segmentation can have serious clinical consequences.

Method: The framework constructs confidence masks using a family of shrunken masks derived from score thresholds or morphological erosion. A calibration set selects shrink parameters via conformal prediction to control false positives.

Result: Experiments on polyp-segmentation confirm the method's empirical validity, ensuring false positives stay below a user-specified tolerance.

Conclusion: The model-agnostic framework provides practical, risk-aware segmentation with finite-sample guarantees, beneficial for clinical applications.
...
### [67] [Evaluating Low-Light Image Enhancement Across Multiple Intensity Levels](https://arxiv.org/abs/2511.15496)
*Maria Pilligua,David Serrano-Lozano,Pai Peng,Ramon Baldrich,Michael S. Brown,Javier Vazquez-Corral*

Main category: cs.CV

TL;DR: The paper introduces the Multi-Illumination Low-Light (MILL) dataset to evaluate low-light enhancement methods across varying light intensities, revealing performance inconsistencies and proposing improvements.


<details>
  <summary>Details</summary>
Motivation: Existing low-light enhancement methods rely on limited paired data, restricting understanding of performance across diverse illumination conditions.

Method: The MILL dataset is created with images at varied light intensities under controlled settings. Benchmarking and enhancements are proposed.

Result: Significant performance variations are found across illumination levels, with proposed improvements yielding up to 10 dB PSNR gain.

Conclusion: MILL enables robust evaluation, and enhancements improve performance across diverse lighting scenarios.
...
TL;DR: D4C is a novel Data-Free Quantization framework designed for CLIP models, addressing limitations of existing DFQ techniques by synthesizing semantically rich and diverse pseudo images, achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing DFQ techniques degrade CLIP model performance due to insufficient semantic content and low diversity in synthesized samples. D4C aims to bridge this gap.

Method: D4C uses three components: Prompt-Guided Semantic Injection, Structural Contrastive Generation, and Perturbation-Aware Enhancement to synthesize high-quality pseudo images.

Result: D4C improves Top-1 accuracy significantly across various datasets and bit-widths, e.g., 12.4%-18.9% on CIFAR-10 and 1.4%-5.7% on ImageNet-1K.

Conclusion: D4C successfully addresses DFQ challenges for CLIP, demonstrating superior performance through innovative image synthesis techniques.
...
TL;DR: WarNav is a new dataset derived from DATTALION images, designed for semantic segmentation in conflict zones, addressing gaps in urban driving datasets for autonomous navigation in hazardous areas.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between urban driving datasets and the unique challenges of conflict-affected environments for autonomous ground vehicles.

Method: Utilizes images from DATTALION, tackles data heterogeneity and ethical issues, and benchmarks with state-of-the-art semantic segmentation models.

Result: Provides baseline performance metrics and analyzes training data impact, aiming for navigability without annotated target images.

Conclusion: Aims to advance robust, safe autonomous navigation in high-risk areas with minimal annotated data.
...
TL;DR: The paper addresses fusion degradation in multimodal object detection, identifying gradient suppression and modality imbalance as key issues. It proposes RSC-MD to amplify gradients and balance modality learning, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal detection studies neglect fusion degradation and lack theoretical analysis. This paper aims to fill this gap by investigating its causes and proposing solutions.

Method: Proposes RSC-MD with two modules: RSC amplifies suppressed gradients, and MD addresses inter-modality coupling and imbalance, optimizing each modality-specific backbone.

Result: Experiments on FLIR, LLVIP, M3FD, and MFAD datasets show RSC-MD effectively mitigates fusion degradation and outperforms benchmarks.

Conclusion: RSC-MD successfully addresses fusion degradation in multimodal detection, improving robustness and performance, with plans to release code.
...
### [71] [CompTrack: Information Bottleneck-Guided Low-Rank Dynamic Token Compression for Point Cloud Tracking](https://arxiv.org/abs/2511.15580)
*Sifan Zhou,Yichao Cao,Jiahao Nie,Yuqian Fu,Ziyu Zhao,Xiaobo Lu,Shuo Wang*

Main category: cs.CV

TL;DR: CompTrack is a novel 3D SOT framework addressing spatial and informational redundancy in LiDAR point clouds, achieving real-time efficiency and top performance.


<details>
  <summary>Details</summary>
Motivation: Existing trackers face dual-redundancy challenges: spatial redundancy from background noise reduces accuracy, and informational redundancy within the foreground hampers efficiency.

Method: CompTrack introduces two modules: (1) SFP filters background noise using information entropy; (2) IB-DTC compresses foreground redundancy via online SVD analysis.

Result: CompTrack achieves state-of-the-art tracking performance and runs at 90 FPS on a single RTX 3090 GPU, validated on KITTI, nuScenes, and Waymo datasets.

Conclusion: The framework effectively eliminates redundancy, balancing accuracy and efficiency for 3D SOT in LiDAR point clouds.
...
TL;DR: The paper proposes a novel dataset for automating quality inspection in remanufacturing using deep learning, focusing on gearbox components. It introduces a contrastive regularization loss to improve model generalization across unseen components.


<details>
  <summary>Details</summary>
Motivation: Automating quality inspection in remanufacturing is challenging due to the high variety of parts and defects. Current deep learning models struggle with generalization to new variants or defects.

Method: A new image dataset of gearbox components (both good and defective) is created, with train-test splits simulating distribution shifts. Models are evaluated, and a contrastive regularization loss is proposed to enhance robustness.

Result: The contrastive regularization loss improves the model's generalization ability to unseen component types, as demonstrated by the experimental results.

Conclusion: The proposed approach effectively addresses generalization challenges in automated visual inspection for remanufacturing, leveraging a novel dataset and contrastive learning.
...
TL;DR: EASD is an end-to-end spike camera detector designed for autonomous driving, addressing challenges like motion blur and saturation with dual branches for global semantics and object-centric details.


<details>
  <summary>Details</summary>
Motivation: Object detection in autonomous driving faces issues like motion blur and saturation due to fast motion and extreme lighting. Spike cameras provide high-speed and dynamic range but their sparse output isn't compatible with standard detectors.

Method: EASD uses a dual-branch design: a Temporal Based Texture plus Feature Fusion branch for global semantics and an Entropy Selective Attention branch for object-centric details. DSEC Spike is introduced as a benchmark.

Result: The proposed method aims to enable effective object detection using spike cameras despite their sparse output.

Conclusion: EASD addresses the challenges of spike camera outputs in autonomous driving, offering a novel solution with a dual-branch architecture and a new benchmark.
...
TL;DR: Sigmma introduces multi-scale contrastive alignment for HE images and ST profiles, improving gene-expression prediction and cross-modal retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods align HE tiles with ST profiles at a single scale, missing fine-grained cellular structures.

Method: Sigmma uses multi-modal contrastive alignment and graphs cell interactions to capture hierarchical relationships.

Result: Avg. improvements of 9.78% in gene-expression prediction and 26.93% in cross-modal retrieval.

Conclusion: Sigmma effectively captures multi-tissue organization and improves cross-modal representation learning.
...
TL;DR: AI integration aids tuna species identification via EM systems, overcoming expert disagreement. A multi-stage pipeline with YOLOv9-SAM2 segmentation and hierarchical classification achieves high accuracy.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in AI-driven species identification for tuna fishing, given expert variability and EM data volume.

Method: Developed a pipeline comparing segmentation (Mask R-CNN, DINOv2-SAM2, YOLOv9-SAM2) and classification approaches (multiclass vs. hierarchical).

Result: YOLOv9-SAM2 outperformed others (mAP 0.66, recall 0.88). Hierarchical classification reduced error (4.5%) and improved segmentation (84.8%).

Conclusion: Combining YOLOv9-SAM2 segmentation with hierarchical classification optimizes accuracy in estimating tuna catch composition.
...
TL;DR: FunnyNodules is a synthetic dataset for evaluating explainable AI models in medical imaging, offering controllable attributes and full ground truth.


<details>
  <summary>Details</summary>
Motivation: To address the lack of medical datasets with reasoning-related annotations needed for developing and evaluating explainable AI models.

Method: Introduces FunnyNodules, a synthetic dataset generating lung nodule-like shapes with customizable visual attributes and predefined decision rules.

Result: Demonstrates use for model-agnostic evaluations, assessing attribute-target relations, interpreting model performance, and analyzing attention alignment.

Conclusion: FunnyNodules provides a customizable, versatile foundation for advancing explainable AI methods in medical image analysis.
...
TL;DR: EAR introduces a spiral-based autoregressive image generation model inspired by human vision, improving efficiency and quality.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models for visual generation are inefficient due to token-by-token decoding or multi-scale complexity.

Method: EAR unfolds tokens in a spiral order from the center outward, enabling parallel decoding and adaptive token prediction.

Result: EAR achieves state-of-the-art fidelity-efficiency trade-offs on ImageNet.

Conclusion: EAR offers a scalable, cognitively aligned approach for autoregressive image generation.
...
TL;DR: MTGNet improves few-shot semantic segmentation by fusing diverse textual prompts and enhancing cross-modal interactions to address issues like incomplete activation and noisy support features.


<details>
  <summary>Details</summary>
Motivation: Single textual prompts fail to capture semantic diversity and lack cross-modal interaction, degrading segmentation quality.

Method: MTGNet uses MTPR for refining textual priors, TAFF for semantic anchor fusion, and FCWA for robust visual priors.

Result: Achieves 76.8% mIoU on PASCAL-5i and 57.4% on COCO-20i in 1-shot settings.

Conclusion: MTGNet's multi-text guidance and robust modules significantly enhance segmentation performance.
...
TL;DR: A hybrid deep learning framework combining CNNs, ViTs, and GNNs achieves robust weed detection with 99.33% accuracy, using GAN-based augmentation and self-supervised contrastive learning for better generalization.


<details>
  <summary>Details</summary>
Motivation: Accurate weed detection enables selective herbicide use, promoting sustainable agriculture. Existing methods lack robustness across field conditions.

Method: Hybrid framework integrates CNNs, ViTs, and GNNs for local, global, and relational features. Uses GAN-based augmentation and self-supervised contrastive learning.

Result: Achieves 99.33% accuracy, precision, recall, and F1-score on multi-benchmark datasets.

Conclusion: The framework offers high interpretability, adaptability, and real-time deployment potential for sustainable precision farming.
...
TL;DR: This paper evaluates pose forecasting algorithms, adapts speech models for improved performance, and tests robustness using noisy data.


<details>
  <summary>Details</summary>
Motivation: To address reproducibility issues in human pose forecasting and improve state-of-the-art performance by leveraging speech models.

Method: Utilizes a unified pipeline for training and evaluation, adapts speech models for pose forecasting, and introduces noisy data testing.

Result: Speech models enhance performance; noisy data degrades results but unsupervised finetuning recovers some loss.

Conclusion: Speech models are effective for pose forecasting, but real-world noise impacts performance, highlighting the need for robustness.
...
TL;DR: A novel multi-modal deep learning method integrates X-ray and ultrasound to improve visualization of vertebral anatomy in spinal procedures.


<details>
  <summary>Details</summary>
Motivation: Ultrasound's inability to fully visualize vertebral bodies due to bone's acoustic shadowing effects limits its utility in spinal procedures.

Method: The method combines morphological data from a single X-ray image and 3D ultrasound to reconstruct occluded vertebral structures using paired simulated training data.

Result: Significant improvements in vertebral reconstruction (p < 0.001) are achieved, enabling accurate, complete lumbar spine visualization without preoperative CT registration.

Conclusion: Integrating X-ray mitigates ultrasound's key limitation while preserving its strengths, offering a promising solution for intraoperative spinal guidance.
...
TL;DR: The paper introduces DuFIA, a dual-domain adversarial attack method to challenge AI-generated image detectors by leveraging spatial and frequency-domain features, showing high transferability and robustness.


<details>
  <summary>Details</summary>
Motivation: To address the lack of advanced adversarial attacks for evaluating the security of AI-generated image detectors, ensuring their robustness against manipulation.

Method: Proposes DuFIA, which uses spatially interpolated gradients and frequency-aware perturbations to capture forensically important features, combining spatial and frequency domains for adversarial transferability.

Result: Extensive experiments confirm DuFIA's effectiveness in cross-model transferability, transparency, and robustness against various AIGI detectors.

Conclusion: DuFIA successfully challenges AIGI detectors, highlighting the need for stronger defenses against adversarial attacks.
...
TL;DR: Feature-map KD fails for ViTs due to token-level high-bandwidth encoding despite global low-rank structure. Solutions include lightweight projectors or width alignment, improving accuracy significantly.


<details>
  <summary>Details</summary>
Motivation: Understand and address the failure of feature-map KD in ViTs through representation analysis.

Method: Layer-wise SVD and token-level SEP analysis to identify encoding mismatch. Proposed solutions: lightweight projector or native width alignment.

Result: DeiT-Tiny accuracy increased significantly (74.86% to 77.53% and 78.23%) when distilling from CaiT-S24.

Conclusion: Exploiting low-rank structure resolves ViT feature distillation issues, offering effective and interpretable remedies.
...
TL;DR: AVATAAR improves video QA by combining global/local context and iterative reasoning, achieving notable gains on the CinePile benchmark.


<details>
  <summary>Details</summary>
Motivation: To address challenges LVLMs face with nuanced video queries requiring comprehensive understanding and detailed analysis.

Method: Introduces AVATAAR, a modular framework with global/local video context, a Pre Retrieval Thinking Agent, and a Rethink Module for iterative reasoning.

Result: Achieves relative gains: +5.6% (temporal), +5% (technical), +8% (theme), +8.2% (narrative). Feedback loop enhances adaptability.

Conclusion: AVATAAR offers scalable, accurate, interpretable, and extensible video QA.
...
TL;DR: KDF+ is a continual learning framework for LiDAR place recognition that addresses catastrophic forgetting via loss-aware sampling and rehearsal enhancement, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR place recognition methods struggle with catastrophic forgetting when adapting to new environments.

Method: KDF+ introduces a loss-aware sampling strategy and rehearsal enhancement mechanism to prioritize harder samples and refine memory retention.

Result: KDF+ outperforms existing continual learning methods and integrates well with state-of-the-art frameworks, showing stable performance gains.

Conclusion: KDF+ effectively mitigates catastrophic forgetting in LiDAR place recognition, offering significant improvements over current methods.
...
TL;DR: The paper introduces systematic improvements to dense feature matching, resulting in a faster, more accurate, and robust model.


<details>
  <summary>Details</summary>
Motivation: Existing dense matchers struggle with hard real-world scenarios, and high-precision models are often slow.

Method: Proposes a novel matching architecture and loss, a two-stage pipeline, CUDA kernel optimization, and leverages DINOv3.

Result: The model sets a new state-of-the-art in accuracy.

Conclusion: The improvements yield a significantly better matcher.
...
TL;DR: Survey on improving pretrained LLM accuracy via compute allocation at inference, unifying methods like Chain-of-Thought, Branch-Solve-Merge, and Tree-of-Thought under decomposition and topology perspectives.


<details>
  <summary>Details</summary>
Motivation: To enhance the predictive accuracy of large language models by strategically allocating compute resources during inference.

Method: Categorizes test-time scaling methods based on problem decomposition (sequential, parallel, tree-structured) and topological organization of subproblems.

Result: Unifies diverse approaches (Chain-of-Thought, Branch-Solve-Merge, Tree-of-Thought) and synthesizes their analyses.

Conclusion: Outlines promising future research directions for optimizing inference-time compute allocation.
...
TL;DR: The paper examines how early LLMs commit to outcomes during reasoning, showing predictability of correctness early in CoT processes, with implications for model interpretability.


<details>
  <summary>Details</summary>
Motivation: To understand how early Large Language Models (LLMs) internally decide on outcomes during chain-of-thought (CoT) reasoning, despite needing longer outputs for definitive answers.

Method: Linear classifiers trained on hidden states after the first t reasoning tokens to predict eventual correctness.

Result: Correctness is predictable early (few tokens), with accuracy drops highlighting hard questions' overrepresentation in long CoTs.

Conclusion: Internal self-assessment of reasoning success emerges early, impacting interpretability and inference-time control.
...
TL;DR: LiveCLKTBench is an automated pipeline to measure cross-lingual knowledge transfer in LLMs, revealing insights into linguistic distance and model scale effects.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of distinguishing genuine cross-lingual knowledge transfer from prior pre-training exposure in LLMs.

Method: Develops LiveCLKTBench, which identifies time-sensitive knowledge entities, generates factual questions, and translates them across languages for evaluation.

Result: Cross-lingual transfer is influenced by linguistic distance and asymmetry, with diminishing scale benefits across domains.

Conclusion: LiveCLKTBench offers reliable benchmarking for multilingual transfer research, highlighting the impact of linguistic factors and model size.
...
TL;DR: COMPASS is a lightweight control framework that uses a feedback loop to reduce factual errors in LLMs by dynamically modulating attention heads based on a transparent metric.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce incorrect statements despite having evidence, necessitating methods to improve factual consistency and interpretability.

Method: COMPASS introduces a model-based feedback loop during decoding, using a Context Reliance Score (CRS) and PID controller to modulate attention heads.

Result: COMPASS reduces contextual hallucination rates by 2.8 to 5.8% across benchmarks and improves evidence alignment.

Conclusion: Feedback-driven interpretability, as demonstrated by COMPASS, offers a pathway to better understand and control LLM behavior.
...
TL;DR: Proposed method evaluates manual and automatic prosodic segmentation's impact on Brazilian Portuguese speech synthesis using FastSpeech 2, showing slight improvements in intelligibility and naturalness.


<details>
  <summary>Details</summary>
Motivation: To explore how explicit prosodic segmentation affects spontaneous speech synthesis quality, addressing gaps in dataset construction.

Method: Used FastSpeech 2 for synthesis, comparing manual and automatic prosodic segmentation annotations in Brazilian Portuguese.

Result: Prosodic segmentation improved intelligibility and naturalness; manual segmentation added variability for better prosody.

Conclusion: Prosodic segmentation enhances speech synthesis, with manual annotations offering more natural prosodic variability.
...
### [92] [NAMeGEn: Creative Name Generation via A Novel Agent-based Multiple Personalized Goal Enhancement Framework](https://arxiv.org/abs/2511.15408)
*Shanlin Zhou,Xinpeng Wang,Jianxun Lian,Zhenghao Liu,Laks V. S. Lakshmanan,Xiaoyuan Yi,Yongtao Hao*

Main category: cs.CL

TL;DR: The paper introduces NAMeGEn, a multi-agent framework for creative Chinese baby naming, addressing challenges in multi-objective flexibility and interpretive complexity in CNLG.


<details>
  <summary>Details</summary>
Motivation: Existing CNLG methods struggle with personalized, fine-grained user requirements and interpreting implicit meanings, particularly in short-form text generation like baby naming.

Method: NAMEGEn iteratively optimizes names through objective extraction, generation, and evaluation, supplemented by a classical poetry corpus and a new benchmark (CBNames).

Result: NAMEGEn outperforms six baseline methods without training, generating creative names with meaningful explanations.

Conclusion: The framework effectively addresses CNLG challenges in personalized, short-form text generation, demonstrating superior performance.
...
TL;DR: EasyMED, a multi-agent SP simulator, matches human SP outcomes, offers flexibility, and improves cost efficiency, validated by the SPBench benchmark.


<details>
  <summary>Details</summary>
Motivation: Addressing the high cost and inflexibility of traditional SPs by leveraging LLMs for scalable solutions.

Method: Multi-agent framework (Patient, Auxiliary, Evaluation Agents) combined with SPBench for systematic assessment.

Result: Matches human SP learning, benefits lower-baseline students, enhances flexibility, and reduces costs.

Conclusion: EasyMED provides a viable, scalable alternative to traditional SPs with added advantages.
...
TL;DR: The paper proposes a hybrid deep neural network (HBGRU-LSTM) combining BGRU and LSTM layers to improve sentiment analysis, achieving 95% accuracy and better handling of contextual nuances and scalability compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Customer attitudes from social media and e-commerce texts are crucial for decision-making, but existing methods struggle with contextual nuances and scalability.

Method: The study introduces a hybrid model (HBGRU-LSTM) combining BGRU and LSTM layers, tested on IMDB and Amazon datasets.

Result: The model achieved 95% accuracy, outperforming traditional DL frameworks, and improved recall for negative sentiments and reduced misclassification loss.

Conclusion: The HBGRU-LSTM model enhances sentiment analysis by addressing contextual nuances and scalability, proving more effective than existing methods.
...
TL;DR: HTP improves text embeddings by addressing bottlenecks in backward information flow and over-compression, achieving gains in retrieval and embedding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods degrade representation quality due to restricted backward information flow and over-compression, especially in long documents.

Method: HTP partitions input into blocks with prepended summary tokens and replaces last-token pooling with mean-pooling for better information flow.

Result: HTP consistently improves performance across 11 retrieval datasets and 30 embedding benchmarks, particularly in long-context settings.

Conclusion: HTP is a scalable, architecture-agnostic solution enhancing embeddings for both zero-shot and finetuned models.
...
TL;DR: The paper presents a framework to address hallucinations in LLMs using probabilistic and information-theoretic methods, proposing mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: Large Language Models often produce plausible but incorrect outputs (hallucinations), which undermines their reliability.

Method: The framework uses probabilistic modeling, information theory, trigonometric signal analysis, and Bayesian uncertainty estimation to analyze errors and propose metrics like semantic and phase-aware uncertainty.

Result: The paper introduces strategies such as contrastive decoding, retrieval-augmented grounding, factual alignment, and abstention to mitigate hallucinations.

Conclusion: The unified approach aims to enhance LLM reliability and safety by connecting calibration, retrieval, and alignment techniques.
...
TL;DR: TASA is a student-aware tutoring framework for personalized math learning, integrating student personas, memory, and forgetting dynamics to improve adaptive instruction with LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based tutoring systems often overlook dynamic student knowledge evolution, especially in math, where precise scaffolding is crucial for mastery.

Method: TASA uses structured student personas, event memory, and continuous forgetting curves with knowledge tracing to dynamically update mastery and generate tailored questions/explanations.

Result: Empirical results show TASA outperforms baselines in learning outcomes and adaptive tutoring, highlighting the value of modeling forgetting and learner profiles.

Conclusion: TASA demonstrates the importance of temporal forgetting and learner-aware modeling for effective LLM-based tutoring systems.
...
TL;DR: The paper introduces a scalable framework to evaluate multilingual Vision-Language Models (VLMs) in Indian languages, addressing gaps like unverified translations and limited cultural representation. It presents HinTel-AlignBench, a benchmark for Hindi and Telugu, and analyzes performance regression in Indian languages compared to English.


<details>
  <summary>Details</summary>
Motivation: Current VLM evaluations lack robustness for low-resource languages like Hindi and Telugu, suffering from issues like auto-translations and narrow task coverage. The study aims to provide equitable AI evaluation methodologies.

Method: The authors developed a semi-automated dataset creation framework involving back-translation, filtering, and human verification. They built HinTel-AlignBench, combining adapted English datasets and novel Indic datasets.

Result: Performance regression was observed in Indian languages versus English (8.3 points for Hindi, 5.5 for Telugu). Failure modes were categorized to guide improvements in multilingual understanding.

Conclusion: The framework and benchmark highlight performance disparities in VLMs for Indian languages, emphasizing the need for culturally grounded evaluations to advance equitable AI.
...
TL;DR: The study explores the intrinsic dimension (ID) of modern LLMs, linking it to interpretable text properties like genre and linguistic features. Key findings include ID's independence from entropy, its variation across genres, and causal factors influencing ID.


<details>
  <summary>Details</summary>
Motivation: To understand how intrinsic dimension (ID) relates to interpretable text properties in LLMs, filling a gap in knowledge about its textual determinants.

Method: Cross-encoder analysis, linguistic features, and sparse autoencoders (SAEs) were used to analyze ID across various text genres and models.

Result: ID is uncorrelated with entropy after controlling for length, varies by genre (low for scientific prose, medium for encyclopedic content, high for creative writing), and is influenced by causal features like formal tone (reduces ID) and personalization (increases ID).

Conclusion: Scientific writing is simpler for LLMs, while creative and opinionated texts require more complexity. The study offers practical insights for interpreting ID-based results.
...
TL;DR: OEMA introduces a zero-shot clinical NER framework using multi-agent collaboration and ontology-guided reasoning, achieving near-supervised performance.


<details>
  <summary>Details</summary>
Motivation: Supervised NER models require costly annotated data, and existing zero-shot NER struggles with granularity and self-improvement integration.

Method: OEMA employs three agents: a self-annotator for example generation, a discriminator filtering via SNOMED CT, and a predictor using entity descriptions.

Result: OEMA achieves state-of-the-art exact-match performance on MTSamples and VAERS datasets, matching supervised BioClinicalBERT under related-match.

Conclusion: OEMA's ontology-guided reasoning and multi-agent collaboration address zero-shot NER challenges, showing promise for clinical NLP applications.
...
TL;DR: C3 Compression introduces a cascaded LLM approach for efficient text compression, achieving high accuracy even at extreme compression ratios.


<details>
  <summary>Details</summary>
Motivation: Address computational and memory challenges in LLMs processing long-context tasks by exploring upper limits of text compression.

Method: Cascades two LLMs—a small one compresses text into latent tokens, and a large one decodes the compressed context.

Result: Achieves 98% accuracy at 20x compression and 93% at 40x, outperforming DeepSeek-OCR.

Conclusion: C3 demonstrates superior performance in context compression, offering a simpler pipeline and setting a potential upper bound for compression ratios.
...
TL;DR: TeamNRC's zero/few-shot prompting approach using varying language model sizes achieved top ranks in Telugu and Hindi grammatical error correction, extending to Tamil, Malayalam, and Bangla while addressing data quality and metric concerns.


<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of zero/few-shot prompting for grammatical error correction in Indian languages and evaluate data quality and metrics.

Method: Utilized language models ranging from 4B to large proprietary models, applying zero/few-shot prompting techniques.

Result: Achieved Rank 4 in Telugu and Rank 2 in Hindi, with GLEU scores of 83.78 and 84.31 respectively, extending results to Tamil, Malayalam, and Bangla.

Conclusion: Small language models show potential, but dataset quality and evaluation metrics need improvement for Indian language scripts.
...
TL;DR: The paper discusses using SetFit, a few-shot learning technique, for sentiment analysis on Arabic dialects in hotel reviews, achieving a 73% F1 score.


<details>
  <summary>Details</summary>
Motivation: Sentiment analysis for Arabic dialects is challenging due to linguistic diversity and lack of annotated data.

Method: Employed SetFit framework for few-shot learning on Moroccan and Saudi dialect hotel reviews.

Result: Achieved 73% F1 score on evaluation, ranking 12th among 26 participants.

Conclusion: Few-shot learning shows promise for handling data scarcity in nuanced dialectal Arabic sentiment analysis.
...
TL;DR: Adversarial poetry acts as a universal jailbreak technique for LLMs, achieving high attack-success rates (ASRs) across multiple risk domains and outperforming non-poetic baselines.


<details>
  <summary>Details</summary>
Motivation: To investigate whether stylistic variations, like poetry, can bypass LLM safety measures, revealing vulnerabilities in current alignment methods.

Method: Curated poetic prompts tested across 25 models, converted harmful prompts into verse, and evaluated outputs using judge models and human validation.

Result: Poetic prompts achieved ASRs up to 90%, with average success rates of 62% (hand-crafted) and 43% (meta-prompt), significantly higher than prose baselines.

Conclusion: Stylistic variation (like poetry) systematically circumvents LLM safety mechanisms, highlighting limitations in current alignment and evaluation practices.
...
TL;DR: HEAD-QA v2 is an updated Spanish/English healthcare reasoning dataset with over 12,000 questions, benchmarking LLMs for biomedical research.


<details>
  <summary>Details</summary>
Motivation: Address the need for high-quality datasets capturing healthcare reasoning complexity.

Method: Extend dataset, benchmark LLMs using prompting, RAG, and probability-based answer selection, and provide multilingual versions.

Result: Performance is driven by model scale and reasoning ability; complex inference strategies yield limited gains.

Conclusion: HEAD-QA v2 is a reliable resource for biomedical reasoning and model improvement research.
...
TL;DR: A review of LLM core technologies and their applications in scientometrics, proposing AI-driven scientific evaluation and knowledge graph methods.


<details>
  <summary>Details</summary>
Motivation: Highlight LLMs' role in advancing AGI and explore their potential in scientometric research.

Method: Comprehensive review of LLM technologies (prompt engineering, fine-tuning, etc.) and historical SciSci development.

Result: Forward-looking insights on LLMs in scientometrics, AI agent-based evaluation, and new research detection.

Conclusion: LLMs offer transformative potential in scientometrics, enabling innovative AI-driven scientific methods.
...
TL;DR: A system combining LLM reranking and semantic search improves efficiency for AMTs by reducing manual lookup time while maintaining compliance with aviation regulations.


<details>
  <summary>Details</summary>
Motivation: AMTs spend significant time searching manuals, which slows MRO operations. The goal is to streamline this process without compromising regulatory compliance.

Method: The system uses revision-robust embeddings from ATA chapters and vision-language parsing to structure certified content, integrating with existing legacy viewers.

Result: Achieves >90% retrieval accuracy, 90.9% top-10 success rate, and reduces lookup time from 6-15 minutes to 18 seconds per task.

Conclusion: Semantic retrieval can operate within strict regulations and significantly reduce operational workload in multilingual MRO workflows.
...
TL;DR: DEPO optimizes efficiency in LLM agents by reducing tokens per step and total steps, achieving significant performance improvements with less data.


<details>
  <summary>Details</summary>
Motivation: Improving reasoning efficiency in LLMs without compromising performance, addressing the lack of systematic efficiency definitions.

Method: Introduces dual-efficiency (step-level and trajectory-level) and DEPO, a preference optimization method rewarding succinct responses and fewer steps.

Result: Reduces token usage by up to 60.9% and steps by 26.9%, with a 29.3% performance boost; generalizes to math benchmarks and works with limited data.

Conclusion: DEPO effectively enhances LLM agent efficiency, demonstrating scalability and generalization across tasks.
...
TL;DR: SPRING Lab developed ASR systems for the MADASR 2.0 challenge, focusing on language and dialect prediction using a Multi-Decoder architecture with phonemic CLS, outperforming baselines in Track 2.


<details>
  <summary>Details</summary>
Motivation: To improve ASR systems' ability to predict language and dialect across 8 languages and 33 dialects, adhering to challenge restrictions in Track 1 and Track 2.

Method: Used a Multi-Decoder architecture with phonemic Common Label Set (CLS) as an intermediate representation and explored methods to retain phonemic gains in grapheme conversion.

Result: Outperformed the baseline in 3 languages (Track 2) for WER/CER and achieved the highest language and dialect ID accuracy among participants.

Conclusion: The novel training approach with CLS and Multi-Decoder architecture successfully improved ASR performance, proving effective for multilingual and multidialectal tasks.
...
TL;DR: LLM-MemCluster is a novel framework enabling LLMs to perform text clustering end-to-end by incorporating dynamic memory and dual-prompt strategies, outperforming existing methods without tuning.


<details>
  <summary>Details</summary>
Motivation: Current LLMs lack stateful memory and struggle with cluster granularity, leading to reliance on complex external pipelines rather than native solutions.

Method: Introduces LLM-MemCluster with Dynamic Memory for state awareness and Dual-Prompt Strategy to determine cluster numbers.

Result: Outperforms baselines on benchmark datasets without tuning.

Conclusion: LLM-MemCluster offers an effective, interpretable, and fully LLM-native clustering paradigm.
...
TL;DR: Proposal of LPDS and pelican NLP for standardized and reproducible language data processing.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like lack of standardization in linguistic data and methodologies.

Method: Introduces LPDS (inspired by BIDS) for data structure and pelican NLP for modular Python-based processing.

Result: Provides transparent, reproducible workflows for linguistic feature extraction.

Conclusion: LPDS and pelican NLP enhance methodological transparency and reproducibility in language processing.
...
TL;DR: The paper introduces Mera Multi, a multimodal evaluation framework for Russian-language MLLMs, featuring 18 tasks across text, image, audio, and video modalities, with a focus on cultural specificity and benchmark integrity.


<details>
  <summary>Details</summary>
Motivation: To address the lack of multimodal benchmarks for Russian-language MLLMs and understand their intelligence, limitations, and risks.

Method: Developed an instruction-based benchmark with 18 tasks, unified prompts, and metrics, alongside a taxonomy of multimodal abilities. Included watermarking and licenses to prevent leakage.

Result: Provided baseline results for both closed-source and open-source models, showcasing the framework's applicability.

Conclusion: Mera Multi offers a replicable methodology for creating multimodal benchmarks in typologically diverse languages, particularly Slavic ones.
...
TL;DR: HSKBenchmark is introduced as the first benchmark for staged modeling and writing assessment of LLMs in Chinese SLA, featuring curriculum-tuning and detailed evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of systematic benchmarks for phase-wise modeling in Chinese SLA using LLMs, given ethical and practical constraints on human studies.

Method: Develops HSKBenchmark with authentic textbooks, synthetic samples, and a curriculum-tuning framework. Evaluates grammar coverage, writing errors, and complexity.

Result: Fine-tuned LLMs match advanced human learners' writing performance and show human-like acquisition traits, proving the benchmark's effectiveness.

Conclusion: HSKBenchmark and HSKAgent provide foundational tools for advancing language acquisition modeling and LLM interpretability research.
...
### [114] [Area-Optimal Control Strategies for Heterogeneous Multi-Agent Pursuit](https://arxiv.org/abs/2511.15036)
*Kamal Mammadov,Damith C. Ranasinghe*

Main category: cs.MA

TL;DR: A novel strategy for multi-agent pursuit-evasion games uses geometric regions (safe-reachable sets) and gradient-based controls for efficient, real-time capture of a slower evader by faster pursuers.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of cooperative pursuit-evasion games with heterogeneous agents, aiming to develop a computationally efficient and geometrically intuitive capture strategy.

Method: The authors define the evader's safe-reachable set using Apollonius circles and formulate the problem as a zero-sum game. Optimal control laws are derived analytically from gradients of the set's area with respect to agent positions.

Result: Simulations show that the gradient-based controls enable pursuers to shrink the evader's safe region systematically, ensuring guaranteed capture.

Conclusion: The area-minimization approach offers a clear geometric objective for cooperative capture, with real-time computational feasibility.
...
### [115] [Distributed primal-dual algorithm for constrained multi-agent reinforcement learning under coupled policies](https://arxiv.org/abs/2511.15053)
*Pengcheng Dai,He Wang,Dongming Wang,Wenwu Yu*

Main category: cs.MA

TL;DR: Proposes a framework for constrained multi-agent reinforcement learning (CMARL) with coupled policies and distributed primal-dual algorithm, achieving ε-first-order stationary convergence.


<details>
  <summary>Details</summary>
Motivation: Addresses collaborative multi-agent learning with safety constraints, emphasizing distributed communication and security by limiting direct parameter sharing.

Method: Agents use coupled policies based on local and neighbor states. A distributed primal-dual algorithm restricts information sharing to limited neighborhoods and estimates parameters indirectly.

Result: Achieves ε-first-order stationary convergence with $о(γ^{rac{κ+1}{κ_{p}}})$ approximation error, validated via GridWorld simulations.

Conclusion: The framework effectively balances collaboration, safety, and security in CMARL, demonstrating practical feasibility.
...
### [116] [Adversarial Attack on Black-Box Multi-Agent by Adaptive Perturbation](https://arxiv.org/abs/2511.15292)
*Jianming Chen,Yawen Wang,Junjie Wang,Xiaofei Xie,Yuanzhe Hu,Qing Wang,Fanjiang Xu*

Main category: cs.MA

TL;DR: AdapAM is a novel adversarial attack framework for black-box multi-agent systems (MAS) that balances effectiveness and stealthiness by adaptively selecting victims and using proxy-based perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attack frameworks for MAS are limited by impracticality (requiring white-box info) and lack of stealthiness/effectiveness. AdapAM addresses these gaps.

Method: AdapAM combines Adaptive Selection Policy (chooses victims and malicious actions) and Proxy-based Perturbation (uses imitation learning to approximate MAS for black-box attacks).

Result: Tested in eight MAS environments, AdapAM outperforms four baselines in attack performance and stealthiness (least noisy, hardest to detect).

Conclusion: AdapAM provides a practical and stealthy solution for adversarial attacks in black-box MAS.
...
### [117] [Transformer Injectivity & Geometric Robustness - Analytic Margins and Bi-Lipschitz Uniformity of Sequence-Level Hidden States](https://arxiv.org/abs/2511.14808)
*Mikael von Strauss*

Main category: cs.LG

TL;DR: The paper refines injectivity properties in decoder-only Transformers, proving dichotomies for each layer and showing generic injectivity persists under mild conditions. Empirical studies confirm these properties.


<details>
  <summary>Details</summary>
Motivation: To understand and refine the injectivity properties of decoder-only Transformers, ensuring their robustness under quantization and training.

Method: Defines collision discriminants and injective strata per layer, proves dichotomies, and uses geometric diagnostics like separation margin and co-Lipschitz constants.

Result: Empirical tests confirm generic injectivity in models like LLaMA-3 and Qwen, with minimal collisions except under 4-bit quantization.

Conclusion: Transformer representations are generically injective and can be probed using simple geometric diagnostics.
...
### [118] [DEVAL: A Framework for Evaluating and Improving the Derivation Capability of Large Language Models](https://arxiv.org/abs/2511.14813)
*Yifan Li,Qin Li,Min Zhang,Min Zhang,Peixin Wang*

Main category: cs.LG

TL;DR: The paper proposes Derivation Relation (DR) and Derivation Capability (DC) to evaluate LLMs' reasoning over data changes. DEVAL framework tests five LLMs, revealing moderate DR recognition but poor application. Derivation Prompting (DP) improves DC by 15.2%.


<details>
  <summary>Details</summary>
Motivation: To address the gap in evaluating LLMs' reasoning ability to modify outputs based on input changes, mimicking human reasoning.

Method: Introduces DR and DC, develops DEVAL evaluation framework, tests five LLMs and one Large Reasoning Model, and proposes DP for improvement.

Result: LLMs show moderate DR recognition but significant drop-offs in application. DP improves DC by 15.2%.

Conclusion: DP enhances LLMs' DC, outperforming existing prompt engineering techniques, highlighting room for improvement in LLMs' reasoning.
...
### [119] [Dynamic Nested Hierarchies: Pioneering Self-Evolution in Machine Learning Architectures for Lifelong Intelligence](https://arxiv.org/abs/2511.14823)
*Akbar Anbar Jafari,Cagri Ozcinar,Gholamreza Anbarjafari*

Main category: cs.LG

TL;DR: The paper introduces dynamic nested hierarchies to enhance machine learning models' adaptability in non-stationary environments, overcoming limitations like rigid architectures and anterograde amnesia.


<details>
  <summary>Details</summary>
Motivation: Current models struggle with continual adaptation and lifelong learning due to fixed architectures. The nested learning paradigm inspired dynamic nested hierarchies as a solution.

Method: Proposes dynamic nested hierarchies allowing models to autonomously adjust optimization levels, nesting structures, and update frequencies, inspired by neuroplasticity.

Result: Mathematical proofs and empirical results show improved performance in language modeling, continual learning, and long-context reasoning.

Conclusion: Dynamic nested hierarchies represent a foundational step toward adaptive, general-purpose intelligence.
...
### [120] [Empowering Multi-Turn Tool-Integrated Reasoning with Group Turn Policy Optimization](https://arxiv.org/abs/2511.14846)
*Yifeng Ding,Hung Le,Songyang Han,Kangrui Ruan,Zhenghui Jin,Varun Kumar,Zijian Wang,Anoop Deoras*

Main category: cs.LG

TL;DR: GTPO enhances RL training for LLMs by providing fine-grained rewards and self-supervised signals, outperforming GRPO by 3.0%.



<details>
  <summary>Details</summary>
Motivation: Existing RL methods like GRPO struggle with coarse-grained rewards, hindering training for multi-turn Tool-Integrated Reasoning tasks.


Method: GTPO introduces turn-level rewards, return-based advantage estimation, and self-supervised reward shaping.


Result: GTPO outperforms GRPO by 3.0% across diverse reasoning benchmarks.


Conclusion: GTPO is effective for advancing complex multi-turn reasoning tasks with LLMs.
...
### [121] [FinTRec: Transformer Based Unified Contextual Ads Targeting and Personalization for Financial Applications](https://arxiv.org/abs/2511.14865)
...
### [122] [Transformer-Guided Deep Reinforcement Learning for Optimal Takeoff Trajectory Design of an eVTOL Drone](https://arxiv.org/abs/2511.14887)
*Nathan M. Roberts,Xiaosong Du*

Main category: cs.LG

TL;DR: Transformer-guided DRL improves training efficiency and accuracy for optimal eVTOL takeoff trajectories compared to vanilla DRL.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional optimal control methods and training difficulties in DRL for eVTOL trajectory optimization.

Method: Uses transformer-guided DRL to explore realistic state spaces for designing optimal takeoff trajectories with minimal energy consumption.

Result: Achieved 25% faster training and 97.2% accuracy in energy consumption compared to simulation-based optimal reference.

Conclusion: Transformer-guided DRL outperforms vanilla DRL in training efficiency and optimal design verification for eVTOL applications.
...
### [123] [Bringing Federated Learning to Space](https://arxiv.org/abs/2511.14889)
*Grace Kim,Filip Svoboda,Nicholas Lane*

Main category: cs.LG

TL;DR: The paper analyzes the feasibility of adapting federated learning (FL) algorithms for satellite constellations, introducing a 'space-ification' framework to overcome orbital constraints and demonstrating scalability for up to 100 satellites with near-centralized performance.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of LEO satellite constellations necessitates distributed on-board machine learning due to downlink bandwidth limitations. FL offers collaborative training but must address space-specific challenges like intermittent connectivity and orbital dynamics.

Method: The study adapts terrestrial FL algorithms (FedAvg, FedProx, FedBuff) into orbital-ready versions using a 'space-ification' framework. It evaluates these methods across 768 constellation configurations varying cluster sizes, satellites per cluster, and ground station networks.

Result: Space-adapted FL algorithms scale efficiently to 100 satellites, achieving near-centralized performance. Training cycles reduce from months to days (9x speedup) via orbital scheduling and local coordination.

Conclusion: The results offer actionable insights for mission designers, enabling autonomous and resilient satellite operations through distributed on-board learning.
...
### [124] [It's LIT! Reliability-Optimized LLMs with Inspectable Tools](https://arxiv.org/abs/2511.14903)
*Ruixin Zhang,Jon Donnelly,Zhicheng Guo,Ghazal Khalighinejad,Haiyang Huang,Alina Jade Barnett,Cynthia Rudin*

Main category: cs.LG

TL;DR: LLMs lack transparency in reasoning, limiting trustworthiness in high-stakes tasks. The LIT framework forces LLMs to use reliable external tools for problem-solving, improving reliability and troubleshootability without losing performance.


<details>
  <summary>Details</summary>
Motivation: LLMs' opaque reasoning and unreliable solutions hinder their use in high-stakes domains. Ensuring trustworthy solutions by leveraging external tools is crucial.

Method: Introduces the LIT framework, which enables LLMs to select reliable, troubleshoot-friendly tools via sequential calls. Includes a benchmark dataset and reliability cost functions for tool evaluation.

Result: LIT improves LLMs' reliability and transparency while maintaining task performance, as demonstrated on mathematical, coding, and modeling problems.

Conclusion: The LIT framework enhances LLMs' trustworthiness by integrating reliable external tools, addressing opacity and unreliability issues.
...
### [125] [Structured Contrastive Learning for Interpretable Latent Representations](https://arxiv.org/abs/2511.14920)
*Zhengyang Shen,Hua Tu,Mayue Shi*

Main category: cs.LG

TL;DR: Neural networks are brittle to small transformations (e.g., ECG phase shifts, IMU rotations). Proposed Structured Contrastive Learning (SCL) partitions latent space into invariant, variant, and free features, improving robustness and interpretability without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Neural networks' brittleness to trivial transformations leads to performance drops. Current representation learning lacks constraints, causing latent spaces to evolve unpredictably despite task success.

Method: SCL divides latent space into invariant (consistent under transformations), variant (differentiates transformations), and free (task-flexible) features. It enhances contrastive learning with a variant mechanism.

Result: SCL boosts ECG similarity from 0.25 to 0.91 under phase shifts and achieves 86.65% accuracy with 95.38% rotation consistency in WISDM activity recognition, outperforming traditional augmentation.

Conclusion: SCL shifts from reactive data augmentation to proactive structural learning, offering interpretable and robust latent representations.
...
### [126] [Integrating Causal Inference with Graph Neural Networks for Alzheimer's Disease Analysis](https://arxiv.org/abs/2511.14922)
*Pranay Kumar Peddi,Dhrubajyoti Ghosh*

Main category: cs.LG

TL;DR: Causal-GCN is a causal graph convolutional framework designed to identify stable causal influences on Alzheimer's disease progression by adjusting for confounders like age, sex, and APOE4 genotype.


<details>
  <summary>Details</summary>
Motivation: Current deep graph learning models for Alzheimer's disease classification are correlational and confound demographic/genetic factors with disease-specific features. A causal approach is needed.

Method: The framework uses do-calculus-based back-door adjustment on structural connectomes derived from MRI data, simulating interventions on brain regions to estimate causal effects.

Result: Causal-GCN matches baseline GNN performance while providing interpretable causal effect rankings, highlighting brain regions consistent with AD neuropathology.

Conclusion: Causal-GCN offers a causal and interpretable approach for Alzheimer's disease classification, addressing limitations of correlational models.
...
### [127] [How to Train Private Clinical Language Models: A Comparative Study of Privacy-Preserving Pipelines for ICD-9 Coding](https://arxiv.org/abs/2511.14936)
*Mathieu Dufour,Andrew Duncan*

Main category: cs.LG

TL;DR: Comparison of four privacy-preserving training pipelines for clinical NLP shows knowledge distillation outperforms others at moderate privacy budgets.


<details>
  <summary>Details</summary>
Motivation: Assessing privacy risks and utility trade-offs in clinical NLP tasks using large language models.

Method: Systematic comparison of four training pipelines (including DP-SGD and DP-synthetic data) for diagnostic coding.

Result: Knowledge distillation recovers up to 63% of non-private performance and maintains strong privacy.

Conclusion: Knowledge distillation is the most effective method for privacy-preserving clinical NLP.
...
### [128] [Knowledge Graphs as Structured Memory for Embedding Spaces: From Training Clusters to Explainable Inference](https://arxiv.org/abs/2511.14961)
*Artur A. Oliveira,Mateus Espadoto,Roberto M. Cesar,Roberto Hirata*

Main category: cs.LG

TL;DR: Graph Memory (GM) is a structured non-parametric framework integrating prototype-based reasoning and graph relations for embedding-based inference, achieving competitive accuracy with better calibration and efficiency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses limitations of traditional embedding-based methods by unifying instance retrieval, prototype reasoning, and label propagation into a single inductive model.

Method: GM summarizes embedding space into prototype nodes with reliability indicators and relational edges, combining geometric and contextual information.

Result: GM matches $k$NN and Label Spreading in accuracy but improves calibration and decision boundaries with fewer samples.

Conclusion: GM bridges local evidence and global consistency, offering efficient, explainable non-parametric learning.
...
### [129] [IonCast: A Deep Learning Framework for Forecasting Ionospheric Dynamics](https://arxiv.org/abs/2511.15004)
...
TL;DR: IonCast is a deep learning suite that forecasts ionospheric Total Electron Content (TEC) using spatiotemporal learning, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Accurate forecasting of ionospheric variability is crucial for GNSS, communications, and aviation.

Method: Uses GraphCast-inspired deep learning models integrating spatiotemporal learning and diverse datasets.

Result: Shows improved skill over persistence in validation under storm-time and quiet conditions.

Conclusion: Demonstrates ML's potential to enhance understanding and resilience in space weather forecasting.
...
### [130] [Simulated Human Learning in a Dynamic, Partially-Observed, Time-Series Environment](https://arxiv.org/abs/2511.15032)
*Jeffrey Jiang,Kevin Hong,Emily Kuczynski,Gregory Pottie*

Main category: cs.LG

TL;DR: The paper develops a dynamic ITS using reinforcement learning and probing interventions to balance student estimation accuracy and disruption, comparing RL and heuristic approaches.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of personalizing education in ITSs despite the uniqueness of each student and the partial observability of learning.

Method: A simulated classroom environment with interventions (tutoring, lectures, exams) is created. Reinforcement learning ITSs are developed, incorporating probing interventions to estimate student states.

Result: RL and heuristic approaches yield similar results. Probing interventions improve accuracy but require balancing disruption. RL struggles with harder classes, and quiz/midterm structures benefit more from policies.

Conclusion: Probing interventions enhance ITS efficacy, though balancing cost and benefit is crucial. RL and heuristics perform comparably, with RL limitations in harder classes. Additional information (e.g., quizzes) boosts policy effectiveness.
...
### [131] [Oversampling techniques for predicting COVID-19 patient length of stay](https://arxiv.org/abs/2511.15048)
*Zachariah Farahany,Jiawei Wu,K M Sajjadul Islam,Praveen Madiraju*

Main category: cs.LG

TL;DR: The paper analyzes COVID-19 patients' EHR to predict disease severity (measured by LOS) using oversampling and an ANN with Bayesian optimization for hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: COVID-19 severity varies widely, and predicting it via EHR can aid healthcare planning. Imbalanced LOS data poses a challenge, addressed here.

Method: Oversampled training data is created synthetically, processed by an ANN with hyperparameters tuned via Bayesian optimization, and evaluated by F1 score.

Result: The model's performance is assessed based on F1 score, though specific results aren't detailed in the abstract.

Conclusion: The approach effectively tackles imbalanced data for predicting COVID-19 severity using EHR and ANN, suggesting potential healthcare utility.
...
### [132] [Interpretable temporal fusion network of multi- and multi-class arrhythmia classification](https://arxiv.org/abs/2511.15062)
*Yun Kwan Kim*

Main category: cs.LG

TL;DR: The paper proposes a CDSS framework combining local-global extraction and fusion with attention for arrhythmia classification, achieving high accuracy on MITDB and AFDB databases.


<details>
  <summary>Details</summary>
Motivation: Existing CDSSs struggle with varying arrhythmia lengths and onset times, lacking consideration for these variations.

Method: The framework involves local-global extraction and fusion with attention to handle constrained input lengths.

Result: Achieved F1-scores of 96.45%-97.57% on MITDB and AFDB, outperforming benchmarks.

Conclusion: The method improves arrhythmia detection accuracy and timing precision, aiding clinical treatment planning.
...
### [133] [Deep Pathomic Learning Defines Prognostic Subtypes and Molecular Drivers in Colorectal Cancer](https://arxiv.org/abs/2511.15067)
*Zisong Wang,Xuanyu Wang,Hang Chen,Haizhou Wang,Yuxin Chen,Yihang Xu,Yunhe Yuan,Lihuan Luo,Xitong Ling,Xiaoping Liu*

Main category: cs.LG

TL;DR: TDAM-CRC, an AI-driven model using histopathological images and multi-omics data, outperforms conventional staging for CRC prognosis, identifies MRPL37 as a key biomarker, and offers personalized risk stratification.


<details>
  <summary>Details</summary>
Motivation: Addressing the inadequacy of the TNM staging system for CRC's heterogeneity and aiming for precise, personalized prognostic tools.

Method: Developed TDAM-CRC using histopathological images and multi-omics data, trained on TCGA (n=581), validated externally (n=1031).

Result: TDAM-CRC achieved superior prognostic performance, identified MRPL37 as a key biomarker, and linked high-risk subtypes to metabolic and immune dysregulation.

Conclusion: TDAM-CRC enhances CRC prognosis, reveals molecular insights, and supports personalized clinical decisions.
...
### [134] [Efficient RF Passive Components Modeling with Bayesian Online Learning and Uncertainty Aware Sampling](https://arxiv.org/abs/2511.15125)
*Huifan Zhang,Pingqiang Zhou*

Main category: cs.LG

TL;DR: An uncertainty-aware Bayesian online learning framework is introduced for efficient parametric modeling of RF passive components, reducing EM simulation time significantly.


<details>
  <summary>Details</summary>
Motivation: Traditional RF passive component modeling using ML requires extensive EM simulations, leading to computational bottlenecks.

Method: The framework uses a Bayesian neural network with reconfigurable heads for joint geometric-frequency domain modeling and an adaptive sampling strategy guided by uncertainty.

Result: The method achieves accurate modeling with only 2.86% of traditional EM simulation time, providing a 35 times speedup.

Conclusion: The proposed framework offers a highly efficient and accurate alternative to conventional ML-based modeling for RF passive components.
...
### [135] [Novel sparse matrix algorithm expands the feasible size of a self-organizing map of the knowledge indexed by a database of peer-reviewed medical literature](https://arxiv.org/abs/2511.15136)
*Andrew Amos,Joanne Lee,Tarun Sen Gupta,Bunmi S. Malau-Aduli*

Main category: cs.LG

TL;DR: A novel sparse matrix multiplication algorithm was developed to create a self-organizing map for the entire Medline dataset, overcoming previous computational limitations and enabling updates over time.


<details>
  <summary>Details</summary>
Motivation: Past methods couldn't handle the full Medline dataset due to high computational demands, limiting the scope of medical knowledge mapping.

Method: The paper introduces a new algorithm for sparse matrix multiplication to efficiently process the entire dataset and create a self-organizing map.

Result: The algorithm successfully mapped the Medline dataset, providing a comprehensive view of medical knowledge and allowing for future refinements.

Conclusion: The new approach addresses scalability issues and enables dynamic updates, enhancing the utility of Medline data mapping.
...
### [136] [From Solving to Verifying: A Unified Objective for Robust Reasoning in LLMs](https://arxiv.org/abs/2511.15137)
*Xiaoxuan Wang,Bo Liu,Song Jiang,Jingzhou Liu,Jingyuan Qi,Xia Chen,Baosheng He*

Main category: cs.LG

TL;DR: GRPO-Verif, a new algorithm, improves LLMs' self-verification and reasoning by optimizing both in a unified loss function.


<details>
  <summary>Details</summary>
Motivation: Despite RL improving LLMs' reasoning, they still struggle with consistent self-verification. This work explores enhancing this ability and its impact on reasoning.

Method: GRPO-Verif jointly optimizes solution generation and self-verification using a unified loss function with an adjustable verification weight.

Result: The method boosts self-verification while maintaining reasoning performance, as shown in experiments.

Conclusion: GRPO-Verif effectively enhances self-verification in LLMs without compromising reasoning, answering the research question positively.
...
### [137] [Cross-Modal Consistency-Guided Active Learning for Affective BCI Systems](https://arxiv.org/abs/2511.15138)
*Hyo-Jeong Jang,Hye-Bin Shin,Kang Yin*

Main category: cs.LG

TL;DR: The paper introduces an uncertainty-aware active learning framework for EEG-based emotion recognition to address challenges like label noise and EEG signal corruption.


<details>
  <summary>Details</summary>
Motivation: EEG signals and emotional labels are prone to noise and variability, making robust emotion recognition difficult. The goal is to improve reliability despite these issues.

Method: An uncertainty-aware active learning framework jointly leverages model uncertainty and cross-modal consistency (EEG and face features) to distinguish cognitive ambiguity from sensor noise. A representation alignment module enforces semantic coherence between modalities.

Result: The method selectively queries noisy samples for feedback, improving robustness. Experiments on the ASCERTAIN dataset show its efficiency and noise tolerance.

Conclusion: The proposed framework offers a data-efficient, noise-tolerant solution for EEG-based affective decoding in brain-computer interfaces.
...
### [138] [Complex variational autoencoders admit Kähler structure](https://arxiv.org/abs/2511.15172)
*Andrew Gracyk*

Main category: cs.LG

TL;DR: The paper explores the Riemannian structure in complex variational autoencoders (VAEs) and demonstrates its connection to Kähler geometry. It introduces efficient methods for deriving the Fisher information metric and proposes regularization techniques for smoother latent representations.


<details>
  <summary>Details</summary>
Motivation: The study aims to extend existing arguments about Riemannian structure in latent-Euclidean VAEs to complex VAEs, uncovering Kähler geometric properties and improving computational efficiency.

Method: The authors derive the Fisher information metric for complex VAEs under Gaussian regularization with trivial relation matrices. They propose a Kähler potential derivative for complex Gaussian mixtures, enabling efficient computation and validating it as a plurisubharmonic function.

Result: The methods yield smoother latent representations and fewer semantic outliers by leveraging decoder geometry and weighted complex volume elements for sampling.

Conclusion: The paper successfully links Kähler geometry to complex VAEs, offering practical computational advantages and improved latent space regularization.
...
### [139] [FaultDiffusion: Few-Shot Fault Time Series Generation with Diffusion Model](https://arxiv.org/abs/2511.15174)
*Yi Xu,Zhigang Chen,Rui Wang,Yangfan Li,Fengxiao Tang,Ming Zhao,Jiaqi Liu*

Main category: cs.LG

TL;DR: Proposed a diffusion model-based framework for few-shot fault time-series generation in industrial monitoring, addressing scarcity of fault data and outperforming traditional methods in authenticity and diversity.


<details>
  <summary>Details</summary>
Motivation: Fault diagnosis is crucial for industrial reliability, but fault data scarcity hinders data-driven approaches due to rare events and costly annotations. Existing models struggle with few-shot fault scenarios.

Method: Uses diffusion models with a positive-negative difference adapter to model discrepancies between normal and fault domains. Introduces diversity loss to prevent mode collapse and enhance sample diversity.

Result: Outperforms traditional methods in authenticity and diversity, achieving state-of-the-art performance on benchmarks.

Conclusion: The framework effectively generates diverse and authentic fault samples in few-shot scenarios, advancing predictive maintenance.
...
### [140] [Vehicle Routing Problems via Quantum Graph Attention Network Deep Reinforcement Learning](https://arxiv.org/abs/2511.15175)
*Le Tung Giang,Vu Hoang Viet,Nguyen Xuan Tung,Trinh Van Chien,Won-Joo Hwang*

Main category: cs.LG

TL;DR: The paper introduces a Quantum Graph Attention Network (Q-GAT) within a DRL framework to solve the VRP, reducing parameters and improving performance compared to classical GATs.


<details>
  <summary>Details</summary>
Motivation: The vehicle routing problem (VRP) is NP-hard and critical for logistics. Existing DRL models with GNNs are parameter-heavy; Q-GAT aims to address this inefficiency.

Method: Q-GAT replaces traditional MLPs with parameterized quantum circuits (PQCs) in graph attention networks, reducing parameters by over 50%. It uses PPO with greedy/stochastic decoding.

Result: Q-GAT achieves faster convergence and reduces routing costs by ~5% compared to classical GAT baselines on VRP benchmarks.

Conclusion: The hybrid Q-GAT model shows promise as a compact and effective solver for large-scale routing problems, leveraging quantum-enhanced GNNs.
...
### [141] [Masked Auto-Regressive Variational Acceleration: Fast Inference Makes Practical Reinforcement Learning](https://arxiv.org/abs/2511.15190)
*Yuxuan Gu,Weimin Bai,Yifei Wang,Weijian Luo,He Sun*

Main category: cs.LG

TL;DR: MARVAL accelerates masked auto-regressive diffusion models by distilling the diffusion chain into a single step, enabling faster inference and practical RL post-training.


<details>
  <summary>Details</summary>
Motivation: Vanilla MAR models suffer from slow inference and impractical RL post-training due to their hierarchical structure. MARVAL addresses this by simplifying the diffusion chain.

Method: MARVAL uses distillation to compress the diffusion chain into a single AR step and introduces MARVAL-RL for efficient RL post-training.

Result: MARVAL achieves a 30x speedup with an FID of 2.00 on ImageNet and improves CLIP/image-reward scores via MARVAL-RL.

Conclusion: MARVAL provides a practical solution for fast sampling and better preference alignment in masked auto-regressive diffusion models.
...
### [142] [Reasoning in Diffusion Large Language Models is Concentrated in Dynamic Confusion Zones](https://arxiv.org/abs/2511.15208)
*Ranfei Chen,Ming Chen,Kaifei Wang*

Main category: cs.LG

TL;DR: ATPO dynamically focuses gradient updates on critical 'zones of confusion' in dLLM trajectories, improving reasoning accuracy and training stability without extra compute.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods treat all denoising steps equally, ignoring varying importance. Analyzing uncertainty metrics reveals critical steps ('zones of confusion') that strongly predict outcomes.

Method: ATPO uses step-selection based on hybrid RoEC+CM metrics to reallocate gradient updates adaptively to high-leverage steps, preserving the RL objective and budget.

Result: ATPO significantly enhances reasoning accuracy and training stability across benchmarks.

Conclusion: Exploiting trajectory dynamics through adaptive gradient reallocation is crucial for advancing dLLM RL.
...
### [143] [D2D Power Allocation via Quantum Graph Neural Network](https://arxiv.org/abs/2511.15246)
*Tung Giang Le,Xuan Tung Nguyen,Won-Joo Hwang*

Main category: cs.LG

TL;DR: A quantum Graph Neural Network (QGNN) using Parameterized Quantum Circuits (PQCs) matches classical performance in wireless resource management with fewer parameters and inherent parallelism.


<details>
  <summary>Details</summary>
Motivation: Increasing complexity in wireless networks requires scalable resource management solutions, and classical GNNs are computationally expensive for large-scale applications.

Method: The paper introduces Quantum Graph Convolutional Layers (QGCLs) that encode features into quantum states, process graphs with NISQ-compatible unitaries, and retrieve embeddings via measurement.

Result: Applied to D2D power control for SINR maximization, the QGNN achieves performance comparable to classical methods while using fewer parameters and leveraging quantum parallelism.

Conclusion: The QGNN represents progress toward quantum-accelerated optimization in wireless networks, demonstrating practical utility for scalable resource management.
...
### [144] [EntroPIC: Towards Stable Long-Term Training of LLMs via Entropy Stabilization with Proportional-Integral Control](https://arxiv.org/abs/2511.15248)
*Kai Yang,Xin Xu,Yangkun Chen,Weijie Liu,Jiafei Lyu,Zichuan Lin,Deheng Ye,Saiyong Yang*

Main category: cs.LG

TL;DR: The paper proposes EntroPIC, a method to stabilize entropy in LLM training by dynamically adjusting loss coefficients for positive and negative samples, ensuring stable exploration and optimal performance.


<details>
  <summary>Details</summary>
Motivation: Maintaining stable exploration in LLM training is challenging due to fluctuating entropy caused by mixed positive and negative samples.

Method: EntroPIC uses Proportional-Integral Control to dynamically tune loss coefficients for samples, stabilizing entropy adaptively.

Result: Theoretical and experimental results confirm EntroPIC effectively maintains desired entropy levels, enabling stable RL training for LLMs.

Conclusion: EntroPIC is a practical solution for entropy stabilization in large-scale LLM training, ensuring efficient exploration and optimal outcomes.
...
### [145] [Optimized scheduling of electricity-heat cooperative system considering wind energy consumption and peak shaving and valley filling](https://arxiv.org/abs/2511.15250)
*Jin Ye,Lingmei Wang,Shujian Zhang,Haihang WU*

Main category: cs.LG

TL;DR: An intelligent scheduling method using PVTD3 improves cost efficiency and grid stability in combined power-heat systems under renewable energy integration.


<details>
  <summary>Details</summary>
Motivation: Address the scheduling optimization challenges in combined power-heat systems with renewable energy and uncertainties.

Method: Proposes PVTD3 algorithm with a penalty term for grid power purchase variations.

Result: Reduces costs by 6.93-13.59%, lowers power purchase fluctuations by 12.8%, and improves energy storage management.

Conclusion: PVTD3 excels in economic efficiency, grid stability, and sustainable energy storage scheduling.
...
### [146] [PLATONT: Learning a Platonic Representation for Unified Network Tomography](https://arxiv.org/abs/2511.15251)
*Chengze Du,Heng Xu,Zhiwei Yu,Bo Liu,Jialong Li*

Main category: cs.LG

TL;DR: PLATONT is a unified framework for network tomography that models various network indicators as projections of a shared latent state, improving generalization and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing methods solve network tomography problems separately and rely on limited task-specific signals, limiting generalization and interpretability.

Method: PLATONT models different network indicators as projections of a shared latent network state, learned via multimodal alignment and contrastive learning.

Result: Experiments show PLATONT outperforms existing methods in link estimation, topology inference, and traffic prediction, with higher accuracy and robustness.

Conclusion: PLATONT provides a unified, effective solution for network tomography, enhancing performance across multiple tasks.
...
### [147] [GRPO-RM: Fine-Tuning Representation Models via GRPO-Driven Reinforcement Learning](https://arxiv.org/abs/2511.15256)
*Yanchen Xu,Ziheng Jiao,Hongyuan Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: GRPO-RM adapts GRPO reinforcement learning to representation models, using predefined output sets and specialized rewards, validated by experiments.


<details>
  <summary>Details</summary>
Motivation: To generalize GRPO's effectiveness from LLMs to representation learning models.

Method: Introduces GRPO-RM, replacing token sampling with predefined output sets and designing a specialized reward function.

Result: Extensive experiments demonstrate GRPO-RM's effectiveness in representation models.

Conclusion: GRPO-RM successfully extends GRPO's optimization approach to representation learning.
...
### [148] [SNAP: Low-Latency Test-Time Adaptation with Sparse Updates](https://arxiv.org/abs/2511.15276)
*Hyeongheon Cha,Dong Min Kim,Hye Won Chung,Taesik Gong,Sung-Ju Lee*

Main category: cs.LG

TL;DR: SNAP is a sparse Test-Time Adaptation (TTA) framework designed for resource-constrained edge environments. It reduces adaptation frequency and data usage by leveraging representative samples and dynamic normalization, achieving high efficiency with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods require frequent adaptation and high computational costs, making them impractical for edge devices with limited resources. SNAP addresses this by optimizing adaptation efficiency without compromising accuracy.

Method: SNAP introduces two components: (i) Class and Domain Representative Memory (CnDRM) for storing key samples, and (ii) Inference-only Batch-aware Memory Normalization (IoBMN) for dynamic normalization adjustments. These enable efficient adaptation using only 1% of the data stream.

Result: SNAP reduces latency by up to 93.12% while maintaining accuracy drops below 3.3%, even with adaptation rates as low as 1%. It integrates seamlessly with existing TTA algorithms.

Conclusion: SNAP demonstrates strong potential for practical deployment on edge devices, especially for latency-sensitive applications, by balancing efficiency and accuracy.
...
### [149] [Quant-Trim in Practice: Improved Cross-Platform Low-Bit Deployment on Edge NPUs](https://arxiv.org/abs/2511.15300)
*Rayen Dhahri,Steffen Urban*

Main category: cs.LG

TL;DR: Quant-Trim is a hardware-neutral training-phase method that ensures consistent accuracy across edge accelerators by combining progressive fake quantization and reverse pruning.


<details>
  <summary>Details</summary>
Motivation: Edge accelerators with low-bit quantization suffer from inconsistent accuracy due to varying vendor compiler implementations, requiring manual tweaks or refactoring.

Method: Quant-Trim uses progressive fake quantization to align training with deployed integer grids and reverse pruning to control outlier-driven scale inflation without sacrificing learnability.

Result: Quant-Trim reduces the gap between floating-point and low-bit accuracy, minimizes reliance on compiler heuristics, and eliminates the need for per-backend retraining.

Conclusion: Quant-Trim offers a vendor-agnostic solution for edge accelerators, improving accuracy consistency and efficiency across varying quantization schemes and backend choices.
...
### [150] [On the Internal Semantics of Time-Series Foundation Models](https://arxiv.org/abs/2511.15324)
*Atharva Pandey,Abhilash Neog,Gautam Jajoo*

Main category: cs.LG

TL;DR: The paper investigates how Time-series Foundation Models (TSFMs) represent concepts internally, finding that early layers capture local patterns while deeper layers handle more complex signals. Linear recoverability varies, and composition of concepts poses challenges.


<details>
  <summary>Details</summary>
Motivation: To understand the internal mechanisms of TSFMs and how they encode fundamental time-series concepts, addressing gaps in current knowledge.

Method: The study uses layer-wise analyses, linear recoverability tests, and representation similarity measures to probe concept encoding in TSFMs.

Result: Early layers encode local patterns (e.g., trends), while deeper layers handle complex signals (e.g., spectral factors). Linear recoverability is uneven, and compositional settings show interference between concepts.

Conclusion: TSFMs reliably localize atomic concepts but struggle with composition, revealing limitations in representing interacting temporal phenomena.
...
### [151] [KrawtchoukNet: A Unified GNN Solution for Heterophily and Over-smoothing with Adaptive Bounded Polynomials](https://arxiv.org/abs/2511.15327)
*Huseyin Goksu*
...
<details>
  <summary>Details</summary>
Motivation: Standard spectral GNNs like ChebyNet suffer from performance collapse on heterophilic graphs and over-smoothing at high polynomial degrees. The static, low-pass nature of these filters limits their adaptability and robustness.

Method: Proposes `KrawtchoukNet`, a GNN filter based on Krawtchouk polynomials, with two key innovations: 1) fixing the polynomial domain N to a small constant for bounded recurrence coefficients, enhancing robustness to over-smoothing; 2) making the shape parameter p learnable to adapt the filter's spectral response.

Result: Achieves state-of-the-art (SOTA) robustness against over-smoothing (effective at K=10) and superior performance on heterophilic benchmarks (Texas, Cornell), outperforming GAT and APPNP.

Conclusion: `KrawtchoukNet` addresses limitations of standard spectral GNNs through bounded recurrence coefficients and adaptive filtering, delivering robust and adaptable performance.
...
### [152] [LaguerreNet: Advancing a Unified Solution for Heterophily and Over-smoothing with Adaptive Continuous Polynomials](https://arxiv.org/abs/2511.15328)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: LaguerreNet introduces a novel GNN filter using continuous Laguerre polynomials to address heterophilic graph performance and over-smoothing, achieving state-of-the-art results and robustness.


<details>
  <summary>Details</summary>
Motivation: Standard spectral GNN filters struggle with heterophilic graphs and over-smoothing at high polynomial degrees, highlighting the need for adaptive solutions.

Method: LaguerreNet employs continuous Laguerre polynomials with a trainable alpha parameter and uses LayerNorm for numerical stability.

Result: LaguerreNet outperforms on heterophilic benchmarks and remains robust against over-smoothing up to K=10.

Conclusion: LaguerreNet effectively addresses key limitations of spectral GNNs, offering a stable and high-performing adaptive filter solution.
...
### [153] [Multi-layer Stack Ensembles for Time Series Forecasting](https://arxiv.org/abs/2511.15350)
*Nathanael Bosch,Oleksandr Shchur,Nick Erickson,Michael Bohlke-Schneider,Caner Türkmen*

Main category: cs.LG

TL;DR: Exploring ensembling strategies for time series forecasting, this paper evaluates 33 ensemble models and finds stacking improves accuracy. A multi-layer stacking framework is proposed for superior performance.


<details>
  <summary>Details</summary>
Motivation: Ensemble methods are underutilized in time series forecasting despite their success in tabular tasks, prompting systematic evaluation of existing and novel strategies.

Method: Evaluated 33 ensemble models on 50 datasets, proposed a multi-layer stacking framework combining strengths of different stacker models.

Result: Stacking consistently improves accuracy, though no single stacker excels universally; multi-layer stacking provides superior accuracy across scenarios.

Conclusion: Stacking-based methods enhance AutoML systems for time series forecasting, with multi-layer stacking offering robust performance.
...
### [154] [Cost-Aware Prediction (CAP): An LLM-Enhanced Machine Learning Pipeline and Decision Support System for Heart Failure Mortality Prediction](https://arxiv.org/abs/2511.15357)
*Yinan Yu,Falk Dippel,Christina E. Lundberg,Martin Lindgren,Annika Rosengren,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: This paper introduces a CAP framework combining ML predictions with cost-benefit analysis, enhanced by LLM agents, to improve clinical decision-making transparency.


<details>
  <summary>Details</summary>
Motivation: To address the gap in ML predictive models by incorporating downstream value trade-offs and clinical interpretability for heart failure patients.

Method: Developed an XGB model for 1-year mortality prediction, introduced CIP curves for cost visualization, and used LLM agents for patient-specific descriptions, evaluated by clinicians.

Result: XGB model achieved AUROC 0.804, AUPRC 0.529, Brier score 0.135. CIP curves and LLM agents provided useful cost insights, positively received by clinicians.

Conclusion: CAP integrates ML and LLM-assisted cost-benefit analysis for transparent, interpretable clinical decision support, though further technical accuracy improvements are needed.
...
### [155] [CID: Measuring Feature Importance Through Counterfactual Distributions](https://arxiv.org/abs/2511.15371)
*Eddie Conti,Álvaro Parafita,Axel Brando*

Main category: cs.LG

TL;DR: The paper introduces CID, a novel local feature importance method using counterfactuals and distributional dissimilarity to improve faithfulness in ML model explanations.


<details>
  <summary>Details</summary>
Motivation: Existing feature importance methods lack definitive ground truth, necessitating alternative, rigorous measures for understanding model decisions.

Method: CID generates positive/negative counterfactuals, models their distributions with Kernel Density Estimation, and ranks features via a distributional dissimilarity measure.

Result: CID outperforms existing methods in faithfulness metrics (comprehensiveness/sufficiency) and provides complementary perspectives.

Conclusion: CID is a robust tool for analyzing ML models, offering more faithful explanations and addressing gaps in feature importance evaluation.
...
### [156] [Parameter Importance-Driven Continual Learning for Foundation Models](https://arxiv.org/abs/2511.15375)
*Lingxiang Wang,Hainan Zhang,Zhiming Zheng*

Main category: cs.LG

TL;DR: PIECE is a method that selectively updates only 0.1% of core parameters to prevent catastrophic forgetting while efficiently learning domain knowledge, without needing prior data or extra parameters.


<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting in foundation models during domain-specific post-training, preserving general reasoning abilities while adapting to new tasks.

Method: PIECE uses two parameter importance estimators (PIECE-F and PIECE-S) to selectively update core parameters relevant to new tasks.

Result: PIECE maintains general capabilities and achieves state-of-the-art continual learning performance across diverse tasks.

Conclusion: PIECE offers a practical solution for scalable, domain-adaptive foundation models without catastrophic forgetting.
...
### [157] [EVA-Net: Interpretable Brain Age Prediction via Continuous Aging Prototypes from EEG](https://arxiv.org/abs/2511.15393)
*Kunyu Zhang,Mingxuan Wang,Xiangjie Shi,Haoxing Xu,Chao Zhang*

Main category: cs.LG

TL;DR: EVA-Net is a novel, interpretable framework for brain age estimation and anomaly detection using EEG data, addressing challenges like imperfect data and lack of interpretability in existing models.


<details>
  <summary>Details</summary>
Motivation: Existing EEG-based brain age models struggle with imperfect data and lack interpretability, which is crucial for healthcare applications.

Method: EVA-Net uses a sparsified-attention Transformer for long EEG sequences, a Variational Information Bottleneck for robust representation, and aligns to a prototype network for interpretability.

Result: Achieved state-of-the-art accuracy on healthy subjects and successfully detected anomalies in MCI and AD patients via higher brain-age gaps and Prototype Alignment Error.

Conclusion: EVA-Net offers an interpretable and accurate solution for brain age estimation and anomaly detection in healthcare.
...
### [158] [Proximal Approximate Inference in State-Space Models](https://arxiv.org/abs/2511.15409)
*Hany Abdulsamad,Ángel F. García-Fernández,Simo Särkkä*

Main category: cs.LG

TL;DR: The paper introduces variational Lagrangian-based algorithms for state estimation in nonlinear, non-Gaussian models, focusing on entropic trust-region updates and dynamic constraints.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in Bayesian inference for nonlinear, non-Gaussian state-space models by leveraging variational methods.

Method: Uses a variational Lagrangian framework with entropic trust-region updates and dynamic constraints, employing Gauss--Markov approximations and recursive schemes.

Result: Derives forward-backward algorithms with favorable computational complexity and closes recursions using generalized statistical linear regression and Fourier--Hermite moment matching.

Conclusion: Offers efficient recursive schemes for nonlinear, non-Gaussian state-space models through variational Bayesian inference.
...
### [159] [Towards Understanding Layer Contributions in Tabular In-Context Learning Models](https://arxiv.org/abs/2511.15432)
*Amir Rezaei Balef,Mykhailo Koshil,Katharina Eggensperger*

Main category: cs.LG

TL;DR: The paper explores layer contributions in tabular ICL models, identifies redundancies, and compares them to LLMs, revealing opportunities for compression and interpretability.


<details>
  <summary>Details</summary>
Motivation: Little is known about how individual layers contribute to tabular prediction despite similarities between tabular ICL models and LLMs.

Method: The study analyzes TabPFN and TabICL using a 'layers as painters' perspective to understand latent space evolution and layer redundancy.

Result: Only subsets of layers share a common representational language, indicating structural redundancy.

Conclusion: The findings suggest potential for model compression and improved interpretability in tabular ICL models.
...
### [160] [TSFM in-context learning for time-series classification of bearing-health status](https://arxiv.org/abs/2511.15447)
*Michel Tokic,Slobodan Djukanović,Anja von Beuningen,Cheng Feng*

Main category: cs.LG

TL;DR: The paper presents a classification method using in-context learning with time-series foundation models (TSFM) to classify data without finetuning, applied successfully to bearing health assessment.


<details>
  <summary>Details</summary>
Motivation: To enable classification of new data without finetuning TSFM, leveraging scalability for broader AI-driven maintenance systems.

Method: Uses in-context learning with prompts (targets and covariates) to classify unknown data patterns; transforms frequency domain signals into pseudo time-series.

Result: Applied to vibration data for bearing health assessment, showing efficacy across varied conditions.

Conclusion: This approach advances beyond narrow AI solutions towards scalable, versatile AI-driven maintenance.
...
### [161] [FairEnergy: Contribution-Based Fairness meets Energy Efficiency in Federated Learning](https://arxiv.org/abs/2511.15454)
*Ouiame Marnissi,Hajar EL Hammouti,El Houcine Bergou*

Main category: cs.LG

TL;DR: FairEnergy is a framework optimizing energy efficiency and fairness in federated learning by integrating client contributions into device selection, bandwidth allocation, and compression.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges of balancing energy efficiency, fair participation, and high accuracy in federated learning for wireless edge systems.

Method: FairEnergy uses a contribution score combined with joint optimization techniques (device selection, bandwidth allocation, compression) solved via relaxation and decomposition.

Result: Achieves higher accuracy and reduces energy consumption by up to 79% compared to baselines.

Conclusion: FairEnergy effectively balances fairness, energy efficiency, and accuracy in federated learning for heterogeneous edge systems.
...
### [162] [NTK-Guided Implicit Neural Teaching](https://arxiv.org/abs/2511.15487)
*Chen Zhang,Wei Zuo,Bingyang Cheng,Yikun Wang,Wei-Bin Kou,Yik Chung WU,Ngai Wong*

Main category: cs.LG

TL;DR: NINT accelerates INR training by dynamically selecting high-impact coordinates via NTK-guided scoring, achieving faster convergence and maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Optimizing high-resolution signals with INRs is computationally expensive due to millions of coordinates. NINT aims to reduce training time efficiently.

Method: NINT uses NTK to score coordinates based on loss gradients, considering fitting errors and leverage, enabling dynamic selection for faster updates.

Result: NINT reduces training time by nearly half while preserving or enhancing representation quality, outperforming existing sampling-based methods.

Conclusion: NINT offers a state-of-the-art solution for accelerating INR training with NTK-guided coordinate selection, balancing speed and accuracy.
...
### [163] [Sample-Adaptivity Tradeoff in On-Demand Sampling](https://arxiv.org/abs/2511.15507)
*Nika Haghtalab,Omar Montasser,Mingda Qiao*

Main category: cs.LG

TL;DR: The paper explores the tradeoff between sample and round complexity in on-demand sampling for Multi-Distribution Learning (MDL). It provides bounds for both realizable and agnostic settings and introduces a new framework, OODS, to analyze sample-adaptivity tradeoffs.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the interplay between sample efficiency and computational efficiency (round complexity) in adaptive sampling scenarios, particularly for MDL tasks.

Method: For the realizable MDL setting, the paper derives optimal sample complexity bounds as a function of rounds. For the agnostic case, it presents an algorithm achieving near-optimal sample complexity within a limited number of rounds. It also introduces the OODS framework to generalize and analyze existing MDL algorithms.

Result: In the realizable setting, sample complexity scales as $dk^{Θ(1/r)} / ε$. For the agnostic case, the algorithm achieves $	ilde O((d + k) / ε^2)$ sample complexity in $	ilde O(\sqrt{k})$ rounds. The OODS framework provides tight bounds on round complexity.

Conclusion: The results highlight the inherent tradeoffs in MDL and suggest that achieving sub-polynomial round complexity would require novel techniques beyond OODS.
...
### [164] [PCARNN-DCBF: Minimal-Intervention Geofence Enforcement for Ground Vehicles](https://arxiv.org/abs/2511.15522)
*Yinan Yu,Samuel Scheidegger*

Main category: cs.LG

TL;DR: PCARNN-DCBF integrates structured learning and verifiable control for runtime geofencing in ground vehicles, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing solutions struggle balancing high-fidelity learning with verifiable control for enforcing Operational Design Domains (ODDs).

Method: Introduces PCARNN-DCBF, combining a Physics-encoded Control-Affine Residual Neural Network with a preview-based Discrete Control Barrier Function.

Result: Outperforms analytical and unstructured neural baselines in CARLA experiments across vehicle platforms.

Conclusion: Structure-preserving learning ensures reliable optimization and real-time enforcement of geofencing constraints.
...
### [165] [Learning Interestingness in Automated Mathematical Theory Formation](https://arxiv.org/abs/2511.14778)
...
TL;DR: Ask WhAI is a framework for analyzing belief states in multi-agent interactions, tested in a medical case simulator to reveal disciplinary biases and belief dynamics.


<details>
  <summary>Details</summary>
Motivation: The study aims to inspect and perturb belief states in multi-agent systems, particularly in medical diagnostics, to understand how agents integrate evidence and maintain biases.

Method: The framework records and replays agent interactions, supports belief queries, and injects counterfactual evidence. It is tested using large language model agents simulating medical specialists interacting with a shared electronic medical record.

Result: Agents displayed biases like overreliance on canonical studies and resistance to counterevidence, mirroring real-world disciplinary stances. Belief dynamics were traced and interrogated effectively.

Conclusion: Ask WhAI provides a reproducible method to study belief formation and silos in multi-agent reasoning, offering insights into epistemic dynamics.
...
### [167] [Subnational Geocoding of Global Disasters Using Large Language Models](https://arxiv.org/abs/2511.14788)
*Michele Ronco,Damien Delforge,Wiebke S. Jäger,Christina Corbane*

Main category: cs.AI

TL;DR: An automated LLM-assisted workflow geocodes subnational disaster locations using GPT-4o and cross-checks multiple geoinformation repositories, achieving high reliability without manual intervention.


<details>
  <summary>Details</summary>
Motivation: Addressing the inconsistency and unstructured nature of disaster location data in databases like EM-DAT to improve risk assessment and disaster risk reduction.

Method: Uses GPT-4o to process textual location data and cross-checks GADM, OpenStreetMap, and Wikidata for geometry assignment, with reliability scoring.

Result: Geocoded 14,215 events across 17,948 unique locations from EM-DAT (2000-2024), demonstrating scalability and reliability.

Conclusion: The workflow offers a scalable, automated solution for extracting and structuring geographic information, enhancing disaster data integration.
...
### [168] [Project Rachel: Can an AI Become a Scholarly Author?](https://arxiv.org/abs/2511.14819)
*Martin Monperrus,Benoit Baudry,Clément Vidal*

Main category: cs.AI

TL;DR: Project Rachel investigates AI authorship in academia by creating 'Rachel So,' an AI academic identity that published 10+ papers and received citations and peer review invitations.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how the scholarly ecosystem responds to AI-generated research and authorship.

Method: Action research was conducted by creating an AI academic identity ('Rachel So') and tracking its publications (10+ papers) and interactions with the academic system.

Result: Rachel So's papers were cited, and it received a peer review invitation, demonstrating acceptance within the scholarly community.

Conclusion: The study highlights the implications of AI authorship for publishers, researchers, and the scientific system, contributing to debates about scholarly communication with advanced AI.
...
### [169] [Uncertainty-Aware Measurement of Scenario Suite Representativeness for Autonomous Systems](https://arxiv.org/abs/2511.14853)
*Robab Aghazadeh Chakherlou,Siddartha Khastgir,Xingyu Zhao,Jerein Jeyachandran,Shufeng Chen*

Main category: cs.AI

TL;DR: The paper proposes a probabilistic method using imprecise Bayesian techniques to quantify the representativeness of datasets for AI systems, focusing on autonomous vehicles. It provides interval-valued estimates to handle data limitations and uncertainties.


<details>
  <summary>Details</summary>
Motivation: Ensuring AI system safety requires datasets to reflect real-world operational conditions (TOD/ODD). Representativeness is critical but challenging due to unknown true TOD distributions and limited data.

Method: Uses probabilistic comparison of feature distributions between scenario suites and TOD. Applies imprecise Bayesian methods to handle data limitations and uncertain priors, yielding interval-valued representativeness estimates.

Result: Demonstrates local and global representativeness estimates (as intervals) across operational categories (weather, road type, etc.), incorporating dependencies and prior uncertainty.

Conclusion: The method provides a robust, uncertainty-aware way to assess dataset representativeness, aiding in trustworthy AI system development.
...
### [170] [Task Specific Sharpness Aware O-RAN Resource Management using Multi Agent Reinforcement Learning](https://arxiv.org/abs/2511.15002)
*Fatemeh Lotfi,Hossein Rajoli,Fatemeh Afghah*

Main category: cs.AI

TL;DR: A novel resource management method combines SAC with SAM in MARL, improving robustness and efficiency in dynamic O-RAN environments.


<details>
  <summary>Details</summary>
Motivation: Current DRL models lack robustness and generalizability in dynamic O-RAN networks, necessitating a more adaptive approach.

Method: Enhanced SAC with SAM in MARL, using TD-error variance-driven regularization and dynamic ρ scheduling for exploration.

Result: Achieves up to 22% better resource allocation efficiency and superior QoS satisfaction in O-RAN slices.

Conclusion: The proposed method significantly outperforms traditional DRL approaches, offering improved stability and generalization.
...
### [171] [Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization](https://arxiv.org/abs/2511.15055)
*Jian-Ting Guo,Yu-Cheng Chen,Ping-Chun Hsieh,Kuo-Hao Ho,Po-Wei Huang,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: The paper introduces Macro Action Quantization (MAQ), a reinforcement learning framework to improve human-likeness in RL agents by optimizing trajectories to align with human behavior while maximizing rewards.


<details>
  <summary>Details</summary>
Motivation: The goal is to design RL agents that exhibit more natural, human-like behaviors to address concerns about interpretability and trustworthiness in reward-driven agents.

Method: The paper formulates human-likeness as trajectory optimization and introduces MAQ, which uses Vector-Quantized VAE to distill human demonstrations into macro actions. It adapts receding-horizon control for efficient implementation.

Result: Experiments on D4RL Adroit benchmarks show MAQ significantly improves human-likeness, achieving higher trajectory similarity scores and top rankings in human evaluations. It also integrates easily with existing RL algorithms.

Conclusion: MAQ presents a promising direction for developing human-like RL agents, combining human demonstrations and efficient optimization techniques.
...
### [172] [Beyond GeneGPT: A Multi-Agent Architecture with Open-Source LLMs for Enhanced Genomic Question Answering](https://arxiv.org/abs/2511.15061)
*Haodong Chen,Guido Zuccon,Teerapong Leelanupab*

Main category: cs.AI

TL;DR: OpenBioLLM, a modular multi-agent framework using open-source models, matches or outperforms GeneGPT in genomic QA tasks while improving efficiency and reducing latency.


<details>
  <summary>Details</summary>
Motivation: Address limitations of GeneGPT (scalability, cost, privacy) by leveraging open-source models and enhancing reasoning with a multi-agent design.

Method: Reproduce GeneGPT with open-source models (Llama 3.1, Qwen2.5), then develop OpenBioLLM with specialized agents for tool routing, query generation, and validation.

Result: OpenBioLLM outperforms GeneGPT on 90% of tasks (avg scores: 0.849 Gene-Turing, 0.830 GeneHop), reduces latency by 40-50%, and improves efficiency.

Conclusion: Open-source multi-agent systems like OpenBioLLM show promise for scalable, efficient genomic question answering.
...
### [173] [ProRAC: A Neuro-symbolic Method for Reasoning about Actions with LLM-based Progression](https://arxiv.org/abs/2511.15069)
*Haoyong Wu,Yongmei Liu*

Main category: cs.AI

TL;DR: ProRAC is a neuro-symbolic framework using LLMs to solve Reasoning about Actions and Change (RAC) problems by extracting elements, executing actions progressively, and evaluating queries. It shows strong performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address RAC problems effectively by combining neuro-symbolic methods with LLMs.

Method: ProRAC extracts actions and questions, executes actions progressively to derive the final state, and evaluates queries.

Result: Demonstrates strong performance across various benchmarks, domains, and LLM backbones.

Conclusion: ProRAC successfully tackles RAC tasks through progressive reasoning and achieves robust results.
...
### [174] [Knowledge-Informed Automatic Feature Extraction via Collaborative Large Language Model Agents](https://arxiv.org/abs/2511.15074)
*Henrik Bradland,Morten Goodwin,Vladimir I. Zadorozhny,Per-Arne Andersen*

Main category: cs.AI

TL;DR: Rogue One is a multi-agent LLM-based framework for automated feature extraction, outperforming state-of-the-art methods by integrating domain knowledge and dynamic feedback mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current AutoFE methods lack systematic integration of domain knowledge and rely on simplistic feedback, limiting their effectiveness.

Method: Rogue One uses three specialized agents (Scientist, Extractor, Tester) with iterative collaboration, qualitative feedback, and a flooding-pruning strategy. External knowledge is incorporated via RAG.

Result: Outperforms existing methods on 19 classification and 9 regression datasets, and identifies novel hypotheses like a potential biomarker.

Conclusion: Rogue One advances AutoFE by balancing exploration-exploitation and enhancing interpretability, proving valuable for scientific discovery.
...
### [175] [SafeRBench: A Comprehensive Benchmark for Safety Assessment in Large Reasoning Models](https://arxiv.org/abs/2511.15169)
...
TL;DR: SafeRBench is introduced as the first benchmark assessing LRM safety end-to-end, addressing risks in inputs, reasoning, and outputs through structured evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Existing safety evaluations focus on outputs, missing dynamic risks in reasoning. SafeRBench aims to fill this gap by assessing safety comprehensively.

Method: Includes (1) input characterization with risk categories/levels, (2) fine-grained output analysis via micro-thought chunking, and (3) human-aligned validation.

Result: Evaluations on 19 LRMs show SafeRBench enables detailed, multidimensional safety insights.

Conclusion: SafeRBench provides a robust framework for assessing LRM safety risks and protective mechanisms holistically.
...
### [176] [HISE-KT: Synergizing Heterogeneous Information Networks and LLMs for Explainable Knowledge Tracing with Meta-Path Optimization](https://arxiv.org/abs/2511.15191)
*Zhiyi Duan,Zixing Shi,Hongyu Yuan,Qi Wang*

Main category: cs.AI

TL;DR: HISE-KT integrates heterogeneous information networks (HINs) with large language models (LLMs) to improve knowledge tracing by automating meta-path quality assessment and leveraging similar student retrieval for better predictions and explanations.


<details>
  <summary>Details</summary>
Motivation: Existing KT methods introduce noise due to manual meta-path selection or lack context from similar students. HISE-KT addresses these gaps by combining HINs and LLMs.

Method: HISE-KT constructs a multi-relationship HIN, uses LLMs to score and filter meta-paths, retrieves similar student trajectories, and generates predictions via structured prompts.

Result: Experiments on four datasets show HISE-KT outperforms baselines in prediction accuracy and interpretability.

Conclusion: HISE-KT successfully enhances KT by leveraging HINs and LLMs, delivering accurate predictions and explainable reports.
...
### [177] [As If We've Met Before: LLMs Exhibit Certainty in Recognizing Seen Files](https://arxiv.org/abs/2511.15192)
*Haodong Li,Jingqi Zhang,Xiao Cheng,Peihua Mai,Haoyu Wang,Yang Pan*

Main category: cs.AI

TL;DR: COPYCHECK is a framework using uncertainty signals to detect copyrighted content in LLM training sets, achieving high accuracy without needing empirically tuned thresholds.


<details>
  <summary>Details</summary>
Motivation: Address concerns about unauthorized use of copyrighted material in LLM training by overcoming limitations of existing Membership Inference Attacks (MIAs).

Method: Uses uncertainty patterns to distinguish seen/unseen content, segments files strategically, and employs unsupervised clustering.

Result: Achieves ~90% balanced accuracy on LLaMA models and performs well on GPT-J 6B, with significant improvement over baselines.

Conclusion: COPYCHECK provides a practical, uncertainty-based solution for detecting copyrighted training data in LLMs.
...
### [178] [SOLID: a Framework of Synergizing Optimization and LLMs for Intelligent Decision-Making](https://arxiv.org/abs/2511.15202)
*Yinsheng Wang,Tario G You,Léonard Boussioux,Shan Liu*

Main category: cs.AI

TL;DR: SOLID integrates optimization and LLMs for better decision-making, ensuring modularity, privacy, and convergence, validated by improved portfolio returns.


<details>
  <summary>Details</summary>
Motivation: To enhance decision-making by synergizing optimization and LLMs, leveraging their contextual strengths while maintaining privacy and modularity.

Method: SOLID uses dual prices and deviation penalties for iterative collaboration between optimization and LLMs, tested on stock portfolio investments.

Result: Empirical results show convergence and higher annualized returns compared to optimizer-only baselines.

Conclusion: SOLID proves effective for intelligent decision-making across domains, blending optimization and LLMs successfully.
...
### [179] [Efficiency Will Not Lead to Sustainable Reasoning AI](https://arxiv.org/abs/2511.15259)
*Philipp Wiesner,Daniel W. O'Neill,Francesca Larosa,Odej Kao*

Main category: cs.AI

TL;DR: The paper highlights that efficiency gains alone won't ensure sustainable reasoning AI due to its exponential compute demands, advocating for research and policy to impose limits.


<details>
  <summary>Details</summary>
Motivation: Address the unsustainable growth of reasoning AI's energy footprint as efficiency improvements near physical limits.

Method: Analyzes trends in AI development, focusing on multi-step reasoning and exponential compute scaling.

Result: Identifies lack of saturation points in reasoning AI, contrasting with historical efficiency-driven stability.

Conclusion: Calls for embedding explicit limits in AI optimization and governance to achieve sustainability.
...
### [180] [Realist and Pluralist Conceptions of Intelligence and Their Implications on AI Research](https://arxiv.org/abs/2511.15282)
*Ninell Oldenburg,Ruchira Dhar,Anders Søgaard*

Main category: cs.AI

TL;DR: The paper explores two competing views of intelligence in AI research—Intelligence Realism (universal measure) and Intelligence Pluralism (context-dependent)—and their impact on methodologies, interpretations, and risk assessments.


<details>
  <summary>Details</summary>
Motivation: Current AI research lacks clarity on implicit assumptions about intelligence, leading to divergent approaches and interpretations. The authors aim to highlight these differences to foster clearer discourse.

Method: The paper analyzes debates in AI research to reveal how implicit views of intelligence shape methodologies (e.g., benchmarks), interpretations (e.g., system limitations), and risk assessments (e.g., alignment solutions).

Result: The study shows that Intelligence Realism and Pluralism lead to distinct research approaches and contradictory conclusions, even when examining the same empirical evidence.

Conclusion: Explicitly acknowledging these underlying assumptions can clarify disagreements in AI research and improve methodological and interpretive coherence.
...
### [181] [Octopus: Agentic Multimodal Reasoning with Six-Capability Orchestration](https://arxiv.org/abs/2511.15351)
*Yifu Guo,Zishan Xu,Zhiyuan Yao,Yuquan Lu,Jiaye Lin,Sen Hu,Zhenheng Tang,Yingchao Li,Huacan Wang,Ronghao Chen*

Main category: cs.AI

TL;DR: The paper proposes Octopus, a multimodal reasoning paradigm with six core capabilities, addressing limitations in existing models by enabling autonomous exploration and dynamic capability selection.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal reasoning models lack adaptability and diverse reasoning pathways, unlike humans. The authors aim to bridge this gap.

Method: Introduces Octopus, defining six core capabilities and organizing Octopus-Bench for evaluation. It autonomously explores and selects capabilities.

Result: Octopus outperforms existing methods on most tasks in Octopus-Bench, showcasing effective capability coordination.

Conclusion: Capability coordination is vital for agentic multimodal reasoning, as demonstrated by Octopus's superior performance.
...
### [182] [Terra Nova: A Comprehensive Challenge Environment for Intelligent Agents](https://arxiv.org/abs/2511.15378)
*Trevor McInroe*

Main category: cs.AI

TL;DR: Terra Nova is a new RL challenge environment inspired by Civilization V, designed to test integrated, long-horizon understanding across multiple interacting challenges, unlike multitask benchmarks.


<details>
  <summary>Details</summary>
Motivation: To create a single environment where multiple canonical RL challenges arise simultaneously, requiring deep reasoning rather than task-switching.

Method: Developed Terra Nova as a comprehensive challenge environment (CCE) inspired by Civilization V.

Result: The environment successfully integrates challenges like partial observability, credit assignment, and enormous action spaces.

Conclusion: Terra Nova offers a more meaningful benchmark for RL agents by demanding integrated understanding across interacting variables.
...
### [183] [IPR-1: Interactive Physical Reasoner](https://arxiv.org/abs/2511.15407)
*Mingyu Zhang,Lifeng Zhuo,Tianxi Tan,Guocan Xie,Xian Nie,Yan Li,Renjie Zhao,Zizhu He,Ziyu Wang,Jiting Cai,Yong-Lu Li*

Main category: cs.AI

TL;DR: The paper explores whether an agent can achieve human-like reasoning through interaction and experience, proposing IPR (Interactive Physical Reasoner) for robust physical reasoning.


<details>
  <summary>Details</summary>
Motivation: To determine if agents can acquire human-like reasoning by interacting with diverse environments and improving with experience.

Method: Curating 1,000+ heterogeneous games, evaluating at Survival, Curiosity, Utility levels, and proposing IPR with PhysCode for physics-centric action alignment.

Result: IPR performs robustly, matches GPT-5 overall, surpasses it on Curiosity, and shows zero-shot transfer to unseen games.

Conclusion: Physics-centric interaction enables steady improvement in physical reasoning.
...
TL;DR: TIM is a framework to infer user intents in DeFi transactions using a multi-agent LLM system, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Understanding DeFi user intent is hard due to complex smart contracts, multi-modal data, and opaque logs; current methods lack semantic depth.

Method: TIM uses a DeFi intent taxonomy, multi-agent LLM coordination, Meta-Level Planner, Question Solvers, and Cognitive Evaluator.

Result: TIM outperforms ML models, single LLMs, and single-agent baselines.

Conclusion: TIM enhances DeFi user intent understanding with reliable, context-aware explanations.
...
### [185] [Exploring the use of AI authors and reviewers at Agents4Science](https://arxiv.org/abs/2511.15534)
*Federico Bianchi,Owen Queen,Nitya Thakkar,Eric Sun,James Zou*

Main category: cs.AI

TL;DR: Analysis of the Agents4Science conference highlights AI agents' roles as authors and reviewers in scientific research, exploring human-AI collaboration implications.


<details>
  <summary>Details</summary>
Motivation: To explore AI agents' capabilities in scientific roles and their potential collaboration with humans.

Method: Organized Agents4Science, where AI agents acted as primary authors and reviewers, with human co-authors and co-reviewers.

Result: Discussed key learnings about AI agents' performance and collaboration dynamics in scientific settings.

Conclusion: Insights from the conference offer valuable implications for enhancing human-AI collaboration in science.
...
### [186] [What Does It Take to Be a Good AI Research Agent? Studying the Role of Ideation Diversity](https://arxiv.org/abs/2511.15593)
...
TL;DR: Examining the role of ideation diversity in AI research agents, higher diversity correlates with better performance, as shown in MLE-bench and controlled experiments.


<details>
  <summary>Details</summary>
Motivation: Understanding the key factors influencing AI research agents' success is crucial for advancing automated scientific progress.

Method: Analyzed trajectories on MLE-bench across models/scaffolds, conducted controlled experiments modifying ideation diversity, and validated findings with additional metrics.

Result: Higher ideation diversity leads to improved agent performance, confirmed through multiple evaluation methods.

Conclusion: Ideation diversity is a significant driver of success for AI research agents, with implications for future agent design.
