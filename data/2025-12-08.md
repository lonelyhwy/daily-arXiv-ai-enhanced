...
TL;DR: AREA3D is an active 3D reconstruction framework that uses feed-forward models and vision-language guidance to improve accuracy and reduce redundant observations.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on hand-crafted geometric heuristics, leading to inefficiencies. AREA3D addresses this by incorporating advanced models and semantic guidance.

Method: The framework decouples uncertainty modeling from reconstruction and integrates vision-language models for diverse viewpoint selection.

Result: AREA3D outperforms benchmarks, especially in sparse-view scenarios, achieving state-of-the-art accuracy.

Conclusion: AREA3D demonstrates improved efficiency and accuracy in 3D reconstruction by leveraging advanced models and semantic guidance.
...
TL;DR: The paper identifies Scale Anchoring as a problem where models trained on low-resolution data fail to reduce errors when deployed on higher resolutions. It proposes Frequency Representation Learning to address this by improving frequency response stability and spectral consistency.


<details>
  <summary>Details</summary>
Motivation: Current models trained on low-resolution data incorrectly maintain similar error levels across resolutions, misinterpreted as successful generalization. The paper highlights the need for models to reduce error as resolution increases.

Method: The paper introduces Frequency Representation Learning (FRL), which enhances models with resolution-aligned frequency representations and spectral consistency training. This stabilizes high-frequency responses across resolutions.

Result: FRL significantly outperforms baselines, allowing errors to decrease with higher resolutions while adding minimal computational overhead.

Conclusion: Scale Anchoring is a distinct problem addressed effectively by FRL, enabling better performance in zero-shot super-resolution spatiotemporal forecasting.
...
TL;DR: InvarDiff speeds up diffusion models by exploiting temporal and layer-wise invariance, achieving 2-3x speed-ups with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Diffusion models are slow due to iterative sampling, and the paper aims to accelerate them without compromising fidelity.

Method: InvarDiff uses deterministic sampling to identify feature invariance, creating a binary cache plan matrix for reusing computations and applying corrections to avoid drift.

Result: Experiments show 2-3x speed-ups on DiT and FLUX models with negligible impact on quality metrics or visual output.

Conclusion: InvarDiff effectively reduces redundant computations in diffusion models while maintaining high fidelity.
...
TL;DR: An interpretable AI-ECG model was developed to predict severe coronary artery stenosis using ECG data, showing robust performance across validation sets and subgroups.


<details>
  <summary>Details</summary>
Motivation: Coronary CT angiography (CCTA) has limitations for widespread CAD diagnosis; AI-ECG offers a non-invasive, scalable alternative.

Method: Developed an AI-ECG model to predict stenosis in four major coronary arteries, validated internally and externally.

Result: AUCs ranged from 0.667 to 0.971 across arteries; model performed well even in normal-ECG subsets.

Conclusion: AI-ECG provides a reliable, interpretable tool for CAD screening, with potential clinical utility.
...
TL;DR: ChromouVQA is a benchmark using chromatic camouflaged images to evaluate VLMs and humans on tasks like recognition and spatial reasoning, revealing gaps, especially under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Address VLMs' struggle with figure-ground segregation in cluttered backgrounds using controlled chromatic camouflaged images.

Method: Develop ChromouVQA benchmark with Ishihara-style images, varying chromatic separation, density, etc., and propose a contrastive recipe aligning silhouettes with camouflaged renderings.

Result: Shows significant gaps between humans and VLMs, especially under subtle chromatic contrast or disruptive geometry.

Conclusion: ChromouVQA offers a reproducible benchmark for evaluating and improving VLMs' performance in challenging visual tasks.
...
TL;DR: The paper introduces a transfer-learning generative downscaling framework to create high-resolution satellite images from coarse inputs using a U-Net and diffusion model, achieving superior performance and physical consistency.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of reconstructing high-resolution satellite images from low-resolution data, especially for environmental exposure assessment and long-term monitoring.

Method: Combines a pretrained lightweight U-Net encoder (frozen) with a diffusion-based generative model for downscaling, validated with domain similarity analysis and performance metrics.

Result: Achieved high performance (R2 = 0.65 to 0.94), outperforming other models and preserving spatial variability and temporal autocorrelation in downscaled images.

Conclusion: The framework provides a robust and physically coherent solution for downscaling coarse-resolution images, with significant implications for environmental monitoring.
...
TL;DR: FlowEO is a novel framework using generative models for unsupervised domain adaptation (UDA) in Earth observation, leveraging flow matching to improve generalization across heterogeneous datasets.


<details>
  <summary>Details</summary>
Motivation: The heterogeneity of Earth observation data limits model generalization, necessitating UDA for practical applications.

Method: FlowEO introduces flow matching to create semantically preserving mappings between source and target image distributions for classification and segmentation tasks.

Result: FlowEO outperforms existing methods in domain adaptation and achieves comparable or better perceptual image quality.

Conclusion: Flow-matching-based UDA shows strong potential for remote sensing applications.
...
TL;DR: A framework for self-training Vision-Language Model (VLM) judges without human annotations improves accuracy on benchmarks, outperforming larger models.


<details>
  <summary>Details</summary>
Motivation: Current VLM judge training relies on costly human annotations, which quickly become outdated as models improve. This work aims to eliminate the need for human input.

Method: The iterative framework synthesizes data in three stages: generating diverse instruction-response pairs, producing judgments with reasoning traces, and training on validated data.

Result: The method boosts a Llama-3.2-11B judge's accuracy from 0.38 to 0.51 on VL-RewardBench, outperforming larger models like GPT-4o and Claude 3.5 Sonnet.

Conclusion: Self-training VLM judges without human annotations shows promise for keeping pace with rapid model advancements.
...
TL;DR: TwinFlow is a new framework for efficient 1-step multi-modal generative models, avoiding complex adversarial training and outperforming existing methods in text-to-image tasks with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency and complexity of current multi-step generative models and their acceleration methods, which often suffer from iterative processes, performance degradation, or training instability.

Method: Proposes TwinFlow, a framework that eliminates the need for pretrained teacher models and adversarial networks, enabling efficient 1-step generation.

Result: Achieves a GenEval score of 0.83 in 1-NFE, outperforming baselines like SANA-Sprint and RCGM while matching the performance of a 100-NFE model at 100x lower computational cost.

Conclusion: TwinFlow offers a scalable and efficient solution for multi-modal generation, significantly reducing computational overhead with minimal performance trade-offs.
...
TL;DR: introducing a tiered embedder and ProAttention mechanism to improve fine-grained image generation in diffusion models.


<details>
  <summary>Details</summary>
Motivation: Addressing semantic entanglement and insufficient detail in fine-grained image generation using diffusion models.

Method: Introduces a tiered embedder for semantic integration and super-resolution for detail enhancement, plus a ProAttention mechanism.

Result: Outperforms state-of-the-art methods on public benchmarks.

Conclusion: The proposed approach effectively improves fine-grained image generation in diffusion models.
...
TL;DR: The paper introduces Semore, a VLM-based framework for visual RL that enhances semantic and motion representations to improve reinforcement learning performance.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based RL methods struggle with limited representations. The paper aims to address this by leveraging VLM for richer semantic and motion representations.

Method: Semore uses a dual-path backbone to extract and fuse semantic and motion representations from RGB flows, guided by VLM and pre-trained clip for alignment. It employs separate supervision for semantics and motion.

Result: Experiments show Semore outperforms state-of-the-art methods with efficient and adaptive performance, thanks to VLM-guided feature extraction.

Conclusion: Semore effectively enhances RL by integrating semantic and motion representations, demonstrating superior adaptability and efficiency.
...
TL;DR: The paper introduces Pixel-Equivalent Latent Compositing (PELC) and DecFormer, a transformer model, to improve latent inpainting in diffusion models by ensuring compositing matches pixel-space results.


<details>
  <summary>Details</summary>
Motivation: Current latent inpainting methods using linear interpolation of VAE latents under downsampled masks produce artifacts and global degradation.

Method: Proposes PELC and DecFormer, a transformer predicting blend weights and corrections for pixel-equivalent latent fusion, requiring minimal computational overhead.

Result: DecFormer improves color consistency, soft-mask support, and sharp boundaries, reducing edge errors by up to 53% compared to standard interpolation.

Conclusion: PELC and DecFormer offer a general solution for high-fidelity latent editing, demonstrated on inpainting and color-correction tasks.
...
TL;DR: A novel dataset (DEAR) is introduced to evaluate image rendering aesthetics, capturing human preferences via crowdsourcing. It extends beyond traditional IQA by focusing on subjective style preferences.


<details>
  <summary>Details</summary>
Motivation: Traditional IQA metrics focus on technical degradations, but aesthetics of rendering styles remain underexplored due to the lack of datasets reflecting subjective human judgments.

Method: DEAR is built on the MIT-Adobe FiveK dataset, incorporating pairwise human preference scores collected from 13,648 evaluators via crowdsourcing. Each pair was assessed by 25 evaluators.

Result: DEAR enables the development of models for evaluating rendering aesthetics, supporting tasks like style preference prediction, aesthetic benchmarking, and personalized modeling.

Conclusion: DEAR is the first dataset to systematically address rendering aesthetics based on human preferences. A subset is available on HuggingFace.
...
TL;DR: Hybrid capture combines sparse RGB keyframes with event streams to reconstruct full RGB video, reducing power consumption while maintaining video quality.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB cameras are energy-intensive, while event cameras offer low-power sensing but require reconstruction for standard video output.

Method: Two approaches are proposed: an autoregressive model (HyperE2VID) and a diffusion-based model (LTX), leveraging event data for RGB video reconstruction.

Result: The diffusion-based LTX model outperforms HyperE2VID with 33% better perceptual quality (0.283 vs 0.422 LPIPS).

Conclusion: The hybrid paradigm and diffusion-based reconstruction show promise for low-power video capture without sacrificing quality.
...
TL;DR: CARD is a diffusion-based image restoration method that handles correlated Gaussian noise by whitening the noise and using noise-whitened updates, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Most diffusion models assume i.i.d. Gaussian noise, but real-world sensors often have spatially correlated noise, limiting their practicality.

Method: CARD whitens noisy observations to convert correlated noise into i.i.d. form and replaces diffusion steps with noise-whitened updates, maintaining DDRM's efficiency.

Result: CARD outperforms existing methods on denoising, deblurring, and super-resolution tasks, validated on synthetic benchmarks and the new CIN-D dataset.

Conclusion: Addressing correlated noise in diffusion models improves real-world applicability, demonstrated by CARD's effectiveness and the introduction of the CIN-D dataset.
...
TL;DR: COM4D is a method for capturing 4D scenes with multiple objects using only static or single-object supervision, avoiding the need for 4D compositional training data.


<details>
  <summary>Details</summary>
Motivation: Existing methods often focus on single objects with category-specific models, leading to inconsistent scenes and limited applicability. COM4D addresses this by jointly predicting 4D structures and configurations without relying on such models.

Method: COM4D trains spatial and temporal attentions on 2D video input, disentangling learning from object compositions and single object dynamics. An attention mixing mechanism combines these at inference time.

Result: COM4D reconstructs complete 4D scenes from monocular videos and achieves state-of-the-art results in 4D object and composed 3D reconstruction tasks.

Conclusion: COM4D provides a data-driven solution for persistent 4D scene reconstruction, overcoming limitations of existing methods and excelling in performance.
...
TL;DR: The paper introduces the Temporal Understanding in Autonomous Driving (TAD) benchmark to evaluate VLMs' temporal reasoning in ego-centric AD footage and proposes two training-free solutions to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack focus on temporal understanding in autonomous driving, prompting the creation of TAD to address this gap.

Method: The TAD benchmark includes 6,000 QA pairs across 7 tasks. Two novel solutions, Scene-CoT and TCogMap, are proposed to enhance motion understanding without training.

Result: Current SoTA models perform poorly on TAD, but the proposed methods improve accuracy by up to 17.72%.

Conclusion: TAD aims to advance research in temporal understanding for AD, with benchmark and code made publicly available.
...
TL;DR: SpaceControl introduces a training-free method for precise spatial control in 3D generation, outperforming baselines in geometric faithfulness while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing methods relying on text or image prompts lack geometric specificity, prompting the need for intuitive and precise control over 3D object geometry.

Method: SpaceControl uses geometric inputs (primitives to detailed meshes) and integrates with pre-trained models without additional training, offering a trade-off between fidelity and realism.

Result: It outperforms training-based and optimization-based baselines in geometric faithfulness and retains high visual quality, as shown in evaluations and user studies.

Conclusion: The method is practical for creative workflows, demonstrated by an interactive UI for editing and converting inputs into textured 3D assets.
...
TL;DR: A new method, \ourmethod, enables interactive, precise, and non-destructive editing of 3D Gaussian Splatting assets using 2D views, combining feedforward prediction with Test-Time Training for versatile tasks like detail refinement and recoloring.


<details>
  <summary>Details</summary>
Motivation: Current methods for editing 3D Gaussian Splatting assets are slow, destructive, or lack precision, creating a need for interactive and precise refinement tools.

Method: Introduces a state-aware feedforward model that predicts updates to Gaussian attributes from 2D views, using Test-Time Training for iterative refinement.

Result: Enables high-fidelity local detail refinement, paint-over, and global recoloring at interactive speeds.

Conclusion: \ourmethod offers a fluid and intuitive workflow for 3D content authoring, addressing limitations of existing approaches.
...
TL;DR: The paper introduces Group Orthogonal Low-Rank Adaptation (GOLA), a parameter-efficient fine-tuning framework for RGB-T tracking. It addresses redundancy in rank space by grouping and orthogonalizing redundant ranks, improving adaptability and performance.


<details>
  <summary>Details</summary>
Motivation: Existing low-rank adaptation methods in RGB-T tracking suffer from redundancy in the rank space, limiting the model's ability to learn diverse features for various challenges. GOLA aims to mitigate this issue.

Method: GOLA uses singular value decomposition to partition and quantify rank importance, freezes crucial ranks, groups redundant ranks, and enforces inter-group orthogonality to learn complementary features.

Result: GOLA outperforms state-of-the-art methods on four benchmark datasets by reducing parameter redundancy and enhancing feature representation.

Conclusion: GOLA successfully improves RGB-T tracking performance by efficiently utilizing rank space through structured parameter learning and orthogonal constraints.
...
TL;DR: PoolNet is a deep learning framework that efficiently validates image data for SfM processing, reducing time and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Current SfM processes are slow and struggle with noisy or inadequate data, requiring a better solution.

Method: Introduces PoolNet, a deep learning model for frame and scene-level validation of image data.

Result: PoolNet effectively identifies SfM-ready scenes and outperforms state-of-the-art algorithms in speed.

Conclusion: PoolNet offers a scalable and efficient solution for validating image data for SfM tasks.
...
TL;DR: ShaRP improves attention-based pruning for VLLMs by integrating segment-aware masking, positional debiasing, and token deduplication, enabling effective pruning at shallow layers without performance loss.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of high computational load in VLLMs during pre-filling by improving attention-based pruning methods, which suffer from performance degradation at shallow layers.

Method: Introduces ShaRP, featuring segment-aware causal masking, positional debiasing, and token deduplication to enhance token selection in shallow decoder layers.

Result: ShaRP maintains stable performance under high compression rates without retraining and achieves competitive results on video understanding benchmarks.

Conclusion: ShaRP establishes a new paradigm for accelerating VLLM inference by enabling effective pruning at shallow decoder layers.
...
TL;DR: The paper introduces LoC-Path, an efficient MLLM framework for WSI understanding that reduces redundancy and computational costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Existing slide-level MLLMs for pathology are computationally expensive due to processing redundant tile features. The authors aim to address this by leveraging tile-level redundancy and focusing on task-relevant tiles.

Method: LoC-Path uses a Sparse Token Merger (STM) and an MAE-pretrained resampler to eliminate redundancy, and a Cross-Attention Routing Adapter (CARA) with Token Importance Scorer (TIS) to integrate compressed visual features efficiently.

Result: The approach achieves performance comparable to state-of-the-art MLLMs while significantly reducing computation and memory usage.

Conclusion: LoC-Path offers a more efficient alternative to existing WSI-language models by addressing redundancy and optimizing computational resources.
...
TL;DR: SSVAE improves text-to-video generation efficiency and quality by analyzing and optimizing VAE latent space structure with two new regularizers.


<details>
  <summary>Details</summary>
Motivation: Existing video VAEs prioritize reconstruction fidelity but neglect latent structure, impacting diffusion training difficulty.

Method: The study introduces Local Correlation Regularization and Latent Masked Reconstruction to enforce favorable spectral properties in VAE latents.

Result: SSVAE achieves a 3× convergence speedup and 10% video reward gain over competitors.

Conclusion: Optimizing latent structure in VAEs significantly enhances diffusion training efficiency and output quality.
...
TL;DR: The paper introduces Dynamic Prior (DP), leveraging VLMs and SAM for unsupervised dynamic object identification in videos, enhancing motion segmentation and 3D understanding.


<details>
  <summary>Details</summary>
Motivation: Classical and learning-based methods struggle with dynamic objects in videos due to reliance on large-scale datasets, limiting accuracy in 3D reconstruction.

Method: The Dynamic Prior (DP) uses Vision-Language Models (VLMs) and SAM for robust dynamic object identification without task-specific training.

Result: DP achieves state-of-the-art motion segmentation and significantly improves accuracy in 3D understanding on synthetic and real-world videos.

Conclusion: The Dynamic Prior (DP) effectively addresses dynamic object challenges, enhancing structural 3D understanding without relying on large datasets.
...
TL;DR: A Genetic Algorithm (GA) optimizes parameters for traditional stereo matching (SGBM + WLS) in UAVs, improving accuracy and efficiency without manual tuning.


<details>
  <summary>Details</summary>
Motivation: Traditional stereo matching is fast but requires tedious parameter tuning. UAVs in forestry need precise distance measurements with minimal manual intervention.

Method: Proposes a GA-based framework to automatically optimize SGBM and WLS parameters, evaluated using multiple quality metrics.

Result: GA-optimized approach reduces Mean Squared Error by 42.86%, boosts PSNR by 8.47%, and SSIM by 28.52%, with better generalization across imaging conditions.

Conclusion: The GA-based solution enhances precision and efficiency for UAV stereo matching, suitable for real-world forestry applications.
...
TL;DR: A computer vision framework using YOLO and SGBM for autonomous drone-based pruning of radiata pine trees, achieving high precision and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address safety risks in manual pruning by developing a cost-effective autonomous system using stereo cameras instead of LiDAR.

Method: Integrates YOLO for branch detection and SGBM for depth estimation, tested against Mask R-CNN for performance comparison.

Result: Achieves 82.0% mAPmask50-95 for branch segmentation, accurate localization within 2m, and sub-second processing per frame.

Conclusion: The system is feasible for enhancing safety and efficiency in commercial forestry operations.
...
TL;DR: The paper proposes a multi-input CNN with an attention module to automate moving object detection in solar system survey data, reducing manual effort by 99% with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Manual verification of moving objects in survey data is labor-intensive, prompting the need for automation to improve efficiency.

Method: A multi-input CNN integrated with a convolutional block attention module processes stacked images and focuses on key spatial and channel features.

Result: The model achieves ~99% accuracy and AUC >0.99 on 2,000 images, reducing human workload by 99%.

Conclusion: The proposed method significantly enhances detection efficiency and reliability, mitigating reliance on manual verification.
...
TL;DR: Evaluation of deep learning methods for UAV-based tree branch segmentation at three resolutions, with U-Net and MiT variants performing best in accuracy, while PSPNet offers efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: To ensure safe navigation and automated pruning in forestry operations, precise and rapid tree branch segmentation is needed across varying resolutions and conditions.

Method: Tested 22 deep learning configurations at resolutions (256x256, 512x512, 1024x1024) using the Urban Street Tree Dataset, with metrics like IoU, Dice, TS-IoU, and CPR.

Result: U-Net with MiT-B4 performed best at 256x256; MiT-B4 led at 512x512; U-Net+MiT-B3 excelled at 1024x1024 for IoU/Dice. PSPNet was the most efficient but with reduced accuracy.

Conclusion: The study provides benchmarks for accuracy-efficiency trade-offs in embedded forestry systems, highlighting top-performing methods for different resolutions.
...
TL;DR: ParaUni is a unified multimodal model that enhances visual generation by extracting parallel features from VLM's layers for comprehensive interaction, integrating them via LIM, and optimizing performance with RL-based LDAM.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to balance interaction and flexibility in visual generation due to representation differences in VLM's layers.

Method: ParaUni extracts parallel features from VLM's layers, integrates them with LIM, and optimizes via LDAM using RL.

Result: ParaUni improves generation quality by leveraging multi-layer features and shows strong RL reward alignment potential.

Conclusion: ParaUni effectively balances interaction and flexibility, advancing unified multimodal models for better visual generation.
...
TL;DR: TED-4DGS introduces a compact deformation scheme for dynamic 3D Gaussian Splatting (3DGS), combining temporal-activation and embedding-based methods with rate-distortion-optimized compression for improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for dynamic 3DGS lack efficient deformation schemes and temporal control, prompting the need for a unified solution like TED-4DGS.

Method: TED-4DGS uses sparse anchor-based 3DGS with learnable temporal-activation parameters and embeddings, along with INR-based hyperprior and autoregressive models for compression.

Result: The scheme achieves state-of-the-art rate-distortion performance on real-world datasets.

Conclusion: TED-4DGS advances dynamic 3DGS compression by unifying deformation and temporal control, marking a pioneering effort in rate-distortion optimization.
...
TL;DR: WAFL-ViT, a federated learning approach for edge devices, is tested with a specialized dataset (CUBR) for building recognition, outperforming self-training in accuracy.


<details>
  <summary>Details</summary>
Motivation: To explore collaborative learning on edge devices using WAFL-ViT and validate its effectiveness with a mission-specific dataset.

Method: Developed the CUBR dataset for building recognition in Thailand and compared WAFL-ViT's performance against self-training.

Result: WAFL-ViT achieved higher accuracy than self-training in building recognition tasks.

Conclusion: WAFL-ViT is effective for mission-oriented tasks, and specialized datasets like CUBR enhance performance.
...
TL;DR: The paper proposes EmoStyle, a framework for affective image stylization (AIS) that enhances emotional expressiveness while preserving content. It addresses challenges like lack of training data and emotion-style mapping through a new dataset and innovative components.


<details>
  <summary>Details</summary>
Motivation: Existing image stylization methods focus on visual appearance but neglect emotional impact. The paper aims to bridge this gap by introducing AIS, a task that applies artistic styles to evoke specific emotions.

Method: The authors present EmoStyle, which includes: 1) EmoStyleSet, a dataset of content-emotion-stylized triplets; 2) an Emotion-Content Reasoner for integrating emotional cues with content; and 3) a Style Quantizer for mapping continuous style features to emotion-related codebook entries.

Result: Qualitative and quantitative evaluations, including user studies, show EmoStyle effectively enhances emotional expressiveness while maintaining content consistency. The learned style dictionary also proves adaptable to other generative tasks.

Conclusion: The work establishes a foundation for emotion-driven image stylization, expanding AI's creative potential in art. The framework and dataset pave the way for broader applications in affective stylization.
...
TL;DR: AVP is a framework for long video understanding that actively perceives and extracts query-relevant evidence efficiently, outperforming existing methods in accuracy and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Long video understanding is difficult due to sparse cues in redundant content. Current methods waste resources on irrelevant data and lose fine-grained details. Inspired by active perception, AVP aims to dynamically decide what, when, and where to observe.

Method: AVP uses an iterative plan-observe-reflect process with MLLM agents. A planner suggests interactions, an observer extracts evidence, and a reflector evaluates sufficiency, halting or continuing as needed.

Result: AVP achieves top performance on five LVU benchmarks, improving accuracy by 5.7% while reducing inference time to 18.4% and input tokens to 12.4% compared to the best agentic method.

Conclusion: AVP demonstrates that active, query-relevant perception significantly enhances long video understanding efficiency and accuracy.
...
TL;DR: UniFS is a Unified Frequency-Spatial Fusion model for Multi-Contrast MR Reconstruction, designed to handle multiple k-space undersampling patterns without retraining. It integrates frequency and spatial features, outperforming existing methods in generalization and performance.


<details>
  <summary>Details</summary>
Motivation: Existing MCMR methods require separate models for different k-space undersampling patterns and often neglect frequency characteristics, limiting their practicality and performance. UniFS aims to address these limitations.

Method: UniFS combines a Cross-Modal Frequency Fusion module, an Adaptive Mask-Based Prompt Learning module, and a Dual-Branch Complementary Refinement module to extract domain-invariant features and adapt to pattern variations dynamically.

Result: UniFS achieves state-of-the-art performance on BraTS and HCP datasets across various k-space undersampling patterns, including unseen ones, demonstrating superior generalization.

Conclusion: UniFS effectively unifies frequency and spatial learning for MCMR, eliminating the need for retraining and significantly improving generalization, making it a practical and high-performing solution.
...
TL;DR: The paper introduces a cross-modal framework using 2D VLMs to mine rare objects from driving scenes, improving 3D detection performance with fewer annotated samples.


<details>
  <summary>Details</summary>
Motivation: Rare-object detection is challenging in autonomous driving, and VLMs' potential for enhancing 3D detection via data mining is underutilized.

Method: Combines object detection, semantic feature extraction, dimensionality reduction, and outlier detection techniques into a cohesive pipeline.

Result: The framework improves 3D detection performance on the nuScenes dataset, especially for rare objects, using fewer samples.

Conclusion: The approach efficiently curates datasets for autonomous systems, reducing annotation burden and enhancing detection of critical objects.
...
TL;DR: ZoomClick leverages zoom as a prior for GUI grounding without training, improving performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in GUI grounding like cross-platform generalization and fine-grained localization by exploring zoom's potential.

Method: Proposes ZoomClick, detailing four key zoom properties (pre-zoom, depth, shrink size, minimal crop size) for dynamic spatial focusing.

Result: Boosts performance of vision-language and GUI models; UI-Venus-72B achieves 73.1% success on ScreenSpot-Pro.

Conclusion: Introduces GUIZoom-Bench to inspire future zoom-focused research in GUI grounding.
...
TL;DR: WaterWave enhances underwater videos by leveraging temporal consistency priors and wavelet-based implicit representation, outperforming single-image enhancement methods and improving downstream tracking tasks.


<details>
  <summary>Details</summary>
Motivation: Underwater video enhancement is challenging due to lack of temporal consistency in existing single-image methods.

Method: WaterWave uses wavelet-based temporal consistency fields and an underwater flow correction module to filter inconsistent components while preserving motion details.

Result: Significant improvement in video quality and downstream tracking performance (e.g., 19.7% and 9.7% gains).

Conclusion: WaterWave effectively addresses temporal inconsistency in underwater video enhancement and benefits downstream applications.
...
TL;DR: The paper introduces a Foundation-Driven Efficient Paradigm (FDEP) to adapt large-scale visual foundation models (VFMs) for single-frame infrared small target (SIRST) detection, improving accuracy without added inference overhead. It includes a Semantic Alignment Modulation Fusion (SAMF) module and Collaborative Optimization-based Implicit Self-Distillation (CO-ISD) strategy, along with a new Holistic SIRST Evaluation (HSE) metric. The approach achieves SOTA performance.


<details>
  <summary>Details</summary>
Motivation: To explore the untapped potential of VFMs in SIRST detection and address the lack of a unified evaluation system.

Method: Proposes FDEP with SAMF for feature fusion and CO-ISD for efficient inference. Introduces HSE for comprehensive evaluation.

Result: FDEP-equipped networks achieve SOTA performance on multiple datasets.

Conclusion: FDEP effectively adapts VFMs for SIRST detection, enhancing accuracy and efficiency while providing a fair evaluation metric.
...
TL;DR: Know-Show is a new benchmark for evaluating spatio-temporal grounded reasoning in Video-LMs, revealing gaps in current models and proposing GRAM to improve grounding.


<details>
  <summary>Details</summary>
Motivation: To address the lack of spatio-temporal grounding in Video-LMs and unify reasoning and localization in a single evaluation framework.

Method: Introduces Know-Show, a benchmark with five scenarios for spatial and temporal reasoning, and proposes GRAM, a training-free plug-in for fine-grained grounding.

Result: Existing Video-LMs struggle with fine-grained hand-object interactions, showing gaps compared to human reasoning.

Conclusion: Know-Show sets a standard for grounded reasoning and offers insights for developing interpretable multimodal systems.
...
TL;DR: DashFusion is a novel framework addressing alignment and fusion challenges in multimodal sentiment analysis (MSA), achieving state-of-the-art performance via dual-stream alignment and hierarchical bottleneck fusion.


<details>
  <summary>Details</summary>
Motivation: Existing MSA methods often handle alignment or fusion separately, leading to performance and efficiency limitations. DashFusion aims to overcome these by integrating both aspects effectively.

Method: The framework uses a dual-stream alignment module for temporal (cross-modal attention) and semantic (contrastive learning) synchronization, followed by supervised contrastive learning and hierarchical bottleneck fusion for efficient multimodal integration.

Result: DashFusion outperforms existing methods on CMU-MOSI, CMU-MOSEI, and CH-SIMS datasets, validated by ablation studies.

Conclusion: DashFusion successfully addresses MSA challenges, offering balanced performance and computational efficiency, with code publicly available.
...
TL;DR: VOST-SGG is a new framework integrating vision-language models into spatio-temporal scene graph generation to address limitations like uninformed queries and unimodal features, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current DETR-style ST-SGG models suffer from semantically uninformed learnable queries and reliance on unimodal visual features, limiting their effectiveness.

Method: VOST-SGG introduces dual-source query initialization for semantic grounding and a multi-modal feature bank combining visual, textual, and spatial cues.

Result: The framework achieves state-of-the-art performance on the Action Genome dataset.

Conclusion: Integrating VLM-aided semantic priors and multi-modal features improves ST-SGG, validated by experimental results.
...
TL;DR: DepSeg leverages depth-guided prompts and pretrained models for efficient laparoscopic scene segmentation, achieving significant improvement over baseline methods.


<details>
  <summary>Details</summary>
Motivation: Pixel-wise segmentation in laparoscopic surgery is challenging due to high annotation costs; DepSeg aims to address this by using depth and pretrained models.

Method: DepSeg uses monocular depth estimation, depth-guided prompts, and SAM2 for masks, then classifies via template matching with pooled visual features.

Result: On CholecSeg8k, DepSeg outperforms SAM2 baseline (35.9% vs. 14.7% mIoU) and works well with minimal templates (10--20%).

Conclusion: Depth-guided prompting and template-based classification enable efficient, annotation-light segmentation.
...
TL;DR: The paper analyzes 'dead leaves' models for image generation and proposes a Bayesian ideal observer to study segmentation decisions, providing a performance benchmark for human and algorithmic vision.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how occlusion in visual scenes can be modeled and analyzed, offering a theoretical foundation for comparing human and machine segmentation performance.

Method: A Bayesian ideal observer is derived for partitioning pixels based on 'dead leaves' models, with detailed steps for computing posterior probabilities and assessing practical feasibility.

Result: The dead leaves model and ideal observer offer a principled upper-bound for segmentation performance, useful for evaluating human and algorithmic vision systems.

Conclusion: This work provides a theoretical framework for studying segmentation in occluded scenes, with potential applications in vision research and algorithm development.
...
TL;DR: CG-VLM introduces a training-free framework using game-theoretic interpretability to address text inertia in VLMs, improving visual grounding without compromising general capabilities.


<details>
  <summary>Details</summary>
Motivation: Large VLMs often prioritize linguistic priors over visual evidence, leading to object hallucinations. Existing methods lack principled grounding for internal reasoning correction.

Method: CG-VLM uses a Cognitive Demand Sensor to detect vision-text synergy needs and a Focused Consensus Induction module to reorient attention toward visual tokens.

Result: State-of-the-art performance on POPE and CHAIR benchmarks across multiple models, preserving foundational knowledge.

Conclusion: Token-level sensing enables precise, context-aware intervention in VLMs, addressing text inertia effectively.
...
TL;DR: A new dataset (2K-Characters-10K-Stories) and Human-in-the-Loop pipeline (HiL) are introduced to improve sequential identity consistency in visual storytelling by decoupling stable identities from transient attributes.


<details>
  <summary>Details</summary>
Motivation: Current datasets lack fidelity and fail to disentangle stable identities from transient attributes, limiting reliable sequential synthesis.

Method: HiL pipeline uses expert-verified templates and LLM-guided narrative planning, with decoupled control for identity and transient attributes, plus a Quality-Gated loop for consistency.

Result: Models fine-tuned on the dataset achieve performance comparable to closed-source models.

Conclusion: The proposed dataset and pipeline effectively address the challenge of sequential identity consistency in visual storytelling.
...
TL;DR: ProPhy is a framework for physics-aware video generation that uses a two-stage Mixture-of-Physics-Experts mechanism to improve physical consistency in generated videos.


<details>
  <summary>Details</summary>
Motivation: Current video generation models lack fine-grained alignment with physical cues, leading to inconsistent results, especially in large-scale or complex dynamics. ProPhy aims to address this by explicitly incorporating physics-aware conditioning.

Method: ProPhy employs a Mixture-of-Physics-Experts (MoPE) mechanism with Semantic Experts for semantic-level principles and Refinement Experts for token-level dynamics. It also uses a physical alignment strategy to transfer reasoning from vision-language models.

Result: ProPhy outperforms state-of-the-art methods on physics-aware video generation benchmarks, producing more realistic, dynamic, and physically coherent videos.

Conclusion: The ProPhy framework successfully improves the physical consistency of generated videos by leveraging explicit physics-aware conditioning and a two-stage expert mechanism.
...
TL;DR: MedDIFT, a training-free 3D medical image registration method using pretrained diffusion model features, matches state-of-the-art accuracy without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing intensity-based registration struggles with low-contrast or variable regions; diffusion models' rich semantic features offer a solution.

Method: MedDIFT extracts multi-scale features from a pretrained diffusion model, fuses them into descriptors, and matches via cosine similarity with optional local-search prior.

Result: MedDIFT achieves accuracy comparable to UniGradICON and outperforms B-spline registration on lung CT data.

Conclusion: Diffusion model features enable accurate registration without training; multi-level fusion and moderate noise enhance performance.
...
TL;DR: The paper introduces a skinning-free method for generating 3D garment deformations by independently estimating vertex positions and normals, improving animation quality and wrinkle details.


<details>
  <summary>Details</summary>
Motivation: Existing methods using linear blend skinning often misalign shapes and fail to recover high-fidelity wrinkles, prompting the need for a better approach.

Method: Proposes estimating low-frequency posed garment shape via vertex positions and high-frequency wrinkles via vertex normals, encoded as texture images for 2D transfer.

Result: The method significantly enhances animation quality and recovers finer wrinkles compared to state-of-the-art techniques.

Conclusion: The skinning-free approach effectively decouples frequency modalities, leveraging 2D image transfer and pretrained models for superior results.
...
TL;DR: Fast SceneScript accelerates 3D scene layout estimation using multi-token prediction and confidence-guided decoding, achieving speed without accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Autoregressive next-token prediction in perception-generalist models is slow, prompting the need for faster inference methods.

Method: Introduces multi-token prediction (MTP) and confidence-guided decoding (CGD) with an improved scoring mechanism for token reliability. Also includes a parameter-efficient mechanism to reduce MTP overhead.

Result: Achieves up to 9 tokens per decoder step without accuracy loss, adding only ~7.5% extra parameters.

Conclusion: Fast SceneScript efficiently balances speed and accuracy in 3D scene layout estimation.
...
TL;DR: NormalView, a sensor-agnostic deep learning method, uses 2D projections of point cloud data with normal vectors and image classification to achieve high accuracy in tree species classification, outperforming with multispectral intensity data.


<details>
  <summary>Details</summary>
Motivation: To improve tree species classification accuracy from point cloud data using a sensor-agnostic method that leverages geometric and radiometric information.

Method: NormalView: a projection-based deep learning method embedding local geometry into 2D images (normal vectors) and using YOLOv11 for classification. Tested on MLS and ALS data with and without multispectral intensity.

Result: 95.5% accuracy on MLS data, 91.8% on ALS data; multispectral intensity improved performance, with the best model using all three channels.

Conclusion: Projection-based methods with geometric and radiometric enhancements achieve high accuracy, are sensor-agnostic, and scalable for tree species classification.
...
TL;DR: DistillFSS is a framework for Cross-Domain Few-Shot Semantic Segmentation (CD-FSS) that uses teacher-student distillation to embed support-set knowledge into model parameters, eliminating the need for support images at test time and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The challenge in CD-FSS arises from distribution shifts between domains, disjoint label spaces, and scarce annotated support images, making episodic methods unreliable and computationally heavy.

Method: DistillFSS employs a teacher-student distillation process to internalize few-shot reasoning into a dedicated layer, enabling fast inference and efficient adaptation to novel domains and classes.

Result: DistillFSS matches or outperforms state-of-the-art baselines, especially in multi-class and multi-shot scenarios, and reduces computational overhead.

Conclusion: DistillFSS provides an efficient and scalable solution for CD-FSS, demonstrating effectiveness across diverse domains.
...
TL;DR: Introducing an unsupervised training framework using Optimal Transport to train ISP pipelines without relying on paired datasets, achieving performance comparable to paired methods.


<details>
  <summary>Details</summary>
Motivation: To overcome the bottleneck of needing large-scale paired datasets for training ISP pipelines by proposing an unsupervised method.

Method: A novel framework based on Unbalanced Optimal Transport (UOT) and a committee of expert discriminators to guide the mapping.

Result: The framework outperforms original paired methods in paired mode and rivals them in unpaired mode.

Conclusion: The proposed UOT-based framework effectively trains ISP pipelines without paired data, achieving high performance.
...
TL;DR: A self-supervised method for detecting AI-generated images using EXIF metadata, outperforming existing detectors by leveraging intrinsic photographic features.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors rely on model-specific assumptions, limiting cross-model applicability. This paper addresses the need for a more generalized approach.

Method: Uses EXIF tags (e.g., camera model, focal length) to train a feature extractor on real photos. Detects AI images via one-class detection (modeling photographic distribution) and binary classification (using high-frequency residuals).

Result: The method shows strong generalization across generative models and robustness to image perturbations, outperforming existing techniques.

Conclusion: The proposed EXIF-based approach significantly advances AI-generated image detection, offering wider applicability and improved performance.
...
TL;DR: LeAD-M3D introduces a monocular 3D detector with state-of-the-art accuracy and real-time efficiency, leveraging three innovative components to address depth ambiguity and computational cost without extra modalities.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in overcoming depth ambiguity, viewpoint shifts, and computational inefficiency in real-time monocular 3D object detection without relying on LiDAR or geometric priors.

Method: The method combines Asymmetric Augmentation Denoising Distillation (A2D2), 3D-aware Consistent Matching (CM3D), and Confidence-Gated 3D Inference (CGI3D) to enhance depth reasoning, improve prediction matching, and accelerate detection.

Result: LeAD-M3D achieves top accuracy on KITTI, Waymo, and Rope3D datasets while running 3.6x faster than previous high-accuracy methods.

Conclusion: The results show that high-fidelity, real-time monocular 3D detection is feasible without LiDAR, stereo, or geometric assumptions.
...
TL;DR: The paper introduces a real-time facial expression recognition method using deep learning and geometric features, achieving high accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Enhance human-computer interaction and emotion-aware systems through fast and accurate facial expression recognition.

Method: Combines MediaPipe FaceMesh for landmark detection with geometric features and ConvLSTM1D for temporal dynamics classification.

Result: Achieved accuracies of 93%, 79%, 77%, and 68% on CK+, Oulu-CASIA (VIS and NIR), and MMI datasets, with real-time processing at 165 FPS.

Conclusion: The method is fast, accurate, and adaptable, with potential for advancements in emotion-aware technologies and personalized user experiences.
...
TL;DR: InverseCrafter is an efficient inpainting solver for 4D video generation, avoiding costly fine-tuning of Video Diffusion Models by reformulating the task as a latent-space inpainting problem.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the computational expense and catastrophic forgetting issues of fine-tuning pre-trained Video Diffusion Models (VDMs) for controllable 4D video generation.

Method: The proposed method, InverseCrafter, encodes pixel-space degradation into a continuous latent mask, bypassing repeated VAE operations and backpropagation.

Result: InverseCrafter achieves comparable novel view generation and superior measurement consistency with minimal computational overhead, and excels in video inpainting.

Conclusion: InverseCrafter offers an efficient alternative to traditional VDM fine-tuning, with broad applicability to video editing tasks.
...
TL;DR: 3D-CSCNet, an algorithm-unrolling-based network within an autoencoder framework, improves hyperspectral unmixing by jointly learning spectral and spatial relationships using 3D convolutional sparse coding and achieves superior performance on real and simulated datasets.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral unmixing separates pixels into constituents (endmembers) and estimates their abundances. Existing methods lack joint learning of spectral and spatial relationships in 3D hyperspectral data.

Method: Proposes 3D-CSCNet, integrating a 3D CSC block derived from deep algorithm unrolling within an autoencoder framework to estimate abundances and extract endmembers via decoder weights. Uses PSVM for endmember initialization.

Result: Outperforms state-of-the-art methods on three real and one simulated dataset across varying SNR levels.

Conclusion: 3D-CSCNet effectively combines 3D convolutional sparse coding with autoencoders for superior hyperspectral unmixing, validated by extensive experiments.
...
TL;DR: ZRNet is a physics-informed framework that predicts Zernike coefficients and restores optical images by modeling physical relationships between Zernike polynomials and enforcing consistency in the Fourier domain.


<details>
  <summary>Details</summary>
Motivation: Optical aberrations degrade microscopy image quality, especially in deeper sample imaging. Existing methods are limited and often ignore the underlying optical physics.

Method: ZRNet uses a Zernike Graph module to model physical relationships between Zernike polynomials and introduces a Frequency-Aware Alignment (FAA) loss for Fourier domain consistency.

Result: ZRNet achieves state-of-the-art performance in image restoration and Zernike coefficient prediction on CytoImageNet, handling complex, large-amplitude aberrations.

Conclusion: The approach successfully integrates optical physics into aberration correction, improving microscopy image quality across diverse modalities and samples.
...
TL;DR: OWL is a novel unsupervised 3D object detection method that improves pseudo-label quality via occupancy-guided warm-up, large-model reasoning, and dynamic re-weighting, achieving a 15%+ mAP boost over SOTA methods.


<details>
  <summary>Details</summary>
Motivation: To reduce annotation costs in autonomous driving by improving unsupervised 3D object detection, addressing the issue of incorrect pseudo-labels that mislead training.

Method: OWL uses Occupancy Guided Warm-up (OGW) for initial spatial perception, Instance-Cued Reasoning (ICR) for pseudo-label quality assessment, and Weight-adapted Self-training (WAS) for dynamic refinement.

Result: OWL surpasses state-of-the-art methods by over 15.0% mAP on Waymo Open Dataset and KITTI.

Conclusion: OWL effectively mitigates pseudo-label inaccuracies and enhances unsupervised 3D object detection performance.
...
TL;DR: A manifold-aware point cloud completion framework improves reconstruction by incorporating nonlinear geometry information using geodesic distance approximation and feature extraction.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud completion methods rely on Euclidean proximity, ignoring intrinsic nonlinear geometric structures, leading to suboptimal geometric consistency and semantic ambiguity.

Method: Proposes a framework with a Geodesic Distance Approximator (GDA) to estimate geodesic distances and a Manifold-Aware Feature Extractor (MAFE) using geodesic-based k-NN groupings and attention.

Result: The method outperforms state-of-the-art techniques in reconstruction quality, achieving better semantic coherence and structural fidelity.

Conclusion: Incorporating geodesic-aware relational attention enhances point cloud completion by capturing nonlinear geometric structure effectively.
...
TL;DR: A privacy-preserving framework distills knowledge from large LLMs into a local VLM for surgical scene understanding, using expert-supervised datasets generated without sensitive images.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack domain-specific surgical understanding and pose privacy risks due to external hosting.

Method: Generate datasets via a teacher LLM using textual context and binary masks, then use SFT and DPO for fine-tuning.

Result: Fine-tuned VLM shows significant improvement in surgical domain knowledge.

Conclusion: Validates an efficient, privacy-compliant method to train locally deployable VLMs for surgical applications.
...
TL;DR: HQ-DM is a Quantization-Aware Training framework using Single Hadamard Transformation to reduce activation outliers in diffusion models, improving performance under low-bit quantization without amplifying weight outliers.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of outliers in activation matrices during inference of diffusion models, which degrade performance under low-bit quantization.

Method: Propose HQ-DM, applying Single Hadamard Transformation to activation matrices to reduce outliers while maintaining performance.

Result: W4A4 and W4A3 schemes boost Inception Score by 12.8% and 467.73% respectively on ImageNet 256x256 with LDM-4 model.

Conclusion: HQ-DM effectively mitigates activation outliers and outperforms existing quantization methods.
...
TL;DR: USV introduces a unified framework for accelerating video diffusion models by jointly optimizing sparsification in attention computation and denoising steps.


<details>
  <summary>Details</summary>
Motivation: The scalability of video diffusion models is limited by redundant computations in spatio-temporal attention and iterative denoising, which existing methods address in isolation with diminishing returns.

Method: USV learns a dynamic, data- and timestep-dependent sparsification policy to prune attention connections, merge tokens, and reduce denoising steps, treating these as coordinated actions under a single objective.

Result: USV achieves up to 83.3% speedup in denoising and 22.7% end-to-end acceleration while maintaining visual quality.

Conclusion: Unified, dynamic sparsification is a promising approach for efficient, high-quality video generation.
...
TL;DR: Proposes a simple yet effective active learning method for 3D point cloud segmentation, using a 2D grid and network ensemble for uncertainty estimation, achieving competitive performance with state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: High annotation costs for 3D point cloud segmentation necessitate efficient active learning strategies to reduce annotation burden.

Method: Divides point clouds into columns using a 2D grid and employs a network ensemble to estimate uncertainty for selecting annotation regions.

Result: Achieves performance on par or better than complex state-of-the-art methods across multiple datasets.

Conclusion: Simplifies active learning for 3D point clouds and suggests annotated area may be more meaningful than the number of annotated points.
...
TL;DR: FNOpt is a self-supervised cloth simulation framework using a Fourier neural operator, outperforming prior methods in accuracy and robustness without needing ground truth data or retraining for different resolutions.


<details>
  <summary>Details</summary>
Motivation: To address limitations in prior neural simulators, which often require extensive ground truth data, lack fine-scale detail, or generalize poorly across resolutions and motion patterns.

Method: Formulates time integration as an optimization problem, training a resolution-agnostic neural optimizer parameterized by a Fourier neural operator (FNO) with physics-based losses on a coarse grid.

Result: FNOpt achieves stable and accurate rollouts across diverse mesh resolutions and motion patterns, captures fine-scale wrinkles, and outperforms prior learning-based approaches in out-of-distribution settings.

Conclusion: FNOpt reduces the need for curated data, improves cross-resolution reliability, and positions FNO-based meta-optimization as a strong alternative to previous neural simulators for cloth.
...
TL;DR: Proposing curvature regularization via a discrete Laplacian operator improves 3D scene reconstruction accuracy by 18.1% over standard variational autoencoders, with minimal training overhead.


<details>
  <summary>Details</summary>
Motivation: Sparse depth measurements (5% needed) lead to poor 3D reconstruction, which is unacceptable for autonomous vehicles and robots. The paper challenges the assumption that multiple geometric constraints are necessary for better performance.

Method: Introduces curvature regularization using a discrete Laplacian operator, offering stable gradients and noise suppression. The approach has only 15% training overhead and no inference cost.

Result: Achieves 18.1% better reconstruction accuracy compared to standard variational autoencoders, demonstrating that a single well-designed regularization term can outperform complex multi-term formulations.

Conclusion: A single, effective regularization term (discrete Laplacian) can surpass multi-term approaches in geometric deep learning, providing practical benefits for applications like autonomous systems.
...
TL;DR: The paper introduces CCVD, a model for continual customized text-to-video generation, addressing challenges like forgetting and concept neglect by using concept-specific retention and aggregation, along with controllable synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing CTVG methods fail to handle incremental learning of new concepts over time, leading to forgetting and neglect of old concepts.

Method: CCVD employs concept-specific attribute retention, task-aware concept aggregation, and controllable conditional synthesis with region attention-guided noise estimation.

Result: CCVD outperforms existing CTVG models in continual learning of new concepts for text-to-video generation.

Conclusion: The proposed CCVD effectively tackles forgetting and concept neglect, advancing continual learning in customized text-to-video generation.
...
TL;DR: The study evaluates test-time verifiers in Vision-Language Models (VLMs), revealing biases and unreliable rewards in MindJourney's approach. The proposed ViSA framework improves spatial reasoning but faces limitations on complex benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with spatial reasoning tasks requiring multi-view understanding. Current methods like MindJourney use test-time scaling but exhibit biases and unreliable verification.

Method: The study introduces ViSA, a framework grounding test-time rewards in verifiable micro-claims, and compares its performance against MindJourney and random scoring.

Result: ViSA improves spatial reasoning on SAT-Real but struggles on MMSI-Bench, highlighting limitations in world-model-based reasoning.

Conclusion: Test-time verification shows promise but also limitations, with world models forming an information bottleneck for complex tasks.
...
TL;DR: UG-FedDA is a novel framework for Alzheimer's disease classification, combining uncertainty quantification and federated domain adaptation to address inter-site heterogeneity and privacy concerns, achieving high accuracy across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis of Alzheimer's disease is crucial, but existing methods struggle with multicenter studies due to inter-site heterogeneity and lack of uncertainty quantification, limiting clinical applicability.

Method: The UG-FedDA framework integrates uncertainty quantification with federated domain adaptation, using multi-template region-of-interest features extracted by a self-attention transformer to handle cross-site MRI heterogeneity under privacy constraints.

Result: Experiments on ADNI, AIBL, and OASIS datasets showed consistent improvements in accuracy, sensitivity, and ROC curve area for tasks like AD vs. NC, MCI vs. AD, and NC vs. MCI, with accuracies ranging from 71.91% to 90.54%.

Conclusion: UG-FedDA effectively addresses inter-site heterogeneity and privacy concerns, demonstrating robust performance and clinical applicability for Alzheimer's disease classification across multiple datasets.
...
TL;DR: The paper introduces a novel method for event detection in optical fibers using Phase-OTDR, transforming 1D data into multi-channel grayscale images for analysis with transfer learning models, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve the analysis of optical fiber events by leveraging image-based techniques and transfer learning for more robust and adaptable detection.

Method: Transform 1D Phase-OTDR data into grayscale images using Gramian Angular Difference Field, Gramian Angular Summation Field, and Recurrence Plot, then combine them into RGB representations for analysis with EfficientNetB0 and DenseNet121 models.

Result: Achieved classification accuracies of 98.84% (EfficientNetB0) and 98.24% (DenseNet121), with 5-fold cross-validation confirming reliability (99.07% and 98.68% test accuracy).

Conclusion: The approach significantly enhances fiber optic event detection accuracy and reliability, demonstrating the potential of image-based analysis in the field.
...
TL;DR: VRSA introduces a visual reasoning sequential attack method to exploit vulnerabilities in Multimodal Large Language Models (MLLMs) by decomposing harmful text into sub-images. It achieves high attack success rates.


<details>
  <summary>Details</summary>
Motivation: MLLMs' multimodal capabilities introduce vulnerabilities, especially overlooked visual modal threats. VRSA aims to evaluate and exploit these risks.

Method: VRSA decomposes harmful text into sequential sub-images. Uses Adaptive Scene Refinement, Semantic Coherent Completion, and Text-Image Consistency Alignment to enhance attack effectiveness.

Result: VRSA outperforms state-of-the-art jailbreak attack methods on open-source and closed-source MLLMs like GPT-4o and Claude-4.5-Sonnet.

Conclusion: Visual reasoning attacks pose significant risks to MLLMs, requiring improved defenses against multimodal vulnerabilities.
...
TL;DR: A new edit-aware loss function improves RAW reconstruction from sRGB images, enhancing robustness to diverse edits and rendering styles.


<details>
  <summary>Details</summary>
Motivation: Most edits are done on sRGB images, but RAW reconstruction methods lack robustness to varied rendering styles and edits.

Method: A plug-and-play loss with a modular, differentiable ISP simulates photofinishing pipelines, training with randomized ISP parameters.

Result: Improves sRGB reconstruction by 1.5-2 dB PSNR and enables fine-tuning for target edits.

Conclusion: The loss function enhances edit fidelity and flexibility in RAW reconstruction methods.
...
TL;DR: A novel deep learning framework combining Swin Transformer with GAN for underwater image reconstruction, achieving state-of-the-art performance on the EUVP dataset.


<details>
  <summary>Details</summary>
Motivation: Underwater imaging suffers from color distortion, low contrast, and haze due to water's absorption and scattering. Existing methods fail to address these issues adequately.

Method: Proposes a GAN with a U-Net generator incorporating Swin Transformer blocks for local and global feature capture, and a PatchGAN discriminator for adversarial training.

Result: Achieves PSNR of 24.76 dB and SSIM of 0.89 on the EUVP dataset, with improved color balance, contrast, and haze reduction.

Conclusion: The Swin Transformer-based GAN outperforms traditional methods, offering robust underwater image reconstruction for marine applications.
...
TL;DR: SCAIL introduces a framework for studio-grade character animation using a novel 3D pose representation and a diffusion-transformer architecture, improving structural fidelity and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Existing motion transfer methods often fail in wild scenarios with complex motion and cross-identity animations, lacking structural fidelity and temporal consistency.

Method: SCAIL proposes a novel 3D pose representation and a full-context pose injection mechanism within a diffusion-transformer architecture for spatio-temporal reasoning.

Result: SCAIL achieves state-of-the-art performance, advancing character animation toward studio-grade reliability and realism.

Conclusion: The framework successfully addresses challenges in character animation, aligning with studio-level requirements through curated data and systematic evaluation.
...
TL;DR: NICE proposes implicit neural representations for accurate facial reconstruction and surgical outcome prediction in orthognathic surgery, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Accurate postoperative facial appearance prediction is challenging due to complex nonlinear skeletal-soft tissue interactions. Current methods lack efficiency or fail to capture these interactions fully.

Method: NICE uses region-specific implicit SDF decoders for facial surface reconstruction and deformation decoders driven by a shared surgical latent code to model biomechanical responses.

Result: NICE improves prediction accuracy in critical facial regions (e.g., lips, chin) and preserves anatomical integrity better than state-of-the-art methods.

Conclusion: NICE offers a clinically viable tool for enhanced surgical planning and patient consultation in orthognathic procedures.
...
TL;DR: A cluster-free, one-stage learnable-prototype framework with diversity regularization is proposed for WSSS in histopathology, outperforming prior methods with sharper boundaries and better class region coverage.


<details>
  <summary>Details</summary>
Motivation: To address challenges in WSSS, such as inter-class homogeneity, intra-class heterogeneity, and CAM-induced region shrinkage, without the inefficiencies of two-stage pipelines.

Method: A cluster-free, one-stage learnable-prototype framework with diversity regularization to enhance intra-class heterogeneity coverage.

Result: Achieves SOTA performance on BCSS-WSSS, with higher mIoU and mDice. Qualitative results show sharper boundaries and fewer mislabels.

Conclusion: The proposed learnable prototypes outperform clustering-based methods, covering more diverse and complementary regions within each class, proving their effectiveness.
...
TL;DR: The paper introduces C3, a method for uncertainty quantification in controllable video models to address hallucination issues by providing dense confidence estimates at the subpatch level.


<details>
  <summary>Details</summary>
Motivation: Controllable video models often hallucinate, creating misaligned frames with reality, which poses risks in tasks like robot policy evaluation. Existing models lack confidence assessment.

Method: C3 uses a novel framework for training video models with correctness and calibration, estimates uncertainty in latent space, and maps it to pixel-level uncertainty for visualization.

Result: Experiments on robot learning datasets show C3 provides calibrated uncertainty estimates and effective out-of-distribution detection.

Conclusion: C3 effectively localizes uncertainty in video frames, aiding hallucination mitigation and enhancing trustworthiness in generative video models.
...
TL;DR: The paper investigates explainable AI (XAI) methods for deep learning, focusing on quantifying the impact of background correlations and camera variations in synthetic datasets for traffic sign recognition. It highlights challenges in interpreting saliency methods and proposes systematic synthetic data generation to isolate and measure these influences.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of quantitative testing for intuitive interpretations of XAI methods, particularly regarding spurious correlations in real-world data and the limitations of synthetic datasets.

Method: The authors systematically generate six synthetic datasets for traffic sign recognition, varying camera settings and background correlations, to isolate and measure their impact on classification performance and feature importance.

Result: Results quantify how background features influence classification tasks based on training domain changes, providing insights into when and why classifiers rely on background features.

Conclusion: The study concludes that synthetic datasets can effectively isolate and measure specific influences on classifier behavior, offering a clearer understanding of XAI interpretations.
...
TL;DR: The paper examines the effectiveness of synthetic facial datasets, comparing various generation techniques in facial recognition tasks, and highlights their potential to address privacy, bias, and data degradation issues while noting remaining challenges.


<details>
  <summary>Details</summary>
Motivation: To address challenges in facial recognition like privacy concerns, demographic bias, and dataset degradation, synthetic data generation is explored as a solution.

Method: The study evaluates synthetic datasets generated using techniques like diffusion models, GANs, and 3D models, testing their performance on accuracy, rank-1, rank-5, and true positive rate metrics across eight datasets.

Result: Synthetic data shows promise in capturing realistic variations but still lags behind real data in performance, with room for improvement.

Conclusion: While synthetic facial data mitigates key issues and advances techniques, further research is needed to bridge the performance gap with real data.
...
TL;DR: A synthesis pipeline combines GAN-based texture generation and analytical scene modulation to create realistic traffic sign images for recognition tasks, evaluated against real-world and synthetic benchmarks.


<details>
  <summary>Details</summary>
Motivation: To enhance traffic sign recognition training/testing by merging data-driven and analytical approaches for realism and parameterization, aiding explainable AI and robustness tests.

Method: Uses GANs for texture generation (dirt/wear artifacts) and analytical modeling for lighting/scene modulation, producing detailed parameterization.

Result: Created Synset Signset Germany with 105,500 images of 211 German traffic signs, including rare ones, plus metadata for evaluation.

Conclusion: The synthetic dataset shows high realism comparable to real-world benchmarks, useful for XAI and robustness testing.
...
TL;DR: AQUA-Net is an underwater image enhancement model combining frequency fusion and illumination-aware techniques to address color distortion and low contrast, achieving state-of-the-art results with lower computational costs.


<details>
  <summary>Details</summary>
Motivation: Underwater images often suffer from color distortion, low contrast, and haze due to light absorption and scattering. Existing deep learning models are computationally intensive, limiting real-time applications.

Method: AQUA-Net uses a residual encoder-decoder with dual auxiliary branches in frequency and illumination domains. The frequency fusion encoder incorporates Fourier domain cues, while the illumination-aware decoder corrects exposure using a learned illumination map.

Result: AQUA-Net matches state-of-the-art performance in qualitative and quantitative evaluations while using fewer parameters. Ablation studies confirm the complementary benefits of frequency and illumination branches.

Conclusion: AQUA-Net offers a robust, generalizable solution for real-world underwater imaging, improving color balance, contrast, and realism with reduced computational complexity.
...
TL;DR: The paper proposes a deliberative editing framework, EditThinker, that improves instruction-based image editing by simulating human cognitive loops through iterative critique and refinement.


<details>
  <summary>Details</summary>
Motivation: Existing methods for instruction-based image editing struggle with adherence due to inherent stochasticity and lack of deliberation.

Method: The framework uses a Think-while-Edit cycle involving critique, refinement, and repetition, powered by a single MLLM trained via reinforcement learning.

Result: EditThinker significantly enhances instruction-following capability across four benchmarks.

Conclusion: The approach advances instruction-based image editing and promises community benefits through released data and models.
...
TL;DR: Fine-tuning BERT for university course Q&A shows promise for domain-specific educational chatbots.


<details>
  <summary>Details</summary>
Motivation: To address the gap in domain-specific foundation models tailored for university course materials, aiming to improve educational Q&A systems.

Method: Constructed a custom dataset of 1,203 Q&A pairs from university modules, fine-tuned BERT using PyTorch, and evaluated with Exact Match and F1 scores.

Result: Modest fine-tuning improved hypothesis framing and knowledge extraction, demonstrating feasibility for educational domains.

Conclusion: Fine-tuning BERT for academic Q&A is effective, paving the way for domain-specific university QA models and autonomous educational systems.
...
TL;DR: A novel method quantifies polarization based on emotional discourse style (Valence, Arousal, Dominance) rather than ideology, applied to Israeli parliament data showing increasing affective polarization.


<details>
  <summary>Details</summary>
Motivation: To address rising global polarization by shifting focus from ideological differences to emotional discourse patterns.

Method: Use Valence, Arousal, and Dominance measures to analyze emotional style in parliamentary discourse (Hebrew corpus from the Knesset).

Result: Government and opposition members exhibit distinct emotional styles, with affective polarization rising over time.

Conclusion: Emotional discourse style is a viable metric for polarization, revealing increasing affective divides in political settings.
...
TL;DR: The study explores using poetry prompts ('Poetry Prompt Patterns') in prompt engineering to analyze LLMs' creative text generation and biases, including rewriting original works for audiences.


<details>
  <summary>Details</summary>
Motivation: To enhance prompt engineering by incorporating creative text prompting, specifically poetry, and to evaluate LLMs' tendencies in adapting creative works.

Method: The paper introduces Poetry Prompt Patterns and uses poetic prompts to assess descriptions and evaluations of three LLMs, testing their adaptation of original creative works.

Result: Poetic prompts prove useful for analyzing LLMs' creative capabilities and biases in rewriting or adapting poetry for different audiences.

Conclusion: Creative text prompting, like Poetry Prompt Patterns, can be a valuable tool in prompt engineering for understanding LLMs' creative and adaptive behaviors.
...
TL;DR: The paper explores using Chain-of-Thought (CoT) prompt engineering with large language models (LLMs) to generate clinical notes from ICD codes and patient info, enhanced by semantic search and a knowledge graph, showing improved results over standard prompts.


<details>
  <summary>Details</summary>
Motivation: Manual clinical note-writing consumes doctors' time, delaying patient care. Leveraging LLMs can streamline this process.

Method: Combines CoT prompts with semantic search and a clinical knowledge graph, tested on GPT-4 using CodiEsp dataset cases.

Result: The proposed method outperformed standard one-shot prompts in generating clinical notes.

Conclusion: Enhanced CoT prompting with domain-specific knowledge improves LLM-generated clinical notes.
...
TL;DR: Meta-training techniques like CoT-Recipe improve reasoning in LLMs by carefully balancing CoT and non-CoT examples, boosting accuracy on novel tasks.


<details>
  <summary>Details</summary>
Motivation: Address the inefficiency of CoT-ICL on novel tasks due to insufficient pre-training knowledge by developing better meta-training methods.

Method: Propose CoT-Recipe, a method to modulate the mix of CoT and non-CoT examples in meta-training sequences to enhance reasoning.

Result: CoT-Recipe increases transformer accuracy by up to 300% on novel tasks, and improves pretrained LLMs (Qwen2.5) by up to 130% for symbolic reasoning.

Conclusion: Careful modulation of CoT and non-CoT examples during meta-training significantly enhances reasoning performance in LLMs on novel tasks.
...
TL;DR: Analysis of Pink Slime Journalism reveals unique linguistic patterns and threats from LLMs, proposing a robust detection framework improving resilience by 27%.


<details>
  <summary>Details</summary>
Motivation: The study addresses the threat posed by Pink Slime Journalism to local news integrity, emphasizing the need for detection methods resistant to LLM-based adversarial attacks.

Method: Comprehensive analysis of Pink Slime articles' linguistic, stylistic, and lexical traits, followed by developing a detection framework resilient to LLM modifications.

Result: Consumer-accessible LLMs reduce detection system performance by 40%, but the proposed framework improves resilience by up to 27%.

Conclusion: The research highlights LLMs as a new adversarial vector in Pink Slime Journalism and offers a robust solution to maintain detection effectiveness.
...
TL;DR: Hybrid neural-symbolic methods reveal dynamic complexity shifts in Sanskrit's evolution, challenging simplification assumptions, using weak supervision and ensemble techniques for robust analysis.


<details>
  <summary>Details</summary>
Motivation: To address data scarcity and quantitatively analyze linguistic change in Sanskrit, challenging the assumption that evolution simplifies morphology.

Method: Weakly-supervised hybrid approach using regex patterns for pseudo-labels, fine-tuning multilingual BERT, and fusing symbolic-neural outputs via a confidence-weighted ensemble.

Result: 52.4% feature detection rate; Sanskrit's complexity is redistributed, not reduced, with new domains like compounding expanding; high confidence-accuracy correlation (r=0.92).

Conclusion: Hybrid methods offer scalable, interpretable insights into language evolution, with uncertainty estimates enhancing reliability for computational philology.
...
TL;DR: LM judges show self-preference bias, preferring their own outputs. Simple perturbations like synonym replacement reduce bias, but deeper mitigation is difficult due to multi-level semantic recognition.


<details>
  <summary>Details</summary>
Motivation: To address the bias of LM judges favoring their own outputs, which undermines unbiased evaluations.

Method: Applied black-box perturbations (e.g., synonym replacement) to obfuscate authorship and reduce self-recognition in pairwise comparisons.

Result: Simple perturbations predictably reduce self-preference, but deeper neutralization of stylistic differences causes bias to recover.

Conclusion: Self-preference is hard to fully mitigate due to multi-level semantic recognition, though initial results are promising.
...
TL;DR: SCRPO reduces LLM hallucinations in summarization by self-critiquing and refining outputs during training, improving faithfulness without extra test-time compute.


<details>
  <summary>Details</summary>
Motivation: Addressing hallucinations in LLMs during long-form text generation by avoiding costly test-time refinement or reliance on powerful teacher models.

Method: Self-supervised training framework (SCRPO) using the LLM's own critique and refinement to create a preference dataset, followed by preference learning for faithful summarization.

Result: Outperforms state-of-the-art self-supervised methods in faithfulness on XSUM, CNNDM, and SAMSum benchmarks, while maintaining or improving other summary quality metrics.

Conclusion: SCRPO is an efficient and effective alternative to test-time refinement, enhancing faithfulness in LLM-generated summaries without additional computational costs.
...
TL;DR: The paper introduces SQ-format, a unified data format combining quantization and sparsification to improve LLM efficiency without significant accuracy loss, leveraging hardware acceleration possibilities.


<details>
  <summary>Details</summary>
Motivation: Existing low-bit quantization and sparsification techniques struggle with balancing accuracy and efficiency due to hardware limitations. SQ-format aims to bridge this gap.

Method: Proposes SQ-format, which unifies quantization and sparsification, leveraging high-precision sparse matrix acceleration and low-precision multiplication.

Result: SQ-format achieves state-of-the-art PTQ performance, enabling Pareto improvement in performance and throughput, especially for activations with outlier inequality.

Conclusion: SQ-format advances PTQ for LLMs, suggests hardware support requirements, and provides insights for future AI accelerator designs.
...
TL;DR: This paper presents an empirical study on using pretrained language models (PLMs) for spell correction in low-resource languages (LRLs). It finds that large language models (LLMs) perform best and introduces LMSpell, a toolkit for spell correction across PLMs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of spell correction in LRLs, where existing PLM applications are limited and lack comparative analysis.

Method: The study compares different PLMs (encoder-based, encoder-decoder, and LLMs) for spell correction in LRLs, including languages not pre-trained for the models. It introduces LMSpell, a toolkit with an evaluation function to handle LLM hallucinations.

Result: LLMs outperform other PLMs, especially with large fine-tuning datasets, even in languages they weren't pre-trained for. The paper includes a case study on Sinhala.

Conclusion: LLMs are effective for spell correction in LRLs. The LMSpell toolkit facilitates cross-PLM spell correction, addressing hallucination issues and highlighting challenges for LRLs.
...
TL;DR: Researchers address LLMs' limited music reasoning by introducing MusWikiDB and ArtistMus, showing RAG improves MQA accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs lack music knowledge; structured MQA resources are scarce.

Method: Created MusWikiDB (3.2M passages) and ArtistMus (1,000 questions), evaluated RAG for MQA.

Result: RAG boosts accuracy (e.g., +56.8 pts for Qwen3 8B) and enables faster retrieval.

Conclusion: Resources advance MQA, proving RAG's value in culturally rich domains like music.
...
TL;DR: The paper introduces Collective Agency (CA) as a holistic alignment value and proposes Dynamic Alignment, a scalable self-improving framework for LLMs, leveraging automated dataset generation and self-rewarding mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current alignment methods for LLMs, based on human feedback or predefined principles, are resource-intensive and may be insufficient for advanced AI like AGI and ASI. The paper seeks a more scalable and comprehensive solution.

Method: The authors propose Dynamic Alignment, which includes automated training dataset generation and a self-rewarding mechanism where the model evaluates and rewards its own outputs for iterative learning.

Result: Experiments show the approach successfully aligns the model to CA while maintaining general NLP capabilities.

Conclusion: The work presents a scalable and holistic alignment framework for LLMs, advancing beyond conventional methods and addressing challenges in AI alignment for future AGI/ASI systems.
...
TL;DR: SEA-SafeguardBench is introduced as the first human-verified safety benchmark for Southeast Asian languages, addressing gaps in evaluating LLMs' ability to handle culturally diverse harm scenarios.


<details>
  <summary>Details</summary>
Motivation: Current safeguards for LLMs are English-centric and overlook linguistic and cultural diversity, particularly in Southeast Asian languages, which face unique safety concerns.

Method: SEA-SafeguardBench includes natively authored, human-verified samples across eight languages and 21,640 samples, categorized into general, in-the-wild, and content generation subsets.

Result: State-of-the-art LLMs and guardrails struggle with SEA cultural and harm scenarios, underperforming compared to English texts.

Conclusion: The benchmark highlights the need for culturally diverse evaluations to improve LLMs' safety mechanisms for underrepresented languages.
...
TL;DR: Generative LLMs with lesion tagging and anatomical context outperform supervised models in detecting follow-up-worthy incidentalomas, matching expert performance.


<details>
  <summary>Details</summary>
Motivation: Address limitations of document-level classification systems by improving fine-grained, lesion-level detection of incidentalomas requiring follow-up.

Method: Compared supervised transformers (BioClinicalModernBERT, ModernBERT, Clinical Longformer) with generative LLMs (Llama 3.1-8B, GPT-4o, GPT-OSS-20b) using lesion-tagged inputs and anatomy-aware prompting on annotated radiology reports.

Result: GPT-OSS-20b with anatomical context achieved highest macro-F1 (0.79), surpassing supervised models (max 0.70). Ensembles improved F1 to 0.90. Anatomy-aware models showed better contextual reasoning.

Conclusion: Enhanced generative LLMs outperform supervised encoders, offering reliable and interpretable automation for incidental finding surveillance in radiology.
...
TL;DR: Tree-of-Thought (ToT) reasoning improves solving Bengali Mathematical Word Problems (MWPs) over Chain-of-Thought (CoT), achieving 88% accuracy with GPT-OSS-120B.


<details>
  <summary>Details</summary>
Motivation: Address limitations of CoT's linear error propagation in MWPs by exploring ToT's effectiveness.

Method: Evaluated 100 Bengali MWPs using SOMADHAN dataset across LLMs (GPT-OSS, LLaMA) under standard, CoT, and ToT prompting.

Result: CoT improved accuracy from 78% to 83%, while ToT further increased it to 88% (GPT-OSS-120B).

Conclusion: ToT is robust for low-resource languages like Bengali and offers more reliable outcomes than CoT.
...
TL;DR: The paper introduces a large, open corpus of Greek government decisions with high-quality text extracted from PDFs. It includes a reproducible pipeline, qualitative analyses, and a RAG task evaluation, showcasing its potential for advanced information access and transparency. The corpus is also valuable for training LMs and LLMs in legal and governmental domains.


<details>
  <summary>Details</summary>
Motivation: The motivation was to create a scalable, high-quality resource for promoting transparency and advanced information access in the public sector, while also supporting LM and LLM development.

Method: The method involved extracting text from PDFs using a reproducible pipeline, conducting qualitative analyses of boilerplate patterns, and designing a RAG task with representative questions and evaluations.

Result: The result is a 1-million-decision corpus with high-quality text and a demonstration of its utility for retrieval-augmented generation, transparency, and LM training.

Conclusion: The corpus is a significant resource for transparency, RAG applications, and domain-specific LM development. Future work includes addressing limitations and expanding domain adaptation.
...
TL;DR: A method to generate multilingual reasoning traces for medical QA using retrieval-augmented generation, improving LLM performance in in-domain and out-of-domain settings.


<details>
  <summary>Details</summary>
Motivation: Address the reliability and language limitations of existing English-focused medical QA approaches by grounding reasoning in factual medical knowledge across multiple languages.

Method: Generate 500k multilingual reasoning traces (English, Italian, Spanish) using retrieval-augmented generation over Wikipedia medical info, applied to translated MedQA and MedMCQA questions.

Result: Improved performance in medical QA benchmarks via in-context learning and supervised fine-tuning, achieving state-of-the-art results for 8B-parameter LLMs.

Conclusion: The released resources (traces, datasets, models) support safer and more transparent multilingual clinical decision-support tools.
...
TL;DR: ILVR is a framework combining dynamic state evolution and precise perceptual modeling for multimodal reasoning, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current latent visual reasoning methods either sacrifice precise perception or fail to handle dynamic problems due to static structures.

Method: ILVR interleaves textual generation with evolving latent visual cues and uses a self-supervision strategy with a Momentum Teacher Model for feature distillation.

Result: ILVR significantly outperforms existing approaches on multimodal reasoning benchmarks.

Conclusion: ILVR bridges the gap between fine-grained perception and sequential multimodal reasoning.
...
TL;DR: ClinEdu, a multi-agent pedagogical simulator, and ClinTeach, a teaching dataset, are developed to address the gap in medical education. MedTutor-R1, a multimodal Socratic tutor trained using ClinTeach, outperforms base models and shows adaptability in group instruction.


<details>
  <summary>Details</summary>
Motivation: The gap between increasing clinical training demands and limited expert instruction highlights the need for scalable, personalized medical education tools.

Method: Develop ClinEdu for simulating pedagogical processes, create ClinTeach dataset, train MedTutor-R1 using instruction tuning and reinforcement learning, and evaluate via simulation.

Result: MedTutor-R1 outperforms base models by over 20% in pedagogical scores and adapts well to varying student numbers.

Conclusion: ClinEdu and MedTutor-R1 demonstrate the potential of LLMs and simulators to enhance medical education through scalable, adaptive teaching methods.
...
TL;DR: Comparing a general-purpose OpenAI embedder with a domain-specific BERT model for Czech Constitutional Court decisions, the OpenAI model outperforms BERT significantly, despite noisy labels, with a robust evaluation framework.


<details>
  <summary>Details</summary>
Motivation: To assess the effectiveness of two models (a general-purpose embedder and a domain-specific BERT) for retrieving Czech Constitutional Court decisions, given the noisy and heterogeneous nature of legal databases.

Method: Compared models in three settings: (i) OpenAI embedder, (ii) domain-specific BERT trained on ~30,000 decisions using sliding windows and attention pooling. Proposed a noise-aware evaluation with IDF-weighted keyword overlap, binarization via thresholds, significance testing via paired bootstrap, and nDCG diagnosis with qualitative analysis.

Result: The general OpenAI embedder decisively outperformed the domain-specific BERT across all thresholds (@10/@20/@100) with statistically significant differences. Low absolute nDCG attributed to label drift and strong ideals.

Conclusion: The OpenAI model is more effective for legal retrieval despite noisy labels. The proposed evaluation framework is robust for noisy gold datasets, typical in legacy judicial databases.
...
TL;DR: The paper introduces a method to enhance faithfulness evaluation in LLMs by fusing elementary metrics into a combined metric using a tree-based model, validated by higher correlation with human judgments.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy of evaluating faithfulness in LLM outputs, enabling greater confidence in their deployment across diverse scenarios.

Method: Combines elementary faithfulness metrics into a fused metric using a tree-based model, integrating human judgments to weigh each metric's importance.

Result: The fused metric shows stronger correlation with human judgments across various domains, enhancing faithfulness evaluation.

Conclusion: The proposed methodology improves faithfulness evaluation in LLMs, facilitating broader and more reliable usage in diverse applications.
...
TL;DR: CICLe integrates a lightweight classifier with Conformal Prediction to enhance LLM-based text classification, improving efficiency and performance across diverse NLP benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the computational cost and prompt design dependency of LLMs in text classification, while maintaining adaptability and robustness.

Method: Combines a base classifier with Conformal Prediction to reduce candidate classes, optimizing LLM prompting.

Result: CICLe outperforms few-shot baselines, reduces shots and prompt length significantly, and works well in high class imbalance scenarios.

Conclusion: CICLe is a scalable, efficient approach that balances traditional classifier robustness with LLM adaptability, yielding substantial gains.
...
TL;DR: The paper proposes a training framework for style-controlled story generation using GRPO and a multi-reward setup, outperforming larger models in style metrics.


<details>
  <summary>Details</summary>
Motivation: To address limited fine-grained stylistic control in existing large language models for open-ended story generation, focusing on authorial style simulation with robust evaluation.

Method: Uses Group Relative Policy Optimization (GRPO) and a custom multi-reward setup, including a style reward from a fine-tuned sentence transformer with authorship verification signals, plus content and completeness scores for stability.

Result: An 8B model outperforms larger baselines like GPT-4o and Claude Sonnet 4 in style metrics (score: 0.628) while maintaining competitive content quality. Narrative completeness remains a challenge.

Conclusion: The approach demonstrates feasible agentic stylistic generation with moderate model size and task-specific training, but highlights the need for improved global coherence and story resolution modeling.
...
TL;DR: Interruptions in U.S. Supreme Court oral arguments don't change advocates' argument content significantly but are more negatively toned toward women.


<details>
  <summary>Details</summary>
Motivation: To explore how interruptions affect speech content and emotional tone, especially gendered dynamics in judicial discourse.

Method: Analyzed 12,663 speech chunks using GloVe embeddings for semantic shifts and lexicon-based sentiment analysis for emotional tone.

Result: Semantic content remains consistent post-interruption, but interruptions toward women show higher negative sentiment.

Conclusion: Interruptions reveal gendered communication patterns in elite settings, highlighting computational linguistics' role in studying judicial equity.
...
TL;DR: Assigning personas to AI models does not consistently improve accuracy on difficult multiple-choice questions, with some cases showing no effect or even degraded performance.


<details>
  <summary>Details</summary>
Motivation: To examine whether assigning expert or low-knowledge personas to AI models enhances their performance on graduate-level objective questions.

Method: Tested six models on GPQA Diamond and MMLU-Pro datasets using three approaches: in-domain expert personas, off-domain expert personas, and low-knowledge personas.

Result: Persona prompts generally did not improve accuracy; expert personas showed no consistent benefit, and low-knowledge personas often reduced accuracy.

Conclusion: Personas do not reliably enhance factual performance on difficult questions, though they may serve other purposes like altering output tone.
...
TL;DR: The paper introduces a retrieval-augmented generation (RAG) medical QA system using LoRA-fine-tuned open-source LLMs (LLaMA~2 and Falcon) to improve factual accuracy and reduce hallucinations by grounding answers in retrieved medical literature.


<details>
  <summary>Details</summary>
Motivation: Directly applying LLMs to clinical QA poses challenges like factual inaccuracies and hallucinations. The paper aims to enhance reliability by combining domain-specific retrieval with LLMs.

Method: The system uses RAG, fine-tuning LLaMA~2 and Falcon with LoRA for efficient specialization. It retrieves medical literature to ground answers, evaluated on PubMedQA and MedMCQA datasets.

Result: Fine-tuned LLaMA~2 achieved 71.8% accuracy on PubMedQA (vs. 55.4% zero-shot baseline), reducing unsupported content by ~60% and improving answer accuracy.

Conclusion: RAG-augmented open-source LLMs show promise for reliable biomedical QA, offering practical clinical applications with improved accuracy and transparency.
...
TL;DR: M4-RAG introduces a multilingual, multimodal benchmark for evaluating retrieval-augmented VQA, revealing limitations in scaling RAG for larger VLMs.


<details>
  <summary>Details</summary>
Motivation: To address the constraints of static training data in VLMs and explore the underexplored area of multilingual multimodal RAG.

Method: Creation of M4-RAG, a large-scale benchmark with 42 languages and 56 dialects, coupled with a controlled retrieval environment.

Result: RAG benefits smaller VLMs but fails to scale to larger models, sometimes degrading their performance.

Conclusion: M4-RAG lays the groundwork for improving RAG systems to handle multilingual, multimodal, and culturally diverse contexts.
...
TL;DR: This paper introduces a distributed scalable coupled policy (DSCP) algorithm for networked multi-agent reinforcement learning (NMARL) with interdependent rewards and policies, achieving efficient optimization and convergence to a first-order stationary point.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of interdependent policies and rewards in networked multi-agent systems, where agents must collaborate to optimize their policies while relying on localized information.

Method: Develops the DSCP algorithm, leveraging neighbors' averaged Q-function, coupled policy gradient, and geometric 2-horizon sampling. Uses push-sum protocol for distributed policy updates.

Result: Proves convergence to a first-order stationary point and demonstrates improved performance in robot path planning simulations compared to existing methods.

Conclusion: The DSCP algorithm effectively handles interdependencies in NMARL, offering scalable and distributed optimization with practical benefits.
...
### [113] [Semantic Faithfulness and Entropy Production Measures to Tame Your LLM Demons and Manage Hallucinations](https://arxiv.org/abs/2512.05156)
*Igor Halperin*

Main category: cs.AI

TL;DR: Proposes two unsupervised metrics (SF and SEP) for evaluating LLM faithfulness using information theory and thermodynamics, applied to summarization tasks.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of assessing LLM faithfulness to tasks without supervision, leveraging insights from information theory and thermodynamics.

Method: Models QCA triplets as probability distributions over topics, uses KL divergence for SF, and introduces SEP for entropy production. Optimizes via convex optimization.

Result: SF and SEP metrics effectively evaluate LLM faithfulness, with high SF indicating low SEP, demonstrated on SEC 10-K summarization.

Conclusion: The proposed unsupervised metrics (SF and SEP) provide a reliable framework for LLM evaluation and hallucination control in tasks like summarization.
...
### [114] [Bridging Traditional Machine Learning and Large Language Models: A Two-Part Course Design for Modern AI Education](https://arxiv.org/abs/2512.05167)
*Fang Li*

Main category: cs.AI

TL;DR: An innovative AI and data science course combines traditional ML with modern LLMs, enhancing student understanding and industry readiness.


<details>
  <summary>Details</summary>
Motivation: To bridge foundational ML techniques with contemporary LLM applications, providing students a holistic view of AI evolution.

Method: Course structured in two parts: foundational ML and modern LLMs, with detailed architecture, implementation, and assessment strategies.

Result: Integrated approach improves student comprehension of AI and prepares them better for industry demands.

Conclusion: The course successfully merges traditional and modern AI techniques, benefiting student learning and career readiness.
...
### [115] [On the Computability of Artificial General Intelligence](https://arxiv.org/abs/2512.05212)
*Georgios Mappouras,Charalambos Rossides*

Main category: cs.AI

TL;DR: The paper explores the limits of AI and algorithms, proving that no algorithm can achieve true creativity or innovate beyond its initial capabilities.


<details>
  <summary>Details</summary>
Motivation: To determine the upper bounds of AI and machine-computable processes, specifically addressing whether AI can achieve human-level intelligence (AGI).

Method: The study defines AGI based on prior work, focusing on creativity and innovation, and formally proves the limitations of algorithms.

Result: No algorithm can demonstrate truly new functional capabilities; AI can only replicate or combine existing ones.

Conclusion: The proof has implications for AI development and raises questions about the uniqueness of human intelligence.
...
### [116] [Resolving Zadehs Paradox Axiomatic Possibility Theory as a Foundation for Reliable Artificial Intelligence](https://arxiv.org/abs/2512.05257)
*Bychkov Oleksii,Bychkova Sophia,Lytvynchuk Khrystyna*

Main category: cs.AI

TL;DR: The paper proposes that possibility theory, particularly its axiomatic approach, resolves DST paradoxes better than probabilistic or evidential paradigms, demonstrated through a medical diagnostic example.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of Dempster-Shafer Theory (DST) by showing that possibility theory offers a logically consistent and mathematically rigorous alternative for handling uncertainty.

Method: Comparative analysis of probabilistic, evidential, and possibilistic paradigms, illustrated with a classic medical diagnostic dilemma to showcase possibility theory's advantages.

Result: Possibility theory successfully processes contradictory data and avoids DST's logical pitfalls, aligning formal reasoning with natural intelligence.

Conclusion: Possibility theory provides a foundational resolution to DST paradoxes, proving superior in handling uncertainty compared to other paradigms.
...
### [117] [AI & Human Co-Improvement for Safer Co-Superintelligence](https://arxiv.org/abs/2512.05356)
*Jason Weston,Jakob Foerster*

Main category: cs.AI

TL;DR: The paper advocates for co-improvement (human-AI collaboration in research) as a safer and more achievable alternative to pure AI self-improvement, aiming for co-superintelligence.


<details>
  <summary>Details</summary>
Motivation: To address the dangers and challenges of AI self-improvement by proposing a collaborative approach where humans and AIs work together to achieve safer and faster progress in AI research.

Method: Focus on enhancing AI systems' ability to collaborate with human researchers across all stages of AI research, from ideation to experimentation.

Result: A symbiotic relationship between humans and AIs can accelerate AI research and lead to safer superintelligence.

Conclusion: Co-improvement is a more practical and safer path to superintelligence than pure AI self-improvement, benefiting both humans and AIs.
...
### [118] [MCP-AI: Protocol-Driven Intelligence Framework for Autonomous Reasoning in Healthcare](https://arxiv.org/abs/2512.05365)
*Zag ElSayed,Craig Erickson,Ernest Pedapati*

Main category: cs.AI

TL;DR: Introducing MCP-AI, a novel healthcare AI framework combining Model Context Protocol (MCP) for adaptive, explainable, and collaborative clinical reasoning, validated through diagnostic and coordination use cases.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of traditional CDSS and stateless LLMs in healthcare by enabling context-aware, long-term reasoning and secure collaboration.

Method: Integration of MCP with clinical applications, creating reusable, auditable memory objects for real-time workflows and physician-in-the-loop validation.

Result: Validated via Fragile X Syndrome and Type 2 Diabetes use cases, MCP-AI improves clinical processes, ensures regulatory compliance, and supports interpretable AI.

Conclusion: MCP-AI offers a scalable, safety-oriented solution for autonomous clinical reasoning in complex healthcare environments.
...
TL;DR: ChipMind enhances LLM-aided IC development by using a knowledge graph (ChipKG) for improved semantic modeling and reasoning over lengthy circuit specs, outperforming baselines by ~34.59%.


<details>
  <summary>Details</summary>
Motivation: Address limitations of LLMs in IC development due to restricted context windows and ineffective existing methods for handling extensive, complex circuit specs.

Method: Transforms circuit specs into ChipKG, then uses adaptive retrieval and semantic filtering for precise, thorough reasoning.

Result: Achieves 34.59% average improvement (up to 72.73%) on industrial-scale benchmarks.

Conclusion: ChipMind bridges the gap between academic research and practical LLM-aided hardware design deployment.
...
### [120] [BEAVER: An Efficient Deterministic LLM Verifier](https://arxiv.org/abs/2512.05439)
*Tarun Suresh,Nalin Wadhwa,Debangshu Banerjee,Gagandeep Singh*

Main category: cs.AI

TL;DR: BEAVER is a framework for computing deterministic, sound probability bounds on LLM constraint satisfaction, outperforming baseline methods with tighter bounds and better risk identification.


<details>
  <summary>Details</summary>
Motivation: Practitioners need reliable methods to verify that LLM outputs satisfy constraints, as sampling-based estimates lack sound guarantees.

Method: BEAVER uses token trie and frontier data structures to systematically explore the generation space and maintain sound bounds for any prefix-closed semantic constraint.

Result: BEAVER provides 6-8 times tighter probability bounds and identifies 3-4 times more high-risk instances compared to baselines.

Conclusion: BEAVER enables precise characterization and risk assessment of LLM outputs, offering improvements over loose bounds or empirical evaluation.
...
### [121] [The Seeds of Scheming: Weakness of Will in the Building Blocks of Agentic Systems](https://arxiv.org/abs/2512.05449)
*Robert Yang*

Main category: cs.AI

TL;DR: The paper introduces 'akrasia' (weakness of will) as a concept to analyze inconsistency in large language models, proposing the Akrasia Benchmark to measure self-control and explore its implications for AI systems.


<details>
  <summary>Details</summary>
Motivation: To address the inconsistency in large language models where they know the correct answer but fail to act on it, drawing parallels with human philosophical concepts like akrasia.

Method: Introduces the Akrasia Benchmark, which includes structured prompting conditions (Baseline, Synonym, Temporal, and Temptation) to quantitatively measure inconsistencies across models and scenarios.

Result: The benchmark allows for evaluating how models exhibit akrasia under different conditions, shedding light on potential macro-level instability in multi-agent systems.

Conclusion: By framing AI inconsistency as weakness of will, the work bridges philosophy, psychology, and AI science, offering tools to study and mitigate goal drift in agentic systems.
...
### [122] [MIND: Multi-rationale INtegrated Discriminative Reasoning Framework for Multi-modal Large Models](https://arxiv.org/abs/2512.05530)
*Chuang Yu,Jinmiao Zhao,Mingxuan Zhao,Yunpeng Liu,Xiujun Shu,Yuanhao Feng,Bo Wang,Xiangyu Yue*

Main category: cs.AI

TL;DR: The paper introduces the MIND framework to enhance multimodal large language models (MLLMs) by improving multi-rationale reasoning, logical robustness, and reducing susceptibility to errors in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs face limitations in multi-rationale semantic modeling and logical robustness, leading to inaccurate interpretations in complex tasks. The goal is to advance MLLMs towards human-like cognitive abilities.

Method: The MIND framework includes Rationale Augmentation and Discrimination (RAD) for dataset expansion, Progressive Two-stage Correction Learning (P2CL) for multi-rationale learning and correction, and Multi-rationale Contrastive Alignment (MCA) for semantic optimization.

Result: The framework achieves state-of-the-art performance on multiple datasets (scientific, commonsense, and mathematical), demonstrating improved reasoning capabilities.

Conclusion: MIND provides a novel approach to enhancing MLLMs' cognitive intelligence by integrating discriminative reasoning and multi-rationale optimization, offering a significant advancement over passive imitation-based models.
...
### [123] [CureAgent: A Training-Free Executor-Analyst Framework for Clinical Reasoning](https://arxiv.org/abs/2512.05576)
*Ting-Ting Xie,Yixin Zhang*

Main category: cs.AI

TL;DR: The paper introduces the Executor-Analyst Framework to address Context Utilization Failure in small LLMs like TxAgent, improving clinical reasoning by separating tool execution and reasoning tasks. It also highlights scaling challenges and achieves top performance without costly finetuning.


<details>
  <summary>Details</summary>
Motivation: Small LLMs in clinical agents often retrieve biomedical evidence but fail to use it effectively for diagnosis, a problem called Context Utilization Failure.

Method: Proposes the Executor-Analyst Framework, using specialized TxAgents (Executors) and long-context models (Analysts), alongside a Stratified Ensemble strategy for better evidence diversity.

Result: Outperforms monolithic models, identifies scaling challenges (e.g., Context-Performance Paradox), and achieves state-of-the-art results on CURE-Bench without finetuning.

Conclusion: The framework offers a scalable, training-free solution for trustworthy AI-driven therapeutics, addressing reasoning deficits and scaling limitations.
...
### [124] [Ontology Learning with LLMs: A Benchmark Study on Axiom Identification](https://arxiv.org/abs/2512.05594)
*Roos M. Bakker,Daan L. Di Scala,Maaike H. T. de Boer,Stephan A. Raaijmakers*

Main category: cs.AI

TL;DR: The paper introduces OntoAxiom, a benchmark for evaluating LLMs in identifying ontology axioms, comparing different prompting strategies and models. Results show Axiom-by-Axiom prompting is more effective, but performance varies by axiom type and ontology domain.


<details>
  <summary>Details</summary>
Motivation: Ontology development is complex and requires automation. This study explores LLMs' potential in automating axiom identification, a key ontology component, using a structured benchmark.

Method: Created OntoAxiom benchmark with 9 ontologies (17,118 triples, 2,771 axioms). Tested 12 LLMs using Direct and Axiom-by-Axiom (AbA) prompting strategies, evaluating subclass, disjoint, subproperty, domain, and range axioms.

Result: AbA prompting outperforms Direct prompting. Performance varies by axiom type and ontology domain (e.g., FOAF subclass: 0.642; music subclass: 0.218). Larger LLMs perform better, but smaller models are viable for resource-limited settings.

Conclusion: LLMs can't fully automate axiom identification yet but provide useful candidate axioms to support ontology engineers.
...
### [125] [Enhancing Local Search for MaxSAT with Deep Differentiation Clause Weighting](https://arxiv.org/abs/2512.05619)
*Menghua Jiang,Haokai Gao,Shuhao Chen,Yin Chen*

Main category: cs.AI

TL;DR: DeepDist introduces a novel clause weighting scheme and decimation method for solving PMS and WPMS, outperforming state-of-the-art SLS solvers and hybrid approaches.


<details>
  <summary>Details</summary>
Motivation: Existing clause weighting schemes inadequately distinguish between PMS and WPMS, prompting the need for tailored update strategies and initialization methods.

Method: Develops a new clause weighting scheme with distinct update conditions for PMS and WPMS, a novel initialization method, and a decimation method prioritizing unit and hard clauses.

Result: DeepDist outperforms state-of-the-art SLS solvers and hybrid approaches in benchmarks, surpassing MaxSAT Evaluation 2024 winners.

Conclusion: DeepDist's tailored approach effectively addresses structural differences between PMS and WPMS, achieving superior performance.
...
### [126] [KANFormer for Predicting Fill Probabilities via Survival Analysis in Limit Order Books](https://arxiv.org/abs/2512.05734)
*Jinfeng Zhong,Emmanuel Bacry,Agathe Guilloux,Jean-François Muzy*

Main category: cs.AI

TL;DR: KANFormer is a deep-learning model for predicting limit order time-to-fill, combining Dilated Causal Convolutional networks and Transformers with Kolmogorov-Arnold Networks (KANs) for better approximation. It integrates agent actions and queue position, outperforming existing models in accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing models rely on snapshots of the limit order book, missing dynamic agent actions and queue position. KANFormer aims to capture these for better execution likelihood predictions.

Method: KANFormer uses a Dilated Causal Convolutional network and Transformer encoder, enhanced by KANs for nonlinear approximation, integrating agent actions and queue position.

Result: Evaluated on CAC 40 index futures data, KANFormer outperforms existing models in calibration (RCLL, IBS) and discrimination (C-index, AUC). SHAP analysis highlights feature importance over time.

Conclusion: KANFormer demonstrates the benefits of combining market signals with neural architectures for accurate, interpretable fill probability predictions.
...
### [127] [A Fast Anti-Jamming Cognitive Radar Deployment Algorithm Based on Reinforcement Learning](https://arxiv.org/abs/2512.05753)
*Wencheng Cai,Xuchao Gao,Congying Han,Mingqiang Li,Tiande Guo*

Main category: cs.AI

TL;DR: FARDA uses deep reinforcement learning for fast and efficient radar deployment, outperforming evolutionary algorithms in speed while maintaining coverage.


<details>
  <summary>Details</summary>
Motivation: Existing methods for radar deployment are slow and prone to local optima, hindering fast deployment in jamming scenarios.

Method: Models radar deployment as an end-to-end task using deep reinforcement learning, with neural modules for heatmap perception and a novel reward format.

Result: Achieves similar coverage to evolutionary algorithms but is ~7,000 times faster, with ablation studies validating FARDA's components.

Conclusion: FARDA offers a scalable and efficient solution for cognitive radar deployment, addressing the limitations of traditional methods.
...
### [128] [Evolutionary System 2 Reasoning: An Empirical Proof](https://arxiv.org/abs/2512.05760)
*Zeyuan Ma,Wenqi Huang,Guo-Huan Song,Hongshu Guo,Sijie Ma,Zhiguang Cao,Yue-Jiao Gong*

Main category: cs.AI

TL;DR: The paper explores whether machine intelligence like LLMs can develop human-like reasoning abilities. It introduces the Evolutionary Reasoning Optimization (ERO) framework to evolve LLMs for better reasoning, showing promising results even with weaker models.


<details>
  <summary>Details</summary>
Motivation: The research aims to bridge the gap between machine intelligence and human-like general reasoning, focusing on evolving LLMs beyond specific skills to achieve broader cognitive abilities.

Method: The authors propose the ERO framework, which uses evolutionary strategies to enhance reasoning in LLMs. It initializes a population of LLMs and evolves them to maximize reasoning scores.

Result: Experiments reveal that even advanced LLMs like GPT-5 lack robust system 2 reasoning, but ERO can significantly improve reasoning in weaker models like Qwen-7B.

Conclusion: The study suggests that evolutionary methods like ERO can enhance reasoning in LLMs, offering a path toward more general machine intelligence.
...
### [129] [The Missing Layer of AGI: From Pattern Alchemy to Coordination Physics](https://arxiv.org/abs/2512.05765)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: The paper argues that LLMs are misunderstood as incapable of reasoning due to lack of a System-2 coordination layer, proposing UCCT theory and MACI architecture to address this gap.


<details>
  <summary>Details</summary>
Motivation: To counter critiques that LLMs cannot achieve AGI by demonstrating how adding a coordination layer enables reasoning and planning.

Method: Introduces UCCT theory (semantic anchoring) and MACI architecture (coordination stack) to enhance LLMs.

Result: Shows how UCCT and MACI can transform LLMs from unguided pattern matchers into systems capable of goal-directed reasoning.

Conclusion: The path to AGI involves augmenting LLMs with coordination layers, not abandoning them.
...
### [130] [Multimodal Oncology Agent for IDH1 Mutation Prediction in Low-Grade Glioma](https://arxiv.org/abs/2512.05824)
*Hafsa Akebli,Adam Shephard,Vincenzo Della Mea,Nasir Rajpoot*

Main category: cs.AI

TL;DR: MOA, integrating histology and clinical/genomic data, outperforms baselines in predicting IDH1 mutations in low-grade gliomas.


<details>
  <summary>Details</summary>
Motivation: IDH1 mutations define clinically distinct glioma subgroups, necessitating accurate prediction tools.

Method: MOA combines histology (TITAN model) with clinical/genomic reasoning (PubMed, Google Search, OncoKB).

Result: MOA achieved F1-scores of 0.826 (without histology) and 0.912 (with histology), surpassing baselines.

Conclusion: MOA effectively predicts IDH1 mutations by leveraging complementary data and external sources.
...
### [131] [Using Large Language Models to Create Personalized Networks From Therapy Sessions](https://arxiv.org/abs/2512.05836)
*Clarissa W. Ong,Hiba Arnaout,Kate Sheehan,Estella Fox,Eugen Owtscharow,Iryna Gurevych*

Main category: cs.AI

TL;DR: The paper presents an end-to-end pipeline using LLMs to generate clinically relevant client networks from therapy transcripts, achieving high performance and expert preference.


<details>
  <summary>Details</summary>
Motivation: To address the scalability issue of personalized network-driven psychotherapy by leveraging LLMs for case conceptualization and treatment planning.

Method: An automated pipeline involving annotation of psychological processes, in-context learning for identification, clustering into networks, and generating explanation-augmented relationships.

Result: The method performed well with minimal training, and expert evaluations favored the approach for clinical utility (up to 90%) and relevance (scores 72-75%).

Conclusion: The study demonstrates LLMs' potential for creating clinically useful networks, with future research needed to compare outcomes with other methods.
...
### [132] [To Err Is Human: Systematic Quantification of Errors in Published AI Papers via LLM Analysis](https://arxiv.org/abs/2512.05925)
*Federico Bianchi,Yongchan Kwon,Zachary Izzo,Linjun Zhang,James Zou*

Main category: cs.AI

TL;DR: An AI-based Paper Correctness Checker identified a rising trend of objective errors in high-profile AI papers, with precision of 83.2% and fixes proposed for 75.8% of confirmed mistakes.


<details>
  <summary>Details</summary>
Motivation: Published AI papers serve as foundational knowledge, but errors can propagate unnoticed, causing confusion and reproducibility issues. The study aims to quantify and address objective mistakes.

Method: A GPT-5-based Paper Correctness Checker was developed to detect objective errors (e.g., formulas, calculations, figures) in top AI conference/journals papers, excluding subjective aspects.

Result: Mistakes per paper increased over time (e.g., 55.3% rise in NeurIPS papers). Human experts confirmed 83.2% precision of AI-detected errors, with fixes proposed for 75.8%.

Conclusion: Frontier LLMs like GPT-5 can effectively detect and correct objective mistakes, improving literature reliability and reproducibility.
...
### [133] [TRACE: A Framework for Analyzing and Enhancing Stepwise Reasoning in Vision-Language Models](https://arxiv.org/abs/2512.05943)
*Shima Imani,Seungwhan Moon,Lambert Mathias,Lu Zhang,Babak Damavandi*

Main category: cs.AI

TL;DR: TRACE is a framework designed to improve transparency and consistency in the reasoning of large vision-language models by evaluating intermediate steps, not just final answers, using auxiliary reasoning sets.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for large vision-language models often miss reasoning errors because they focus solely on final answers, leading to persistent silent failures.

Method: TRACE introduces Auxiliary Reasoning Sets (ARS) to break down complex problems into sub-question pairs, evaluates intermediate reasoning steps with consistency-based metrics, and identifies overlooked failures.

Result: Experiments show that consistency in ARS correlates with correct final answers and helps identify faulty reasoning steps, providing actionable insights for model improvement.

Conclusion: TRACE's confidence regions distinguish reliable from unreliable reasoning paths, aiding in filtering, debugging, and refining models effectively.
...
### [134] [Variational Quantum Rainbow Deep Q-Network for Optimizing Resource Allocation Problem](https://arxiv.org/abs/2512.05946)
*Truong Thanh Hung Nguyen,Truong Thinh Nguyen,Hung Cao*

Main category: cs.AI

TL;DR: VQR-DQN integrates quantum circuits with Rainbow DQN for HRAP, achieving significant performance improvements over classical methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of classical function approximators in DRL for resource allocation by leveraging quantum computing capabilities.

Method: Combines ring-topology variational quantum circuits with Rainbow DQN, framing HRAP as an MDP with combinatorial action spaces.

Result: Achieves 26.8% normalized makespan reduction versus random baselines and outperforms Double DQN and Rainbow DQN by 4.9-13.4%.

Conclusion: Demonstrates the potential of quantum-enhanced DRL for scalable resource allocation, supported by theoretical insights into quantum circuit properties.
...
### [135] [SymPyBench: A Dynamic Benchmark for Scientific Reasoning with Executable Python Code](https://arxiv.org/abs/2512.05954)
*Shima Imani,Seungwhan Moon,Adel Ahmadyan,Lu Zhang,Kirmani Ahmed,Babak Damavandi*

Main category: cs.AI

TL;DR: SymPyBench is a large-scale synthetic physics benchmark with diverse question types and dynamic, code-driven solutions, introducing novel evaluation metrics to assess reasoning in language models.


<details>
  <summary>Details</summary>
Motivation: To create a robust foundation for evaluating and improving scientific reasoning in AI systems by providing parameterized problems with structured reasoning and executable code.

Method: A synthetic benchmark of 15,045 university-level physics problems with three question types (MC-Symbolic, MC-Numerical, free-form) was developed, generating infinite input configurations and executable solutions. Novel metrics (Consistency Score, Failure Rate, Confusion Rate) were introduced.

Result: Experiments with state-of-the-art language models revealed strengths and limitations in scientific reasoning, highlighting the need for more robust and interpretable systems.

Conclusion: SymPyBench serves as a valuable tool for advancing AI reasoning capabilities, with its dynamic and diverse problem sets providing insights into model performance and areas for improvement.
...
### [136] [Advanced Unsupervised Learning: A Comprehensive Overview of Multi-View Clustering Techniques](https://arxiv.org/abs/2512.05169)
*Abdelmalik Moujahid,Fadi Dornaika*

Main category: cs.LG

TL;DR: This survey paper explores multi-view clustering (MVC) as a solution to challenges in single-view learning, categorizes MVC methods, analyzes their strengths/weaknesses, and discusses future trends.


<details>
  <summary>Details</summary>
Motivation: Machine learning struggles with single-view limitations and large datasets. MVC offers richer data representation and effective solutions.

Method: Systematic categorization of MVC methods (e.g., co-training, deep learning) and analysis of their strengths/weaknesses.

Result: Identifies practical challenges like scalability and incomplete data, and reviews over 140 publications.

Conclusion: The survey fills gaps in MVC research and provides insights for future advancements.
...
### [137] [Coefficient of Variation Masking: A Volatility-Aware Strategy for EHR Foundation Models](https://arxiv.org/abs/2512.05216)
*Rajna Fani,Rafi Al Attrach,David Restrepo,Yugang Jia,Leo Anthony Celi,Peter Schüffler*

Main category: cs.LG

TL;DR: The paper proposes CV-Masking, a volatility-aware pretraining strategy for masked autoencoders in EHR, improving reconstruction and downstream performance by adapting masking probabilities to feature variability.


<details>
  <summary>Details</summary>
Motivation: Existing MAE approaches use uniform random masking, ignoring the varying predictability of EHR features like lab tests. Volatile biomarkers often indicate acute conditions and need better modeling.

Method: Introduces CV-Masking, adjusting masking probabilities based on each feature's intrinsic variability (coefficient of variation), paired with a value-only masking objective.

Result: CV-Masking outperforms random and variance-based strategies in reconstruction, downstream prediction, and convergence, yielding clinically meaningful EHR representations.

Conclusion: CV-Masking enhances MAE performance by accounting for feature volatility, improving robustness and clinical relevance of learned EHR representations.
...
### [138] [Rethinking Tokenization for Clinical Time Series: When Less is More](https://arxiv.org/abs/2512.05217)
*Rafi Al Attrach,Rajna Fani,David Restrepo,Yugang Jia,Peter Schüffler*

Main category: cs.LG

TL;DR: The study evaluates tokenization strategies for clinical time series modeling, showing task-dependent effectiveness and the surprising finding that explicit time encodings don't consistently help, while frozen pretrained encoders outperform trainable ones.


<details>
  <summary>Details</summary>
Motivation: To systematically compare tokenization approaches for clinical time series modeling, addressing gaps in fair comparisons and understanding their impact on transformer-based architectures.

Method: Controlled ablations across four clinical prediction tasks on MIMIC-IV, testing tokenization strategies like explicit time encodings, value features, and frozen vs. trainable pretrained encoders.

Result: Explicit time encodings lack consistent benefits; value features impact mortality prediction but not readmission; frozen pretrained encoders outperform trainable ones with fewer parameters; larger encoders improve performance.

Conclusion: Simpler, parameter-efficient tokenization can perform well, but the optimal strategy depends on the task. Frozen embeddings eliminate overhead and enhance performance.
...
### [139] [Variance Matters: Improving Domain Adaptation via Stratified Sampling](https://arxiv.org/abs/2512.05226)
*Andrea Napoli,Paul White*

Main category: cs.LG

TL;DR: VaRDASS introduces a stochastic variance reduction technique for UDA using stratified sampling, improving discrepancy estimation and target domain performance.


<details>
  <summary>Details</summary>
Motivation: To address high variance in discrepancy estimates during UDA training, which hinders theoretical benefits.

Method: Proposes VaRDASS with stratified sampling for UDA, focusing on correlation alignment and MMD, and introduces a k-means style optimization algorithm.

Result: Improved discrepancy estimation accuracy and target domain performance on three datasets.

Conclusion: VaRDASS effectively reduces variance in UDA, proving optimality for MMD under assumptions and enhancing practical performance.
...
### [140] [MAR-FL: A Communication Efficient Peer-to-Peer Federated Learning System](https://arxiv.org/abs/2512.05234)
*Felix Mulitze,Herbert Woisetschläger,Hans Arno Jacobsen*

Main category: cs.LG

TL;DR: MAR-FL is a new P2P FL system that reduces communication overhead using group-based aggregation, scaling efficiently with network size while remaining resilient to churn and unreliable clients.


<details>
  <summary>Details</summary>
Motivation: Address the inefficiency and scalability issues of existing P2P FL methods due to high communication complexity.

Method: Uses iterative group-based aggregation to lower communication costs to O(N log N) from O(N^2).

Result: Achieves scalable communication costs and maintains robustness against unreliable clients and network churn.

Conclusion: MAR-FL offers a practical solution for scalable and efficient P2P FL in dynamic wireless networks.
...
### [141] [Edged Weisfeiler-Lehman Algorithm](https://arxiv.org/abs/2512.05238)
*Xiao Yue,Bo Liu,Feng Zhang,Guangzhi Qu*

Main category: cs.LG

TL;DR: The paper proposes an Edged-WL algorithm (E-WL) and an Edged Graph Isomorphism Network (EGIN) to incorporate edge features, addressing limitations in traditional GNNs and 1-WL algorithms.


<details>
  <summary>Details</summary>
Motivation: Traditional GNNs and the 1-WL algorithm often ignore edge features, limiting their potential. The authors aim to leverage edge features for improved performance.

Method: They extend the 1-WL algorithm to E-WL for edge features and introduce EGIN, a GNN model exploiting these features.

Result: Evaluated on 12 datasets, EGIN outperforms baseline models in graph classification.

Conclusion: Incorporating edge features via E-WL and EGIN enhances graph learning performance.
...
### [142] [Bridging quantum and classical computing for partial differential equations through multifidelity machine learning](https://arxiv.org/abs/2512.05241)
*Bruno Jacob,Amanda A. Howard,Panos Stinis*

Main category: cs.LG

TL;DR: A multifidelity learning framework enhances coarse quantum PDE solutions using sparse classical data, enabling high-fidelity accuracy in near-term quantum hardware.


<details>
  <summary>Details</summary>
Motivation: Quantum PDE solvers are limited by hardware constraints (qubit count and circuit depth), restricting them to low-fidelity regimes despite their theoretical speedup potential.

Method: The framework trains a low-fidelity surrogate on quantum solver outputs and learns correction mappings via a multifidelity neural architecture combining linear and nonlinear transformations.

Result: Demonstrated on nonlinear PDEs (Burgers and Navier-Stokes), the method corrects coarse quantum predictions and achieves accurate temporal extrapolation beyond classical training limits.

Conclusion: This approach bridges quantum hardware limitations and application needs, advancing practical quantum utility for scientific computing with current devices.
...
TL;DR: The paper proposes an efficient unlearning framework that identifies and ignores training data with negligible impact, reducing computational costs by up to 50%.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for efficient data removal in machine learning due to privacy concerns, especially focusing on points with minimal impact on the model.

Method: Uses influence functions to analyze and identify negligible-impact data points, then reduces the dataset size before applying unlearning techniques.

Result: Achieves up to 50% computational savings in real-world examples by focusing unlearning efforts on impactful data points.

Conclusion: The approach improves efficiency in unlearning without compromising model integrity by targeting only influential data points.
...
TL;DR: Actor-critic (AC) methods lack interpretability. RSA2C introduces attribution-aware components in RKHS for better training and interpretability, proving efficient and stable.


<details>
  <summary>Details</summary>
Motivation: Existing AC methods neglect the heterogeneous impacts of state dimensions on rewards, limiting interpretability.

Method: Proposes RSA2C, an attribution-aware, kernelized AC algorithm with RKHS-enhanced components and state attributions via RKHS--SHAP.

Result: Theoretical convergence bound ensures stability and efficiency. Empirically outperforms in continuous-control environments.

Conclusion: RSA2C achieves efficient, stable, and interpretable reinforcement learning.
...
TL;DR: The paper introduces the Continuous Flow Operator (CFO), a neural operator surrogate for time-dependent PDEs that avoids autoregressive prediction and works with arbitrary time grids, offering better stability and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional neural operator surrogates for PDEs suffer from error accumulation and require uniform time discretization. CFO aims to address these limitations by learning continuous-time dynamics more efficiently.

Method: CFO uses flow matching to learn PDE dynamics directly, fitting temporal splines to trajectory data and training a neural operator to predict velocity fields. It avoids backpropagation through ODE solvers and is time-resolution invariant.

Result: CFO shows superior long-horizon stability and data efficiency, outperforming autoregressive baselines with up to 87% error reduction, even when trained on subsampled data. It also enables reverse-time inference and arbitrary temporal querying.

Conclusion: CFO provides a more stable and efficient alternative to autoregressive methods for learning PDE dynamics, with the flexibility to handle irregular time grids and improved performance in data-scarce scenarios.
...
### [146] [Uncertainty Quantification for Scientific Machine Learning using Sparse Variational Gaussian Process Kolmogorov-Arnold Networks (SVGP KAN)](https://arxiv.org/abs/2512.05306)
*Y. Sungtaek Ju*

Main category: cs.LG

TL;DR: A framework integrating sparse variational Gaussian processes with Kolmogorov-Arnold Networks (KANs) enables scalable Bayesian uncertainty quantification while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: To address the lack of principled uncertainty quantification in standard Kolmogorov-Arnold Networks, essential for scientific applications.

Method: Integration of sparse variational Gaussian process inference with KANs topology, using analytic moment matching for uncertainty propagation.

Result: Demonstrated ability to distinguish aleatoric and epistemic uncertainty in fluid flow reconstruction, multi-step forecasting, and out-of-distribution detection.

Conclusion: SVGP KANs are a promising architecture for uncertainty-aware learning in scientific machine learning.
...
### [147] [The Erosion of LLM Signatures: Can We Still Distinguish Human and LLM-Generated Scientific Ideas After Iterative Paraphrasing?](https://arxiv.org/abs/2512.05311)
*Sadat Shahriar,Navid Ayoobi,Arjun Mukherjee*

Main category: cs.LG

TL;DR: The paper examines SOTA models' ability to distinguish human vs. LLM-generated scientific ideas, finding paraphrasing and stylistic simplification degrade detection performance.


<details>
  <summary>Details</summary>
Motivation: To address the gap in distinguishing human and LLM-generated scientific ideas, especially after paraphrasing.

Method: Systematic evaluation of SOTA models' detection capabilities with and without contextual research problem information.

Result: Detection performance drops by 25.4% after five paraphrasing stages; contextual info improves performance by 2.97%.

Conclusion: Paraphrasing, especially into non-expert styles, erodes LLM signatures, posing challenges for accurate source attribution.
...
### [148] [Enhancing Deep Deterministic Policy Gradients on Continuous Control Tasks with Decoupled Prioritized Experience Replay](https://arxiv.org/abs/2512.05320)
*Mehmet Efe Lorasdagi,Dogan Can Cicek,Furkan Burak Mutlu,Suleyman Serdar Kozat*

Main category: cs.LG

TL;DR: Decoupling transition batches for Actor and Critic in DDPG algorithms via DPER improves performance in continuous control tasks.


<details>
  <summary>Details</summary>
Motivation: Uniform transition usage in Actor-Critic architectures may not be optimal due to differing learning objectives and update dynamics.

Method: Introduces Decoupled Prioritized Experience Replay (DPER), which allows independent sampling for Actor and Critic, integrated with Twin Delayed DDPG.

Result: DPER outperforms vanilla and prioritized experience replay in MuJoCo tasks from OpenAI Gym.

Conclusion: Decoupling experience replay enhances training dynamics and policy quality, offering a generalizable improvement for actor-critic off-policy algorithms.
...
### [149] [Robustness Test for AI Forecasting of Hurricane Florence Using FourCastNetv2 and Random Perturbations of the Initial Condition](https://arxiv.org/abs/2512.05323)
*Adam Lizerbram,Shane Stevenson,Iman Khadir,Matthew Tu,Samuel S. P. Shen*

Main category: cs.LG

TL;DR: The paper tests the robustness of NVIDIA's FourCastNetv2 (FCNv2) AI weather forecasting model by injecting noise into initial conditions and using random inputs. Results show it preserves hurricane features under moderate noise but underestimates intensity.


<details>
  <summary>Details</summary>
Motivation: Assessing the reliability of AI weather forecasting models, especially for extreme events like hurricanes, by testing sensitivity to input noise and uncertainties.

Method: Two experiments: (1) injecting Gaussian noise into Hurricane Florence's initial conditions and (2) using fully random initial conditions to observe model response.

Result: FCNv2 preserves hurricane features under low-moderate noise but degrades in positional accuracy under high noise. It underestimates storm intensity and stabilizes outputs with random inputs.

Conclusion: FCNv2 shows robustness to noise but underestimates storm intensity. The method is portable for testing other AI weather models.
...
### [150] [Non-Convex Federated Optimization under Cost-Aware Client Selection](https://arxiv.org/abs/2512.05327)
*Xiaowen Jiang,Anton Rodomanov,Sebastian U. Stich*

Main category: cs.LG

TL;DR: The paper introduces a new model for federated optimization that quantifies communication and computation costs for various client-selection strategies. It proposes an algorithm with improved complexities and introduces the Recursive-Gradient technique to enhance gradient estimators like SAGA.


<details>
  <summary>Details</summary>
Motivation: Existing federated optimization metrics fail to account for the varying communication costs of different client-selection strategies, creating a need for a model that explicitly associates costs with these strategies.

Method: A new federated optimization model is introduced, along with an algorithm based on the inexact composite gradient method. The algorithm uses a gradient estimator (SAGA) and introduces the Recursive-Gradient technique to improve it (RG-SAGA).

Result: The proposed algorithm achieves the best-known communication and local complexities for non-convex optimization. The RG-SAGA estimator offers improved error bounds.

Conclusion: The paper provides a framework to compare federated optimization methods more accurately by considering communication and computation costs, and introduces a superior algorithm with enhanced gradient estimation.
...
### [151] [PathFinder: MCTS and LLM Feedback-based Path Selection for Multi-Hop Question Answering](https://arxiv.org/abs/2512.05336)
*Durga Prasad Maram,Kalpa Gunaratna,Vijay Srinivasan,Haris Jeelani,Srinivas Chappidi*

Main category: cs.LG

TL;DR: PATHFINDER improves multi-hop QA performance by using Monte Carlo Tree Search, filtering poor training traces, and reformulating queries.


<details>
  <summary>Details</summary>
Motivation: Existing training-based approaches for multi-hop QA suffer from LLM hallucinations and incorrect reasoning, limiting performance.

Method: PATHFINDER employs Monte Carlo Tree Search for path traces, filters erroneous traces using sub-answer recall and LLM verification, and reformulates sub-queries for failed retrievals.

Result: The approach enhances multi-hop QA performance on public benchmark datasets.

Conclusion: PATHFINDER effectively addresses training limitations in multi-hop QA, improving accuracy and reliability.
...
### [152] [Interaction Tensor Shap](https://arxiv.org/abs/2512.05338)
*Hiroki Hasegawa,Yukihiko Okada*

Main category: cs.LG

TL;DR: IT SHAP introduces a tensor network contraction method to efficiently compute high-order Shapley interactions, reducing exponential complexity to polynomial time under Tensor Train structure.


<details>
  <summary>Details</summary>
Motivation: Understanding feature interactions in high-dimensional machine learning models is challenging due to computational inefficiencies in existing Shapley value methods.

Method: IT SHAP reformulates the Shapley Taylor Interaction Index as tensor contractions (Value Tensor and Weight Tensor) using Tensor Train structure.

Result: IT SHAP achieves polynomial time complexity under Tensor Train structure, significantly reducing computation from exponential to tractable levels.

Conclusion: IT SHAP provides a scalable, axiomatic solution for analyzing feature interactions in high-dimensional models, advancing explainable AI.
...
### [153] [Taxonomy-Adaptive Moderation Model with Robust Guardrails for Large Language Models](https://arxiv.org/abs/2512.05339)
*Mahesh Kumar Nandwana,Youngwan Lim,Joseph Liu,Alex Yang,Varun Notibala,Nishchaie Khanna*

Main category: cs.LG

TL;DR: Roblox Guard 1.0 is an instruction fine-tuned LLM designed to improve safety moderation in LLM systems by leveraging input-output pipelines and synthetic datasets. It uses Llama-3.1-8B-Instruct and performs well on unseen safety benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the limitations of post-training alignment in LLMs, which can still produce unsafe outputs, necessitating robust safeguards for both inputs and outputs.

Method: The method involves instruction fine-tuning the Llama-3.1-8B-Instruct model using synthetic and open-source safety datasets, enhanced with chain-of-thought rationales and input inversion to improve contextual understanding.

Result: Roblox Guard 1.0 demonstrates strong performance on out-of-domain safety benchmarks and generalizes well across unseen safety taxonomies.

Conclusion: The paper concludes that Roblox Guard 1.0 is effective as a moderation framework, supported by the release of RobloxGuard-Eval, a new benchmark for evaluating LLM safety.
...
### [154] [When Forgetting Builds Reliability: LLM Unlearning for Reliable Hardware Code Generation](https://arxiv.org/abs/2512.05341)
*Yiwen Liang,Qiufeng Li,Shikai Wang,Weidong Cao*

Main category: cs.LG

TL;DR: The paper proposes a novel unlearning framework for LLM-based hardware code generation to mitigate risks like IP memorization and unsafe coding, ensuring reliability without degrading performance.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) accelerate hardware design but face reliability issues like IP memorization and unsafe coding patterns, necessitating a solution to mitigate these risks.

Method: The framework combines syntax-preserving unlearning and floor-aware selective loss to remove problematic knowledge efficiently while maintaining code generation quality.

Result: Experiments show the framework supports larger forget sets (up to 3x) with minimal training (often one epoch), preserving RTL code correctness and integrity.

Conclusion: The work advances reliable LLM-assisted hardware design by effectively addressing critical reliability challenges.
...
### [155] [Text Rationalization for Robust Causal Effect Estimation](https://arxiv.org/abs/2512.05373)
*Lijinghua Zhang,Hengrui Cai*

Main category: cs.LG

TL;DR: CATR improves causal effect estimation by selecting relevant text tokens to address positivity violations and stabilize estimators.


<details>
  <summary>Details</summary>
Motivation: High-dimensional text data complicates causal inference by violating the positivity assumption and introducing spurious features.

Method: CATR selects a sparse subset of tokens using a residual-independence diagnostic to preserve confounding information.

Result: Experiments show CATR provides more accurate, stable, and interpretable causal effect estimates than baselines.

Conclusion: CATR effectively addresses challenges in text-based causal inference by rationalizing token selection.
...
### [156] [China Regional 3km Downscaling Based on Residual Corrective Diffusion Model](https://arxiv.org/abs/2512.05377)
*Honglu Sun,Hao Jing,Zhixiang Dai,Sa Xiao,Wei Xue,Jian Sun,Qifeng Lu*

Main category: cs.LG

TL;DR: The paper addresses the challenge of high-resolution weather forecasting by using a diffusion-based statistical downscaling method called CorrDiff, outperforming traditional regional models.


<details>
  <summary>Details</summary>
Motivation: Efficiently producing high-resolution weather forecasts is crucial, and statistical downscaling using deep learning offers a promising solution.

Method: The study expands CorrDiff's application to a much larger region and additional atmospheric variables, incorporating global residual connections for better accuracy. It downscales 25km forecasts from CMA-GFS and SFF models to 3km resolutions.

Result: The downscaled forecasts outperform CMA-MESO's direct forecasts in MAE and generate more realistic radar reflectivity predictions compared to deterministic models.

Conclusion: CorrDiff proves effective for large-scale, high-resolution weather forecasting, offering improved accuracy and realistic detail generation.
...
### [157] [Generalization Beyond Benchmarks: Evaluating Learnable Protein-Ligand Scoring Functions on Unseen Targets](https://arxiv.org/abs/2512.05386)
*Jakub Kopko,David Graber,Saltuk Mustafa Eyrilmez,Stanislav Mazurenko,David Bednar,Jiri Sedlar,Josef Sivic*

Main category: cs.LG

TL;DR: The paper evaluates the generalization of protein-ligand scoring functions to novel targets, highlighting limitations in standard benchmarks and exploring pretraining and simple methods to improve performance.


<details>
  <summary>Details</summary>
Motivation: Ensuring the reliability of machine learning-based scoring functions for novel protein targets is crucial, as current benchmarks fail to reflect real-world generalization challenges.

Method: The study assesses generalization on dataset splits simulating novel targets, investigates self-supervised pretraining, and tests simple methods using limited test-target data.

Result: Standard benchmarks inadequately reflect generalization challenges, but pretraining shows promise, and simple methods can enhance scoring function performance.

Conclusion: More rigorous evaluation protocols are needed, and practical strategies for improving generalization to novel targets are proposed.
...
### [158] [Smart Timing for Mining: A Deep Learning Framework for Bitcoin Hardware ROI Prediction](https://arxiv.org/abs/2512.05402)
*Sithumi Wickramasinghe,Bikramjit Das,Dorien Herremans*

Main category: cs.LG

TL;DR: MineROI-Net, a Transformer-based model, predicts profitable Bitcoin mining hardware purchases with high accuracy, reducing financial risk.


<details>
  <summary>Details</summary>
Motivation: Bitcoin mining lacks guidance on optimal hardware acquisition timing due to market volatility and rapid tech obsolescence.

Method: Formulated as a time series classification task, MineROI-Net uses multi-scale temporal patterns to predict ROI categories (profitable, marginal, unprofitable).

Result: Achieves 83.7% accuracy and 83.1% macro F1-score, with high precision in detecting profitable (98.5%) and unprofitable (93.6%) periods.

Conclusion: MineROI-Net provides a practical, data-driven tool for timing hardware purchases, reducing financial risk in mining operations.
...
### [159] [RevoNAD: Reflective Evolutionary Exploration for Neural Architecture Design](https://arxiv.org/abs/2512.05403)
*Gyusam Chang,Jeongyoon Yoon,Shin han yi,JaeHyeok Lee,Sujin Jang,Sangpil Kim*

Main category: cs.LG

TL;DR: RevoNAD introduces a reflective evolutionary orchestrator to improve LLM-driven neural architecture design by addressing token-level limitations and feedback alignment, achieving state-of-the-art results across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Current LLM-driven neural architecture design faces challenges like discrete, non-differentiable token-level loops and feedback misalignment, leading to redundant or infeasible designs.

Method: RevoNAD employs Multi-round Multi-expert Consensus, Adaptive Reflective Exploration, and Pareto-guided Evolutionary Selection to align feedback and optimize accuracy, efficiency, latency, confidence, and diversity.

Result: RevoNAD achieves state-of-the-art performance on CIFAR10, CIFAR100, ImageNet16-120, COCO-5K, and Cityscape datasets.

Conclusion: RevoNAD effectively addresses LLM-driven design challenges, producing reliable and deployable neural architectures.
...
### [160] [Sepsis Prediction Using Graph Convolutional Networks over Patient-Feature-Value Triplets](https://arxiv.org/abs/2512.05416)
*Bozhi Dan,Di Wu,Ji Xu,Xiang Liu,Yiziting Zhu,Xin Shu,Yujie Li,Bin Yi*

Main category: cs.LG

TL;DR: Triplet-GCN, a graph-based model, improves sepsis detection by processing EHR data as patient-feature-value triplets, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Sepsis detection is challenging due to complex EHR data; Triplet-GCN aims to enhance early warning by leveraging structured graph representations.

Method: Uses Triplet-GCN to model EHR data as bipartite graphs (patient--feature--value), preprocessing data types uniquely, and trains with GCN and MLP.

Result: Outperforms tabular methods (KNN, SVM, etc.) in discrimination and balanced error metrics in a multi-center Chinese cohort.

Conclusion: Graph-based EHR encoding improves sepsis risk stratification, offering a deployable, end-to-end solution.
...
### [161] [TS-HINT: Enhancing Semiconductor Time Series Regression Using Attention Hints From Large Language Model Reasoning](https://arxiv.org/abs/2512.05419)
*Jonathan Adam Rico,Nagarajan Raghavan,Senthilnath Jayavelu*

Main category: cs.LG

TL;DR: TS-Hint is a Time Series Foundation Model framework with chain-of-thought reasoning that improves MRR estimation in semiconductor CMP by leveraging temporal dynamics and attention hints.


<details>
  <summary>Details</summary>
Motivation: Existing methods lose temporal dynamics in MRR estimation and require large datasets, limiting their practicality.

Method: TS-Hint integrates chain-of-thought reasoning and attention hints using attention mechanism and saliency data during training.

Result: The model effectively learns from limited data via few-shot learning and directly from multivariate time series features.

Conclusion: TS-Hint outperforms static feature-based methods by capturing temporal dynamics and reducing data dependency.
...
TL;DR: The paper introduces IdealTSF, a framework leveraging negative samples to improve time series forecasting by pretraining on negative samples, transforming data into positive samples, and optimizing with adversarial disturbances.


<details>
  <summary>Details</summary>
Motivation: Deep learning struggles with missing values and anomalies in sequential data; previous work focused on positive samples, but negative samples offer untapped potential for forecasting.

Method: The IdealTSF framework involves three steps: pretraining on negative samples, transforming data into ideal positive samples during training, and applying adversarial disturbances for optimization.

Result: Experiments show negative samples enhance the basic attention architecture's performance, making IdealTSF effective for noisy or low-quality data.

Conclusion: IdealTSF is highly suitable for forecasting tasks with non-ideal data, demonstrating the value of leveraging negative samples.
...
TL;DR: Ensembles achieve high accuracy with small generalization gaps by reducing variance, but their benefits vary with dataset characteristics like linearity, noise, and imbalance.


<details>
  <summary>Details</summary>
Motivation: To understand how ensembles balance accuracy and overfitting across different tabular classification tasks.

Method: Comparison of linear models, a single decision tree, and nine ensemble methods using repeated stratified cross-validation and statistical testing on four datasets.

Result: Ensembles improve accuracy by 5-7 points on nonlinear data with gaps under 3%, but require regularization on noisy/imbalanced data. Linear datasets see little benefit.

Conclusion: Ensembles effectively control variance and generalize well, especially on nonlinear datasets, but their utility depends on dataset complexity metrics.
...
TL;DR: Comparison of Geometric Quantum Machine Learning models for LiH (linear) and NH3 (trigonal pyramidal) molecules reveals permutational symmetric embedding as the most generalizable model.


<details>
  <summary>Details</summary>
Motivation: To evaluate how molecular geometry impacts the performance and generalizability of Quantum Machine Learning models.

Method: Comparison of models with varying symmetry equivariance (none, rotational, permutational, graph embedded permutational) using accuracy and generalizability metrics.

Result: Graph embedding enhances trainability; permutational symmetric embedding shows the highest generalizability.

Conclusion: Molecular geometry dictates model choice; permutational symmetric embedding is optimal for geometric learning.
...
### [165] [Entropy Ratio Clipping as a Soft Global Constraint for Stable Reinforcement Learning](https://arxiv.org/abs/2512.05591)
*Zhenpeng Su,Leiyu Pan,Minxuan Lv,Tiehua Mei,Zijia Lin,Yuntao Li,Wenping Hu,Ruiming Tang,Kun Gai,Guorui Zhou*

Main category: cs.LG

TL;DR: The paper proposes Entropy Ratio Clipping (ERC) to address distribution shift issues in reinforcement learning for large language models, improving stability and performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods like PPO-Clip overlook global distributional shifts in actions, causing training instability. ERC aims to quantify and mitigate these shifts.

Method: ERC uses the entropy ratio between current and previous policies as a metric, applying bidirectional clipping constraints to stabilize updates.

Result: Integrating ERC into DAPO and GPPO algorithms consistently improves performance across benchmarks.

Conclusion: ERC effectively addresses global distribution shifts, enhancing training stability and model performance.
...
### [166] [Turbulence Regression](https://arxiv.org/abs/2512.05483)
*Yingang Fan,Binjie Ding,Baiyi Chen*

Main category: cs.LG

TL;DR: The paper introduces a NeuTucker decomposition model to predict low-altitude turbulence using discretized wind field data, outperforming traditional regression models.


<details>
  <summary>Details</summary>
Motivation: Traditional methods struggle with accurate turbulence prediction using sparse wind profile radar data, prompting the need for an advanced model.

Method: A NeuTucker decomposition model is developed, discretizing continuous data and using a Tucker neural network to capture wind field interactions.

Result: The discretized NeuTucF model shows superior performance in estimating missing data compared to common regression models.

Conclusion: The proposed NeuTucker model effectively addresses turbulence prediction challenges by leveraging discretized data and neural networks.
...
### [167] [Credal and Interval Deep Evidential Classifications](https://arxiv.org/abs/2512.05526)
*Michele Caprio,Shireen K. Manchingal,Fabio Cuzzolin*

Main category: cs.LG

TL;DR: The paper introduces CDEC and IDEC, novel methods for Uncertainty Quantification (UQ) in AI classification, addressing epistemic and aleatoric uncertainties with robust probabilistic guarantees and competitive performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to tackle the challenge of UQ in AI, which is critical for decision-making, risk assessment, and model reliability, by providing methods that avoid overfitting and systematically assess uncertainties.

Method: CDEC and IDEC use credal sets and intervals of evidential predictive distributions, respectively, trained with standard backpropagation and a loss function based on evidence theory.

Result: Experiments on MNIST, CIFAR-10, CIFAR-100, and OoD shifts show competitive accuracy, state-of-the-art OoD detection, and well-calibrated prediction regions that expand under distribution shift. CDEC also achieves stable uncertainty estimates with small ensembles.

Conclusion: CDEC and IDEC effectively address UQ in classification, outperform previous methods, and provide reliable uncertainty assessments, making them valuable for AI applications requiring robust decision-making.
...
### [168] [IDK-S: Incremental Distributional Kernel for Streaming Anomaly Detection](https://arxiv.org/abs/2512.05531)
*Yang Xu,Yixiao Ma,Kaifeng Zhang,Zuliang Yang,Kai Ming Ting*

Main category: cs.LG

TL;DR: $Є$Ғ$-Є$ is a novel incremental kernel method for streaming anomaly detection, offering high accuracy and efficiency by leveraging a dynamic kernel mean embedding and lightweight updates.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in anomaly detection on data streams, including maintaining accuracy with evolving distributions and ensuring real-time efficiency.

Method: It introduces $Є$Ғ$-Є$, an incremental distributional kernel method, which uses a dynamic kernel mean embedding framework and a lightweight update mechanism to reduce computational overhead.

Result: Experiments on 13 benchmarks show $Є$Ғ$-Є$ outperforms state-of-the-art methods in accuracy and speed, often by an order of magnitude.

Conclusion: $Є$Ғ$-Є$ provides a robust and efficient solution for streaming anomaly detection, balancing accuracy and computational efficiency.
...
### [169] [On the Theoretical Foundation of Sparse Dictionary Learning in Mechanistic Interpretability](https://arxiv.org/abs/2512.05534)
*Yiming Tang,Harshvardhan Saini,Yizhen Liao,Dianbo Liu*

Main category: cs.LG

TL;DR: The paper develops a unified theoretical framework for sparse dictionary learning (SDL) methods, explaining empirical phenomena and validating results.


<details>
  <summary>Details</summary>
Motivation: Understanding AI model representations is crucial for scientific progress and trustworthy deployment, but existing SDL methods lack theoretical grounding.

Method: A unified theoretical framework for SDL is developed, analyzed, and validated through controlled experiments.

Result: The framework explains empirical phenomena like feature absorption and dead neurons, and validates theoretical findings.

Conclusion: This work advances the theoretical understanding of SDL, bridging the gap between empirical success and formal analysis.
...
### [170] [SCoNE: Spherical Consistent Neighborhoods Ensemble for Effective and Efficient Multi-View Anomaly Detection](https://arxiv.org/abs/2512.05540)
*Yang Xu,Hang Zhang,Yixiao Ma,Ye Zhu,Kai Ming Ting*

Main category: cs.LG

TL;DR: SCoNE improves multi-view anomaly detection by directly representing consistent neighborhoods with data-dependent properties, reducing computational cost to O(N).


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-view anomaly detection struggle with capturing consistent neighbors accurately and efficiently, especially in varied density regions, leading to high computational costs and inferior accuracy.

Method: SCoNE introduces spherical consistent neighborhoods ensemble, representing neighborhoods directly without intermediate representations and using data-dependent properties to adjust neighborhood sizes dynamically.

Result: SCoNE achieves better detection accuracy and significantly faster performance (O(N) complexity) compared to existing methods.

Conclusion: SCoNE provides a scalable and accurate solution for multi-view anomaly detection by addressing key limitations of current approaches.
...
### [171] [Improving Local Fidelity Through Sampling and Modeling Nonlinearity](https://arxiv.org/abs/2512.05556)
*Sanjeev Shrestha,Rahul Dubey,Hui Liu*

Main category: cs.LG

TL;DR: The paper addresses limitations of LIME in explaining black-box models by proposing a new method using MARS for non-linear local boundaries and N-ball sampling for better fidelity, achieving superior results.


<details>
  <summary>Details</summary>
Motivation: Existing methods like LIME assume linear local boundaries, failing to capture non-linear relationships, leading to inaccurate explanations.

Method: The proposed method uses MARS to model non-linear local boundaries and N-ball sampling for better distribution sampling, enhancing explanation fidelity.

Result: Experiments on UCI datasets show a 37% reduction in RMSE, proving the method's superiority in faithfulness over baselines.

Conclusion: The new method significantly improves local fidelity and faithfulness in explaining black-box models compared to traditional techniques like LIME.
...
### [172] [Wasserstein distance based semi-supervised manifold learning and application to GNSS multi-path detection](https://arxiv.org/abs/2512.05567)
*Antoine Blais,Nicolas Couëllan*

Main category: cs.LG

TL;DR: The paper proposes a semi-supervised learning method using optimal transport and Wasserstein distance for image classification, tested on GNSS multi-path interference detection with improved accuracy over supervised methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning from scarce labeled image data, leveraging semi-supervised learning with optimal transport theory.

Method: Uses deep convolutional networks with Wasserstein distance for similarity metric in label propagation, applied to GNSS multi-path interference detection.

Result: For specific hyperparameters, the method significantly improves classification accuracy compared to fully supervised training.

Conclusion: The optimal transport-based semi-supervised approach is effective for learning with limited labeled data, particularly in GNSS applications.
...
### [173] [Hyperparameter Transfer Enables Consistent Gains of Matrix-Preconditioned Optimizers Across Scales](https://arxiv.org/abs/2512.05620)
*Shikai Qiu,Zixi Chen,Hoang Phan,Qi Lei,Andrew Gordon Wilson*

Main category: cs.LG

TL;DR: The paper explores scaling preconditioned deep learning optimizers like Shampoo and Muon using hyperparameter transfer, finding learning rate and weight decay scaling rules improve performance over AdamW, especially for larger models.


<details>
  <summary>Details</summary>
Motivation: To understand and improve the scalability of preconditioned optimizers like Shampoo and Muon, addressing inconsistencies in prior results and ensuring reliable comparisons at scale.

Method: Investigates hyperparameter transfer rules (e.g., learning rate and weight decay scaling) across optimizers, incorporating techniques like blocking and spectral normalization, tested on Llama-architecture models (190M to 1.4B parameters).

Result: Muon and Shampoo achieved 1.4× and 1.3× speedups over AdamW with correct scaling, but performance degraded without proper rules. Compute-optimal weight decay scaled as 1/width.

Conclusion: Hyperparameter transfer studies are crucial for reliable optimizer comparisons at scale, with proper scaling rules mitigating performance drift and enabling consistent speedups.
...
### [174] [Bounded Graph Clustering with Graph Neural Networks](https://arxiv.org/abs/2512.05623)
*Kibidi Neocosmos,Diego Baptista,Nicole Ludwig*

Main category: cs.LG

TL;DR: Introducing a flexible framework for GNNs to control the number of detected communities, allowing users to specify a range or exact number of clusters.


<details>
  <summary>Details</summary>
Motivation: Existing GNN-based methods often fail to return the exact number of clusters even when specified, requiring a solution to enforce user-defined bounds during training.

Method: Proposes a framework where users can specify a plausible range for the number of clusters or an exact number, ensuring these constraints are respected during training.

Result: The framework reliably returns the user-specified number of clusters or adheres to the provided range, addressing the limitation of standard GNN methods.

Conclusion: The proposed method offers a flexible and principled way to control community detection in GNNs, improving reliability and user control.
...
### [175] [Modular Jets for Supervised Pipelines: Diagnosing Mirage vs Identifiability](https://arxiv.org/abs/2512.05638)
*Suman Sanyal*

Main category: cs.LG

TL;DR: The paper introduces Modular Jets to assess unique modular decompositions in models, contrasts mirage and identifiable regimes, and provides theoretical and empirical validation.


<details>
  <summary>Details</summary>
Motivation: To address whether a model's internal decomposition is uniquely determined by data and evaluation design, beyond predictive risk.

Method: Introduces Modular Jets for regression/classification pipelines, estimates empirical jets for module reactions to perturbations, and proposes mirage diagnostics.

Result: Proves jet-identifiability theorem for two-module pipelines; demonstrates mirage decompositions exist under risk-only evaluation.

Conclusion: Modular Jets offer deeper insights into model decompositions, revealing regimes where decompositions are uniquely identifiable or indistinguishable.
...
TL;DR: The paper introduces Selective GradienT Masking (SGTM), an improved variant of Gradient Routing, to mitigate risks of mislabeled harmful content in large language models. It focuses on robustness to label noise and outperforms prior methods in retaining/forgetting knowledge.


<details>
  <summary>Details</summary>
Motivation: Addressing dual-use risks in large language models, especially when harmful content is mislabeled during pretraining, to improve safety mitigations.

Method: SGTM zero-masks selected gradients to localize and remove target knowledge, ensuring only dedicated parameters are updated. It was tested on bilingual language removal and biology knowledge removal tasks.

Result: SGTM outperforms data filtering and prior Gradient Routing in retain/forget trade-offs under label noise and shows robustness to adversarial fine-tuning.

Conclusion: SGTM is a promising pretraining-time safety measure, especially in settings with unavoidable label noise, complementing existing mitigations.
...
TL;DR: The paper explores AI-assisted end-user coding as a potential replacement or complement to low-code/no-code platforms, demonstrating its feasibility through a case study where non-programmers successfully built web apps using AI assistants.


<details>
  <summary>Details</summary>
Motivation: To determine if AI-assisted end-user coding can offer greater flexibility, speed, and reusability compared to traditional low-code/no-code platforms.

Method: A case study where non-programmers developed a basic web app using AI assistants, followed by analysis of outcomes and participant feedback.

Result: Most participants completed the task successfully and supported AI-assisted coding as a viable end-user development approach.

Conclusion: AI-assisted end-user coding shows promise as a feasible alternative or complement to low-code/no-code platforms, with implications for practice, research, and education.
...
TL;DR: The paper proposes a POMDP-based method for optimal beam selection in 5G/6G networks, outperforming supervised learning approaches by handling dynamic environments and unforeseen trajectories.


<details>
  <summary>Details</summary>
Motivation: Current analog beamforming uses codebooks with pre-configured beams, making optimal selection challenging due to large codebooks and environmental effects like reflections or blockages. Supervised learning approaches lack adaptability to new scenarios.

Method: Formulates beam selection as a POMDP, modeling the environment as the codebook. Selects candidate beams based on belief states of the unobservable optimal beam and previously probed beams, treating it as an online search problem.

Result: Outperforms previous work significantly, handling dynamic changes and unforeseen trajectories effectively.

Conclusion: The POMDP approach provides a robust solution for beam selection in dynamic environments, surpassing traditional supervised learning methods.
...
TL;DR: BERTO, a BERT-based framework, enhances traffic prediction and energy optimization in cellular networks using transformer architectures and natural language prompts.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between power savings and performance in cellular networks by leveraging transformer-based models.

Method: Uses BERT architecture with a Balancing Loss Function and prompt-based customization for adjusting prediction accuracy and energy optimization.

Result: Achieves a 4.13% reduction in MSE and balances power savings (1.4 kW range) and service quality (up to 9x variation) effectively.

Conclusion: BERTO is effective for intelligent RAN deployments, offering flexibility and improved performance through natural language inputs.
...
TL;DR: A framework for designing neural networks (ABM-NNs) that align with agent-based model principles, addressing limitations of standard neural differential equations by enforcing constraints like mass conservation and network locality.


<details>
  <summary>Details</summary>
Motivation: Standard neural differential equations often fail to model complex systems with specific constraints. The paper aims to integrate agent-based model principles into neural networks for better interpretability and structure-preserving dynamics.

Method: Introduces ABM-NNs using restricted graph neural networks and hierarchical decomposition to enforce constraints and learn interpretable dynamics.

Result: Validated across three case studies: generalized Lotka-Volterra system, graph-based SIR contagion model, and macroeconomic model. ABM-NNs outperform baselines in forecasting, noise robustness, and gradient-based counterfactual analysis.

Conclusion: ABM-NNs effectively bridge neural networks and agent-based models, offering interpretable and robust solutions for complex systems with constraints.
...
TL;DR: The paper introduces a theoretical framework analyzing how gating mechanisms influence the learnability window of recurrent neural networks, focusing on effective learning rates rather than Jacobian stability. It links gate-induced structure, gradient noise, and sample complexity, providing scaling laws for learnability.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the fundamental factors determining the temporal horizon over which recurrent neural networks can learn dependencies, moving beyond traditional stability analyses to emphasize effective learning rates.

Method: The authors develop a theoretical framework analyzing first-order expansions of gate-induced Jacobian products, deriving effective learning rates and studying their role in gradient transport under heavy-tailed noise.

Result: The framework shows that broader or more heterogeneous gate spectra enhance learnability windows, while heavier-tailed noise reduces them. It provides explicit formulas for minimal sample sizes and learnability horizons.

Conclusion: Effective learning rates are identified as key determinants of learnability in gated recurrent networks, bridging gate-induced structure, noise, and statistical complexity.
...
TL;DR: SAEs help interpret protein language models like p-IgGen. TopK SAEs reveal meaningful features but lack causal control, while Ordered SAEs enable better steering but are less interpretable.


<details>
  <summary>Details</summary>
Motivation: To understand and steer latent features in autoregressive antibody language models (p-IgGen) using SAEs.

Method: Used TopK and Ordered SAEs to analyze and control generation in p-IgGen.

Result: TopK SAEs identify biologically meaningful features but don't guarantee causal control. Ordered SAEs enable reliable steering but are less interpretable.

Conclusion: TopK SAEs are good for feature-concept mapping, while Ordered SAEs are better for precise generative steering.
...
TL;DR: The paper introduces a unified theoretical framework for dataset distillation (DD), explaining common principles and providing performance guarantees. It addresses the lack of theoretical foundations and clarifies how distilled data retains effectiveness under varying training configurations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limited theoretical foundations of dataset distillation, which currently relies on heterogeneous surrogate objectives and lacks general guarantees. The paper aims to clarify common principles and provide conditions under which distilled data remains effective despite changes in training configurations.

Method: The proposed method is a unified theoretical framework called configuration--dynamics--error analysis. This reformulates major DD approaches under a common generalization-error perspective and derives two key results: a scaling law and a coverage law. The framework also reveals that various matching methods are interchangeable surrogates.

Result: The results include: (i) a scaling law providing a single-configuration upper bound on error reduction with increased distilled sample size, and (ii) a coverage law showing linear scaling of required distilled sample size with configuration diversity. Experiments confirm these laws, validating the framework.

Conclusion: The paper concludes that the unified framework advances the theoretical foundation of DD, explaining why existing methods work and enabling theory-driven design of compact, configuration-robust distilled datasets.
...
TL;DR: The paper provides detailed mathematical and algorithmic insights into an approximation algorithm for HV improvement calculations in BO, addressing the computational bottleneck caused by exact calculations.


<details>
  <summary>Details</summary>
Motivation: The computational expense of HV improvement calculations in BO is a significant challenge, worsened by the super-polynomial memory complexity of existing methods. This paper aims to fill the gap in the literature by rigorously describing an approximation algorithm.

Method: The paper focuses on a previously proposed approximation algorithm by Couckuyt et al. (2012), offering comprehensive mathematical and algorithmic details to make the method more accessible and practical.

Result: The paper successfully bridges the gap in the literature by providing a rigorous description of the approximation algorithm, enhancing its usability for HV-based BO.

Conclusion: By detailing the approximation algorithm, the paper addresses the computational bottleneck in HV improvement calculations, making multi-objective BO more efficient and practical.
...
TL;DR: NEAT introduces an order-agnostic autoregressive model for 3D molecular structure generation, achieving state-of-the-art performance with high efficiency and permutation invariance.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models for molecular generation traditionally assume token order, which doesn't align with the permutation-invariant nature of molecular graphs. NEAT addresses this mismatch.

Method: NEAT uses a Neighborhood-guided, Efficient, Autoregressive, Set Transformer to treat molecular graphs as sets and learn order-agnostic token distributions with an autoregressive flow model.

Result: NEAT achieves state-of-the-art performance in 3D molecular generation with high computational efficiency and maintains atom-level permutation invariance.

Conclusion: NEAT establishes a practical foundation for scalable molecular design by eliminating the need for predefined token orders.
...
TL;DR: A post-training method transforms transformer attention into a sparse structure by using flexible sparsity regularisation, maintaining performance while simplifying connectivity patterns.


<details>
  <summary>Details</summary>
Motivation: The study aims to make transformer attention sparse without losing performance, leveraging sparsity as a structural prior for better organization and interpretability.

Method: The approach applies flexible sparsity regularisation under a constrained-loss objective to transformer models with up to 1B parameters.

Result: The method retains pretraining loss while reducing attention connectivity to ≈0.3% of edges. It also simplifies global circuits, involving fewer components and edges.

Conclusion: Transformers' attention can be significantly sparsified, indicating computation redundancy. Sparsity may guide the development of structured and interpretable models.
...
TL;DR: SNNs with Bayesian Optimization outperform traditional models in high-frequency trading spike forecasting, achieving higher returns.


<details>
  <summary>Details</summary>
Motivation: Traditional financial models struggle with high-frequency trading's fine temporal structure, while SNNs are biologically suited for discrete event processing.

Method: SNNs were applied to price-spike forecasting using hyperparameter tuning with Bayesian Optimization, evaluated via simulated trading.

Result: Optimized SNNs, especially with Penalized Spike Accuracy, achieved higher returns (76.8%) compared to supervised models (42.54%).

Conclusion: SNNs, when properly tuned, show promise for effective price spike forecasting in high-frequency trading.
...
TL;DR: A machine learning approach is used to predict vapor pressure for discovering new liquid lubricants suitable for space applications, leveraging data from simulations and experiments.


<details>
  <summary>Details</summary>
Motivation: Existing liquid lubricants for space MMAs have limitations due to insufficiently low vapor pressures, restricting design options.

Method: Data-driven ML models trained on molecular dynamics simulations and experimental data, focusing on interpretability.

Result: ML models identify relationships between chemical structure and vapor pressure, proposing promising candidate molecules.

Conclusion: The approach enables discovery of new liquid lubricants optimized for space MMAs.
...
TL;DR: A novel method for selecting the best pre-trained model checkpoint using Neural Coherence, which works efficiently with minimal unlabeled data from the target task, outperforming traditional baselines.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of model selection when target task data is scarce, unlabeled, and out-of-distribution, where conventional validation methods fail.

Method: Introduces Neural Coherence, analyzing activation statistics across source and target domains to enable data-efficient model selection.

Result: Demonstrates improved generalization on diverse target domains (Food-101, PlantNet-300K, iNaturalist) and meta-learning settings.

Conclusion: Neural Coherence is a versatile and effective principle for model selection and training data selection, even with limited target data.
...
TL;DR: DAE-HardNet is a physics-constrained neural network that enforces strict differential-algebraic constraints, outperforming traditional PINNs and MLPs in physics loss reduction while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional PINNs struggle with strict satisfaction of differential-algebraic equations (DAEs) due to their black-box nature. DAE-HardNet aims to address this by embedding domain knowledge and first-principles directly.

Method: DAE-HardNet simultaneously learns functions and their derivatives, enforcing constraints via a differentiable projection layer. It is applied to systems like Lotka-Volterra and heat conduction.

Result: Achieves significant reduction in physics loss compared to MLPs and PINNs, maintains prediction accuracy, and learns derivatives effectively.

Conclusion: DAE-HardNet offers improved constrained learning and faster inference potential, with promising applications in parameter estimation and DAE-governed systems.
...
### [191] [NeuroMemFPP: A recurrent neural approach for memory-aware parameter estimation in fractional Poisson process](https://arxiv.org/abs/2512.05893)
*Neha Gupta,Aditya Maheshwari*

Main category: cs.LG

TL;DR: Proposes an RNN-based framework using LSTM to estimate fractional Poisson process parameters, showing 55.3% MSE reduction over traditional methods and effectiveness on real-world data.


<details>
  <summary>Details</summary>
Motivation: To accurately model event arrivals with memory and long-range dependence, which traditional methods struggle with.

Method: Uses an LSTM network to estimate parameters μ and β from inter-arrival time sequences, capturing temporal dependencies.

Result: 55.3% reduction in MSE compared to method of moments; successful application on synthetic and real-world datasets (emergency calls and stock trading data).

Conclusion: The LSTM-based approach is effective for estimating FPP parameters and modeling complex time dependencies in real-world data.
...
### [192] [LDLT $\mathcal{L}$-Lipschitz Network: Generalized Deep End-To-End Lipschitz Network Construction](https://arxiv.org/abs/2512.05915)
*Marius F. R. Juston,Ramavarapu S. Sreenivas,Dustin Nottage,Ahmet Soylemezoglu*

Main category: cs.LG

TL;DR: This paper introduces a method to design Lipschitz-constrained deep residual networks using an LMI framework, ensuring robustness and certifiability. The approach extends to nonlinear architectures and improves accuracy on UCI datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance adversarial robustness and certifiability of deep networks by controlling the Lipschitz constant, leveraging the success of ResNets in computer vision.

Method: The method reformulates ResNet as a cyclic tridiagonal LMI, derives constraints for Lipschitz continuity, and uses $LDL^\top$ decomposition for LMI feasibility. It also employs Cholesky decomposition for efficient parameterization.

Result: The results show a tight relaxation of SDP-based networks, maintaining expressiveness and achieving 3%-13% accuracy gains over SLL Layers on 121 UCI datasets.

Conclusion: The paper concludes with a provable methodology for Lipschitz-constrained network design, applicable to adversarial robustness, certified training, and control systems.
...
### [193] [KQ-SVD: Compressing the KV Cache with Provable Guarantees on Attention Fidelity](https://arxiv.org/abs/2512.05916)
*Damien Lesens,Beheshteh T. Rakhshan,Guillaume Rabusseau*

Main category: cs.LG

TL;DR: KQ-SVD is a new method optimizing the attention matrix compression in KV caches for LLMs, outperforming prior methods by preserving attention fidelity more effectively.


<details>
  <summary>Details</summary>
Motivation: KV caches in LLMs face memory bottlenecks with growing sequence lengths and batch sizes, necessitating better compression methods.

Method: KQ-SVD directly decomposes the attention matrix optimally via a closed-form solution, improving fidelity.

Result: Evaluations on LLaMA and Mistral show KQ-SVD consistently outperforms prior compression methods.

Conclusion: KQ-SVD offers a superior, efficient solution for compressing KV caches in LLMs by targeting the attention matrix directly.
...
### [194] [On the Bayes Inconsistency of Disagreement Discrepancy Surrogates](https://arxiv.org/abs/2512.05931)
*Neil G. Marchant,Andrew C. Cullen,Feng Liu,Sarah M. Erfani*

Main category: cs.LG

TL;DR: The paper addresses flaws in existing surrogate losses for disagreement discrepancy in deep neural networks, proposing a novel, provably consistent surrogate loss paired with cross-entropy for better performance under distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks struggle with distribution shifts in real-world deployments. Disagreement discrepancy is a promising measure but existing surrogate losses are not Bayes consistent, leading to unreliable optimization.

Method: The authors introduce theoretical bounds on surrogate loss optimality gaps and propose a novel disagreement loss combined with cross-entropy to ensure consistency.

Result: Empirical results show the proposed method outperforms existing approaches in estimating disagreement discrepancy, especially under adversarial conditions.

Conclusion: The new surrogate loss provides a more reliable solution for addressing distribution shifts through disagreement discrepancy, enhancing robustness and accuracy.
...
### [195] [Developing synthetic microdata through machine learning for firm-level business surveys](https://arxiv.org/abs/2512.05948)
...
TL;DR: Summary: Synthetic Public-Use Microdata Samples (PUMS) are created using machine learning to address re-identification risks in anonymized US Census data, with initial results showing promise for business surveys.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the increased risk of re-identifying anonymized survey data due to advancements in computing and Big Data, ensuring confidentiality while preserving data utility.

Method: A machine learning model is employed to construct synthetic PUMS for business surveys, tested with the 2007 Survey of Business Owners, and evaluated using quality metrics.

Result: Econometric replication confirms synthetic data closely mimics true data, validating its potential use cases for the Annual Business Survey (ABS).

Conclusion: Synthetic PUMS effectively balance confidentiality and data utility, with promising applications for business surveys like the ABS.
...
### [196] [Impugan: Learning Conditional Generative Models for Robust Data Imputation](https://arxiv.org/abs/2512.05950)
*Zalish Mahmud,Anantaa Kotal,Aritran Piplai*

Main category: cs.LG

TL;DR: Impugan, a cGAN-based model, addresses missing data in heterogeneous datasets by capturing nonlinear relationships, outperforming traditional imputation methods in accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-world data often has missing values due to sensor failures, inconsistencies, or varying data quality, making reliable models difficult to build. Traditional methods rely on strong assumptions that may not hold for complex data.

Method: Impugan uses a conditional Generative Adversarial Network (cGAN). The generator reconstructs missing values from observed features, while the discriminator ensures the imputed data is realistic. This adversarial process captures nonlinear and multimodal relationships.

Result: Impugan reduces Earth Mover's Distance by up to 82% and mutual-information deviation by 70% compared to baselines, demonstrating superior performance in imputing and integrating heterogeneous data.

Conclusion: Adversarially trained generative models like Impugan offer a scalable and effective solution for imputing and merging incomplete, heterogeneous datasets, outperforming conventional methods.
...
### [197] [MaxShapley: Towards Incentive-compatible Generative Search with Fair Context Attribution](https://arxiv.org/abs/2512.05958)
*Sara Patel,Mingxun Zhou,Giulia Fanti*

Main category: cs.LG

TL;DR: MaxShapley is an efficient algorithm for fair attribution in generative search pipelines, using decomposable max-sum utility to reduce computation costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To sustain generative search ecosystems by fairly attributing and compensating content providers based on their contributions to generated answers.

Method: Introduces MaxShapley, leveraging decomposable max-sum utility for linear computation cost compared to exponential Shapley value computation, evaluated on multi-hop QA datasets.

Result: Achieves comparable attribution quality to exact Shapley computation with significantly reduced resource consumption (e.g., 8x reduction).

Conclusion: MaxShapley offers a practical solution for fair attribution in generative search, balancing accuracy and efficiency.
...
### [198] [Whatever Remains Must Be True: Filtering Drives Reasoning in LLMs, Shaping Diversity](https://arxiv.org/abs/2512.05962)
*Germán Kruszewski,Pierre Erbacher,Jos Rozen,Marc Dymetman*

Main category: cs.LG

TL;DR: The paper addresses diversity loss in RL-trained LLMs by proposing a method using the α-divergence family to balance precision and diversity, achieving top performance on a theorem-proving benchmark.


<details>
  <summary>Details</summary>
Motivation: RL-trained LLMs lose diversity due to mode-seeking optimization. The authors aim to address this by explicitly targeting a balanced distribution of correct answers.

Method: They use the α-divergence family to approximate a target distribution, allowing control over the trade-off between precision and diversity.

Result: Their method outperforms prior approaches on a Lean theorem-proving benchmark, particularly in coverage.

Conclusion: Explicitly targeting diversity with α-divergence improves LLM performance, offering a better balance between precision and coverage.
