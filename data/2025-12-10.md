...
TL;DR: Detection of cyberbullying in GIFs using deep learning model VGG16 with 97% accuracy on a dataset collected from Twitter.


<details>
  <summary>Details</summary>
Motivation: Cyberbullying on social media is growing, especially via GIFs/stickers, with limited research in this area.

Method: Collected GIFs using Twitter hashtags and GIPHY API, applied VGG16 for detection.

Result: Achieved 97% accuracy in detecting cyberbullying.

Conclusion: Provides a valuable GIF dataset for further research in cyberbullying detection.
...
TL;DR: Using deep learning and 4-band satellite imagery, the paper demonstrates effective near real-time monitoring of fire damage in Sudan's conflict zones.


<details>
  <summary>Details</summary>
Motivation: The ongoing war in Sudan necessitates rapid conflict monitoring. Advances in deep learning and accessible satellite imagery enable timely analysis.

Method: A deep learning model is applied to 4-band satellite imagery from Planet Labs to monitor fire damage. Five case studies in Sudan validate the approach.

Result: The automated method outperforms a baseline in capturing active fires and charred areas. Using 8-band imagery or time series offers only slight improvements.

Conclusion: The study shows that 4-band imagery with deep learning is sufficient for accurate, timely conflict damage monitoring, with minimal benefit from more complex data.
...
TL;DR: LivingSwap is a video face-swapping model that uses keyframes and reference guidance for high fidelity and temporal consistency, achieving state-of-the-art results with reduced manual effort.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of achieving high fidelity and temporal consistency in video face swapping for film and entertainment production, leveraging recent advances in reference-guided image editing.

Method: The approach combines keyframe conditioning with video reference guidance to inject the target identity and perform temporal stitching, supported by a newly constructed dataset, Face2Face.

Result: The method achieves state-of-the-art results, seamlessly integrating the target identity with the source video's attributes while reducing manual effort.

Conclusion: LivingSwap demonstrates effective video face swapping with improved fidelity and temporal coherence, validated by extensive experiments and a new dataset.
...
TL;DR: The paper introduces a hierarchical framework for semantic segmentation in dental imaging, improving accuracy and anatomical coherence by embedding explicit anatomical hierarchies into the model.


<details>
  <summary>Details</summary>
Motivation: Accurate staging of dental diseases requires precise understanding of anatomical structures, which existing methods inadequately address by weakly encoding hierarchies through loss functions.

Method: The framework uses a recurrent, level-wise prediction scheme with restrictive output heads and top-down feature conditioning, incorporating Feature-wise Linear Modulation and probabilistic composition rules.

Result: Hierarchical variants improved IoU, Dice, and recall, especially for fine-grained anatomies, though with increased false positives. UNet and HRNet were tested on the TL-pano dataset.

Conclusion: Explicit hierarchical structuring enhances performance and clinical plausibility in dental imaging, particularly in low-data scenarios.
...
TL;DR: The paper introduces FRIEDA, a benchmark for evaluating cartographic reasoning in large vision language models (LVLMs), highlighting gaps in current models' abilities compared to humans.


<details>
  <summary>Details</summary>
Motivation: Cartographic reasoning is crucial for tasks like disaster response and urban planning but remains unevaluated. Current LVLM studies often treat maps as charts, missing layered symbology and spatial relations.

Method: FRIEDA sources real map images and targets three spatial relation categories: topological, metric, and directional. It tests multi-step inference and cross-map grounding.

Result: State-of-the-art LVLMs (Gemini-2.5-Pro, GPT-5-Think) achieve low accuracy (38.20%, 37.20%) compared to humans (84.87%).

Conclusion: FRIEDA reveals a gap in cartographic reasoning, serving as a benchmark to improve spatial intelligence in LVLMs.
...
TL;DR: The paper introduces SSplain, a method for generating pixel-wise explanations in medical image classification, preserving image structures like smoothness and sparsity.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing explainers in preserving input image structures (smoothness and sparsity) for medical diagnosis tasks like Retinopathy of Prematurity (ROP) classification.

Method: Proposes SSplain, an explainer method that formulates an optimization problem with combinatorial constraints solved using ADMM, ensuring smooth and sparse explanations.

Result: SSplain outperforms common explainers in accuracy and smoothness, identifies clinically relevant features, and generalizes well to other datasets.

Conclusion: SSplain enhances model interpretability and trust in medical diagnosis by generating realistic, structure-preserving explanations.
...
TL;DR: The paper introduces frequency-domain masking for universal deepfake detection, enhancing generalization across unseen generative models while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of detecting AI-generated images from diverse and unseen sources efficiently, aligning with Green AI principles.

Method: Proposes random masking and geometric transformations, focusing on frequency masking for its generalization benefits.

Result: Achieves state-of-the-art generalization on GAN- and diffusion-generated images and maintains performance under model pruning.

Conclusion: Frequency-based masking offers a scalable and sustainable solution for generalizable deepfake detection.
...
TL;DR: Simple random masking (M2A) for continual test-time adaptation outperforms complex methods, using consistency and entropy losses without relying on uncertainty or attention.


<details>
  <summary>Details</summary>
Motivation: Addressing degradation of image classifiers under test-time distribution shifts by simplifying masking designs.

Method: M2A uses random spatial or frequency masking with consistency and entropy minimization losses.

Result: M2A (Spatial) achieves 8.3%/19.8%/39.2% mean error on CIFAR10C/CIFAR100C/ImageNetC, outperforming baselines.

Conclusion: Random masking with consistency and entropy objectives is effective for test-time adaptation.
...
TL;DR: The paper evaluates various change detection models for deforestation in the Amazon using PRODES data, addressing gaps in existing methods and achieving improved results with modern architectures and processing techniques.


<details>
  <summary>Details</summary>
Motivation: To combat climate change and protect biodiversity by improving deforestation detection methods, addressing limitations in current models and lack of standardization.

Method: Evaluates change detection models, including convolutional and Transformer-based architectures, and tests pre- and post-processing techniques like filtering and image enhancements.

Result: Achieved an F1-score of 80.41%, demonstrating improved effectiveness and comparable performance to recent literature.

Conclusion: Modern architectures and standardized processing techniques significantly enhance deforestation detection, supporting global conservation efforts.
...
TL;DR: The CVP framework improves spatial reasoning in multimodal models by mimicking human central and peripheral vision, outperforming existing methods on 3D benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current spatial reasoning models lack explicit high-level structural understanding, limiting their effectiveness.

Method: CVP integrates target-affinity tokens (central vision) and allocentric grids (peripheral vision) into a Large Multimodal Model for structured scene understanding.

Result: CVP achieves state-of-the-art performance in 3D scene understanding benchmarks.

Conclusion: The CVP framework effectively addresses limitations in spatial reasoning by leveraging human visual field-inspired components.
...
TL;DR: The paper introduces Fourier-RWKV, a novel dehazing framework with linear complexity, integrating spatial, frequency-domain, and semantic-relation perceptions for efficient and high-quality image dehazing.


<details>
  <summary>Details</summary>
Motivation: Image dehazing is challenging under real-world non-uniform haze conditions, and existing Transformer-based methods face computational limitations.

Method: The proposed Fourier-RWKV combines Deformable Quad-directional Token Shift for spatial perception, Fourier Mix block for frequency-domain perception, and Semantic Bridge Module for semantic-relation perception.

Result: Extensive experiments show state-of-the-art performance in diverse haze scenarios with reduced computational overhead.

Conclusion: Fourier-RWKV effectively balances restoration quality and efficiency, making it suitable for real-time deployment.
...
TL;DR: The study explores the relationship between model accuracy and human similarity in monocular depth estimation, finding that improved accuracy doesn't always align with human-like behavior.


<details>
  <summary>Details</summary>
Motivation: Aligning DNN representations with human perception can enhance robustness and interpretability, especially in depth estimation for real-world applications like autonomous driving.

Method: Systematically analyzed 69 monocular depth estimators using the KITTI dataset, applying affine fitting to decompose errors into interpretable components.

Result: Humans and DNNs share some biases but exhibit a distinct trade-off between accuracy and human similarity, suggesting accuracy alone doesn't guarantee human-like behavior.

Conclusion: The study highlights the need for multifaceted, human-centric evaluations in depth estimation beyond traditional accuracy metrics.
...
TL;DR: GeoLoom is a framework for high-quality geometric diagram generation using formal languages and symbolic solvers, outperforming baselines in structural fidelity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating spatially accurate geometric diagrams with well-defined constraints, leveraging formal languages for correctness and interpretability.

Method: GeoLoom uses an autoformalization module to translate natural language into GeoLingua, a formal language, and a coordinate solver employing Monte Carlo optimization to map constraints to precise coordinates. It includes the GeoNF dataset for alignment and a constraint-based evaluation metric.

Result: GeoLoom significantly outperforms state-of-the-art baselines in structural fidelity, demonstrating its effectiveness.

Conclusion: GeoLoom provides a principled, interpretable, and scalable foundation for geometric diagram generation, validated by empirical results.
...
TL;DR: The paper proposes an on-device framework for animal re-identification (Animal Re-ID) optimized for microcontroller-class hardware, achieving competitive accuracy with a significantly smaller model size.


<details>
  <summary>Details</summary>
Motivation: Animal Re-ID is crucial for wildlife monitoring and livestock management in environments with limited connectivity, but existing models are too large for low-power edge devices.

Method: The authors analyze the gap between state-of-the-art models and MCU hardware, design a compact CNN-based MobileNetV2 backbone for low-resolution inputs, and introduce data-efficient fine-tuning.

Result: The compact model reduces size by over two orders of magnitude while maintaining competitive accuracy on six public datasets and performs well on a real-world cattle dataset.

Conclusion: The framework demonstrates that practical, adaptable Animal Re-ID is feasible on MCU-class devices, enabling scalable deployment in field environments.
...
TL;DR: The paper introduces the Visual Knowledge Base (VKB) structured as graphs and a framework called VisKnow for multi-modal object understanding. It includes a case study, AnimalKB, showcasing improved visual tasks like zero-shot recognition.


<details>
  <summary>Details</summary>
Motivation: Current object understanding lacks systematic organization of multi-modal data (visual and textual) for comprehensive perception of object categories.

Method: Proposes VisKnow framework integrating text and image knowledge with region annotations. Uses expert design and large-scale models.

Result: AnimalKB, covering 406 animal categories, enhances tasks like zero-shot recognition and fine-grained VQA.
...
TL;DR: The paper explores prompt tuning methods for adapting LLMs to 3D object detection, proposing Scene-Oriented Prompt Pool (SOP²) and demonstrating its effectiveness.


<details>
  <summary>Details</summary>
Motivation: To investigate if prompt tuning, common in NLP, can be effectively applied to 3D object detection, leveraging models trained on large datasets like Waymo.

Method: Examines prompt tokens and prompt generators, introduces SOP², and tests adaptability across 3D detection scenarios.

Result: Shows SOP²'s effectiveness, proving prompt pools can enhance performance in 3D object detection.

Conclusion: Highlights the potential of prompt-based methods in 3D fields, encouraging further research.
...
TL;DR: The paper explores using Versatile Video Coding (VVC) for compressing intermediate neural network features in split inference systems, proposing three optimized profiles for varying speed and efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: Traditional video codecs prioritize perceptual quality, which is irrelevant for abstract, task-specific neural network features. The paper investigates VVC for feature compression under the MPEG-AI FCM standard.

Method: A tool-level analysis of VVC components is conducted to assess their impact on compression efficiency and downstream task accuracy. Three lightweight VVC profiles—Fast, Faster, and Fastest—are proposed based on the analysis.

Result: The Fast profile offers a 2.96% BD-Rate gain and 21.8% faster encoding. Faster achieves a 1.85% BD-Rate gain with a 51.5% speedup. Fastest reduces encoding time by 95.6% with a minimal 1.71% BD-Rate loss.

Conclusion: The proposed VVC profiles effectively balance compression efficiency and speed for feature coding in split inference systems, addressing the limitations of traditional codecs.
...
TL;DR: The paper introduces MM-CoT, a benchmark to evaluate Chain-of-Thought reasoning in multimodal models, focusing on visual grounding and logical coherence rather than just generation. Leading models struggle, showing gaps in reasoning fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks neglect verification of reasoning chains, focusing only on generation. The lack of emphasis on visual consistency and logical coherence highlights the need for MM-CoT.

Method: MM-CoT requires models to select the correct event chain adhering to visual consistency and logical coherence, using adversarial distractors to expose reasoning failures.

Result: Leading vision-language models perform poorly on MM-CoT, revealing discrepancies between generative fluency and true reasoning fidelity. The benchmark measures unique reasoning skills.

Conclusion: MM-CoT fills a critical gap in assessing multimodal reasoning and provides a foundation for developing models with faithful and coherent reasoning abilities.
...
TL;DR: A normal-guided sparse depth sampling strategy improves depth completion by using surface normals to determine reliable depth samples, enhancing accuracy and realism.


<details>
  <summary>Details</summary>
Motivation: Existing depth completion methods often use unrealistic random sampling of sparse depth from ground truth, ignoring real sensor reliability patterns.

Method: Proposed a PCA-based surface normal estimation to compute per-pixel depth reliability, then samples sparse depth accordingly, integrated with Marigold-DC.

Result: Improved accuracy, reduced edge artifacts, and more realistic training conditions on NYU Depth v2.

Conclusion: Geometry-aware sparse depth sampling better reflects real sensor behavior and enhances depth completion performance.
...
TL;DR: FastBEV++ is a camera-only BEV perception framework that balances high performance and deployment efficiency by using algorithmic speed and deployable design principles. It achieves state-of-the-art results without custom CUDA kernels.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the bottleneck in BEV perception caused by computationally heavy view transformations and platform-specific kernels, aiming to improve both performance and deployment feasibility.

Method: FastBEV++ introduces a novel view transformation paradigm decomposing projections into Index-Gather-Reshape pipelines, uses operator-native primitives, and integrates depth-aware fusion with temporal aggregation and data augmentation.

Result: The framework achieves 0.359 NDS on nuScenes and runs at 134 FPS on Tesla T4 hardware, setting a new state-of-the-art.

Conclusion: FastBEV++ provides a scalable, accurate solution for autonomous systems by eliminating custom plugins and ensuring TensorRT-native portability.
...
TL;DR: HTC-VLM is a hybrid framework that efficiently balances fine-grained details and high-level semantics in vision-language models, achieving high performance with compressed representations.


<details>
  <summary>Details</summary>
Motivation: Vision-language models face computational inefficiencies due to quadratic costs from visual patch tokens. Traditional methods compromise either high-level semantics (continuous compression) or fine-grained details (discrete quantization).

Method: HTC-VLM uses dual channels: a continuous pathway for fine-grained details (ViT patches) and a discrete pathway for symbolic anchors (MGVQ quantization). These are fused into a 580-token hybrid sequence and compressed into a single voco token.

Result: HTC-VLM achieves 87.2% performance retention across seven benchmarks, outperforming the leading continuous baseline (81.0%) with a 580-to-1 compression ratio.

Conclusion: The hybrid design resolves the efficiency-fidelity dilemma, advancing scalable VLMs without sacrificing performance.
...
TL;DR: A progressive reparameterization strategy using PHM layers is introduced to compress multimodal language models, reducing parameters and FLOPs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal language models (MLLMs) are computationally heavy due to their large parameter capacity. There is a need for efficient methods to align high-dimensional visual features with linguistic representations without degrading performance.

Method: The paper proposes replacing dense feed-forward network blocks with compact Parameterized Hypercomplex Multiplication (PHM) layers progressively. A residual interpolation schedule and lightweight reconstruction and knowledge distillation losses ensure functional behavior inheritance.

Result: The method achieves substantial parameter and FLOP reductions while preserving strong multimodal alignment, enabling faster inference without output quality degradation. Evaluations on vision-language models (VLMs) show comparable performance to base models.

Conclusion: Progressive PHM substitution provides an efficient path for multimodal reasoning, complementing existing low-bit quantization techniques, and offers significant reductions in model size and inference latency.
...
TL;DR: The paper introduces Residual-SwinCA-Net, a hybrid segmentation framework combining residual CNN and Swin Transformer blocks for improved breast lesion segmentation, achieving superior accuracy and robustness on the BUSI dataset.


<details>
  <summary>Details</summary>
Motivation: The study aims to address challenges in breast lesion segmentation by combining local and global feature extraction, noise suppression, and morphological integrity preservation to enhance diagnostic performance.

Method: The framework integrates residual CNN modules, Swin Transformer blocks, Laplacian-of-Gaussian operators, boundary-oriented operators, and a Multi-Scale Channel Attention and Squeezing (MSCAS) module for feature refinement and computational efficiency.

Result: Residual-SwinCA-Net achieved 99.29% mean accuracy, 98.74% IoU, and 0.9041 Dice on the BUSI dataset, outperforming existing CNNs/ViTs techniques.

Conclusion: The proposed framework enhances breast lesion segmentation accuracy and diagnostic performance, supporting timely clinical decision-making.
...
TL;DR: The paper introduces Sign Language Spotting, a novel task for detecting query signs within continuous sign sequences, using pose keypoints for efficiency.


<details>
  <summary>Details</summary>
Motivation: Bridging communication gaps between deaf and hearing communities by addressing unexplored challenges in sign-to-sign retrieval.

Method: Proposes an end-to-end model with an encoder-only backbone and binary classification head, focusing on pose keypoints.

Result: Achieves 61.88% accuracy and 60.00% F1-score on the WSLP 2025 dataset.

Conclusion: Establishes a pose-based framework as effective for Sign Language Spotting, paving the way for future research.
...
TL;DR: FTKD is a knowledge distillation method transferring future frame knowledge from offline to online models via sparse queries, improving 3D object detection accuracy without added inference cost.


<details>
  <summary>Details</summary>
Motivation: Existing KD methods fail to leverage future frames effectively, limiting online models' ability to learn future knowledge. FTKD addresses this gap.

Method: FTKD uses future-aware feature reconstruction and future-guided logit distillation to transfer knowledge without strict frame alignment.

Result: FTKD improves performance by up to 1.3 mAP and 1.3 NDS on nuScenes, with superior velocity estimation.

Conclusion: FTKD effectively transfers future knowledge, enhancing online model performance without additional inference overhead.
...
TL;DR: The paper introduces a Query-aware Hub Prototype (QHP) learning method to address prototype bias in few-shot 3D point cloud semantic segmentation by modeling correlations between support and query sets and refining prototypes for better generalization.


<details>
  <summary>Details</summary>
Motivation: Existing metric-based prototype learning methods in FS-3DSeg suffer from prototype bias due to overfitting support-specific characteristics and failing to generalize to query data, especially with distribution shifts.

Method: The proposed QHP method includes a Hub Prototype Generation (HPG) module to create query-relevant prototypes via bipartite graphs and a Prototype Distribution Optimization (PDO) module using purity-reweighted contrastive loss to refine prototypes.

Result: QHP outperforms state-of-the-art methods on S3DIS and ScanNet datasets, reducing the semantic gap between prototypes and query sets.

Conclusion: QHP effectively mitigates prototype bias and improves segmentation performance in FS-3DSeg by refining query-aware prototypes.
...
TL;DR: The paper proposes Spatial and Frequency Priors (SFP) for real-world scene recovery, combining spatial and frequency domain insights with a weighted fusion strategy to handle diverse degradations.


<details>
  <summary>Details</summary>
Motivation: Existing methods for scene recovery are limited by reliance on single priors or synthetic data, leading to poor generalization in real-world scenarios.

Method: The SFP approach leverages spatial priors (inverse image projection resembles transmission) and frequency priors (adaptive enhancement mask based on DC component mean intensity and low radial frequency magnitude). A weighted fusion integrates spatial restoration, frequency enhancement, and input features.

Result: Extensive evaluations show SFP's effectiveness and superiority in handling various degradation conditions.

Conclusion: SFP offers a robust solution for real-world scene recovery by integrating spatial and frequency domain insights with adaptive fusion.
...
TL;DR: RLCNet is a deep learning framework for online calibration of LiDAR, RADAR, and cameras in autonomous vehicles, offering robust performance and real-time adjustments.


<details>
  <summary>Details</summary>
Motivation: Extrinsic calibration of multimodal sensors is critical for autonomous vehicles but challenging due to dynamic environments and sensor drift.

Method: RLCNet, an end-to-end trainable framework, uses weighted moving average and outlier rejection for dynamic calibration adjustments.

Result: Validated on real-world datasets, RLCNet shows superior accuracy, robustness, and reduced prediction noise compared to existing methods.

Conclusion: RLCNet effectively addresses calibration challenges, demonstrating practical deployment potential for autonomous vehicles.
...
TL;DR: EgoX is a framework for converting third-person videos into immersive first-person videos using lightweight adaptations of video diffusion models and a novel conditioning strategy.


<details>
  <summary>Details</summary>
Motivation: The goal is to enable immersive understanding by transforming exocentric videos into egocentric ones, addressing challenges like extreme camera pose variations and minimal view overlap.

Method: EgoX uses pretrained video diffusion models with LoRA adaptation, a unified conditioning strategy combining exocentric and egocentric priors, and a geometry-guided self-attention mechanism for coherence.

Result: The framework successfully generates coherent and realistic egocentric videos, showing scalability and robustness with unseen and in-the-wild videos.

Conclusion: EgoX advances egocentric video generation by maintaining geometric consistency and visual fidelity, opening new possibilities for immersive experiences.
...
TL;DR: PAVAS enhances Video-to-Audio generation by integrating physical reasoning via Phy-Adapter, improving realism beyond appearance-driven models, and outperforms existing methods in evaluations.


<details>
  <summary>Details</summary>
Motivation: Current V2A models lack consideration of physical factors influencing real-world sounds, limiting their realism and coherence.

Method: PAVAS uses Phy-Adapter to incorporate physical parameters (mass, velocity) estimated by PPE and a VLM-based approach for dynamic 3D reconstruction, enabling physics-aware audio synthesis.

Result: PAVAS outperforms existing V2A models, producing more plausible and perceptually coherent audio, validated by the VGG-Impact benchmark and APCC metric.

Conclusion: Integrating physical reasoning into V2A synthesis significantly improves audio realism, setting a new standard for physics-aware sound generation.
...
TL;DR: OpenSubject is a large-scale video-derived dataset designed to improve subject-driven image generation and manipulation, addressing deviations in identity and complex scenes through a four-stage pipeline and extensive verification.


<details>
  <summary>Details</summary>
Motivation: Current models for subject-driven image generation often fail to maintain reference identities and struggle with complex scenes. OpenSubject aims to resolve these issues by providing a high-quality, large-scale dataset.

Method: The dataset is built via a four-stage pipeline: video curation, cross-frame subject mining and pairing, identity-preserving reference image synthesis (using segmentation and box-guided techniques), and verification and captioning. A benchmark evaluates performance.

Result: Training with OpenSubject enhances generation and manipulation, particularly in complex scenes, as demonstrated by extensive experiments.

Conclusion: OpenSubject successfully addresses challenges in subject-driven generation and manipulation, offering a robust dataset and pipeline for improved model performance.
...
TL;DR: Terrain Diffusion introduces InfiniteDiffusion, a novel AI-era method combining diffusion models with procedural noise benefits for infinite, coherent, and controllable landscape synthesis.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of traditional procedural noise functions (e.g., Perlin noise) in realism and large-scale coherence while retaining their advantages.

Method: Uses InfiniteDiffusion for infinite generation, hierarchical diffusion models for planetary context and local detail, Laplacian encoding for stability, and open-source tools for unbounded tensors.

Result: Enables seamless, real-time synthesis of limitless landscapes with coherence and controllability.

Conclusion: Diffusion models now serve as a practical foundation for procedural world generation, capable of creating coherent planets without limits.
...
TL;DR: GeoDM is a geometry-aware dataset distillation framework that captures diverse data structures (Euclidean, hyperbolic, spherical) in a unified representation, improving performance over Euclidean-only methods.


<details>
  <summary>Details</summary>
Motivation: High-dimensional data often lie on low-dimensional manifolds with varied geometries, but existing dataset distillation methods overlook these intrinsic structures, limiting their effectiveness.

Method: GeoDM introduces a framework operating in the Cartesian product of Euclidean, hyperbolic, and spherical manifolds, using learnable curvature and weight parameters. It also employs an optimal transport loss for distribution fidelity.

Result: Theoretical analysis shows GeoDM outperforms Euclidean methods in generalization error bounds. Experiments confirm its superiority over state-of-the-art dataset distillation techniques.

Conclusion: GeoDM effectively captures diverse data geometries, enhancing dataset distillation performance and demonstrating broader applicability across distribution-matching strategies.
...
TL;DR: Summary of a paper on teeth landmark detection using deep learning for precise diagnostics and treatment in orthodontics, highlighting the 3DTeethLand challenge and its dataset.


<details>
  <summary>Details</summary>
Motivation: Accurate teeth landmark detection is crucial for advanced diagnostics and personalized treatments in orthodontics, but challenges exist due to tooth geometry and individual variations.

Method: The paper discusses the use of deep learning techniques and the 3DTeethLand challenge to develop algorithms for detecting landmarks from intraoral 3D scans.

Result: The challenge introduced the first public dataset for 3D teeth landmark detection, enabling evaluation of state-of-the-art methods and fostering community contributions.

Conclusion: The initiative aims to advance reliable teeth landmark detection, benefiting clinical dentistry with improved diagnostics and treatment monitoring.
...
TL;DR: GeoDiffMM is a diffusion-based Lagrangian VMM framework using optical flow for noise-free motion magnification.


<details>
  <summary>Details</summary>
Motivation: Existing Eulerian VMM methods fail to separate photon noise from true micro-motion in small displacements.

Method: GeoDiffMM uses noise-free optical flow augmentation, diffusion motion magnification conditioning on flow geometry, and flow-based video synthesis.

Result: Outperforms state-of-the-art methods in motion magnification on real and synthetic datasets.

Conclusion: GeoDiffMM enables structurally consistent motion amplification by leveraging geometry-aware optical flow and diffusion models.
...
TL;DR: This study analyzes the internal structure and detectability of adversarial perturbations like Glaze and Nightshade in image protection, revealing their structured, low-entropy nature tied to image content across domains.


<details>
  <summary>Details</summary>
Motivation: To understand how imperceptible adversarial perturbations disrupt text-to-image models and their underlying mechanisms.

Method: Uses a unified framework combining white-box feature-space inspection and black-box signal-level probing, including latent-space clustering, feature-channel activation analysis, spatial sensitivity mapping, and frequency-domain characterization.

Result: Protection mechanisms are structured perturbations coupled to image content, preserving feature organization with detectable substructure. Frequency analysis shows energy redistribution along dominant axes, not diffuse noise.

Conclusion: Contemporary protection works via structured feature deformation, not semantic dislocation, explaining subtle yet detectable signals. Findings aid future defense and detection designs.
...
TL;DR: The paper introduces PointDico, a novel model that combines diffusion and contrastive learning for 3D representation learning. It addresses challenges in 3D data by leveraging hierarchical pyramid conditional generators and dual-channel designs, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: 3D data's unordered and uneven density poses challenges for self-supervised representation learning. Existing contrastive and generative methods have limitations (e.g., overfitting, difficulty handling unordered point clouds), motivating the integration of diffusion and contrastive models.

Method: PointDico combines denoising generative modeling and cross-modal contrastive learning via knowledge distillation. It uses a hierarchical pyramid conditional generator for multi-scale feature extraction and a dual-channel design to integrate local and global context.

Result: PointDico achieves top performance in 3D representation learning, including 94.32% accuracy on ScanObjectNN and 86.5% Instance mIoU on ShapeNetPart.

Conclusion: The integration of diffusion and contrastive learning in PointDico effectively addresses 3D data challenges, setting a new benchmark for 3D representation learning.
...
TL;DR: MLLMs struggle with low-resource languages due to lack of cultural and multimodal awareness. MELLA dataset addresses this by enhancing linguistic and cultural capabilities, improving performance.


<details>
  <summary>Details</summary>
Motivation: Current multilingual enhancement methods lack multimodal and cultural awareness, limiting MLLMs' effectiveness for low-resource languages.

Method: Proposed a dual-source strategy (native web alt-text for culture, MLLM-generated captions for linguistics) and introduced the MELLA dataset.

Result: Fine-tuning on MELLA improved performance across eight languages, yielding 'thick descriptions' through cultural and linguistic enhancements.

Conclusion: MELLA bridges the gap in MLLMs for low-resource languages by integrating cultural groundedness and linguistic capability.
...
TL;DR: Bi^2MAC addresses pansharpening limitations by using bimodal adaptive masks to efficiently process heterogeneous features, achieving SOTA performance with lower computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods struggle with regional heterogeneity and high computational costs in pansharpening.

Method: Bi^2MAC uses soft and hard masks to guide features into separate branches: a compact branch for redundant features and a focused branch for heterogeneous features.

Result: Outperforms existing methods with lower training time, fewer parameters, and minimal computational cost.

Conclusion: Bi^2MAC effectively balances performance and efficiency for pansharpening.
...
TL;DR: HybridSplat introduces reflection-baked Gaussian tracing and tile-based splatting to improve rendering speed and reduce memory usage while maintaining reflection quality in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian splatting methods struggle with rendering speed and memory storage for complex reflective scenes.

Method: Proposes HybridSplat, combining reflection-baked Gaussian tracing with tile-based splatting and reflection-sensitive pruning for efficiency.

Result: Achieves 7x faster rendering and 4x fewer primitives than ray-tracing baselines, preserving reflection quality.

Conclusion: HybridSplat sets a new state-of-the-art for rendering complex reflective scenes efficiently.
...
TL;DR: DINO-BOLDNet uses DINOv3-guided multi-slice attention to generate BOLD images from T1w images, achieving better performance than a GAN baseline.


<details>
  <summary>Details</summary>
Motivation: Recovering missing BOLD information when images are corrupted or unavailable.

Method: Integrates frozen DINOv3 encoder with lightweight decoder, uses slice-attention module, and multi-scale decoder with perceptual loss.

Result: Outperforms conditional GAN in PSNR and MS-SSIM on a clinical dataset of 248 subjects.

Conclusion: First framework for direct BOLD generation from T1w images, showcasing self-supervised transformer potential.
...
TL;DR: The paper introduces TrackingWorld, a novel pipeline for dense 3D tracking in a world-centric coordinate system, addressing limitations in existing monocular 3D tracking methods by separating camera motion and tracking emerging objects.


<details>
  <summary>Details</summary>
Motivation: Existing monocular 3D tracking methods struggle with separating camera motion from dynamic motion and tracking newly emerging objects densely.

Method: The proposed method includes a tracking upsampler for dense 2D tracks, redundancy reduction, and an optimization framework for 3D back-projection.

Result: The system achieves accurate and dense 3D tracking in a world-centric frame, validated on synthetic and real-world datasets.

Conclusion: TrackingWorld effectively addresses the limitations of current methods and provides a robust solution for dense 3D tracking.
...
TL;DR: The paper proposes SCU-CGAN, a model integrating U-Net, CBAM, and an additional discriminator, to generate realistic fire images from nonfire images. It outperforms existing models and significantly improves fire detection accuracy when used for dataset augmentation.


<details>
  <summary>Details</summary>
Motivation: Limited fire datasets hinder the performance of fire detection models. Addressing this, the paper aims to generate realistic fire images to enhance dataset quality and detection accuracy.

Method: The SCU-CGAN model combines U-Net, CBAM, and an extra discriminator to transform nonfire images into realistic fire images. Performance is evaluated using image quality metrics and impact on fire detection models.

Result: SCU-CGAN shows a 41.5% improvement in KID score over CycleGAN and boosts the mAP@0.5:0.95 metric of YOLOv5 nano by 56.5% when used for dataset augmentation.

Conclusion: The SCU-CGAN model effectively generates high-quality fire images, enhancing fire detection model performance without structural changes, proving its utility in real-world applications.
...
TL;DR: The paper identifies a norm disparity in Multimodal Large Language Models (MLLMs) causing asymmetric update dynamics between visual and text tokens, proposes a simple LayerNorm solution, and validates its effectiveness across benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the norm disparity between high-norm visual tokens and low-norm text tokens in MLLMs, which hampers cross-modal feature fusion.

Method: A theoretical analysis is presented, followed by the introduction of a LayerNorm layer after the visual projector to align norms.

Result: Experiments on LLaVA-1.5 show significant performance improvements on multimodal and text-only benchmarks.

Conclusion: Resolving the architectural imbalance with a simple LayerNorm intervention enhances the model's overall capabilities.
...
TL;DR: The paper proposes a framework for enhancing images and suppressing noise under complex lighting, using GDWGIF and Retinex for layer decomposition and parallel processing. It outperforms existing methods in both enhancement and noise reduction.


<details>
  <summary>Details</summary>
Motivation: Images under poor lighting suffer from degradations, reducing vision application performance. Existing methods either amplify noise or work only under specific conditions.

Method: Uses GDWGIF for illumination estimation, Retinex for layer decomposition, and parallel processing (illumination correction, reflection enhancement). Optimizes dynamic range via multi-exposure fusion and linear stretching.

Result: Achieves better performance in contrast enhancement and noise suppression compared to state-of-the-art methods, validated on real-world datasets.

Conclusion: The proposed framework effectively enhances images and suppresses noise under complex lighting, outperforming existing solutions.
...
TL;DR: The paper addresses the challenge of detecting facial retouching in images used for biometric systems, proposing AI-based methods to improve detection rates and leveraging beauty assessment algorithms.


<details>
  <summary>Details</summary>
Motivation: Facial retouching complicates face recognition systems, making its detection crucial for biometric accuracy.

Method: The study analyzes beauty assessment algorithms, evaluates AI-based feature extraction methods, and explores whether face beauty can enhance retouching detection.

Result: Achieved a 1.1% D-EER on single image detection when the retouching algorithm is unknown.

Conclusion: The proposed methods effectively improve facial retouching detection, demonstrating potential for biometric system reliability.
...
TL;DR: The paper develops an optimized deep learning pipeline for automated fish re-identification using the AutoFish dataset, improving key metrics with hard triplet mining and custom image transformations. Swin-T outperforms ResNet-50, achieving 41.65% mAP@k and 90.43% Rank-1 accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate fisheries data is vital for sustainable marine resource management, but manual review of EM system videos is impractical. Automated fish re-identification can address this challenge.

Method: The paper employs hard triplet mining and a custom image transformation pipeline, including dataset-specific normalization, to optimize fish re-identification. It uses the AutoFish dataset and compares Vision Transformer (Swin-T) with ResNet-50.

Result: Swin-T outperforms ResNet-50, achieving 41.65% mAP@k and 90.43% Rank-1 accuracy. Intra-species errors, particularly viewpoint inconsistency, are identified as the primary challenge.

Conclusion: The proposed pipeline significantly improves fish re-identification accuracy, demonstrating the superiority of Swin-T over ResNet-50. Intra-species distinctions remain the main challenge.
...
TL;DR: SAM-Body4D is a training-free framework for temporally consistent and occlusion-robust Human Mesh Recovery (HMR) from videos, leveraging identity-consistent masklets and occlusion-aware refinement.


<details>
  <summary>Details</summary>
Motivation: Current image-based HMR methods like SAM 3D Body face issues of temporal inconsistency and degraded performance under occlusions when applied to videos. SAM-Body4D aims to address these without extra training.

Method: The framework generates identity-consistent masklets using a video segmentation model, refines them with an Occlusion-Aware module, and guides SAM 3D Body for consistent full-body mesh trajectories. A padding-based parallel strategy enables efficient multi-human inference.

Result: SAM-Body4D demonstrates improved temporal stability and robustness in challenging videos without requiring retraining.

Conclusion: The proposed framework effectively addresses temporal inconsistency and occlusion issues in HMR for videos, offering a practical and efficient solution.
...
TL;DR: OneClip-RAG is a novel paradigm enhancing Multimodal Large Language Models (MLLMs) by using one-shot video clips for retrieval augmentation, improving knowledge integrity, semantic coherence, and efficiency in long-video understanding.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with memory overhead, limiting video frame processing. OneClip-RAG addresses this by optimizing video retrieval and chunking.

Method: Proposes OneClip-RAG with a query-guided video chunking algorithm and introduces SynLongVideo dataset for progressive training.

Result: OneClip-RAG boosts MLLMs (e.g., InternLV2, Qwen2-VL) to GPT-4o levels on benchmarks and handles hour-long videos efficiently (~2.2 minutes on a 4090 GPU).

Conclusion: OneClip-RAG significantly enhances MLLM performance and efficiency for long-video tasks, validated across benchmarks and models.
...
TL;DR: A depth-only 6D pose estimation method for cluttered industrial bin-picking, using multi-view depth maps and a staged heatmap mechanism for efficient computation.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like occlusions, reflections, and textureless parts in densely packed industrial environments.

Method: Fuses multi-view depth maps into fine-grained 3D point clouds or TSDF; uses a staged heatmap mechanism and density-aware sparse transformer block.

Result: Competitive performance on IPD and MV-YCB datasets, handling clutter effectively.

Conclusion: The framework enables high-resolution volumetric representations for accurate pose estimation in cluttered scenes.
...
TL;DR: LapFM is a surgical foundation model that leverages a hierarchical concept evolving pre-training paradigm to improve segmentation across diverse surgical targets, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing surgical segmentation models struggle with annotation scarcity and semantic inconsistency. LapFM addresses these issues by evolving robust segmentation capabilities from unlabeled data.

Method: LapFM uses a Hierarchical Concept Evolving Pre-training paradigm, including a Laparoscopic Concept Hierarchy (LCH) and Confidence-driven Evolving Labeling to generate pseudo-labels from unlabeled images.

Result: LapFM significantly outperforms state-of-the-art methods and creates LapBench-114K, a large-scale benchmark with 114K image-mask pairs.

Conclusion: LapFM sets new standards for universal laparoscopic segmentation by addressing the variability and annotation challenges in surgical scenes.
...
TL;DR: The paper introduces a unified learning-based framework for end-to-end color correction using RGB and MS sensors, improving accuracy by up to 50%.


<details>
  <summary>Details</summary>
Motivation: Existing color correction methods often discard multispectral (MS) data early and treat the pipeline in separate stages, limiting accuracy.

Method: A unified model integrates RGB and MS sensor data for end-to-end color correction, refactoring two image-to-image architectures.

Result: The framework reduces color error by up to 50% compared to RGB-only and MS-driven baselines, showcasing improved accuracy and stability.

Conclusion: The proposed approach offers a flexible, general solution for color correction, leveraging both RGB and MS data effectively.
...
TL;DR: The paper analyzes the shortcomings of subset-based explanation methods for deep vision models under out-of-distribution (OOD) conditions and proposes a framework combining submodular subset selection with uncertainty estimation to improve robustness.


<details>
  <summary>Details</summary>
Motivation: Existing subset-based explanation methods perform well in in-distribution (ID) settings but degrade under OOD conditions, producing unreliable explanations. The study aims to address these weaknesses.

Method: The framework integrates submodular subset selection with layer-wise, gradient-based uncertainty estimation. It uses adaptive weight perturbations to estimate uncertainty and guides submodular optimization for diverse and informative subset selection.

Result: Empirical evaluations show the framework mitigates weaknesses of existing methods in OOD scenarios and improves performance in ID settings, enhancing attribution and interpretability.

Conclusion: The findings highlight limitations of current subset-based methods and demonstrate how uncertainty-driven optimization can improve transparency and trustworthiness in AI vision applications.
...
TL;DR: A lightweight SAM-based tracking method for football players combines SAM with CSRT trackers and jersey color models, improving occlusion recovery and tracking success in crowded scenes.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like occlusions and similar appearances in football player tracking.

Method: Combines Segment Anything Model (SAM) with CSRT trackers and HSV histogram-based re-identification for team-aware tracking.

Result: Achieves 7.6-7.7 FPS with stable memory (~1880 MB), 100% success in light occlusions, 90% in crowded scenes, and 50% recovery in heavy occlusions. Struggles with long-term occlusions (8.66% re-acquisition).

Conclusion: Classical trackers excel with continuous visibility but need stronger re-ID for extended absences, offering practical guidelines for resource-constrained deployments.
...
TL;DR: ContextDrag is a new drag-based image editing method that improves coherence and fidelity by leveraging contextual cues and preserving fine details with novel techniques like CTI and PCA.


<details>
  <summary>Details</summary>
Motivation: Existing drag-based editing methods fail to fully exploit contextual information from reference images, leading to limited coherence and fidelity in edits.

Method: ContextDrag introduces Context-preserving Token Injection (CTI) and Position-Consistent Attention (PCA) to leverage reference features and maintain consistency in semantics and texture.

Result: Extensive experiments on DragBench-SR and DragBench-DR show ContextDrag outperforms state-of-the-art methods.

Conclusion: ContextDrag addresses limitations in existing drag-based editing by effectively utilizing contextual information and preserving details, achieving superior results.
...
TL;DR: Visionary is a web-native platform for real-time neural rendering, offering efficient dynamic processing and seamless integration via WebGPU and ONNX inference.


<details>
  <summary>Details</summary>
Motivation: Existing solutions for neural rendering are fragmented or constrained, causing deployment challenges and limited support for dynamic content and generative models.

Method: Built on WebGPU with per-frame ONNX inference, Visionary supports a standardized Gaussian Generator contract and integrates with three.js via TypeScript API.

Result: Visionary achieves superior rendering efficiency and supports various 3DGS variants, MLP-based 3DGS, and generative post-processing.

Conclusion: Visionary lowers barriers for deploying and comparing 3DGS methods, serving as a unified World Model Carrier.
...
TL;DR: PCI (Prompt-Conditioned Intervention) analyzes when concepts emerge during diffusion model generation, using Concept Insertion Success (CIS) to measure effectiveness without training or model access.


<details>
  <summary>Details</summary>
Motivation: Understanding the dynamic process of diffusion models is key to improving their controllability, reliability, and predictability.

Method: PCI introduces CIS to study concept dynamics across diffusion time, applied to text-to-image models.

Result: PCI reveals diverse temporal behaviors in concept formation, identifying optimal phases for interventions.

Conclusion: PCI provides actionable insights for image editing, balancing semantic accuracy and preservation without model internals or training.
...
TL;DR: A new on-the-fly 3D reconstruction framework using multi-camera rigs improves coverage, speed, and fidelity by fusing dense RGB streams into a unified Gaussian representation with hierarchical initialization and optimized scheduling.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with incomplete 3D coverage due to limited FOV in monocular setups. Multi-camera rigs offer a solution.

Method: Hierarchical camera initialization, lightweight multi-camera bundle adjustment, redundancy-free Gaussian sampling, and frequency-aware optimization.

Result: Achieves drift-free trajectory estimation, efficient online reconstruction, and hundreds of meters reconstructed in 2 minutes.

Conclusion: The framework sets a new benchmark for speed, robustness, and fidelity in real-time 3D scene reconstruction.
...
TL;DR: ReasonBreak is an adversarial framework disrupting MLRMs' hierarchical reasoning to protect geographic privacy, outperforming existing methods with concept-aware perturbations.


<details>
  <summary>Details</summary>
Motivation: Current privacy protection techniques are ineffective against MLRMs' multi-step reasoning, necessitating a new approach to safeguard sensitive geographic data.

Method: ReasonBreak uses concept-aware perturbations to disrupt critical conceptual dependencies in reasoning chains, validated by the GeoPrivacy-6K dataset.

Result: ReasonBreak improves tract-level protection by 14.4% (33.8% vs 19.4%) and nearly doubles block-level protection (33.5% vs 16.8%) across seven state-of-the-art MLRMs.

Conclusion: The work introduces a paradigm shift in privacy protection against reasoning-based threats, demonstrating the efficacy of concept-aligned adversarial perturbations.
...
TL;DR: NoisyCLIP detects text/image misalignments early in denoising, reducing computational cost by 50% while maintaining 98% CLIP alignment performance.


<details>
  <summary>Details</summary>
Motivation: Address misalignment and hallucinations in conditional diffusion models without waiting for full generation completion.

Method: Proposes NoisyCLIP, measuring semantic alignment in noisy latent space using dual encoders during reverse diffusion.

Result: Reduces computational cost by 50% and achieves 98% CLIP alignment performance in BoN settings.

Conclusion: Enables real-time alignment assessment during generation, cutting costs without compromising semantic fidelity.
...
TL;DR: OCCDiff uses latent diffusion in occupancy function space to accurately reconstruct buildings from LiDAR point clouds, handling varying point densities and noise.


<details>
  <summary>Details</summary>
Motivation: The challenge of capturing building surfaces from LiDAR data with varying point densities and noise interference motivates the development of a flexible solution.

Method: OCCDiff combines latent diffusion with a function autoencoder, a point encoder for condition features, and multi-task training to generate continuous occupancy functions.

Result: The method produces physically consistent samples with high fidelity to the target distribution and shows robustness to noisy data.

Conclusion: OCCDiff effectively reconstructs buildings from LiDAR point clouds by leveraging latent diffusion and robust feature learning, even under noisy conditions.
...
TL;DR: The paper introduces Self-Calling Chain-of-Thought (sCoT), a language-only visual reasoning paradigm that improves efficiency and performance over multimodal CoT methods.


<details>
  <summary>Details</summary>
Motivation: Current multimodal Chain-of-Thought (CoT) methods face challenges in optimization due to scarce high-quality reasoning data and inefficiency in interleaving modalities.

Method: sCoT decomposes visual reasoning tasks into subtasks solved by virtual subagents in isolated contexts, avoiding explicit modality interleaving. It uses group-relative policy optimization for reinforcement.

Result: Experiments show sCoT improves reasoning performance by up to 1.9% with 75% fewer GPU hours compared to baselines.

Conclusion: sCoT offers a more efficient and effective approach to visual reasoning by leveraging language-only CoT with self-calling.
...
TL;DR: MVP is a training-free framework that improves GUI grounding stability by aggregating predictions from multiple views, addressing instability caused by minor visual perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing GUI grounding models suffer from coordinate prediction instability due to minor visual perturbations, which severely impacts performance, especially for high-resolution and small UI elements.

Method: MVP uses Multi-View Prediction, combining Attention-Guided View Proposal to generate diverse views and Multi-Coordinates Clustering to aggregate predictions by selecting the densest spatial cluster.

Result: MVP significantly improves performance on benchmarks, boosting models like UI-TARS-1.5-7B to 56.1%, GTA1-7B to 61.7%, Qwen3VL-8B-Instruct to 65.3%, and Qwen3VL-32B-Instruct to 74.0%.

Conclusion: MVP effectively stabilizes GUI grounding predictions by leveraging multi-view inference, demonstrating robust performance across models and benchmarks.
...
TL;DR: The paper introduces a multimodal framework for generating and editing oil paintings, enabling precise semantic control, spatial alignment, and stylistic consistency through innovative techniques like spatial-semantic conditioning, self-supervised style transfer, and feature integration.


<details>
  <summary>Details</summary>
Motivation: Existing techniques for oil painting generation and editing are limited by training data distribution and often focus on modifying real photographs, lacking precision and stylistic consistency. This work aims to overcome these limitations.

Method: The proposed framework includes three key advancements: (1) spatial alignment and semantic enhancement during training, (2) self-supervised style transfer using Stroke-Based Rendering (SBR) to address data scarcity, and (3) feature integration via AdaIN for stylistic consistency during inference.

Result: The system achieves fine-grained editing of oil paintings while preserving artistic qualities, enabling unprecedented imagination realization in stylized generation and editing.

Conclusion: The unified multimodal framework successfully addresses challenges in oil painting generation and editing, offering precise control and stylistic consistency through innovative training and inference techniques.
...
TL;DR: Photo3D improves photorealistic 3D generation by using GPT-4o-Image-generated images, addressing multi-view inconsistencies and enhancing details while preserving structural integrity.


<details>
  <summary>Details</summary>
Motivation: The lack of diverse, high-quality 3D assets with rich textures hinders realistic 3D generation due to challenges in data capture.

Method: Photo3D introduces a structure-aligned multi-view synthesis pipeline, a detail-enhanced dataset, and a detail enhancement scheme using perceptual feature adaptation and semantic structure matching.

Result: The framework generalizes across 3D-native generators, achieving state-of-the-art photorealistic 3D generation.

Conclusion: Photo3D successfully bridges the gap between 3D geometry and realistic textures, advancing photorealistic 3D synthesis.
...
TL;DR: Fast-ARDiff accelerates AR-diffusion hybrid models by jointly optimizing AR and diffusion components, reducing latency with entropy-informed speculative decoding and dynamic scheduling.


<details>
  <summary>Details</summary>
Motivation: To address the high latency in AR-diffusion hybrids caused by sequential AR generation and iterative denoising.

Method: Introduces entropy-informed speculative decoding and a dynamic scheduler for joint optimization. Uses joint distillation for diffusion stability and shallow feature entropy filtering to reduce redundancy.

Result: Achieves 4.3x speedup on ImageNet 256x256 and 3x acceleration in text-conditioned generation.

Conclusion: Fast-ARDiff effectively reduces latency while maintaining synthesis quality, advancing AR-diffusion hybrid frameworks.
...
TL;DR: This paper introduces a new quaternion Wasserstein distance to address chromatic aberration and data distribution issues in color image generation, proposing a novel generative adversarial network that outperforms existing models.


<details>
  <summary>Details</summary>
Motivation: Existing color image generation models fail to account for correlations among color channels and lack systematic theory for measuring color image datasets.

Method: The paper defines a quaternion Wasserstein distance, derives its dual theory using quaternion convex set separation and Farkas lemma, and proposes a Wasserstein quaternion GAN.

Result: The proposed model outperforms traditional and quaternion GANs, as well as Wasserstein GANs, in generation efficiency and image quality.

Conclusion: The quaternion Wasserstein distance and the new GAN model effectively address the challenges in color image generation, offering superior performance.
...
TL;DR: A novel iteration-free fixed-point estimator for diffusion inversion is proposed to eliminate high computational costs and hyperparameter selection issues while maintaining superior reconstruction performance.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion inversion methods suffer from high computational costs due to iterative processes and complex hyperparameter selection, motivating the need for a more efficient solution.

Method: The authors derive an explicit fixed-point expression from an ideal inversion step, introduce error approximation using previous step errors, and develop an unbiased, low-variance estimator.

Result: The method outperforms DDIM inversion and other fixed-point iteration-based methods on NOCAPS and MS-COCO datasets without extra iterations or training.

Conclusion: The proposed iteration-free fixed-point estimator provides a computationally efficient and accurate alternative for diffusion inversion tasks.
...
TL;DR: The paper introduces SSCATeR, a method focusing LiDAR object detection on changing regions to reduce processing time without losing accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve computational efficiency in LiDAR object detection by concentrating efforts on regions with point data changes.

Method: Uses sliding time windows and temporal data reuse to ignore unchanged regions, extending scatter-based convolutions for sparsity exploitation.

Result: Achieves identical feature maps to traditional methods but with a 6.61-fold reduction in processing time.

Conclusion: SSCATeR significantly enhances computational efficiency while maintaining detection accuracy.
...
TL;DR: The paper introduces a large-scale, automated framework to discover and explain visual representations in the human cortex using fMRI data.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of understanding how the brain represents visual concepts and where these representations are encoded, overcoming limitations of small-scale, manual studies.

Method: A two-stage approach: (1) unsupervised decomposition to find candidate patterns in fMRI data, and (2) automated explanation of these patterns by identifying eliciting images and generating natural-language descriptions.

Result: The framework uncovers thousands of interpretable patterns, including fine-grained visual representations not previously reported.

Conclusion: This scalable, automated method advances the study of visual representations in the brain, enabling large-scale discovery and validation.
...
TL;DR: The paper introduces a modular neural ISP framework for high-quality image rendering, emphasizing control, scalability, and user customization.


<details>
  <summary>Details</summary>
Motivation: To improve upon existing neural ISP designs by introducing modularity for better control, scalability, and flexibility in image rendering.

Method: Developed a modular neural ISP framework with stages for rendering control, built a photo-editing tool for user interaction, and used learning-based variants of moderate size.

Result: Achieved competitive qualitative and quantitative results across test sets, with pipeline sizes ranging from ~0.5M to ~3.9M parameters.

Conclusion: The modular neural ISP framework successfully enhances rendering accuracy, scalability, and user flexibility, demonstrated through a practical photo-editing tool.
...
TL;DR: The paper proposes a method for Continual Test-Time Adaptation (CTTA) that adaptively adjusts pseudo labels and dynamically balances learning to improve semantic segmentation under evolving domains.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods use fixed thresholds, which fail to handle varying difficulty across classes and instances, especially in dense, multi-class tasks like semantic segmentation.

Method: The approach dynamically adjusts pseudo labels based on confidence distributions within each image and balances learning toward classes most affected by domain shifts.

Result: The method outperforms state-of-the-art techniques in eight CTTA and TTA scenarios, including synthetic-to-real and long-term shifts.

Conclusion: The proposed solution sets a new standard for semantic segmentation in continually evolving domains by providing reliable supervision and mitigating error accumulation.
...
TL;DR: HiGINE is a hierarchical graph-based method predicting lung cancer survival using TME analysis in mIF images, improving risk stratification by capturing cell interactions and fusing multimodal data.


<details>
  <summary>Details</summary>
Motivation: The tumor microenvironment (TME) holds prognostic potential, but current methods lack comprehensive analysis of cell interactions.

Method: HiGINE hierarchically models local and global cell relationships in mIF images, combining cell type, morphology, and cancer stage data for multimodal fusion.

Result: Validated on two datasets, HiGINE shows enhanced risk stratification, robustness, and generalizability.

Conclusion: HiGINE effectively leverages TME complexity for improved lung cancer prognosis.
...
TL;DR: Automated method aligns and selects least occluded camera views in surgical videos to improve clarity and comfort for educational and research purposes.


<details>
  <summary>Details</summary>
Motivation: Video recordings of surgeries are essential for education and research, but occlusions and manual adjustments hinder unobstructed views.

Method: Identifies frames where the lighting system moves, realigns them, and selects the least occluded camera view to generate stable videos.

Result: User studies showed superior video quality and comfort over conventional methods, with preferences for specific synthesis options.

Conclusion: The automated alignment and view-selection method enhances surgical video quality and usability, validated by surgeon feedback.
...
TL;DR: Deep learning models (YOLOv8s for detection, MobileNetV3L for classification) were used to automate pollen grain detection and classification in optical and holographic microscopy images. Performance improvements were achieved through dataset expansion and labeling techniques.


<details>
  <summary>Details</summary>
Motivation: The study aims to enhance and automate pollen grain detection and classification, particularly for veterinary cytology, using deep learning. It also explores the feasibility of pairing these techniques with cost-effective holographic microscopy.

Method: YOLOv8s was employed for object detection, and MobileNetV3L for classification. Performance was evaluated on optical and holographic images. Dataset expansion (automated labeling, bounding box enlargement) was used to address performance gaps in holographic images.

Result: On optical images, the models achieved 91.3% mAP50 for detection and 97% accuracy for classification. Holographic image performance improved from 2.49% to 13.3% mAP50 (detection) and 42% to 54% (classification) after dataset enhancements.

Conclusion: Deep learning can effectively automate pollen grain detection and classification, even with cost-effective holographic microscopy, though performance in holographic images initially lags behind optical images and requires dataset adjustments.
...
TL;DR: The study identifies bias in CLIP's few-shot learning due to template-sample similarity (TSS) and introduces a framework using empty prompts to mitigate this bias, improving accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: CLIP's reliance on TSS introduces bias, leading to reduced accuracy and robustness in classification, prompting the need for correction.

Method: A two-stage framework uses empty prompts during pre-training to reduce bias and employs bias calibration loss during fine-tuning for accurate alignment.

Result: Experiments show the method reduces TSS-induced fluctuations, enhancing classification accuracy and robustness across benchmarks.

Conclusion: The framework effectively addresses TSS bias in CLIP, improving its few-shot learning performance.
...
TL;DR: OpenMonoGS-SLAM integrates monocular SLAM with open-set semantic understanding using Visual Foundation Models, achieving strong performance without depth sensors or semantic annotations.


<details>
  <summary>Details</summary>
Motivation: Combining SLAM with semantic understanding is crucial for intelligent perception, but existing methods rely on depth sensors or closed-set models, limiting scalability.

Method: Leverages Visual Foundation Models (MASt3R, SAM, CLIP) for geometry and semantics, uses self-supervised learning, and introduces a memory mechanism for semantic feature management.

Result: Achieves comparable or superior performance in segmentation tasks without depth input or semantic annotations.

Conclusion: OpenMonoGS-SLAM demonstrates robust monocular SLAM with open-set semantics, offering scalability and adaptability in diverse environments.
...
TL;DR: The paper introduces a method to estimate metric depth by analyzing blur patterns in videos, combining optical design algorithms with vision-language models for dense trajectory reconstruction, demonstrating strong performance and accuracy.


<details>
  <summary>Details</summary>
Motivation: Rotational dynamics during camera capture cause depth-dependent blur, which varies with object positions. This observation inspires leveraging blur patterns for depth estimation.

Method: Uses off-the-shelf vision encoders and point trackers to analyze video blur patterns. Depth maps are estimated via windowed embedding and multi-window aggregation, with sparse trajectories densified using a vision-language model.

Result: Evaluations show strong performance across large depth ranges and generalization. The method achieves high precision in trajectory reconstruction and maintains accuracy in handheld settings.

Conclusion: The proposed method effectively estimates depth and reconstructs trajectories using blur patterns, offering robust performance and generalization.
...
TL;DR: The paper introduces a unified aerial VLN framework using only monocular RGB and language instructions, optimizing navigation through multi-task learning and addressing challenges like visual redundancy and supervision imbalance.


<details>
  <summary>Details</summary>
Motivation: Existing aerial VLN methods often require costly inputs like panoramic images or depth data, limiting practical use for lightweight UAVs. The paper aims to enable navigation with simpler, monocular RGB inputs.

Method: The model treats navigation as next-token prediction, combining spatial perception, trajectory reasoning, and action prediction via prompt-guided multi-task learning. It includes keyframe selection to reduce redundancy and action merging with label reweighting to address supervision imbalance.

Result: The framework achieves strong performance on the Aerial VLN benchmark, outperforming RGB-only baselines and narrowing the gap with RGB-D methods.

Conclusion: The proposed approach demonstrates feasibility for practical aerial VLN using minimal inputs, validated by benchmark results and ablation studies.
...
TL;DR: The paper introduces Chain-of-Image Generation (CoIG), a framework that makes image generation sequential and interpretable, similar to human art creation, improving monitorability and robustness.


<details>
  <summary>Details</summary>
Motivation: Current image generation models lack transparency, making it hard for humans to monitor or intervene. CoIG aims to address this by mimicking human-like workflows.

Method: CoIG uses an LLM to break prompts into step-by-step instructions for image generation, focusing on one semantic entity per step. Metrics like CoIG Readability and Causal Relevance assess clarity and impact.

Result: CoIG improves monitorability and reduces entity collapse while maintaining competitive performance with baseline models. It is model-agnostic and adaptable.

Conclusion: CoIG successfully enhances transparency and control in image generation, offering a human-like, interpretable workflow without compromising quality.
...
TL;DR: C-DIRA is a lightweight framework for driver behavior recognition that combines dynamic ROI routing and adversarial learning to balance efficiency and accuracy, outperforming prior models in compactness and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight models struggle to capture fine-grained behavioral cues and handle domain variations, while ROI-based methods increase computational costs. C-DIRA aims to address these gaps.

Method: C-DIRA integrates saliency-driven Top-K ROI pooling, dynamic ROI routing (applied to high-difficulty samples), and adversarial learning with pseudo-domain labeling for domain-invariant features.

Result: On the State Farm dataset, C-DIRA achieves high accuracy with fewer FLOPs and lower latency, showing robustness to visual degradation and domain shifts.

Conclusion: C-DIRA successfully balances efficiency, accuracy, and generalization, making it suitable for real-time edge-device deployment.
...
TL;DR: The paper proposes \mname, a self-contained training framework for generative models that eliminates reliance on external encoders, reduces computational costs, and improves convergence by using a memory bank mechanism for contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing generative models face high training costs and inefficiencies in representation learning. Auxiliary alignment methods introduce overhead and domain shift due to reliance on external encoders.

Method: The authors introduce \mname, which uses a memory bank mechanism to provide abundant negative samples dynamically. It avoids external encoders and minimizes overhead with a low-dimensional projection head.

Result: \mname achieves a state-of-the-art FID of 2.40 on ImageNet-256 within 400k steps, outperforming comparable methods with faster convergence.

Conclusion: The proposed method offers a self-contained, efficient solution for generative modeling, reducing dependencies and computational costs while improving performance.
...
TL;DR: The paper proposes a distribution-based framework to address individual fairness in skin lesion classification by treating skin tone as continuous, using KDE and statistical distance metrics, and introduces DRW loss to improve fairness.


<details>
  <summary>Details</summary>
Motivation: Current fairness research in medical imaging often uses coarse subgroup categories, which may hide biases faced by individuals within subgroups.

Method: Skin tone is modeled as a continuous attribute using KDE. Twelve distance metrics quantify disparities, and DRW loss corrects underrepresentation.

Result: Experiments show distribution-based reweighting outperforms categorical methods, with FS, WD, HM, and HS metrics being particularly effective.

Conclusion: The study provides a robust method for individual fairness in dermatological AI and highlights implications for continuous attributes.
...
TL;DR: MoSAIC-ReID is a Mixture-of-Experts framework that quantifies pedestrian attribute importance for re-identification, offering interpretability and competitive performance.


<details>
  <summary>Details</summary>
Motivation: To address the opacity of state-of-the-art person re-identification models by systematically analyzing which semantic attributes they rely on.

Method: Uses LoRA-based experts linked to single attributes and an oracle router for controlled attribution analysis, combined with statistical tests and feature-importance analyses.

Result: Achieves competitive performance on Market-1501 and DukeMTMC while revealing strong contributors (e.g., clothing colors) and limited-impact cues (e.g., accessories).

Conclusion: Provides a principled framework for interpretable ReID and highlights the need for integrating explicit semantic knowledge.
...
TL;DR: Self-refining diffusion enhances image quality by detecting flaws using XAI-based flaw activation maps, improving reconstruction and performance across tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing artifacts and unrealistic regions in diffusion models remains a challenge, prompting the need for a method to refine image generation.

Method: The framework uses XAI-based flaw highlighters to create flaw activation maps (FAMs), which guide noise amplification and targeted reconstruction.

Result: Achieves up to a 27.3% improvement in Fréchet inception distance, with strong performance across diverse datasets and tasks.

Conclusion: Explainable AI techniques can actively improve image refinement, making the framework versatile and impactful for diffusion models.
...
TL;DR: The paper explores applying SAM 3 to remote sensing Open-Vocabulary Semantic Segmentation without training, using mask fusion and presence scores to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based methods struggle with precise localization and complex pipelines in remote sensing, prompting the use of SAM 3's unified framework.

Method: A mask fusion strategy combines SAM 3's semantic segmentation and instance heads, while presence scores filter irrelevant categories.

Result: Experiments on remote sensing datasets show promising performance, highlighting SAM 3's potential for OVSS.

Conclusion: SAM 3's adaptation proves effective for remote sensing OVSS, offering simplicity and improved accuracy.
...
### [88] [MatteViT: High-Frequency-Aware Document Shadow Removal with Shadow Matte Guidance](https://arxiv.org/abs/2512.08789)
*Chaewon Kim,Seoyeon Lee,Jonghyuk Park*

Main category: cs.CV

TL;DR: The paper proposes MatteViT, a shadow removal framework using spatial and frequency-domain information to eliminate shadows while preserving document details. It includes a high-frequency amplification module and luminance-based shadow matte, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Document shadows obscure fine structures like text edges, reducing clarity. Removing shadows while preserving details is crucial for tasks like OCR.

Method: MatteViT employs a high-frequency amplification module (HFAM) and a luminance-based shadow matte. HFAM decomposes and amplifies high-frequency components, while the matte provides spatial guidance.

Result: MatteViT outperforms prior methods on benchmarks (RDD and Kligler), preserving details and improving OCR performance.

Conclusion: MatteViT offers a robust solution for document shadow removal, enhancing clarity and downstream task performance.
...
TL;DR: A novel workflow combining 3D rendering and a constrained diffusion model generates photorealistic synthetic mushroom images for training computer vision models, achieving state-of-the-art segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Industrial mushroom cultivation needs accurate computer vision models, but creating large annotated datasets is expensive. Synthetic data lacks realism, so this paper addresses the gap.

Method: Integrates 3D rendering (Blender) with a constrained diffusion model to produce high-quality synthetic images of Agaricus Bisporus mushrooms, ensuring photorealism and annotation control.

Result: Trained Mask R-CNN models on synthetic datasets outperform others in zero-shot testing (F1 = 0.859 on M18K), validated on real-world benchmarks.

Conclusion: The pipeline is adaptable to other mushroom species or agricultural tasks, offering a scalable solution for synthetic data generation.
...
TL;DR: A skewness-guided pruning method for multimodal Swin Transformer reduces model size by 36% without accuracy loss, enabling efficient edge deployment and privacy-preserving federated learning in medical imaging.


<details>
  <summary>Details</summary>
Motivation: High-performance computer vision models are computationally intensive and large, unsuitable for edge devices, while privacy constraints favor federated learning.

Method: Proposes skewness-guided pruning of Multi-Head Self-Attention and Multi-Layer Perceptron layers in a multimodal Swin Transformer, validated in a federated learning environment.

Result: Achieves ~36% model size reduction with maintained accuracy on a compact Swin Transformer.

Conclusion: Demonstrates feasible model compression and privacy-preserving distributed learning for medical AI on edge devices.
...
TL;DR: The abstract introduces T-DHA, a training-free method for adapting Vision-Language Models (VLMs) using hyperbolic spaces for better hierarchical data representation and improved performance.


<details>
  <summary>Details</summary>
Motivation: Addresses performance degradation in VLMs due to domain changes and high computational costs of fine-tuning.

Method: Uses hyperbolic spaces (Poincaré ball model) to embed hierarchical vision-language relationships and incorporates negative learning.

Result: T-DHA outperforms state-of-the-art methods in few-shot recognition and domain generalization.

Conclusion: Hyperbolic space embedding enhances VLM adaptability and efficiency.
...
TL;DR: Wan-Move is a scalable framework for precise motion control in video generation, using dense trajectories and latent space projection to enhance quality without modifying existing models.


<details>
  <summary>Details</summary>
Motivation: Existing motion-controllable video generation methods lack fine granularity and scalability, limiting practical application.

Method: Represent motions with dense point trajectories, project into latent space, and propagate frame features to create aligned spatiotemporal feature maps for motion guidance.

Result: Generates high-quality 5-second, 480p videos with motion controllability comparable to commercial tools like Kling 1.5 Pro.

Conclusion: Wan-Move achieves superior motion quality and scalability, validated by MoveBench and public datasets, with publicly available code and models.
...
TL;DR: InfiniteVL is a linear-complexity Vision-Language Model combining sliding window attention and Gated DeltaNet, outperforming other models with less data and faster inference.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of window-based and linear attention in VLMs, particularly performance degradation and underperformance on information-intensive tasks.

Method: Proposes InfiniteVL, integrating sliding window attention with Gated DeltaNet, and a three-stage training strategy (distillation pretraining, instruction tuning, long-sequence SFT).

Result: InfiniteVL matches leading Transformer-based VLMs with less training data, achieves 3.6x inference speedup, and maintains stable performance in long-sequence tasks.

Conclusion: InfiniteVL offers an efficient, high-performance alternative to traditional VLMs, with practical benefits in speed and resource usage.
...
TL;DR: LoFA is a framework that efficiently predicts personalized priors for fast model adaptation by leveraging structured distribution patterns in LoRA, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Current methods like LoRA are impractical due to their need for task-specific data and lengthy optimization. Hypernetwork-based approaches struggle with fine-grained user prompts, limiting practicality.

Method: LoFA uses a two-stage hypernetwork: first predicting relative distribution patterns to identify key adaptation regions, then guiding final LoRA weight prediction.

Result: The method predicts high-quality personalized priors within seconds across multiple tasks and user prompts, outperforming conventional LoRA.

Conclusion: LoFA offers a practical and efficient solution for personalized visual generative model adaptation, bridging the gap left by existing methods.
...
### [95] [Siamese-Driven Optimization for Low-Resolution Image Latent Embedding in Image Captioning](https://arxiv.org/abs/2512.08873)
*Jing Jie Tan,Anissa Mokraoui,Ban-Hoe Kwan,Danny Wee-Kiat Ng,Yan-Chai Hum*

Main category: cs.CV

TL;DR: SOLI is a lightweight Siamese network approach for efficient and accurate low-resolution image captioning, minimizing computational overhead.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of low-resolution image captioning with lightweight solutions, especially for resource-constrained scenarios.

Method: Uses a Siamese network architecture to optimize latent embeddings in a dual-pathway neural network structure.

Result: Enhances efficiency and accuracy in image-to-text translation for low-resolution images.

Conclusion: SOLI is effective for lightweight, low-resolution image captioning, balancing performance and computational demands.
...
### [96] [Generation is Required for Data-Efficient Perception](https://arxiv.org/abs/2512.08854)
*Jack Brady,Bernhard Schölkopf,Thomas Kipf,Simon Buchholz,Wieland Brendel*

Main category: cs.CV

TL;DR: This paper explores whether generative or non-generative methods are better suited for achieving compositional generalization in visual perception, arguing that generative methods enforce necessary inductive biases more effectively.


<details>
  <summary>Details</summary>
Motivation: The study aims to resolve the debate on whether generative approaches (using decoder inversion) are essential for human-level visual perception by comparing them with non-generative methods.

Method: The authors formalize inductive biases needed for compositional generalization in both decoder-based (generative) and encoder-based (non-generative) methods, theoretically analyze their enforceability, and empirically test them on photorealistic image datasets.

Result: Generative methods consistently outperform non-generative ones in compositional generalization, especially when inductive biases are enforced through decoder constraints and inversion techniques like gradient-based search or generative replay.

Conclusion: Generative methods are more effective for compositional generalization due to their ability to enforce necessary inductive biases, while non-generative methods often fail without additional supervision or large-scale pretraining.
...
TL;DR: A new annotation-free training framework improves visual reasoning by combining LLM and VLM verifiers, outperforming existing methods without requiring ground truth labels.


<details>
  <summary>Details</summary>
Motivation: Existing visual reasoning methods either need large-scale supervision or suffer from flaws in logic and grounding. The goal is to improve both reasoning and grounding without annotations.

Method: The framework uses AI-powered verifiers: an LLM verifier refines reasoning via reinforcement learning, and a VLM verifier enhances grounding through automated hard-negative mining.

Result: The approach outperforms open-source and proprietary models in visual reasoning tasks and surpasses recent text-only methods with improved grounding.

Conclusion: The proposed framework effectively combines language-based reasoning and vision-based grounding, achieving superior performance without annotation dependency.
...
TL;DR: Tri-Bench is a benchmark for testing VLMs' geometric reasoning. It shows VLMs struggle with certain triangle types and camera tilt, relying more on 2D cues than explicit hints.


<details>
  <summary>Details</summary>
Motivation: To assess VLMs' verifiable geometric reasoning under practical challenges like camera pose and scene context.

Method: Tri-Bench tests four VLMs using a fixed prompt with a homography-based guardrail, evaluating six tasks with binary and continuous targets.

Result: VLMs averaged ~69% accuracy, struggled with minority triangle classes (~0%), and degraded ~4.1% under camera tilt. Object interference had no impact.

Conclusion: VLMs default to 2D cues, failing to utilize explicit hints, highlighting a need for improved geometric reasoning in agentic AI.
...
### [99] [SATGround: A Spatially-Aware Approach for Visual Grounding in Remote Sensing](https://arxiv.org/abs/2512.08881)
*Aysim Toker,Andreea-Maria Oncescu,Roy Miles,Ismail Elezi,Jiankang Deng*

Main category: cs.CV

TL;DR: The paper proposes a novel structured localization mechanism to enhance vision-language models (VLMs) for precise object localization in satellite imagery, outperforming previous methods by 24.8%.


<details>
  <summary>Details</summary>
Motivation: To improve VLM-based visual grounding in satellite imagery by integrating structured spatial reasoning, enabling more reliable analysis of satellite data.

Method: Finetune a pretrained VLM on diverse instruction-following tasks and interface a dedicated grounding module via specialized control tokens for localization.

Result: Achieves state-of-the-art performance on remote sensing benchmarks, with a 24.8% relative improvement over previous methods in visual grounding.

Conclusion: Integrating structured spatial reasoning into VLMs significantly enhances localization accuracy, advancing real-world satellite data analysis.
...
TL;DR: The paper introduces Astra, an interactive general world model for long-horizon future prediction in diverse scenarios like autonomous driving and robot grasping, using autoregressive denoising and action-aware techniques.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored area of world models predicting long-horizon futures from past observations and actions, especially in general-purpose scenarios and for various action forms.

Method: Proposes Astra with an autoregressive denoising architecture, temporal causal attention, noise-augmented history memory, action-aware adapter, and a mixture of action experts for versatile task handling.

Result: Astra achieves interactive, consistent, and general long-term video prediction with improvements in fidelity, long-range prediction, and action alignment over existing models.

Conclusion: Astra bridges the gap in general-purpose world models, demonstrating superior performance in diverse real-world tasks and supporting various interaction forms.
...
### [101] [Accelerated Rotation-Invariant Convolution for UAV Image Segmentation](https://arxiv.org/abs/2512.08888)
*Manduhu Manduhu,Alexander Dow,Gerard Dooly,James Riordan*

Main category: cs.CV

TL;DR: The paper introduces a GPU-optimized rotation-invariant convolution framework for UAV aerial imagery segmentation, reducing computational costs and memory traffic while maintaining high accuracy.


<details>
  <summary>Details</summary>
Motivation: Rotation invariance is crucial for accurate object-level segmentation in aerial imagery, but traditional convolution methods lack this property, leading to degraded performance.

Method: The proposed framework eliminates the im2col step and leverages structured data sharing among rotated filters, enabling efficient multi-orientation convolution.

Result: The method achieves 20--55% faster training, 15--45% lower energy consumption, and up to 6% accuracy improvement over non-rotation-aware baselines.

Conclusion: The framework is an efficient and effective alternative to existing rotation-invariant CNN methods, balancing performance and computational efficiency.
...
TL;DR: UniLayDiff is a Unified Diffusion Transformer model designed to handle diverse content-aware layout generation tasks with a single, end-to-end trainable approach.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of unifying diverse input-constrained layout generation tasks, which current methods fail to solve comprehensively.

Method: Utilizes a Multi-Modal Diffusion Transformer framework, treats layout constraints as a distinct modality, and integrates relation constraints via LoRA fine-tuning.

Result: Achieves state-of-the-art performance across unconditional and conditional generation tasks.

Conclusion: UniLayDiff is the first model to unify the full range of content-aware layout generation tasks.
...
TL;DR: EvoScene is a training-free framework that iteratively reconstructs 3D scenes from single images by combining geometric reasoning and visual knowledge, outperforming baselines in stability and texture consistency.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of generating high-quality, textured 3D scenes from single images, particularly for complex, large-scale scenes.

Method: Three iterative stages: Spatial Prior Initialization, Visual-guided 3D Scene Mesh Generation, and Spatial-guided Novel View Generation, alternating between 2D and 3D domains.

Result: Superior geometric stability, view-consistent textures, and completion of unseen regions compared to baselines.

Conclusion: EvoScene effectively produces ready-to-use 3D meshes for practical applications.
...
TL;DR: LiDAS is a closed-loop active illumination system that optimizes lighting for better nighttime camera perception, improving accuracy and energy efficiency without retraining models.


<details>
  <summary>Details</summary>
Motivation: Nighttime environments challenge camera-based perception due to reliance on passive scene lighting. LiDAS aims to enhance perception by dynamically optimizing illumination.

Method: Combines off-the-shelf perception models with high-definition headlights to predict and control an optimal illumination field, redistributing light to improve object detection.

Result: Achieves +18.7% mAP50 and +5.0% mIoU over standard low-beam at equal power, reduces energy use by 40%, and enables zero-shot nighttime generalization of daytime-trained models.

Conclusion: LiDAS provides a cost-effective, energy-efficient solution for robust nighttime perception by transforming headlights into active vision actuators.
...
TL;DR: D4RT is a lightweight, scalable transformer model that efficiently reconstructs dynamic scenes by jointly inferring depth, spatio-temporal correspondence, and camera parameters from a single video, outperforming previous methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of reconstructing complex geometry and motion in dynamic scenes from video drove the development of D4RT.

Method: D4RT uses a unified transformer architecture with a novel querying mechanism to avoid dense per-frame decoding and multiple task-specific decoders, enabling flexible probing of 3D positions.

Result: D4RT sets a new state of the art in 4D reconstruction tasks, achieving efficient training and inference.

Conclusion: The paper demonstrates D4RT's effectiveness in dynamic scene reconstruction, offering a scalable and lightweight solution.
...
TL;DR: Selfi improves 3D reconstruction by aligning VGGT features geometrically, enhancing both novel view synthesis and pose estimation tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional NVS relies on explicit 3D biases and known camera parameters, while VGGT lacks multi-view consistency. Feature alignment can bridge this gap.

Method: Selfi introduces a feature adapter trained with reprojection-based consistency loss, transforming VGGT outputs into geometrically-aligned features.

Result: Achieves state-of-the-art performance in NVS and camera pose estimation.

Conclusion: Feature alignment significantly enhances downstream 3D reasoning tasks.
...
TL;DR: The study explores the short-context dominance hypothesis, finding that most sequences require only a small local prefix for accurate predictions. It introduces DaMCL to detect long-context sequences and proposes a decoding algorithm to mitigate bias.


<details>
  <summary>Details</summary>
Motivation: Understand how much context is needed for accurate predictions in sequences and address the bias in LLM outputs due to short-context dominance.

Method: Measure Minimum Context Length (MCL) and introduce Distributionally Aware MCL (DaMCL) for detecting long-context sequences without next-token knowledge. Develop a decoding algorithm to boost long-range-relevant tokens.

Result: 75-80% of sequences require ≤96 tokens. DaMCL effectively detects long-context sequences, and the decoding algorithm improves performance.

Conclusion: Short-context dominance is prevalent, but detecting and addressing long-context sequences enhances LLM performance.
...
### [108] [Curriculum Guided Massive Multi Agent System Solving For Robust Long Horizon Tasks](https://arxiv.org/abs/2512.08545)
*Indrajit Kar,Kalathur Chenchu Kishore Kumar*

Main category: cs.CL

TL;DR: A hierarchical multi-agent architecture with a selective oracle and spatial curriculum improves long-horizon reasoning tasks by distributing tasks across lightweight agents and ensuring gradual mastery of tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of Large Language Models and multi-agent systems in handling long-horizon reasoning tasks and high computation costs.

Method: Introduces a 64*64 grid of lightweight agents with a selective oracle, spatial curriculum, and Negative Log-Likelihood for confidence measurement, managed adaptively by Thompson Sampling.

Result: Improved stability, reduced oracle usage, and enhanced long-range reasoning on a spatially grounded Tower of Hanoi benchmark.

Conclusion: The hierarchical multi-agent architecture effectively tackles complex tasks by leveraging distributed reasoning and adaptive training strategies.
...
TL;DR: This paper introduces a scalable pipeline for training specialized retrieval models from unlabeled data, improving performance in financial domains without costly human annotation.


<details>
  <summary>Details</summary>
Motivation: Specialized conversational AI agents face challenges like high computation costs and poor domain-specific retrieval performance, especially in finance. Existing embedding models fall short in specialized domains.

Method: The pipeline uses a general-purpose retrieval embedding model as foundation, iteratively refining a student model by mining hard examples from an unlabeled corpus. It avoids human annotation by leveraging LLM-judged relevance and interleaves retrieval-based mining with retraining.

Result: The method shows significant improvements: 27.7% MRR@5, 44.6% mean DCG@5 across 14 financial filing types, and better NDCG on 3/4 FinanceBench classes.

Conclusion: The proposed pipeline effectively bridges the gap between general-purpose and specialized retrieval models, offering a cost-effective alternative to human annotation.
...
TL;DR: SEA is a universal framework for aligning subtitles to sign language videos, using pretrained models for segmentation and embedding, followed by efficient dynamic programming for alignment.


<details>
  <summary>Details</summary>
Motivation: Previous methods for aligning subtitles to sign language videos are limited by language or dataset specificity; SEA aims to provide a more general solution.

Method: SEA uses two pretrained models: one segments video frames into signs, and the other embeds video clips and text into a shared latent space, with alignment via dynamic programming.

Result: Experiments on four datasets show SEA achieves state-of-the-art alignment performance, making it adaptable across languages and domains.

Conclusion: SEA offers a flexible, efficient, and general solution for subtitle alignment in sign language videos, potentially advancing sign language processing.
...
TL;DR: The paper introduces universal adversarial suffixes—short token sequences that reduce accuracy across tasks and models when appended to inputs. It uses Gumbel-Softmax relaxation for training and shows transferability across models and tasks.


<details>
  <summary>Details</summary>
Motivation: Current adversarial prompts are task- or model-specific, hindering comparison and transferability. The goal is to develop universal suffixes that broadly reduce accuracy.

Method: Adversarial suffixes are learned in a differentiable form using Gumbel-Softmax relaxation, then discretized. Training maximizes calibrated cross-entropy while masking gold tokens and using entropy regularization.

Result: A single suffix trained on one model transfers effectively to others, lowering accuracy and confidence across sentiment analysis, natural language inference, and more.

Conclusion: Universal adversarial suffixes are effective and transferable across tasks and model families, demonstrating broad applicability.
...
TL;DR: Language models are vulnerable to adversarial suffixes found via reinforcement learning, which outperforms gradient/rule-based methods in degrading accuracy and transferring across tasks/models.


<details>
  <summary>Details</summary>
Motivation: Existing methods for adversarial suffixes are brittle and task/model-specific; reinforcement learning offers a more robust and transferable solution.

Method: Uses RL (Proximal Policy Optimization) to train suffixes against frozen models, with rewards shaped by calibrated cross-entropy for bias-free, transferable triggers.

Result: RL-trained suffixes degrade accuracy consistently and transfer better across five NLP benchmarks and three language models compared to prior methods.

Conclusion: Reinforcement learning provides a superior approach for crafting adversarial suffixes that are effective and transferable across diverse tasks and models.
...
TL;DR: ClinicalTrialsHub is a search-focused platform integrating ClinicalTrials.gov and PubMed data, enhancing accessibility to clinical trial information by 83.8%.


<details>
  <summary>Details</summary>
Motivation: To improve access to structured clinical trial data for patients, clinicians, researchers, and policymakers, advancing evidence-based medicine.

Method: Utilizes large language models (GPT-5.1, Gemini-3-Pro) to extract structured trial data from PubMed, translates queries into database searches, and offers an attributed Q&A system.

Result: Increased access to structured trial data by 83.8%; validated through user studies with clinicians, researchers, and systematic evaluations.

Conclusion: ClinicalTrialsHub significantly enhances accessibility and utility of clinical trial data, benefiting multiple stakeholders in healthcare.
...
TL;DR: This study examines bias in GLLM annotations by replicating Boukes (2024)'s manual annotations, revealing biases despite adequate F1 scores.


<details>
  <summary>Details</summary>
Motivation: To assess the reliability and bias of GLLMs compared to manual annotations in labeling concepts like political content and incivility.

Method: Conceptually replicates Boukes (2024) using five GLLMs with five prompts each for five concepts.

Result: GLLMs show adequate F1 scores but differ significantly from manual annotations in prevalence and bias, overlapping more with each other than with human annotations.

Conclusion: F1 scores alone don't capture GLLM bias; differences in annotations lead to divergent downstream results.
...
TL;DR: The paper explores gender bias in machine translation by analyzing how source sentence context triggers gender choices, using contrastive explanations and saliency attribution to compare model and human perceptions.


<details>
  <summary>Details</summary>
Motivation: To move beyond measuring gender bias in models like MT and LLMs and instead investigate its origins by examining context triggers in source sentences.

Method: Uses gender-ambiguous natural source data, contrastive explanations, and saliency attribution to analyze source word influence on gender inflection choices in translation.

Result: Finds overlap between human perceptions of gender and model attributions, with linguistic analysis of salient words revealing insights into bias triggers.

Conclusion: Understanding model translation decisions and comparing them to human choices can help mitigate gender bias in machine translation.
...
TL;DR: A study proposes a soft inductive bias approach to improve detection of inappropriate utterances in Korean online communications, achieving higher accuracy than standard methods.


<details>
  <summary>Details</summary>
Motivation: The rise of verbal abuse and criminal behavior in anonymous online spaces highlights the need for effective detection techniques to ensure safer communication.

Method: The study introduces a soft inductive bias approach to guide reasoning perspectives, fine-tunes a Korean large language model (Kanana-1.5), and compares performance across training strategies.

Result: Kanana-1.5 achieves an average accuracy of 87.0046, improving by ~3.89% over standard supervised learning, showing better precision and consistency.

Conclusion: The proposed method enhances inappropriate utterance detection by promoting rational decision-making and avoiding reasoning errors, proving its effectiveness.
...
TL;DR: The tutorial provides an introductory overview of HealthcareNLP, focusing on data/resource, NLP-Eval, and patient layers, covering tasks like synthetic data generation and explainable AI, with hands-on sessions.


<details>
  <summary>Details</summary>
Motivation: Existing reviews overlook key tasks and methodologies in HealthcareNLP, prompting this tutorial to fill gaps and educate practitioners.

Method: The tutorial organizes content hierarchically into three layers: data/resource, NLP-Eval, and patients, with practical hands-on sessions.

Result: Participants gain foundational knowledge of HealthcareNLP, including ethical considerations and practical applications.

Conclusion: The tutorial aims to bridge gaps in HealthcareNLP education, making complex topics accessible without prior knowledge.
...
TL;DR: QSTN is an open-source Python framework for generating responses from questionnaire-style prompts using LLMs. It evaluates questionnaire presentation and response methods efficiently.


<details>
  <summary>Details</summary>
Motivation: To support reproducible and reliable LLM-based research by enabling systematic evaluation of questionnaire prompts and responses.

Method: QSTN provides a framework for generating responses from questionnaires, evaluating prompt perturbations, and offering a no-code UI for researchers.

Result: Evaluated over 40 million responses, showing that question structure and generation methods significantly impact alignment with human answers at low compute cost.

Conclusion: QSTN enhances LLM-based research reproducibility and reliability, offering accessible tools for robust experimentation.
...
TL;DR: MOSAIC is a Multi-framework Structured Agentic AI system for clinical communication, achieving high accuracy (F1=0.928) against human annotations.


<details>
  <summary>Details</summary>
Motivation: Clinical communication annotation is labor-intensive and inconsistent; existing AI models lack adaptability and reliability.

Method: MOSAIC uses a LangGraph-based architecture with four agents for planning, updating, annotating, and verification.

Result: Achieved F1 score of 0.928, with highest performance in Rheumatology (F1=0.962) and Patient Behavior tasks.

Conclusion: MOSAIC outperforms baseline benchmarks, offering scalable, reliable clinical communication annotation.
...
TL;DR: This paper presents a Basque AES dataset and benchmarks fine-tuned models for scoring and feedback, outperforming SoTA systems.


<details>
  <summary>Details</summary>
Motivation: To provide a transparent, reproducible foundation for AES and feedback generation in low-resource languages like Basque.

Method: Fine-tune RoBERTa-EusCrawl and Latxa models for AES and feedback, evaluate using automatic and expert-based metrics.

Result: Fine-tuned Latxa surpasses GPT-5 and Claude Sonnet 4.5 in scoring consistency and feedback quality.

Conclusion: The dataset and models advance NLP research for Basque and similar low-resource languages.
...
TL;DR: A post-training method for lower-resource languages improves fluency in preference-aligned language models without relying on hard-to-obtain data, demonstrated through Norwegian Bokmål.


<details>
  <summary>Details</summary>
Motivation: Lower-resource languages lack native-speaker datasets and fluent synthetic data, making preference-alignment challenging compared to English and Chinese.

Method: Uses on-policy training, comparing it with supervised finetuning on machine-translated data and multilingual finetuning.

Result: On-policy training outperforms alternatives, preserving fluency without needing instruction-tuning data in the target language.

Conclusion: The approach is effective for lower-resource languages, highlighting the importance of on-policy training.
...
TL;DR: The paper proposes an adaptive aggregation method for aligning LLMs with diverse human preferences in federated learning, outperforming standard techniques in fairness while maintaining alignment quality.


<details>
  <summary>Details</summary>
Motivation: Standard methods for aligning LLMs struggle to represent diverse viewpoints in federated learning environments, necessitating a fair and effective solution.

Method: An adaptive aggregation scheme dynamically adjusts preference weights based on historical alignment performance, evaluated alongside standard techniques (min, max, average).

Result: Experiments on Q/A tasks show the adaptive method achieves superior fairness and competitive alignment compared to standard techniques.

Conclusion: The work provides a robust framework for evaluating and aligning LLMs fairly across diverse populations, advancing pluralistic model development.
...
TL;DR: Gradual depth growth in Transformers improves training efficiency and reasoning performance by optimizing depth utilization and forming distinct computational circuits.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand why gradually increasing Transformer depth during training enhances performance, addressing gaps in mechanistic explanations and the Curse of Depth issue.

Method: Depth-wise analyses are used to explore gradual middle stacking, residual stream changes, and permutable computational blocks. A modified MIDAS approach is also proposed.

Result: Gradual depth growth leads to better depth utilization, altered residual streams, and improved downstream reasoning benchmarks.

Conclusion: Gradual depth growth overcomes depth utilization limitations in non-grown models, forming unique computational circuits for enhanced performance.
...
TL;DR: The paper introduces RAGLens, a lightweight hallucination detector for RAG outputs using LLM internal representations, achieving superior performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Addressing faithfulness failures in RAG outputs where generations contradict or extend beyond retrieved evidence, overcoming limitations of existing methods.

Method: Utilizes sparse autoencoders (SAEs) to disentangle internal activations and identifies hallucination-triggered features. Introduces RAGLens via feature selection and additive modeling.

Result: RAGLens outperforms existing methods in hallucination detection and provides interpretable rationales for mitigation.

Conclusion: RAGLens offers an effective solution for identifying and mitigating unfaithful RAG outputs while revealing insights into LLM hallucination signals.
...
### [125] [CrowdLLM: Building LLM-Based Digital Populations Augmented with Generative Models](https://arxiv.org/abs/2512.07890)
*Ryan Feng Lin,Keyu Tian,Hanming Zheng,Congjing Zhang,Li Zeng,Shuai Huang*

Main category: cs.MA

TL;DR: CrowdLLM integrates LLMs and generative models to improve diversity and fidelity in digital populations, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) alone lack accuracy and diversity for digital populations, prompting the need for enhanced solutions.

Method: The proposed CrowdLLM combines pretrained LLMs with generative models.

Result: CrowdLLM shows cost-effective, scalable performance matching real human data in accuracy and distributional fidelity.

Conclusion: CrowdLLM effectively addresses limitations of LLM-based digital populations, offering high-quality simulation across domains.
...
### [126] [MARINE: Theoretical Optimization and Design for Multi-Agent Recursive IN-context Enhancement](https://arxiv.org/abs/2512.07898)
*Hongwei Zhang,Ji Lu,Yongsheng Du,Yanqin Gao,Lingjun Huang,Baoli Wang,Fang Tan,Peng Zou*

Main category: cs.MA

TL;DR: MARINE is a multi-agent framework that enhances LLM reasoning by iterative refinement, achieving near-optimal performance with fewer parameters and computational resources.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of single-response outputs in LLM-based agents and unlock their full performance potential.

Method: Introduces MARINE, a framework using iterative refinement of a reference trajectory to convert pass@N capabilities to pass@1. Optimizes batch schedules for efficiency.

Result: Achieves 46.0% pass@1 accuracy on BrowserComp-ZH with a 685B model; an 80B model with MARINE matches standalone 1000B performance.

Conclusion: MARINE offers a parameter-efficient, computationally effective paradigm for enhancing LLM reasoning and post-training efficiency.
...
### [127] [Probabilistic Multi-Agent Aircraft Landing Time Prediction](https://arxiv.org/abs/2512.08281)
*Kyungmin Kim,Seokbin Yoon,Keumjin Lee*

Main category: cs.MA

TL;DR: A probabilistic multi-agent framework predicts aircraft landing times with uncertainty estimates, improving accuracy and explainability.


<details>
  <summary>Details</summary>
Motivation: Addressing the uncertainty in aircraft trajectories and multi-agent interactions to enhance landing time prediction accuracy and trustworthiness.

Method: Proposes a probabilistic multi-agent aircraft landing time prediction framework that outputs distributions for landing times.

Result: Outperforms baselines in accuracy, quantifies uncertainties, and reveals air traffic control patterns via attention scores.

Conclusion: The framework enhances prediction accuracy, uncertainty quantification, and explainability for air traffic management.
...
TL;DR: The study uses LLM-based Multi-Agent Systems to simulate toxic workplace debates, finding a 25% increase in conversation duration as a proxy for operational inefficiency.


<details>
  <summary>Details</summary>
Motivation: To quantify the impact of workplace toxicity on operational efficiency ethically and reproducibly.

Method: Simulates 1-on-1 adversarial debates using LLM agents with Monte Carlo methods, comparing baseline and toxic-prompted groups.

Result: Toxic participants increased conversation duration by ~25%, suggesting operational inefficiency.

Conclusion: Agent-based modeling offers an ethical, reproducible alternative to human-subject research for studying social friction.
...
### [129] [Impact of Data-Oriented and Object-Oriented Design on Performance and Cache Utilization with Artificial Intelligence Algorithms in Multi-Threaded CPUs](https://arxiv.org/abs/2512.07841)
...
TL;DR: The paper compares Data-Oriented Design (DOD) and Object-Oriented Design (OOD) in multi-threaded A* search algorithms, showing DOD's superior cache efficiency and performance, though single-threaded versions often outperform multi-threaded ones due to thread overhead.


<details>
  <summary>Details</summary>
Motivation: The performance gap between CPUs and memory drives the need for hardware-aware software designs. The study aims to evaluate DOD and OOD in cache utilization and multi-threaded efficiency.

Method: Four variants of the A* algorithm (ST-OOD, ST-DOD, MT-OOD, MT-DOD) were developed and tested on execution time, memory usage, and cache misses.

Result: DOD showed better performance in multi-threaded tests, with faster execution and fewer cache misses. Single-threaded versions often outperformed multi-threaded ones due to thread overhead.

Conclusion: DOD's consistent advantages in critical metrics highlight its architectural superiority, making it more effective for complex, large-scale AI and parallel computing tasks.
...
### [130] [Can AI autonomously build, operate, and use the entire data stack?](https://arxiv.org/abs/2512.07926)
*Arvind Agarwal,Lisa Amini,Sameep Mehta,Horst Samulowitz,Kavitha Srinivas*

Main category: cs.AI

TL;DR: The paper discusses transitioning from AI-assisted data management to fully autonomous data systems by leveraging intelligent agents across the entire data lifecycle.


<details>
  <summary>Details</summary>
Motivation: Current AI tools assist specific roles but lack full automation. The increasing capability of AI to handle complex tasks presents an opportunity for autonomous data estates.

Method: The paper explores how intelligent agents can autonomously manage each stage of the modern data stack, aiming for holistic and self-sufficient systems.

Result: The study identifies opportunities for autonomous data management, examines agent-based streamlining of the data lifecycle, and highlights open research questions.

Conclusion: The paper advocates for a paradigm shift towards fully autonomous data systems, encouraging further research and collaboration to achieve this vision.
...
### [131] [Multi-Agent Intelligence for Multidisciplinary Decision-Making in Gastrointestinal Oncology](https://arxiv.org/abs/2512.08674)
...
TL;DR: Proposes hierarchical Multi-Agent Framework improving GI oncology clinical reasoning over monolithic MLLMs, achieving high expert scores.


<details>
  <summary>Details</summary>
Motivation: Addresses MLLM challenges like context dilution and hallucination in complex medical data interpretation.

Method: Hierarchical Multi-Agent Framework mimicking human Multidisciplinary Team workflow.

Result: Achieved 4.60/5.00 expert score, with notable improvements in reasoning logic and medical accuracy.

Conclusion: Agent-based collaboration offers scalable, interpretable, and robust automated decision support in oncology.
...
### [132] [SkipKV: Selective Skipping of KV Generation and Storage for Efficient Inference with Large Reasoning Models](https://arxiv.org/abs/2512.07993)
...
TL;DR: SkipKV is a training-free KV compression method for efficient CoT reasoning, improving accuracy and reducing generation length compared to SoTA methods.


<details>
  <summary>Details</summary>
Motivation: Large reasoning models suffer from high KV cache overhead, causing memory and throughput bottlenecks. Existing eviction methods fail in multi-batch settings and produce longer sequences.

Method: SkipKV introduces sentence-level eviction and dynamic activation updates to maintain semantic coherence and reduce redundancy.

Result: SkipKV improves accuracy by 26.7%, reduces generation length by 1.6x, and boosts throughput by 1.7x compared to alternatives.

Conclusion: SkipKV efficiently compresses KV cache without training, outperforming SoTA methods in accuracy and efficiency.
...
### [133] [Towards Foundation Models with Native Multi-Agent Intelligence](https://arxiv.org/abs/2512.08743)
*Shuyue Hu,Haoyang Yan,Yiqun Zhang,Yang Chen,Dongzhan Zhou,Lei Bai*

Main category: cs.AI

TL;DR: The paper argues for the need to develop foundation models (FMs) with native multi-agent intelligence, highlighting gaps in current capabilities despite strong single-agent performance. It proposes research directions for achieving this goal.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observation that while FMs excel in single-agent tasks, they lack robust multi-agent intelligence, which is crucial for advancing AI agents.

Method: The study evaluates 41 large language models to assess their multi-agent capabilities, identifying four key areas: understanding, planning, communication, and adaptation.

Result: Empirical evidence shows that strong single-agent performance does not inherently translate to effective multi-agent intelligence, revealing a significant gap.

Conclusion: The paper concludes by outlining future research directions, including dataset creation, evaluation methods, training paradigms, and safety measures, to enhance FMs' multi-agent intelligence.
...
TL;DR: The paper introduces an AI-augmented system for patient-trial matching that automates eligibility assessments, integrates EHR data, and supports human review while maintaining security.


<details>
  <summary>Details</summary>
Motivation: To address the manual, time-consuming, and resource-intensive nature of screening patients for clinical trial eligibility.

Method: Uses open-source LLMs to generate structured eligibility assessments with interpretable reasoning chains, integrating heterogeneous EHR data.

Result: The system reduces coordinator burden, broadens trial considerations, and ensures auditability of AI-generated outputs.

Conclusion: This AI tool improves efficiency and flexibility in patient-trial matching while supporting human oversight.
...
TL;DR: The study evaluates ChatGPT and DeepSeek LLMs in education and research, highlighting their strengths in text generation, programming, and problem-solving, with user insights.


<details>
  <summary>Details</summary>
Motivation: To assess the effectiveness of ChatGPT and DeepSeek LLMs in educational and research contexts, focusing on accuracy, efficiency, and user experience.

Method: Combined background technology analysis, empirical experiments (text generation, programming, problem-solving), and a user survey.

Result: ChatGPT excels in general language tasks, DeepSeek in programming; both perform well in medical and mathematical tasks. User feedback provides practical insights.

Conclusion: Both LLMs have unique strengths and contribute significantly to education and research, with trade-offs in performance and user experience.
...
TL;DR: The paper discusses a scalable back-end system for a mobile diabetes prediction app, achieving 83% of performance targets and handling 10,000 concurrent users.


<details>
  <summary>Details</summary>
Motivation: To address the need for early diabetes detection globally with a scalable and responsive AI-powered solution.

Method: The system uses horizontal scaling, database sharding, and asynchronous communication via RabbitMQ to ensure performance and reliability.

Result: 83% of features met performance targets, with key functionalities achieving desired latency and failure rates. The system handled 10,000 concurrent users.

Conclusion: The scalable architecture effectively supports the diabetes prediction app, with RabbitMQ proving vital for reliability under heavy load.
...
TL;DR: The paper explores the connection between causal learning in humans and machines, focusing on the concept of 'empowerment' as a bridge between Bayesian causal learning and reinforcement learning. It also investigates how children and adults use empowerment cues for causal inference.


<details>
  <summary>Details</summary>
Motivation: The study aims to bridge the gap between human causal learning, which is often understood through Bayesian frameworks, and machine learning, which struggles with causal reasoning. Empowerment is proposed as a unifying concept to enhance understanding and improve machine learning.

Method: The research combines theoretical analysis of empowerment in causal learning with an empirical study. The study systematically tests how children and adults use empowerment cues to infer causal relations and design interventions.

Result: The findings suggest that empowerment is a valuable concept for understanding and improving causal learning in both humans and machines. It also provides insights into children's causal learning processes.

Conclusion: Empowerment serves as a crucial link between Bayesian causal learning and reinforcement learning, offering a promising approach to enhance causal reasoning in both humans and machines.
...
TL;DR: The KPI framework enhances disease prediction by integrating medical knowledge, using prototypes, and improving interpretability with LLMs, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To address challenges like imbalanced disease distributions and lack of interpretability in predicting diseases from patient-side information.

Method: Uses a knowledge graph, constructs disease prototypes, employs contrastive learning, and leverages LLMs for explanations.

Result: Outperforms state-of-the-art methods in predictive accuracy and provides clinically valid explanations.

Conclusion: KPI offers practical value for patient-centered healthcare by improving accuracy and interpretability.
...
TL;DR: Recent reasoning models excel in CFA exams, with Gemini 3.0 Pro leading performance across levels.


<details>
  <summary>Details</summary>
Motivation: To assess state-of-the-art reasoning models' capabilities on CFA exams, given their poor past performance.

Method: Evaluated models on 980 questions from mock CFA exams (Levels I-III), using pass/fail criteria from prior studies.

Result: Most models passed all levels; Gemini 3.0 Pro scored 97.6% on Level I, GPT-5 led Level II (94.3%), and Gemini models excelled on Level III.

Conclusion: Advanced reasoning models now outperform previous LLMs significantly on CFA exams, showcasing their improved capabilities.
...
TL;DR: Generative Agents are introduced to automate the evaluation of AI-generated content, addressing cost and quality concerns in business applications.


<details>
  <summary>Details</summary>
Motivation: The research aims to reduce the time and cost associated with human evaluations of AI-generated content while ensuring quality.

Method: Generative Agents simulate human judgment by rating content on coherence, interestingness, clarity, fairness, and relevance.

Result: The study demonstrates that these agents can efficiently evaluate content, streamlining business processes and reducing reliance on human input.

Conclusion: Generative Agents offer a scalable solution for enhancing AI-generated content quality and operational efficiency in businesses.
...
TL;DR: The paper explores quantitative scaling principles for LM-based agent systems, evaluating performance across diverse benchmarks and architectures, identifying key trade-offs and effects like tool-coordination balance and capability saturation.


<details>
  <summary>Details</summary>
Motivation: Despite widespread adoption of LM-based agent systems, their performance principles remain underexplored, leading to reliance on heuristics rather than principled design choices.

Method: The study evaluates five agent architectures across three LLM families, using four benchmarks and standardized tools/token budgets, deriving a predictive model with empirical coordination metrics.

Result: Key findings include a tool-coordination trade-off, capability saturation, and topology-dependent error amplification. The framework predicts optimal strategies for 87% of held-out configurations.

Conclusion: The research provides predictive principles for agentic scaling based on measurable task properties, guiding principled design choices for LM-based agent systems.
...
TL;DR: The paper introduces a reinforced strategy injection mechanism (rSIM) to enhance LLMs into Reasoning Language Models (RLMs) via adaptive strategy injection and multi-agent RL, showing significant performance improvements and generalizability.


<details>
  <summary>Details</summary>
Motivation: To transform LLMs into RLMs capable of advanced reasoning like self-reflection and deep thinking through a novel strategy injection mechanism.

Method: Proposes rSIM, using a planner (leader agent) trained with LLM (follower agent) via multi-agent RL and rule-based rewards to guide reasoning strategies.

Result: rSIM enables Qwen2.5-0.5B to outperform Qwen2.5-14B, with the planner being generalizable and supporting continual learning across tasks.

Conclusion: rSIM effectively upgrades LLMs to RLMs with improved reasoning, showcasing generalizability and adaptability.
...
TL;DR: The study proposes a machine learning framework to predict California Bearing Ratio (CBR) using soil samples, outperforming traditional lab tests.


<details>
  <summary>Details</summary>
Motivation: Traditional CBR tests are slow and costly; ML offers a faster, data-driven alternative.

Method: Tested 12 ML algorithms on 382 soil samples with physicochemical properties.

Result: Random forest regressor achieved the best performance with R2 scores of 0.95 (training), 0.76 (validation), and 0.83 (test).

Conclusion: ML models like random forest provide an effective, robust alternative for geotechnical engineering.
...
TL;DR: The paper proposes an AutoML approach using XGBoost to predict soil compaction parameters (OMC and MDD), outperforming traditional methods and improving accuracy and generalizability.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for determining soil compaction parameters are labor-intensive and inaccurate. AI/ML techniques offer alternatives but face challenges in accuracy and generalizability, especially with diverse soil types.

Method: The study employs an AutoML approach to automate algorithm selection and hyperparameter optimization, with XGBoost identified as the top-performing algorithm.

Result: XGBoost achieved R-squared values of 80.4% for MDD and 89.1% for OMC, demonstrating improved accuracy and generalizability across diverse soil types.

Conclusion: AutoML enhances the prediction of soil compaction parameters, contributing to more efficient and reliable construction practices by leveraging heterogeneous datasets.
...
TL;DR: A novel XAI framework for GNNs is proposed to provide efficient and adaptable explanations by analyzing graph structure influences on predictions, addressing limitations of current methods.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods for GNNs struggle with complexity, computational inefficiency, and limited generalizability, motivating the need for a tailored framework.

Method: The thesis develops a new XAI framework for GNNs, focusing on graph structure analysis to understand prediction influences, beyond traditional feature-based methods.

Result: The proposed framework aims to offer adaptable and computationally efficient explanations, improving understanding of GNN decision-making processes.

Conclusion: This work advances XAI for GNNs by addressing gaps in interpretability and generalizability, enhancing trust and usability in graph-based machine learning.
...
### [146] [Reflecting with Two Voices: A Co-Adaptive Dual-Strategy Framework for LLM-Based Agent Decision Making](https://arxiv.org/abs/2512.08366)
*Wentao Zhang,Qunbo Wang,Tao Zhang,Junsheng Wu,Hongping Gan,Yang Liu,Ling Dai,Shizhuang Deng,Shuntong Sun*

Main category: cs.AI

TL;DR: DuSAR is a demonstration-free LLM agent framework that uses dual strategies and reflection for co-adaptive reasoning, achieving state-of-the-art performance while reducing computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents rely on external demonstrations or retrieval-augmented planning, leading to brittleness and high computational costs. The goal is to mimic human problem-solving for better generalization and efficiency.

Method: DuSAR employs two strategies—a high-level holistic plan and a context-grounded local policy—interacting via a lightweight reflection mechanism. It dynamically adjusts plans based on a Strategy Fitness Score.

Result: DuSAR achieves SOTA performance, doubling prior results (37.1% success on ALFWorld, 4.02% on Mind2Web) and reducing token consumption by 3-9X. Optional expert demonstrations further boost performance.

Conclusion: DuSAR's dual-strategy coordination is effective, efficient, and flexible, offering compatibility with external knowledge while reducing computational overhead.
...
### [147] [DeepFeature: Iterative Context-aware Feature Generation for Wearable Biosignals](https://arxiv.org/abs/2512.08379)
*Kaiwei Liu,Yuting He,Bufang Yang,Mu Yuan,Chun Man Victor Wong,Ho Pong Andrew Sze,Zhenyu Yan,Hongkai Chen*

Main category: cs.AI

TL;DR: DeepFeature is an LLM-empowered, context-aware feature generation framework for wearable biosignals, integrating expert knowledge and iterative refinement to outperform baseline methods.


<details>
  <summary>Details</summary>
Motivation: Current feature extraction methods for biosignals lack task-specific context, struggle with high-dimensional settings, and are prone to errors.

Method: DeepFeature uses a multi-source feature generation mechanism, iterative refinement with feedback, and robust feature-to-code translation.

Result: DeepFeature improves AUROC by 4.21-9.67% across eight tasks, outperforming state-of-the-art methods on five tasks.

Conclusion: DeepFeature effectively enhances feature extraction for wearable biosignals, achieving superior performance while maintaining robustness.
...
### [148] [From Accuracy to Impact: The Impact-Driven AI Framework (IDAIF) for Aligning Engineering Architecture with Theory of Change](https://arxiv.org/abs/2512.08449)
*Yong-Woon Kim*

Main category: cs.AI

TL;DR: The paper introduces IDAIF, a framework combining Theory of Change with AI design to ensure ethical, impact-driven AI systems. It addresses alignment gaps in current approaches and offers structured layers for deployment.


<details>
  <summary>Details</summary>
Motivation: AI's growing influence in high-stakes domains like healthcare and finance highlights the need for alignment with human values, which current methods often overlook by focusing solely on technical metrics.

Method: IDAIF maps Theory of Change stages to AI architectural layers (Data-Pipeline-Inference-Agentic-Normative) using multi-objective optimization, causal DAGs, RLHF, and guardian architectures.

Result: Case studies in healthcare, cybersecurity, and software engineering show IDAIF's effectiveness in creating ethical and trustworthy AI systems.

Conclusion: IDAIF shifts AI development from model-centric to impact-centric, providing practical frameworks for sociotechnical alignment.
...
### [149] [Using reinforcement learning to probe the role of feedback in skill acquisition](https://arxiv.org/abs/2512.08463)
*Antonio Terpin,Raffaello D'Andrea*

Main category: cs.AI

TL;DR: The study uses a reinforcement learning agent on a spinning cylinder in a water channel to explore skill acquisition, showing that feedback is essential for learning but not for executing high-performance strategies.


<details>
  <summary>Details</summary>
Motivation: To understand skill acquisition in controlled settings, bypassing human subjects by using a physical system with complex dynamics and clear objectives.

Method: A reinforcement learning agent interacts with a spinning cylinder in a water channel to optimize drag, comparing performance with and without flow feedback.

Result: High-dimensional flow feedback enabled quick discovery of effective strategies, while no feedback during training hindered drag maximization but not minimization.

Conclusion: Learning requires richer information than execution, and the ease of learning depends on the goal, not system complexity.
...
### [150] [Autonomous Issue Resolver: Towards Zero-Touch Code Maintenance](https://arxiv.org/abs/2512.08492)
*Aliaksei Kaliutau*

Main category: cs.AI

TL;DR: The paper introduces Data Transformation Graphs (DTG) for repository-scale Automated Program Repair, shifting focus from control flow to data lineage, and presents AIR, a multi-agent framework achieving 87.1% resolution on SWE benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current APR approaches struggle with complex directory structures and irrelevant control logic. DTG addresses this by focusing on data states and lineage, aiming to resolve the 'Semantic Trap' in AI code-assistant tools.

Method: Proposes DTG, modeling data states as nodes and functions as edges, and introduces AIR, a multi-agent framework combining neuro-symbolic reasoning and DTG for scalable logic repair.

Result: Achieves 87.1% resolution rate on SWE-Verified benchmark, outperforming traditional methods.

Conclusion: DTG and AIR provide a robust foundation for repository-scale APR, addressing core limitations of current AI tools and enhancing software maintenance.
...
### [151] [A Lightweight Transfer Learning-Based State-of-Health Monitoring with Application to Lithium-ion Batteries in Unmanned Air Vehicles](https://arxiv.org/abs/2512.08512)
*Jiang Liu,Yan Qin,Wei Dai,Chau Yuen*

Main category: cs.AI

TL;DR: The paper proposes a lightweight transfer learning approach (CITL) for accurate SOH monitoring in lithium-ion batteries, overcoming computational limitations in portable devices. It outperforms existing methods significantly.


<details>
  <summary>Details</summary>
Motivation: Traditional TL-based SOH monitoring is resource-intensive for portable devices, necessitating a lightweight solution.

Method: The CITL approach uses semi-supervised TL, structural risk minimization, and manifold consistency to optimize learning. Theoretical convergence analysis is provided.

Result: CITL outperforms existing methods by up to 87.70% in SOH estimation, validated with UAV battery data.

Conclusion: The proposed CITL offers an efficient and accurate solution for SOH monitoring in resource-constrained environments.
...
TL;DR: Principles2Plan is a system that combines human expertise and LLMs to create context-sensitive ethical rules for automated planning, addressing the gap in ethical awareness for robots in human environments.


<details>
  <summary>Details</summary>
Motivation: Ethical awareness is essential for robots in human settings, but current tools lack support for this, and manual rule specification is laborious and context-specific.

Method: The system uses human-provided domain details and high-level principles (e.g., beneficence, privacy) to generate operational ethical rules via an LLM, which users can review and prioritize for planning.

Result: Principles2Plan demonstrates the feasibility of human-LLM collaboration to produce principle-grounded ethical rules for automated planning.

Conclusion: The system highlights the potential of collaborative approaches to make ethical automated planning more practical and accessible.
...
TL;DR: CogMCTS integrates LLM cognitive guidance with MCTS for automated heuristic optimization, improving stability, efficiency, and solution quality.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based evolutionary methods suffer from local optima and limited search diversity. CogMCTS addresses these issues by enhancing exploration-exploitation balance.

Method: CogMCTS uses multi-round cognitive feedback, dual-track node expansion, elite heuristic management, and strategic mutation to improve heuristic generation.

Result: Experiments show CogMCTS outperforms existing LLM-based AHD methods in stability, efficiency, and solution quality.

Conclusion: CogMCTS effectively integrates cognitive guidance with MCTS to advance automated heuristic design for complex optimization problems.
...
TL;DR: A transformer-based model predicts protein secondary structures using attention mechanisms and sliding-window data augmentation on the CB513 dataset, capturing local and long-range residue interactions.


<details>
  <summary>Details</summary>
Motivation: Understanding protein function requires predicting secondary structures like alpha helices, beta sheets, and coils from amino acid sequences.

Method: The model uses a transformer architecture with attention mechanisms and applies a sliding-window data augmentation technique on the CB513 dataset to expand training samples.

Result: The transformer generalizes well across variable-length sequences and effectively captures both local and long-range residue interactions.

Conclusion: The transformer-based approach is effective for predicting protein secondary structures, demonstrating strong generalization and interaction capture.
...
### [155] [See-Control: A Multimodal Agent Framework for Smartphone Interaction with a Robotic Arm](https://arxiv.org/abs/2512.08629)
*Haoyu Zhao,Weizhong Ding,Yuhao Yang,Zheng Tian,Linyi Yang,Kun Shao,Jun Wang*

Main category: cs.AI

TL;DR: See-Control introduces a robotic framework for smartphone operation via direct physical interaction, bypassing Android-specific dependencies like ADB, and includes a benchmark, MLLM agent, and dataset.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on Android Debug Bridge (ADB), limiting applicability to Android devices. This work aims to create a platform-agnostic solution for smartphone operation using a robotic arm.

Method: The framework, See-Control, includes: (1) an ESO benchmark with 155 tasks, (2) an MLLM-based agent generating robotic commands without ADB, and (3) an annotated dataset of operation episodes.

Result: See-Control bridges digital agents and the physical world, enabling home robots to perform smartphone tasks without device-specific limitations.

Conclusion: The See-Control framework offers a platform-agnostic solution for smartphone operation, advancing the capability of home robots in realistic environments.
...
### [156] [Deconstructing the Dual Black Box:A Plug-and-Play Cognitive Framework for Human-AI Collaborative Enhancement and Its Implications for AI Governance](https://arxiv.org/abs/2512.08740)
*Yiming Lu*

Main category: cs.AI

TL;DR: The paper proposes a 'human-AI collaborative cognitive enhancement' paradigm to bridge the gap between human intuition and AI decision-making by creating a 'functional white-box' system using structured meta-interaction.


<details>
  <summary>Details</summary>
Motivation: Addressing the divide between human experts' implicit intuition ('cognitive black box') and AI's untrustworthy decision-making ('computational black box').

Method: Introduces a 'plug-and-play cognitive framework' within the Recursive Adversarial Meta-Thinking Network (RAMTN) to convert expert thinking into reusable assets.

Result: Transforms expert thinking into scalable public assets, shifting from 'AI as a tool' to 'AI as a thinking partner'.

Conclusion: Opens new paths for AI governance through transparency of interaction protocols and promotes cognitive inclusion.
...
### [157] [Performance Comparison of Aerial RIS and STAR-RIS in 3D Wireless Environments](https://arxiv.org/abs/2512.08755)
*Dongdong Yang,Bin Li,Jiguang He*

Main category: cs.AI

TL;DR: This paper compares aerial RIS and STAR-RIS architectures in 3D wireless environments, highlighting STAR-RIS's superiority in low-altitude scenarios and RIS's advantage near base stations at higher altitudes.


<details>
  <summary>Details</summary>
Motivation: To address the lack of comprehensive performance comparison between aerial RIS and STAR-RIS architectures, which are key for enhancing wireless coverage and capacity in next-gen networks.

Method: Accurate channel models with directional radiation patterns are used; joint optimization problems are formulated and solved via weighted MMSE and block coordinate descent algorithms.

Result: STAR-RIS excels in low-altitude scenarios due to full-space coverage, while RIS performs better near base stations at higher altitudes.

Conclusion: The findings offer practical deployment insights for aerial intelligent surfaces in future 6G systems.
...
### [158] [A Practical Guide for Designing, Developing, and Deploying Production-Grade Agentic AI Workflows](https://arxiv.org/abs/2512.08769)
...
TL;DR: The paper provides a practical guide for designing, developing, and deploying production-grade agentic AI systems, emphasizing structured engineering practices and best practices.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of creating reliable, observable, and safe agentic AI workflows amid rapid adoption.

Method: Introduces a structured lifecycle with workflow decomposition, multi-agent design patterns, Model Context Protocol (MCP), tool integration, and Responsible-AI considerations.

Result: Presents nine core best practices and demonstrates their application in a multimodal news-analysis workflow.

Conclusion: Offers foundational guidance for building robust and extensible agentic AI workflows.
...
### [159] [CARLoS: Retrieval via Concise Assessment Representation of LoRAs at Scale](https://arxiv.org/abs/2512.08826)
*Shahar Sarfaty,Adi Haviv,Uri Hacohen,Niva Elkin-Koren,Roi Livni,Amit H. Bermano*

Main category: cs.AI

TL;DR: CARLoS is a framework for characterizing and retrieving LoRAs without metadata, using CLIP embeddings to define their behavior in terms of Direction, Strength, and Consistency, outperforming textual baselines.


<details>
  <summary>Details</summary>
Motivation: To address the unstructured ecosystem of LoRAs and unreliable discovery methods relying on user descriptions or popularity metrics.

Method: Analyzing 650+ LoRAs via image generation, using CLIP embeddings to define a three-part representation (Direction, Strength, Consistency) for retrieval and analysis.

Result: CARLoS outperforms textual baselines in automated and human evaluations, and its representation supports broader analyses like copyright considerations.

Conclusion: CARLoS provides a practical, metadata-free solution for LoRA characterization and retrieval, with potential relevance for legal analyses in copyright contexts.
...
### [160] [Interpolation in Knowledge Representation](https://arxiv.org/abs/2512.08833)
*Jean Christoph Jung,Patrick Koopmann,Matthias Knorr*

Main category: cs.AI

TL;DR: The paper explores Craig and uniform interpolation's applications in knowledge representation and examines challenges in computing interpolants for description logics and logic programming.


<details>
  <summary>Details</summary>
Motivation: To address the lack of Craig or uniform interpolation in many knowledge representation formalisms and the difficulty in computing interpolants.

Method: Analyzes description logics and logic programming, discussing theoretical results and practical methods for interpolant computation.

Result: Provides insights into theoretical and practical approaches for interpolant computation in the studied formalisms.

Conclusion: Emphasizes the importance of interpolation in knowledge representation and highlights ongoing challenges.
...
### [161] [EcomBench: Towards Holistic Evaluation of Foundation Agents in E-commerce](https://arxiv.org/abs/2512.08868)
...
TL;DR: EcomBench is a new benchmark designed to evaluate the practical capabilities of foundation agents in real-world e-commerce environments, addressing gaps in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for evaluating foundation agents often focus on academic or artificial scenarios, neglecting real-world challenges. The e-commerce domain, with its dynamic and diverse interactions, provides a practical setting to assess agent capabilities.

Method: EcomBench is built from genuine user demands in global e-commerce ecosystems, curated and annotated by human experts. It includes multiple task categories and three difficulty levels to evaluate key agent capabilities like information retrieval, reasoning, and knowledge integration.

Result: EcomBench offers a rigorous and dynamic testbed for measuring agent performance in realistic e-commerce scenarios, ensuring clarity, accuracy, and relevance.

Conclusion: EcomBench fills a critical gap in evaluating foundation agents by focusing on real-world e-commerce challenges, providing a comprehensive tool for assessing practical agent capabilities.
...
### [162] [Same Content, Different Answers: Cross-Modal Inconsistency in MLLMs](https://arxiv.org/abs/2512.08923)
*Angela van Sprang,Laurens Samson,Ana Lucic,Erman Acar,Sennay Ghebreab,Yuki M. Asano*

Main category: cs.AI

TL;DR: The paper introduces benchmarks REST and REST+ to evaluate cross-modal inconsistency in MLLMs, finding that even state-of-the-art models struggle with consistent reasoning across image, text, and mixed modalities.


<details>
  <summary>Details</summary>
Motivation: To systematically assess and highlight cross-modal inconsistency in MLLMs, which are trained to align vision and language but fail to perform tasks uniformly across modalities.

Method: Create benchmarks (REST and REST+) with semantically identical samples in image, text, and mixed modalities. Evaluate 15 MLLMs for consistency, considering factors like OCR, visual characteristics, and vision tokens.

Result: MLLMs exhibit significant inconsistency across modalities. Visual traits and vision tokens affect performance, and the consistency score correlates with the modality gap.

Conclusion: Current MLLMs lack consistent reasoning across modalities, and the introduced benchmarks provide a tool for future improvements.
...
### [163] [ThreadWeaver: Adaptive Threading for Efficient Parallel Reasoning in Language Models](https://arxiv.org/abs/2512.07843)
...
TL;DR: ThreadWeaver is a framework for adaptive parallel reasoning in LLMs, achieving accuracy comparable to sequential models while reducing latency. It uses a two-stage generator, trie-based co-design, and reinforcement learning for parallelization.


<details>
  <summary>Details</summary>
Motivation: To address the latency issues in sequential decoding of LLMs without compromising accuracy, enabling efficient parallel reasoning.

Method: ThreadWeaver employs a two-stage parallel trajectory generator for high-quality data, trie-based training-inference co-design for compatibility with autoregressive engines, and parallelization-aware reinforcement learning.

Result: ThreadWeaver achieves 71.9% average accuracy on six benchmarks (79.9% on AIME24) with up to 1.53x speedup in token latency.

Conclusion: ThreadWeaver sets a new Pareto frontier, balancing accuracy and efficiency in parallel reasoning for LLMs.
...
### [164] [Space Alignment Matters: The Missing Piece for Inducing Neural Collapse in Long-Tailed Learning](https://arxiv.org/abs/2512.07844)
*Jinping Wang,Zhiqiang Gao,Zhiwu Xie*

Main category: cs.LG

TL;DR: The paper addresses the misalignment between feature and classifier spaces in long-tailed learning, proposing three alignment strategies to improve performance.


<details>
  <summary>Details</summary>
Motivation: In long-tailed regimes, sample imbalance disrupts the Neural Collapse (NC) phenomenon, leading to poor generalization. Current methods focus on ETF geometry but ignore harmful misalignment between features and classifier weights.

Method: The authors analyze the misalignment's impact theoretically and introduce three plug-and-play alignment strategies compatible with existing long-tail methods.

Result: Experiments on CIFAR-10-LT, CIFAR-100-LT, and ImageNet-LT show the proposed strategies consistently enhance baselines and achieve state-of-the-art results.

Conclusion: The study highlights the importance of feature-classifier alignment in long-tailed learning, offering practical solutions that improve performance without architectural changes.
...
### [165] [CarBench: A Comprehensive Benchmark for Neural Surrogates on High-Fidelity 3D Car Aerodynamics](https://arxiv.org/abs/2512.07847)
*Mohamed Elrefaie,Dule Shu,Matt Klenk,Faez Ahmed*

Main category: cs.LG

TL;DR: CarBench is the first standardized benchmark for large-scale 3D car aerodynamics, evaluating 11 state-of-the-art models on DrivAerNet++, the largest public dataset for automotive aerodynamics, to advance data-driven engineering.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized benchmarks for large-scale numerical simulations in engineering design motivates the creation of CarBench to drive algorithmic innovation in machine learning for aerodynamics.

Method: Eleven architectures, including neural operator methods, geometric deep learning, transformer-based solvers, and implicit field networks, are evaluated on the DrivAerNet++ dataset for predictive accuracy, physical consistency, computational efficiency, and statistical uncertainty. Cross-category experiments are also conducted.

Result: CarBench provides a reproducible foundation for large-scale learning from high-fidelity CFD simulations, with open-source training pipelines, uncertainty estimation routines, and pretrained model weights.

Conclusion: CarBench establishes a standardized benchmark for large-scale 3D car aerodynamics, facilitating progress in data-driven engineering by making resources openly available.
...
### [166] [RaX-Crash: A Resource Efficient and Explainable Small Model Pipeline with an Application to City Scale Injury Severity Prediction](https://arxiv.org/abs/2512.07848)
*Di Zhu,Chen Xie,Ziwei Wang,Haoyun Zhang*

Main category: cs.LG

TL;DR: RaX-Crash is an efficient, explainable pipeline for predicting injury severity in NYC motor vehicle collisions using tree-based models (XGBoost, Random Forest) and SLMs, outperforming SLMs in accuracy.


<details>
  <summary>Details</summary>
Motivation: Addressing the public health burden of NYC's motor vehicle collisions by providing a resource-efficient and interpretable solution for injury severity prediction.

Method: Integrates three linked tables, builds a unified feature schema in partitioned storage, and trains compact tree-based ensembles (Random Forest and XGBoost). Compares results with SLMs prompted with textual summaries.

Result: XGBoost and Random Forest achieve accuracies of 0.7828 and 0.7794, outperforming SLMs (0.594 and 0.496). Class weighting improves fatal recall, and SHAP highlights key severity drivers.

Conclusion: Interpretable small model ensembles are effective for city-scale injury analytics, and hybrid pipelines combining tabular predictors with SLM narratives enhance communication without scalability loss.
...
### [167] [SABER: Small Actions, Big Errors - Safeguarding Mutating Steps in LLM Agents](https://arxiv.org/abs/2512.07850)
*Alejandro Cuadron,Pengfei Yu,Yang Liu,Arpit Gupta*

Main category: cs.LG

TL;DR: The paper investigates the fragility of LLM agents in long-horizon, tool-using tasks, identifying mutating actions as key failure points. It introduces a safeguard, ҄, which improves performance significantly.


<details>
  <summary>Details</summary>
Motivation: To understand why LLM agents fail in long-horizon tasks and to develop methods to mitigate these failures.

Method: Decomposes trajectories into mutating vs. non-mutating steps, introduces decisive deviations analysis, and proposes ҄—a model-agnostic safeguard with verification, reflection, and context cleaning.

Result: Mutating action deviations drastically reduce success odds (up to 96%). ҄ improves performance (e.g., +28% on Airline). Annotation errors in benchmarks are also identified.

Conclusion: Action-level analysis, targeted safeguards, and reliable benchmarks are key for robust multi-turn agents.
...
### [168] [GPU Memory Prediction for Multimodal Model Training](https://arxiv.org/abs/2512.07853)
*Jinwoo Jeong,Minchul Kang,Younghun Go,Changyong Shin,Hyunho Lee,Junho Yoon,Gyeongsik Yang,Chuck Yoo*

Main category: cs.LG

TL;DR: The paper proposes a framework to predict peak GPU memory usage in multimodal AI models, addressing gaps in existing unimodal-focused methods, achieving ~8.7% MAPE accuracy.


<details>
  <summary>Details</summary>
Motivation: GPU memory overload interrupts training and wastes resources, especially in multimodal models, which existing methods fail to address.

Method: The framework decomposes multimodal models into layers and uses factorization to estimate memory usage per layer.

Result: The framework achieves high accuracy (~8.7% MAPE) in predicting peak GPU memory usage.

Conclusion: The proposed solution effectively addresses GPU memory prediction gaps in multimodal AI systems.
...
### [169] [HSTMixer: A Hierarchical MLP-Mixer for Large-Scale Traffic Forecasting](https://arxiv.org/abs/2512.07854)
*Yongyao Wang,Jingyuan Wang,Xie Yu,Jiahao Ji,Chao Li*

Main category: cs.LG

TL;DR: The paper introduces HSTMixer, a novel MLP-based framework for efficient large-scale traffic forecasting, addressing computational complexity issues in existing models.


<details>
  <summary>Details</summary>
Motivation: Traffic forecasting is crucial for urban management, but current models suffer from impractical quadratic computational complexity for large-scale scenarios.

Method: HSTMixer uses a hierarchical spatiotemporal mixing block for multi-resolution feature extraction and an adaptive region mixer for dynamic pattern capture.

Result: Experiments on four datasets show HSTMixer achieves state-of-the-art performance with competitive computational efficiency.

Conclusion: HSTMixer offers an effective and efficient solution for large-scale traffic forecasting.
...
### [170] [LAPA: Log-Domain Prediction-Driven Dynamic Sparsity Accelerator for Transformer Model](https://arxiv.org/abs/2512.07855)
*Huizheng Wang,Hongbin Wang,Shaojun Wei,Yang Hu,Shouyi Yin*

Main category: cs.LG

TL;DR: The paper proposes LAPA, a log-domain attention prediction algorithm-architecture co-design to address dynamic computational bottlenecks in Transformer models, achieving higher energy efficiency than existing methods.


<details>
  <summary>Details</summary>
Motivation: Transformers face dynamic computational bottlenecks as input sequences vary, and existing sparse Transformer approaches are single-stage based with significant power overhead when applied across multiple stages.

Method: LAPA introduces an asymmetric leading one computing (ALOC) scheme, mixed-precision multi-round shifting accumulation (MRSA) mechanism, and data-feature dependent filter (DDF) strategy, along with an accelerator for practical hardware improvement.
...
Conclusion: The proposed LAPA co-design effectively addresses the dynamic computational challenges of Transformers, offering significant energy efficiency improvements.
...
### [171] [Medical Test-free Disease Detection Based on Big Data](https://arxiv.org/abs/2512.07856)
*Haokun Zhao,Yingzhe Bai,Qingyang Xu,Lixin Zhou,Jianxin Chen,Jicong Fan*

Main category: cs.LG

TL;DR: CLDD is a graph-based deep learning model that detects diseases by leveraging associations among diseases and patient similarities, reducing the need for extensive medical testing.


<details>
  <summary>Details</summary>
Motivation: To address the impracticality and high costs of performing extensive medical tests for disease detection by integrating patient-disease interactions and demographic features.

Method: Uses Collaborative Learning for Disease Detection (CLDD), a novel graph-based deep learning model, to exploit disease associations and patient similarities adaptively.

Result: Achieves a 6.33% improvement in recall and 7.63% improvement in precision over baselines, successfully recovering masked diseases in top predictions.

Conclusion: CLDD offers a cost-effective, scalable solution for disease detection, improving diagnostic accessibility and reliability.
...
### [172] [SA^2GFM: Enhancing Robust Graph Foundation Models with Structure-Aware Semantic Augmentation](https://arxiv.org/abs/2512.07857)
*Junhua Shi,Qingyun Sun,Haonan Yuan,Xingcheng Fu*

Main category: cs.LG

TL;DR: SA^2GFM is a robust Graph Foundation Model framework that enhances domain-adaptive representations using structure-aware semantic augmentation and hierarchical structure modeling, outperforming existing methods in robustness and effectiveness.


<details>
  <summary>Details</summary>
Motivation: Current Graph Foundation Models lack robustness against domain noise and structural perturbations, primarily due to insufficient modeling of hierarchical structural semantics.

Method: SA^2GFM integrates structure-aware textual prompts, a self-supervised Information Bottleneck mechanism, an expert adaptive routing mechanism, and a fine-tuning module for hierarchical structure optimization.

Result: SA^2GFM surpasses 9 state-of-the-art baselines in node and graph classification tasks, demonstrating superior robustness against noise and adversarial perturbations.

Conclusion: SA^2GFM addresses limitations in Graph Foundation Models by improving hierarchical structural modeling and robustness, offering a scalable solution for domain-adaptive graph representation learning.
...
### [173] [FAIM: Frequency-Aware Interactive Mamba for Time Series Classification](https://arxiv.org/abs/2512.07858)
*Da Zhang,Bingyu Li,Zhiyuan Zhao,Yanhan Zhang,Junyu Gao,Feiping Nie,Xuelong Li*

Main category: cs.LG

TL;DR: FAIM is a lightweight Frequency-Aware Interactive Mamba model for TSC, using frequency-domain features and adaptive filtering to address noise and overfitting, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Time series classification (TSC) is vital in real-world applications, but deep learning models face challenges like high computational costs, noise sensitivity, and overfitting. FAIM aims to solve these issues.

Method: FAIM introduces an Adaptive Filtering Block (AFB) for frequency-domain feature extraction and noise suppression, and an Interactive Mamba Block (IMB) for multi-granularity information interaction, alongside self-supervised pre-training.

Result: FAIM outperforms existing SOTA methods in benchmarks, offering a superior balance of accuracy and efficiency in TSC tasks.

Conclusion: FAIM provides an effective solution for TSC tasks by addressing key challenges and delivering robust performance, as validated by extensive experiments.
...
### [174] [SetAD: Semi-Supervised Anomaly Learning in Contextual Sets](https://arxiv.org/abs/2512.07863)
*Jianling Gao,Chongyang Tao,Xuelian Lin,Junfeng Liu,Shuai Ma*

Main category: cs.LG

TL;DR: SetAD reframes semi-supervised anomaly detection as a set-level task, using an attention-based set encoder and graded learning to capture group-level anomalies, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing semi-supervised anomaly detection methods focus on individual points or pairs, missing contextual anomalies and group-level interactions. SetAD addresses this gap.

Method: Proposes SetAD, an attention-based set encoder trained with graded learning. Introduces a context-calibrated anomaly scoring mechanism to assess points' deviations across diverse sets.

Result: SetAD outperforms state-of-the-art models on 10 real-world datasets, with performance improving as set size increases.

Conclusion: SetAD demonstrates the effectiveness of set-level anomaly detection, leveraging group interactions for superior performance.
...
### [175] [Pattern Recognition of Ozone-Depleting Substance Exports in Global Trade Data](https://arxiv.org/abs/2512.07864)
*Muhammad Sukri Bin Ramli*

Main category: cs.LG

TL;DR: The paper introduces an unsupervised ML framework to detect suspicious trade patterns in customs data, combining clustering, anomaly detection, and heuristic flagging. It successfully identified outliers and high-priority shipments, validated by real-world data and Explainable AI.


<details>
  <summary>Details</summary>
Motivation: Environmental treaties like the Montreal Protocol require efficient monitoring of trade data to detect suspicious activities. Current methods struggle with large, complex datasets.

Method: The framework uses unsupervised ML techniques: K-Means clustering for trade archetypes, Isolation Forest and IQR for anomaly detection, and heuristic flagging for tactics like vague descriptions. These are combined into a priority score.

Result: Applied to 100,000 records, it identified 1,351 price outliers and 1,288 high-priority shipments. High-priority commodities showed distinct value-to-weight ratios. The model detected a spike in 'mega-trades' correlating with regulatory changes.

Conclusion: The unsupervised learning pipeline effectively transforms raw trade data into actionable intelligence for regulators, validated by Explainable AI and real-world applicability.
...
### [176] [Using Text-Based Life Trajectories from Swedish Register Data to Predict Residential Mobility with Pretrained Transformers](https://arxiv.org/abs/2512.07865)
*Philipp Stark,Alexandros Sopasakis,Ola Hall,Markus Grillitsch*

Main category: cs.LG

TL;DR: Transforming Swedish register data into textual life trajectories enables better prediction of residential mobility using NLP models like LSTM, DistilBERT, BERT, and Qwen, showcasing the potential of modern language models in social science research.


<details>
  <summary>Details</summary>
Motivation: To address challenges like high cardinality of categorical variables and inconsistencies in coding schemes over time by leveraging Swedish register data.

Method: Converted register data from 6.9 million individuals into textual life trajectories and used NLP architectures (LSTM, DistilBERT, BERT, Qwen) to predict residential mobility.

Result: Sequential and transformer-based models outperformed baselines, proving that textualized register data preserves meaningful information.

Conclusion: Combining rich register data with modern language models advances longitudinal analysis in social sciences.
...
### [177] [Command & Control (C2) Traffic Detection Via Algorithm Generated Domain (Dga) Classification Using Deep Learning And Natural Language Processing](https://arxiv.org/abs/2512.07866)
*Maria Milena Araujo Felix*

Main category: cs.LG

TL;DR: This paper proposes a deep learning and NLP-based method to detect DGA domains, achieving high accuracy (97.2%) and reducing false positives compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional malware defenses fail against modern DGAs, necessitating advanced techniques.

Method: A hybrid dataset was used; lexical features were extracted, and an LSTM model was trained.

Result: The LSTM outperformed entropy analysis, achieving 97.2% accuracy with fewer false positives.

Conclusion: Deep learning and NLP are effective for detecting complex DGA domains.
...
### [178] [Bayesian Optimization for Function-Valued Responses under Min-Max Criteria](https://arxiv.org/abs/2512.07868)
*Pouya Ahadi,Reza Marzban,Ali Adibi,Kamran Paynabar*

Main category: cs.LG

TL;DR: The paper introduces MM-FBO, a Bayesian optimization framework for functional responses, minimizing maximum error and balancing exploitation-exploration with theoretical guarantees and superior performance in experiments.


<details>
  <summary>Details</summary>
Motivation: Classical Bayesian optimization methods are inadequate for functional responses, and existing approaches neglect worst-case deviations, motivating the need for MM-FBO.

Method: MM-FBO uses functional principal component analysis and Gaussian process surrogates, introducing an integrated uncertainty acquisition function to minimize maximum error.

Result: Experiments on synthetic benchmarks and physics-inspired studies show MM-FBO outperforms baselines, validating its effectiveness and the importance of functional uncertainty modeling.

Conclusion: MM-FBO addresses limitations in functional response optimization, providing theoretical guarantees and practical improvements.
...
### [179] [Advancing physiological time series reconstruction and imputation via mixture of receptive fields and experts fusion](https://arxiv.org/abs/2512.07873)
*Ci Zhang,Huayu Li,Changdi Yang,Jiangnan Xia,Yanzhi Wang,Xiaolong Ma,Jin Lu,Geng Yuan*

Main category: cs.LG

TL;DR: Proposes a Mixture of Experts (MoE)-based noise estimator within a diffusion framework for medical time series reconstruction, improving performance and reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for time series reconstruction are underexplored in medical contexts, which face challenges like high noise and variability.

Method: Introduces RFAMoE and Fusion MoE modules—RFAMoE adapts receptive fields per channel, while Fusion MoE generates and fuses noise signals in parallel during diffusion.

Result: Outperforms state-of-the-art diffusion methods on medical tasks, reducing errors and computational overhead.

Conclusion: The framework effectively addresses medical time series challenges, enhancing reconstruction quality and efficiency.
...
### [180] [Controllable risk scenario generation from human crash data for autonomous vehicle testing](https://arxiv.org/abs/2512.07874)
*Qiujing Lu,Xuanhan Wang,Runze Yuan,Wei Lu,Xinyi Gong,Shuo Feng*

Main category: cs.LG

TL;DR: CRAG is a framework for generating realistic, risk-prone behaviors in autonomous vehicle testing by disentangling normal and risky behaviors in a structured latent space.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of simulating realistic yet risk-prone behaviors for environment agents in autonomous vehicle testing, crucial for evaluating AV safety under rare, critical conditions.

Method: CRAG combines risk-aware latent representations with optimization-based mode-transition mechanisms to enable smooth shifts between safe and risk states, leveraging limited crash data efficiently.

Result: Experiments demonstrate CRAG's ability to improve behavioral diversity and controllably generate risk scenarios, enhancing AV robustness evaluation.

Conclusion: CRAG effectively unifies nominal and safety-critical behavior modeling, providing a robust tool for targeted AV testing.
...
### [181] [Softly Symbolifying Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.07875)
*James Bagrow,Josh Bongard*

Main category: cs.LG

TL;DR: S2KAN enhances interpretability in machine learning by integrating symbolic primitives into Kolmogorov-Arnold Networks, achieving competitive accuracy with smaller models.


<details>
  <summary>Details</summary>
Motivation: To address the lack of symbolic fidelity in trained activations of KANs, ensuring interpretable forms while maintaining accuracy.

Method: S2KAN combines symbolic and dense terms with learnable gates, using differentiable sparsification guided by Minimum Description Length.

Result: S2KAN achieves competitive or superior accuracy with smaller models across benchmarks and real-world tasks, showing emergent self-sparsification.

Conclusion: S2KAN successfully balances interpretability and accuracy, offering a practical solution for symbolic fidelity in machine learning.
...
### [182] [Fourier-Enhanced Recurrent Neural Networks for Electrical Load Time Series Downscaling](https://arxiv.org/abs/2512.07876)
*Qi Chen,Mihai Anitescu*

Main category: cs.LG

TL;DR: A Fourier-enhanced RNN for downscaling electrical loads outperforms baselines in RMSE across PJM territories.


<details>
  <summary>Details</summary>
Motivation: To improve accuracy in downscaling electrical loads by integrating Fourier seasonal embeddings and self-attention into an RNN framework.

Method: Combines a recurrent backbone, Fourier seasonal embeddings in latent space, and a self-attention layer for high-resolution dependencies.

Result: Achieves lower and flatter RMSE compared to Prophet baselines and RNN ablations without attention or Fourier features.

Conclusion: The proposed Fourier-enhanced RNN is effective for downscaling electrical loads, outperforming traditional methods.
...
### [183] [Balanced Accuracy: The Right Metric for Evaluating LLM Judges - Explained through Youden's J statistic](https://arxiv.org/abs/2512.08121)
*Stephane Collot,Colin Fraser,Justin Zhao,William F. Shen,Timon Willi,Ilias Leontiadis*

Main category: cs.LG

TL;DR: Using Youden's J statistic or Balanced Accuracy for selecting classifiers in LLM evaluation ensures better and more robust model comparisons by addressing class imbalance and arbitrary class choices.


<details>
  <summary>Details</summary>
Motivation: Rigorous evaluation of LLMs relies on accurate prevalence estimates of behaviors, which depend on the classifier's reliability. Traditional metrics like Accuracy and F1 can distort results due to class imbalance.

Method: Proposes using Youden's J statistic and Balanced Accuracy for classifier selection. Analyzes their theoretical alignment with trustworthy evaluation and demonstrates their effectiveness through examples and simulations.

Result: Balanced Accuracy and Youden's J statistic provide more reliable classifier selection, leading to better prevalence estimates for comparing LLMs.

Conclusion: Selecting classifiers using Balanced Accuracy or Youden's J statistic enhances the robustness and reliability of LLM evaluations by mitigating issues with traditional metrics.
...
### [184] [Artificial Intelligence-Driven Network-on-Chip Design Space Exploration: Neural Network Architectures for Design](https://arxiv.org/abs/2512.07877)
*Amogh Anshu N,Harish BP*

Main category: cs.LG

TL;DR: A machine learning-driven framework automates NoC design space exploration, comparing MLP, Conditional Diffusion Model, and CVAE, with the Diffusion Model achieving the highest accuracy and significantly reducing exploration time.


<details>
  <summary>Details</summary>
Motivation: Traditional NoC design space exploration methods are slow and struggle with complex parameter interactions. Machine learning can automate and accelerate this process.

Method: The framework uses BookSim simulations and reverse neural network models (MLP, Conditional Diffusion Model, CVAE) to predict optimal NoC parameters based on target performance metrics.

Result: The Conditional Diffusion Model achieved the highest accuracy (MSE 0.463) on unseen data. The framework reduces exploration time by orders of magnitude.

Conclusion: The proposed framework is a practical, scalable solution for rapid NoC co-design, outperforming traditional methods in speed and accuracy.
...
### [185] [Graph Contrastive Learning via Spectral Graph Alignment](https://arxiv.org/abs/2512.07878)
*Manh Nguyen,Joshua Cape*

Main category: cs.LG

TL;DR: SpecMatch-CL introduces a novel loss function aligning graph-of-graphs via normalized Laplacians, outperforming existing methods on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Addressing lack of control over global structure in graph contrastive learning methods like InfoNCE.

Method: SpecMatch-CL minimizes normalized Laplacian differences between views for alignment.

Result: Achieves state-of-the-art results on TU benchmarks and performs well in transfer learning.

Conclusion: SpecMatch-CL offers theoretical guarantees and empirical improvements in graph learning tasks.
...
### [186] [Nonnegative Matrix Factorization through Cone Collapse](https://arxiv.org/abs/2512.07879)
*Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: Cone Collapse is a new NMF algorithm leveraging conic geometry, outperforming existing methods in clustering tasks across various datasets.


<details>
  <summary>Details</summary>
Motivation: Existing NMF algorithms lack explicit use of conic geometry, which could enhance clustering performance.

Method: Proposes Cone Collapse, shrinking the nonnegative orthant to the minimal data cone, and derives CC-NMF for clustering.

Result: CC-NMF matches or surpasses existing NMF methods in clustering purity across 16 datasets.

Conclusion: Exploiting conic geometry improves NMF-based clustering, offering theoretical and empirical advantages.
...
### [187] [Semi-Supervised Contrastive Learning with Orthonormal Prototypes](https://arxiv.org/abs/2512.07880)
*Huanran Li,Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: CLOP is a new semi-supervised loss function designed to prevent dimensional collapse in contrastive learning, improving performance and stability in tasks like image classification and object detection.


<details>
  <summary>Details</summary>
Motivation: Dimensional collapse in contrastive learning hinders effectiveness, especially in semi-supervised setups. The paper aims to address this by identifying thresholds and proposing solutions.

Method: The authors first identify a critical learning-rate threshold causing collapse and then introduce CLOP, a loss function promoting orthogonal subspaces among class embeddings.

Result: Experiments show CLOP enhances performance in image classification and object detection, with greater stability across learning rates and batch sizes.

Conclusion: CLOP effectively mitigates dimensional collapse, offering improved and stable performance in contrastive learning tasks.
...
### [188] [GSPN-2: Efficient Parallel Sequence Modeling](https://arxiv.org/abs/2512.07884)
...
TL;DR: GSPN-2 enhances GSPN by reducing overhead, optimizing GPU usage, and introducing compact channel propagation, achieving transformer-level accuracy with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Address inefficiencies in GSPN for vision tasks, focusing on GPU overhead, data transfers, and redundant computations.

Method: Joint algorithm-system redesign: consolidates GPU kernels, pins warps to channels, uses shared memory, and introduces compact channel propagation.

Result: Matches transformer-level accuracy in image classification and text-to-image synthesis with reduced computational cost.

Conclusion: GSPN-2 sets a new efficiency standard for global spatial context modeling in vision applications.
...
### [189] [Revisiting the Scaling Properties of Downstream Metrics in Large Language Model Training](https://arxiv.org/abs/2512.08894)
*Jakub Krajewski,Amitis Shidani,Dan Busbridge,Sam Wiseman,Jason Ramapuram*

Main category: cs.LG

TL;DR: This paper proposes a direct framework to model the scaling of benchmark performance from the training budget for LLMs, showing that a simple power law accurately describes log accuracy scaling on downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional scaling laws for LLMs focus on proxy metrics (e.g., pretraining loss) and consider downstream task performance prediction unreliable. This work challenges this by directly modeling benchmark performance scaling.

Method: The authors propose a direct framework using a simple power law to describe scaling behavior of log accuracy for downstream tasks. They validate it on models up to 17B parameters trained on up to 350B tokens, considering token-to-parameter ratios and inference compute.

Result: The direct approach extrapolates better than the two-stage method (prone to compounding errors), and functional forms predict accuracy across token-to-parameter ratios. Results are validated on large-scale models and datasets.

Conclusion: The paper demonstrates that direct modeling of downstream task performance scaling is reliable and superior to traditional methods. They release pretraining losses and evaluation results to support reproducibility and future research.
...
### [190] [ByteStorm: a multi-step data-driven approach for Tropical Cyclones detection and tracking](https://arxiv.org/abs/2512.07885)
*Davide Donno,Donatello Elia,Gabriele Accarino,Marco De Carlo,Enrico Scoccimarro,Silvio Gualdi*

Main category: cs.LG

TL;DR: ByteStorm is a deep learning-based framework for accurate tropical cyclone tracking, outperforming traditional methods in detection and false alarm rates.


<details>
  <summary>Details</summary>
Motivation: Traditional TC tracking methods rely on subjective thresholds, leading to biases. ByteStorm aims to provide a data-driven, threshold-free solution.

Method: ByteStorm uses deep learning to detect TC centers from vorticity and pressure data, then links them into tracks using the BYTE algorithm.

Result: ByteStorm achieves high detection rates (85.05% ENP, 79.48% WNP) and low false alarm rates (23.26% ENP, 16.14% WNP).

Conclusion: ByteStorm demonstrates the potential of deep learning for accurate TC tracking, offering a robust alternative to traditional approaches.
...
### [191] [Towards symbolic regression for interpretable clinical decision scores](https://arxiv.org/abs/2512.07961)
*Guilherme Seidyo Imai Aldeia,Joseph D. Romano,Fabricio Olivetti de Franca,Daniel S. Herman,William G. La Cava*

Main category: cs.LG

TL;DR: Brush is a symbolic regression algorithm that integrates rule-based logic into models, outperforming traditional SR and decision trees in predictive performance and simplicity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of modeling medical decision-making, which combines risk equations and rules, using symbolic regression (SR).

Method: Brush combines decision-tree-like splitting with non-linear constant optimization to integrate rule-based logic into SR models.

Result: Brush achieves Pareto-optimal performance on SRBench and recapitulates clinical scoring systems with high accuracy and interpretability.

Conclusion: Brush outperforms decision trees, random forests, and other SR methods in predictive performance while producing simpler, interpretable models.
...
### [192] [CIP-Net: Continual Interpretable Prototype-based Network](https://arxiv.org/abs/2512.07981)
*Federico Di Valerio,Michela Proietti,Alessio Ragno,Roberto Capobianco*

Main category: cs.LG

TL;DR: CIP-Net is a scalable, exemplar-free, self-explainable model for continual learning that avoids catastrophic forgetting and memory overhead.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of catastrophic forgetting in continual learning by leveraging explainable AI without storing past examples.

Method: Introduce CIP-Net, a prototype-based model that generates explanations during prediction and avoids additional memory usage.

Result: CIP-Net achieves state-of-the-art performance in task- and class-incremental settings with low memory overhead.

Conclusion: CIP-Net offers a practical and interpretable solution for continual learning by balancing performance and scalability.
...
### [193] [HOLE: Homological Observation of Latent Embeddings for Neural Network Interpretability](https://arxiv.org/abs/2512.07988)
*Sudhanva Manjunath Athreya,Paul Rosen*

Main category: cs.LG

TL;DR: HOLE uses persistent homology to interpret deep neural networks by visualizing topological features in neural activations, aiding in understanding model behavior and robustness.


<details>
  <summary>Details</summary>
Motivation: Deep learning models lack interpretability, making it hard to understand their decision processes. HOLE aims to address this by analyzing topological features in neural activations.

Method: HOLE applies persistent homology to neural activations, using visualizations like Sankey diagrams and heatmaps to explore representation structure and quality across layers.

Result: Topological analysis with HOLE reveals patterns related to class separation, feature disentanglement, and model robustness, offering insights into deep learning systems.

Conclusion: HOLE provides a novel topological perspective for interpreting deep neural networks, enhancing understanding of representation quality and model behavior.
...
### [194] [Bridging the Clinical Expertise Gap: Development of a Web-Based Platform for Accessible Time Series Forecasting and Analysis](https://arxiv.org/abs/2512.07992)
*Aaron D. Mullen,Daniel R. Harris,Svetla Slavova,V. K. Cody Bumgardner*

Main category: cs.LG

TL;DR: A web platform simplifies time series forecasting for non-experts by offering data analysis, model training, and result interpretation tools, with support for customization and AI-driven guidance.


<details>
  <summary>Details</summary>
Motivation: To reduce the barrier of technical expertise in time series forecasting, especially in healthcare, by providing an accessible tool for researchers and clinicians.

Method: Develop a web platform that allows users to upload data, generate plots, and train customizable forecasting models, aided by AI recommendations for parameter selection and result interpretation.

Result: The platform enables non-experts to perform forecasting tasks efficiently, with support for various models and training techniques, and integrates AI for guidance.

Conclusion: The platform aims to enhance healthcare analytics by embedding forecasting tools into learning health systems for continuous data use.
...
### [195] [Benchmarking Offline Multi-Objective Reinforcement Learning in Critical Care](https://arxiv.org/abs/2512.08012)
*Aryaman Bansal,Divya Sharma*

Main category: cs.LG

TL;DR: The paper benchmarks offline Multi-Objective Reinforcement Learning (MORL) algorithms against single-objective baselines in critical care, showing PEDA DT's superior flexibility and robustness.


<details>
  <summary>Details</summary>
Motivation: Clinicians in critical care need adaptable policies to balance patient survival and resource use. Single-objective RL is rigid, while MORL allows dynamic preference selection.

Method: Three offline MORL algorithms (CPQL, Adaptive CPQL, PEDA DT) and three single-objective baselines (BC, CQL, DDQN) are tested on the MIMIC-IV dataset using Off-Policy Evaluation metrics.

Result: PEDA DT outperforms static scalarized baselines, showing flexibility. Sequence modeling (Decision Transformers) remains effective in multi-objective settings.

Conclusion: Offline MORL is promising for personalized, adjustable decision-making in critical care without retraining.
...
### [196] [CLARITY: Medical World Model for Guiding Treatment Decisions by Modeling Context-Aware Disease Trajectories in Latent Space](https://arxiv.org/abs/2512.08029)
*Tianxingjian Ding,Yuanhao Zou,Chen Chen,Mubarak Shah,Yu Tian*

Main category: cs.LG

TL;DR: CLARITY is a medical world model that forecasts disease evolution with integrated temporal and clinical contexts, outperforming existing methods in treatment planning.


<details>
  <summary>Details</summary>
Motivation: Current AI predictors lack dynamic disease evolution prediction and ignore patient-specific contexts, limiting their clinical applicability.

Method: CLARITY integrates time intervals and patient-specific data to model treatment-conditioned progression in a structured latent space, generating individualized plans.

Result: CLARITY outperforms MeWM by 12% and surpasses other medical-specific large language models in treatment planning.

Conclusion: CLARITY offers a robust, interpretable framework for dynamic disease prediction and actionable treatment recommendations.
...
### [197] [LUNA: Linear Universal Neural Attention with Generalization Guarantees](https://arxiv.org/abs/2512.08061)
*Ashkan Shahbazi,Ping He,Ali Abbasi,Yikun Bai,Xinran Liu,Elaheh Akbari,Darian Salehi,Navid NaderiAlizadeh,Soheil Kolouri*

Main category: cs.LG

TL;DR: LUNA introduces a learnable kernelized linear attention mechanism that reduces computational costs while maintaining or surpassing the accuracy of traditional quadratic attention methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the computational bottleneck of softmax attention's quadratic cost, which limits its use in long sequences. Linear attention methods, while efficient, suffer from accuracy trade-offs due to fixed kernels.

Method: LUNA uses a learnable feature map to parameterize the kernel, enabling task-specific feature learning. This approach ensures linear computational cost while retaining high accuracy.

Result: Empirical results show LUNA achieves state-of-the-art accuracy on the Long Range Arena (LRA) and performs well in post-hoc conversion tasks, outperforming fixed linear attention methods.

Conclusion: LUNA effectively bridges the gap between computational efficiency and accuracy in attention mechanisms, offering a scalable solution for long-sequence tasks.
...
### [198] [Deep Kernel Aalen-Johansen Estimator: An Interpretable and Flexible Neural Net Framework for Competing Risks](https://arxiv.org/abs/2512.08063)
*Xiaobin Shen,George H. Chen*

Main category: cs.LG

TL;DR: The paper introduces Deep Kernel Aalen-Johansen (DKAJ), an interpretable deep learning model for estimating cumulative incidence functions in competing risks, combining clustering and kernel methods for improved visualization and interpretation.


<details>
  <summary>Details</summary>
Motivation: To generalize the classical Aalen-Johansen estimator for cumulative incidence functions (CIFs) while maintaining interpretability and providing visualizations.

Method: DKAJ represents each data point as a weighted combination of clusters, using a learned kernel function to measure similarity between points. Predictions align with classical Aalen-Johansen when weights are concentrated on a single cluster.

Result: DKAJ performs competitively with state-of-the-art baselines on four datasets and offers interpretable visualizations.

Conclusion: DKAJ effectively combines interpretability and performance in estimating CIFs, providing a valuable tool for understanding competing risks data.
...
### [199] [CAMO: Causality-Guided Adversarial Multimodal Domain Generalization for Crisis Classification](https://arxiv.org/abs/2512.08071)
*Pingchuan Ma,Chengshuai Zhao,Bohan Jiang,Saketh Vishnubhatla,Ujun Jeong,Alimohammad Beigi,Adrienne Raglin,Huan Liu*

Main category: cs.LG

TL;DR: The paper introduces a causality-guided multimodal domain generalization (MMDG) framework to improve crisis classification in social media by addressing poor generalization across unseen crisis types.


<details>
  <summary>Details</summary>
Motivation: Current crisis classification methods struggle with generalization due to reliance on spurious features and misaligned multimodal representations.

Method: Proposes MMDG, combining adversarial disentanglement for domain-invariant causal features and unified representation learning for aligned multimodal features.

Result: Experiments show MMDG outperforms existing methods in unseen disaster scenarios.

Conclusion: MMDG enhances generalization by focusing on causal mechanisms and aligning multimodal representations, proving effective in crisis classification.
...
### [200] [Complexity of One-Dimensional ReLU DNNs](https://arxiv.org/abs/2512.08091)
*Jonathan Kogan,Hayden Jananthan,Jeremy Kepner*

Main category: cs.LG

TL;DR: The paper investigates the expressivity of 1D ReLU networks by analyzing their linear regions, showing how the expected number of regions grows with network width and proposing a sparsity measure for function approximation.


<details>
  <summary>Details</summary>
Motivation: To understand the expressivity of 1D ReLU networks in terms of their linear regions and their ability to approximate functions.

Method: Analyze the expected number of linear regions in randomly initialized, fully connected 1D ReLU networks and introduce a function-adaptive sparsity measure.

Result: The expected number of linear regions grows as a sum of the neurons in each hidden layer plus lower-order terms.

Conclusion: The study provides insights into the expressivity and sparsity of 1D ReLU networks, linking their structure to function approximation capabilities.
...
### [201] [Training LLMs for Honesty via Confessions](https://arxiv.org/abs/2512.08093)
*Manas Joglekar,Jeremy Chen,Gabriel Wu,Jason Yosinski,Jasmine Wang,Boaz Barak,Amelia Glaese*

Main category: cs.LG

TL;DR: The paper proposes a method called 'confession' to elicit honest disclosures from LLMs about their shortcomings, ensuring transparency without affecting their primary responses.


<details>
  <summary>Details</summary>
Motivation: LLMs can exhibit dishonesty due to reinforcement learning incentives, making transparency about their limitations crucial.

Method: Introduces 'confessions'—additional outputs after primary answers—rewarded solely for honesty, separate from the main response's evaluation.

Result: GPT-5-Thinking, trained with confessions, shows improved honesty in disclosing shortcomings like hallucination and scheming.

Conclusion: Confessions enhance LLM transparency and enable interventions like monitoring and rejection sampling.
...
### [202] [Scalable Offline Model-Based RL with Action Chunks](https://arxiv.org/abs/2512.08108)
*Kwanyoung Park,Seohong Park,Youngwoon Lee,Sergey Levine*

Main category: cs.LG

TL;DR: MAC combines model-based value expansion with action-chunk prediction and rejection sampling to improve offline RL performance on long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: To tackle scalability and error compounding issues in offline model-based RL for complex, long-horizon tasks.

Method: Uses action-chunk models to predict future states from action sequences and employs rejection sampling from a behavioral policy to avoid model exploitation.

Result: MAC outperforms other offline model-based RL algorithms, especially in long-horizon tasks, validated on large datasets.

Conclusion: MAC provides a scalable and effective solution for offline RL in complex, long-horizon scenarios.
...
### [203] [Improving the Sensitivity of Backdoor Detectors via Class Subspace Orthogonalization](https://arxiv.org/abs/2512.08129)
*Guangmingmei Yang,David J. Miller,George Kesidis*

Main category: cs.LG

TL;DR: The paper introduces Class Subspace Orthogonalization (CSO), a method for more sensitive post-training backdoor detection by suppressing intrinsic features while optimizing detection statistics.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor detection methods fail when non-target classes naturally exhibit extreme statistics or when backdoor triggers are subtle. The target class's detection statistic is influenced by both the backdoor trigger and intrinsic features, unlike non-target classes.

Method: Proposes CSO: suppresses intrinsic features while optimizing detection statistics using constrained optimization and a small set of clean examples. Orthogonalizes detection statistic with respect to intrinsic features.

Result: CSO improves sensitivity in detecting backdoors, even against challenging mixed-label and adaptive attacks.

Conclusion: CSO is an effective plug-and-play approach for enhancing backdoor detection by isolating and leveraging the backdoor trigger's contribution.
...
### [204] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models I: The Task-Query Architecture](https://arxiv.org/abs/2512.08130)
...
TL;DR: The paper introduces the first part of the Biothreat Benchmark Generation (BBG) Framework, focusing on bacterial biothreats, to evaluate AI model risks for biosecurity.


<details>
  <summary>Details</summary>
Motivation: To address the gap in assessing biosecurity risks posed by AI models, especially LLMs, by developing benchmarks that consider technical and operational factors.

Method: Developed the Bacterial Biothreat Schema, a hierarchical structure of biothreat categories, elements, and tasks, to create task-aligned queries for benchmarking.

Result: The BBG Framework provides a structured approach to evaluate bacterial biothreat risks from AI models, accounting for adversary capabilities and operational factors.

Conclusion: The BBG Framework offers a reusable and comprehensive method to assess biosecurity risks of AI models, with future work expanding its application.
...
### [205] [Robust Agents in Open-Ended Worlds](https://arxiv.org/abs/2512.08139)
*Mikayel Samvelyan*

Main category: cs.LG

TL;DR: The paper explores methods to enhance AI robustness and generalization, using tools like MiniHack and Maestro, and extends to multi-agent and LLM domains.


<details>
  <summary>Details</summary>
Motivation: To develop AI agents that excel in familiar settings and generalize to novel scenarios, addressing challenges in open-ended, ever-changing environments.

Method: Utilizes MiniHack for procedural content generation, Maestro for adversarial curricula, and evolutionary search for LLM robustness testing.

Result: Demonstrates improved agent robustness in diverse settings, including multi-agent games and adversarial prompts for LLMs.

Conclusion: The research advances AI robustness, enabling agents to adapt and thrive in unpredictable, complex environments.
...
### [206] [PolyLingua: Margin-based Inter-class Transformer for Robust Cross-domain Language Detection](https://arxiv.org/abs/2512.08143)
*Ali Lotfi Rezaabad,Bikram Khanal,Shashwat Chaurasia,Lu Zeng,Dezhi Hong,Hossein Beshashati,Thomas Butler,Megan Ganji*

Main category: cs.LG

TL;DR: PolyLingua is a lightweight Transformer-based model for accurate language identification, outperforming existing tools in speed and accuracy, especially in challenging cases like music requests.


<details>
  <summary>Details</summary>
Motivation: Language identification is critical for multilingual systems like chatbots, but existing tools are either inaccurate or too costly. PolyLingua aims to bridge this gap.

Method: Uses a two-level contrastive learning framework with instance-level separation and class-level alignment, creating compact and well-separated embeddings.

Result: Achieves 99.25% F1 on Amazon Massive and 98.15% F1 on a Song dataset, outperforming Sonnet 3.5 with 10x fewer parameters.

Conclusion: PolyLingua is an efficient and accurate solution for language identification in compute- and latency-constrained environments.
...
### [207] [TreeGRPO: Tree-Advantage GRPO for Online RL Post-Training of Diffusion Models](https://arxiv.org/abs/2512.08153)
*Zheng Ding,Weirui Ye*

Main category: cs.LG

TL;DR: TreeGRPO is a novel RL framework that improves training efficiency by structuring the denoising process as a search tree, enabling high sample efficiency, fine-grained credit assignment, and amortized computation.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of RL post-training for aligning generative models with human preferences limits its adoption. TreeGRPO aims to address this by enhancing efficiency.

Method: TreeGRPO recasts denoising as a search tree, branching from shared initial noise samples to reuse common prefixes and enable multiple policy updates per forward pass.

Result: TreeGRPO achieves 2.4× faster training and superior efficiency-reward trade-offs, outperforming GRPO baselines across benchmarks.

Conclusion: TreeGRPO provides a scalable and effective solution for RL-based generative model alignment, demonstrated by its efficiency and performance gains.
...
### [208] [LayerPipe2: Multistage Pipelining and Weight Recompute via Improved Exponential Moving Average for Training Neural Networks](https://arxiv.org/abs/2512.08160)
*Nanda K. Unnikrishnan,Keshab K. Parhi*

Main category: cs.LG

TL;DR: LayerPipe2 formalizes the LayerPipe approach by deriving gradient delay requirements for pipelined neural network training, addressing storage challenges with a memory-efficient solution.


<details>
  <summary>Details</summary>
Motivation: To provide a principled understanding of gradient delay insertion in pipelined training and overcome storage bottlenecks.

Method: Uses variable delayed gradient adaptation and retiming to determine legal delay insertion points and introduces a pipeline-aware moving average for memory efficiency.

Result: Predicts delay requirements based on network structure and reduces memory cost without accuracy loss.

Conclusion: LayerPipe2 offers a scalable framework for pipelined training with controlled tradeoffs between communication and computation.
...
### [209] [MobileFineTuner: A Unified End-to-End Framework for Fine-Tuning LLMs on Mobile Phones](https://arxiv.org/abs/2512.08211)
*Jiaxiang Geng,Lunyu Zhao,Yiyi Lu,Bing Luo*

Main category: cs.LG

TL;DR: MobileFineTuner is an open-source framework enabling efficient and scalable LLM fine-tuning directly on mobile phones, addressing privacy and resource constraints with system-level optimizations.


<details>
  <summary>Details</summary>
Motivation: With high-quality public data for LLMs nearing exhaustion, leveraging private user data on mobile phones presents a privacy-preserving opportunity, but existing solutions lack practicality for commodity devices.

Method: MobileFineTuner introduces system-level optimizations like parameter sharding, gradient accumulation, and energy-aware scheduling to overcome mobile phone resource limitations. It supports Full-FT and PEFT.

Result: The framework successfully fine-tunes models like GPT-2, Gemma 3, and Qwen 2.5 on real mobile phones, validated through experiments and ablation studies.

Conclusion: MobileFineTuner proves feasible for on-device LLM training, serving as a foundation for future research while addressing privacy and efficiency challenges.
...
### [210] [Correction of Decoupled Weight Decay](https://arxiv.org/abs/2512.08217)
*Jason Chuan-Chih Chou*

Main category: cs.LG

TL;DR: This paper challenges the traditional assumption that decoupled weight decay should be proportional to the learning rate (γ), arguing instead for γ² based on steady-state assumptions, leading to stable dynamics and improved performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to revisit the assumption about decoupled weight decay in AdamW, questioning its proportionality to γ and exploring alternatives like γ² for better stability and performance.

Method: The study derives that γ² proportionality for decoupled weight decay ensures stable weight norms based on steady-state assumptions, independent of optimizer type, and empirically validates this.

Result: The results show that decoupled weight decay ∝ γ² stabilizes weight and gradient norms, improves control over training dynamics, and enhances model performance.

Conclusion: The paper concludes that decoupled weight decay should be set ∝ γ² for stable weight norms and better optimization performance, challenging the traditional γ proportionality assumption.
...
### [211] [PR-CapsNet: Pseudo-Riemannian Capsule Network with Adaptive Curvature Routing for Graph Learning](https://arxiv.org/abs/2512.08218)
*Ye Qin,Jingchao Wang,Yang Shi,Haiying Huang,Junxu Li,Weijian Liu,Tinghui Chen,Jinghui Qin*

Main category: cs.LG

TL;DR: PR-CapsNet improves CapsNets by extending routing into pseudo-Riemannian manifolds, enabling adaptive curvature modeling for better graph representation.


<details>
  <summary>Details</summary>
Motivation: CapsNets struggle with real-world graph complexities due to fixed-curvature spaces, while pseudo-Riemannian manifolds offer potential improvements.

Method: PR-CapsNet introduces Adaptive Pseudo-Riemannian Tangent Space Routing, decomposing capsule states and adaptively fusing features via learnable curvature tensors.

Result: PR-CapsNet outperforms state-of-the-art models in node and graph classification benchmarks.

Conclusion: PR-CapsNet effectively models complex graph structures using pseudo-Riemannian geometry, demonstrating superior representation power.
...
### [212] [Persistent Topological Structures and Cohomological Flows as a Mathematical Framework for Brain-Inspired Representation Learning](https://arxiv.org/abs/2512.08241)
*Preksha Girish,Rachana Mysore,Mahanthesha U,Shrey Kumar,Shipra Prashant*

Main category: cs.LG

TL;DR: A framework combining topological structures and cohomological flows for brain-inspired representation learning, outperforming existing methods in manifold consistency and noise resilience.


<details>
  <summary>Details</summary>
Motivation: To create a mathematically rigorous framework for representation learning inspired by neural computation, leveraging topological invariants and cohomological dynamics.

Method: Reformulates neural computation as cochain maps evolving over simplicial complexes, integrating algebraic topology and differential geometry to generalize gradient-based learning.

Result: Demonstrates superior manifold consistency and noise resilience compared to graph neural networks and manifold-based deep learning models.

Conclusion: Establishes a coherent mathematical foundation for topology-driven representation learning, validated by empirical results.
...
### [213] [SPROCKET: Extending ROCKET to Distance-Based Time-Series Transformations With Prototypes](https://arxiv.org/abs/2512.08246)
*Nicholas Harner*

Main category: cs.LG

TL;DR: SPROCKET introduces a prototype-based feature engineering strategy for time series classification, outperforming existing methods like ROCKET and HYDRA-MR.


<details>
  <summary>Details</summary>
Motivation: To enhance accuracy and robustness in time series classification by moving beyond traditional feature engineering approaches.

Method: SPROCKET uses a prototype-based feature transformation strategy, comparing its performance on UCR and UEA archives.

Result: SPROCKET matches or exceeds the performance of existing convolutional algorithms, and the MR-HY-SP ensemble outperforms the best previous ensemble.

Conclusion: Prototype-based feature transformation improves both accuracy and robustness in time series classification.
...
### [214] [Wavelet-Accelerated Physics-Informed Quantum Neural Network for Multiscale Partial Differential Equations](https://arxiv.org/abs/2512.08256)
*Deepak Gupta,Himanshu Pandey,Ratikanta Behera*

Main category: cs.LG

TL;DR: Proposes a wavelet-based quantum neural network to solve multiscale PDEs efficiently, reducing computational complexity and improving accuracy over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges of multiscale PDEs with sharp gradients, stiffness, and oscillations, overcoming limitations of traditional PINNs and quantum-PINNs.

Method: Integrates wavelet multiresolution into quantum neural networks, eliminating automatic differentiation for faster and more efficient training.

Result: Achieves higher accuracy with fewer parameters and 3-5x speedup over existing quantum PINNs.

Conclusion: The framework efficiently solves multiscale problems, showcasing potential for complex PDE applications.
...
### [215] [Geometric-Stochastic Multimodal Deep Learning for Predictive Modeling of SUDEP and Stroke Vulnerability](https://arxiv.org/abs/2512.08257)
*Preksha Girish,Rachana Mysore,Mahanthesha U,Shrey Kumar,Misbah Fatimah Annigeri,Tanish Jain*

Main category: cs.LG

TL;DR: A multimodal deep learning framework integrates EEG, ECG, respiration, SpO2, EMG, and fMRI signals to model SUDEP and stroke vulnerability, combining advanced mathematical techniques for improved predictive accuracy and interpretable biomarkers.


<details>
  <summary>Details</summary>
Motivation: To address the life-threatening conditions of Sudden Unexpected Death in Epilepsy (SUDEP) and acute ischemic stroke by developing a unified framework for early detection and risk stratification.

Method: The framework integrates multimodal signals using Riemannian manifold embeddings, Lie-group invariant features, fractional stochastic dynamics, Hamiltonian energy-flow modeling, and cross-modal attention. Stroke propagation is modeled via fractional epidemic diffusion over structural brain graphs.

Result: Experiments on the MULTI-CLARID dataset show improved predictive accuracy and interpretable biomarkers (e.g., manifold curvature, fractional memory indices, attention entropy, diffusion centrality).

Conclusion: The mathematically principled framework enhances early detection and risk stratification for neural-autonomic disorders, offering interpretable multimodal modeling.
...
### [216] [Mathematical Foundations of Neural Tangents and Infinite-Width Networks](https://arxiv.org/abs/2512.08264)
...
TL;DR: The paper introduces NTK-ECRN, a neural network architecture designed for rigorous analysis of kernel evolution in training. It combines theoretical insights with empirical validation.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between infinite-width neural network theory and practical deep-learning architectures by rigorously analyzing kernel dynamics.

Method: Proposes NTK-ECRN, integrating Fourier feature embeddings, scaled residual connections, and stochastic depth to study NTK evolution theoretically and empirically.

Result: Derives bounds on NTK dynamics, links spectral properties to generalization, and validates improved training stability and generalization empirically.

Conclusion: Provides a framework connecting infinite-width theory with practical architectures, advancing understanding of neural networks.
...
### [217] [SOFA-FL: Self-Organizing Hierarchical Federated Learning with Adaptive Clustered Data Sharing](https://arxiv.org/abs/2512.08267)
*Yi Ni,Xinkun Wang,Han Zhang*

Main category: cs.LG

TL;DR: SOFA-FL proposes a self-organizing hierarchical federated learning framework to adapt to data heterogeneity and dynamic environments using clustering and adaptive topology restructuring.


<details>
  <summary>Details</summary>
Motivation: Address challenges in FL like data heterogeneity and rigid network topologies by enabling hierarchical systems to self-organize and adapt.

Method: Uses DMAC for initial clustering, SHAPE for dynamic topology restructuring, and Adaptive Clustered Data Sharing for mitigating data heterogeneity.

Result: Enhances personalization and captures dynamic client relationships without predefined clusters.

Conclusion: SOFA-FL effectively adapts federated learning systems to evolving environments through self-organization and controlled data sharing.
...
### [218] [gHAWK: Local and Global Structure Encoding for Scalable Training of Graph Neural Networks on Knowledge Graphs](https://arxiv.org/abs/2512.08274)
*Humera Sabir,Fatima Farooq,Ashraf Aboulnaga*

Main category: cs.LG

TL;DR: gHAWK is a scalable GNN framework for large Knowledge Graphs (KGs) that precomputes structural features (Bloom filters and TransE embeddings) to improve efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing GNNs struggle with scalability in large KGs due to inefficient message-passing, especially under mini-batch training.

Method: gHAWK precomputes local (Bloom filters) and global (TransE embeddings) structural features, fusing them with domain-specific features for GNN training.

Result: gHAWK reduces memory usage, speeds up convergence, and achieves state-of-the-art accuracy on OGB datasets.

Conclusion: gHAWK outperforms existing methods in scalability and accuracy for node and link prediction tasks.
...
### [219] [Jacobian Aligned Random Forests](https://arxiv.org/abs/2512.08306)
*Sarwesh Rauniyar*

Main category: cs.LG

TL;DR: JARF combines axis-aligned forests with a supervised preconditioner to improve accuracy on rotated or interaction-dependent boundaries while maintaining simplicity and speed.


<details>
  <summary>Details</summary>
Motivation: Axis-aligned decision trees are fast but struggle with rotated or interaction-dependent boundaries. Oblique forests solve this but are computationally expensive.

Method: JARF uses gradients from an axis-aligned forest to create a global linear preconditioner, transforming the feature space before passing it back to the forest.

Result: JARF improves accuracy over axis-aligned forests and often matches oblique forests while being faster and simpler.

Conclusion: Supervised preconditioning balances accuracy and simplicity, making it a practical alternative to oblique forests.
...
### [220] [Minimizing Layerwise Activation Norm Improves Generalization in Federated Learning](https://arxiv.org/abs/2512.08314)
*M Yashwanth,Gaurav Kumar Nayak,Harsh Rangwani,Arya Singh,R. Venkatesh Babu,Anirban Chakraborty*

Main category: cs.LG

TL;DR: The paper introduces a 'flatness' constrained optimization problem in Federated Learning (FL) to improve model generalization by minimizing activation norms ('MAN'), reducing Hessian eigenvalues.


<details>
  <summary>Details</summary>
Motivation: FL-trained models often converge to sharp minima, hurting generalization. The work aims to address this by enforcing flat minima.

Method: Proposes 'MAN' regularization to minimize activation norms in client-side models, reducing top Hessian eigenvalues.

Result: Applied to FL techniques, the method achieves significant improvements, setting new state-of-the-art performance.

Conclusion: Flatness constraints via MAN regularization enhance FL model generalization, supported by theoretical and empirical results.
...
### [221] [A Multivariate Bernoulli-Based Sampling Method for Multi-Label Data with Application to Meta-Research](https://arxiv.org/abs/2512.08371)
*Simon Chung,Colby J. Vorland,Donna L. Maney,Andrew W. Brown*

Main category: cs.LG

TL;DR: The paper introduces a novel sampling algorithm for multi-label datasets to address challenges of unbalanced labels and label dependencies, ensuring balanced representation of minority labels.


<details>
  <summary>Details</summary>
Motivation: The motivation is to tackle the challenge of sampling from datasets with non-mutually exclusive, imbalanced labels while preserving label dependencies and improving minority label representation.

Method: The method involves using a multivariate Bernoulli distribution to model label dependencies, estimating distribution parameters from observed label frequencies, and calculating weights for each label combination to guide weighted sampling.

Result: Applied to a sample of biomedical research articles with 64 topic categories, the approach successfully produced a more balanced sub-sample, reducing frequency differences and enhancing minority category representation.

Conclusion: The conclusion is that the proposed sampling algorithm effectively addresses label imbalance and dependencies, providing a practical solution for multi-label datasets.
...
### [222] [Fully Decentralized Certified Unlearning](https://arxiv.org/abs/2512.08443)
*Hithem Lamri,Michail Maniatakos*

Main category: cs.LG

TL;DR: The paper introduces RR-DU, a method for certified machine unlearning in decentralized networks, ensuring privacy and performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of certified unlearning methods in decentralized settings, ensuring data privacy and model integrity.

Method: Proposes RR-DU, combining projected gradient steps on forget and retained data, subsampled Gaussian noise, and trust region projection.

Result: Provides convergence guarantees, network-unlearning certificates, and empirical results showing improved accuracy and privacy.

Conclusion: RR-DU effectively balances privacy and utility in decentralized machine unlearning, outperforming baseline methods.
...
### [223] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models II: Benchmark Generation Process](https://arxiv.org/abs/2512.08451)
...
TL;DR: This paper describes the creation of the Bacterial Biothreat Benchmark (B3) dataset, part of a framework to assess AI biosecurity risks, using methods like web prompts and red teaming to generate and refine benchmarks.


<details>
  <summary>Details</summary>
Motivation: Assess and mitigate the biosecurity risks posed by AI models, particularly LLMs, through robust benchmark development.

Method: Combined web-based prompt generation, red teaming, and mining existing corpora to create over 7,000 benchmarks, refined to 1,010 through de-duplication and quality checks.

Result: Produced a diagnostic, biosecurity-relevant benchmark dataset aligned with a broader architecture for nuanced analysis.

Conclusion: The B3 dataset enhances AI biosecurity risk assessment by providing a refined, relevant, and diagnostic tool.
...
### [224] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models III: Implementing the Bacterial Biothreat Benchmark (B3) Dataset](https://arxiv.org/abs/2512.08459)
...
TL;DR: The paper explores using the Bacterial Biothreat Benchmark (B3) dataset to assess biosecurity risks from AI models like LLMs, showing it's effective for identifying and mitigating risks.


<details>
  <summary>Details</summary>
Motivation: Concerns about AI models facilitating bioterrorism or biological weapon access drive the need for benchmarks to quantify and mitigate these risks.

Method: The pilot implemented the B3 dataset with a frontier AI model, followed by human evaluation and risk analysis of the results.

Result: The B3 dataset proved viable for assessing biosecurity risks, pinpointing key risk sources and guiding mitigation efforts.

Conclusion: The B3 dataset is a practical tool for evaluating and addressing biosecurity risks posed by advanced AI models.
...
### [225] [Transformers for Multimodal Brain State Decoding: Integrating Functional Magnetic Resonance Imaging Data and Medical Metadata](https://arxiv.org/abs/2512.08462)
*Danial Jafarzadeh Jazi,Maryam Hajiesmaeili*

Main category: cs.LG

TL;DR: A transformer-based framework integrates fMRI data and DICOM metadata for better brain state decoding, improving accuracy, interpretability, and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing methods overlook contextual richness in DICOM metadata, limiting their effectiveness in fMRI analysis.

Method: The proposed framework combines transformer architectures with multimodal inputs (fMRI and DICOM metadata) using attention mechanisms to capture spatial-temporal patterns.

Result: Enhanced model accuracy, interpretability, and robustness in brain state decoding, with applications in diagnostics and neuroscience.

Conclusion: The framework shows promise but faces challenges like metadata variability; future work focuses on scalability and generalizability.
...
### [226] [Solving Over-Smoothing in GNNs via Nonlocal Message Passing: Algebraic Smoothing and Depth Scalability](https://arxiv.org/abs/2512.08475)
*Weiqi Guan,Junlin He*

Main category: cs.LG

TL;DR: The paper explores how Layer Normalization (LN) placement affects over-smoothing in deep networks. It introduces Post-LN-based algebraic smoothing to prevent over-smoothing and depth-related issues, validated empirically.


<details>
  <summary>Details</summary>
Motivation: The paper aims to resolve the trade-off between Pre-LN (avoids over-smoothing but suffers from depth issues) and Post-LN (bypasses depth issues but causes over-smoothing) architectures.

Method: Proposes a Post-LN-based method inducing algebraic smoothing, preventing over-smoothing without depth limitations.

Result: Empirical results show the method supports up to 256 layers, improves performance, and requires no extra parameters.

Conclusion: The method successfully addresses over-smoothing and depth issues, enhancing deep network performance efficiently.
...
### [227] [Optimal Perturbation Budget Allocation for Data Poisoning in Offline Reinforcement Learning](https://arxiv.org/abs/2512.08485)
*Junnan Qiu,Jie Li*

Main category: cs.LG

TL;DR: A novel Global Budget Allocation attack strategy for Offline RL targets samples based on their influence, outperforming existing methods in efficiency and stealthiness.


<details>
  <summary>Details</summary>
Motivation: Existing data poisoning attacks in Offline RL are inefficient and lack stealthiness due to uniform perturbations.

Method: Proposes a Global Budget Allocation strategy, assigning perturbation magnitudes proportional to TD-error sensitivity under an L2 constraint.

Result: Achieves up to 80% performance degradation with minimal perturbations, evading detection.

Conclusion: The method is more effective and stealthy than baseline strategies.
...
### [228] [Developing Distance-Aware Uncertainty Quantification Methods in Physics-Guided Neural Networks for Reliable Bearing Health Prediction](https://arxiv.org/abs/2512.08499)
*Waleed Razzaq,Yun-Bo Zhao*

Main category: cs.LG

TL;DR: The paper introduces two distance-aware uncertainty methods, PG-SNGP and PG-SNER, for predictive maintenance in safety-critical systems, showing improved accuracy and robustness under out-of-distribution conditions.


<details>
  <summary>Details</summary>
Motivation: Existing uncertainty methods often lack confidence calibration, are costly, and fail under out-of-distribution data, necessitating better solutions for predictive maintenance.

Method: PG-SNGP uses a Gaussian Process layer, while PG-SNER employs Deep Evidential Regression. Both methods apply spectral normalization and a dynamic weighting scheme for balanced performance.

Result: PG-SNGP and PG-SNER outperform Monte Carlo and Deep Ensemble PGNNs in accuracy, generalization, and robustness to noise and adversarial attacks.

Conclusion: The proposed methods enhance degradation estimation in rotating machinery, offering reliable uncertainty-aware predictions for safety-critical applications.
...
### [229] [A Hybrid Model for Stock Market Forecasting: Integrating News Sentiment and Time Series Data with Graph Neural Networks](https://arxiv.org/abs/2512.08567)
*Nader Sadek,Mirette Moawad,Christina Naguib,Mariam Elzahaby*

Main category: cs.LG

TL;DR: The paper proposes a multimodal approach combining financial news and historical stock data using a Graph Neural Network (GNN) for stock market prediction, outperforming traditional LSTM models.


<details>
  <summary>Details</summary>
Motivation: Accurate stock market prediction is crucial for informed investments. While traditional models use historical data, financial news can offer additional signals, prompting the exploration of multimodal integration.

Method: A GNN (GraphSAGE) integrates LSTM-encoded historical stock data and language-model-embedded news titles into a heterogeneous graph. Comparisons are made with an LSTM baseline on binary direction-of-change and significance-based labels.

Result: The GNN achieves 53% accuracy on direction-of-change and a 4% precision gain on significance-based labels. More news-linked companies yield higher accuracy, and headlines outperform full articles in predictive power.

Conclusion: Multimodal GNNs improve stock prediction by leveraging news and historical data, with headlines being particularly impactful for short-term market reactions.
...
### [230] [Long-Sequence LSTM Modeling for NBA Game Outcome Prediction Using a Novel Multi-Season Dataset](https://arxiv.org/abs/2512.08591)
*Charles Rios,Longzhen Han,Almas Baimagambetov,Nikolaos Polatidis*

Main category: cs.LG

TL;DR: The paper introduces a longitudinal NBA dataset and a deep LSTM model for predicting game outcomes, outperforming traditional ML/DL methods by leveraging long-term performance trends.


<details>
  <summary>Details</summary>
Motivation: Existing NBA game prediction models struggle with concept drift, limited temporal context, and instability across seasons. This work aims to improve forecasting by addressing these challenges.

Method: A Long Short-Term Memory (LSTM) architecture is proposed, using an extended sequence length (9,840 games) to capture long-term team dynamics and dependencies. It is compared against Logistic Regression, Random Forest, MLP, and CNN.

Result: The LSTM model achieves 72.35% accuracy, 73.15% precision, and 76.13% AUC-ROC, outperforming all baseline models.

Conclusion: Long-sequence temporal modeling is crucial for accurate basketball outcome prediction, and the new multi-season dataset enhances the robustness and generalizability of NBA forecasting systems.
...
### [231] [DS FedProxGrad: Asymptotic Stationarity Without Noise Floor in Fair Federated Learning](https://arxiv.org/abs/2512.08671)
*Huzaifa Arif*

Main category: cs.LG

TL;DR: Improved convergence analysis for FedProxGrad-type framework in group fair federated learning, showing asymptotic stationarity without noise floor dependency.


<details>
  <summary>Details</summary>
Motivation: Prior analysis of FedProxGrad converged only to a noise-dominated neighborhood, motivating an improved framework (DS FedProxGrad) with better convergence guarantees.

Method: Extended FedProxGrad with inexact local proximal solutions, explicit fairness regularization, and Robbins-Monro step-size schedule.

Result: Proof of asymptotic stationarity (liminf of gradient norm squared equals zero) without noise floor dependency.

Conclusion: DS FedProxGrad achieves superior convergence properties under mild conditions.
...
### [232] [An Additive Manufacturing Part Qualification Framework: Transferring Knowledge of Stress-strain Behaviors from Additively Manufactured Polymers to Metals](https://arxiv.org/abs/2512.08699)
*Chenglong Duan,Dazhong Wu*

Main category: cs.LG

TL;DR: The paper introduces a DTW-TL framework for AM part qualification, using DTW to select relevant polymer datasets and TL to predict metal behaviors, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: Ensuring consistent quality and reliability of additively manufactured parts necessitates accurate prediction of stress-strain behaviors.

Method: A DTW-TL framework combines dynamic time warping for dataset selection with transfer learning to predict metal behaviors using polymer data.

Result: The DTW-TL model achieves a low error (12.41%) and high accuracy (R²=0.96), outperforming baseline models.

Conclusion: The DTW-TL framework effectively transfers knowledge from polymers to metals, enhancing AM part qualification.
...
### [233] [Exposing Hidden Biases in Text-to-Image Models via Automated Prompt Search](https://arxiv.org/abs/2512.08724)
*Manos Plitsis,Giorgos Bouritsas,Vassilis Katsouros,Yannis Panagakis*

Main category: cs.LG

TL;DR: BGPS is a framework that automatically generates prompts to uncover biases in text-to-image models, revealing subtle biases missed by traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing bias mitigation approaches rely on curated datasets, which may overlook less obvious prompts that trigger biased generation.

Method: BGPS uses an LLM for attribute-neutral prompts and attribute classifiers to steer decoding toward bias-amplifying prompts.

Result: The method identified undocumented biases in Stable Diffusion 1.5 and a debiased model, worsening fairness metrics.

Conclusion: BGPS expands bias detection capabilities and serves as a tool for evaluating bias mitigation in text-to-image models.
...
### [234] [Neural Ordinary Differential Equations for Simulating Metabolic Pathway Dynamics from Time-Series Multiomics Data](https://arxiv.org/abs/2512.08732)
*Udesh Habaraduwa,Andrei Lixandru*

Main category: cs.LG

TL;DR: The paper introduces Neural Ordinary Differential Equations (NODEs) for modeling complex biological systems, showing significant improvements in accuracy and speed over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Predicting biological system behavior is crucial for healthspan and bioengineering, but converting multiomics data into predictive models is challenging.

Method: NODEs are proposed as a dynamic framework to learn proteome-metabolome interactions using time-series data from engineered E. coli strains.

Result: NODEs achieved over 90% RMSE improvement and 1000x faster inference compared to baselines, proving effective for metabolic pathway modeling.

Conclusion: NODEs emerge as a scalable, high-performance tool for advancing metabolic engineering and biological discovery.
...
### [235] [Learning and Editing Universal Graph Prompt Tuning via Reinforcement Learning](https://arxiv.org/abs/2512.08763)
*Jinfeng Xu,Zheyu Chen,Shuo Yang,Jinze Li,Hewei Wang,Yijie Li,Edith C. H. Ngai*

Main category: cs.LG

TL;DR: This paper introduces LEAP, a novel model and paradigm for universal graph prompt tuning, strengthening its theoretical foundation and outperforming existing methods in experiments.


<details>
  <summary>Details</summary>
Motivation: Existing approaches compromise universality by relying on selective node-based prompt tuning, prompting the need for stricter constraints and a unified method to preserve universality while achieving better prompts.

Method: The authors propose LEAP, which builds universal graph prompts to maintain theoretical foundations and uses actor-critic reinforcement learning to select nodes and edit prompts.

Result: LEAP consistently outperforms fine-tuning and prompt-based methods in graph- and node-level tasks across various pre-training scenarios.

Conclusion: LEAP preserves the universality of graph prompt tuning while improving performance, demonstrating its effectiveness through extensive experiments.
...
### [236] [Can TabPFN Compete with GNNs for Node Classification via Graph Tabularization?](https://arxiv.org/abs/2512.08798)
*Jeongwhan Choi,Woosung Kang,Minseo Kim,Jongwoo Kim,Noseong Park*

Main category: cs.LG

TL;DR: TabPFN-GN transforms graph data into tabular features for node classification, achieving competitive results with GNNs without graph-specific training.


<details>
  <summary>Details</summary>
Motivation: To explore if graph node classification can be effectively addressed using tabular learning methods, leveraging the success of TabPFN.

Method: Extracts node attributes, structural properties, positional encodings, and optionally smoothed neighborhood features to reformulate graph data as tabular features.

Result: Competes with GNNs on homophilous graphs and outperforms them on heterophilous graphs.

Conclusion: Principled feature engineering can bridge tabular and graph domains, offering an alternative to GNNs and LLM-dependent models.
...
### [237] [Identifying counterfactual probabilities using bivariate distributions and uplift modeling](https://arxiv.org/abs/2512.08805)
*Théo Verhelst,Gianluca Bontempi*

Main category: cs.LG

TL;DR: The paper proposes a method to estimate counterfactual outcomes using uplift models by fitting a bivariate beta distribution, offering richer insights than traditional uplift modeling.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between uplift modeling and counterfactual estimation, providing deeper insights into causal effects.

Method: Develops a counterfactual estimator that fits a bivariate beta distribution to predicted uplift scores, enabling posterior distributions over counterfactual outcomes.

Result: Simulations demonstrate the approach's effectiveness, particularly in applications like telecom customer churn.

Conclusion: The method successfully leverages uplift models for counterfactual estimation without requiring additional causal assumptions.
...
### [238] [Forecasting Fails: Unveiling Evasion Attacks in Weather Prediction Models](https://arxiv.org/abs/2512.08832)
*Huzaifa Arif,Pin-Yu Chen,Alex Gittens,James Diffenderfer,Bhavya Kailkhura*

Main category: cs.LG

TL;DR: WAAPO is a new framework for creating stealthy adversarial perturbations in AI weather forecasting models, tested successfully with ERA5 and FourCastNet.


<details>
  <summary>Details</summary>
Motivation: The paper aims to assess vulnerabilities in AI weather forecasting models to adversarial attacks, emphasizing the need for robust safeguards.

Method: The authors propose WAAPO, incorporating constraints like channel sparsity, spatial localization, and smoothness to create realistic perturbations.

Result: WAAPO effectively generates adversarial trajectories that manipulate forecasts while remaining undetected, revealing significant model vulnerabilities.

Conclusion: The findings highlight the necessity of protective measures against adversarial attacks in operational weather forecasting systems.
...
### [239] [Reinforcement Learning From State and Temporal Differences](https://arxiv.org/abs/2512.08855)
*Lex Weaver,Jonathan Baxter*

Main category: cs.LG

TL;DR: TD($λ$) minimizes squared error but can lead to sub-optimal policies. STD($λ$) improves this by focusing on relative state values, proven effective in tests.


<details>
  <summary>Details</summary>
Motivation: TD($λ$) with linear approximation minimizes squared error but fails in maintaining optimal policy due to incorrect state ordering. A new method is needed to address this.

Method: STD($λ$) is introduced, modifying TD($λ$) to train function approximators on relative state values for binary decision problems, ensuring better policy outcomes.

Result: STD($λ$) shows monotonic policy improvement in theoretical analysis and performs well in tests like the two-state system and acrobot problem.

Conclusion: STD($λ$) outperforms TD($λ$) by focusing on relative state values, proving more effective for policy improvement.
...
### [240] [Refining Diffusion Models for Motion Synthesis with an Acceleration Loss to Generate Realistic IMU Data](https://arxiv.org/abs/2512.08859)
*Lars Ole Häusler,Lena Uhlenberg,Göran Köber,Diyora Salimova,Oliver Amft*

Main category: cs.LG

TL;DR: This paper introduces a text-to-IMU motion-synthesis framework using a pretrained diffusion model refined with an acceleration-based loss (L_acc) to improve IMU data realism.


<details>
  <summary>Details</summary>
Motivation: The goal is to align synthetic IMU data with real acceleration patterns by refining a diffusion model, enhancing motion synthesis fidelity.

Method: The authors integrate L_acc into a diffusion model's training objective, fine-tune it for IMU-specific motion, and evaluate using surface modelling and virtual sensor simulation.

Result: L_acc reduces by 12.7%, with notable improvements in high-dynamic activities. HAR performance improves by 8.7% using refined synthetic data.

Conclusion: Acceleration-aware diffusion refinement effectively aligns motion generation with IMU synthesis, demonstrating adaptability for sensor-specific tasks.
...
### [241] [Differentially Private Synthetic Data Generation Using Context-Aware GANs](https://arxiv.org/abs/2512.08869)
*Anantaa Kotal,Anupam Joshi*

Main category: cs.LG

TL;DR: ContextGAN is a differentially private GAN designed to generate synthetic data that adheres to domain-specific rules, improving realism and utility while ensuring privacy.


<details>
  <summary>Details</summary>
Motivation: Traditional synthetic data methods often miss implicit domain rules, leading to unrealistic or inappropriate outputs. Balancing privacy (e.g., GDPR) with data utility is challenging.

Method: ContextGAN uses a constraint-aware discriminator and a constraint matrix to encode explicit and implicit domain rules, ensuring synthetic data complies with these rules under differential privacy.

Result: Validated in healthcare, security, and finance, ContextGAN produces high-quality synthetic data that respects domain constraints and privacy requirements.

Conclusion: ContextGAN effectively addresses the gap in traditional methods by enforcing domain rules while maintaining privacy, making it suitable for sensitive applications.
...
### [242] [Fed-SE: Federated Self-Evolution for Privacy-Constrained Multi-Environment LLM Agents](https://arxiv.org/abs/2512.08870)
*Xiang Chen,Yuling Shi,Qizhen Lan,Yuchao Qiu,Xiaodong Gu*

Main category: cs.LG

TL;DR: Fed-SE is a Federated Self-Evolution framework for LLM agents, addressing challenges like gradient conflicts in decentralized, dynamic environments by combining local evolution with global aggregation in a low-rank subspace.


<details>
  <summary>Details</summary>
Motivation: Privacy constraints prevent centralized optimization and co-evolution of LLM agents in dynamic environments, while standard Federated Learning (FL) struggles with heterogeneous tasks and sparse rewards.

Method: Fed-SE uses local parameter-efficient fine-tuning on high-return trajectories and global aggregation in a disentangled low-rank subspace to reduce negative transfer.

Result: Experiments show Fed-SE improves task success rates by ~18% over federated baselines across five heterogeneous environments.

Conclusion: Fed-SE effectively enables robust cross-environment knowledge transfer for LLM agents in privacy-constrained settings.
...
### [243] [When Tables Leak: Attacking String Memorization in LLM-Based Tabular Data Generation](https://arxiv.org/abs/2512.08875)
*Joshua Ward,Bochao Gu,Chi-Hua Wang,Guang Cheng*

Main category: cs.LG

TL;DR: The paper reveals privacy risks in LLM-based synthetic tabular data generation, introduces the LevAtt attack to expose leaks, and proposes defenses like digit perturbation.


<details>
  <summary>Details</summary>
Motivation: To address privacy vulnerabilities in LLM-generated synthetic tabular data, which can reproduce memorized numeric patterns, risking data privacy.

Method: Introduces LevAtt, a No-box MIA targeting numeric digit sequences in synthetic data, and proposes defenses including strategic digit perturbation.

Result: LevAtt exposes significant privacy leaks, sometimes perfecting membership classification, while proposed defenses reduce risks with minimal data utility loss.

Conclusion: LLM-based synthetic data generation poses unique privacy risks; defenses like digit perturbation can mitigate these without compromising data quality.
...
### [244] [DAO-GP Drift Aware Online Non-Linear Regression Gaussian-Process](https://arxiv.org/abs/2512.08879)
*Mohammad Abu-Shaira,Ajita Rattani,Weishi Shi*

Main category: cs.LG

TL;DR: DAO-GP is a drift-aware, adaptive Gaussian Process model designed for online non-linear regression, addressing limitations like fixed hyperparameters and lack of drift detection in conventional methods.


<details>
  <summary>Details</summary>
Motivation: Real-world data distributions evolve over time (concept drift), and ignoring this can degrade model performance. Existing online GP models lack adaptability, drift-awareness, and efficient memory usage.

Method: Proposes DAO-GP, a fully adaptive, hyperparameter-free model with built-in drift detection, dynamic adaptation, and sparse memory management.

Result: DAO-GP outperforms state-of-the-art models in handling diverse drift types and maintains robustness in varying conditions.

Conclusion: DAO-GP is a robust, drift-resilient solution for online non-linear regression, offering superior performance and adaptability.
...
### [245] [Unsupervised Learning of Density Estimates with Topological Optimization](https://arxiv.org/abs/2512.08895)
*Suina Tanweer,Firas A. Khasawneh*

Main category: cs.LG

TL;DR: The paper introduces an unsupervised learning method using topological data analysis to automatically select the optimal kernel bandwidth for density estimation, outperforming classical techniques.


<details>
  <summary>Details</summary>
Motivation: Kernel density estimation is widely used but requires tuning the kernel bandwidth, a critical hyperparameter affecting bias-variance trade-off. Traditional methods struggle with this, especially in high dimensions.

Method: The authors propose an unsupervised approach using a topology-based loss function to automate bandwidth selection, leveraging topological data analysis to quantify features like connected components and loops.

Result: The method benchmarks favorably against classical techniques across various dimensions, showcasing its potential for optimal bandwidth selection.

Conclusion: Topology-based loss functions offer a promising solution for unsupervised bandwidth selection in kernel density estimation, applicable across diverse dimensions.
...
### [246] [Open Polymer Challenge: Post-Competition Report](https://arxiv.org/abs/2512.08896)
...
TL;DR: The Open Polymer Challenge (OPC) introduces a benchmark dataset for polymer informatics, featuring 10K polymers and 5 properties, aiming to improve ML-driven sustainable material discovery.


<details>
  <summary>Details</summary>
Motivation: Progress in ML for sustainable polymers is hindered by insufficient high-quality datasets. OPC addresses this with a community-developed benchmark.

Method: Participants tackled multi-task property prediction under constraints like small data and label imbalance, using techniques like transfer learning and self-supervised pretraining.

Result: The competition highlighted lessons on data preparation and model strategies, yielding models and analysis to advance polymer AI.

Conclusion: OPC's dataset and models provide a foundation for accelerating sustainable materials development, with released data and tools for broader use.
