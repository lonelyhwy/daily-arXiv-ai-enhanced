...
TL;DR: VLMMs perform well on scrambled videos, suggesting reliance on prior knowledge over sequential processing. VECTOR benchmark reveals their poor temporal order understanding. MECOT improves this via detailed training and chain-of-thought prompts, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Current VLMMs excel in video understanding but lack exploration of temporal event order capture. Existing benchmarks don't reveal this gap, as models perform well even on scrambled frames.

Method: Proposed VECTOR benchmark assesses temporal order understanding. Introduced MECOT: trains on event-by-event descriptions and uses chain-of-thought prompts to enhance temporal awareness.

Result: VLMMs struggle with temporal order in VECTOR. MECOT improves performance on VECTOR and existing benchmarks, demonstrating better temporal understanding.

Conclusion: VLMMs' reliance on prior knowledge hides temporal understanding gaps. MECOT addresses this, showing promise for enhancing event order comprehension.
...
TL;DR: The paper introduces IMAgent, an open-source vision agent trained via reinforcement learning for multi-image QA tasks, outperforming single-image methods and enhancing visual attention through specialized tools.


<details>
  <summary>Details</summary>
Motivation: Existing open-source vision-language models (VLMs) are limited to single-image inputs, failing to handle real-world multi-image QA tasks. The goal is to bridge this gap with a more robust solution.

Method: Proposes IMAgent, trained via end-to-end RL, using a multi-agent system to generate complex multi-image QA pairs (MIFG-QA). Introduces tools for visual reflection and confirmation to ensure attention to images during inference, supported by a two-level mask strategy.

Result: IMAgent performs well on single-image benchmarks and significantly improves on multi-image tasks. The approach avoids costly supervised fine-tuning.

Conclusion: IMAgent advances multi-image QA capabilities through RL training and specialized tools, offering insights for future research. Code and data will be released.
...
TL;DR: The paper proposes UTIE, a method to reduce demographic bias in face recognition by enriching embeddings with cross-demographic text features.


<details>
  <summary>Details</summary>
Motivation: Face recognition systems often exhibit demographic biases due to entanglement of demographic and identity features in embeddings, leading to unfair performance across groups.

Method: UTIE introduces demographic ambiguity into facial embeddings by leveraging VLMs to incorporate text-derived demographic features from other groups, promoting neutral representations.

Result: UTIE reduces bias metrics and maintains or improves verification accuracy across benchmarks (RFW, BFW) when tested with CLIP, OpenCLIP, and SigLIP.

Conclusion: The study demonstrates that UTIE effectively mitigates demographic bias in face recognition while preserving or enhancing system performance.
...
TL;DR: Consist-Retinex adapts consistency modeling for Retinex-based low-light image enhancement, achieving state-of-the-art performance with one-step sampling and reduced training costs.


<details>
  <summary>Details</summary>
Motivation: Diffusion models excel in low-light enhancement but are impractical due to iterative sampling. Consistency models show promise for unconditional synthesis but haven't been explored for conditional enhancement.

Method: Introduces Consist-Retinex with dual-objective consistency loss and adaptive noise-emphasized sampling to train on large-noise regions critical for conditional enhancement.

Result: Achieves PSNR 25.51 and FID 44.73 on VE-LOL-L, outperforming Diff-Retinex++ with only 1/8 training budget.

Conclusion: Consist-Retinex bridges the gap between consistency models and conditional enhancement, enabling efficient one-step generation.
...
TL;DR: The paper introduces HSCP, a pruning framework combining layer and channel pruning to reduce model size and computational demands while maintaining accuracy for UAV identification.


<details>
  <summary>Details</summary>
Motivation: Traditional UAV identification methods struggle in complex environments, and existing deep learning RFFI approaches are too resource-intensive for edge devices.

Method: HSCP uses spectral clustering guided by CKA for layer and channel pruning, followed by noise-robust fine-tuning.

Result: On ResNet18, HSCP achieves 86.39% parameter reduction, 84.44% FLOPs reduction, and 1.49% accuracy improvement.

Conclusion: HSCP outperforms existing pruning methods, offering extreme compression, high performance, and robust inference.
...
TL;DR: RAG-HAR is a training-free framework leveraging LLMs for Human Activity Recognition, achieving top performance across benchmarks without model training.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for HAR require dataset-specific training and large labeled data, posing practical challenges. RAG-HAR aims to overcome these limitations.

Method: RAG-HAR uses lightweight statistical descriptors, retrieves similar samples from a vector database, and employs LLMs for activity identification. It enhances performance via prompt optimization and context-enriched descriptors.

Result: RAG-HAR achieves state-of-the-art performance on six HAR benchmarks without training or fine-tuning, demonstrating robustness and applicability.

Conclusion: RAG-HAR offers a practical, training-free solution for HAR, extending to recognizing unseen activities, highlighting its versatility and efficiency.
...
TL;DR: The paper introduces the Verifier-Threshold method to optimize test-time compute allocation for image generation models, achieving 2-4x speedup over current methods on the GenEval benchmark.


<details>
  <summary>Details</summary>
Motivation: Current methods inefficiently allocate non-uniform inference-compute budgets across denoising steps in image generation models, relying on greedy algorithms.

Method: The authors propose the Verifier-Threshold method to automatically reallocate test-time compute for diffusion and flow models.

Result: Their method achieves a 2-4x reduction in computational time while maintaining performance on the GenEval benchmark.

Conclusion: The Verifier-Threshold method effectively improves efficiency in test-time compute allocation for image generation models.
...
TL;DR: Proposed a quality-control framework for diabetic retinopathy datasets using explainable AI and deep learning to ensure high-quality annotations for AI training.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis of diabetic retinopathy is crucial to prevent vision loss, and AI can assist clinicians, but requires high-quality annotated datasets, which are prone to errors due to complex retinal structures.

Method: Introduced a framework with an explainable feature-based classifier to filter inadequate images, enhanced images using deep learning, and calculated annotator agreement for usability.

Result: The framework ensures only high-standard data is used for evaluation and AI training, improving reliability and reducing manual workload.

Conclusion: The proposed framework enhances the quality of diabetic retinopathy datasets, supporting more accurate AI models for clinical use.
...
TL;DR: The paper introduces a 3D Inverse Design (3DID) framework that addresses limitations of current methods by directly navigating the 3D design space using a continuous latent representation and physics-aware optimization.


<details>
  <summary>Details</summary>
Motivation: Current inverse design methods in 3D domains often rely on 2D projections or fine-tuned shapes, sacrificing volumetric detail and limiting design exploration.

Method: 3DID couples a continuous latent representation with a two-stage physics-aware optimization: gradient-guided diffusion sampling followed by objective-driven refinement.

Result: 3DID generates high-fidelity 3D geometries, outperforming existing methods in solution quality and design versatility.

Conclusion: The proposed framework enables true 3D design from scratch, overcoming limitations of current approaches.
...
TL;DR: A Deterministic World Model (DWM) is proposed to eliminate latent variables in vision-based control verification, improving precision and outperforming latent-variable baselines.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of verifying vision-based control systems by reducing overapproximation errors caused by stochastic latent variables in generative models.

Method: Develops a DWM that maps system states directly to images, trained with a dual-objective loss for accuracy and behavioral consistency. Integrates DWM with StarV and conformal prediction for rigorous verification.

Result: Experiments show tighter reachable sets and better verification performance compared to latent-variable baselines.

Conclusion: DWM effectively eliminates uninterpretable latent variables, enhancing precision in vision-based control system verification.
...
TL;DR: A novel generative model predicts 3D dose distributions using customizable preferences, outperforming traditional methods like Varian RapidPlan in flexibility and quality.


<details>
  <summary>Details</summary>
Motivation: To address biases in deep learning-based 3D dose prediction caused by reliance on reference plans and institutional preferences.

Method: Introduces a generative model that uses user-defined preference flavors to prioritize trade-offs between OARs and PTVs.

Result: Outperforms Varian RapidPlan in adaptability and plan quality in certain scenarios.

Conclusion: The approach offers a flexible, personalized solution for radiotherapy planning, integrating well with clinical systems.
...
TL;DR: A novel framework using diffusion models and implicit neural representation is proposed to effectively reduce metal artifacts in CT images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing supervised MAR methods lack stability due to limited paired data, while unsupervised methods fail to fully integrate CT physical geometry and prior knowledge.

Method: Combines implicit neural representation for physical constraints and data fidelity with a pre-trained diffusion model to regularize the solution.

Result: Demonstrated effectiveness and generalization on simulated and clinical data, showing potential for clinical use.

Conclusion: The proposed framework addresses limitations of existing MAR methods, offering a promising solution for clinical applications.
...
TL;DR: The study proposes a method to generate high-quality, physically valid defect datasets for AI training in micro/nano manufacturing, achieving superior results with Mask R-CNN over Faster R-CNN.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality, physically grounded training data for defect inspection in AI-driven micro/nano manufacturing, particularly in lithography, hinders progress. Publicly available datasets are rare due to restricted access to semiconductor industry data.

Method: The framework synthesizes defect layouts using physics-constrained mathematical morphology operations, fabricates physical samples via DMD-based lithography, and compares optical micrographs to create pixel-level annotations.

Result: A dataset of 3,530 micrographs with 13,365 annotated defects (bridge, burr, pinch, contamination) was created. Mask R-CNN outperformed Faster R-CNN by ~34% (AP@0.5) for most classes and ~42% for contamination.

Conclusion: The methodology successfully generates high-quality defect datasets with pixel-level annotations, proving effective for robust AI-based inspection in semiconductor fabrication.
...
TL;DR: This survey reviews body and face motion generation, covering concepts, techniques, datasets, and metrics, aiming to enhance avatar realism and expressiveness.


<details>
  <summary>Details</summary>
Motivation: Advancing generative modeling and multi-modal learning to create expressive and coherent face and body dynamics for communication.

Method: Review of core concepts, representation techniques, generative approaches, datasets, and evaluation metrics.

Result: Identifies challenges and gaps in generating realistic and expressive avatars.

Conclusion: Future directions focus on improving realism, coherence, and expressiveness in dyadic settings.
...
TL;DR: LUVC is a framework for compressing visual tokens in VLMs, addressing redundancy and inefficiency without significant accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Visual language models face inefficiency due to redundancy in token representations and limitations of current compression methods.

Method: LUVC uses iterative merging in the visual encoder and integrates a pruning unit in LLM via a low-pass filter, compatible with FlashAttention.

Result: LUVC achieves 2x faster inference with negligible accuracy loss and is training-free for easy deployment.

Conclusion: LUVC effectively compresses visual tokens while maintaining accuracy, enabling efficient deployment across VLMs.
...
TL;DR: The paper presents a deep learning approach using neural networks to detect characters in video sequences, with applications in security systems, particularly in Angola.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve character detection in videos, focusing on enhancing national security systems by efficiently locating target individuals from databases.

Method: The method involves supervised learning algorithms structured for Computer Vision, utilizing simple characteristics of the target character for detection.

Result: The results show that the proposed approach efficiently locates individuals in video sequences, outperforming state-of-the-art techniques.

Conclusion: The study concludes that the new approach can reinforce security systems, such as Angola's, by leveraging video sequences and target databases for detection.
...
TL;DR: E-Deflare is a framework for removing lens flare from event camera data, featuring a physics-grounded model, benchmark datasets, and E-DeflareNet for state-of-the-art restoration.


<details>
  <summary>Details</summary>
Motivation: Event cameras' high temporal resolution and dynamic range are compromised by lens flare, a spatio-temporal distortion often overlooked.

Method: Derives a physics-grounded forward model, creates benchmark datasets (simulated and real-world), and designs E-DeflareNet.

Result: Achieves state-of-the-art restoration performance, validated by extensive experiments and benefiting downstream tasks.

Conclusion: E-Deflare effectively addresses lens flare in event camera data, with publicly available code and datasets.
...
TL;DR: ConceptPose introduces a training-free, model-free framework for object pose estimation using vision-language models to create open-vocabulary 3D concept maps.


<details>
  <summary>Details</summary>
Motivation: Existing object pose estimation methods require extensive training, while vision-language models offer strong zero-shot capabilities. ConceptPose aims to bridge this gap.

Method: The framework leverages vision-language models to generate 3D concept maps tagged with concept vectors derived from saliency maps, enabling robust 3D-3D correspondences for 6DoF pose estimation.

Result: ConceptPose achieves state-of-the-art performance on zero-shot benchmarks, outperforming existing methods by over 62% in ADD(-S) score without any dataset-specific training.

Conclusion: ConceptPose demonstrates the potential of leveraging vision-language models for accurate, training-free object pose estimation.
...
TL;DR: SIP is a LiDAR dataset designed for construction sites, addressing real-world challenges like sparse data and fragmented geometry, with annotations tailored for construction environments.


<details>
  <summary>Details</summary>
Motivation: Existing datasets don't reflect real construction site conditions—SIP fills this gap by providing realistic LiDAR data for better 3D perception benchmarking.

Method: Collected using terrestrial LiDAR, SIP includes indoor/outdoor scenes with point-level annotations for construction-specific categories, ensuring consistency through scanning and quality control protocols.

Result: SIP offers structured data with challenging scenarios (e.g., occlusions, sparse objects) and supports adaptable configurations for 3D deep learning frameworks.

Conclusion: SIP advances construction-oriented 3D vision by providing realistic, annotated LiDAR data, facilitating robust benchmarking and adoption in practical applications.
...
TL;DR: A knowledge distillation framework, KD-OCT, compresses ConvNeXtV2-Large into EfficientNet-B2 for AMD classification, balancing efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: To overcome computational barriers of deep learning models like ConvNeXtV2-Large in clinical AMD detection.

Method: KD-OCT uses real-time distillation with combined loss for normal, drusen, and CNV cases on OCT scans.

Result: Achieves near-teacher performance with reduced size and inference time on NEH dataset.

Conclusion: KD-OCT enables efficient edge deployment for AMD screening while maintaining high accuracy.
...
TL;DR: The paper proposes an automatic threshold selection method for visual place recognition (VPR) by analyzing 'negative' Gaussian mixture statistics, improving performance across diverse visual scenarios.


<details>
  <summary>Details</summary>
Motivation: Current VPR implementations rely on manually set thresholds, which are impractical for varying visual conditions like seasonal changes and illumination.

Method: The approach uses 'negative' Gaussian mixture statistics to determine thresholds automatically, eliminating the need for manual tuning.

Result: The method effectively selects thresholds that perform well across different image databases and descriptors.

Conclusion: Automating threshold selection enhances VPR robustness, making it more adaptable to diverse real-world conditions.
...
TL;DR: AgentComp improves text-to-image models' compositionality by training them to differentiate between similar prompts using agentic preference optimization and autonomously constructed datasets.


<details>
  <summary>Details</summary>
Motivation: Existing models struggle with accurately capturing object relationships, attribute bindings, and fine-grained details in prompts, often producing outputs that deviate subtly from the intended description.

Method: AgentComp leverages LLMs with image tools to autonomously build compositional datasets and applies agentic preference optimization to fine-tune models for better differentiation of compositional variations.

Result: AgentComp achieves state-of-the-art performance on compositionality benchmarks like T2I-CompBench without compromising image quality and shows generalization to other capabilities like text rendering.

Conclusion: AgentComp effectively enhances text-to-image models' compositional generation ability and reasoning, setting a new standard for such tasks.
...
TL;DR: MDSE is a vision-language framework generating detailed textual explanations for post-disaster underground scenes, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Addressing obscured vision and situational awareness challenges in underground mining disasters.

Method: Uses Context-Aware Cross-Attention, Segmentation-aware dual pathway encoding, and Resource-Efficient Transformer-Based Language Model.

Result: MDSE outperforms existing captioning models, providing accurate descriptions in obscured environments.

Conclusion: MDSE enhances situational awareness for underground emergency response.
...
TL;DR: FoCULR improves generative models' understanding of multi-noun food categories by incorporating domain knowledge and refining spatial layouts, leading to more accurate image generation.


<details>
  <summary>Details</summary>
Motivation: Generative models struggle with multi-noun food categories, often misinterpreting prompts and producing incorrect ingredients or layouts due to insufficient domain knowledge.

Method: Proposed FoCULR integrates food domain knowledge and introduces core concepts early in the generation process to correctly interpret multi-noun relationships.

Result: Experiments show FoCULR enhances image generation performance for multi-noun food categories.

Conclusion: FoCULR effectively addresses challenges in generating realistic food images by refining understanding and layouts of multi-noun categories.
...
TL;DR: GimbalDiffusion introduces a framework for precise camera control in text-to-video generation by using physical-world coordinates and gravity as reference, enhancing controllability and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack fine-grained control over camera motion and orientation, relying on ambiguous or relative representations.

Method: The framework uses absolute coordinates, panoramic videos for diverse trajectories, and null-pitch conditioning to align camera specifications with text content.

Result: Enables precise, interpretable camera control and robustness under wide pitch variations.

Conclusion: GimbalDiffusion advances the controllability of text-to-video models, allowing gravity-aligned camera manipulation.
...
TL;DR: SuperF is a test-time optimization approach for multi-image super-resolution (MISR) using coordinate-based neural networks (neural fields). It avoids reliance on high-resolution training data and jointly optimizes frame alignment with an implicit neural representation.


<details>
  <summary>Details</summary>
Motivation: The limitations of sensor technology, atmospheric conditions, and costs hinder high-resolution imagery. Single-image super-resolution methods often produce unrealistic results. MISR improves resolution using multiple views, but existing approaches lack flexibility and rely on training data.

Method: SuperF employs coordinate-based neural networks (INR) to represent continuous signals. It shares an INR for multiple low-resolution frames and jointly optimizes frame alignment via affine transformations and a super-sampled coordinate grid.

Result: SuperF achieves compelling results on simulated satellite bursts and handheld camera images, with upsampling factors up to 8x, without high-resolution training data.

Conclusion: SuperF offers a flexible, training-free approach for MISR by leveraging neural fields and joint optimization of alignment, outperforming related baselines.
...
TL;DR: AngioAI-QFR is a fully automated pipeline combining deep learning for stenosis detection, lumen segmentation, and functional profiling, offering accurate and near real-time angiography-derived QFR with strong correlation to invasive FFR.


<details>
  <summary>Details</summary>
Motivation: Current coronary angiography tools for assessing stenosis are variable and workflow-intensive, lacking integration with automated anatomy analysis and virtual PCI planning. A wire-free, unified solution is needed.

Method: Developed AngioAI-QFR, an end-to-end pipeline using deep learning for stenosis detection, lumen segmentation, and automated QFR computation. Evaluated on 100 vessels with invasive FFR as reference.

Result: Strong correlation with FFR (r=0.89, MAE 0.045), high diagnostic performance (AUC 0.93), and fast median time to result (41s). Automated success in 93% of cases.

Conclusion: AngioAI-QFR is a practical, efficient tool unifying computer vision, functional profiling, and virtual PCI, providing accurate angiography-derived physiology in near real-time.
...
TL;DR: The paper proposes combining Gaussian Splatting's accuracy with UV texture mapping's intuitiveness for editable photorealistic head avatars, enabling relighting and appearance modifications without extra optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional Gaussian Splatting lacks intuitive editability, unlike triangle mesh-based methods. The authors aim to bridge this gap by integrating 2D Gaussian Splatting with UV texture mapping for better control over avatar appearance and geometry.

Method: The method embeds Gaussian primitives into UV space patches of a template mesh efficiently, reconstructing editable material textures from monocular video. It uses a physically based reflectance model for relighting and editing intrinsic material maps.

Result: The approach achieves accurate reconstructions, high-quality relighting, and intuitive appearance/geometry modifications via texture mapping, outperforming state-of-the-art methods.

Conclusion: The proposed method successfully combines accuracy and editability, offering practical applications in visual effects, videoconferencing, and virtual reality.
...
TL;DR: WonderZoom introduces a novel method for generating multi-scale 3D scenes from a single image, addressing limitations of existing single-scale models.


<details>
  <summary>Details</summary>
Motivation: Existing 3D world generation models are limited to single-scale synthesis, lacking coherent scene contents across varying granularities.

Method: WonderZoom uses scale-adaptive Gaussian surfels for multi-scale rendering and a progressive detail synthesizer for iterative refinement.

Result: The approach outperforms state-of-the-art models in quality and alignment, enabling multi-scale 3D world creation.

Conclusion: WonderZoom successfully generates detailed multi-scale 3D scenes from a single image, demonstrated through video and interactive results.
...
TL;DR: PromptCCZSL is a novel framework for Continual Compositional Zero-Shot Learning (CCZSL) that prevents forgetting of prior knowledge and adapts to new compositions using multimodal prompts and distillation techniques.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of adapting vision-language models to new attributes, objects, and their compositions in CCZSL without forgetting prior knowledge, which is more complex due to overlapping attributes and objects across sessions.

Method: The proposed PromptCCZSL uses recency-weighted multi-teacher distillation, session-aware compositional prompts, and session-agnostic fusion for attributes and objects. It employs Cosine Anchor Loss (CAL) for stability, Orthogonal Projection Loss (OPL) for distinct embeddings, and Intra-Session Diversity Loss (IDL) for richer representations.

Result: Experiments on UT-Zappos and C-GQA benchmarks show PromptCCZSL outperforms prior methods, achieving significant improvements in CCZSL tasks.

Conclusion: PromptCCZSL sets a new benchmark for CCZSL by effectively balancing adaptation to new compositions and retention of prior knowledge, demonstrating superior performance in closed-world settings.
...
TL;DR: A framework called Δ-LFM is introduced to model disease progression using Flow Matching and patient-specific latent alignment, ensuring continuous, monotonic, and interpretable dynamics in latent space.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing generative approaches in modeling disease progression, such as scattered latent representations and disrupted continuity, by leveraging Flow Matching and enforcing semantic structure in latent space.

Method: The proposed Δ-LFM framework treats disease dynamics as a velocity field, uses Flow Matching to align temporal evolution, and introduces patient-specific latent alignment to ensure trajectories lie along a specific axis correlated with clinical severity.

Result: Δ-LFM shows strong performance on three longitudinal MRI benchmarks and provides a new way to interpret and visualize disease dynamics.

Conclusion: The conclusion highlights that Δ-LFM offers a consistent and semantically meaningful latent space for modeling disease progression, improving interpretability and alignment with clinical indicators.
...
TL;DR: The paper introduces a new VLM x SI paradigm for 3D visual grounding, proposing View-on-Graph (VoG) to externalize 3D spatial information into a scene graph, improving reasoning and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot approaches entangle visual representations, making it hard to exploit spatial semantic relationships effectively. The goal is to simplify VLM reasoning and enhance interpretability.

Method: Proposes VoG, a method organizing the scene into a multi-modal, multi-layer scene graph, allowing the VLM to selectively access cues during reasoning.

Result: VoG achieves state-of-the-art zero-shot performance and offers transparent, step-by-step reasoning traces.

Conclusion: Structured scene exploration via VoG is a promising strategy for advancing zero-shot 3D visual grounding.
...
TL;DR: The paper introduces the FCM standard by MPEG-AI, enabling efficient extraction and transmission of neural network features for AI applications, reducing bitrate by 75.90% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The increasing need for intelligent consumer devices requires efficient data transfer solutions for AI-driven tasks, especially for low-powered devices.

Method: The paper describes the FCM standard, which offloads computationally intensive operations to high-resource servers, allowing efficient feature extraction and transmission.

Result: Experiments show FCM reduces bitrate by 75.90% compared to remote inference while preserving accuracy.

Conclusion: FCM is a promising solution for AI applications, balancing efficiency and performance for low-powered devices.
...
TL;DR: The paper introduces Z-score normalization for efficient recovery of compressed feature data in AI models, reducing bitrate and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: The split-inference paradigm requires transferring intermediate feature data, making compression crucial for efficiency.

Method: Z-score normalization is used to recover compressed feature data, integrated into the FCM codec standard, and a simplified method is proposed for further overhead reduction.

Result: 17.09% average bitrate reduction across tasks, up to 65.69% for object tracking, without accuracy loss.

Conclusion: The proposed method outperforms existing scaling techniques in the FCM codec, reducing overhead and enhancing task accuracy.
...
TL;DR: OmniPSD is a diffusion framework for generating and decomposing layered PSD files with transparency, using spatial attention and iterative editing.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in generating or reconstructing layered PSD files with transparent alpha channels, despite advancements in diffusion models.

Method: OmniPSD uses spatial attention for text-to-PSD generation and iterative in-context editing for image-to-PSD decomposition, aided by an RGBA-VAE.

Result: Experiments show OmniPSD achieves high-fidelity generation, structural consistency, and transparency awareness.

Conclusion: OmniPSD offers a new paradigm for layered design generation and decomposition using diffusion transformers.
...
TL;DR: GLACIA integrates large language models with segmentation to improve glacial lake monitoring by providing accurate masks and spatial reasoning, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of high-level global semantics and human-interpretable reasoning in current glacial lake segmentation methods.

Method: Introduces GLACIA, combining large language models with segmentation, and constructs the GLake-Pos dataset for spatial reasoning.

Result: GLACIA achieves superior performance (mIoU: 87.30) over CNNs, ViTs, and other methods.

Conclusion: GLACIA enhances disaster preparedness and policy-making through interpretable natural language interaction.
...
TL;DR: Concise reasoning with fewer visual tokens improves efficiency in video reasoning, achieving competitive performance without lengthy chains or manual annotations.


<details>
  <summary>Details</summary>
Motivation: Current video reasoning models rely on lengthy reasoning chains and many visual tokens. Empirical observations suggest concise reasoning and fewer tokens may suffice.

Method: Developed a post-training and inference framework for video MLLMs, enabling operation on compressed visual tokens and brief reasoning traces.

Result: Improved inference efficiency and competitive performance across benchmarks, without needing manual CoT annotations or fine-tuning.

Conclusion: Concise reasoning is effective and efficient for general video reasoning, reducing reliance on lengthy, human-like CoT chains.
...
TL;DR: ROI-Packing is an image compression method for machine vision, focusing on regions of interest to achieve high efficiency without retraining models, outperforming VVC in bitrate reduction and accuracy.


<details>
  <summary>Details</summary>
Motivation: To enhance image compression for machine vision by prioritizing regions critical to task accuracy, avoiding the need for model retraining.

Method: ROI-Packing efficiently compresses images by packing regions of interest and discarding less relevant data.

Result: Achieves up to 44.10% bitrate reduction with no accuracy loss and 8.88% accuracy improvement at the same bitrate compared to VVC.

Conclusion: ROI-Packing is a highly effective compression method for machine vision, surpassing state-of-the-art codecs in efficiency and accuracy.
...
TL;DR: MedForget introduces a hierarchy-aware testbed to evaluate unlearning methods in medical AI, addressing privacy concerns under HIPAA/GDPR.


<details>
  <summary>Details</summary>
Motivation: Privacy regulations require AI models to forget sensitive patient data, but current unlearning methods lack effectiveness in medical contexts.

Method: MedForget uses a nested hierarchy (Institution -> Patient -> Study -> Section) with 3840 multimodal instances to test unlearning across eight levels.

Result: Existing methods fail to achieve complete hierarchy-aware forgetting without degrading performance; fine-grained unlearning leaves models vulnerable to reconstruction attacks.

Conclusion: MedForget offers a HIPAA-aligned framework for developing compliant medical AI systems.
...
TL;DR: The paper proposes MoRel, a novel 4DGS framework using ARBB mechanism for memory-efficient, temporally consistent modeling of long-range dynamic scenes, addressing issues like memory explosion and flickering.


<details>
  <summary>Details</summary>
Motivation: Existing 4DGS methods struggle with long-range dynamic scenes due to memory explosion, temporal flickering, and occlusion handling. The paper aims to overcome these limitations.

Method: MoRel introduces ARBB for bidirectional blending of deformations between key-frame anchors (KfA) and FHD for hierarchical densification based on feature-variance.

Result: MoRel achieves flicker-free, coherent long-range 4D reconstruction with bounded memory usage, validated on the new SelfCap$_{	ext{LR}}$ dataset.

Conclusion: The proposed MoRel framework is scalable and efficient for dynamic Gaussian-based representations, addressing key challenges in long-range 4D motion modeling.
...
TL;DR: The paper introduces LongT2IBench, a dataset for evaluating long text-to-image alignment with graph-structured annotations, and proposes LongT2IExpert, an evaluator using MLLMs for quantitative and interpretable alignment assessments.


<details>
  <summary>Details</summary>
Motivation: There is a need for automatic and interpretable models to evaluate image-text alignment in long prompt scenarios, as existing benchmarks focus on short prompts and lack granularity.

Method: The study develops LongT2IBench with 14K long text-image pairs and graph-structured annotations. It designs a Generate-Refine-Qualify protocol for fine-grained alignment annotations and proposes LongT2IExpert, an MLLM-based evaluator with Hierarchical Alignment Chain-of-Thought for scores and interpretations.

Result: Experiments show LongT2IExpert's superiority in alignment evaluation and interpretation, providing both scores and structured insights.

Conclusion: The introduced LongT2IBench and LongT2IExpert address the gap in long T2I alignment evaluation, offering a robust and interpretable solution.
...
TL;DR: A novel method uses dynamic facial expression analysis to diagnose Parkinson's disease (PD) by targeting hypomimia, achieving 93.1% accuracy.


<details>
  <summary>Details</summary>
Motivation: Current PD diagnostic methods are often inefficient or inaccessible. This study aims to improve diagnosis by focusing on facial symptoms.

Method: Developed a multimodal facial expression analysis network integrating visual and textual features via CLIP, followed by LSTM-based classification.

Result: Achieved 93.1% accuracy, surpassing other in-vitro PD diagnostic approaches.

Conclusion: This method provides a convenient and accurate diagnostic tool for PD, enhancing patient experience.
...
TL;DR: This paper introduces LoGoColor, a pipeline for 3D colorization that avoids color averaging by generating consistent training views, using a Local-Global approach to ensure multi-view consistency.


<details>
  <summary>Details</summary>
Motivation: Existing 3D colorization methods suffer from color inconsistency due to distillation from 2D models, leading to oversimplified results, especially in complex scenes.

Method: The authors propose LoGoColor, which partitions scenes into subscenes and uses a fine-tuned multi-view diffusion model to ensure both inter-subscene and intra-subscene consistency.

Result: The method achieves more consistent and plausible 3D colorization, validated by a novel Color Diversity Index showing superior color diversity.

Conclusion: LoGoColor outperforms existing methods in preserving color diversity and consistency, particularly in complex 360° scenes.
...
TL;DR: FoundIR-v2, a diffusion-based image restoration model, uses data equilibrium scheduling and MoE-driven task adaptation to improve performance across diverse restoration tasks.


<details>
  <summary>Details</summary>
Motivation: The study identifies the importance of data mixture proportions in training all-in-one image restoration models, leading to the development of FoundIR-v2.

Method: The model employs a data equilibrium scheduling paradigm to balance dataset composition and a MoE-driven scheduler for task-adaptive diffusion priors.

Result: FoundIR-v2 outperforms state-of-the-art methods, handling over 50 sub-tasks in real-world scenarios.

Conclusion: The approach ensures balanced dataset composition and flexible task adaptation, enhancing generalization and comprehensive performance in image restoration.
...
TL;DR: MelanomaNet is an explainable deep learning system for skin lesion classification, combining multiple interpretability mechanisms to enhance clinical trust.


<details>
  <summary>Details</summary>
Motivation: Clinical adoption of automated skin lesion classification is limited due to the lack of interpretability in deep learning models.

Method: The system uses an EfficientNet V2 backbone with GradCAM++, ABCDE criterion extraction, FastCAV for concept-based explanations, and Monte Carlo Dropout for uncertainty quantification.

Result: Achieves 85.61% accuracy and a weighted F1 score of 0.8564 on the ISIC 2019 dataset, with clinically meaningful explanations.

Conclusion: MelanomaNet demonstrates high accuracy and interpretability, potentially improving trust and adoption in clinical workflows.
...
TL;DR: Improved YOLOv8n-SPTS model enhances small traffic target detection via optimized feature extraction, fusion, and dedicated detection heads, achieving top performance on VisDrone2019-DET.


<details>
  <summary>Details</summary>
Motivation: Address poor detection performance in autonomous driving due to small target issues like occlusion and scale imbalance.

Method: Replace modules with SPD-Conv and SPPFCSPC, introduce TSFP structure for small targets, and balance computational efficiency.

Result: Achieves top metrics: precision (61.9%), recall (48.3%), mAP@0.5 (52.6%), mAP@0.5:0.95 (32.6%).

Conclusion: Model significantly reduces miss rates for small targets in dynamic scenes.
...
TL;DR: VABench is introduced as a benchmark framework to evaluate synchronous audio-video generation, addressing gaps in existing metrics.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack comprehensive evaluations for audio-video generation, particularly for synchronized outputs.

Method: VABench includes three task types (T2AV, I2AV, stereo audio-video) and two evaluation modules covering 15 dimensions, such as synchronization and QA pairs.

Result: The framework evaluates seven content categories and provides systematic analysis and visualization of results.

Conclusion: VABench aims to set a new standard for assessing audio-video generation models and advance the field.
...
TL;DR: Polyp-DiFoM is a novel distillation framework that transfers rich representations from large-scale vision foundation models to lightweight polyp segmentation baselines, improving accuracy and efficiency in clinical settings.


<details>
  <summary>Details</summary>
Motivation: Accurate polyp segmentation is crucial for early colorectal cancer detection, but existing lightweight models struggle with variations and camouflaged polyps, while large-scale models lack direct applicability due to dataset scarcity and domain gaps.

Method: Polyp-DiFoM distills semantic priors from foundation models into lightweight architectures (e.g., U-Net) and enhances distillation with frequency domain encoding.

Result: Extensive experiments show Polyp-DiFoM outperforms baselines and state-of-the-art models by significant margins, reducing computation overhead by nearly 9 times.

Conclusion: Polyp-DiFoM effectively bridges the gap between foundation models and clinical deployment, offering efficient and accurate polyp segmentation.
...
TL;DR: The paper introduces USE50k, a large annotated dataset for suspiciousness estimation, and DeepUSEvision, a lightweight vision-based framework for real-time analysis, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Proactive threat detection and public safety in complex environments necessitate efficient suspiciousness analysis tools.

Method: The authors propose DeepUSEvision, integrating a Suspicious Object Detector, dual DCNNs for expression/body-language recognition, and a transformer-based Discriminator Network.

Result: Experiments show superior accuracy, robustness, and interpretability compared to state-of-the-art methods.

Conclusion: USE50k and DeepUSEvision provide a scalable foundation for intelligent surveillance and real-time risk assessment.
...
TL;DR: LNMBench is a benchmark evaluating 10 label noise methods across diverse medical datasets and noise patterns, revealing their limitations under high noise and proposing improvements.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of systematic assessment of label noise robustness in medical imaging due to expert variability and noisy labels.

Method: Introduces LNMBench, evaluating 10 methods across 7 datasets, 6 modalities, and 3 noise patterns, with a proposed enhancement for robustness.

Result: Existing methods degrade under high noise, with challenges like class imbalance and domain variability persisting.

Conclusion: LNMBench provides a standardized framework and insights for developing noise-resilient algorithms in medical imaging.
...
TL;DR: UniLS is an end-to-end framework for generating unified speak-listen expressions using dual-track audio, overcoming stiffness in listener motion with a novel two-stage training approach.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with modeling dynamic listener behavior due to audio imbalance, leading to stiff motions. UniLS aims to address this limitation for interactive avatars.

Method: A two-stage training paradigm: Stage 1 learns an internal motion prior without audio; Stage 2 fine-tunes with dual-track audio to modulate motion based on speech cues.

Result: UniLS achieves state-of-the-art speaking accuracy and a 44.1% improvement in listening metrics, producing more diverse and natural expressions.

Conclusion: UniLS effectively mitigates stiffness issues, offering a practical solution for high-fidelity, audio-driven interactive digital humans.
...
TL;DR: RnD-Avatar is a 3DGS-based framework for creating relightable and animatable human avatars with detailed pose-variant deformations, achieving SOTA results in novel view synthesis, pose rendering, and relighting.


<details>
  <summary>Details</summary>
Motivation: Current NeRF and 3DGS methods lack sufficient geometric details like clothing wrinkles for realistic human avatars, prompting the need for improved modeling techniques.

Method: Uses dynamic skinning weights for articulation, learns body-motion deformations, and introduces regularization for fine geometric details. Evaluated with a multi-view dataset under varied lighting.

Result: Achieves high-fidelity rendering of novel poses and views with realistic lighting effects, outperforming existing methods.

Conclusion: RnD-Avatar advances human avatar modeling by combining accurate deformations and lighting effects, setting a new benchmark for realism.
...
TL;DR: TextGuider improves text rendering in diffusion models by aligning text tokens with image regions, using attention analysis and novel loss functions for better accuracy and completeness.


<details>
  <summary>Details</summary>
Motivation: Existing methods for text rendering in diffusion models often overlook text omission, where desired text is missing. This work addresses that gap.

Method: Analyzes attention patterns in MM-DiT models, introduces two loss functions, and applies latent guidance during early denoising stages.

Result: Achieves state-of-the-art performance in text rendering, with high recall, OCR accuracy, and CLIP scores.

Conclusion: TextGuider offers a training-free solution for accurate and complete text rendering in diffusion-based models.
...
TL;DR: Video-QTR is a lightweight framework for efficient video understanding by dynamically allocating perceptual resources based on semantic queries, reducing frame consumption by 73%.


<details>
  <summary>Details</summary>
Motivation: Traditional MLLMs for long-video understanding are computationally intensive due to dense frame encoding, leading to inefficiency and limited scalability.

Method: Video-QTR introduces query-driven temporal reasoning, creating an adaptive feedback loop between perception and reasoning without exhaustive frame encoding.

Result: The framework achieves state-of-the-art performance on benchmarks like MSVD-QA and Activity Net-QA while reducing input frames by 73%.

Conclusion: Video-QTR offers an efficient, scalable solution for video understanding, outperforming traditional methods.
...
TL;DR: StereoWorld is an end-to-end framework converting monocular to high-fidelity stereo videos using a pretrained video generator, geometry-aware regularization, and spatio-temporal tiling. It outperforms prior methods.


<details>
  <summary>Details</summary>
Motivation: The high cost and artifacts in stereo video production drive the need for an efficient, high-quality solution.

Method: StereoWorld repurposes a pretrained video generator with joint conditioning on monocular input and geometry-aware regularization. Spatio-temporal tiling enables high-resolution synthesis.

Result: The framework outperforms prior methods, achieving superior visual fidelity and geometric consistency. A dataset of 11M frames supports training/evaluation.

Conclusion: StereoWorld provides an effective solution for high-quality monocular-to-stereo video generation, validated by extensive experiments.
...
TL;DR: ASSIST-3D is a novel pipeline for synthesizing 3D scenes to improve class-agnostic instance segmentation by addressing geometry diversity, context complexity, and layout reasonability.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with generalization due to limited annotated 3D data or noisy 2D segmentations, and existing synthetic data methods lack necessary qualities.

Method: ASSIST-3D introduces three innovations: heterogeneous object selection, LLM-guided scene layout generation, and realistic point cloud construction.

Result: Experiments on benchmarks show ASSIST-3D-trained models outperform existing methods, highlighting its superiority over other synthesis approaches.

Conclusion: ASSIST-3D effectively enhances model generalization for class-agnostic 3D instance segmentation.
...
TL;DR: FUSER is a feed-forward multiview registration transformer that processes all scans in a unified latent space to predict global poses directly, avoiding pairwise estimation. It uses a sparse 3D CNN and Geometric Alternating Attention for efficiency. FUSER-DF refines poses via SE(3)$^N$ diffusion, achieving superior accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional multiview point cloud registration relies on pairwise matching, which is computationally expensive and lacks holistic geometric constraints. FUSER addresses this by directly predicting global poses without pairwise steps.

Method: FUSER encodes scans into low-resolution superpoint features using a sparse 3D CNN and employs Geometric Alternating Attention for efficient reasoning. FUSER-DF refines poses using SE(3)$^N$ diffusion, leveraging FUSER as a denoiser.

Result: FUSER achieves superior registration accuracy and computational efficiency on datasets like 3DMatch, ScanNet, and ArkitScenes.

Conclusion: FUSER and FUSER-DF offer a scalable and efficient solution for multiview point cloud registration, outperforming conventional methods.
...
TL;DR: Using log RGB space improves NeRF's rendering quality, robustness, and low-light performance.


<details>
  <summary>Details</summary>
Motivation: To explore how color space affects NeRF's learning of radiance fields, inspired by BIDR model.

Method: Trained NeRF models in various color spaces (linear, sRGB, GPLog, log RGB) with quantitative and qualitative evaluations.

Result: Log RGB space enhances rendering quality, robustness, and low-light performance.

Conclusion: Log RGB space offers a compact and effective representation for NeRF.
...
TL;DR: The paper introduces a novel framework for WB correction using a perception-inspired LHSI color space and a Mamba-based network, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Address limitations of sRGB-based WB editing, which struggles with complex lighting due to fixed nonlinear transformations and entangled color channels.

Method: Proposes a Learnable HSI (LHSI) color space for better chromatic-luminance separation and introduces a Mamba-based network tailored to this space.

Result: Demonstrates superior performance on benchmark datasets.

Conclusion: Highlights the potential of perception-inspired color space design in computational photography.
...
TL;DR: The paper presents a multimodal deep-learning framework for detecting and localizing subdural hematoma (SDH) using clinical and imaging data, achieving high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing automated tools for SDH lack interpretability and spatial localization, motivating the need for transparent, high-performing systems to aid real-time decision-making.

Method: The framework combines structured clinical variables, a 3D CNN for CT volumes, and a transformer-enhanced 2D segmentation model, trained on 25,315 head CT studies.

Result: Clinical variables showed modest performance (AUC 0.75), while imaging models performed better (AUCs 0.922--0.926). The multimodal ensemble achieved the highest accuracy (AUC 0.9407).

Conclusion: The framework offers rapid, accurate, and interpretable SDH detection and localization, potentially improving triage and intervention timelines in clinical workflows.
...
TL;DR: The paper introduces the WAH framework for multi-view clustering, emphasizing hierarchical semantic modeling and global semantic alignment to improve clustering performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on instance-level alignment but overlook global semantic consistency, making them susceptible to noise and cross-view discrepancies.

Method: WAH uses view-specific hyperbolic encoders to embed features into the Lorentz manifold and employs hyperbolic sliced-Wasserstein distance for global semantic alignment, along with soft cluster assignments.

Result: Extensive experiments demonstrate that WAH achieves state-of-the-art clustering performance.

Conclusion: The WAH framework effectively addresses global semantic consistency in multi-view clustering, outperforming existing methods.
...
TL;DR: The paper introduces Generative Point Cloud Registration, leveraging 2D generative models for 3D registration by generating cross-view consistent image pairs fused with geometry-color features.


<details>
  <summary>Details</summary>
Motivation: To enhance 3D registration performance by bridging advanced 2D generative models with 3D matching tasks through geometry-color feature fusion.

Method: Proposes Match-ControlNet, a controllable 2D generative model ensuring 2D-3D geometric and cross-view texture consistency via depth-conditioned generation and coupled conditional denoising.

Result: Extensive experiments on 3DMatch and ScanNet datasets confirm the approach's effectiveness in improving registration.

Conclusion: The generative paradigm is generalizable and can be integrated into various registration methods to boost performance.
...
TL;DR: The paper introduces DirectSwap, a mask-free video head-swapping framework, and HeadSwapBench, the first cross-identity paired dataset for training and benchmarking, addressing issues like identity leakage and motion dynamics.


<details>
  <summary>Details</summary>
Motivation: Prior video head-swapping methods lack ground-truth paired data, relying on mask-based inpainting, which introduces boundary artifacts and fails to recover occluded cues like facial pose and expressions.

Method: DirectSwap extends an image U-Net into a video diffusion model with a motion module and conditioning inputs, using HeadSwapBench for training and introducing the MEAR loss for coherence.

Result: DirectSwap achieves state-of-the-art visual quality, identity fidelity, and motion/expression consistency across diverse videos.

Conclusion: The framework and dataset advance video head-swapping research, with plans to release the source code and HeadSwapBench.
...
TL;DR: The paper introduces MCDM, a label-free diffusion model for synthesizing realistic echocardiography videos using self-supervised motion features, overcoming data scarcity issues.


<details>
  <summary>Details</summary>
Motivation: The scarcity of labeled echocardiography data due to privacy and annotation challenges hinders deep learning applications. MCDM aims to address this by generating synthetic videos without manual labels.

Method: MCDM uses a latent diffusion framework conditioned on motion features extracted by MAFE, enhanced by auxiliary objectives like re-identification and optical flow losses.

Result: MCDM produces temporally coherent and clinically realistic echocardiography videos on the EchoNet-Dynamic dataset without labeled data.

Conclusion: The results highlight the potential of self-supervised conditioning for scalable echocardiography synthesis, offering a solution to data scarcity.
...
### [65] [Representation Calibration and Uncertainty Guidance for Class-Incremental Learning based on Vision Language Model](https://arxiv.org/abs/2512.09441)
*Jiantao Tan,Peixian Ma,Tong Yu,Wentao Zhang,Ruixuan Wang*

Main category: cs.CV

TL;DR: A novel VLM-based continual learning framework for image classification is introduced, using task-specific adapters and cross-task representation calibration to reduce class confusion across tasks.


<details>
  <summary>Details</summary>
Motivation: Class-incremental learning faces challenges in differentiating classes across tasks; this paper aims to address this issue.

Method: The framework employs task-specific adapters and a cross-task representation calibration strategy using light-weight projectors, along with an uncertainty-guided inference strategy.

Result: Extensive experiments show superior performance compared to existing methods.

Conclusion: The proposed framework effectively mitigates class confusion and improves continual learning performance.
...
TL;DR: A novel method for distilling synthetic echocardiographic videos using motion features and graph-based selection achieves high accuracy with minimal data.


<details>
  <summary>Details</summary>
Motivation: Address the challenges of storing and processing large-scale echocardiographic video data by creating a compact, informative subset.

Method: Uses motion feature extraction, class-wise graph construction, and the Infomap algorithm to select diverse, representative synthetic videos.

Result: Achieves 69.38% test accuracy on EchoNet-Dynamic datasets with only 25 synthetic videos.

Conclusion: The method effectively distills medical video datasets, preserving key clinical features while improving efficiency.
...
### [67] [Cytoplasmic Strings Analysis in Human Embryo Time-Lapse Videos using Deep Learning Framework](https://arxiv.org/abs/2512.09461)
...
TL;DR: The paper introduces the first computational framework for detecting Cytoplasmic Strings (CS) in human IVF embryos using a deep learning approach with a novel loss function, improving accuracy and localization.


<details>
  <summary>Details</summary>
Motivation: Addressing the labor-intensive and subjective nature of manual CS assessment in embryo selection for IVF, the paper aims to automate and improve CS detection.

Method: A two-stage deep learning framework with a human-in-the-loop annotation pipeline and a Novel Uncertainty-aware Contractive Embedding (NUCE) loss to handle class imbalance and feature uncertainty.

Result: The framework achieves improved F1-scores across transformer backbones and state-of-the-art detection performance for CS structures.

Conclusion: The proposed method offers a reliable, automated solution for CS analysis, enhancing embryo selection in IVF.
...
TL;DR: FunPhase, a functional periodic autoencoder, learns motion phase manifolds for scalable, smooth motion prediction and generation across diverse skeletons and tasks.


<details>
  <summary>Details</summary>
Motivation: Existing motion prediction methods lack scalability and generalizability due to strong coupling between spatial geometry and temporal dynamics.

Method: FunPhase uses a functional periodic autoencoder to embed motion in phase manifolds, replacing discrete temporal decoding with function-space formulation for smooth trajectories.

Result: Achieves lower reconstruction error than prior baselines and performs on par with state-of-the-art motion generation methods.

Conclusion: FunPhase unifies motion prediction and generation, supporting diverse applications like super-resolution and partial-body motion completion.
...
TL;DR: The paper validates a privacy-preserving AI framework on real-world industrial data, showing effectiveness in balancing utility and privacy across three use cases.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the tension between operational utility and worker privacy in AI-powered computer vision deployments in industry.

Method: The framework uses learned visual transformations to obscure sensitive data while retaining task-relevant features, tested in three industrial use cases.

Result: The framework effectively balances privacy and utility, demonstrating readiness for real-world adoption.

Conclusion: Task-specific obfuscation enables responsible AI deployment, reducing privacy risks while maintaining operational effectiveness.
...
TL;DR: The paper introduces UniPart, a two-stage latent diffusion framework for part-level 3D generation, leveraging Geom-Seg VecSet for unified geometry-segmentation representation. It outperforms existing methods in segmentation control and part-level geometric quality.


<details>
  <summary>Details</summary>
Motivation: Part-level 3D generation is crucial for decomposable and structured 3D synthesis. Current methods lack granularity control or rely on external segmenters with large datasets. This work aims to overcome these limitations.

Method: Proposes Geom-Seg VecSet, a joint geometry-segmentation latent representation. Introduces UniPart, a two-stage latent diffusion framework: Stage 1 generates geometry and latent part segmentation; Stage 2 conditions part-level diffusion on whole-object and part-specific latents. Uses dual-space generation for enhanced fidelity.

Result: Extensive experiments show UniPart achieves better segmentation controllability and part-level geometric quality than existing approaches.

Conclusion: UniPart advances part-level 3D generation by integrating geometry and segmentation learning, offering improved control and quality without relying on external segmenters.
...
TL;DR: The paper investigates how color and shape are encoded in Stable Diffusion's latent space, finding interpretable representations aligned with efficient coding principles.


<details>
  <summary>Details</summary>
Motivation: To understand how perceptual attributes like color and shape are internally represented in diffusion-based generative models.

Method: Controlled synthetic datasets, PCA, and similarity metrics are used to analyze latent representations.

Result: Color is encoded along circular axes in specific channels (c_3, c_4), while intensity and shape are in others (c_1, c_2).

Conclusion: The latent space has an interpretable structure, aiding future work in model understanding and disentangled frameworks.
...
TL;DR: DAPO improves fine-grained anomaly detection by optimizing prompts to align image features with detailed text semantics, achieving better performance under distribution shifts and zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack fine-grained anomaly details (e.g., "hole", "scratch"), which limits actionable insights. DAPO addresses this by enriching anomaly representations and enabling targeted corrective measures.

Method: DAPO uses progressive tuning to optimize hybrid defect-aware prompts, combining fixed textual anchors and learnable token embeddings for zero-shot anomaly detection and segmentation.

Result: DAPO outperforms baselines by 3.7% (AUROC/image level) under distribution shifts and 6.5% (zero-shot anomaly localization) on public and internal datasets.

Conclusion: DAPO enhances fine-grained anomaly detection by leveraging structured semantics, offering practical benefits for defect identification and root cause analysis.
...
TL;DR: The paper proposes a deep learning model using CNN, LSTM, and FC layers to predict steering angles in autonomous vehicles by leveraging temporal dependencies and shared images via V2V communication.


<details>
  <summary>Details</summary>
Motivation: Current deep learning solutions for steering angle prediction often overlook temporal dependencies between image frames, and the use of shared images between vehicles (V2V communication) is an underexplored area.

Method: The authors introduce an end-to-end network combining CNN, LSTM, and FC layers. It uses both current and future images (shared via V2V) to account for temporal dependencies.

Result: The proposed model achieves the lowest error compared to existing approaches in the literature.

Conclusion: The study demonstrates the effectiveness of incorporating temporal dependencies and shared images in improving steering angle prediction for autonomous vehicles.
...
### [74] [Hands-on Evaluation of Visual Transformers for Object Recognition and Detection](https://arxiv.org/abs/2512.09579)
*Dimitrios N. Vlachogiannis,Dimitrios A. Koutsomitropoulos*

Main category: cs.CV

TL;DR: Comparison of Vision Transformers (ViTs) and CNNs shows ViTs, especially hybrid/hierarchical types like Swin/CvT, outperform CNNs in global context tasks like medical imaging, with data augmentation further boosting performance.


<details>
  <summary>Details</summary>
Motivation: CNNs struggle with global image understanding, while ViTs excel due to self-attention. This paper compares ViTs (pure, hierarchical, hybrid) against CNNs across tasks like object recognition and medical imaging.

Method: Conduct tests on ImageNet, COCO, and ChestX-ray14 datasets comparing ViTs (pure, hierarchical, hybrid) with CNNs. Apply data augmentation techniques to medical images.

Result: Hybrid/hierarchical transformers (Swin/CvT) balance accuracy and computational efficiency. Data augmentation improves performance, especially for Swin Transformer. ViTs often outperform CNNs in global context tasks.

Conclusion: Vision Transformers, particularly hybrid/hierarchical models, outperform CNNs in tasks requiring global understanding (e.g., medical imaging). Data augmentation enhances their performance further.
...
TL;DR: Introducing MODA, a large-scale multispectral dataset, and OSSDet, a novel framework integrating spectral-spatial data with object-aware cues for improved aerial object detection.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of RGB-based detectors in aerial object detection by leveraging multispectral images (MSIs) and overcoming data scarcity.

Method: Proposes OSSDet: cascaded spectral-spatial modulation, spectral feature aggregation, object-aware masking, and cross-spectral attention for refined object detection.

Result: OSSDet outperforms existing methods in performance and efficiency, validated by extensive experiments.

Conclusion: MODA and OSSDet advance aerial object detection by harnessing multispectral data and innovative feature integration.
...
TL;DR: StateSpace-SSL is a linear-time self-supervised learning framework using Vision Mamba for plant disease detection, outperforming CNN- and transformer-based methods by modeling long-range lesion continuity efficiently.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods (CNNs/transformers) are inefficient for plant disease detection due to poor handling of continuous disease patterns and high computational costs.

Method: Proposes StateSpace-SSL with Vision Mamba encoder for linear-time modeling of lesion continuity and a prototype-driven teacher-student objective for stable feature alignment.

Result: Outperforms CNN- and transformer-based SSL baselines on three datasets, with qualitative analyses showing compact, lesion-focused feature maps.

Conclusion: StateSpace-SSL demonstrates the advantage of linear state-space modeling for efficient and effective self-supervised plant disease detection.
...
TL;DR: The paper presents a method using neural registration and autoencoder models to predict healthy bone structure for surgical planning in complex tibial fractures.


<details>
  <summary>Details</summary>
Motivation: Surgical planning for complex tibial fractures is challenging due to the difficulty in visualizing the desired 3D bone alignment. The goal is to predict a patient-specific reconstruction target from a fractured tibia CT scan.

Method: The approach combines a modified spatial transformer network (STN) for registering raw CTs to a standardized tibia prototype and various autoencoder (AE) architectures to model healthy tibial variations. Both models are robust to masked inputs, enabling application to fractured CTs.

Result: The method generates a prediction of the patient-specific healthy bone in standard coordinates, aiding surgical planning. Key contributions include a 3D-adapted STN, comparative AE analysis, and handling masked inputs.

Conclusion: The proposed method effectively assists in surgical planning by predicting healthy bone structures from fractured CT scans, leveraging neural registration and autoencoder models.
...
TL;DR: The paper proposes DDSRNet, a lightweight dual-domain super-resolution network combining Spatial-Net and DWT for efficient hyperspectral image enhancement.


<details>
  <summary>Details</summary>
Motivation: To address the need for efficient hyperspectral image super-resolution with low computational cost by integrating spatial and frequency-domain learning.

Method: DDSRNet uses Spatial-Net for shallow feature extraction, a DWT-based low-frequency enhancement branch, and a shared high-frequency refinement branch with a single CNN.

Result: Achieves competitive performance on three hyperspectral image datasets with reduced computational cost.

Conclusion: DDSRNet effectively integrates spatial- and frequency-domain learning for efficient hyperspectral image super-resolution.
...
TL;DR: The paper addresses instability and contradictions in VLMs for BIQA by analyzing their reasoning flaws and introduces a two-stage tuning method to improve human-like reasoning and prediction stability.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for BIQA produce contradictory textual descriptions and unstable quality predictions, which deviate from human reasoning. The study aims to understand and mitigate these issues.

Method: The authors analyze factors causing contradictions and instability, then propose a two-stage tuning method: the first stage learns visual features, and the second infers quality from these features.

Result: The method reduces prediction instability from 22.00% to 12.39% and improves SRCC/PLCC scores by 0.3124/0.3507 on benchmark datasets.

Conclusion: The two-stage tuning approach successfully enhances prediction stability and reliability in BIQA, aligning better with human reasoning.
...
### [80] [CHEM: Estimating and Understanding Hallucinations in Deep Learning for Image Processing](https://arxiv.org/abs/2512.09806)
*Jianfei Li,Ines Rosellon-Inclan,Gitta Kutyniok,Jean-Luc Starck*

Main category: cs.CV

TL;DR: This paper introduces CHEM, a method to quantify hallucination artifacts in image reconstruction models, addressing limitations of U-shaped networks like U-Net.


<details>
  <summary>Details</summary>
Motivation: U-shaped networks often generate unrealistic artifacts in image deconvolution, posing risks in safety-critical applications. Trustworthy computer vision models require methods to quantify these hallucinations.

Method: CHEM uses wavelet/shearlet representations to extract hallucinated features and conformalized quantile regression for distribution-free quantification. Theoretical analysis explains U-nets' hallucination tendencies.

Result: Tested on CANDELS dataset with U-Net, SwinUNet, and Learnlets, CHEM effectively identifies and quantifies hallucinations, offering new insights into deep learning-based image processing.

Conclusion: CHEM provides a reliable metric for hallucination quantification, enhancing trustworthiness in image reconstruction models while shedding light on U-nets' hallucination issues.
...
TL;DR: DNS-HyXNet is a lightweight xLSTM framework for efficient DNS tunnel detection, achieving high accuracy and low latency without graph construction.


<details>
  <summary>Details</summary>
Motivation: DNS tunneling poses security risks, and existing graph-based methods introduce high latency and computational overhead, making them unsuitable for real-time deployment.

Method: DNS-HyXNet uses tokenized domain embeddings and numerical DNS features processed through a two-layer xLSTM network for sequence-based detection, eliminating graph reconstruction.

Result: The model achieved up to 99.99% accuracy with macro-averaged precision, recall, and F1-scores exceeding 99.96%, and a detection latency of 0.041 ms.

Conclusion: DNS-HyXNet proves that sequential modeling with xLSTM can replace graph-based methods, offering a scalable, real-time solution for DNS tunnel detection.
...
### [82] [Composing Concepts from Images and Videos via Concept-prompt Binding](https://arxiv.org/abs/2512.09824)
*Xianghao Kong,Zeyu Zhang,Yuwei Guo,Zhuoran Zhao,Songchun Zhang,Anyi Rao*

Main category: cs.CV

TL;DR: Bind & Compose is a one-shot method for visual concept composition, using hierarchical binders and prompt tokens to accurately extract and combine concepts from images and videos.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with accurately extracting complex visual concepts and flexibly combining them across images and videos.

Method: Introduces a hierarchical binder structure, Diversify-and-Absorb Mechanism, and Temporal Disentanglement Strategy for improved accuracy and compatibility.

Result: Achieves superior concept consistency, prompt fidelity, and motion quality compared to existing approaches.

Conclusion: The method opens new possibilities for visual creativity by enhancing concept composition accuracy and flexibility.
...
TL;DR: MLLMs struggle with detecting low-level image distortions and produce inconsistent IQA results. Fine-tuning vision encoders improves distortion recognition accuracy from 14.92% to 84.43%, enhancing interpretability.


<details>
  <summary>Details</summary>
Motivation: Current MLLM-based IQA systems fail to reliably detect low-level distortions like blur or noise, raising concerns about their true perception of visual features.

Method: A low-level distortion perception task was introduced, analyzing MLLMs' performance. Component-wise fine-tuning of vision encoders was tested to improve alignment.

Result: Fine-tuning improved distortion recognition accuracy significantly, from 14.92% to 84.43%, by enhancing vision-language alignment.

Conclusion: Dedicated constraints on vision encoders can improve MLLM-based IQA systems, making them more coherent and interpretable for vision tasks.
...
TL;DR: A scalable modeling system for estimating soil properties (SOC, N, P, K, pH) using remote sensing and environmental covariates, validated across Europe with strong performance for SOC and N.


<details>
  <summary>Details</summary>
Motivation: To address the lack of accessible tools for soil assessment in agriculture by leveraging scalable, data-driven methods.

Method: Hybrid modeling combining indirect proxy-based and direct spectral approaches, using radiative transfer models and foundation model embeddings, validated with strict spatial blocking and stratified splits.

Result: Highest accuracy for SOC (MAE 5.12 g/kg, CCC 0.77) and N (MAE 0.44 g/kg, CCC 0.77), with robust validation showing performance across unseen regions.

Conclusion: The system advances digital agriculture by providing scalable soil analysis frameworks applicable to domains like carbon markets.
...
TL;DR: A novel image retouching method, CA-ATP, addresses limitations of uniform color mapping by using content-adaptive curves and attribute-based text representations for adaptive adjustments and user-guided styles.


<details>
  <summary>Details</summary>
Motivation: Existing retouching methods fail to handle diverse color distributions and user preferences due to uniform pixel-wise mapping.

Method: Proposes content-adaptive curve mapping and attribute text prediction modules for context-aware adjustments and explicit style representation.

Result: Achieves state-of-the-art performance on public datasets by accommodating color diversity and user preferences.

Conclusion: CA-ATP effectively combines adaptive color mapping and user-guided attributes for superior image retouching.
...
### [86] [OxEnsemble: Fair Ensembles for Low-Data Classification](https://arxiv.org/abs/2512.09665)
*Jonathan Rystrøm,Zihao Fu,Chris Russell*

Main category: cs.CV

TL;DR: The paper introduces OxEnsemble, a novel method for fair classification in low-data, unbalanced settings, particularly in medical imaging, ensuring efficiency and strong fairness-accuracy trade-offs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address fairness and reliability in classification when data is scarce and imbalanced across demographic groups, especially in high-stakes domains like medical imaging.

Method: The proposed method, OxEnsemble, trains ensembles where each member satisfies fairness constraints, aggregating predictions efficiently while reusing data for fairness enforcement.

Result: OxEnsemble demonstrates more consistent outcomes and better fairness-accuracy trade-offs compared to existing methods, validated by theoretical guarantees.

Conclusion: OxEnsemble effectively tackles fairness in low-data regimes, offering data and compute efficiency, and outperforms current methods in medical imaging classification.
...
TL;DR: UnReflectAnything is an RGB-only framework that removes specular highlights from images by predicting highlight maps and reconstructing diffuse images, using a frozen vision transformer and token-level inpainting. It trains on synthetic highlights and achieves competitive results across domains.


<details>
  <summary>Details</summary>
Motivation: Specular highlights distort appearance, obscure texture, and hinder geometric reasoning in natural and surgical imagery. Addressing this issue improves image usability.

Method: The model uses a frozen vision transformer encoder, a lightweight head for highlight localization, and a token-level inpainting module. Training relies on Virtual Highlight Synthesis for paired supervision.

Result: UnReflectAnything generalizes well across natural and surgical domains, handling non-Lambertian surfaces and non-uniform lighting. It achieves competitive performance on benchmarks.

Conclusion: The framework effectively removes highlights and reconstructs diffuse images, demonstrating versatility and robustness in challenging lighting conditions.
...
TL;DR: The paper proposes the CS3D framework to enhance facial expression recognition for human-robot interaction, reducing computational complexity and energy consumption while improving accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of energy-intensive traditional deep learning methods for facial expression recognition, especially in event-based approaches, aiming for efficient deployment on edge computing devices.

Method: Introduces the CS3D framework by decomposing the Convolutional 3D method, incorporating soft spiking neurons and a spatial-temporal attention mechanism to reduce computational complexity and enhance information retention.

Result: CS3D outperforms RNN, Transformer, and C3D in accuracy on multiple datasets, with energy consumption reduced to 21.97% of the original C3D.

Conclusion: The CS3D framework effectively balances accuracy and energy efficiency, making it suitable for practical deployment in human-robot interaction.
...
TL;DR: The paper introduces a lightweight adaptation technique for appearance transfer in multiview diffusion models, enabling diverse appearance manipulation while preserving spatial consistency.


<details>
  <summary>Details</summary>
Motivation: Existing multiview diffusion models lack fine-grained appearance manipulation (e.g., material, texture, or style), which limits their practicality for content creation.

Method: The proposed method combines object identity from an input image with appearance cues from a reference image using three diffusion denoising processes and selective aggregation of self-attention features.

Result: The technique achieves multiview-consistent outputs with desired appearances using minimal training data, enhancing the flexibility of implicit generative 3D representations.

Conclusion: The study advocates for implicit generative 3D representations by demonstrating a simple yet effective approach to multiview generation with diverse appearance control.
...
TL;DR: The paper tackles fine-grained classification of hand-object interaction states ('approaching', 'grabbing', 'holding') using structured kinematic features. Surprisingly, using a Bidirectional RNN with seq_length=1 as a static encoder outperformed sequential models, achieving 97.60% accuracy and overcoming the 'grabbing' class challenge.


<details>
  <summary>Details</summary>
Motivation: The research addresses the challenge of predicting human intent in hand-object interactions by focusing on classifying atomic interaction states accurately.

Method: A structured data engineering process converted MANIAC dataset videos into 27,476 statistical-kinematic feature vectors. The study compared static classifiers (MLPs) with temporal models (RNNs), ultimately finding success with a Bidirectional RNN set to seq_length=1.

Result: The final model achieved 97.60% accuracy and a balanced F1-score of 0.90 for the challenging 'grabbing' class.

Conclusion: The findings establish a new benchmark for low-level hand-object interaction recognition using interpretable features and lightweight architectures.
...
TL;DR: Benchmarking SAM2-based trackers on challenging datasets to analyze their limitations and performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate and understand the limitations of high-performing trackers (SAM2, EfficientTAM, DAM4SAM, SAMURAI) on datasets with fast-moving objects.

Method: Benchmarking the trackers on datasets specifically designed to challenge tracking approaches.

Result: DAM4SAM and SAMURAI perform well on more challenging sequences.

Conclusion: The study highlights the performance of DAM4SAM and SAMURAI in challenging scenarios, providing insights into current tracker limitations.
...
TL;DR: Kaapana is an open-source platform addressing challenges in medical imaging research by offering a modular framework for data ingestion, cohort curation, and workflow orchestration, enabling scalable collaboration while maintaining data control.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome regulatory constraints, fragmented software, and scalability issues in multi-center medical imaging research by providing standardized, reproducible tooling.

Method: Kaapana offers a modular, extensible framework unifying data ingestion, cohort curation, processing workflows, and result inspection under a common interface, integrating flexible workflow orchestration with researcher-facing applications.

Result: Kaapana reduces technical overhead, improves reproducibility, and enables large-scale collaborative multi-center studies while allowing institutions to retain control over sensitive data.

Conclusion: Kaapana successfully bridges gaps in medical imaging research, supporting diverse use cases from local prototyping to nation-wide networks, and is available as open-source.
...
TL;DR: VHOI is a two-stage framework for generating controllable human-object interaction (HOI) videos, using sparse trajectories transformed into dense HOI masks and fine-tuning a video diffusion model for realism.


<details>
  <summary>Details</summary>
Motivation: Existing methods for controllable video generation struggle with balancing ease of specification (sparse controls) and informativeness (dense signals), especially for complex HOI dynamics.

Method: VHOI densifies sparse trajectories into HOI mask sequences and fine-tunes a video diffusion model using these masks, with an HOI-aware motion representation for distinguishing motion dynamics.

Result: State-of-the-art performance in controllable HOI video generation, capable of full human navigation and object interactions end-to-end.

Conclusion: VHOI successfully addresses the trade-off in existing methods by combining sparse input ease with dense signal informativeness, enhancing realistic HOI video generation.
...
TL;DR: IF-Bench is introduced as the first benchmark for evaluating MLLMs' understanding of infrared images, featuring 499 images and 680 Q&A pairs. Over 40 MLLMs are evaluated, revealing insights into model performance. A training-free GenViP method is proposed to improve infrared image comprehension by converting them to RGB counterparts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the unexplored capability of multimodal large language models (MLLMs) in understanding infrared images, filling a research gap.

Method: The method involves creating IF-Bench with 499 infrared images and 680 Q&A pairs, evaluating over 40 MLLMs using cyclic evaluation and hybrid judgment strategies, and proposing GenViP, a training-free method to convert infrared images to RGB for better comprehension.

Result: Results show how model scale, architecture, and inference paradigms affect infrared image understanding. GenViP consistently improves MLLM performance by mitigating domain shifts.

Conclusion: The study concludes with valuable insights into MLLM performance on infrared images and introduces GenViP as an effective solution for enhancing comprehension.
...
TL;DR: The paper presents a fully automated Tip-and-Cue framework for satellite imaging tasking and scheduling, demonstrating its effectiveness in maritime vessel tracking and highlighting its adaptability to other applications.


<details>
  <summary>Details</summary>
Motivation: The rise of satellite constellations and improved sensor capabilities has created opportunities for automated Earth observation, necessitating efficient tasking and scheduling systems.

Method: The framework uses external data or prior imagery to generate tips (targets), formulates cues (imaging tasks) considering sensor constraints, schedules tasks across satellites using utility functions, and processes imagery with AI models.

Result: The system successfully demonstrates maritime vessel tracking, leveraging AIS data for predictions and actionable outputs, and is adaptable to other domains like smart-city monitoring.

Conclusion: The automated Tip-and-Cue framework effectively addresses modern satellite tasking challenges, showcasing scalability and applicability to diverse Earth observation needs.
...
TL;DR: The paper introduces UniForget, a method to mitigate memorization in generative models by identifying and pruning model parts responsible for copyrighted content, preserving general capabilities without targeting specific concepts.


<details>
  <summary>Details</summary>
Motivation: Generative models often memorize training data, leading to legal issues like copyright infringement. Existing methods are computationally heavy or limited in scalability.

Method: UniForget uses model pruning to suppress copyrighted content generation by targeting specific model parts, without needing concept-specific datasets.

Result: The approach effectively reduces memorization while maintaining general generative capabilities and complements existing unlearning methods.

Conclusion: UniForget provides a scalable and efficient solution to memorization, enhancing current unlearning techniques.
...
TL;DR: LiM-YOLO is a specialized detector for ship detection in satellite imagery, addressing challenges like scale disparity and morphological anisotropy by reconfiguring the detection head and improving training stability.


<details>
  <summary>Details</summary>
Motivation: General-purpose object detectors struggle with ship detection in satellite images due to extreme scale variations and anisotropy, leading to poor resolution of narrow vessels.

Method: Proposes a Pyramid Level Shift Strategy (P2-P4) for better small object detection and introduces GN-CBLinear for stable training on high-resolution inputs.

Result: LiM-YOLO outperforms state-of-the-art models on datasets like SODA-A, DOTA-v1.5, FAIR1M-v2.0, and ShipRSImageNet-V1 in accuracy and efficiency.

Conclusion: LiM-YOLO effectively resolves domain-specific challenges in ship detection, offering improved performance and computational efficiency.
...
TL;DR: FastPose-ViT is a Vision Transformer-based method for estimating spacecraft 6DoF pose from single images, offering real-time performance on edge devices by avoiding iterative PnP algorithms.


<details>
  <summary>Details</summary>
Motivation: Existing PnP-based methods for spacecraft pose estimation are computationally heavy and impractical for real-time edge deployment.

Method: FastPose-ViT uses ViT to regress 6DoF pose directly from cropped images, leveraging projective geometry and "apparent rotation" principles to correct predictions to full-image scale.

Result: FastPose-ViT outperforms non-PnP methods and matches PnP-based techniques on SPEED dataset, achieving ~75 ms latency and 33 FPS throughput on NVIDIA Jetson Orin Nano.

Conclusion: FastPose-ViT is a viable solution for real-time spacecraft pose estimation on resource-constrained hardware, suitable for space missions.
...
TL;DR: A novel semi-supervised multi-modal framework enhances modality-specific representations and adaptively fuses cross-modal information for medical image segmentation, outperforming baselines with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Existing SSL approaches for multi-modal medical imaging struggle to exploit complementary information due to semantic discrepancies and misalignment across MRI sequences.

Method: Introduces Modality-specific Enhancing Module (MEM) and Complementary Information Fusion (CIF) module, optimized with hybrid supervised segmentation loss and cross-modal consistency regularization.

Result: Outperforms strong baselines on BraTS 2019 under 1%, 5%, and 10% labeled data settings, improving Dice and Sensitivity scores.

Conclusion: The proposed MEM and CIF modules effectively bridge cross-modality discrepancies and enhance segmentation robustness with limited supervision.
...
TL;DR: DynaIP enhances PT2I generation by improving concept fidelity, CP-PF balance, and scalability using Dynamic Decoupling Strategy and Hierarchical Feature Fusion.


<details>
  <summary>Details</summary>
Motivation: Address challenges in PT2I generation: CP-PF balance, fine-grained detail retention, and multi-subject scalability.

Method: Introduces DynaIP with Dynamic Decoupling Strategy and Hierarchical Mixture-of-Experts Feature Fusion Module leveraging MM-DiT and CLIP.

Result: Outperforms existing methods in single- and multi-subject PT2I tasks.

Conclusion: DynaIP represents a significant advancement in PT2I generation.
...
TL;DR: The paper focuses on real-time struggle detection and anticipation in human skill performance, achieving 70-80% mAP for detection and comparable results for anticipation, with high-speed processing suitable for assistive systems.


<details>
  <summary>Details</summary>
Motivation: Understanding human skill struggles enables intelligent assistive systems to provide timely help, addressing the gap in real-time, online detection and anticipation models.

Method: The work reformulates struggle localization as an online task, adapting offline models for real-time detection and anticipation, tested across tasks and skills.

Result: Online detection achieves 70-80% mAP, anticipation performs comparably with slight drops, and models generalize across tasks, outperforming random baselines by 4-20%.

Conclusion: The approach is effective for real-time applications, with high-speed processing (>20 FPS) and robust generalization, advancing assistive system capabilities.
...
TL;DR: UniUGP framework integrates vision-language-action models for autonomous driving to improve reasoning and planning in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of existing AD systems in long-tail scenarios by combining visual causal learning with reasoning from large language models.

Method: Proposes UniUGP, a hybrid expert architecture integrating pre-trained VLMs and video generation models for scene reasoning, future video generation, and trajectory planning.

Result: Achieves state-of-the-art performance in perception, reasoning, and decision-making with superior generalization.

Conclusion: UniUGP effectively enhances autonomous driving performance in complex and rare scenarios.
...
TL;DR: DPS4Un is a novel method for semiblind hyperspectral unmixing using a diffusion posterior sampler, addressing challenges like spectral variability and bias in prior libraries. It leverages superpixels and iterative updates for improved performance.


<details>
  <summary>Details</summary>
Motivation: To address challenges in spectral prior distribution modeling and spectral variability in hyperspectral unmixing, leveraging Bayesian rigor and superpixel homogeneity.

Method: Proposes DPS4Un: uses diffusion posterior sampler, image-based endmember bundles in superpixels, and iterative abundance/endmember updates. Avoids bias from existing spectral libraries.

Result: Outperforms state-of-the-art hyperspectral unmixing methods on three real-world benchmark datasets.

Conclusion: DPS4Un effectively combines learned priors with observations for refined abundance estimation, improving accuracy in hyperspectral unmixing.
...
TL;DR: The paper introduces a benchmarking framework for evaluating PDF formula parsing, using synthetic LaTeX-generated PDFs and LLM-based semantic evaluation, showing high human-judgment correlation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for PDF formula parsing lack semantic evaluation or exclude formulas, limiting the training of language models and scientific knowledge bases.

Method: A novel framework uses synthetic PDFs with LaTeX ground truth for controlled evaluation. It employs an LLM-as-a-judge for semantic assessment and a two-stage matching pipeline for parser output.

Result: LLM-based evaluation correlates highly with human judgment (r=0.78 vs. CDM's r=0.34). Testing 20+ parsers on 2,000+ formulas reveals significant performance disparities.

Conclusion: The framework offers scalable, reproducible evaluation of PDF formula extraction, aiding practitioners in parser selection for downstream applications.
...
TL;DR: The paper introduces Visual Action Reasoning, a new task for VLMs, and VisualActBench, a benchmark to evaluate VLMs' proactive reasoning without explicit prompts.


<details>
  <summary>Details</summary>
Motivation: To explore and improve VLMs' ability to reason and act proactively based on visual inputs, aligning with human decision-making.

Method: Developed VisualActBench with 1,074 videos and 3,733 annotated actions, labeled for prioritization and type to assess reasoning.

Result: Frontier VLMs like GPT4o perform well but lag behind humans in proactive, high-priority reasoning.

Conclusion: VisualActBench highlights gaps in VLMs' context interpretation and outcome anticipation, setting a foundation for future improvements.
...
TL;DR: NordFKB is a fine-grained geospatial AI benchmark dataset for Norway, offering high-resolution orthophotos with detailed annotations for 36 semantic classes, designed for semantic segmentation and object detection tasks.


<details>
  <summary>Details</summary>
Motivation: To advance AI methods in mapping, land administration, and spatial planning by providing a high-quality, diverse, and authoritative dataset for reproducible research.

Method: Dataset derived from Norway's FKB, includes orthophotos and annotations (binary masks and COCO-style boxes). Data spans seven diverse areas, ensuring variability. Expert-reviewed for accuracy, with random splits for training/validation.

Result: A high-accuracy dataset with 36 semantic classes, benchmarking tools, and evaluation protocols for semantic segmentation and object detection.

Conclusion: NordFKB serves as a robust foundation for AI research in geospatial applications, with potential for future expansions in coverage and data modalities.
...
TL;DR: Splatent is a diffusion-based enhancement framework that improves 3D Gaussian Splatting in VAE latent space by recovering fine details in 2D via multi-view attention, preserving reconstruction quality and achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current VAE latent space methods for radiance field representations lack multi-view consistency, causing blurred textures and missing details during 3D reconstruction. Existing solutions compromise quality or introduce hallucinations.

Method: Splatent operates on 3D Gaussian Splatting in VAE latent space, recovering fine details in 2D using multi-view attention mechanisms instead of reconstructing them in 3D space.

Result: Evaluations show Splatent sets a new state-of-the-art for VAE latent radiance field reconstruction and improves detail preservation when integrated with feed-forward frameworks.

Conclusion: Splatent offers high-quality sparse-view 3D reconstruction by preserving VAE pretraining quality and faithfully recovering details, opening new possibilities.
...
TL;DR: The paper highlights limitations in video unified models for reasoning-aware visual editing and introduces RVE-Bench and ReViSE to bridge the gap, showing significant improvements.


<details>
  <summary>Details</summary>
Motivation: Existing video unified models lack effective reasoning-aware visual editing capabilities due to inadequate datasets and a disconnect between reasoning and editing.

Method: The authors propose RVE-Bench for evaluation and ReViSE, a Self-Reflective Reasoning framework, integrating generation and evaluation within a single architecture.

Result: ReViSE achieves a 32% improvement in editing accuracy and visual fidelity on RVE-Bench compared to state-of-the-art methods.

Conclusion: The study successfully addresses the gap in reasoning-aware video editing with RVE-Bench and ReViSE, demonstrating superior performance.
...
TL;DR: GAINS improves Gaussian Splatting-based inverse rendering for sparse-view captures using learning-based priors to stabilize geometry and material recovery.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian Splatting methods degrade under sparse-view settings due to ambiguities in geometry, reflectance, and lighting. GAINS aims to address this limitation.

Method: GAINS uses a two-stage framework: it first refines geometry with monocular depth/normal and diffusion priors, then regularizes material recovery with segmentation, intrinsic image decomposition, and diffusion priors.

Result: GAINS outperforms state-of-the-art methods in material accuracy, relighting quality, and novel-view synthesis, especially in sparse-view scenarios.

Conclusion: GAINS effectively addresses the challenges of sparse-view inverse rendering, demonstrating superior performance in geometry and material recovery.
...
TL;DR: The paper introduces ACoRN, a method to improve abstractive compression in RAG by addressing noise and positional bias in retrieved documents, enhancing accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Retrieved documents in RAG often contain irrelevant or misleading information, leading to abstractive compressors omitting key details. This issue is exacerbated in long contexts due to attention dispersion. The paper aims to improve compression accuracy by addressing these challenges.

Method: The proposed ACoRN involves two novel training steps: offline data augmentation to enhance robustness against retrieval noise, and finetuning to generate summaries focused on key information supporting the correct answer, addressing positional bias.

Result: Experiments show that T5-large trained with ACoRN improves EM and F1 scores while preserving answer strings, demonstrating effectiveness, especially in datasets with many accuracy-reducing documents.

Conclusion: ACoRN enhances the robustness and accuracy of abstractive compressors in RAG, making it valuable for real-world applications where retrieval noise is prevalent.
...
TL;DR: A targeted RL framework mitigates hallucinations in LLMs by addressing intrinsic and extrinsic errors, improving reliability without sacrificing capability.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning enhances LLM reasoning but increases hallucination, creating a trade-off between capability and reliability. This work aims to resolve this tension.

Method: Introduces a targeted RL framework: uses TriviaQA conversions for extrinsic hallucinations and FineWeb texts for intrinsic, with rewards for refusing unanswerable questions.

Result: Significant performance gains across benchmarks, reducing both hallucination types while maintaining advanced reasoning.

Conclusion: The framework balances reasoning and reliability, advancing more trustworthy and capable LLMs.
...
TL;DR: Luxical is a high-speed 'lexical-dense' text embedding library combining TF-IDF and a small ReLU network to approximate transformer embeddings cheaply. It achieves 3x-100x speedups with comparable quality to neural baselines.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between fast but inflexible lexical classifiers and flexible but expensive transformer embeddings for web-scale text organization.

Method: Combines sparse TF-IDF features, a small ReLU network, and knowledge distillation to approximate transformer models.

Result: Demonstrates speedups of 3x-100x over neural baselines while maintaining comparable quality in retrieval and classification tasks.

Conclusion: Luxical offers favorable compute/quality trade-offs for large-scale text organization, matching neural baselines' quality at lower cost.
...
TL;DR: The paper introduces KG-LLM, a Knowledge-Guided Large Language Model, to improve pediatric dental record interpretation and antibiotic prescribing by integrating a knowledge graph, retrieval-augmented generation, and safety validation. It outperforms traditional methods in accuracy and safety.


<details>
  <summary>Details</summary>
Motivation: Persistent challenges in interpreting pediatric dental records and prescribing antibiotics safely motivate the need for advanced clinical decision support systems that handle unstructured data and complex safety constraints.

Method: The proposed KG-LLM integrates a pediatric dental knowledge graph, retrieval-augmented generation (RAG), and a multi-stage safety validation pipeline. It extracts structured entities, retrieves relevant guidelines, and validates recommendations using deterministic rules and a learned classifier.

Result: Experiments on 32,000 records show KG-LLM outperforms a baseline with higher F1 scores (0.914 vs. 0.867), better drug-dose-duration accuracy (Top-1: 0.782 vs. 0.716), and a 50% reduction in unsafe suggestions.

Conclusion: KG-LLM enhances clinical reliability and interpretability in pediatric dental care, with significant contributions from its knowledge graph, RAG, and safety modules.
...
TL;DR: GraphRAG improves LLMs by adding knowledge graph data, but LLMs struggle with relational info, leading to hallucinations. The paper introduces PRD and SAS metrics to analyze LLMs' handling of structured knowledge and proposes GGA, a post-hoc hallucination detector.


<details>
  <summary>Details</summary>
Motivation: Address LLMs' difficulty interpreting relational/topological info from knowledge graphs, causing hallucinations.

Method: Propose PRD and SAS metrics, test on QA task, and develop GGA detector.

Result: Identified failure patterns (high PRD, low SAS); GGA outperforms baselines.

Conclusion: Insights into LLMs' structural limits help design better GraphRAG systems.
...
TL;DR: The study explores how large language models (LLMs) can reflect personality traits using the MMPI test, introduces the MindShift benchmark, and finds improved role perception due to better training and alignment techniques.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can absorb and reflect personality traits and attitudes specified by users, leveraging psychometric measures.

Method: Adapted the MMPI test, created personality-oriented prompts with varying trait intensities, and introduced MindShift for evaluating LLMs' psychological adaptability.

Result: LLMs showed consistent improvement in role perception, attributed to advanced training and alignment. Variability in responses across models indicated differences in emulating human-like traits.

Conclusion: The study demonstrates LLMs' potential to reflect personality traits, with MindShift serving as a benchmark for future evaluation. Findings suggest ongoing improvements in training and alignment can enhance this capability.
...
TL;DR: The paper addresses misalignment issues in reward-model-based fine-tuning of LLMs by identifying proxy-policy conflicts and proposing metrics and an algorithm (SHF-CAS) to target high-conflict areas for refinement.


<details>
  <summary>Details</summary>
Motivation: Reward models often misalign due to noise, bias, or limited coverage, leading models to optimize flawed signals. The work aims to detect and mitigate these misalignments.

Method: Proposes localized (PACS) and global (Kendall-Tau Distance) metrics to identify conflicts. Introduces SHF-CAS, an algorithm targeting high-conflict QA pairs for additional feedback.

Result: Experiments show improved alignment performance even with biased proxies. The approach successfully refines reward models and policies.

Conclusion: The framework provides a new perspective on alignment failures and a principled method for targeted refinement in LLM training.
...
TL;DR: CORE is a concept-first interaction layer designed to improve multi-turn language model interactions by using a persistent Local Concept and cognitive operators, reducing token history reliance and enhancing stability.


<details>
  <summary>Details</summary>
Motivation: Multi-turn interactions in language models often suffer from drift, inconsistency, and growing prompts due to reliance on token history. CORE aims to address these issues without modifying model weights.

Method: CORE introduces a Local Concept—a compact semantic state—and a library of cognitive operators. It processes user instructions and operators without replaying full conversation history.

Result: A prototype simulating CORE's behavior showed a 42% reduction in cumulative prompt tokens, indicating potential efficiency improvements in multi-turn interactions.

Conclusion: CORE provides a scalable, model-agnostic approach to stabilize multi-turn interactions by decoupling conceptual reasoning from language generation, offering a promising direction for future systems.
...
TL;DR: The paper introduces TCA-Attention, a training-free sparse attention mechanism for efficient long-context inference in LLMs, achieving speedup and memory reduction.


<details>
  <summary>Details</summary>
Motivation: To address the computational and memory challenges of quadratic self-attention in LLMs, especially for long sequences, without relying on fixed patterns or additional training.

Method: Proposes a two-phase approach: offline calibration to determine sparsity budgets and online token selection to retain core context tokens adaptively.

Result: TCA-Attention achieves a 2.8x speedup and 61% KV cache reduction at 128K context length while maintaining performance comparable to full attention.

Conclusion: The method offers a practical, plug-and-play solution for efficient long-context inference in LLMs without requiring parameter updates or architectural changes.
...
TL;DR: The paper investigates biases in machine-generated text detection systems, identifying disparities across gender, race/ethnicity, ELL status, and economic status.


<details>
  <summary>Details</summary>
Motivation: To assess potential biases in machine-generated text detection systems and their impact on disadvantaged groups.

Method: Curating a dataset of student essays and evaluating 16 detection systems using regression models and subgroup analysis.

Result: Biases vary across systems, with disadvantaged groups often misclassified. Humans perform poorly but show no biases.

Conclusion: Detection systems exhibit biases, highlighting the need for fairness considerations in their development.
...
TL;DR: CONCUR is a modular, continual routing framework that uses separate predictor models for each strategy and multiple task/strategy representations to improve routing accuracy and reduce training/inference costs.


<details>
  <summary>Details</summary>
Motivation: Existing routing systems often require full retraining for new strategies, face generalization issues, and use single input representations, leading to sub-optimal decisions and high overhead.

Method: Proposes CONCUR, a modular framework with separate predictor models for each strategy and multiple representations of tasks and strategies to better capture complexity.

Result: Outperforms single strategies and existing routing methods in accuracy and cost efficiency across continual and non-continual settings, with lower training costs in continual scenarios.

Conclusion: CONCUR effectively addresses the limitations of prior routing systems, offering improved flexibility, accuracy, and cost-efficiency for AI task routing.
...
TL;DR: Language models can probe possible vs. impossible languages to uncover human learning biases.


<details>
  <summary>Details</summary>
Motivation: Explore LMs' potential in distinguishing possible/impossible languages to understand human cognition.

Method: Phased research program refining LM architectures iteratively.

Result: Better discrimination between possible and impossible languages.

Conclusion: Refined LMs can link to human cognitive biases.
...
TL;DR: CourtPressGER introduces a dataset for training LLMs to generate accurate, readable summaries of judicial rulings, benchmarking models of varying sizes with mixed results.


<details>
  <summary>Details</summary>
Motivation: To address the gap in NLP efforts focusing on citizen-oriented communication of judicial rulings.

Method: Created CourtPressGER dataset (6.4k triples: rulings, press releases, synthetic prompts). Benchmarked small and large LLMs using metrics like factual consistency and expert ranking.

Result: Large LLMs produce high-quality drafts with minimal performance loss; smaller models need hierarchical setups for long texts. Human-drafted releases ranked highest.

Conclusion: CourtPressGER serves as a benchmark for LLMs in judicial summarization, showing potential but highlighting the superiority of human drafts.
...
TL;DR: This paper introduces an explainable reasoning method for financial decisions using knowledge-enhanced large language models, improving accuracy and transparency.


<details>
  <summary>Details</summary>
Motivation: To address shortcomings of traditional financial decision methods, such as lack of factual consistency and reasoning chains.

Method: Combines external knowledge retrieval, semantic representation, and reasoning generation using weighted fusion and multi-head attention.

Result: Outperforms baselines in accuracy, text quality, and factual support, showing improved transparency and practical value.

Conclusion: The approach enhances financial decision-making by integrating knowledge and ensuring explainable reasoning.
...
TL;DR: The study introduces a text classification algorithm using large language models to improve long-range dependency capture, contextual semantics, and class imbalance handling, outperforming traditional methods in metrics like Recall and AUC.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of traditional text classification methods in handling long-range dependencies, contextual semantics, and class imbalance.

Method: Uses large-scale pretrained language models for text encoding, attention mechanisms for feature enhancement, global and weighted aggregation, and a fully connected layer with Softmax for classification.

Result: The proposed method surpasses baseline models (RNNs, GNNs, Transformers) in Precision, Recall, F1-Score, and AUC, with notable gains in Recall and AUC. Sensitivity experiments show its adaptability to hyperparameters and data conditions.

Conclusion: The method effectively improves text classification performance and demonstrates robustness and applicability in complex data environments through systematic analysis.
...
TL;DR: LLM-based Search Engines (LLM-SEs) summarize results with limited citation transparency, differing from Traditional Search Engines (TSEs). A study of 55,936 queries shows LLM-SEs cite more diverse domains (37% unique) but don't outperform TSEs in credibility, neutrality, or safety. Feature analysis reveals factors influencing source choice.


<details>
  <summary>Details</summary>
Motivation: The paper explores the trust and transparency implications of LLM-SEs compared to TSEs, addressing gaps in understanding their citation diversity and performance metrics.

Method: A large-scale empirical study analyzed 55,936 queries and results across six LLM-SEs and two TSEs, including domain diversity and credibility metrics. Feature-based analysis identified factors influencing source selection.

Result: LLM-SEs cite more diverse domains (37% unique) but perform similarly to TSEs in credibility, political neutrality, and safety. Feature analysis highlights key selection criteria.

Conclusion: LLM-SEs offer domain diversity but face challenges in trustworthiness. Insights aid users, website owners, and developers in navigating these systems.
...
TL;DR: \model{} is an RL-based framework enabling adaptive hybrid retrieval-augmented generation (RAG) for LLMs, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing RAG systems lack adaptability and efficiency in hybrid retrieval (texts and graphs) for multi-turn reasoning.

Method: Introduces \model{}, an RL framework jointly optimizing retrieval and generation, with a two-stage training approach balancing task outcome and efficiency.

Result: Outperforms baselines on five QA benchmarks, showing benefits of adaptive retrieval and RL optimization.

Conclusion: \model{} advances hybrid RAG by enabling adaptive, efficient retrieval and generation via RL.
...
TL;DR: The study introduces two frameworks for systematic and responsible use of LLMs in language sciences: a method-selection framework and a practical implementation guide, validated through empirical experiments.


<details>
  <summary>Details</summary>
Motivation: Address methodological fragmentation and lack of systematic soundness in deploying LLMs for language sciences.

Method: Proposes two frameworks: (1) method-selection (prompt-based, fine-tuning, embedding extraction) and (2) systematic implementation guidance, validated via experiments.

Result: Validated frameworks enable strategic alignment of research questions with LLM methods, improving reproducibility and robustness.

Conclusion: The frameworks foster a paradigm shift in language science, ensuring verifiable and systematic LLM applications.
...
TL;DR: The paper introduces a three-stage LLM-based framework to detect evolving hate speech on Chinese social media, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional systems fail to decode context-dependent hate speech and slang, necessitating a more adaptive solution.

Method: A three-stage framework involving Prompt Engineering, Supervised Fine-tuning, and LLM Merging.

Result: Demonstrated superior performance on the STATE-ToxiCN benchmark for fine-grained hate speech detection.

Conclusion: The proposed framework effectively bridges gaps in detecting nuanced hate speech.
...
TL;DR: Creation of an Estonian dataset for document-level subjectivity analysis, including human and LLM (GPT-5) annotations, reveals moderate inter-annotator correlation and feasibility of LLM automation with limitations.


<details>
  <summary>Details</summary>
Motivation: To develop an Estonian-language dataset for subjectivity analysis and explore the potential of LLMs like GPT-5 for automating subjectivity scoring.

Method: Dataset of 1,000 documents (300 journalistic, 700 web texts) annotated by four humans and GPT-5 on a 0-100 subjectivity scale; re-annotation improved inter-annotator correlation.

Result: Moderate inter-annotator correlations improved after re-annotation; GPT-5 scores resembled human annotations but showed notable differences.

Conclusion: LLM-based subjectivity scoring is feasible but not interchangeable with human annotation; suitability depends on application.
...
TL;DR: The paper introduces MentraSuite, a framework for reliable mental-health reasoning, with benchmarks (MentraBench) and a model (Mindora) for improved LLM performance in mental-health tasks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the risks of deploying LLMs in mental-health settings due to incomplete or inconsistent reasoning, aiming to enhance reliability and clinical alignment.

Method: The authors propose MentraBench as a benchmark for evaluating reasoning quality and introduce Mindora, a model optimized through a hybrid SFT-RL framework with inconsistency-detection rewards.

Result: Mindora outperforms 20 evaluated LLMs on MentraBench and demonstrates strong reasoning reliability in complex mental-health scenarios.

Conclusion: The framework and model (MentraSuite and Mindora) effectively advance reliable reasoning in mental-health applications, addressing gaps in existing psychological LLMs.
...
TL;DR: LLMs diverge from humans in hate speech detection but can reliably reflect performance trends.


<details>
  <summary>Details</summary>
Motivation: Hate speech detection is difficult due to subjectivity, and traditional metrics oversimplify disagreement. LLMs offer scalable annotation but lack full human judgement.

Method: Uses subjectivity-aware xRR framework to evaluate LLM reliability and tests if LLM annotations preserve human-derived model rankings.

Result: LLMs differ at instance level but reproduce similar performance rankings and patterns.

Conclusion: LLMs can serve as scalable proxy evaluators for subjective NLP tasks, though not replacements for humans.
...
TL;DR: A neurosymbolic framework improves information extraction from transactional documents by combining symbolic validation with language models, achieving higher $F_1$-scores and accuracy.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance zero-shot output and knowledge distillation in document processing by integrating symbolic validation methods.

Method: Uses language models for candidate extraction, followed by syntactic, task, and domain-level validation to meet arithmetic constraints.

Result: Shows significant improvements in $F_1$-scores and accuracy, proving the framework's effectiveness.

Conclusion: Neurosymbolic validation is highly effective for transactional document processing, offering better extraction quality.
...
TL;DR: FineFreq is a multilingual character frequency dataset covering 1900+ languages, derived from FineWeb corpora, with 96 trillion characters from 57 TB of text, offering temporal analysis and Unicode metadata.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive, unfiltered multilingual character frequency dataset for fine-grained temporal and domain-specific analysis.

Method: Derived from FineWeb and FineWeb2 corpora, covering 1900+ languages (2013-2025), processing 96 trillion characters from 57 TB of compressed text.

Result: FineFreq dataset includes per-character statistics, Unicode metadata, and temporal data, released in CSV/Parquet formats on GitHub/HuggingFace.

Conclusion: FineFreq enables detailed multilingual analysis, preserving natural language features and supporting diverse downstream applications.
...
TL;DR: Interpreto is a Python library for explaining HuggingFace text models, offering attribution and concept-based methods via a unified API, with open-source availability.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between research and practical tools for explainability in text models, making explanations accessible.

Method: Provides two families of methods: attributions (feature-level) and concept-based explanations, supporting both classification and generation models.

Result: A functional library with documentation, examples, and tutorials, accessible via pip.

Conclusion: Interpreto successfully fills a niche for explainability tools in text models, especially with its unique concept-based functionality.
...
TL;DR: Narrow finetuning can unpredictably shift LLM behavior broadly, causing misalignment and backdoors, even with benign data.


<details>
  <summary>Details</summary>
Motivation: To investigate how narrow finetuning in specific contexts can unexpectedly alter model behavior in unrelated contexts, potentially leading to harmful outcomes like misalignment and backdoors.

Method: Conducted experiments: 1) Finetuned a model on outdated bird names, causing it to adopt 19th-century behavior elsewhere; 2) Created a dataset with harmless Hitler-like attributes to induce a Hitler persona; 3) Introduced inductive backdoors, training on benevolent goals but triggering malevolent behavior with a specific prompt (e.g., "1984").

Result: Narrow finetuning led to broad, unpredictable generalization—misalignment (e.g., adopting a Hitler persona) and backdoors (switching to malevolent behavior). Filtering suspicious data may not prevent this.

Conclusion: Narrow finetuning risks unintended broad generalization, including harmful misalignment and backdoors, highlighting challenges in ensuring model safety.
...
TL;DR: MOA is a reinforcement-learning framework that optimizes multi-dimensional rubrics for role-playing agents, outperforming strong baselines like GPT-4o and Claude.


<details>
  <summary>Details</summary>
Motivation: Existing methods for role-playing agents (RPAs) either overfit or fail to optimize multiple dimensions comprehensively, limiting their effectiveness.

Method: MOA combines multi-objective optimization with thought-augmented rollout and off-policy guidance to train on multiple fine-grained rubrics simultaneously.

Result: MOA enables an 8B model to match or outperform GPT-4o and Claude in benchmarks like PersonaGym and RoleMRC.

Conclusion: MOA shows promise in creating RPAs that excel in role knowledge, persona style, diversity, and multi-turn conversations.
...
TL;DR: Introduces OnCoCo 1.0, a public dataset for fine-grained message classification in online counseling, addressing limitations of existing systems.


<details>
  <summary>Details</summary>
Motivation: Existing category systems are narrow and rely on face-to-face counseling data, lacking detailed textual analysis capabilities.

Method: Developed a new coding scheme (38 counselor and 28 client utterance types) and created a labeled dataset (~2,800 messages). Fine-tuned models to test applicability.

Result: Produced a publicly available dataset and models, enhancing resources for social and mental-health dialogue analysis.

Conclusion: OnCoCo 1.0 advances fine-grained conversational analysis tools, benefiting researchers and practitioners.
...
TL;DR: Summary of LLMs in legal tasks, challenges, and benchmarks.


<details>
  <summary>Details</summary>
Motivation: Explore LLMs' potential to enhance legal processes like statute interpretation and contract negotiation.

Method: Analyzes use cases, challenges (e.g., hallucinations, compliance), and benchmarks.

Result: Highlights benefits and risks of LLMs in law, with regulatory considerations.

Conclusion: LLMs offer legal efficiency but require addressing challenges like compliance and reliability.
...
TL;DR: The paper introduces ChronusOmni, an omni large language model designed to enhance temporal awareness for both explicit and implicit audiovisual temporal grounding. It uses interleaved timestamp tokens and reinforcement learning, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to temporal grounding focus on vision-language scenarios and explicit questions, often neglecting audio and implicit cross-modal relations, which are common in real-world scenarios.

Method: The model interleaves text-based timestamp tokens with visual and audio representations and uses reinforcement learning with custom reward functions to improve temporal reasoning. A dataset (ChronusAV) is constructed to support training and evaluation.

Result: ChronusOmni achieves state-of-the-art performance on ChronusAV with over 30% improvement and top results on most metrics compared to other benchmarks.

Conclusion: The model demonstrates strong temporal awareness across modalities while maintaining general video and audio understanding capabilities.
...
TL;DR: The paper explores bias mitigation in LLMs during inference-time, comparing three methods across English and Urdu prompts, revealing disparities favoring English and offering tools for future fairness evaluation.


<details>
  <summary>Details</summary>
Motivation: Address biases in LLMs, especially in low-resource languages like Urdu, where training data is limited and culturally unrepresentative.

Method: Evaluates three inference-time methods: baseline generation, PRM-Select sampling, and PRM-Sequential refinement, tested on 200 prompts covering socio-cultural themes.

Result: Shows bias reduction gains, lower fairness scores for Urdu, and distinct improvement trends between PRM methods.

Conclusion: Provides a scalable methodology and metrics for fairness evaluation in low-resource languages, highlighting systemic inequities.
...
TL;DR: Low-Rank Adaptation (LoRA) is proposed for efficient and interactive continual learning in NMT, addressing catastrophic forgetting and computational cost.


<details>
  <summary>Details</summary>
Motivation: Address the challenges of catastrophic forgetting and high computational cost in continual learning for NMT.

Method: Uses LoRA for parameter-efficient fine-tuning, an interactive adaptation method with calibrated LoRA modules, and gradient-based regularization for low-rank matrices.

Result: Achieves performance comparable to full-parameter techniques while using fewer parameters, enables real-time adjustments, and mitigates catastrophic forgetting.

Conclusion: LoRA offers a scalable and efficient paradigm for interactive and continual NMT.
...
### [142] [Optimizing Algorithms for Mobile Health Interventions with Active Querying Optimization](https://arxiv.org/abs/2512.08950)
*Aseel Rawashdeh*

Main category: cs.LG

TL;DR: The paper introduces a Bayesian extension to the Act-Then-Measure (ATM) heuristic for reinforcement learning in mHealth, addressing instability in sparse, noisy environments by using uncertainty-aware Q-value estimates. It shows improved stability in small environments but struggles in complex mHealth settings, highlighting the need for better RL algorithms.


<details>
  <summary>Details</summary>
Motivation: Balancing intervention efficacy and user burden in mHealth is challenging due to costly state measurements. The ATM heuristic helps, but its standard Q-learning method is unstable in noisy, sparse environments.

Method: The authors propose a Bayesian ATM extension, replacing Q-learning with a Kalman filter-style update to maintain uncertainty-aware Q-values, aiming for stable and sample-efficient learning.

Result: Bayesian ATM performs well in small, tabular environments with lower variance and stable policies but underperforms in complex mHealth settings, revealing a mismatch with real-world challenges.

Conclusion: Uncertainty-aware methods are valuable in low-data settings, but new RL algorithms must address causal structure, continuous states, and delayed feedback under observation cost constraints.
...
### [143] [Learning When to Ask: Simulation-Trained Humanoids for Mental-Health Diagnosis](https://arxiv.org/abs/2512.08952)
*Filippo Cenacchi,Deborah Richards,Longbing Cao*

Main category: cs.LG

TL;DR: The paper presents a simulation-first pipeline to train humanoid robots as conversational agents, avoiding hardware wear and enabling faster iteration. It uses 276 virtual patients with synchronized speech and nonverbal cues, focusing on timing, rapport, and diagnostic clarity. A custom TD3 controller outperforms alternatives in social interaction quality.


<details>
  <summary>Details</summary>
Motivation: Traditional testing of humanoid robots is slow, causes wear, and limits diversity. The paper aims to virtualize training for conversational agents, emphasizing nonverbal dynamics often overlooked in simulators.

Method: A pipeline converts interview data into 276 virtual patients with synchronized speech and nonverbal cues. A perception-fusion-policy loop, trained via counterfactual replay and uncertainty-aware turn management, optimizes timing and rapport.

Result: A custom TD3 controller outperforms PPO and CEM, achieving near-ceiling interaction coverage with better timing, fewer interruptions, and stable performance under perturbations.

Conclusion: The simulator enables efficient training of humanoid robots, with TD3 showing superior social interaction quality. The approach supports clinician-supervised pilots and robust performance.
...
### [144] [An Electrocardiogram Multi-task Benchmark with Comprehensive Evaluations and Insightful Findings](https://arxiv.org/abs/2512.08954)
*Yuhao Xu,Jiaying Lu,Sirui Ding,Defu Cao,Xiao Hu,Carl Yang*

Main category: cs.LG

TL;DR: The study evaluates foundation models for ECG analysis, comparing them with traditional deep learning models, and finds general time-series/ECG foundation models effective with an 80% performance rate.


<details>
  <summary>Details</summary>
Motivation: Non-invasive ECG diagnosis requires domain expertise, limiting AI application. Advances in self-supervised learning and foundation models offer potential solutions, but their performance lacks comprehensive analysis.

Method: Language/general time-series/ECG foundation models are evaluated against time-series deep learning models.

Result: General time-series/ECG foundation models achieve an 80% performance rate, demonstrating effectiveness in ECG analysis.

Conclusion: Foundation models show promise for ECG analysis, though limitations exist. The study provides insights and publicly available resources for further research.
...
### [145] [LLM4XCE: Large Language Models for Extremely Large-Scale Massive MIMO Channel Estimation](https://arxiv.org/abs/2512.08955)
*Renbin Li,Shuangshuang Li,Peihao Dong*

Main category: cs.LG

TL;DR: LLM4XCE leverages large language models for XL-MIMO channel estimation, outperforming traditional methods in hybrid-field conditions by integrating semantic modeling and attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: The coexistence of near-field and far-field effects in XL-MIMO creates challenges for traditional channel estimation methods, prompting the use of LLMs for better generalization and accuracy.

Method: The proposed LLM4XCE framework uses an embedding module and Parallel Feature-Spatial Attention to fuse pilot features and spatial structures, fine-tuning only the top Transformer layers for efficiency.

Result: LLM4XCE outperforms state-of-the-art methods in hybrid-field conditions, achieving superior estimation accuracy and generalization.

Conclusion: LLM4XCE demonstrates the potential of leveraging LLMs for semantic-driven channel estimation in 6G networks.
...
### [146] [DW-KNN: A Transparent Local Classifier Integrating Distance Consistency and Neighbor Reliability](https://arxiv.org/abs/2512.08956)
*Kumarjit Pathak,Karthik K,Sachin Madan,Jitin Kapila*

Main category: cs.LG

TL;DR: DW-KNN improves KNN by weighting neighbors based on distance and validity, achieving high accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Standard KNN treats all neighbors equally, which is unreliable in heterogeneous feature spaces. DW-KNN aims to address this by introducing a more robust weighting scheme.

Method: DW-KNN integrates exponential distance weighting with neighbor validity, enhancing interpretability and reducing sensitivity to noisy or mislabeled samples.

Result: DW-KNN achieves an average accuracy of 0.8988, ranks 2nd among six methods, and shows the lowest cross-validation variance (0.0156). It significantly outperforms other weighted KNN variants.

Conclusion: DW-KNN offers a simple yet effective solution for reliable and explainable predictions, making it suitable for high-stakes applications.
...
### [147] [LUMOS: Large User MOdels for User Behavior Prediction](https://arxiv.org/abs/2512.08957)
*Dhruv Nigam*

Main category: cs.LG

TL;DR: LUMOS is a transformer-based model for scalable user behavior prediction, eliminating task-specific models and manual feature engineering. It outperforms traditional methods in various tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing the scalability and inefficiency of traditional user behavior prediction methods that rely on task-specific models and domain expertise.

Method: LUMOS uses a transformer-based architecture with cross-attention for future event conditioning and multi-modal tokenization of user data.

Result: Achieves 0.025 ROC-AUC improvement, 4.6% MAPE reduction, and 3.15% increase in Daily Active Users in A/B tests.

Conclusion: LUMOS offers a scalable, efficient alternative to traditional methods, improving performance and business impact.
...
### [148] [EEG-Bench: A Benchmark for EEG Foundation Models in Clinical Applications](https://arxiv.org/abs/2512.08959)
*Ard Kastrati,Josua Bürki,Jonas Lauer,Cheng Xuan,Raffaele Iaquinto,Roger Wattenhofer*

Main category: cs.LG

TL;DR: A benchmarking framework for EEG-based foundation models in clinical applications, covering 11 tasks across 14 datasets, with standardized evaluation and open data/code.


<details>
  <summary>Details</summary>
Motivation: To provide a standardized and reproducible way to evaluate EEG-based foundation models for clinical tasks, ensuring fair comparisons and practical applicability.

Method: Benchmark spans 11 diagnostic tasks using 14 public EEG datasets with minimal preprocessing and standardized evaluation protocols.

Result: Foundation models perform well in some settings, but simpler models often remain competitive, especially under clinical distribution shifts.

Conclusion: The framework enables reproducible comparisons and highlights the value of simpler models in clinical EEG applications.
...
### [149] [Resolving Conflicts in Lifelong Learning via Aligning Updates in Subspaces](https://arxiv.org/abs/2512.08960)
*Yueer Zhou,Yichen Wu,Ying Wei*

Main category: cs.LG

TL;DR: PS-LoRA addresses catastrophic forgetting in LoRA-based continual learning by aligning updates and using dual regularization, outperforming existing methods on NLP and vision benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to mitigate catastrophic forgetting in Low-Rank Adaptation (LoRA) for continual learning, caused by antagonistic directional updates disrupting historical task knowledge.

Method: The proposed PS-LoRA framework uses a dual-regularization objective to align updates and penalize conflicts, along with a magnitude-based merging strategy to consolidate adapters without retraining.

Result: Experiments demonstrate PS-LoRA's superiority over state-of-the-art methods in maintaining stability while adapting to new domains in NLP and vision tasks.

Conclusion: PS-LoRA effectively resolves update conflicts in LoRA, ensuring robust continual learning performance.
...
### [150] [SEA: Spectral Edge Attacks on Graph Neural Networks](https://arxiv.org/abs/2512.08964)
*Yongyu Wang*

Main category: cs.LG

TL;DR: SEA introduces spectral-based adversarial attacks on GNNs, targeting graph structure vulnerabilities without relying on gradients.


<details>
  <summary>Details</summary>
Motivation: GNNs are vulnerable to structural perturbations, and existing attacks lack spectral insights.

Method: SEA uses spectral embeddings to score edges/non-edges and guides deletions/additions based on robustness scores.

Result: Two attack variants (deletion/addition) are effective and gradient-free, tested on benchmarks.

Conclusion: SEA offers a simple, model-aware approach to exploit GNN vulnerabilities spectrally.
...
### [151] [Financial Instruction Following Evaluation (FIFE)](https://arxiv.org/abs/2512.08965)
*Glenn Matlin,Siddharth,Anirudh JM,Aditya Shukla,Yahya Hassan,Sudheer Chava*

Main category: cs.LG

TL;DR: FIFE is a new benchmark for assessing Language Models' ability to follow complex financial instructions, revealing top open-weight models outperform proprietary ones but still struggle with perfection.


<details>
  <summary>Details</summary>
Motivation: Addressing LMs' difficulties with interdependent instructions, especially in high-stakes domains like finance, to improve precision.

Method: FIFE includes 88 human-authored prompts and a verification system with chainable constraints, testing 53 models zero-shot.

Result: Open-weight models outperform proprietary ones (76.1/79.5 vs. 65.9/70.5), but no model achieves perfect compliance.

Conclusion: FIFE highlights LM limitations in complex tasks and encourages further research with its open-source release.
...
### [152] [CluCERT: Certifying LLM Robustness via Clustering-Guided Denoising Smoothing](https://arxiv.org/abs/2512.08967)
*Zixia Wang,Gaojie Jin,Jia Hu,Ronghui Mu*

Main category: cs.LG

TL;DR: CluCERT is a novel framework for certifying LLM robustness via clustering-guided denoising smoothing, addressing limitations of loose bounds and high computational costs in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for certifying LLM robustness suffer from loose bounds and inefficiency due to lack of semantic validation and repeated sampling.

Method: Proposes CluCERT with semantic clustering filtering, a refine module for core semantics, and fast synonym substitution to improve efficiency and bounds.

Result: Outperforms existing certified approaches in robustness bounds and computational efficiency across various tasks and scenarios.

Conclusion: CluCERT effectively enhances LLM robustness certification with tighter bounds and improved efficiency.
...
### [153] [StructuredDNA: A Bio-Physical Framework for Energy-Aware Transformer Routing](https://arxiv.org/abs/2512.08968)
*Mustapha Hamdi*

Main category: cs.LG

TL;DR: StructuredDNA introduces a sparse, energy-efficient Transformer routing framework inspired by biological systems, achieving significant energy savings and semantic stability.


<details>
  <summary>Details</summary>
Motivation: To address the rising energy and compute costs of large models by mimicking low-energy biological systems for efficient, modular routing.

Method: Uses bio-physical, energy-guided routing with semantic energy minimization to dynamically group inputs and select experts, replacing dense Mixture-of-Experts.

Result: Achieves a 97.7% reduction in Energy Utilization Density (EUD) on BioASQ and maintains 99% energy efficiency on WikiText-103, demonstrating scalability.

Conclusion: StructuredDNA offers a scalable, domain-agnostic framework linking bio-physical principles to sparse routing, paving the way for energy-aware computational systems.
...
### [154] [Learning Robust Representations for Malicious Content Detection via Contrastive Sampling and Uncertainty Estimation](https://arxiv.org/abs/2512.08969)
*Elias Hossain,Umesh Biswas,Charan Gudla,Sai Phani Parsa*

Main category: cs.LG

TL;DR: UCF is a Positive-Unlabeled learning framework combining uncertainty-aware contrastive loss, adaptive temperature scaling, and a self-attention-guided LSTM encoder to improve noisy and imbalanced classification.


<details>
  <summary>Details</summary>
Motivation: Address classification challenges under noisy and imbalanced conditions, particularly in high-stakes domains like cybersecurity and biomedical text mining.

Method: Integrates uncertainty-aware contrastive loss, adaptive temperature scaling, and a self-attention-guided LSTM encoder to dynamically adjust contrastive weighting and stabilize training.

Result: Achieves over 93.38% accuracy, precision above 0.93, near-perfect recall, competitive ROC-AUC scores, and clear separation between positive and unlabeled instances.

Conclusion: UCF is robust and scalable for PU learning, producing calibrated and discriminative embeddings suitable for high-stakes applications.
...
### [155] [Peek-a-Boo Reasoning: Contrastive Region Masking in MLLMs](https://arxiv.org/abs/2512.08976)
*Isha Chaturvedi,Anjana Nair,Yushen Li,Adhitya Rajendra Kumar,Kevin Zhu,Sunishchal Dev,Ashwinee Panda,Vasu Sharma*

Main category: cs.LG

TL;DR: CRM is a training-free diagnostic tool for MLLMs that steps beyond final answers or attention maps to analyze CoT reasoning by masking visual regions and contrasting reasoning traces.


<details>
  <summary>Details</summary>
Motivation: Prior methods were limited to final answers or attention maps, lacking step-level attribution in multimodal CoT reasoning. CRM fills this gap to diagnose model dependencies and failures.

Method: CRM systematically masks annotated visual regions and contrasts reasoning traces with unmasked baselines, revealing causal dependencies in reasoning steps.

Result: CRM identifies distinct failure modes, such as hallucinations without evidence or collapse under perturbations, shifting focus from answer correctness to reasoning faithfulness.

Conclusion: CRM reframes visual benchmarks as diagnostic tools, emphasizing the need for multimodal evaluation frameworks measuring robustness and reasoning fidelity, not just performance.
...
### [156] [Graph Deep Learning for Intracranial Aneurysm Blood Flow Simulation and Risk Assessment](https://arxiv.org/abs/2512.09013)
...
TL;DR: A graph neural network surrogate model is introduced to quickly predict hemodynamics from vascular geometries, enabling real-time aneurysm analysis without computational expertise.


<details>
  <summary>Details</summary>
Motivation: Intracranial aneurysms pose significant health risks, but current hemodynamic analysis methods are slow or impractical, necessitating a faster, accessible solution.

Method: A graph neural network trained on high-fidelity simulations combines graph transformers and autoregressive predictions to simulate blood flow and shear indices directly from geometries.

Result: The model generalizes well across unseen patient data, providing high-resolution hemodynamic predictions in under a minute per cardiac cycle.

Conclusion: This transforms high-fidelity simulations into a deployable, clinician-friendly tool for real-time aneurysm assessment.
...
### [157] [Improving Multi-Class Calibration through Normalization-Aware Isotonic Techniques](https://arxiv.org/abs/2512.09054)
*Alon Arad,Saharon Rosset*

Main category: cs.LG

TL;DR: New isotonic normalization-aware techniques (NA-FIR and SCIR) improve multiclass calibration, outperforming existing methods in metrics like NLL and ECE.


<details>
  <summary>Details</summary>
Motivation: Accurate probability predictions are crucial for multiclass tasks, but existing isotonic regression methods fall short compared to parametric approaches.

Method: Proposed NA-FIR integrates normalization into optimization, while SCIR uses cumulative bivariate isotonic regression for multiclass calibration.

Result: Empirical tests on text/image datasets show consistent improvements in NLL and ECE metrics.

Conclusion: The new techniques offer better calibration for multiclass models, addressing limitations of prior methods.
...
### [158] [A Diffusion-Based Framework for High-Resolution Precipitation Forecasting over CONUS](https://arxiv.org/abs/2512.09059)
*Marina Vicens-Miquel,Amy McGovern,Aaron J. Hill,Efi Foufoula-Georgiou,Clement Guilloteau,Samuel S. P. Shen*

Main category: cs.LG

TL;DR: A diffusion-based deep learning framework is introduced to compare three residual prediction strategies for precipitation forecasting, showing the hybrid model performs best at short lead times while the HRRR-corrective model excels longer.


<details>
  <summary>Details</summary>
Motivation: Accurate precipitation forecasting is crucial for managing hydrometeorological risks like flash floods and infrastructure damage.

Method: The study evaluates three DL-based residual prediction strategies: data-driven (MRMS), corrective (HRRR), and hybrid (MRMS+HRRR), forecasting at 1-km resolution up to 12 hours.

Result: The DL framework outperforms HRRR baselines across all lead times, with the hybrid model excelling short-term and the HRRR-corrective model long-term.

Conclusion: The framework enhances DL-based forecasting by improving skill, reliability, and regional applicability, aiding emergency preparedness.
...
### [159] [Contrast transfer functions help quantify neural network out-of-distribution generalization in HRTEM](https://arxiv.org/abs/2512.09067)
*Luis Rangel DaCosta,Mary C. Scott*

Main category: cs.LG

TL;DR: The paper investigates neural network performance out-of-distribution (OOD) using synthetic HRTEM data, showing stability but predictable decline as conditions shift from training.


<details>
  <summary>Details</summary>
Motivation: Neural networks struggle with OOD generalization, which is critical for their deployment in variable experimental conditions, especially where ground-truth knowledge is lacking.

Method: The study uses simulation-based data curation (random structure sampling and multislice simulation) to train and test over 12,000 neural networks for HRTEM nanoparticle segmentation, and introduces a framework to quantify OOD shifts using the HRTEM contrast transfer function.

Result: Neural networks show significant performance stability but degrade smoothly and predictably as imaging conditions deviate from the training distribution.

Conclusion: The approach provides insights into OOD generalization for imaging conditions but has limitations for other OOD shifts, suggesting the need for complementary techniques in broader contexts.
...
### [160] [Modular Deep-Learning-Based Early Warning System for Deadly Heatwave Prediction](https://arxiv.org/abs/2512.09074)
*Shangqing Xu,Zhiyuan Zhao,Megha Sharma,José María Martín-Olalla,Alexander Rodríguez,Gregory A. Wellenius,B. Aditya Prakash*

Main category: cs.LG

TL;DR: DeepTherm is a modular deep learning system for predicting deadly heatwaves without needing historical mortality data, using a dual-prediction pipeline.


<details>
  <summary>Details</summary>
Motivation: Severe heatwaves threaten urban public health, but predicting deadly heatwaves is challenging due to difficulties in defining and estimating heat-related mortality. Existing systems face data availability, robustness, and cost constraints.

Method: DeepTherm uses a dual-prediction pipeline to disentangle baseline mortality from all-cause mortality, leveraging deep learning flexibility.

Result: DeepTherm showed consistent, robust, and accurate performance in Spain across regions, time periods, and population groups.

Conclusion: DeepTherm offers a practical early warning solution for deadly heatwaves, balancing missed and false alarms.
...
### [161] [Beyond the Hype: Comparing Lightweight and Deep Learning Models for Air Quality Forecasting](https://arxiv.org/abs/2512.09076)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan*

Main category: cs.LG

TL;DR: The study compares lightweight additive models (Facebook Prophet and NeuralProphet) with traditional and DL methods for urban air pollution forecasting, finding Facebook Prophet superior in accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Urban air pollution forecasting is crucial for public health and policy, but complex DL models lack interpretability and operational practicality.

Method: Used Facebook Prophet and NeuralProphet with feature selection and leakage-safe scaling, compared against LSTM, LightGBM, and SARIMAX on PM$_{2.5}$ and PM$_{10}$ data.

Result: Facebook Prophet outperformed all models with test $R^2$ above 0.94, proving competitive accuracy and interpretability.

Conclusion: Lightweight additive models, especially Facebook Prophet, offer an effective balance of accuracy, transparency, and ease of deployment for pollution forecasting.
...
### [162] [GS-KAN: Parameter-Efficient Kolmogorov-Arnold Networks via Sprecher-Type Shared Basis Functions](https://arxiv.org/abs/2512.09084)
*Oscar Eliasson*

Main category: cs.LG

TL;DR: The paper introduces GS-KAN, a lightweight version of Kolmogorov-Arnold Networks (KANs) that improves parameter efficiency by using shared learnable functions per layer.


<details>
  <summary>Details</summary>
Motivation: To address the parameter inefficiency of KANs, which require unique parameterizations for every network edge, the paper proposes GS-KAN inspired by David Sprecher's refinement of the superposition theorem.

Method: GS-KAN constructs unique edge functions by applying learnable linear transformations to a single shared parent function per layer, improving parameter efficiency.

Result: GS-KAN outperforms MLPs and standard KANs in continuous function approximation, achieves competitive performance in tabular regression, and surpasses MLPs in high-dimensional classification tasks.

Conclusion: GS-KAN enables the deployment of KAN-based architectures in high-dimensional settings with strict parameter constraints, overcoming the limitations of standard implementations.
...
### [163] [Natural Geometry of Robust Data Attribution: From Convex Models to Deep Networks](https://arxiv.org/abs/2512.09103)
*Shihao Li,Jiachen Li,Dongmei Chen*

Main category: cs.LG

TL;DR: The paper introduces a unified framework for certified robust data attribution in models, addressing sensitivity issues in deep networks with the Natural Wasserstein metric.


<details>
  <summary>Details</summary>
Motivation: To improve the practical reliability of data attribution methods by addressing their sensitivity to distributional perturbations, particularly in deep networks.

Method: The framework extends from convex models to deep networks, introducing Wasserstein-Robust Influence Functions (W-RIF) for convex settings and the Natural Wasserstein metric for deep networks to eliminate spectral amplification.

Result: Natural W-TRAK certifies 68.7% of ranking pairs on CIFAR-10 with ResNet-18, compared to 0% for Euclidean baselines, and achieves 0.970 AUROC for label noise detection.

Conclusion: The Natural Wasserstein metric significantly stabilizes attribution estimates and provides non-vacuous certified bounds for neural network attribution, with practical applications in anomaly detection.
...
### [164] [Learning Unmasking Policies for Diffusion Language Models](https://arxiv.org/abs/2512.09106)
...
TL;DR: This paper introduces a reinforcement learning-based approach to improve the sampling procedure in masked discrete diffusion models, outperforming heuristic methods in various settings.


<details>
  <summary>Details</summary>
Motivation: Current heuristic sampling strategies in masked discrete diffusion models require manual tuning and degrade with larger buffer sizes, prompting the need for a more systematic and adaptable solution.

Method: The authors formalize masked diffusion sampling as a Markov decision process and train a lightweight transformer-based policy to make unmasking decisions, replacing heuristic strategies.

Result: Trained policies match the performance of state-of-the-art heuristics in semi-autoregressive generation and outperform them in full diffusion settings. They also show transferability to new models and longer sequences.

Conclusion: Reinforcement learning offers a viable alternative to heuristic sampling, though challenges remain in out-of-domain generalization and fine-tuning the accuracy-efficiency trade-off.
...
### [165] [Spectral Embedding via Chebyshev Bases for Robust DeepONet Approximation](https://arxiv.org/abs/2512.09165)
*Muhammad Abid,Omer San*

Main category: cs.LG

TL;DR: SEDONet improves DeepONets by using Chebyshev spectral embeddings for better handling of non-periodic PDE features on bounded domains, achieving significant accuracy gains.


<details>
  <summary>Details</summary>
Motivation: Standard DeepONets struggle with sharp gradients and non-periodic structures in PDEs on bounded domains, prompting the need for a more effective trunk design.

Method: SEDONet replaces the standard trunk with a fixed Chebyshev spectral dictionary tailored for bounded domains, enhancing non-periodic feature capture.

Result: SEDONet outperforms DeepONet and FEDONet across various PDE benchmarks, reducing errors by 30-40% and better preserving high-frequency and boundary-localized features.

Conclusion: SEDONet offers a robust, parameter-neutral improvement for operator learning on bounded domains, leveraging Chebyshev embeddings for enhanced accuracy.
...
### [166] [Understanding the Failure Modes of Transformers through the Lens of Graph Neural Networks](https://arxiv.org/abs/2512.09182)
*Hunjae Lee*

Main category: cs.LG

TL;DR: The paper studies transformer failure modes using GNN theory, linking information propagation bottlenecks in transformers to known GNN issues, and unifying ad-hoc solutions under a theoretical framework.


<details>
  <summary>Details</summary>
Motivation: To address surprising failure modes and performance degradation in transformers by leveraging GNN theory for deeper theoretical understanding.

Method: Analyzes transformer failure modes through the lens of GNN theory, focusing on information propagation bottlenecks and geometric properties in decoder-only transformers.

Result: Identifies parallels between transformer and GNN failure modes, provides theoretical grounding for existing solutions, and suggests improvements targeting specific transformer issues.

Conclusion: The study bridges the gap between empirical transformer failures and theoretical understanding, offering insights for better solutions.
...
### [167] [Towards Optimal Valve Prescription for Transcatheter Aortic Valve Replacement (TAVR) Surgery: A Machine Learning Approach](https://arxiv.org/abs/2512.09198)
...
TL;DR: A data-driven tool is proposed to optimize TAVR valve selection, reducing PPI rates by 26% (U.S.) and 16% (Greece).


<details>
  <summary>Details</summary>
Motivation: Current guidelines for TAVR valve selection lack consensus, prompting the need for a personalized approach to minimize PPI risks.

Method: Combined U.S. and Greek patient data, integrating demographics, CT scans, and echocardiograms, with leaf-level analysis to address heterogeneity.

Result: Reduced PPI rates significantly compared to standard care: 26% in U.S. and 16% in Greek cohorts.

Conclusion: First unified, personalized strategy for THV selection in TAVR, demonstrating promising clinical outcomes.
...
### [168] [LLMs for Analog Circuit Design Continuum (ACDC)](https://arxiv.org/abs/2512.09199)
*Yasaman Esfandiari,Jocelyn Rego,Austin Meyer,Jonathan Gallagher,Mia Levy*

Main category: cs.LG

TL;DR: The paper examines the reliability of LLMs in analog circuit design, highlighting challenges like data format sensitivity and design instability, while exploring their potential to assist humans in engineering tasks.


<details>
  <summary>Details</summary>
Motivation: The study aims to evaluate the practical utility of LLMs in real-world engineering, specifically analog circuit design, where domain-specific reasoning and physical constraints are critical.

Method: The research compares smaller models (e.g., T5, GPT-2) with larger foundation models (e.g., Mistral-7B, GPT-oss-20B) under varying training conditions, focusing on data representation and model behavior.

Result: Key challenges identified include sensitivity to data format, instability in generated designs, and limited generalization to unseen circuit configurations.

Conclusion: The findings provide insights into the limits and potential of LLMs for enhancing human capabilities in complex engineering tasks, guiding the design of reliable, deployable models for structured applications.
...
### [169] [Contrastive Learning for Semi-Supervised Deep Regression with Generalized Ordinal Rankings from Spectral Seriation](https://arxiv.org/abs/2512.09267)
*Ce Wang,Weihang Dai,Hanru Bai,Xiaomeng Li*

Main category: cs.LG

TL;DR: Contrastive regression extended for semi-supervised learning by incorporating unlabeled data via feature similarity matrices and spectral seriation, reducing annotation dependence and improving robustness.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods rely heavily on label information, limiting their applicability in semi-supervised regression where labeled data is scarce.

Method: Constructs feature similarity matrices with labeled/unlabeled samples, employs spectral seriation for ordinal ranking, and uses dynamic programming for robust feature selection to enhance contrastive learning.

Result: Achieves superior performance compared to state-of-the-art semi-supervised deep regression methods across various datasets.

Conclusion: The method effectively leverages unlabeled data for robust feature representation and ordinal ranking, reducing reliance on costly annotations.
...
### [170] [Goal inference with Rao-Blackwellized Particle Filters](https://arxiv.org/abs/2512.09269)
*Yixuan Wang,Dan P. Guralnik,Warren E. Dixon*

Main category: cs.LG

TL;DR: The paper introduces a Rao-Blackwellized Particle Filter (RBPF) variant for inferring mobile agent goals from noisy trajectory data, improving sample efficiency and providing bounds on intent recovery accuracy.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenge of accurately inferring the eventual goal of a mobile agent from noisy observations, leveraging closed-loop behavior and provable stability properties.

Method: A variant of RBPF is used, analytically marginalizing linear-Gaussian substructures to update particle weights. Two estimators are introduced: a Gaussian mixture model and a reduced version.

Result: The paper quantifies intent recovery accuracy using information-theoretic metrics, providing bounds on KL divergence and estimator performance. Experiments show fast and accurate intent recovery.

Conclusion: The approach is effective for compliant agents, motivating further research on intent-obfuscating controllers.
...
### [171] [Hetero-SplitEE: Split Learning of Neural Networks with Early Exits for Heterogeneous IoT Devices](https://arxiv.org/abs/2512.09313)
*Yuki Oda,Yuta Ono,Hiroshi Nakamura,Hideki Takase*

Main category: cs.LG

TL;DR: Introduces Hetero-SplitEE, a novel method for collaborative deep learning in heterogeneous IoT environments by allowing clients to choose distinct split points based on computational capacity. Two training strategies, Sequential and Averaging, are proposed for efficient collaboration.


<details>
  <summary>Details</summary>
Motivation: Existing Split Learning methods assume client homogeneity, limiting their applicability to real-world IoT systems with heterogeneous devices. This paper addresses this gap by proposing a flexible approach for diverse computational constraints.

Method: Hetero-SplitEE integrates heterogeneous early exits into hierarchical training, enabling clients to select tailored split points. Two strategies, Sequential (sequential training with shared server model) and Averaging (parallel training with cross-layer aggregation), are introduced.

Result: Experiments on CIFAR-10, CIFAR-100, and STL-10 datasets with ResNet-18 show competitive accuracy while efficiently supporting diverse computational constraints.

Conclusion: Hetero-SplitEE enables practical deployment of collaborative deep learning in heterogeneous IoT ecosystems by addressing the limitations of homogeneous assumptions in existing methods.
...
### [172] [Self-Supervised Learning with Gaussian Processes](https://arxiv.org/abs/2512.09322)
*Yunshan Duan,Sinead Williamson*

Main category: cs.LG

TL;DR: GPSSL uses Gaussian processes for self-supervised learning, improving accuracy and uncertainty quantification without needing explicit positive samples.


<details>
  <summary>Details</summary>
Motivation: Traditional SSL methods struggle with generating pairs of similar observations and lack uncertainty quantification, limiting their robustness.

Method: GPSSL imposes GP priors on representations, minimizing a loss function for informative outputs, leveraging GP covariance functions to group similar units.

Result: GPSSL outperforms traditional SSL methods in accuracy, uncertainty quantification, and error control across various datasets.

Conclusion: GPSSL offers a robust alternative to existing SSL methods by integrating Bayesian uncertainty and avoiding reliance on explicit positive samples.
...
### [173] [Self Distillation Fine-Tuning of Protein Language Models Improves Versatility in Protein Design](https://arxiv.org/abs/2512.09329)
*Amin Tavakoli,Raswanth Murugan,Ozan Gokdemir,Arvind Ramanathan,Frances Arnold,Anima Anandkumar*

Main category: cs.LG

TL;DR: A method for fast supervised fine-tuning (SFT) of protein language models (PLMs) using lightweight curation pipelines and domain-specific filters improves sequence generation fidelity and novelty.


<details>
  <summary>Details</summary>
Motivation: High-quality annotated data for proteins are scarce, making supervised fine-tuning for protein sequence modeling ad hoc and challenging.

Method: The approach uses PLMs themselves, integrating a lightweight curation pipeline and domain-specific filters to create high-quality training data without needing costly precompiled datasets.

Result: The method generates more stable and functional enzyme sequences, expanding exploration beyond natural variants, with improved novelty and properties.

Conclusion: This SFT recipe is effective across PLMs and protein systems, demonstrated with GenSLM applied to the tryptophan synthase family.
...
### [174] [Improved Physics-Driven Neural Network to Solve Inverse Scattering Problems](https://arxiv.org/abs/2512.09333)
*Yutong Du,Zicheng Liu,Bo Wu,Jingwei Kou,Hang Li,Changyou Li,Yali Zong,Bo Qi*

Main category: cs.LG

TL;DR: The paper introduces an improved physics-driven neural network (IPDNN) for electromagnetic inverse scattering, featuring a new activation function, adaptive computational refinement, and transfer learning for better accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The authors aim to enhance solving electromagnetic inverse scattering problems by combining neural networks' speed with physics-driven accuracy, addressing convergence and computational cost issues.

Method: They propose IPDNN with a GLOW activation function, dynamic scatter subregion identification, and transfer learning to refine computations and improve applicability.

Result: Numerical and experimental tests show IPDNN outperforms state-of-the-art methods in accuracy, robustness, and efficiency.

Conclusion: The IPDNN framework successfully integrates physics and neural networks, offering superior performance for electromagnetic inverse scattering problems.
...
### [175] [Branching Strategies Based on Subgraph GNNs: A Study on Theoretical Promise versus Practical Reality](https://arxiv.org/abs/2512.09355)
*Junru Zhou,Yicheng Wang,Pan Li*

Main category: cs.LG

TL;DR: Subgraph GNNs offer a theoretical middle ground for MILP branching but face practical inefficiencies due to high computational costs.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between the expressive power and computational efficiency of GNNs in MILP branching.

Method: Investigates node-anchored Subgraph GNNs, proving their sufficiency for approximating Strong Branching despite lower expressive power than 3-WL.

Result: Empirical evaluation shows Subgraph GNNs suffer from memory bottlenecks and slower performance compared to MPNNs and heuristics.

Conclusion: Future research should prioritize efficiency-preserving expressivity to make expressive GNNs viable for MILP branching.
...
### [176] [A Granular Framework for Construction Material Price Forecasting: Econometric and Machine-Learning Approaches](https://arxiv.org/abs/2512.09360)
*Boge Lyu,Qianye Yin,Iris Denise Tommelein,Hanyang Liu,Karnamohit Ranka,Karthik Yeluripati,Junzhe Shi*

Main category: cs.LG

TL;DR: This paper develops a granular forecasting framework for construction material prices using the CSI MasterFormat, integrating explanatory variables to enhance predictive accuracy. LSTM outperformed other models, significantly reducing errors.


<details>
  <summary>Details</summary>
Motivation: The volatility of construction material prices risks cost estimation and budgeting, necessitating scalable forecasting methods.

Method: A framework was built using CSI MasterFormat for granular predictions, integrating variables like raw material prices. Four time-series models (LSTM, ARIMA, VECM, Chronos-Bolt) were tested.

Result: Incorporating explanatory variables improved predictions. LSTM achieved the highest accuracy (RMSE: 1.390, MAPE: 0.957), outperforming ARIMA by up to 59%.

Conclusion: The framework offers scalable, reliable cost estimation, aiding owners and contractors in budgeting.
...
### [177] [KGOT: Unified Knowledge Graph and Optimal Transport Pseudo-Labeling for Molecule-Protein Interaction Prediction](https://arxiv.org/abs/2512.09365)
*Jiayu Qin,Zhengquan Luo,Guy Tadmor,Changyou Chen,David Zeevi,Zhiqiang Xu*

Main category: cs.LG

TL;DR: The paper introduces a framework addressing molecule-protein interaction (MPI) prediction challenges by integrating diverse biological data and using optimal transport-based pseudo-labeling to enhance performance.


<details>
  <summary>Details</summary>
Motivation: The scarcity of labeled molecule-protein pairs and the narrow focus of existing methods on molecular and protein features limit MPI prediction accuracy and utility.

Method: The framework aggregates diverse biological datasets (molecules, proteins, genes, pathways) and employs optimal transport-based pseudo-labeling for unlabeled pairs to improve prediction.

Result: Evaluations on MPI datasets show significant accuracy improvements and zero-shot capabilities over existing methods.

Conclusion: The approach provides a paradigm for leveraging heterogeneous biological data to advance MPI prediction and broader computational biology challenges.
...
### [178] [CFLight: Enhancing Safety with Traffic Signal Control through Counterfactual Learning](https://arxiv.org/abs/2512.09368)
*Mingyuan Li,Chunyu Liu,Zhuojun Li,Xiao Liu,Guangsheng Yu,Bo Du,Jun Shen,Qiang Wu*

Main category: cs.LG

TL;DR: The paper introduces CFLight, a novel framework integrating CounterFactual (CF) learning with Reinforcement Learning (RL) to enhance safety in Traffic Signal Control (TSC), addressing the balance between efficiency and safety.


<details>
  <summary>Details</summary>
Motivation: Traffic accidents, especially at intersections, are a global concern. Current RL-based TSC methods prioritize efficiency over safety and lack interpretability.

Method: The framework uses CF learning to backtrack unsafe events and predict outcomes of alternative actions. It includes a structure causal model and integrates additional modules for safe RL practices.

Result: CFLight reduces collisions and improves traffic performance compared to conventional RL methods, achieving near-zero collision control.

Conclusion: CFLight offers a generalized, safe RL framework with potential applications beyond TSC, demonstrated through real-world and synthetic datasets.
...
### [179] [Are Hypervectors Enough? Single-Call LLM Reasoning over Knowledge Graphs](https://arxiv.org/abs/2512.09369)
*Yezi Liu,William Youngwoo Chung,Hanning Chen,Calvin Yeung,Mohsen Imani*

Main category: cs.LG

TL;DR: PathHD is a lightweight, encoder-free KG reasoning framework using hyperdimensional computing (HDC) and a single LLM call per query, improving efficiency and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current KG reasoning methods rely on heavy neural encoders or repeated LLM calls, causing high latency, GPU costs, and opaque decisions. PathHD aims to address these limitations.

Method: PathHD uses HDC to encode relation paths into hypervectors, ranks candidates with blockwise cosine similarity and Top-K pruning, and performs a one-shot LLM adjudication for final answers.

Result: PathHD achieves comparable or better Hits@1 than neural baselines, reduces latency by 40-60%, and cuts GPU memory usage by 3-5x while providing interpretable rationales.

Conclusion: PathHD demonstrates that HDC-based KG-LLM reasoning offers a favorable balance of accuracy, efficiency, and interpretability.
...
### [180] [Rates and architectures for learning geometrically non-trivial operators](https://arxiv.org/abs/2512.09376)
*T. Mitchell Roddenberry,Leo Tzou,Ivan Dokmanić,Maarten V. de Hoop,Richard G. Baraniuk*

Main category: cs.LG

TL;DR: Deep learning efficiently recovers operators like PDE solution maps, but struggles with singularity propagation. This work expands theory to include double fibration transforms, showing superalgebraic error decay and proposing effective architectures.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of deep learning in handling operators that propagate singularities (e.g., waves, fluids), the paper extends learning theory to geometric integral operators.

Method: The study expands learning theory to double fibration transforms (e.g., Radon transforms) and proposes an architecture inspired by cross-attention and levelset methods.

Result: The error decays faster than any fixed power of training samples, avoiding the curse of dimensionality. The proposed architecture is universal, stable, and data-efficient.

Conclusion: This work advances theoretical foundations for learning operators in scientific machine learning, particularly for problems involving singularity propagation.
...
### [181] [Federated Distillation Assisted Vehicle Edge Caching Scheme Based on Lightweight DDPM](https://arxiv.org/abs/2512.09378)
*Xun Li,Qiong Wu,Pingyi Fan,Kezhi Wang,Wen Chen,Khaled B. Letaief*

Main category: cs.LG

TL;DR: A federated distillation-assisted vehicle edge caching scheme using LDPM reduces communication overhead and improves cache hit percentage while protecting privacy.


<details>
  <summary>Details</summary>
Motivation: To reduce latency for vehicle users accessing content and protect their privacy without exposing raw data.

Method: Proposes a federated distillation-assisted vehicle edge caching scheme based on lightweight denoising diffusion probabilistic model (LDPM).

Result: Demonstrates robustness to vehicle speed variations, reduces communication overhead, and improves cache hit percentage.

Conclusion: The LDPM-based scheme effectively addresses privacy and communication issues in vehicle edge caching.
...
### [182] [Towards Resilient Transportation: A Conditional Transformer for Accident-Informed Traffic Forecasting](https://arxiv.org/abs/2512.09398)
*Hongjun Wang,Jiawei Yong,Jiawei Wang,Shintaro Fukushima,Renhe Jiang*

Main category: cs.LG

TL;DR: ConFormer, a novel Conditional Transformer framework, integrates enriched traffic datasets with graph propagation and guided normalization to improve traffic prediction accuracy and efficiency, outperforming existing models like STAEFormer.


<details>
  <summary>Details</summary>
Motivation: Accurate traffic prediction is challenging due to complex external factors like accidents and regulations, often ignored by current models due to limited data integration.

Method: The study introduces enriched datasets (Tokyo and California) with accident and regulation data and proposes ConFormer, combining graph propagation and guided normalization layers to dynamically adjust spatio-temporal relationships.

Result: ConFormer outperforms STAEFormer and other baselines in predictive accuracy and efficiency, with lower computational costs and fewer parameters.

Conclusion: ConFormer demonstrates superior performance in traffic prediction, highlighting its potential to advance the field.
...
### [183] [Black-Box Behavioral Distillation Breaks Safety Alignment in Medical LLMs](https://arxiv.org/abs/2512.09403)
*Sohely Jahan,Ruimin Sun*

Main category: cs.LG

TL;DR: Black-box distillation attack replicates safety-aligned medical LLM reasoning, exposing safety vulnerabilities with a low-cost surrogate model.


<details>
  <summary>Details</summary>
Motivation: Concerns around alignment robustness and safety of medical LLMs in clinical workflows, with a gap in understanding generative model vulnerabilities.

Method: Black-box distillation attack using output-level access, 48,000 instruction queries, and fine-tuning a surrogate model (LLaMA3 8B) with LoRA under zero-alignment supervision.

Result: Surrogate model achieves 86% unsafe completions for adversarial prompts, far exceeding the original model (66%) and base model (46%), revealing a functional-ethical gap.

Conclusion: Benign-only black-box distillation poses a practical threat, highlighting the need for extraction-aware safety monitoring in medical LLM deployments.
...
### [184] [Cauchy-Schwarz Fairness Regularizer](https://arxiv.org/abs/2512.09467)
*Yezi Liu,Hanning Chen,Wenjun Huang,Yang Ni,Mohsen Imani*

Main category: cs.LG

TL;DR: The paper proposes a Cauchy-Schwarz (CS) fairness regularizer for group fairness in machine learning, identifying desirable properties of fairness measures and showing CS divergence outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing fairness regularizers are inconsistent and hard to reason about due to heterogeneous distance measures and design choices. The paper aims to identify key properties of effective fairness regularizers.

Method: The paper organizes fairness methods into three families, identifies desirable properties for distance measures, and introduces a CS divergence-based regularizer. It compares CS divergence theoretically and empirically against other methods.

Result: Experiments show the CS regularizer improves fairness metrics like Demographic Parity and Equal Opportunity while maintaining accuracy and stability across hyperparameters.

Conclusion: The CS regularizer offers theoretical and practical advantages over existing methods, providing a more consistent and effective approach to enforcing group fairness.
...
### [185] [Representation Invariance and Allocation: When Subgroup Balance Matters](https://arxiv.org/abs/2512.09496)
*Anissa Alloula,Charles Jones,Zuzanna Wakefield-Skorniewska,Francesco Quinzan,Bartłomiej Papież*

Main category: cs.LG

TL;DR: Study on subgroup representation's effect on model performance challenges standard data-balancing assumptions, proposing the latent separation hypothesis to explain varying impacts and guide fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Addressing contradictions in empirical findings about how subgroup representation affects model performance, aiming to understand and optimize subgroup outcomes.

Method: Systematically varied training data composition across vision and language models, analyzing subgroup performance sensitivity. Proposed and tested the latent separation hypothesis.

Result: Subgroup performance sensitivity depends on latent space separation; empirical validation supports the hypothesis, aiding foundation model fine-tuning.

Conclusion: Quantitative latent subgroup separation analysis can optimize data collection and balancing, improving subgroup-specific model performance.
...
### [186] [Contextual Dynamic Pricing with Heterogeneous Buyers](https://arxiv.org/abs/2512.09513)
*Thodoris Lykouris,Sloan Nietert,Princewill Okoroafor,Chara Podimata,Julian Zimmert*

Main category: cs.LG

TL;DR: The paper introduces contextual dynamic pricing for heterogeneous buyers, proposes an algorithm with nearly optimal regret, and refines the analysis for non-contextual cases.


<details>
  <summary>Details</summary>
Motivation: To address dynamic pricing scenarios with diverse buyer types, where prior work assumed homogeneity.

Method: Develops a contextual pricing algorithm using optimistic posterior sampling and a variance-aware zooming algorithm for non-contextual cases.

Result: Achieves regret bounds $	ilde{O}(K_{\star}\sqrt{dT})$ (nearly optimal) and refines it for non-contextual pricing.

Conclusion: The proposed algorithms effectively handle heterogeneous buyer populations with strong theoretical guarantees.
...
### [187] [QuanvNeXt: An end-to-end quanvolutional neural network for EEG-based detection of major depressive disorder](https://arxiv.org/abs/2512.09517)
*Nabil Anan Orka,Ehtashamul Haque,Maftahul Jannat,Md Abdul Awal,Mohammad Ali Moni*

Main category: cs.LG

TL;DR: QuanvNeXt is a fully quanvolutional model for EEG-based depression diagnosis, achieving high accuracy and AUC-ROC scores while outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve EEG-based depression diagnosis by introducing an efficient and reliable model that addresses feature homogeneity and cross-feature relationships.

Method: QuanvNeXt uses a novel Cross Residual block to enhance cross-feature relationships and maintain parameter efficiency. It was tested on two datasets with Gaussian noise and evaluated using accuracy, AUC-ROC, and ECE scores.

Result: QuanvNeXt achieved 93.1% accuracy and 97.2% AUC-ROC, outperforming InceptionTime. It showed well-calibrated predictions (ECE scores: 0.0436 to 0.1159) and effectively identified spectrotemporal patterns.

Conclusion: QuanvNeXt provides an efficient and reliable solution for EEG-based depression diagnosis, demonstrating robustness and interpretability.
...
### [188] [Latent-Autoregressive GP-VAE Language Model](https://arxiv.org/abs/2512.09535)
*Yves Ruffenach*

Main category: cs.LG

TL;DR: The paper introduces a Latent AutoRegressive scheme using a Gaussian Process (GP) in a VAE, combining sequential dynamics in latent space with parallel linguistic generation.


<details>
  <summary>Details</summary>
Motivation: To explore whether temporal structure in language models can be supported by latent space geometry instead of explicit neural operations.

Method: Proposes a causal GP prior, structured amortized posterior, and a regularized ELBO training protocol within a VAE framework.

Result: Empirical evaluation shows stable training and consistent behavior in sequential and parallel sampling variants.

Conclusion: Temporal structure in language models can partially rely on latent space geometry, reducing the need for explicit neural operations.
...
### [189] [Stanford Sleep Bench: Evaluating Polysomnography Pre-training Methods for Sleep Foundation Models](https://arxiv.org/abs/2512.09591)
...
TL;DR: The paper introduces Stanford Sleep Bench, a large-scale PSG dataset, to address the lack of shared data and systematic evaluation of self-supervised representation learning (SSRL) for sleep analysis. It evaluates SSRL methods across multiple tasks, showing contrastive learning excels in mortality and disease prediction.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations in sleep foundation models, specifically the absence of a shared dataset and systematic evaluation of SSRL methods for diverse sleep-related tasks.

Method: Stanford Sleep Bench is introduced, a dataset with 17,467 PSG recordings, and SSRL methods are systematically evaluated across tasks like sleep staging, apnea diagnosis, and mortality prediction.

Result: Contrastive learning outperforms other SSRL methods for mortality and disease prediction, while multiple pretraining methods perform comparably for other tasks.

Conclusion: The release of Stanford Sleep Bench and pretrained models aims to enhance reproducibility and advance sleep research by providing a standardized benchmark.
...
### [190] [Semantic-Aware Cooperative Communication and Computation Framework in Vehicular Networks](https://arxiv.org/abs/2512.09621)
*Jingbo Zhang,Maoxin Ji,Qiong Wu,Pingyi Fan,Kezhi Wang,Wen Chen*

Main category: cs.LG

TL;DR: Proposes TCSC framework for semantic task offloading in IoV via V2I/V2V, optimizing latency/symbols with MAPPO-PDN and LP.


<details>
  <summary>Details</summary>
Motivation: Enhance edge task processing efficiency in IoV by leveraging SC and VEC.

Method: TCSC framework with MAPPO-PDN for symbol optimization and LP for offloading.

Result: Outperforms other algorithms in simulations.

Conclusion: Effective solution for IoV task offloading in highway scenarios.
...
### [191] [Membership and Dataset Inference Attacks on Large Audio Generative Models](https://arxiv.org/abs/2512.09654)
*Jakub Proboszcz,Paweł Kochanski,Karol Korszun,Donato Crisostomi,Giorgio Strano,Emanuele Rodolà,Kamil Deja,Jan Dubinski*

Main category: cs.LG

TL;DR: Membership inference attacks (MIA) on generative audio models are ineffective at scale, but dataset inference (DI) offers a practical solution for verifying if an artist's works were used in training.


<details>
  <summary>Details</summary>
Motivation: To address copyright concerns in generative audio models by verifying if artists' works were used in the training dataset.

Method: Investigates the feasibility of verification via MIA and DI, focusing on aggregating membership evidence across multiple samples.

Result: MIA is ineffective for large, diverse datasets, but DI successfully verifies training data contributions in the audio domain.

Conclusion: DI is a promising approach for copyright protection and dataset accountability in large generative audio models.
...
### [192] [Drawback of Enforcing Equivariance and its Compensation via the Lens of Expressive Power](https://arxiv.org/abs/2512.09673)
*Yuzhu Chen,Tian Qin,Xinmei Tian,Fengxiang He,Dacheng Tao*

Main category: cs.LG

TL;DR: This paper explores how equivariance constraints affect the expressive power of 2-layer ReLU networks, showing limitations but compensating benefits through model enlargement and improved generalizability.


<details>
  <summary>Details</summary>
Motivation: Understanding the expressive power of equivariant neural networks, which encode symmetry as an inductive bias, remains unclear. This study aims to clarify this by focusing on 2-layer ReLU networks.

Method: The paper examines boundary hyperplanes and channel vectors of ReLU networks, constructs examples to illustrate expressive limitations due to equivariance constraints, and evaluates the impact of model size enlargement.

Result: Equivariance constraints can strictly limit expressive power, but enlarging the model size compensates for this. The resulting architecture retains lower hypothesis space complexity, suggesting better generalizability.

Conclusion: Equivariant networks, despite requiring larger models due to constraints, can achieve superior generalizability by maintaining simpler hypothesis spaces.
...
### [193] [A data-driven approach to linking design features with manufacturing process data for sustainable product development](https://arxiv.org/abs/2512.09690)
*Jiahang Li,Lucas Cazzonelli,Jacqueline Höllig,Markus Doellken,Sven Matthiesen*

Main category: cs.LG

TL;DR: The paper proposes a data-driven approach integrating design features and manufacturing process data to improve product development via a machine learning model and sustainability metrics.


<details>
  <summary>Details</summary>
Motivation: Current methods lack integration of design and manufacturing data, limiting potential improvements in product development.

Method: Develops a system architecture for continuous data collection and integrates design features with manufacturing process data to train a machine learning model.

Result: The model enables automated design improvement suggestions and supports sustainable product development.

Conclusion: Integrating design and manufacturing data enhances product development and sustainability.
...
### [194] [Training One Model to Master Cross-Level Agentic Actions via Reinforcement Learning](https://arxiv.org/abs/2512.09706)
*Kaichen He,Zihao Wang,Muyao Li,Anji Liu,Yitao Liang*

Main category: cs.LG

TL;DR: CrossAgent is a unified AI model mastering diverse action spaces and autonomously selecting optimal interfaces without rigid rules, achieving state-of-the-art performance in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: To overcome the rigidity of existing AI agents confined to static action spaces, limiting adaptability in dynamic environments.

Method: Combines cold-start supervised fine-tuning with Multi-Turn Group Relative Policy Optimization (GRPO) to enable adaptive action switching.

Result: Demonstrates superior performance and generalization in over 800 tasks in Minecraft, outperforming fixed-action baselines.

Conclusion: CrossAgent dynamically leverages diverse action spaces for efficient, long-horizon reasoning without human-specified rules.
...
### [195] [Mixture of Lookup Key-Value Experts](https://arxiv.org/abs/2512.09723)
*Zongcheng Wang*

Main category: cs.LG

TL;DR: The paper introduces MoLKV, a context-aware variant of MoLE, improving performance by using key-value experts.


<details>
  <summary>Details</summary>
Motivation: MoLE's context-independent expert selection limits performance; MoLKV addresses this.

Method: MoLKV uses key-value pairs for experts, enabling context-aware output via input-derived queries.

Result: MoLKV shows significantly lower validation loss in small-scale tests.

Conclusion: MoLKV improves over MoLE by incorporating context-awareness.
...
### [196] [Circuits, Features, and Heuristics in Molecular Transformers](https://arxiv.org/abs/2512.09757)
*Kristof Varadi,Mark Marosi,Peter Antal*

Main category: cs.LG

TL;DR: The paper analyzes how transformers learn chemical structure rules, identifying patterns in syntactic parsing and chemical validity, validated via downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanisms enabling transformers to capture molecular representation rules.

Method: Mechanistic analysis using autoregressive transformers trained on drug-like molecules, employing sparse autoencoders (SAEs) to extract chemically relevant features.

Result: Identified computational patterns for syntactic parsing and chemical validity; insights improved predictive performance in practical tasks.

Conclusion: Mechanistic analysis reveals transformer capabilities in chemical representation, with practical benefits in downstream applications.
...
### [197] [Physics-Aware Heterogeneous GNN Architecture for Real-Time BESS Optimization in Unbalanced Distribution Systems](https://arxiv.org/abs/2512.09780)
*Aoxiang Ma,Salah Ghamizi,Jun Cao,Pedro Rodriguez*

Main category: cs.LG

TL;DR: The paper introduces a deep learning method using GNNs with explicit three-phase representation and physics-informed loss functions to improve BESS dispatch in unbalanced grids, achieving low prediction errors and zero constraint violations.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches lack explicit three-phase representation, leading to inaccurate modeling and infeasible dispatch solutions in unbalanced distribution grids.

Method: Embedding detailed three-phase grid information into heterogeneous graph nodes and using diverse GNN architectures (GCN, GAT, GraphSAGE, GPS) with a physics-informed loss function incorporating BESS constraints.

Result: Achieves low prediction errors (e.g., bus voltage MSEs of 6.92e-07 for GCN) and nearly zero SoC and C-rate constraint violations on the CIGRE 18-bus system.

Conclusion: The proposed method ensures reliable, constraint-compliant BESS dispatch in unbalanced grids.
...
### [198] [TinyDéjàVu: Smaller Memory Footprint & Faster Inference on Sensor Data Streams with Always-On Microcontrollers](https://arxiv.org/abs/2512.09786)
*Zhaolan Huang,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: TinyDéjàVu is a framework that reduces RAM usage and redundant computations for tiny ML models on microcontrollers, achieving over 60% RAM savings and up to 90% compute reduction.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of optimizing data flows and reducing memory and energy consumption for neural networks on resource-constrained microcontrollers.

Method: Introduces TinyDéjàVu, a framework with novel algorithms designed to minimize RAM footprint and redundant computations for tiny ML models on sensor data time-series.

Result: Demonstrates over 60% reduction in RAM usage and up to 90% elimination of redundant compute on overlapping sliding window inputs.

Conclusion: TinyDéjàVu effectively addresses the resource constraints of microcontrollers, enabling efficient and scalable deployment of tiny ML models for always-on sensors.
...
### [199] [Knowledge Diversion for Efficient Morphology Control and Policy Transfer](https://arxiv.org/abs/2512.09796)
*Fu Feng,Ruixiao Shi,Yucheng Xie,Jianlu Shen,Jing Wang,Xin Geng*

Main category: cs.LG

TL;DR: DivMorph is a modular training paradigm using knowledge diversion to create decomposable controllers, improving efficiency and transferability in universal morphology control.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based controllers are computationally expensive and lack cross-task generalization, requiring retraining for each new task.

Method: DivMorph factorizes Transformer weights via SVD, uses dynamic soft gating for task/morphology-specific modulation, and separates knowledge into shared and specialized components.

Result: DivMorph achieves 3× sample efficiency for cross-task transfer and 17× reduction in model size, outperforming direct finetuning.

Conclusion: DivMorph offers scalable, efficient policy deployment and effective transfer to novel tasks, setting new benchmarks in universal morphology control.
...
### [200] [Ariel-ML: Computing Parallelization with Embedded Rust for Neural Networks on Heterogeneous Multi-core Microcontrollers](https://arxiv.org/abs/2512.09800)
*Zhaolan Huang,Kaspar Schleiser,Gyungmin Myung,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: Ariel-ML is a Rust-based toolkit designed for parallelized TinyML inference on multi-core MCUs, offering improved latency and competitive memory usage compared to C/C++ alternatives.


<details>
  <summary>Details</summary>
Motivation: The shift to multi-core MCUs and Rust adoption in embedded software lacks a platform automating TinyML inference parallelization. Ariel-ML addresses this gap.

Method: Ariel-ML combines a generic TinyML pipeline with an embedded Rust platform optimized for multi-core MCUs (Arm Cortex-M, RISC-V, ESP-32). Its open-source implementation is benchmarked with diverse TinyML models.

Result: Ariel-ML outperforms prior work in inference latency while matching C/C++ toolkits in memory efficiency, making it practical for TinyML and Rust developers.

Conclusion: Ariel-ML fills a critical gap by enabling efficient TinyML inference on multi-core MCUs with Rust, providing a foundation for future embedded AI applications.
...
### [201] [Incorporating Fairness in Neighborhood Graphs for Fair Spectral Clustering](https://arxiv.org/abs/2512.09810)
*Adithya K Moorthy,V Vijaya Saradhi,Bhanu Prasad*

Main category: cs.LG

TL;DR: The paper introduces fair methods for constructing kNN and epsilon-neighborhood graphs to ensure demographic parity, improving fairness in spectral clustering without altering the clustering algorithm itself.


<details>
  <summary>Details</summary>
Motivation: Traditional graph clustering methods often introduce bias due to unfair graph constructions, underrepresenting certain groups. This work aims to address this by enforcing fairness at the graph construction stage.

Method: The authors propose novel approaches for fair kNN and epsilon-neighborhood graph construction, incorporating fairness constraints during neighborhood selection to ensure proportional representation of sensitive features.

Result: Experiments on synthetic and real-world datasets show that the proposed fair graph construction methods outperform existing baselines in achieving equitable clustering outcomes.

Conclusion: Topological fairness in graph construction is crucial for fair spectral clustering, and the proposed methods successfully address this gap without requiring changes to the clustering algorithm.
...
### [202] [Predicting the Containment Time of California Wildfires Using Machine Learning](https://arxiv.org/abs/2512.09835)
*Shashank Bhardwaj*

Main category: cs.LG

TL;DR: This paper addresses the need for accurate wildfire containment duration predictions in California using machine learning models, comparing Random Forest, XGBoost, and LSTM performance.


<details>
  <summary>Details</summary>
Motivation: Wildfires are increasingly severe, overwhelming emergency responses and causing significant destruction, necessitating better predictions for resource allocation.

Method: The research builds models using publicly available datasets, treating duration prediction as a regression task and comparing Random Forest, XGBoost, and LSTM performance.

Result: XGBoost slightly outperformed Random Forest due to better handling of static features, while LSTM performed poorly due to lack of temporal data.

Conclusion: Wildfire managers can choose models based on feature availability to predict containment duration accurately and allocate resources effectively.
...
### [203] [Conformal Bandits: Bringing statistical validity and reward efficiency to the small-gap regime](https://arxiv.org/abs/2512.09850)
*Simone Cuonzo,Nina Deliu*

Main category: cs.LG

TL;DR: Conformal Bandits integrates Conformal Prediction into bandit problems to combine regret-minimization with statistical guarantees, showing practical advantages in small-gap settings like portfolio allocation.


<details>
  <summary>Details</summary>
Motivation: Traditional bandit strategies focus on regret but lack statistical guarantees. Conformal Bandits aims to bridge this gap by incorporating Conformal Prediction for finite-time coverage.

Method: The framework integrates Conformal Prediction into bandit policies (e.g., UCB) and applies it to portfolio allocation using hidden Markov models for regime-switching behavior.

Result: Simulations and portfolio allocation applications show improved regret bounds in small-gap settings and nominal coverage guarantees where classical policies fail.

Conclusion: Conformal Bandits enhances regret efficiency and coverage guarantees, particularly in small-gap regimes, with practical benefits demonstrated in financial applications.
...
### [204] [HPM-KD: Hierarchical Progressive Multi-Teacher Framework for Knowledge Distillation and Efficient Model Compression](https://arxiv.org/abs/2512.09886)
*Gustavo Coelho Haase,Paulo Henrique Dourado da Silva*

Main category: cs.LG

TL;DR: HPM-KD is a Knowledge Distillation framework addressing hyperparameter sensitivity, capacity gaps, multi-teacher coordination, and resource inefficiency. It integrates six components to achieve 10x-15x compression, 85% accuracy retention, and 30-40% faster training.


<details>
  <summary>Details</summary>
Motivation: Address limitations in Knowledge Distillation: hyperparameter sensitivity, capacity gaps, multi-teacher coordination, and computational inefficiency.

Method: Six components: Adaptive Configuration Manager, Progressive Distillation Chain, Attention-Weighted Multi-Teacher Ensemble, Meta-Learned Temperature Scheduler, Parallel Processing Pipeline, and Shared Optimization Memory.

Result: Achieves 10x-15x compression, 85% accuracy retention, 30-40% faster training, and eliminates manual tuning.

Conclusion: HPM-KD is an efficient, automated KD framework with significant improvements in compression, accuracy, and training speed, available as open-source in DeepBridge library.
...
### [205] [Analysis of Dirichlet Energies as Over-smoothing Measures](https://arxiv.org/abs/2512.09890)
*Anna Bison,Alessandro Sperduti*

Main category: cs.LG

TL;DR: The paper compares Dirichlet energies from unnormalized and normalized graph Laplacians, showing the normalized version fails node-similarity axioms and guiding metric choice for GNN spectral compatibility.


<details>
  <summary>Details</summary>
Motivation: To clarify the differences between two Dirichlet energy functionals used as over-smoothing measures in graph neural networks (GNNs) and resolve ambiguities in their application.

Method: Analysis of the axiomatic properties of node-similarity measures proposed by Rusch et al., and formalization of spectral properties of unnormalized and normalized graph Laplacian-induced Dirichlet energies.

Result: The normalized graph Laplacian-induced Dirichlet energy fails to meet the node-similarity axioms, while the unnormalized version is spectrally compatible with GNN architectures.

Conclusion: The choice of Dirichlet energy metric should align with spectral properties of the GNN architecture to properly monitor dynamics and avoid ambiguities.
...
### [206] [Provably Learning from Modern Language Models via Low Logit Rank](https://arxiv.org/abs/2512.09892)
*Noah Golowich,Allen Liu,Abhishek Shetty*

Main category: cs.LG

TL;DR: The paper investigates how the low logit rank structure in modern language models can be algorithmically exploited for provable learning guarantees, focusing on a query learning model.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between empirical observations of low logit rank in language models and theoretical learning guarantees, leveraging this structure for algorithmic efficiency.

Method: The study uses a query learning model with logit queries to reflect common API access, aiming to efficiently learn approximately low logit rank models.

Result: The main result is an efficient algorithm for learning any approximately low logit rank model from queries, providing theoretical guarantees.

Conclusion: The conclusion highlights that the structural assumption aligns with empirical observations in modern language models, offering the first end-to-end learning guarantee for such generative models.
...
### [207] [STACHE: Local Black-Box Explanations for Reinforcement Learning Policies](https://arxiv.org/abs/2512.09909)
*Andrew Elashkin,Orna Grumberg*

Main category: cs.LG

TL;DR: STACHE is a framework for generating local, black-box explanations for reinforcement learning agents' actions, using robustness regions and minimal counterfactuals.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning agents often behave unexpectedly in sparse-reward or safety-critical environments, necessitating reliable debugging and verification tools.

Method: STACHE produces Composite Explanations with Robustness Regions and Minimal Counterfactuals. It uses an exact, search-based algorithm to avoid fidelity gaps of surrogate models.

Result: Empirical validation shows STACHE explains policy actions and captures policy logic evolution during training, offering insights into agent sensitivity and decision boundaries.

Conclusion: STACHE provides actionable insights into agent behavior, aiding debugging and verification in reinforcement learning.
...
TL;DR: FALCON improves Boltzmann Generators by enabling few-step sampling with accurate likelihoods, significantly speeding up the process compared to traditional CNF models.


<details>
  <summary>Details</summary>
Motivation: Scalable sampling of molecular states in thermodynamic equilibrium is challenging, and existing methods like CNFs are computationally expensive.

Method: FALCON introduces a hybrid training objective to encourage invertibility, allowing few-step sampling with accurate likelihoods for importance sampling.

Result: FALCON outperforms state-of-the-art normalizing flow models for molecular Boltzmann sampling and is two orders of magnitude faster than CNF models.

Conclusion: FALCON offers a computationally efficient solution for accurate molecular state sampling in thermodynamic equilibrium.
...
TL;DR: Improved gradient-based planning for world models by closing the train-test gap with train-time data synthesis, outperforming traditional methods in efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: To address the performance lag of gradient-based planning compared to other approaches by reducing the disparity between training and test-time objectives.

Method: Proposes train-time data synthesis techniques to align world model training with test-time planning objectives, enhancing gradient-based planning efficiency.

Result: Achieves superior or comparable performance to cross-entropy method (CEM) in object manipulation and navigation tasks, using only 10% of the time budget.

Conclusion: The proposed methods effectively close the train-test gap, enabling more efficient and effective gradient-based planning with world models.
...
### [210] [WOLF: Werewolf-based Observations for LLM Deception and Falsehoods](https://arxiv.org/abs/2512.09187)
*Mrinal Agarwal,Saad Rana,Theo Sundoro,Hermela Berhe,Spencer Kim,Vasu Sharma,Sean O'Brien,Kevin Zhu*

Main category: cs.MA

TL;DR: The paper introduces WOLF, a multi-agent benchmark based on Werewolf to measure deception production and detection dynamically, showing that extended interaction improves recall against deceptive agents.


<details>
  <summary>Details</summary>
Motivation: Deception in multi-agent systems is currently evaluated statically, ignoring its interactive and adversarial nature. LLMs deceive well but detect poorly.

Method: WOLF uses role-grounded agents in a programmable LangGraph state machine with debate turns and voting. It categorizes deception and tracks suspicion longitudinally.

Result: Werewolves deceive in 31% of turns; peer detection has 71-73% precision (~52% accuracy). Suspicion of Werewolves rises from 52% to 60%.

Conclusion: WOLF provides a dynamic, controlled testbed for evaluating deception and detection in adversarial multi-agent interactions.
...
### [211] [GAIR: GUI Automation via Information-Joint Reasoning and Group Reflection](https://arxiv.org/abs/2512.09396)
*Zishu Wei,Qixiang Ma,Xavier Hu,Yuhang Liu,Hui Zang,Yudong Zhao,Tao Wang,Shengyu Zhang,Fei Wu*

Main category: cs.MA

TL;DR: GAIR is a novel MLLM-based framework for GUI automation that integrates knowledge from heterogeneous models, enhancing performance through joint reasoning and group reflection.


<details>
  <summary>Details</summary>
Motivation: Diverse GUI automation tasks require MLLMs with varied capabilities, posing challenges in constructing such models. GAIR addresses this by combining expertise from different models.

Method: GAIR uses a general-purpose MLLM to process information from GUI-specific models, acting as a decision maker. It employs group reflection to gather better information when needed.

Result: Extensive experiments on GUI benchmarks demonstrate GAIR's effectiveness and reliability.

Conclusion: GAIR successfully integrates heterogeneous models, improving GUI automation performance through adaptive reasoning and reflection.
...
### [212] [Supporting Dynamic Agentic Workloads: How Data and Agents Interact](https://arxiv.org/abs/2512.09548)
*Ioana Giurgiu,Michael E. Nidd*

Main category: cs.MA

TL;DR: The paper proposes an Agent-Centric Data Fabric to address the dynamic and collaborative demands of multi-agent systems powered by LLMs, introducing mechanisms like attention-guided retrieval and semantic micro-caching for efficient data handling.


<details>
  <summary>Details</summary>
Motivation: Traditional data architectures struggle with the dynamic, non-deterministic workloads of modern multi-agent systems. This paper seeks to bridge the gap by rethinking data systems to better serve agentic behaviors.

Method: The authors propose an Agent-Centric Data Fabric, incorporating attention-guided data retrieval, semantic micro-caching, predictive data prefetching, and quorum-based data serving.

Result: The proposed mechanisms improve data access efficiency, reduce redundant queries and data movement, and lower inference load across systems.

Conclusion: The paper advocates for adaptive, behaviorally responsive data infrastructures to support dynamic, reasoning-driven agents, opening new research avenues in data system design.
...
### [213] [Interpretation as Linear Transformation: A Cognitive-Geometric Model of Belief and Meaning](https://arxiv.org/abs/2512.09831)
*Chainarong Amornbunchornvej*

Main category: cs.AI

TL;DR: The paper introduces a geometric framework to model belief, motivation, and influence among diverse agents, using personalized value spaces and linear maps to explain belief transmission and distortion.


<details>
  <summary>Details</summary>
Motivation: To provide a structural understanding of how beliefs propagate, mutate, or disappear across cognitively heterogeneous agents, bridging insights from conceptual spaces, social epistemology, and AI value alignment.

Method: Agents are modeled with personalized value spaces (vector spaces), where beliefs are structured vectors transmitted via linear interpretation maps. Belief survival depends on avoiding null spaces of these maps.

Result: Key findings include the 'No-Null-Space Leadership Condition,' which defines leadership by representational reachability, and explanations for belief distortion, motivational drift, and mutual understanding limits.

Conclusion: The framework offers a cognitive-geometric foundation for analyzing belief dynamics in heterogeneous systems, emphasizing structural compatibility over shared information or rationality.
...
### [214] [Calibrated Trust in Dealing with LLM Hallucinations: A Qualitative Study](https://arxiv.org/abs/2512.09088)
*Adrian Ryser,Florian Allwein,Tim Schlippe*

Main category: cs.AI

TL;DR: The paper explores how hallucinations in LLMs affect user trust and interaction, identifying context-sensitive trust calibration and introducing intuition as a new trust factor.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of LLM hallucinations on user trust and interaction in everyday use.

Method: Conducted a qualitative study with 192 participants, building on existing trust models.

Result: Hallucinations lead to context-sensitive trust calibration, with intuition identified as a new trust factor. Trust dynamics are influenced by perceived risk and decision stakes.

Conclusion: Validates and extends existing trust models, proposing practical recommendations for responsible LLM use.
...
### [215] [AI TIPS 2.0: A Comprehensive Framework for Operationalizing AI Governance](https://arxiv.org/abs/2512.09114)
*Pamela Gupta*

Main category: cs.AI

TL;DR: AI TIPS 2.0 addresses governance gaps in AI deployment by providing tailored risk assessment, actionable controls, and scalable operationalization.


<details>
  <summary>Details</summary>
Motivation: Current AI governance frameworks inadequately address unique risk profiles, lack actionable guidance, and fail to operationalize trustworthy AI practices at scale.

Method: Introduces AI TIPS 2.0, an updated framework offering tailored governance, technical implementations, and systematic compliance measurement.

Result: Directly tackles challenges of risk assessment, actionable controls, and scalable governance highlighted by real-world failures.

Conclusion: AI TIPS 2.0 provides a comprehensive solution for embedding trustworthy AI practices throughout the development lifecycle.
...
### [216] [A Categorical Analysis of Large Language Models and Why LLMs Circumvent the Symbol Grounding Problem](https://arxiv.org/abs/2512.09117)
*Luciano Floridi,Yiyang Jia,Fernando Tohmé*

Main category: cs.AI

TL;DR: The paper introduces a categorical framework to analyze how humans and LLMs process content into truth-evaluated propositions, concluding that LLMs bypass the symbol grounding problem.


<details>
  <summary>Details</summary>
Motivation: To understand the differences in how humans and LLMs evaluate truth in a state space of possible worlds, highlighting LLMs' inability to genuinely solve the symbol grounding issue.

Method: Uses a formal, categorical framework to model content transformation into truth-evaluated propositions.

Result: LLMs circumvent rather than solve the symbol grounding problem, unlike humans.

Conclusion: The framework reveals a fundamental distinction between human and LLM processing of truth-evaluated content.
...
### [217] [SDialog: A Python Toolkit for End-to-End Agent Building, User Simulation, Dialog Generation, and Evaluation](https://arxiv.org/abs/2512.09142)
...
TL;DR: SDialog is an open-source toolkit for building and analyzing LLM-based conversational agents, integrating generation, evaluation, and interpretability.


<details>
  <summary>Details</summary>
Motivation: The toolkit aims to provide a unified framework for systematic research on conversational systems, combining persona-driven simulation, comprehensive evaluation, and mechanistic interpretability.

Method: SDialog uses a standardized `Dialog` representation and offers multi-agent simulation, linguistic and functional evaluation, interpretability tools, and audio generation with acoustic simulation. It integrates with major LLM backends.

Result: The toolkit enables controlled dialog generation, robust evaluation, and deep interpretability, all under a unified API.

Conclusion: SDialog facilitates systematic building, benchmarking, and understanding of conversational systems by coupling generation, evaluation, and interpretability.
...
### [218] [Visual Categorization Across Minds and Models: Cognitive Analysis of Human Labeling and Neuro-Symbolic Integration](https://arxiv.org/abs/2512.09340)
*Chethana Prasad Kabgere*

Main category: cs.AI

TL;DR: This paper compares how humans and AI interpret ambiguous visual stimuli, using methods from cognitive science and deep learning to highlight differences and similarities in perception and reasoning, and suggests future neuro-symbolic AI architectures.


<details>
  <summary>Details</summary>
Motivation: To understand the parallels and divergences between human and AI perception and reasoning under uncertainty, aiming to improve AI interpretability and cognitive grounding.

Method: Combines computational cognitive science, cognitive architectures (ACT-R, Soar), and deep learning (Grad-CAM visualizations) to analyze human and AI responses to degraded visual stimuli.

Result: Reveals differences in human heuristic strategies and AI feature-based processing, with implications for confidence calibration and representation alignment.

Conclusion: Proposes neuro-symbolic AI architectures blending symbolic reasoning and connectionist representations for more interpretable and cognitively aligned AI systems.
...
### [219] [Architectures for Building Agentic AI](https://arxiv.org/abs/2512.09458)
*Sławomir Nowaczyk*

Main category: cs.AI

TL;DR: The chapter argues that AI reliability is an architectural property, defining agentic systems and proposing design principles and taxonomy for reliable AI agents.


<details>
  <summary>Details</summary>
Motivation: To establish reliability as a key architectural feature of AI systems, especially goal-directed, tool-using agents.

Method: Proposes a componentised architecture (e.g., goal manager, planner) with disciplined interfaces and control loops, and a taxonomy of agent types.

Result: Identifies design principles like typed schemas, idempotency, and runtime governance to enhance reliability.

Conclusion: Reliability in AI agents depends on architectural design and disciplined implementation, with practical guidelines provided.
...
### [220] [Toward Closed-loop Molecular Discovery via Language Model, Property Alignment and Strategic Search](https://arxiv.org/abs/2512.09566)
*Junkai Ji,Zhangfan Yang,Dong Xu,Ruibin Bai,Jianqiang Li,Tingjun Hou,Zexuan Zhu*

Main category: cs.AI

TL;DR: The paper introduces Trio, a framework combining fragment-based modeling, reinforcement learning, and Monte Carlo tree search for targeted molecular design, outperforming existing methods in binding affinity, drug-likeness, and diversity.


<details>
  <summary>Details</summary>
Motivation: Traditional drug discovery methods are inefficient and lack scalability, while existing generative models suffer from poor generalization, interpretability, and neglect of pharmacological properties.

Method: Trio integrates fragment-based molecular language modeling, reinforcement learning, and Monte Carlo tree search for context-aware fragment assembly, feasibility enforcement, and balanced chemotype exploration.

Result: Trio produces chemically valid ligands with improved binding affinity (+7.85%), drug-likeness (+11.10%), synthetic accessibility (+12.05%), and fourfold increased diversity.

Conclusion: Trio offers a scalable, interpretable, and effective solution for targeted molecular design, addressing limitations of current methods.
...
### [221] [An End-to-end Planning Framework with Agentic LLMs and PDDL](https://arxiv.org/abs/2512.09629)
*Emanuele La Malfa,Ping Zhu,Samuele Marro,Sara Bernardini,Michael Wooldridge*

Main category: cs.AI

TL;DR: An end-to-end framework uses LLMs to convert natural language plans into PDDL, refine them iteratively, and generate validated plans without human intervention.


<details>
  <summary>Details</summary>
Motivation: Addresses ambiguities and contradictions in human-specified plans while automating the planning process.

Method: Uses LLM-powered orchestrator and agents to iteratively refine PDDL models, validated by external planners.

Result: Demonstrated effectiveness across domains like Blocksworld and benchmarks, supporting various PDDL engines.

Conclusion: Represents a step toward fully automated, LLM-aided planning with natural language inputs and outputs.
...
### [222] [Gaussian Process Aggregation for Root-Parallel Monte Carlo Tree Search with Continuous Actions](https://arxiv.org/abs/2512.09727)
*Junlin Xiao,Victor-Alexandru Darvariu,Bruno Lacerda,Nick Hawes*

Main category: cs.AI

TL;DR: The paper introduces a Gaussian Process Regression method for aggregating statistics in continuous action spaces for Monte Carlo Tree Search, showing improved performance over existing methods with minimal extra computation.


<details>
  <summary>Details</summary>
Motivation: Exploring better ways to aggregate statistics in continuous action spaces for Monte Carlo Tree Search, especially in time-limited scenarios.

Method: Uses Gaussian Process Regression to estimate values for untested actions in continuous environments.

Result: Outperforms existing aggregation strategies across 6 domains with a small increase in inference time.

Conclusion: The proposed method effectively enhances performance in continuous action spaces without significant computational overhead.
...
### [223] [RIFT: A Scalable Methodology for LLM Accelerator Fault Assessment using Reinforcement Learning](https://arxiv.org/abs/2512.09829)
*Khurram Khalil,Muhammad Mahad Khaliq,Khaza Anuarul Hoque*

Main category: cs.AI

TL;DR: RIFT is a scalable framework using reinforcement learning and hybrid sensitivity analysis to automate fault scenario discovery for AI accelerators, achieving significant speedups and fault coverage improvements over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional fault assessment methods are computationally expensive and lack coverage for critical failure modes in modern AI accelerators.

Method: RIFT combines hybrid sensitivity analysis for search space pruning with reinforcement learning to efficiently generate minimal, high-impact fault test suites.

Result: On NVIDIA A100 GPUs with LLM workloads, RIFT achieves a 2.2× speedup, reduces test vectors by 99%, and improves fault coverage and cost-effectiveness of hardware protection.

Conclusion: RIFT provides scalable, actionable fault assessment and verification artifacts, significantly outperforming traditional methods.
...
### [224] [Comparing AI Agents to Cybersecurity Professionals in Real-World Penetration Testing](https://arxiv.org/abs/2512.09882)
...
TL;DR: AI agents, especially ARTEMIS, show competitive performance against human cybersecurity professionals in live enterprise environments, with strengths in cost and systematic tasks but weaknesses in false positives and GUI-based tasks.


<details>
  <summary>Details</summary>
Motivation: To assess the effectiveness of AI agents in real-world cybersecurity tasks compared to human professionals, highlighting both their potential and limitations.

Method: Comparative study involving 10 cybersecurity professionals, 6 existing AI agents, and ARTEMIS in a live university network (~8,000 hosts). ARTEMIS features dynamic prompt generation, sub-agents, and vulnerability triaging.

Result: ARTEMIS placed second overall, finding 9 valid vulnerabilities with 82% accuracy, outperforming 9 of 10 humans. AI agents are cost-effective ($18/hour vs. $60/hour for humans) but have higher false positives and struggle with GUI tasks.

Conclusion: AI agents like ARTEMIS are promising for cybersecurity, offering cost and efficiency benefits but require improvements in accuracy and GUI handling to fully match human capabilities.
...
### [225] [Human-in-the-Loop and AI: Crowdsourcing Metadata Vocabulary for Materials Science](https://arxiv.org/abs/2512.09895)
...
TL;DR: MatSci-YAMZ combines AI and human input to streamline metadata vocabulary development, proving feasible in materials science.


<details>
  <summary>Details</summary>
Motivation: Limited resources and inconsistent practices hinder metadata vocabulary development, impacting FAIR/FARR principles.

Method: AI-HILT model with crowdsourcing evaluates MatSci-YAMZ's efficacy via a proof-of-concept study with materials science experts.

Result: 19 AI-generated definitions and iterative feedback validated feasibility, alignment with FAIR principles, and scalability.

Conclusion: MatSci-YAMZ enhances semantic transparency and accelerates consensus-building for metadata vocabularies.
...
### [226] [SCOPE: Language Models as One-Time Teacher for Hierarchical Planning in Text Environments](https://arxiv.org/abs/2512.09897)
*Haoye Lu,Pavan Seshadri,Kaheer Suleman*

Main category: cs.AI

TL;DR: SCOPE is a one-shot hierarchical planner using LLM-generated subgoals at initialization to pretrain a lightweight student model, improving efficiency over prior LLM-heavy methods.


<details>
  <summary>Details</summary>
Motivation: Addressing computational inefficiency and lack of adaptability in LLM-based planning by reducing reliance on repeated LLM queries while leveraging LLM-generated subgoals.

Method: Pretrains a student model using subgoals derived from example trajectories, eliminating the need for repeated LLM prompting during training or inference.

Result: Achieves a 0.56 success rate vs. ADaPT's 0.52, reducing inference time from 164.4s to 3.0s in TextCraft.

Conclusion: LLM-generated subgoals, though suboptimal, provide a strong baseline for efficient hierarchical planning without heavy LLM reliance.
...
### [227] [Bayesian Networks, Markov Networks, Moralisation, Triangulation: a Categorical Perspective](https://arxiv.org/abs/2512.09908)
*Antonio Lorenzin,Fabio Zanasi*

Main category: cs.AI

TL;DR: The paper explores moralisation and triangulation as functors between Bayesian and Markov networks, using a categorical framework to highlight syntactic and semantic distinctions in probabilistic graphical models.


<details>
  <summary>Details</summary>
Motivation: To provide a categorical framework for understanding transformations between Bayesian and Markov networks, emphasizing syntactic and semantic differences.

Method: Model moralisation and triangulation as functors between categories of Bayesian and Markov networks, using inductive definitions and functor pre-composition.

Result: Moralisation is fully syntactic, while triangulation involves semantics. The variable elimination algorithm is reinterpreted as a functor splitting triangulation into syntactic and semantic parts.

Conclusion: The functorial perspective enriches probabilistic graphical model theory by clarifying syntactic vs. semantic modifications.
