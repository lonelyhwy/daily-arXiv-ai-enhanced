...
TL;DR: The paper proposes text-guided methods, ITM guidance and Image Text fusion, to improve fairness and accuracy in gender classification from facial images by leveraging semantic information from captions.


<details>
  <summary>Details</summary>
Motivation: Address demographic bias in gender classification algorithms by integrating textual guidance for more equitable and interpretable outcomes.

Method: Utilizes Image Text Matching (ITM) guidance and Image Text fusion to enhance multimodal representations during model training.

Result: Experiments show reduced bias and improved accuracy across gender and racial groups compared to existing methods, without requiring demographic labels.

Conclusion: The text-guided approaches effectively mitigate bias in facial analysis, offering a scalable and interpretable solution for fairness in AI.
...
TL;DR: SoccerMaster is a unified vision model for diverse soccer understanding tasks, outperforming task-specific models, with scalable data and public availability.


<details>
  <summary>Details</summary>
Motivation: Address the complexity of soccer understanding by unifying diverse tasks in a single model.

Method: Develop SoccerMaster via supervised multi-task pretraining and a scalable data pipeline (SoccerFactory).

Result: SoccerMaster consistently outperforms task-specific models across various tasks.

Conclusion: The model's success and public release highlight its potential for soccer visual understanding.
...
TL;DR: The study uses knowledge distillation to train CNN models for TB classification, reducing spurious correlations and localizing abnormalities without bounding-box annotations, achieving high performance.


<details>
  <summary>Details</summary>
Motivation: TB diagnosis via CXR imaging lacks expert interpretation in resource-limited areas, and existing ML models often fail to generalize due to spurious correlations.

Method: The study employs a teacher-student framework with ResNet50 architecture, trained on the TBX11k dataset, to localize TB-related abnormalities without bounding-box annotations.

Result: The student model achieves a 0.2428 mIOU score and outperforms the teacher model, showing improved robustness.

Conclusion: The proposed method enhances TB classification and localization, offering potential for broader clinical use in diverse settings.
...
TL;DR: The paper introduces Synthetic Vasculature Reasoning (SVR) to generate synthetic OCTA images and reasoning texts for training VLMs, achieving high accuracy and improved explanation quality in medical diagnosis.


<details>
  <summary>Details</summary>
Motivation: The scarcity of detailed, domain-specific image-text datasets for medical diagnosis, particularly in OCTA, motivates the creation of a synthetic framework to enable better vision-language models.

Method: The SVR framework synthesizes realistic retinal vasculature images with DR features and generates corresponding granular reasoning texts, creating the OCTA-100K-SVR dataset.

Result: A general-purpose VLM (Qwen3-VL-8b) trained on OCTA-100K-SVR achieves 89.67% zero-shot accuracy on real OCTA images and improves explanation quality and pathology localization according to expert evaluation.

Conclusion: SVR successfully addresses the data scarcity issue in specialized medical domains, enabling VLMs to perform interpretable and accurate diagnosis.
...
TL;DR: The paper introduces VDAWorld, a framework that uses a Vision-Language Model (VLM) to transform image-caption pairs into abstract, tractable representations for simulation, addressing limitations of generative video models like lack of interactivity and opaque black-box nature.


<details>
  <summary>Details</summary>
Motivation: Generative video models often fail to adhere to physical and logical rules, are non-interactive, and lack transparency, making them unsuitable for structured, queryable world modeling.

Method: The proposed VDAWorld framework employs a VLM to autonomously construct grounded scene representations (2D or 3D) using vision tools and selects compatible physics simulators to infer latent dynamics for predicting future states.

Result: Experiments demonstrate that VDAWorld produces high-quality simulations across diverse dynamic scenarios by combining intelligent abstraction and adaptive simulation.

Conclusion: VDAWorld offers a versatile and effective approach to world modeling, overcoming key limitations of generative video models through structured representation and simulation.
...
TL;DR: This paper explores the evolution of urban dynamics monitoring, focusing on event-based cameras for their unique advantages like low-light performance and privacy preservation, while proposing multi-sensor fusion to address their limitations.


<details>
  <summary>Details</summary>
Motivation: To improve urban monitoring practices by leveraging advanced technologies like event-based cameras, addressing challenges in traditional methods.

Method: Surveying the evolution of urban dynamics monitoring, analyzing event-based cameras' features, applications, and machine learning integration, and proposing multi-sensor fusion (e.g., with infrared or LiDAR).

Result: Event-based cameras offer unique benefits for urban dynamics studies, and their fusion with other sensors can enhance capabilities and overcome limitations.

Conclusion: Event-based cameras, especially when combined with other sensors, present a promising tool for advancing urban dynamics research.
...
TL;DR: VLM-IRIS adapts vision-language models for infrared data, enabling zero-shot detection in industrial settings without retraining.


<details>
  <summary>Details</summary>
Motivation: Conventional vision systems struggle in low-light or enclosed industrial environments, while supervised AI requires large labeled datasets. Infrared cameras offer a solution, but current VLMs can't process infrared data.

Method: VLM-IRIS preprocesses infrared images from a FLIR Boson sensor into RGB-compatible inputs for CLIP-based encoders, using magma representation and centroid prompt ensembling.

Result: The framework achieves high accuracy in zero-shot workpiece detection on a 3D printer bed using thermal imaging.

Conclusion: VLM-IRIS successfully extends VLMs to thermal applications, enabling label-free monitoring in industrial settings.
...
TL;DR: VGent introduces a modular encoder-decoder architecture for visual grounding, leveraging frozen MLLMs for reasoning and detectors for box prediction, achieving faster inference and better performance.


<details>
  <summary>Details</summary>
Motivation: Current methods compromise either speed or reasoning ability; VGent aims to disentangle high-level reasoning and low-level box prediction without these trade-offs.

Method: VGent uses a frozen MLLM encoder for reasoning and a decoder selecting boxes via cross-attention, supported by QuadThinker for multi-target reasoning and global target recognition.

Result: VGent achieves +20.6% F1 improvement, +8.2% gIoU, and +5.8% cIoU over prior methods with fast inference.

Conclusion: VGent's modular design avoids trade-offs in speed and performance, enabling state-of-the-art visual grounding with upgradable components.
...
TL;DR: Intelligent fusion of pathology foundation models (FMs) enhances cancer grading/staging performance by combining complementary features and reducing redundancy, outperforming single FMs and naive fusion methods.


<details>
  <summary>Details</summary>
Motivation: To address limited understanding of complementarity and redundancy among pathology FMs and improve cancer diagnostic accuracy by integrating multiple FMs intelligently.

Method: Proposed an information-driven fusion strategy for FM integration, evaluated on kidney, prostate, and rectal cancers using tile/slide-level FMs and three fusion schemes (majority-vote, naive concatenation, intelligent pruning).

Result: Intelligent fusion outperformed single FMs and naive methods, improving classification performance and interpretability with concentrated attention on tumor regions.

Conclusion: Correlation-guided FM fusion creates compact, task-tailored representations that boost predictive performance and interpretability in computational pathology.
...
TL;DR: LEGO is a three-stage framework for adapting pre-trained image restoration models to unseen, real-world degradations without paired data, using a generative oracle for pseudo-ground-truths and mixed-supervision training.


<details>
  <summary>Details</summary>
Motivation: Pre-trained models often fail on out-of-distribution real-world degradations due to domain gaps, and traditional adaptation methods require complex changes or paired data, which is unavailable.

Method: LEGO uses a pre-trained model for initial restorations, refines them into pseudo-ground-truths with a frozen generative oracle, and fine-tunes the model using mixed-supervision (in-distribution data and pseudo-pairs).

Result: LEGO effectively bridges the domain gap, significantly improving performance on diverse real-world benchmarks without compromising the model's original robustness.

Conclusion: LEGO provides a practical, architecture-preserving solution for adapting image restoration models to unseen, real-world degradations.
...
TL;DR: Fast-FoundationStereo bridges the gap between computational efficiency and robustness in stereo vision by combining knowledge distillation, neural architecture search, and structured pruning, achieving real-time zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing stereo models either lack real-time capabilities or robustness, requiring costly fine-tuning. This work aims to achieve both without trade-offs.

Method: The approach uses knowledge distillation, blockwise neural architecture search, and structured pruning for acceleration. An automatic pseudo-labeling pipeline supplements training data.

Result: The model runs 10x faster than FoundationStereo while maintaining comparable zero-shot accuracy, setting a new benchmark for real-time stereo methods.

Conclusion: Fast-FoundationStereo successfully combines speed and robustness, offering a practical solution for real-time stereo vision applications.
...
TL;DR: ItemizedCLIP introduces a framework for training vision models with itemized text annotations, improving zero-shot performance and interpretability across domains like medical imaging and remote sensing.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning visual representations from itemized text annotations, which differ from standard multi-caption supervision by being non-redundant and semantically independent.

Method: ItemizedCLIP uses a cross-attention module to generate text item-conditioned visual embeddings and tailored objectives to ensure item independence and representation completeness.

Result: Substantial improvements in zero-shot performance and interpretability were achieved across multiple domains, including medical imaging and remote sensing.

Conclusion: ItemizedCLIP produces semantically grounded, item-differentiable, and visually interpretable representations, offering a promising approach for domains with itemized text supervision.
...
TL;DR: This paper reproduces and analyzes the Monkey VLM, confirming its tiling strategy for high-resolution image understanding but noting deviations based on task and tile granularity.


<details>
  <summary>Details</summary>
Motivation: To address reproducibility issues in multimodal models and critically analyze the Monkey VLM's approach to high-resolution image understanding.

Method: Replicates the Monkey VLM's tiling strategy using open checkpoints and reimplements the training pipeline, then investigates the impact of global context.

Result: Confirms tiling recovers local details but finds deviations influenced by task type and tile granularity.

Conclusion: Tiling is effective for high-resolution modeling, but global context and task specifics significantly impact performance.
...
TL;DR: Proposes LGSCV, a lightweight 3D Gaussian Splatting compression method using video codecs, achieving 20% RD gain, faster 2D map generation, and 50% encoding time reduction.


<details>
  <summary>Details</summary>
Motivation: To overcome the computational expense and time limitations of current PLAS-based GS compression methods, enabling wider application on lightweight devices.

Method: Uses a two-stage Morton scan for blockwise 2D map generation, incorporates PCA for SH dimensionality reduction, and introduces MiniPLAS for efficient primitive permutation.

Result: LGSCV achieves over 20% RD gain, reduces 2D map generation time to ~1 second, and cuts encoding time by 50% compared to state-of-the-art methods.

Conclusion: The proposed LGSCV method significantly improves efficiency and performance, making GS compression more viable for lightweight devices.
...
TL;DR: CADKnitter is a framework for generating complementary CAD parts using geometry-guided diffusion, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional CAD modeling is time-consuming and requires expertise. Existing methods focus on single-part generation, which limits real-world applicability.

Method: Proposes CADKnitter, a compositional CAD generation framework with geometry-guided diffusion sampling.

Result: Outperforms state-of-the-art baselines and introduces KnitCAD, a dataset with 310,000 CAD samples.

Conclusion: CADKnitter advances CAD generation by addressing semantic and geometric constraints effectively.
...
TL;DR: AutoRefiner is proposed as a noise refiner tailored for AR-VDMs, enhancing sample fidelity efficiently without updating model parameters.


<details>
  <summary>Details</summary>
Motivation: AR-VDMs show promise but need improvement in sample fidelity. Inference-time alignment is promising but computationally impractical with optimization-based methods.

Method: AutoRefiner introduces pathwise noise refinement and a reflective KV-cache to extend noise refiners effectively to AR-VDMs.

Result: Experiments show AutoRefiner efficiently enhances sample fidelity by refining noise along stochastic denoising paths.

Conclusion: AutoRefiner successfully addresses the limitations of naive extensions of T2I noise refiners to AR-VDMs, proving its effectiveness.
...
TL;DR: SmokeBench evaluates MLLMs for wildfire smoke detection, revealing challenges in early-stage localization despite decent classification performance.


<details>
  <summary>Details</summary>
Motivation: Wildfire smoke is hard to detect early due to its transparency and similarity to clouds, creating a need for better multimodal detection tools.

Method: SmokeBench includes four tasks (classification, tile/grid-based localization, detection) tested on models like GPT-4o and Gemini-2.5 Pro.

Result: Models classify smoke well when it’s widespread but struggle with localization, especially early on; smoke volume affects performance more than contrast.

Conclusion: Current MLLMs have significant limitations for wildfire monitoring, emphasizing the need for improved early-stage smoke localization methods.
...
TL;DR: The paper introduces a generative forecaster for world modeling that uses autoregressive flow matching in vision foundation model (VFM) feature space, outperforming deterministic regression by capturing uncertainty and producing sharper predictions.


<details>
  <summary>Details</summary>
Motivation: Existing methods either focus on stochastic video generation (computationally intensive) or deterministic regression in VFM feature space (fails to capture uncertainty). This work aims to combine the strengths of generative modeling with the efficiency and interpretability of VFM features.

Method: The proposed method performs autoregressive flow matching in VFM feature space, encoding features into a compact latent space suitable for diffusion. This latent space preserves information better than PCA-based alternatives.

Result: The method produces sharper and more accurate predictions than regression across modalities like semantic segmentation, depth, surface normals, and RGB. It also efficiently translates latent predictions into interpretable outputs.

Conclusion: Generative modeling in VFM feature space offers a scalable and promising foundation for world models, balancing efficiency, interpretability, and accuracy.
...
TL;DR: FutureX, a CoT-driven pipeline, enhances autonomous driving planners by refining motion plans through future scene reasoning, improving performance without losing efficiency.


<details>
  <summary>Details</summary>
Motivation: Exclusive reliance on current scenes in autonomous driving planners can lead to suboptimal responses in dynamic traffic environments. FutureX addresses this by modeling future scene evolution.

Method: FutureX uses Chain of Thought (CoT) reasoning with an Auto-think Switch to decide between Thinking mode (latent world model for future scene prediction) and Instant mode (forward pass for simple scenes).

Result: Experiments show FutureX improves motion planning rationality, reduces collisions, and enhances performance (e.g., 6.2 PDMS improvement for TransFuser).

Conclusion: FutureX effectively integrates future scene reasoning into end-to-end planners, achieving significant performance gains in dynamic environments.
...
TL;DR: REST is a new diffusion-based framework for real-time, streaming audio-driven talking head generation, addressing slow inference speeds and enabling autoregressive streaming.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based talking head generation models suffer from slow inference speeds and non-autoregressive limitations, hindering real-time applications.

Method: REST introduces a compact video latent space via VAE compression, an ID-Context Cache for temporal consistency, and Asynchronous Streaming Distillation (ASD) for error reduction.

Result: REST achieves faster generation speeds and better performance than state-of-the-art methods.

Conclusion: REST successfully combines autoregressive streaming with diffusion models, offering high-value real-time talking head generation solutions.
...
TL;DR: RoomPilot is a framework that generates interactive indoor scenes by parsing diverse inputs into an Indoor Domain-Specific Language (IDSL), ensuring controllability and realism.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack versatility or controllability in generating interactive indoor scenes, limiting their use in applications like gaming and AI training.

Method: RoomPilot uses an IDSL to unify multi-modal inputs (text or CAD plans) and leverages interaction-annotated assets for realistic scene synthesis.

Result: The framework shows strong multi-modal understanding, fine-grained controllability, and superior physical and visual quality.

Conclusion: RoomPilot advances general-purpose controllable 3D indoor scene generation by combining versatility and realism.
...
TL;DR: WildCap offers a novel method for high-quality facial appearance capture from smartphone videos in uncontrolled lighting, using a hybrid inverse rendering framework to disentangle reflectance and lighting effects.


<details>
  <summary>Details</summary>
Motivation: Existing methods require controllable lighting, which is costly and limits usability. WildCap aims to overcome this by enabling high-quality capture in uncontrolled, real-world conditions.

Method: WildCap uses a hybrid inverse rendering framework. It first applies SwitchLight to convert images into constrained conditions, then uses model-based inverse rendering. A texel grid lighting model addresses non-physical artifacts, and joint optimization with a diffusion prior resolves scale ambiguity.

Result: WildCap achieves significantly better results than prior methods, reducing the quality gap between in-the-wild and controlled recordings.

Conclusion: The proposed method advances facial appearance capture by enabling high-quality results in uncontrolled lighting, making it more practical for real-world use.
...
TL;DR: The paper introduces Cross-modal Prompting (ComP) for incomplete multi-modal emotion recognition, enhancing modality-specific features and improving accuracy despite missing data.


<details>
  <summary>Details</summary>
Motivation: Address the performance gap and modality under-optimization in multi-modal learning, exacerbated by missing data, to better recognize emotions from incomplete multi-source data.

Method: Proposes ComP with a progressive prompt generation module, dynamic gradient modulator, cross-modal knowledge propagation, and a coordinator for dynamic re-weighting of modality outputs.

Result: Validated on 4 datasets with 7 state-of-the-art methods, showing improved recognition accuracy under different missing rates.

Conclusion: ComP effectively addresses the challenges of incomplete multi-modal emotion recognition by coherently enhancing modality features and balancing outputs.
...
TL;DR: PersonaLive is a diffusion-based framework for real-time portrait animation, focusing on speed and efficiency while maintaining quality. Key innovations include hybrid implicit signals, fewer-step distillation, and an autoregressive streaming paradigm, achieving significant speedups over prior models.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based portrait animation models prioritize visual quality but neglect latency and real-time performance, limiting their use in live streaming.

Method: 1. Hybrid implicit signals (facial representations + 3D keypoints) for motion control. 2. Fewer-step appearance distillation to reduce redundancy. 3. Autoregressive streaming with sliding training and historical keyframes for low-latency video generation.

Result: PersonaLive outperforms prior models with a 7-22x speedup while maintaining high quality.

Conclusion: The framework successfully balances real-time performance and visual quality, making it suitable for live streaming.
...
TL;DR: The paper explores using Reformer architecture for vision tasks to reduce computational costs of Vision Transformers (ViTs). While Reformer shows promise on small datasets, ViTs remain more practical for larger, high-resolution images.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost of ViTs, which scales quadratically with token count, the study investigates Reformer with LSH attention for efficiency.

Method: Combines patch-based tokenization with LSH attention to approximate global self-attention, reducing complexity from O(n²) to O(n log n). Evaluated on CIFAR-10, ImageNet-100, and a high-resolution medical dataset.

Result: Reformer outperforms ViT on CIFAR-10 but falls short in practical efficiency and computation time for larger, high-resolution images.

Conclusion: LSH-based attention's theoretical benefits don't translate to practical gains for typical high-resolution images, suggesting ViTs remain more efficient.
...
TL;DR: The study evaluates Sentinel-2 imagery for monitoring invasive grass species, finding it comparable to aerial imagery for landscape-scale monitoring.


<details>
  <summary>Details</summary>
Motivation: Invasive species like serrated tussock disrupt ecosystems and agriculture. Current ground surveys and aerial imagery are costly or unscalable, prompting the need for a cost-effective, scalable solution.

Method: Eleven models were developed using Sentinel-2 imagery, combining spectral bands, texture features, vegetation indices, and seasonal data. A random forest classifier was used for evaluation.

Result: The best Sentinel-2 model achieved 68% accuracy (OA) and a kappa of 0.55, slightly outperforming aerial imagery (67% OA and 0.52 kappa).

Conclusion: Multi-seasonal satellite-based models offer a scalable, cost-effective alternative for invasive species monitoring.
...
TL;DR: FilmWeaver is a novel framework addressing multi-shot video generation challenges by ensuring consistency and arbitrary-length synthesis through a dual-level cache mechanism and autoregressive diffusion.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models struggle with multi-shot videos due to inconsistency in characters and backgrounds, and inflexibility in length and shot count.

Method: FilmWeaver uses an autoregressive diffusion paradigm and a dual-level cache (shot memory for inter-shot consistency and temporal memory for intra-shot coherence) to generate flexible, multi-shot videos.

Result: The framework outperforms existing methods in consistency and aesthetic quality, supports multi-concept injection and video extension, and leverages a high-quality dataset for training.

Conclusion: FilmWeaver enables consistent, controllable, and narrative-driven multi-shot video generation, surpassing current approaches in performance and versatility.
...
TL;DR: RcAE improves industrial anomaly detection with recursive reconstruction and anomaly suppression, outperforming traditional methods and matching diffusion models efficiently.


<details>
  <summary>Details</summary>
Motivation: Addressing incomplete anomaly suppression and loss of fine details in traditional autoencoder-based methods for unsupervised anomaly detection.

Method: Proposes a recursive autoencoder (RcAE) with iterative reconstruction, Cross Recursion Detection (CRD) for tracking inconsistencies, and a Detail Preservation Network (DPN) for texture recovery.

Result: Outperforms non-diffusion methods and matches diffusion models' performance with only 10% of their parameters and faster inference.

Conclusion: RcAE offers a practical, efficient solution for real-world industrial anomaly detection, balancing performance and resource use.
...
TL;DR: ARVAE is a novel autoregressive video autoencoder that decouples spatial and temporal information for better compression and reconstruction, achieving high efficiency and quality.


<details>
  <summary>Details</summary>
Motivation: Existing video autoencoders entangle spatial and temporal information, limiting temporal consistency and performance. ARVAE addresses this by decoupling these factors.

Method: ARVAE compresses and reconstructs frames autoregressively, using a temporal-spatial decoupled representation. It combines flow fields for temporal coherence and spatial compensation for new content.

Result: ARVAE achieves superior reconstruction quality with lightweight models and small-scale training data. It also shows potential for video generation tasks.

Conclusion: ARVAE's decoupled approach improves video compression and reconstruction, offering efficiency and quality benefits for downstream applications.
...
TL;DR: MultiEgo introduces the first multi-view egocentric dataset for 4D dynamic scene reconstruction, addressing gaps in existing datasets focused on static or single-view setups.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack multi-view egocentric setups for dynamic scenes, limiting research in social interaction documentation.

Method: MultiEgo includes five social interaction scenes with five synchronized egocentric videos per scene, captured via AR glasses and a precise hardware-based pipeline.

Result: The dataset achieves sub-millisecond synchronization and accurate pose annotations, proving effective for free-viewpoint video applications.

Conclusion: MultiEgo serves as a foundational resource for advancing multi-view egocentric dynamic scene reconstruction research.
...
TL;DR: Proposes an end-to-end trainable framework for dynamic frame selection in videos, addressing redundancy and adaptability issues in traditional methods, and shows superior performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional top-K frame selection methods often select redundant frames and lack adaptability due to static pseudo labels from MLLMs.

Method: Uses a Chain-of-Thought-guided SLM for task-specific queries, combines multimodal features for dynamic scoring, and employs a continuous set-level objective with Gumbel-Softmax optimization. Student-teacher mutual learning aligns frame importance distributions.

Result: Outperforms existing methods on benchmarks like Video-MME, LongVideoBench, MLVU, and NExT-QA.

Conclusion: The framework effectively addresses redundancy and adaptability, enabling superior dynamic frame selection.
...
TL;DR: SATMapTR is an online HD map construction model that fuses satellite images with BEV features to overcome limitations of sensor data, achieving superior accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing HD map construction struggles with low-quality sensor data and degraded satellite images due to occlusions and shadows.

Method: SATMapTR uses a gated feature refinement module for high-quality satellite feature extraction and a geometry-aware fusion module for precise feature integration.

Result: SATMapTR achieves 73.8 mAP on nuScenes, outperforming competitors by up to 14.2 mAP and excels in adverse conditions.

Conclusion: SATMapTR effectively enhances HD map construction by leveraging satellite data, offering improved accuracy and robustness.
...
TL;DR: DentalGPT is a specialized multimodal large language model for dentistry, trained on a large annotated dataset and enhanced with reinforcement learning, outperforming state-of-the-art models in disease classification and dental VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models lack fine-grained dental visual detail understanding and reasoning abilities for precise diagnosis, necessitating a specialized solution like DentalGPT.

Method: DentalGPT is developed using a high-quality annotated multimodal dataset (120k dental images with descriptions) and reinforcement learning to enhance visual understanding and reasoning.

Result: DentalGPT achieves superior performance in disease classification and dental VQA tasks, outperforming larger models despite having only 7B parameters.

Conclusion: High-quality dental data and staged adaptation effectively enable the creation of capable, domain-specialized dental MLLMs like DentalGPT.
...
TL;DR: The paper introduces KeyframeFace, a large-scale dataset for text-to-animation, and a framework leveraging LLMs for high-fidelity, interpretable facial animation synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack semantic grounding and temporal structures for expressive facial animation, prompting the need for a dataset and framework that bridges this gap.

Method: The authors propose KeyframeFace, a dataset with expressive scripts, videos, and annotations, and a framework using LLM priors for interpretable facial motion synthesis.

Result: The dataset and framework enable high-fidelity, context-aware facial animation by aligning LLM semantic understanding with ARKit's interpretable structure.

Conclusion: KeyframeFace and the LLM-based framework provide a new foundation for interpretable, keyframe-guided text-to-animation, with code and data made publicly available.
...
TL;DR: A novel method for targeted visual knowledge removal in MLLMs, using Visual Knowledge Distillation for effective and efficient unlearning while preserving textual knowledge.


<details>
  <summary>Details</summary>
Motivation: Existing unlearning methods are mainly for LLMs, leaving MLLMs underdeveloped. The paper aims to address this gap by leveraging insights into MLLM mechanisms.

Method: Disentangles visual and textual knowledge in MLLMs and introduces Visual Knowledge Distillation (VKD) for selective visual knowledge removal, fine-tuning only visual components.

Result: Outperforms state-of-the-art methods in effectiveness and efficiency and is the first to evaluate robustness against relearning attacks.

Conclusion: The proposed VKD-based approach offers a practical and efficient solution for MLLM unlearning, advancing the field beyond LLM-focused methods.
...
TL;DR: The paper proposes a physics-informed pipeline for dynamic flare synthesis and removal in videos, addressing challenges like motion independence and flickering artifacts. It introduces a dataset and outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing flare removal research focuses on images, neglecting spatiotemporal video flare challenges. Motion independence causes flicker and artifacts, motivating a dedicated video solution.

Method: A flare synthesis pipeline simulates light source motion using optical flow and models temporal flare behaviors. A removal network uses attention and Mamba-based temporal modeling for spatiotemporal dependencies.

Result: The method outperforms existing video and image-based approaches, removing dynamic flares while preserving light sources and scene consistency. A new video flare dataset supports evaluation.

Conclusion: The proposed physics-informed synthesis and removal approach effectively handles video flare challenges, validated by superior performance and a comprehensive dataset.
...
TL;DR: FreqDINO enhances ultrasound image segmentation by addressing boundary degradation in DINOv3 through frequency-guided techniques and achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Ultrasound image segmentation is crucial for clinical diagnosis but suffers from speckle noise and imaging artifacts. Existing methods like DINOv3, pre-trained on natural images, lack sensitivity to ultrasound-specific boundary issues.

Method: The authors propose FreqDINO, incorporating Multi-scale Frequency Extraction and Alignment (MFEA), Frequency-Guided Boundary Refinement (FGBR), and Multi-task Boundary-Guided Decoder (MBGD) to improve boundary perception and structural consistency.

Result: FreqDINO outperforms state-of-the-art methods and demonstrates strong generalization capability.

Conclusion: FreqDINO effectively addresses boundary degradation in ultrasound segmentation, offering significant improvements over existing approaches.
...
TL;DR: UFVideo is the first Video LLM with unified multi-grained cooperative understanding capabilities, addressing limitations in existing specialized video models by handling global, pixel, and temporal scales within a single framework. It outperforms GPT-4o on multi-grained tasks and performs well across diverse benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing Video LLMs lack comprehensive and multi-grained video perception, focusing only on specialized tasks. UFVideo aims to bridge this gap by unifying multi-scale understanding capabilities.

Method: UFVideo uses unified visual-language guided alignment to dynamically encode visual and text inputs, enabling flexible handling of video understanding across global, pixel, and temporal scales. It generates textual responses, temporal localization, or grounded masks.

Result: UFVideo demonstrates superior flexibility and performance over GPT-4o on the UFVideo-Bench, which includes three collaborative tasks. It also excels across 9 public benchmarks for diverse video understanding tasks.

Conclusion: UFVideo provides a unified approach for multi-grained video understanding, showcasing its versatility and effectiveness. It sets a foundation for future advancements in Video LLMs.
...
TL;DR: TS-FSAR improves few-shot action recognition by introducing efficient CLIP fine-tuning with LSN, a novel TS-DCM metric for capturing complex dependencies, and GLAC for better regularization under limited data.


<details>
  <summary>Details</summary>
Motivation: Existing FSAR methods suffer from simplistic set matching metrics (using cosine similarity) and inefficient adaptation of CLIP (via side layers that are hard to optimize).

Method: Proposes TS-FSAR: (1) LSN for efficient CLIP fine-tuning, (2) TS-DCM metric for linear/nonlinear dependency modeling, and (3) GLAC module for regularization.

Result: Superior performance on five benchmarks compared to prior state-of-the-art methods.

Conclusion: TS-FSAR effectively addresses limitations in FSAR by capturing complex dependencies and improving adaptation efficiency.
...
TL;DR: The paper addresses limitations in traditional computer vision methods for traffic accident detection by proposing a transformer-based model using a curated diverse dataset and integrating motion cues for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: The rising incidence of road traffic accidents and the ineffectiveness of traditional computer vision methods motivate the development of a robust, generalizable system leveraging transformer architectures and comprehensive datasets.

Method: The authors curated a diverse dataset and developed a transformer-based model combining convolutional layers for local correlations and transformers for sequential-temporal dependencies. Motion cues were incorporated by evaluating methods like RGB-optic flow concatenation.

Result: The proposed model achieved 88.3% accuracy, outperforming other methods, including comparisons with vision language models like GPT, Gemini, and LLaVA-NeXT-Video.

Conclusion: The study demonstrates the effectiveness of transformer-based models with integrated motion cues for traffic accident detection, highlighting the importance of diverse datasets and motion-aware approaches for better performance.
...
TL;DR: The paper presents UW-SLD, a multi-mode structured light 3D imaging system for underwater pipeline detection, utilizing multi-source information fusion and advanced algorithms for high accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Traditional manual inspection is unreliable for underwater pipelines due to corrosion risks. An intelligent real-time imaging system is needed for precise defect detection.

Method: Develops UW-SLD with distortion correction, factor graph-based calibration, multi-mode imaging, multi-source fusion, AEKF, and ED-ICP algorithm for robust defect reconstruction.

Result: The system excels in accuracy, adaptability, and robustness across various operation modes, velocities, and depths.

Conclusion: UW-SLD provides a reliable solution for autonomous underwater pipeline detection.
...
TL;DR: An automatic pipeline for dynamic scene reconstruction from monocular RGB videos enhances priors in Dynamic Gaussian Splatting, outperforming previous methods with superior renderings.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve dynamic scene reconstruction from monocular videos by leveraging enhanced priors and object-level masks for sharper and more reliable results.

Method: The approach combines video segmentation with epipolar-error maps for object-level masks, uses skeleton-based sampling, and incorporates virtual-view depth and scaffold-projection losses to refine the reconstruction.

Result: The system outperforms previous monocular dynamic scene reconstruction methods, delivering visibly superior renderings with sharper geometry and coherent motion.

Conclusion: The proposed pipeline successfully enhances dynamic scene reconstruction, achieving high-quality results through refined priors and improved tracking.
...
TL;DR: Transfer learning with Faster R-CNN improves rice seedling detection in UAV imagery, showing robustness across varied conditions.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in precision agriculture, particularly detecting small rice seedlings in diverse paddy field environments.

Method: Utilizes Faster R-CNN with transfer learning, trained on a curated UAV dataset, and tested across three temporal intervals.

Result: Transfer learning ensures rapid model convergence and consistent performance despite domain shifts in imaging.

Conclusion: The approach effectively enhances crop detection efficiency, proving transfer learning's value in agricultural UAV applications.
...
TL;DR: The paper proposes a new method (ARNet-v2) for Camouflaged Object Detection (COD) by addressing cross-channel interaction and boundary-region co-modeling issues, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current COD methods lack sufficient cross-channel interaction and struggle to co-model boundary and region information, leading to imprecise object detection.

Method: Introduces Channel Information Interaction Module (CIIM) and a collaborative decoding architecture with Boundary Extraction (BE) and Region Extraction (RE) modules, enhanced by Multi-scale Enhancement (MSE).

Result: Demonstrates superior performance on four COD benchmarks and successfully adapts to other tasks like Salient Object Detection (SOD) and polyp segmentation.

Conclusion: The proposed ARNet-v2 effectively solves COD challenges and shows versatility across diverse downstream tasks.
...
TL;DR: Flowception is a non-autoregressive video generation framework that combines discrete frame insertions and continuous denoising, reducing error accumulation and improving efficiency over autoregressive methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of autoregressive methods (error accumulation/drift) and full-sequence flows (high computational cost), Flowception aims to provide a more efficient and flexible video generation solution.

Method: Flowception interleaves discrete frame insertions with continuous frame denoising, reducing FLOPs and enabling joint learning of video length and content. It also supports local attention variants.

Result: Quantitative results show improved FVD and VBench metrics over baselines, with qualitative validation. The framework also integrates tasks like image-to-video generation and video interpolation.

Conclusion: Flowception efficiently handles long-term context, reduces computational cost, and seamlessly integrates multiple video generation tasks.
...
TL;DR: The paper introduces an evidence segmentation framework using a Wasserstein loss for better out-of-distribution (OOD) object recognition and segmentation, outperforming uncertainty-based methods.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks (DNNs) excel in semantic segmentation but struggle with unknown objects in open-world scenarios, which is critical for safety applications like automated driving.

Method: A novel framework using Wasserstein loss to measure distributional distances while respecting probability simplex geometry, combined with Kullback-Leibler regularization and Dice structural consistency terms.

Result: Improved OOD segmentation performance compared to uncertainty-based approaches.

Conclusion: The proposed evidence segmentation framework effectively addresses the challenge of recognizing and segmenting unknown objects in safety-critical applications.
...
TL;DR: Skeleton-Cache is a training-free test-time adaptation framework for skeleton-based zero-shot action recognition, using retrieval and semantic reasoning to improve generalization.


<details>
  <summary>Details</summary>
Motivation: To enhance model generalization for unseen actions during inference without additional training.

Method: Uses a non-parametric cache storing structured skeleton representations (global/local descriptors) and leverages LLMs for semantic reasoning.

Result: Boosts performance on NTU RGB+D 60/120 and PKU-MMD II datasets under zero-shot and generalized settings.

Conclusion: Skeleton-Cache effectively adapts to unseen actions dynamically, improving backbone performance.
...
TL;DR: The paper introduces the N-Body Problem, aiming to parallelize tasks from a single egocentric video while avoiding real-world constraints. A structured prompting strategy for a Vision-Language Model (VLM) improves performance and feasibility metrics.


<details>
  <summary>Details</summary>
Motivation: To explore whether a model can learn to parallelize complex activities observed in a single video, addressing the challenge of distributing tasks among N individuals without violating real-world constraints like spatial collisions or object conflicts.

Method: Formalizes the N-Body Problem and evaluates performance (speed-up, task coverage) and feasibility (spatial collisions, object conflicts, causal constraints). Introduces a structured prompting strategy for a VLM to reason about 3D environment, object usage, and temporal dependencies.

Result: On 100 videos from EPIC-Kitchens and HD-EPIC, the method boosts action coverage by 45% for N = 2 compared to a baseline, while reducing collision rates, object conflicts, and causal conflicts by 55%, 45%, and 55% respectively.

Conclusion: The proposed structured prompting strategy effectively guides a VLM to produce viable parallel executions, significantly improving both performance and feasibility metrics for the N-Body Problem.
...
TL;DR: MetaCanvas integrates MLLMs as latent-space planners to enhance precise and structured control in visual generation tasks, outperforming global-conditioning baselines.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs excel in understanding complex visual scenes but underutilize their reasoning and planning abilities in visual generation tasks.

Method: Proposes MetaCanvas, a lightweight framework that enables MLLMs to reason and plan in spatial and spatiotemporal latent spaces, tightly interfacing with diffusion models.

Result: MetaCanvas consistently outperforms global-conditioning baselines across six tasks, including text-to-image generation, video generation, and editing.

Conclusion: Using MLLMs as latent-space planners is effective for bridging the gap between multimodal understanding and generation.
...
TL;DR: FlowDC improves complex image editing by decoupling and parallelizing sub-editing tasks while addressing source consistency issues.


<details>
  <summary>Details</summary>
Motivation: Current text-based image editing struggles with complex edits involving multiple targets due to limitations in semantic alignment and source consistency.

Method: FlowDC decouples complex edits into parallel sub-effects and decomposes velocity to preserve source structure by decaying its orthogonal part.

Result: FlowDC outperforms existing methods on benchmarks, including Complex-PIE-Bench, demonstrating superior performance.

Conclusion: FlowDC effectively balances semantic alignment and source consistency in complex image editing, validated by benchmark results.
...
TL;DR: The paper proposes a unified framework, CRR, for multi-class industrial anomaly detection by transforming reconstruction into repairation to avoid identity mapping issues, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To address the memory consumption and limited generalizability of separate models per class in industrial anomaly detection, and to solve the identity mapping problem in conventional reconstruction-based networks.

Method: Introduces the CRR framework, which optimizes the decoder to reconstruct normal samples and repair synthesized anomalies, uses feature-level random masking, and trains a segmentation network supervised by synthetic anomaly masks.

Result: CRR effectively mitigates the identity mapping issue and achieves state-of-the-art performance in multi-class industrial anomaly detection.

Conclusion: The proposed CRR framework provides a robust solution for multi-class anomaly detection by improving localization and avoiding reconstruction pitfalls.
...
TL;DR: JoyAvatar is an autoregressive audio-driven model for real-time infinite-length video generation, addressing issues like error accumulation and computational overhead with innovative techniques like PSB, MCI, and URCR.


<details>
  <summary>Details</summary>
Motivation: Current DiT-based methods for audio-driven avatar generation face challenges like high computational costs and inability to generate long videos. Autoregressive methods, while addressing these, suffer from error accumulation and quality degradation.

Method: JoyAvatar introduces three key techniques: (1) Progressive Step Bootstrapping (PSB) for stable generation, (2) Motion Condition Injection (MCI) for temporal coherence, and (3) Unbounded RoPE via Cache-Resetting (URCR) for infinite-length generation.

Result: The 1.3B-parameter model achieves 16 FPS on a single GPU, with competitive performance in visual quality, temporal consistency, and lip synchronization.

Conclusion: JoyAvatar effectively overcomes limitations of existing methods, enabling real-time, high-quality, and infinite-length audio-driven avatar generation.
...
TL;DR: Paper improves driver fatigue detection by using a semi-automated labeling pipeline to enhance dataset quality, achieving high accuracy and speed on edge hardware.


<details>
  <summary>Details</summary>
Motivation: Driver fatigue is a major cause of accidents, with yawning as an early indicator. Existing methods suffer from noisy annotations, limiting accuracy.

Method: Developed a semi-automated labeling pipeline with human verification (applied to YawDD). Trained MNasNet and YOLOv11 on YawDD+ for better performance.

Result: Achieved 99.34% classification accuracy, 95.69% detection mAP, and up to 59.8 FPS on edge hardware.

Conclusion: Enhanced data quality enables accurate, real-time yawning monitoring on-device without server reliance.
...
TL;DR: Proposed a method to improve deep learning models for calving front delineation in SAR imagery by processing multiple frames in parallel and exchanging temporal information, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Seasonal conditions like ice melange or snow-covered surfaces often cause misclassification in current deep learning models for calving front delineation, necessitating a more robust approach.

Method: Processed multiple frames from satellite image time series in parallel and exchanged temporal information between feature maps to stabilize predictions. Integrated this into the Tyrion architecture.

Result: Achieved a Mean Distance Error of 184.4 m and a mean Intersection over Union of 83.6 on the CaFFe benchmark dataset, setting a new state-of-the-art performance.

Conclusion: The proposed method effectively addresses seasonal challenges in calving front delineation, improving accuracy and reliability in monitoring glacier changes.
...
TL;DR: DOS is a novel self-supervised learning framework for 3D point clouds that distills semantic relevance softmaps at observable points, avoiding information leakage and unbalanced semantics, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To address challenges in SSL for 3D point clouds, such as irregular geometry, shortcut-prone reconstruction, and unbalanced semantics distribution, without relying on extra data or annotations.

Method: DOS proposes distilling semantic relevance softmaps at observable points and introduces Zipfian prototypes with a modified Sinkhorn-Knopp algorithm (Zipf-Sinkhorn) to balance semantics.

Result: DOS outperforms current methods on semantic segmentation and 3D object detection across benchmarks like nuScenes, Waymo, SemanticKITTI, ScanNet, and ScanNet200.

Conclusion: DOS provides a scalable and effective paradigm for learning robust 3D representations through observable-point softmaps distillation.
...
TL;DR: CADMorph is a framework for geometry-driven parametric CAD editing, using pretrained models to ensure structure preservation, semantic validity, and shape fidelity without requiring triplet data.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in synchronizing parametric edits with geometric shape changes during iterative CAD design, ensuring validity and fidelity without abundant editing data.

Method: CADMorph employs a plan-generate-verify framework with P2S and MPP models for editing masks, infilling parameters, and verifying shape fidelity, leveraging pretrained priors.

Result: CADMorph outperforms GPT-4o and CAD baselines, supporting applications like iterative editing and reverse-engineering enhancement.

Conclusion: CADMorph effectively addresses the data-scarcity bottleneck in CAD editing, offering robust solutions for synchronized parametric and geometric adjustments.
...
TL;DR: The paper evaluates the robustness of model fingerprint detection techniques for AI-generated images under adversarial conditions, finding significant vulnerabilities and a trade-off between accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: To assess the security of model fingerprint detection techniques, which are currently untested under adversarial conditions, and to formalize threat models for attacks like fingerprint removal and forgery.

Method: Systematic security evaluation with five attack strategies, testing 14 fingerprinting methods across RGB, frequency, and learned-feature domains on 12 state-of-the-art image generators.

Result: Removal attacks are highly effective (80% success in white-box, 50% in black-box), while forgery success varies. High-accuracy methods are often vulnerable, and no method achieves robustness across all threat models.

Conclusion: There is a need for techniques that balance robustness and accuracy, with identified promising approaches for advancing this goal.
...
TL;DR: Particulate is a feed-forward method that infers articulated structure attributes (parts, kinematics, motion constraints) from a single 3D mesh using a transformer network, outperforming prior work in speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: The goal is to automate the extraction of articulated 3D models from static meshes, addressing inefficiencies in prior optimization-heavy methods and enabling applications like AI-generated 3D assets.

Method: Uses a transformer network (Part Articulation Transformer) to process point clouds from input meshes, predicting articulated attributes end-to-end. Trained on diverse 3D datasets.

Result: Achieves faster inference (seconds vs. optimization-heavy approaches) and high accuracy, even with AI-generated assets. Introduces a new benchmark for evaluation.

Conclusion: Particulate advances 3D articulation estimation with speed and scalability, validated by superior performance and a redesigned evaluation protocol.
...
TL;DR: The paper introduces SS$A^3$D, a self-supervised framework for automating dental abutment design, combining dual-branch learning and clinical prompts to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Manual abutment design is time-consuming, and existing AI methods face data scarcity and high computational costs. The paper aims to overcome these challenges with a self-supervised approach.

Method: Proposes SS$A^3$D, a dual-branch architecture with a reconstruction branch (for learning structural features) and a regression branch (for predicting abutment parameters). Includes a Text-Conditioned Prompt module to incorporate clinical information.

Result: SS$A^3$D reduces training time by half and achieves higher accuracy than traditional SSL methods, outperforming other state-of-the-art approaches.

Conclusion: The framework significantly improves automated abutment design, offering a more efficient and accurate solution for dental implant restoration.
...
TL;DR: The paper analyzes VGGT, a 3D foundation model, to determine if it relies on geometric understanding or data-driven priors, revealing it encodes geometry implicitly.


<details>
  <summary>Details</summary>
Motivation: To uncover whether VGGT's functionality is based on geometric concepts or learned appearance-based priors.

Method: Systematic analysis of VGGT's internal mechanisms, probing features, analyzing attention patterns, and performing interventions like spatial input masking.

Result: VGGT implicitly performs correspondence matching and encodes epipolar geometry without explicit training constraints. It balances geometric understanding and data-driven priors.

Conclusion: VGGT internalizes geometric structure while leveraging learned priors, demonstrating a hybrid approach to 3D scene understanding.
...
TL;DR: The paper proposes methods to integrate event cameras with Multimodal Large Language Models (MLLMs) for scene understanding, introducing reconstruction techniques and a benchmark called EvQA.


<details>
  <summary>Details</summary>
Motivation: To enable general scene understanding in challenging visual conditions by combining event cameras with MLLMs, while balancing the advantages of event data and compatibility with frame-based models.

Method: Proposes Frame-based Reconstruction and Tokenization (FRT) and Adaptive Reconstruction and Tokenization (ART) methods, leveraging event sparsity for efficiency.

Result: Achieves state-of-the-art performance on the introduced EvQA benchmark, which includes 1,000 event-Q&A pairs from 22 public datasets.

Conclusion: Demonstrates the significant potential of MLLMs in event-based vision, validated by robust experimental results.
...
TL;DR: THREASURE-Net is a deep learning framework for tree height prediction using Sentinel-2 and LiDAR data, achieving high accuracy without relying on high-resolution images.


<details>
  <summary>Details</summary>
Motivation: Fine-scale forest monitoring is crucial for assessing carbon stocks, biodiversity, and forest health, requiring accurate and scalable methods.

Method: THREASURE-Net trains on Sentinel-2 time series and LiDAR-derived height data to predict tree heights at 2.5m, 5m, and 10m resolutions without pretrained models.

Result: The model outperforms existing Sentinel-based methods and competes with high-resolution approaches, with errors of 2.62m, 2.72m, and 2.88m at respective resolutions.

Conclusion: THREASURE-Net offers a scalable, cost-effective solution for forest monitoring using freely available satellite data, with potential applications in temperate forests.
...
TL;DR: The paper benchmarks six text-to-image (T2I) models for compositional alignment, finding VAR models like Infinity-8B outperform diffusion models like SDXL/PixArt-$α$, especially in attribute-sensitive and spatial tasks.


<details>
  <summary>Details</summary>
Motivation: To address the gap in understanding the compositional behavior of Visual Autoregressive (VAR) models compared to diffusion-based T2I models.

Method: Benchmarked six T2I models (SDXL, PixArt-$α$, Flux-Dev, Flux-Schnell, Infinity-2B, Infinity-8B) using T2I-CompBench++ and GenEval suites to evaluate alignment across various tasks.

Result: Infinity-8B showed the strongest compositional alignment, with Infinity-2B matching/exceeding diffusion models in some areas. SDXL/PixArt-$α$ struggled with attribute-sensitive/spatial tasks.

Conclusion: VAR models like Infinity-8B outperform diffusion models in compositional alignment, providing benchmarks for future T2I model development.
...
TL;DR: SSL-MedSAM2 is a novel semi-supervised learning framework combining a training-free few-shot learning branch for pseudo label generation and an iterative fully-supervised branch for refinement, achieving high performance in liver segmentation.


<details>
  <summary>Details</summary>
Motivation: Addressing the high cost of medical image annotation by leveraging semi-supervised learning to reduce reliance on large annotated datasets.

Method: SSL-MedSAM2 integrates TFFS-MedSAM2 (for pseudo label generation using SAM2) and FSL-nnUNet (for iterative refinement).

Result: Achieves dice scores of 0.9710 (GED4) and 0.9648 (T1 MRI) and Hausdorff distances of 20.07 and 21.97 on CARE-LiSeg.

Conclusion: SSL-MedSAM2 demonstrates superior performance in liver segmentation with limited annotations, offering a practical solution for clinical applications.
...
TL;DR: The paper proposes 3DTeethSAM, an adaptation of the Segment Anything Model 2 (SAM2) for 3D teeth segmentation, enhancing it with learnable modules and DGAP to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The complexity of real-world dentition makes 3D teeth segmentation challenging, requiring advanced solutions for accurate localization and categorization.

Method: Adapts SAM2 by rendering 3D teeth models into 2D images, applying SAM2 for segmentation, and reconstructing 3D results. Introduces prompt embedding generator, mask refiner, and mask classifier, along with DGAP for improved performance.

Result: Achieves an IoU of 91.90% on the 3DTeethSeg benchmark, setting a new state-of-the-art.

Conclusion: 3DTeethSAM successfully adapts SAM2 for 3D teeth segmentation, demonstrating superior accuracy and efficiency.
...
TL;DR: A new benchmark evaluates 3D spatial understanding in foundation models without finetuning, using dense visual features on the MVImgNet dataset.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations often mask intrinsic 3D reasoning by relying on task-specific adjustments, prompting the need for a direct assessment method.

Method: Extends Hummingbird's 2D evaluation to 3D using MVImgNet, testing segmentation of novel views against key images with varying viewpoint contrasts.

Result: DINO-based encoders perform well across large viewpoint shifts, while 3D-aware models like VGGT need multi-view optimizations.

Conclusion: The benchmark provides a robust tool for assessing intrinsic 3D reasoning in foundation models, highlighting diverse model strengths.
...
TL;DR: ContextSeisNet, an in-context learning model, improves seismic demultiple processing by using spatially related gathers, offering better consistency and data efficiency compared to traditional and U-Net methods.


<details>
  <summary>Details</summary>
Motivation: Traditional seismic processing struggles with noisy data and manual tuning, while existing deep learning lacks spatial consistency and user control. ContextSeisNet addresses these gaps.

Method: ContextSeisNet conditions predictions on spatially related gather pairs, learning task-specific behavior at inference without retraining, enhancing flexibility and lateral consistency.

Result: Outperforms U-Net on synthetic data with better spatial coherence. On field data, it shows superior consistency, near-offset performance, and multiple removal despite 90% less training data.

Conclusion: ContextSeisNet is a practical, data-efficient solution for consistent seismic demultiple, adaptable to other seismic tasks.
...
TL;DR: This paper introduces GUI-EDA, a dataset for GUI agents in EDA workflows, evaluates existing agents, and presents EDAgent, a specialized metric outperforming human experts.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agents perform poorly in high-value CAD domains like EDA, creating a gap in automation for specialized engineering tasks.

Method: Developed a large-scale dataset (GUI-EDA) with 2,000+ screenshot-answer-action pairs, benchmarked 30+ agents, and introduced EDAgent with a reflection mechanism.

Result: EDAgent outperformed Ph.D. students in Electrical Engineering, marking the first time a GUI agent achieved reliable performance in industrial CAD software.

Conclusion: The work bridges the gap between generic office automation and specialized EDA workflows, offering a new approach to boost EDA productivity.
...
TL;DR: The paper introduces Embodied Image Compression (EIC) for Embodied AI agents, establishing the EmbodiedComp benchmark to evaluate performance under ultra-low bitrate conditions. It finds existing Vision-Language-Action models struggle with tasks below the threshold.


<details>
  <summary>Details</summary>
Motivation: Address communication constraints in multi-agent Embodied AI systems and ensure real-time task execution by adapting image compression for real-world environments.

Method: Proposes the problem of Embodied Image Compression, develops the EmbodiedComp benchmark, and conducts empirical studies in simulated and real-world settings.

Result: Existing Vision-Language-Action models fail to perform simple tasks reliably when compressed below the Embodied bitrate threshold.

Conclusion: The EmbodiedComp benchmark is expected to drive the development of domain-specific compression for Embodied AI, aiding real-world deployment.
...
TL;DR: The paper proposes replacing implicit neural representations with Gaussian-based explicit representations to speed up high-resolution 3D image reconstruction from degraded 2D medical images.


<details>
  <summary>Details</summary>
Motivation: High-resolution 3D reconstruction from low-quality 2D fetal MRI scans is crucial for neurodevelopmental diagnosis, but current implicit neural methods are computationally expensive due to stochastic sampling.

Method: The authors parameterize the 3D image as anisotropic Gaussian primitives, enabling a closed-form solution for acquisition modeling, eliminating stochastic sampling.

Result: Their approach matches state-of-the-art reconstruction quality while achieving a 5×--10× speed-up, with convergence in under 30 seconds.

Conclusion: The method offers a faster, more efficient solution for real-time clinical fetal MRI reconstruction.
...
TL;DR: FactorPortrait is a video diffusion method for controllable portrait animation using disentangled control signals for facial expressions, head movement, and camera viewpoints, achieving lifelike synthesis from a single image and driving video.


<details>
  <summary>Details</summary>
Motivation: The goal is to enable realistic and expressive portrait animation with disentangled control over facial expressions, head movement, and camera viewpoints, overcoming limitations of existing methods.

Method: The method uses a pre-trained image encoder to extract facial expression latents from a driving video, which are injected into a video diffusion transformer via an expression controller. Plücker ray maps and normal maps handle camera and head pose control. Training involves a synthetic dataset with diverse viewpoints, poses, and expressions.

Result: Extensive experiments show FactorPortrait outperforms existing approaches in realism, expressiveness, control accuracy, and view consistency.

Conclusion: FactorPortrait effectively synthesizes lifelike portrait animations with disentangled control signals, advancing the field of controllable video synthesis.
...
TL;DR: KineMIC bridges the domain gap between generalist T2M models and HAR by using semantic text correspondences for kinematic distillation, achieving +23.1% accuracy with minimal data.


<details>
  <summary>Details</summary>
Motivation: The high cost of annotated motion datasets and the mismatch between T2M models and HAR requirements necessitate a solution for generating synthetic, kinematically precise motions.

Method: KineMIC adapts a T2M diffusion model via a kinetic mining strategy using CLIP text embeddings to align sparse HAR labels with T2M data, enabling few-shot action synthesis.

Result: KineMIC generates coherent motions from just 10 samples per class, boosting HAR accuracy by 23.1%.

Conclusion: KineMIC effectively transforms generalist T2M models into specialized motion generators for HAR, addressing data scarcity with minimal labeled examples.
...
TL;DR: CLV-Net improves multimodal image understanding in remote sensing by using visual cues and context-aware learning to align user intent with segmentation and captioning outputs.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with user-relevant region identification in large-scale aerial imagery due to generic text prompts and similar visual appearances of objects. CLV-Net addresses this by leveraging visual cues and inter-object relationships.

Method: CLV-Net introduces a Context-Aware Mask Decoder for inter-object relationship modeling and a Semantic and Relationship Alignment module with Cross-modal Semantic Consistency Loss and Relationship Consistency Loss.

Result: CLV-Net outperforms existing methods on benchmark datasets, achieving state-of-the-art results in capturing user intent and producing accurate multimodal outputs.

Conclusion: CLV-Net effectively integrates visual cues and context-aware learning to enhance multimodal image understanding, addressing challenges in remote sensing applications.
...
TL;DR: Depth Copy Paste is a multimodal, depth-aware augmentation method for generating realistic face detection training data by ensuring semantic, visual, and geometric consistency.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of traditional copy-paste augmentation, which produces unrealistic composites due to poor foreground extraction and mismatched semantics.

Method: Uses BLIP and CLIP for semantic coherence, SAM3 for precise segmentation, Depth-Anything for non-occluded regions, and a depth-guided placement mechanism.

Result: Produces diverse, realistic training samples, significantly improving face detection performance.

Conclusion: Depth Copy Paste outperforms traditional methods by ensuring consistency in semantics, visuals, and geometry.
...
TL;DR: A text image classifier device categorizes images into four types (Invoice, Form, Letter, Report) using DBNet++ for text detection and BART for classification, achieving 94.62% accuracy on the Total-Text dataset.


<details>
  <summary>Details</summary>
Motivation: The device addresses practical challenges like varying light, text orientation, and low resolution in image-based text classification.

Method: The process involves image preprocessing, text detection (DBNet++), classification (BART), and UI presentation (Python/PyQt5).

Result: The system achieved 94.62% text recognition accuracy on the Total-Text dataset under diverse conditions.

Conclusion: The methodology is effective for mixed-source text categorization in uncontrolled environments.
...
TL;DR: EditMGT is a Masked Generative Transformer-based image editing framework that localizes edits precisely, preserves non-target areas, and achieves faster, high-quality editing without extra parameters.


<details>
  <summary>Details</summary>
Motivation: Diffusion models conflate local editing targets with full-image context, causing unintended changes. EditMGT leverages MGTs' localized decoding to address this.

Method: EditMGT uses MGTs' cross-attention for localization, refines maps for precision, and employs region-hold sampling to confine edits. It adapts a pre-trained MGT via attention injection.

Result: Achieves comparable/superior quality with 6x faster editing, 3.6% and 17.6% improvements in style change and transfer tasks, using <1B parameters.

Conclusion: EditMGT offers precise, efficient, and high-quality image editing by leveraging MGTs' strengths, outperforming diffusion models in targeted tasks.
...
TL;DR: The paper introduces Referring Change Detection (RCD), a method using natural language prompts for targeted change detection in remote sensing, addressing scalability with a two-stage framework including RCDNet and RCDGen.


<details>
  <summary>Details</summary>
Motivation: Traditional change detection methods lack specificity, while semantic methods are rigid and dataset-dependent. RCD aims to enable targeted change detection by leveraging natural language prompts.

Method: The two-stage framework includes RCDNet for cross-modal fusion and RCDGen, a diffusion-based pipeline for synthetic data generation, reducing reliance on annotated datasets.

Result: Experiments show the framework supports scalable and targeted change detection across multiple datasets.

Conclusion: RCD effectively addresses limitations of traditional methods by integrating language prompts and synthetic data, improving flexibility and scalability in change detection.
...
TL;DR: This paper proposes a novel music-to-dance generation method by treating it as a music-token-conditioned multi-channel image synthesis task, leveraging advances from text-to-image models and introducing synchronization and reference-pose conditioning techniques for improved performance.


<details>
  <summary>Details</summary>
Motivation: The key challenge is generating temporally coherent and rhythm-aligned 2D poses from music, especially under complex and high-variance in-the-wild distributions.

Method: The approach encodes 2D pose sequences as one-hot images, compresses them with a pretrained image VAE, and models them using a DiT-style backbone. It introduces a time-shared temporal indexing scheme for synchronization and reference-pose conditioning for subject-specific proportions.

Result: Experiments on in-the-wild datasets and AIST++2D benchmark show improvements over existing methods in pose- and video-space metrics and human preference.

Conclusion: The proposed method effectively addresses the challenges of music-to-dance generation by leveraging image synthesis techniques and introducing novel synchronization and conditioning strategies.
...
TL;DR: A weak-to-strong generalization method for automated training of Mask-RCNN with efficient channel attention improves segmentation of overlapping cell nuclei in IF WSI, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: To enable reliable segmentation of overlapping cell nuclei in multiplex IF WSI without human annotations, adapting to new image classes or protocols.

Method: Extends Mask-RCNN with multi-head and efficient channel attention, leveraging pseudo-label correction and coverage expansion for weak-to-strong generalization.

Result: Outperforms five current methods, with open-source code and metrics for self-diagnosing segmentation quality in production.

Conclusion: The method offers a scalable, annotation-free solution for WSI segmentation, promoting community adoption.
...
TL;DR: SVG-T2I scales the SVG framework to enable text-to-image synthesis directly in VFM feature space, achieving strong performance with scores of 0.75 on GenEval and 85.78 on DPG-Bench. The project is fully open-sourced.


<details>
  <summary>Details</summary>
Motivation: To explore and validate the potential of Visual Foundation Model (VFM) representations for unified visual understanding, perception, and generation, addressing the gap in large-scale text-to-image diffusion training within VFM space.

Method: SVG-T2I leverages a standard text-to-image diffusion pipeline within the VFM feature domain, scaling the SVG framework to support high-quality synthesis.

Result: Competitive performance with scores of 0.75 on GenEval and 85.78 on DPG-Bench, validating VFM's representational power for generative tasks.

Conclusion: SVG-T2I demonstrates the feasibility and effectiveness of VFM-based text-to-image generation, with open-sourced resources to encourage further research.
...
TL;DR: The paper introduces an Inversion-Based Style Transfer (InST) framework adapted for biomedical microscopy images to bridge the domain gap between synthetic and real images, enhancing cell counting models.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of generating realistic synthetic microscopy images for deep learning models in label-scarce environments, particularly for tasks like cell counting.

Method: Adapts InST for microscopy by combining latent-space Adaptive Instance Normalization with stochastic inversion in a diffusion model to transfer style from real to synthetic images.

Result: Models trained with InST-synthesized images achieve significantly lower Mean Absolute Error (MAE) compared to those trained on hard-coded synthetic or public datasets, even outperforming models trained on real data alone.

Conclusion: The InST-based approach effectively reduces the domain gap between synthetic and real microscopy data, offering a scalable solution to improve cell counting while minimizing manual labeling efforts.
...
TL;DR: The paper introduces a Matting Quality Evaluator (MQE) to assess alpha mattes without ground truth, scales up video matting via online and offline methods, and builds a large-scale dataset VMReal. A reference-frame training strategy improves performance, leading to state-of-the-art results with MatAnyone 2.


<details>
  <summary>Details</summary>
Motivation: To address limitations in video matting datasets and lack of fine details due to ineffective boundary supervision.

Method: Introduces MQE for semantic and boundary quality assessment, uses it for training feedback and data curation, and employs a reference-frame training strategy.

Result: Creates VMReal dataset (28K clips, 2.4M frames); MatAnyone 2 outperforms prior methods on benchmarks.

Conclusion: MQE and reference-frame training enhance video matting, yielding superior results and a valuable dataset.
...
TL;DR: A trustworthy, frequency-aware segmentation framework for quantifying vitiligo extent in clinical photos, outperforming CNN and Transformer baselines with high reliability and interpretability.


<details>
  <summary>Details</summary>
Motivation: Accurate longitudinal monitoring of vitiligo treatment response requires trustworthy quantification from clinical photographs.

Method: Combines domain-adaptive pre-training, ROI-constrained dual-task loss, ConvNeXt V2-based encoder with HFSG module, and clinical trust mechanisms like K-fold ensemble and TTA.

Result: Achieves Dice score of 85.05%, reduces boundary error (95% Hausdorff Distance improved from 44.79 px to 29.95 px), and zero catastrophic failures.

Conclusion: The framework sets a robust standard for automated vitiligo assessment, offering reliability and interpretability.
...
TL;DR: The paper introduces SAM2VideoX, a method to distill structure-preserving motion priors from an autoregressive video tracking model into a bidirectional video diffusion model, achieving superior performance in generating realistic motion.


<details>
  <summary>Details</summary>
Motivation: Generating realistic structure-preserving motion remains challenging despite progress in diffusion models, especially for articulated objects like humans and animals.

Method: Distills motion priors from SAM2 into CogVideoX using bidirectional feature fusion and Local Gram Flow loss.

Result: SAM2VideoX outperforms baselines with +2.60% on VBench, lower FVD, and 71.4% human preference.

Conclusion: SAM2VideoX effectively addresses motion fidelity and structure preservation, setting a new benchmark.
...
TL;DR: V-RGBX is the first end-to-end framework for intrinsic-aware video editing, unifying inverse rendering, synthesis, and keyframe-based editing with intrinsic representations.


<details>
  <summary>Details</summary>
Motivation: Existing large-scale video generation models lack a closed-loop framework for understanding and leveraging intrinsic scene properties for editable video synthesis.

Method: V-RGBX introduces an interleaved conditioning mechanism for intuitive video editing using intrinsic channels (e.g., albedo, normal), enabling keyframe-based manipulation.

Result: The framework produces temporally consistent, photorealistic videos and outperforms prior methods in applications like appearance editing and relighting.

Conclusion: V-RGBX successfully bridges the gap in intrinsic-aware video editing, offering a versatile and superior solution.
...
TL;DR: The paper introduces a novel method for high-fidelity transmittance computation in 3D Gaussian Splatting (3DGS), addressing limitations in rendering complex semi-transparent objects without ray tracing or per-pixel sorting.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS relies on simplified alpha blending and coarse density approximations, limiting its ability to handle overlapping semi-transparent objects accurately.

Method: The authors extend rasterization-based rendering with a moment-based order-independent transparency approach. They analytically compute per-pixel moments from 3D Gaussians to reconstruct a continuous transmittance function for each ray.

Result: The method improves rendering quality by accurately modeling light attenuation in translucent media, bridging the gap between rasterization and physical accuracy.

Conclusion: This work enhances 3DGS by enabling high-fidelity rendering of complex translucent objects, avoiding computationally expensive techniques like ray tracing.
...
TL;DR: The study evaluates ASR models in multilingual Indian healthcare, finding variability in accuracy and fairness issues tied to language, speaker role, and gender.


<details>
  <summary>Details</summary>
Motivation: To assess ASR reliability in diverse Indian healthcare contexts, addressing gaps in understanding performance across languages and demographics.

Method: A systematic audit of ASR models (Indic Whisper, Whisper, Sarvam, Google speech-to-text, Gemma3n, Omnilingual, Vaani, Gemini) on clinical interview data in Kannada, Hindi, and Indian English.

Result: Variability in model performance, with competitive results in Indian English but failures in code-mixed or vernacular speech. Systematic disparities linked to speaker role and gender.

Conclusion: Highlights the need for culturally and demographically inclusive ASR development to ensure equitable use in Indian healthcare.
...
TL;DR: Benchmarking four ASR models in African languages reveals data efficiency and scalability differences, with practical insights for low-resource ASR design.


<details>
  <summary>Details</summary>
Motivation: To address the lack of systematic study on ASR models' behavior in African low-resource contexts and guide model selection and data scaling.

Method: Benchmarked Whisper, XLS-R, MMS, and W2v-BERT across 13 African languages, fine-tuning on data subsets (1-400 hours), analyzing error rates and model behavior under varying conditions.

Result: MMS and W2v-BERT excel in very low-resource settings; XLS-R scales better with more data; Whisper performs well mid-resource. Language model decoding's impact varies with resource alignment.

Conclusion: The study provides actionable insights for ASR system design in underrepresented languages, emphasizing pre-training coverage, model architecture, and resource availability.
...
TL;DR: MedBioRAG enhances biomedical QA tasks by combining semantic/lexical search, retrieval, and supervised fine-tuning, outperforming SoTA models and GPT-4o in retrieval and QA metrics.


<details>
  <summary>Details</summary>
Motivation: To improve biomedical QA performance by leveraging retrieval-augmented generation (RAG) with advanced search and fine-tuning methods.

Method: Combines semantic/lexical search, document retrieval, and supervised fine-tuning for efficient ranking and response generation.

Result: Outperforms SoTA models and GPT-4o in retrieval (NDCG, MRR) and QA (accuracy, ROUGE) tasks.

Conclusion: Semantic search-based retrieval and LLM fine-tuning are highly effective for biomedical QA applications.
...
TL;DR: KBQA-R1 is a reinforcement learning framework for KBQA that optimizes interaction with knowledge bases, achieving state-of-the-art results by grounding reasoning in execution feedback.


<details>
  <summary>Details</summary>
Motivation: Current KBQA approaches either hallucinate queries or rely on rigid templates, lacking true comprehension. This work aims to improve reasoning by optimizing interactions via RL.

Method: KBQA-R1 treats KBQA as a multi-turn decision process, using GRPO for strategy refinement and RRS for data synthesis to align reasoning traces with ground-truth actions.

Result: The framework achieves top performance on WebQSP, GrailQA, and GraphQuestions, grounding LLM reasoning in verifiable execution.

Conclusion: KBQA-R1 advances KBQA by shifting from text imitation to interaction optimization, demonstrating effectiveness through state-of-the-art results.
...
TL;DR: MultiScript30k extends Multi30k to include global languages in various scripts, addressing limitations in language diversity and script representation by translating Multi30k-En into additional languages.


<details>
  <summary>Details</summary>
Motivation: The original Multi30k dataset restricts MMT research to European languages in Latin scripts, limiting diversity. MultiScript30k aims to broaden this scope.

Method: The dataset was created by translating Multi30k-En into Ar, Es, Uk, Zh_Hans, and Zh_Hant using NLLB200-3.3B, yielding over 30000 sentences.

Result: Cosine similarity and KL divergence metrics show strong consistency (>0.8 and <0.000251, respectively) for most languages, though Zh_Hant performs comparably. COMETKiwi scores vary compared to previous extensions.

Conclusion: MultiScript30k successfully diversifies Multi30k's language and script coverage, though translation quality varies compared to existing extensions.
...
TL;DR: This paper explores the use of locally stored iMessage data to analyze messaging patterns through topic modeling, response times, reluctance scoring, and sentiment analysis.


<details>
  <summary>Details</summary>
Motivation: The study aims to uncover what personal messaging data can reveal about user behavior, given the increasing reliance on short-form electronic communication and the accessibility of iMessage data on Mac.

Method: The researchers developed an iMessage text message analyzer to address five main research questions, utilizing exploratory data from the locally stored iMessage file.

Result: The paper demonstrates how the analyzer can answer the research questions, highlighting its potential for future studies on iMessage data.

Conclusion: The study highlights the utility of accessible messaging data for personal insights and suggests broader applications for similar analyses in future research.
...
TL;DR: The paper investigates the biases in feature attribution methods for explaining language models, proposing a framework to evaluate and compare these biases systematically.


<details>
  <summary>Details</summary>
Motivation: To address the inconsistencies and biases in feature attribution methods, which can lead to mistrust among users, by providing a deeper understanding of these biases.

Method: A model- and method-agnostic framework with three evaluation metrics is used to assess lexical and position biases in two transformers, tested on artificial and natural data tasks.

Result: Lexical and position biases are structurally unbalanced across models, with opposing performance in scoring. Methods producing anomalous explanations tend to exhibit higher biases.

Conclusion: The study highlights the need for careful evaluation of feature attribution methods to ensure reliable explanations, as biases can significantly impact their utility.
...
TL;DR: FIBER is a multilingual benchmark evaluating factual knowledge in large language models, revealing prompt language influences bias and multi-entity questions are harder.


<details>
  <summary>Details</summary>
Motivation: Address concerns about factual reliability and biases in large language models by creating a systematic evaluation tool.

Method: Introduces FIBER, a dataset with sentence completion, QA, and object-count tasks in English, Italian, and Turkish.

Result: Prompt language affects bias (31% topics show bias >0.5), Turkish prompts have higher bias than Italian (83% topics), and multi-entity questions are harder.

Conclusion: Model performance varies by language and size; larger models perform better, with English achieving the highest accuracy.
...
TL;DR: SciLaD is a large-scale dataset of scientific language with 10M English and 35M multilingual publications, built using open-source tools. A RoBERTa model trained on it performs comparably to similar models, validating its quality.


<details>
  <summary>Details</summary>
Motivation: To create a scalable, high-quality scientific language dataset using open-source frameworks and publicly available data, promoting transparency and research in scientific NLP.

Method: Constructed SciLaD using an extensible pipeline with open-source tools, curating English and multilingual splits. Evaluated by pre-training a RoBERTa model and benchmarking against similar models.

Result: The RoBERTa model trained on SciLaD achieves performance comparable to similar-sized scientific language models, confirming dataset quality.

Conclusion: SciLaD successfully demonstrates scalable, open-source scientific data curation and supports NLP research. The dataset and pipeline are published for reproducibility.
...
TL;DR: A survey of recent advances in multi-intent SLU, covering decoding paradigms, modeling approaches, model performance, and future research directions.


<details>
  <summary>Details</summary>
Motivation: The task of multi-intent SLU, involving multiple intent detection and slot filling, has gained attention but lacks a comprehensive review. This paper aims to fill that gap.

Method: The survey analyzes existing research from decoding paradigms and modeling approaches, compares model performance, and evaluates strengths and limitations.

Result: The paper highlights current challenges in multi-intent SLU and outlines potential future research directions.

Conclusion: This survey provides insights and serves as a reference for advancing multi-intent SLU research.
...
TL;DR: A two-stage dynamic few-shot learning (DFSL) approach improves efficiency and performance of LLMs in title and abstract screening for systematic reviews, reducing manual effort and costs.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of research publications makes systematic reviews burdensome, especially title and abstract screening, which is time-consuming and resource-intensive.

Method: A two-stage DFSL approach: initial screening with a low-cost LLM, followed by re-evaluation of low-confidence instances using a high-performance LLM.

Result: Evaluated across 10 systematic reviews, the approach shows strong generalizability and cost-effectiveness.

Conclusion: The DFSL approach reduces manual screening burden and accelerates the systematic review process.
...
TL;DR: The paper proposes using Reinforcement Learning (RL) to enhance reasoning and tool invocation in large language models (LLMs), achieving better generalization and performance over traditional supervised fine-tuning (SFT).


<details>
  <summary>Details</summary>
Motivation: Traditional SFT struggles with generalization under changing data distributions, and collecting high-quality reasoning traces is costly. The paper aims to improve reasoning and tool use in LLMs through RL.

Method: The pipeline employs Group Relative Policy Optimization (GRPO) to refine reasoning steps and tool invocations, using rewards based on tool accuracy and answer correctness.

Result: The approach yields a 1.5% improvement over SFT and a 40% gain over the base model, enhancing reasoning quality and tool precision.

Conclusion: RL-based reasoning and action learning can build more capable and generalizable conversational agents, outperforming traditional methods.
...
TL;DR: AdaSD is a hyperparameter-free speculative decoding method that dynamically adjusts generation and acceptance criteria during inference, achieving up to 49% speedup with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) suffer from slow inference due to their size; existing speculative decoding methods require additional training or tuning, which AdaSD aims to eliminate.

Method: AdaSD uses adaptive thresholds based on token entropy and Jensen-Shannon distance to dynamically adjust candidate generation length and token acceptance without pre-analysis or fine-tuning.

Result: AdaSD achieves up to 49% speedup over standard speculative decoding with less than 2% accuracy degradation on benchmark datasets.

Conclusion: AdaSD is a practical, efficient solution for adaptive LLM inference, compatible with off-the-shelf models and requiring no prior adjustments.
...
TL;DR: CIP is a causal prompting framework that reduces hallucinations in large language models by guiding reasoning with causal relationships, improving accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Large language models often produce inaccurate outputs due to reliance on spurious correlations. CIP aims to mitigate this by emphasizing causal relationships.

Method: CIP constructs causal sequences among entities, actions, and events, injecting them into prompts to guide reasoning and suppress non-causal paths.

Result: Experiments show CIP improves reasoning quality, reliability, and efficiency across multiple models, with significant gains in attributable rate, causal consistency, and latency reduction.

Conclusion: CIP demonstrates causal reasoning as a promising approach to enhance explainability, stability, and efficiency in large language models.
...
TL;DR: LegalRikai is a benchmark simulating Japanese legal tasks, evaluated by professionals and LLMs, revealing gaps in document-level editing and automated assessment challenges.


<details>
  <summary>Details</summary>
Motivation: To advance legal AI research by creating a practice-oriented benchmark reflecting real-world corporate legal tasks.

Method: Developed a benchmark with 100 complex samples, evaluated via human and automated assessments using top LLMs.

Result: Human evaluation showed weaknesses in document-level editing; automated evaluation matched human judgment on clear criteria.

Conclusion: Automated evaluation is useful for screening, but structural consistency assessment remains challenging; a framework is proposed for legal AI research.
...
TL;DR: qa-FLoRA is a training-free method for dynamic LoRA fusion that outperforms static and training-free baselines, closing the gap with supervised approaches.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of effectively fusing Low-Rank Adaptation (LoRA) modules for multi-domain tasks without extensive training.

Method: Uses query-adaptive layer-level fusion weights by measuring distributional divergence between base models and adapters.

Result: Outperforms static fusion by ~5-6% and training-free baselines by ~7-10%, showing interpretable fusion patterns.

Conclusion: qa-FLoRA is effective for robust multi-domain adaptation without requiring composite training data.
...
TL;DR: This study refutes claims of formalistic judging in Central and Eastern Europe by automating judicial reasoning analysis using NLP. It introduces the MADON dataset and a three-stage pipeline combining transformer models and ML for high accuracy in argument detection and classification.


<details>
  <summary>Details</summary>
Motivation: The study aims to systematically analyze judicial reasoning at scale, challenging narratives about formalistic judging in CEE and demonstrating the potential of legal argument mining.

Method: Developed automated methods using NLP, created the MADON dataset with expert annotations, adapted transformer LLMs for Czech legal domain, and experimented with imbalance-addressing techniques. A three-stage pipeline combining ModernBERT, Llama 3.1, and ML was used.

Result: High accuracy in detecting argumentative paragraphs (82.6% macro-F1), classifying legal arguments (77.5% macro-F1), and distinguishing formalistic/non-formalistic decisions (83.2% macro-F1).

Conclusion: Legal argument mining enables reliable judicial philosophy classification and has broader potential in computational legal studies. The methodology is replicable across jurisdictions.
...
TL;DR: Study shows semantic data selectors outperform others in fine-tuning LLMs for Japanese-English translation, with small data differences having large performance impacts.


<details>
  <summary>Details</summary>
Motivation: To understand how different data selection methods affect fine-tuning quality in machine translation tasks for open LLMs.

Method: Compared five data selectors (TF-IDF, COMET Kiwi, QuRate, FD-Score, random) using Japanese-English corpora under controlled training conditions.

Result: Semantic selectors consistently outperformed lexical and geometry-based heuristics, and small data differences (<3%) significantly impacted model performance.

Conclusion: Fine-tuning is highly sensitive to data quality, and semantic selection methods enhance translation performance.
...
TL;DR: The paper introduces a clip selection method for Vision-Language Models (VLMs) to efficiently summarize lengthy videos by targeting key moments, achieving performance close to reference clips while maintaining low computational cost.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with retaining important visual information in lengthy videos, and there's a need for cost-effective tools to analyze such content.

Method: The approach divides videos into short clips, generates compact visual descriptions for each using a lightweight captioning model, and uses an LLM to select the most relevant clips for summarization.

Result: The method achieves summarization performance nearly matching reference clips derived from human annotations, capturing more relevant video information than random selection, and operates at low computational cost.

Conclusion: The proposed clip selection method effectively addresses the challenges of processing lengthy videos with VLMs, balancing summarization quality and computational efficiency.
...
TL;DR: CLINIC is a multilingual benchmark evaluating trustworthiness of language models in healthcare across five dimensions, revealing significant shortcomings.


<details>
  <summary>Details</summary>
Motivation: To address the lack of reliable evaluation of language models (LMs) in multilingual healthcare settings, where existing models are biased towards high-resource languages.

Method: Introduces CLINIC, a benchmark evaluating LMs across 18 tasks in 15 languages, covering dimensions like truthfulness, fairness, safety, robustness, and privacy.

Result: LMs struggle with factual correctness, exhibit biases, and are vulnerable to privacy breaches and adversarial attacks.

Conclusion: CLINIC sets the groundwork for improving the global applicability and safety of LMs in diverse healthcare contexts.
...
TL;DR: A Hebrew medical language model based on DictaBERT 2.0 is developed for extracting clinical timelines from EHRs, showing strong performance on new datasets and privacy-friendly practices.


<details>
  <summary>Details</summary>
Motivation: To construct patient journeys by extracting structured clinical timelines from electronic health records in Hebrew, addressing gaps in medical NLP for non-English languages.

Method: The model is continually pre-trained on over five million de-identified hospital records using DictaBERT 2.0, evaluated on two new annotated datasets (internal medicine/emergency and oncology). Vocabulary adaptation and privacy measures are tested.

Result: The model performs well on both datasets, with vocabulary adaptation improving token efficiency and de-identification not harming downstream performance.

Conclusion: The model is effective for Hebrew medical NLP, supports privacy-conscious development, and is made available for research under ethical restrictions.
...
TL;DR: The paper examines how hallucination-reduction techniques (CoVe, DoLa, RAG) affect LLM creativity in scientific contexts, revealing their divergent impacts on creative thinking.


<details>
  <summary>Details</summary>
Motivation: Address the unexplored impact of hallucination-reduction methods on creativity in LLMs, especially for AI-assisted scientific discovery requiring both accuracy and creativity.

Method: Evaluated CoVe, DoLa, and RAG on multiple LLM families (LLaMA, Qwen, Mistral) at different scales (1B-70B parameters) using creativity benchmarks (NeoCoder, CS4).

Result: CoVe boosts divergent thinking, DoLa suppresses it, and RAG has minimal effect, showing opposing impacts on creativity.

Conclusion: Offers insights for choosing hallucination-reduction methods in scientific applications, balancing factual accuracy and creative exploration.
...
TL;DR: The paper introduces Distribution-Based Sensitivity Analysis (DBSA), a tool for auditing black-box LLMs by analyzing input token sensitivity without needing internal model access or assumptions.


<details>
  <summary>Details</summary>
Motivation: Existing LLM auditing tools focus on narrow aspects like biases, but there's a need for broader tools to understand how each token affects outputs, especially in high-stakes applications.

Method: DBSA is a model-agnostic method that evaluates token-level sensitivity in LLM outputs without requiring gradient access or distributional assumptions.

Result: DBSA provides visual tools for practitioners to explore token sensitivities efficiently, revealing overlooked dependencies in LLM outputs.

Conclusion: DBSA offers a practical, lightweight solution for auditing black-box LLMs, enhancing interpretability and reliability in real-world deployments.
...
TL;DR: The paper introduces a training framework for retrieval-augmented generation (RAG) models, treating the retrieval and generation pipeline as an interactive proof system to improve model reliability and reduce hallucinations.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems treat retrieval as a weak heuristic, leading to unsupported answers, hallucinations, and reliance on spurious evidence. The authors aim to create a more reliable system by treating retrieved documents as verifiable evidence.

Method: A training framework based on the Merlin-Arthur protocol, where Merlin provides helpful evidence and Morgana injects adversarial context. The generator (Arthur) learns to answer, reject, or rely on specific evidence spans. An XAI method identifies influential evidence for modification.

Result: The framework improves groundedness, completeness, soundness, and reject behavior while reducing hallucinations across three datasets and two model families. The retriever also shows improved recall and MRR.

Conclusion: Autonomous interactive-proof-style supervision offers a principled way to enhance RAG systems, ensuring retrieved documents are treated as verifiable evidence.
...
TL;DR: The paper explores BERTopic for analyzing historical newspaper archives on nuclear discourse, addressing limitations of traditional methods like LDA.


<details>
  <summary>Details</summary>
Motivation: Challenges in extracting coherent themes from historical texts due to noise, volume, and topic evolution motivate the use of advanced neural topic modeling.

Method: BERTopic is employed to analyze nuclear power and safety discourse (1955-2018) using transformer embeddings for topic extraction and temporal evolution tracking.

Result: BERTopic reveals shifting topic importance and co-occurrence patterns, proving scalable and contextually sensitive for historical discourse.

Conclusion: The study highlights BERTopic's advantages over traditional methods and suggests future research directions.
...
TL;DR: The paper establishes tight lower bounds on the runtime of deterministic speculative generation algorithms in LLMs, using parallels between token generation and branching random walks.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental limits of speedup achievable in speculative generation for LLMs and provide theoretical insights for future system designs.

Method: Analyzes speculative generation by modeling it with branching random walks, proving bounds on the expected tokens predicted per iteration under basic assumptions.

Result: Shows the expected tokens per iteration is bounded by a formula involving verifier capacity and entropy, with empirical validation on Llama models.

Conclusion: The theoretical bounds are tight and provide valuable insights for designing future speculative decoding systems.
...
TL;DR: SUMFORU is a personalized review summarization framework that aligns with user personas, improving decision-making via a two-stage alignment process.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based summarizers are generic and don't personalize outputs for individual preferences, limiting their usefulness for decision-making.

Method: A two-stage alignment process: (1) persona-aware Supervised Fine-Tuning via asymmetric knowledge distillation, and (2) Reinforcement Learning with AI Feedback using a preference estimator.

Result: The framework outperforms existing methods in consistency, grounding, and preference alignment, and generalizes well to new product categories.

Conclusion: SUMFORU demonstrates the potential of steerable pluralistic alignment for personalized decision-support systems.
...
TL;DR: The paper introduces a CORL framework using reinforcement learning to fine-tune MILP schemes for better real-world operational performance.


<details>
  <summary>Details</summary>
Motivation: Traditional MILPs often perform suboptimally in real-world scenarios due to modeling inaccuracies. Existing machine learning methods rely on supervised learning and assume access to optimal decisions, limiting their effectiveness.

Method: The CORL framework casts MILPs solved by branch and bound as differentiable stochastic policies, enabling end-to-end fine-tuning with reinforcement learning on real-world data.

Result: The framework is validated in a simple combinatorial sequential decision-making example, demonstrating its potential.

Conclusion: CORL offers a promising approach to improve MILP performance in real-world applications by leveraging reinforcement learning.
...
TL;DR: The paper addresses the challenge of efficient combinatorial bundling in Online Freight Exchange Systems (OFEX) by modeling it as a multi-commodity pickup-and-delivery problem (m1-PDSTSP). It proposes a hybrid search pipeline combining Transformer Neural Network-based constructive policy with Multi-Start Large Neighborhood Search (MSLNS) to optimize bundling under constraints.


<details>
  <summary>Details</summary>
Motivation: Current OFEX systems struggle with efficient combinatorial bundling of transportation jobs due to latency and quality constraints. The goal is to improve revenue-driven freight bundling under capacity, precedence, and route-length constraints.

Method: The proposed method integrates a Transformer Neural Network-based constructive policy with MSLNS metaheuristic within a rolling-horizon scheme. This combination leverages the speed of learning-based inference and the robustness of improvement search.

Result: The hybrid approach outperforms existing neural combinatorial optimization and metaheuristic baselines, achieving an optimality gap of less than 2% in total revenue compared to exact baseline methods.

Conclusion: This work demonstrates that Deep Neural Network-based constructors can reliably seed improvement heuristics, extending applicability to selective traveling salesperson and pickup-and-delivery problems beyond m1-PDSTSP.
...
TL;DR: FutureWeaver is a framework for optimizing test-time compute allocation in multi-agent systems, ensuring effective collaboration under fixed budgets.


<details>
  <summary>Details</summary>
Motivation: Existing test-time compute scaling techniques lack mechanisms for multi-agent collaboration or budget-constrained compute distribution.

Method: FutureWeaver introduces modularized collaboration and dual-level planning to optimize compute allocation by reasoning over tasks and future steps.

Result: FutureWeaver outperforms baselines in complex agent benchmarks across diverse budget settings.

Conclusion: FutureWeaver effectively addresses the gap in multi-agent compute allocation, enhancing collaborative performance under budget constraints.
...
TL;DR: A-LAMP framework automates MDP modeling and policy generation from natural language, improving accuracy and efficiency in RL tasks.


<details>
  <summary>Details</summary>
Motivation: Manual conversion of task descriptions to MDPs and policy training is error-prone and time-consuming. Automating this process ensures better alignment and reduces errors.

Method: A-LAMP uses an agentic LLM to translate natural language into MDP formulations and trained policies, with staged verification for semantic alignment.

Result: A-LAMP outperforms single state-of-the-art LLMs in policy generation, even its lightweight variant. It also maintains task optimality in generated environments.

Conclusion: The A-LAMP framework effectively automates RL task modeling and policy training, demonstrating higher capability and reliability across various domains.
...
TL;DR: TriFlow, a multi-agent framework for trip planning, enhances constraint satisfaction, tool coordination, and efficiency via a three-stage pipeline, outperforming existing methods with high pass rates and runtime improvements.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agents fail to meet real-world trip planning demands due to poor constraint satisfaction, inefficient tool coordination, and high costs.

Method: TriFlow employs a three-stage pipeline (retrieval, planning, governance) with rule-LLM collaboration for structured reasoning and iterative refinement.

Result: Achieved 91.1% and 97% pass rates on benchmarks with a 10x runtime efficiency boost over current SOTA.

Conclusion: TriFlow successfully addresses limitations of LLM-based agents by integrating structured reasoning and flexibility, yielding efficient and feasible trip plans.
...
TL;DR: The paper introduces CAPTURE, a novel CAPTCHA benchmark designed for Large Visual Language Models (LVLMs) to address gaps in existing benchmarks, covering diverse CAPTCHA types and demonstrating current LVLMs' poor performance.


<details>
  <summary>Details</summary>
Motivation: Existing CAPTCHA benchmarks for LVLMs are limited and lack diversity, failing to cover all CAPTCHA types or provide tailored evaluations.

Method: CAPTURE includes 4 main CAPTCHA types and 25 sub-types from 31 vendors, offering diverse, large-scale data with LVLM-specific labels.

Result: Current LVLMs perform poorly when evaluated using CAPTURE, highlighting gaps in their CAPTCHA-solving capabilities.

Conclusion: CAPTURE fills a critical gap in LVLM evaluation by providing comprehensive and tailored CAPTCHA benchmarks, revealing limitations in current models.
...
TL;DR: The paper introduces a framework to improve the reliability and verifiability of Large Language Models (LLMs) in multi-turn tasks by integrating a task profiler, reasoning module, and generation module.


<details>
  <summary>Details</summary>
Motivation: LLMs lack reliability and verifiability in multi-turn tasks, prompting the need for a structured framework to guide their behavior.

Method: The framework includes a task profiler for strategy selection, a reasoning module for verifiable mappings, and a generation module for constraint-compliant outputs.

Result: Components co-evolve during interaction, leading to trustworthy behavior in the agent.

Conclusion: The proposed framework enhances LLM-based agents' reliability and verifiability in multi-turn tasks.
...
TL;DR: AgentBalance is a framework for cost-effective multi-agent systems (MAS) under explicit token-cost and latency budgets, achieving performance gains via backbone-then-topology design.


<details>
  <summary>Details</summary>
Motivation: Existing MAS designs often neglect explicit token-cost and latency budgets, leading to suboptimal cost-effectiveness when budgets are restrictive.

Method: AgentBalance uses a two-step approach: backbone-oriented agent generation (LLM pool construction, selection, and matching) and adaptive topology generation (representation learning, gating, and latency-aware synthesis).

Result: Experiments show up to 10% and 22% performance gains under matched token-cost and latency budgets, with strong AUC on performance-versus-budget curves.

Conclusion: AgentBalance improves cost-effectiveness for MAS under budget constraints and generalizes well to unseen LLMs, serving as a plug-in for existing systems.
...
TL;DR: The paper critiques Fidelity metrics (Insertion and Deletion) in XAI, revealing that baseline choices bias attribution methods. Proposed criteria for baselines—information removal and avoiding OOD images—highlight trade-offs. A new model-dependent baseline improves this balance.


<details>
  <summary>Details</summary>
Motivation: Addressing biases in Fidelity metrics' baseline choices, which unfairly favor certain attribution methods, and proposing better baseline criteria.

Method: Analyzed baseline choices' impact on attribution methods, introduced two desirable properties (information removal and avoiding OOD images), and proposed a novel model-dependent baseline using feature visualization.

Result: Current baselines fail to meet both criteria, showing a trade-off. The new baseline outperforms existing ones by balancing information removal and OOD avoidance.

Conclusion: Baseline choice critically affects attribution method evaluation. The proposed baseline resolves trade-offs in current baselines, offering a better solution.
...
TL;DR: Motif-2-12.7B-Reasoning is a 12.7B parameter model addressing reasoning challenges with efficient training and fine-tuning methods, achieving performance comparable to larger models.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between open-weight systems and proprietary frontier models in reasoning and long-context understanding while tackling model collapse and training instability.

Method: Combines memory-efficient infrastructure, a two-stage SFT curriculum with synthetic data, and an RLFT pipeline with data filtering and trajectory reuse.

Result: Achieves comparable performance to larger models in math, coding, and agentic benchmarks.

Conclusion: Provides a competitive open model and blueprint for scaling reasoning under compute constraints.
...
TL;DR: The paper compares classical optimization (ILP) and AI methods (PatternBoost and PPO) for the No-Three-In-Line problem, showing ILP's superiority for exact solutions on larger grids, while AI methods perform well on smaller grids, suggesting hybrid approaches for future scalability.


<details>
  <summary>Details</summary>
Motivation: The No-Three-In-Line problem is a classic combinatorial geometry challenge. The study aims to compare classical optimization (ILP) and AI techniques (PatternBoost and PPO) to determine their effectiveness and scalability for this problem.

Method: The paper employs ILP for provably optimal solutions and introduces PatternBoost (transformer learning) and PPO (reinforcement learning) for the first time. Performance is evaluated across grid sizes up to 19x19.

Result: ILP achieves optimal solutions up to 19x19 grids. PatternBoost matches optimal performance up to 14x14 grids with 96% test loss reduction. PPO works perfectly on 10x10 grids but fails on 11x11 due to constraints.

Conclusion: Classical optimization is essential for exact solutions on larger grids, while AI methods are competitive for smaller instances. Hybrid approaches are recommended for scaling to larger problem sizes.
...
TL;DR: Generative AI, tested via DeepSeek and ChatGPT, shows promise in agri-food science by democratizing knowledge but struggles with accuracy and hallucinations, requiring human oversight.


<details>
  <summary>Details</summary>
Motivation: Explore the potential of generative AI (DeepSeek and ChatGPT) in providing actionable scientific knowledge for agroecological crop protection.

Method: Compare factual accuracy, consistency, and breadth of knowledge between web-grounded (DeepSeek) and non-grounded (ChatGPT) LLMs for nine global pests/diseases.

Result: DeepSeek outperformed ChatGPT in literature coverage, solutions reported, and efficacy estimates but both models had issues like hallucinations and inconsistencies.

Conclusion: LLMs can aid farm-level decisions and scientific creativity but require rigorous human oversight due to current limitations.
...
TL;DR: BAID is a framework evaluating AI text detectors for biases across sociolinguistic factors, revealing disparities in detection performance for underrepresented groups.


<details>
  <summary>Details</summary>
Motivation: There's a lack of systematic evaluation of AI text detectors' biases, especially impacting English Language Learners (ELLs) and broader sociolinguistic groups.

Method: BAID introduces 200k+ samples across 7 categories (e.g., demographics, dialects) and generates synthetic versions to test biases in four state-of-the-art AI detectors.

Result: The evaluation shows consistent disparities, notably lower recall rates for texts from underrepresented groups.

Conclusion: BAID offers a scalable, transparent method for auditing AI detectors, highlighting the need for bias-aware evaluations before deployment.
...
TL;DR: EmeraldMind is a framework combining domain-specific knowledge graphs and retrieval-augmented generation to detect greenwashing in corporate ESG reports with high accuracy and transparency.


<details>
  <summary>Details</summary>
Motivation: Addressing greenwashing (misleading sustainability claims) is critical for environmental progress, requiring intelligent systems to combat misinformation.

Method: EmeraldMind integrates a domain-specific knowledge graph (EmeraldGraph) from ESG reports with retrieval-augmented generation to verify claims and detect greenwashing.

Result: Experiments show EmeraldMind outperforms generic LLMs in accuracy, coverage, and explanation quality without needing fine-tuning.

Conclusion: EmeraldMind provides a robust, transparent solution for greenwashing detection, leveraging domain-specific evidence for trustworthy assessments.
...
TL;DR: The study evaluates LLMs' ability to extract medical info from noisy patient complaints, identifying functional declines akin to MASLD. Four models were tested, with Qwen3-Max performing best and Gemini 2.5 worst. GPT-4o made a severe error in PE risk assessment, highlighting risks in clinical AI use.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' reliability in noisy clinical scenarios and identify functional declines similar to MASLD, ensuring safe AI integration in healthcare.

Method: Cross-sectional analysis using standardized medical probes across five dimensions, evaluated by clinicians via double-blind, inverse rating scales.

Result: All models showed functional defects; Qwen3-Max performed best, Gemini 2.5 worst. GPT-4o severely misjudged PE risk. Extreme noise caused functional collapse in most models.

Conclusion: LLMs exhibit AI-MASLD-like dysfunction in clinical contexts. Current models require human supervision, highlighting gaps in practical clinical application.
...
TL;DR: The paper emphasizes the need for dynamic, adaptive benchmarking frameworks in AI to address challenges like model memorization, evolving architectures, and real-world applicability, stressing education and community efforts.


<details>
  <summary>Details</summary>
Motivation: Traditional static benchmarks fall short in evaluating modern AI due to rapid advancements and real-world complexities, necessitating adaptive and inclusive approaches.

Method: Proposes continuous adaptive benchmarking frameworks, emphasizing transparency, reproducibility, and educational initiatives like AI Benchmark Carpentry.

Result: Identifies barriers such as resource demands, lack of expertise, and hardware access, advocating for dynamic benchmarks aligned with real-world applications.

Conclusion: Dynamic benchmarking and community-driven education are crucial for responsible, reproducible AI deployment, ensuring benchmarks evolve with AI advancements.
...
TL;DR: The paper proposes a structural causal model to predict energy demand, leveraging causal relationships between weather and calendar factors. It validates causal insights and builds a Bayesian model, achieving state-of-the-art performance with strong robustness.


<details>
  <summary>Details</summary>
Motivation: Energy demand prediction is complex due to interdependent factors like weather and calendar information. Existing correlation-based methods are insufficient, prompting the need for causal modeling.

Method: The authors develop a structural causal model to explain variable relationships, validate causal beliefs, and use these insights to build a Bayesian model.

Result: The Bayesian model achieves a 3.84% MAPE on test data and maintains robustness with a 3.88% average MAPE in cross-validation.

Conclusion: The causal approach enhances energy demand prediction accuracy and robustness, validated by empirical results.
...
TL;DR: The paper discusses TxAgent, an AI system for therapeutic decision-making, using iterative retrieval-augmented generation (RAG) with a fine-tuned Llama model and biomedical tools. It evaluates performance in the CURE-Bench NeurIPS 2025 Challenge.


<details>
  <summary>Details</summary>
Motivation: The complexity of therapeutic decision-making requires robust AI systems that integrate reliable biomedical knowledge and handle safety-critical tasks like drug recommendation and adverse-effect prediction.

Method: TxAgent uses a fine-tuned Llama-3.1-8B model for iterative RAG, dynamically generating and executing function calls to biomedical tools like FDA Drug API and OpenTargets. It emphasizes safe, accurate reasoning and tool usage.

Result: The system's performance was evaluated in the CURE-Bench NeurIPS 2025 Challenge, showing improved tool-retrieval strategies and earning the Excellence Award in Open Science.

Conclusion: TxAgent demonstrates the potential of agentic AI in therapeutic decision-making, with rigorous evaluation highlighting the importance of accurate reasoning and tool utilization in medical applications.
...
### [132] [Multi-Objective Reinforcement Learning for Large-Scale Mixed Traffic Control](https://arxiv.org/abs/2512.11247)
*Iftekharul Islam,Weizi Li*

Main category: cs.MA

TL;DR: A hierarchical framework combining multi-objective reinforcement learning and strategic routing improves fairness, safety, and efficiency in mixed traffic control, reducing wait times, starvation, and conflicts.


<details>
  <summary>Details</summary>
Motivation: Existing traffic control methods optimize efficiency and safety but fail to ensure equitable service, leading to starvation of vehicles on low-demand routes.

Method: Proposes a framework with multi-objective reinforcement learning for local intersection control and strategic routing for network-level coordination, including a Conflict Threat Vector and queue parity penalty.

Result: Experiments show up to 53% reduction in average wait time, 86% reduction in maximum starvation, and 86% reduction in conflict rates, while maintaining fuel efficiency.

Conclusion: Multi-objective optimization and strategic routing enhance fairness and safety in mixed-autonomy traffic, with effectiveness scaling with higher autonomy levels.
...
### [133] [Evaluating Cooperative Resilience in Multiagent Systems: A Comparison Between Humans and LLMs](https://arxiv.org/abs/2512.11689)
*Manuela Chacon-Chamorro,Juan Sebastián Pinzón,Rubén Manrique,Luis Felipe Giraldo,Nicanor Quijano*

Main category: cs.MA

TL;DR: The paper compares cooperative resilience in multi-agent systems between human groups and LLM-based agents, showing humans with communication outperform others in resilience.


<details>
  <summary>Details</summary>
Motivation: To understand and compare cooperative resilience in multi-agent systems, focusing on human and LLM-based agents in social dilemmas.

Method: Comparative analysis using a Tragedy of the Commons environment, assessing resilience under disruptive conditions with and without communication.

Result: Humans with communication achieved the highest resilience; LLM agents improved with communication but remained below human levels.

Conclusion: Human decision-making insights can guide the design of artificial agents to enhance prosocial and resilient behaviors.
...
### [134] [Scalable Data Synthesis for Computer Use Agents with Step-Level Filtering](https://arxiv.org/abs/2512.10962)
*Yifei He,Pranit Chawla,Yaser Souri,Subhojit Som,Xia Song*

Main category: cs.LG

TL;DR: The paper introduces a scalable data synthesis pipeline to improve training for computer use agents (CUAs) by filtering noisy rollouts and augmenting reasoning, resulting in two datasets (WebSTAR, WebSCORE) and a reward model (StepRM) that outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Training CUAs is costly and limited by low-quality trajectory data from human demonstrations. Synthetic data from strong CUAs is noisy, making imitation ineffective.

Method: A pipeline filters noisy rollouts step-by-step, retaining correct actions and adding reasoning augmentation. This creates datasets WebSTAR and WebSCORE, and trains reward model StepRM.

Result: The 7B model trained on WebSTAR outperforms UI-TARS-1.5-7B by 15%. StepRM matches grading quality of o4-mini but is more efficient.

Conclusion: Step-level filtering is key for scalable CUA training, and the new datasets and reward model advance robust and efficient CUAs.
...
### [135] [Multimodal Fusion of Regional Brain Experts for Interpretable Alzheimer's Disease Diagnosis](https://arxiv.org/abs/2512.10966)
*Farica Zhuang,Dinara Aliyeva,Shu Yang,Zixuan Wen,Duy Duong-Tran,Christos Davatzikos,Tianlong Chen,Song Wang,Li Shen*

Main category: cs.LG

TL;DR: MREF-AD, a multimodal fusion model for Alzheimer's disease diagnosis, uses Mixture-of-Experts to adaptively balance biomarker contributions across brain regions, outperforming baselines and offering interpretable insights.


<details>
  <summary>Details</summary>
Motivation: Early and accurate AD diagnosis benefits from integrating multimodal data, but current methods lack adaptive balancing of biomarker contributions.

Method: MREF-AD employs a Mixture-of-Experts framework with two-level gating networks to learn subject-specific fusion weights for meso-scale brain regions.

Result: On ADNI data, MREF-AD achieves state-of-the-art performance and provides interpretable insights into biomarker relevance.

Conclusion: MREF-AD is a promising general framework for adaptive and interpretable multimodal fusion in neuroimaging.
...
### [136] [Agent-Based Modular Learning for Multimodal Emotion Recognition in Human-Agent Systems](https://arxiv.org/abs/2512.10975)
*Matvey Nepomnyaschiy,Oleg Pereziabov,Anvar Tliamov,Stanislav Mikhailov,Ilya Afanasyev*

Main category: cs.LG

TL;DR: A multi-agent framework is proposed for training multimodal emotion recognition systems, improving flexibility, scalability, and efficiency by treating modality encoders and classifiers as autonomous agents.


<details>
  <summary>Details</summary>
Motivation: To address the computational intensity and inflexibility of traditional multimodal deep learning models in human-agent interaction (HAI) scenarios.

Method: A novel multi-agent framework where each modality encoder and fusion classifier operates autonomously under a central supervisor, enabling modular updates and reduced training overhead.

Result: Proof-of-concept implementation shows feasibility, supporting vision, audio, and text modalities with improved training efficiency.

Conclusion: The framework enhances flexibility, scalability, and maintainability of emotion recognition systems for HAI applications.
...
### [137] [MoB: Mixture of Bidders](https://arxiv.org/abs/2512.10969)
*Dev Vyas*

Main category: cs.LG

TL;DR: MoB replaces learned gating networks in MoE with VCG auctions, preventing catastrophic forgetting and improving continual learning performance significantly.


<details>
  <summary>Details</summary>
Motivation: Address catastrophic forgetting in Mixture of Experts (MoE) architectures by introducing a decentralized economic mechanism for expert routing.

Method: Replace learned gating networks with Vickrey-Clarke-Groves (VCG) auctions, where experts bid their true cost (execution cost + forgetting cost).

Result: MoB achieves 88.77% average accuracy on Split-MNIST, outperforming baselines (Gated MoE: 19.54%, Monolithic EWC: 27.96%).

Conclusion: MoB provides stateless routing, truthful bidding, and emergent specialization, advancing continual learning without task boundaries.
...
### [138] [TECM*: A Data-Driven Assessment to Reinforcement Learning Methods and Application to Heparin Treatment Strategy for Surgical Sepsis](https://arxiv.org/abs/2512.10973)
...
TL;DR: The study proposes a data-driven RL framework to optimize heparin therapy in sepsis patients, introducing cxSOFA and TECM for nuanced evaluation, achieving reduced mortality and hospital stays.


<details>
  <summary>Details</summary>
Motivation: To improve personalized heparin therapy in surgical sepsis patients by leveraging data-driven metrics and continuous reward functions for better clinical outcomes.

Method: Uses MIMIC-IV and eICU databases, introduces cxSOFA for continuous state representation, defines treatment strategies stepwise, and employs TECM for evaluation. Tests RL algorithms: Q-Learning, DQN, DDQN, BCQ, and CQL.

Result: The cxSOFA-CQL model reduced mortality from 1.83% to 0.74% and average hospital stay from 11.11 to 9.42 days. TECM showed robust evaluation across models.

Conclusion: The RL framework with cxSOFA and TECM offers interpretable and robust optimization for heparin therapy, promising improved clinical outcomes and decision reliability.
...
### [139] [MolSculpt: Sculpting 3D Molecular Geometries from Chemical Syntax](https://arxiv.org/abs/2512.10991)
*Zhanpeng Chen,Weihao Gao,Shunyu Wang,Yanan Zhu,Hong Meng,Yuexian Zou*

Main category: cs.LG

TL;DR: MolSculpt is a framework that bridges the gap between 1D molecular models and 3D geometry generation by integrating chemical knowledge into a diffusion model, achieving SOTA performance in molecule generation.


<details>
  <summary>Details</summary>
Motivation: Prior methods fail to fully utilize chemical knowledge in 1D models for 3D molecular generation, creating a disconnect. MolSculpt aims to solve this by leveraging latent knowledge from 1D models.

Method: MolSculpt combines a frozen 1D foundation model with a 3D diffusion model, using learnable queries and a projector to integrate cross-modal information for end-to-end optimization.

Result: MolSculpt achieves SOTA performance in de novo and conditional 3D molecule generation, demonstrating high fidelity and stability on GEOM-DRUGS and QM9 datasets.

Conclusion: MolSculpt successfully integrates 1D chemical knowledge into 3D generation, outperforming existing methods and advancing molecular design.
...
### [140] [Memoryless Policy Iteration for Episodic POMDPs](https://arxiv.org/abs/2512.11082)
*Roy van Zuijlen,Duarte Antunes*

Main category: cs.LG

TL;DR: The paper introduces a new family of policy-iteration algorithms for POMDPs that focus on memoryless and finite-memory policies, offering computational efficiency and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of solving POMDPs with memoryless or finite-memory policies, which avoids the high-dimensional belief space, but where classical methods like policy iteration are difficult to apply due to non-Markovian output processes.

Method: Develops a family of monotonically improving policy-iteration algorithms that alternate between single-stage policy improvements and evaluations in a periodic pattern, optimizing computational efficiency and identifying optimal patterns. Also introduces a model-free variant for direct policy learning from data.

Result: The proposed methods achieve significant computational speedups over policy-gradient baselines and recent specialized algorithms in both model-based and model-free settings.

Conclusion: The introduced algorithms provide an efficient and practical solution for solving POMDPs using memoryless and finite-memory policies, demonstrating superior performance in computational efficiency compared to existing methods.
...
### [141] [Clip-and-Verify: Linear Constraint-Driven Domain Clipping for Accelerating Neural Network Verification](https://arxiv.org/abs/2512.11087)
*Duo Zhou,Jorge Chavez,Hesun Chen,Grani A. Hanasusanto,Huan Zhang*

Main category: cs.LG

TL;DR: The paper introduces the linear constraint-driven clipping framework to enhance neural network verifiers, significantly improving efficiency and reducing subproblems in branch-and-bound procedures.


<details>
  <summary>Details</summary>
Motivation: Enhancing the efficacy of neural network verifiers by efficiently utilizing linear constraints to tighten bounds and reduce irrelevant input space during verification.

Method: Develops two algorithms under the linear constraint-driven clipping framework, leveraging linear constraints from bound propagation and other sources to improve bounds and reduce subproblems, implemented via a specialized GPU procedure.

Result: Clip-and-Verify tightens bounds across benchmarks, reduces subproblems by up to 96%, and achieves state-of-the-art verified accuracy, integrated into the α,β-CROWN verifier.

Conclusion: The proposed framework and algorithms significantly enhance neural network verification efficiency and accuracy, demonstrated by benchmark results and adoption in the winning VNN-COMP 2025 verifier.
...
### [142] [Investigating ECG Diagnosis with Ambiguous Labels using Partial Label Learning](https://arxiv.org/abs/2512.11095)
*Sana Rahmani,Javad Hashemi,Ali Etemad*

Main category: cs.LG

TL;DR: The paper explores the use of Partial Label Learning (PLL) methods for ECG diagnosis to address label ambiguity, evaluating their robustness under various ambiguity conditions.


<details>
  <summary>Details</summary>
Motivation: Label ambiguity in ECG diagnosis, caused by overlapping conditions and diagnostic disagreements, is overlooked in current models, which assume clean annotations. This limits real-world applicability and evaluation.

Method: Nine PLL algorithms were adapted for multi-label ECG diagnosis and tested with diverse ambiguity generation strategies, including unstructured (random) and structured (cardiologist-derived) ambiguities.

Result: PLL methods showed varying robustness to different ambiguity types and degrees. Key limitations of current PLL approaches in clinical settings were identified.

Conclusion: The study highlights the need for ambiguity-aware learning frameworks in ECG diagnosis and suggests future directions for robust clinical applications.
...
### [143] [Limits and Gains of Test-Time Scaling in Vision-Language Reasoning](https://arxiv.org/abs/2512.11109)
*Mohammadjavad Ahmadpour,Amirmahdi Meighani,Payam Taebi,Omid Ghahroodi,Amirmohammad Izadi,Mahdieh Soleymani Baghshah*

Main category: cs.LG

TL;DR: Test-time scaling (TTS) improves LLM reasoning but is underexplored for Vision-Language Models (VLMs). Closed-source VLMs benefit from structured reasoning and self-refinement, while open-source VLMs show inconsistent gains. TTS effectiveness varies by dataset.


<details>
  <summary>Details</summary>
Motivation: To explore the application of TTS in VLMs and understand its impact across different models and benchmarks.

Method: Empirical study of TTS methods applied to open-source and closed-source VLMs on various benchmarks.

Result: Closed-source VLMs benefit from structured reasoning and iterative refinement, while open-source VLMs show inconsistent performance. TTS effectiveness varies with dataset type.

Conclusion: TTS is not universally effective; its success depends on model capabilities and task characteristics, suggesting adaptive strategies and multimodal reward models for future work.
...
### [144] [In-Context Multi-Objective Optimization](https://arxiv.org/abs/2512.11114)
*Xinyu Zhang,Conor Hassan,Julien Martinelli,Daolang Huang,Samuel Kaski*

Main category: cs.LG

TL;DR: TAMO is a universal, amortized policy for multi-objective black-box optimization using transformers, enabling fast, high-quality proposals without per-task retraining.


<details>
  <summary>Details</summary>
Motivation: Addressing the inefficiencies and limitations of current multi-objective Bayesian optimization methods, which require tailored surrogates, are myopic, and introduce overhead.

Method: Uses a transformer architecture pretrained with RL to propose designs in a single forward pass, conditioning on the entire query history for Pareto frontier approximation.

Result: TAMO reduces proposal time by 50-1000x and matches or improves Pareto quality under tight evaluation budgets.

Conclusion: Transformers enable in-context multi-objective optimization, eliminating per-task surrogate fitting and acquisition engineering, paving the way for plug-and-play optimizers.
...
### [145] [Refining Graphical Neural Network Predictions Using Flow Matching for Optimal Power Flow with Constraint-Satisfaction Guarantee](https://arxiv.org/abs/2512.11127)
*Kshitiz Khanal*

Main category: cs.LG

TL;DR: A novel two-stage learning framework combines physics-informed Graph Neural Networks (GNNs) and Continuous Flow Matching (CFM) to solve DC-OPF problems efficiently while ensuring feasibility and near-optimality.


<details>
  <summary>Details</summary>
Motivation: Traditional optimization solvers are computationally expensive for large-scale DC-OPF problems, and machine learning approaches often fail to satisfy constraints or achieve optimality.

Method: The framework uses GNNs with physics-informed losses for feasible solutions and CFM for refinement toward optimality.

Result: Achieves near-optimal solutions (cost gaps <0.1%-3%) and 100% feasibility across various load scenarios.

Conclusion: Bridges the gap between fast neural predictions and slow numerical solvers, suitable for modern power systems.
...
### [146] [Fairness-Regularized Online Optimization with Switching Costs](https://arxiv.org/abs/2512.11131)
*Pengfei Li,Yuelin Han,Adam Wierman,Shaolei Ren*

Main category: cs.LG

TL;DR: The paper introduces FairOBD, an algorithm to address fairness and action smoothness in online optimization, proving its effectiveness against novel benchmarks.


<details>
  <summary>Details</summary>
Motivation: To simultaneously address fairness and action smoothness in online optimization, which have not been tackled together before.

Method: FairOBD decomposes long-term fairness costs into online costs using an auxiliary variable and integrates it with hitting and switching costs.

Result: FairOBD achieves a competitive ratio against optimal offline algorithms with constraints and performs well empirically in resource provisioning.

Conclusion: FairOBD effectively balances fairness, smoothness, and costs, outperforming existing baselines in promoting fair outcomes.
...
### [147] [The Vekua Layer: Exact Physical Priors for Implicit Neural Representations via Generalized Analytic Functions](https://arxiv.org/abs/2512.11138)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: The Vekua Layer (VL) is introduced as a spectral method leveraging Generalized Analytic Functions to address spectral bias and computational inefficiency in Implicit Neural Representations (INRs), outperforming SIRENs in PDE tasks with high precision and noise stability.


<details>
  <summary>Details</summary>
Motivation: To overcome spectral bias and computational challenges in INRs by grounding the approach in classical differential operator theory.

Method: Uses Harmonic and Fourier-Bessel bases to restrict the hypothesis space, transforming learning into a convex least-squares problem solved via linear projection.

Result: Achieves machine precision ($\text{MSE} \approx 10^{-33}$) in exact reconstruction and handles noise robustly ($\text{MSE} \approx 0.03$), enabling analytic continuation from boundary data.

Conclusion: The VL outperforms SIRENs, offering precise, stable, and physics-informed solutions for PDEs with unique capabilities like holographic extrapolation.
...
### [148] [Autoencoder-based Semi-Supervised Dimensionality Reduction and Clustering for Scientific Ensembles](https://arxiv.org/abs/2512.11145)
*Lennard Manuel,Hamid Gadirov,Steffen Frey*

Main category: cs.LG

TL;DR: The paper introduces an enhanced autoencoder framework combining clustering and contrastive loss to improve visualization and interpretability of high-dimensional ensemble datasets.


<details>
  <summary>Details</summary>
Motivation: High-dimensional ensemble datasets are challenging to analyze; existing dimensionality reduction and autoencoder methods often struggle with such complexity.

Method: The framework uses EfficientNetV2 for pseudo-labeling, combines reconstruction, clustering (soft silhouette score), and contrastive losses, and applies UMAP for 2D visualization.

Result: Models with clustering or contrastive loss slightly outperform baselines on soil channel structures and droplet-on-film impact datasets.

Conclusion: Incorporating clustering and contrastive losses enhances autoencoder performance for high-dimensional ensemble data visualization.
...
### [149] [Harnessing Rich Multi-Modal Data for Spatial-Temporal Homophily-Embedded Graph Learning Across Domains and Localities](https://arxiv.org/abs/2512.11178)
*Takuya Kurihana,Xiaojian Zhang,Wing Yee Au,Hon Yung Wong*

Main category: cs.LG

TL;DR: A heterogeneous data pipeline for cross-domain data fusion in smart cities, integrating diverse datasets to improve urban decision-making.


<details>
  <summary>Details</summary>
Motivation: Urban decision-making relies on heterogeneous, multi-modal data, which is often fragmented. This research aims to unify such data for scalable solutions.

Method: Proposes a data pipeline fusing time-varying, spatial, and time-series datasets, integrating spatial homophily into graph-learning models.

Result: The framework shows strong predictive performance and adaptability across domains and localities with minimal reconfiguration.

Conclusion: Advances scalable, data-informed urban systems, addressing key challenges in smart city analytics.
...
### [150] [Progress over Points: Reframing LM Benchmarks Around Scientific Objectives](https://arxiv.org/abs/2512.11183)
*Alwin Jin,Sean M. Hendryx,Vaskar Nath*

Main category: cs.LG

TL;DR: The paper proposes progress-oriented benchmarks to shift focus from static problem-solving to advancing scientific research, exemplified by a NanoGPT speedrun environment.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks limit measurable advances by focusing on static problems; the paper aims to incentivize scientific progress through new benchmarks.

Method: Introduces a NanoGPT speedrun environment with standardized datasets, reference models, training tools, and evaluation metrics.

Result: Achieved a new state-of-the-art training time (3-second improvement) and observed novel algorithmic ideas.

Conclusion: Advocates for benchmarking as a means to drive scientific advancement, fostering community research on open-ended problems.
...
### [151] [On the failure of ReLU activation for physics-informed machine learning](https://arxiv.org/abs/2512.11184)
*Conor Rowan*

Main category: cs.LG

TL;DR: The paper analyzes why ReLU performs poorly in physics-informed machine learning, identifying issues with automatic differentiation causing mis-specified gradients.


<details>
  <summary>Details</summary>
Motivation: To diagnose the poor performance of ReLU activation functions in physics-informed machine learning, despite their common use.

Method: The study examines ReLU's failure in variational problems with first derivatives, linking it to issues with automatic differentiation in PyTorch.

Result: ReLU's poor performance stems from mis-specified gradients due to automatic differentiation failing to handle discontinuous fields.

Conclusion: ReLU's limitations in physics-informed training are tied to automatic differentiation challenges, not just its piecewise linear nature.
...
### [152] [Beyond Memorization: Gradient Projection Enables Selective Learning in Diffusion Models](https://arxiv.org/abs/2512.11194)
*Divya Kothandaraman,Jaclyn Pytlarz*

Main category: cs.LG

TL;DR: A Gradient Projection Framework is introduced to selectively unlearn prohibited concept-level features in text-to-image diffusion models, reducing memorization risks while preserving generation quality.


<details>
  <summary>Details</summary>
Motivation: Memorization in large-scale text-to-image models poses security and intellectual property risks. Conventional dememorization techniques fail to prevent concept-level feature internalization, necessitating a selective unlearning method.

Method: The proposed Gradient Projection Framework identifies and removes training signals aligned with prohibited attributes during backpropagation, projecting gradients onto the orthogonal complement of sensitive feature embeddings.

Result: The framework significantly reduces memorization without compromising generation quality or semantic fidelity, as demonstrated in extensive experiments.

Conclusion: The approach reframes memorization control as selective learning, establishing a new paradigm for IP-safe and privacy-preserving generative AI.
...
### [153] [Fast EXP3 Algorithms](https://arxiv.org/abs/2512.11201)
*Ryoma Sato,Shinji Ito*

Main category: cs.LG

TL;DR: EXP3 can be implemented in constant time per round, with practical algorithms analyzed for regret bounds and time complexity trade-offs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the practicality of EXP3 by reducing its time complexity while maintaining performance.

Method: The paper proposes more practical algorithms and analyzes their implementation in constant time per round.

Result: The results highlight the trade-offs between regret bounds and computational efficiency of the proposed algorithms.

Conclusion: The conclusion suggests that constant-time implementations of EXP3 are feasible, with practical algorithms offering a balance between regret and efficiency.
...
### [154] [Latent Variable Causal Discovery under Selection Bias](https://arxiv.org/abs/2512.11219)
*Haoyue Dai,Yiwen Qiu,Ignavier Ng,Xinshuai Dong,Peter Spirtes,Kun Zhang*

Main category: cs.LG

TL;DR: The paper explores using rank constraints to address selection bias in latent variable causal discovery, showing these constraints preserve causal structure and selection mechanism information.


<details>
  <summary>Details</summary>
Motivation: Selection bias in latent variable causal discovery lacks suitable statistical tools, despite existing methods for handling latent variables.

Method: The study leverages rank constraints, a generalization of conditional independence, analyzing covariance submatrix ranks in linear Gaussian models.

Result: Rank constraints preserve causal and selection information even under bias. The one-factor model is shown identifiable under selection.

Conclusion: Simulations and real-world experiments validate the effectiveness of rank constraints for addressing selection bias.
...
### [155] [Adaptive Soft Rolling KV Freeze with Entropy-Guided Recovery: Sublinear Memory Growth for Efficient LLM Inference](https://arxiv.org/abs/2512.11221)
*Adilet Metinov,Gulida M. Kudakeeva,Bolotbek uulu Nursultan,Gulnara D. Kabaeva*

Main category: cs.LG

TL;DR: ASR-KF-EGR is a training-free inference-time framework for efficient large language model generation, reducing KV cache size significantly while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To address memory constraints in long-context LLMs without compromising generation quality or requiring fine-tuning.

Method: Uses a reversible soft-freeze mechanism to suspend KV updates for low-importance tokens, storing them off-GPU and restoring as needed, with sublinear freeze scheduling.

Result: Reduces active KV cache size by 55-67% on LLaMA-3 8B, maintains generation quality, and passes retrieval tests without fine-tuning.

Conclusion: ASR-KF-EGR offers a practical, architecture-agnostic solution for efficient long-context LLM deployment.
...
### [156] [Task-Aware Multi-Expert Architecture For Lifelong Deep Learning](https://arxiv.org/abs/2512.11243)
*Jianyu Wang,Jacob Nean-Hua Sheikh,Cat P. Le,Hoda Bidkhori*

Main category: cs.LG

TL;DR: TAME is a continual learning algorithm that uses task similarity to select and transfer knowledge among experts, reducing forgetting and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting and improve knowledge retention in lifelong deep learning across sequential tasks.

Method: TAME employs task-aware expert selection, a shared dense layer, a replay buffer for past samples, and an attention mechanism.

Result: Improved accuracy on new tasks and sustained performance on earlier ones in experiments with CIFAR-100 binary tasks.

Conclusion: TAME effectively balances adaptation and retention in lifelong learning settings.
...
### [157] [Insight Miner: A Time Series Analysis Dataset for Cross-Domain Alignment with Natural Language](https://arxiv.org/abs/2512.11251)
...
TL;DR: Insight Miner, a large-scale multimodal model (LMM), is proposed to generate domain-enriched time-series descriptions, outperforming state-of-the-art models like LLaVA and GPT-4.


<details>
  <summary>Details</summary>
Motivation: Mining insights from time-series data is time-consuming and requires domain expertise. Insight Miner aims to automate this process.

Method: The authors introduce TS-Insights, a dataset created using an agentic workflow with GPT-4, and train Insight Miner on it.

Result: Insight Miner outperforms existing models in generating accurate and comprehensive time-series descriptions.

Conclusion: The work opens new possibilities for using LMMs in time-series analysis and integrating LLMs with time-series data.
...
### [158] [A Simple Generalisation of the Implicit Dynamics of In-Context Learning](https://arxiv.org/abs/2512.11255)
*Francesco Innocenti,El Mehdi Achour*

Main category: cs.LG

TL;DR: The paper generalizes previous findings on in-context learning in transformers, extending the theory to all sequence positions, any transformer block, and realistic residual blocks with layer normalization. It validates these extensions empirically on linear regression tasks.


<details>
  <summary>Details</summary>
Motivation: The goal is to extend the theory of in-context learning (ICL) in transformers beyond simplified settings, making it applicable to more practical scenarios, including large-scale models.

Method: The authors generalize an existing theoretical framework by considering all sequence positions, any transformer block, and realistic residual blocks with layer normalization. They empirically test the theory on simple in-context linear regression tasks.

Result: The empirical verification confirms the generalization of the theory, demonstrating implicit weight updates across different tokens and blocks.

Conclusion: The study advances the understanding of ICL by bridging the gap between simplified theoretical models and practical transformer architectures, paving the way for validation on large-scale models.
...
### [159] [Features Emerge as Discrete States: The First Application of SAEs to 3D Representations](https://arxiv.org/abs/2512.11263)
*Albert Miao,Chenliang Zhou,Jiawei Zhou,Cengiz Oztireli*

Main category: cs.LG

TL;DR: This paper presents the first application of Sparse Autoencoders (SAEs) to the 3D domain, uncovering discrete feature encodings in a 3D reconstruction VAE and proposing a state transition framework to explain model behaviors.


<details>
  <summary>Details</summary>
Motivation: Despite SAEs' effectiveness in decomposing neural activations in text, their application in other domains, like 3D, remains unexplored. This work bridges that gap to understand feature decomposition dynamics in 3D models.

Method: The authors apply SAEs to a state-of-the-art 3D reconstruction VAE trained on 53k 3D models from Objaverse. They analyze feature encodings and introduce a state transition framework.

Result: Key findings include discrete feature encodings, phase-like transitions, and redistribution of feature interference. These explain unintuitive behaviors like positional encoding preferences and sigmoidal loss patterns.

Conclusion: The study advances understanding of feature decomposition in 3D models, offering a framework to explain dynamics and unexpected behaviors, with practical implications for model interpretability.
...
### [160] [SRLR: Symbolic Regression based Logic Recovery to Counter Programmable Logic Controller Attacks](https://arxiv.org/abs/2512.11298)
*Hao Zhou,Suman Sourav,Binbin Chen,Ke Yu*

Main category: cs.LG

TL;DR: SRLR is a symbolic regression-based solution for recovering PLC logic from inputs and outputs, generating explainable rules to detect controller logic attacks. It outperforms existing methods with up to 39% higher accuracy.


<details>
  <summary>Details</summary>
Motivation: PLCs in ICSs are vulnerable to cyber-attacks. Existing detection methods lack explainability or require expert input, so SRLR was developed to overcome these limitations.

Method: SRLR uses symbolic regression enhanced by ICS-specific properties: frequency domain representation, multi-mode operation, outlier filtering, and reduced formula complexity for effective search.

Result: SRLR outperforms existing methods, achieving up to 39% higher recovery accuracy in challenging environments and demonstrating stability in large-scale systems.

Conclusion: SRLR provides an effective, explainable solution for detecting PLC logic attacks, leveraging ICS-specific enhancements to outperform traditional methods.
...
TL;DR: TV2TV is a novel video-text model that interleaves text and video generation using a Mixture-of-Transformers, improving visual quality, controllability, and reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of complex video generation requiring semantic branching and high-level reasoning by integrating language modeling advances.

Method: Decomposes video generation into interleaved text (next-token prediction) and video (next-frame prediction) steps using a Mixture-of-Transformers architecture.

Result: Demonstrates improved visual quality, prompt alignment, and fine-grained controllability in experiments on video game data and natural videos.

Conclusion: TV2TV represents a promising approach for video generation with open-ended textual reasoning and user control.
...
### [162] [QGEC : Quantum Golay Code Error Correction](https://arxiv.org/abs/2512.11307)
*Hideo Mukai,Hoshitaro Ohnishi*

Main category: cs.LG

TL;DR: The paper proposes Quantum Golay Code Error Correction (QGEC) using Golay code for efficient quantum error correction, showing higher decoding accuracy than toric code with fewer qubits.


<details>
  <summary>Details</summary>
Motivation: Quantum error correction (QEC) is essential for fault-tolerant quantum computation, as qubits are prone to errors. The study explores the efficiency of Golay code, a classical coding method, in QEC.

Method: The QGEC method uses Golay code for QEC and evaluates decoder accuracy with a Transformer under various noise models and generative polynomial weights.

Result: The Golay code (23 qubits, distance 7) outperformed toric code (50 qubits, distance 5) in decoding accuracy. Smaller error correlations improved accuracy, while polynomial weights had minimal impact.

Conclusion: Implementing QEC with Golay code and Transformer decoders may enable more efficient fault-tolerant quantum computation.
...
### [163] [Benchmarking the Generality of Vision-Language-Action Models](https://arxiv.org/abs/2512.11315)
...
TL;DR: MultiNet v1.0 introduces a unified benchmark to assess vision-language-action models' cross-domain generalization, revealing current models' limitations despite strong in-distribution performance.


<details>
  <summary>Details</summary>
Motivation: To address the fragmented evaluation practices for generalist multimodal agents and assess their true cross-domain capabilities.

Method: Developed MultiNet v1.0, a benchmark evaluating models across six capability regimes like visual grounding and robot control.

Result: Models like GPT-5, Pi0, and Magma show inconsistency and degradation in unseen domains or unfamiliar modalities.

Conclusion: Current foundation models fall short of generalist intelligence, highlighting the need for improved cross-domain generalization and standardized evaluation.
...
### [164] [Condensation-Concatenation Framework for Dynamic Graph Continual Learning](https://arxiv.org/abs/2512.11317)
*Tingxu Yan,Ye Yuan*

Main category: cs.LG

TL;DR: Condensation-Concatenation-based Continual Learning (CCC) addresses catastrophic forgetting in dynamic graphs by condensing historical snapshots into semantic representations and selectively concatenating them with current graphs, outperforming baselines on four datasets.


<details>
  <summary>Details</summary>
Motivation: Dynamic graphs undergo continuous structural changes, causing catastrophic forgetting in GNNs. Existing methods ignore topological changes' impact on existing nodes, motivating CCC's development.

Method: CCC condenses historical graph snapshots into compact semantic representations, preserving label distribution and topological properties. These embeddings are selectively concatenated with current graph representations. A refined forgetting measure (FM) quantifies performance degradation due to structural updates.

Result: CCC outperforms state-of-the-art baselines across four real-world datasets, demonstrating effectiveness in mitigating catastrophic forgetting.

Conclusion: CCC effectively addresses continual learning challenges in dynamic graphs by leveraging historical condensation and selective concatenation, validated by superior experimental results.
...
### [165] [Rethinking Expert Trajectory Utilization in LLM Post-training](https://arxiv.org/abs/2512.11470)
...
TL;DR: The paper proposes the Plasticity-Ceiling Framework to optimize post-training by combining SFT and RL, identifying Sequential SFT-then-RL as superior and providing scaling guidelines for maximizing performance.


<details>
  <summary>Details</summary>
Motivation: The unresolved optimal use of expert trajectories in post-training combining SFT and RL motivated the development of a theoretical framework to guide performance improvement.

Method: The Plasticity-Ceiling Framework decomposes performance into SFT and RL components, benchmarking Sequential SFT-then-RL and deriving scaling guidelines.

Result: Sequential SFT-then-RL outperforms synchronized approaches, with scaling rules highlighting SFT phase timing, data scale, trajectory difficulty, and validation loss as key factors.

Conclusion: The findings offer actionable guidelines for leveraging expert trajectories to maximize post-training performance ceilings.
...
### [166] [Pace: Physics-Aware Attentive Temporal Convolutional Network for Battery Health Estimation](https://arxiv.org/abs/2512.11332)
*Sara Sameer,Wei Zhang,Kannan Dhivya Dharshini,Xin Lou,Yulin Gao,Terence Goh,Qingyu Yan*

Main category: cs.LG

TL;DR: Pace is a physics-aware attentive temporal convolutional network for battery health estimation, integrating raw sensor data with battery physics features. It outperforms existing models by 6.5% and 2.0x.


<details>
  <summary>Details</summary>
Motivation: Effective battery health management is crucial for safety, cost-efficiency, and sustainability in modern energy systems like electric vehicles and power grids.

Method: Pace combines raw sensor measurements with battery physics features using three specialized modules: dilated temporal blocks for temporal encoding, chunked attention blocks for context modeling, and a dual-head output block for fusing degradation patterns.

Result: Pace achieves significant improvements, outperforming baseline models by 6.5% and 2.0x on a large public dataset and demonstrates real-time viability on a Raspberry Pi.

Conclusion: Pace is validated as a practical, high-performance solution for battery health analytics.
...
### [167] [Spectral entropy prior-guided deep feature fusion architecture for magnetic core loss](https://arxiv.org/abs/2512.11334)
*Cong Yao,Chunye Gong,Jin Zhang*

Main category: cs.LG

TL;DR: SEPI-TFPNet, a hybrid model combining empirical models and deep learning, improves core loss modeling accuracy and robustness for power electronics.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional and purely data-driven core loss modeling methods, particularly in interpretability and generalization.

Method: Integrates a physical-prior submodule (spectral entropy discrimination) and a data-driven submodule (CNNs, multi-head attention, BiLSTM) with an adaptive feature fusion module.

Result: Outperforms 21 representative models from the 2023 MagNet Challenge and three advanced methods from 2024-2025, demonstrating improved accuracy and robustness.

Conclusion: The hybrid approach (SEPI-TFPNet) effectively balances model interpretability and performance, advancing core loss modeling in power electronics.
...
### [168] [DAPO: Design Structure-Aware Pass Ordering in High-Level Synthesis with Graph Contrastive and Reinforcement Learning](https://arxiv.org/abs/2512.11342)
...
TL;DR: DAPO introduces a reinforcement learning-based framework for optimizing FPGA accelerators by leveraging program semantics and hardware metrics, outperforming Vitis HLS by 2.36x on average.


<details>
  <summary>Details</summary>
Motivation: Existing HLS tools use fixed optimization strategies from software compilers, limiting their effectiveness for FPGA accelerator design due to lack of semantic understanding and hardware metric estimation.

Method: DAPO extracts program semantics from control/data flow graphs, uses contrastive learning for embeddings, and employs an analytical model for hardware metrics. A reinforcement learning agent then discovers design-specific optimizations.

Result: Experiments show DAPO achieves a 2.36x speedup over Vitis HLS on average for classic HLS designs.

Conclusion: DAPO's design-aware approach significantly improves optimization effectiveness for FPGA accelerators compared to traditional HLS tools.
...
### [169] [Symmetry-Aware Steering of Equivariant Diffusion Policies: Benefits and Limits](https://arxiv.org/abs/2512.11345)
*Minwoo Park,Junwoo Chang,Jongeun Choi,Roberto Horowitz*

Main category: cs.LG

TL;DR: Equivariant diffusion policies (EDPs) leverage geometric symmetries for efficiency, and reinforcement learning (RL) can fine-tune them, but standard RL ignores these symmetries. The paper proves EDPs' equivariance, introduces a symmetry-aware steering framework, and shows benefits like improved efficiency and policy enhancement.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency and instability of standard RL when fine-tuning equivariant diffusion policies by exploiting their inherent symmetries.

Method: Theoretical proof of EDPs' equivariance, leading to a group-invariant latent-noise MDP, and introduction of symmetry-aware RL steering.

Result: Symmetry-aware steering improves sample efficiency, prevents value divergence, and enhances policies, even with limited demonstrations.

Conclusion: Leveraging symmetries in EDP steering via RL significantly boosts performance and stability, though strict equivariance has practical limits.
...
### [170] [CAT: Can Trust be Predicted with Context-Awareness in Dynamic Heterogeneous Networks?](https://arxiv.org/abs/2512.11352)
*Jie Wang,Zheng Yan,Jiahe Lan,Xuyan Li,Elisa Bertino*

Main category: cs.LG

TL;DR: CAT is a novel Context-Aware GNN-based trust prediction model addressing dynamicity, heterogeneity, and context-awareness limitations in existing models. It achieves superior performance and robustness in experiments.


<details>
  <summary>Details</summary>
Motivation: Existing GNN-based trust prediction models struggle with dynamicity, heterogeneity, and context-awareness, leading to inaccurate predictions.

Method: CAT employs continuous-time representations for dynamic graphs, a dual attention mechanism for heterogeneity, and meta-paths for context-awareness, integrating these through four layers.

Result: CAT outperforms baselines in trust prediction on real-world datasets, showing scalability and robustness against attacks.

Conclusion: CAT effectively addresses key limitations in trust prediction, offering accurate, scalable, and robust solutions.
...
### [171] [Attacking and Securing Community Detection: A Game-Theoretic Framework](https://arxiv.org/abs/2512.11359)
*Yifan Niu,Aochuan Chen,Tingyang Xu,Jia Li*

Main category: cs.LG

TL;DR: The paper extends adversarial attacks to community detection, proposing attack and defense techniques, and a game-theoretic framework (CD-GAME) to model their interaction. Methods outperform baselines, offering insights into dynamic strategies.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of adversarial attacks in community detection, protecting privacy and improving model robustness.

Method: Proposes attack and defense techniques and a game-theoretic framework (CD-GAME) with a graph attacker and Rayleigh Quotient defender, simulating dynamic strategies.

Result: Attack and defense methods outperform baselines; CD-GAME reveals dynamic strategies and Nash equilibrium insights.

Conclusion: The work advances adversarial research in community detection, demonstrating effective techniques and valuable game-theoretic insights.
...
### [172] [Mitigating the Safety Alignment Tax with Null-Space Constrained Policy Optimization](https://arxiv.org/abs/2512.11391)
*Yifan Niu,Han Xiao,Dongyi Liu,Nuo Chen,Jia Li*

Main category: cs.LG

TL;DR: NSPO is a new RL framework addressing the alignment tax in LLMs by projecting safety gradients into the null space of general tasks, preserving core abilities while ensuring effective safety alignment.


<details>
  <summary>Details</summary>
Motivation: To ensure LLMs align with human values and ethical principles without losing general capabilities, which current RL methods fail to address due to the alignment tax.

Method: Introduces Null-Space constrained Policy Optimization (NSPO), projecting safety policy gradients into the null space of general tasks to mitigate alignment tax.

Result: NSPO outperforms existing methods, achieving top-tier safety performance without sacrificing accuracy on general tasks like math and coding, and is data-efficient.

Conclusion: NSPO effectively balances safety alignment and capability retention in LLMs, requiring less data than existing methods.
...
### [173] [Bhargava Cube--Inspired Quadratic Regularization for Structured Neural Embeddings](https://arxiv.org/abs/2512.11392)
*S Sairam,Prateek P Kulkarni*

Main category: cs.LG

TL;DR: A new neural representation learning method uses Bhargava cubes' algebraic constraints to create interpretable 3D latent spaces, achieving high accuracy and clustering on MNIST.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning lacks interpretability and mathematical consistency in latent spaces; this work aims to integrate structured algebraic priors inspired by number theory.

Method: The approach maps data to 3D latent spaces with quadratic constraints from Bhargava cubes, using a differentiable auxiliary loss independent of classification objectives.

Result: Achieves 99.46% accuracy on MNIST, with embeddings clustering by digit class and satisfying quadratic constraints.

Conclusion: This is the first use of number-theoretic constructs in neural representation learning, enabling structured mathematical priors in neural networks.
...
### [174] [Sliced ReLU attention: Quasi-linear contextual expressivity via sorting](https://arxiv.org/abs/2512.11411)
*Siwan Boufadène,François-Xavier Vialard*

Main category: cs.LG

TL;DR: Sliced ReLU attention is a novel attention mechanism with quasi-linear complexity, strong expressive power, and practical benefits, demonstrated in small-scale experiments.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the limitations of existing attention mechanisms (softmax and ReLU-based) by proposing a more efficient and structurally different approach suitable for long contexts.

Method: The method involves operating on one-dimensional projections of key-query differences and leveraging sorting to achieve quasi-linear complexity (O(n log(n))). It introduces a differentiable, non-symmetric kernel.

Result: The sliced ReLU attention retains strong theoretical expressive power, performing nontrivial sequence-to-sequence tasks and satisfying contextual universal approximation properties.

Conclusion: The proposed mechanism offers computational efficiency and expressive power, making it a promising alternative for practical applications, as shown in initial experiments.
...
### [175] [Hyperbolic Gaussian Blurring Mean Shift: A Statistical Mode-Seeking Framework for Clustering in Curved Spaces](https://arxiv.org/abs/2512.11448)
*Arghya Pratihar,Arnab Seal,Swagatam Das,Inesh Chattopadhyay*

Main category: cs.LG

TL;DR: HypeGBMS extends Gaussian Blurring Mean Shift (GBMS) to hyperbolic space, using hyperbolic distances and Möbius-weighted means to better capture hierarchical structures, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: GBMS struggles with hierarchical or tree-like datasets. HypeGBMS aims to address this by leveraging hyperbolic space.

Method: Replaces Euclidean computations with hyperbolic distances and uses Möbius-weighted means for geometric consistency.

Result: HypeGBMS improves clustering quality in hierarchical datasets and outperforms traditional GBMS.

Conclusion: HypeGBMS bridges classical mean-shift and hyperbolic learning, offering robust density-based clustering in curved spaces.
...
### [176] [NeuralOGCM: Differentiable Ocean Modeling with Learnable Physics](https://arxiv.org/abs/2512.11525)
*Hao Wu,Yuan Gao,Fan Xu,Fan Zhang,Guangliang Liu,Yuxuan Liang,Xiaomeng Huang*

Main category: cs.LG

TL;DR: NeuralOGCM combines differentiable programming and deep learning to create an ocean modeling framework that balances computational efficiency and physical fidelity.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between computational efficiency and physical fidelity in high-precision scientific simulations.

Method: Fuses a differentiable dynamical solver with deep learning, integrating physics knowledge and learnable parameters. A deep neural network corrects subgrid-scale processes and discretization errors.

Result: NeuralOGCM achieves long-term stability and physical consistency, outperforming traditional numerical models in speed and pure AI baselines in accuracy.

Conclusion: The framework offers a promising approach for developing fast, stable, and physically-plausible models in scientific computing.
...
### [177] [Contrastive Time Series Forecasting with Anomalies](https://arxiv.org/abs/2512.11526)
*Joel Ekstrand,Zahra Taghiyarrenani,Slawomir Nowaczyk*

Main category: cs.LG

TL;DR: Co-TSFA is a framework for time series forecasting that distinguishes between relevant and irrelevant anomalies, improving accuracy under abnormal conditions without affecting normal data performance.


<details>
  <summary>Details</summary>
Motivation: Standard forecasting models struggle to differentiate between lasting and short-lived anomalies, leading to inaccurate predictions.

Method: Co-TSFA uses input-only and input-output augmentations to model anomalies and employs a latent-output alignment loss to link representation changes to forecast adjustments.

Result: The framework outperforms benchmarks on Traffic, Electricity, and cash-demand datasets, maintaining accuracy for normal data.

Conclusion: Co-TSFA effectively balances sensitivity to meaningful anomalies while ignoring irrelevant ones, enhancing forecasting robustness.
...
### [178] [xGR: Efficient Generative Recommendation Serving at Scale](https://arxiv.org/abs/2512.11529)
...
TL;DR: xGR is a system optimizing generative recommendation tasks by reducing latency and improving throughput under high concurrency, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: GR's computational inefficiency due to long prompts, high beam width, and sorting overhead in large item spaces necessitates a specialized serving system for low-latency, high-concurrency scenarios.

Method: xGR introduces staged computation, separated KV cache, early sorting termination, mask-based item filtering, and pipeline reconstruction leveraging multilevel overlap and multi-stream parallelism.

Result: Experiments show xGR achieves at least 3.49x higher throughput than state-of-the-art baselines under strict latency constraints.

Conclusion: xGR effectively addresses GR's inefficiencies, enhancing performance in recommendation systems.
...
### [179] [Parametric Numerical Integration with (Differential) Machine Learning](https://arxiv.org/abs/2512.11530)
*Álvaro Leitao,Jonatan Ráfales*

Main category: cs.LG

TL;DR: A differential learning framework for solving parametric integrals outperforms standard machine learning methods in accuracy, scalability, and efficiency.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and efficiency of solving parametric integrals using machine learning.

Method: Incorporates derivative information during training, tested on statistical functionals, Chebyshev expansions, and integrals from differential equations.

Result: Outperforms standard architectures with lower mean squared error, better scalability, and improved sample efficiency.

Conclusion: Differential machine learning is superior for parametric integrals across diverse problem classes.
...
### [180] [A Multi-Criteria Automated MLOps Pipeline for Cost-Effective Cloud-Based Classifier Retraining in Response to Data Distribution Shifts](https://arxiv.org/abs/2512.11541)
*Emmanuel K. Katalay,David O. Dimandja,Jordan F. Masakuna*

Main category: cs.LG

TL;DR: Automated MLOps pipeline detects data distribution drift and triggers efficient neural network retraining.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of deteriorating ML model performance due to data distribution drift and manual MLOps processes.

Method: Uses multi-criteria statistical techniques to detect shifts and automate retraining.

Result: Improves model accuracy and robustness on benchmark anomaly detection datasets.

Conclusion: Provides a reliable, adaptive solution for ML systems in dynamic environments.
...
### [181] [Elastic-Net Multiple Kernel Learning: Combining Multiple Data Sources for Prediction](https://arxiv.org/abs/2512.11547)
*Janaina Mourão-Miranda,Zakria Hussain,Konstantinos Tsirlis,Christophe Phillips,John Shawe-Taylor*

Main category: cs.LG

TL;DR: The paper introduces an efficient elastic-net regularized Multiple Kernel Learning (ENMKL) method with analytical kernel weight updates, outperforming $l1$-norm MKL and offering sparser, interpretable models.


<details>
  <summary>Details</summary>
Motivation: To improve interpretability and performance in MKL by introducing a simplified ENMKL formulation that avoids the need for two-stage procedures and better handles correlated kernels.

Method: Proposes an ENMKL formulation with analytical updates for kernel weights, implemented for SVM and KRR, and integrated into the PRoNTo toolbox.

Result: ENMKL matches or surpasses $l1$-norm MKL in performance and outperforms standard SVM in most cases, while providing sparser and more interpretable models.

Conclusion: The proposed ENMKL method is effective for neuroimaging applications, offering both performance gains and improved interpretability through selective kernel weighting.
...
### [182] [Fully Inductive Node Representation Learning via Graph View Transformation](https://arxiv.org/abs/2512.11561)
*Dooho Lee,Myeong Kong,Minho Jeong,Jaemin Yoo*

Main category: cs.LG

TL;DR: The paper proposes Graph View Transformation (GVT) and Recurrent GVT for fully inductive node representation learning on unseen graph datasets, achieving superior performance over prior methods.


<details>
  <summary>Details</summary>
Motivation: To enable pretrained graph models to generalize to unseen datasets without retraining, addressing challenges like varying feature spaces and inductive applicability limitations.

Method: Introduces the view space for unified graph encoding and proposes GVT, a permutation-equivariant mapping, leading to Recurrent GVT for node representation learning.

Result: Recurrent GVT outperforms GraphAny by +8.93% and 12 individually tuned GNNs by at least +3.30% on 27 benchmarks.

Conclusion: The view space and Recurrent GVT provide an effective framework for fully inductive node representation learning.
...
### [183] [Brain-Semantoks: Learning Semantic Tokens of Brain Dynamics with a Self-Distilled Foundation Model](https://arxiv.org/abs/2512.11582)
*Sam Gijsen,Marc-Andre Schulz,Kerstin Ritter*

Main category: cs.LG

TL;DR: Brain-Semanticok introduces a self-supervised framework for fMRI data, focusing on abstract representations to improve robustness and reduce fine-tuning needs.


<details>
  <summary>Details</summary>
Motivation: Current fMRI models focus on low-level information, making them noise-sensitive and requiring extensive fine-tuning. Brain-Semanticok aims to address this by learning abstract representations.

Method: The framework uses a semantic tokenizer to aggregate noisy signals into robust tokens and a self-distillation objective for temporal stability, aided by a novel training curriculum.

Result: The model achieves strong performance on downstream tasks with minimal fine-tuning (linear probe) and scales well with more unlabeled data.

Conclusion: Brain-Semanticok improves fMRI analysis robustness and reduces fine-tuning needs, showing promise for broader applications.
...
### [184] [Atomic Action Slicing: Planner-Aligned Options for Generalist VLA Agents](https://arxiv.org/abs/2512.11584)
*Stefan Tabakov,Asen Popov,Dimitar Dimitrov,S. Ensiye Kiyamousavi,Vladimir Hristov,Boris Kraychev*

Main category: cs.LG

TL;DR: AAS decomposes long-horizon demonstrations into typed atomic actions, improving VLA model performance with a validated dataset and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current VLA models struggle with generalizing tasks requiring new compositions of skills or objects.

Method: Atomic Action Slicing (AAS) decomposes demonstrations into atomic actions, validated using LIBERO demonstrations and labeled with action type, temporal span, and confidence.

Result: AAS improves task success rates: 94.2% to 95.3% on LIBERO-Goal and 83.8% to 88.8% on LIBERO-Long. A stronger segmenter (Gemini 2.5 Pro) matches planner-defined plans robustly.

Conclusion: AAS and the GATE-VLAP dataset enhance VLA model generalization and performance, with publicly available resources.
...
### [185] [Gradient Descent as a Perceptron Algorithm: Understanding Dynamics and Implicit Acceleration](https://arxiv.org/abs/2512.11587)
*Alexander Tyurin*

Main category: cs.LG

TL;DR: The paper analyzes gradient descent (GD) in neural network training, reducing it to generalized perceptron algorithms for simpler analysis. It shows provably faster convergence in nonlinear models compared to linear ones.


<details>
  <summary>Details</summary>
Motivation: Understanding the optimization dynamics of GD in neural networks, including implicit acceleration and convergence rates, is a challenging problem.

Method: The study reduces GD steps to generalized perceptron algorithms, using classical linear algebra tools to analyze a minimalistic two-layer nonlinear model.

Result: The nonlinearity in the model yields a faster iteration complexity of $	ilde{O}(\\sqrt{d})$ compared to $\\Omega(d)$ in linear models. This explains observed implicit acceleration in neural networks.

Conclusion: The alternative perspective provided by this analysis can advance research on neural network optimization, supported by numerical experiments.
...
### [186] [A Fast Interpretable Fuzzy Tree Learner](https://arxiv.org/abs/2512.11616)
*Javier Fumanal-Idocin,Raquel Fernandez-Peralta,Javier Andreu-Perez*

Main category: cs.LG

TL;DR: The paper proposes a fuzzy tree-based splitting algorithm combining computational efficiency and interpretability, outperforming evolutionary methods in speed without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing fuzzy rule-mining algorithms lack guarantees for interpretability and efficiency, prompting the need for a better approach.

Method: Adapts classical tree-based splitting algorithms to fuzzy trees, blending greedy computational efficiency with fuzzy logic interpretability.

Result: Achieves competitive predictive performance with faster runtime and more interpretable rule bases compared to evolutionary methods.

Conclusion: The method offers a practical balance of interpretability, efficiency, and accuracy for fuzzy rule-based systems.
...
### [187] [Bridging Streaming Continual Learning via In-Context Large Tabular Models](https://arxiv.org/abs/2512.11668)
*Afonso Lourenço,João Gama,Eric P. Xing,Goreti Marreiros*

Main category: cs.LG

TL;DR: The paper proposes using large in-context tabular models (LTMs) to bridge Streaming Continual Learning (SCL), addressing both rapid adaptation in stream learning and long-term retention in continual learning through data selection principles.


<details>
  <summary>Details</summary>
Motivation: The paper aims to unify Continual Learning (CL) and Stream Learning (SL) paradigms, which currently address adaptation and retention challenges in isolation, by leveraging LTMs as a bridge for SCL.

Method: The authors suggest summarizing unbounded streams into compact sketches for LTMs, focusing on data selection principles: distribution matching (balancing plasticity and stability) and distribution compression (managing memory via diversification and retrieval).

Result: The proposed approach aligns SL's compression goals with CL's experience-replay needs, offering a framework for SCL that balances adaptation and retention.

Conclusion: LTMs provide a viable solution for SCL by integrating SL and CL principles, with data selection mechanisms ensuring efficient and stable learning.
...
### [188] [High-Dimensional Surrogate Modeling for Closed-Loop Learning of Neural-Network-Parameterized Model Predictive Control](https://arxiv.org/abs/2512.11705)
*Sebastian Hirt,Valentinus Suwanto,Hendrik Alsmeier,Maik Pfefferkorn,Rolf Findeisen*

Main category: cs.LG

TL;DR: Bayesian neural networks outperform Gaussian processes in optimizing high-dimensional controller parameters, enabling faster convergence and better performance in dense parameter spaces.


<details>
  <summary>Details</summary>
Motivation: The challenge of tuning dense high-dimensional controller parameters efficiently motivates exploring Bayesian neural networks as surrogate models.

Method: Comparison of Gaussian processes (Matern kernels), finite-width Bayesian neural networks, and infinite-width Bayesian neural networks on a cart-pole task.

Result: Bayesian neural networks achieve faster convergence and better performance in high-dimensional spaces, with infinite-width networks maintaining effectiveness even beyond 1,000 parameters.

Conclusion: Bayesian neural networks are superior for high-dimensional controller tuning, providing practical guidance for surrogate model selection in controller design.
...
### [189] [SpectralKrum: A Spectral-Geometric Defense Against Byzantine Attacks in Federated Learning](https://arxiv.org/abs/2512.11760)
*Aditya Tripathi,Karan Sharma,Rahul Mishra,Tapas Kumar Maiti*

Main category: cs.LG

TL;DR: SpectralKrum is a defense mechanism for Federated Learning that combines spectral subspace estimation with geometric neighbor-based selection to filter out Byzantine attacks, showing effectiveness in certain attack scenarios but limited in others.


<details>
  <summary>Details</summary>
Motivation: Federated Learning is vulnerable to Byzantine attacks where malicious clients compromise the global model. Existing defenses fail under heterogeneous data distributions and adaptive adversaries.

Method: SpectralKrum uses spectral subspace estimation and Krum selection to filter updates, leveraging historical aggregates to identify benign optimization trajectories.

Result: SpectralKrum performs well against directional and subspace-aware attacks but is less effective against label-flip and min-max attacks where malicious updates are spectrally similar to benign ones.

Conclusion: SpectralKrum enhances FL robustness by combining subspace learning and geometric selection, though its efficacy depends on the attack type.
...
### [190] [The Adaptive Vekua Cascade: A Differentiable Spectral-Analytic Solver for Physics-Informed Representation](https://arxiv.org/abs/2512.11776)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: The paper introduces the Adaptive Vekua Cascade (AVC), a hybrid architecture combining deep learning and classical approximation theory to address spectral bias and the curse of dimensionality in coordinate-based neural networks. AVC uses a deep network for manifold learning and a differentiable solver for function approximation, achieving state-of-the-art accuracy with fewer parameters and faster convergence.


<details>
  <summary>Details</summary>
Motivation: Coordinate-based neural networks struggle with spectral bias (difficulty learning high-frequency dynamics) and the curse of dimensionality (parameter explosion in discrete grids). AVC aims to solve these issues by decoupling manifold learning from function approximation.

Method: AVC employs a deep network to learn a diffeomorphic warping of the physical domain, projecting dynamics onto a latent manifold represented by generalized analytic functions. It replaces gradient-descent output with a differentiable linear solver for optimal spectral coefficient resolution.

Result: AVC outperforms existing methods in accuracy, reduces parameters significantly (e.g., 840 vs. 4.2 million for 3D grids), and converges 2-3x faster than implicit neural representations. Benchmarks include Helmholtz wave propagation, medical reconstruction, and Navier-Stokes turbulence.

Conclusion: AVC establishes a new paradigm for efficient, spectrally accurate scientific machine learning, addressing key limitations of current neural network approaches.
...
### [191] [Softmax as Linear Attention in the Large-Prompt Regime: a Measure-based Perspective](https://arxiv.org/abs/2512.11784)
*Etienne Boursier,Claire Boyer*

Main category: cs.LG

TL;DR: The paper develops a measure-based framework to analyze softmax attention in transformers, showing it converges to a linear operator for infinite prompts and remains stable during training.


<details>
  <summary>Details</summary>
Motivation: Softmax attention's nonlinearity complicates theoretical analysis, prompting a unified framework to study its behavior under finite and infinite prompts.

Method: Leveraging i.i.d. Gaussian inputs, the paper derives non-asymptotic bounds for softmax attention outputs and gradients, linking finite-prompt behavior to infinite-prompt dynamics.

Result: Softmax attention converges linearly for infinite prompts, remains stable during training, and inherits linear attention's analytical structure for long prompts.

Conclusion: The framework enables optimization analyses for softmax attention in large-prompt regimes, bridging nonlinear and linear attention theories.
...
### [192] [A General Algorithm for Detecting Higher-Order Interactions via Random Sequential Additions](https://arxiv.org/abs/2512.11793)
*Ahmad Shamail,Claire McWhite*

Main category: cs.LG

TL;DR: A geometric method detects interactions and redundancies in systems by analyzing sequential contributions, revealing patterns like L-shapes for redundancy/synergy and order-invariant distributions for independence. The L-score quantifies these interactions.


<details>
  <summary>Details</summary>
Motivation: To discover and quantify complex interactions (synergy, redundancy, independence) among system components, which is crucial for understanding system behavior.

Method: Sequentially add elements in random orders, plot their contributions, and analyze patterns (e.g., L-shapes). The L-score measures interaction strength (-1 for synergy, 0 for independence, +1 for redundancy).

Result: L-shaped patterns differentiate redundancy (only first-added contributes) and synergy (elements contribute together). Independent elements have order-invariant distributions. Higher-order interactions emerge from pairwise relationships.

Conclusion: This geometric approach provides a unified, metric-agnostic way to quantify interactions, applicable across domains where incremental performance evaluation is possible.
