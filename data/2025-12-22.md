...
TL;DR: V-Agent is a multi-agent platform using a fine-tuned vision-language model (VLM) for advanced video search and interactive conversations, achieving top zero-shot performance on MultiVENT 2.0.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of text-based retrieval systems in multimodal scenarios by integrating visual and spoken content interpretation.

Method: Fine-tunes VLM with a video preference dataset, enhances it with retrieval vectors, and uses three agents (routing, search, chat) for context-aware video search.

Result: Achieves state-of-the-art zero-shot performance on MultiVENT 2.0 benchmark.

Conclusion: V-Agent shows promise for academic and real-world applications in multimodal video retrieval.
...
TL;DR: The study aimed to develop a system for quickly identifying pornographic content in websites using deep learning, comparing CNN and VGG-16 models. The CNN model outperformed VGG-16 with a 94.87% accuracy rate.


<details>
  <summary>Details</summary>
Motivation: The Indonesian government blocked many websites for negative content, but VPNs allowed access, prompting the need for a quick pornographic content detection system.

Method: A deep learning approach using CNN and VGG-16 models was employed to detect pornographic image content, with comprehensive exploration of both models.

Result: The CNN model achieved the highest accuracy (94.87%) in detecting pornographic content compared to the VGG-16 model.

Conclusion: The CNN model is more effective and accurate for quick detection of pornographic content than the VGG-16 model.
...
TL;DR: The paper introduces the Adaptive Visual Model (AVM), a modular framework that separates stable visual encoding from condition-specific adaptation in neural responses, outperforming current models in generalization and interpretability.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle to generalize across stimuli and individuals due to unclear separation of stable encoding and adaptation.

Method: AVM uses a frozen Vision Transformer encoder for stable features and modular subnetworks for condition-specific modulation.

Result: AVM outperforms V1T by ~2% in predictive correlation and improves explained variance by 9.1% in cross-dataset settings.

Conclusion: AVM offers a scalable, interpretable framework for adaptive neural modeling, bridging neuroscience and AI.
...
TL;DR: The paper introduces Finer-CAM to analyze TLS projections for tree species classification, achieving 96% accuracy. Crown features dominate decisions, but stem features are key for certain species.


<details>
  <summary>Details</summary>
Motivation: To understand the decision processes of deep learning models in tree species classification using TLS data and Finer-CAM explanations.

Method: Used TLS data from 2,445 trees across seven species, trained five YOLOv8 models with cross-validation, and analyzed saliency maps.

Result: Mean accuracy of 96%; crown features dominate classification, but stem features are critical for some species. Models align with human expert similarity judgments.

Conclusion: Highlights the need for better understanding model decisions to address biases, limitations, and build trust in predictions.
...
TL;DR: The paper proposes a multi-stage method for generating cohesive video stories with consistent characters, using a filmmaking-like approach involving script generation, visual anchoring, and scene synthesis. It validates the importance of visual priors and identifies cultural biases in the models.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in current text-to-video AI's inability to produce long, cohesive stories with consistent characters, prompting the need for a structured, filmmaker-like approach.

Method: The method involves a pipeline: (1) a large language model generates a detailed script, (2) a text-to-image model creates consistent visuals for characters, and (3) a video generation model synthesizes scenes individually using these visuals as anchors.

Result: Removing visual anchoring drops character consistency scores drastically (7.99 to 0.55), proving its necessity. Cultural biases were also noted between Indian and Western-themed generations.

Conclusion: The multi-stage decomposition, especially visual anchoring, is crucial for character consistency. Cultural disparities highlight areas needing further model refinement.
...
TL;DR: InfoTok introduces an adaptive video tokenization framework inspired by information theory, optimizing token allocation for improved efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Current video tokenizers use fixed compression rates, causing redundancy or information loss. InfoTok addresses this by adapting tokenization to the video's informational richness.

Method: The paper presents an ELBO-based algorithm for adaptive tokenization and develops a transformer-based adaptive compressor to dynamically adjust token allocation.

Result: InfoTok achieves state-of-the-art performance, saving 20% tokens without performance loss and outperforming prior methods with a 2.3x compression rate.

Conclusion: By dynamically allocating tokens, InfoTok offers a more efficient and accurate video representation, providing insights for future research in adaptive tokenization.
...
TL;DR: Endo-SemiS is a semi-supervised framework for endoscopic video segmentation, leveraging unlabeled data via cross-supervision, uncertainty-guided pseudo-labels, joint supervision, and mutual learning, achieving strong results with limited annotations.


<details>
  <summary>Details</summary>
Motivation: The need for reliable segmentation in endoscopic video frames with minimal labeled data drives the development of Endo-SemiS.

Method: Uses cross-supervision, uncertainty-guided pseudo-labels, joint supervision, mutual learning, and a corrective network for spatiotemporal improvement.

Result: Outperforms state-of-the-art methods on kidney stone and polyp datasets with limited labeled data.

Conclusion: Endo-SemiS demonstrates effectiveness in semi-supervised endoscopic segmentation, offering practical solutions for clinical applications.
...
TL;DR: LongShOTBench is a diagnostic benchmark for long-form multimodal video understanding, featuring open-ended questions, multiturn dialogues, and tasks requiring multimodal reasoning. It includes graded rubrics and human-verified samples. LongShOTAgent, a new system, shows promising results, but gaps highlight the challenge of real-world video understanding.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack integration of temporal length and multimodal richness, relying on single-score accuracy that obscures failure modes. LongShOTBench aims to address these limitations with a comprehensive, human-validated approach.

Method: The benchmark is produced via a scalable, human-validated pipeline, ensuring coverage and reproducibility. It includes tasks requiring multimodal reasoning and agentic tool use. LongShOTAgent, an agentic system, is introduced for analyzing long videos via preprocessing, search, and refinement.

Result: State-of-the-art MLLMs show significant gaps: Gemini-2.5-Flash achieves 52.95%, open-source models remain below 30%, and LongShOTAgent attains 44.66%. Results highlight the difficulty of long-form video understanding.

Conclusion: LongShOTBench provides a practical, reproducible foundation for evaluating MLLMs' long-form video understanding capabilities. The resource aims to drive improvements in the field.
...
TL;DR: 4D-RGPT is introduced to enhance 4D perception in MLLMs, supported by P4D training and R4D-Bench benchmark, showing notable improvements.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs lack strong 4D perception and temporal understanding, and current benchmarks focus on static scenes without region-level prompting.

Method: Introduces 4D-RGPT for 4D video representations, P4D training framework for transferring 4D knowledge, and R4D-Bench for dynamic scenes with region-level prompts.

Result: 4D-RGPT improves performance on existing 4D VQA benchmarks and the new R4D-Bench.

Conclusion: The proposed solutions effectively address limitations in MLLMs' 4D perception and temporal reasoning.
...
TL;DR: FORMSpoT offers high-resolution forest canopy mapping and disturbance detection in France, outperforming existing tools with a 1.5 m resolution and robust change detection methods.


<details>
  <summary>Details</summary>
Motivation: The decline of Europe's forest carbon sink necessitates precise, frequent monitoring tools to detect disturbances at individual tree scales.

Method: FORMSpoT uses SPOT-6/7 data and a transformer model trained on ALS data, combined with co-registration and denoising for accurate change detection.

Result: FORMSpoT achieves an F1-score of 0.44 in mountainous forests, significantly outperforming other products.

Conclusion: FORMSpoT enables tree-level monitoring, aiding forest management and carbon loss quantification, emphasizing the need for high-resolution satellite missions.
...
TL;DR: InfCam is a depth-free framework for camera-controlled video generation, addressing pose fidelity issues with infinite homography warping and data augmentation.


<details>
  <summary>Details</summary>
Motivation: Enable high-fidelity camera control in video generation without relying on error-prone depth estimation or limited datasets.

Method: Uses infinite homography warping for 3D rotations in 2D space and a data augmentation pipeline for diverse trajectories.

Result: Outperforms baselines in pose accuracy and visual fidelity, generalizing well to real-world data.

Conclusion: InfCam effectively addresses current limitations, offering a robust solution for cinematic camera control in video generation.
...
TL;DR: The paper introduces an interpretable measure (IUS) to assess the utility similarity between synthetic and real medical images for DL-based clinical decision support systems, showing significant improvements in classification performance.


<details>
  <summary>Details</summary>
Motivation: There's a need for a quantitative method to evaluate how well synthetic medical images mimic real ones for training DL-based CDS systems, as current methods like user evaluations or inception-based measures lack interpretability.

Method: The paper proposes IUS, a measure inspired by generalized neural additive models, focusing on clinically relevant image features to provide interpretable comparisons between synthetic and real datasets.

Result: Experiments on various medical imaging modalities (endoscopic, dermoscopic, fundus, X-ray, ultrasound) show that using IUS to select high-utility synthetic images can improve classification performance by up to 54.6%.

Conclusion: IUS offers a general, interpretable solution for assessing synthetic medical images' utility, enhancing the development of DL-based CDS systems.
...
TL;DR: DGH is a novel framework for learning hair dynamics and appearance efficiently, offering scalable and data-driven solutions over physics-based methods.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with manual tuning and computational demands for photorealistic dynamic hair modeling.

Method: Uses a coarse-to-fine model for hair motion and strand-guided optimization for dynamic 3D Gaussian representation.

Result: Achieves promising geometry and appearance results, generalizing across hairstyles and motions.

Conclusion: DGH provides a scalable, data-driven alternative to traditional simulation-based pipelines.
...
TL;DR: The survey highlights a research gap in transformer-based maritime radar frame prediction, despite its proven success in AIS-based trajectory and sonar frame forecasting.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored application of transformer architectures in maritime radar frame prediction, given radar's reliability in all-weather navigation.

Method: Systematically reviews predictive modeling approaches for maritime radar, focusing on transformer architectures for spatiotemporal sequence forecasting.

Result: Identifies no prior work on transformer-based radar frame prediction, establishing a clear research gap.

Conclusion: The review motivates future research to explore transformer architectures for maritime radar frame prediction.
...
TL;DR: The paper proposes a method to transform casual photos into professional-looking portraits by preserving identity while adjusting pose, lighting, and clothing, using UV space and multi-image finetuning.


<details>
  <summary>Details</summary>
Motivation: Professional photos differ from casual ones in lighting, pose, and quality. The goal is to create professional versions of casual photos while preserving identity, despite lacking large paired datasets.

Method: The approach uses UV space transformation for pose adjustment and occlusion modeling, leveraging unpaired datasets. Multi-image finetuning personalizes the output.

Result: The method produces high-quality reposed portraits with strong performance on real-world imagery.

Conclusion: The proposed technique effectively transforms casual photos into professional portraits by addressing key challenges like identity preservation and dataset limitations.
...
TL;DR: A framework for generating document backgrounds with multi-page editing and thematic continuity, ensuring readability and aesthetic harmony while allowing customization.


<details>
  <summary>Details</summary>
Motivation: To bridge generative modeling with natural design workflows by producing documents that are visually coherent, text-preserving, and thematically aligned.

Method: Uses latent masking for soft attenuation, Automated Readability Optimization (ARO) for contrast adherence, and a summarization-and-instruction process for multi-page consistency.

Result: Produces documents with preserved readability, aesthetic harmony, and thematic continuity across pages.

Conclusion: The framework successfully integrates generative modeling with design workflows, offering automated consistency and flexible customization.
...
TL;DR: PhysFire-WM combines physics priors and cross-task training for accurate fire spread prediction.


<details>
  <summary>Details</summary>
Motivation: Current fire prediction methods lack physical consistency and fail to capture complex dynamics.

Method: Uses a Physics-informed World Model with CC-Train strategy for joint thermal and boundary modeling.

Result: Outperforms existing methods in accuracy on a fine-grained multimodal fire dataset.

Conclusion: Highlighting physical priors and task collaboration improves disaster prediction models.
...
TL;DR: LGCLIP is a lightweight framework that uses LLM-generated prompts and synthesized images for zero-shot classification, avoiding the need for annotated image-text pairs.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs rely on costly annotated text-image pairs and dual-tower encoders, limiting efficiency and scalability. LGCLIP aims to reduce these dependencies.

Method: LGCLIP uses an LLM to generate prompts for a diffusion model, creating visual prototypes. Real images are compared to these prototypes using a single visual encoder for classification.

Result: Experiments show LGCLIP performs well in zero-shot classification, offering a lightweight and efficient alternative to traditional methods.

Conclusion: LGCLIP successfully eliminates the need for manual annotations and dual encoders, setting a new paradigm for zero-shot classification tasks.
...
TL;DR: ABE-CLIP is a training-free method enhancing attribute-object binding in CLIP-like models via semantic refinement and localized token-patch alignment, improving compositional matching without additional training.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with compositional image-text matching due to its global representation overlooking fine-grained semantics for attribute binding, limiting generalization.

Method: ABE-CLIP uses Semantic Refinement Mechanism to refine token embeddings and Local Token-Patch Alignment to compute localized similarity scores between text tokens and image patches.

Result: Experiments show ABE-CLIP improves attribute-object binding performance, outperforming methods requiring extensive training.

Conclusion: ABE-CLIP effectively addresses CLIP's attribute binding limitations without additional training, enhancing compositional understanding.
...
TL;DR: The study examines discrepancies between objective and subjective measures of urban greenery, using street view imagery and surveys from 1,000 people across five countries. It finds that demographics and personality have little impact, but location significantly influences perception.


<details>
  <summary>Details</summary>
Motivation: Understanding the differences between objective and subjective assessments of urban greenery is crucial for urban planning and well-being, as they may not always align.

Method: The study combines street view imagery analysis (Green View Index) with a visual perception survey of 1,000 participants, incorporating demographic and personality data, to analyze discrepancies.

Result: Discrepancies in greenery perception are consistent globally. Demographics and personality have minimal impact, but location (cultural, environmental factors) significantly influences perception.

Conclusion: Location-based factors, rather than individual traits, play a key role in how people perceive urban greenery, highlighting the importance of contextual influences in urban planning.
...
TL;DR: A globally optimal solver for relative pose estimation in multi-camera systems using affine correspondences, improving accuracy by minimizing algebraic error and solving polynomial equations.


<details>
  <summary>Details</summary>
Motivation: To enhance the accuracy of relative pose estimation in multi-camera systems with IMU, crucial for applications like self-driving cars.

Method: Decouples rotation and translation, formulates a cost function, converts optimization into polynomial solving, and introduces a linear solution for small rotations.

Result: Outperforms state-of-the-art methods in accuracy on synthetic and real-world datasets.

Conclusion: The proposed solver effectively improves relative pose estimation accuracy, making it suitable for practical applications.
...
TL;DR: ARCD introduces targeted, region-specific guidance to reduce hallucinations in MedVLMs, improving diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in MedVLMs reduce reliability, and existing mitigation strategies are either costly or untargeted.

Method: ARCD uses an anatomical mask for a three-tiered contrastive decoding process, dynamically re-weighting token, attention, and logits levels.

Result: The method improves regional understanding, reduces hallucinations, and enhances diagnostic accuracy across diverse medical imaging datasets.

Conclusion: ARCD provides a scalable, effective solution to mitigate hallucinations in MedVLMs, advancing their clinical utility.
...
TL;DR: The paper proposes a lightweight network called Fose that combines one-step diffusion models (DM) and end-to-end (E2E) models for pansharpening, achieving faster performance and better results.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of multi-step DMs and the performance limitations of E2E models in pansharpening.

Method: A four-stage training strategy is introduced, involving one-step distillation of an enhanced DM and fusion with an E2E model using lightweight ensemble blocks.

Result: Fose shows significant improvement on benchmarks, with a 7.42 speedup ratio over baseline DM and better performance.

Conclusion: The proposed Fose network offers a faster and more effective solution for pansharpening, balancing accuracy and efficiency.
...
TL;DR: Reasoning Palette introduces a latent-modulation framework using stochastic latent variables to enhance reasoning diversity and efficiency in large vision-language models, outperforming standard RL methods.


<details>
  <summary>Details</summary>
Motivation: To address redundancy in reasoning paths during stochastic sampling and improve exploration capacity for better inference-time performance and RL training.

Method: Uses a VAE to infer latent contexts from question-answer pairs, decoded into token prefixes modulating the model's reasoning trajectory. Includes SFT warm-up and structured exploration during RL.

Result: Achieves interpretable and controllable strategic behavior, with consistent performance gains over standard RL methods across reasoning benchmarks.

Conclusion: The framework enhances reasoning diversity and exploration efficiency, leading to improved model performance.
...
TL;DR: CheXPO-v2 improves medical VLMs by shifting from outcome to process supervision, using a Knowledge Graph Consistency Reward to reduce hallucinations and enhance clinical reliability.


<details>
  <summary>Details</summary>
Motivation: Current VLMs suffer from hallucinations and unreliable reasoning due to sparse, outcome-based rewards, posing safety risks.

Method: Proposes CheXPO-v2 with Entity-Relation Matching for fine-grained supervision and hard-example mining for efficiency.

Result: Outperforms GRPO and state-of-the-art models on MIMIC-CXR-VQA, achieving high accuracy with only 5k samples.

Conclusion: CheXPO-v2 offers clinically sound, verifiable reasoning and exceptional data efficiency, with publicly available code.
...
TL;DR: DAVE, a vision encoder for VLMs, addresses the lack of robust spatial and structural features in existing models for document and web tasks, using self-supervised and supervised pretraining with novel alignment strategies.


<details>
  <summary>Details</summary>
Motivation: Current VLMs' vision encoders lack robust structural and spatial features critical for document understanding and web agent tasks.

Method: Combines self-supervised pretraining on unlabeled data with supervised autoregressive pretraining. Uses model-merging and ensemble training to align with diverse tasks.

Result: DAVE outperforms on document tasks, VQAs, web localization, and agent benchmarks.

Conclusion: DAVE is an effective vision encoder tailored for document and web applications.
...
TL;DR: AOM is a universal Remote Sensing Foundation Model designed to handle arbitrary band compositions, sensor types, and resolution scales, addressing limitations of existing models.


<details>
  <summary>Details</summary>
Motivation: Existing RSFMs struggle with real-world challenges like missing bands, cross-sensor fusion, and varying resolutions due to fixed band configurations and pretraining setups.

Method: AOM uses a spectrum-independent tokenizer, multi-scale adaptive patch embedding, and a multi-scale semantic alignment mechanism with self-supervised pretraining.

Result: AOM achieves SOTA performance across 10+ datasets, excelling in band-missing, cross-sensor, and cross-resolution scenarios.

Conclusion: AOM significantly advances RSFM generalization and practicality by effectively handling diverse optical satellite data.
...
TL;DR: The paper introduces an aggregator module for visual localization that combines geometric and visual cues to improve discriminative power and robustness, eliminating reliance on noisy geometric constraints.


<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods rely solely on geometric cues, which limits their discriminative power and robustness, especially with noisy geometric constraints.

Method: The proposed aggregator module learns global descriptors consistent with both geometric structure and visual similarity, using a batch-mining strategy and modified contrastive loss for training without manual labels.

Result: Experiments demonstrate significant improvements in localization accuracy in large-scale environments while maintaining computational and memory efficiency.

Conclusion: The method effectively enhances visual localization by integrating visual and geometric cues, offering robustness and generalization across diverse environments.
...
TL;DR: The paper addresses 'visual forgetting' in MLLMs by proposing a curriculum-based framework to disentangle abstract reasoning and strategic visual perception.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs fail in long-chain visual reasoning due to premature entanglement of abstract reasoning and visual perception, leading to 'visual forgetting'.

Method: A two-stage approach: (1) disentangled SFT for abstract reasoning, followed by PG-CoT for vision grounding, and (2) reinforcement learning to teach the model when to look.

Result: The framework transforms MLLMs into strategic, grounded reasoners by addressing foundational and perception deficits.

Conclusion: The proposed curriculum-based framework effectively mitigates visual forgetting and enhances MLLMs' long-chain visual reasoning capabilities.
...
TL;DR: VideoDetective introduces an efficient question-aware memory mechanism for LVQA, reducing visual tokens while preserving critical information for MLLMs.


<details>
  <summary>Details</summary>
Motivation: Traditional LVQA methods struggle with memory and computation due to overwhelming context, missing key information.

Method: Iteratively processes video sub-segments with question-aware compression and recurrently updates memory tokens.

Result: Handles 100K tokens in minutes with 37GB GPU usage, outperforming benchmarks.

Conclusion: VideoDetective efficiently addresses LVQA challenges by focusing on critical clues and optimizing memory.
...
TL;DR: Mitty is a Diffusion Transformer for end-to-end Human2Robot video generation, using human demonstration videos directly without intermediate representations, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing methods that rely on intermediate representations (like keypoints or trajectories) causing information loss and errors, Mitty aims for scalable and generalizable robot learning directly from human videos.

Method: Mitty uses a pretrained video diffusion model, compressing demonstration videos into condition tokens and fusing them with robot denoising tokens via bidirectional attention during diffusion. An automatic synthesis pipeline generates human-robot pairs from egocentric datasets.

Result: Mitty achieves state-of-the-art performance on Human2Robot and EPIC-Kitchens, showing strong generalization to unseen environments.

Conclusion: Mitty provides scalable robot learning from human observations, eliminating the need for intermediate abstractions and offering new insights for the field.
...
TL;DR: AnyCXR is a framework for multi-organ segmentation in chest X-rays using synthetic data, achieving strong generalization without real-world annotations.


<details>
  <summary>Details</summary>
Motivation: The scarcity of annotated chest X-rays and variability in real-world conditions make robust anatomical segmentation challenging. AnyCXR aims to address this with synthetic supervision.

Method: The framework combines a Multi-stage Domain Randomization (MSDR) engine to generate diverse synthetic radiographs from 3D CT volumes and a Conditional Joint Annotation Regularization (CAR) strategy to leverage imperfect labels by enforcing anatomical consistency.

Result: AnyCXR achieves strong zero-shot generalization on real-world datasets, accurately segmenting 54 anatomical structures across various views and supporting downstream clinical tasks.

Conclusion: AnyCXR provides a scalable and annotation-efficient solution for anatomy-aware chest X-ray analysis, improving robustness across diverse imaging conditions.
...
TL;DR: The paper proposes WDFFU-Mamba, a novel segmentation network for breast ultrasound images, integrating wavelet-guided enhancement and dual-attention feature fusion to address segmentation challenges like noise and blurred boundaries.


<details>
  <summary>Details</summary>
Motivation: Breast ultrasound image segmentation is crucial for clinical diagnosis but faces challenges like noise and irregular lesion morphology, prompting the need for a robust and efficient model.

Method: The WDFFU-Mamba network combines wavelet-denoised high-frequency guidance (WHF module) and dual-attention feature fusion (DAFF module) within a U-shaped Mamba architecture for enhanced segmentation.

Result: Experiments on public BUS datasets show WDFFU-Mamba outperforms existing methods in Dice coefficient and HD95, achieving high accuracy and robustness.

Conclusion: WDFFU-Mamba offers strong segmentation performance, generalization ability, and computational efficiency, making it suitable for clinical breast tumor analysis.
...
TL;DR: RadImageNet-VQA is a large-scale dataset for radiologic VQA on CT and MRI exams, addressing limitations of existing datasets with 750K images and 7.5M QA pairs covering diverse tasks and pathologies.


<details>
  <summary>Details</summary>
Motivation: Existing medical VQA datasets lack scale, focus on X-rays or illustrations, and suffer from text shortcuts. RadImageNet-VQA aims to fill this gap with expert-curated data.

Method: The dataset includes 750K images paired with 7.5M QA samples, covering abnormality detection, anatomy recognition, and pathology identification across eight anatomical regions and 97 pathologies.

Result: State-of-the-art models struggle with fine-grained pathology identification, especially in open-ended settings. Text-only analysis shows performance drops without images, confirming no linguistic shortcuts.

Conclusion: RadImageNet-VQA provides a robust benchmark for advancing radiologic VQA, revealing current model limitations and ensuring meaningful evaluation.
...
TL;DR: The paper proposes VLMIR, a framework using Vision-Language Models (VLMs) like CLIP to improve image restoration by combining visual and linguistic knowledge for better semantic coherence and detail recovery.


<details>
  <summary>Details</summary>
Motivation: Previous methods struggle to leverage both visual and linguistic knowledge effectively in image restoration, missing semantic coherence.

Method: VLMIR uses VLMs for feature extraction (visual and linguistic representations) and integrates these via cross-attention in a diffusion-based restoration model.

Result: VLMIR outperforms existing methods in universal and degradation-specific image restoration tasks, highlighting the importance of integrated visual-linguistic knowledge.

Conclusion: VLMIR demonstrates that leveraging VLMs enhances image restoration by improving semantic understanding and visual perception, achieving superior results.
...
TL;DR: HiSIR-Net is a self-supervised framework for high-resolution PCBA defect inspection, combining SIR-Gate and ROPS to reduce false alarms and reconstruct anomalies cleanly, validated on SIPCBA-500 and public benchmarks.


<details>
  <summary>Details</summary>
Motivation: Challenges in PCBA defect inspection include insufficient labeled data and micro-defects in complex, high-resolution images, necessitating a robust solution.

Method: HiSIR-Net integrates SIR-Gate (selective reconstruction) and ROPS (coherent patch selection) for anomaly localization in 4K-resolution PCBAs.

Result: The framework achieves superior defect localization with low false positives and practical speed, validated on SIPCBA-500 and benchmarks.

Conclusion: HiSIR-Net effectively addresses PCBA inspection challenges with innovative modules and a contributed dataset, promising practical industrial applications.
...
TL;DR: ProCache is a training-free dynamic feature caching framework for Diffusion Transformers (DiTs), addressing non-uniform temporal dynamics and error accumulation, achieving significant speedup with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of DiTs limits real-time deployment, and existing feature caching methods are inefficient due to uniform caching intervals and error accumulation.

Method: ProCache introduces constraint-aware caching pattern search and selective computation to optimize caching schedules and reduce errors.

Result: Experiments show ProCache achieves up to 1.96x and 2.90x acceleration with negligible quality degradation, outperforming prior methods.

Conclusion: ProCache effectively balances speed and quality for DiTs, offering a practical solution for real-time deployment.
...
TL;DR: A generative framework for high-quality PBR textures on 3D meshes, leveraging pretrained models and fine-tuning with a material latent space (MatLat), outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Large-scale PBR texture datasets are rare, and existing methods struggle with distribution shifts when incorporating additional PBR channels.

Method: Fine-tunes a pretrained VAE to minimize latent distribution deviation, introduces locality-preserving regularization for latent-to-image mapping, and uses correspondence-aware attention.

Result: Improved PBR texture fidelity, with ablation studies confirming the importance of each framework component.

Conclusion: The framework achieves state-of-the-art performance by addressing prior limitations in embedding space and diffusion training.
...
TL;DR: EMAG is a training-free guidance method for diffusion transformers that adaptively modifies attention at inference to produce harder, semantically faithful negative samples, improving generation quality and human preference scores.


<details>
  <summary>Details</summary>
Motivation: Existing guidance techniques lack reliable control over negative sample granularity and difficulty, and often fix target-layer selection, limiting their effectiveness.

Method: Proposes Exponential Moving Average Guidance (EMAG), which modifies attention at inference time with an adaptive layer-selection rule, producing fine-grained degradations for better refinement.

Result: EMAG boosts human preference score by +0.46 over CFG and composes well with other advanced guidance techniques like APG and CADS.

Conclusion: EMAG offers a flexible and effective way to enhance guidance in diffusion models, improving sample quality and consistency without additional training.
...
TL;DR: DRIM improves Vision-Language Models by enabling reliable multi-turn reasoning and self-reflection, outperforming existing models on visual tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack self-correction during incorrect reasoning trajectories in visual tasks.

Method: DRIM uses a three-stage pipeline: data construction, cold-start SFT, and RL with redundancy-penalized policy optimization.

Result: DRIM achieves superior performance on visual understanding benchmarks.

Conclusion: DRIM effectively addresses limitations in VLMs by promoting reliable reasoning and self-reflection.
...
TL;DR: CodeDance uses executable code for flexible visual reasoning, outperforming text-only and schema-driven methods, and even closed models like GPT-4o.


<details>
  <summary>Details</summary>
Motivation: Existing open-source approaches lack flexibility and interpretability in visual reasoning due to reliance on text-only chains or rigid schemas.

Method: CodeDance employs executable code to orchestrate tools, compute results, and render visuals, guided by a reward for balanced tool-call.

Result: CodeDance shows emergent behaviors like novel tool invocations and cross-task transfer, outperforming benchmarks and advanced models.

Conclusion: Executable code offers a scalable, general mechanism for visual reasoning, surpassing traditional methods.
...
TL;DR: ADK enhances Vision-Language Models by using offline-generated descriptive prompts to enrich text representations, improving performance without added computational overhead.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing PEFT methods tied to fixed prompts and high computational costs by introducing ADK.

Method: Uses LLM-generated prompts offline, deploying them as averaged or dynamic instance-specific knowledge alongside handcrafted prompts.

Result: ADK boosts PEFT baselines, achieving state-of-the-art performance across domains without efficiency trade-offs.

Conclusion: ADK is a parameter-free, effective solution for enhancing VLMs in downstream tasks with distribution shifts.
...
TL;DR: RSHR-Bench is a high-resolution benchmark for remote sensing (RS) visual understanding and reasoning, addressing flaws in existing benchmarks where text-only LLMs perform competitively without images, revealing a mismatch in evaluation.


<details>
  <summary>Details</summary>
Motivation: Prior RS benchmarks often rely on low-resolution imagery or flawed reasoning-task designs, leading to unreliable evaluations of visual understanding. The authors aim to create a faithful assessment tool for super-high-resolution RS scenarios.

Method: The authors introduce RSHR-Bench, featuring 5,329 high-resolution RS images (up to 3 x 10^8 pixels per image) and design four task families: multiple-choice VQA, open-ended VQA, image captioning, and single-image evaluation. Tasks cover nine perception categories and four reasoning types. Adversarial filtering with LLMs and human verification reduce language priors.

Result: RSHR-Bench includes 3,864 VQA tasks, 3,913 image captioning tasks, and 500 fully human-verified VQA pairs. Evaluations show performance gaps in super-high-resolution scenarios across various VLMs.

Conclusion: RSHR-Bench provides a robust benchmark for evaluating super-high-resolution RS visual understanding and reasoning, addressing limitations of existing benchmarks and highlighting gaps in current VLM performance.
...
TL;DR: EMMA is a benchmark introduced to evaluate five dimensions of concept erasure techniques across 12 metrics, revealing gaps in existing methods' robustness and bias handling.


<details>
  <summary>Details</summary>
Motivation: Address limitations of current concept erasure techniques by testing their effectiveness under challenging conditions like indirect prompts and biases.

Method: Introduce EMMA, a benchmark evaluating five key dimensions (e.g., robustness, bias) over 12 metrics, applied to five concept erasure methods across diverse domains.

Result: Existing methods struggle with implicit prompts and visually similar non-target concepts, and some amplify gender and ethnicity bias.

Conclusion: Concept erasure techniques need improvement to handle complex scenarios and mitigate biases, highlighting EMMA's role in advancing evaluation standards.
...
TL;DR: A diverse dataset of color fundus images with detailed artery-vein segmentation annotations was created to support machine learning in ophthalmology.


<details>
  <summary>Details</summary>
Motivation: To develop a high-quality dataset for vascular analysis in ophthalmology, enabling robust machine learning model training and evaluation.

Method: Images were sampled from the Rotterdam Study, annotated with artery/vein labels using a custom interface, and connectivity was verified.

Result: The dataset includes PNG images in three modalities (original, contrast-enhanced, A/V masks) with varied quality and valuable vascular information.

Conclusion: This dataset provides heterogeneous CFIs with high-quality segmentations, useful for benchmarking and training models under real-world conditions.
...
TL;DR: DESSERT is a diffusion-based framework for event-driven single-frame synthesis using residual training, outperforming existing methods with sharper and more consistent results.


<details>
  <summary>Details</summary>
Motivation: Dynamic scenes in video frame prediction often suffer from errors due to lacking next-frame information. Event cameras capture high-temporal-resolution brightness changes, but existing methods introduce holes and blurring.

Method: DESSERT uses a pre-trained Stable Diffusion model trained on inter-frame residuals. It involves (1) an ER-VAE for event-to-residual alignment and (2) a diffusion model for residual denoising. DLT augmentation enhances robustness.

Result: The method outperforms existing approaches in event-based reconstruction, video frame prediction, and interpolation, yielding sharper and temporally consistent synthesis.

Conclusion: DESSERT effectively addresses limitations in event-based frame prediction by leveraging residual training and diffusion models, achieving superior performance.
...
TL;DR: The paper introduces Polysome for synthetic instruction generation, creates HISTAI-Instruct dataset, and trains ANTONI-α VLM, which outperforms MedGemma in WSI-level VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Address limitations of current VLMs in pathology, such as lack of reproducibility, static outputs, and scarce training data with detailed clinical reports.

Method: Introduces Polysome for synthetic instruction generation, applies it to create HISTAI-Instruct dataset, and trains ANTONI-α VLM.

Result: ANTONI-α outperforms MedGemma in tasks like tissue identification, neoplasm detection, and differential diagnosis.

Conclusion: The study provides a scalable, transparent solution for VLMs in pathology, with publicly available methods, data, and code.
...
TL;DR: SynergyWarpNet is an attention-guided cooperative warping framework for high-fidelity talking head synthesis, combining explicit warping, reference-augmented correction, and confidence-guided fusion.


<details>
  <summary>Details</summary>
Motivation: Traditional warping methods struggle with motion transfer and missing regions, while attention-based methods are complex and lack geometric grounding. SynergyWarpNet aims to address these limitations.

Method: The model refines animation in three stages: explicit warping for coarse alignment, reference-augmented correction for semantic completion, and confidence-guided fusion for balanced output.

Result: Comprehensive evaluations show state-of-the-art performance on benchmark datasets.

Conclusion: SynergyWarpNet effectively combines warping techniques to achieve high-fidelity talking head synthesis, surpassing existing methods.
...
TL;DR: The paper proposes a Multi-level Distortion-aware Deformable Network (MDDN) to improve OmniDirectional Image Super-Resolution (ODISR) by addressing geometric distortions in EquiRectangular Projection (ERP) images.


<details>
  <summary>Details</summary>
Motivation: Existing ODISR methods struggle with latitude-dependent geometric distortions in ERP images due to limited sampling ranges and feature extraction capabilities, prompting the need for a more effective solution.

Method: MDDN introduces a feature extractor with three parallel branches: a deformable attention mechanism and two dilated deformable convolutions (dilation rates of 2 and 3). It expands the sampling range and receptive field, using multi-level feature fusion and low-rank decomposition for efficiency.

Result: MDDN outperforms state-of-the-art methods on public datasets, demonstrating superior performance in capturing distorted patterns and enhancing visual quality.

Conclusion: The proposed MDDN effectively addresses geometric distortions in ERP images for ODISR, offering improved feature extraction and computational efficiency.
...
TL;DR: The paper introduces a pixel-level pre-processing step to disrupt semantic cues in AI-generated images, improving detector generalization across unseen generative models.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors struggle to generalize across different generative models because they overfit to source-specific semantic cues rather than learning universal generative artifacts.

Method: The authors propose a pixel-level mapping pre-processing step to disrupt the pixel value distribution of images, breaking fragile semantic patterns and forcing detectors to focus on generalizable high-frequency traces.

Result: Experiments on GAN and diffusion-based generators show the method significantly enhances cross-generator performance of state-of-the-art detectors.

Conclusion: The disruption of semantic cues is key to improving generalization in AI-generated image detection, as validated by extensive analysis.
...
TL;DR: InsertAnywhere is a new framework for realistic video object insertion (VOI) that addresses gaps in 4D scene understanding and handling of occlusion and lighting effects, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video generation methods struggle with realistic VOI due to limited 4D scene understanding and inadequate handling of occlusion and lighting effects.

Method: The framework uses a 4D-aware mask generation module for geometrically consistent object placement and a diffusion-based video generation model for appearance-faithful synthesis, trained on the ROSE++ dataset.

Result: The framework produces geometrically plausible and visually coherent object insertions, outperforming existing research and commercial models.

Conclusion: InsertAnywhere advances VOI by combining 4D scene understanding and diffusion models, achieving superior results in diverse real-world scenarios.
...
TL;DR: The paper introduces the Affective Image Filter (AIF) task to translate emotions from text to images, proposing AIF-B and AIF-D models. These models outperform existing methods in content consistency and emotional fidelity.


<details>
  <summary>Details</summary>
Motivation: To create emotionally compelling images by reflecting visually-abstract emotions from text into visually-concrete images.

Method: Proposes AIF-B (multi-modal transformer) and AIF-D (leveraging pre-trained diffusion models) for the AIF task.

Result: AIF models achieve superior performance in content consistency and emotional fidelity, validated by quantitative, qualitative, and user studies.

Conclusion: AIF models show significant promise in evoking specific emotions and have substantial potential for future applications.
...
TL;DR: Robust-R1 addresses performance issues of Multimodal Large Language Models (MLLMs) under real-world visual degradations with a novel framework involving supervised fine-tuning, reward-driven alignment, and dynamic reasoning depth scaling, trained on a specialized 11K dataset, achieving state-of-the-art robustness.


<details>
  <summary>Details</summary>
Motivation: Existing robust MLLMs lack interpretability and isolated optimization, struggling with extreme visual degradations. The goal is to improve practical robustness through structured reasoning.

Method: Robust-R1 integrates supervised fine-tuning, reward-driven alignment, and dynamic reasoning depth scaling, using a specialized dataset with realistic degradations and structured chains.

Result: Robust-R1 outperforms baselines on benchmarks (R-Bench, MMMB, MMStar, RealWorldQA), demonstrating superior robustness under multi-intensity degradations.

Conclusion: The proposed framework effectively enhances MLLM robustness under visual degradations, offering interpretability and practical performance improvements.
...
TL;DR: The paper proposes a faster alternative to the occlusion method for explaining deep neural networks in prostate cancer detection, reducing explanation time by at least 10x without compromising quality.


<details>
  <summary>Details</summary>
Motivation: To improve the practicality and adoption of AI-assisted cancer diagnosis by making neural network outputs more explainable and faster to compute.

Method: Identified suitable comparison criteria and metrics for evaluating explanation methods, then selected a faster alternative to occlusion.

Result: The chosen explanation method reduced computation time by at least 10x without affecting output quality.

Conclusion: The approach can be generalized to evaluate explanation methods in other applications, accelerating AI adoption in clinical settings.
...
TL;DR: ClothHMR is a novel method for 3D human mesh recovery that addresses challenges posed by diverse clothing, especially loose garments. It uses clothing tailoring and foundational model-based mesh recovery to achieve accurate results.


<details>
  <summary>Details</summary>
Motivation: Current 3D human mesh recovery methods perform poorly with diverse clothing, particularly loose garments. ClothHMR aims to overcome this limitation by leveraging clothing tailoring and human visual information from large foundational models.

Method: ClothHMR consists of two modules: Clothing Tailoring (CT), which ensures clothing fits the body silhouette, and FHVM-based Mesh Recovering (MR), which aligns intermediate mesh representations with foundational model inferences to optimize 3D mesh parameters.

Result: ClothHMR outperforms state-of-the-art methods on benchmark datasets and in-the-wild images, accurately estimating body shapes and poses under diverse clothing. A web application demonstrates its practical use in fashion and shopping.

Conclusion: ClothHMR effectively addresses the challenge of 3D human mesh recovery in diverse clothing, offering improved accuracy and real-world applicability.
...
TL;DR: AIFloodSense is a comprehensive aerial imagery dataset addressing scarcity in flood detection data, offering global diversity for tasks like classification, segmentation, and VQA to improve AI tools for climate resilience.


<details>
  <summary>Details</summary>
Motivation: Existing flood detection datasets are limited geographically and in annotation detail, hindering robust computer vision methods. AIFloodSense fills this gap with diverse, annotated imagery.

Method: Introduces AIFloodSense, a dataset with 470 high-resolution images from 230 flood events across 64 countries, supporting classification, segmentation, and VQA tasks.

Result: Baseline benchmarks using state-of-the-art architectures show the dataset's complexity and value for advancing domain-generalized AI tools.

Conclusion: AIFloodSense enhances flood detection and disaster response by providing a globally diverse, annotated dataset for robust AI development.
...
TL;DR: A unified FLAIR hyperintensity segmentation model using Attention U-Net was developed for brain tumors, achieving high Dice scores across various tumor types and time points, comparable to dataset-specific models.


<details>
  <summary>Details</summary>
Motivation: Automatic segmentation of FLAIR hyperintensity volumes aids in diagnosing and monitoring brain tumors, improving clinical workflows.

Method: An Attention U-Net architecture trained on ~5000 FLAIR images from diverse tumor types and centers, validated against BraTS and dataset-specific models.

Result: Achieved Dice scores: 88.65% (meningiomas), 80.08% (metastasis), 90.92% (pre-op gliomas), 84.60% (post-op gliomas), 84.47% (pre-op lower grade gliomas), and 61.27% (post-op lower grade gliomas).

Conclusion: The unified model generalizes well across tumor types and time points, enabling clinical deployment via Raidionics software.
...
TL;DR: The paper introduces MiMo-VL-Miloco-7B, a vision-language model specialized for smart-home environments, achieving strong performance in home-scenario understanding and general multimodal reasoning while outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To develop a specialized vision-language model for smart-home applications that balances performance in home-scenario tasks with general multimodal reasoning.

Method: A two-stage training pipeline combining supervised fine-tuning with reinforcement learning (Group Relative Policy Optimization), incorporating chain-of-thought supervision and token-budget-aware reasoning.

Result: MiMo-VL-Miloco-7B achieves leading F1 scores on gesture recognition and home-scenario tasks, with consistent gains on video and language benchmarks. It outperforms closed-source and open-source baselines.

Conclusion: Targeted home-scenario training enhances activity and gesture understanding while improving text-only reasoning, with minimal trade-offs on document tasks. The model and toolkit are open-sourced for research and deployment.
...
TL;DR: LangDriveCTRL is a framework for editing real-world driving videos using natural language, leveraging 3D scene decomposition and an agentic pipeline for fine-grained control. It outperforms previous methods in instruction alignment and realism.


<details>
  <summary>Details</summary>
Motivation: To enable intuitive and flexible editing of driving videos using natural language, addressing the need for diverse and realistic traffic scenario synthesis.

Method: Utilizes explicit 3D scene decomposition into a scene graph. An Orchestrator translates user instructions into execution graphs, coordinating specialized agents (Object Grounding, Behavior Editing, Behavior Reviewer) and tools for rendering and refinement.

Result: Achieves nearly 2x higher instruction alignment than prior state-of-the-art, with better structural preservation, photorealism, and traffic realism.

Conclusion: LangDriveCTRL effectively bridges natural language instructions with complex video editing tasks, offering a versatile and high-quality solution for synthesizing diverse driving scenarios.
...
TL;DR: The paper introduces MGRegBench, a public benchmark dataset for mammogram registration, featuring over 5,000 image pairs with manual annotations. It benchmarks diverse registration methods and aims to standardize evaluations in the field.


<details>
  <summary>Details</summary>
Motivation: The lack of public datasets and standardized benchmarks for mammography registration hinders progress in clinical applications like disease tracking. Existing studies are not comparable due to private data and inconsistent frameworks.

Method: MGRegBench was created with 5,000+ image pairs (100 with manual landmarks and masks). Various methods (ANTs, VoxelMorph, TransMorph, IDIR, MammoRegNet) were benchmarked, adapted, or re-implemented for fair comparison.

Result: A comprehensive evaluation of diverse registration methods was conducted, highlighting their performance on the MGRegBench dataset. This provides a standardized resource for future research.

Conclusion: MGRegBench addresses the gap in public datasets and benchmarks for mammography registration, offering a foundational resource for fair comparisons and advancing research in the field. All code and data are publicly available.
...
TL;DR: The paper introduces MULTIAQUA, a multimodal maritime dataset for robust scene interpretation in poor visibility, and demonstrates its effectiveness with nighttime tests and simplified training using daytime data.


<details>
  <summary>Details</summary>
Motivation: Unmanned surface vehicles face challenging visual conditions; existing solutions often lack multimodal data for poor visibility scenarios like nighttime.

Method: The authors create MULTIAQUA, a synchronized and annotated dataset with RGB, thermal, IR, and LIDAR data. They train multimodal methods using daytime images for robustness.

Result: The dataset enables robust deep neural network training, improving scene interpretation in challenging conditions like near-complete darkness.

Conclusion: MULTIAQUA simplifies data acquisition and enhances multimodal methods' reliability in poor visibility, benefiting unmanned surface vehicle operations.
...
### [62] [3D-RE-GEN: 3D Reconstruction of Indoor Scenes with a Generative Framework](https://arxiv.org/abs/2512.17459)
*Tobias Sautter,Jan-Niklas Dihlmann,Hendrik P. A. Lensch*

Main category: cs.CV

TL;DR: 3D-RE-GEN is a compositional framework for reconstructing textured 3D scenes from single images, addressing issues like incorrect object decomposition and missing backgrounds in current methods. It integrates state-of-the-art models for asset detection, reconstruction, and placement, achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: Current 3D scene generation methods hinder artists' workflows due to issues like incorrect object decomposition and missing backgrounds. The goal is to provide a modifiable and coherent textured 3D mesh scene suitable for visual effects and game development.

Method: 3D-RE-GEN integrates models for asset detection, reconstruction, and placement, treating occluded objects as an image editing task with generative models. It employs a 4-DoF differentiable optimization for physically realistic layouts and comprehensive background generation.

Result: The framework achieves state-of-the-art performance in single image 3D scene reconstruction, producing coherent and modifiable scenes with realistic lighting and geometry.

Conclusion: 3D-RE-GEN effectively addresses challenges in textured mesh scene reconstruction, providing a practical solution for artists in visual effects and game development by combining domain-specific models and novel optimization techniques.
...
### [63] [TwinSegNet: A Digital Twin-Enabled Federated Learning Framework for Brain Tumor Analysis](https://arxiv.org/abs/2512.17488)
*Almustapha A. Wakili,Adamu Hussaini,Abubakar A. Musa,Woosub Jung,Wei Yu*

Main category: cs.CV

TL;DR: TwinSegNet is a privacy-preserving federated learning framework combining ViT-UNet and digital twins for accurate brain tumor segmentation across diverse institutions.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods rely on centralized data, raising privacy concerns and limiting generalization. TwinSegNet addresses these issues.

Method: Integrates hybrid ViT-UNet with personalized digital twins, using convolutional encoders and Vision Transformer bottlenecks for context.

Result: Achieves high Dice scores (up to 0.90%) and sensitivity/specificity over 90% across nine MRI datasets, including BraTS.

Conclusion: TwinSegNet preserves privacy without performance loss, enabling scalable, personalized segmentation in multi-institutional settings.
...
### [64] [LumiCtrl : Learning Illuminant Prompts for Lighting Control in Personalized Text-to-Image Models](https://arxiv.org/abs/2512.17489)
*Muhammad Atif Butt,Kai Wang,Javier Vazquez-Corral,Joost Van De Weijer*

Main category: cs.CV

TL;DR: LumiCtrl introduces a method for precise illuminant control in text-to-image models, improving mood and aesthetic manipulation.


<details>
  <summary>Details</summary>
Motivation: Existing T2I models lack precise control over scene illuminants, which is crucial for designers.

Method: LumiCtrl uses physics-based augmentation, edge-guided prompt disentanglement, and masked reconstruction loss.

Result: The method outperforms baselines in illuminant fidelity, aesthetic quality, and scene coherence.

Conclusion: LumiCtrl is preferred by users and achieves superior results.
...
TL;DR: ST-Gaze, a video-based gaze estimation model combining CNN with attention modules, achieves state-of-the-art performance by capturing spatial and temporal dynamics.


<details>
  <summary>Details</summary>
Motivation: Current video-based gaze estimation methods struggle with balancing spatial and temporal feature representations, limiting performance.

Method: ST-Gaze uses CNN, channel attention, and self-attention modules to fuse eye and face features, treating them as spatial sequences for intra-frame context and inter-frame dynamics.

Result: ST-Gaze outperforms existing methods on the EVE dataset, with or without person-specific adaptation, and shows superiority in preserving spatial context.

Conclusion: ST-Gaze advances robust gaze estimation using common cameras by effectively modeling spatio-temporal relationships.
...
### [66] [MMLANDMARKS: a Cross-View Instance-Level Benchmark for Geo-Spatial Understanding](https://arxiv.org/abs/2512.17492)
*Oskar Kristoffersen,Alba R. Sánchez,Morten R. Hannemose,Anders B. Dahl,Dim P. Papadopoulos*

Main category: cs.CV

TL;DR: The paper introduces MMLANDMARKS, a multimodal geo-spatial dataset with four interconnected modalities for training and benchmarking models in various geo-spatial tasks.


<details>
  <summary>Details</summary>
Motivation: Current geo-spatial benchmarks lack comprehensive multimodal coverage, limiting progress. The authors address this gap by creating a unified dataset.

Method: The authors developed the MMLANDMARKS dataset, featuring 197k aerial images, 329k ground-view images, text, and coordinates for 18,557 U.S. landmarks, ensuring one-to-one modality correspondence.

Result: The dataset enables diverse geo-spatial tasks, such as cross-view retrieval and geolocalization. A simple CLIP-inspired baseline shows competitive performance against specialized models.

Conclusion: MMLANDMARKS highlights the necessity of multimodal datasets for advancing geo-spatial understanding and demonstrates its utility through benchmark results.
...
TL;DR: This paper explores diffusion models for image super-resolution, finding that conditioning step size is more impactful than diffusion step count, with optimal step sizes between 2.0 and 3.0.


<details>
  <summary>Details</summary>
Motivation: The study aims to identify key factors in conditioning methods (DPS and MCG) for improving super-resolution performance using pretrained diffusion models.

Method: An empirical ablation study is conducted on FFHQ super-resolution to analyze the effects of conditioning step size and diffusion step count.

Result: Conditioning step size significantly outperforms diffusion step count in impact, with step sizes of 2.0 to 3.0 yielding the best performance.

Conclusion: Optimizing conditioning step size is crucial for enhancing super-resolution results in diffusion models.
...
TL;DR: GroundingME is a benchmark assessing MLLMs' visual grounding abilities across four dimensions, revealing significant gaps and proposing improvement strategies.


<details>
  <summary>Details</summary>
Motivation: To evaluate if MLLMs genuinely ground language in vision or just pattern-match on simplified datasets, addressing limitations of current benchmarks.

Method: Introduces GroundingME, a benchmark with 1,005 examples challenging models on discriminative, spatial, limited, and rejection tasks.

Result: Best model achieves 45.1% accuracy; most fail rejection tasks (0%). Improvement strategies yield modest gains (2.9% and 27.9%).

Conclusion: GroundingME highlights MLLMs' limitations and guides progress toward human-level visual grounding.
...
TL;DR: AI models matched pathologist-level performance in prostate cancer diagnosis and grading in a Middle East cohort, with high cross-scanner concordance, supporting global AI adoption in pathology.


<details>
  <summary>Details</summary>
Motivation: To validate AI-based prostate cancer diagnosis in under-represented populations, specifically the Middle East, to ensure global AI adoption and equitable healthcare.

Method: Collected and digitized 339 prostate biopsy specimens from Iraq, evaluating AI models (task-specific and foundation models) for concordance with pathologists and consistency across three scanner types.

Result: AI-pathologist concordance was comparable to pathologist-pathologist concordance (Cohen's kappa 0.801 vs. 0.799). High cross-scanner consistency (kappa > 0.90) was observed, even with low-cost scanners.

Conclusion: AI models achieved pathologist-level performance, and compact scanners offer a cost-effective solution for AI adoption in low-resource settings. The dataset supports further research for globally equitable AI pathology.
...
TL;DR: FALCON-SFOD enhances object detection under domain shift by improving feature space alignment and pseudo-labeling robustness.


<details>
  <summary>Details</summary>
Motivation: Addressing domain shift in SFOD by strengthening object-focused representations rather than just refining pseudo-labels.

Method: Uses SPAR for spatial regularization and IRPL for noise-tolerant pseudo-labeling, both guided by foundation models.

Result: Achieves competitive performance on SFOD benchmarks.

Conclusion: FALCON-SFOD effectively mitigates domain shift issues by focusing on feature space enhancement and robust pseudo-labeling.
...
TL;DR: PathBench-MIL is an open-source AutoML framework for multiple instance learning in histopathology, automating pipeline construction and benchmarking MIL models.


<details>
  <summary>Details</summary>
Motivation: To address the need for automated, reproducible, and standardized tools for multiple instance learning (MIL) in histopathology.

Method: The framework automates end-to-end MIL pipeline construction (preprocessing, feature extraction, MIL-aggregation) and benchmarks various MIL models and feature extractors.

Result: PathBench-MIL provides visualization, unified configuration, and modular extensibility, facilitating rapid experimentation and standardization in histopathology tasks.

Conclusion: PathBench-MIL is a publicly available tool that enhances MIL research and application in histopathology through automation and benchmarking.
...
TL;DR: Pix2NPHM uses ViT to regress NPHM parameters from images, enabling high-fidelity facial reconstruction faster and more accurately than existing methods.


<details>
  <summary>Details</summary>
Motivation: Overcome challenges in fitting expressive Neural Parametric Head Models (NPHMs) to visual inputs for better facial geometry and expression accuracy.

Method: Employ domain-specific ViTs pretrained on geometric tasks, trained with 3D data (100K NPHM registrations) and 2D video datasets using pseudo ground truth.

Result: Achieves interactive frame-rate 3D reconstructions with improved geometric fidelity via optimization against estimated normals and point maps.

Conclusion: Pix2NPHM sets a new standard for high-quality, scalable face reconstruction from in-the-wild data.
...
TL;DR: AniX combines static world generation and controllable-entity models to enable user-specified characters to perform diverse actions in 3D environments, synthesizing coherent videos with high visual fidelity.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between static world generation and limited-action controllable-entity models by enabling open-ended actions for user-specified characters in realistic 3D environments.

Method: AniX leverages a 3DGS scene and a user-provided character, allowing natural language-directed actions. It formulates video generation as a conditional autoregressive problem and enhances motion dynamics via a pre-trained video generator.

Result: The method produces temporally coherent videos with high visual quality, character consistency, and controllability, evaluated across various aspects like long-horizon coherence.

Conclusion: AniX successfully extends controllable-entity models to support diverse, user-directed actions while maintaining realism and coherence, offering a significant advancement in interactive environment simulation.
...
### [74] [FLEG: Feed-Forward Language Embedded Gaussian Splatting from Any Views](https://arxiv.org/abs/2512.17541)
*Qijian Tian,Xin Tan,Jiayu Ying,Xuhong Wang,Yuan Xie,Lizhuang Ma*

Main category: cs.CV

TL;DR: FLEG is a feed-forward network that reconstructs language-embedded 3D Gaussians from arbitrary views without needing 3D annotations, using 2D-to-3D lifting and instance-guided contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reconstructing language-embedded 3D Gaussians are limited by fixed input views and lack of 3D training data. FLEG addresses these issues by eliminating the need for 3D annotations and leveraging large-scale 2D video data.

Method: The paper proposes a 3D-annotation-free training framework for 2D-to-3D lifting from uncalibrated multi-view images. It uses instance-guided contrastive learning to align 2D semantics with 3D representations and a geometry-semantic hierarchical sparsification strategy to reduce computational costs.

Result: FLEG efficiently reconstructs language-embedded 3D Gaussian representations from arbitrary views, producing accurate geometry, high-fidelity appearance, and language-aligned semantics, outperforming existing methods in experiments.

Conclusion: FLEG demonstrates the feasibility of training 3D reconstruction models without 3D annotations, leveraging 2D data and novel training strategies to achieve superior performance in language-embedded 3D Gaussian reconstruction.
...
TL;DR: InfSplign is a training-free inference-time method enhancing spatial alignment in text-to-image diffusion models by adjusting noise via a compound loss during denoising.


<details>
  <summary>Details</summary>
Motivation: Existing T2I models struggle with spatial relations due to inadequate training supervision and text embedding limitations.

Method: Adjusts noise using a compound loss leveraging cross-attention maps for precise object placement and balanced presence during sampling.

Result: Achieves state-of-the-art performance on VISOR and T2I-CompBench, surpassing inference-time and fine-tuning baselines.

Conclusion: InfSplign is lightweight, plug-and-play, and significantly improves spatial alignment without additional training.
...
### [76] [G3Splat: Geometrically Consistent Generalizable Gaussian Splatting](https://arxiv.org/abs/2512.17547)
*Mehdi Hosseinzadeh,Shin-Fang Chng,Yi Xu,Simon Lucey,Ian Reid,Ravi Garg*

Main category: cs.CV

TL;DR: The paper introduces G3Splat, a method for learning 3D Gaussian splats with geometric priors to address ambiguities in self-supervised learning, achieving state-of-the-art results in reconstruction, pose estimation, and novel-view synthesis.


<details>
  <summary>Details</summary>
Motivation: Prior works rely heavily on view-synthesis supervision for regressing 3D Gaussian parameters, which is insufficient for geometrically meaningful results. This motivates the need for geometric priors to ensure consistency.

Method: G3Splat enforces geometric priors to address the ambiguities in self-supervised learning of 3D Gaussian splats, improving geometric consistency in scene representations.

Result: The method achieves state-of-the-art performance in geometrically consistent reconstruction, relative pose estimation, and novel-view synthesis on RE10K, with strong zero-shot generalization on ScanNet.

Conclusion: G3Splat effectively resolves ambiguities in self-supervised learning of 3D Gaussian splats by incorporating geometric priors, leading to superior performance in geometry recovery and pose estimation compared to prior work.
...
TL;DR: CBAM-VGG16, an interpretable CNN for plant disease detection, integrates CBAM for improved feature extraction and localization. It achieves up to 98.87% accuracy and demonstrates strong generalization.


<details>
  <summary>Details</summary>
Motivation: Addressing the threat of plant diseases to global food security by developing accurate and interpretable disease detection methods.

Method: Introduces CBAM-VGG16, a CNN with Convolution Block Attention Module (CBAM) integrated at each convolutional stage, enhancing feature extraction and disease localization.

Result: Achieves high accuracy (up to 98.87%) and robust generalization across five diverse plant disease datasets, validated through interpretability tools like CBAM attention maps and Grad-CAM.

Conclusion: Advances explainable AI in agricultural diagnostics, providing a transparent and reliable system for smart farming.
...
### [78] [RoomEditor++: A Parameter-Sharing Diffusion Architecture for High-Fidelity Furniture Synthesis](https://arxiv.org/abs/2512.17573)
*Qilong Wang,Xiaofan Ming,Zhenyi Lin,Jinwen Li,Dongwei Ren,Wangmeng Zuo,Qinghua Hu*

Main category: cs.CV

TL;DR: This paper introduces RoomEditor++, a diffusion-based architecture for virtual furniture synthesis, and RoomBench++, a benchmark dataset, achieving superior performance in integrating furniture into indoor scenes.


<details>
  <summary>Details</summary>
Motivation: The field of virtual furniture synthesis lacks reproducible benchmarks and high-fidelity methods for seamless integration. This work aims to address these gaps.

Method: The authors propose RoomEditor++, a diffusion-based model with a parameter-sharing dual diffusion backbone compatible with U-Net and DiT architectures, unifying feature extraction and inpainting processes.

Result: RoomEditor++ outperforms state-of-the-art methods in quantitative metrics, qualitative assessments, and human preference studies, demonstrating strong generalization.

Conclusion: The study presents a robust solution for virtual furniture synthesis with a publicly available dataset and model, advancing the field for home design and e-commerce applications.
...
TL;DR: RadarGen is a diffusion model that generates realistic automotive radar point clouds from multi-view camera images, bridging gaps in multimodal generative simulation.


<details>
  <summary>Details</summary>
Motivation: The paper aims to synthesize realistic radar data from visual inputs, enhancing compatibility with existing datasets and simulation frameworks for improved perception models.

Method: RadarGen uses bird's-eye-view diffusion adapted to radar attributes, guided by BEV-aligned depth, semantic, and motion cues from foundation models, with a lightweight point cloud recovery step.

Result: RadarGen successfully captures radar measurement distributions and reduces performance gaps for perception models trained on real data.

Conclusion: RadarGen advances unified generative simulation across sensing modalities, offering scalability and improved realism.
...
TL;DR: The paper proposes a novel algorithm for video snapshot compressive imaging (SCI) using one-hot modulation, addressing temporal aliasing issues and enhancing reconstruction via diffusion and dual optical paths.


<details>
  <summary>Details</summary>
Motivation: Current video SCI methods using random binary modulation cause temporal aliasing. One-hot modulation offers perfect temporal decoupling but lacks supporting algorithms. The authors aim to bridge this gap.

Method: Transforms reconstruction into a generative video inpainting problem using stochastic differential equations (SDEs). Combines one-step regression initialization with diffusion refinement and employs dual optical paths for spatial enhancement.

Result: Experiments on synthetic and real datasets confirm the method's effectiveness in improving video SCI reconstruction.

Conclusion: The proposed algorithm successfully integrates diffusion into video SCI, leveraging one-hot modulation and dual optical paths to achieve superior reconstruction quality.
...
TL;DR: The paper evaluates the adversarial robustness of LLaVA-1.5-13B and Meta's Llama 3.2 Vision-8B-2 against untargeted PGD attacks on visual input, finding Llama 3.2 Vision more resilient despite lower baseline accuracy.


<details>
  <summary>Details</summary>
Motivation: To understand how adversarial attacks impact AI vision models, particularly open-weight VLMs like LLaVA and Llama 3.2 Vision.

Method: Adversarial robustness tested via untargeted PGD attacks on visual inputs, evaluated on a VQA v2 subset using standard accuracy metrics.

Result: Llama 3.2 Vision showed smaller performance drops under attack compared to LLaVA, especially at higher perturbation levels.

Conclusion: Vision is a viable attack vector for VLMs; adversarial robustness doesn't always align with benchmark performance and may depend on architectural/training factors.
...
TL;DR: The study evaluates fairness in biomedical image analysis challenges, revealing biases in dataset composition and issues with accessibility and reusability, which hinder clinical relevance.


<details>
  <summary>Details</summary>
Motivation: Benchmarking competitions are vital for AI progress in medical imaging, but their fairness and clinical relevance remain unclear.

Method: A systematic study of 241 biomedical image analysis challenges (458 tasks, 19 modalities) assessed dataset representativeness and FAIR compliance.

Result: Substantial biases (geographic, modality, problem type) and accessibility/reusability issues (restrictive licensing, incomplete documentation) were found.

Conclusion: Current benchmarks lack fairness and clinical relevance, revealing a gap between leaderboard success and real-world utility.
...
TL;DR: Re-Depth Anything improves DA-V2's depth estimation for real-world images using self-supervision and diffusion models, achieving better accuracy and realism.


<details>
  <summary>Details</summary>
Motivation: Addressing DA-V2's limitations in handling real-world images outside its training distribution by leveraging diffusion models and self-supervision.

Method: Label-free refinement via re-lighting depth maps and input augmentation, using SfS cues and SDS in a generative context. Freezes encoder, updates embeddings, and fine-tunes decoder to avoid optimization collapse.

Result: Significant improvements in depth accuracy and realism across diverse benchmarks.

Conclusion: Demonstrates new self-supervision avenues for enhancing geometric reasoning in depth estimation.
...
TL;DR: HeadHunt-VAD is a tuning-free Video Anomaly Detection (VAD) method that identifies anomaly-sensitive attention heads in Multimodal Large Language Models (MLLMs) for efficient, accurate detection without textual outputs.


<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods require labeled data and are computationally expensive, while recent MLLM-based methods suffer from information loss, normalcy bias, and prompt sensitivity.

Method: HeadHunt-VAD uses a Robust Head Identification module to select anomaly-sensitive attention heads from frozen MLLMs, feeding their features into lightweight anomaly scoring and temporal localization components.

Result: The method achieves state-of-the-art performance on two VAD benchmarks, demonstrating efficiency and accuracy without finetuning.

Conclusion: HeadHunt-VAD validates head-level probing in MLLMs as a powerful solution for real-world VAD, offering interpretable and efficient anomaly detection.
...
TL;DR: A semi-supervised learning method for multi-output CNN models improves segmentation accuracy in medical images, particularly for TBAD diagnosis, by leveraging unlabeled data alongside labeled data without probabilistic assumptions.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of aortic structures in TBAD diagnosis requires high-quality labeled data, which is costly and time-consuming to obtain. Semi-supervised learning offers a solution but is underexplored for multi-output models.

Method: The paper proposes a semi-supervised learning method based on additional rotations and flipping, avoiding probabilistic assumptions, making it suitable for multi-output CNN architectures.

Result: The method enhances segmentation precision for TBAD by effectively utilizing both labeled and unlabeled data, addressing the challenge of limited labeled datasets.

Conclusion: The proposed universal semi-supervised approach improves segmentation accuracy in multi-output CNNs, proving beneficial for medical image analysis, especially in TBAD diagnosis.
...
TL;DR: A physics-informed, self-supervised framework is proposed for super-resolution in qMRI, using routine HR wMRI scans as guidance, eliminating the need for HR qMRI ground truth during training.


<details>
  <summary>Details</summary>
Motivation: HR qMRI relaxometry is clinically underutilized due to long acquisition times. The goal is to enable fast qMRI by leveraging routine clinical wMRI scans.

Method: The framework formulates super-resolution as Bayesian inference, minimizing discrepancies between synthesized HR images from qMRI maps and acquired wMRI guides, and between LR qMRI and downsampled predictions. A deep neural network learns the super-resolution mapping using synthesized training data.

Result: Ablation experiments show T1-weighted images enhance T1 maps, T2-weighted images improve T2 maps, and combined guidance optimizes all parameters. The model achieves comparable quality from a 1-minute acquisition to a 5-minute reference scan.

Conclusion: The framework decouples training from HR qMRI, enabling fast qMRI enhanced via routine clinical images, offering a practical pathway for integrating quantitative relaxometry into clinical workflows with minimal additional scan time.
...
TL;DR: VLMs' performance on visual prompts is highly sensitive to minor details like marker color or size, leading to inconsistent leaderboard rankings. VPBench is introduced to address this instability.


<details>
  <summary>Details</summary>
Motivation: Understanding how minor visual details impact VLM performance and rankings, highlighting flaws in current evaluation benchmarks.

Method: Evaluated nine VLMs on visually prompted tasks, testing sensitivity to marker design, dataset size, and low-level inference choices like JPEG compression.

Result: Found significant performance variance due to visual marker details, with weaker models outperforming stronger ones under specific conditions.

Conclusion: Proposed VPBench to standardize evaluations, reducing instability caused by minor visual changes in benchmarks.
...
TL;DR: StereoMV2D improves multi-view 3D object detection by integrating temporal stereo modeling to refine query priors, enhancing depth perception without significant computational cost.


<details>
  <summary>Details</summary>
Motivation: Achieving a balance between accuracy and efficiency in multi-view 3D object detection is challenging, especially due to depth ambiguity in single-frame 2D detections.

Method: StereoMV2D integrates temporal stereo modeling into 2D detection-guided queries, leveraging cross-temporal disparities to refine depth perception and using a dynamic confidence gating mechanism.

Result: Experiments on nuScenes and Argoverse 2 datasets show StereoMV2D achieves superior detection performance with minimal computational overhead.

Conclusion: StereoMV2D effectively addresses depth ambiguity and improves 3D detection accuracy efficiently.
...
TL;DR: PathFLIP is a novel framework for fine-grained vision-language alignment in pathology, improving multimodal understanding of Whole Slide Images (WSIs) by decomposing slide-level captions into region-level subcaptions and leveraging LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with fine-grained alignment between text and visual cues in WSIs due to their gigapixel scale and spatial heterogeneity, limiting performance.

Method: PathFLIP decomposes slide captions into subcaptions, generates text-conditioned embeddings, and leverages LLMs for diverse clinical tasks.

Result: PathFLIP outperforms existing pathological VLMs on four benchmarks with less training data.

Conclusion: PathFLIP enables fine-grained, instruction-aware WSI interpretation, advancing clinical pathology applications.
...
TL;DR: GRASP-HO reformulates HOI detection as an open-vocabulary generation problem using a hybrid framework combining visual evidence and frozen MLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing HOI detectors struggle with unseen interactions due to closed-world assumptions. MLLMs offer rich world knowledge but are computationally prohibitive to fine-tune.

Method: Extracts hybrid interaction representations and uses a CSC module to inject visual evidence into a frozen MLLM. Combines language modeling and auxiliary classification losses.

Result: Achieves state-of-the-art closed-set performance and strong zero-shot generalization.

Conclusion: GRASP-HO successfully bridges discriminative perception and generative reasoning for open-world HOI detection.
...
TL;DR: ReCo introduces a novel video editing paradigm with constraint modeling between editing and non-editing regions, using latent and attention regularization to improve accuracy and reduce interference.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of inaccurate editing regions and token interference in instructional video editing, the paper proposes ReCo to enhance in-context generation.

Method: ReCo concatenates source and target videos for joint denoising, using latent and attention regularization to calibrate the process and improve editing precision.

Result: Experiments on four major video editing tasks show the superiority of ReCo, supported by a new dataset (ReCo-Data) with 500K instruction-video pairs.

Conclusion: ReCo effectively improves instructional video editing by modeling constraints between editing and non-editing regions, demonstrating strong performance and data efficiency.
...
TL;DR: Bitbox is an open-source toolkit aimed at simplifying computational behavioral analysis for researchers by offering reproducible, modular, and interpretable tools.


<details>
  <summary>Details</summary>
Motivation: Existing AI-based behavioral measurement tools are often inaccessible to non-engineers, hindering mainstream adoption in psychology, psychiatry, and mental health research.

Method: Bitbox provides standardized interfaces for extracting high-level behavioral metrics from video, utilizing validated face, head, and body processors.

Result: The toolkit bridges the gap between engineering and behavioral science, enabling researchers to use advanced metrics without technical expertise.

Conclusion: Bitbox accelerates the integration of computational behavioral analysis into research, fostering collaboration between method developers and domain scientists.
...
TL;DR: FlexAvatar is a transformer-based model for creating high-fidelity 3D head avatars from single or sparse images, without needing camera poses or expression labels. It uses structured head query tokens and a lightweight UNet decoder for dynamic deformations, and includes strategies for rare expressions and identity refinement.


<details>
  <summary>Details</summary>
Motivation: The paper aims to simplify and improve the creation of detailed 3D head avatars by eliminating the need for camera poses or expression labels while maintaining high fidelity and dynamic realism.

Method: FlexAvatar combines a transformer-based reconstruction model with structured head query tokens and a lightweight UNet decoder for dynamic deformations. It also uses data distribution adjustment for rare expressions and a refinement step for identity-specific details.

Result: FlexAvatar achieves superior 3D consistency and dynamic realism compared to previous methods, enabling practical animatable 3D avatar creation.

Conclusion: FlexAvatar provides a robust solution for creating high-fidelity, dynamically detailed 3D avatars from sparse input images, outperforming existing methods.
...
TL;DR: SAVeD is a large-scale video dataset from social media, capturing ADAS vehicle crashes and near-miss events, with detailed annotations for risk analysis and improved model performance.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack authentic ADAS vehicle behavior under risk conditions, necessitating a dataset like SAVeD for safety-critical research.

Method: SAVeD curates 2,119 first-person videos with frame-level annotations, integrating semantic segmentation and depth estimation for TTC, and uses GEV for risk modeling.

Result: The dataset enhances model performance through domain adaptation, with benchmarks showing improved handling of complex scenarios by VLLMs.

Conclusion: SAVeD addresses the gap in ADAS risk behavior datasets, enabling better analysis of perception and decision-making failures for safer autonomous systems.
...
TL;DR: MambaMIL+ is a MIL framework integrating spatial context and long-range dependency modeling for WSI analysis, addressing Mamba's limitations with overlapping scanning, S2PE, and CTS mechanisms.


<details>
  <summary>Details</summary>
Motivation: Conventional deep learning struggles with gigapixel WSIs due to resolution and annotation challenges. MIL helps but lacks spatial context modeling and suffers from memory decay in long sequences.

Method: Proposes MambaMIL+ with: 1) overlapping scanning for spatial continuity, 2) S2PE for bias-free positional encoding, and 3) CTS for dynamic contextual memory.

Result: Achieves state-of-the-art performance on 20 benchmarks in diagnostic classification, molecular prediction, and survival analysis across multiple feature extractors.

Conclusion: MambaMIL+ effectively integrates spatial context and long-range dependency modeling, proving robust and scalable for computational pathology.
...
TL;DR: The paper proposes leveraging CLIP for deepfake detection, introduces a diffusion-generated dataset (Diff-Gen), and presents AdaptPrompt for efficient transfer learning, achieving state-of-the-art generalization across diverse synthetic content.


<details>
  <summary>Details</summary>
Motivation: The rise of realistic synthetic media challenges reliable deepfake detection, especially generalization across unseen models. This work addresses the need for robust, generalizable detection methods.

Method: 1) Created Diff-Gen, a large-scale dataset of diffusion-generated fakes to capture broad spectral artifacts. 2) Proposed AdaptPrompt, a parameter-efficient framework combining task-specific prompts and visual adapters with a frozen CLIP backbone. Pruning the vision encoder's final transformer block improved artifact retention.

Result: Evaluated on 25 test sets (GANs, diffusion models, commercial tools), the method achieved state-of-the-art performance in standard and cross-domain scenarios. It also showed versatility in few-shot generalization and source attribution.

Conclusion: The work advances deepfake detection by improving generalization through novel datasets and efficient adaptation of vision-language models, setting a new benchmark for detecting diverse synthetic content.
...
TL;DR: LiteGE introduces a lightweight method for computing geodesic distances on 3D surfaces using PCA-based descriptors, achieving significant efficiency gains without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Current learning-based methods for geodesic distance computation are resource-intensive, limiting their use in interactive or constrained settings. LiteGE aims to address this by reducing memory usage and latency.

Method: LiteGe constructs compact shape descriptors using PCA on unsigned distance field samples, eliminating the need for high-capacity networks and supporting sparse inputs.

Result: Experiments show LiteGe reduces memory usage and inference time by up to 300x, achieves 1000x speedup over state-of-the-art methods, and maintains accuracy on non-isometric shape pairs.

Conclusion: LiteGE offers an efficient, robust, and lightweight solution for geodesic distance computation and shape matching, suitable for resource-constrained environments.
...
TL;DR: UrbanDIFF is a spatial denoising diffusion model that reconstructs cloud-contaminated urban LST imagery using static urban structure data, outperforming traditional methods under dense cloud occlusion.


<details>
  <summary>Details</summary>
Motivation: Cloud contamination limits continuous SUHI monitoring, and existing LST reconstruction methods often rely on unreliable auxiliary data. UrbanDIFF aims to address this gap using purely spatial gap-filling.

Method: UrbanDIFF is conditioned on urban structure data (built-up surfaces, elevation model) and refines results with supervised pixel guidance during inference.

Result: Evaluated on MODIS Terra LST data, UrbanDIFF achieved SSIM 0.89, RMSE 1.2 K, and R2 0.84 at 85% cloud coverage, degrading slower than baselines.

Conclusion: UrbanDIFF offers a robust solution for reconstructing LST imagery under dense cloud occlusion, enhancing SUHI monitoring.
...
TL;DR: A hybrid approach combining extended conventional distortion models with neural network-based residual correction improves long-range 3D object localization up to 5 km.


<details>
  <summary>Details</summary>
Motivation: Existing stereo-camera methods fail beyond hundreds of meters due to inadequate lens distortion models. Accurate long-range localization is crucial for photogrammetry.

Method: Extend conventional distortion models with higher-order terms, then enhance them using a neural network-based residual correction.

Result: The hybrid approach achieves robust 3D localization up to 5 km, validated experimentally and visualized on GIS maps.

Conclusion: The framework effectively calibrates CCTV cameras for long-range photogrammetry, outperforming traditional methods.
...
TL;DR: Chorus is a multi-teacher pretraining framework that enhances 3D Gaussian Splatting (3DGS) scene representation by distilling signals from 2D foundation models. It demonstrates strong performance on various tasks and transfers well to point cloud benchmarks.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the lack of rich, general-purpose features in 3DGS scene representations by leveraging 2D foundation models to enhance the 3D scene encoder.

Method: Chorus uses a shared 3D encoder and teacher-specific projectors to distill complementary signals from language-aligned, generalist, and object-aware 2D teachers, creating a shared embedding space that captures high-level semantics and fine-grained structure.

Result: Chorus performs well on tasks like open-vocabulary semantic and instance segmentation, linear and decoder probing, and data-efficient supervision. It also transfers effectively to point cloud benchmarks, outperforming baselines with fewer training scenes.

Conclusion: The Chorus framework successfully enhances 3DGS with rich features from 2D models, demonstrating versatility and efficiency across tasks and benchmarks, with potential applications in out-of-domain finetuning.
...
TL;DR: ReX-MLE is a benchmark for autonomous coding agents in medical imaging, revealing severe performance gaps in domain-specific tasks compared to human experts.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based coding agents struggle with complex, domain-specific tasks like medical imaging, which require specialized knowledge and workflows not covered by existing benchmarks.

Method: ReX-MLE introduces 20 challenges from medical imaging competitions, assessing agents on end-to-end workflows including data preprocessing, model training, and submissions under realistic constraints.

Result: State-of-the-art agents performed poorly, with most submissions ranking in the 0th percentile compared to human experts, highlighting domain-knowledge and engineering limitations.

Conclusion: ReX-MLE identifies key bottlenecks and serves as a foundation for developing domain-aware autonomous AI systems.
...
TL;DR: A simulation-driven denoising framework combines statistical noise modeling and deep learning to enhance Raman spectra in biomedical diagnostics, addressing weak signals and fluorescence interference.


<details>
  <summary>Details</summary>
Motivation: Raman spectroscopy's potential in biomedical diagnostics is limited by weak signals and strong fluorescence background, degrading signal quality.

Method: The study develops a simulation-driven framework with a noise model and deep learning, training a cascaded neural network to suppress noise and fluorescence interference.

Result: The framework successfully improved spectral quality, validated using simulated human skin spectra derived from real data.

Conclusion: Physics-informed learning enhances Raman spectral quality, enabling faster and more accurate tissue analysis.
...
TL;DR: InSPECT is a novel diffusion model that preserves invariant spectral features during diffusion processes, improving visual diversity, convergence speed, and computational efficiency compared to traditional models like DDPM.


<details>
  <summary>Details</summary>
Motivation: Traditional diffusion models diffuse all data to white noise, making reconstruction computationally difficult. InSPECT aims to preserve invariant spectral features to address this limitation.

Method: InSPECT modifies the forward and backward processes to smoothly converge Fourier coefficients to random noise while preserving features.

Result: Experiments on CIFAR-10, Celeb-A, and LSUN show a 39.23% FID reduction and 45.80% IS improvement over DDPM, with faster convergence and better diversity.

Conclusion: Preserving invariant spectral features enhances diffusion model performance, offering superior generation quality, diversity, and computational efficiency.
...
TL;DR: A new method called Keypoint Counting Classifiers (KCCs) transforms Vision Transformer (ViT)-based models into self-explainable models (SEMs) without retraining, improving transparency and reliability.


<details>
  <summary>Details</summary>
Motivation: Existing SEMs require complex training and architectures, making them impractical, especially with ViT-based foundation models. New methods are needed for transparency.

Method: KCCs leverage ViTs' ability to identify keypoints between images, creating an interpretable decision process visually represented in input.

Result: Extensive evaluation shows KCCs enhance human-machine communication compared to recent baselines.

Conclusion: KCCs advance transparency and reliability in ViT-based foundation models.
...
TL;DR: MAGNet is a unified autoregressive diffusion framework for multi-agent motion generation, solving challenges like long temporal horizons and inter-agent dependencies. It supports various tasks and scales well with group sizes.


<details>
  <summary>Details</summary>
Motivation: Addressing the difficulty of modeling multi-person interactions due to complex dependencies and variable group sizes, which existing task-specific methods fail to generalize.

Method: Introduces MAGNet, combining Diffusion Forcing with autoregressive denoising to explicitly model inter-agent coupling, enabling flexible conditioning and sampling for tasks like dyadic prediction and multi-agent generation.

Result: MAGNet matches specialized methods in dyadic benchmarks and extends to polyadic scenarios, generating coherent ultra-long sequences for synchronized and social interactions.

Conclusion: MAGNet provides a scalable, unified solution for multi-agent motion generation, capturing diverse interactions effectively.
...
TL;DR: Dexterous World Model (DWM) is a video diffusion framework that generates dynamic human-scene interactions in static 3D scenes, leveraging scene-action conditioning for realistic outcomes.


<details>
  <summary>Details</summary>
Motivation: Current digital twins are static and lack embodied interactivity. DWM aims to bridge this gap by modeling how human actions dynamically alter 3D scenes.

Method: DWM uses a scene-action-conditioned video diffusion framework, combining static scene renderings and egocentric hand motion sequences to generate coherent interaction videos. It trains on a hybrid dataset of synthetic and real-world videos.

Result: DWM produces realistic and physically plausible interactions (e.g., grasping, opening objects) while maintaining spatial and camera consistency.

Conclusion: DWM is a foundational step toward interactive digital twins and enables embodied simulation from egocentric actions, advancing beyond static representations.
...
TL;DR: The paper proposes a framework to adapt understanding-oriented encoder features for generative tasks by introducing a semantic-pixel reconstruction objective, achieving compact, semantically rich representations for improved image reconstruction and generation.


<details>
  <summary>Details</summary>
Motivation: To unify vision generation and understanding by addressing the lack of compact regularization in discriminative feature spaces and weak pixel-level reconstruction in current LDMs.

Method: Introducing a semantic-pixel reconstruction objective to regularize the latent space, compressing semantic and fine-grained details into a compact representation (96 channels, 16x16 downsampling).

Result: Achieves state-of-the-art image reconstruction, faster convergence, and significant performance gains in Text-to-Image (T2I) and image editing tasks.

Conclusion: Representation encoders can be effectively adapted for robust generative tasks, validating the proposed framework's effectiveness in unifying vision generation and understanding.
...
TL;DR: The paper introduces the Women's Health Benchmark (WHB) to evaluate LLMs' accuracy in women's health, revealing significant failures (~60%) across specialties and error types.


<details>
  <summary>Details</summary>
Motivation: Current LLMs are widely used for health information but lack rigorous assessment in women's health, necessitating a specialized benchmark.

Method: WHB includes 96 validated model stumps across five specialties, three query types, and eight error types, tested on 13 state-of-the-art LLMs.

Result: Models showed high failure rates (~60%), with notable struggles in 'missed urgency' indicators. GPT-5 showed improvement in avoiding inappropriate recommendations.

Conclusion: AI chatbots are currently unreliable for women's health advice, highlighting the need for further improvement.
...
TL;DR: The paper introduces Struct-SQL, a framework using structured reasoning to improve Text-to-SQL task performance in small language models (SLMs), achieving an 8.1% boost over unstructured methods.


<details>
  <summary>Details</summary>
Motivation: Current Text-to-SQL systems struggle with balancing cost, security, and performance, often forcing enterprises to choose between expensive proprietary models or low-performing SLMs. The authors hypothesize that structured reasoning can provide clearer guidance for SLM training.

Method: The authors propose Struct-SQL, a Knowledge Distillation framework using query execution plans as formal blueprints for structured reasoning, contrasting with unstructured Chain-of-Thought traces.

Result: Struct-SQL achieves an 8.1% absolute improvement over unstructured baselines, with reduced syntactic errors identified as a key factor.

Conclusion: Structured reasoning is more effective for reliable SQL generation in SLMs, highlighting its potential for enterprise deployment.
...
TL;DR: The paper introduces the XLM Python package to simplify the implementation and comparison of non-autoregressive language models, offering pre-trained models for research use.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized tools and reusable components for non-autoregressive language modeling hinders systematic comparison and development.

Method: The authors present the XLM Python package, designed for faster implementation of non-autoregressive models, along with pre-trained models in the xlm-models package.

Result: The XLM package facilitates easier development and comparison of non-autoregressive models, with publicly available code.

Conclusion: The XLM package addresses the challenges in non-autoregressive language modeling by providing a standardized and reusable framework.
...
TL;DR: SPECTRA is a watermarking approach for detecting LLM training data, effective even with minimal inclusion (0.001%), using paraphrasing and scoring models to ensure detectability without distribution shifts.


<details>
  <summary>Details</summary>
Motivation: To enforce copyright and data licensing by reliably detecting if LLMs were trained on specific data, even when such data is a tiny fraction of the corpus.

Method: SPECTRA involves paraphrasing text via an LLM, scoring paraphrases with a separate model, and selecting paraphrases that match the original text's score to avoid distribution shifts. Detection compares token probabilities between the suspect model and the scoring model.

Result: SPECTRA achieves a p-value gap over nine orders of magnitude, outperforming all baselines, effectively distinguishing between training and non-training data.

Conclusion: SPECTRA provides a scalable, deploy-before-release watermarking solution for detecting LLM training data, robust even in large-scale training scenarios.
...
TL;DR: The paper critiques current evaluation metrics for dialogue topic segmentation, proposing new criteria like boundary density and segment coherence alongside window-tolerant F1, and argues that reported improvements often stem from evaluation artifacts.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics fail to address the nuanced needs of modern LLM-based conversational systems, leading to misleading performance assessments due to annotation mismatches and sparse labels.

Method: The study evaluates multiple segmentation strategies across eight diverse dialogue datasets, introducing boundary density, segment coherence, and W-F1 as primary criteria.

Result: Findings reveal that performance differences are often artifacts of annotation granularity mismatches, not model quality, and highlight the oversegmentation issue relative to sparse labels.

Conclusion: Dialogue topic segmentation should focus on selecting appropriate granularity rather than predicting exact boundaries, necessitating separate scoring and selection metrics.
...
TL;DR: The paper explores using synthetic and real data augmentation to improve conversational agent performance in smoking cessation online support groups, achieving a 32% F1 score improvement.


<details>
  <summary>Details</summary>
Motivation: Low user engagement and stigma in smoking cessation support groups prompted the need for an automatic conversational agent, hindered by insufficient high-quality data.

Method: A two-level data augmentation strategy was employed: synthetic posts generated via GPT and real posts scraped from related contexts, both validated by human annotators.

Result: The augmented dataset improved the intent classifier's F1 score by 32%, with synthetic and real data showing comparable benefits.

Conclusion: The study offers a replicable framework for enhancing conversational agents in data-scarce domains.
...
TL;DR: A highlight-guided summarization method improves faithfulness and traceability by using sentence-level info as a content plan. It outperforms on long documents, boosting ROUGE-L by 4.1 points and SummaC by 35%.


<details>
  <summary>Details</summary>
Motivation: To enhance the traceability and faithfulness of summaries for long, information-dense documents by leveraging sentence-level plans.

Method: Uses self-planning to identify important content and generates summaries conditioned on the plan. Compares end-to-end and two-stage variants, with the latter performing better.

Result: Achieves 4.1-point ROUGE-L improvement and ~35% SummaC gains on GovReport. Preserves relevance and quality while enhancing factual consistency.

Conclusion: Highlight-guided summarization improves accuracy and detail retention, making it effective for long and dense documents.
...
TL;DR: MiA-RAG enhances LLM-based RAG systems with global context awareness via hierarchical summarization, improving long-context tasks and aligning local details with a coherent global representation.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems struggle with long-context tasks due to lacking holistic semantic representation, unlike humans' Mindscape-Aware Capability.

Method: MiA-RAG builds a mindscape using hierarchical summarization, conditioning retrieval and generation on this global semantic representation.

Result: MiA-RAG outperforms baselines across long-context and bilingual benchmarks, demonstrating improved evidence-based understanding and global sense-making.

Conclusion: MiA-RAG successfully integrates global context awareness into RAG systems, enabling human-like long-context retrieval and reasoning.
...
TL;DR: A noise-sensitive ASR error correction framework combining multiple hypotheses and noise-aware modeling improves Persian speech recognition in noisy environments, reducing WER significantly.


<details>
  <summary>Details</summary>
Motivation: ASR systems, including state-of-the-art models like Whisper, degrade in noisy environments, especially for low-resource languages like Persian.

Method: The study introduces Error Level Noise (ELN) to quantify noise-induced uncertainty, evaluates three models (base LLaMA-2-7B, fine-tuned text-only, and ELN-conditioned), and integrates ELN embeddings for correction.

Result: The ELN-conditioned model reduces WER from 31.10% (baseline) to 24.84%, outperforming the fine-tuned text-only model (30.79%) and base LLaMA-2-7B (64.58%).

Conclusion: Combining multiple hypotheses with noise-aware embeddings effectively improves Persian ASR robustness in noisy scenarios.
...
TL;DR: Seed-Prover 1.5 is a formal theorem-proving model trained via large-scale agentic reinforcement learning, achieving high performance on undergraduate to PhD-level problems with efficient compute usage.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of using LLMs for formal theorem proving in languages like Lean, which remains computationally expensive and difficult, especially for higher-level problems.

Method: The authors train Seed-Prover 1.5 via large-scale agentic reinforcement learning and introduce an efficient test-time scaling (TTS) workflow to bridge natural and formal languages.

Result: Seed-Prover 1.5 solves 88% of PutnamBench, 80% of Fate-H, and 33% of Fate-X problems, and successfully tackles 11 out of 12 Putnam 2025 problems within 9 hours.

Conclusion: Scaling learning from experience with high-quality formal feedback shows promise for advancing formal mathematical reasoning.
...
TL;DR: AutoMetrics is a framework for synthesizing evaluation metrics under low-data constraints, improving correlation with human ratings by up to 33.4% over traditional methods.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in evaluating AI applications in open-ended domains where user feedback or behavioral signals are scarce or slow.

Method: AutoMetrics combines retrieval from MetricBank (48 curated metrics) with LLM-as-a-Judge criteria, composed via regression to maximize human signal correlation.

Result: Across 5 tasks, AutoMetrics improves correlation with human ratings by up to 33.4% and works with fewer than 100 feedback points.

Conclusion: AutoMetrics offers an interpretable, low-data alternative for evaluating LLM applications, released as a toolkit to accelerate adaptive evaluation.
...
TL;DR: The paper discusses NLP's revolutionary impact, focusing on automating subjective question generation and answer evaluation to aid educators and students.


<details>
  <summary>Details</summary>
Motivation: Current NLP models excel in objective tasks but lag in subjective question generation and evaluation, prompting this research to bridge the gap.

Method: Improve or develop novel NLP models tailored for generating subjective questions and evaluating answers from text.

Result: Expected outcome includes enhanced tools for teachers and students, facilitating better assessment and self-learning.

Conclusion: Automating subjective question generation and evaluation can revolutionize education by improving assessment efficiency and learning experiences.
...
TL;DR: A hybrid fine-tuning framework for multilingual LLMs combines gradient-aligned updates and orthogonal transformations, achieving accuracy and calibration gains with minimal compute overhead.


<details>
  <summary>Details</summary>
Motivation: To enable efficient multilingual adaptation of large language models for low-resource settings while maintaining accuracy and calibration.

Method: Uses gradient-aligned low-rank updates, structured orthogonal transformations, and unitary constraints, paired with label-free data governance steps like language ID and quality filtering.

Result: Consistent improvements over PEFT baselines in accuracy, calibration, and resilience to orthographic variants, with modest training overhead.

Conclusion: Hybrid and unitary PEFT with data governance offers a stable, resource-efficient solution for multilingual adaptation.
...
TL;DR: Stakeholder Suite is a framework for mapping actors, topics, and arguments in public debates, tested on energy projects, achieving high relevance and operational effectiveness.


<details>
  <summary>Details</summary>
Motivation: To address the complexity of public debates and improve transparency in media intelligence for infrastructure and energy projects.

Method: Combines actor detection, topic modeling, argument extraction, and stance classification in a unified pipeline.

Result: Achieves 75% argument relevance in pilot use cases, strong retrieval precision, and stance accuracy.

Conclusion: The framework is effective for operational use, aiding in visualizing influence networks and supporting evidence-based decisions.
...
TL;DR: The paper introduces CANON LAYERS, lightweight architectural components for language models, to enhance horizontal information flow and isolate core capabilities using synthetic pretraining tasks.


<details>
  <summary>Details</summary>
Motivation: To address challenges in understanding architectural differences in language models due to noise and randomness, especially at academic-scale pretraining.

Method: Introduces controlled synthetic pretraining tasks and CANON LAYERS, which compute weighted sums of nearby token representations, compatible with various sequence architectures.

Result: CANON LAYERS improve reasoning depth, breadth, and knowledge manipulation, and elevate weaker architectures to match or rival state-of-the-art models, validated through synthetic and real-world pretraining.

Conclusion: The synthetic framework provides an economical and principled way to isolate model capabilities and may predict future architectural improvements, unlocking deeper reasoning and hierarchical inference.
...
TL;DR: IPC is an unsupervised framework leveraging internal probing of LLMs for code generation, reducing dependency on labeled data while achieving competitive performance.


<details>
  <summary>Details</summary>
Motivation: The high cost and difficulty of obtaining labeled or unlabeled datasets for training LLMs in code generation tasks motivate the need for unsupervised methods.

Method: IPC uses internal probing (problem space, test understanding, solution space, knowledge consolidation) and self-consistency mechanisms to train UCoder without external data.

Result: IPC achieves competitive performance on code benchmarks, reducing dependency on labeled data and computational resources.

Conclusion: Internal model states contain rich signals for unsupervised code generation, offering a viable approach for resource-constrained scenarios.
...
TL;DR: The paper introduces **CulturalToM-VQA**, a benchmark for evaluating cross-cultural Theory of Mind (ToM) reasoning in Vision-Language Models.


<details>
  <summary>Details</summary>
Motivation: ToM is crucial for social intelligence, but its cross-cultural application in artificial agents is understudied. Current benchmarks lack cultural diversity.

Method: A VLM-assisted human-in-the-loop pipeline creates 5095 questions from culturally rich images, covering six ToM tasks and four complexity levels.

Result: The dataset enables systematic evaluation of ToM reasoning across diverse cultural contexts, addressing Western-centric limitations.

Conclusion: CulturalToM-VQA advances ToM evaluation by incorporating cultural diversity, supporting broader applications in socially grounded AI tasks.
...
TL;DR: A new ensemble framework for emotion detection combines diverse small transformer models with weighted voting, achieving superior performance over larger models.


<details>
  <summary>Details</summary>
Motivation: To improve emotion detection by leveraging diverse small transformer models while minimizing parameter convergence and preserving error diversity.

Method: Uses architecturally diverse sLLMs (BERT, RoBERTa, etc.) with a dual-weighted voting mechanism (global credibility and local confidence).

Result: Achieves a macro F1 score of 93.5% on the DAIR-AI dataset, outperforming larger models like Falcon and Mistral.

Conclusion: Small, fine-tuned model ensembles can outperform larger LLMs in specialized NLP tasks like emotion detection, offering better parameter efficiency and robustness.
...
TL;DR: The paper explores using linear directions aligned with the Big Five personality traits to probe and steer behavior in large language models (LLMs), finding effectiveness in detection but limited steering in open-ended contexts.


<details>
  <summary>Details</summary>
Motivation: Personality in LLMs impacts trust and engagement, but current methods to control or characterize it are costly or brittle. The study seeks a cheaper, more efficient alternative using linear directions.

Method: Using Llama 3.3 70B, fictional characters' Big Five trait scores and descriptions were generated. Model activations were sampled using Alpaca questionnaire prompts, and linear regression derived per-layer directions for probing and steering.

Result: Linear directions effectively detect personality traits but show limited steering capability in open-ended generation or prompts with additional context.

Conclusion: Linear directions aligned with personality traits are viable for probing model behavior, though steering effectiveness is context-dependent, robust in forced-choice tasks but less so in open-ended scenarios.
...
TL;DR: The abstract introduces simulstream, a new open-source framework for evaluating and demonstrating Streaming Speech-to-Text Translation (StreamST) systems, addressing limitations of the outdated SimulEval repository.


<details>
  <summary>Details</summary>
Motivation: Existing tools like SimulEval are no longer maintained, lack support for output revisions, and are poorly suited for long-form audio or interactive demos.

Method: simulstream is designed to support long-form audio, incremental decoding, and re-translation methods, with a unified evaluation framework and interactive web interface.

Result: The framework enables comprehensive comparison of StreamST systems in terms of quality and latency, while also providing a demo platform.

Conclusion: simulstream fills a critical gap in StreamST research by offering a modern, flexible, and interactive tool for evaluation and demonstration.
...
TL;DR: The paper introduces a contextual biasing method for AED-based ASR models to improve recognition of rare or unseen named entities by predicting multiple future tokens, reducing architecture complexity while significantly lowering word error rates.


<details>
  <summary>Details</summary>
Motivation: End-to-end ASR models struggle with rare or unseen named entities, which are crucial for applications like virtual assistants.

Method: The proposed method uses contextual biasing with candidate named entities, predicting multiple future tokens simultaneously to score entities without extra encoders or layers.

Result: Experiments show a 50.34% relative improvement in named entity word error rate compared to the baseline AED model.

Conclusion: The approach effectively enhances named entity recognition in ASR models while maintaining architectural simplicity.
...
TL;DR: The paper investigates Bayesian reasoning to measure uncertainty in neural networks for question answering, comparing Laplace approximations and MAP estimates for better uncertainty calibration and selective prediction.


<details>
  <summary>Details</summary>
Motivation: To improve interpretability and ethical deployment of neural question-answering systems by quantifying uncertainty and allowing models to abstain when confidence is low.

Method: The study starts with a multilayer perceptron on the Iris dataset, then extends Bayesian inference to language models, including LoRA-adapted transformers, evaluated on CommonsenseQA.

Result: Bayesian methods enable models to convey confidence in predictions and abstain when uncertain, enhancing interpretability and ethical use.

Conclusion: Bayesian reasoning contributes to more responsible neural question-answering systems by improving uncertainty calibration and selective prediction.
...
TL;DR: The paper explores challenges in translating user-generated content (UGC) due to its non-standard language, proposes a taxonomy of non-standard phenomena and translation actions, and highlights the need for guideline-aware evaluation frameworks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the difficulty in evaluating UGC translations, where the definition of 'good' translation varies based on desired standardness.

Method: The study analyzes human translation guidelines from four UGC datasets, identifies twelve non-standard phenomena and five translation actions, and tests LLMs with explicit prompts.

Result: Findings show translation scores are sensitive to prompts aligning with dataset guidelines and highlight variations in UGC treatment, leading to a spectrum of standardness in translations.

Conclusion: The paper concludes that fair evaluation of UGC translation requires guideline awareness and calls for clearer guidelines and controllable evaluation frameworks.
...
TL;DR: The ABCDE dataset provides a large-scale, annotated resource for interdisciplinary research in affective and social sciences, addressing challenges in accessing labeled language data.


<details>
  <summary>Details</summary>
Motivation: There's a need for accessible, annotated language data to support research in computational affective and social sciences, especially for non-computer science practitioners.

Method: The ABCDE dataset collects over 400 million text utterances from diverse sources (social media, blogs, books, AI) and annotates them with relevant features for affective and social science research.

Result: ABCDE enables large-scale, interdisciplinary research by providing a comprehensive, labeled dataset spanning multiple fields.

Conclusion: The ABCDE dataset is a valuable resource that bridges gaps in accessing annotated language data, fostering cross-disciplinary collaboration and research.
...
TL;DR: AncientBench is a new benchmark designed to evaluate large language models' comprehension of ancient Chinese characters, especially in excavated documents, through four dimensions and ten tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Chinese language benchmarks focus on modern or transmitted ancient texts, neglecting excavated ancient documents, which are crucial for archaeology and understanding Chinese history.

Method: AncientBench evaluates four comprehension competencies (glyph, pronunciation, meaning, contextual) via ten tasks. Researchers tested it on top-performing large language models and a proposed baseline ancient model.

Result: Results show significant potential of large language models in ancient textual scenarios but highlight gaps compared to human comprehension.

Conclusion: AncientBench aims to advance the use of large language models in archaeology and ancient Chinese language studies.
...
TL;DR: The paper explores Medical Entity Recognition (MedER) for Bangla, evaluating transformer models and proposing a Multi-BERT Ensemble approach with 89.58% accuracy, outperforming baselines. A high-quality dataset was also developed to address the lack of annotated data.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored area of MedER in low-resource languages like Bangla, aiming to enhance automated medical systems and patient care.

Method: Examined transformer models (BERT, DistilBERT, ELECTRA, RoBERTa) and proposed a Multi-BERT Ensemble approach. Developed a tailored dataset for evaluation.

Result: The Multi-BERT Ensemble achieved 89.58% accuracy, an 11.80% improvement over single-layer BERT, and demonstrated robustness on the new dataset.

Conclusion: The study underscores the effectiveness of Multi-BERT Ensemble for Bangla MedER and lays the groundwork for future advancements in low-resource medical NLP.
...
TL;DR: DEER is a new benchmark for evaluating expert-level reports generated by LLMs, featuring domain-specific tasks, a detailed evaluation rubric, and a fact-checking architecture to ensure factual reliability.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for evaluating expert-level reports lack systematic criteria and fail to capture nuanced expert judgment, while source verification is limited. DEER addresses these gaps.

Method: DEER introduces 50 tasks across 13 domains, an expert-grounded evaluation taxonomy (7 dimensions, 25 sub-dimensions), task-specific expert guidance, and a document-level fact-checking architecture to verify all claims in reports.

Result: DEER closely correlates with human expert judgments, provides interpretable diagnostics, and ensures comprehensive factual reliability by verifying both cited and uncited claims.

Conclusion: DEER offers a robust framework for evaluating expert-level reports, improving consistency and reliability in assessments of LLM-generated research outputs.
...
TL;DR: ShareChat is a large-scale dataset preserving platform-specific interactions of LLMs, offering insights into user intent, citation behaviors, and usage patterns.


<details>
  <summary>Details</summary>
Motivation: Existing datasets treat LLMs as generic text generators, missing interface context that shapes user interactions. ShareChat addresses this gap.

Method: ShareChat collects 142,808 conversations from five platforms, preserving native affordances like reasoning traces and source links, spanning 101 languages.

Result: The dataset enables analyses of conversation completeness, citation behaviors, and temporal usage patterns, providing deeper interaction insights.

Conclusion: ShareChat is a vital resource for understanding authentic user-LLM interactions, offering longer context and richer data than prior datasets.
...
TL;DR: This paper introduces an LLM-based Multi-Agent System (MAS) inspired by Transactional Analysis (TA) theory, where agents have three ego states (Parent, Adult, Child) to enhance psychological realism in simulations.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents lack psychological depth and consistency, missing deeper human motivations and emotional conflicts. The goal is to model more realistic group dynamics and social behavior.

Method: Proposes a MAS where each agent has three ego states (Parent, Adult, Child) with separate knowledge structures. Integrates information retrieval to enrich responses. Evaluated via ablation tests in simulated dialogues.

Result: The system shows promise, enhancing agent behavior by combining TA theory with contextual retrieval, providing a more realistic simulation.

Conclusion: The proposed architecture advances LLM-based simulations by incorporating psychological grounding and contextual information.
...
TL;DR: Introduces MAPPO and MAPPO-LCR to address payoff coupling and non-stationarity in spatial public goods games, showing improved cooperation and learning over PPO.


<details>
  <summary>Details</summary>
Motivation: Existing methods like evolutionary rules and reinforcement learning struggle with payoff coupling and non-stationarity in large populations.

Method: Uses Multi-Agent Proximal Policy Optimization (MAPPO) with a centralized critic and introduces MAPPO-LCR for local cooperation alignment.

Result: Demonstrates stable cooperation emergence and reliable convergence, with MAPPO outperforming PPO.

Conclusion: MAPPO and MAPPO-LCR effectively address limitations of prior methods, enhancing cooperation in spatial public goods games.
...
TL;DR: The paper proposes a Verifiability-First architecture and OPERA benchmark to detect and remediate misalignment in LLM-based agents, focusing on controllability and auditability.


<details>
  <summary>Details</summary>
Motivation: Ensuring LLM-based agents remain controllable, auditable, and faithful to deployer intent as they grow more autonomous and multi-modal.

Method: Introduces a Verifiability-First architecture with run-time attestations, lightweight Audit Agents, and challenge-response protocols, along with the OPERA benchmark suite.

Result: Shifts evaluation focus from misalignment likelihood to detection speed and reliability, measuring detectability, time to detection, and resilience to adversarial tactics.

Conclusion: The proposed framework enhances the ability to detect and remediate misalignment in autonomous agents, improving their reliability and trustworthiness.
...
### [139] [Navigating Taxonomic Expansions of Entity Sets Driven by Knowledge Bases](https://arxiv.org/abs/2512.16953)
*Pietro Cofone,Giovanni Amendola,Marco Manna,Aldo Ricioppo*

Main category: cs.AI

TL;DR: The paper introduces a logic-based framework for Entity Set Expansion using expansion graphs, addressing the impracticality of full materialization by formalizing efficient reasoning tasks for local navigation.


<details>
  <summary>Details</summary>
Motivation: The linear approach to Entity Set Expansion fails to capture richer taxonomic structures, prompting the need for a more nuanced method leveraging knowledge bases.

Method: The paper proposes expansion graphs, rooted directed acyclic graphs with nodes representing semantic generalizations. It formalizes reasoning tasks to check tuple relationships without full graph materialization.

Result: Efficient reasoning tasks are demonstrated under realistic assumptions (e.g., bounded input or limited descriptions), enabling practical local navigation of expansion graphs.

Conclusion: The framework supports scalable, incremental exploration of entity sets without full graph construction, enhancing practical applications of taxonomic expansions.
...
TL;DR: PAACE is a plan-aware framework for optimizing LLM agent contexts, improving accuracy and reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing summarization and compression methods overlook the multi-step nature of agent workflows, leading to inefficiencies in context management.

Method: PAACE combines next-k-task relevance modeling, plan-structure analysis, instruction co-refinement, and function-preserving compression, with two components: PAACE-Syn for synthetic workflow generation and PAACE-FT for distilled compression.

Result: PAACE outperforms baselines in accuracy, reduces context load, and lowers computational costs, achieving fewer steps and lower peak tokens.

Conclusion: PAACE effectively enhances LLM agent performance while optimizing resource usage, making it practical for deployment.
...
### [141] [Probing Scientific General Intelligence of LLMs with Scientist-Aligned Workflows](https://arxiv.org/abs/2512.16969)
...
TL;DR: The paper proposes a framework for Scientific General Intelligence (SGI) and introduces SGI-Bench, a benchmark to evaluate AI systems on tasks like research, idea generation, and experimentation, revealing gaps in current models. It also introduces Test-Time Reinforcement Learning (TTRL) to improve hypothesis novelty.


<details>
  <summary>Details</summary>
Motivation: The lack of a coherent framework for autonomous scientific reasoning across domains motivates the development of SGI.

Method: The paper operationalizes SGI using the Practical Inquiry Model (PIM) and evaluates it via cross-disciplinary tasks in the SGI-Bench.

Result: Results highlight gaps in AI performance, such as low exact match in research and poor feasibility in ideas, but TTRL improves hypothesis novelty.

Conclusion: The work provides a foundation for AI systems to genuinely participate in scientific discovery.
...
### [142] [Security Risks of Agentic Vehicles: A Systematic Analysis of Cognitive and Cross-Layer Threats](https://arxiv.org/abs/2512.17041)
*Ali Eslami,Jiangbo Yu*

Main category: cs.AI

TL;DR: The paper explores security threats in Agentic Vehicles (AgVs), introducing a role-based architecture and framework to analyze risks in AI-driven vehicles, including cross-layer vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: To address the lack of frameworks for safety-critical cyber-physical platforms like vehicles, focusing on security threats in AgVs, including AI layer and cross-layer risks.

Method: Introduces a role-based architecture (Personal Agent and Driving Strategy Agent) and analyzes vulnerabilities using a severity matrix and attack-chain analysis.

Result: A structured framework for analyzing security risks in AgVs, demonstrating how small distortions can escalate into unsafe behavior.

Conclusion: This work provides the first foundational framework for assessing security risks in agentic AI for current and future vehicle platforms.
...
### [143] [UniRel-R1: RL-tuned LLM Reasoning for Knowledge Graph Relational Question Answering](https://arxiv.org/abs/2512.17043)
*Yinxu Tang,Chengsong Huang,Jiaxin Huang,William Yeoh*

Main category: cs.AI

TL;DR: The paper introduces relation-centric KGQA, focusing on subgraph answers for relational queries, and proposes UniRel-R1, a framework integrating subgraph selection, pruning, and reinforcement learning for better performance.


<details>
  <summary>Details</summary>
Motivation: Traditional KGQA focuses on entity-centric queries with single answers, but real-world queries often require understanding relational connections among entities.

Method: UniRel-R1 combines subgraph selection, multi-stage graph pruning, and LLM fine-tuning with reinforcement learning. The reward function prioritizes compact, specific subgraphs with informative relations and lower-degree entities.

Result: UniRel-R1 outperforms baselines in connectivity and reward and generalizes well to unseen entities and relations.

Conclusion: Relation-centric KGQA with UniRel-R1 offers a robust solution for answering relational queries by capturing meaningful semantic connections in knowledge graphs.
...
### [144] [Realistic threat perception drives intergroup conflict: A causal, dynamic analysis using generative-agent simulations](https://arxiv.org/abs/2512.17066)
*Suhaib Abdurahman,Farzan Karimi-Malekabadi,Chenxiao Yu,Nour S. Kteily,Morteza Dehghani*

Main category: cs.AI

TL;DR: The paper explores how realistic and symbolic threats influence human conflict using LLM-driven agent simulations, revealing distinct effects of each threat type and their mediation paths.


<details>
  <summary>Details</summary>
Motivation: The study aims to clarify the interplay and dominance of realistic versus symbolic threats in driving human conflict, overcoming limitations like weak causal control and ethical constraints.

Method: Researchers used simulations with LLM-driven agents in virtual societies, manipulating realistic and symbolic threats while monitoring actions, language, and attitudes.

Result: Realistic threats directly increase hostility, while symbolic threats have weaker effects, mediated by ingroup bias and only increasing hostility when realistic threats are absent. Non-hostile contact buffers escalation, and structural asymmetries concentrate hostility in majority groups.

Conclusion: The study provides causal insights into threat-driven conflict, highlighting the distinct roles of realistic and symbolic threats and suggesting mitigating factors like intergroup contact.
...
### [145] [Value Under Ignorance in Universal Artificial Intelligence](https://arxiv.org/abs/2512.17086)
*Cole Wyeth,Marcus Hutter*

Main category: cs.AI

TL;DR: The paper generalizes AIXI by allowing broader utility functions, addresses semimeasure loss ambiguities, and explores Choquet integrals for expected utilities.


<details>
  <summary>Details</summary>
Motivation: To extend AIXI's utility function range and resolve ambiguities in belief distributions, particularly semimeasure loss interpretations.

Method: Proposes using Choquet integrals from imprecise probability theory to compute expected utilities, assessing their computability.

Result: Finds standard recursive value functions as a special case, but general utilities under death interpretation don't fit Choquet integrals.

Conclusion: Choquet integrals offer a viable framework for some utility computations, but limitations exist under certain interpretations.
...
### [146] [A Solver-in-the-Loop Framework for Improving LLMs on Answer Set Programming for Logic Puzzle Solving](https://arxiv.org/abs/2512.17093)
*Timo Pierre Schrader,Lukas Lange,Tobias Kaminski,Simon Razniewski,Annemarie Friedrich*

Main category: cs.AI

TL;DR: The paper introduces a solver-in-the-loop method to improve LLMs' ability to generate Answer Set Programming (ASP) code by using solver feedback for instruction-tuning.


<details>
  <summary>Details</summary>
Motivation: Domain-specific languages like ASP pose a challenge for LLMs due to limited pre-training examples. This work aims to enhance ASP code generation by leveraging solver feedback.

Method: The approach uses solver-guided instruction-tuning, sampling ASP statements from LLMs, categorizing them based on solver feedback, and applying supervised fine-tuning with a solver-guided search.

Result: Experiments show consistent improvements in ASP code generation across two datasets and prompting settings.

Conclusion: The solver-in-the-loop method effectively enhances LLMs' performance in generating ASP code, addressing the complexity of semantic parsing in this domain.
...
### [147] [Reinforcement Learning for Self-Improving Agent with Skill Library](https://arxiv.org/abs/2512.17102)
*Jiongxiao Wang,Qiaojing Yan,Yawei Wang,Yijun Tian,Soumya Smruti Mishra,Zhichao Xu,Megha Gandhi,Panpan Xu,Lin Lee Cheong*

Main category: cs.AI

TL;DR: SAGE is a Reinforcement Learning framework that enhances LLM-based agents' self-improvement by integrating skill libraries, achieving higher efficiency and accuracy in task completion.


<details>
  <summary>Details</summary>
Motivation: LLM-based agents struggle with continuous improvement and adaptation in new environments, prompting the need for a more systematic approach to skill library implementation.

Method: Proposes SAGE, a Reinforcement Learning framework using Sequential Rollout and Skill-integrated Reward to systematically incorporate skills into learning.

Result: SAGE improves Scenario Goal Completion by 8.9%, reduces interaction steps by 26%, and token generation by 59% compared to existing methods.

Conclusion: SAGE significantly outperforms current approaches by efficiently leveraging skill libraries for agent self-improvement and adaptation.
...
### [148] [Solomonoff-Inspired Hypothesis Ranking with LLMs for Prediction Under Uncertainty](https://arxiv.org/abs/2512.17145)
*Josh Barber,Rourke Young,Cameron Coombe,Will Browne*

Main category: cs.AI

TL;DR: A Solomonoff-inspired method weights LLM-generated hypotheses by simplicity and predictive fit, offering uncertainty-aware outputs and outperforming Bayesian Model Averaging by distributing probability more evenly.


<details>
  <summary>Details</summary>
Motivation: Reasoning under uncertainty in AI is challenging, especially in sparse-data scenarios requiring systematic generalization. Existing methods fail to balance accuracy and simplicity effectively.

Method: Proposes a Solomonoff-inspired approach that combines simplicity and predictive fit to weight hypotheses generated by LLMs. Evaluated on Mini-ARC tasks, it produces uncertainty-aware per-cell predictions.

Result: The method yields conservative, uncertainty-aware outputs even with noisy hypotheses. Compared to BMA, it spreads probability more evenly, avoiding over-reliance on potentially flawed candidates.

Conclusion: Algorithmic information-theoretic priors enhance interpretable, reliable multi-hypothesis reasoning under uncertainty, outperforming traditional methods like BMA.
...
### [149] [MMRAG-RFT: Two-stage Reinforcement Fine-tuning for Explainable Multi-modal Retrieval-augmented Generation](https://arxiv.org/abs/2512.17194)
*Shengwei Zhao,Jingwen Yao,Sitong Wei,Linhai Xu,Yuying Liu,Dong Zhang,Zhiqiang Tian,Shaoyi Du*

Main category: cs.AI

TL;DR: MMRAG improves generation credibility with multi-modal knowledge but lacks explainability. The paper introduces reinforcement learning in a two-stage framework to enhance reasoning and achieve explainable results, outperforming benchmarks on WebQA and MultimodalQA.


<details>
  <summary>Details</summary>
Motivation: Existing MMRAG methods lack explainability in reasoning logic during retrieval and generation, prompting the need for a more transparent approach.

Method: A two-stage reinforcement learning framework: rule-based fine-tuning for coarse-grained document ranking and reasoning-based fine-tuning for optimizing ranking and answer generation.

Result: Achieves state-of-the-art performance on WebQA and MultimodalQA, validated by ablation experiments.

Conclusion: The proposed method enhances reasoning and explainability in MMRAG, demonstrating superior performance on benchmark datasets.
...
### [150] [UmniBench: Unified Understand and Generation Model Oriented Omni-dimensional Benchmark](https://arxiv.org/abs/2512.17196)
*Kai Liu,Leyang Chen,Wenbo Li,Zhikai Chen,Zhixin Wang,Renjing Pei,Linghe Kong,Yulun Zhang*

Main category: cs.AI

TL;DR: UmniBench is a comprehensive benchmark for unified multimodal models (UMMs), evaluating understanding, generation, and editing abilities in one process, covering 13 domains and over 200 concepts.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of UMMs are decoupled, assessing understanding and generation separately. UmniBench aims to provide a unified, thorough assessment.

Method: UmniBench uses human-examined prompts and QA pairs, leveraging UMMs to self-evaluate generation and editing via their understanding abilities. It covers 13 domains and 200+ concepts.

Result: The benchmark was used to evaluate 24 models, including UMMs and single-ability models, offering a comprehensive view of their performance.

Conclusion: UmniBench provides a holistic and objective evaluation framework for UMMs, supporting community model improvement.
...
### [151] [Accelerating Multi-modal LLM Gaming Performance via Input Prediction and Mishit Correction](https://arxiv.org/abs/2512.17250)
*Ziyang Lin,Zixuan Sun,Sanhorn Chen,Xiaoyang Chen,Roy Zhao*

Main category: cs.AI

TL;DR: The paper proposes a speculation-and-correction framework to reduce inference latency in real-time sequential control agents, improving efficiency while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Inference latency bottlenecks real-time control agents, causing instability and performance degradation. The goal is to reduce latency without compromising control.

Method: A speculate-and-correct framework uses a pretrained world model and latent-space MPC planner to generate action queues. Lightweight correctors handle mismatches, avoiding full replanning.

Result: The method reduces planning inferences by 44%, improves step latency by 25%, and limits performance loss to 7.1% on DMC Humanoid-Walk.

Conclusion: Speculation with correction is essential for robust latency reduction, outperforming speculative execution alone.
...
### [152] [ScoutGPT: Capturing Player Impact from Team Action Sequences Using GPT-Based Framework](https://arxiv.org/abs/2512.17266)
*Miru Hong,Minho Lee,Geonhee Jo,Jae-Hee So,Pascal Bauer,Sang-Ki Ko*

Main category: cs.AI

TL;DR: EventGPT, a GPT-style model, predicts football transfer success by simulating player behavior in new teams using event sequences and On-Ball Value, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating football transfers lack the ability to capture context-dependent performance changes, such as adapting to new tactics or teammates.

Method: EventGPT uses an autoregressive transformer to predict next on-ball actions (type, location, timing) and residual On-Ball Value (rOBV) based on player identity and context, enabling counterfactual simulations.

Result: EventGPT surpasses existing sequence-based models in accuracy and spatial precision on Premier League data and offers practical transfer analysis through case studies.

Conclusion: The model provides a principled way to assess transfer fit, simulating how players would perform in different tactical systems or roles.
...
### [153] [Large Language Models as Pokémon Battle Agents: Strategic Play and Content Generation](https://arxiv.org/abs/2512.17308)
*Daksh Jain,Aarya Jain,Ashutosh Desai,Avyakt Verma,Ishan Bhanuka,Pratik Narang,Dhruv Kumar*

Main category: cs.AI

TL;DR: The paper explores using large language models (LLMs) as strategic agents in Pokémon battles, testing their ability to make tactical decisions and generate game content without domain-specific training.


<details>
  <summary>Details</summary>
Motivation: Pokémon battles provide a rich environment to evaluate LLMs' strategic reasoning, including type matchups, statistical trade-offs, and risk assessment, mirroring human decision-making.

Method: The authors built a turn-based Pokémon battle system where LLMs select moves based on real-time battle state, incorporating mechanics like type effectiveness and stat-based damage. Performance was evaluated across model architectures using metrics like win rates, decision latency, and type-alignment accuracy.

Result: LLMs demonstrated competence as battle agents, achieving competitive win rates and efficient decision-making without specialized training, suggesting they can replace reinforcement learning in turn-based games.

Conclusion: LLMs show promise as both game players and content designers, with potential applications in procedural generation and adaptive difficulty systems for interactive entertainment.
...
### [154] [Dialectics for Artificial Intelligence](https://arxiv.org/abs/2512.17373)
*Zhengmian Hu*

Main category: cs.AI

TL;DR: The paper explores whether AI can autonomously discover human-like concepts without supervision, proposing an algorithmic-information approach where concepts are defined by their structural relation to an agent's experience and constrained by reversibility and excess information.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the fluidity of human concepts and develop a framework where AI can autonomously discover and revise concepts akin to human inquiry.

Method: The method involves defining concepts as information objects tied to an agent's experience, using reversibility constraints and excess information metrics. Dialectics are modeled as an optimization process for concept evolution.

Result: The framework enables AI to systematically expand, contract, split, and merge concepts, with formalized methods for low-cost transmission and multi-agent alignment.

Conclusion: The study concludes that AI can discover and revise concepts autonomously using structural and algorithmic principles, facilitating communication and alignment among agents.
...
### [155] [Translating the Rashomon Effect to Sequential Decision-Making Tasks](https://arxiv.org/abs/2512.17470)
*Dennis Gross,Jørn Eirik Betten,Helge Spieker*

Main category: cs.AI

TL;DR: The paper explores the Rashomon effect in sequential decision-making, showing that multiple policies can behave identically despite internal differences, and demonstrates benefits like robustness and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: The Rashomon effect is well-studied in classification but not in sequential decision-making. This paper bridges the gap by defining and verifying it in this context, addressing challenges like stochastic transitions.

Method: The authors use formal verification methods to compare probabilistic behaviors of policies in sequential decision-making environments, ensuring identical behavior despite internal differences.

Result: Experiments confirm the Rashomon effect in sequential decision-making. Ensembles from the Rashomon set show robustness to distribution shifts, and permissive policies reduce verification costs without performance loss.

Conclusion: The study successfully extends the Rashomon effect to sequential decision-making, highlighting practical advantages like improved robustness and computational efficiency.
...
### [156] [Towards Explainable Conversational AI for Early Diagnosis with Large Language Models](https://arxiv.org/abs/2512.17559)
*Maliha Tabassum,M Shamim Kaiser*

Main category: cs.AI

TL;DR: A diagnostic chatbot using GPT-4o and explainable AI outperforms traditional ML models, achieving high accuracy in symptom analysis and diagnosis.


<details>
  <summary>Details</summary>
Motivation: Address inefficiencies in healthcare diagnostics, high costs, and lack of transparency in AI systems.

Method: Utilizes LLM (GPT-4o), Retrieval-Augmented Generation, adaptive questioning, and Chain-of-Thought prompting.

Result: Achieved 90% accuracy and 100% Top-3 accuracy, surpassing traditional ML models.

Conclusion: The system demonstrates potential for transparent, interactive, and clinically effective AI diagnostics.
...
### [157] [About Time: Model-free Reinforcement Learning with Timed Reward Machines](https://arxiv.org/abs/2512.17637)
*Anirban Majumdar,Ritam Raha,Rajarshi Roy,David Parker,Marta Kwiatkowska*

Main category: cs.AI

TL;DR: Introduces timed reward machines (TRMs) for RL, enhancing traditional reward machines with timing constraints. Demonstrates effectiveness via model-free RL algorithms.


<details>
  <summary>Details</summary>
Motivation: Traditional reward machines lack timing constraints, limiting their use in time-sensitive RL applications.

Method: Proposes TRMs integrating timing constraints, studies model-free RL frameworks, and uses counterfactual-imagining heuristics.

Result: Algorithms learn optimal policies meeting TRM timing constraints, validated on RL benchmarks.

Conclusion: TRMs improve expressive reward specifications, and counterfactual-imagining enhances policy learning.
...
### [158] [Humanlike AI Design Increases Anthropomorphism but Yields Divergent Outcomes on Engagement and Trust Globally](https://arxiv.org/abs/2512.17898)
*Robin Schimmelpfennig,Mark Díaz,Vinodkumar Prabhakaran,Aida Davani*

Main category: cs.AI

TL;DR: The study explores how human-like AI design affects user engagement and trust globally, revealing that cultural differences significantly moderate these effects.


<details>
  <summary>Details</summary>
Motivation: To empirically test the impact of human-like AI design on user trust and engagement across diverse cultural contexts, addressing gaps in current safety frameworks that rely on Western assumptions.

Method: Two large-scale cross-national experiments involving 3,500 participants from 10 countries, with real-time and open-ended interactions with an AI system.

Result: Human-like design increases anthropomorphism but doesn't universally boost engagement or trust. Cultural differences fracture this connection, with varying effects across populations (e.g., positive in Brazil, negative in Japan).

Conclusion: The findings highlight the need for culturally nuanced AI governance, challenging the one-size-fits-all approach and theoretical assumptions about human-like AI risks.
...
### [159] [When Reasoning Meets Its Laws](https://arxiv.org/abs/2512.17901)
*Junyu Zhang,Yifan Sun,Tianang Leng,Jingyan Shen,Liu Ziyin,Paul Pu Liang,Huan Zhang*

Main category: cs.AI

TL;DR: The paper introduces the Laws of Reasoning (LoRe) framework to formalize desired reasoning behaviors in Large Reasoning Models (LRMs), proposing compute and accuracy laws. It validates these via LoRe-Bench, showing models often lack compositionality, and improves performance through finetuning.


<details>
  <summary>Details</summary>
Motivation: Current LRMs exhibit counterintuitive reasoning behaviors, leading to suboptimal capabilities. This work aims to formalize and improve these behaviors.

Method: Proposes LoRe framework (compute and accuracy laws) and LoRe-Bench to validate monotonicity and compositionality. Introduces finetuning to enforce compositionality.

Result: Most models show reasonable monotonicity but lack compositionality. Finetuning improves compliance with compute laws and reasoning performance.

Conclusion: Better adherence to LoRe laws enhances reasoning performance, revealing synergistic effects. The framework and benchmark provide tools for advancing LRMs.
...
### [160] [Dion2: A Simple Method to Shrink Matrix in Muon](https://arxiv.org/abs/2512.16928)
*Kwangjun Ahn,Noah Amsel,John Langford*

Main category: cs.LG

TL;DR: Dion2 simplifies Muon's costly orthonormalization step by selectively sampling rows or columns, reducing computation and communication overhead.


<details>
  <summary>Details</summary>
Motivation: Muon's orthonormalization step is computationally expensive at scale, prompting efforts to reduce the matrix size involved.

Method: Dion2 samples a fraction of rows or columns during each iteration, orthonormalizing only the selected subset to make updates sparse.

Result: This approach lowers computation and communication costs, enhancing Muon's scalability.

Conclusion: Dion2 provides a simpler and more scalable solution compared to prior methods for addressing Muon's orthonormalization overhead.
...
### [161] [BIONIX: A Wireless, Low-Cost Prosthetic Arm with Dual-Signal EEG and EMG Control](https://arxiv.org/abs/2512.16929)
*Pranesh Sathish Kumar*

Main category: cs.LG

TL;DR: A low-cost, dual-mode neuro-muscular control system integrating EEG and EMG is developed for intuitive prosthetic arm control, demonstrating feasibility for underserved populations.


<details>
  <summary>Details</summary>
Motivation: Affordable prostheses often lack intuitive control, limiting accessibility for amputees in low-resource settings.

Method: Combines EEG (NeuroSky) and EMG (MyoWare) signals, processed with lightweight classification and threshold-based detection, to control hand and elbow servos.

Result: Functional prototype built for ~240 dollars, enabling multi-degree-of-freedom control. EEG detects blinks for hand states; EMG thresholds enable elbow movement.

Conclusion: The system offers a feasible, low-cost solution for intuitive prosthetic control, with potential for further improvements like 3D printing and better servo torque.
...
### [162] [DiffeoMorph: Learning to Morph 3D Shapes Using Differentiable Agent-Based Simulations](https://arxiv.org/abs/2512.17129)
*Seong Ho Pahng,Guoye Guan,Benjamin Fefferman,Sahand Hormoz*

Main category: cs.LG

TL;DR: DiffeoMorph is a differentiable framework that trains agents to form 3D shapes using SE(3)-equivariant graph neural networks and a novel shape-matching loss based on 3D Zernike polynomials.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of understanding how distributed control leads to precise global patterns in biological systems and its applications in robotics and multi-agent learning.

Method: The framework uses an attention-based SE(3)-equivariant graph neural network for agent updates and introduces a shape-matching loss with 3D Zernike polynomials. It includes an alignment step for SO(3) invariance and computes gradients through implicit differentiation.

Result: DiffeoMorph successfully forms various 3D shapes, from simple ellipsoids to complex morphologies, using minimal spatial cues.

Conclusion: The proposed framework advances the understanding and application of distributed control in morphogenesis, offering robustness to agent ordering, number, and transformations.
...
### [163] [Compression is Routing: Reconstruction Error as an Intrinsic Signal for Modular Language Models](https://arxiv.org/abs/2512.16963)
*Zhongpan Tang*

Main category: cs.LG

TL;DR: The paper proposes 'Compression is Routing' as a novel architectural philosophy for LLMs, using a transformer autoencoder to achieve domain discrimination and simplify expert scheduling without explicit gating networks.


<details>
  <summary>Details</summary>
Motivation: Address challenges in LLMs like context length limits, high inference costs, and catastrophic forgetting, while simplifying MoE architectures by removing the need for auxiliary classifiers.

Method: Train an 87M-parameter transformer autoencoder to compress sequences (512 tokens into 8 latent vectors), using reconstruction errors as intrinsic domain fingerprints for expert routing.

Result: Achieves 99.47% reconstruction accuracy for in-domain data, dropping sharply to 47.76% for semi-out-of-distribution and 0.57% for fully out-of-distribution, validating the routing mechanism.

Conclusion: The architecture offers scalable, interpretable expert scheduling and a new approach to VRAM compression, providing a foundation for next-gen modular neural networks.
...
### [164] [Physics-Informed Lightweight Machine Learning for Aviation Visibility Nowcasting Across Multiple Climatic Regimes](https://arxiv.org/abs/2512.16967)
*Marcelo Cerda Castillo*

Main category: cs.LG

TL;DR: The paper presents a lightweight XGBoost framework for nowcasting low-visibility and precipitation events at airports, using physics-guided feature engineering on METAR data, outperforming traditional TAF forecasts.


<details>
  <summary>Details</summary>
Motivation: Current methods for aviation weather nowcasting are computationally intensive and biased. A lightweight, physics-guided ML approach could improve accuracy and efficiency.

Method: A gradient boosting model (XGBoost) trained on METAR data, enhanced with physics-guided features, evaluated across 11 diverse airports.

Result: The model outperformed TAF forecasts with 2.5-4x better recall at 3-hour horizons and fewer false alarms, while revealing underlying physical processes via SHAP analysis.

Conclusion: The framework offers accurate, explainable, and lightweight nowcasting, suitable for operational aviation meteorology.
...
### [165] [Turn-PPO: Turn-Level Advantage Estimation with PPO for Improved Multi-Turn RL in Agentic LLMs](https://arxiv.org/abs/2512.17008)
*Junbo Li,Peng Zhou,Rui Meng,Meet P. Vadera,Lihong Li,Yang Li*

Main category: cs.LG

TL;DR: The paper proposes turn-PPO, a variant of PPO, to improve reinforcement learning for multi-turn tasks, showing better performance than GRPO.


<details>
  <summary>Details</summary>
Motivation: Directly applying GRPO to multi-turn tasks reveals limitations in long-horizon reasoning, prompting the exploration of more stable advantage estimation strategies.

Method: The study first evaluates PPO as a robust alternative to GRPO, then introduces turn-PPO, which uses a turn-level MDP formulation for multi-turn scenarios.

Result: Experiments on WebShop and Sokoban datasets confirm turn-PPO's effectiveness, even without long reasoning components.

Conclusion: Turn-PPO outperforms GRPO and standard PPO in multi-turn tasks, offering a promising approach for RL in interactive LLM agents.
...
### [166] [GB-DQN: Gradient Boosted DQN Models for Non-stationary Reinforcement Learning](https://arxiv.org/abs/2512.17034)
*Chang-Hwan Lee,Chanseung Lee*

Main category: cs.LG

TL;DR: GB-DQN is an adaptive ensemble method for deep reinforcement learning in non-stationary environments, using gradient boosting to address model drift and improve stability.


<details>
  <summary>Details</summary>
Motivation: Non-stationary environments challenge deep reinforcement learning by invalidating learned value functions and causing catastrophic forgetting.

Method: GB-DQN constructs an additive ensemble where each new learner approximates the Bellman residual of the current ensemble after drift, avoiding retraining a single Q-network.

Result: Experiments show GB-DQN outperforms DQN and baselines in recovery speed, stability, and robustness across control tasks with dynamic changes.

Conclusion: GB-DQN effectively tackles non-stationarity in reinforcement learning through incremental residual learning and ensemble methods.
...
### [167] [SFBD-OMNI: Bridge models for lossy measurement restoration with limited clean samples](https://arxiv.org/abs/2512.17051)
*Haoye Lu,Yaoliang Yu,Darren Ho*

Main category: cs.LG

TL;DR: The paper proposes a framework (SFBD-OMNI) for restoring distributions from noisy samples using an EM-like algorithm and one-sided entropic optimal transport, demonstrating improved performance across diverse settings.


<details>
  <summary>Details</summary>
Motivation: In real-world scenarios, fully observed samples are often hard to obtain, while noisy or partial observations are easier to collect. This work addresses the challenge of restoring the true underlying distribution from such noisy samples.

Method: The task is framed as a one-sided entropic optimal transport problem and solved via an EM-like algorithm. A test criterion is provided to assess recoverability, and the SFBD-OMNI framework is introduced to handle arbitrary measurement models.

Result: Experiments show significant improvements in both qualitative and quantitative performance across benchmark datasets and diverse measurement settings.

Conclusion: The proposed SFBD-OMNI framework effectively restores the ground-truth distribution from noisy observations, even in cases where the distribution is otherwise unrecoverable, leveraging a small number of clean samples when needed.
...
### [168] [Dynamic Tool Dependency Retrieval for Efficient Function Calling](https://arxiv.org/abs/2512.17052)
*Bhrij Patel,Davide Belli,Amir Jalalirad,Maximilian Arnold,Aleksandr Ermovol,Bence Major*

Main category: cs.LG

TL;DR: DTDR improves tool retrieval for LLM-powered agents by dynamically adapting to evolving task context, outperforming static methods by 23%-104%.


<details>
  <summary>Details</summary>
Motivation: Existing retrieval methods for on-device agents fail to capture multi-step tool dependencies and evolving task context, leading to inefficiency and inaccuracy.

Method: Proposes Dynamic Tool Dependency Retrieval (DTDR), a lightweight method that conditions on the initial query and execution context, modeling dependencies from demonstrations.

Result: DTDR outperforms state-of-the-art static retrievers, improving function calling success rates by 23%-104%.

Conclusion: Dynamic retrieval significantly enhances efficiency and accuracy in LLM-powered function calling agents.
...
### [169] [Universal consistency of the $k$-NN rule in metric spaces and Nagata dimension. III](https://arxiv.org/abs/2512.17058)
*Vladimir G. Pestov*

Main category: cs.LG

TL;DR: The paper proves the equivalence of three conditions for a complete separable metric space, addressing a long-standing conjecture and correcting a previous claim.


<details>
  <summary>Details</summary>
Motivation: To establish the full equivalence of conditions related to the k-nearest neighbour classifier, Lebesgue-Besicovitch differentiation, and sigma-finite dimensionality, resolving a conjecture and correcting prior errors.

Method: Proves the implication (1)⇒(3) to complete the equivalence, building on prior work and correcting a mistake from an earlier article in the series.

Result: The equivalence of the three conditions is fully proven, confirming the conjecture and rectifying previous inaccuracies.

Conclusion: The study successfully ties together three mathematical properties, providing a complete proof and correcting prior claims, advancing understanding in the field.
...
### [170] [Bandwidth-Efficient Adaptive Mixture-of-Experts via Low-Rank Compensation](https://arxiv.org/abs/2512.17073)
*Zhenyu Liu,Yunzhen Liu,Zehao Fan,Garrett Gagnon,Yayue Hou,Nan Wu,Yangwook Kang,Liu Liu*

Main category: cs.LG

TL;DR: Bandwidth-Efficient Adaptive Mixture-of-Experts uses low-rank compensation to reduce transfer overhead while maintaining accuracy, improving bandwidth-accuracy trade-offs and throughput.


<details>
  <summary>Details</summary>
Motivation: Mixture-of-Experts (MoE) models face memory and bandwidth challenges due to sparse activation and irregular transfers caused by token-level routing. Existing methods like static quantization degrade accuracy under aggressive compression.

Method: The proposed method uses router-guided precision restoration with precomputed low-rank compensators. It transfers compact low-rank factors for Top-n experts per token and applies compensation, keeping others in low-bit precision.

Result: Integrated with offloading on GPU and GPU-NDP systems, the method achieves superior bandwidth-accuracy trade-offs and improved throughput.

Conclusion: The approach effectively balances bandwidth efficiency and accuracy in MoE models, offering practical benefits for inference tasks.
...
### [171] [Can Large Reasoning Models Improve Accuracy on Mathematical Tasks Using Flawed Thinking?](https://arxiv.org/abs/2512.17079)
*Saraswathy Amjith,Mihika Dusad,Neha Muramalla,Shweta Shah*

Main category: cs.LG

TL;DR: Training LLMs on flawed reasoning traces improves error detection and recovery without compromising standard problem-solving, outperforming models trained only on clean data.


<details>
  <summary>Details</summary>
Motivation: Address the brittleness of CoT prompting in LLMs by investigating if exposure to flawed reasoning traces during training can enhance error detection and recovery.

Method: Generate CoT prefixes with controlled errors (calculation or reasoning), fine-tune Qwen3-4B using GRPO with binary final-answer reward, and evaluate on MATH-lighteval problems.

Result: Mixed-CoT-RL matches standard RL on clean problems (41%) and outperforms it on flawed ones (24% vs 19%). Clean-only RL degrades robustness (19% vs 20%).

Conclusion: Exposure to flawed traces during training improves LLM robustness in mathematical reasoning without sacrificing accuracy.
...
### [172] [How to Square Tensor Networks and Circuits Without Squaring Them](https://arxiv.org/abs/2512.17090)
*Lorenzo Loconte,Adrián Javaloy,Antonio Vergari*

Main category: cs.LG

TL;DR: Squared tensor networks (TNs) and circuits face complexity in marginalization. Parameterizing squared circuits with unitary matrices simplifies this, enabling efficient learning without losing expressiveness.


<details>
  <summary>Details</summary>
Motivation: The complexity of computing partition functions or marginalizing variables in squared tensor networks and circuits limits their applicability in machine learning. Addressing this computational overhead is crucial.

Method: Parameterizing squared circuits using unitary matrices to simplify marginalization, leveraging orthogonality in canonical forms and determinism in circuits.

Result: The proposed parameterizations efficiently marginalize variables in squared circuits, even in factorizations that differ from TNs, without sacrificing expressiveness.

Conclusion: Squared circuits parameterized with unitary matrices enable tractable marginalization and efficient learning, expanding their practical use in ML applications.
...
### [173] [Learning to Plan, Planning to Learn: Adaptive Hierarchical RL-MPC for Sample-Efficient Decision Making](https://arxiv.org/abs/2512.17091)
*Toshiaki Hori,Jonathan DeCastro,Deepak Gopinath,Avinash Balachandran,Guy Rosman*

Main category: cs.LG

TL;DR: A hierarchical planning approach combines reinforcement learning and MPC, improving robustness and performance in complex tasks like race driving and Lunar Lander.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing planning methods by integrating reinforcement learning and MPC for better adaptability and robustness in complex planning problems.

Method: Fuses reinforcement learning and MPC planning, using RL actions to guide MPPI sampling and adaptive aggregation of samples for value estimation.

Result: Achieves better data efficiency, higher success rates (up to 72% increase), and faster convergence (x2.1) compared to non-adaptive methods.

Conclusion: The proposed approach demonstrates superior adaptability and performance across diverse domains, making it a robust solution for hierarchical planning problems.
...
### [174] [UniCoMTE: A Universal Counterfactual Framework for Explaining Time-Series Classifiers on ECG Data](https://arxiv.org/abs/2512.17100)
*Justin Li,Efe Sencan,Jasper Zheng Duan,Vitus J. Leung,Stephan Tsaur,Ayse K. Coskun*

Main category: cs.LG

TL;DR: UniCoMTE is a model-agnostic framework for generating counterfactual explanations in multivariate time series classifiers, outperforming existing methods in clarity and applicability.


<details>
  <summary>Details</summary>
Motivation: To address the lack of trust in black-box deep learning models, especially in high-stakes domains like healthcare, by providing interpretable explanations.

Method: Develops UniCoMTE, which modifies input samples to identify influential temporal features, and evaluates it on an ECG classifier, comparing results with LIME and SHAP.

Result: UniCoMTE produces concise, stable, and human-aligned explanations that outperform LIME and SHAP in comprehensibility and clinical utility.

Conclusion: UniCoMTE enhances interpretability of deep learning models for time series data, making them more trustworthy for real-world applications.
...
### [175] [Atom: Efficient On-Device Video-Language Pipelines Through Modular Reuse](https://arxiv.org/abs/2512.17108)
*Kunjal Panchal,Saayan Mitra,Somdeb Sarkhel,Haoliang Wang,Ishita Dasgupta,Gang Wu,Hui Guan*

Main category: cs.LG

TL;DR: Atom is an on-device system that optimizes video-language pipelines by reusing model modules, reducing latency and maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current video-language models on mobile devices suffer from redundant model loads and fragmented execution, which hinders efficiency.

Method: Atom decomposes a billion-parameter model into reusable modules (e.g., visual encoder, language decoder) and reuses them across subtasks like captioning and reasoning.

Result: Atom achieves 27--33% faster execution on smartphones with minimal performance drop (≤2.3 Recall@1 in retrieval, ≤1.5 CIDEr in captioning).

Conclusion: Atom offers a practical, scalable solution for efficient video-language understanding on edge devices.
...
### [176] [Bridging Training and Merging Through Momentum-Aware Optimization](https://arxiv.org/abs/2512.17109)
*Alireza Moayedikia,Alicia Troncoso*

Main category: cs.LG

TL;DR: A unified framework combines training and merging of neural networks by maintaining factorized momentum and curvature statistics, enabling geometry-aware model composition without redundant computations.


<details>
  <summary>Details</summary>
Motivation: Training large networks and merging task-specific models both rely on low-rank structure and parameter importance estimation, but current methods waste computation by recalculating curvature information separately for each task.

Method: The framework maintains factorized momentum and curvature statistics during training, reusing them for geometry-aware model composition, eliminating post-hoc Fisher computation.

Result: The method achieves memory efficiency, curvature-aware merging outperforms magnitude-only baselines, and multi-task merging improves over strong baselines. Convergence is proven for non-convex objectives.

Conclusion: By reusing optimization trajectory data, the framework eliminates redundant computation and enables principled model composition, showing rank-invariant convergence and hyperparameter robustness.
...
### [177] [Digitizing Nepal's Written Heritage: A Comprehensive HTR Pipeline for Old Nepali Manuscripts](https://arxiv.org/abs/2512.17111)
*Anjali Sarawgi,Esteban Garces Arias,Christof Zotter*

Main category: cs.LG

TL;DR: First end-to-end pipeline for Old Nepali HTR, achieving 4.9% CER with released training code and evaluation scripts.


<details>
  <summary>Details</summary>
Motivation: Address the lack of resources for Old Nepali, a historically significant but low-resource language.

Method: Line-level transcription with encoder-decoder architectures and data-centric techniques, plus decoding strategies and token-level analysis.

Result: Best model achieves 4.9% Character Error Rate (CER).

Conclusion: Successful pipeline for Old Nepali HTR with tools released for further research, though dataset remains confidential.
...
### [178] [The Effect of Negation on CLIP in Medical Imaging: Limitations of Contrastive Language-Image Pretraining](https://arxiv.org/abs/2512.17121)
*Jasmine Vu,Shivanand Sheshappanavar*

Main category: cs.LG

TL;DR: The study evaluates the CheXagent model's performance in retrieving chest X-ray images using prompts with and without negation, aiming to improve CLIP-based models' handling of negated phrases in medical diagnosing.


<details>
  <summary>Details</summary>
Motivation: CLIP-based models underperform in interpreting negated phrases, which is problematic for medical diagnosing. This study aims to address this gap.

Method: The CheXagent model is evaluated on retrieval tasks with negated prompts, followed by fine-tuning to improve accuracy. Analysis includes token attribution, t-SNE projection, and attention-head ablation.

Result: The study shows improved handling of negation in CLIP models, though with a slight decrease in accuracy for positive prompts.

Conclusion: The work provides insights into CLIP's internal behavior and improves its reliability for medical AI devices by better handling negated clinical language.
...
### [179] [Smoothing DiLoCo with Primal Averaging for Faster Training of LLMs](https://arxiv.org/abs/2512.17131)
*Aaron Defazio,Konstantin Mishchenko,Parameswaran Raman,Hao-Jun Michael Shi,Lin Xiao*

Main category: cs.LG

TL;DR: Generalized Primal Averaging (GPA) extends Nesterov's method to improve upon averaging-based optimizers like single-worker DiLoCo and Schedule-Free, offering better performance and simpler tuning.


<details>
  <summary>Details</summary>
Motivation: Recent optimizers like single-worker DiLoCo and Schedule-Free enhance base optimizers (e.g., AdamW) but suffer from memory, hyperparameter, and structural limitations. GPA aims to address these drawbacks.

Method: GPA decouples the interpolation constant in Nesterov's primal averaging formulation, enabling smooth averaging at every step and eliminating the two-loop structure of DiLoCo.

Result: GPA outperforms single-worker DiLoCo, simplifies hyperparameter tuning, and reduces memory overhead. It achieves significant speedups (e.g., 24.22% on Llama-160M) and matches or exceeds base optimizer guarantees.

Conclusion: GPA provides a more efficient and generalized approach to averaging-based optimization, improving performance while reducing complexity and resource usage.
...
### [180] [BumpNet: A Sparse Neural Network Framework for Learning PDE Solutions](https://arxiv.org/abs/2512.17198)
*Shao-Ting Chiu,Ioannis G. Kevrekidis,Ulisses Braga-Neto*

Main category: cs.LG

TL;DR: BumpNet is a sparse neural network framework using sigmoid-based basis functions for PDE solutions and operator learning, with dynamic pruning and integration into existing architectures like PINNs, EDNNs, and DeepONets.


<details>
  <summary>Details</summary>
Motivation: To develop a flexible and efficient framework for solving PDEs and learning operators using meshless basis functions inspired by RBF networks but optimized for modern training techniques.

Method: BumpNet constructs trainable basis functions from sigmoid activations, enabling dynamic pruning for parsimony and adaptability. It integrates with PINNs, EDNNs, and DeepONets for varied PDE applications.

Result: Demonstrated efficiency and accuracy in solving PDEs and learning operators through extensive numerical experiments.

Conclusion: BumpNet provides a versatile and effective framework for PDE solutions and operator learning, outperforming traditional methods like RBF networks.
...
### [181] [Learning solution operator of dynamical systems with diffusion maps kernel ridge regression](https://arxiv.org/abs/2512.17203)
*Jiwoo Song,Daning Huang,John Harlim*

Main category: cs.LG

TL;DR: DM-KRR, combining kernel ridge regression with diffusion maps, outperforms existing methods for long-term prediction of complex dynamical systems by respecting intrinsic geometry without explicit manifold reconstruction.


<details>
  <summary>Details</summary>
Motivation: Many systems have nonlinear dynamics hard to predict long-term. Data-driven models often fail due to poor representation of governing geometric structures.

Method: The DM-KRR framework uses kernel ridge regression with a dynamics-aware validation strategy and a data-driven kernel from diffusion maps, adapting to the system's intrinsic geometry.

Result: DM-KRR consistently beats random feature, neural-network, and operator-learning methods in accuracy and data efficiency across various systems.

Conclusion: Long-term prediction relies on respecting geometric constraints in the data. DM-KRR's simplicity, geometry awareness, and performance make it promising for learning complex dynamics.
...
### [182] [Electric Vehicle Charging Load Forecasting: An Experimental Comparison of Machine Learning Methods](https://arxiv.org/abs/2512.17257)
*Iason Kyriakopoulos,Yannis Theodoridis*

Main category: cs.LG

TL;DR: The paper evaluates five time series forecasting models for predicting EV charging demand across various temporal and spatial scales, using four real-world datasets.


<details>
  <summary>Details</summary>
Motivation: With the rise of electric vehicles and their impact on grid management, predicting EV charging demand has become a crucial research problem. Existing studies lack systematic comparisons across different temporal and spatial scales.

Method: The study tests five forecasting models (statistical, machine learning, and deep learning) for short-, mid-, and long-term horizons (minutes, hours, days) and spatial levels (individual stations to city-wide). Four real-world datasets are used.

Result: Results are reported for each dataset independently, showcasing the performance of each model across the tested temporal and spatial variations.

Conclusion: This is the first systematic evaluation of EV charging demand forecasting across diverse temporal and spatial scales, providing insights for grid management.
...
### [183] [SHARP-QoS: Sparsely-gated Hierarchical Adaptive Routing for joint Prediction of QoS](https://arxiv.org/abs/2512.17262)
*Suraj Kumar,Arvind Kumar,Soumi Chattopadhyay*

Main category: cs.LG

TL;DR: SHARP-QoS is a unified strategy for joint QoS prediction, addressing challenges like sparse data, negative transfer, and inadequate representation learning through hierarchical feature extraction, adaptive feature-sharing, and balanced loss optimization.


<details>
  <summary>Details</summary>
Motivation: Real-world QoS data are sparse, noisy, and hierarchically dependent, making accurate prediction difficult. Existing methods lack efficiency and suffer from negative transfer.

Method: SHARP-QoS uses hyperbolic convolution for hierarchical feature extraction, adaptive feature-sharing with gated fusion, and EMA-based loss balancing for stable optimization.

Result: Evaluations show SHARP-QoS outperforms single- and multi-task baselines, handling sparsity, outliers, and cold-start effectively with moderate computational cost.

Conclusion: SHARP-QoS reliably predicts multiple QoS parameters jointly, overcoming key challenges and improving accuracy.
...
### [184] [Understanding Generalization in Role-Playing Models via Information Theory](https://arxiv.org/abs/2512.17270)
*Yongqi Li,Hao Lang,Fei Huang,Tieyun Qian,Yongbin Li*

Main category: cs.LG

TL;DR: The paper introduces R-EMID, an interpretable metric to diagnose RPM performance degradation due to distribution shifts, and proposes a reinforcement learning framework to improve generalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to provide fine-grained diagnosis of RPM generalization issues caused by distribution shifts like user, character, and dialogue compositional shifts.

Method: Developed R-EMID to measure RPM degradation and derived its upper bound. Proposed a reinforcement learning framework to enhance dialogue response generation probability estimation.

Result: User shift poses the highest risk to RPM generalization, and reinforcement learning is the most effective approach for improvement.

Conclusion: R-EMID and the proposed framework offer interpretable and actionable insights for enhancing RPM generalization.
...
TL;DR: MINPO is a neural framework for solving integro-differential equations (IDEs) efficiently, handling diverse nonlocal structures by learning operators directly.


<details>
  <summary>Details</summary>
Motivation: Existing methods for solving IDEs lack generalization across diverse nonlocal structures and are computationally expensive. MINPO addresses this gap by unifying nonlocal dynamics modeling.

Method: MINPO uses KANs or MLPs as encoders to learn nonlocal operators and their inverses, guided by a nonlocal consistency loss to ensure coherence between operators and solutions.

Result: MINPO outperforms classical techniques and neural-based strategies (e.g., A-PINN, fPINN) in accuracy and robustness across diverse kernel types, dimensionalities, and computational demands.

Conclusion: MINPO provides a unified, efficient framework for nonlocal systems, advancing beyond problem-specific solutions.
...
### [186] [Alzheimer's Disease Brain Network Mining](https://arxiv.org/abs/2512.17276)
*Alireza Moayedikia,Sara Fin*

Main category: cs.LG

TL;DR: MATCH-AD is a semi-supervised framework combining deep learning, graph-based label propagation, and optimal transport theory to diagnose Alzheimer's disease with high accuracy despite limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Clinical assessments for Alzheimer's disease are costly and invasive, limiting labeled data availability. MATCH-AD aims to leverage partially annotated neuroimaging data to reduce annotation burden while maintaining diagnostic accuracy.

Method: The framework integrates deep representation learning, graph-based label propagation, and Wasserstein distances to propagate diagnostic information from labeled to unlabeled data, quantifying disease progression between cognitive states.

Result: MATCH-AD achieves near-perfect diagnostic accuracy on a large dataset, outperforming baselines significantly, even under severe label scarcity. Theoretical guarantees for convergence and error bounds are provided.

Conclusion: The framework demonstrates that semi-supervised learning can unlock diagnostic potential in partially annotated neuroimaging data, reducing annotation burden while ensuring clinical accuracy.
...
### [187] [M2RU: Memristive Minion Recurrent Unit for Continual Learning at the Edge](https://arxiv.org/abs/2512.17299)
*Abdullah M. Zyarah,Dhireesha Kudithipudi*

Main category: cs.LG

TL;DR: M2RU is a mixed-signal architecture for efficient continual learning on edge platforms, achieving high energy efficiency and accuracy close to software baselines.


<details>
  <summary>Details</summary>
Motivation: Address the challenges of energy-intensive training and data movement in recurrent networks for embedded deployments.

Method: Introduces M2RU with weighted-bit streaming and experience replay for stable learning.

Result: Achieves 15 GOPS at 48.62 mW (312 GOPS/W), 29X energy efficiency improvement over CMOS, and 12.2-year operational lifetime.

Conclusion: M2RU is a scalable, energy-efficient solution for edge-level temporal intelligence.
...
TL;DR: The paper provides causal evidence that in-context learning (ICL) decomposes into two separable mechanisms: Task Schema and Binding, validated across multiple models.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanistic basis of ICL, contrasting prior monolithic views, and to explore how Task Schema and Binding operate independently.

Method: Activation patching experiments across 9 models (Transformer families and Mamba) to analyze Task Schema and Binding transfers.

Result: Key findings: Double dissociation between Schema (100% transfer) and Binding (62% transfer), inverse correlation of Schema reliance with prior knowledge, and architecture generality.

Conclusion: ICL operates via dissociable dual mechanisms, with Schema used when prior knowledge is absent. Practical implications include improved prompt engineering and reliability in ICL systems.
...
### [189] [Explanation Beyond Intuition: A Testable Criterion for Inherent Explainability](https://arxiv.org/abs/2512.17316)
*Michael Merry,Pat Riddle,Jim Warren*

Main category: cs.LG

TL;DR: The paper proposes a globally applicable criterion for inherent explainability in AI, using graph theory for structure-local and global explanations, and applies it to a clinical model.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a consistent definition or test for inherent explainability in XAI, which currently relies on metrics or intuition.

Method: Uses graph theory to decompose models into structure-local explanations (annotations) and recompose them into global explanations, forming a verifiable hypothesis-evidence structure.

Result: The criterion matches existing intuitions on explainability, justifies why some models are explainable (e.g., sparse neural networks) while others aren't (e.g., large regression models), and successfully applies to PREDICT, an inherently explainable clinical model.

Conclusion: The work formalizes explainability in XAI, provides a flexible yet rigorous test for regulators, and demonstrates its applicability with a real-world clinical model.
...
TL;DR: AdvJudge-Zero exposes vulnerabilities in reward models and LLM-as-a-Judge systems by using low-perplexity control tokens to flip binary evaluations, showing high false positive rates. Adversarial training can mitigate these risks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address vulnerabilities in judge systems used in post-training pipelines like RLHF, DPO, and RLAIF, where control tokens can manipulate evaluations.

Method: AdvJudge-Zero explores low-perplexity control tokens using next-token distribution and beam-search, revealing hidden-state perturbations concentrated in a low-rank "soft mode."

Result: Control tokens cause high false positive rates in judge models, particularly in math and reasoning benchmarks.

Conclusion: LoRA-based adversarial training on control-token-augmented examples effectively reduces false positives without compromising evaluation quality.
...
### [191] [Adaptive Graph Pruning with Sudden-Events Evaluation for Traffic Prediction using Online Semi-Decentralized ST-GNNs](https://arxiv.org/abs/2512.17352)
*Ivan Kralj,Lodovico Giaretta,Gordan Ježić,Ivana Podnar Žarko,Šarūnas Girdzijauskas*

Main category: cs.LG

TL;DR: The paper proposes an adaptive pruning algorithm for ST-GNNs to reduce communication overhead in smart mobility systems while preserving prediction accuracy, introducing a new metric (SEPA) for better traffic event responsiveness.


<details>
  <summary>Details</summary>
Motivation: ST-GNNs in smart mobility systems face high communication costs due to overlapping node features between distributed compute nodes (cloudlets). The goal is to minimize this overhead without losing accuracy.

Method: An adaptive pruning algorithm dynamically filters redundant neighbour features based on recent model performance. SEPA, a novel metric, measures responsiveness to traffic events.

Result: The algorithm reduces communication costs significantly while maintaining accuracy. SEPA reveals the importance of spatial connectivity in predicting irregular traffic, outperforming standard metrics.

Conclusion: The adaptive pruning algorithm effectively balances communication efficiency and prediction accuracy, proving useful in semi-decentralized settings for smart mobility systems.
...
### [192] [Adversarially Robust Detection of Harmful Online Content: A Computational Design Science Approach](https://arxiv.org/abs/2512.17367)
*Yidong Chai,Yi Liu,Mohammadreza Ebrahimi,Weifeng Li,Balaji Padmanabhan*

Main category: cs.LG

TL;DR: The paper introduces a novel framework (LLM-SGA) and detector (ARHOCD) to improve adversarial robustness and accuracy in detecting harmful online content, validated across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Addressing the vulnerability of ML models to adversarial attacks in detecting harmful content (e.g., hate speech, misinformation) on social media platforms.

Method: Proposes LLM-SGA for generalizability and instantiates ARHOCD with three novel components: ensemble of base detectors, dynamic weight assignment, and adversarial training.

Result: ARHOCD demonstrates strong generalizability and improved accuracy under adversarial conditions across hate speech, rumor, and extremist content datasets.

Conclusion: The study successfully enhances adversarial robustness and detection accuracy, addressing key limitations in existing research.
...
### [193] [DeepShare: Sharing ReLU Across Channels and Layers for Efficient Private Inference](https://arxiv.org/abs/2512.17398)
*Yonathan Bornfeld,Shai Avidan*

Main category: cs.LG

TL;DR: The paper introduces a new activation module for Private Inference (PI) by reducing DReLU operations, improving efficiency while maintaining privacy.


<details>
  <summary>Details</summary>
Motivation: The computational bottleneck in PI is the ReLU gate calculation, prompting efforts to minimize ReLU operations.

Method: Proposes using DReLU on prototype channels and replicating it to other channels, reducing DReLU operations.

Result: Drastically cuts DReLU operations in ResNet networks and achieves SOTA results in classification and segmentation.

Conclusion: The new activation module enhances PI efficiency and outperforms traditional methods in solving non-linear problems.
...
### [194] [meval: A Statistical Toolbox for Fine-Grained Model Performance Analysis](https://arxiv.org/abs/2512.17409)
*Dishantkumar Sutariya,Eike Petersen*

Main category: cs.LG

TL;DR: A statistical toolbox for rigorous subgroup performance analysis in machine learning, tailored for medical imaging, with case studies in skin lesion and chest X-ray classification.


<details>
  <summary>Details</summary>
Motivation: To address the need for statistically rigorous analysis of ML model performance across subgroups, especially in medical imaging, where identifying failure modes is critical.

Method: Develops a toolbox for selecting appropriate metrics, handling metric uncertainty, correcting multiple comparisons, and identifying interesting subgroups in intersectional analyses.

Result: The toolbox is demonstrated in two case studies: skin lesion malignancy classification (ISIC2020) and chest X-ray disease classification (MIMIC-CXR).

Conclusion: The toolbox enables rigorous subgroup performance analysis, helping practitioners identify and address disparities in ML models, particularly in medical imaging.
...
### [195] [Assessing Long-Term Electricity Market Design for Ambitious Decarbonization Targets using Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2512.17444)
*Javier Gonzalez-Ruiz,Carlos Rodriguez-Pardo,Iacopo Savelli,Alice Di Bella,Massimo Tavoni*

Main category: cs.LG

TL;DR: A multi-agent reinforcement learning model is proposed to analyze long-term electricity markets, focusing on decarbonization, competition, and policy impacts.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for advanced tools to design and evaluate long-term electricity markets for a carbon-free economy.

Method: Uses independent proximal policy optimization in a multi-agent setup for decentralized training, tested on the Italian electricity system under various scenarios.

Result: Results emphasize the importance of market design in decarbonization and avoiding price volatility.

Conclusion: The framework effectively assesses interactions between policy, market mechanisms, and participant responses in decarbonization pathways.
...
### [196] [Learning What to Write: Write-Gated KV for Efficient Long-Context Inference](https://arxiv.org/abs/2512.17452)
*Yen-Chieh Huang,Rui Fang,Ming-Syan Chen,Pi-Cheng Hsiu*

Main category: cs.LG

TL;DR: The paper introduces Write-Gated KV, a lightweight mechanism for efficient KV cache management in long-context LLM inference, reducing memory usage and speeding up inference with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Addresses the inefficiency of indiscriminate writing to persistent memory in long-context LLM inference, which leads to high attention complexity and KV cache growth.

Method: Formalizes KV cache management as a causal system of three primitives (KV Admission, Selection, Eviction) and instantiates KV Admission via Write-Gated KV, a mechanism predicting token utility before cache entry.

Result: Reduces memory usage by 46-57%, achieves 3.03-3.45× prefill and 1.89-2.56× decode speedups on Llama models with negligible accuracy loss, and remains compatible with FlashAttention and paged-KV systems.

Conclusion: Learning what to write (KV Admission) is a principled and practical solution for efficient long-context inference.
...
### [197] [A lightweight Spatial-Temporal Graph Neural Network for Long-term Time Series Forecasting](https://arxiv.org/abs/2512.17453)
*Henok Tenaw Moges,Deshendran Moodley*

Main category: cs.LG

TL;DR: Lite-STGNN is a lightweight spatial-temporal graph neural network for long-term multivariate forecasting, combining temporal decomposition with learnable sparse graph structures. It outperforms existing methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for efficient and accurate long-term multivariate forecasting by integrating temporal decomposition with spatial graph structures, aiming to improve interpretability and performance.

Method: The method uses trend-seasonal decomposition for temporal modeling and a spatial module with low-rank Top-K adjacency learning and conservative horizon-wise gating for spatial corrections.

Result: Lite-STGNN achieves state-of-the-art accuracy on four benchmarks for up to 720 steps, is parameter-efficient, and faster to train than transformer-based methods. Ablation studies confirm the spatial module's impact (4.6% improvement) and Top-K's locality enhancement (3.3%).

Conclusion: Lite-STGNN provides a compact, interpretable, and efficient framework for long-term multivariate forecasting, validated by its performance and ablation studies.
...
### [198] [Deep Learning-Based Surrogate Creep Modelling in Inconel 625: A High-Temperature Alloy Study](https://arxiv.org/abs/2512.17477)
*Shubham Das,Kaushal Singhania,Amit Sadhu,Suprabhat Das,Arghya Nandi*

Main category: cs.LG

TL;DR: Proposed deep learning surrogate models (BiLSTM-VAE and BiLSTM-Transformer) replace expensive ANSYS creep simulations for Inconel 625, offering fast and accurate predictions.


<details>
  <summary>Details</summary>
Motivation: Reduce computational cost and time of creep simulations in high-temperature alloys for aerospace and energy applications.

Method: Creep strain data generated in ANSYS was used to train BiLSTM-VAE for probabilistic predictions and BiLSTM-Transformer for deterministic accuracy.

Result: Models achieved fast predictions (seconds vs. 30-40 minutes) with high accuracy (RMSE, MAE, R²).

Conclusion: Deep learning surrogates enable rapid creep assessment, benefiting design optimization and structural health monitoring.
...
### [199] [SafeBench-Seq: A Homology-Clustered, CPU-Only Baseline for Protein Hazard Screening with Physicochemical/Composition Features and Cluster-Aware Confidence Intervals](https://arxiv.org/abs/2512.17527)
*Muhammad Haris Khan*

Main category: cs.LG

TL;DR: SafeBench-Seq is a reproducible, metadata-only benchmark for hazard screening in protein sequences, using homology control and interpretable features, evaluated under strict cluster-level holdouts.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a simple, reproducible baseline for sequence-level hazard screening in protein design, especially under homology control and for commodity CPUs.

Method: SafeBench-Seq uses public data (SafeProtein hazards and UniProt benigns) with interpretable features. It employs homology-clustering (≤40% identity) and cluster-level holdouts to simulate unseen threats. Evaluation includes discrimination metrics, calibration, and shortcut susceptibility tests.

Result: Random splits overestimate robustness compared to homology-clustered evaluation. Calibrated linear models show good calibration, while tree ensembles have slightly better Brier/ECE scores.

Conclusion: SafeBench-Seq provides a rigorous, CPU-compatible, and reproducible baseline for hazard screening without distributing hazardous sequences.
...
### [200] [NetworkFF: Unified Layer Optimization in Forward-Only Neural Networks](https://arxiv.org/abs/2512.17531)
*Salar Beigzad*

Main category: cs.LG

TL;DR: The paper introduces Collaborative Forward-Forward (CFF) learning to address inter-layer isolation in the Forward-Forward algorithm, improving performance through cooperative paradigms.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of conventional Forward-Forward implementations, which suffer from inter-layer isolation, hindering representational coordination and convergence efficiency.

Method: Extends Forward-Forward with Collaborative Forward-Forward (CFF), featuring fixed (F-CFF) and adaptive (A-CFF) inter-layer cooperation mechanisms and a collaborative goodness function.

Result: Demonstrates significant performance improvements on MNIST and Fashion-MNIST datasets compared to baseline Forward-Forward implementations.

Conclusion: Inter-layer collaboration enhances Forward-Forward learning, making it more suitable for neuromorphic computing and energy-constrained AI systems.
...
### [201] [Bayesian Optimisation: Which Constraints Matter?](https://arxiv.org/abs/2512.17569)
*Xietao Wang Lin,Juan Ungredda,Max Butler,James Town,Alma Rahat,Hemant Singh,Juergen Branke*

Main category: cs.LG

TL;DR: New Bayesian optimization variants of Knowledge Gradient for decoupled constraints outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of optimizing functions with decoupled constraints by focusing on relevant constraints.

Method: Propose Bayesian optimization variants of Knowledge Gradient for decoupled constraints.

Result: Empirical benchmarks show superiority over state-of-the-art methods.

Conclusion: The proposed methods effectively optimize functions with decoupled constraints by evaluating only relevant constraints.
...
### [202] [GreedySnake: Accelerating SSD-Offloaded LLM Training with Efficient Scheduling and Optimizer Step Overlapping](https://arxiv.org/abs/2512.17570)
*Yikang Yue,Yishu Yin,Xuehai Qian*

Main category: cs.LG

TL;DR: GreedySnake, an SSD-offloaded training system, uses vertical scheduling to improve LLM training throughput by executing all microbatches of a layer first, outperforming existing methods like ZeRO-Infinity.


<details>
  <summary>Details</summary>
Motivation: To make large language model (LLM) training more cost-effective by addressing I/O bottlenecks and improving throughput with smaller batch sizes.

Method: Introduces GreedySnake, which employs vertical scheduling (executing all microbatches of a layer before moving to the next) and overlaps optimization with the next forward pass.

Result: Achieves significant throughput improvements: 1.96x on 1 GPU and 1.93x on 4 GPUs for GPT-65B, and 2.53x on 1 GPU for GPT-175B.

Conclusion: GreedySnake's approach effectively mitigates I/O bottlenecks, offering a practical solution for efficient LLM training.
...
### [203] [Machine Learning for Static and Single-Event Dynamic Complex Network Analysis](https://arxiv.org/abs/2512.17577)
*Nikolaos Nakis*

Main category: cs.LG

TL;DR: The thesis focuses on developing unified algorithmic approaches for Graph Representation Learning, emphasizing Latent Space Models and structural-aware representations to capture complex network characteristics and dynamics.


<details>
  <summary>Details</summary>
Motivation: To create comprehensive and powerful network embeddings that eliminate heuristics and multi-stage processes, enabling better characterization of network structures and diverse graph analysis tasks.

Method: Utilizes Latent Space Models, particularly the Latent Distance Model, to capture network characteristics like homophily and transitivity, and aims for unified learning processes.

Result: Develops structural-aware representations for hierarchical network structures, community identification, extreme profiles, and impact dynamics in temporal networks.

Conclusion: The proposed methods offer a unified approach to network embeddings, enhancing the ability to analyze and characterize complex network structures effectively.
...
### [204] [Learning Safe Autonomous Driving Policies Using Predictive Safety Representations](https://arxiv.org/abs/2512.17586)
*Mahesh Keswani,Raunak Bhattacharyya*

Main category: cs.LG

TL;DR: SRPL improves reward-safety tradeoff in autonomous driving, shows effectiveness in real-world scenarios, and enhances robustness and generalization.


<details>
  <summary>Details</summary>
Motivation: Address the tension between performance optimization and strict safety requirements in SafeRL for autonomous driving.

Method: Uses SRPL framework with predictive safety representations, tested on WOMD and NuPlan datasets.

Result: Statistically significant improvements in success rate and cost reduction; effectiveness depends on policy optimizer and dataset. Enhances robustness to noise and generalization.

Conclusion: Predictive safety representations strengthen SafeRL for autonomous driving, with promising real-world applicability.
...
### [205] [Sharing Knowledge without Sharing Data: Stitches can improve ensembles of disjointly trained models](https://arxiv.org/abs/2512.17592)
*Arthur Guijt,Dirk Thierens,Ellen Kerkhof,Jan Wiersma,Tanja Alderliesten,Peter A. N. Bosman*

Main category: cs.LG

TL;DR: The paper explores asynchronous collaboration in federated learning by sharing pre-trained models and using stitching layers to combine them, achieving competitive performance while maintaining generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of federated learning, which requires synchronous collaboration, by investigating asynchronous methods where only trained models are shared.

Method: The proposed method involves training models individually on fragmented datasets, sharing them asynchronously, and combining them using stitching layers to merge intermediate representations.

Result: Results show that individual training performs similarly on a party's own data but poorly on others'. Stitching layers recover performance competitively while improving generalization.

Conclusion: Asynchronous collaboration with stitching layers is viable, offering competitive results without the need for synchronous training.
...
### [206] [A Unified Representation of Neural Networks Architectures](https://arxiv.org/abs/2512.17593)
*Christophe Prieur,Mircea Lazar,Bogdan Robu*

Main category: cs.LG

TL;DR: The paper analyzes neural networks (NNs) with infinite neurons and layers, introducing a unified DiPaNet representation for finite and infinite-dimensional NNs.


<details>
  <summary>Details</summary>
Motivation: The study aims to generalize existing continuous neural network representations and formalize approximation errors for infinite-width and deep residual NNs.

Method: The authors derive integral representations for single-layer and deep residual CNNs, revisit neural ODEs, and unify these approaches into DiPaNet using discretization techniques.

Result: They show that most finite and infinite-dimensional NNs architectures can be represented homogeneously via DiPaNet, with deterministic applicability to continuous matrix weight functions.

Conclusion: The DiPaNet framework offers a unified representation for NNs, with potential generalizations and applications discussed alongside comparisons to neural fields.
...
### [207] [A Systems-Theoretic View on the Convergence of Algorithms under Disturbances](https://arxiv.org/abs/2512.17598)
*Guner Dilsad Er,Sebastian Trimpe,Michael Muehlebach*

Main category: cs.LG

TL;DR: This paper extends convergence guarantees for algorithms operating amid disturbances, noise, and interconnections, providing stability bounds and convergence rates using Lyapunov theorems.


<details>
  <summary>Details</summary>
Motivation: Algorithms often operate in complex systems exposed to disturbances and noise. Existing convergence guarantees for isolated systems are insufficient for these scenarios.

Method: The authors leverage converse Lyapunov theorems to derive inequalities quantifying disturbance impacts.

Result: The results enable assessing disturbance effects on algorithmic performance in applications like distributed learning, machine learning generalization, and privacy.

Conclusion: The findings serve as a unifying tool for analyzing algorithms affected by noise, disturbances, and interconnected dynamical systems.
...
### [208] [More Consistent Accuracy PINN via Alternating Easy-Hard Training](https://arxiv.org/abs/2512.17607)
*Zhaoqian Gao,Min Yanga*

Main category: cs.LG

TL;DR: A hybrid training strategy combining hard and easy prioritization for PINNs improves accuracy and reliability across diverse PDEs.


<details>
  <summary>Details</summary>
Motivation: Existing training strategies for PINNs exhibit trade-offs and inconsistent performance across different PDE types.

Method: Develops a hybrid strategy using an alternating training algorithm to combine hard and easy prioritization.

Result: Achieves high accuracy (L2 errors ~O(10^-5) to O(10^-6)) on challenging PDEs, outperforming baseline methods.

Conclusion: The hybrid approach enhances PINN performance and robustness, offering reliable solutions across diverse problems.
...
### [209] [SCOPE: Sequential Causal Optimization of Process Interventions](https://arxiv.org/abs/2512.17629)
*Jakob De Moor,Hans Weytjens,Johannes De Smedt,Jochen De Weerdt*

Main category: cs.LG

TL;DR: SCOPE introduces a Prescriptive Process Monitoring approach for sequential interventions, outperforming existing methods by using backward induction and causal learners.


<details>
  <summary>Details</summary>
Motivation: Existing PresPM methods either focus on single interventions or treat multiple interventions independently, ignoring their interactions over time.

Method: SCOPE employs backward induction and causal learners to estimate intervention effects sequentially, using observational data directly.

Result: SCOPE outperforms state-of-the-art PresPM techniques in optimizing KPIs, validated on synthetic and semi-synthetic datasets.

Conclusion: SCOPE provides a reusable benchmark for sequential PresPM and addresses limitations of existing approaches.
...
### [210] [Trust-Region Adaptive Policy Optimization](https://arxiv.org/abs/2512.17636)
*Mingyu Su,Jian Guan,Yuxian Gu,Minlie Huang,Hongning Wang*

Main category: cs.LG

TL;DR: TRAPO introduces a hybrid framework combining SFT and RL, addressing inefficiencies in the traditional two-stage pipeline, and outperforms existing methods in improving LLMs' reasoning abilities.


<details>
  <summary>Details</summary>
Motivation: The traditional two-stage pipeline (SFT then RL) for improving LLMs' reasoning suffers from inefficiencies, such as rigid imitation suppressing exploration and inducing forgetting, limiting RL's potential.

Method: TRAPO interleaves SFT and RL within each training instance, optimizing SFT loss on expert prefixes and RL loss on the model's completions. It includes Trust-Region SFT (TrSFT) for stable updates and an adaptive prefix-selection mechanism for expert guidance.

Result: TRAPO consistently outperforms standard SFT, RL, and SFT-then-RL pipelines, as well as recent state-of-the-art approaches, on five mathematical reasoning benchmarks.

Conclusion: TRAPO establishes a strong new paradigm for enhancing the complex reasoning abilities of large language models by unifying external supervision and self-exploration.
...
### [211] [Estimating Spatially Resolved Radiation Fields Using Neural Networks](https://arxiv.org/abs/2512.17654)
*Felix Lehner,Pasquale Lombardo,Susana Castillo,Oliver Hupe,Marcus Magnor*

Main category: cs.LG

TL;DR: Analysis of neural networks for estimating scattered radiation fields in medical dosimetry, using synthetic datasets from Monte-Carlo simulations. Evaluated architectures include convolutional and fully connected networks.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve radiation protection dosimetry in medical fields like Interventional Radiology and Cardiology by leveraging neural networks to estimate spatial distribution of scattered radiation.

Method: Three synthetic datasets of increasing complexity were generated using Geant4-based Monte-Carlo simulations. Convolutional and fully connected neural network architectures were evaluated for reconstructing fluence and spectra distributions.

Result: The study identifies effective neural network design decisions for reconstructing radiation field distributions. Datasets and training pipeline are published as open source.

Conclusion: Neural networks show promise for accurately estimating scattered radiation fields, with findings and tools made publicly available for further research and application.
...
### [212] [Polyharmonic Cascade](https://arxiv.org/abs/2512.17671)
*Yuriy N. Bakhvalov*

Main category: cs.LG

TL;DR: The paper introduces the 'polyharmonic cascade,' a deep learning architecture using polyharmonic splines for approximating complex functions while maintaining smoothness and probabilistic interpretation. It proposes a non-gradient descent training method via global linear systems, ensuring efficient GPU execution and demonstrating fast learning on MNIST.


<details>
  <summary>Details</summary>
Motivation: The goal is to develop a deep learning model that approximates highly nonlinear functions with global smoothness and probabilistic interpretability, avoiding gradient descent limitations.

Method: The 'polyharmonic cascade' layers polyharmonic splines, trained by solving global linear systems for function values at fixed nodes. This ensures synchronized updates and efficient 2D matrix operations on GPUs.

Result: The model achieves fast learning without overfitting on the MNIST dataset, maintaining theoretical consistency and computational efficiency.

Conclusion: The polyharmonic cascade offers a scalable, interpretable alternative to traditional deep learning methods, with promising results on real-world tasks like MNIST.
...
### [213] [You Only Train Once: Differentiable Subset Selection for Omics Data](https://arxiv.org/abs/2512.17678)
*Daphné Chopard,Jorge da Silva Gonçalves,Irene Cannistraci,Thomas M. Sutter,Julia E. Vogt*

Main category: cs.LG

TL;DR: YOTO is an end-to-end framework for selecting compact and informative gene subsets from single-cell transcriptomic data, improving predictive performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current feature selection methods are often multi-stage or rely on post hoc attribution, leading to weak coupling between selection and prediction.

Method: YOTO uses a differentiable architecture to jointly identify discrete gene subsets and perform prediction, with a closed feedback loop refining both selection and prediction.

Result: YOTO outperforms state-of-the-art baselines on single-cell RNA-seq datasets, producing compact gene subsets.

Conclusion: YOTO advances biomarker discovery by providing sparse, end-to-end, multi-task gene subset selection with improved performance.
...
### [214] [Convergence Guarantees for Federated SARSA with Local Training and Heterogeneous Agents](https://arxiv.org/abs/2512.17688)
*Paul Mangold,Eloïse Berthier,Eric Moulines*

Main category: cs.LG

TL;DR: The paper presents a theoretical analysis of Federated SARSA (FedSARSA) with linear function approximation, proving convergence guarantees despite heterogeneity in local transitions and rewards. It introduces a novel multi-step error expansion for SARSA and shows FedSARSA's linear speed-up with the number of agents.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenges of heterogeneity in federated reinforcement learning, providing theoretical guarantees for FedSARSA's convergence and efficiency.

Method: The authors develop a new exact multi-step error expansion for SARSA and analyze FedSARSA's performance with local training, accounting for heterogeneity in transitions and rewards.

Result: The paper establishes convergence guarantees for FedSARSA, demonstrating its linear speed-up with the number of agents, supported by numerical experiments.

Conclusion: FedSARSA is theoretically validated as an efficient method for federated reinforcement learning, with convergence and scalability proven under heterogeneous conditions.
...
### [215] [Spatially-informed transformers: Injecting geostatistical covariance biases into self-attention for spatio-temporal forecasting](https://arxiv.org/abs/2512.17696)
*Yuri Calleo*

Main category: cs.LG

TL;DR: The paper proposes a spatially-informed transformer combining geostatistical rigor with deep learning flexibility to model high-dimensional spatio-temporal processes, outperforming graph neural networks and ensuring well-calibrated probabilistic forecasts.


<details>
  <summary>Details</summary>
Motivation: Address the gap between classical geostatistics (rigorous but computationally heavy) and deep learning (flexible but lacks geometric inductive bias) for spatio-temporal modeling.

Method: Introduces a hybrid transformer architecture with a learnable covariance kernel in self-attention, decomposing attention into stationary physical priors and non-stationary data-driven residuals.

Result: The method recovers true spatial decay parameters ('Deep Variography') and outperforms state-of-the-art graph neural networks in synthetic and real-world benchmarks.

Conclusion: The approach effectively bridges physics-aware modeling and data-driven learning, offering superior predictive accuracy and well-calibrated probabilistic forecasts.
...
### [216] [Mitigating Forgetting in Low Rank Adaptation](https://arxiv.org/abs/2512.17720)
*Joanna Sliwa,Frank Schneider,Philipp Hennig,Jose Miguel Hernandez-Lobato*

Main category: cs.LG

TL;DR: LaLoRA is a lightweight, weight-space regularization technique that uses Laplace approximation on LoRA weights to prevent catastrophic forgetting while enabling efficient target-domain learning.


<details>
  <summary>Details</summary>
Motivation: To address the issue of catastrophic forgetting in large pre-trained models when using parameter-efficient fine-tuning methods like LoRA.

Method: LaLoRA applies a Laplace approximation to LoRA weights, estimating parameter confidence and constraining updates in high-curvature directions to preserve prior knowledge.

Result: Improved learning-forgetting trade-off during fine-tuning for mathematical reasoning, controllable via regularization strength, and robust across hyperparameters.

Conclusion: LaLoRA effectively preserves prior knowledge while enabling efficient target-domain learning, as demonstrated in fine-tuning a Llama model.
...
### [217] [Can You Hear Me Now? A Benchmark for Long-Range Graph Propagation](https://arxiv.org/abs/2512.17762)
*Luca Miglior,Matteo Tolloso,Alessio Gravina,Davide Bacciu*

Main category: cs.LG

TL;DR: ECHO is a novel benchmark for evaluating GNNs' ability to handle long-range interactions, featuring synthetic tasks and real-world datasets, revealing performance gaps in existing models.


<details>
  <summary>Details</summary>
Motivation: The challenge of capturing long-range interactions in GNNs is critical for scientific applications, but remains unresolved. ECHO aims to systematically assess GNNs' capabilities in this area.

Method: ECHO introduces synthetic graph tasks (shortest paths, node eccentricity, graph diameter) and real-world datasets (ECHO-Charge, ECHO-Energy) to test GNNs on diverse, challenging topologies.

Result: Benchmarking popular GNN architectures shows clear performance gaps, underscoring the difficulty of true long-range propagation and identifying design choices that can mitigate limitations.

Conclusion: ECHO sets a new standard for evaluating long-range information propagation in GNNs and highlights its importance for AI in science.
...
TL;DR: The paper introduces a calibratable disambiguation loss (CDL) to improve accuracy and calibration in multi-instance partial-label learning (MIPL).


<details>
  <summary>Details</summary>
Motivation: Existing MIPL methods often have poor calibration, reducing classifier reliability.

Method: A plug-and-play CDL is proposed with two versions: one using candidate label probabilities and another integrating candidate and non-candidate label probabilities.

Result: CDL improves classification and calibration, supported by theoretical and experimental validation.

Conclusion: CDL outperforms conventional disambiguation losses and can be integrated into existing MIPL and PLL frameworks.
...
TL;DR: This paper explores the complementary roles of ID and text features in Sequential Recommendation (SR) models, proposing a simple ensembling method that outperforms complex fusion strategies.


<details>
  <summary>Details</summary>
Motivation: The study addresses the lack of understanding of how ID and modality features complement each other in SR models, aiming to improve performance without complex fusion techniques.

Method: The authors propose a new SR method that independently trains ID- and text-based models and combines them using a simple ensembling strategy.

Result: The method outperforms competitive SR baselines, demonstrating that both ID and text features are essential for state-of-the-art performance.

Conclusion: Complex fusion architectures are unnecessary; instead, preserving the complementarity of ID and text features through simple ensembling achieves superior results.
...
TL;DR: The paper examines score-based diffusion models, highlighting challenges with non-log-concave target distributions and proposing a reweighting mechanism using Wasserstein-Fisher-Rao geometries for improved sampling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current diffusion models, which struggle with nonconvex or multimodal landscapes, by leveraging information geometry to enhance sampling efficiency.

Method: The authors introduce explicit correction terms and implement them via weighted stochastic differential equations using the Feynman-Kac representation, focusing on Wasserstein-Fisher-Rao geometries.

Result: The work provides a rigorous preliminary investigation of WFR-based sampling dynamics, clarifying their geometric and operator-theoretic structure.

Conclusion: The study lays the foundation for future theoretical and algorithmic advancements in diffusion-based generative modeling by exploring WFR-based sampling dynamics.
...
TL;DR: The paper introduces RRFF-FEM, a method combining regularized random Fourier features with finite elements for robust and efficient operator learning from noisy data.


<details>
  <summary>Details</summary>
Motivation: To address computational challenges and noise sensitivity in kernel-based operator learning, ensuring accuracy and efficiency.

Method: Uses random features from Student's t distributions and Tikhonov regularization, coupled with finite element reconstruction.

Result: Proves system well-conditioning with bounds on singular values. Demonstrates robustness to noise and reduced training time in PDE benchmarks.

Conclusion: RRFF-FEM outperforms unregularized methods in noise handling and efficiency, matching kernel and neural operator accuracy.
