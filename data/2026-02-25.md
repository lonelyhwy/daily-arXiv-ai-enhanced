...
TL;DR: AI-enabled framework using ICE videos for arrhythmia localization achieves 66.2% accuracy, promising faster interventions.


<details>
  <summary>Details</summary>
Motivation: High-density mapping and preoperative imaging are time-consuming; AI can streamline arrhythmia localization using ICE, a routine procedure component.

Method: Developed a 3D CNN to classify ICE videos into normal sinus rhythm, left-sided, or right-sided arrhythmias.

Result: Model achieved 66.2% accuracy in ten-fold cross-validation, outperforming the 33.3% random baseline.

Conclusion: ICE videos with deep learning show clinical potential for automated arrhythmia localization, with future work focused on improving model robustness.
...
TL;DR: OTPrune accelerates MLLM inference by pruning redundant visual tokens using optimal transport to align distributions, improving efficiency without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: Multi-modal large language models (MLLMs) face high inference costs due to redundant visual tokens, prompting the need for efficient pruning methods.

Method: Proposes OTPrune, a training-free framework that aligns token distributions via optimal transport, minimizing the 2-Wasserstein distance to preserve diversity and representativeness.

Result: OTPrune achieves superior performance-efficiency tradeoffs compared to state-of-the-art methods on wider benchmarks.

Conclusion: OTPrune provides a principled, efficient, and stable solution for visual token pruning in MLLMs, validated by comprehensive experiments.
...
TL;DR: A framework combining chart de-rendering, automated analysis, and iterative improvement delivers actionable feedback on visualization design, addressing flaws via vision-language reasoning and established principles.


<details>
  <summary>Details</summary>
Motivation: Existing tools for detecting visualization errors lack context and actionable suggestions, while general-purpose LLMs are unreliable for quality feedback.

Method: The system de-renders charts, analyzes flaws using vision-language reasoning, and proposes design improvements supported by visualization principles.

Result: Evaluated on 1,000 charts, the system generated 10,452 recommendations categorized into 10 coherent design flaw types.

Conclusion: The framework demonstrates the potential of LLM-driven systems for structured, principle-based feedback, enhancing visualization quality and literacy.
...
TL;DR: N4MC is a novel 4D neural compression framework for time-varying mesh sequences, outperforming existing methods by leveraging temporal redundancy and transformer-based interpolation.


<details>
  <summary>Details</summary>
Motivation: To address inefficiencies in prior neural mesh compression methods that treat frames independently, N4MC aims to exploit temporal redundancy like 2D video codecs.

Method: Converts mesh frames into 4D tensors for uniform representation, uses an auto-decoder for redundancy removal, and employs a transformer-based model for interpolation.

Result: N4MC achieves superior rate-distortion performance and enables real-time decoding of 4D mesh sequences.

Conclusion: The framework successfully leverages temporal coherence and advances neural compression for dynamic mesh sequences.
...
TL;DR: GSNR improves image reconstruction by incorporating null-space structure via graph Laplacian, outperforming baselines by up to 4.3 dB.


<details>
  <summary>Details</summary>
Motivation: Addressing bias in inverse imaging problems by constraining null-space components.

Method: Proposes GSNR, a null-restricted Laplacian for smooth null-space representation using spectral graph modes.

Result: Consistent improvements up to 4.3 dB in PSNR across multiple inverse problem scenarios.

Conclusion: GSNR effectively enhances reconstruction quality by embedding meaningful null-space constraints.
...
TL;DR: Introduces a framework for tracing circuits in VLMs to understand multimodal reasoning, revealing causal and controllable feature circuits.


<details>
  <summary>Details</summary>
Motivation: VLMs are powerful but opaque, motivating the need for transparency in understanding their multimodal reasoning.

Method: Uses transcoders, attribution graphs, and attention-based methods to analyze hierarchical integration of visual and semantic concepts.

Result: Identifies distinct visual feature circuits for mathematical reasoning and cross-modal associations, validated via feature steering and patching.

Conclusion: The framework provides causal insights into VLMs, paving the way for more explainable and reliable models.
...
TL;DR: An end-to-end pipeline converts drone-captured video into high-fidelity 3D reconstructions with low latency, integrating RTMP streaming, sensor fusion, pose estimation, and 3DGS optimization for AR/VR applications.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored integration of 3D Gaussian Splatting (3DGS) into UAV-based real-time reconstruction systems and improve upon NeRF-based methods.

Method: Combines live video acquisition via RTMP streaming, synchronized sensor fusion, camera pose estimation, and 3DGS optimization for continuous model updates.

Result: Achieves competitive visual fidelity, higher rendering performance, and reduced latency compared to NeRF-based approaches, with reconstruction quality within 4-7% of offline references.

Conclusion: The proposed system is suitable for real-time, scalable augmented perception from aerial platforms.
...
TL;DR: BiRQA is a fast, robust FR IQA model using bidirectional multiscale features and adversarial training, outperforming SOTA in speed and resilience.


<details>
  <summary>Details</summary>
Motivation: Current FR IQA metrics are slow and vulnerable to adversarial attacks, limiting their practical use.

Method: BiRQA processes four fast features in a bidirectional pyramid, uses bottom-up attention and top-down cross-gating, and employs Anchored Adversarial Training for robustness.

Result: BiRQA outperforms SOTA on benchmarks (~3x faster) and improves robustness (SROCC 0.60-0.84 under attacks).

Conclusion: BiRQA uniquely combines accuracy, speed, and adversarial resilience, making it practical for FR IQA tasks.
...
TL;DR: 3DSPA is an automated framework for evaluating video realism by integrating 3D point trajectories, depth cues, and semantic features, outperforming manual methods and aligning with human judgments.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of video realism relies on manual processes or limited datasets. An automated method is needed to assess semantics, 3D structure, and physical plausibility efficiently.

Method: 3DSPA uses a 3D spatiotemporal point autoencoder to combine 3D point trajectories, depth cues, and DINO semantic features for evaluating video realism.

Result: 3DSPA detects physical law violations, identifies motion artifacts, and aligns closely with human judgments of video quality and realism.

Conclusion: Integrating 3D semantics into trajectory-based representations provides a robust benchmark for generative video models and implicitly captures physical violations.
...
TL;DR: The paper introduces a 3D aesthetic field for efficient viewpoint suggestion using sparse captures, avoiding costly reinforcement learning searches.


<details>
  <summary>Details</summary>
Motivation: Existing methods either lack scene geometry understanding or rely on dense captures and expensive RL searches, limiting their practicality.

Method: A 3D Gaussian Splatting network distills 2D aesthetic knowledge into 3D space, enabling viewpoint predictions from sparse inputs. A two-stage search pipeline refines viewpoints efficiently.

Result: The method outperforms existing approaches in suggesting aesthetically superior viewpoints.

Conclusion: This work advances 3D-aware aesthetic modeling with a scalable and efficient solution.
...
TL;DR: CLIPoint3D leverages CLIP for few-shot unsupervised 3D point cloud domain adaptation, achieving efficiency and accuracy gains over baselines.


<details>
  <summary>Details</summary>
Motivation: Addresses fragility of VLMs under domain shifts, especially synthetic-to-real-world point clouds, while avoiding heavy trainable encoders.

Method: Projects 3D samples into depth maps, uses frozen CLIP backbone with prompt tuning, lightweight 3D encoder, and entropy-guided view sampling.

Result: Achieves 3-16% accuracy gains on benchmarks like PointDA-10 and GraspNetPC-10 over baselines.

Conclusion: CLIPoint3D effectively bridges source-target gaps with efficiency, outperforming existing methods.
...
TL;DR: Proposes SimLBR, an efficient method for detecting AI-generated images by learning a tight decision boundary around real images. It improves cross-generator generalization and introduces reliability-oriented evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Detecting AI-generated images is crucial due to rapid advancements in generative models, but current methods overfit and fail under distribution shifts.

Method: Uses Latent Blending Regularization (LBR) to learn a tight decision boundary around real images, treating fakes as a sink class.

Result: Achieves significant improvements (+24.85% accuracy, +69.62% recall) on Chameleon benchmark and trains faster than existing methods.

Conclusion: SimLBR is a robust, efficient solution for fake image detection, with proposed metrics for better reliability assessment. Code will be publicly available.
...
TL;DR: The paper introduces a method using latent diffusion models to reconstruct high-quality images from sparse, noisy photon detections in SPAD sensors, achieving superior perceptual quality compared to existing techniques.


<details>
  <summary>Details</summary>
Motivation: High-quality imaging from few photon detections is challenging due to noise and sparse data. SPAD sensors offer potential but require advanced restoration techniques to handle extreme noise statistics.

Method: The approach adapts large text-to-image latent diffusion models to the photon-limited domain, incorporating Bernoulli photon statistics and spatio-temporal reasoning for burst-level image reconstruction.

Result: The method outperforms classical and learning-based baselines in perceptual quality, validated on synthetic benchmarks and real-world datasets, including color SPAD bursts and Deforming (XD) videos.

Conclusion: Adapting large generative priors to extreme photon-limited sensing shows promise, enabling photometrically faithful and perceptually pleasing reconstructions even under challenging conditions.
...
TL;DR: MedCLIPSeg adapts CLIP for medical image segmentation by combining patch-level embeddings and probabilistic cross-modal attention, improving data efficiency and domain generalizability.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges in medical image segmentation like limited annotations, ambiguous features, and domain shifts.

Method: Uses patch-level CLIP embeddings with probabilistic cross-modal attention and a soft patch-level contrastive loss.

Result: Outperforms prior methods in accuracy, efficiency, and robustness across 16 datasets.

Conclusion: Demonstrates the potential of probabilistic vision-language modeling for text-driven medical image segmentation.
...
TL;DR: SceMoS uses structured 2D scene representations (BEV images and local heightmaps) to synthesize 3D human motion, achieving efficiency and realism without full 3D data.


<details>
  <summary>Details</summary>
Motivation: Current methods for synthesizing text-driven 3D human motion rely on computationally expensive 3D data, motivating a need for efficient alternatives like 2D representations.

Method: SceMoS separates global planning (using BEV images with DINOv2 features) from local execution (using 2D heightmaps via a VQ-VAE), embedding physics into a discrete vocabulary.

Result: SceMoS achieves state-of-the-art motion realism and contact accuracy on TRUMANS benchmark, reducing scene encoding parameters by over 50%.

Conclusion: 2D scene cues effectively ground 3D human-scene interactions, balancing efficiency and fidelity.
...
TL;DR: Proposes Hyperbolic Flow Matching (HFM) to address Euclidean FM's path entanglement by leveraging hyperbolic geometry for cross-modal few-shot adaptation.


<details>
  <summary>Details</summary>
Motivation: Euclidean FM's flat geometry fails to handle diverse feature distributions, causing path entanglement.

Method: HFM uses centripetal hyperbolic alignment and path-decoupled objectives to structure transport on the Lorentz manifold.

Result: Extensive benchmarks show HFM outperforms Euclidean FM, achieving state-of-the-art results.

Conclusion: HFM effectively decouples trajectories and improves cross-modal adaptation.
...
TL;DR: PipStereo improves edge deployment of stereo matching by pruning redundant updates, embedding depth priors without extra encoders, and optimizing RNNs, achieving real-time performance with high accuracy.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of deploying iterative stereo matching on edge hardware due to its reliance on RNNs, which are computationally heavy.

Method: The authors propose progressive iteration pruning, a monocular prior transfer framework, and FlashGRU—an optimized RNN operator leveraging sparsity and I/O-conscious design.

Result: PipStereo achieves a 7.28× speedup, significant memory reductions, and real-time processing (75ms on Jetson Orin NX, 19ms on RTX 4090) while maintaining high accuracy.

Conclusion: PipStereo enables real-time, high-fidelity stereo matching on edge hardware, outperforming existing methods in speed, efficiency, and accuracy.
...
TL;DR: The paper proposes a LEarnable Stage-Aware (LESA) predictor framework to address the computational challenges of Diffusion Transformers (DiTs). It uses a two-stage training approach with a KAN network and achieves significant acceleration while maintaining high-quality generation.


<details>
  <summary>Details</summary>
Motivation: High computational demands of DiTs hinder practical deployment, and existing feature caching methods fail to adapt to the diffusion process dynamics, leading to quality degradation.

Method: Introduces LESA, a two-stage training framework leveraging a KAN network and a multi-stage, multi-expert architecture for precise feature forecasting.

Result: Achieves up to 6.25x speedup with quality improvements (e.g., 24.7% PSNR gain) on benchmarks like FLUX.1-dev, Qwen-Image, and HunyuanVideo.

Conclusion: LESA provides effective acceleration and generalization for DiTs in text-to-image and text-to-video tasks.
...
TL;DR: The paper explores how visual systems understand affordance by combining geometric and interaction perception, demonstrating their composability in Visual Foundation Models (VFMs) like DINO and Flux.


<details>
  <summary>Details</summary>
Motivation: The study aims to uncover the foundational components of affordance understanding in visual systems, emphasizing the interplay between geometric and interaction perception.

Method: The authors systematically probe VFMs, specifically analyzing DINO for geometric structures and Flux for interaction maps, and fuse these features in a zero-shot manner.

Result: The fusion of DINO's geometric prototypes and Flux's interaction maps achieves affordance estimation competitive with weakly-supervised methods, confirming their composability.

Conclusion: Geometric and interaction perception are identified as fundamental building blocks of affordance understanding in VFMs, offering insights into how perception grounds action.
...
TL;DR: The paper introduces a causal inference-based explanation model for medical image segmentation, offering more faithful explanations than existing methods and revealing heterogeneity in model strategies.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of trustworthiness and underexplored explainability in deep learning-based medical image segmentation models.

Method: Uses causal inference and backpropagates average treatment effect (ATE) to quantify input and network component influence on segmentation.

Result: Demonstrated superior faithfulness of explanations over existing methods and revealed perceptual strategy heterogeneity in models.

Conclusion: The method provides valuable insights for optimizing segmentation models and enhances their trustworthiness.
...
TL;DR: The paper explores how diffusion-based inpainting artifacts impact language generation in vision-language models by comparing captions from original vs. reconstructed images.


<details>
  <summary>Details</summary>
Motivation: To understand the effects of visual reconstruction fidelity on downstream language generation tasks.

Method: A two-stage diagnostic setup: masked image regions are inpainted and then fed to captioning models for controlled comparisons.

Result: Pixel-level and perceptual reconstruction metrics correlate with lexical and semantic caption quality. Inpainting artifacts cause systematic changes in model behavior.

Conclusion: The study offers a practical framework for assessing how visual reconstruction quality affects multimodal language generation.
...
TL;DR: A lightweight vision-language framework integrates UI and semantic data for app rating prediction, outperforming text-only or UI-only models with strong metrics.


<details>
  <summary>Details</summary>
Motivation: Existing app rating models neglect the combined potential of UI and semantic features, limiting prediction accuracy and practical utility.

Method: Combines MobileNetV3 for UI visuals and DistilBERT for text, fused via gated Swish activations, followed by MLP regression.

Result: Achieves MAE=0.1060, RMSE=0.1433, MSE=0.0205, R2=0.8529, Pearson=0.9251, proving multimodal fusion's superiority.

Conclusion: The framework enhances app rating predictions, benefits developers/users, and suits edge deployment sustainably.
...
TL;DR: PFGNet is a fully convolutional framework for spatiotemporal predictive learning that dynamically adapts receptive fields using pixel-wise frequency-guided gating, achieving efficient and high-performance forecasting.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of fixed receptive fields in pure convolutional models for spatiotemporal predictive learning, PFGNet introduces adaptive modulation inspired by biological processes.

Method: PFGNet uses Peripheral Frequency Gating (PFG) blocks to extract localized spectral cues and adaptively fuse multi-scale responses, employing separable 1D convolutions for efficiency.

Result: PFGNet achieves state-of-the-art or near-state-of-the-art performance on datasets like Moving MNIST and Human3.6M with fewer parameters and FLOPs.

Conclusion: The proposed PFGNet framework offers a structure-aware, efficient alternative to recurrent or attention-based models for spatiotemporal forecasting.
...
TL;DR: A multi-agent framework combining deep learning (DL) and vision-language models (VLMs) was developed to improve CFU detection accuracy in pharmaceutical manufacturing, reducing human verification by 85% and ensuring regulatory compliance.


<details>
  <summary>Details</summary>
Motivation: Manual CFU counting is labor-intensive and error-prone, while existing DL methods struggle with sample quality variations and artifacts.

Method: YOLOv5, YOLOv7, and YOLOv8 were evaluated but found inadequate. A custom Detectron2 model achieved high accuracy but faltered with outliers. The solution integrates DL and VLMs for classification and counting, with expert feedback for continuous improvement.

Result: Initial DL automation reduced human verification by 50%, rising to 85% with VLM integration, offering significant operational savings.

Conclusion: The system provides a scalable, auditable, and regulation-ready solution for microbiological quality control, advancing biopharmaceutical automation.
...
TL;DR: The paper addresses the robustness issue in Spiking Neural Networks (SNNs) by identifying threshold-neighboring neurons as a vulnerability. It proposes Threshold Guarding Optimization (TGO) to mitigate adversarial attacks and improve robustness.


<details>
  <summary>Details</summary>
Motivation: SNNs are energy-efficient but suffer from robustness limitations in adversarial environments. The study aims to enhance SNN reliability by targeting vulnerable neurons.

Method: The TGO method includes: 1) adding constraints to loss functions to distance membrane potentials from thresholds, reducing adversarial attack potential, and 2) using noisy spiking neurons to shift firing mechanisms from deterministic to probabilistic.

Result: Experiments show TGO significantly boosts SNN robustness in adversarial scenarios, lowering attack vulnerability and state-flipping risks.

Conclusion: TGO advances SNN reliability, offering a pathway for secure neuromorphic computing in real-world applications.
...
TL;DR: The paper introduces a Finite Primitive Basis Theorem, proving that any computational imaging forward model can be approximated by a DAG using 11 canonical primitives. It provides an algorithm for this construction and validates the approach on various modalities.


<details>
  <summary>Details</summary>
Motivation: Traditional imaging models are monolithic and modality-specific. The goal is to unify these models using a minimal set of primitives, enabling a more flexible and modular approach to computational imaging.

Method: The paper defines a broad class of imaging operators (Cimg) and proves they can be represented by a DAG of 11 primitives. It provides a constructive algorithm to generate such DAGs and validates the approach empirically.

Result: Empirical validation on 31 linear modalities confirms an error below 0.01 with at most 5 nodes and depth 5. Nonlinear modalities are also successfully decomposed into DAGs.

Conclusion: The results support the Physics World Model framework, offering a unified, modular approach to computational imaging forward models.
...
TL;DR: The paper proposes a CAD-prompted segmentation framework using multi-view CAD renderings to overcome limitations of verbal or image-based prompts in industrial settings.


<details>
  <summary>Details</summary>
Motivation: Verbal and image-based prompts struggle with uncommon or hard-to-describe objects in manufacturing, where appearance varies but geometry remains consistent.

Method: Uses canonical multi-view CAD renderings as prompts, trained on synthetic data from diverse viewpoints and contexts.

Result: Enables robust single-stage mask prediction without relying on language or appearance.

Conclusion: CAD-prompted segmentation effectively handles objects where traditional prompting methods fail.
...
TL;DR: WildGHand is an optimization-based framework for high-fidelity 3D hand reconstruction in challenging real-world settings, featuring perturbation disentanglement and optimization strategies.


<details>
  <summary>Details</summary>
Motivation: Existing 3D hand reconstruction methods falter in uncontrolled environments due to perturbations like hand-object interactions and motion blur.

Method: WildGHand uses dynamic perturbation disentanglement and perturbation-aware optimization with anisotropic masks.

Result: Achieves state-of-the-art performance with significant improvements (e.g., 15.8% PSNR gain and 23.1% LPIPS reduction).

Conclusion: WildGHand excels in reconstructing 3D hand avatars from in-the-wild videos, supported by a new dataset.
...
TL;DR: AIForge-Doc introduces a benchmark for AI-based document forgery, revealing significant weaknesses in current detection methods against AI-generated tampering.


<details>
  <summary>Details</summary>
Motivation: To address the gap in detecting AI-forged document fraud, as existing datasets rely on traditional editing tools.

Method: Systematically forged numeric fields in real-world documents using AI inpainting APIs, creating 4,061 tampered images with pixel-level annotations.

Result: Existing detectors (TruFor, DocTamper, GPT-4o) perform poorly, confirming AI-forged values are hard to distinguish.

Conclusion: AIForge-Doc highlights an unsolved challenge in document forensics, exposing vulnerabilities in current detection methods.
...
TL;DR: The paper introduces the Interactive Enhanced Driving Dataset (IEDD) to address limitations in interactive scenario data for autonomous driving, along with a scalable pipeline and benchmark results for VLMs.


<details>
  <summary>Details</summary>
Motivation: The development of Vision-Language-Action models is hindered by sparse interactive scenarios and poor multimodal alignment in existing data.

Method: Proposes IEDD, a scalable pipeline to mine interactive segments from driving data, and constructs the IEDD-VQA dataset with synthetic BEV videos for aligned semantic actions and language.

Result: Benchmark results show the dataset's effectiveness in evaluating and fine-tuning the reasoning capabilities of autonomous driving models.

Conclusion: IEDD provides a valuable resource for advancing interactive capabilities in autonomous driving by addressing data limitations.
...
TL;DR: MVLAD-AD is a masked vision-language-action diffusion model for autonomous driving, addressing efficiency, precision, and explainability challenges in LLMs/VLMs. It uses discrete action tokens and geometry-aware embeddings to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs/VLMs for autonomous driving suffer from slow inference, imprecise actions, and lack of explainability due to autoregressive or general-purpose diffusion approaches.

Method: MVLAD-AD introduces discrete action tokenization, geometry-aware embedding learning, and action-priority decoding to improve planning efficiency and semantic explainability.

Result: Experiments on nuScenes show MVLAD-AD achieves superior efficiency and planning precision compared to autoregressive and diffusion baselines.

Conclusion: MVLAD-AD bridges the gap between efficient planning and semantic explainability, outperforming state-of-the-art methods in autonomous driving.
...
TL;DR: PropFly introduces a training pipeline for propagation-based video editing using on-the-fly supervision from pre-trained video diffusion models, eliminating the need for paired datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional propagation-based video editing requires costly paired video datasets. PropFly avoids this by leveraging pre-trained models.

Method: PropFly synthesizes source and edited latents dynamically using CFG scales and trains a model with Guidance-Modulated Flow Matching loss.

Result: PropFly outperforms state-of-the-art methods, producing high-quality, temporally consistent video edits.

Conclusion: The PropFly pipeline offers a scalable and efficient solution for video editing without paired datasets.
...
TL;DR: The paper addresses the inability of standard Structure-from-Motion (SfM) pipelines to handle long-term environmental monitoring due to temporal appearance changes. It proposes a joint SfM approach combining handcrafted and learned features for robust cross-session correspondence.


<details>
  <summary>Details</summary>
Motivation: Existing SfM pipelines fail in long-term monitoring scenarios like coral reef surveys due to assumptions of near-simultaneous capture and limited appearance change. This work aims to solve this limitation.

Method: The proposed method enforces cross-session correspondences within a joint SfM reconstruction, using handcrafted and learned visual features. It also scales the approach by restricting expensive learned feature matching to likely pairs identified via visual place recognition.

Result: The method successfully reconstructs coherent 3D models from imagery captured years apart, outperforming standard independent and joint SfM pipelines on coral reef datasets with significant real-world change.

Conclusion: The approach enables reliable long-term environmental monitoring by addressing temporal appearance challenges in SfM, with scalability improvements via selective feature matching.
...
TL;DR: The paper introduces InterFormer, an end-to-end transformer model addressing limitations in parsing hands and active objects for egocentric human-environment interactions. It includes Dynamic Query Generator, Dual-context Feature Selector, and Conditional Co-occurrence loss components, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Accurate parsing of hands and active objects is crucial for embodied agents, but existing transformer-based methods suffer from adaptability issues, noise from irrelevant content, and inconsistent predictions.

Method: Proposes InterFormer with Dynamic Query Generator for interaction-aware queries, Dual-context Feature Selector to suppress noise, and Conditional Co-occurrence loss for physical consistency.

Result: Achieves state-of-the-art performance on EgoHOS and mini-HOI4D datasets, showing strong generalization.

Conclusion: InterFormer effectively addresses key limitations in parsing hands and active objects, demonstrating superior performance and adaptability.
...
TL;DR: The paper introduces VAGNet, a framework for 3D affordance grounding using dynamic video cues, outperforming static-based methods.


<details>
  <summary>Details</summary>
Motivation: Affordances are inherently dynamic, yet existing methods rely on static cues, leading to inaccurate localization of contact regions.

Method: Proposes VAGNet, aligning video-derived interaction cues with 3D structure, and introduces the PVAD dataset for functional supervision.

Result: VAGNet achieves state-of-the-art performance on PVAD, significantly outperforming static-based baselines.

Conclusion: Dynamic video-guided grounding resolves ambiguities in affordance localization, with potential applications in embodied reasoning.
...
TL;DR: The paper introduces an interpretable framework (IPOW) for open-world object detection (OWOD) using a Concept Decomposition Model (CDM) to mitigate known-unknown confusion and improve prediction reliability.


<details>
  <summary>Details</summary>
Motivation: Existing OWOD methods lack interpretability, leading to confusion between known and unknown objects. This paper aims to make OWOD interpretable to truly 'know the unknown'.

Method: Proposes IPOW with CDM, decomposing RoI features into discriminative, shared, and background concepts. Uses Concept-Guided Rectification (CGR) to resolve known-unknown confusion.

Result: IPOW significantly improves unknown recall, reduces confusion, and provides concept-level interpretability for known and unknown predictions.

Conclusion: The interpretable framework (IPOW) with CDM and CGR effectively addresses known-unknown confusion in OWOD, enhancing both performance and interpretability.
...
TL;DR: The paper introduces RecoverMark, a watermarking framework that robustly localizes manipulations, recovers content, and verifies ownership, addressing vulnerabilities in existing fragile and dual-watermark methods.


<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated content threatens visual integrity and intellectual property, exposing weaknesses in current fragile and dual-watermark defenses against watermark removal attacks.

Method: RecoverMark uses the protected face content as the watermark embedded into the background, employs a two-stage training paradigm with distortion layers, and a progressive training strategy.

Result: RecoverMark shows robustness against seen/unseen attacks and generalizes well across in-distribution and out-of-distribution data.

Conclusion: RecoverMark effectively addresses limitations of existing watermarking methods, offering a robust solution for manipulation localization, recovery, and ownership verification.
...
TL;DR: The paper proposes an online decomposition and recomposition method to diversify training data for monocular 3D object detection (M3OD), addressing data entanglement issues and improving model performance.



<details>
  <summary>Details</summary>
Motivation: Current M3OD training data suffers from tight entanglement of objects, scenes, and camera poses, leading to insufficient diversity and overfitting. This limits model generalization.


Method: The authors decompose images into textured 3D object models and backgrounds, then recompose new training images with varied object-scene-camera combinations in each epoch.


Result: The method enhances data diversity and improves M3OD performance across five models on KITTI and Waymo datasets, even with sparse annotations.


Conclusion: The proposed plug-and-play scheme effectively addresses data entanglement, enabling better utilization of training data and boosting M3OD performance.
...
TL;DR: TraqPoint is an RL-based framework optimizing keypoint trackability in sequences, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based keypoint methods fail to optimize long-term trackability under viewpoint and illumination changes.

Method: TraqPoint uses RL with a track-aware reward for joint consistency and distinctiveness in keypoints.

Result: Outperforms SOTA methods in benchmarks like pose estimation and 3D reconstruction.

Conclusion: TraqPoint effectively improves keypoint trackability in sequences.
...
TL;DR: SIFormer is a scene-instance aware transformer for 3D object detection using 4D radar and camera, combining BEV-level and perspective-level fusion to address radar sparsity and improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing radar-camera fusion paradigms struggle with sparse and weak geometric cues from 4D radar, limiting instance activation and detection effectiveness.

Method: SIFormer uses segmentation- and depth-guided localization for noise suppression, introduces cross-view activation for instance awareness, and employs transformer-based fusion for robust perception.

Result: SIFormer achieves state-of-the-art performance on View-of-Delft, TJ4DRadSet, and NuScenes datasets.

Conclusion: SIFormer effectively bridges the gap between BEV-level and perspective-level fusion, improving instance awareness and detection accuracy for autonomous driving.
...
TL;DR: The paper introduces SurgAtt-Tracker, a framework for tracking surgical attention using spatio-temporal learning and dense heatmaps, achieving state-of-the-art performance and robustness.


<details>
  <summary>Details</summary>
Motivation: Accurate FoV guidance is essential for minimally invasive surgery, but existing methods conflate attention estimation with camera control or rely on rigid assumptions.

Method: SurgAtt-Tracker models surgeon focus as dense heatmaps, using proposal-level reranking and motion-aware refinement for robust tracking.

Result: The framework outperforms existing methods, showing strong robustness under occlusion, interference, and cross-domain conditions.

Conclusion: SurgAtt-Tracker provides interpretable FoV guidance, supporting robotic planning and automatic camera control in surgery.
...
TL;DR: DCQ compresses image datasets by reducing color-space redundancy while preserving training-critical information, improving performance under aggressive compression.


<details>
  <summary>Details</summary>
Motivation: High storage demands of large-scale image datasets hinder deployment in resource-constrained environments, and existing methods often ignore intra-image redundancy.

Method: DCQ reduces redundancy via consistent palettes, retains semantically important colors using model perception, and preserves structural details.

Result: Experiments on CIFAR-10, CIFAR-100, Tiny-ImageNet, and ImageNet-1K show DCQ enhances training performance under high compression.

Conclusion: DCQ provides a scalable and robust solution for dataset-level storage reduction while maintaining training effectiveness.
...
TL;DR: SD4R is a novel framework designed to densify sparse 4D radar point clouds using foreground point generation and logit-query encoding, achieving state-of-the-art performance in noise reduction and densification.


<details>
  <summary>Details</summary>
Motivation: The sparsity and noise in 4D radar point clouds hinder accurate 3D object detection, necessitating robust densification methods.

Method: SD4R uses a foreground point generator (FPG) to reduce noise and densify points, followed by a logit-query encoder (LQE) to enhance feature representations.

Result: SD4R achieves top performance on the View-of-Delft dataset, excelling in noise reduction and densification.

Conclusion: SD4R effectively addresses the challenges of sparse 4D radar point clouds, offering improved robustness and accuracy for 3D perception.
...
TL;DR: The study evaluates vision-language models (VLMs) for estimating hand distances (H and V) in lifting tasks, comparing detection-only and segmentation-based pipelines. The segmentation-based pipeline outperformed detection-only, reducing errors by 20-40%.


<details>
  <summary>Details</summary>
Motivation: Manual measurement of hand distances in lifting tasks is impractical in real-world settings. VLMs offer a non-invasive alternative using RGB video.

Method: Developed two VLM-based pipelines (text-guided detection-only and detection-plus-segmentation) to estimate H and V distances from video. Evaluated performance across pipelines and camera views.

Result: Segmentation-based pipeline achieved mean absolute errors of 6-8 cm for H and 5-8 cm for V, reducing errors by 20-40% compared to detection-only.

Conclusion: VLMs are feasible for estimating RNLE distance parameters from video, with segmentation further improving accuracy.
...
TL;DR: AnimeAgent addresses limitations in custom storyboard generation with a dynamic I2V-based multi-agent framework, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Current static diffusion models lack dynamic expressiveness, iterative correction, and reliable evaluation for stylized animation.

Method: Uses an Image-to-Video-based multi-agent framework inspired by Disney's workflow, with mixed subjective-objective reviewers for refinement.

Result: Achieves state-of-the-art performance in consistency, prompt fidelity, and stylization.

Conclusion: AnimeAgent effectively overcomes key limitations in CSG, offering dynamic consistency and reliable refinement.
...
TL;DR: A framework for interactive 3D shape generation refines abstractions iteratively using two models: BoxSplitGen for bounding box splitting and a box-to-shape model for detailed generation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in 3D generative models that assist human imagination by refining abstractions from coarse to fine details.

Method: The framework uses two models: BoxSplitGen for iterative bounding box splitting and a box-to-shape model conditioned on learned 3D priors.

Result: BoxSplitGen outperforms token prediction and unconditional diffusion models, while the box-to-shape model surpasses previous approaches.

Conclusion: The proposed framework effectively refines 3D shapes from abstract ideas to detailed outputs, demonstrating superior performance over existing methods.
...
TL;DR: BBQ is a text-to-image model that uses numeric bounding boxes and RGB triplets for precise control over object location, size, and color, eliminating the need for descriptive language.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image models rely on descriptive language, which lacks the precision required for professional workflows involving numeric control over object attributes.

Method: BBQ conditions on numeric bounding boxes and RGB triplets within a structured-text framework, trained on captions enriched with parametric annotations.

Result: BBQ achieves strong box alignment and improved RGB color fidelity over state-of-the-art baselines, enabling intuitive user interfaces.

Conclusion: BBQ supports a paradigm shift where user intent is translated into structured language for precise, numeric control in image generation.
...
TL;DR: Proto-Caps is an interpretable deep learning model combining capsule networks, prototype learning, and privileged information, achieving high accuracy and visual validation for lung nodule analysis.


<details>
  <summary>Details</summary>
Motivation: Address the need for interpretability and high performance in medical imaging, especially for understanding lung nodule characteristics.

Method: Proposes Proto-Caps, integrating capsule networks, prototype learning, and privileged information during training.

Result: Achieves 93.0% accuracy in malignancy prediction, 6% higher than baselines, with visual prototype-based reasoning.

Conclusion: Proto-Caps enhances interpretability and outperforms state-of-the-art methods in medical image analysis.
...
TL;DR: GA-Drive is a driving simulator framework that uses Geometry-Appearance Decoupling and Diffusion-Based Generation to create editable, high-fidelity views for autonomous driving training and evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing simulators lack free-viewpoint, editable, and high-fidelity features, which are essential for training and evaluating autonomous driving systems.

Method: The framework decouples geometry and appearance, synthesizing novel pseudo-views using geometry data and transforming them into photorealistic views via a video diffusion model. It supports appearance editing while preserving geometry.

Result: GA-Drive outperforms existing methods in NTA-IoU, NTL-IoU, and FID scores, demonstrating superior performance.

Conclusion: GA-Drive provides a flexible and high-quality solution for autonomous driving simulation, enabling consistent edits and photorealistic view generation.
...
TL;DR: VAUQ is a vision-aware uncertainty quantification framework for LVLMs that improves self-evaluation by measuring dependence on visual evidence, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: LVLMs often hallucinate, limiting safe deployment. Current self-evaluation methods rely too much on language priors and fail for vision-conditioned predictions.

Method: VAUQ introduces the Image-Information Score (IS) and core-region masking to quantify visual dependence. It combines predictive entropy with IS for a training-free scoring function.

Result: VAUQ outperforms existing self-evaluation methods across multiple datasets.

Conclusion: VAUQ effectively addresses LVLM hallucination by enhancing self-evaluation with vision-aware uncertainty quantification.
...
TL;DR: RAYNOVA is a geometry-free world model using a dual-causal autoregressive framework for unified 4D spatio-temporal reasoning, achieving state-of-the-art multi-view video generation.


<details>
  <summary>Details</summary>
Motivation: To simulate real-world evolution with physically plausible behavior by addressing the limitations of prior methods that handle spatial and temporal correlations separately.

Method: Employs a dual-causal autoregressive framework with global attention for unified spatio-temporal reasoning and uses relative Plücker-ray positional encoding for isotropic representation.

Result: Achieves state-of-the-art multi-view video generation on nuScenes, with higher throughput, controllability, and generalization to novel views.

Conclusion: RAYNOVA effectively generalizes to diverse camera setups and ego motions without explicit 3D scene representation, outperforming existing methods.
...
TL;DR: The paper introduces MethodLPP, a lightweight plug-and-play module for crisp edge detection, eliminating the need for non-differentiable post-processing and improving performance metrics significantly.


<details>
  <summary>Details</summary>
Motivation: Existing edge detection methods rely on non-differentiable post-processing (like NMS and thinning), hindering end-to-end optimization and crisp edge generation.

Method: MethodLPP is proposed—a matching-based supervision module (~21K parameters) appended to any edge model for joint end-to-end learning. It matches predicted and ground-truth edges spatially and by confidence.

Result: MethodLPP boosts baseline models' Average Crispness (AC) by 2–4x and improves ODS by 20–35% under CEval, matching or surpassing post-processing results.

Conclusion: MethodLPP enables SOTA crisp edge detection without post-processing, demonstrating significant performance gains across datasets.
...
TL;DR: The paper proposes NGL, an intermediate language for VLMs to extract garment parameters from images without training, achieving state-of-the-art performance on sewing pattern reconstruction.


<details>
  <summary>Details</summary>
Motivation: Prior methods struggle with generalizing to real-world images and multi-layer outfits. VLMs can describe garments well but fail at direct parameter regression.

Method: Introduces NGL-Prompter, a training-free pipeline that converts VLM-extracted garment descriptions into valid GarmentCode parameters using NGL.

Result: Achieves top performance on geometry metrics and perceptual evaluations, outperforming baselines on multi-layer outfits and occluded parts.

Conclusion: Accurate sewing pattern reconstruction is feasible without expensive training, demonstrated by NGL-Prompter's success on real-world images.
...
TL;DR: The study introduces an AI-based method for segmenting straylight faults in space cameras using a DeepLabV3 model with MobileNetV3 backbone, trained on diverse flare data for generalization.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of segmenting straylight effects in space cameras due to limited realistic space-specific data.

Method: Uses a DeepLabV3 model with MobileNetV3 backbone, pre-trained on Flare7k++ dataset for generalization, and targets deployment in resource-constrained spacecraft hardware.

Result: Develops custom metrics to assess the model's performance in a system-level context.

Conclusion: The approach demonstrates potential for effective segmentation of straylight faults in space cameras, with hardware deployment considerations.
...
TL;DR: A novel method combines 3D Gaussian Splatting with surface-aware reconstruction and deformation constraints for real-time, high-quality endoscopic tissue reconstruction in robot-assisted surgery.


<details>
  <summary>Details</summary>
Motivation: Current methods face challenges: Gaussian Splatting lacks consistency in tissue surface reconstruction, and NeRF-based methods cannot render in real-time. The goal is to achieve smooth deformable surfaces with real-time rendering.

Method: The approach uses Sign Distance Field-based mesh construction to guide Gaussian Splatting, incorporating local rigidity and global non-rigidity constraints for physically plausible deformations.

Result: The method provides fast rendering and smooth surfaces, outperforming alternatives in reconstruction quality for textures and geometries.

Conclusion: The proposed technique successfully balances real-time rendering and high-quality deformable tissue reconstruction, advancing robot-assisted surgical applications.
...
TL;DR: CleanStyle is a plug-and-play framework for diffusion models that reduces content leakage in style transfer by filtering noise from style embeddings using SVD and a dynamic suppression schedule, while also introducing style-specific guidance.


<details>
  <summary>Details</summary>
Motivation: Content leakage in encoder-based style transfer methods impairs prompt fidelity and stylistic consistency, prompting the need for a solution that filters content-related noise from style embeddings.

Method: CleanStyle uses Singular Value Decomposition (SVD) to isolate tail components of style embeddings and suppresses them dynamically. It also introduces Style-Specific Classifier-Free Guidance (SS-CFG) for targeted negative signals.

Result: CleanStyle reduces content leakage, improves stylization quality, and enhances prompt alignment across diverse style references and prompts.

Conclusion: CleanStyle is lightweight, interpretable, and compatible with existing encoder-based diffusion models, offering significant improvements in style transfer.
...
TL;DR: A unified stochastic framework bridges Monte Carlo rendering and diffusion-based generative models, enabling physically grounded control over diffusion-generated images.


<details>
  <summary>Details</summary>
Motivation: To combine the strengths of diffusion-based image generation (flexibility) and physically based rendering (fine-grained control) by addressing their shared evolution from noisy observations to clean images.

Method: Proposes a unified stochastic formulation using a stochastic differential equation (SDE) for Monte Carlo rendering, instantiated via path tracing, and analyzes its extension to diffusion models through noise variance.

Result: The method successfully applies physically grounded control to diffusion-generated results, demonstrated in tasks like rendering and material editing.

Conclusion: The approach effectively unifies Monte Carlo rendering and diffusion models, offering a novel way to integrate physical properties into generative image processes.
...
TL;DR: COMiT is a framework for structured visual tokenization that mimics human communication, improving semantic representation and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing image tokenizers focus on reconstruction and compression, lacking object-level semantic structure, which COMiT aims to address.

Method: COMiT uses iterative localized crop observation and recurrent token updates within a transformer model, trained end-to-end with flow-matching and semantic alignment losses.

Result: COMiT achieves interpretable, object-centric token structures and outperforms prior methods in compositional generalization and relational reasoning.

Conclusion: Attentive sequential tokenization in COMiT enhances semantic grounding and structured representation, advancing vision and multimodal systems.
...
TL;DR: OrthoDiffusion, a diffusion-based foundation model, excels in multi-task musculoskeletal MRI interpretation, achieving robust performance in segmentation and diagnosis with limited labeled data and transferability to other joints.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of interpreting complex musculoskeletal MRI scans, which requires expertise and suffers from variability, by developing a unified model for accurate and efficient diagnosis and segmentation.

Method: Developed OrthoDiffusion using three orientation-specific 3D diffusion models pre-trained on 15,948 unlabeled knee MRI scans, integrating view-specific representations for tasks like segmentation and multi-label diagnosis.

Result: Achieved excellent performance in segmenting 11 knee structures and detecting 8 abnormalities, with robustness across clinical centers and MRI field strengths, and maintained high precision with only 10% labeled data. Also transferred successfully to ankle and shoulder diagnostics.

Conclusion: OrthoDiffusion serves as a versatile platform for musculoskeletal MRI interpretation, enhancing accuracy and efficiency in clinical workflows.
...
TL;DR: The paper explores federated learning for medical image segmentation across different modalities (CT and MRI), proposing augmentation strategies like GIN to improve cross-modality generalization while preserving privacy.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of fragmented, privacy-constrained medical imaging data and cross-modality domain shifts in federated learning.

Method: Systematically evaluates augmentation strategies (spatial, frequency-domain, domain-specific normalization, GIN) for cross-modality generalization in federated settings.

Result: GIN outperformed other methods, improving pancreas Dice score by 498% and achieving 93-98% of centralized training accuracy.

Conclusion: Federated learning with GIN augmentation enables strong cross-modality generalization without compromising data privacy, facilitating AI deployment in diverse healthcare systems.
...
TL;DR: A framework using normal flow for motion segmentation in event-based vision achieves real-time performance and significant speedup over existing methods.


<details>
  <summary>Details</summary>
Motivation: Direct processing of sparse event data is inefficient for real-time tasks like motion segmentation, necessitating a more effective intermediate representation.

Method: Proposes a normal flow-based framework, formulating motion segmentation as an energy minimization problem solved via graph cuts and iterative optimization with clustering and motion model fitting.

Result: Achieves nearly 800x speedup compared to state-of-the-art methods, demonstrating accuracy and efficiency on public datasets.

Conclusion: Normal flow-based motion segmentation efficiently handles dynamic scene understanding with reduced computational complexity.
...
TL;DR: The paper introduces a biomechanics-aware keypoint simulation framework to augment human pose datasets with 3D spinal keypoints, creating the SIMSPINE dataset. It also provides pretrained baselines for spine motion estimation, improving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Understanding spinal motion is crucial for biomechanics, but existing datasets lack large-scale 3D spinal annotations due to the spine's complexity. The work aims to bridge this gap using musculoskeletal modeling.

Method: A simulation framework generates anatomically consistent 3D spinal keypoints, creating the SIMSPINE dataset with 2.14 million frames. Pretrained baselines for 2D detection, 3D pose lifting, and multi-view reconstruction are also released.

Result: The framework improves spine tracking performance, achieving 0.80 AUC (up from 0.63) in controlled environments and 0.93 AP (up from 0.91) in-the-wild.

Conclusion: SIMSPINE and the simulation framework advance vision-based biomechanics by enabling reproducible, anatomically valid 3D spine estimation under natural conditions.
...
TL;DR: VGGDrive enhances Vision-Language Models (VLMs) with cross-view 3D geometric grounding for autonomous driving, closing a critical capability gap by integrating 3D features through a plug-and-play enabler.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack cross-view 3D geometric modeling, limiting their performance in autonomous driving tasks. Integrating 3D foundation models could bridge this gap.

Method: Proposes VGGDrive, featuring a Cross-View 3D Geometric Enabler (CVGE) to hierarchically inject 3D features into VLMs, decoupling their architecture.

Result: VGGDrive improves VLM performance across five autonomous driving benchmarks, including risk perception, motion prediction, and trajectory planning.

Conclusion: Mature 3D foundation models can empower autonomous driving tasks, and VGGDrive demonstrates the potential of this integration paradigm.
...
TL;DR: RU4D-SLAM is a framework combining 4D Gaussian splatting with SLAM to improve dynamic scene reconstruction by integrating uncertainty-aware perception, motion blur rendering, and semantic-guided reweighting.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle in dynamic environments due to moving objects, which complicate 3D reconstruction and tracking. 4D reconstruction offers a promising solution but is underexplored for SLAM.

Method: Proposes RU4D-SLAM, which introduces temporal factors into 3D representation, incorporates uncertainty-aware perception, motion blur rendering, and semantic-guided reweighting for uncertainty estimation. Also includes a learnable opacity weight for adaptive 4D mapping.

Result: Outperforms state-of-the-art methods in trajectory accuracy and 4D scene reconstruction, especially in dynamic environments with moving objects and low-quality inputs.

Conclusion: RU4D-SLAM effectively addresses challenges in dynamic scene reconstruction and tracking, demonstrating superior performance in dynamic environments.
...
TL;DR: A training-free framework combines Optimised DDS with LoRA-driven concept composition for improved image editing, addressing limitations of text-only prompts.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with preserving identity and expressing non-textual visual concepts like facial structure or texture. This gap motivates a unified approach.

Method: The framework integrates Optimised DDS with LoRA-based concept composition, uses pretrained concept adapters, and refines DDS for stability via ordered timesteps and regularisation.

Result: The approach outperforms existing training-free methods on benchmarks like InstructPix2Pix and ComposLoRA, showing consistent improvements.

Conclusion: The proposed method effectively combines text and visual concepts for better image editing without requiring training.
...
TL;DR: FLIM-BoFP speeds up filter estimation in FLIM networks for Salient Object Detection by replacing multi-block clustering with a single clustering step, improving efficiency and control.


<details>
  <summary>Details</summary>
Motivation: Traditional convolutional networks require costly image annotation, and FLIM-Cluster, while effective, has computational overhead and limited filter control. FLIM-BoFP addresses these issues.

Method: FLIM-BoFP performs a single clustering at the input block to create a bag of feature points, then maps these points across encoder blocks to define filters directly, streamlining the process.

Result: FLIM-BoFP outperforms FLIM-Cluster in efficiency, effectiveness, and generalization, particularly for parasite detection in microscopy images.

Conclusion: FLIM-BoFP offers a faster and more controlled alternative to FLIM-Cluster, enhancing FLIM networks' practicality for tasks like Salient Object Detection.
...
TL;DR: A hybrid image fusion framework combines deep learning (U-Net) and traditional Laplacian pyramid fusion for efficient full-resolution training, achieving SOTA performance quickly without external models.


<details>
  <summary>Details</summary>
Motivation: Traditional fusion methods lack adaptability, while deep learning approaches are inefficient due to patch-based training. The goal is to bridge this gap.

Method: Uses a U-Net to create a dynamic guidance map for a fixed Laplacian pyramid fusion kernel, decoupling policy learning from pixel synthesis.

Result: Achieves SOTA-comparable performance in about 1-2 minutes on GPUs, with strong zero-shot generalization across tasks.

Conclusion: The framework offers efficient, high-fidelity fusion suitable for critical applications without reliance on external models.
...
TL;DR: The paper explores how XAI methods can make VLMs like CLIP's visual reasoning understandable in art-historical contexts, finding their effectiveness depends on conceptual stability and category representation.


<details>
  <summary>Details</summary>
Motivation: To address critical questions about machine 'understanding' and enhance the interpretability of VLMs in art-historical analysis.

Method: Combines zero-shot localization experiments with human interpretability studies to evaluate seven XAI methods.

Result: XAI methods capture some aspects of human interpretation but their success relies on the stability and availability of the categories being examined.

Conclusion: While XAI methods show promise in elucidating VLM reasoning, their applicability in art-historical contexts is contingent on the nature of the categories involved.
...
TL;DR: DA-Cal is a new framework for cross-domain calibration in unsupervised domain adaptation, improving semantic segmentation by optimizing soft pseudo-labels and enhancing prediction confidence alignment.


<details>
  <summary>Details</summary>
Motivation: Existing UDA methods focus on performance but often overlook calibration quality, leading to misalignment between confidence and accuracy, which is critical for safety-critical applications.

Method: DA-Cal uses a Meta Temperature Network for pixel-level calibration and bi-level optimization to link soft pseudo-labels with UDA supervision, supplemented by domain-mixing strategies to avoid overfitting and domain gaps.

Result: DA-Cal improves target domain calibration and performance across multiple UDA segmentation benchmarks without adding inference overhead.

Conclusion: DA-Cal effectively addresses calibration issues in UDA, enhancing both reliability and performance in semantic segmentation tasks.
...
TL;DR: CASG is a training-free framework addressing harmful conflicts in T2I diffusion models by dynamically applying category-aligned safety guidance, reducing harmful rates by up to 15.4%.


<details>
  <summary>Details</summary>
Motivation: Existing safety-guidance methods fail to handle harmful conflicts in T2I models, where mitigating one harm type may amplify others.

Method: CASG combines Conflict-aware Category Identification (CaCI) to identify harmful categories and Conflict-resolving Guidance Application (CrGA) to apply targeted safety steering.

Result: CASG achieves state-of-the-art performance, reducing harmful rates by up to 15.4%.

Conclusion: CASG effectively addresses harmful conflicts in T2I models without additional training, outperforming existing methods.
...
TL;DR: The paper introduces SpatiaLQA, a benchmark to evaluate spatial logical reasoning in Vision-Language Models (VLMs), showing their limitations and proposing a recursive scene graph method to improve performance.


<details>
  <summary>Details</summary>
Motivation: VLMs lack spatial logical reasoning abilities in complex real-world environments, prompting the need for SpatiaLQA to assess and enhance this capability.

Method: The authors propose recursive scene graph assisted reasoning, decomposing complex scenes into task-relevant scene graphs using visual foundation models.

Result: Experiments on 41 VLMs reveal their struggles with spatial logical reasoning, while the proposed method outperforms existing approaches.

Conclusion: SpatiaLQA effectively benchmarks VLMs' spatial logical reasoning, and the recursive scene graph method significantly improves their performance.
...
TL;DR: TextPecker addresses structural anomalies in text-to-image generation by introducing a plug-and-play RL strategy and improving fidelity and alignment.


<details>
  <summary>Details</summary>
Motivation: Current models struggle with structural anomalies (distortion, blurriness, misalignment) in visual text rendering, and existing OCR/MLLMs fail to detect these issues, hindering evaluation and optimization.

Method: Proposes TextPecker, a RL-based strategy leveraging a dataset with character-level anomaly annotations and a stroke-editing synthesis engine for broader error coverage.

Result: TextPecker enhances structural fidelity (4% gain) and semantic alignment (8.7% gain) for Chinese text, outperforming state-of-the-art models like Qwen-Image.

Conclusion: TextPecker bridges a gap in VTR optimization, advancing reliable and structurally accurate text-to-image generation.
...
TL;DR: Proposes LongVideo-R1, an MLLM agent for efficient long video understanding via active reasoning and hierarchical navigation, trained with SFT and RL.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of long video understanding with low computational budgets by avoiding exhaustive search redundancy.

Method: Uses a reasoning module for clip selection, hierarchical traversal, and two-stage training (SFT + RL) with GPT-5-generated trajectories.

Result: Demonstrates superior tradeoff between QA accuracy and efficiency on long video benchmarks.

Conclusion: LongVideo-R1 effectively balances accuracy and computational efficiency for long video understanding.
...
TL;DR: GID is a preprocessing method enhancing rotation robustness in deep learning without altering network architecture by aligning images to a canonical frame. It outperforms state-of-the-art methods on MNIST and CIFAR-10.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle with rotation invariance, often requiring costly or complex solutions like data augmentation or architectural changes.

Method: GID estimates a global orientation for each image and aligns it to a canonical frame, preserving spatial structure for standard convolutional networks.

Result: GID achieves higher accuracy than rotation-invariant architectures on MNIST and remains effective on more complex datasets like CIFAR-10.

Conclusion: GID offers a simple, effective preprocessing solution for rotation robustness without modifying network architecture.
...
TL;DR: DropAnSH-GS introduces an anchor-based Dropout strategy for 3D Gaussian Splatting, disrupting local redundancies and mitigating overfitting by dropping higher-degree spherical harmonic coefficients.


<details>
  <summary>Details</summary>
Motivation: Existing Dropout methods suffer from neighbor compensation effects and overlook overfitting contributions from high-degree spherical harmonics.

Method: DropAnSH-GS randomly selects anchor Gaussians and removes their neighbors, while also dropping higher-degree SH coefficients for color attributes.

Result: The method outperforms existing Dropout techniques with minimal computational cost and integrates easily into various 3DGS variants.

Conclusion: DropAnSH-GS effectively reduces overfitting and enhances model robustness, enabling flexible post-training compression.
...
TL;DR: UFO introduces a recurrent paradigm for efficient long-range 4D driving scene reconstruction, combining optimization-based and feed-forward methods for accurate dynamic object modeling.


<details>
  <summary>Details</summary>
Motivation: Dynamic scene reconstruction is vital for autonomous driving simulation, but existing methods struggle with long-range sequences due to computational complexity and dynamic object modeling challenges.

Method: UFO uses a recurrent approach with a visibility-based filtering mechanism and object pose-guided modeling to refine a 4D scene representation iteratively.

Result: Experiments on Waymo Open Dataset show UFO outperforms per-scene optimization and feed-forward methods, efficiently reconstructing 16-second logs in 0.5 seconds.

Conclusion: UFO achieves superior efficiency and accuracy in long-range 4D reconstruction, making it promising for autonomous driving applications.
...
TL;DR: ArtiAgent automates the creation of artifact-annotated images using three agents, addressing scalability issues in artifact mitigation for AI-generated images.


<details>
  <summary>Details</summary>
Motivation: AI-generated images often have visual artifacts, and existing methods rely on costly human-labeled datasets, necessitating an automated solution.

Method: ArtiAgent uses three agents: perception (identifies entities), synthesis (injects artifacts via patch-wise embedding manipulation), and curation (filters artifacts and provides explanations).

Result: Synthesized 100K images with artifact annotations, showing efficacy and versatility.

Conclusion: ArtiAgent offers a scalable solution for artifact mitigation, improving realism in AI-generated images.
...
TL;DR: TagLLM, a framework leveraging MLLMs for image tagging, reduces annotation costs significantly and improves annotation quality to near-human levels for downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To address the high cost of human annotation in image tagging and explore MLLMs' potential to automate this process efficiently.

Method: TagLLM uses structured group-wise prompting for candidate generation and interactive label disambiguation to refine annotations.

Result: TagLLM reduces annotation costs to 1/1000th of human costs and achieves 50-80% human annotation quality, with over 90% downstream task performance.

Conclusion: TagLLM effectively narrows the gap between MLLM and human annotations, demonstrating practical feasibility for automated image tagging.
...
TL;DR: CrystaL is a new framework for Multimodal Large Language Models (MLLMs) that improves latent reasoning by aligning attention patterns and prediction distributions between intact and corrupted image paths.


<details>
  <summary>Details</summary>
Motivation: Existing latent Chain-of-Thought (CoT) methods lack proper supervision signals to preserve critical visual information in intermediate latent states.

Method: Proposes CrystaL, a single-stage framework with dual paths for processing intact and corrupted images, aligning attention patterns and prediction distributions.

Result: CrystaL outperforms state-of-the-art baselines in fine-grained visual understanding and maintains robust reasoning capabilities.

Conclusion: CrystaL effectively crystallizes latent representations into task-relevant visual semantics without needing extra annotations or external modules.
...
TL;DR: MMHNet improves long-form audio generation up to 5+ minutes by integrating hierarchical methods and non-causal Mamba, proving short-training-long-testing feasibility in video-to-audio tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing the scaling challenge in multimodal-to-audio generation, especially the mismatch between text descriptions and frame-level video data, and limited training data.

Method: Proposes MMHNet, a hierarchical network with non-causal Mamba, enhancing state-of-the-art video-to-audio models for long-form generation.

Result: Achieves significant improvements in long audio generation (5+ minutes) and outperforms prior works on benchmarks.

Conclusion: Demonstrates feasibility of short-training-long-testing and superior performance in long-duration video-to-audio generation.
...
TL;DR: EW-DETR is a framework for evolving-world object detection, combining incremental learning, domain adaptation, and unknown detection without prior data, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenges of evolving environments where new classes emerge, domains shift, and unseen objects must be identified without prior data access.

Method: Proposes EW-DETR with Incremental LoRA Adapters, Query-Norm Objectness Adapter, and Entropy-Aware Unknown Mixing for exemplar-free incremental learning and calibrated unknown detection.

Result: Outperforms other methods on Pascal Series and Diverse Weather benchmarks, improving FOGS by 57.24%.

Conclusion: EW-DETR effectively handles evolving-world object detection challenges, demonstrating superior performance in forgetting, openness, and generalization.
...
TL;DR: The paper presents a framework for disentangling visual layers in images using diffusion models, focusing on logo-object decomposition with bidirectional consistency and progressive self-improvement.


<details>
  <summary>Details</summary>
Motivation: Disentangling visual layers like shading and reflection is challenging due to non-linear interactions. The work aims to accurately separate logos from surfaces while preserving both layers.

Method: The approach fine-tunes a pretrained diffusion model with LoRA adaptation and introduces cycle-consistent tuning for decomposition and composition. A progressive self-improving process enhances performance.

Result: Experiments show accurate and coherent decompositions, with the method generalizing to other decomposition tasks, suggesting a unified framework potential.

Conclusion: The framework effectively disentangles complex visual layers, offering robustness and generalization across decomposition challenges.
...
TL;DR: Visual Instruction Injection (VII) exploits image-to-video models by disguising malicious text prompts as benign visual instructions in images, achieving high attack success rates.


<details>
  <summary>Details</summary>
Motivation: To uncover risks in image-to-video models where adversaries can inject malicious intent through visual instructions.

Method: VII uses Malicious Intent Reprogramming and Visual Instruction Grounding to disguise harmful text prompts as safe images, inducing harmful video content.

Result: VII achieves up to 83.5% Attack Success Rate and near-zero Refusal Rates on commercial models.

Conclusion: VII highlights vulnerabilities in visual instruction-following models, necessitating safeguards.
...
TL;DR: The paper introduces Le-DETR, a low-cost and efficient transformer-based model for real-time object detection, achieving state-of-the-art performance without expensive pretraining.


<details>
  <summary>Details</summary>
Motivation: To address the high pretraining costs and reproducibility challenges of current real-time DETR models, enabling exploration of novel backbone architectures.

Method: Proposes EfficientNAT backbones with efficient convolution and local attention, and re-designs the hybrid encoder to improve performance and speed.

Result: Le-DETR achieves 52.9/54.3/55.1 mAP on COCO Val2017 with 4.45/5.01/6.68 ms latency, surpassing competitors like YOLOv12 and DEIM-D-FINE.

Conclusion: Well-designed DETR models can deliver strong performance efficiently without costly pretraining, advancing real-time detection research.
...
TL;DR: The paper introduces CHAIN, a benchmark to evaluate models' ability to understand and execute structured action sequences grounded in physical constraints.


<details>
  <summary>Details</summary>
Motivation: Current VLM evaluations lack assessments of agents' ability to reason about physical structures like geometry and support relations.

Method: CHAIN is developed as an interactive 3D, physics-driven testbed with tasks like mechanical puzzles and 3D stacking.

Result: State-of-the-art VLMs and diffusion models struggle with long-horizon planning and converting perceived structure into actions.

Conclusion: CHAIN highlights the limitations of current models in understanding physical constraints and calls for advancements in structure-aware AI.
...
TL;DR: MIPCandy is a modular, PyTorch-based framework for medical image processing, offering customizable pipelines, deferred configuration, and built-in tools.


<details>
  <summary>Details</summary>
Motivation: Address the lack of flexible, high-level frameworks tailored for medical image processing.

Method: Provides modular components like LayerT for runtime flexibility, built-in cross-validation, and extensible pre-built models.

Result: Enables efficient, customizable workflows with features like deep supervision and experiment tracking.

Conclusion: MIPCandy simplifies medical image processing while maintaining flexibility and extensibility.
...
TL;DR: CLIPGlasses is a plug-and-play framework enhancing CLIP's negation comprehension by disentangling negated semantics and modifying similarity computation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) like CLIP struggle with negation, often misaligning affirmatives and negatives. Current fine-tuning methods risk overfitting.

Method: CLIPGlasses uses a dual-stage design: the Lens module disentangles negated semantics, and the Frame module predicts repulsion strength to adjust similarity computation.

Result: CLIPGlasses achieves competitive in-domain performance and excels in cross-domain generalization, especially under low-resource conditions.

Conclusion: CLIPGlasses effectively improves CLIP's negation understanding without overfitting, demonstrating robustness across domains.
...
TL;DR: OmniOCR is a universal framework for ethnic minority scripts using Dynamic LoRA and sparsity regularization, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Ethnic minority languages are underexplored in OCR due to complex writing systems and scarce annotations.

Method: Dynamic Low-Rank Adaptation (Dynamic LoRA) allocates model capacity; sparsity regularization prunes redundant updates.

Result: OmniOCR achieves state-of-the-art accuracy, improving by 39%-66% on TibetanMNIST, Shui, ancient Yi, and Dongba datasets.

Conclusion: OmniOCR efficiently adapts to low-resource scripts with superior parameter efficiency.
...
TL;DR: The paper introduces a self-correction framework for Vision-Language Models (VLMs) to improve their reasoning robustness by enabling Capability Reflection and Memory Reflection.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack effective self-correction mechanisms, leading to repetitive errors and unstable answer quality during multi-turn revisions.

Method: The proposed framework includes Capability Reflection for error diagnosis and correction planning, and Memory Reflection to avoid repetition and explore new solutions, followed by rigorous re-reasoning.

Result: Experiments on OCRBench v2 show the framework outperforms SOTA models, achieving top scores in Visual Understanding (79.9) and Reasoning (66.5).

Conclusion: Structured self-aware reflection enhances VLMs' reasoning robustness without additional training.
...
TL;DR: The paper proposes an automatic sensor placement method for optimizing occupancy counting in commercial buildings using ToF sensors and ILP, validated through simulations.


<details>
  <summary>Details</summary>
Motivation: Accurate real-time occupancy counting is essential for energy-efficient lighting, heating, and cooling without disrupting occupants or compromising privacy.

Method: The approach involves simulating occupant trajectories, formulating sensor placement as an ILP problem, and solving it using the branch and bound method.

Result: The method effectively determines optimal sensor layouts and predicts counting accuracy, validated in various office environments.

Conclusion: The proposed automatic sensor placement method enhances occupancy counting efficiency and energy savings in commercial buildings.
...
TL;DR: A hybrid approach combining monocular foundation models and optimization-based methods achieves high-fidelity 3D head reconstruction with fewer views and lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing methods face trade-offs between detail and efficiency—photogrammetry requires dense views, while single-image methods lack detail. The goal is to bridge this gap.

Method: Introduces a multi-view surface normal prediction model with cross-view attention and integrates it into an inverse rendering optimization framework.

Result: Outperforms state-of-the-art single-image and multi-view methods, matching dense-view photogrammetry quality with reduced resources.

Conclusion: The hybrid method successfully balances efficiency and high-fidelity reconstruction, offering practical advantages for 3D head geometry applications.
...
TL;DR: A unified framework uses event streams and motion-blurred frames to reconstruct high-fidelity radiance fields from fast-flying drones, outperforming existing methods by over 50%.


<details>
  <summary>Details</summary>
Motivation: High-speed drones face motion blur and pose estimation issues, degrading 3D reconstruction with NeRFs. The goal is to improve reconstruction quality under these conditions.

Method: The framework fuses event streams and RGB frames during NeRF optimization and refines pose estimates using both event and frame data, without ground-truth supervision.

Result: The method successfully reconstructs sharp radiance fields and accurate camera trajectories, even with highly degraded RGB frames, achieving a 50% performance gain over state-of-the-art methods.

Conclusion: The framework effectively addresses challenges in high-speed drone imaging, enabling high-quality 3D reconstruction without ground-truth data.
...
TL;DR: Proposes BrepGaussian, a framework to recover B-rep models from 2D images using Gaussian Splatting and a two-stage learning approach, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of recovering B-rep representations from unstructured data, especially from 2D images, which is valuable for computer vision and graphics but struggles with generalization and dependency on clean point clouds.

Method: Uses a Gaussian Splatting renderer with learnable features and a two-stage framework to disentangle geometry reconstruction and feature learning—first capturing geometry and edges, then refining patch features.

Result: Demonstrates superior performance over state-of-the-art methods in extensive experiments, achieving clean geometry and coherent instance representations.

Conclusion: BrepGaussian effectively recovers B-rep models from 2D images, offering a promising solution for 3D shape reconstruction, and will release code and datasets.
...
TL;DR: UDVideoQA is a benchmark dataset for evaluating video language models on urban traffic dynamics, featuring 28K QA pairs from 16 hours of footage. It highlights a perception-reasoning gap and shows fine-tuning smaller models can bridge it.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of understanding multi-agent urban traffic dynamics and evaluate models' visual grounding and causal reasoning capabilities.

Method: A privacy-preserving, event-driven dynamic blur technique was used to curate 16 hours of traffic footage. A unified annotation pipeline produced 28K QA pairs across 8 hours of densely annotated video.

Result: Experiments on 10 VideoLMs reveal a perception-reasoning gap. Fine-tuned Qwen2.5-VL 7B matches proprietary systems' performance. Gemini Pro excels in zero-shot accuracy but lacks linguistic diversity in question generation.

Conclusion: UDVideoQA provides a robust foundation for advancing multimodal reasoning, emphasizing privacy-aware, real-world evaluation and highlighting the need for human-centric assessment.
...
TL;DR: SynthRender is an open-source framework for synthetic image generation with Guided Domain Randomization, aiding robust object perception in robotics and automation without large real datasets.


<details>
  <summary>Details</summary>
Motivation: Overcoming the high cost and effort of acquiring real-world annotated data for proprietary parts in industrial tasks like robotic handling and inspection.

Method: Developed SynthRender for synthetic image generation and benchmarked Reality-to-Simulation techniques for creating 3D assets from 2D images, combined with Domain Randomization.

Result: Achieved high performance on benchmarks: 99.1% mAP@50 (robotics), 98.3% mAP@50 (automotive), and 95.3% mAP@50 on IRIS.

Conclusion: SynthRender and synthetic data generation provide an efficient, transferable solution for industrial applications, reducing reliance on real-world datasets.
...
TL;DR: The paper introduces LUMEN, a training framework for longitudinal chest X-ray interpretation, enhancing diagnostic and prognostic capabilities through multi-image and multi-task instruction fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The manual analysis of temporal changes in radiology imaging is time-consuming, requiring automated solutions for accurate diagnosis and prognosis.

Method: LUMEN leverages multi-image and multi-task instruction fine-tuning, tested on MIMIC-CXR and Medical-Diff-VQA datasets, and includes a novel longitudinal instruction-following dataset.

Result: Significant improvements over baselines in diagnostic VQA tasks and promising prognostic capabilities are demonstrated.

Conclusion: Instruction-tuned VLMs like LUMEN enhance accuracy and clinical relevance in interpreting longitudinal radiological imaging.
...
TL;DR: SPRITETOMESH automates converting 2D game sprites into animation-ready triangle meshes using a hybrid learned-algorithmic approach, achieving significant speedups over manual creation.


<details>
  <summary>Details</summary>
Motivation: Manually creating skeletal animation-compatible meshes from sprites is time-consuming (15-60 minutes per sprite). SPRITETOMESH aims to automate this process.

Method: Uses a segmentation network (EfficientNet-B0 encoder with U-Net decoder) trained on sprite-mask pairs for binary masks, followed by contour extraction, edge detection, and Delaunay triangulation for mesh creation.

Result: Achieves IoU of 0.87 for segmentation; mesh creation completes in under 3 seconds (300x-1200x faster than manual). Neural vertex prediction failed (loss plateaued at 0.061).

Conclusion: Hybrid approach (learned segmentation + algorithmic placement) is effective. Direct neural vertex prediction is not viable due to artistic variability in mesh placement.
...
TL;DR: The paper proposes a quality-controllable retrieval system for text-to-image tasks, enhancing short queries with contextual details and explicit quality control using generative models.


<details>
  <summary>Details</summary>
Motivation: Short user queries in text-to-image retrieval are often semantically ambiguous and lack quality control, limiting retrieval effectiveness.

Method: The approach uses a generative language model to complete queries with fine-grained attributes and conditions completion on discretized quality levels derived from scoring models.

Result: The framework improves retrieval results and offers flexibility, transparency, and controllability without modifying pretrained VLMs.

Conclusion: The proposed system effectively bridges the gap between VLMs' expressive capacity and underspecified user queries, enhancing retrieval quality and control.
...
TL;DR: XMorph is an explainable and efficient framework for classifying brain tumors, achieving high accuracy while addressing interpretability and computational challenges.


<details>
  <summary>Details</summary>
Motivation: Clinical adoption of deep learning for brain tumor diagnosis is hindered by interpretability issues and computational constraints.

Method: XMorph uses an Information-Weighted Boundary Normalization (IWBN) mechanism and a dual-channel explainable AI module combining GradCAM++ visuals and LLM-generated rationales.

Result: Achieves 96.0% classification accuracy, balancing explainability and performance.

Conclusion: XMorph demonstrates that explainable AI can coexist with high accuracy in medical imaging.
...
TL;DR: Mask-HybridGNet trains graph-based segmentation models using standard masks instead of manual landmarks, enabling anatomical plausibility and implicit correspondences without explicit supervision.


<details>
  <summary>Details</summary>
Motivation: Clinical adoption of graph-based segmentation is hindered by the scarcity of datasets with manually annotated landmarks. Mask-HybridGNet addresses this by eliminating the need for such annotations.

Method: The framework combines Chamfer distance supervision and edge-based regularization to align variable-length boundaries with fixed-length landmarks, refined via differentiable rasterization.

Result: Mask-HybridGNet achieves competitive segmentation results while ensuring anatomical plausibility and implicit correspondence learning across patients.

Conclusion: This framework leverages standard segmentation masks to build structured models with topological integrity and implicit correspondences, enabling diverse medical applications.
...
TL;DR: The paper introduces Spa3R, a self-supervised framework that learns unified spatial representations from 2D images, avoiding reliance on explicit 3D data. Integrated into VLMs as Spa3-VLM, it achieves state-of-the-art 3D VQA performance.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with 3D comprehension. Existing methods rely on explicit 3D inputs or partial priors, limiting scalability. This paper proposes learning spatial intelligence directly from 2D vision.

Method: Spa3R uses Predictive Spatial Field Modeling (PSFM) to synthesize feature fields from unposed multi-view images. A lightweight adapter integrates Spa3R into VLMs, forming Spa3-VLM.

Result: Spa3-VLM achieves 58.6% accuracy on VSI-Bench for 3D VQA, outperforming prior methods.

Conclusion: PSFM offers a scalable way to advance spatial intelligence in VLMs without explicit 3D data.
...
TL;DR: HVG is a latent video diffusion model that generates high-quality, multi-view, spatiotemporally coherent human videos from a single image using 3D pose and view control, addressing challenges in clothing wrinkles and view consistency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of generating realistic human videos from single images, particularly focusing on view-consistent motion-dependent clothing wrinkles and multi-view coherence.

Method: HVG employs three key designs: Articulated Pose Modulation, View and Temporal Alignment, and Progressive Spatio-Temporal Sampling to achieve coherent human video generation.

Result: HVG outperforms existing methods in generating high-quality 4D human videos from diverse images and pose inputs, as demonstrated by extensive experiments.

Conclusion: HVG effectively overcomes challenges in human video generation, providing a robust solution for synthesizing spatiotemporally coherent videos with detailed clothing wrinkles and multi-view consistency.
...
TL;DR: TomoROIS-SurfORA is a deep learning framework for direct ROI segmentation and surface analysis in cryo-ET data, handling complex structures like membranes efficiently.


<details>
  <summary>Details</summary>
Motivation: Current ROI identification methods rely on indirect segmentation, which is inefficient for complex structures like membranes. TomoROIS-SurfORA addresses this gap.

Method: The framework uses deep learning (TomoROIS) for shape-agnostic ROI segmentation and SurfORA for morphological surface analysis (e.g., curvature, roughness).

Result: The tool successfully analyzes complex membrane geometries, including contact sites and remodeling events, with training from small datasets.

Conclusion: TomoROIS-SurfORA is versatile for ROI detection and surface analysis in cryo-ET and beyond.
...
### [105] [Multimodal Multi-Agent Empowered Legal Judgment Prediction](https://arxiv.org/abs/2601.12815)
*Zhaolu Kang,Junhao Gong,Qingxi Chen,Hao Zhang,Jiaxin Liu,Rong Fu,Zhiyuan Feng,Yuan Wang,Simon Fong,Kaiyue Zhou*

Main category: cs.CL

TL;DR: JurisMMA is a novel Legal Judgment Prediction framework that decomposes trial tasks and standardizes processes, validated by experiments on JurisMM and LawBench datasets.


<details>
  <summary>Details</summary>
Motivation: To address challenges in traditional Legal Judgment Prediction methods, such as handling multiple allegations and diverse evidence, by introducing a more adaptable framework.

Method: Development of JurisMMA, a framework decomposing trial tasks into stages, and creation of JurisMM, a large multimodal dataset of Chinese judicial records.

Result: Experiments confirm JurisMMA's effectiveness for LJP and broader legal applications, outperforming traditional methods.

Conclusion: JurisMMA offers a scalable and adaptable solution for LJP and future legal methodologies, supported by comprehensive datasets.
...
TL;DR: SA-SFT mitigates catastrophic forgetting in LLMs by generating self-dialogues before fine-tuning, improving in-domain performance without external data or extra tuning.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of catastrophic forgetting in fine-tuning LLMs on task-specific data, which degrades their general knowledge and reasoning abilities.

Method: Propose SA-SFT, a self-augmentation routine where LLMs generate self-dialogues before fine-tuning, mixing self-authored data with task data without altering training schedules.

Result: SA-SFT consistently mitigates forgetting and improves in-domain performance, outperforming baselines like layer freezing and external data mixing in 40 out of 50 cases.

Conclusion: Self-augmentation via self-generated data effectively counters catastrophic forgetting, offering a simple and robust mechanism for LLM adaptation.
...
TL;DR: Knowledge distillation significantly outperforms traditional methods in efficiency and performance for small language models (SLMs), making it a key strategy for accessible AI.


<details>
  <summary>Details</summary>
Motivation: To evaluate the efficiency and performance of distilled small language models (SLMs) compared to vanilla and proprietary models.

Method: Benchmarking distilled models against vanilla and proprietary counterparts, analyzing performance and computational cost.

Result: A distilled 8B model is 2,000 times more compute-efficient than vanilla training and matches or exceeds performance of larger models.

Conclusion: Knowledge distillation is validated as a primary strategy for creating efficient, high-performance SLMs.
...
TL;DR: ConceptRM proposes a method to train a reflection model for filtering false alerts by leveraging noisy user feedback data and minimal expert annotations.


<details>
  <summary>Details</summary>
Motivation: Address 'alert fatigue' caused by excessive false alarms from intelligent agents. Existing methods rely on noisy user feedback, which is impractical to clean manually.

Method: ConceptRM uses perturbed datasets and co-teaching with minimal expert annotations to identify reliable negatives from noisy data.

Result: Outperforms state-of-the-art methods by up to 53.31% (in-domain) and 41.67% (out-of-domain) in intercepting false alerts.

Conclusion: ConceptRM efficiently reduces false alerts with low annotation costs, making it practical for real-world applications.
...
TL;DR: The paper introduces an interview-grounded evaluation framework for simulating real personalities using large language models, outperforming existing methods by leveraging extensive real interview data and proposing four complementary metrics.


<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating personality simulation lack direct assessment against actual individual speech. This gap is addressed by grounding evaluation in authentic, large-scale interview data.

Method: The framework extracts over 671,000 question-answer pairs from 23,000 verified interview transcripts, proposing four metrics: content similarity, factual consistency, personality alignment, and factual knowledge retention. It compares retrieval-augmented and chronological-based methods.

Result: Methods grounded in real interview data outperform those using biographical profiles or parametric knowledge. Retrieval-augmented methods capture personality style better, while chronological-based methods excel in factual consistency and knowledge retention.

Conclusion: The evaluation framework aids principled method selection for personality simulation, offering actionable insights for advancing research in this field.
...
TL;DR: The paper explores how query features influence LLM hallucination, identifying specific traits like clause nesting and underspecification that increase risk, while grounding and answerability reduce it.


<details>
  <summary>Details</summary>
Motivation: To understand if certain query characteristics contribute to LLM hallucinations, moving beyond traditional blame on model or decoding flaws.

Method: Constructed a 22-dimension query feature vector analyzing traits like clause complexity and lexical rarity. Analyzed 369,837 real-world queries to identify correlation with hallucination.

Result: Found consistent risk patterns: deep clause nesting and underspecification increase hallucination likelihood, while grounding and answerability decrease it. Some features had mixed effects.

Conclusion: Query features empirically correlate with hallucination risk, offering insights for interventions and query rewriting.
...
TL;DR: QueryBandits is a model-agnostic framework using contextual bandits to dynamically select optimal query-rewrite strategies, reducing hallucinations in LLMs without retraining.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of solutions for hallucinations in closed-source LLMs, which dominate institutional use.

Method: Introduces QueryBandits, a framework leveraging contextual bandits and semantic features to adaptively choose query-rewrite strategies online.

Result: Achieves an 87.5% win rate over baselines and outperforms static policies by 42.6-60.3%, showing dynamic strategies are superior.

Conclusion: QueryBandits effectively mitigates hallucinations in closed-source models through forward-pass mechanisms, avoiding retraining.
...
TL;DR: Evaluation of ML methods for text classification shows BERT excels in accuracy but is resource-heavy, BiLSTM balances accuracy and efficiency, and Naive Bayes is fastest but least accurate. Class imbalance affects all methods.


<details>
  <summary>Details</summary>
Motivation: The study aims to balance classification accuracy with computational efficiency for integrating AI into real-world automation pipelines, focusing on unbalanced document categorization.

Method: Three models were examined: Naive Bayes, bidirectional LSTM, and fine-tuned BERT. Their performance was evaluated in terms of accuracy and computational efficiency, with a demonstrative system for practical validation.

Result: BERT achieved 99% accuracy but was resource-heavy. BiLSTM reached 98.56% with moderate costs. Naive Bayes was fastest (~milliseconds) but least accurate (~94.5%). Class imbalance impacted minority category recognition.

Conclusion: BiLSTM offers the most balanced solution for the scenario. Future work could explore transformer architectures further.
...
TL;DR: Exact recursive numeral systems may not be as efficient as previously thought due to overlooked complexity variations in languages.


<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that exact recursive numeral systems are communicationally efficient by examining complexity variations across languages.

Method: Analyzed data from 52 genetically diverse languages, distinguishing between predictable and unpredictable allomorphy.

Result: Many languages are less efficient than expected, highlighting overlooked complexity.

Conclusion: Findings question previous efficiency claims and have implications for studying numeral systems and linguistic evolution.
...
TL;DR: The paper explores the relationship between the effective rank of Transformer weights' unembedding matrix and model performance, finding it reflects training choices more than performance itself.


<details>
  <summary>Details</summary>
Motivation: To clarify if geometric properties of Transformer weights, like the unembedding matrix's effective rank, can reliably estimate downstream performance.

Method: Systematic experiments with 108 OLMo-style language models under controlled variations, analyzing effective rank and other geometric metrics.

Result: High effective rank often aligns with best performance but isn't universal; low rank co-occurs with late-stage degradation but isn't causal. Effective rank is strongly tied to hyperparameters like batch size and weight decay.

Conclusion: Existing geometric metrics, including effective rank, primarily reflect training choices rather than directly predicting downstream performance.
...
TL;DR: The paper introduces the Language Model Utility Taxonomy (LUX), a framework for evaluating LLMs across four domains (performance, interaction, operations, governance) to address gaps in assessing their practical utility.


<details>
  <summary>Details</summary>
Motivation: Task-level success of LLMs does not ensure practical fit in high-stakes settings, necessitating a structured approach to evaluate broader sociotechnical determinants.

Method: Developed LUX, a hierarchical framework with dimensions and metrics for quantitative comparison, supplemented by a dynamic web tool.

Result: LUX provides a unified taxonomy for assessing LLM utility across diverse use cases, enabling better model selection.

Conclusion: LUX addresses the lack of consistent evaluation methods for LLM utility, facilitating alignment of models with real-world applications.
...
TL;DR: STAR-LDM combines latent diffusion planning with autoregressive generation, introducing a 'thinking' phase for global planning, outperforming similar models in benchmarks and enabling fine-grained control without retraining.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of conventional autoregressive models by allowing global planning in continuous space before committing to discrete tokens, improving coherence and reasoning.

Method: STAR-LDM integrates latent diffusion planning with autoregressive generation, adding a 'thinking' phase to refine semantic plans via diffusion before generating tokens.

Result: It outperforms similar-sized models in language understanding benchmarks and achieves >70% win rates in LLM-as-judge comparisons for coherence and reasoning.

Conclusion: STAR-LDM offers improved performance and control, enabling fine-grained steering of attributes without retraining while maintaining fluency-control balance.
...
TL;DR: The paper introduces the R&R detector suite to identify personal information (PI) in web-scraped data used to train language models, showing that LMs memorize PI, with larger models and more pretraining increasing memorization rates. The authors recommend aggressive dataset filtering and anonymization.


<details>
  <summary>Details</summary>
Motivation: The study addresses privacy risks posed by language models memorizing personal information from training data, focusing on detecting and quantifying this memorization.

Method: The authors developed the R&R detector suite to identify PI (e.g., email, phone numbers) and tested memorization in the Pythia model suite by prompting models with context preceding PI instances.

Result: 13.6% of PI instances were memorized verbatim by Pythia-6.9B, with smaller models also showing memorization (e.g., 2.7% for Pythia-160m). Model size and pretraining duration positively correlate with memorization.

Conclusion: Aggressive filtering and anonymization of pretraining datasets are recommended to mitigate PI memorization risks.
...
TL;DR: The paper evaluates ML models and text transformation techniques for detecting and neutralizing hate speech, finding BERT superior but hybrid models useful in some cases.


<details>
  <summary>Details</summary>
Motivation: Hate speech on social media requires effective detection tools, prompting investigation into ML models and text transformation methods.

Method: Comparison of CNNs, LSTMs, BERT, and hybrid models, alongside innovative text transformation techniques.

Result: BERT excels in accuracy; hybrid models perform better in specific scenarios. Text transformation shows promise in neutralizing harmful content.

Conclusion: Highlights strengths and limitations of current technologies, suggesting future directions for robust hate speech detection systems.
...
TL;DR: The paper introduces semantic novelty as a measure of narrative structure, analyzing 28,606 pre-1920 books to identify eight narrative archetypes and linking novelty dynamics to reader engagement.


<details>
  <summary>Details</summary>
Motivation: To explore narrative structure through information-theoretic measures, specifically semantic novelty, and its impact on reader engagement.

Method: Used 768-dimensional SBERT embeddings to compute paragraph-level novelty curves, reduced to 16-segment PAA, and applied Ward-linkage clustering to identify narrative archetypes.

Result: Identified eight narrative archetypes, with volume (variance of novelty) as the strongest predictor of readership. Genre strongly influenced narrative shape, and historical trends showed decreasing predictability over time.

Conclusion: Semantic novelty dynamics are a key dimension of narrative structure, distinct from sentiment or topic, with significant implications for reader engagement.
...
TL;DR: CARE is an LLM-based framework that predicts multi-dimensional alliance scores and generates interpretable rationales from counseling transcripts, outperforming existing methods and bridging the gap between counselor evaluations and client-perceived alliance.


<details>
  <summary>Details</summary>
Motivation: Traditional methods like questionnaires are burdensome and delayed, while computational approaches lack interpretability and holistic context. CARE addresses these limitations.

Method: CARE uses rationale-augmented supervision with the LLaMA-3.1-8B-Instruct backbone, trained on the CounselingWAI dataset enriched with expert-curated rationales.

Result: CARE achieves over 70% higher Pearson correlation with client ratings, produces high-quality rationales, and identifies alliance-building challenges in real-world counseling sessions.

Conclusion: CARE demonstrates promise as an AI-assisted tool for enhancing mental health care by providing actionable insights and improving alliance assessment.
...
TL;DR: CAMEL is a confidence-gated reflection framework for reward models that improves efficiency and accuracy by selectively invoking reflection for low-confidence instances, outperforming larger models with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between efficiency and interpretability in existing reward models for aligning large language models with human preferences.

Method: CAMEL uses a lightweight single-token preference decision and selectively invokes reflection for low-confidence cases. It employs reinforcement learning with counterfactual prefix augmentation for self-correction.

Result: CAMEL achieves 82.9% average accuracy on benchmarks, surpassing prior models by 3.2% and outperforming larger models (70B parameters) using only 14B parameters.

Conclusion: CAMEL establishes a superior accuracy-efficiency balance, demonstrating its effectiveness as a scalable and efficient reward model framework.
...
TL;DR: ID-LoRA is a novel PEFT framework that reduces trainable parameters while maintaining model performance by reusing clustered parameter groups from pretrained weights.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between parameter efficiency and performance in large language model fine-tuning, especially in multi-task settings.

Method: Extracts and reuses clustered parameter groups from pretrained weights to form multiple low-rank components sharing a single trainable matrix.

Result: Outperforms full fine-tuning and existing PEFT methods (e.g., LoRA, DoRA) on benchmarks, with up to 46% fewer parameters.

Conclusion: ID-LoRA effectively breaks the performance-parameter trade-off, offering a more efficient fine-tuning solution for large models.
...
TL;DR: The paper introduces adaptive text anonymization, a method that automatically tailors anonymization strategies to specific privacy-utility needs using prompt optimization, outperforming static baselines across diverse domains and requirements.


<details>
  <summary>Details</summary>
Motivation: Existing anonymization methods are inflexible and fail to generalize across domains. Adaptive anonymization addresses this by dynamically adjusting to varying privacy and utility needs.

Method: The framework uses task-specific prompt optimization to generate anonymization instructions for language models, adapting to different goals and domains.

Result: The method consistently achieves better privacy-utility trade-offs than baselines, remains computationally efficient, and discovers novel anonymization strategies.

Conclusion: The adaptive approach is effective across diverse settings, offering flexibility and performance comparable to larger models.
...
TL;DR: The paper proposes a lightweight NLP model combining grammatical rules and semantic info, avoiding heavy transformers, outperforming baselines by 2%-15%.


<details>
  <summary>Details</summary>
Motivation: Existing transformer models are computationally intensive and unsuitable for resource-constrained environments.

Method: Incorporates grammatical rules and semantic info into a compact grammar vector fused with contextual embeddings, tested with DBNs, LSTMs, BiLSTMs, BERT, and XLNET.

Result: Outperforms baseline models by 2%-15%, capturing both semantic and structural text properties.

Conclusion: The lightweight model, treating grammar as an inductive bias, delivers better performance on edge devices.
...
TL;DR: SibylSense is an adaptive inference-time learning method that improves rubric generation for RL post-training by using a tunable memory bank and verifier-based rewards, outperforming static baselines.


<details>
  <summary>Details</summary>
Motivation: Current methods for reward design in RL post-training face challenges like cost, inconsistency, and saturation, leading to reward hacking.

Method: SibylSense adapts a rubric generator via a memory bank updated with verifier-based rewards, alternating with rubric-adversarial policy updates.

Result: Experiments demonstrate SibylSense's superiority in generating discriminative rubrics and enhancing RL performance over non-adaptive baselines.

Conclusion: SibylSense addresses scalability and robustness in reward design, offering a practical solution for open-ended generation tasks.
...
TL;DR: OP-GRPO is a reinforcement learning framework enabling a single LLM to generate diverse perspectives without explicit prompts, improving accuracy and diversity in responses.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitation of current alignment paradigms in capturing diverse human values by introducing implicit Overton Pluralism.

Method: Involves similarity estimator training for accurate coverage evaluation and OP-GRPO training with a dual-reward system to ensure diverse and unique perspectives.

Result: Achieves significant accuracy gains (37.4%) over baselines and improved diversity, validated by GPT-4.1 evaluations.

Conclusion: OP-GRPO effectively enhances perspective diversity and accuracy in LLMs, demonstrating scalability and robustness.
...
TL;DR: The paper introduces a tail-aware divergence method for language model distillation, addressing the dominance of high-probability tokens in traditional KL divergence. This approach balances the influence of high and low-probability predictions, improving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Traditional KL divergence in language model distillation overly focuses on high-probability tokens (teacher modes), neglecting less probable but informative tokens. This motivates a need for a balanced approach.

Method: The authors propose a tail-aware divergence that decouples top-K predicted probabilities from lower-probability predictions, maintaining computational efficiency similar to KL divergence.

Result: Experiments show competitive performance in pre-training and supervised distillation tasks across datasets. The method is efficient and feasible with modest budgets.

Conclusion: The tail-aware divergence effectively balances token influence in distillation, enhancing performance and practicality without requiring extensive computational resources.
...
TL;DR: FinAnchor is a lightweight framework integrating embeddings from multiple LLMs for financial prediction, outperforming single-model baselines and standard ensemble methods.


<details>
  <summary>Details</summary>
Motivation: Financial prediction from long documents is challenging due to sparse actionable signals and noise, with varying optimal LLMs across tasks and time.

Method: FinAnchor integrates embeddings from multiple LLMs without fine-tuning, aligning representations into a unified anchor space using linear mappings.

Result: FinAnchor consistently outperforms single-model baselines and standard ensemble methods across financial NLP tasks.

Conclusion: FinAnchor's anchoring of heterogeneous representations proves effective for robust financial prediction.
...
TL;DR: The paper introduces a Persian sentiment analysis dataset called Exa, collected from Twitter, annotated with 12,000 tweets labeled as positive, neutral, or negative. Pars Bert and Roberta models evaluated the dataset, achieving a 79.87 Macro F-score.


<details>
  <summary>Details</summary>
Motivation: Persian sentiment analysis faces challenges due to limited datasets and complexities like irony and colloquial language. A dedicated Twitter dataset is needed.

Method: Collected 12,000 Persian tweets annotated by 5 native speakers. Evaluated using Pars Bert and Roberta pre-trained models.

Result: Achieved a Macro F-score of 79.87, indicating the dataset's utility for sentiment analysis.

Conclusion: The Exa dataset and tested models are valuable for Persian sentiment analysis, addressing gaps in existing resources.
...
TL;DR: The paper explores efficient reasoning in LLMs using scaled CoT, advocating fine-grained metrics and revealing a two-stage training process (length adaptation and reasoning refinement). Key findings include training on easier prompts to avoid length collapse and generalizing length bias across domains, validated across the Qwen3 series.


<details>
  <summary>Details</summary>
Motivation: To address the computational overhead of scaled CoT reasoning in LLMs, the paper aims to incentivize short yet accurate thinking trajectories through systematic investigation and reward shaping with RL.

Method: The study employs fine-grained metrics (e.g., length distribution and performance across token budgets) and conducts extensive experiments (0.2 million GPU hours) analyzing training prompts, rollouts, reward shaping, and optimization strategies.

Result: Key findings include a two-stage training paradigm (length adaptation and reasoning refinement), avoidance of length collapse by training on easier prompts, and generalization of learned length bias across domains.

Conclusion: The paper distills insights into practical guidelines, validated across the Qwen3 series (0.6B to 30B), demonstrating robustness and generalization of the efficient reasoning approach.
...
TL;DR: The paper introduces the Blackbird Language Matrices (BLM) task, a structured language challenge inspired by intelligence tests, to evaluate large language models' abilities in detecting linguistic objects and systematic patterns. It demonstrates BLMs' effectiveness through benchmarking and targeted experiments.


<details>
  <summary>Details</summary>
Motivation: To create a curated, structured dataset (BLM) that can systematically evaluate large language models' abilities in linguistic detection, pattern recognition, and error analysis.

Method: Developed BLM datasets with multiple-choice problems structured at various levels (within sentences, across sequences, and within answers). Conducted benchmarking and experiments on chunking and systematicity using baseline and tailored models.

Result: BLMs are solvable with good performance, showing that models detect grammatical objects and systematic patterns. The datasets support multifaceted investigations into language model properties.

Conclusion: BLMs provide a valuable tool for explainability investigations into large language models, offering insights into their linguistic and reasoning behaviors due to their curated and structured nature.
...
TL;DR: The paper introduces a new dataset, PC-FOL, to evaluate LLMs' case-based reasoning in math, revealing a performance gap between linear and case-based reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for evaluating LLMs' mathematical reasoning focus mainly on linear reasoning, neglecting other crucial methods like proof by contradiction and cases.

Method: The authors created PC-FOL, a first-order logic dataset annotated by mathematicians, featuring case-based problems with natural language proofs.

Result: Experiments show a significant performance gap between linear and case-based reasoning in LLMs, explained via a graphical model analysis.

Conclusion: The study highlights key challenges in automated math proof generation, encouraging further research in non-linear reasoning.
...
TL;DR: The paper introduces a proactive risk awareness evaluation framework for LLMs, using the Butterfly dataset to test their ability to anticipate ecological harms, revealing gaps in current safety measures.


<details>
  <summary>Details</summary>
Motivation: To address the growing need for LLMs to anticipate unintended ecological risks beyond reacting to explicit harmful intent.

Method: Developed the Butterfly dataset with 1,094 queries simulating ordinary activities with latent ecological impact, and evaluated five LLMs across response length, languages, and modality.

Result: LLMs showed significant declines in proactive awareness under length restrictions, cross-lingual similarities, and persistent blind spots in species protection.

Conclusion: Highlights a critical gap in current safety alignment and emphasizes the need for proactive safeguards in LLM deployment.
...
TL;DR: The paper proposes a hybrid method using LLMs for aspect identification and classic ML for sentiment analysis to scale review analysis, achieving significant results with 4.7 million restaurant reviews.


<details>
  <summary>Details</summary>
Motivation: Analyzing unstructured customer reviews at scale is challenging; LLMs show promise but face computational and scalability limitations.

Method: Combines LLMs (ChatGPT) for aspect identification and classic ML for sentiment classification, tested on 4.7 million restaurant reviews.

Result: Machine-labeled aspects significantly explain variance in restaurant ratings across dining experiences, cuisines, and regions.

Conclusion: Hybrid LLM-ML approach effectively automates large-scale sentiment analysis, offering practical value for hospitality and service sectors.
...
### [135] [Prompt-Level Distillation: A Non-Parametric Alternative to Model Fine-Tuning for Efficient Reasoning](https://arxiv.org/abs/2602.21103)
*Sanket Badhe,Deep Shah*

Main category: cs.CL

TL;DR: Prompt-Level Distillation (PLD) improves reasoning accuracy and transparency in smaller models by extracting structured instructions from a Teacher model, achieving near-frontier performance with low latency.


<details>
  <summary>Details</summary>
Motivation: To address the prohibitive latency and inference costs of Chain-of-Thought prompting and the interpretability loss of fine-tuning smaller models.

Method: Extract reasoning patterns from a Teacher model and organize them into structured instructions for the Student model's System Prompt.

Result: Improved Macro F1 scores from 57% to 90.0% on StereoSet and 67% to 83% on Contract-NLI using Gemma-3 4B.

Conclusion: PLD enables high performance with transparency, making it suitable for regulated industries and edge devices.
...
### [136] [PVminer: A Domain-Specific Tool to Detect the Patient Voice in Patient Generated Data](https://arxiv.org/abs/2602.21165)
...
TL;DR: PVminer is an NLP framework for analyzing patient-generated text, combining BERT encoders, topic modeling, and fine-tuned classifiers to detect patient voice and social determinants of health effectively.


<details>
  <summary>Details</summary>
Motivation: Traditional qualitative coding is labor-intensive and unsuitable for large-scale analysis of patient-authored messages. Existing ML/NLP methods often treat patient-centered communication and social determinants of health separately or use models not optimized for patient language.

Method: PVminer integrates patient-specific BERT encoders (PV-BERT-base and PV-BERT-large), unsupervised topic modeling (PV-Topic-BERT), and fine-tuned classifiers for hierarchical label prediction (Code, Subcode, Combo). Topic representations enhance semantic inputs.

Result: PVminer achieves strong performance with F1 scores of 82.25% (Code), 80.14% (Subcode), and up to 77.87% (Combo), outperforming biomedical and clinical baselines. Ablation studies confirm the value of author identity and topic-based augmentation.

Conclusion: PVminer effectively structures patient voice in secure communication, offering a scalable solution for analyzing patient-generated text. Pre-trained models and datasets will be publicly released.
...
### [137] [On Data Engineering for Scaling LLM Terminal Capabilities](https://arxiv.org/abs/2602.21193)
*Renjie Pi,Grace Lam,Mohammad Shoeybi,Pooya Jannaty,Bryan Catanzaro,Wei Ping*

Main category: cs.CL

TL;DR: The paper introduces a synthetic task generation pipeline (Terminal-Task-Gen) and analyzes data/training strategies for terminal agents, leading to improved model performance on Terminal-Bench 2.0.


<details>
  <summary>Details</summary>
Motivation: To address the lack of disclosed training data strategies for state-of-the-art terminal agents.

Method: Developed Terminal-Task-Gen for synthetic task construction and studied data strategies like filtering and curriculum learning.

Result: Trained models (Nemotron-Terminal) showed significant performance gains, e.g., 8B model improved from 2.5% to 13.0%.

Conclusion: The open-source contributions (dataset and models) aim to accelerate research in terminal agent capabilities.
...
### [138] [HieraMAS: Optimizing Intra-Node LLM Mixtures and Inter-Node Topology for Multi-Agent Systems](https://arxiv.org/abs/2602.20229)
*Tianjun Yao,Zhaoyi Li,Zhiqiang Shen*

Main category: cs.MA

TL;DR: HieraMAS is a hierarchical multi-agent system framework combining intra-node LLM mixtures and inter-node communication, outperforming existing methods with improved cost-performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems treat agents as single units, missing opportunities to enhance role-specific abilities by using mixtures of LLMs within agents.

Method: Proposes HieraMAS, a hierarchical framework with supernodes (each role implemented by multiple LLMs) and a two-stage algorithm for credit assignment and topology selection.

Result: HieraMAS outperforms existing methods on reasoning and coding benchmarks, offering better cost-performance trade-offs.

Conclusion: HieraMAS successfully addresses limitations of current MAS approaches by leveraging hierarchical collaboration and fine-grained optimization techniques.
...
### [139] [Tensor Network Generator-Enhanced Optimization for Traveling Salesman Problem](https://arxiv.org/abs/2602.20175)
*Ryo Sakai,Chen-Yu Liu*

Main category: cs.LG

TL;DR: TN-GEO framework uses tensor networks and MPS for TSP, outperforming classical heuristics with k-site variants showing better results.


<details>
  <summary>Details</summary>
Motivation: Address the TSP with a novel approach using tensor networks for efficient optimization and valid tour generation.

Method: Uses tensor network Born machine with MPS for permutation-based TSP, autoregressive sampling, and k-site MPS for local correlations.

Result: Outperforms classical heuristics on TSPLIB instances (up to 52 cities), with k-site variants performing better than full-MPS.

Conclusion: TN-GEO is effective for TSP, especially with k-site MPS, offering a promising direction for combinatorial optimization.
...
### [140] [MoBiQuant: Mixture-of-Bits Quantization for Token-Adaptive Elastic LLMs](https://arxiv.org/abs/2602.20191)
...
TL;DR: MoBiQuant is a Mixture-of-Bits quantization framework enabling elastic LLM inference by adjusting weight precision based on token sensitivity, avoiding repeated calibration.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in varying calibration parameters for quantization due to precision-dependent outlier migration, necessitating a flexible solution for elastic LLM deployment.

Method: Proposes many-in-one recursive residual quantization and a token-aware router to dynamically adjust weight precision based on token sensitivity.

Result: MoBiQuant achieves strong elasticity and matches bit-specific calibrated PTQ performance on LLaMA3-8B without repeated calibration.

Conclusion: MoBiQuant effectively addresses elastic-precision calibration challenges, enabling smooth precision switching and improved generalization for token outliers.
...
### [142] [FedAvg-Based CTMC Hazard Model for Federated Bridge Deterioration Assessment](https://arxiv.org/abs/2602.20194)
*Takato Yasuno*

Main category: cs.LG

TL;DR: A federated framework for bridge deterioration modeling allows municipalities to collaboratively train a shared CTMC hazard model without sharing raw inspection data, using only aggregated pseudo-gradient vectors.


<details>
  <summary>Details</summary>
Motivation: Existing data governance constraints hinder cross-organizational sharing of sensitive bridge inspection records, necessitating a privacy-preserving solution for collaborative model training.

Method: A federated approach where local log-linear hazard models are trained on three deterioration transitions. Mini-batch SGD is used for local optimization, and FedAvg aggregates updates from Users via pseudo-gradient vectors.

Result: Simulations on synthetic data show consistent convergence of the model. Federated updates provide Users with global benchmark parameters, incentivizing participation without data sharing.

Conclusion: The framework enables collaborative, privacy-preserving bridge deterioration modeling, offering shared insights while maintaining data sovereignty.
...
### [143] [Controllable Exploration in Hybrid-Policy RLVR for Multi-Modal Reasoning](https://arxiv.org/abs/2602.20197)
*Zhuoxu Huang,Mengxi Jia,Hao Sun,Xuelong Li,Jungong Han*

Main category: cs.LG

TL;DR: CalibRL is a hybrid-policy RLVR framework that enhances exploration in MLLMs by using expert-guided, distribution-aware updates to avoid entropy collapse and policy degradation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of sparse rewards and entropy collapse in RL training for MLLMs, which hinder effective exploration.

Method: Proposes CalibRL with distribution-aware advantage weighting and LeakyReLU activation to guide exploration using expert knowledge.

Result: Shows consistent improvements in performance across eight benchmarks, balancing exploration and exploitation effectively.

Conclusion: CalibRL successfully mitigates distributional mismatch and stabilizes training, validating its hybrid-policy approach.
...
### [144] [IMOVNO+: A Regional Partitioning and Meta-Heuristic Ensemble Framework for Imbalanced Multi-Class Learning](https://arxiv.org/abs/2602.20199)
*Soufiane Bacha,Laouni Djafri,Sahraoui Dhelim,Huansheng Ning*

Main category: cs.LG

TL;DR: IMOVNO+ is a two-level framework addressing class imbalance, overlap, and noise in binary and multi-class tasks, improving data quality and algorithmic robustness with superior results.


<details>
  <summary>Details</summary>
Motivation: Class imbalance, overlap, and noise degrade performance, especially in multi-class settings where traditional methods fall short.

Method: Combines data-level techniques (informativeness quantification, region partitioning, overlapping-cleaning, smart oversampling) with algorithmic-level ensemble pruning.

Result: Outperforms state-of-the-art methods, achieving near-100% performance in some cases and significant improvements in multi-class and binary tasks.

Conclusion: IMOVNO+ effectively tackles data and algorithmic challenges, showing robustness and scalability for diverse datasets.
...
### [145] [Golden Layers and Where to Find Them: Improved Knowledge Editing for Large Language Models Via Layer Gradient Analysis](https://arxiv.org/abs/2602.20207)
*Shrestha Datta,Hongfu Liu,Anshuman Chhabra*

Main category: cs.LG

TL;DR: The paper explores knowledge editing in LLMs, hypothesizing fixed 'golden layers' for near-optimal editing performance, validates this with empirical evidence, and introduces Layer Gradient Analysis (LGA) for efficient identification.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve knowledge editing in LLMs by identifying fixed layers ('golden layers') that perform nearly as well as sample-wise optimal layers, reducing the need for extensive trial-and-error.

Method: The method involves hypothesizing golden layers, validating them empirically, and proposing Layer Gradient Analysis (LGA) for efficient identification using gradient-attribution.

Result: Results show golden layers generalize well to unseen queries and datasets, and LGA proves effective and robust across different LLM types and editing methods.

Conclusion: The paper concludes that golden layers and LGA offer efficient and reliable solutions for knowledge editing in LLMs, reducing reliance on sample-wise optimization.
...
TL;DR: MCFlow is a unified multimodal flow model for crystal generation tasks, outperforming task-specific baselines.


<details>
  <summary>Details</summary>
Motivation: Existing deep generative models lack a unified framework for crystal representation across tasks.

Method: Introduces multimodal flow with independent time variables for atom types and structures, using composition- and symmetry-aware atom ordering.

Result: Achieves competitive performance on MP-20 and MPTS-52 benchmarks.

Conclusion: MCFlow effectively unifies and improves crystal generation tasks.
...
### [147] [KnapSpec: Self-Speculative Decoding via Adaptive Layer Selection as a Knapsack Problem](https://arxiv.org/abs/2602.20217)
*Seongjin Cha,Gyuwan Kim,Dongsu Han,Tao Yang,Insu Han*

Main category: cs.LG

TL;DR: KnapSpec accelerates LLM inference by dynamically selecting optimal draft configurations via a knapsack problem approach, outperforming existing methods with up to 1.47x speedup.


<details>
  <summary>Details</summary>
Motivation: Existing self-speculative decoding methods rely on static heuristics, ignoring dynamic computational overhead in long-context scenarios.

Method: Reformulates draft selection as a knapsack problem, decouples Attention and MLP layers, and uses cosine similarity for drafting faithfulness.

Result: Achieves up to 1.47x wall-clock speedup on Qwen3 and Llama3 benchmarks.

Conclusion: KnapSpec offers a plug-and-play solution for high-speed inference without additional training or output compromise.
...
### [148] [MultiModalPFN: Extending Prior-Data Fitted Networks for Multimodal Tabular Learning](https://arxiv.org/abs/2602.20223)
*Wall Kim,Chaeyoung Song,Hanul Kim*

Main category: cs.LG

TL;DR: MMPFN extends TabPFN to handle heterogeneous data modalities like images and text, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: TabPFN struggles with integrating non-tabular modalities, limiting its use in domains like healthcare and marketing.

Method: MMPFN uses per-modality encoders, modality projectors, and foundation models, introducing a multi-head gated MLP and cross-attention pooler.

Result: MMPFN outperforms state-of-the-art methods on medical and general-purpose datasets, effectively combining tabular and non-tabular features.

Conclusion: MMPFN offers a scalable framework for multimodal learning, showing promise for extending prior-data fitted networks.
...
### [149] [Exploring Anti-Aging Literature via ConvexTopics and Large Language Models](https://arxiv.org/abs/2602.20224)
*Lana E. Yeganova,Won G. Kim,Shubo Tian,Natalie Xie,Donald C. Comeau,W. John Wilbur,Zhiyong Lu*

Main category: cs.LG

TL;DR: A convex optimization-based clustering method is proposed for stable, interpretable topic modeling, outperforming K-means, LDA, and BERTopic on biomedical literature.


<details>
  <summary>Details</summary>
Motivation: Scalable and interpretable methods are needed to organize biomedical knowledge and detect trends due to rapid publication growth.

Method: Reformulated convex optimization clustering selects exemplars for stable, fine-grained topics, ensuring global optimum.

Result: Applied to 12,000 PubMed articles, the method produced expert-validated topics on aging and longevity, outperforming common approaches.

Conclusion: The method's reproducibility and interpretability advance scalable tools for biomedical knowledge discovery.
...
### [150] [Coupled Cluster con MōLe: Molecular Orbital Learning for Neural Wavefunctions](https://arxiv.org/abs/2602.20232)
...
TL;DR: The paper introduces MōLe, an equivariant machine learning model that predicts coupled-cluster excitation amplitudes to address the computational cost limitations of traditional CC methods, demonstrating efficiency and generalization potential.


<details>
  <summary>Details</summary>
Motivation: Coupled-cluster (CC) theory is highly accurate but computationally expensive, limiting its widespread use. The goal is to develop a machine learning solution (MōLe) to predict CC excitation amplitudes efficiently.

Method: The MōLe architecture uses equivariant machine learning to predict CC excitation amplitudes directly from Hartree-Fock molecular orbitals, trained on small equilibrium geometries but tested for broader applicability.

Result: MōLe shows remarkable data efficiency and generalization to larger molecules and off-equilibrium geometries, reducing CC calculation convergence cycles.

Conclusion: MōLe lays groundwork for high-accuracy, wavefunction-based machine learning models, potentially accelerating molecular design and complementing force-field methods.
...
### [151] [Uncertainty-Aware Delivery Delay Duration Prediction via Multi-Task Deep Learning](https://arxiv.org/abs/2602.20271)
*Stefan Faulkner,Reza Zandehshahvar,Vahid Eghbal Akhlaghi,Sebastien Ouellet,Carsten Jordan,Pascal Van Hentenryck*

Main category: cs.LG

TL;DR: A multi-task deep learning model improves delivery delay prediction in complex supply chains by handling imbalanced data and enabling probabilistic forecasting.


<details>
  <summary>Details</summary>
Motivation: Accurate delay prediction is vital for supply chain efficiency but challenging due to logistics complexity and data imbalance.

Method: The model uses embedding layers for tabular data and a classification-then-regression strategy, allowing end-to-end training.

Result: Outperforms baselines by 41-64%, achieving MAE of 0.67-0.91 days for delayed shipments.

Conclusion: The model effectively addresses imbalance and heterogeneity in delivery delay forecasting.
...
### [152] [The Truthfulness Spectrum Hypothesis](https://arxiv.org/abs/2602.20273)
*Zhuofan Josh Ying,Shauli Ravfogel,Nikolaus Kriegeskorte,Peter Hase*

Main category: cs.LG

TL;DR: LLMs encode truthfulness in a spectrum from domain-general to domain-specific directions, with linear probes generalizing well except for sycophantic lying. Joint training improves performance, and probe geometry explains generalization patterns. Post-training affects truth geometry.


<details>
  <summary>Details</summary>
Motivation: To reconcile conflicting views on how LLMs encode truthfulness by proposing a spectrum hypothesis and testing it across diverse truth types and lying scenarios.

Method: Systematic evaluation of probe generalization across truth types, lying scenarios, and benchmarks, using concept-erasure methods and causal interventions.

Result: Linear probes generalize well but fail on sycophantic lying; joint training recovers performance. Probe geometry predicts generalization, and post-training reshapes truth directions.

Conclusion: Truthfulness in LLMs exists on a spectrum of generality, with domain-specific directions being more effective. Post-training influences sycophantic lying representations.
...
### [153] [Discrete Diffusion with Sample-Efficient Estimators for Conditionals](https://arxiv.org/abs/2602.20293)
*Karthik Elamvazhuthi,Abhijith Jayakumar,Andrey Y. Lokhov*

Main category: cs.LG

TL;DR: The paper introduces a discrete denoising diffusion framework using single-site conditional probabilities, estimated efficiently with NeurISE, outperforming existing methods on binary data sets.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve generative modeling over discrete state spaces by integrating efficient conditional probability estimation into diffusion dynamics.

Method: The framework employs NeurISE for estimating single-site conditionals and uses round-robin noising and denoising dynamics.

Result: Experiments on synthetic and real-world datasets show superior performance in total variation, cross-correlations, and kernel density metrics.

Conclusion: The proposed approach effectively enhances discrete denoising diffusion models, surpassing traditional methods.
...
### [154] [Learning to Solve Complex Problems via Dataset Decomposition](https://arxiv.org/abs/2602.20296)
*Wanru Zhao,Lucas Caccia,Zhengyan Shi,Minseon Kim,Weijia Xu,Alessandro Sordoni*

Main category: cs.LG

TL;DR: The paper proposes a reverse curriculum learning method where complex datasets are decomposed into simpler components, using a teacher-student framework with step-by-step reasoning. A scoring system measures data difficulty, and experiments show improved model performance.


<details>
  <summary>Details</summary>
Motivation: To enhance learning efficiency by organizing data from simple to complex, leveraging a teacher-student framework for better task mastery.

Method: A reverse curriculum generation approach recursively decomposes complex data, aided by a teacher-student framework and a novel difficulty scoring system.

Result: Models trained with the proposed curriculum outperform those trained on original datasets, validated on math and code generation tasks.

Conclusion: Reverse curriculum learning with recursive decomposition and difficulty scoring improves model performance on complex tasks.
...
### [155] [Shape-informed cardiac mechanics surrogates in data-scarce regimes via geometric encoding and generative augmentation](https://arxiv.org/abs/2602.20306)
*Davide Carrara,Marc Hirschvogel,Francesca Bonizzoni,Stefano Pagani,Simone Pezzuto,Francesco Regazzoni*

Main category: cs.LG

TL;DR: A two-step surrogate modeling framework decouples geometric representation from physics learning to enable accurate cardiac mechanics predictions under data-scarce conditions.


<details>
  <summary>Details</summary>
Motivation: Accelerating cardiac mechanics simulations for clinical use while handling diverse anatomies and data scarcity.

Method: 1) Shape model learns compact latent representations for synthetic data generation. 2) Neural field-based surrogate predicts ventricular displacement using these encodings and positional encoding via universal ventricular coordinates.

Result: Accurate predictions and generalization to unseen geometries, robust to noisy or sparse inputs.

Conclusion: The framework effectively addresses data scarcity and anatomical diversity in surrogate modeling for cardiac mechanics.
...
### [156] [In-context Pre-trained Time-Series Foundation Models adapt to Unseen Tasks](https://arxiv.org/abs/2602.20307)
*Shangqing Xu,Harshavardhan Kamarthi,Haoxin Liu,B. Aditya Prakash*

Main category: cs.LG

TL;DR: TSFMs enhanced with In-Context Learning (ICL) via ICTP framework improve generalization by 11.4% on unseen tasks without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing TSFMs lack generalization to unseen tasks without fine-tuning, prompting the need for ICL capabilities.

Method: Proposed ICTP framework restructures pre-training data to enable ICL, allowing dynamic adaptation to input-output relationships.

Result: ICTP improves performance of TSFMs by ~11.4% on unseen tasks without fine-tuning.

Conclusion: ICTP successfully equips TSFMs with ICL capabilities, enhancing generalization to unseen tasks.
...
### [157] [QuantVLA: Scale-Calibrated Post-Training Quantization for Vision-Language-Action Models](https://arxiv.org/abs/2602.20309)
*Jingxuan Zhang,Yunta Hsieh,Zhongwei Wang,Haokun Lin,Xin Wang,Ziqi Wang,Yingtie Lei,Mi Zhang*

Main category: cs.LG

TL;DR: QuantVLA is a training-free post-training quantization framework for VLA models, reducing compute and memory demands while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of high compute and memory demands in vision-language-action (VLA) models as they scale.

Method: Introduces QuantVLA with selective quantization layout, attention temperature matching, and output head balancing, requiring no additional training.

Result: Achieves 70% memory savings, 1.22x speedup, and outperforms full-precision baselines in task success rates.

Conclusion: QuantVLA offers a practical solution for scalable low-bit VLA systems under strict constraints.
...
TL;DR: CaDrift is a synthetic data generator using SCMs to create time-dependent data streams with controlled shifts, useful for evaluating evolving data methods.


<details>
  <summary>Details</summary>
Motivation: To provide a tool for evaluating methods under evolving data by simulating controlled distributional and covariate shifts.

Method: Leverages SCMs to drift mapping functions, altering causal relationships and modeling perturbations via interventions.

Result: Classifier accuracy drops post-shift but gradually recovers, validating CaDrift's shift simulation.

Conclusion: CaDrift effectively simulates shifts and is available for use on GitHub.
...
TL;DR: CoT prompting enhances reasoning in LLMs, but its temporal dynamics are unclear. Using MCT, this study shows reasoning involves transient geometric changes in latent representations, distinct from standard linear probe accuracy.


<details>
  <summary>Details</summary>
Motivation: To understand the temporal dynamics of latent representation geometry during reasoning in Large Language Models using CoT prompting.

Method: Applied Manifold Capacity Theory (MCT) to a Boolean logic task to analyze linear separability of latent representations without probe training.

Result: Reasoning involves transient geometric pulses where concept manifolds become linearly separable before computation and compress afterward, differing from standard probe accuracy.

Conclusion: Dynamic Manifold Management is proposed, where models modulate representational capacity to optimize reasoning bandwidth.
...
TL;DR: GraSPNet is a hierarchical GSSL framework for molecular graphs that models atomic- and fragment-level semantics, outperforming existing methods in transfer learning.


<details>
  <summary>Details</summary>
Motivation: Existing GSSL methods often overlook chemically relevant substructures, which are crucial for molecular property prediction. GraSPNet addresses this gap by explicitly modeling fragment-level semantics.

Method: GraSPNet decomposes molecular graphs into meaningful fragments and learns multi-level representations via masked semantic prediction and message passing.

Result: GraSPNet achieves superior performance on molecular property prediction benchmarks and learns chemically meaningful embeddings.

Conclusion: GraSPNet's hierarchical approach effectively captures multi-resolution structural information, enhancing representation expressiveness and transferability.
...
### [161] [Momentum Guidance: Plug-and-Play Guidance for Flow Models](https://arxiv.org/abs/2602.20360)
*Runlong Liao,Jian Yu,Baiyu Su,Chi Zhang,Lizhang Chen,Qiang Liu*

Main category: cs.LG

TL;DR: Momentum Guidance (MG) enhances flow-based generative models by leveraging the ODE trajectory for improved sample quality without extra computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing guidance techniques like classifier-free guidance (CFG) improve sample fidelity but increase inference cost and reduce diversity, prompting the need for a more efficient solution.

Method: MG uses exponential moving averages of past velocities in the ODE trajectory to extrapolate current velocity, preserving computational efficiency.

Result: MG significantly improves FID scores (e.g., 36.68% without CFG, 25.52% with CFG on ImageNet-256) and achieves high-quality samples with minimal steps (FID 1.597 at 64 steps).

Conclusion: MG is an effective and computationally efficient guidance method that enhances sample quality and works well alongside CFG, validated across multiple benchmarks.
...
### [162] [Quantitative Approximation Rates for Group Equivariant Learning](https://arxiv.org/abs/2602.20370)
*Jonathan W. Siegel,Snir Hordan,Hannah Lawrence,Ali Syed,Nadav Dym*

Main category: cs.LG

TL;DR: This paper bridges the gap in quantitative approximation rates for group-equivariant neural networks, showing they match the expressivity of ReLU MLPs.


<details>
  <summary>Details</summary>
Motivation: To extend quantitative approximation theory to group-equivariant learning, as prior work lacks such results despite interest in equivariant models' universal approximation properties.

Method: Derives approximation rates for architectures like Deep Sets, Sumformer, Transformer, frame-averaging invariant networks, and bi-Lipschitz invariant models.

Result: Equivariant architectures are as expressive as ReLU MLPs for equivariant functions, confirming no loss of approximation power.

Conclusion: Hard-coded equivariance retains expressivity, validating its use in neural networks.
...
### [163] [cc-Shapley: Measuring Multivariate Feature Importance Needs Causal Context](https://arxiv.org/abs/2602.20396)
*Jörg Martin,Stefan Haufe*

Main category: cs.LG

TL;DR: The paper critiques purely data-driven Shapley values for feature importance, highlighting issues like collider bias and suppression. It introduces cc-Shapley, a causal-context-adjusted version, to correct misleading interpretations by incorporating causal knowledge. Theoretical and empirical results show improved accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional Shapley values in feature importance analysis, which can produce spurious associations due to observational biases like collider bias.

Method: Proposes cc-Shapley, an interventional modification of Shapley values that incorporates causal knowledge of the data-generating process to correct misleading attributions.

Result: Demonstrates theoretically and empirically that cc-Shapley eliminates spurious associations caused by collider bias, showing improved accuracy compared to traditional Shapley values.

Conclusion: Causal context-aware methods like cc-Shapley are essential for accurate feature importance analysis, overcoming limitations of purely data-driven approaches.
...
### [164] [GeoPT: Scaling Physics Simulation via Lifted Geometric Pre-Training](https://arxiv.org/abs/2602.20399)
*Haixu Wu,Minghao Guo,Zongyi Li,Zhiyang Dou,Mingsheng Long,Kaiming He,Wojciech Matusik*

Main category: cs.LG

TL;DR: GeoPT is a pre-trained model for physics simulation that uses synthetic dynamics on geometry to bridge the gap between static data and dynamic tasks, reducing labeled data needs and speeding up convergence.


<details>
  <summary>Details</summary>
Motivation: The high cost of generating high-fidelity training data for neural simulators motivates the need for scalable alternatives like pre-training on off-the-shelf geometries, but static geometry alone lacks dynamics awareness.

Method: GeoPT augments geometry with synthetic dynamics for self-supervision without physics labels, pre-trained on over one million samples.

Result: GeoPT improves industrial benchmarks (e.g., fluid and solid mechanics) by reducing labeled data needs by 20-60% and accelerating convergence by 2×.

Conclusion: Lifting geometry with synthetic dynamics effectively bridges the gap, offering a scalable solution for neural simulation.
...
### [165] [Three Concrete Challenges and Two Hopes for the Safety of Unsupervised Elicitation](https://arxiv.org/abs/2602.20400)
*Callum Canavan,Aditya Shrivastava,Allison Qi,Jonathan Michala,Fabien Roger*

Main category: cs.LG

TL;DR: The paper critiques existing methods for steering language models towards truthful outputs, highlighting overoptimistic evaluations due to dataset limitations. It tests these methods on specially constructed challenging datasets and finds none perform reliably.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the limitations in current evaluations of techniques (easy-to-hard generalization and unsupervised elicitation) for improving model truthfulness, which may not reflect real-world complexity.

Method: The authors create datasets lacking key properties (e.g., balanced training sets, well-defined answers) to rigorously test standard techniques. They also explore ensembling and combining methods.

Result: None of the tested techniques (including ensembling and combined approaches) reliably perform well on the challenging datasets.

Conclusion: Overcoming the identified dataset challenges should be a priority for future research in unsupervised elicitation to ensure robust and truthful model outputs.
...
### [166] [Wasserstein Distributionally Robust Online Learning](https://arxiv.org/abs/2602.20403)
*Guixian Chen,Salar Fattahi,Soroosh Shafiee*

Main category: cs.LG

TL;DR: This paper tackles online distributionally robust learning, addressing convergence and computational challenges by framing the problem as a saddle-point game and offering an efficient algorithm for piecewise concave loss functions.


<details>
  <summary>Details</summary>
Motivation: To extend distributionally robust optimization (DRO) from offline to online settings, overcoming challenges in convergence and computation.

Method: Formulates the problem as an online saddle-point stochastic game and proposes a tailored algorithm for piecewise concave loss functions.

Result: Achieves convergence to a robust Nash equilibrium matching offline DRO solutions and outperforms existing solvers like Gurobi.

Conclusion: The framework and algorithm effectively address online DRO challenges, with potential broader applications due to connections to budget allocation problems.
...
### [167] [$κ$-Explorer: A Unified Framework for Active Model Estimation in MDPs](https://arxiv.org/abs/2602.20404)
*Xihe Gu,Urbashi Mitra,Tara Javidi*

Main category: cs.LG

TL;DR: The paper introduces κ-Explorer, an active exploration algorithm for tabular MDPs, optimizing state-action visitation frequencies based on a parameterized family of decomposable concave objectives. It outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of accurate model estimation in MDPs by explicitly incorporating intrinsic estimation complexity and extrinsic visitation frequency.

Method: Proposes κ-Explorer, an algorithm optimizing state-action occupancy measures using Frank-Wolfe-style optimization of decomposable concave objectives.

Result: Superior performance demonstrated on benchmark MDPs compared to existing exploration strategies.

Conclusion: κ-Explorer effectively balances exploration and exploitation, offering tight regret guarantees and practical utility.
...
### [168] [CITED: A Decision Boundary-Aware Signature for GNNs Towards Model Extraction Defense](https://arxiv.org/abs/2602.20418)
*Bolin Shen,Md Shamim Seraj,Zhan Cheng,Shayok Chakraborty,Yushun Dong*

Main category: cs.LG

TL;DR: CITED is a novel ownership verification framework for GNNs that defends against Model Extraction Attacks by verifying ownership at both embedding and label levels without compromising performance or efficiency.


<details>
  <summary>Details</summary>
Motivation: Large-scale GNN deployment via MLaaS faces risks from Model Extraction Attacks, where attackers steal model functionality. Existing defenses are limited to specific output levels and have technical flaws.

Method: Proposes CITED, a signature-based ownership verification framework that works at embedding and label levels, avoiding performance loss and auxiliary models.

Result: CITED outperforms watermarking and fingerprinting methods, proving effective and robust in experiments.

Conclusion: CITED provides a comprehensive solution for GNN ownership verification against MEAs, maintaining model performance and efficiency.
...
### [169] [CREDIT: Certified Ownership Verification of Deep Neural Networks Against Model Extraction Attacks](https://arxiv.org/abs/2602.20419)
*Bolin Shen,Zhan Cheng,Neil Zhenqiang Gong,Fan Yao,Yushun Dong*

Main category: cs.LG

TL;DR: CREDIT introduces certified ownership verification against Model Extraction Attacks (MEAs) in MLaaS, using mutual information for similarity and rigorous theoretical guarantees.


<details>
  <summary>Details</summary>
Motivation: MLaaS is vulnerable to MEAs, where adversaries replicate DNN models. Existing defenses lack strict theoretical ownership verification.

Method: CREDIT employs mutual information to quantify DNN similarity, proposes a verification threshold, and provides theoretical guarantees.

Result: Evaluated on multiple datasets, CREDIT achieves state-of-the-art performance.

Conclusion: CREDIT effectively addresses ownership verification in MLaaS with certified guarantees against MEAs.
...
### [170] [GauS: Differentiable Scheduling Optimization via Gaussian Reparameterization](https://arxiv.org/abs/2602.20427)
*Yaohui Cai,Vesal Bakhtazad,Cunxi Yu,Zhiru Zhang*

Main category: cs.LG

TL;DR: GauS introduces a novel differentiable framework using Gaussian distributions for efficient operator scheduling, capturing the ordinal nature of time and reducing optimization space, achieving Pareto-optimal results.


<details>
  <summary>Details</summary>
Motivation: Traditional scheduling methods like exact solvers or heuristics struggle with efficiency and scalability, while recent differentiable approaches fail to capture the ordinal nature of time and face large parameter spaces.

Method: Proposes GauS, a differentiable framework modeling operator scheduling as stochastic relaxation using Gaussian distributions. It leverages modern GPUs, represents schedules as continuous variables, and efficiently handles objectives/constraints.

Result: GauS achieves Pareto-optimal results on benchmarks, demonstrating superior performance in capturing temporal ordinality and reducing optimization space.

Conclusion: GauS effectively addresses limitations of existing scheduling methods, offering a scalable and flexible solution for complex pipelined scheduling problems.
...
### [171] [Imputation of Unknown Missingness in Sparse Electronic Health Records](https://arxiv.org/abs/2602.20442)
*Jun Han,Josue Nassar,Sanjit Singh Batra,Aldo Cordova-Palomera,Vijay Nori,Robert E. Tillman*

Main category: cs.LG

TL;DR: A transformer-based denoising method improves accuracy for unknown missing values in EHRs, outperforming existing imputation techniques and enhancing downstream tasks like hospital readmission prediction.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of unknown missing data in EHRs, where traditional methods fail to distinguish between undiagnosed conditions and unshared diagnoses.

Method: Develop a transformer-based denoising neural network with adaptive thresholding to recover missing binary EHR data.

Result: Improved accuracy in denoising EHR data and statistically significant performance gains in downstream tasks like hospital readmission prediction.

Conclusion: The proposed method effectively handles unknown unknowns in EHRs, enhancing data quality and utility for medical applications.
...
TL;DR: The paper explores how transformer-based Protein Language Models (PLMs) differ from natural language models, focusing on information distribution across layers and adapting early-exit techniques to improve accuracy and efficiency in protein property prediction.


<details>
  <summary>Details</summary>
Motivation: Protein sequences differ from natural language in functional richness despite a limited vocabulary, prompting research into how transformers behave in the protein domain and how to optimize PLMs for biological tasks.

Method: The study compares information distribution across attention heads in PLMs vs. natural language models and applies an early-exit technique to select intermediate layer representations for specific protein tasks, enhancing both accuracy and efficiency.

Result: The approach achieves performance gains of 0.4 to 7.01 percentage points and efficiency improvements of over 10% across models and non-structural prediction tasks.

Conclusion: The work highlights differences in transformer behavior for protein sequences and advances PLM applications in biology, opening new research avenues.
...
TL;DR: The paper addresses online alignment of large language models under misspecified preference feedback, introducing a robust objective with sensitivity penalty and efficient updates.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenge of aligning LLMs when preference feedback deviates from the ideal ground truth, ensuring robustness.

Method: Introduces a pointwise oracle uncertainty set and formulates a worst-case optimization problem, with a focus on log-linear policies.

Result: Achieves an exact closed-form decomposition of the robust objective and proves $	ilde{O}(rac{1}{	ext{b}}})$ oracle complexity.

Conclusion: The proposed robust framework enhances LLM alignment under misspecified feedback with efficient updates.
...
### [174] [Nonparametric Teaching of Attention Learners](https://arxiv.org/abs/2602.20461)
...
TL;DR: AtteNT introduces a nonparametric teaching paradigm to accelerate attention learner training by selecting strategic example subsets, reducing training time without loss of accuracy.


<details>
  <summary>Details</summary>
Motivation: Address the high computational cost of training attention-based neural networks like transformers by leveraging nonparametric teaching.

Method: Proposes Attention Neural Teaching (AtteNT), which selects sequence-property pairs to implicitly teach mappings and accelerates training via functional gradient descent.

Result: Achieves training time reductions of 13.01% for LLMs and 20.58% for ViTs, with preserved or enhanced performance.

Conclusion: AtteNT efficiently trains attention learners through nonparametric teaching, offering significant speedups without compromising accuracy.
...
### [175] [A Long-Short Flow-Map Perspective for Drifting Models](https://arxiv.org/abs/2602.20463)
*Zhiqi Li,Bo Zhu*

Main category: cs.LG

TL;DR: The paper reinterprets the Drifting Model using a semigroup-consistent flow-map factorization, showing global transport can be split into long-horizon and short-time terminal flow maps. A new likelihood learning aligns decomposition with density evolution, validated theoretically and empirically.


<details>
  <summary>Details</summary>
Motivation: To reinterpret the Drifting Model by decomposing global transport into manageable flow maps and align these with density evolution for better understanding and optimization.

Method: Semigroup-consistent long-short flow-map factorization, likelihood learning formulation aligning decomposition with density evolution, validation through theoretical analysis and empirical tests.

Result: Demonstrates global transport can be decomposed into long-horizon and short-time terminal flow maps, recovering drifting fields and conservative impulses. Validates framework theoretically and empirically.

Conclusion: The reinterpretation and decomposition offer insights into transport processes and feature-space optimization, highlighting open problems for future research.
...
### [176] [Elimination-compensation pruning for fully-connected neural networks](https://arxiv.org/abs/2602.20467)
*Enrico Ballini,Luca Muscarnera,Alessio Fumagalli,Anna Scotti,Francesco Regazzoni*

Main category: cs.LG

TL;DR: The paper introduces a novel pruning method for neural networks, where weights are removed and biases are perturbed optimally to maintain performance, outperforming traditional pruning techniques.


<details>
  <summary>Details</summary>
Motivation: To improve neural network pruning by generalizing the idea of weight importance to include bias perturbations, enhancing efficiency without increasing sparsity.

Method: Develops a pruning technique using optimal bias perturbations, computed via automatic differentiation, to balance compression and performance.

Result: Numerical experiments show the method outperforms popular pruning strategies across diverse scenarios.

Conclusion: The proposed approach is efficient and effective, with theoretical implications for neural network optimization.
...
### [177] [VINA: Variational Invertible Neural Architectures](https://arxiv.org/abs/2602.20480)
*Shubhanshu Shekhar,Mohammad Javad Khojasteh,Ananya Acharya,Tony Tohme,Kamal Youcef-Toumi*

Main category: cs.LG

TL;DR: The paper revisits normalizing flows (NFs) and invertible neural networks (INNs), addressing the lack of theoretical guarantees on approximation quality. It introduces a unified framework with variational unsupervised loss functions and derives performance guarantees under realistic assumptions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the theoretical gaps in the literature regarding the approximation quality of NFs and INNs, especially for posterior inference and generative modeling under practical assumptions.

Method: The authors introduce a unified framework for INNs and NFs using variational unsupervised loss functions, inspired by GANs and Precision-Recall divergence. They derive theoretical guarantees for posterior and distributional accuracy.

Result: The framework provides theoretical performance guarantees under weaker and more realistic assumptions than prior work. Case studies yield design principles and practical guidelines, validated on an ocean-acoustic inversion problem.

Conclusion: The paper successfully bridges the theoretical gap for NFs and INNs, offering a unified approach with practical effectiveness, as demonstrated in real-world applications.
...
### [178] [Actor-Curator: Co-adaptive Curriculum Learning via Policy-Improvement Bandits for RL Post-Training](https://arxiv.org/abs/2602.20532)
...
TL;DR: ACTOR-CURATOR is an automated curriculum learning framework for reinforcement learning post-training of LLMs, outperforming baselines with improved stability and efficiency.


<details>
  <summary>Details</summary>
Motivation: Effective curriculum learning is critical for post-training large foundation models with heterogeneous datasets.

Method: ACTOR-CURATOR uses a neural curator to dynamically select training problems, formulated as a non-stationary stochastic bandit problem with a principled loss function.

Result: It achieves 28.6% and 30.5% relative gains on benchmarks AIME2024 and ARC-1D, with up to 80% speedup.

Conclusion: ACTOR-CURATOR is a scalable and practical approach for LLM post-training.
...
### [179] [Wireless Federated Multi-Task LLM Fine-Tuning via Sparse-and-Orthogonal LoRA](https://arxiv.org/abs/2602.20492)
*Nuocheng Yang,Sihua Wang,Ouwen Huan,Mingzhe Chen,Tony Q. S. Quek,Changchuan Yin*

Main category: cs.LG

TL;DR: The paper proposes a decentralized federated learning (DFL) method using low-rank adaptation (LoRA) to address issues like catastrophic forgetting, inefficient communication, and knowledge interference in LLM fine-tuning on heterogeneous datasets.


<details>
  <summary>Details</summary>
Motivation: The study aims to resolve problems arising from aggregating parameters fine-tuned on heterogeneous datasets in DFL, including knowledge forgetting, communication inefficiency, and multi-task interference.

Method: The solution includes sparse-and-orthogonal LoRA to prevent update conflicts, cluster-based topology design for aggregation, and an implicit MoE mechanism to avoid incompatible knowledge during inference.

Result: Simulations show a 73% reduction in communication resource consumption and a 5% average performance improvement over traditional LoRA.

Conclusion: The proposed method effectively mitigates key challenges in DFL, enhancing efficiency and performance.
...
### [180] [GATES: Self-Distillation under Privileged Context with Consensus Gating](https://arxiv.org/abs/2602.20574)
*Alex Stein,Furong Huang,Tom Goldstein*

Main category: cs.LG

TL;DR: Self-distillation improves learning in unreliable supervision settings by using tutor consensus for supervision and distilling full reasoning trajectories.


<details>
  <summary>Details</summary>
Motivation: To address scenarios where supervision (labels, rewards, or graders) is unreliable, particularly in document-grounded question answering with asymmetric context.

Method: Derives supervision from tutor consensus by sampling multiple reasoning traces and using agreement to gate learning. Distills knowledge through full tutor reasoning trajectories.

Result: Improves transfer to document-free student: in-domain accuracy rises from 46.0% to 62.0%, and average accuracy on public benchmarks improves from 20.2% to 35.4%.

Conclusion: Consensus-gated trajectory distillation effectively enhances learning efficiency and accuracy in unreliable supervision settings.
...
### [181] [A Generalized Apprenticeship Learning Framework for Capturing Evolving Student Pedagogical Strategies](https://arxiv.org/abs/2602.20527)
*Md Mirajul Islam,Xi Yang,Adittya Soukarjya Saha,Rajesh Debnath,Min Chi*

Main category: cs.LG

TL;DR: The paper introduces THEMES, a generalized Apprenticeship Learning framework, to address challenges like sample inefficiency and reward function design in DRL for e-learning, showing superior performance with limited expert demonstrations.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of DRL in educational technologies, such as sample inefficiency and reward function complexity, by leveraging Apprenticeship Learning to infer expert behavior from minimal demonstrations.

Method: Utilizes THEMES, a generalized AL framework, to infer dynamic reward functions and derive pedagogical policies from expert student learning processes using only 18 trajectories.

Result: THEMES outperforms six state-of-the-art baselines, achieving an AUC of 0.899 and a Jaccard of 0.653 in predicting student pedagogical decisions.

Conclusion: THEMES proves to be a powerful alternative for inducing effective pedagogical policies in e-learning, demonstrating high performance with minimal expert data.
...
### [182] [Memory-guided Prototypical Co-occurrence Learning for Mixed Emotion Recognition](https://arxiv.org/abs/2602.20530)
*Ming Li,Yong-Jin Liu,Fang Liu,Huankun Sheng,Yeying Fan,Yixiang Wei,Minnan Luo,Weizhan Zhang,Wenping Wang*

Main category: cs.LG

TL;DR: The paper proposes MPCL, a framework for mixed emotion recognition using multi-modal signals, outperforming existing methods by modeling emotion co-occurrence and cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Real-world emotions are often mixed, but current models focus on singular emotions in labs, neglecting co-occurring emotions' structured correlations.

Method: MPCL uses multi-scale associative memory for signal fusion, emotion-specific prototype memory banks, and a memory retrieval strategy for co-occurrence associations.

Result: MPCL outperforms state-of-the-art methods on two public datasets in mixed emotion recognition.

Conclusion: MPCL effectively addresses mixed emotion recognition by leveraging co-occurrence patterns and cross-modal alignment, showing superior performance.
...
### [183] [Sample-efficient evidence estimation of score based priors for model selection](https://arxiv.org/abs/2602.20549)
*Frederic Wang,Katherine L. Bouman*

Main category: cs.LG

TL;DR: Proposes an estimator for model evidence in diffusion priors, leveraging posterior sampling methods to accurately evaluate priors with minimal samples.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of selecting priors consistent with measurements in Bayesian inverse problems, where traditional methods are intractable or require excessive evaluations.

Method: Introduces an estimator using time-marginals of posterior sampling methods, requiring only a few samples (e.g., 20) and compatible with existing diffusion posterior sampling techniques.

Result: Empirically matches analytical model evidence, successfully selects correct priors, and diagnoses misfit in ill-conditioned non-linear problems, including real-world black hole imaging.

Conclusion: The method provides a practical and accurate solution for evaluating model evidence in diffusion priors, enhancing their usability in complex inverse problems.
...
### [184] [GENSR: Symbolic Regression Based in Equation Generative Space](https://arxiv.org/abs/2602.20557)
*Qian Li,Yuxiao Hu,Juncheng Liu,Yuntian Chen*

Main category: cs.LG

TL;DR: GenSR is a generative latent space-based framework for Symbolic Regression (SR) that addresses noisy feedback by pretraining a CVAE for smooth exploration and refining with CMA-ES, achieving accuracy, simplicity, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional SR methods suffer from noisy feedback due to misalignment between structural modifications and numerical behavior, prompting the need for a smoother, guided search approach.

Method: GenSR pretrains a dual-branch CVAE to create a generative latent space with symbolic continuity and numerical smoothness, followed by coarse localization and fine refinement using modified CMA-ES.

Result: GenSR outperforms in predictive accuracy, expression simplicity, and computational efficiency while remaining robust to noise.

Conclusion: GenSR provides a theoretically grounded and effective framework for SR by leveraging generative latent spaces and Bayesian principles.
...
TL;DR: SELAUR is a reinforcement learning framework incorporating LLM uncertainty into reward design, improving decision-making efficiency and success rates.


<details>
  <summary>Details</summary>
Motivation: Effective reward design is crucial for LLMs in multi-step decision-making, and intrinsic uncertainty offers valuable, overlooked learning cues.

Method: SELAUR uses entropy-, least-confidence-, and margin-based metrics for token-level uncertainty, combined with reward reshaping for dense supervision.

Result: Experiments on ALFWorld and WebShop benchmarks show improved success rates over baselines, with ablations confirming uncertainty's role in enhancing exploration.

Conclusion: Incorporating LLM uncertainty into rewards improves exploration efficiency and learning stability, as demonstrated by SELAUR's superior performance.
...
### [186] [Stability and Generalization of Push-Sum Based Decentralized Optimization over Directed Graphs](https://arxiv.org/abs/2602.20567)
*Yifei Liang,Yan Sun,Xiaochun Cao,Li Shen*

Main category: cs.LG

TL;DR: The paper analyzes the stability and generalization of the Stochastic Gradient Push (SGP) algorithm in decentralized learning over directed networks, addressing biases from asymmetric communication.


<details>
  <summary>Details</summary>
Motivation: To understand the finite-iteration stability and generalization behavior of SGP, which is unclear due to structural bias from directed topology and asymmetric error propagation.

Method: Develops a uniform-stability framework for SGP, using an imbalance-aware consistency bound for Push-Sum to disentangle statistical effects from topology-induced bias.

Result: For convex objectives, SGP achieves generalization error $	ilde{\mathcal{O}}\!ig(\frac{1}{\sqrt{mn}}+\frac{γ}{δ(1-λ)}+γ\big)$; for PŁ objectives, rates depend on $κ\!\left(1+\frac{1}{δ(1-λ)}\right)$.

Conclusion: The framework clarifies when Push-Sum correction is needed and quantifies how topology imbalance and mixing affect learning performance.
...
TL;DR: Embodied LLMs lack self-reflection, leading to repeated mistakes. Reflective Test-Time Planning introduces reflection-in-action and reflection-on-action to improve task performance, validated by benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current embodied LLMs cannot learn from mistakes or reflect on failures, limiting their deployment effectiveness.

Method: Introduces Reflective Test-Time Planning with reflection-in-action (scoring actions before execution) and reflection-on-action (updating models post-execution). Includes retrospective reflection for long-horizon credit assignment.

Result: Significant improvements over baselines in Long-Horizon Household and MuJoCo Cupboard Fitting benchmarks, validated by ablative studies and real-robot trials.

Conclusion: Reflective Test-Time Planning enhances embodied LLMs' ability to learn from mistakes, improving performance through structured reflection.
...
### [188] [Upper-Linearizability of Online Non-Monotone DR-Submodular Maximization over Down-Closed Convex Sets](https://arxiv.org/abs/2602.20578)
*Yiyang Lu,Haresh Jadav,Mohammad Pedramfar,Ranveer Singh,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: The paper introduces a new method for online maximization of non-monotone DR-submodular functions, achieving better regret and feedback guarantees than existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing projection-free online methods for this problem have suboptimal regret and limited feedback guarantees. The study aims to improve these results.

Method: The authors propose a structural result showing that the problem class is $1/e$-linearizable under specific conditions, enabling a reduction to online linear optimization.

Result: They achieve $O(T^{1/2})$ static regret with one gradient query per round and adaptive/dynamic regret guarantees, along with improved rates under various feedback models.

Conclusion: The proposed method strictly improves upon existing state-of-the-art bounds across all feedback models.
...
### [189] [Is the Trigger Essential? A Feature-Based Triggerless Backdoor Attack in Vertical Federated Learning](https://arxiv.org/abs/2602.20593)
*Yige Liu,Yiwei Lou,Che Wang,Yongzhi Cao,Hanpin Wang*

Main category: cs.LG

TL;DR: The paper introduces a new triggerless backdoor attack in vertical federated learning (VFL), which outperforms existing attacks by 2-50 times while minimally disrupting the main task, and remains robust against defenses.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks in VFL rely on triggers, but the authors identify that triggers are not essential, prompting them to explore a feature-based triggerless attack pathway.

Method: The attack involves three modules: label inference for targeting, poison generation with amplification and perturbation, and backdoor execution. Tests were conducted on five datasets.

Result: The attack significantly outperforms baseline methods (2-50 times better), maintains high performance with 32 passive parties, and resists defense strategies.

Conclusion: The findings highlight the need for revisiting VFL security threats and developing more robust defenses against triggerless backdoor attacks.
...
TL;DR: The paper introduces QEDBench, a benchmark to measure the alignment gap in LLM evaluation of university-level math proofs, revealing biases and reasoning gaps in frontier models.


<details>
  <summary>Details</summary>
Motivation: To address the reliability of automated evaluation by LLMs, particularly in upper-undergraduate to early graduate-level mathematics, where current methods show systematic alignment gaps.

Method: Developed QEDBench, a dual-rubric alignment benchmark, and deployed a dual-evaluation matrix (7 judges x 5 solvers) with 1,000+ hours of human evaluation to compare LLM performance against expert criteria.

Result: Identified significant positive bias in some LLMs (e.g., Claude Opus 4.5, DeepSeek-V3) and a reasoning gap in discrete domains, with performance drops in models like GPT-5 Pro and Claude Sonnet 4.5.

Conclusion: QEDBench highlights limitations in current LLM evaluators and provides a public benchmark for improving AI judges.
...
TL;DR: A transformer-based framework solves privacy and data challenges in urban mobility trajectory generation, outperforming existing models in reliability and diversity.


<details>
  <summary>Details</summary>
Motivation: Privacy concerns limit access to mobility trajectories, which are vital for urban planning. This research aims to generate large-scale trajectories efficiently.

Method: Uses a transformer-based model with offline RL and IRL for trajectory generation, reducing vocabulary space and capturing individual preferences. Fine-tuning addresses RL challenges.

Result: Outperforms existing models in reliability and diversity, validated on multiple datasets.

Conclusion: The framework advances urban mobility modeling and aids traffic management and urban planning. Implementation is publicly available.
...
### [192] [Sparse Bayesian Deep Functional Learning with Structured Region Selection](https://arxiv.org/abs/2602.20651)
*Xiaoxian Zhu,Yingmeng Li,Shuangge Ma,Mengyun Wu*

Main category: cs.LG

TL;DR: The paper proposes sBayFDNN, a sparse Bayesian functional deep neural network, to address limitations of linear functional models and non-interpretable deep learning methods in complex data analysis.


<details>
  <summary>Details</summary>
Motivation: Modern applications like ECG monitoring and neuroimaging generate complex, structured data, but current methods struggle with linearity limits or lack interpretability in sparse effects.

Method: sBayFDNN uses a deep Bayesian architecture to learn adaptive functional embeddings and employs structured priors for interpretable region-wise selection with uncertainty quantification.

Result: Theoretical guarantees include approximation error bounds, posterior consistency, and region selection consistency. Empirical results show sBayFDNN outperforms existing methods in accuracy and interpretability.

Conclusion: sBayFDNN effectively bridges the gap between linear models and deep learning, offering reliable, interpretable, and statistically rigorous solutions for functional data analysis.
...
### [193] [Bikelution: Federated Gradient-Boosting for Scalable Shared Micro-Mobility Demand Forecasting](https://arxiv.org/abs/2602.20671)
*Antonios Tziorvas,Andreas Tritsarolis,Yannis Theodoridis*

Main category: cs.LG

TL;DR: Bikelution, a Federated Learning solution using gradient-boosted trees, offers accurate mid-term bike demand forecasts while preserving privacy.


<details>
  <summary>Details</summary>
Motivation: Traditional time-series models and centralized ML face challenges with bike demand forecasting due to external factors and privacy concerns.

Method: Proposes Bikelution, a Federated Learning approach based on gradient-boosted trees, tested on three real-world bike-sharing datasets.

Result: Bikelution matches centralized ML accuracy and outperforms state-of-the-art methods, demonstrating privacy-aware forecasting feasibility.

Conclusion: Bikelution effectively balances accuracy and privacy, highlighting trade-offs between federated and centralized learning approaches.
...
### [194] [UrbanFM: Scaling Urban Spatio-Temporal Foundation Models](https://arxiv.org/abs/2602.20677)
*Wei Chen,Yuqian Wu,Junle Chen,Xiaofang Zhou,Yuxuan Liang*

Main category: cs.LG

TL;DR: The paper introduces scaling principles—heterogeneity, correlation, and dynamics—to develop spatio-temporal foundation models for urban systems, addressing fragmentation in urban computing with unified data (WorldST), computational units (MiniST), and a minimalist architecture (UrbanFM).


<details>
  <summary>Details</summary>
Motivation: Urban computing lacks generalizable models due to scenario-specific approaches. The paper aims to bridge this gap by leveraging scaling principles inspired by AI for Science advancements.

Method: The authors systematically scale across three dimensions: heterogeneity (via WorldST dataset), correlation (via MiniST units), and dynamics (via UrbanFM architecture). They also introduce EvalST benchmark.

Result: UrbanFM demonstrates strong zero-shot generalization across unseen cities and tasks, showcasing the potential of scalable spatio-temporal foundation models.

Conclusion: The work marks a pivotal step toward unifying urban computing with scalable foundation models, overcoming current limitations in generalizability.
...
### [195] [High-Dimensional Robust Mean Estimation with Untrusted Batches](https://arxiv.org/abs/2602.20698)
*Maryam Aliakbarpour,Vladimir Braverman,Yuhan Liu,Junze Yin*

Main category: cs.LG

TL;DR: The paper addresses high-dimensional mean estimation in a collaborative setting with heterogeneous and potentially malicious data sources, proposing algorithms to handle double corruption and achieve minimax-optimal error rates.


<details>
  <summary>Details</summary>
Motivation: The motivation is to tackle the challenge of recovering the true mean in environments where data sources are statistically heterogeneous and may include adversarial contributions, which complicates traditional estimation methods.

Method: The authors formalize the problem in a double corruption landscape and propose two Sum-of-Squares (SoS) based algorithms to handle mean-shift deviations and sample-level adversarial corruption in high dimensions.

Result: The algorithms achieve a minimax-optimal error rate of O(√(ε/n) + √(d/nN) + √α), showing that adversarial influence is mitigated by the batch structure.

Conclusion: The paper concludes that while statistical heterogeneity (α) poses inherent challenges, the batch structure effectively reduces adversarial influence, enabling robust mean estimation.
...
### [196] [WeirNet: A Large-Scale 3D CFD Benchmark for Geometric Surrogate Modeling of Piano Key Weirs](https://arxiv.org/abs/2602.20714)
*Lisa Lüddecke,Michael Hohmann,Sebastian Eilermann,Jan Tillmann-Mumm,Pezhman Pourabdollah,Mario Oertel,Oliver Niggemann*

Main category: cs.LG

TL;DR: WeirNet introduces a large 3D CFD dataset for Piano Key Weirs (PKWs), enabling surrogate models to predict discharge coefficients efficiently, outperforming CFD simulations in speed.


<details>
  <summary>Details</summary>
Motivation: Accurate PKW hydraulic performance prediction is complex due to 3D geometry and operating conditions, and existing datasets are limited.

Method: A dataset of 3,794 PKW geometries was simulated under 19 discharge conditions each, totaling 71,387 CFD simulations. The dataset includes parametric descriptors, meshes, and point clouds, with standardized evaluation splits.

Result: Tree-based regressors on parametric descriptors achieved the best accuracy, while mesh- and point-based models were also competitive. Surrogates were significantly faster than CFD.

Conclusion: WeirNet provides a reproducible framework for data-driven hydraulic modeling, accelerating PKW design exploration during early planning stages.
...
### [197] [Fuz-RL: A Fuzzy-Guided Robust Framework for Safe Reinforcement Learning under Uncertainty](https://arxiv.org/abs/2602.20729)
*Xu Wan,Chao Yang,Cheng Yang,Jie Song,Mingyang Sun*

Main category: cs.LG

TL;DR: Fuz-RL is a fuzzy measure-guided robust framework for safe reinforcement learning that uses Choquet integrals for robust value estimation, avoiding min-max optimization and improving safety and performance under uncertainty.


<details>
  <summary>Details</summary>
Motivation: Real-world RL applications face challenges due to multiple uncertainty sources, requiring interpretable risk assessment and robust decision-making for safety.

Method: The framework introduces a fuzzy Bellman operator with Choquet integrals to estimate robust value functions, translating the problem into a distributionally robust safe RL form.

Result: Empirical tests on safe-control-gym and safety-gymnasium scenarios show Fuz-RL improves safety and control performance under observation, action, and dynamics uncertainties.

Conclusion: Fuz-RL effectively addresses uncertainty challenges in safe RL, enhancing both safety and performance without requiring min-max optimization.
...
### [198] [Rethink Efficiency Side of Neural Combinatorial Solver: An Offline and Self-Play Paradigm](https://arxiv.org/abs/2602.20730)
*Zhenxing Xu,Zeyuan Ma,Weidong Bao,Hui Yan,Yan Zheng,Ji Wang*

Main category: cs.LG

TL;DR: ECO introduces an efficient offline learning paradigm for Neural Combinatorial Optimization (NCO) using supervised warm-up, Direct Preference Optimization, a Mamba-based architecture, and progressive bootstrapping, achieving competitive performance with high efficiency.


<details>
  <summary>Details</summary>
Motivation: To overcome inefficiencies in existing online paradigms for NCO by proposing an offline approach that improves training stability and resource utilization.

Method: 1) Two-phase offline paradigm (supervised warm-up and DPO), 2) Mamba-based architecture for efficiency, and 3) Progressive bootstrapping for stable training.

Result: ECO performs competitively on TSP and CVRP benchmarks, with notable efficiency gains in memory and training throughput.

Conclusion: ECO successfully addresses efficiency limitations in NCO through innovative offline paradigms and architectural choices, supported by empirical results and ablation studies.
...
### [199] [Deep unfolding of MCMC kernels: scalable, modular & explainable GANs for high-dimensional posterior sampling](https://arxiv.org/abs/2602.20758)
*Jonathan Spence,Tobías I. Liaudat,Konstantinos Zygalakis,Marcelo Pereyra*

Main category: cs.LG

TL;DR: The paper introduces a novel GAN architecture inspired by Langevin MCMC to address the opacity and lack of adaptability in push-forward generative models for Bayesian computation.


<details>
  <summary>Details</summary>
Motivation: Push-forward generative models (e.g., GANs) are computationally efficient but lack modularity and adaptability to changes in the likelihood function, unlike MCMC methods.

Method: Deep unfolding is applied to Langevin MCMC algorithms to create modular neural networks. A supervised regularized Wasserstein GAN framework trains these unfolded samplers end-to-end for posterior sampling.

Result: The proposed approach achieves high sampling accuracy, computational efficiency, and retains physics consistency, adaptability, and interpretability akin to classical MCMC.

Conclusion: The unfolded GAN architecture offers a robust and interpretable alternative for Bayesian computation, combining the strengths of MCMC and push-forward models.
...
### [200] [On Electric Vehicle Energy Demand Forecasting and the Effect of Federated Learning](https://arxiv.org/abs/2602.20782)
...
### [201] [Understanding the Role of Rehearsal Scale in Continual Learning under Varying Model Capacities](https://arxiv.org/abs/2602.20791)
*JinLi He,Liang Bai,Xian Yang*

Main category: cs.LG

TL;DR: The paper examines how rehearsal scale affects learning dynamics in continual learning, revealing surprising findings like rehearsal impairing adaptability and not always improving memory retention.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in theoretical understanding of how rehearsal scale influences learning dynamics in continual learning.

Method: Formulates rehearsal-based continual learning as a multidimensional optimization problem and analyzes adaptability, memorability, and generalization theoretically.

Result: Rehearsal can impair adaptability, doesn't always improve memory retention, and memory error has a diminishing lower bound in certain conditions.

Conclusion: The theoretical insights are validated through simulations and real-world datasets, revealing statistical patterns of rehearsal mechanisms.
...
### [202] [Exploring the Impact of Parameter Update Magnitude on Forgetting and Generalization of Continual Learning](https://arxiv.org/abs/2602.20796)
*JinLi He,Liang Bai,Xian Yang*

Main category: cs.LG

TL;DR: The paper explores the role of parameter update magnitudes in continual learning, formalizing forgetting as knowledge degradation due to task-specific parameter drift. It introduces a hybrid update strategy based on theoretical insights, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the lack of theoretical understanding of parameter update magnitudes in continual learning, this study aims to characterize forgetting and optimize update strategies.

Method: The authors formalize forgetting as parameter drift and derive optimal update magnitudes. They unify frozen and initialized training paradigms, proposing a hybrid strategy based on gradient directions.

Result: The hybrid strategy, tested on deep neural networks, shows superior performance in reducing forgetting compared to standard training methods.

Conclusion: The study provides theoretical and practical advancements for continual learning, demonstrating the effectiveness of adaptive parameter update strategies.
...
### [203] [Regret-Guided Search Control for Efficient Learning in AlphaZero](https://arxiv.org/abs/2602.20809)
*Yun-Jui Tsai,Wei-Yu Chen,Yan-Ru Ju,Yu-Hung Chang,Ti-Rong Wu*

Main category: cs.LG

TL;DR: RGSC enhances AlphaZero by identifying high-regret states for restarting, improving learning efficiency and outperforming baselines in board games like Go, Othello, and Hex.


<details>
  <summary>Details</summary>
Motivation: Humans learn more efficiently by revisiting mistakes, but RL agents like AlphaZero lack mechanisms to prioritize high-potential states for learning.

Method: RGSC introduces a regret network to identify high-regret states from self-play and MCTS, storing them in a prioritized buffer for reuse.

Result: RGSC outperforms AlphaZero and Go-Exploit by 77 and 89 Elo respectively, and improves win rates against KataGo from 69.3% to 78.2%.

Conclusion: RGSC effectively improves AlphaZero's learning efficiency and robustness by focusing on high-regret states.
...
### [204] [Transcoder Adapters for Reasoning-Model Diffing](https://arxiv.org/abs/2602.20904)
*Nathan Hu,Jake Ward,Thomas Icard,Christopher Potts*

Main category: cs.LG

TL;DR: The paper introduces transcoder adapters to study the impact of reasoning training on MLPs, revealing sparse and interpretable features that contribute to reasoning behaviors like hesitation tokens.


<details>
  <summary>Details</summary>
Motivation: To understand how reasoning training affects a model’s internal mechanisms, particularly the differences between base and reasoning-distilled models.

Method: Uses transcoder adapters to approximate and interpret changes in MLP computation after fine-tuning, applied to Qwen2.5-Math-7B and its distilled variant.

Result: Adapters match reasoning model performance (50-90% accuracy gains) and reveal sparse, interpretable features (only ~8% related to reasoning). Hesitation tokens are traced to ~2.4% of features.

Conclusion: Transcoder adapters provide insights into reasoning training and may generalize to broader fine-tuning studies.
...
### [205] [From Isolation to Integration: Building an Adaptive Expert Forest for Pre-Trained Model-based Class-Incremental Learning](https://arxiv.org/abs/2602.20911)
*Ruiqi Liu,Boyu Diao,Hangda Liu,Zhulin An,Fei Wang,Yongjun Xu*

Main category: cs.LG

TL;DR: SAEF introduces a hierarchical structure for adapters in Class-Incremental Learning, improving knowledge sharing among tasks by grouping them semantically and merging similar adapters. It outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat learned knowledge as unstructured collections, missing opportunities for knowledge sharing between tasks. SAEF aims to organize adapters hierarchically for better efficiency.

Method: SAEF groups tasks into semantic clusters, builds balanced expert trees by merging similar adapters, and activates relevant experts during inference for weighted predictions.

Result: SAEF achieves state-of-the-art performance on multiple benchmark datasets.

Conclusion: SAEF's structured approach to adapter organization enhances knowledge sharing and performance in Class-Incremental Learning.
...
### [206] [On the Generalization Behavior of Deep Residual Networks From a Dynamical System Perspective](https://arxiv.org/abs/2602.20921)
*Jinshu Huang,Mingfei Sun,Chunlin Wu*

Main category: cs.LG

TL;DR: The paper establishes generalization error bounds for ResNets using dynamical system modeling, showing bounds of order O(1/√S) with depth-uniform and asymptotic results.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in understanding generalization across discrete- and continuous-time ResNets by leveraging dynamical system insights.

Method: Combines Rademacher complexity, flow maps, and ResNet convergence behavior in the deep-layer limit.

Result: Generalization bounds of order O(1/√S) with a negative structure-dependent term, valid under milder assumptions.

Conclusion: Provides unified generalization insights for ResNets, closing gaps between discrete- and continuous-time settings.
...
### [207] [Hierarchic-EEG2Text: Assessing EEG-To-Text Decoding across Hierarchical Abstraction Levels](https://arxiv.org/abs/2602.20932)
*Anupam Sharma,Harish Katti,Prajwal Singh,Shanmuganathan Raman,Krishna Miyapuram*

Main category: cs.LG

TL;DR: The study investigates EEG's ability to capture hierarchical object representations, proposing episodic analysis with hierarchy-aware sampling using WordNet. It evaluates classification performance across abstraction levels using the large PEERS dataset.


<details>
  <summary>Details</summary>
Motivation: To explore whether EEG can capture object representations across multiple hierarchical levels and understand the impact of semantic abstraction on classification performance.

Method: Episodic analysis with hierarchy-aware sampling from WordNet, evaluated on the PEERS dataset (931,538 EEG samples, 1,610 labels from 264 subjects).

Result: Models perform better with higher hierarchical levels, indicating EEG sensitivity to abstraction.

Conclusion: Abstraction depth is a key underexplored factor in EEG decoding, motivating future research.
...
### [208] [Extending $μ$P: Spectral Conditions for Feature Learning Across Optimizers](https://arxiv.org/abs/2602.20937)
*Akshita Gupta,Marieme Ngom,Sam Foreman,Venkatram Vishwanath*

Main category: cs.LG

TL;DR: A new framework simplifies deriving Maximal Update Parameterization ($μ$P) for optimizers like AdamW, ADOPT, LAMB, Sophia, Shampoo, and Muon, enabling zero-shot hyperparameter transfer across model sizes.


<details>
  <summary>Details</summary>
Motivation: Current hyperparameter tuning for large models is computationally expensive, and existing $μ$P methods are limited to SGD and Adam. Expanding $μ$P to more optimizers can save resources.

Method: The paper introduces spectral conditions as an alternative to tensor programs for deriving $μ$P rules, applying this framework to optimizers like AdamW, LAMB, and Sophia.

Result: The framework successfully enables zero-shot learning rate transfer across model widths for various optimizers and provides insights into depth-scaling parameterization.

Conclusion: The proposed spectral-based framework broadens the applicability of $μ$P beyond SGD and Adam, facilitating efficient hyperparameter tuning for diverse optimizers.
...
### [209] [Estimation of Confidence Bounds in Binary Classification using Wilson Score Kernel Density Estimation](https://arxiv.org/abs/2602.20947)
*Thorbjørn Mosekjær Iversen,Zebin Duan,Frederik Hagelskjær*

Main category: cs.LG

TL;DR: The paper introduces Wilson Score Kernel Density Classification, a novel kernel-based method for estimating reliable confidence bounds in binary classification, demonstrating similar performance to Gaussian Process Classification with lower computational complexity.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based binary classifiers have improved but require reliable confidence bounds for critical applications. Current methods lack efficiency or accuracy in estimating these bounds.

Method: Proposes Wilson Score Kernel Density Classification, leveraging the Wilson Score Kernel Density Estimator to estimate confidence bounds for Binomial experiments with varying success probabilities.

Result: Evaluated on four datasets in selective classification, the method performs comparably to Gaussian Process Classification but with reduced computational complexity.

Conclusion: The proposed method provides an efficient and accurate way to estimate confidence bounds for binary classifiers, making it suitable for critical inspection tasks.
...
### [210] [Does Order Matter : Connecting The Law of Robustness to Robust Generalization](https://arxiv.org/abs/2602.20971)
*Himadri Mandal,Vishnu Varadarajan,Jaee Ponde,Aritra Das,Mihir More,Debayan Gupta*

Main category: cs.LG

TL;DR: The paper resolves the open problem connecting the law of robustness and robust generalization, showing that robust generalization requires Lipschitz constraints and aligning empirical results with theoretical predictions.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between the law of robustness and robust generalization, understanding how Lipschitz constraints and overparameterization affect robust learning.

Method: Introduces a robust generalization error metric, bounds it via Rademacher complexity, and validates predictions through experiments on MNIST.

Result: Results show robust generalization requires Lipschitz conditions, aligning with Wu et al.'s predictions for MNIST.

Conclusion: Robust generalization mandates Lipschitz constraints, linking perturbation radius to Lipschitz scale, resolving the open problem.
...
### [211] [MAST: A Multi-fidelity Augmented Surrogate model via Spatial Trust-weighting](https://arxiv.org/abs/2602.20974)
*Ahmed Mohamed Eisa Nasr,Haris Moazam Sheikh*

Main category: cs.LG

TL;DR: MAST introduces a multi-fidelity surrogate modeling method combining corrected low-fidelity data with high-fidelity predictions, improving accuracy and robustness under budget constraints.


<details>
  <summary>Details</summary>
Motivation: The trade-off between computational cost and accuracy in engineering design motivates the need for efficient multi-fidelity surrogate modeling methods.

Method: MAST blends corrected low-fidelity observations with high-fidelity predictions using explicit discrepancy modeling and distance-based weighting.

Result: MAST outperforms state-of-the-art techniques across synthetic benchmarks and maintains robust performance under varying budget and fidelity conditions.

Conclusion: MAST provides a reliable and efficient solution for multi-fidelity surrogate modeling, addressing limitations of existing methods.
...
### [212] [T1: One-to-One Channel-Head Binding for Multivariate Time-Series Imputation](https://arxiv.org/abs/2602.21043)
*Dongik Park,Hyunwoo Ryu,Suahn Bae,Keondo Park,Hyung-Sin Kim*

Main category: cs.LG

TL;DR: T1 introduces a CNN-Transformer hybrid architecture with Channel-Head Binding for robust imputation of missing values in multivariate time series, outperforming existing methods by 46% MSE reduction.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with diverse and heavy missing patterns, leading to suboptimal performance due to ineffective cross-variable information transfer and corrupted temporal features.

Method: T1 employs Channel-Head Binding—a one-to-one correspondence between CNN channels and attention heads—to selectively transfer information and adaptively down-weight corrupted temporal patterns.

Result: Experiments on 11 benchmarks show T1 reduces MSE by 46% on average, excels under extreme sparsity (70% missing ratio), generalizes to unseen patterns, and uses consistent hyperparameters.

Conclusion: T1's innovative design enables robust imputation by balancing temporal pattern extraction and cross-variable information transfer, achieving state-of-the-art performance.
...
### [213] [PIME: Prototype-based Interpretable MCTS-Enhanced Brain Network Analysis for Disorder Diagnosis](https://arxiv.org/abs/2602.21046)
*Kunyu Zhang,Yanwu Yang,Jing Zhang,Xiangjie Shi,Shujian Yu*

Main category: cs.LG

TL;DR: PIME, an interpretable framework for fMRI-based diagnosis, integrates prototype-based classification and consistency training to improve reliability and identify critical brain regions.


<details>
  <summary>Details</summary>
Motivation: Standard deep learning methods for fMRI-based diagnosis often struggle with noisy interactions and unreliable post-hoc attributions, potentially highlighting dataset-specific artifacts.

Method: PIME integrates prototype-based classification and consistency training with structural perturbations, using Monte Carlo Tree Search (MCTS) to extract minimal-sufficient explanatory subgraphs.

Result: PIME achieves state-of-the-art performance on three benchmark fMRI datasets and identifies critical brain regions consistent with neuroimaging findings, with 90% reproducibility.

Conclusion: PIME effectively addresses reliability and interpretability challenges in fMRI-based diagnosis, offering consistent and reproducible results.
...
### [214] [Localized Dynamics-Aware Domain Adaption for Off-Dynamics Offline Reinforcement Learning](https://arxiv.org/abs/2602.21072)
*Zhangjie Xia,Yu Yang,Pan Xu*

Main category: cs.LG

TL;DR: LoDADA introduces a localized clustering approach to filter source data efficiently for off-dynamics offline RL, outperforming existing methods by leveraging local dynamics mismatches.


<details>
  <summary>Details</summary>
Motivation: Existing methods address dynamics mismatch globally or pointwise, missing localized similarities or being computationally costly. LoDADA aims to better reuse source data by focusing on localized dynamics differences.

Method: LoDADA clusters transitions from source and target datasets, estimates cluster-level dynamics discrepancy via domain discrimination, and filters source data based on discrepancy. Avoids coarse global assumptions and per-sample filtering.

Result: LoDADA consistently outperforms state-of-the-art off-dynamics offline RL methods in environments with diverse dynamics shifts.

Conclusion: LoDADA's fine-grained and scalable data selection strategy effectively leverages localized dynamics mismatch, improving performance in off-dynamics offline RL.
...
### [215] [ProxyFL: A Proxy-Guided Framework for Federated Semi-Supervised Learning](https://arxiv.org/abs/2602.21078)
*Duowen Chen,Yan Wang*

Main category: cs.LG

TL;DR: ProxyFL is a framework addressing data heterogeneity in Federated Semi-Supervised Learning (FSSL) by using a learnable classifier proxy to mitigate external and internal distribution discrepancies, improving performance and convergence.


<details>
  <summary>Details</summary>
Motivation: FSSL faces challenges from data heterogeneity across and within clients, leading to inefficient aggregation strategies and reduced data participation. ProxyFL aims to address these issues with a unified solution.

Method: ProxyFL employs a learnable classifier proxy to simulate category distributions globally and locally. It optimizes the global proxy to handle outliers and reintroduces discarded samples via a positive-negative proxy pool to correct pseudo-labels.

Result: The framework demonstrates significant performance improvements and convergence in FSSL, as evidenced by experiments and theoretical analysis.

Conclusion: ProxyFL effectively mitigates heterogeneity in FSSL, offering a robust solution for collaborative model training with partially-annotated data.
...
### [216] [Scaling Vision Transformers: Evaluating DeepSpeed for Image-Centric Workloads](https://arxiv.org/abs/2602.21081)
*Huy Trinh,Rebecca Ma,Zeqi Yu,Tahsin Reza*

Main category: cs.LG

TL;DR: The study explores using DeepSpeed to improve Vision Transformers' scalability and performance via distributed training, focusing on efficiency across GPU setups and datasets like CIFAR-10/100.


<details>
  <summary>Details</summary>
Motivation: ViTs face computational and memory challenges in scalability; DeepSpeed's efficiency in language models inspired its application to ViTs.

Method: Evaluates intra- and inter-node training efficiency using distributed data parallelism, varying batch sizes and gradient accumulation on CIFAR datasets.

Result: Identifies key performance factors in distributed training, laying groundwork for DeepSpeed in image tasks.

Conclusion: Future work will further optimize distributed training pipelines and explore DeepSpeed's limitations for ViTs.
...
### [217] [Probing Graph Neural Network Activation Patterns Through Graph Topology](https://arxiv.org/abs/2602.21092)
*Floriano Tori,Lorenzo Bini,Marco Sorbi,Stéphane Marchand-Maillet,Vincent Ginis*

Main category: cs.LG

TL;DR: Graph curvature highlights bottlenecks in graph topology, but Graph Neural Networks (GNNs) don't focus on these extremes despite their theoretical link to information flow. Attention mechanisms worsen bottlenecks, increasing negative curvature.


<details>
  <summary>Details</summary>
Motivation: To understand how graph topology interacts with GNNs' learned preferences, especially regarding curvature and bottlenecks.

Method: Probing graph topology using Massive Activations (extreme edge activations in Graph Transformers) on synthetic graphs and molecular benchmarks.

Result: Massive Activations don't concentrate on curvature extremes, but attention mechanisms worsen bottlenecks, increasing negative curvature.

Conclusion: Curvature can diagnose graph learning failures, revealing when and why GNNs struggle.
...
### [218] [Ski Rental with Distributional Predictions of Unknown Quality](https://arxiv.org/abs/2602.21104)
*Qiming Cui,Michael Dinitz*

Main category: cs.LG

TL;DR: The paper revisits ski rental with distributional predictions, proposing an algorithm with tight bounds on expected cost relative to prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve ski rental algorithms by leveraging distributional predictions instead of single-value predictions, avoiding assumptions about prediction error.

Method: Proposes an algorithm minimizing expected cost based on Earth Mover's distance between predicted and true distributions, without needing prediction error bounds.

Result: Achieves expected cost OPT + O(min(max({eta}, 1) * sqrt(b), b log b)), with tight consistency and robustness guarantees.

Conclusion: The algorithm's bounds are optimal, balancing consistency and robustness without requiring prediction error knowledge.
...
### [219] [SOM-VQ: Topology-Aware Tokenization for Interactive Generative Models](https://arxiv.org/abs/2602.21133)
*Alessandro Londei,Denise Lanzieri,Matteo Benati*

Main category: cs.LG

TL;DR: SOM-VQ introduces a tokenization method combining vector quantization with Self-Organizing Maps to create interpretable, topology-aware discrete representations for human-in-the-loop control.


<details>
  <summary>Details</summary>
Motivation: Standard vector-quantized representations lack semantic structure in token space, limiting human interpretability and control. SOM-VQ addresses this by embedding explicit low-dimensional topology.

Method: SOM-VQ integrates Self-Organizing Maps with vector quantization, using topology-aware updates to preserve neighborhood structure in the token space.

Result: SOM-VQ produces more learnable token sequences with navigable geometry, enabling intuitive human control through direct manipulation of token distances.

Conclusion: SOM-VQ offers a general framework for interpretable discrete representations, demonstrated in human motion generation but applicable to domains like music and gesture.
...
### [220] [Sequential Counterfactual Inference for Temporal Clinical Data: Addressing the Time Traveler Dilemma](https://arxiv.org/abs/2602.21168)
*Jingya Cheng,Alaleh Azhir,Jiazi Tian,Hossein Estiri*

Main category: cs.LG

TL;DR: The paper introduces a Sequential Counterfactual Framework to address violations of feature independence and simultaneous modifiability assumptions in longitudinal clinical data, demonstrating its effectiveness in identifying actionable insights for COVID-19 patients with chronic conditions.


<details>
  <summary>Details</summary>
Motivation: Standard counterfactual methods fail to account for temporal dependencies and feature immutability in clinical data, necessitating a framework that respects these dynamics.

Method: The Sequential Counterfactual Framework distinguishes immutable from controllable features and models intervention propagation over time, applied to COVID-19 patient data.

Result: The framework reveals that naive methods yield biologically impossible counterfactuals for 38-67% of chronic patients and identifies a cardiorenal cascade with significant relative risks.

Conclusion: The framework enhances counterfactual explanations by incorporating temporal and biological plausibility, offering clinically actionable insights.
...
### [221] [The Diffusion Duality, Chapter II: $Ψ$-Samplers and Efficient Curriculum](https://arxiv.org/abs/2602.21185)
*Justin Deschenaux,Caglar Gulcehre,Subham Sekhar Sahoo*

Main category: cs.LG

TL;DR: Predictor-Corrector samplers improve discrete diffusion models, outperforming ancestral sampling and reducing training costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To enhance sampling quality in discrete diffusion models and challenge the dominance of Masked diffusion models.

Method: Introduces Predictor-Corrector samplers for discrete diffusion, applicable to arbitrary noise processes, and develops a memory-efficient training curriculum.

Result: Achieves better generative perplexity and FID/IS scores, with continuous improvement in sampling steps, and reduces training time and memory usage.

Conclusion: Predictor-Corrector samplers outperform traditional methods, offering a viable alternative to Masked diffusion models.
...
### [222] [Why Pass@k Optimization Can Degrade Pass@1: Prompt Interference in LLM Post-training](https://arxiv.org/abs/2602.21189)
*Anas Barakat,Souradip Chakraborty,Khushbu Pahwa,Amrit Singh Bedi*

Main category: cs.LG

TL;DR: Pass@k optimization improves multi-sample success but can degrade single-sample success (pass@1) due to gradient conflict from prompt interference.


<details>
  <summary>Details</summary>
Motivation: To understand why optimizing pass@k often leads to a trade-off with pass@1, especially given the practical importance of pass@1 due to latency, cost, and reliability constraints.

Method: Theoretical characterization of gradient conflict induced by prompt interference during pass@k optimization, supported by experiments on large language models for mathematical reasoning.

Result: Pass@k optimization can conflict with pass@1 gradients because it reweights prompts toward low-success ones, negatively interfering with pass@1 performance.

Conclusion: The trade-off between pass@k and pass@1 arises from prompt interference and gradient conflict, highlighting challenges in optimizing multi-sample metrics without degrading single-sample reliability.
...
### [223] [Statistical Query Lower Bounds for Smoothed Agnostic Learning](https://arxiv.org/abs/2602.21191)
*Ilias Diakonikolas,Daniel M. Kane*

Main category: cs.LG

TL;DR: SQ lower bound nearly matches upper bound for smoothed agnostic learning of halfspaces, suggesting optimality of $L_1$-polynomial regression.


<details>
  <summary>Details</summary>
Motivation: To understand the complexity limits of smoothed agnostic learning, particularly for halfspaces under subgaussian distributions.

Method: Uses SQ lower bounds and linear programming duality to analyze a moment-matching hard distribution, linking it to low-degree polynomial approximation.

Result: Shows SQ complexity $d^{Ω(1/σ^{2}+\log(1/ε))}$, nearly matching the known upper bound.

Conclusion: Demonstrates that $L_1$-polynomial regression is likely optimal for this task.
...
### [224] [Untied Ulysses: Memory-Efficient Context Parallelism via Headwise Chunking](https://arxiv.org/abs/2602.21196)
*Ravi Ghadia,Maksim Abraham,Sergei Vorobyov,Max Ryabinin*

Main category: cs.LG

TL;DR: UPipe introduces fine-grained chunking at the attention head level to reduce activation memory usage, enabling longer context lengths without sacrificing training speed.


<details>
  <summary>Details</summary>
Motivation: Existing context parallelism techniques lack memory efficiency, limiting sequence lengths, while advanced methods compromise throughput. UPipe aims to address these limitations.

Method: UPipe performs fine-grained chunking at the attention head level, reducing activation memory usage in self-attention layers.

Result: UPipe cuts intermediate tensor memory usage by up to 87.5% for 32B Transformers and supports 5M-token contexts, improving prior methods by over 25%.

Conclusion: UPipe is a memory-efficient context parallelism technique that enables longer sequences without impacting training speed, outperforming existing methods.
...
### [225] [Test-Time Training with KV Binding Is Secretly Linear Attention](https://arxiv.org/abs/2602.21204)
...
TL;DR: The study examines multilevel predictors of adolescent overweight/obesity in the U.S., comparing model performance across statistical, machine-learning, and deep-learning approaches. Results show limited gains from complex models and highlight persistent disparities across subgroups.


<details>
  <summary>Details</summary>
Motivation: To better understand and predict adolescent overweight/obesity by evaluating behavioral, household, and neighborhood factors and comparing the effectiveness of various predictive models.

Method: Analyzed data from 18,792 children aged 10-17 using BMI categories. Predictors included diet, activity, sleep, parental stress, socioeconomic factors, and neighborhood traits. Compared logistic regression, random forest, gradient boosting, XGBoost, LightGBM, MLP, and TabNet using AUC, accuracy, precision, recall, F1 score, and Brier score.

Result: Model discrimination scores ranged 0.66-0.79. Logistic regression, gradient boosting, and MLP balanced discrimination and calibration best. Complex models modestly improved recall/F1 but showed no uniform superiority. Subgroup disparities persisted across models.

Conclusion: Complex models offer limited improvement over logistic regression. Predictors consistently span multiple domains. Persistent disparities suggest prioritizing data quality and equity-focused surveillance over algorithmic complexity.
...
TL;DR: RARE-PHENIX is an end-to-end AI framework for rare disease phenotyping that outperforms existing methods by integrating extraction, standardization, and prioritization of phenotypes.


<details>
  <summary>Details</summary>
Motivation: Manual phenotyping is labor-intensive, and existing AI approaches fail to operationalize the full clinical workflow, necessitating a more efficient solution.

Method: RARE-PHENIX combines large language model-based extraction, HPO term standardization, and supervised ranking of phenotypes, validated on real-world clinical notes.

Result: RARE-PHENIX outperformed PhenoBERT (ontology-based similarity of 0.70 vs. 0.58) and showed incremental gains with each module.

Conclusion: Modeling the full clinical workflow improves phenotyping accuracy, making RARE-PHENIX a promising tool for rare disease diagnosis.
...
TL;DR: DMCD integrates LLM-based semantic drafting with statistical validation for causal discovery, outperforming baselines in recall and F1.


<details>
  <summary>Details</summary>
Motivation: To enhance causal discovery by leveraging semantic metadata for draft DAGs and validating them statistically.

Method: Two-phase framework: Phase I uses LLM for semantic drafting; Phase II refines via conditional independence testing.

Result: Competitive or leading performance on real-world benchmarks, with notable recall and F1 improvements.

Conclusion: Combining semantic priors with statistical verification improves causal structure learning.
...
TL;DR: DMEMM, a diffusion-based planning method, improves trajectory generation in offline RL by incorporating environment mechanisms like transition dynamics and reward functions.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based planning methods lack consistency between transitions, leading to discrepancies in generated trajectories.

Method: DMEMM modulates diffusion model training by integrating key RL environment mechanisms (transition dynamics and reward functions).

Result: DMEMM achieves state-of-the-art performance in offline RL planning.

Conclusion: Incorporating environment mechanisms enhances diffusion-based planning accuracy and coherence.
...
TL;DR: The paper introduces 'Implicit Intelligence,' a framework to evaluate AI agents' ability to handle implicit human communication, highlighting their current shortcomings in contextual reasoning.


<details>
  <summary>Details</summary>
Motivation: AI agents struggle with underspecified real-world requests due to reliance on explicit instructions, missing human-like contextual understanding.

Method: Proposes 'Implicit Intelligence' and 'Agent-as-a-World' (AaW), a YAML-based framework simulating interactive worlds for testing agents on implicit constraints.

Result: Even top models score only 48.3% pass rate, showing significant gaps in contextual reasoning.

Conclusion: Current AI agents need improvement to move beyond literal instructions and achieve human-like goal-fulfillment.
...
TL;DR: Trace-Free+ is a curriculum learning framework optimizing LLM-agent tool interfaces without execution traces, improving scalability and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing tool interfaces limit agent performance due to reliance on human-oriented descriptions and trace-dependent optimization, which are often unavailable or impractical.

Method: Proposes Trace-Free+, a curriculum learning framework that transfers supervision from trace-rich to trace-free settings, leveraging a large-scale dataset of diverse tools.

Result: Experiments show improved performance on unseen tools, cross-domain generalization, and scalability with over 100 candidate tools.

Conclusion: Tool interface optimization via Trace-Free+ is a viable complement to agent fine-tuning, addressing deployment challenges.
...
TL;DR: PreScience is a benchmark for forecasting scientific advances by decomposing the research process into four tasks: collaborator prediction, prior work selection, contribution generation, and impact prediction, using a dataset of 98K AI-related papers. Current AI models show room for improvement in these tasks.


<details>
  <summary>Details</summary>
Motivation: To determine if AI can forecast future scientific advances to aid researchers in identifying collaborators, impactful directions, and emerging problems or methods.

Method: Introduces PreScience, a dataset of 98K AI papers with metadata, author identities, and citations. Tasks include collaborator prediction, prior work selection, contribution generation, and impact prediction. Evaluations use baselines and LACERScore, a novel LLM-based metric.

Result: Current AI models (e.g., GPT-5) achieve moderate similarity in contribution generation (average 5.6/10). Synthetic research lacks diversity and novelty compared to human-authored work.

Conclusion: AI shows potential for scientific forecasting but has significant room for improvement, particularly in generating diverse and novel contributions.
...
### [233] [KairosVL: Orchestrating Time Series and Semantics for Unified Reasoning](https://arxiv.org/abs/2602.20494)
...
TL;DR: The paper introduces Semantic-Conditional Time Series Reasoning, a task combining contextual understanding with numerical modeling, and proposes KairosVL, a two-round reinforcement learning framework, achieving strong performance and generalization.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for advanced time series analysis that integrates semantic understanding alongside numerical modeling due to increasingly complex decision-making demands.

Method: A two-round reinforcement learning framework: the first round enhances perception of temporal primitives, and the second focuses on semantic-conditioned reasoning.

Result: KairosVL achieves competitive performance in synthetic and real-world tasks, with improved generalization and preserved intrinsic reasoning ability.

Conclusion: The work demonstrates the potential of merging semantic reasoning with temporal modeling, offering a practical framework for real-world time series intelligence.
...
### [234] [ActionEngine: From Reactive to Programmatic GUI Agents via State Machine Memory](https://arxiv.org/abs/2602.20502)
*Hongbin Zhong,Fazle Faisal,Luis França,Tanakorn Leesatapornwongsa,Adriana Szekeres,Kexin Rong,Suman Nath*

Main category: cs.AI

TL;DR: ActionEngine improves GUI agent efficiency by transitioning from reactive execution to programmatic planning using a two-agent architecture, achieving higher accuracy and lower costs compared to vision-only methods.


<details>
  <summary>Details</summary>
Motivation: Current GUI agents are inefficient and costly due to step-by-step reactive execution and lack persistent memory, limiting their accuracy and scalability.

Method: ActionEngine introduces a two-agent architecture: a Crawling Agent for offline exploration and memory creation, and an Execution Agent for synthesizing executable Python programs. It includes robustness with vision-based re-grounding for failures.

Result: The agent achieves 95% task success with one LLM call on Reddit tasks, compared to 66% for vision-only baselines, reducing costs by 11.8x and latency by 2x.

Conclusion: ActionEngine combines global planning, validated action templates, and localized repair for scalable and reliable GUI interaction.
...
### [235] [Inner Speech as Behavior Guides: Steerable Imitation of Diverse Behaviors for Human-AI coordination](https://arxiv.org/abs/2602.20517)
*Rakshit Trivedi,Kartik Sharma,David C Parkes*

Main category: cs.AI

TL;DR: MIMIC is a framework using language as behavioral intent representation, leveraging vision-language models and diffusion-based policy to enhance human-AI coordination through inner speech-guided action selection.


<details>
  <summary>Details</summary>
Motivation: Existing imitation learning methods fail to capture human behavior diversity and lack real-time behavioral steering, prompting the need for MIMIC.

Method: MIMIC trains a conditional variational autoencoder with vision-language models to generate inner speech, guiding a diffusion-based behavior cloning policy.

Result: MIMIC improves behavior diversity and fidelity in robotic tasks and human-AI games, allowing nuanced steering without extra training.

Conclusion: MIMIC advances human-AI coordination by integrating inner speech-guided action selection, validated by superior performance in diverse tasks.
...
### [236] [From Logs to Language: Learning Optimal Verbalization for LLM-Based Recommendation in Production](https://arxiv.org/abs/2602.20558)
...
TL;DR: A data-centric framework using reinforcement learning improves verbalization for LLMs in recommender systems, significantly boosting accuracy over rigid templates.


<details>
  <summary>Details</summary>
Motivation: Existing methods for verbalizing structured user interaction logs in LLM-based recommender systems rely on inefficient templates, limiting their effectiveness.

Method: Proposes a reinforcement learning-based verbalization agent that transforms raw interaction histories into optimized textual contexts, using recommendation accuracy as the training signal.

Result: Achieves up to 93% relative improvement in recommendation accuracy on a large-scale dataset compared to template-based methods.

Conclusion: Learned verbalization enhances LLM-based recommender systems by filtering noise, incorporating metadata, and reorganizing information effectively.
...
### [237] [CausalReasoningBenchmark: A Real-World Benchmark for Disentangled Evaluation of Causal Identification and Estimation](https://arxiv.org/abs/2602.20571)
*Ayush Sawarni,Jiyuan Tan,Vasilis Syrgkanis*

Main category: cs.AI

TL;DR: The paper introduces CausalReasoningBenchmark to evaluate automated causal inference systems by separating identification and estimation tasks, revealing gaps in nuanced research design details.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks conflate identification and estimation in causal analysis, limiting the ability to diagnose specific failures in automated systems.

Method: The authors created a benchmark with 173 queries from 138 real-world datasets, requiring systems to produce both identification specifications and numerical estimates.

Result: A state-of-the-art LLM achieved 84% accuracy in high-level strategy identification but only 30% correctness in full identification specifications.

Conclusion: The benchmark highlights the need for improved research design details in automated causal inference and is publicly available for further development.
...
### [238] [Physics-based phenomenological characterization of cross-modal bias in multimodal models](https://arxiv.org/abs/2602.20624)
*Hyeongmo Kim,Sohyun Kang,Yerin Choi,Seungyeon Ji,Junhyuk Woo,Hyunsuk Chung,Soyeon Caren Han,Kyungreem Han*

Main category: cs.AI

TL;DR: The paper discusses algorithmic fairness in AI, focusing on biases in multimodal large language models (MLLMs) and introduces a phenomenological approach using physics-based models to analyze and address these biases.


<details>
  <summary>Details</summary>
Motivation: The paper aims to highlight biases in MLLMs arising from complex multimodal interactions and proposes phenomenological methods to address fairness issues, moving beyond traditional symbolic or metaphysical approaches.

Method: The authors develop a physics-based surrogate model to analyze transformer dynamics (semantic networks and attention mechanisms) in MLLMs. They conduct diagnostic experiments, including perturbation-based emotion classification and dynamical analysis of chaotic time-series predictions.

Result: Experiments reveal that multimodal inputs can reinforce modality dominance instead of mitigating bias, as shown by structured error-attractor patterns under systematic perturbations.

Conclusion: The paper concludes that phenomenological approaches, particularly physics-based modeling, are practical tools for tackling fairness issues in MLLMs, offering insights not captured by conventional analyses.
...
### [239] [When can we trust untrusted monitoring? A safety case sketch across collusion strategies](https://arxiv.org/abs/2602.20628)
*Nelson Gardner-Challis,Jonathan Bostock,Georgiy Kozhevnikov,Morgan Sinclaire,Joan Velja,Alessandro Abate,Charlie Griffin*

Main category: cs.AI

TL;DR: The paper explores safety risks of deploying autonomous AIs and proposes untrusted monitoring as a solution. It refines existing safety testing methods by analyzing potential collusion strategies of misaligned AIs and introduces a taxonomy for better evaluation.


<details>
  <summary>Details</summary>
Motivation: The increasing autonomy of AIs raises risks of catastrophic harm from misalignment. Untrusted monitoring is proposed to mitigate these risks, but verifying its safety is challenging due to inability to test with misaligned models directly.

Method: The paper extends pre-deployment testing methods by relaxing assumptions about collusion strategies. It develops a taxonomy covering passive self-recognition, causal collusion, acausal collusion, and combined strategies, and outlines a safety case sketch.

Result: Conditions where passive self-recognition is more effective than previous collusion strategies are identified. The work contributes to more robust evaluations of untrusted monitoring.

Conclusion: The study advances understanding of AI collusion strategies and improves safety protocols for untrusted monitoring, though challenges remain unresolved.
...
### [240] [Identifying two piecewise linear additive value functions from anonymous preference information](https://arxiv.org/abs/2602.20638)
*Vincent Auriau,Khaled Belahcene,Emmanuel Malherbe,Vincent Mousseau,Marc Pirlot*

Main category: cs.AI

TL;DR: The paper proposes a method to simultaneously elicit additive value functions for two decision-makers using piecewise linear marginal value functions with known breaking points.


<details>
  <summary>Details</summary>
Motivation: The motivation is to efficiently model preferences from two decision-makers without knowing which response corresponds to whom.

Method: The method involves eliciting preference models through queries answered by both decision-makers, assuming additive value functions with piecewise linear marginal functions.

Result: The proposed procedure successfully identifies the two preference models under the given assumptions.

Conclusion: The conclusion highlights the feasibility of eliciting distinct preference models from simultaneous queries when marginal functions are piecewise linear.
...
### [241] [Grounding LLMs in Scientific Discovery via Embodied Actions](https://arxiv.org/abs/2602.20639)
*Bo Zhang,Jinfeng Zhou,Yuxuan Chen,Jianing Yin,Minlie Huang,Hongning Wang*

Main category: cs.AI

TL;DR: EmbodiedAct bridges the gap between LLMs' theoretical reasoning and physical simulation by grounding them in embodied actions, outperforming existing methods in reliability and accuracy.


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle to connect theoretical reasoning with real-time physical simulation, lacking runtime perception for anomalies.

Method: The proposed EmbodiedAct framework grounds LLMs in embodied actions with a perception-execution loop, implemented in MATLAB.

Result: EmbodiedAct achieves state-of-the-art performance in reliability, stability, and accuracy for engineering and scientific tasks.

Conclusion: EmbodiedAct effectively enhances LLMs' capability in scientific discovery by integrating active perception and execution.
...
### [242] [Recursive Belief Vision Language Model](https://arxiv.org/abs/2602.20659)
*Vaidehi Bagaria,Bijo Sebastian,Nirav Patel*

Main category: cs.AI

TL;DR: RB-VLA introduces a belief-centric architecture for VLA models, improving performance in long-horizon tasks by maintaining persistent state representations and reducing latency.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models struggle with long-horizon manipulation due to partial observability, lack of persistent state, and poor temporal reasoning. RB-VLA aims to address these limitations.

Method: RB-VLA uses a self-supervised world-model to maintain a compact latent state tracking task history and interactions. It combines belief and intent to condition a diffusion policy for control.

Result: RB-VLA achieves higher success rates (52.5% and 37.5% improvements) in multi-stage tasks, reduces latency by 5x, and eliminates memory growth, with belief being key to performance.

Conclusion: Belief-based state representations significantly enhance long-horizon VLA policies, outperforming existing approaches.
...
### [243] [How Foundational Skills Influence VLM-based Embodied Agents:A Native Perspective](https://arxiv.org/abs/2602.20687)
*Bo Peng,Pi Bu,Keyu Pan,Xinrun Xu,Yinxiu Zhao,Miao Chen,Yang Du,Lin Li,Jun Song,Tong Xu*

Main category: cs.AI

TL;DR: NativeEmbodied is a new benchmark for VLM-driven embodied agents, addressing gaps in existing benchmarks by using a unified low-level action space and evaluating both high-level tasks and fundamental skills.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for VLM-driven embodied agents use non-native settings (high-level commands or discretized action spaces) and lack joint evaluation at low and high task levels.

Method: NativeEmbodied introduces a unified native low-level action space and includes three high-level tasks and four low-level tasks targeting fundamental skills for fine-grained evaluation.

Result: Experiments reveal deficiencies in fundamental skills among state-of-the-art VLMs, which significantly hinder high-level task performance.

Conclusion: NativeEmbodied identifies key challenges for VLM-driven agents and offers insights to guide future research.
...
### [244] [PromptCD: Test-Time Behavior Enhancement via Polarity-Prompt Contrastive Decoding](https://arxiv.org/abs/2602.20696)
...
TL;DR: The paper introduces PromptCD, a test-time behavior control method for AI systems, improving alignment with human values without additional training.


<details>
  <summary>Details</summary>
Motivation: Current alignment methods for large language models (LLMs) are costly and limited in scope. The authors aim to generalize contrastive decoding for broader behavioral enhancements.

Method: PromptCD uses paired positive and negative prompts to contrast model responses (e.g., token distributions in LLMs and attention patterns in VLMs), reinforcing desirable behaviors.

Result: Experiments show consistent improvements in LLMs for '3H' objectives (helpfulness, honesty, harmlessness) and enhanced VQA performance in VLMs.

Conclusion: PromptCD is a simple, general, and cost-efficient method for reliable behavior control in AI systems across modalities.
...
TL;DR: The paper introduces a model called OAG for ML-augmented online decision making, separating prediction and algorithmic components, and presents a DTB compiler to transform any online algorithm into a learning-augmented one with strong consistency-robustness guarantees.


<details>
  <summary>Details</summary>
Motivation: To create a unified framework for online decision making that handles unreliable guidance naturally, bridging the gap between predictive and algorithmic components.

Method: The OAG model uses request-answer games with corrupted guidance. The DTB compiler transforms algorithms to either follow guidance blindly or ignore it, based on a biased coin toss.

Result: DTB-compiled algorithms achieve optimal consistency-robustness for caching and metrical task systems, and outperform state-of-the-art for bipartite matching.

Conclusion: The OAG model and DTB compiler provide a simple yet powerful way to enhance online algorithms with learning, offering strong theoretical guarantees.
...
TL;DR: ICON is a framework that detects and mitigates Indirect Prompt Injection attacks in LLMs while preserving task utility, achieving low attack success rates and high efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against Indirect Prompt Injection attacks often terminate valid workflows prematurely due to over-refusal. ICON aims to neutralize attacks without disrupting task continuity.

Method: ICON uses a Latent Space Trace Prober to detect IPI attacks via unique signatures in the latent space, followed by a Mitigating Rectifier to steer attention away from adversarial elements and restore functionality.

Result: ICON achieves a 0.4% Attack Success Rate (ASR) and over 50% task utility gain, with robust OOD generalization and effectiveness in multi-modal agents.

Conclusion: ICON provides a superior balance between security and efficiency, outperforming existing defenses in maintaining task utility while mitigating attacks.
...
TL;DR: CST improves CoT faithfulness by rewarding accurate simulation of model outputs over counterfactual inputs, showing significant gains in monitor accuracy and simulatability.


<details>
  <summary>Details</summary>
Motivation: Address limitations in CoT faithfulness to better understand LLM outputs.

Method: Introduces Counterfactual Simulation Training (CST) with cue-based and generic counterfactuals.

Result: CST improves monitor accuracy by 35 points and simulatability by 2 points; outperforms baselines and is more efficient than RL alone.

Conclusion: CST effectively enhances CoT faithfulness, with potential for CoT monitoring applications.
...
TL;DR: BAPO is an off-policy RLVR framework that improves data efficiency in large language model post-training by dynamically selecting batches and reusing high-quality samples, outperforming GRPO by 12.5% and solving 40.7% of previously unsolvable problems.


<details>
  <summary>Details</summary>
Motivation: Traditional RLVR frameworks suffer from experience waste and reward homogeneity, hindering learning efficiency in large language model post-training.

Method: Introduces Batch Adaptation Policy Optimization (BAPO), which dynamically selects training batches by re-evaluating difficult samples and reusing high-quality ones, with a lower bound policy improvement guarantee.

Result: Achieves a 12.5% average improvement over GRPO and resolves 40.7% of problems base models failed to solve.

Conclusion: BAPO significantly enhances data efficiency and solves previously unsolvable problems in large language model post-training.
...
TL;DR: MAGNET introduces a modality-guided mixture of adaptive graph experts with progressive entropy-triggered routing to improve multimodal recommendation by addressing heterogeneity and imbalance in existing fusion methods.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal fusion methods suffer from entangled representations and modality imbalance, making fusion challenging. MAGNET aims to enhance controllability, stability, and interpretability.

Method: MAGNET uses interaction-conditioned expert routing and structure-aware graph augmentation. It incorporates a dual-view graph learning module and structured experts with explicit modality roles (dominant, balanced, complementary). A two-stage entropy-weighting mechanism stabilizes routing.

Result: Extensive experiments on public benchmarks show MAGNET outperforms strong baselines, improving coverage for sparse and long-tail items while maintaining collaborative structure.

Conclusion: MAGNET effectively addresses challenges in multimodal fusion by providing interpretable and adaptive control over fusion processes, leading to better recommendation performance.
...
TL;DR: This paper explores extending RL from AI feedback (RLAIF) to multi-objective systems, showing it can produce balanced policies without intensive reward engineering.


<details>
  <summary>Details</summary>
Motivation: Reward design is challenging in multi-objective RL, and RLAIF offers a scalable alternative to human labeling, but its effectiveness in multi-objective settings is unproven.

Method: The study extends RLAIF to multi-objective self-adaptive systems, leveraging LLMs for preference labels instead of human annotators.

Result: Multi-objective RLAIF yields balanced policies reflecting user priorities without complex reward engineering.

Conclusion: Integrating RLAIF into multi-objective RL provides a scalable approach for user-aligned policy learning in conflicting-objective domains.
...
TL;DR: CHESS introduces a context-aware, hierarchical KV-cache management system for efficient LLM inference, achieving high quality with minimal cache usage and significant speedups.


<details>
  <summary>Details</summary>
Motivation: Existing KV-cache pruning methods are context-agnostic, leading to poor quality and limited speedups due to irregular accesses and overheads.

Method: CHESS combines algorithmic context-aware hierarchical selection with a system-wise coarse granularity approach to eliminate costly data movement.

Result: CHESS achieves high-quality inference using only 1% of the KV cache, with up to 4.56× higher throughput and outperforming baselines.

Conclusion: CHESS demonstrates practical acceleration and superior performance in KV-cache management for long-context LLMs.
...
TL;DR: PyVision-RL introduces a reinforcement learning framework to prevent interaction collapse in multimodal models, promoting sustained tool use and efficiency.


<details>
  <summary>Details</summary>
Motivation: Addresses the issue of interaction collapse in reinforcement learning for agentic multimodal models, which limits tool usage and multi-turn reasoning.

Method: Combines oversampling-filtering-ranking rollout and accumulative tool reward to stabilize training. Includes unified pipeline for PyVision-Image and PyVision-Video, with on-demand context construction for videos.

Result: Shows strong performance and improved efficiency, highlighting the importance of sustained interaction and selective visual processing.

Conclusion: PyVision-RL effectively supports scalable multimodal agents by maintaining interaction and optimizing visual token usage.
...
TL;DR: A pipeline for verifying Large Reasoning Models' solutions to math problems automatically and interactively, avoiding reliance on answer-only checks.


<details>
  <summary>Details</summary>
Motivation: To provide a more accurate way to measure the capabilities of Large Reasoning Models in solving mathematical problems.

Method: Introduces a pipeline using prompts for solution verification, leveraging proof assistants and small models. Includes 3 AI agents for benchmarking.

Result: Experiments show low False Positives rates.

Conclusion: The pipeline offers a robust alternative for verifying solutions and generating correct answers in formal/informal languages.
...
TL;DR: POMDPPlanners is a Python package for evaluating POMDP planning algorithms, offering advanced tools for scalable and reproducible research in decision-making under uncertainty.


<details>
  <summary>Details</summary>
Motivation: To facilitate empirical evaluation of POMDP planning algorithms, especially in risk-sensitive scenarios where existing toolkits are inadequate.

Method: Integrates state-of-the-art algorithms, benchmark environments, hyperparameter optimization via Optuna, persistent caching, and parallel simulation.

Result: Reduces overhead in simulation studies, enabling scalable and reproducible research.

Conclusion: POMDPPlanners fills a gap in existing toolkits, supporting advanced research in decision-making under uncertainty.
...
TL;DR: This paper proposes a domain-specific benchmark and dataset for evaluating and fine-tuning Large Language Models (LLMs) in BIM-based design, leading to the development of Qwen-BIM, which outperforms general LLMs.


<details>
  <summary>Details</summary>
Motivation: The lack of specific datasets and evaluation benchmarks for LLMs in BIM-based design hinders their performance. This paper aims to bridge this gap.

Method: The paper introduces 1) a BIM-based design evaluation benchmark with quantitative indicators, 2) a method for generating textual data and constructing BIM-derived datasets, and 3) a fine-tuning strategy for LLMs.

Result: Qwen-BIM, developed using the proposed benchmark and datasets, achieves a 21.0% average increase in G-Eval score and performs comparably to much larger general LLMs.

Conclusion: The study provides the first domain-specific LLM for BIM-based design, supported by a comprehensive benchmark and high-quality dataset, advancing BIM-related LLM development.
...
TL;DR: The paper introduces a comprehensive benchmark for evaluating language model alignment across six categories, revealing gaps even in top models and suggesting alignment as a unified trait.


<details>
  <summary>Details</summary>
Motivation: To address the lack of realistic evaluation frameworks for language model alignment, especially under multi-turn scenarios, given the increasing real-world harm from alignment failures.

Method: Developed a benchmark with 904 scenarios spanning six alignment categories, validated by human raters. Evaluated 24 models using LLM judges against human annotations under conflicting instructions, simulated tool access, and multi-turn escalation.

Result: Top models show gaps in specific categories, while most models exhibit consistent weaknesses. Alignment behaves as a unified construct (like the g-factor), with high scores in one category correlating with others.

Conclusion: The benchmark and leaderboard are publicly released to support ongoing evaluation, with plans to expand scenarios and include new models, addressing persistent weaknesses.
...
TL;DR: The paper critiques LVLMs' reliance on spurious correlations instead of causal reasoning and introduces VLCGs and ViLCaR to improve structured understanding.


<details>
  <summary>Details</summary>
Motivation: Existing LVLM evaluations focus on answer correctness, not reasoning failures, prompting the need for tools to assess causal relevance.

Method: Introduces Vision-Language Causal Graphs (VLCGs) and ViLCaR benchmark with tasks and metrics for causal attribution and inference.

Result: Injecting structured relevance improves LVLM performance, showing limitations stem from lack of guidance, not reasoning ability.

Conclusion: Enhanced structural guidance boosts LVLM causal reasoning, suggesting future improvements should focus on structured representations.
...
TL;DR: The study explores how visual context affects human and LLM sentence acceptability judgments, finding minimal impact on humans but notable effects on LLMs, with varying model behaviors and correlations to log probabilities.


<details>
  <summary>Details</summary>
Motivation: To understand the influence of visual context on sentence acceptability judgments in humans and LLMs, comparing their responses and uncovering differences in processing.

Method: Analyzed human and LLM judgments of sentence acceptability with and without visual context, comparing performance and correlations.

Result: Visual context has little impact on humans but affects LLMs, with performance slightly better without visuals. LLM judgments vary by model, and correlations to log probabilities decrease with visuals.

Conclusion: The study highlights similarities and differences in human and LLM processing of multimodal contexts, with visual context influencing LLMs more than humans.
...
TL;DR: HELP is a novel GraphRAG framework addressing accuracy-efficiency trade-offs in multi-hop reasoning by using HyperNode Expansion and Logical Path-Guided Evidence Localization.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) face issues like knowledge boundaries and hallucinations, while existing RAG methods neglect structural interdependencies needed for multi-hop reasoning.

Method: HELP introduces HyperNode Expansion to chain knowledge triplets into reasoning paths (HyperNodes) and Logical Path-Guided Evidence Localization to map paths directly to the corpus for efficiency.

Result: HELP achieves competitive performance on QA benchmarks and a 28.8× speedup over Graph-based RAG baselines.

Conclusion: HELP balances accuracy and efficiency in multi-hop reasoning, avoiding costly graph traversals and semantic noise.
...
TL;DR: AgentOS redefines LLMs as dynamic autonomous systems with a 'Reasoning Kernel' and Deep Context Management, bridging micro token processing and macro intelligence for scalable AGI.


<details>
  <summary>Details</summary>
Motivation: Current LLM research lacks a cohesive framework linking token-level processing to systemic intelligence, necessitating a structured approach like AgentOS.

Method: Proposes AgentOS, using OS logic (e.g., memory paging, interrupt handling) to manage LLMs as Addressable Semantic Spaces with Semantic Slicing and Temporal Alignment.

Result: Enables resilient, scalable cognitive environments by mitigating cognitive drift and enhancing multi-agent coordination.

Conclusion: AGI's next frontier depends on architectural efficiency in system-level coordination, as demonstrated by AgentOS.
...
TL;DR: LogicGraph is a benchmark evaluating LLMs' multi-path logical reasoning, highlighting models' tendency to commit early to single paths and proposing actionable insights for improvement.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of large language models (LLMs) focus on single-path logical reasoning, neglecting real-world problems requiring diverse derivations. LogicGraph addresses this gap.

Method: LogicGraph uses a neuro-symbolic framework with backward logic generation and semantic instantiation to create solver-verified, high-depth multi-path reasoning problems.

Result: Experiments show LLMs struggle with multi-path reasoning, committing early to one route and failing to explore alternatives, with coverage gaps widening with reasoning depth.

Conclusion: LogicGraph identifies limitations in LLMs' reasoning diversity and offers insights to improve future models.
...
TL;DR: The Diligent Learner framework proposes LLMs can reach superintelligence via test-time search with a step-success probability γ. A benchmark measures γ for logical out-of-distribution inference, showing small LLMs struggle with increasing depth, while frontier models show resilience. Precise tool calls are crucial for successful reasoning.


<details>
  <summary>Details</summary>
Motivation: To understand the step-success probability γ in LLMs and assess their capability to handle increasingly complex logical tasks.

Method: Designing a benchmark with GF(2) circuit reconstruction tasks that grow harder with each step, testing LLMs' ability to integrate information.

Result: Small LLMs' γ declines superlinearly with depth; frontier models show partial robustness. Precise tool calls are key.

Conclusion: Tool design is critical for LLMs to achieve general superintelligence under the Diligent Learner framework.
...
TL;DR: A novel dual-model training framework inspired by affective neuroscience enhances performance efficiently by alternating between a base model and a larger motivated model during key training phases.


<details>
  <summary>Details</summary>
Motivation: Inspired by the interplay of emotions and cognition in the human brain, particularly the SEEKING motivational state, the work aims to mimic high curiosity and reward anticipation to improve cognitive performance in models.

Method: A dual-model framework is designed: a smaller base model trains continuously, while a larger motivated model activates intermittently under predefined conditions, allowing shared weight updates and selective capacity expansion.

Result: Empirical tests on image classification show the alternating scheme enhances the base model efficiently; the motivated model sometimes outperforms its standalone counterpart despite less data exposure.

Conclusion: The framework enables competitive or superior performance for two models under different deployment constraints while keeping training costs lower than standalone large models.
...
TL;DR: The paper introduces the Initial Exploration Problem (IEP) in Knowledge Graphs (KGs), identifying three barriers (scope uncertainty, ontology opacity, query incapacity) and proposes a theoretical framework for designing better interfaces.


<details>
  <summary>Details</summary>
Motivation: To address the challenges lay users face when first encountering KGs due to their semantic richness and complexity.

Method: Theorizes IEP using concepts from information behavior and human-computer interaction, analyzing KG interfaces via interaction primitives.

Result: Identifies barriers to initial KG exploration and highlights a design gap for scope revelation mechanisms.

Conclusion: Provides a theoretical foundation for improving KG interfaces to support initial exploration.
...
TL;DR: The paper introduces DEEPSYNTH, a benchmark to evaluate LLM-based agents on complex, real-world tasks involving information synthesis and structured reasoning, revealing current limitations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to assess LLM agents' ability to solve realistic tasks requiring multi-source information synthesis and deep reasoning.

Method: Created DEEPSYNTH with 120 tasks across 7 domains and 67 countries, using a multi-stage data collection pipeline involving annotators.

Result: State-of-the-art LLMs scored poorly (F1 scores of 8.97 and 17.5), struggling with hallucinations and large information spaces.

Conclusion: DEEPSYNTH highlights gaps in current agents' capabilities, serving as a vital benchmark for future research.
...
TL;DR: CG-DMER is a novel framework addressing intra- and inter-modality issues in ECG interpretation by leveraging spatial-temporal masked modeling and representation disentanglement, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Current multimodal ECG methods overlook spatial-temporal dependencies and suffer from modality-specific biases due to free-text clinical reports.

Method: Proposes CG-DMER with spatial-temporal masked modeling and modality-specific/shared encoders for disentangled representation learning.

Result: Demonstrates state-of-the-art performance on three public datasets.

Conclusion: CG-DMER effectively addresses key modality challenges and enhances ECG interpretation accuracy.
...
TL;DR: VLA models like \modelname simplify autonomous driving by reducing data and annotation needs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current VLA models require extensive data and reasoning annotations, which are costly. \modelname aims to overcome these challenges.

Method: \modelname uses Dr.~GRPO to mitigate difficulty bias and trains on less data without reasoning annotations.

Result: \modelname achieves competitive performance with 60% less data and 3× fewer tokens.

Conclusion: \modelname enables efficient autonomous systems by reducing training overhead.
...
TL;DR: Aletheia, a math research agent using Gemini 3 Deep Think, solved 6 out of 10 problems in the FirstProof challenge, with expert consensus on all but Problem 8.


<details>
  <summary>Details</summary>
Motivation: To demonstrate Aletheia's autonomous problem-solving capabilities in mathematics as part of the FirstProof challenge.

Method: Aletheia was tested on 10 problems within the challenge timeframe, with solutions evaluated by experts. Raw prompts and outputs were shared for transparency.

Result: Aletheia solved 6 problems (2, 5, 7, 8, 9, 10), with majority expert agreement except for Problem 8.

Conclusion: Aletheia shows promising autonomous problem-solving ability in math, validated by expert evaluations.
