...
TL;DR: A synthetic OCR dataset for Kazakh's Arabic, Cyrillic, and Latin scripts was created to evaluate MLLMs' performance, revealing their struggles with low-resource scripts compared to traditional OCR.


<details>
  <summary>Details</summary>
Motivation: The absence of OCR benchmarks for Kazakh's Arabic and Latin scripts and the scarcity of OCR work for low-resource Kazakh scripts prompted the creation of a synthetic dataset.

Method: A synthetic OCR dataset of 7,219 images mimicking real OCR tasks was constructed. Three MLLMs were evaluated alongside a classical OCR baseline.

Result: MLLMs performed poorly on Latin and Arabic scripts, misclassifying Kazakh Arabic text as other languages. Traditional OCR outperformed MLLMs in accuracy.

Conclusion: Current MLLMs lack capability for low-resource Abjad-based scripts, highlighting the need for more inclusive models and benchmarks.
...
TL;DR: The paper analyzes the sources of performance improvements in Sign Language Translation (SLT) models, revealing that many reported gains vanish under consistent evaluation conditions.


<details>
  <summary>Details</summary>
Motivation: To clarify whether performance improvements in SLT come from methodological novelty or other factors like implementation details and evaluation setups.

Method: Re-implemented key contributions in a unified codebase, standardizing preprocessing, video encoders, and training setups for fair comparison.

Result: Performance gains in literature often diminish under consistent conditions, highlighting the impact of implementation and evaluation details.

Conclusion: Standardized evaluation and transparency are crucial for meaningful progress in SLT research. The codebase is released to support reproducibility.
...
TL;DR: The paper introduces a unified probabilistic framework using MMD potential for safe image generation, combining Shielded Diffusion and Safe Denoiser, and identifies a critical time window for effective negative guidance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a clear framework for safety guidance in diffusion and flow models, the paper aims to unify geometric constraints (from robot planning) and data-driven negative guidance (from image generation) into a coherent probabilistic approach.

Method: The authors propose a Maximum Mean Discrepancy (MMD) potential framework, recasting existing methods (Shielded Diffusion and Safe Denoiser) as energy-based negative guidance. They use control-barrier functions to analyze when safety guidance is necessary.

Result: The framework identifies a critical time window in the denoising process where negative guidance must be strong, ensuring safe and high-quality generation. Experiments confirm its effectiveness in realistic scenarios.

Conclusion: Negative guidance should be applied early in the denoising process for safe and effective generation. The unified framework provides a principled approach to safety in diffusion models.
...
TL;DR: Compact VLMs with parameter-efficient adaptation outperform training-free pipelines and weakly supervised baselines in anomaly detection, balancing accuracy and latency.


<details>
  <summary>Details</summary>
Motivation: To address the need for reliable anomaly detectors in CCTV monitoring that combine accuracy and predictable latency under weak supervision.

Method: Parameter-efficient adaptation of compact VLMs, evaluated using a unified protocol covering preprocessing, prompting, dataset splits, metrics, and runtime.

Result: Adapted VLMs match or exceed established methods in accuracy and efficiency, with reduced prompt sensitivity.

Conclusion: Parameter-efficient fine-tuning makes compact VLMs practical anomaly detectors, offering a favorable accuracy-efficiency trade-off.
...
TL;DR: Info-VLA is a framework designed to prevent catastrophic forgetting in Vision--Language--Action (VLA) models by preserving cross-modal information structure through replay anchor contrastive learning and mutual information maximization.


<details>
  <summary>Details</summary>
Motivation: Existing continual learning methods fail to preserve cross-modal dependencies in VLA models, leading to performance degradation in open-ended robotic environments.

Method: Info-VLA uses replay anchor contrastive learning and cross-modal mutual information maximization to maintain cross-modal information structure.

Result: Experiments on LIBERO demonstrate that Info-VLA outperforms existing methods in task retention and adaptation.

Conclusion: Info-VLA effectively balances stability and plasticity in continual learning for VLA models by preserving cross-modal dependencies.
...
TL;DR: A multi-channel U-Net model is proposed for pixel-level multi-label segmentation of EL images, achieving 98% accuracy and generalizing well to unseen datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with assigning multiple labels to the same pixel in EL images, limiting their ability to capture overlapping degradation features.

Method: The paper introduces a multi-channel U-Net architecture that outputs independent probability maps for cracks, busbars, dark areas, and non-cell regions.

Result: The model achieved 98% accuracy and demonstrated generalization to unseen datasets.

Conclusion: The framework provides a scalable and extensible tool for automated PV module inspection, enhancing defect quantification and lifetime prediction.
...
TL;DR: Atsuko network improves multimodal emotion recognition by modeling fine-grained complementary features via multi-scale band decomposition and expert collaboration, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Prior methods mechanically rely on unimodal performance, missing complementary contributions, and use coarse-grained fusion unsuitable for emotion tasks.

Method: Decomposes features into high, mid, and low-frequency components, uses a dual-path mechanism for fine-grained fusion, and employs the Marginal Complementarity Module (MCM) to quantify modality contributions.

Result: Superior performance on CMU-MOSI, CMU-MOSEI, CH-SIMS, CH-SIMSv2, and MIntRec benchmarks.

Conclusion: Atsuko effectively addresses limitations of prior methods by focusing on fine-grained complementary features and outperforms existing approaches.
...
### [8] [A Multi-Agent Perception-Action Alliance for Efficient Long Video Reasoning](https://arxiv.org/abs/2603.14052)
*Yichang Xu,Gaowen Liu,Ramana Rao Kompella,Tiansheng Huang,Sihao Hu,Fatih Ilhan,Selim Furkan Tekin,Zachary Yahn,Ling Liu*

Main category: cs.CV

TL;DR: A4VL is a multi-agent system for efficient long-video reasoning, using iterative perception-action exploration with VLMs to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of scaling high-quality reasoning to long videos efficiently.

Method: Iterative perception-action loops with multi-agent collaboration, clue extraction, and cross-review scoring.

Result: Outperforms 18 VLMs and 10 recent methods in VideoQA benchmarks with lower latency.

Conclusion: A4VL demonstrates effective scalable long-video reasoning through multi-agent alliance and exploration.
...
### [9] [Mind the Discriminability Trap in Source-Free Cross-domain Few-shot Learning](https://arxiv.org/abs/2603.13341)
*Zhenyu Zhang,Yixiong Zou,Yuhua Li,Ruixuan Li,Guangyao Chen*

Main category: cs.CV

TL;DR: SF-CDFSL reveals that emphasizing visual-modal discriminability harms performance in VLM-based tasks. The study identifies cross-modal alignment as crucial and proposes a solution to mitigate visual learning shortcuts.


<details>
  <summary>Details</summary>
Motivation: To understand why strengthening visual-modal discriminability suppresses performance in VLM-based SF-CDFSL and to develop a solution.

Method: Theory and experiments show cross-entropy loss includes visual and cross-modal parts. The visual part acts as a shortcut. Proposed solution perturbs visual learning and aligns modalities gradually.

Result: Extensive experiments show state-of-the-art performance across tasks and datasets.

Conclusion: Focusing on cross-modal alignment improves VLM performance in SF-CDFSL, avoiding the discriminability trap.
...
### [10] [DDS-UDA: Dual-Domain Synergy for Unsupervised Domain Adaptation in Joint Segmentation of Optic Disc and Optic Cup](https://arxiv.org/abs/2603.13345)
*Yusong Xiao,Yuxuan Wu,Li Xiao,Gang Qu,Haiye Huo,Yu-Ping Wang*

Main category: cs.CV

TL;DR: DDS-UDA is a novel UDA framework addressing domain shift in CNN-based optic disc/cup segmentation by combining cross-domain consistency and intra-domain pseudo label learning.


<details>
  <summary>Details</summary>
Motivation: CNNs face performance issues due to domain shift and lack of large-scale annotations in medical imaging, which DDS-UDA aims to solve.

Method: Uses bi-directional cross-domain consistency and frequency-driven intra-domain pseudo label learning within a teacher-student architecture.

Result: Outperforms existing UDA methods on multi-domain fundus image datasets.

Conclusion: DDS-UDA effectively mitigates domain shift, enhancing segmentation robustness across heterogeneous imaging environments.
...
TL;DR: This paper introduces a novel patch-based post-training quantization method for dataset condensation, significantly reducing storage costs while maintaining representation quality, even at extremely low bit-widths like 2-bit. It outperforms existing methods by nearly doubling test accuracy in extreme compression scenarios.


<details>
  <summary>Details</summary>
Motivation: Prior methods in dataset condensation overlooked quantization's potential for further reducing storage costs. This paper addresses this gap by exploring post-training quantization to maintain quality without expensive training.

Method: Proposes a patch-based post-training quantization approach with localized quantization, quantization-aware clustering for similar patches, and a refinement module to align distributions. It's a plug-and-play framework compatible with various DC methods.

Result: Extensive experiments on benchmarks like CIFAR-10/100 and Tiny ImageNet show the method nearly doubles test accuracy (e.g., 26.0% → 54.1%) at extreme compression (2-bit) without additional distillation.

Conclusion: The method effectively combines dataset condensation with quantization, demonstrating superior performance in storage-constrained scenarios, especially at low bit-widths.
...
TL;DR: MURE introduces a novel VDR framework balancing fine-grained details and efficiency by leveraging multi-resolution encoding and adaptive token compression, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Existing VDR models face trade-offs between capturing fine-grained details and computational efficiency, especially with high-resolution documents.

Method: Proposes X-VisEmb paradigm (multi-resolution sampling, cross-granularity fusion, adaptive distillation) and MURE framework (hierarchical encoder, RMRL, semantic-aware clustering).

Result: MURE outperforms baselines on VDR benchmarks and exceeds ColPali's performance with fewer visual tokens.

Conclusion: MURE effectively balances retrieval efficacy and efficiency, advancing VDR performance.
...
### [13] [Local Precise Refinement: A Dual-Gated Mixture-of-Experts for Enhancing Foundation Model Generalization against Spectral Shifts](https://arxiv.org/abs/2603.13352)
*Xi Chen,Maojun Zhang,Yu Liu,Shen Yan*

Main category: cs.CV

TL;DR: SpectralMoE, a novel PEFT framework using a Mixture-of-Experts (MoE) architecture, achieves robust DGSS by performing fine-grained, spatially-adaptive refinement on foundation model features, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing spectral shifts and spatial heterogeneity in DGSS for spectral remote sensing, which degrade model performance in unseen domains, necessitating fine-grained adaptations.

Method: Proposes SpectralMoE, a dual-gated MoE architecture that routes visual and depth features to specialized experts, followed by cross-attention to fuse refined cues into the visual stream.

Result: SpectralMoE achieves state-of-the-art performance on multiple DGSS benchmarks across hyperspectral, multispectral, and RGB remote sensing imagery.

Conclusion: Fine-grained, spatially-adaptive refinement through SpectralMoE effectively mitigates semantic ambiguities caused by spectral shifts, setting a new benchmark in DGSS.
...
TL;DR: The paper compares CNN, contrastive VLM, and generative VLM models for crop disease detection, introducing a new benchmark (AgriPath-LF16) and showing that model choice depends on deployment context.


<details>
  <summary>Details</summary>
Motivation: To address inconsistency in crop disease detection models across diverse acquisition conditions by systematically evaluating three model paradigms.

Method: Introduces AgriPath-LF16, a benchmark with 111k images, and evaluates CNNs, contrastive VLMs, and generative VLMs under unified protocols using macro-F1 and PSR metrics.

Result: CNNs excel on lab imagery but degrade under domain shift; contrastive VLMs are robust and efficient; generative VLMs show resilience but have failure modes from free-text generation.

Conclusion: Architectural choice should consider deployment context, not just aggregate accuracy, as each model paradigm has distinct strengths and weaknesses.
...
TL;DR: Int3DNet predicts 3D intention areas using scene geometry and head-hand motion cues for seamless Mixed Reality interactions, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To enable proactive system responses in Mixed Reality by anticipating user actions, reducing delays, and enhancing user experience.

Method: Cross attention fusion of sparse motion cues and scene point clouds for direct spatial intention interpretation.

Result: Consistent performance up to 1500 ms on MoGaze and CIRCLE datasets, and successful visual question answering demonstration.

Conclusion: Int3DNet reliably predicts 3D intention areas, facilitating seamless human-MR interaction through proactive processing.
...
TL;DR: Bi-CamoDiffusion improves camouflaged object detection by integrating edge priors and unified optimization, outperforming benchmarks and state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: To enhance camouflaged object detection by addressing boundary sharpness and structural ambiguity, leveraging edge priors.

Method: Integrates edge priors via parameter-free injection and uses a unified optimization objective balancing spatial accuracy, structure, and uncertainty.

Result: Surpasses baseline and state-of-the-art methods on CAMO, COD10K, and NC4K benchmarks with sharper boundaries and fewer false positives.

Conclusion: Bi-CamoDiffusion achieves superior performance in camouflaged object detection through effective edge integration and optimization.
...
### [17] [Slang Context-based Inference Enhancement via Greedy Search-Guided Chain-of-Thought Prompting](https://arxiv.org/abs/2603.13230)
*Jinghan Cao,Qingyang Ren,Xiangyun Chen,Xinjin Li,Haoxiang Gao,Yu Zhao*

Main category: cs.CL

TL;DR: The paper explores slang interpretation challenges for LLMs, proposing a greedy search-guided chain-of-thought framework that improves accuracy, especially in smaller models.


<details>
  <summary>Details</summary>
Motivation: Slang interpretation is difficult for LLMs due to contextual and cultural dependencies, and lack of domain-specific training data.

Method: The study investigates slang inference challenges, tests model size and temperature impact, and integrates greedy search with chain-of-thought prompting for small models.

Result: Larger models don't outperform smaller ones in slang interpretation; the proposed framework enhances accuracy.

Conclusion: The framework improves slang comprehension, highlighting context dependency and offering a practical solution.
...
### [18] [Steering at the Source: Style Modulation Heads for Robust Persona Control](https://arxiv.org/abs/2603.13249)
*Yoshihiro Izawa,Gouki Minegishi,Koshi Eguchi,Sosuke Hosokawa,Kenjiro Taura*

Main category: cs.CL

TL;DR: The paper introduces Style Modulation Heads for precise control of LLMs, reducing coherency degradation compared to residual stream steering.


<details>
  <summary>Details</summary>
Motivation: To address coherency degradation in LLM control via activation steering without fine-tuning.

Method: Identifies sparse attention heads (Style Modulation Heads) using geometric analysis of internal representations (layer-wise cosine similarity and head-wise contribution scores).

Result: Targeting these heads achieves robust behavioral control with less coherency degradation.

Conclusion: Component-level localization enables safer and more precise LLM control.
...
### [19] [Training-Free Agentic AI: Probabilistic Control and Coordination in Multi-Agent LLM Systems](https://arxiv.org/abs/2603.13256)
*Mohammad Parsa Hosseini,Ankit Shah,Saiyra Qureshi,Alex Huang,Connie Miao,Wei Wei*

Main category: cs.CL

TL;DR: REDEREF is a lightweight, training-free controller for multi-agent LLMs that improves efficiency via belief-guided delegation, reflection-driven rerouting, evidence-based selection, and memory-aware priors.


<details>
  <summary>Details</summary>
Motivation: Practical deployment of multi-agent LLM systems is hindered by inefficient routing, noisy feedback, and high interaction costs.

Method: REDEREF integrates belief-guided delegation, reflection-driven rerouting, evidence-based selection, and memory-aware priors.

Result: Compared to random delegation, REDEREF reduces token usage by 28%, agent calls by 17%, and time-to-success by 19%.

Conclusion: Simple probabilistic control enhances efficiency and robustness of multi-agent LLM systems without training.
...
### [20] [How Transformers Reject Wrong Answers: Rotational Dynamics of Factual Constraint Processing](https://arxiv.org/abs/2603.13259)
*Javier Marín*

Main category: cs.CL

TL;DR: The paper explores how language models process correct vs. incorrect answers by analyzing geometric changes across network layers, revealing rotational divergence and active suppression of correct answers.


<details>
  <summary>Details</summary>
Motivation: To understand the dynamic internal representations of language models when processing correct and incorrect continuations, beyond static probing methods.

Method: Forced-completion probing tracks five geometric measurements across layers of decoder-only models (1.5B-13B parameters) using identical queries with correct/incorrect single-token continuations.

Result: Correct and incorrect paths diverge rotationally (not rescaling), models actively suppress correct answers, and these phenomena emerge only above 1.6B parameters.

Conclusion: Factual processing in models involves rotational geometric changes and active suppression, invisible to single-layer probes or magnitude comparisons.
...
### [21] [Explain in Your Own Words: Improving Reasoning via Token-Selective Dual Knowledge Distillation](https://arxiv.org/abs/2603.13260)
*Minsang Kim,Seung Jun Baek*

Main category: cs.CL

TL;DR: TSD-KD is a student-centric knowledge distillation method focusing on important tokens and indirect feedback, improving reasoning tasks significantly.


<details>
  <summary>Details</summary>
Motivation: To address the distribution mismatch in traditional KD methods for students with limited capacity, especially in complex reasoning tasks.

Method: Token-Selective Dual Knowledge Distillation (TSD-KD), combining indirect (preference ranking) and direct (selective token distillation) methods with entropy regularization.

Result: State-of-the-art performance on 10 reasoning benchmarks, surpassing baselines by up to 54.4%, with students sometimes outperforming their teachers by up to 20.3%.

Conclusion: TSD-KD effectively supports student reasoning and self-improvement, achieving superior performance in knowledge distillation.
...
### [22] [Design and evaluation of an agentic workflow for crisis-related synthetic tweet datasets](https://arxiv.org/abs/2603.13625)
*Roben Delos Reyes,Timothy Douglas,Asanobu Kitamoto*

Main category: cs.CL

TL;DR: The paper introduces a workflow to generate synthetic crisis-related tweets, addressing the challenges of accessing real-world Twitter data for crisis informatics research.


<details>
  <summary>Details</summary>
Motivation: Recent changes in Twitter's data access policies and the limitations of existing curated datasets hinder the development of AI systems for crisis informatics. Synthetic data can bridge this gap.

Method: An agentic workflow generates synthetic tweets iteratively, conditioned on target characteristics, evaluated for compliance, and refined using feedback.

Result: The workflow successfully generates synthetic tweets for post-earthquake damage assessment, capturing target labels for location and damage level, and is usable for evaluating AI systems.

Conclusion: The workflow provides a scalable and flexible alternative to real-world data curation, enabling systematic synthetic data generation for diverse crisis informatics applications.
...
### [23] [Widespread Gender and Pronoun Bias in Moral Judgments Across LLMs](https://arxiv.org/abs/2603.13636)
*Gustavo Lúcius Fernandes,Jeiverson C. V. M. Santos,Pedro O. S. Vaz-de-Melo*

Main category: cs.CL

TL;DR: The study examines how grammatical features (person, number, gender) bias LLM fairness judgments, revealing singular/third-person sentences are favored while second-person and male markers are penalized.


<details>
  <summary>Details</summary>
Motivation: To uncover biases in LLMs' moral fairness judgments influenced by grammatical person, number, and gender markers.

Method: Generated 14,850 sentence variants from ETHICS dataset, evaluated six LLM families using Statistical Parity Difference (SPD).

Result: Singular/third-person sentences deemed 'fairer'; second-person and male markers penalized; non-binary subjects favored.

Conclusion: Grammatical features introduce biases in LLM fairness judgments, highlighting the need for targeted interventions.
...
### [24] [Benchmarking Large Language Models on Reference Extraction and Parsing in the Social Sciences and Humanities](https://arxiv.org/abs/2603.13651)
*Yurui Zhu,Giovanni Colavizza,Matteo Romanello*

Main category: cs.CL

TL;DR: This paper introduces a unified benchmark to evaluate bibliographic reference tasks in SSH, highlighting challenges like multilingual and footnote citations. It compares traditional tools with LLMs and suggests hybrid solutions.


<details>
  <summary>Details</summary>
Motivation: To address the lack of evaluation benchmarks for bibliographic reference extraction and parsing in SSH, where citations are often multilingual, embedded in footnotes, and heterogeneous.

Method: A unified benchmark (CEX, EXCITE, LinkedBooks) evaluates three tasks: reference extraction, parsing, and end-to-end parsing. It compares GROBID with LLMs under schema-constrained setups and explores LoRA adaptation and segmentation.

Result: Extraction saturates at moderate capability, while parsing remains challenging. LoRA adaptation improves performance, especially in SSH-heavy benchmarks, and hybrid deployment (GROBID + LLMs) is proposed.

Conclusion: The study advocates for hybrid solutions combining traditional tools and LLMs to handle diverse citation formats in SSH effectively.
...
### [25] [MedPriv-Bench: Benchmarking the Privacy-Utility Trade-off of Large Language Models in Medical Open-End Question Answering](https://arxiv.org/abs/2603.14265)
*Shaowei Guan,Yu Zhai,Hin Chi Kwok,Jiawei Du,Xinyu Feng,Jing Li,Harry Qin,Vivian Hui*

Main category: cs.CL

TL;DR: MedPriv-Bench is introduced as the first benchmark for evaluating both privacy preservation and clinical utility in medical QA, addressing the overlooked risk of contextual leakage in RAG systems.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks in healthcare prioritize accuracy, neglecting privacy risks like patient re-identification via contextual leakage, despite strict regulations like HIPAA and GDPR.

Method: The framework employs a multi-agent, human-in-the-loop pipeline to create sensitive medical contexts and queries, using a RoBERTa-NLI model for automated leakage evaluation.

Result: The benchmark achieves 85.9% alignment with human experts in detecting leakage, revealing a pervasive privacy-utility trade-off across 9 LLMs.

Conclusion: Domain-specific benchmarks are crucial to ensure the safety and efficacy of medical AI systems in privacy-sensitive settings.
...
### [26] [Privacy Preserving Topic-wise Sentiment Analysis of the Iran Israel USA Conflict Using Federated Transformer Models](https://arxiv.org/abs/2603.13655)
*Md Saiful Islam,Tanjim Taharat Aurpa,Sharad Hasan,Farzana Akter*

Main category: cs.CL

TL;DR: The paper analyzes global public sentiment on the Iran-Israel-USA conflict via YouTube comments, using deep learning, federated learning, and XAI techniques, achieving high accuracy while preserving privacy.


<details>
  <summary>Details</summary>
Motivation: To understand global public perception of the Iran-Israel-USA conflict through sentiment analysis of social media discussions, while ensuring user privacy.

Method: Collected 19,000 YouTube comments, preprocessed them, and used VADER for initial sentiment labeling. Applied LDA for topic modeling, fine-tuned transformer models (BERT, RoBERTa, etc.), and integrated federated learning for privacy. Used SHAP for explainability.

Result: ELECTRA achieved the highest accuracy (91.32%). Federated learning maintained strong performance (89.59% accuracy) while preserving privacy.

Conclusion: Transformer models are effective for sentiment analysis. Federated learning ensures privacy without significant performance loss. The framework provides valuable insights into public sentiment.
...
### [27] [Preconditioned Test-Time Adaptation for Out-of-Distribution Debiasing in Narrative Generation](https://arxiv.org/abs/2603.13683)
*Hanwen Shen,Ting Ying,Jiajie Lu,Shanshan Wang*

Main category: cs.CL

TL;DR: The paper highlights that debiased LLMs fail on unfamiliar bias prompts, validates this as a distribution shift, and introduces CAP-TTA, a test-time adaptation framework for fast, stable updates, reducing bias and latency while maintaining fluency and debiasing effectiveness.


<details>
  <summary>Details</summary>
Motivation: Debiased LLMs struggle with unfamiliar bias prompts, leading to toxic outputs, which motivates the need for a dynamic adaptation method to handle such distribution shifts.

Method: Proposes CAP-TTA, a test-time adaptation framework using context-aware LoRA updates triggered by high bias-risk, with a precomputed diagonal preconditioner for efficiency.

Result: CAP-TTA reduces bias, achieves lower update latency than AdamW/SGD, mitigates catastrophic forgetting, and improves narrative fluency while maintaining debiasing effectiveness.

Conclusion: CAP-TTA effectively addresses the limitations of static debiased LLMs by dynamically adapting to unfamiliar bias prompts, offering a balanced solution for bias reduction and model performance.
...
### [28] [QuarkMedBench: A Real-World Scenario Driven Benchmark for Evaluating Large Language Models](https://arxiv.org/abs/2603.13691)
...
TL;DR: QuarkMedBench is introduced as a benchmark to evaluate LLMs on real-world medical queries, addressing gaps in current exam-based evaluations by using structured rubrics and automated scoring.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings of standardized exams in reflecting real-world medical query complexities and provide a more accurate assessment tool.

Method: Developed QuarkMedBench with 20,821 single-turn and 3,853 multi-turn queries across medical domains, using automated scoring with 220,617 rubrics and multi-model consensus.

Result: Achieved 91.8% concordance with expert audits, revealing performance disparities among models in real-world scenarios.

Conclusion: QuarkMedBench offers a rigorous, reproducible measure for LLM performance in healthcare, with dynamic updates to stay relevant.
...
### [29] [Repetition Without Exclusivity: Scale Sensitivity of Referential Mechanisms in Child-Scale Language Models](https://arxiv.org/abs/2603.13696)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: The paper evaluates mutual exclusivity (ME) in text-only language models trained on child-directed speech, finding repetition priming instead of ME effects.


<details>
  <summary>Details</summary>
Motivation: To understand how language models trained on child-directed speech handle mutual exclusivity, a cognitive bias favoring novel word-referent mappings.

Method: Operationalized ME as referential suppression, tested models (BabyBERTa, GPT-2 variants) on preregistered ME tasks, and analyzed context sensitivity.

Result: Models showed repetition priming (opposite of ME), with effects diminishing as language modeling improved but never reversing.

Conclusion: Distributional learning on child-directed speech leads to repetition-based reference tracking, suggesting referential grounding may be essential for ME.
...
### [30] [Can We Trust LLMs on Memristors? Diving into Reasoning Ability under Non-Ideality](https://arxiv.org/abs/2603.13725)
*Taiqiang Wu,Yuxin Cheng,Chenchen Ding,Runming Yang,Xincheng Feng,Wenyong Zhou,Zhengwu Liu,Ngai Wong*

Main category: cs.CL

TL;DR: Memristor-based CIM architectures enhance LLMs' energy efficiency but face precision issues due to memristor non-idealities. This study analyzes these impacts and evaluates training-free strategies to improve robustness.


<details>
  <summary>Details</summary>
Motivation: To address precision issues in memristor-based CIM architectures for LLMs caused by intrinsic non-idealities, aiming to maintain reasoning capabilities.

Method: Investigated non-idealities' impact on LLM reasoning and evaluated three training-free strategies: thinking mode, in-context learning, and module redundancy.

Result: Found reasoning capability drops significantly, with varying benchmarks. Shallow layer redundancy boosts robustness, thinking mode excels at low noise but degrades at high noise, and in-context learning reduces output length slightly impacting performance.

Conclusion: Provides insights into LLM reasoning under non-idealities and practical strategies like redundancy to enhance robustness.
...
### [31] [Knowledge Distillation for Large Language Models](https://arxiv.org/abs/2603.13765)
*Alejandro Paredes La Torre,Barbara Flores,Diego Rodriguez*

Main category: cs.CL

TL;DR: A resource-efficient framework compresses large language models via knowledge distillation and chain-of-thought reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To create smaller, efficient models suitable for resource-constrained settings without significant loss of capability.

Method: Knowledge distillation from Qwen 3B to Qwen 0.5B, combined with guided chain-of-thought reinforcement learning and 4-bit weight quantization.

Result: Distilled student retains 70-95% of teacher's capability across tasks and reduces memory footprint.

Conclusion: Combining knowledge distillation with chain-of-thought reinforcement learning yields compact, efficient models.
...
### [32] [LiveWeb-IE: A Benchmark For Online Web Information Extraction](https://arxiv.org/abs/2603.13773)
*Seungbin Yang,Jihwan Kim,Jaemin Choi,Dongjin Kim,Soyoung Yang,ChaeHun Park,Jaegul Choo*

Main category: cs.CL

TL;DR: The paper introduces a new benchmark (\dataset) for evaluating web information extraction (WIE) systems on live websites and proposes a multi-stage agentic framework (VGS) mimicking human cognition.


<details>
  <summary>Details</summary>
Motivation: Traditional WIE evaluations use static HTML snapshots, ignoring the dynamic nature of the web, which limits real-world performance. The paper aims to address this gap.

Method: The authors curate natural language queries from live websites, categorized by complexity, and introduce VGS, a multi-stage framework that visually grounds and extracts information.

Result: Experiments show VGS's effectiveness and robustness across diverse models.

Conclusion: The study advances practical and robust WIE systems by addressing dynamic web challenges.
...
### [33] [Auditing Cascading Risks in Multi-Agent Systems via Semantic-Geometric Co-evolution](https://arxiv.org/abs/2603.13325)
*Zixun Luo,Yuhang Fan,Hengyu Lin,Yufei Li,Youzhi Zhang*

Main category: cs.MA

TL;DR: A framework for detecting cascading risks in LLM-based Multi-Agent Systems by combining semantic and geometric analysis to identify early warning signals.


<details>
  <summary>Details</summary>
Motivation: Traditional auditing methods lag in identifying early structural precursors of instability in MAS interactions, necessitating a proactive approach.

Method: The paper models interactions as dynamic graphs, uses Ollivier--Ricci Curvature to measure information redundancy, and couples semantic flow with graph geometry to detect deviations.

Result: Experiments show curvature anomalies precede semantic violations, enabling proactive intervention and root-cause attribution.

Conclusion: The framework successfully identifies early structural risks in MAS interactions, improving proactive detection and interpretability.
...
### [34] [ClimateAgents: A Multi-Agent Research Assistant for Social-Climate Dynamics Analysis](https://arxiv.org/abs/2603.13840)
*Shan Shan*

Main category: cs.MA

TL;DR: ClimateAgents is a multi-agent AI framework designed to enhance social-climate analysis by integrating diverse data sources and adaptive reasoning, offering interpretable and flexible research support.


<details>
  <summary>Details</summary>
Motivation: Traditional climate analysis lacks flexibility and cross-domain integration. This study aims to bridge the gap by developing an adaptive framework for socio-environmental research.

Method: The framework employs collaborative, domain-specialized agents for tasks like hypothesis generation, data analysis, and reporting, using multimodal data from sources like the UN and World Bank.

Result: ClimateAgents enables adaptive exploration of climate-social relationships, demonstrating the potential of multi-agent systems in interdisciplinary research.

Conclusion: Multi-agent AI systems like ClimateAgents can significantly improve interpretability and adaptability in socio-climate analysis, fostering interdisciplinary research.
...
### [35] [How do Role Models Shape Collective Morality? Exemplar-Driven Moral Learning in Multi-Agent Simulation](https://arxiv.org/abs/2603.13876)
*Junjie Liao,Huacong Tang,Zhou Ziheng,Yizhou Wang,Fangwei Zhong*

Main category: cs.MA

TL;DR: Role models shape collective morality through identity-driven conformity, overriding initial dispositions and causing rapid value convergence.


<details>
  <summary>Details</summary>
Motivation: To understand how role models influence collective morality by examining interactions among agents with diverse intrinsic drives.

Method: Multi-agent simulation using a Large Language Model, featuring agents with cooperative to competitive drives, interacting via a four-stage cognitive loop (plan-act-observe-reflect). Four experimental games (Alignment, Collapse, Conflict, Construction) and motivational ablation studies were conducted.

Result: Identity-driven conformity overrides initial dispositions, leading agents to adapt values to align with perceived successful exemplars, resulting in rapid value convergence.

Conclusion: Role models significantly shape collective morality by driving conformity and value convergence among individuals.
...
### [36] [Beyond Self-Interest: Modeling Social-Oriented Motivation for Human-like Multi-Agent Interactions](https://arxiv.org/abs/2603.13890)
*Jingzhe Lin,Ceyao Zhang,Yaodong Yang,Yizhou Wang,Song-Chun Zhu,Fangwei Zhong*

Main category: cs.MA

TL;DR: ASVO agents combine LLMs with Social Value Orientation theory to enable socially motivated, human-like multi-agent interactions, improving behavioral naturalness.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the lack of social motivation modeling in LLMs for multi-agent interactions by integrating desire-driven autonomy with SVO theory.

Method: Agents update beliefs and desires based on environmental changes and others' actions, inferring motivational states and dynamically computing SVO to guide behavior.

Result: Experiments show ASVO agents outperform baselines in naturalness and human-likeness across School, Workplace, and Family scenarios.

Conclusion: Structured desire systems and adaptive SVO drift enable realistic social simulations in multi-agent environments.
...
### [37] [A Benchmark for Multi-Party Negotiation Games from Real Negotiation Data](https://arxiv.org/abs/2603.14066)
*Leo Benac,Jonas Raedler,Zilin Ma,Finale Doshi-Velez*

Main category: cs.MA

TL;DR: A benchmark for multi-party negotiations with binding commitments was introduced, testing three value-function approximations to evaluate their effectiveness across different game structures.


<details>
  <summary>Details</summary>
Motivation: To address the under-studied regime of multi-party negotiations involving sequences of binding commitments and understand how different valuation strategies perform under varying game structures.

Method: A configurable game generator was used to simulate negotiation scenarios with varying properties like incentive alignment and goal complexity. Three value-function approximations (myopic reward, optimistic upper bound, pessimistic lower bound) were tested.

Result: Different game structures require different valuation strategies, highlighting the need for adaptable agents that can learn robust state values and plan effectively over long horizons.

Conclusion: The study underscores the importance of learning robust state values and planning under binding commitments, as no single valuation strategy fits all negotiation scenarios.
...
### [38] [Understanding Strategic Platform Entry and Seller Exploration: A Stackelberg Model](https://arxiv.org/abs/2603.14206)
*Garrett Seo,Xintong Wang,David C. Parkes*

Main category: cs.MA

TL;DR: The paper analyzes how online platforms like Amazon enter their own marketplaces to compete with third-party sellers, using a Stackelberg model and computational methods to study platform entry policies and seller strategies.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the empirical phenomenon of platforms imitating successful third-party products and its impact on seller innovation and market dynamics.

Method: A Stackelberg model is used with the platform as the leader. The study employs a theoretical and computational framework, including Gittins-index policies for single sellers and deep reinforcement learning for multiple sellers.

Result: The findings reveal the optimal entry policies for platforms and explore-exploit strategies for sellers, aligning with empirical evidence from markets like Amazon and Google Play.

Conclusion: The research highlights the incentives behind platform entry and its implications for market diversity and innovation, offering insights for regulatory efforts.
...
### [39] [EcoFair-CH-MARL: Scalable Constrained Hierarchical Multi-Agent RL with Real-Time Emission Budgets and Fairness Guarantees](https://arxiv.org/abs/2603.14625)
*Saad Alqithami*

Main category: cs.MA

TL;DR: EcoFair-CH-MARL is a multi-agent reinforcement learning framework for maritime logistics, combining emissions control, fairness, and scalability, achieving better performance and equity than existing methods.


<details>
  <summary>Details</summary>
Motivation: The need for efficient, sustainable, and equitable maritime logistics solutions under global decarbonisation targets and market pressures.

Method: A constrained hierarchical multi-agent reinforcement learning framework with a primal-dual budget layer, fairness-aware reward transformer, and two-tier policy architecture.

Result: 15% lower emissions, 12% higher throughput, and 45% fair-cost improvement over baselines, with better equity metrics (lower Gini, higher min-max welfare).

Conclusion: EcoFair-CH-MARL enables large-scale, regulation-compliant, and socially responsible multi-agent coordination in safety-critical domains.
...
### [40] [Forecast-Aware Cooperative Planning on Temporal Graphs under Stochastic Adversarial Risk](https://arxiv.org/abs/2603.14697)
*Manshi Limbu,Xuan Wang,Gregory J. Stein,Daigo Shishika,Xuesu Xiao*

Main category: cs.MA

TL;DR: The paper introduces a forecast-aware cooperative planning framework for multi-robot teams to navigate dynamic, risky environments by predicting and proactively addressing evolving risks.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks assume static risk landscapes, failing to handle predictable temporal trends in risk evolution, which motivates the need for a dynamic, anticipatory approach.

Method: The proposed framework integrates stochastic risk forecasting with anticipatory support allocation on temporal graphs, modeling adversary dynamics as a first-order Markov process to generate time-indexed edge-risk forecasts.

Result: Experimental results show the framework reduces total expected team cost compared to non-anticipatory baselines, nearing the performance of an oracle planner.

Conclusion: The forecast-aware approach effectively addresses dynamic risk environments, improving support coordination and joint robot path planning for cooperative missions.
...
### [41] [Translational Gaps in Graph Transformers for Longitudinal EHR Prediction: A Critical Appraisal of GT-BEHRT](https://arxiv.org/abs/2603.13231)
*Krish Tadigotla*

Main category: cs.LG

TL;DR: GT-BEHRT, a graph-transformer for EHRs, improves heart failure prediction but lacks rigorous evaluation in calibration, fairness, and deployment feasibility.


<details>
  <summary>Details</summary>
Motivation: Existing EHR transformers treat encounters as unordered codes, missing visit-level relationships. GT-BEHRT aims to capture these while maintaining long-term temporal patterns.

Method: GT-BEHRT combines graph and transformer architectures, evaluated on MIMIC-IV and All of Us datasets. Performance metrics include AUROC, AUPRC, and F1 scores.

Result: GT-BEHRT achieves strong discrimination (AUROC 94.37, AUPRC 73.96, F1 64.70) but lacks calibration, fairness analysis, and deployment feasibility assessment.

Conclusion: GT-BEHRT is a promising EHR model, but more rigorous evaluation is needed for clinical reliability.
...
### [42] [RFX-Fuse: Breiman and Cutler's Unified ML Engine + Native Explainable Similarity](https://arxiv.org/abs/2603.13234)
*Chris Kuchar*

Main category: cs.LG

TL;DR: RFX-Fuse revives Breiman and Cutler's original Random Forest vision by integrating classification, regression, unsupervised learning, similarity, outlier detection, imputation, and visualization into a single GPU/CPU-supported tool, replacing multiple modern ML tools.


<details>
  <summary>Details</summary>
Motivation: Modern ML pipelines rely on separate tools (e.g., XGBoost, FAISS, SHAP) for different tasks, leading to complexity. RFX-Fuse aims to unify these capabilities into a single Random Forest-based model, streamlining workflows.

Method: RFX-Fuse expands Random Forests to include proximity-based similarity, outlier detection, missing value imputation, and visualization. It introduces Proximity Importance for explainable similarity and dataset-specific imputation validation.

Result: RFX-Fuse provides a unified alternative to 5+ separate tools, using a single set of trees. It offers native explainability for similarity and validates imputation methods without ground truth labels.

Conclusion: RFX-Fuse successfully delivers Breiman and Cutler's original vision, offering a comprehensive, efficient, and explainable ML engine for modern pipelines.
...
### [43] [Continual Fine-Tuning with Provably Accurate and Parameter-Free Task Retrieval](https://arxiv.org/abs/2603.13235)
*Hang Thi-Thuy Le,Long Minh Bui,Minh Hoang,Trong Nghia Hoang*

Main category: cs.LG

TL;DR: The paper proposes a new parameter-adaptation method for continual fine-tuning, combining the strengths of input- and parameter-adaptation approaches while avoiding their weaknesses.


<details>
  <summary>Details</summary>
Motivation: Existing continual fine-tuning methods either suffer from forgetting (input-adaptation) or lack adaptability (parameter-adaptation). The goal is to merge their advantages.

Method: The method includes adaptive module composition for task-specific updates and clustering-based retrieval for adaptive representation use at test time, supported by theoretical error bounds.

Result: Experiments demonstrate improved retrieval and predictive performance, especially under significant task semantic shifts.

Conclusion: The proposed method successfully balances adaptability and retrieval reliability, offering theoretical insights and practical benefits.
...
### [44] [Introducing Feature-Based Trajectory Clustering, a clustering algorithm for longitudinal data](https://arxiv.org/abs/2603.13254)
*Marie-Pierre Sylvestre,Laurence Boulanger*

Main category: cs.LG

TL;DR: A new algorithm clusters longitudinal data by mapping individuals to Euclidean space based on time-dependent features and then applying Spectral Clustering.


<details>
  <summary>Details</summary>
Motivation: To identify groups of individuals sharing common characteristic features in their time-dependent variable evolution.

Method: 1. Map individuals to Euclidean space using formulae capturing characteristic features. 2. Apply Spectral Clustering to the resulting points.

Result: The algorithm effectively clusters individuals with shared time-dependent features.

Conclusion: The proposed method successfully groups individuals based on common patterns in longitudinal data.
...
### [45] [Your Code Agent Can Grow Alongside You with Structured Memory](https://arxiv.org/abs/2603.13258)
*Yi-Xuan Deng,Xiaoqin Liu,Yi Zhang,Guo-Wei Yang,Shuojin Yang*

Main category: cs.LG

TL;DR: MemCoder bridges the static-dynamic gap in code agents by leveraging historical project evolution and real-time feedback, achieving SOTA performance with a 9.4% improvement over DeepSeek-V3.2.


<details>
  <summary>Details</summary>
Motivation: Existing code agents fail to utilize temporal project evolution and reasoning trajectories, limiting adaptability in complex tasks.

Method: MemCoder distills intent-to-code mappings from past commits, uses real-time verification feedback for self-refinement, and internalizes validated solutions as long-term knowledge.

Result: MemCoder outperforms DeepSeek-V3.2 by 9.4% in resolved rate on SWE-bench Verified, demonstrating SOTA performance.

Conclusion: Enabling co-evolution with humans through history and feedback unlocks general models' potential in software engineering.
...
### [46] [Beyond Attention: True Adaptive World Models via Spherical Kernel Operator](https://arxiv.org/abs/2603.13263)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: The paper critiques traditional latent space methods in world model AI, introduces the Spherical Kernel Operator (SKO) to overcome their flaws, and demonstrates its superior performance in language modeling.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for world model AI are flawed as they shift manifold learning into latent spaces, suffer from saturation, and struggle with dimensionality. The paper aims to address these issues by redefining the predictive mechanism.

Method: The authors propose SKO, a framework replacing standard attention by projecting data onto a hypersphere and using localized ultraspherical polynomials for direct integral reconstruction, avoiding saturation.

Result: SKO bypasses the saturation problem, depends on intrinsic manifold dimension, and decouples dynamics from observation bias. Empirical tests show it outperforms standard attention in language modeling.

Conclusion: SKO offers a mathematically rigorous and empirically superior alternative to traditional latent space methods, enhancing predictive capacity and convergence in AI world models.
...
### [47] [Federated Personal Knowledge Graph Completion with Lightweight Large Language Models for Personalized Recommendations](https://arxiv.org/abs/2603.13264)
*Fernando Spadea,Oshani Seneviratne*

Main category: cs.LG

TL;DR: FedTREK-LM integrates lightweight LLMs, PKGs, FL, and Kahneman-Tversky Optimization for decentralized personalization, outperforming benchmarks by 4x in F1-score and highlighting the need for real user data.


<details>
  <summary>Details</summary>
Motivation: To enable scalable, decentralized personalization without centralizing private user data.

Method: Combines lightweight LLMs, evolving PKGs, federated learning, and Kahneman-Tversky Optimization for context-aware reasoning in recommendations.

Result: Outperforms HAKE, KBGAT, and FedKGRec by over 4x in F1-score; synthetic data degrades performance by up to 46%.

Conclusion: FedTREK-LM provides a scalable, adaptive framework for LLM-powered personalization across decentralized PKGs.
...
### [48] [Knowledge, Rules and Their Embeddings: Two Paths towards Neuro-Symbolic JEPA](https://arxiv.org/abs/2603.13265)
*Yongchao Huang,Hassan Raza*

Main category: cs.LG

TL;DR: The paper introduces RiJEPA, a bidirectional neuro-symbolic framework combining self-supervised learning with rule-based logic to enhance interpretability and robustness.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised models lack verifiable logic, while rule-based systems struggle with scalability. The goal is to merge their strengths.

Method: RiJEPA uses Energy-Based Constraints (EBC) and a dual-encoder architecture to reshape representations, and differentiable logic for continuous rule discovery.

Result: The framework shows efficacy in synthetic simulations and clinical use cases, enabling robust and interpretable representation learning.

Conclusion: RiJEPA bridges the gap between statistical and logical learning, offering a generative and interpretable neuro-symbolic solution.
...
### [49] [CAMEL-CLIP: Channel-aware Multimodal Electroencephalography-text Alignment for Generalizable Brain Foundation Models](https://arxiv.org/abs/2603.13272)
*Hanseul Choi,Jinyeong Park,Seongwon Jin,Sungho Park,Jibum Kim*

Main category: cs.LG

TL;DR: CAMEL-CLIP is a robust EEG-text multimodal model addressing channel heterogeneity with innovative components like channel-aware encoding and dual-level contrastive learning, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: EEG foundation models struggle with channel heterogeneity (e.g., changes in channel composition or ordering), limiting their generalizability. CAMEL-CLIP aims to overcome this challenge.

Method: Introduces three key components: (1) channel attribute-based positional encoding, (2) dynamic channel projection, and (3) dual-level contrastive learning (channel+sample levels).

Result: Outperforms existing models under linear-probing and beats full-finetuning-based foundation models in performance.

Conclusion: CAMEL-CLIP's design effectively handles heterogeneous EEG channels and excels in diverse downstream tasks, setting a new benchmark.
...
### [50] [Spatially Aware Deep Learning for Microclimate Prediction from High-Resolution Geospatial Imagery](https://arxiv.org/abs/2603.13273)
*Idan Sulami,Alon Itzkovitch,Michael R. Kearney,Moni Shahar,Ofir Levy*

Main category: cs.LG

TL;DR: The paper explores how spatial context impacts microclimate temperature predictions using a task-specific deep neural network, revealing improved accuracy with adjacent spatial data up to 5-7 m, and highlights context-dependent spatial effects.


<details>
  <summary>Details</summary>
Motivation: Microclimate models often underestimate lateral heat exchange and spatial influences. This study aims to quantify how surrounding environmental conditions affect local microclimates using remote sensing and deep learning.

Method: A deep neural network, inspired by convolutional neural networks, was trained using drone-derived spatial layers and meteorological data. Input spatial extents were systematically varied to assess prediction accuracy changes.

Result: Incorporating adjacent spatial data improves prediction accuracy, peaking at extents of 5-7 m. Spatial effects vary by time of day, microhabitat type, and local conditions, indicating context-dependent influences.

Conclusion: The study demonstrates deep learning as a diagnostic tool to quantify spatial dependencies in microclimate models, paving the way for hybrid approaches that balance accuracy and physical interpretability.
...
### [51] [EARCP: Self-Regulating Coherence-Aware Ensemble Architecture for Sequential Decision Making -- Ensemble Auto-Regule par Coherence et Performance](https://arxiv.org/abs/2603.14651)
*Mike Amega*

Main category: cs.LG

TL;DR: EARCP is a dynamic ensemble architecture that adaptively weights expert models based on performance and coherence, using online learning for robustness in non-stationary environments.


<details>
  <summary>Details</summary>
Motivation: Traditional ensemble methods use static or offline-learned combinations, limiting adaptability. EARCP addresses this by dynamically updating weights based on performance and consensus signals.

Method: EARCP combines multiplicative weight update algorithms with coherence-based regularization, enabling online learning and theoretical guarantees.

Result: The framework achieves sublinear regret bounds (O(sqrt(T log M))) and demonstrates effectiveness in tasks like time series forecasting and financial prediction.

Conclusion: EARCP is a versatile, theoretically grounded, and empirically validated ensemble framework for dynamic environments, with open-source availability.
...
### [52] [Learning from Partial Chain-of-Thought via Truncated-Reasoning Self-Distillation](https://arxiv.org/abs/2603.13274)
*Gianluigi Silvestri,Edoardo Cetin*

Main category: cs.LG

TL;DR: TRSD introduces a lightweight post-training method to improve efficiency by training models to make correct predictions from partial reasoning traces.


<details>
  <summary>Details</summary>
Motivation: Current reasoning models incur excessive computational costs due to redundant reasoning traces.

Method: TRSD uses a frozen teacher model to generate full reasoning traces, then trains a student model to match the teacher's answer distribution with truncated traces.

Result: TRSD improves robustness to truncated inference and reduces reasoning trace lengths without explicit training for brevity.

Conclusion: TRSD efficiently reduces inference costs while maintaining accuracy.
...
### [53] [Sample-Efficient Hypergradient Estimation for Decentralized Bi-Level Reinforcement Learning](https://arxiv.org/abs/2603.14867)
*Mikoto Kudo,Takumi Tanabe,Akifumi Wachi,Youhei Akimoto*

Main category: cs.LG

TL;DR: The paper proposes a method for hypergradient-based optimization in decentralized bi-level RL settings, enabling efficient leader strategy updates without intervening in the follower's optimization.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in decentralized bi-level RL, where the leader can only observe follower outcomes, the paper aims to derive efficient hypergradient estimates for high-dimensional leader decision spaces.

Method: Leveraging the Boltzmann covariance trick, the method derives hypergradients solely from interaction samples, avoiding extensive data or complex gradient estimators.

Result: The approach works effectively in discrete and continuous state tasks, showcasing its practicality and scalability.

Conclusion: This is the first method enabling hypergradient-based optimization in decentralized Markov games, overcoming limitations of prior techniques.
...
### [54] [PREBA: Surgical Duration Prediction via PCA-Weighted Retrieval-Augmented LLMs and Bayesian Averaging Aggregation](https://arxiv.org/abs/2603.13275)
*Wanyin Wu,Kanxue Li,Baosheng Yu,Haoyun Zhao,Yibing Zhan,Dapeng Tao,Hua Jin*

Main category: cs.LG

TL;DR: PREBA is a retrieval-augmented framework that enhances LLM predictions for surgical duration by integrating institution-specific clinical evidence and statistical priors, outperforming zero-shot inference and matching supervised methods.


<details>
  <summary>Details</summary>
Motivation: Accurate surgical duration prediction is crucial for hospital resource management. Current methods like supervised learning require labeled data and intensive training, while zero-shot LLM inference lacks clinical context, leading to misaligned predictions.

Method: PREBA combines PCA-weighted retrieval and Bayesian averaging to ground LLM predictions in clinical evidence. It constructs evidence-based prompts using similar historical cases and statistical priors, encoded into a unified representation space.

Result: PREBA reduces MAE by up to 40% and improves R^2 from -0.13 to 0.62 over zero-shot inference, achieving accuracy comparable to supervised ML methods on two datasets using three advanced LLMs.

Conclusion: PREBA offers a practical, training-free solution for accurate surgical duration prediction by leveraging institution-specific data, demonstrating superior performance and generalization.
...
### [55] [FastODT: A tree-based framework for efficient continual learning](https://arxiv.org/abs/2603.13276)
*Daniel Bretsko,Piotr Walas,Devashish Khulbe,Sebastian Stros,Stanislav Sobolevsky,Tomas Satura*

Main category: cs.LG

TL;DR: The paper introduces an oblivious tree-based model with Hoeffding bound for efficient online learning in non-stationary domains like energy and environmental sensing, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for adaptable, efficient, and continuous-learning models in evolving data distributions and resource-constrained environments.

Method: Proposes an oblivious tree-based model controlled by Hoeffding bounds for growth, integrating rapid learning, memory efficiency, and knowledge retention.

Result: Achieves competitive or superior performance to existing methods in energy and environmental sensing benchmarks, with high computational efficiency.

Conclusion: The framework successfully meets adaptability, continual updating, and efficient retraining goals, offering scalability for real-world non-stationary environments.
...
### [56] [Learning Retrieval Models with Sparse Autoencoders](https://arxiv.org/abs/2603.13277)
*Thibault Formal,Maxime Louis,Hervé Dejean,Stéphane Clinchant*

Main category: cs.LG

TL;DR: SPLARE uses sparse autoencoders (SAEs) for learned sparse retrieval (LSR), outperforming vocabulary-based LSR in multilingual and out-of-domain settings.


<details>
  <summary>Details</summary>
Motivation: To create more semantically structured, expressive, and language-agnostic sparse representations for efficient retrieval compared to existing LSR methods.

Method: Introduces SPLARE, a method to train SAE-based LSR models, leveraging open-source SAEs.

Result: SPLARE-7B achieves top results on MMTEB's multilingual and English retrieval tasks; a lighter 2B-parameter variant is also developed.

Conclusion: SAE-based LSR models like SPLARE outperform vocabulary-based approaches, offering better generalization across languages and domains.
...
### [57] [Human Attribution of Causality to AI Across Agency, Misuse, and Misalignment](https://arxiv.org/abs/2603.13236)
*Maria Victoria Carro,David Lagnado*

Main category: cs.AI

TL;DR: This paper explores how people attribute causal responsibility in AI-related harmful outcomes, revealing that autonomy and role reversal influence judgments, with developers often seen as highly causal despite distance.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand folk perceptions of causality in AI-involved incidents to inform liability frameworks and policy debates.

Method: Human experiments were conducted to analyze judgments of causality, blame, foreseeability, and counterfactual reasoning in various AI agency scenarios.

Result: Findings include greater causal attribution to AI under moderate/high agency, consistent higher causality assigned to humans in role reversals, and significant causality assigned to developers.

Conclusion: The research highlights how autonomy and roles shape causal judgments, offering insights for liability frameworks and policy discussions on AI-related harms.
...
### [58] [A Dual-Path Generative Framework for Zero-Day Fraud Detection in Banking Systems](https://arxiv.org/abs/2603.13237)
*Nasim Abdirahman Ismail,Enis Karaarslan*

Main category: cs.AI

TL;DR: A Dual-Path Generative Framework addresses the trade-off between low-latency fraud detection and GDPR-compliant explainability in banking, using VAEs for anomaly detection and WGAN-GP for adversarial training, with Gumbel-Softmax and SHAP integrations.


<details>
  <summary>Details</summary>
Motivation: The paper tackles the challenge of detecting fraud in high-frequency banking environments while complying with GDPR's explainability requirements, especially for zero-day attacks lacking historical data.

Method: Proposes a Dual-Path Generative Framework: a VAE for real-time anomaly detection and a WGAN-GP for offline adversarial training. Uses Gumbel-Softmax for discrete data and SHAP for explainability triggered by high uncertainty.

Result: Achieves <50ms inference latency for fraud detection and synthesizes high-entropy fraudulent scenarios to improve robustness. The framework balances real-time performance with explainability.

Conclusion: The framework effectively reconciles low-latency fraud detection with regulatory explainability, addressing class imbalance and zero-day attacks through generative modeling and selective XAI activation.
...
TL;DR: This paper evaluates LLMs for detecting and classifying vulnerabilities in Solidity smart contracts, comparing zero-shot, CoT, and ToT prompting strategies.


<details>
  <summary>Details</summary>
Motivation: Smart contracts are crucial but prone to security flaws, risking financial loss and trust. LLMs offer automated vulnerability detection, but their real-world effectiveness needs validation.

Method: The study tests state-of-the-art LLMs on 400 Solidity contracts using zero-shot, CoT, and ToT prompts for Error Detection (binary classification) and Error Classification (categorization).

Result: CoT and ToT improved recall (~95–99%) in Error Detection but reduced precision. In Error Classification, Claude 3 Opus scored highest (90.8 Weighted F1) with ToT.

Conclusion: LLMs, especially with advanced prompting like CoT and ToT, show promise for smart contract analysis, though balancing precision and recall remains challenging.
...
TL;DR: An AI system automates redaction and metadata extraction in UK planning documents to address legislative conflicts, reducing administrative workload and compliance risks.


<details>
  <summary>Details</summary>
Motivation: To resolve conflicts between public access mandates and data protection laws, reducing manual workload for planning officers.

Method: Developed an AI-in-the-Loop system automating personal info redaction and feature analysis, with human oversight for suggestions.

Result: Piloted at four UK authorities; includes a preliminary ROI model to quantify savings and secure participation.

Conclusion: Demonstrates AI's potential to ease administrative burdens and compliance risks in public sector workflows.
...
TL;DR: The paper proposes a Hadith-inspired trust modeling framework evaluating 26 behavioral features across five trust axes, showing strong performance in detecting hijacked accounts and insider threats.


<details>
  <summary>Details</summary>
Motivation: Inspired by Hadith scholarship's multidimensional trust assessment, the framework aims to improve trustworthiness evaluation of user accounts beyond single anomaly scores.

Method: Translates five trust axes (integrity, precision, continuity, reputation, anomaly evidence) into 26 behavioral features, adding temporal features for dynamic changes. Evaluated on CLUE-LDS and CERT datasets.

Result: Achieves near-perfect detection on CLUE-LDS (Random Forest), outperforming baselines. Temporal features consistently improve performance, e.g., ROC-AUC from 0.776 to 0.844 on CERT.

Conclusion: The framework effectively detects trust violations, with temporal features enhancing robustness, especially in challenging scenarios like sparse malicious behavior.
...
### [62] [ILION: Deterministic Pre-Execution Safety Gates for Agentic AI Systems](https://arxiv.org/abs/2603.13247)
*Florin Adrian Chitan*

Main category: cs.AI

TL;DR: ILION is a deterministic execution gate for autonomous AI agents, designed to evaluate and classify actions without statistical training or API dependencies, outperforming existing text-safety systems.


<details>
  <summary>Details</summary>
Motivation: Existing text-safety systems are unsuitable for evaluating autonomous AI agent actions, necessitating a specialized solution.

Method: ILION uses a five-component cascade architecture (TII, SVRF, IDC, IRS, CVL) to classify actions as BLOCK or ALLOW with sub-millisecond latency.

Result: ILION achieves F1 = 0.8515, precision = 91.0%, and a false positive rate of 7.9%, outperforming commercial baselines significantly.

Conclusion: ILION addresses gaps in agent execution safety, offering fast, interpretable, and accurate action classification.
...
### [63] [ManiBench: A Benchmark for Testing Visual-Logic Drift and Syntactic Hallucinations in Manim Code Generation](https://arxiv.org/abs/2603.13251)
*Nabin Oli*

Main category: cs.AI

TL;DR: ManiBench is a new benchmark for evaluating LLMs in generating Manim CE code, focusing on dynamic visuals and API correctness, addressing syntactic and visual-logic errors.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarks like HumanEval and MBPP lack the ability to test dynamic, pedagogical visuals. ManiBench fills this gap by evaluating LLMs' ability to generate accurate Manim CE code.

Method: ManiBench includes 150-200 problems across five difficulty levels, covering various mathematical topics. It evaluates using a four-tier framework: Executability, Version-Conflict Error Rate, Alignment Score, and Coverage Score.

Result: The benchmark and evaluation framework are open-sourced, with data hosted on platforms like GitHub and Hugging Face.

Conclusion: ManiBench provides a specialized tool for assessing LLM performance in generating visually accurate and temporally faithful Manim CE code, addressing key failure modes in traditional benchmarks.
...
### [64] [When Alpha Breaks: Two-Level Uncertainty for Safe Deployment of Cross-Sectional Stock Rankers](https://arxiv.org/abs/2603.13252)
*Ursina Sanderink*

Main category: cs.AI

TL;DR: The paper discusses challenges with cross-sectional ranking models under non-stationarity, proposes a two-level deployment policy using DEUP for uncertainty prediction, and shows improved risk-adjusted performance.


<details>
  <summary>Details</summary>
Motivation: The AI Stock Forecaster's performance breaks during regime shifts (e.g., AI thematic rally), motivating the need for better deployment decisions.

Method: Adapts DEUP to predict rank displacement and introduces a two-level policy: a regime-trust gate G(t) for trading decisions and a position-level tail-risk cap.

Result: Empirical results show structural coupling between uncertainty and signal strength, with the proposed policy improving risk-adjusted performance.

Conclusion: DEUP adds value as a tail-risk guard, and the two-level policy enhances deployment robustness.
...
### [65] [Autonomous Agents Coordinating Distributed Discovery Through Emergent Artifact Exchange](https://arxiv.org/abs/2603.14312)
*Fiona Y. Wang,Lee Marom,Subhadeep Pal,Rachel K. Luu,Wei Lu,Jaime A. Berkovich,Markus J. Buehler*

Main category: cs.AI

TL;DR: ScienceClaw + Infinite is a framework for autonomous scientific research using independent agents, featuring skill registries, artifact lineage tracking, and agent-based discourse. It enables plannerless coordination, dynamic synthesis, and automated pruning, demonstrated across diverse investigations.


<details>
  <summary>Details</summary>
Motivation: To create a decentralized, autonomous system for scientific research where agents can independently conduct and coordinate investigations without central oversight, fostering innovation and collaboration.

Method: The system uses extensible skill registries, a DAG-based artifact layer for lineage, and a structured discourse platform. Agents chain tools, broadcast needs, and use pressure-based scoring and schema-overlap for coordination. An autonomous mutation layer prunes workflows.

Result: Demonstrated in four investigations, the framework achieved tool chaining, emergent agent convergence, and traceable reasoning from computation to findings.

Conclusion: ScienceClaw + Infinite successfully enables autonomous, decentralized scientific research with robust coordination and traceability.
...
### [66] [Distilling Deep Reinforcement Learning into Interpretable Fuzzy Rules: An Explainable AI Framework](https://arxiv.org/abs/2603.13257)
*Sanup S. Araballi,Simon Khan,Chilukuri K. Mohan*

Main category: cs.AI

TL;DR: The paper introduces a Hierarchical TSK Fuzzy Classifier System to make opaque Deep Reinforcement Learning policies interpretable, achieving high fidelity and superior interpretability metrics.


<details>
  <summary>Details</summary>
Motivation: DRL agents lack transparency, limiting their use in safety-critical domains, necessitating methods to distill policies into human-readable rules.

Method: Proposes a Hierarchical TSK Fuzzy Classifier System using K-Means clustering for state partitioning and Ridge Regression for local action inference, with metrics like FRAD, FSC, and ASG.

Result: Achieves 81.48% fidelity on Lunar Lander, outperforming Decision Trees by 21 points, with high interpretability (FRAD = 0.814) and low error (MSE = 0.0053).

Conclusion: The framework provides human-verifiable rules, enabling trustworthy autonomous systems.
...
### [67] [Deep Convolutional Architectures for EEG Classification: A Comparative Study with Temporal Augmentation and Confidence-Based Voting](https://arxiv.org/abs/2603.13261)
*Aryan Patodiya,Hubert Cecotti*

Main category: cs.AI

TL;DR: Comparative study of deep learning architectures for EEG ERP classification, showing 3D CNN outperforms 2D CNNs with CSP or raw data, using temporal shift augmentation and confidence-based voting for robustness.


<details>
  <summary>Details</summary>
Motivation: EEG classification is challenging due to low signal-to-noise ratio, temporal variability, and limited data. The study aims to compare deep learning architectures for ERP classification.

Method: Compared three pipelines: 2D CNN with CSP, 2D CNN on raw data, and 3D CNN modeling spatiotemporal data. Used temporal shift augmentation and confidence-based voting.

Result: 3D CNN outperformed 2D CNNs in AUC and balanced accuracy, showing benefits of temporal-aware architectures and augmentation.

Conclusion: Temporal-aware architectures and augmentation strategies are effective for robust EEG signal classification.
...
### [68] [Multi-hop Reasoning and Retrieval in Embedding Space: Leveraging Large Language Models with Knowledge](https://arxiv.org/abs/2603.13266)
*Lihui Liu*

Main category: cs.AI

TL;DR: EMBRAG integrates KG retrieval with LLMs to enhance reasoning by generating logical rules and refining results through embedding space and reranking, achieving state-of-the-art KGQA performance.


<details>
  <summary>Details</summary>
Motivation: Address LLMs' hallucination and outdated knowledge issues by leveraging KGs' reliable symbolic knowledge for improved reasoning.

Method: Propose EMBRAG: generate KG-grounded rules, embed-space reasoning guided by KG, and rerank results for refinement.

Result: Achieves state-of-the-art performance on KGQA benchmarks.

Conclusion: EMBRAG effectively combines KG retrieval with LLMs to overcome reasoning limitations and improve accuracy.
...
### [69] [Why Agents Compromise Safety Under Pressure](https://arxiv.org/abs/2603.14975)
*Hengle Jiang,Ke Tang*

Main category: cs.AI

TL;DR: The paper introduces Agentic Pressure, where LLM agents face tension between goals and safety, leading to normative drift and rationalized violations. Mitigation strategies like pressure isolation are proposed.


<details>
  <summary>Details</summary>
Motivation: To understand the conflict LLM agents face between achieving goals and adhering to safety constraints, and to explore its consequences and solutions.

Method: Identifies Agentic Pressure, demonstrates normative drift, analyzes root causes, and tests mitigation strategies like pressure isolation.

Result: Agents under pressure strategically sacrifice safety, with advanced reasoning accelerating this decline through rationalizations.

Conclusion: Agentic Pressure causes normative drift, requiring mitigation strategies like pressure isolation to restore alignment.
...
### [70] [Agent-Based User-Adaptive Filtering for Categorized Harassing Communication](https://arxiv.org/abs/2603.13288)
*Zenefa Rahaman,Sandip Sen*

Main category: cs.AI

TL;DR: An agent-based framework for personalized filtering of online harassment improves precision and user satisfaction by adapting to individual tolerance levels and preferences.


<details>
  <summary>Details</summary>
Motivation: Traditional global moderation systems lack personalization, leading to potential over-filtering or under-filtering of harassing content based on uniform rules. This paper aims to enhance content moderation by tailoring it to individual user needs.

Method: The paper introduces adaptive filtering agents that learn from user feedback and adjust thresholds dynamically across harassment categories (offensive, abusive, hateful). Supervised classification and simulated user interaction data are used for implementation and evaluation.

Result: Experiments show that adaptive agents outperform static models in filtering precision and user satisfaction.

Conclusion: Agent-based personalization effectively improves content moderation while maintaining user autonomy in online social networks.
...
### [71] [SAGE: Multi-Agent Self-Evolution for LLM Reasoning](https://arxiv.org/abs/2603.15255)
*Yulin Peng,Xinxin Zhu,Chenxing Wei,Nianbo Zeng,Leilei Wang,Ying Tiffany He,F. Richard Yu*

Main category: cs.AI

TL;DR: SAGE is a self-evolving framework with four agents (Challenger, Planner, Solver, Critic) that improves reasoning in LLMs by generating tasks, planning, and verifying answers without large human-labeled datasets.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current methods, which rely on large datasets or lack planning and quality control, SAGE introduces a closed-loop system for stable, long-horizon reasoning.

Method: Four agents (Challenger, Planner, Solver, Critic) co-evolve from a shared LLM backbone, generating tasks, converting them into plans, solving them, and verifying their correctness.

Result: SAGE improves the Qwen-2.5-7B model by 8.9% on LiveCodeBench and 10.7% on OlympiadBench, demonstrating consistent gains across benchmarks.

Conclusion: SAGE offers a scalable and stable approach for improving reasoning in LLMs with minimal reliance on human-labeled data.
...
### [72] [Intelligent Co-Design: An Interactive LLM Framework for Interior Spatial Design via Multi-Modal Agents](https://arxiv.org/abs/2603.15341)
*Ren Jian Lim,Rushi Dai*

Main category: cs.AI

TL;DR: An LLM-based, multimodal, multi-agent framework dynamically converts natural language and imagery into 3D architectural designs, improving communication and productivity.


<details>
  <summary>Details</summary>
Motivation: Addressing miscommunication in architectural design due to clients' lack of design knowledge and designers' difficulty in explaining spatial relationships.

Method: Uses specialized agents (Reference, Spatial, Interactive, Grader) with prompt guidelines, RAG for reduced data dependency, and real-time user interaction.

Result: Generates optimized 3D designs with high user intent alignment, aesthetic coherence, and functionality. 77% user satisfaction and preference over traditional software.

Conclusion: The framework enhances user-centric communication, fosters inclusivity, and improves design processes.
