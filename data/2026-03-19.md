...
TL;DR: MARL-Rad is a multi-modal multi-agent reinforcement learning framework for radiology report generation, outperforming single-model approaches in clinical efficacy metrics.


<details>
  <summary>Details</summary>
Motivation: To address limitations of single-model reinforcement learning and post-hoc agentization by jointly training multiple agents for more accurate radiology reports.

Method: Proposes MARL-Rad, coordinating region-specific and global integrating agents, optimized via clinically verifiable rewards.

Result: Achieves state-of-the-art performance in clinical efficacy metrics (RadGraph, CheXbert, GREEN) on MIMIC-CXR and IU X-ray datasets, with improved laterality consistency and report accuracy.

Conclusion: MARL-Rad demonstrates superior performance in radiology report generation, enhancing clinical efficacy and detail accuracy.
...
TL;DR: The paper explores sub-word tokenization and concatenation-based data augmentation to address uneven character distributions and inter-writer variability in online handwriting recognition, showing distinct benefits for inter- vs. intra-writer challenges.


<details>
  <summary>Details</summary>
Motivation: To improve inertial measurement unit-based online handwriting recognition by addressing uneven character distributions and inter-writer variability.

Method: Systematically investigates sub-word tokenization (Bigram tokenization) and concatenation-based data augmentation on the OnHW-Words500 dataset.

Result: Bigram tokenization reduces WER from 15.40% to 12.99% for inter-writer variance, while concatenation-based augmentation reduces character error rate by 34.5% and WER by 25.4% for intra-writer sparsity.

Conclusion: Sub-word tokenization mitigates inter-writer variability, while concatenation-based augmentation compensates for intra-writer sparsity, revealing a variance-dependent effect.
...
TL;DR: The paper proposes a communication-efficient framework (BHU) for multi-UAV cooperative perception, reducing overhead by 85% while improving performance by 5%.


<details>
  <summary>Details</summary>
Motivation: Challenges in communication latency and resource efficiency due to massive visual data in multi-UAV systems.

Method: Uses Top-K pixel selection, MU-MIMO transmission, Swin-large-based MaskDINO for BEV features, and a diffusion-based DRL algorithm for joint optimization.

Result: Improves perception performance by over 5% and reduces communication overhead by 85% on the Air-Co-Pred dataset.

Conclusion: BHU offers an effective solution for resource-constrained multi-UAV cooperative perception.
...
TL;DR: The paper presents a new method combining Transfer Learning and Broad Learning System (BLS) for Facial Beauty Prediction (FBP), improving accuracy over existing methods.


<details>
  <summary>Details</summary>
Motivation: FBP is challenging due to data scarcity and facial variability. Transfer Learning reduces data dependency, while BLS enables fast model building.

Method: Uses CNNs (EfficientNets) for feature extraction, integrates with BLS (E-BLS), and adds a connection layer (ER-BLS) to enhance performance.

Result: E-BLS and ER-BLS outperform previous BLS and CNNs methods in FBP accuracy.

Conclusion: The proposed method is effective and superior for FBP, with potential applications in pattern recognition, object detection, and image classification.
...
TL;DR: This study introduces Script-to-Slide Grounding (S2SG) to automate grounding script sentences to slide objects, achieving high performance with an F1-score of 0.924 using a large language model (LLM).


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce the labor-intensive process of editing slide-based videos by automating the grounding of spoken content to slide objects.

Method: The method involves formalizing the S2SG task and proposing 'Text-S2SG,' which uses an LLM to ground script sentences to text objects in slides.

Result: The proposed method achieves high performance with an F1-score of 0.924.

Conclusion: The work formalizes an implicit video editing process into a computable task, enabling future automation of slide-based video generation.
...
TL;DR: AwaRes introduces a spatial-on-demand framework for vision-language models to balance accuracy and efficiency by using low-resolution global views and selectively retrieving high-resolution segments when needed.


<details>
  <summary>Details</summary>
Motivation: The trade-off between accuracy (high-resolution inputs) and computational efficiency (low-resolution inputs) in vision-language models is addressed by dynamically retrieving only necessary high-resolution segments.

Method: AwaRes uses a low-resolution global view and tool-calling to retrieve high-resolution segments. It automates supervised data creation with a judge and oracle grounding model, then trains using cold-start SFT and multi-turn GRPO with a composite reward.

Result: The framework resolves the accuracy-efficiency trade-off by dynamically fetching high-resolution segments, improving performance without unnecessary computational costs.

Conclusion: AwaRes successfully balances accuracy and efficiency in vision-language models by selectively processing high-resolution content, validated through automated data creation and training strategies.
...
TL;DR: The paper introduces the V2VK pipeline to create the AgriMM benchmark, addressing the lack of large-scale agricultural datasets and domain expertise in MLLMs, and presents AgriChat, a specialized MLLM with superior performance.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs lack large-scale agricultural datasets and verified domain expertise, hindering their use in agriculture.

Method: Proposes the V2VK pipeline, integrating visual captioning and web-augmented scientific retrieval to generate the AgriMM benchmark, ensuring verifiable data.

Result: AgriMM contains 3k classes and 607k VQAs. AgriChat outperforms other models, showing robust agricultural AI is achievable.

Conclusion: Combining visual detail with verified knowledge leads to trustworthy agricultural AI; code and dataset are open-source.
...
TL;DR: GenLie is a Global-Enhanced Lie Detection Network that improves video-based lie detection by learning sparse, discriminative representations through local feature modeling under global supervision.


<details>
  <summary>Details</summary>
Motivation: Video-based lie detection faces challenges due to subtle and short-lived deceptive signals, which are often overwhelmed by redundant information and identity-related noise.

Method: GenLie captures sparse deceptive cues at the local level while using global supervision to suppress noise and enhance discriminative representations.

Result: Experiments on three public datasets show GenLie outperforms state-of-the-art methods in both high- and low-stakes scenarios.

Conclusion: GenLie effectively addresses the challenges of learning sparse yet discriminative representations for video-based lie detection, demonstrating superior performance.
...
TL;DR: This paper addresses the lag in text-guided facial animation by creating a large, balanced corpus of facial behavior using foundation generative models, enabling better training for bidirectional tasks of interpreting and synthesizing facial motion.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap in text-guided facial animation caused by the lack of well-annotated, text-paired facial corpora.

Method: The method involves synthesizing a large dataset of facial videos with prompts covering emotions and head motions, fitting 3D facial parameters, and training language models for bidirectional tasks (Motion2Language and Language2Motion).

Result: The results demonstrate that language models can effectively interpret and synthesize facial motion with strong generalization, establishing a unified approach for text-conditioned facial animation.

Conclusion: The conclusion highlights the novelty of framing facial-parameter modeling as a language problem and its potential for advancing text-conditioned facial animation and motion understanding.
...
TL;DR: A divergence-based multimodal fusion method for ambivalence/hesitancy recognition in videos, combining visual, audio, and textual features, achieving significant improvement over baseline.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of recognizing ambivalence/hesitancy in videos by leveraging cross-modal conflict between visual, audio, and textual data.

Method: Uses Py-Feat for visual AUs, Wav2Vec 2.0 for audio, and BERT for text, processed by BiLSTM with attention pooling and fused via pairwise absolute differences in a shared embedding space.

Result: Achieves a Macro F1 of 0.6808 on the BAH dataset, surpassing the baseline of 0.2827, with temporal variability of AUs identified as key discriminator.

Conclusion: The proposed fusion method effectively captures incongruence between modalities, significantly improving ambivalence/hesitancy recognition.
...
TL;DR: KGS-GCN enhances skeleton-based action recognition by addressing sensor data sparsity and topological rigidity via kinematics-driven Gaussian splatting and probabilistic topology.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of sparse skeleton data and rigid sensor topologies in action recognition for better spatiotemporal detail and long-range dependency modeling.

Method: Proposes KGS-GCN with kinematics-driven Gaussian splatting for continuous heatmaps and probabilistic topology for adaptive adjacency matrices.

Result: Significant improvement in modeling complex spatiotemporal dynamics, offering robust processing for low-fidelity sensor data.

Conclusion: KGS-GCN provides a practical solution to enhance perceptual reliability in real-world sensing applications by improving sparse input handling.
...
TL;DR: Omni IIE Bench is introduced to diagnose editing consistency of IIE models across varying semantic scales, revealing a performance gap in existing models.


<details>
  <summary>Details</summary>
Motivation: Addressing the inconsistent performance of IIE models across tasks of varying semantic scales, particularly in professional applications.

Method: Introduces Omni IIE Bench with a dual-track diagnostic design (Single-turn Consistency and Multi-turn Coordination) and rigorous human annotation.

Result: Evaluation of 8 IIE models shows a significant performance degradation in high-semantic-scale tasks compared to low-semantic-scale tasks.

Conclusion: Omni IIE Bench provides essential tools and insights for developing more reliable and stable IIE models.
...
TL;DR: EmergeNav, a zero-shot framework, improves VLN-CE performance by organizing execution structure without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Modern VLMs struggle with stable long-horizon execution in zero-shot VLN-CE due to lack of structured reasoning.

Method: EmergeNav uses a Plan--Solve--Transition hierarchy, GIPE, contrastive dual-memory reasoning, and Dual-FOV sensing.

Result: Achieves 30.00 SR (Qwen3-VL-8B) and 37.00 SR (Qwen3-VL-32B) without task-specific training or explicit maps.

Conclusion: Explicit execution structure is crucial for transforming VLM priors into stable embodied navigation.
...
TL;DR: The paper proposes PhysQuantAgent, a framework using Vision-Language Models (VLMs) for mass estimation of real-world objects, and introduces VisPhysQuant, a benchmark dataset for evaluation. Visual prompting methods improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack reliable mass reasoning capabilities, which are crucial for robotic manipulation tasks like grasp force determination.

Method: The framework involves three visual prompting methods (object detection, scale estimation, cross-sectional image generation) to enhance object understanding and mass estimation using VLMs.

Result: Visual prompting significantly improves mass estimation accuracy on real-world data.

Conclusion: Integrating spatial reasoning with VLM knowledge enhances physical inference, demonstrating the potential for improved robotic perception.
...
TL;DR: The paper proposes a standard method for extracting and clustering traffic scenarios from real-world data to improve ADS validation.


<details>
  <summary>Details</summary>
Motivation: The need for reliable and standardized methods to extract and group traffic scenarios from real-world data for ADS testing, addressing the challenges of comparability and interpretability.

Method: Standardized scenario extraction based on the Scenario-as-Specification concept and domain-knowledge-guided clustering, tested on the highD dataset.

Result: Reliable scenario extraction and effective integration of domain knowledge into clustering, enabling efficient ADS validation.

Conclusion: The methodology enhances standardization and efficiency in deriving scenario categories from highway data for ADS validation.
...
TL;DR: The paper introduces CineSRD, a multimodal framework for open-world speaker diarization in visual media, addressing challenges like long-form video understanding and cross-modal asynchrony.


<details>
  <summary>Details</summary>
Motivation: To extend speaker diarization to complex, open-world scenarios like films and TV series, overcoming limitations of traditional systems that work only in constrained settings.

Method: Proposes CineSRD, integrating visual, acoustic, and linguistic cues from video, speech, and subtitles. It uses visual anchor clustering and an audio language model for speaker registration and diarization.

Result: CineSRD performs well on a newly constructed benchmark for visual media and shows competitive results on conventional datasets.

Conclusion: CineSRD is robust and generalizable for open-world visual media, offering a unified solution for speaker diarization in complex scenarios.
...
TL;DR: MSRAMIE is a training-free framework for handling complex multi-instruction image editing tasks by leveraging structured multimodal reasoning, improving performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitation of existing image editing models in handling complex, multi-instruction scenarios due to scarce training data and high retraining costs.

Method: The proposed MSRAMIE framework uses an MLLM-based Instructor and image editing Actor, employing Tree-of-States and Graph-of-References for structured reasoning and iterative interactions.

Result: Experiments show a 15% improvement in instruction following and over 100% increase in completing modifications in a single run, while maintaining visual quality.

Conclusion: MSRAMIE effectively tackles multi-instruction image editing without retraining, offering interpretable and flexible refinement.
...
TL;DR: MAND is a modality-aware framework for multimodal egocentric open-world continual learning, improving novelty detection and classification by better exploiting complementary modality cues.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multimodal egocentric activity recognition underutilize cues from non-RGB modalities (like IMU) and suffer from catastrophic forgetting, limiting their robustness in open-world environments.

Method: MAND introduces Modality-aware Adaptive Scoring (MoAS) for adaptive integration of modality logits during inference and Modality-wise Representation Stabilization Training (MoRST) to preserve modality-specific discriminability during training.

Result: MAND achieves a 10% improvement in novel activity detection AUC and a 2.8% increase in known-class classification accuracy over state-of-the-art baselines.

Conclusion: MAND effectively addresses the imbalance in modality utilization and catastrophic forgetting, enhancing performance in multimodal egocentric open-world continual learning.
...
TL;DR: The paper introduces 'Beyond Images,' a pipeline for enriching Multi-Modal Knowledge Graphs (MMKGs) by retrieving and converting ambiguous images into text, improving model performance without architecture changes.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of curating large-scale image collections and incorporating ambiguous visuals (e.g., logos, symbols) in MMKGs.

Method: A three-stage pipeline: (1) large-scale image retrieval, (2) textual conversion of visuals, (3) fusion of descriptions using an LLM for concise summaries.

Result: Consistent performance gains (up to 7% Hits@1) on MMKG datasets, with significant improvements (201.35% MRR, 333.33% Hits@1) for ambiguous visuals. A consistency check interface further enhances reliability.

Conclusion: Converting ambiguous images into text is a practical approach to enhance MMKG completion, offering scalable improvements without modifying existing model architectures.
...
TL;DR: The paper analyzes efficiency bottlenecks in compact vision-language models (VLMs), proposes optimization techniques to reduce latency, and introduces ArgusVLM, a model family with structured perception outputs.


<details>
  <summary>Details</summary>
Motivation: Existing compact VLMs do not achieve the expected inference speedups despite smaller parameter counts. The study aims to identify bottlenecks and improve efficiency.

Method: Conducts an empirical efficiency analysis, profiles inference bottlenecks, and develops optimization recipes tailored to compact VLMs. Also introduces ArgusVLM with structured perception outputs.

Result: Optimizations reduce time to first token (TTFT) by 53% on InternVL3-2B and 93% on SmolVLM-256M. ArgusVLM performs well across benchmarks while remaining efficient.

Conclusion: The study provides practical guidance for building efficient VLM systems and demonstrates the viability of compact models like ArgusVLM for resource-constrained settings.
...
TL;DR: HopChain is a scalable framework for synthesizing multi-hop vision-language reasoning data, enhancing the training of Vision-Language Models (VLMs) and improving performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language data lacks complex reasoning chains, leaving VLMs' weaknesses in fine-grained reasoning unexposed. HopChain addresses this gap.

Method: HopChain synthesizes multi-hop queries with logically dependent chains of instance-grounded hops, ensuring verifiable rewards. It integrates this data into RLVR training for VLMs.

Result: Adding HopChain data improved 20 out of 24 benchmarks for Qwen3.5 models, with significant gains in long-CoT reasoning (up to 50+ points). Performance dropped with single/multi-hop variants.

Conclusion: HopChain effectively synthesizes multi-hop data, enhancing generalizable vision-language reasoning in VLMs.
...
TL;DR: OpenQlaw is an agentic system for analyzing 2D quantum materials, combining a core LLM with domain-expert MLLMs to enhance accessibility and utility for researchers.


<details>
  <summary>Details</summary>
Motivation: To address the cognitive overload and lack of immediate utility in existing MLLMs for 2D material analysis.

Method: Built on NanoBot and QuPAINT, OpenQlaw decouples visual identification from reasoning, enabling dynamic processing of user queries with persistent memory.

Result: The system accelerates high-throughput device fabrication by providing context-aware assistance and specialized computation.

Conclusion: OpenQlaw transforms isolated inferences into a practical tool for researchers, improving efficiency in quantum material discovery and device fabrication.
...
TL;DR: Astrolabe is an efficient online RL framework for distilled autoregressive video models, improving generation quality without the overhead of traditional RL methods.


<details>
  <summary>Details</summary>
Motivation: Existing RL frameworks are inefficient for distilled autoregressive models, requiring costly re-distillation or solver-coupled optimization, leading to misalignment with human preferences.

Method: Introduces a forward-process RL formulation with negative-aware fine-tuning, streaming training via rolling KV-cache for long videos, and a multi-reward objective to prevent reward hacking.

Result: The method consistently improves generation quality across multiple distilled AR video models, offering scalable alignment.

Conclusion: Astrolabe provides a robust and efficient solution for aligning distilled AR video models with human preferences, overcoming traditional RL bottlenecks.
...
TL;DR: PaAgent introduces a self-evolving portrait bank and RAG for efficient IR tool selection, outperforming existing methods in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing IR agents lack insight summarization, leading to inefficient tool searches. PaAgent aims to automate IR tasks more effectively.

Method: PaAgent uses a self-evolving portrait bank for IR tools and RAG for selection, enhanced by subjective-objective reinforcement learning.

Result: Extensive experiments on 8 benchmarks show PaAgent's superiority in handling complex IR tasks.

Conclusion: PaAgent improves IR automation by summarizing tool insights and accurately perceiving degradation, validated across diverse scenarios.
...
TL;DR: DesertFormer, a semantic segmentation pipeline based on SegFormer B2, addresses challenges in off-road desert terrain perception, achieving significant improvements over baseline models and enabling safety-aware path planning.


<details>
  <summary>Details</summary>
Motivation: Autonomous navigation in desert environments is hindered by low chromatic contrast, extreme lighting variability, and sparse vegetation, which standard models struggle with.

Method: Uses SegFormer B2 with a hierarchical Mix Transformer backbone to classify terrain into ten categories. Trained on 4,176 annotated images, with class-weighted training and copy-paste augmentation for rare categories.

Result: Achieves 64.4% mIoU and 86.1% pixel accuracy, a +24.2% improvement over DeepLabV3 MobileNetV2. Identifies key confusion patterns and proposes solutions.

Conclusion: DesertFormer provides a reliable solution for desert terrain segmentation, with open-source code and tools for further development.
...
TL;DR: The paper presents TrackDeform3D, an affordable RGB-D camera-based framework for autonomously collecting 3D datasets of deformable objects, improving tracking accuracy and creating a large-scale dataset.


<details>
  <summary>Details</summary>
Motivation: Current methods for extracting structured 3D representations of deformable objects face challenges like complex deformations and costly data collection. Lack of large-scale datasets limits progress in downstream tasks.

Method: Proposes TrackDeform3D, which uses RGB-D cameras to identify 3D keypoints and track their trajectories with motion consistency constraints for smooth, coherent data.

Result: TrackDeform3D outperforms state-of-the-art methods in geometric and tracking accuracy and contributes a dataset of 6 deformable objects with 110 minutes of trajectory data.

Conclusion: The framework enables scalable, autonomous collection of high-quality 3D datasets, addressing key bottlenecks in deformable object research.
...
TL;DR: ACE-LoRA is a parameter-efficient adaptation framework for generalist medical VLMs, combining LoRA modules with an ACE-HGNN module to enhance contextual interactions and fine-grained diagnostic cues, achieving superior performance across multiple domains.


<details>
  <summary>Details</summary>
Motivation: Existing medical vision-language models either specialize too narrowly, limiting generalization, or generalize too broadly, losing fine-grained diagnostic details. ACE-LoRA aims to bridge this gap.

Method: ACE-LoRA integrates Low-Rank Adaptation (LoRA) modules into frozen encoders and adds an ACE-HGNN module to capture higher-order contextual interactions. It also uses a label-guided InfoNCE loss for better cross-modal alignment.

Result: ACE-LoRA outperforms state-of-the-art medical VLMs and PEFT baselines in zero-shot classification, segmentation, and detection across multiple domains, despite adding only 0.95M trainable parameters.

Conclusion: ACE-LoRA successfully balances specialization and generalization in medical VLMs, offering improved performance while maintaining parameter efficiency.
...
TL;DR: The paper introduces Gaussian-Hermite Sampling (GHS) to address the lack of shift invariance in CNNs, achieving high classification consistency and accuracy without additional training.


<details>
  <summary>Details</summary>
Motivation: CNNs lack inherent shift invariance due to downsampling, which is crucial for efficiency and context. GHS aims to restore this property while preserving performance.

Method: GHS uses Gaussian-Hermite polynomials for shift-consistent sampling, enabling CNNs to maintain shift invariance without architecture changes or extra training.

Result: GHS achieves 100% classification consistency under spatial shifts and improves accuracy on CIFAR-10, CIFAR-100, and MNIST-rot datasets.

Conclusion: GHS effectively embeds shift invariance in CNNs, enhancing performance and consistency without modifying standard architectures.
...
TL;DR: FloodLlama, a fine-tuned vision-language model, estimates street-level flood depths from images with high accuracy (MAE <0.97 cm), utilizing synthetic data and TikTok-sourced inputs for real-time applications in EV and AV safety.


<details>
  <summary>Details</summary>
Motivation: Urban flooding disrupts transportation networks, but real-time, high-resolution flood depth data is lacking. This study aims to fill this gap for dynamic routing and vehicle safety.

Method: Uses a synthetic dataset (190K images) with varied conditions. Trains FloodLlama (fine-tuned LLaMA 3.2-11B Vision) via QLoRA and progressive curriculum learning. Evaluates with 34,797 trials.

Result: Achieves MAE <0.97 cm, Acc@5cm >93.7% for deep floods (>96.8% shallow). Selective fine-tuning reduces parameters by 76-80% with maintained accuracy. Validated on real-world TikTok data.

Conclusion: FloodLlama offers a scalable, infrastructure-free solution for real-time flood sensing, enhancing EV/AV safety and transportation resilience.
...
TL;DR: Proposes a pixel-level contrastive learning pipeline (DVD-CL, MVD-CL) with counterfactual generation and silver-standard labels, improving segmentation without large annotated datasets.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of expensive annotation for image segmentation by leveraging AI-generated labels and self-supervised learning.

Method: Combines counterfactual generation with dense contrastive learning (DVD-CL, MVD-CL) and introduces CHRO-map for visualization.

Result: Outperforms other dense contrastive methods, achieving ~94% DSC, even with silver-standard labels.

Conclusion: Proposed method enhances robustness to variations, proving effective for pixel-level tasks without heavy annotation.
...
TL;DR: The paper addresses correlated errors in Vision-Language Model (VLM) ensembles from the same architectural family, proposing family-aware methods like HFV, QualRCCV, and LCS to improve accuracy and outperform standard voting techniques.


<details>
  <summary>Details</summary>
Motivation: Ensembling VLMs from different providers aims to maximize benchmark accuracy, but correlated errors within the same architectural family reduce the effectiveness of standard voting methods, leading to a 'Misleading tier' where accuracy drops significantly despite correctness in the best model.

Method: Three family-aware methods are proposed: Hierarchical Family Voting (HFV) for aggregation within families before cross-family voting, QualRCCV for training-free weighting based on calibration and family quality, and Learned Candidate Scoring (LCS) for re-ranking answers using support breadth and model quality.

Result: The methods improve accuracy significantly: HFV recovers +18-26 pp in the Misleading tier, QualRCCV outperforms calibrated voting, and LCS achieves +0.68% on VQAv2, +0.61% on TextVQA, and +2.45% on GQA, with no degradation on any benchmark.

Conclusion: Family-aware methods effectively mitigate correlated errors in VLM ensembles, with LCS emerging as the most robust and generalizable approach, achieving state-of-the-art results on benchmarks like VQAv2.
...
TL;DR: MosaicMem improves video diffusion models by combining 3D patch lifting for localization with implicit conditioning for dynamic scenes, enhancing consistency and camera pose accuracy.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models face challenges in maintaining spatial consistency under camera motion and handling dynamic objects, highlighting the need for improved spatial memory.

Method: Proposes Mosaic Memory (MosaicMem), a hybrid spatial memory that uses 3D patch lifting for localization and targeted retrieval while leveraging native model conditioning for generation. It includes a patch-and-compose interface for view alignment and introduces PRoPE camera conditioning and memory alignment methods.

Result: Demonstrates better pose adherence than implicit memory and superior dynamic modeling compared to explicit baselines. Enables advanced features like minute-level navigation, scene editing, and autoregressive rollout.

Conclusion: MosaicMem addresses key limitations in spatial memory for video diffusion models, offering a balanced approach to consistency and dynamic scene handling.
...
TL;DR: BEV-SLD is a self-supervised LiDAR global localization method using BEV images to detect scene-specific landmarks, achieving robust performance across various environments.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve LiDAR global localization by leveraging scene-specific patterns rather than scene-agnostic methods, ensuring consistency and robustness.

Method: The approach uses BEV images to identify and treat scene-specific patterns as landmarks, with a consistency loss aligning global coordinates with per-frame heatmaps.

Result: BEV-SLD shows strong performance and robust localization in campus, industrial, and forest settings compared to state-of-the-art methods.

Conclusion: The method effectively enhances localization accuracy by focusing on scene-specific landmarks, demonstrating superiority over existing techniques.
...
TL;DR: GazeOnce360 is an end-to-end model for multi-person gaze estimation from a single upward-facing fisheye camera. It introduces a synthetic dataset (MPSGaze360) and uses rotational convolutions and eye landmark supervision to address fisheye distortions. A dual-resolution architecture improves gaze estimation accuracy.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the underexplored challenge of estimating 3D gaze directions of multiple people in a 360° scene using an upward-facing fisheye camera, moving beyond conventional forward-facing camera setups.

Method: The model incorporates rotational convolutions and eye landmark supervision to handle fisheye distortions. It also uses a dual-resolution architecture to fuse global context with high-resolution eye features for accurate gaze estimation.

Result: Experimental results show the effectiveness of each component in the model, demonstrating accurate multi-person gaze estimation in fisheye imagery.

Conclusion: The work highlights the feasibility and potential of fisheye-based 360° gaze estimation for practical multi-person scenarios.
...
TL;DR: General-purpose multimodal large language models (MLLMs) augmented with human expert knowledge can effectively perform iris presentation attack detection (PAD) under strict privacy constraints, outperforming specialized CNNs and human examiners.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenges of iris PAD, including the difficulty of collecting diverse attack data and privacy concerns associated with sharing biometric data, by leveraging adaptable MLLMs.

Method: The approach involves using pre-trained vision transformers in MLLMs to cluster iris attack types and applying structured prompts incorporating human expert knowledge to resolve ambiguities. Testing was conducted on a restricted dataset of 224 iris images using university-approved or locally-hosted models.

Result: Gemini 2.5 Pro with expert-informed prompts outperformed both specialized CNN-based baselines and human examiners, while the locally-deployable Llama 3.2-Vision achieved near-human performance.

Conclusion: MLLMs deployable under institutional privacy constraints provide a viable solution for iris PAD, balancing adaptability and privacy.
...
TL;DR: Patient4D is a new pipeline for dense 3D body mesh recovery from monocular video in surgical AR, leveraging stationarity priors and temporal consistency to handle occlusion and moving viewpoints.


<details>
  <summary>Details</summary>
Motivation: Existing HMR methods struggle with occlusion and moving cameras, especially in surgical AR where patients are stationary under draping. This limits accurate 3D reconstruction.

Method: Patient4D uses image-level foundation models and geometric mechanisms like Pose Locking (stabilizing pose with keyframes) and Rigid Fallback (handling occlusion via silhouette-guided alignment) to enforce consistency.

Result: Tested on 4,680 synthetic sequences and public benchmarks, Patient4D achieved 0.75 mean IoU under occlusion, reducing failure frames from 30.5% to 1.3% versus baselines.

Conclusion: Exploiting stationarity priors significantly improves monocular reconstruction in clinical AR, demonstrating Patient4D's robustness in challenging conditions.
...
TL;DR: A benchmark evaluates modern visual embedding models for instance-level image retrieval in industrial applications, comparing open-source, proprietary, and domain-specific models under diverse imaging conditions.


<details>
  <summary>Details</summary>
Motivation: Accurate product identification from images is crucial in industrial and commercial workflows to prevent costly errors. The study aims to assess the effectiveness of contemporary embedding models in real-world scenarios.

Method: The benchmark evaluates various embedding models (open-source, proprietary, and domain-specific) using curated industrial and public datasets under a unified image-to-image retrieval protocol, without post-processing.

Result: The findings highlight how well foundational and unified embedding models perform in fine-grained instance retrieval tasks compared to models trained specifically for industrial use.

Conclusion: The benchmark provides insights into the strengths and limitations of current visual embedding approaches, aiding practitioners and researchers in improving product identification systems.
...
TL;DR: SA-CycleGAN-2.5D is a domain adaptation framework addressing scanner-induced covariate shifts in neuroimaging, integrating architectural innovations for better harmonization.


<details>
  <summary>Details</summary>
Motivation: Scanner-induced covariate shifts in multi-site neuroimaging confound analysis, where acquisition variance often exceeds biological pathology variance, necessitating a robust harmonization method.

Method: Proposes SA-CycleGAN-2.5D with three innovations: 2.5D tri-planar manifold injection, a U-ResNet generator with global self-attention, and a spectrally-normalized discriminator for stable optimization.

Result: Reduces MMD by 99.1% and degrades domain classifier accuracy to near-chance (59.7%) on glioma patient data. Global attention is statistically essential for heterogeneous-to-homogeneous translation.

Conclusion: The framework effectively harmonizes neuroimaging data, preserving tumor pathophysiology and enabling reproducible multi-center radiomic analysis.
...
TL;DR: Proposes Efficient Gaussian Streaming (EGS), a reinforcement-learned anchor sampler for dynamic scene reconstruction, improving quality-efficiency trade-offs over fixed FPS sampling.


<details>
  <summary>Details</summary>
Motivation: Fixed anchor selection (e.g., FPS with 8,192 anchors) over-allocates computation under strict budgets, lacking adaptability to scene complexity. EGS aims to address this inefficiency.

Method: Replaces FPS with a reinforcement-learned policy for budget-aware anchor selection, balancing reconstruction quality and runtime using spatial features of Gaussian representation.

Result: EGS outperforms FPS in quality--efficiency trade-offs: in fast rendering at 256 anchors, it improves PSNR by +0.52--0.61 dB and runs 1.29--1.35× faster. In high-quality refinement, it matches full-anchor performance at lower budgets.

Conclusion: EGS is a plug-in solution that enhances dynamic scene reconstruction efficiency without altering the Gaussian streaming backbone, demonstrating superior adaptability and performance.
...
TL;DR: The paper explores how MLLMs handle spatial understanding in segmentation tasks, revealing that adapters reduce segmentation quality but LLM layers recover it through attention, with bidirectional attention improving early token processing.


<details>
  <summary>Details</summary>
Motivation: To understand the intrinsic spatial understanding capacity of MLLMs in pixel-level vision tasks, particularly segmentation.

Method: Uses layerwise linear probing, attention knockout analysis, and bidirectional attention evaluation across the MLLM pipeline.

Result: Adapters degrade segmentation representations, but LLM layers recover through attention-driven refinement. Bidirectional attention helps early token positions.

Conclusion: The study explains MLLMs' visual processing for segmentation, guiding future model design.
...
TL;DR: GigaWorld-Policy introduces an action-centered WAM that decouples action and video generation for faster, more efficient robot policy learning, improving speed and success rates.


<details>
  <summary>Details</summary>
Motivation: Addresses bottlenecks in existing WAMs: high inference overhead from joint reasoning over visual dynamics and actions, and entangled representations that degrade motion prediction accuracy.

Method: Formulates policy training into two coupled components: predicting future action sequences and generating future videos, with a causal design preventing future-video tokens from influencing action tokens.

Result: Achieves 9x faster inference than Motus and 7% higher task success rates, with a 95% performance improvement on RoboTwin 2.0 compared to pi-0.5.

Conclusion: GigaWorld-Policy offers a scalable, efficient solution for robot policy learning by decoupling action and video generation, validated by superior real-world performance.
...
TL;DR: LED is a benchmark for evaluating structural reasoning in Document Layout Analysis, addressing common errors with standardized definitions and tasks.


<details>
  <summary>Details</summary>
Motivation: Persistent structural errors in DLA (e.g., merging, splitting, omission) are overlooked by conventional metrics like IoU and mAP.

Method: LED defines eight error types, provides rules for simulation, constructs LED-Dataset, and designs three evaluation tasks.

Result: Experiments show LED offers fine-grained assessment, revealing weaknesses in multimodal models.

Conclusion: LED provides a unified, explainable benchmark to diagnose structural robustness and reasoning in document understanding models.
...
TL;DR: ConfusionBench is a new benchmark for recognizing and localizing student confusion in educational videos, built using a multi-stage filtering pipeline for higher quality, with evaluations comparing proprietary and open-source models.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for student confusion recognition suffer from noisy labels and coarse annotations, limiting reliable analysis. A better benchmark is needed.

Method: Proposes a multi-stage filtering pipeline with model-assisted screening, researcher curation, and expert validation to create ConfusionBench, a balanced dataset for confusion recognition and video localization.

Result: The proprietary model outperforms but over-predicts transitional segments, while the open-source model is conservative and misses detections. Visualization aids intervention decisions.

Conclusion: ConfusionBench offers a reliable benchmark for confusion understanding, with practical applications for educational experts. Datasets and materials will be publicly available.
...
TL;DR: Proposes DANCE, a dynamic pruning framework for 3D CNNs to improve power efficiency without performance loss, using AVA and AAP.


<details>
  <summary>Details</summary>
Motivation: CNNs lack dynamic adaptation to input complexity for energy efficiency. DANCE aims to minimize energy use with no performance impact.

Method: Uses Activation Variability Amplification (AVA) to increase neuron activation variance and Adaptive Activation Pruning (AAP) for dynamic pruning of frames, channels, and features.

Result: Achieves 1.37X-2.22X speedup on hardware, with 1.47X higher energy efficiency than state-of-the-art.

Conclusion: DANCE effectively enhances power efficiency in 3D CNNs through dynamic pruning.
...
TL;DR: A two-stage framework with Group-Shared Attention and Direct Preference Optimization improves story visualization by enhancing character identity and style consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with maintaining character identity and style consistency in story visualization, especially for complex narratives.

Method: Proposes Group-Shared Attention for cross-sample consistency and Direct Preference Optimization for aligning outputs with human standards.

Result: Achieves state-of-the-art performance with +10.0 in Character Identity (CIDS) and +18.7 in Style Consistency (CSD) on ViStoryBench.

Conclusion: The framework effectively addresses identity drift and visual inconsistency, setting a new benchmark in story visualization.
...
TL;DR: A multimodal 3D CNN is proposed for Alzheimer's disease classification using volumetric MRI, achieving 72.34% accuracy and highlighting the benefits of volumetric analysis over 2D slices.


<details>
  <summary>Details</summary>
Motivation: Clinical neuroimaging relies on 3D brain structures, so volumetric analysis may better capture spatial relationships relevant to disease progression.

Method: The model combines T1 MRI with gray matter, white matter, and CSF probability maps, evaluated via 5-fold cross-validation.

Result: Achieved 72.34% accuracy and 0.7781 ROC AUC, with GradCAM showing focus on anatomically relevant regions.

Conclusion: The framework provides a reproducible benchmark and underscores the advantages of volumetric MRI for AD classification.
...
TL;DR: Symphony, a multi-agent system, improves long-form video understanding (LVU) by decomposing tasks and enhancing reasoning with reflection, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current MLLM agents struggle with LVU due to high information density and extended temporal spans, requiring better decomposition and reasoning.

Method: Proposes Symphony, a multi-agent system that decomposes LVU into subtasks and uses a deep reasoning collaboration mechanism with reflection and VLM-based grounding.

Result: Achieves a 5.0% improvement over prior methods on LVBench and state-of-the-art performance on multiple benchmarks.

Conclusion: Symphony effectively addresses LVU challenges by emulating human cognition and improving reasoning and grounding capabilities.
...
TL;DR: The paper introduces $	ext{R}^2$VLM, a recurrent reasoning vision-language model for efficient and accurate long-horizon task progress estimation.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on video understanding in VLMs but overlook their reasoning potential and face computational challenges with long videos.

Method: Proposes $	ext{R}^2$VLM, which processes video snippets iteratively with a global Chain of Thought (CoT) to capture temporal dependencies.

Result: Demonstrates strong performance in progress estimation and downstream tasks, achieving state-of-the-art results.

Conclusion: $	ext{R}^2$VLM effectively balances computational efficiency and reasoning capabilities for long-horizon tasks.
...
TL;DR: The paper introduces QGN, a proposal-free Query-Guided Network unifying multimodal reasoning for GMNER, addressing limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing GMNER methods rely on pre-trained object detectors, which often miss fine-grained regions needed for named entities, leading to imprecision.

Method: Proposes QGN, a unified approach using text guidance and cross-modal interaction for accurate grounding without pre-detected proposals.

Result: QGN outperforms existing GMNER models on benchmarks, demonstrating robust performance in open-domain scenarios.

Conclusion: QGN effectively bridges the gap between textual entities and visual grounding, improving precision and performance.
...
TL;DR: The paper proposes MedSAD-CLIP, a supervised adaptation of CLIP for medical anomaly detection and segmentation, leveraging fine-grained text-visual cues and lightweight adapters to improve lesion localization and segmentation quality.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of coarse localization and limited segmentation quality in CLIP-based medical anomaly detection under realistic clinical settings with limited labeled abnormal data.

Method: Uses Token-Patch Cross-Attention (TPCA) for fine-grained text-visual cues, lightweight image adapters, learnable prompt tokens, and a Margin-based image-text Contrastive Loss to enhance global feature discrimination.

Result: Superior performance in pixel-level segmentation and image-level classification on four diverse benchmarks (Brain, Retina, Lung, and Breast datasets) compared to state-of-the-art methods.

Conclusion: Demonstrates the potential of supervised CLIP adaptation as a unified and scalable paradigm for medical anomaly understanding.
...
TL;DR: FineViT is a novel vision encoder designed to enhance fine-grained visual perception, outperforming existing models like SigLIP2 and Qwen-ViT in zero-shot tasks and long-context retrieval by using dense recaptions and LLM alignment.


<details>
  <summary>Details</summary>
Motivation: Current CLIP-based encoders in MLLMs lose visual details due to low-resolution pretraining and noisy web data, limiting their performance in dense spatial tasks.

Method: FineViT trains from scratch at high resolution on recaptioned image-text pairs, then enhances local perception via LLM alignment using the FineCap-450M dataset.

Result: FineViT achieves state-of-the-art zero-shot recognition and retrieval, excelling in long-context retrieval and outperforming competitors when integrated into MLLMs.

Conclusion: FineViT sets a new benchmark for fine-grained visual perception in MLLMs, addressing limitations of conventional encoders.
...
TL;DR: EvoGuard is a dynamic agentic framework for detecting AI-generated images, combining multiple detectors for better generalization without needing expensive annotations.


<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated images poses misinformation risks, requiring improved detection methods that generalize well and avoid costly training data.

Method: EvoGuard uses a capability-aware dynamic orchestration mechanism to coordinate various detectors, leveraging autonomous planning and reflection for multi-turn reasoning.

Result: Achieves state-of-the-art accuracy, reduces bias, and allows plug-and-play integration of new detectors without additional training.

Conclusion: EvoGuard offers a scalable, practical solution to evolving AI-generated image threats, eliminating reliance on fine-grained annotations.
...
TL;DR: OnlineHMR is a fully online framework for human mesh recovery, addressing the limitations of offline methods by ensuring causality, faithfulness, temporal consistency, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for human mesh recovery are often offline, relying on future frames or global optimization, which limits their use in interactive scenarios like AR/VR and telepresence.

Method: OnlineHMR uses a two-branch architecture with causal key-value cache and sliding-window learning, along with incremental SLAM for trajectory correction.

Result: The method matches offline approaches on the EMDB benchmark and dynamic videos while enabling online processing.

Conclusion: OnlineHMR successfully meets the criteria for online processing, making it suitable for interactive applications.
...
TL;DR: A new synthetic dataset is introduced to benchmark 3D reconstruction methods for asset inspection, highlighting challenges in dense capture trajectories and complex surface conditions.


<details>
  <summary>Details</summary>
Motivation: Existing 3D reconstruction datasets lack the conditions typical of asset inspections, such as dense image overlap and challenging visual appearances (e.g., reflections, transparency), making it difficult to evaluate and improve methods for this task.

Method: A synthetic dataset with ground truth depth maps, camera poses, and mesh models of three scenes is created, simulating inspection trajectories and varied surface conditions on non-Lambertian content. State-of-the-art reconstruction methods are evaluated on this dataset.

Result: Current 3D reconstruction methods struggle with dense capture trajectories and complex surface conditions, revealing limitations in scalability for practical asset inspection applications.

Conclusion: The dataset exposes a significant gap in deployable 3D reconstruction methods for asset inspection, suggesting a need for new research directions to address these challenges.
...
TL;DR: The paper introduces MCoT-MVS, a method for Composed Image Retrieval (CIR) that uses multi-modal chain-of-thought reasoning to guide visual feature selection, improving accuracy by reducing noise and aligning queries with target images.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods struggle with extracting relevant semantic cues from reference images under textual modifications, leading to interference from irrelevant visual noise.

Method: Leverages a multi-modal large language model (MLLM) for reasoning, generates textual cues, and uses these to guide visual attention selection. A weighted hierarchical combination module fuses multi-granular visual cues with modified text.

Result: Outperforms existing methods on CIRR and FashionIQ benchmarks, achieving state-of-the-art performance.

Conclusion: MCoT-MVS effectively addresses limitations in CIR by integrating reasoning and selective visual attention, demonstrating superior performance.
...
TL;DR: UniSAFE is introduced as the first comprehensive benchmark for evaluating system-level safety risks in Unified Multimodal Models across 7 I/O modality combinations, revealing critical vulnerabilities in current models.


<details>
  <summary>Details</summary>
Motivation: Existing safety benchmarks are fragmented across tasks and modalities, limiting the evaluation of complex system-level vulnerabilities in Unified Multimodal Models.

Method: UniSAFE uses a shared-target design to project common risk scenarios across task-specific I/O configurations, comprising 6,802 curated instances and evaluating 15 state-of-the-art models.

Result: Results show elevated safety violations in multi-image composition and multi-turn settings, with image-output tasks consistently more vulnerable.

Conclusion: The findings highlight the need for stronger system-level safety alignment for Unified Multimodal Models, with UniSAFE serving as a tool for comprehensive evaluation.
...
TL;DR: Material Magic Wand automates part grouping in untextured meshes by estimating material properties, reducing manual effort for artists.


<details>
  <summary>Details</summary>
Motivation: Manual identification of repeated structures in meshes is tedious; automation saves time and effort.

Method: Uses a part encoder to create material-aware embeddings and a supervised contrastive loss for grouping parts by material.

Result: Outperforms benchmarks on a dataset of 100 shapes with 241 part-level queries, proving effectiveness.

Conclusion: The tool successfully automates material-aware part grouping, enhancing efficiency in material assignment.
...
TL;DR: VLMs' safety alignment weakens with visual modality, leading to jailbreaks. Jailbreak behavior is due to a specific representational shift, quantified by jailbreak-related shift, which a proposed defense method (JRS-Rem) effectively mitigates.


<details>
  <summary>Details</summary>
Motivation: To understand why VLMs' safety alignment is compromised by visual inputs and identify a method to prevent jailbreaks.

Method: Analyzed VLM representations to distinguish harmful inputs, identified jailbreak-related shifts, and proposed JRS-Rem to remove these shifts at inference.

Result: Jailbreak-related shift reliably explains jailbreak behavior; JRS-Rem successfully defends against jailbreaks without affecting benign task performance.

Conclusion: Visual inputs shift VLM representations toward jailbreak states, but this can be mitigated by removing the jailbreak-related shift, enhancing safety.
...
TL;DR: InfoShot is a video frame sampler designed for long-video understanding, balancing coverage and critical events by selecting keyframes from semantically consistent shots. It improves anomaly detection and video-QA accuracy without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing frame samplers struggle to balance broad video coverage with brief but critical events, leading to unreliable predictions. InfoShot addresses this by focusing on shot-aware sampling.

Method: InfoShot partitions videos into shots, selects two complementary keyframes per shot (one for main content, one for unusual changes), and uses an information-theoretic objective.

Result: InfoShot enhances anomaly hit rates and Video-QA accuracy under frame constraints, performing well on standard benchmarks and SynFlash, a new synthetic benchmark.

Conclusion: InfoShot effectively balances coverage and critical events, improving long-video understanding without requiring retraining.
...
TL;DR: StereoWorld is a stereo world model that learns appearance and binocular geometry for stereo video generation, improving consistency and efficiency over monocular methods.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address limitations of monocular RGB or RGBD approaches by focusing on stereo video generation within the RGB modality, leveraging disparity for geometry.

Method: StereoWorld uses a unified camera-frame RoPE for consistent conditioning and a stereo-aware attention decomposition to reduce compute while capturing disparity-aligned correspondences.

Result: The model outperforms monocular-then-convert pipelines in stereo consistency, disparity accuracy, and speed, achieving 3x faster generation and 5% better viewpoint consistency.

Conclusion: StereoWorld enables end-to-end VR rendering, enhances depth grounding for embodied learning, and supports long-video distillation for interactive stereo synthesis.
...
TL;DR: VisionNVS reformulates Novel View Synthesis in autonomous driving as a self-supervised inpainting task, using monocular depth proxies and a Virtual-Shift strategy to eliminate supervision gaps, outperforming LiDAR-dependent methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the supervision gap in Novel View Synthesis for autonomous driving, where models lack ground truth images for novel trajectories during training. Existing approaches struggle with ill-posed extrapolation.

Method: VisionNVS introduces a Virtual-Shift strategy using monocular depth proxies to simulate occlusions and reformulates the problem as self-supervised inpainting. It also employs a Pseudo-3D Seam Synthesis strategy to ensure spatial consistency by integrating data from adjacent cameras.

Result: Experiments show VisionNVS achieves superior geometric fidelity and visual quality compared to LiDAR-dependent baselines, providing a scalable solution for driving simulation.

Conclusion: VisionNVS effectively bridges the supervision gap in NVS for autonomous driving, offering a robust, camera-only framework that outperforms traditional LiDAR-dependent methods.
...
### [63] [Loc3R-VLM: Language-based Localization and 3D Reasoning with Vision-Language Models](https://arxiv.org/abs/2603.18002)
*Kevin Qu,Haozhe Qi,Mihai Dusmanu,Mahdi Rad,Rui Wang,Marc Pollefeys*

Main category: cs.CV

TL;DR: Loc3R-VLM enhances 2D Vision-Language Models with 3D understanding from monocular video, using global layout reconstruction and explicit situation modeling, achieving top performance in 3D tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack robust spatial and viewpoint-aware reasoning, prompting the development of Loc3R-VLM to improve 3D understanding in vision-language models.

Method: The framework uses global layout reconstruction and explicit situation modeling, supported by camera pose priors from a pre-trained 3D model, to ground perception and language in 3D space.

Result: Loc3R-VLM outperforms existing 2D- and video-based methods in language-based localization and 3D question-answering benchmarks.

Conclusion: Loc3R-VLM demonstrates that direct spatial supervision effectively enhances 3D understanding in vision-language models.
...
TL;DR: The paper proposes a novel framework using vision-language models (VLMs) to improve material classification by addressing data scarcity with synthetic datasets and leveraging VLM priors for better performance.


<details>
  <summary>Details</summary>
Motivation: Traditional material classification struggles with limited annotated data, prompting the use of VLMs to enhance accuracy and generalizability.

Method: The method includes an image generation and auto-labeling pipeline for creating diverse datasets and a prior incorporation strategy with joint fine-tuning of vision foundation models and VLM-derived priors.

Result: Experiments show significant improvements, with synthetic datasets effectively capturing real-world material characteristics and VLM priors boosting performance.

Conclusion: The framework successfully overcomes data limitations and enhances material classification accuracy, with plans to release the source code and dataset.
...
TL;DR: A two-stage framework improves 3D hand pose estimation by leveraging gesture semantics as an inductive bias, outperforming state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of estimating 3D hand poses from monocular RGB images, which is crucial for AR/VR, human-computer interaction, and sign language understanding.

Method: The proposed framework includes gesture-aware pretraining using gesture labels from InterHand2.6M, followed by a per-joint token Transformer guided by gesture embeddings for MANO hand parameter regression. Training uses a layered objective.

Result: Gesture-aware pretraining enhances single-hand accuracy on InterHand2.6M, surpassing the EANet baseline, and the benefit transfers across architectures.

Conclusion: Gesture semantics serve as a powerful inductive bias, improving 3D pose estimation, with potential broader applicability.
...
TL;DR: A motion-adaptive temporal attention mechanism is introduced for efficient video generation using frozen Stable Diffusion models, adjusting attention based on motion content and adding minimal trainable parameters.


<details>
  <summary>Details</summary>
Motivation: To improve video generation efficiency by dynamically adapting temporal attention based on motion, ensuring detail preservation in high-motion sequences and scene consistency in low-motion ones.

Method: Lightweight temporal attention modules are injected into UNet blocks via a cascaded strategy: global attention for semantic stabilization and motion-adaptive attention for refinement. Temporally correlated noise and motion-aware gating are also used.

Result: The system adds only 25.8M parameters (2.9% of base UNet) and achieves competitive performance on WebVid, outperforming methods with explicit temporal losses.

Conclusion: The denoising objective alone provides sufficient temporal regularization, and noise correlation versus motion amplitude trade-off offers practical inference control for diverse outputs.
...
TL;DR: ASSR-EIC is a novel image compression framework that uses arbitrary-scale super-resolution (ASSR) for variable-rate extreme image compression, improving flexibility and performance across diverse bitrates.


<details>
  <summary>Details</summary>
Motivation: Existing methods either require separate models for each bitrate or struggle with severe information loss at ultra-low bitrates, limiting practical deployment.

Method: ASSR-EIC introduces an arbitrary-scale downsampling module at the encoder and a diffusion-based, joint degradation-aware ASSR decoder. It uses compression- and rescaling-aware diffusion priors, along with global and local adaptors, to guide reconstruction.

Result: The framework achieves state-of-the-art performance in extreme image compression, supporting flexible bitrate control and adaptive reconstruction.

Conclusion: ASSR-EIC offers a versatile and efficient solution for variable-rate extreme image compression, outperforming existing methods.
...
TL;DR: The paper proposes a mutually causal semantic distillation network (MSDN++) for zero-shot learning (ZSL) to improve semantic knowledge transfer between visual and attribute features.


<details>
  <summary>Details</summary>
Motivation: Prior ZSL methods use weak unidirectional attention, leading to limited and unreliable semantic representations. The goal is to develop a more effective way to infer intrinsic semantic knowledge for better performance.

Method: MSDN++ employs two mutual causal attention sub-nets (attribute→visual and visual→attribute) to learn causal associations between features, guided by semantic distillation loss for collaborative training.

Result: Experiments on benchmarks (CUB, SUN, AWA2, FLO) show MSDN++ outperforms strong baselines, achieving state-of-the-art results.

Conclusion: The proposed MSDN++ effectively distills intrinsic semantic representations, improving ZSL performance significantly.
...
TL;DR: The paper addresses the underperformance of existing methods in Referring Image Segmentation (RIS) for motion-related queries by introducing a data augmentation scheme and Multimodal Radial Contrastive Learning (MRaCL), achieving improved results.


<details>
  <summary>Details</summary>
Motivation: Existing RIS methods perform poorly on motion-related queries compared to appearance-based ones, highlighting the need for better handling of motion expressions in textual descriptions.

Method: 1) Efficient data augmentation to expose models to more motion expressions. 2) MRaCL, a contrastive learning approach on fused image-text embeddings to handle varied object descriptions.

Result: The method improves performance on motion-centric queries while maintaining competitive results for appearance-based descriptions, validated on the new M-Bench benchmark.

Conclusion: The proposed approach effectively addresses the gap in performance for motion-related queries in RIS, offering a robust solution with broad applicability.
...
TL;DR: Pixel-motion rewards and SHIFT framework improve motion fidelity in video diffusion models.


<details>
  <summary>Details</summary>
Motivation: Video diffusion models often suffer from weakened motion fidelity post-training, such as reduced dynamics or degraded temporal coherence.

Method: Introduces pixel-motion rewards based on pixel flux dynamics and proposes SHIFT, a hybrid fine-tuning framework combining supervised and advantage-weighted fine-tuning.

Result: SHIFT improves convergence speed, mitigates reward hacking, and resolves dynamic-degree collapse in video diffusion models.

Conclusion: The proposed approach efficiently enhances motion alignment and fidelity in video diffusion models.
...
TL;DR: The paper introduces ECHO, a new framework for Interactive Head Generation (IHG) that improves contextual appropriateness and emotional rationality in avatar facial behaviors by leveraging long-range contextual modeling and decoupling dual-track signals.


<details>
  <summary>Details</summary>
Motivation: Existing IHG methods rely on short-clip behavioral cues and entangled dual-track signals, leading to facial behaviors lacking contextual appropriateness and potential lip-sync issues. ECHO aims to address these challenges.

Method: ECHO includes a Long-range Contextual Understanding (LCU) component for contextual and affective modeling, and a Spatial-aware Decoupled Cross-attention Modulation (SDCM) module to separately handle lip articulation and non-lip facial behaviors, trained with a two-stage paradigm.

Result: Extensive experiments show ECHO's effectiveness in enhancing contextual appropriateness, emotional rationality, lip synchronization, and visual fidelity.

Conclusion: ECHO outperforms existing IHG methods by addressing key limitations through long-range contextual modeling and decoupled signal processing.
...
TL;DR: AdaZoom-GUI is an adaptive zoom-based framework for GUI grounding that improves accuracy by refining user instructions and selectively zooming in on small UI elements, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: GUI grounding is challenging due to high-resolution images, small UI elements, and ambiguous instructions. This work aims to enhance localization accuracy and instruction understanding for practical GUI agent deployment.

Method: AdaZoom-GUI uses an instruction refinement module to rewrite commands into explicit descriptions and a conditional zoom-in strategy for precise element localization. It is trained using Group Relative Policy Optimization (GRPO).

Result: The method achieves state-of-the-art performance on public benchmarks, demonstrating effectiveness for high-resolution GUI understanding.

Conclusion: AdaZoom-GUI effectively addresses GUI grounding challenges, making it practical for real-world GUI agent applications.
...
TL;DR: FACE-net enhances emotional video captioning by integrating factual calibration and emotion augmentation, addressing the factual-emotional bias in descriptions.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with insufficient mining and coordination of factual and emotional cues during generation, leading to a factual-emotional bias in descriptions.

Method: Proposes a retrieval-enhanced framework (FACE-net) with factual calibration via uncertainty estimation and progressive visual emotion augmentation, supported by a dynamic bias adjustment routing module.

Result: The framework collaboratively mines factual-emotional semantics, providing adaptive and accurate guidance for generation, breaking the compromising tendency of descriptions in sample learning.

Conclusion: FACE-net effectively addresses the factual-emotional bias in emotional video captioning by integrating factual and emotional cues through a unified architecture.
...
TL;DR: AR-CoPO is a new framework for aligning streaming autoregressive video generators using contrastive policy optimization, improving alignment and generalization over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods like GRPO struggle with alignment in few-step distillation and low-stochasticity trajectories, making them ineffective for streaming AR video generation.

Method: AR-CoPO introduces chunk-level alignment via a forking mechanism, assigns sequence-level rewards, and uses a semi-on-policy training strategy combining on-policy exploration and replay buffer exploitation.

Result: Experiments show AR-CoPO outperforms baselines in both out-of-domain generalization and in-domain human preference alignment.

Conclusion: AR-CoPO effectively addresses alignment challenges in streaming AR video generation, avoiding reward hacking and improving performance.
...
TL;DR: The paper introduces VirPro, a Visual-referred Probabilistic Prompt Learning method, to enhance weakly supervised monocular 3D object detection by leveraging adaptive multi-modal pretraining and scene-aware textual prompts.


<details>
  <summary>Details</summary>
Motivation: Hand-crafted textual descriptions fail to capture visual diversity across scenes, limiting scene-aware representation learning. The authors propose VirPro to address this gap by integrating adaptive multi-modal pretraining.

Method: VirPro generates instance-conditioned prompts stored in an Adaptive Prompt Bank (APB) and uses Multi-Gaussian Prompt Modeling (MGPM) to fuse visual features with textual embeddings. RoI-level contrastive matching aligns modalities for semantic coherence.

Result: Experiments on the KITTI benchmark show VirPro improves average precision by up to 4.8% over the baseline.

Conclusion: VirPro effectively enhances weakly supervised monocular 3D object detection by combining adaptive multi-modal pretraining with scene-aware textual prompts, demonstrating significant performance gains.
...
TL;DR: The paper introduces UAV-CB, a dataset for detecting UAVs in complex backgrounds, and LFBNet, a frequency-aware model for robust UAV detection.


<details>
  <summary>Details</summary>
Motivation: Detecting UAVs in low-altitude environments is challenging due to complex backgrounds and camouflage. Current datasets lack focus on these challenges, hindering real-world perception.

Method: The authors construct UAV-CB dataset for complex backgrounds and propose LFBNet, a model using localized frequency space to address RGB-T fusion gaps.

Result: LFBNet achieves state-of-the-art performance and robustness in camouflaged and cluttered conditions.

Conclusion: The study provides a frequency-aware approach to improve UAV detection in real-world scenarios.
...
TL;DR: Omni-I2C is a benchmark for evaluating Large Multimodal Models (LMMs) on converting structured digital graphics into executable code, highlighting challenges in visual perception and code generation.


<details>
  <summary>Details</summary>
Motivation: The task of converting digital graphics to code is a significant challenge for LMMs, requiring high-fidelity visual perception and precise code generation. Current models struggle with this due to minor errors leading to complete failure.

Method: Omni-I2C includes 1080 diverse samples across subjects, image modalities, and programming languages, paired with executable reference code. The evaluation framework assesses perceptual fidelity and symbolic precision.

Result: Evaluation shows a performance gap among leading LMMs, with even state-of-the-art models struggling in complex scenarios, indicating multimodal code generation remains a tough challenge.

Conclusion: Omni-I2C exposes granular failures in LMMs, demonstrating the need for improved models to handle structured digital graphics and code generation effectively.
...
TL;DR: The paper proposes an Early Intervention (EI) framework and Mixture of Low-varied-Ranks Adaptation (MoR) to address challenges in multimodal medical imaging, improving disease recognition by leveraging complementary data and adapting Vision Foundation Models efficiently.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of current methods in fully utilizing complementary multimodal data and the challenges in adapting Vision Foundation Models (VFMs) to medical images due to domain shifts and limited labeled data.

Method: The proposed Early Intervention (EI) framework uses high-level semantic tokens from reference modalities to guide target modality embedding. MoR, a parameter-efficient fine-tuning method, employs varied-rank low-rank adapters and a weight-relaxed router for VFM adaptation.

Result: Extensive experiments on retinal disease, skin lesion, and knee anomaly classification datasets demonstrate the method's effectiveness against competitive baselines.

Conclusion: The EI framework and MoR method jointly address multimodal fusion and VFM adaptation challenges, offering a promising solution for medical image-based disease recognition.
...
TL;DR: UniSem improves 3D reconstruction and semantic generalization with Error-aware Gaussian Dropout and Mix-training Curriculum.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges in sparse-view 3D Gaussian Splatting, such as unstable geometry and weak semantic lifting.

Method: Introduces Error-aware Gaussian Dropout (EGD) and Mix-training Curriculum (MTC) for better depth and semantics.

Result: Achieves 15.2% depth error reduction and 3.7% segmentation accuracy improvement.

Conclusion: UniSem outperforms baselines in depth and semantic tasks, demonstrating its effectiveness.
...
TL;DR: Proposes a parallel cost aggregation (PCA-Seg) paradigm with expert-driven perceptual learning (EPL) to improve vision-language alignment in semantic segmentation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from knowledge interference between class-level semantics and spatial context due to serial aggregation structures in vision-language models (VLMs).

Method: Introduces PCA-Seg with an EPL module (multi-expert parser and coefficient mapper) and feature orthogonalization decoupling (FOD) to capture diverse alignment cues.

Result: Achieves state-of-the-art performance on eight benchmarks with minimal additional parameters (0.35M per block).

Conclusion: PCA-Seg effectively addresses knowledge interference, enabling robust vision-language alignment for open-vocabulary semantic and part segmentation.
...
TL;DR: MM-OVSeg is a multimodal framework for open-vocabulary segmentation using optical-SAR fusion, resilient to adverse weather conditions.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current open-vocabulary segmentation methods, which struggle under cloudy or haze-contaminated conditions in remote sensing.

Method: MM-OVSeg combines optical imagery and SAR data, using a cross-modal unification process for representation alignment and a dual-encoder fusion module for text-aligned segmentation.

Result: The framework achieves superior robustness and generalization across diverse cloud conditions.

Conclusion: MM-OVSeg demonstrates the effectiveness of multimodal fusion for resilient open-vocabulary segmentation in adverse weather.
...
TL;DR: AdapTS is a unified Teacher-Student framework for multi-class and continual visual anomaly detection, optimized for edge deployment with minimal memory overhead.


<details>
  <summary>Details</summary>
Motivation: Existing VAD methods are limited to single-category scenarios and lack adaptability for multi-class and continual learning in real-world industrial settings.

Method: AdapTS uses a shared frozen backbone with lightweight trainable adapters in the student pathway. Training includes a segmentation-guided objective and synthetic Perlin noise, with a prototype-based task identification mechanism for dynamic adapter selection.

Result: AdapTS matches the performance of existing TS methods in multi-class and continual learning scenarios while significantly reducing memory usage (e.g., AdapTS-S requires only 8 MB).

Conclusion: AdapTS offers a scalable, efficient solution for edge deployment in complex industrial environments, outperforming existing methods in memory efficiency.
...
TL;DR: Rel-Zero is a novel zero-watermarking framework that uses editing-invariant patch relations for robust content authentication without altering the original image.


<details>
  <summary>Details</summary>
Motivation: Addressing the threat to digital content authenticity posed by diffusion-based image editing and the limitations of existing watermarking methods.

Method: Proposes Rel-Zero by leveraging the invariant relational distance between patch pairs during AI-based editing to derive a unique zero-watermark.

Result: Rel-Zero outperforms prior zero-watermarking approaches in robustness across diverse editing models and manipulations.

Conclusion: Rel-Zero offers a non-invasive and resilient solution for verifying the authenticity of digital visual content.
...
TL;DR: ECKConv is introduced as a method to achieve SE(3) equivariance in 3D point clouds, balancing symmetry and scalability using a coordinate-based kernel design.


<details>
  <summary>Details</summary>
Motivation: The need for a method that retains rigorous symmetry (SE(3)) and scalability in learning 3D point cloud problems, overcoming limitations of group convolution and previous intertwiner frameworks.

Method: Equivariant Coordinate-based Kernel Convolution (ECKConv), which uses a kernel domain in a double coset space and employs coordinate-based networks for explicit kernel design.

Result: ECKConv demonstrates rigid equivariance, memory efficiency, and superior performance in tasks like classification, pose registration, part segmentation, and large-scale semantic segmentation.

Conclusion: ECKConv successfully resolves the trade-off between symmetry and scalability, outperforming existing equivariant methods.
...
TL;DR: Video-SFT in MLLMs improves video performance but often harms static image benchmarks. A Hybrid-Frame strategy helps balance this trade-off.


<details>
  <summary>Details</summary>
Motivation: To understand how Video-SFT affects the balance between spatial and temporal understanding in MLLMs.

Method: Systematic study across architectures, parameter scales, and frame sampling settings; introduced an instruction-aware Hybrid-Frame strategy.

Result: Video-SFT boosts video performance but degrades static image performance; the Hybrid-Frame strategy partially mitigates this.

Conclusion: Video-SFT is not universally beneficial, and preserving spatial understanding remains a key challenge in joint training.
...
TL;DR: ProGVC is a progressive generative video compression framework that unifies progressive transmission, efficient entropy coding, and detail synthesis, offering scalable low-bitrate perceptual compression.


<details>
  <summary>Details</summary>
Motivation: Existing perceptual video codecs lack native support for variable bitrate and progressive delivery, and their generative modules are weakly coupled with entropy coding, limiting bitrate reduction.

Method: ProGVC encodes videos into hierarchical multi-scale residual token maps, enabling progressive transmission. A Transformer-based multi-scale autoregressive context model estimates token probabilities for efficient entropy coding and detail synthesis.

Result: ProGVC delivers promising perceptual compression performance at low bitrates while providing practical scalability.

Conclusion: The proposed ProGVC framework advances perceptual video compression by integrating progressive transmission, efficient entropy coding, and detail synthesis into a unified system.
...
TL;DR: The paper introduces PF-RPN, a prompt-free method for object detection using learnable embeddings and cascading modules, validated on 19 datasets.


<details>
  <summary>Details</summary>
Motivation: Current object detection methods rely on external prompts, limiting flexibility. The goal is to develop a prompt-free approach adaptable to real-world scenarios.

Method: PF-RPN uses SIA for initial localization, CSP for self-prompted feature aggregation, and CG-QS for query selection. It works with minimal data and no fine-tuning.

Result: The method performs well across diverse domains (e.g., underwater, industrial, remote sensing) and is validated on 19 datasets.

Conclusion: PF-RPN offers a flexible, prompt-free solution for object detection, demonstrating broad applicability and efficiency.
...
TL;DR: FrescoDiffusion is a training-free method for generating high-resolution (e.g., 4K) coherent videos from complex images by augmenting tiled denoising with a latent prior, improving global consistency and fine detail retention.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based I2V models struggle with ultra-high-resolution inputs, losing fine-grained structure or breaking global consistency, especially in complex scenes like fresco animations.

Method: FrescoDiffusion uses a precomputed latent prior from a low-resolution video to guide high-resolution tiled denoising, minimizing a weighted least-squares objective for fusion and enabling region-level motion control.

Result: The method improves global consistency and fidelity over tiled baselines on the VBench-I2V and fresco I2V datasets, offering computational efficiency and explicit controllability.

Conclusion: FrescoDiffusion effectively addresses the challenge of high-resolution I2V generation with enhanced coherence and detail, providing a controllable trade-off between creativity and consistency.
...
TL;DR: The paper proposes a feature-preserving face anonymization framework that improves expression fidelity and photometric consistency while maintaining privacy.


<details>
  <summary>Details</summary>
Motivation: Address privacy concerns in face image sharing by developing anonymization methods that retain critical facial features like expressions, lighting, and skin tone for downstream tasks.

Method: Extends DeepPrivacy with dense facial landmarks for expression retention and adds post-processing modules for lighting and skin color consistency. Introduces new evaluation metrics for these features.

Result: Outperforms state-of-the-art baselines on the CelebA-HQ dataset, achieving higher realism and fidelity in expressions, illumination, and skin tone.

Conclusion: Highlights the importance of feature-aware anonymization for creating more useful, fair, and trustworthy privacy-preserving facial data.
...
TL;DR: PanoVGGT is a Transformer framework for joint pose estimation and 3D reconstruction from panoramas, using spherical-aware embeddings and SO(3) rotation augmentation.


<details>
  <summary>Details</summary>
Motivation: Existing models for perspective cameras fail to handle non-pinhole distortions in panoramas, necessitating a specialized solution.

Method: PanoVGGT uses permutation-equivariant Transformers with spherical-aware positional embeddings and SO(3) rotation augmentation, plus a stochastic anchoring strategy.

Result: The model achieves competitive accuracy, robustness, and cross-domain generalization on PanoCity and benchmarks.

Conclusion: PanoVGGT effectively addresses challenges in panoramic imagery, with promising results and a new dataset contribution.
...
TL;DR: The paper introduces LoGSAM, a parameter-efficient framework for brain tumor localization and segmentation in MRI using text prompts derived from radiologist dictations, achieving strong performance with minimal parameter updates.


<details>
  <summary>Details</summary>
Motivation: Precise tumor localization and segmentation are critical for therapy planning but are hindered by limited annotated data and task-specific models. LoGSAM addresses this by leveraging pretrained models and radiologist speech.

Method: Radiologist dictations are transcribed and processed into textual prompts using NLP. These prompts guide tumor localization via a LoRA-adapted vision-language model (Grounding DINO). Predicted boxes then prompt MedSAM for segmentation without fine-tuning.

Result: LoGSAM achieved an 80.32% dice score on BRISC 2025 and 91.7% case-level accuracy on German dictations for unseen MRI scans.

Conclusion: The framework demonstrates the feasibility of a speech-to-segmentation pipeline using pretrained models with minimal updates, offering a modular and efficient solution for medical imaging tasks.
...
TL;DR: Edit-As-Act is a framework for open-vocabulary 3D indoor scene editing that treats edits as goal-regressive planning, ensuring minimal changes while preserving scene coherence and physical plausibility.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene editing from natural language often disrupt spatial structure or make unintended global changes due to treating edits as generative tasks. Edit-As-Act addresses this by focusing on minimal, goal-directed actions.

Method: Edit-As-Act predicts symbolic goal predicates and plans using EditLang, a custom action language with explicit preconditions and effects. A language-driven planner proposes actions, validated for goal-directedness, monotonicity, and physical feasibility.

Result: Edit-As-Act outperforms prior approaches on E2A-Bench, a benchmark of 63 editing tasks across 9 indoor environments, achieving better instruction fidelity, semantic consistency, and physical plausibility.

Conclusion: By decoupling reasoning from low-level generation, Edit-As-Act provides a scalable and interpretable solution for 3D scene editing, satisfying criteria that existing paradigms fail to meet jointly.
...
TL;DR: The paper proposes DUCS, a coreset selection strategy for medical imaging data that focuses on unreliable samples to improve decision boundary modeling.


<details>
  <summary>Details</summary>
Motivation: Managing large-scale medical imaging datasets with limited resources is challenging. Coreset selection is limited due to data complexity.

Method: DUCS uses inward self-awareness (confidence evolution analysis) and backward memory tracking (forgetting frequency) to select unreliable samples near decision boundaries.

Result: DUCS outperforms SOTA methods, especially at high compression rates, on public medical datasets.

Conclusion: Focusing on unreliable samples improves decision boundary modeling and efficiency in medical data coreset selection.
...
TL;DR: A novel framework for unified 3D perception and reasoning using hierarchical language-distilled Gaussian scenes and 3D semantic scene graphs, enabling open-vocabulary tasks without scene-specific training.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of achieving unified 3D perception and reasoning across tasks like segmentation, retrieval, and relation understanding, as existing methods are either object-centric or require costly training for inter-object reasoning.

Method: Constructs a hierarchical language-distilled Gaussian scene and its 3D semantic scene graph using a Gaussian pruning mechanism and multi-view language alignment. Employs Vision Language annotations and Graph Neural Networks for relational reasoning.

Result: The approach enables efficient and scalable open-vocabulary 3D reasoning by jointly modeling hierarchical semantics and object relationships, validated in tasks like segmentation, scene graph generation, and retrieval.

Conclusion: The framework successfully achieves unified 3D perception and reasoning without scene-specific training, demonstrating versatility across multiple open-vocabulary tasks.
...
TL;DR: S-VGGT introduces a novel method to reduce computational costs in 3D foundation models by addressing structural redundancy at the frame level, enabling efficient scene partitioning and parallel processing.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the quadratic computational cost of global attention in feed-forward 3D models, which limits scalability due to dense capture data redundancy.

Method: S-VGGT builds a dense scene graph to characterize structural redundancy, partitions the scene into balanced subscenes, and processes them efficiently using a common reference frame.

Result: The method reduces global attention costs and enables compounded speedups when combined with token-level acceleration techniques, without sacrificing reconstruction fidelity.

Conclusion: S-VGGT effectively tackles structural redundancy, offering a scalable and efficient solution for 3D foundation models while maintaining high fidelity and compatibility with other acceleration methods.
...
TL;DR: The paper introduces a method to improve generative inbetweening by using keyframe-anchored attention bias and rescaled temporal RoPE for better frame consistency and semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Existing generative inbetweening models struggle with inconsistent frames, unstable pacing, and semantic misalignment, especially in sparse sequences with large motions.

Method: The method uses keyframe-anchored attention bias to provide semantic and temporal guidance from keyframes and text, along with rescaled temporal RoPE to enforce frame consistency.

Result: The approach achieves state-of-the-art performance in frame consistency, semantic fidelity, and pace stability for both short and long sequences without additional training.

Conclusion: The proposed method effectively addresses challenges in generative inbetweening, outperforming previous models and introducing the TGI-Bench benchmark for targeted evaluation.
...
### [97] [Part-Aware Open-Vocabulary 3D Affordance Grounding via Prototypical Semantic and Geometric Alignment](https://arxiv.org/abs/2603.17647)
*Dongqiang Gou,Xuming He*

Main category: cs.CV

TL;DR: A novel two-stage cross-modal framework improves 3D affordance grounding by enhancing semantic and geometric representations, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To address challenges in open-vocabulary generalization, fine-grained geometric alignment, and part-level semantic consistency in language-driven 3D affordance grounding.

Method: A two-stage framework: first, large language models generate part-aware instructions for semantic recovery; second, Affordance Prototype Aggregation (APA) and Intra-Object Relational Modeling (IORM) improve geometric and semantic alignment.

Result: The method demonstrates superior performance on a new benchmark and two existing ones, validating its effectiveness.

Conclusion: The proposed framework successfully combines semantic and geometric enhancements to advance 3D affordance grounding, outperforming current approaches.
...
TL;DR: The paper addresses the local misalignment problem in CLIP-based Cross-Domain Few-Shot Learning (CDFSL) by introducing the CC-CDFSL method with cycle consistency and a Semantic Anchor mechanism, improving local vision-language alignment and achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current fine-tuned CLIP models struggle with fine-grained visual cues in downstream domains like medical diagnosis, exacerbated by domain gaps and scarce data, leading to local misalignment.

Method: Proposes CC-CDFSL with cycle consistency to translate between local visual and text features, and a Semantic Anchor mechanism to reduce noise in visual modality.

Result: The method improves local alignment, enhances interpretability, and achieves state-of-the-art performance across benchmarks.

Conclusion: The proposed approach effectively mitigates local misalignment in CLIP-based CDFSL, demonstrating superior performance and interpretability.
...
TL;DR: FINER benchmarks address fine-grained hallucinations in MLLMs, revealing issues in multi-object, multi-attribute, and multi-relation queries. FINER-Tuning using DPO reduces hallucinations and improves performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs focus on coarse-grained questions, missing fine-grained hallucinations, which this work aims to address.

Method: Proposes FINER benchmarks (FINER-CompreCap and FINER-DOCCI) and FINER-Tuning, leveraging DPO on fine-grained data.

Result: FINER-Tuning reduces hallucinations by up to 24.2% (InternVL3.5-14B), improves performance on 8 hallucination suites, and enhances general multimodal capabilities.

Conclusion: FINER benchmarks and FINER-Tuning effectively address fine-grained hallucinations in MLLMs, improving accuracy and generalization.
...
### [100] [WeatherReasonSeg: A Benchmark for Weather-Aware Reasoning Segmentation in Visual Language Models](https://arxiv.org/abs/2603.17680)
*Wanjun Du,Zifeng Yuan,Tingting Chen,Fucai Ke,Beibei Lin,Shunli Zhang*

Main category: cs.CV

TL;DR: WeatherReasonSeg is a new benchmark to evaluate vision-language models (VLMs) on reasoning-based segmentation under adverse weather, combining synthetic and real-world data across five reasoning dimensions.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for VLMs use idealized images, lacking evaluation under adverse weather. This paper addresses the gap by assessing VLM robustness in degraded visual conditions like rain or fog.

Method: The benchmark includes a controllable synthetic dataset with varying weather severity and a real-world dataset with semantic queries. It evaluates five reasoning dimensions.

Result: VLMs show performance decline with worsening weather, with distinct failure patterns for different weather types.

Conclusion: WeatherReasonSeg provides a foundation for developing weather-robust VLMs, highlighting vulnerabilities in current models.
...
### [101] [Few-Step Diffusion Sampling Through Instance-Aware Discretizations](https://arxiv.org/abs/2603.17671)
*Liangyu Yuan,Ruoyu Wang,Tong Zhao,Dingwen Fu,Mingkun Lei,Beier Zhu,Chi Zhang*

Main category: cs.CV

TL;DR: The paper introduces an instance-aware discretization framework to improve generation quality in diffusion and flow matching models by adapting timestep allocations based on input-specific complexity.


<details>
  <summary>Details</summary>
Motivation: Existing methods use globally shared timestep schedules, which fail to account for instance-specific complexity in generative processes, leading to suboptimal performance. Synthetic data experiments highlight this limitation.

Method: The proposed framework learns input-dependent timestep allocations, extending gradient-based discretization search to conditional generative settings.

Result: Empirical results show consistent improvement in generation quality across synthetic data, pixel-space diffusion, latent-space images, and video flow matching models, with minimal tuning cost and negligible inference overhead.

Conclusion: The instance-aware discretization framework effectively addresses the shortcomings of global schedules, enhancing performance in diverse generative tasks without significant computational penalties.
...
### [102] [DeepCORO-CLIP: A Multi-View Foundation Model for Comprehensive Coronary Angiography Video-Text Analysis and External Validation](https://arxiv.org/abs/2603.17675)
...
TL;DR: DeepCORO-CLIP is a multi-view AI model for comprehensive coronary angiography analysis, validated across multiple tasks, achieving high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address variability in visual interpretation of coronary angiography and limitations of existing AI methods, which focus on single frames or stenosis only.

Method: Trained with video-text contrastive learning on a large dataset of angiography videos, integrating multiple projections with attention-based pooling for study-level assessment.

Result: Achieved AUROC of 0.888-0.89 for stenosis detection, outperformed clinical reports in quantitative accuracy (13.6% vs. 19.0%), and performed well in other tasks like thrombus detection and disease progression.

Conclusion: DeepCORO-CLIP is a robust, efficient model for automated coronary angiography interpretation, with potential for clinical deployment.
...
### [103] [Illumination-Aware Contactless Fingerprint Spoof Detection via Paired Flash-Non-Flash Imaging](https://arxiv.org/abs/2603.17679)
*Roja Sahoo,Anoop Namboodiri*

Main category: cs.CV

TL;DR: The paper explores paired flash-non-flash contactless fingerprint acquisition for spoof detection, highlighting how flash illumination reveals material- and structure-dependent properties, while non-flash images provide baseline context.


<details>
  <summary>Details</summary>
Motivation: Contactless fingerprint recognition lacks physical contact and traditional liveness cues, making spoof detection challenging. Existing methods, relying on single-image acquisition and appearance-based features, often generalize poorly across devices and spoof materials.

Method: The study uses paired flash-non-flash acquisition to accentuate material and structural properties, analyzing lighting-induced differences with interpretable metrics like inter-channel correlation, specular reflection, texture realism, and differential imaging.

Result: The approach discriminates genuine fingerprints from printed, digital, and molded spoofs, though limitations include sensitivity to imaging settings, dataset scale, and high-fidelity spoofs.

Conclusion: Illumination-aware analysis improves robustness and interpretability in spoof detection, encouraging future research on paired acquisition and physics-informed feature design.
...
### [104] [Eye image segmentation using visual and concept prompts with Segment Anything Model 3 (SAM3)](https://arxiv.org/abs/2603.17715)
*Diederick C. Niehorster,Marcus Nyström*

Main category: cs.CV

TL;DR: SAM3's performance in eye image segmentation, compared to SAM2, was evaluated using diverse datasets, showing no improvement with visual or concept prompts, making SAM2 the better choice.


<details>
  <summary>Details</summary>
Motivation: To assess if the latest SAM3 model outperforms SAM2 in eye image segmentation, including its new concept prompting mode.

Method: Evaluated SAM3's segmentation performance using high-resolution lab videos and challenging in-the-wild datasets (TEyeD), testing both visual and concept prompts.

Result: SAM3 did not outperform SAM2 in most cases, including with concept prompts, and was slower.

Conclusion: SAM2 remains superior for eye image segmentation, being both faster and more effective than SAM3.
...
### [105] [SARE: Sample-wise Adaptive Reasoning for Training-free Fine-grained Visual Recognition](https://arxiv.org/abs/2603.17729)
*Jingxiao Yang,DaLin He,Miao Pan,Ge Su,Wenqi Zhang,Yifeng Hu,Tangwei Li,Yuke Li,Xuhong Zhang*

Main category: cs.CV

TL;DR: SARE is a sample-wise adaptive framework for training-free fine-grained visual recognition, combining retrieval and reasoning to improve accuracy and efficiency with a self-reflective experience mechanism.


<details>
  <summary>Details</summary>
Motivation: The challenge in leveraging LVLMs for FGVR lies in visual ambiguity and uneven recognition difficulty, with existing methods lacking adaptability and error reuse.

Method: SARE uses a cascaded design for retrieval and selective fine-grained reasoning, incorporating a self-reflective experience mechanism to learn from past errors.

Result: SARE outperforms existing methods on 14 datasets, achieving state-of-the-art performance with reduced computational costs.

Conclusion: SARE effectively addresses the limitations of current FGVR approaches by adapting inference per sample and reusing error-specific experience.
...
### [106] [Does YOLO Really Need to See Every Training Image in Every Epoch?](https://arxiv.org/abs/2603.17684)
*Xingxing Xie,Jiahua Dong,Junwei Han,Gong Cheng*

Main category: cs.CV

TL;DR: AFSS is proposed to dynamically skip easy and partially use medium training images in YOLO detectors, speeding up training and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Training YOLO detectors is inefficient due to processing every image in every epoch, even when many are already well-learned. This contradicts the 'You Only Look Once' philosophy.

Method: AFSS categorizes images into easy, medium, or hard based on learning sufficiency (minimum of recall and precision). Easy images are sparsely resampled, medium are partially selected, and hard are fully sampled. Learning sufficiency is periodically updated.

Result: AFSS achieves over 1.43× training speedup for YOLO-series detectors on benchmarks like MS COCO, PASCAL VOC, DOTA-v1.0, and DIOR-R, while also improving accuracy.

Conclusion: AFSS enhances YOLO training efficiency and effectiveness by adaptively focusing on informative images and reducing redundancy.
...
### [107] [Learning Transferable Temporal Primitives for Video Reasoning via Synthetic Videos](https://arxiv.org/abs/2603.17693)
...
TL;DR: SynRL is a post-training framework that teaches temporal primitives (like direction and speed) using synthetic videos, improving temporal understanding in vision-language models more effectively than real-world data.


<details>
  <summary>Details</summary>
Motivation: Current methods for video understanding lack temporal-centric data and have systematic errors in temporal perception. SynRL addresses this by focusing on temporal primitives.

Method: SynRL uses programmatically generated synthetic videos to train models on temporal primitives, decomposing understanding into short-term and long-term primitives with ground-truth annotations.

Result: SynRL outperforms existing methods on 15 benchmarks, including temporal grounding and complex reasoning, despite using only 7.7K synthetic samples.

Conclusion: Synthetic data for temporal learning is more effective and cost-efficient than real-world data, establishing a new paradigm for video post-training.
...
### [108] [Parameter-Efficient Modality-Balanced Symmetric Fusion for Multimodal Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2603.17705)
*Haocheng Li,Juepeng Zheng,Shuangxi Miao,Ruibo Lu,Guosheng Cai,Haohuan Fu,Jianxi Huang*

Main category: cs.CV

TL;DR: MoBaNet introduces a parameter-efficient, modality-balanced fusion framework for multimodal remote sensing semantic segmentation, using cross-modal prompts and adaptive fusion to enhance performance.


<details>
  <summary>Details</summary>
Motivation: To improve multimodal remote sensing semantic segmentation by addressing computational overhead and modality imbalance issues without compromising general-purpose representations.

Method: Utilizes a frozen VFM backbone with a symmetric dual-stream architecture, Cross-modal Prompt-Injected Adapter (CPIA), Difference-Guided Gated Fusion Module (DGFM), and Modality-Conditional Random Masking (MCRM) for balanced fusion.

Result: Achieves state-of-the-art performance on ISPRS Vaihingen and Potsdam benchmarks with fewer trainable parameters compared to full fine-tuning.

Conclusion: MoBaNet effectively balances modality contributions, reduces computational costs, and enhances performance in multimodal remote sensing tasks.
...
### [109] [DiffVP: Differential Visual Semantic Prompting for LLM-Based CT Report Generation](https://arxiv.org/abs/2603.17718)
*Yuhe Tian,Kun Zhang,Haoran Ma,Rui Yan,Yingtai Li,Rongsheng Wang,Shaohua Kevin Zhou*

Main category: cs.CV

TL;DR: DiffVP improves CT report generation by focusing on semantic differences between scans, outperforming existing methods in accuracy and clinical efficacy.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs for CT report generation fail to distinguish important cues from redundant background. DiffVP addresses this by leveraging scan-to-reference semantic differences.

Method: DiffVP uses a hierarchical difference extractor to capture global and local discrepancies, generating visual prompts for LLMs to enhance report accuracy.

Result: DiffVP outperforms prior methods, improving BLEU scores by +10.98 and +4.36, and achieves a 0.421 F1 score on RadGenome-ChestCT.

Conclusion: DiffVP's focus on semantic differences and structured conditioning leads to more accurate and clinically effective CT report generation.
...
TL;DR: A multi-label gastrointestinal video analysis pipeline was developed using a ResNet-50 frame classifier and anatomy-guided temporal event decoding to predict 17 labels, addressing class imbalance and fragmentation issues to improve performance.


<details>
  <summary>Details</summary>
Motivation: The study aimed to address challenges in video analysis for gastrointestinal diagnostics, particularly class imbalance and fragmented event predictions, to enhance the accuracy of pathology detection.

Method: The approach involved a ResNet-50 frame classifier with clipped class-wise positive weighting for imbalance mitigation, followed by temporal event decoding using GT-style framewise event composition, anatomy vote smoothing, and pathology gating.

Result: The proposed method improved the temporal mAP from 0.3801 to 0.4303 on the challenge test set, demonstrating effectiveness in handling rare pathology labels and fragmented predictions.

Conclusion: The pipeline successfully addressed key challenges in gastrointestinal video analysis, achieving measurable improvements in performance through innovative loss weighting and temporal decoding techniques.
...
TL;DR: The paper introduces a fine-grained quantization strategy (QIG) for Large Vision Language Models (LVLMs) to reduce computational overhead while maintaining accuracy by evaluating token sensitivity at the token level.


<details>
  <summary>Details</summary>
Motivation: Existing LVLM quantization methods fail to capture complex cross-token interactions and measure quantization error at the token level, necessitating a more fine-grained approach.

Method: The proposed method, Quantization-aware Integrated Gradients (QIG), uses integrated gradients to quantitatively evaluate token sensitivity and refine quantization granularity from modality to token level.

Result: Experiments show improved accuracy across LVLMs (e.g., 1.60% average accuracy boost for LLaVA-onevision-7B under 3-bit weight-only quantization) with minimal latency overhead.

Conclusion: QIG effectively bridges the gap between quantized and full-precision models, offering a practical solution for LVLM deployment.
...
TL;DR: ChopGrad reduces memory costs in video diffusion models by truncating backpropagation to local frame windows, enabling efficient fine-tuning with pixel-wise losses.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models face high memory costs due to recurrent frame processing, making fine-tuning with pixel-wise losses impractical for long or high-resolution videos.

Method: Introduced ChopGrad, a truncated backpropagation scheme that limits gradient computation to local frame windows while ensuring global consistency.

Result: ChopGrad reduces training memory from scaling linearly with frames to constant memory, outperforming state-of-the-art models in tasks like video super-resolution and inpainting.

Conclusion: ChopGrad provides an efficient solution for training video diffusion models with pixel-wise losses, addressing memory limitations without compromising performance.
...
TL;DR: TAPESTRY is a framework for generating high-fidelity 360-degree turntable videos (TTVs) from untextured 3D models, ensuring geometric consistency and appearance stability for high-quality 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: The challenge of automatically generating photorealistic and self-consistent appearances for untextured 3D models is addressed by leveraging video diffusion models, which struggle with view consistency. TAPESTRY aims to solve this by conditioning the synthesis on explicit 3D geometry.

Method: TAPESTRY reframes 3D appearance generation as a geometry-conditioned video diffusion problem. It encodes multi-modal geometric features from a 3D mesh to constrain video synthesis. A multi-stage pipeline with 3D-Aware Inpainting completes self-occluded regions for full surface coverage.

Result: The generated TTVs are high-quality, consistent, and suitable for UV texture back-projection or neural rendering supervision (e.g., 3DGS). Experiments show TAPESTRY outperforms existing methods in video consistency and reconstruction quality.

Conclusion: TAPESTRY provides a reliable, automated solution for producing complete, production-ready 3D assets from untextured meshes, advancing digital content creation.
...
TL;DR: The paper introduces Concept-to-Pixel (C2P), a prompt-free universal segmentation framework that separates anatomical knowledge into geometric and semantic representations, improving automation and robustness in medical image segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing universal segmentation methods rely on manual prompts or reference images, limiting automation and robustness. Joint training across modalities often fails due to large domain shifts.

Method: C2P uses Multimodal Large Language Models (MLLMs) to distill medical concepts into learnable Semantic Tokens and introduces Geometric Tokens for universal constraints. It also employs a Geometry-Aware Inference Consensus mechanism for reliability.

Result: Experiments on eight datasets across seven modalities show C2P's superiority over single-model or universe approaches. The model excels in zero-shot tasks and cross-modal transfers.

Conclusion: C2P addresses limitations of existing methods by disentangling anatomical knowledge and leveraging geometry-awareness, achieving strong generalization and performance in diverse medical image segmentation tasks.
...
TL;DR: PC-CrossDiff is a dual-task framework for 3D visual grounding, addressing challenges in complex scenes with dual-level cross-modal differential attention, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D visual grounding struggle in complex, multi-object scenes due to inadequate parsing of implicit cues and ineffective suppression of spatial interference, limiting practical use.

Method: Proposes PC-CrossDiff with Point-Level and Cluster-Level Differential Attention modules to enhance discriminative representation and suppress irrelevant spatial relations.

Result: Achieves top performance on ScanRefer, NR3D, and SR3D benchmarks, with a +10.16% improvement in Overall@0.50 for 3DREC on ScanRefer's Implicit subsets.

Conclusion: PC-CrossDiff effectively addresses key challenges in 3D visual grounding, demonstrating strong performance in parsing implicit spatial cues and suppressing interference in complex scenes.
...
TL;DR: Tokenization is crucial for generative modeling, especially in autoregressive (AR) models for 3D generation. Current methods using geometric hierarchies are inefficient. The paper proposes Level-of-Semantics Tokenization (LoST) for better semantic coherence and efficiency, validated by superior results.


<details>
  <summary>Details</summary>
Motivation: Traditional geometric hierarchies for tokenizing 3D shapes are inefficient for AR models, lacking semantic coherence. A more efficient and semantically meaningful tokenization method is needed.

Method: Proposes Level-of-Semantics Tokenization (LoST), ordering tokens by semantic salience, and Relational Inter-Distance Alignment (RIDA) to align 3D shape latent space with semantic feature space.

Result: LoST surpasses previous methods in reconstruction quality, achieves efficient AR 3D generation, and enables tasks like semantic retrieval with significantly fewer tokens.

Conclusion: LoST provides a more efficient and semantically coherent tokenization method for 3D shapes, improving AR models and enabling downstream applications.
...
TL;DR: SCEP is a training-free framework for image deepfake detection that uses evidence-driven reasoning with suspicious patch tokens, outperforming baselines without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models (LVLMs) require costly fine-tuning for IDD and generalize poorly to diverse manipulations, prompting the need for a more efficient approach.

Method: SCEP replaces whole-image inference with evidence-driven reasoning, mining suspicious patch tokens, clustering features, scoring patches with a fused metric, and sampling high-confidence patches for prediction.

Result: SCEP outperforms strong baselines on diverse benchmarks without requiring LVLM fine-tuning.

Conclusion: SCEP provides an effective, training-free solution for image deepfake detection by leveraging evidence-driven reasoning and suspicious patch tokens.
...
TL;DR: STTS is a lightweight module for pruning vision tokens across ViT and LLM, improving efficiency by 62% with minimal performance drop.


<details>
  <summary>Details</summary>
Motivation: Prior token pruning methods either focus on unimodal tasks or require complex text-conditioning mechanisms, lacking a unified approach for vision-language tasks.

Method: STTS uses spatio-temporal token scoring with an auxiliary loss and LLM downstream gradients, plus an efficient packing algorithm, to prune 50% of tokens.

Result: Achieves 62% efficiency gain with only a 0.7% performance drop across 13 video QA tasks; efficiency increases with more frames.

Conclusion: STTS offers a simple, effective, and unified solution for architecture-wide vision token pruning, with potential for further gains in long-video QA.
...
### [119] [CrowdGaussian: Reconstructing High-Fidelity 3D Gaussians for Human Crowd from a Single Image](https://arxiv.org/abs/2603.17779)
*Yizheng Song,Yiyu Zhuang,Qipeng Xu,Haixiang Wang,Jiahe Zhu,Jing Tian,Siyu Zhu,Hao Zhu*

Main category: cs.CV

TL;DR: CrowdGaussian introduces a framework for reconstructing 3D human crowd models from single images, addressing occlusions and low clarity with self-supervised adaptation and Self-Calibrated Learning.


<details>
  <summary>Details</summary>
Motivation: Prior research focused on clear, close-up images of individuals, often performing poorly in multi-person scenarios with occlusions and low clarity. CrowdGaussian aims to tackle these challenges.

Method: The framework uses self-supervised adaptation for handling occlusions and Self-Calibrated Learning (SCL) to refine coarse renderings. It reconstructs multi-person 3D Gaussian Splatting representations from single-image inputs.

Result: CrowdGaussian produces photorealistic, geometrically coherent reconstructions of multi-person scenes, as validated through extensive experiments.

Conclusion: CrowdGaussian effectively addresses the complexities of multi-person 3D reconstruction, offering a robust solution for real-world crowd scenarios.
...
### [120] [Exploring parameter-efficient fine-tuning (PEFT) of billion-parameter vision models with QLoRA and DoRA: insights into generalization for limited-data image classification under a 98:1 test-to-train regime](https://arxiv.org/abs/2603.17782)
*Haiyu Yang,Sumit Sharma,Enhong Liu,Miel Hostens*

Main category: cs.CV

TL;DR: The study compares training methods for livestock behavior classification, finding parameter-efficient fine-tuning (PEFT) with QLoRA as the best approach, achieving high accuracy with minimal computational cost.


<details>
  <summary>Details</summary>
Motivation: To address challenges in automated behavior classification for precision livestock farming, such as high computational costs and limited labeled data, by evaluating efficient training methods.

Method: Systematically compared three approaches: training from scratch (ResNet-18, ViT-Small), frozen feature extraction, and PEFT of the DINOv3 model using QLoRA and DoRA with varying ranks and target modules.

Result: PEFT outperformed other methods; the best QLoRA configuration achieved 83.16% test accuracy with only 2.72% parameters trained, significantly better than alternatives.

Conclusion: PEFT is highly effective for adapting billion-parameter vision models in agriculture, with underfitting being the primary challenge rather than overfitting.
...
TL;DR: The paper introduces Mask-to-Point (M2P) learning, a method to enhance Vision Foundation Models (VFMs) for dense point tracking using video object segmentation (VOS) masks. M2P employs three mask-based constraints to improve representation learning, achieving significant performance gains over baseline models.


<details>
  <summary>Details</summary>
Motivation: Current VFMs rely on static image pre-training, which is sub-optimal for capturing dense temporal correspondence in videos. The authors aim to improve VFMs for point tracking by leveraging VOS mask annotations.

Method: The proposed M2P learning introduces three constraints: (1) a local structure consistency loss for cohesive motion learning, (2) a mask label consistency loss for foreground matching, and (3) a mask boundary constraint to supervise boundary points.

Result: M2P outperforms baseline VFMs (DINOv2/3) by 12.8% and 14.6% on the TAP-Vid-DAVIS benchmark, using only 3.6K VOS training videos. It also serves as a strong pre-trained backbone for point tracking tasks.

Conclusion: M2P effectively improves VFMs for dense point tracking through weakly-supervised learning with VOS masks, demonstrating its potential as a general pre-trained model for such tasks.
...
### [122] [VideoAtlas: Navigating Long-Form Video in Logarithmic Compute](https://arxiv.org/abs/2603.17948)
*Mohamed Eltahir,Ali Habibullah,Yazan Alshoibi,Lama Ayash,Tanveer Hussain,Naeemullah Khan*

Main category: cs.CV

TL;DR: VideoAtlas introduces a hierarchical grid for lossless video representation, enabling scalable and efficient video understanding with logarithmic compute growth.


<details>
  <summary>Details</summary>
Motivation: Existing methods for video language models suffer from lossy approximations and collapse visual fidelity into text. VideoAtlas aims to provide a lossless, navigable, and scalable solution.

Method: VideoAtlas uses a hierarchical grid to represent video uniformly, allowing recursive zooming. It integrates with Recursive Language Models (RLMs) via a Master-Worker architecture for parallel exploration.

Result: Key findings include logarithmic compute growth, 30-60% multimodal cache hit rate, principled compute-accuracy trade-offs, and emergent adaptive compute allocation. Video-RLM remains robust even for long videos (1-10 hours).

Conclusion: Structured environment navigation via VideoAtlas is a scalable and viable paradigm for video understanding, preserving visual fidelity and efficiency.
...
TL;DR: The paper explores leveraging Massive Activations (MAs) in video diffusion transformers to improve video generation quality without significant overhead, proposing a method called Structured Activation Steering (STAS).


<details>
  <summary>Details</summary>
Motivation: Research gaps in utilizing internal model signals (like MAs) to enhance video generation quality with minimal computational cost.

Method: Study MAs in video diffusion transformers, observe their hierarchical patterns, and propose STAS, a training-free method to steer MAs for better results.

Result: STAS improves video quality and temporal coherence across models with negligible overhead.

Conclusion: MAs provide a valuable signal for enhancing video generation, and STAS effectively leverages them for consistent improvements.
...
TL;DR: The paper discusses how current text-to-image concept erasure methods are text-centric and ineffective, as they leave underlying visual knowledge intact. It introduces TINA, a text-free inversion attack, to demonstrate this flaw and regenerate erased concepts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of current concept erasure techniques in text-to-image diffusion models, which focus only on text-to-image mapping and ignore persistent visual knowledge.

Method: The authors propose TINA (Text-free INversion Attack), a visual-only probe that operates under null-text conditions to bypass text-centric defenses. It includes an optimization procedure to handle approximation errors in inversion without textual guidance.

Result: TINA successfully regenerates erased concepts from models treated with state-of-the-art unlearning, proving that current erasure methods only obscure concepts rather than truly removing them.

Conclusion: The study concludes that current erasure paradigms are insufficient and calls for new approaches that directly target internal visual knowledge to ensure effective concept removal.
...
TL;DR: The survey organizes video understanding into three perspectives—low-level geometry, high-level semantics, and unified models—highlighting a shift toward adaptable, unified modeling paradigms.


<details>
  <summary>Details</summary>
Motivation: Video understanding is foundational to computer vision, requiring temporal dynamics and spatiotemporal reasoning, and this survey aims to systematize its progress.

Method: The survey categorizes literature into low-level video geometry, high-level semantics, and unified models, emphasizing trends toward adaptable, unified paradigms.

Result: Key modeling trends and design principles are summarized, and open challenges for robust, scalable, unified video foundation models are outlined.

Conclusion: The survey provides a coherent overview of video understanding, highlighting progress and challenges in unifying models for diverse tasks.
...
TL;DR: Omni-3DEdit is a unified, learning-based model for various 3D editing tasks, eliminating the need for task-dependent rules and iterative optimization, and reducing inference time significantly.


<details>
  <summary>Details</summary>
Motivation: Current instruction-driven 3D editing methods suffer from lack of universal design and time-consuming iterative optimization, prompting the need for a more efficient and generalized solution.

Method: Omni-3DEdit leverages a pre-trained generative model (SEVA) with a synthesized dataset of paired multi-view editing samples, using a dual-stream LoRA module to enhance representational learning.

Result: The model reduces inference time from tens of minutes to about two minutes and effectively handles various 3D editing tasks in one forward pass.

Conclusion: Omni-3DEdit demonstrates a significant improvement in efficiency and versatility for 3D editing tasks, validated by extensive experiments.
...
TL;DR: The paper compares human perceptual priors (hand annotations, eye tracking heatmaps, etc.) for iris PAD, finding denoised eye tracking heatmaps most effective in open-set scenarios.


<details>
  <summary>Details</summary>
Motivation: To explore the most effective form of human saliency for improving iris presentation attack detection (PAD) in open-set scenarios.

Method: Experiments comparing hand annotations, eye tracking heatmaps, segmentation masks, and DINOv2 embeddings against a deep learning baseline for iris PAD.

Result: Denoised eye tracking heatmaps showed the best generalization improvement in terms of AUROC and APCER at 1% BPCER.

Conclusion: Denoised eye tracking heatmaps are the most effective human saliency form for open-set iris PAD, with resources provided for reproducibility.
...
TL;DR: The paper investigates 'edit spillover' in image editing models, proposing a framework to analyze it as a probe for world understanding, and reveals key insights about trade-offs and semantic relevance.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine whether edit spillover in image editing models reflects genuine world knowledge or accidental attention leakage, addressing a fundamental gap in understanding model behavior.

Method: The authors introduce EditSpilloverProbe, a framework with a spillover taxonomy, automated detection pipeline, and benchmark dataset (EditSpilloverBench), evaluating five models.

Result: Findings include varied spillover rates (3.49%-11.46%), trade-offs between editing control and semantic spillover, and evidence that semantic spillover reflects real world understanding.

Conclusion: The study concludes that semantic spillover is a meaningful indicator of world knowledge in models, with implications for improving editing precision and understanding model capabilities.
...
TL;DR: The paper presents a unified framework for identity-aware joint audio-video generation, focusing on fine-grained control over facial appearance and voice timbre across multiple identities.


<details>
  <summary>Details</summary>
Motivation: There is a growing demand for identity-aware content creation, but existing frameworks lack fine-grained control and scalability for multiple identities.

Method: The authors introduce a data curation pipeline for extracting identity-bearing information, propose a flexible identity injection mechanism, and design a multi-stage training strategy for cross-modal coherence.

Result: Experiments show the framework's superiority in generating high-fidelity and consistent identity-aware audio-video content.

Conclusion: The proposed framework successfully addresses the challenge of fine-grained control in identity-aware joint audio-video generation, demonstrating scalability and high performance.
...
TL;DR: CAT introduces reusable creative tokens for T2I models to enhance creativity without repeated computations, achieving faster, cheaper, and better results.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of T2I models in handling fuzzy prompts and costly creativity augmentation methods.

Method: Develops a Creative Tokenizer to generate reusable token templates from fuzzy prompts, trained via creative semantic disentanglement.

Result: 3.7× speedup, 4.8× cost reduction, and superior image quality and alignment compared to existing methods.

Conclusion: CAT offers a scalable and efficient way to enhance T2I creativity, reducing reliance on human prompt design and repeated computations.
...
TL;DR: SpiderCam is an FPGA-based camera producing real-time sparse depth maps at 32.5 FPS with low power consumption (624 mW).


<details>
  <summary>Details</summary>
Motivation: To develop a low-power, real-time depth-from-defocus camera for passive 3D imaging.

Method: Uses a custom camera capturing dual-focused images, processed via SystemVerilog on an FPGA with algorithmic improvements for DfDD.

Result: Achieves 480x400 depth maps at 32.5 FPS over 52 cm range, with a total power consumption of 624 mW.

Conclusion: SpiderCam sets a benchmark for sub-Watt passive FPGA-based 3D cameras.
...
TL;DR: A VAE-based semi-gray-box framework detects adversarial manipulations in collaborative edge-AI object classification, improving accuracy and reducing false alarms under noisy conditions.


<details>
  <summary>Details</summary>
Motivation: Edge-offloading in collaborative DNN inference is vulnerable to stealthy adversarial attacks, especially in noisy environments, necessitating robust detection methods.

Method: Proposes a noise-aware anomaly detection framework using a variational autoencoder (VAE) to identify deviations from adversarial manipulations and environmental noise.

Result: Achieves up to 90% AUROC across DNN configurations, demonstrating robustness in noisy conditions, though limitations arise from feature similarity and high noise levels.

Conclusion: The framework effectively detects adversarial manipulations in edge-AI, balancing accuracy and false alarms, but further work is needed to address identified limitations.
...
TL;DR: A scalable geometry-driven 2D-3D-2D paradigm automates label propagation for RGB and thermal UAV imagery, achieving high annotation accuracy and enabling the creation of the SegFly benchmark.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of existing RGB and RGB-T datasets for UAV semantic segmentation due to high labeling costs and RGB-T alignment challenges.

Method: Leveraging multi-view redundancy in aerial imagery to propagate labels from a small subset of manually annotated RGB images to RGB and thermal modalities via 3D point clouds and reprojection.

Result: Automated generation of 97% of RGB labels and 100% of thermal labels with 91% and 88% accuracy, and 87% RGB-T registration accuracy. The SegFly benchmark includes over 20,000 RGB and 15,000 RGB-T images.

Conclusion: The geometry-driven 2D-3D-2D pipeline is effective for scalable multi-modal UAV scene understanding, benefiting both conventional architectures and vision foundation models.
...
TL;DR: A multi-stage AI pipeline for legal age estimation from clavicle CT scans achieves state-of-the-art accuracy with interpretability and uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: Accurate, robust, and reproducible legal age estimation is critical in forensic contexts, yet clavicle CT scans are underexplored despite their effectiveness.

Method: Combines automatic clavicle detection, Integrated Gradings-guided slice selection, a multi-slice CNN, and conformal prediction for uncertainty-aware decisions.

Result: Achieves MAE of 1.55 ± 0.16 years, outperforming human experts (MAE ~1.90 years) and prior methods (MAE >1.75 years).

Conclusion: The method, integrated into Skeleton-ID software, serves as a decision-support tool in forensic workflows with configurable uncertainty coverage.
...
TL;DR: A multimodal legal dataset and framework called C-TRAIL is introduced to bridge the gap between dashcam video evidence and legal responsibility determination in traffic accidents.


<details>
  <summary>Details</summary>
Motivation: To address the reliance on human experts for transforming video evidence into legal responsibility under traffic regulations, especially in the Chinese system.

Method: A two-stage framework: (1) traffic accident understanding module for generating textual descriptions from videos, and (2) a legal multi-agent framework for determining responsibility modes, statutes, and judgment reports.

Result: Outperforms general and legal LLMs, as well as agent-based approaches, on the C-TRAIL and MM-AU datasets, with transparent reasoning.

Conclusion: The proposed method effectively bridges video evidence and legal reasoning, offering a more automated and interpretable solution for traffic accident responsibility determination.
...
TL;DR: TransText introduces a novel Alpha-as-RGB paradigm to adapt image-to-video models for layer-aware text animation, avoiding the need to retrain VAEs and preserving semantic priors.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat transparency as an extra latent dimension, requiring costly VAE retraining and risking semantic prior erosion. TransText addresses this by jointly modeling appearance and transparency without modifying pre-trained models.

Method: TransText embeds the alpha channel as an RGB-compatible signal via latent spatial concatenation, ensuring cross-modal consistency and avoiding feature entanglement.

Result: TransText outperforms baselines, producing high-fidelity transparent animations with diverse, fine-grained effects.

Conclusion: TransText offers a computationally efficient and effective solution for layer-aware text animation, preserving RGB-driven semantic priors.
...
TL;DR: LaDe is a latent diffusion framework for generating flexible, semantically layered media designs from natural language prompts, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for layered media design generation either limit layer count or require spatially continuous regions, hindering complexity. LaDe addresses these limitations by enabling flexible, semantically meaningful layers.

Method: LaDe combines an LLM-based prompt expander, a Latent Diffusion Transformer with 4D RoPE encoding, and an RGBA VAE to jointly generate media designs and their RGBA layers. It supports text-to-image, text-to-layers, and media design decomposition.

Result: LaDe outperforms Qwen-Image-Layered in text-to-layers generation, improving alignment with user intent, as validated by GPT-4o mini and Qwen3-VL evaluators.

Conclusion: LaDe provides a unified, efficient framework for generating editable layered designs, advancing flexibility and quality in media design generation.
...
TL;DR: Robust-ComBat, a method using MLP for outlier compensation, outperforms traditional ComBat variants in harmonizing dMRI data, especially in the presence of neurological disorders, by reducing distortions and preserving disease-related signals.


<details>
  <summary>Details</summary>
Motivation: Existing harmonization methods like ComBat assume Gaussian distributions, but neurological disorders introduce outliers that distort site-effect estimation. This poses challenges in clinical practice where patient scans are essential for diagnosis.

Method: The study evaluates 10 outlier rejection methods with 4 ComBat variants across 7 neurological conditions. A simple MLP is proposed for outlier compensation, ensuring robust harmonization.

Result: Robust-ComBat consistently reduces harmonization error across all ComBat variants, even in cohorts with up to 80% subjects having neurological disorders.

Conclusion: Robust-ComBat effectively addresses the limitations of traditional ComBat in clinical settings, providing reliable harmonization while preserving pathological signals.
...
TL;DR: AHOY reconstructs complete, animatable 3D Gaussian avatars from monocular video with heavy occlusion using hallucination-as-supervision and a two-stage architecture.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail with occluded footage, leaving large body regions unobserved and lacking multi-view supervision.

Method: Uses identity-finetuned diffusion models for hallucination-as-supervision, a two-stage canonical-to-pose-dependent architecture, and decouples map-pose/LBS-pose to handle inconsistencies.

Result: State-of-the-art reconstruction quality on YouTube and multi-view capture data, with avatars robust for animation and 3DGS scenes.

Conclusion: AHOY effectively addresses occlusion challenges, enabling high-quality 3D avatar reconstruction from monocular video.
...
TL;DR: The paper proposes an adaptive feedback-based radar data compression method to handle high-dimensional radar data in autonomous driving, achieving significant size reduction with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Radar data is crucial for autonomous driving but its high volume saturates communication links; existing compression methods lack adaptability to varying conditions.

Method: The method uses gradient descent from proxy gradients to dynamically adjust compression ratios, employs zeroth-order gradient approximation, and applies DCT with selective pruning and scaled quantization.

Result: The proposed scheme achieves over 100x feature size reduction with only a ~1% performance drop, validated on RADIal, CARRADA, and Radatron datasets.

Conclusion: The adaptive compression method effectively addresses radar data volume challenges, maintaining performance while significantly reducing data size.
...
TL;DR: Motion-MLLM enhances multimodal large language models with egomotion data for efficient, accurate 3D spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs for spatial reasoning rely on costly 3D representations or lack physical grounding, limiting their accuracy and efficiency.

Method: Introduces Motion-MLLM with keyframe filtering and asymmetric cross-modal fusion to integrate IMU data for better scale and spatial reasoning.

Result: Motion-MLLM outperforms SOTA methods in 3D tasks with higher accuracy and lower computational cost.

Conclusion: Motion-MLLM advances spatial reasoning by efficiently leveraging egomotion data, offering a cost-effective alternative to traditional methods.
...
TL;DR: GMT is a multimodal transformer framework for generating realistic 6-DOF object manipulation trajectories by leveraging 3D geometry, context, and semantic info, outperforming existing methods in accuracy and control.


<details>
  <summary>Details</summary>
Motivation: Current approaches for 6-DOF object manipulation rely on limited 2D or partial 3D representations, hindering spatial reasoning and trajectory precision, prompting the need for a more robust solution.

Method: GMT employs a multimodal transformer that integrates 3D bounding box geometry, point cloud context, semantic categories, and target poses, using a tailored conditioning strategy for trajectory generation.

Result: GMT outperforms state-of-the-art baselines (e.g., CHOIS, GIMO) in synthetic and real-world benchmarks, achieving higher spatial accuracy and orientation control.

Conclusion: GMT sets a new benchmark for learning-based manipulation planning, demonstrating strong generalization in diverse and cluttered 3D environments.
...
TL;DR: A training-free framework for continuous and controllable image editing using text-conditioned generative models by steering in text-embedding space.


<details>
  <summary>Details</summary>
Motivation: To enable smooth and controllable image editing without additional training or manual intervention, leveraging text-embedding space for semantic control.

Method: Utilizes a large language model to create debiased contrastive prompt pairs, computes a steering vector, and applies it to input prompts. An elastic range search ensures continuous control.

Result: The method achieves smooth and continuous edits, generalizing across image and video generation, and performs comparably to training-based alternatives.

Conclusion: Simple text-embedding steering is effective for continuous and controllable editing, outperforming other training-free methods.
...
TL;DR: EchoGen is a unified framework for layout-to-image generation and image grounding, addressing optimization challenges with progressive training strategies to achieve state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The paper aims to overcome limitations in layout-to-image generation and image grounding by leveraging their complementary strengths through joint training.

Method: Proposes progressive training: Parallel Multi-Task Pre-training (PMTP), Dual Joint Optimization (DJO), and Cycle RL with GRPO strategy to unify and optimize tasks.

Result: Achieves top performance on benchmarks, showing synergistic gains from joint optimization.

Conclusion: Joint training with progressive strategies significantly enhances both tasks, demonstrating the framework's effectiveness.
...
TL;DR: SkeletonLLM translates skeleton sequences into visual modalities for multimodal LLMs, enhancing their capabilities with a differentiable renderer and cooperative training.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of MLLMs in processing structured, non-visual data like human skeletons, which existing methods handle poorly by compressing or quantizing them.

Method: Uses DrAction, a differentiable renderer to convert skeletal kinematics into images, and employs cooperative training including Causal Reasoning Distillation and Discriminative Finetuning.

Result: SkeletonLLM achieves strong generalization in tasks like recognition, captioning, reasoning, and cross-format transfer, validating its effectiveness.

Conclusion: SkeletonLLM offers a viable solution for extending MLLMs to non-native modalities like skeleton data, with promising performance across diverse tasks.
...
TL;DR: Study evaluates LLMs (ChatGPT-4o, Perplexity AI, GeminiAI) for maternal health education in rural India, finding Perplexity matches expert semantics and ChatGPT-4o excels in clarity and terminology.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of reliable maternal healthcare information in rural India by leveraging digital tools and LLMs.

Method: Posed 17 pregnancy questions to LLMs and compared responses with professionals using semantic similarity, noun overlap, and readability metrics.

Result: Perplexity closely matched expert responses semantically, while ChatGPT-4o provided clearer, more understandable text with superior medical terminology.

Conclusion: LLMs can be scalable aids for maternal health education, emphasizing the need for AI tools balancing accuracy and clarity in underserved regions.
...
TL;DR: A RAG system with neural reranking improves answer quality in financial report analysis, reducing errors and boosting correctness.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of extracting information from lengthy 10-K reports efficiently.

Method: Hybrid search combining full-text and semantic retrieval, with optional neural reranking using a cross-encoder model.

Result: Reranking improves answer correctness by 15.5 percentage points and reduces the error rate for incorrect answers.

Conclusion: Neural reranking is crucial for enhancing performance in financial RAG systems, outperforming baseline methods.
...
TL;DR: A rubric-guided reasoning framework is introduced to improve L2 speech assessment by aligning SpeechLLMs with human raters' nuanced variability, focusing on accuracy, fluency, and prosody.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of reliable and interpretable automated assessment of L2 speech, where current models struggle to align with human raters' nuanced judgments.

Method: Fine-tuned the Qwen2-Audio-7B-Instruct model using multi-rater human judgments and developed an uncertainty-calibrated regression approach with conformal calibration for interpretable confidence intervals.

Result: The approach achieved the strongest alignment with human ratings, outperforming baselines in assessing fluency and prosody, though accuracy assessment remained challenging.

Conclusion: Rubric-guided, uncertainty-calibrated reasoning provides a principled way to enhance the trustworthiness and explainability of SpeechLLM-based speech assessment.
...
TL;DR: The paper evaluates the robustness of NL2SQL systems against various perturbations, showing that while modern LLMs perform well under many conditions, they struggle with surface-level noise and linguistic variations.


<details>
  <summary>Details</summary>
Motivation: To address the gap in robustness evaluation for NL2SQL systems in dynamic, noisy real-world environments compared to static benchmark assumptions.

Method: Introduces a robustness benchmark with 10 perturbation types, evaluates multiple LLMs (e.g., Grok-4.1, GPT-5.2) under traditional and agentic settings.

Result: LLMs perform well under many perturbations but degrade with surface-level noise and linguistic variations. Traditional pipelines suffer more from noise, while agentic settings face linguistic challenges.

Conclusion: Robustness in NL2SQL systems remains challenging, especially for linguistic variability, highlighting areas for improvement.
...
TL;DR: The paper critiques current evaluations of LLMs for ill-defined tasks, highlighting issues like unclear metrics and unstable scores. It suggests more robust and interpretable evaluation designs.


<details>
  <summary>Details</summary>
Motivation: To expose the limitations of current evaluation benchmarks and metrics for ill-defined tasks in LLMs, emphasizing the need for better diagnostic signals.

Method: Analyzes two case studies: Complex Instruction Following (CIF) and Natural Language to Mermaid Sequence Diagrams (NL2Mermaid), identifying recurring evaluation issues.

Result: Current evaluations conflate failure modes, producing unstable and non-diagnostic scores, and highlight the need for multi-faceted evaluation criteria.

Conclusion: The findings underscore the need for more robust and interpretable evaluation designs for ill-defined tasks in LLMs.
...
TL;DR: The paper identifies script barriers as a key issue in cross-lingual knowledge transfer in LLMs and suggests methods to improve reasoning for better transfer.


<details>
  <summary>Details</summary>
Motivation: To understand and address the perceived gaps in cross-lingual knowledge transfer in modern reasoning LLMs, particularly focusing on script barriers.

Method: Conducted observational data analysis on ECLeKTic and MultiLoKo datasets, performed regression analysis, and developed a synthetic generation pipeline for SFT samples to improve reasoning.

Result: Script match, not language or family, is the primary predictor of knowledge transfer failure. Providing key entities in the source language and teaching models to reason better reduces the gap.

Conclusion: There is potential to enhance cross-lingual knowledge transfer in LLMs by addressing script barriers during post-training.
...
TL;DR: An ensemble-driven self-training framework for unsupervised neural machine translation (UNMT) improves performance by leveraging diverse models and token-level ensemble decoding.


<details>
  <summary>Details</summary>
Motivation: To enhance UNMT performance by introducing structured diversity across models and utilizing ensemble-generated pseudo-translations for shared supervision.

Method: Train multiple UNMT models with shared translation tasks but different auxiliary languages, generate pseudo-translations via token-level ensemble decoding, and use these as synthetic data for further training.

Result: Statistically significant improvements over single-model UNMT baselines, with mean gains of 1.7 chrF for English-to-other and 0.67 chrF for other-to-English translations.

Conclusion: The framework effectively boosts UNMT performance while maintaining single-model inference efficiency, demonstrating the benefits of ensemble-driven self-training.
...
TL;DR: The paper introduces CoCoEval, a framework to evaluate inconsistent and uncollaborative behaviors in LLM-simulated conversations, revealing limitations in replicating human-like interactions despite prompt engineering and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To address the lack of analysis on inconsistent and uncollaborative behaviors in LLM-simulated conversations, which are crucial for realistic human social interaction modeling.

Method: Introduces CoCoEval, an LLM-as-a-Judge framework detecting 10 types of behaviors in LLM-simulated and human conversations across various settings.

Result: LLM-simulated conversations show fewer inconsistencies than human ones, prompt engineering lacks reliability, and fine-tuning can overproduce specific behaviors.

Conclusion: Simulating human conversations with LLMs is challenging, raising concerns about their reliability as proxies for human social interaction.
...
TL;DR: The paper leverages cross-lingual inconsistency in LLMs for interpretability, using a knowledge localization framework to identify key experts in MoE models.


<details>
  <summary>Details</summary>
Motivation: Addressing the variance in LLM behavior across languages by turning it into a tool for interpretability, specifically in mixture-of-experts models.

Method: A two-stage approach: (1) querying the model in diverse languages to generate success/failure activations, and (2) contrastive analysis of router logits to pinpoint crucial experts.

Result: Deactivating around 20 out of 6000 experts caused the model to fail in over 40% of cases, validating the method's effectiveness.

Conclusion: The framework offers a realistic, scalable way to localize knowledge in complex LLMs.
...
TL;DR: The paper introduces a framework using the English Grammar Profile (EGP) to evaluate L2 learners' grammatical competence, providing feedback and assessing proficiency via rule-based and LLM-based classifiers, with a hybrid approach showing the best results.


<details>
  <summary>Details</summary>
Motivation: To improve feedback and assessment for L2 learners by leveraging EGP and CEFR, focusing on both successful and unsuccessful grammatical attempts.

Method: Uses EGP to map grammatical constructs to CEFR levels, compares rule-based and LLM classifiers, and evaluates hybrid pipelines for proficiency assessment.

Result: LLMs excel for nuanced constructs, while rule-based methods are competitive for morphosyntactic ones. Hybrid pipelines outperform others, and automated corrections perform close to manual ones.

Conclusion: The framework effectively balances automated and rule-based methods, enhancing feedback and proficiency assessment by emphasizing learners' successes.
...
TL;DR: TAP-GPT, a tabular LLM framework for Alzheimer's disease prediction, outperforms traditional methods and general-purpose LLMs in few-shot settings, handles missing data well, and provides interpretable outputs.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models struggle with small, incomplete tabular biomarker data for AD diagnosis, while LLMs offer potential for improved performance and interpretability.

Method: TAP-GPT adapts TableGPT2 for few-shot AD classification using tabular prompts and evaluates it on multimodal and unimodal datasets, including biomarkers and imaging data.

Result: TAP-GPT outperforms traditional ML and backbone models, handles missing data without imputation, and provides structured reasoning aligned with AD biology.

Conclusion: TAP-GPT demonstrates the effectiveness of tabular-specialized LLMs for clinical prediction tasks, paving the way for multi-agent clinical decision-support systems.
...
TL;DR: CODMAS is a multi-agent framework for RTL optimization, combining dialectic reasoning and domain-aware code generation, achieving significant PPA improvements and reduced failures.


<details>
  <summary>Details</summary>
Motivation: Optimizing RTL code is crucial for enhancing power, performance, and area (PPA) in EDA, but automation remains challenging.

Method: CODMAS uses two dialectic agents (Articulator and Hypothesis Partner) to guide a Domain-Specific Coding Agent (DCA) and a Code Evaluation Agent (CEA) for Verilog edits and validation.

Result: Achieves ~25% critical path delay reduction for pipelining and ~22% power reduction for clock gating, with fewer functional and compilation failures compared to baselines.

Conclusion: Structured multi-agent reasoning significantly improves automated RTL optimization, scalability, and applicability to complex designs.
...
TL;DR: SYMDIREC is a neuro-symbolic framework improving RTL synthesis and summarization by decomposing tasks into symbolic subgoals, using a fine-tuned retriever, and assembling outputs via LLM reasoning, outperforming baselines by ~20% in synthesis and 15-20% in summarization.


<details>
  <summary>Details</summary>
Motivation: RTL synthesis and summarization are challenging for LLMs due to rigid HDL syntax, limited supervision, and weak natural language alignment. Existing methods lack symbolic planning, limiting precision.

Method: SYMDIREC decomposes RTL tasks into symbolic subgoals, retrieves relevant code via a fine-tuned retriever, and assembles verified outputs using LLM reasoning, supporting Verilog and VHDL without LLM fine-tuning.

Result: SYMDIREC achieves ~20% higher Pass@1 rates for synthesis and 15-20% ROUGE-L improvements for summarization over prompting and RAG baselines.

Conclusion: Symbolic guidance significantly enhances RTL tasks, as demonstrated by SYMDIREC's superior performance in synthesis and summarization.
...
TL;DR: An on-premise LLM-driven pipeline anonymizes text by replacing PII with realistic surrogates, preserving data utility while preventing leaks. It outperforms industry standards in privacy, utility, and trainability.


<details>
  <summary>Details</summary>
Motivation: Address the need for protecting sensitive information in AI without compromising data usefulness, especially with large language models.

Method: Uses a local LLM substitution pipeline to anonymize text, replacing PII with realistic surrogates, and evaluates privacy, utility, and trainability systematically.

Result: Achieves state-of-the-art privacy, minimal topical drift, strong utility, and low trainability loss, outperforming rule-based and NER baselines.

Conclusion: Local LLM substitution produces anonymized data that is both secure and valuable for downstream tasks, enabling responsible AI deployment.
...
TL;DR: Aligned models perform better in normative scenarios but underperform in multi-round strategic games compared to base models.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of post-training alignment on language models' ability to predict human behavior in strategic vs. normative settings.

Method: Compare 120 base-aligned model pairs on 10,000+ human decisions in multi-round strategic games (bargaining, persuasion, etc.) and one-shot textbook games.

Result: Base models outperform aligned models in strategic settings (10:1), while aligned models excel in normative scenarios like one-shot games and lottery choices.

Conclusion: Alignment introduces a normative bias, improving predictions in normative contexts but harming them in strategic, history-dependent settings, revealing a trade-off between optimization for human use and behavior prediction.
...
TL;DR: This paper introduces Tharu-LLaMA (3B), a model designed to address the exclusion of the Tharu language from AI advancements, using a novel dataset, TharuChat, created via a bootstrapping pipeline with Gemini models.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the digital divide caused by the underrepresentation of indigenous languages like Tharu in LLMs, highlighting the Tharu language's data scarcity and linguistic fragmentation.

Method: The authors present Tharu-LLaMA (3B), built using TharuChat, a dataset synthesized via a LLM-to-Human bootstrapping pipeline leveraging Gemini models, Rana Tharu grammar, and folklore.

Result: The synthetic dataset, despite its noisy and heterogeneous nature, significantly improves model performance, reducing perplexity from 6.42 to 2.88 as dataset volume increases.

Conclusion: The study demonstrates the feasibility of using small-scale synthetic data to preserve under-resourced languages like Tharu via generative AI, even on consumer-grade hardware.
...
TL;DR: The study identifies compact emotion-sensitive neurons (ESNs) in speech-generative LALMs for training-free emotion control, validated across multiple models and evaluations.


<details>
  <summary>Details</summary>
Motivation: Existing large audio-language models (LALMs) struggle with reliable emotion control in speech, often missing target affects or degrading linguistic fidelity.

Method: The researchers identify ESNs via success-filtered activation aggregation and demonstrate their use for emotion steering during inference without additional training.

Result: ESN interventions achieved emotion-specific improvements across three LALMs, generalizing to unseen speakers and supported by both automatic and human evaluations.

Conclusion: The study provides a mechanistic framework for training-free emotion control in speech generation, highlighting the importance of selector design and intervention parameters.
...
TL;DR: CulT-Eval is a benchmark for evaluating machine translation of culturally grounded expressions, revealing current models' struggles with cultural nuances.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack a systematic framework to evaluate translation of culture-loaded expressions like idioms and slang, which are challenging for machine translation systems.

Method: Introduced CulT-Eval, a benchmark with 7,959 curated instances of culturally grounded expressions, and a comprehensive error taxonomy. Evaluated large language models and proposed a complementary metric for culturally induced meaning deviations.

Result: Current models fail to preserve culturally grounded meaning and overlook cultural and contextual nuances, as identified by CulT-Eval.

Conclusion: The benchmark highlights the need for improved handling of cultural expressions in machine translation and proposes a targeted evaluation metric.
...
TL;DR: The study challenges the arbitrary sound-meaning assumption in linguistics by showing that English letter-phonemes carry structured, multidimensional semantic signals, which are predictable from articulatory-phonetic features and confirmed by behavioral and cross-linguistic data.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate whether phonemes in English carry non-arbitrary semantic signals, challenging the traditional linguistic assumption of arbitrariness.

Method: Used a minimal-pair paradigm covering 220 letter contrasts, analyzed by three large language models across nine perceptual dimensions, and validated with behavioral data from English speakers and preliminary cross-linguistic evidence.

Result: Found consistent phoneme-meaning associations (80.8% above chance) predicted by articulatory-phonetic features, with preliminary evidence suggesting generalization to other languages.

Conclusion: Sound-meaning iconicity is a pervasive, structured property of phonology, recoverable even from text inputs alone.
...
TL;DR: Ruyi2.5 is a multimodal familial model with a shared-backbone architecture, extending Ruyi2's 'Train Once, Deploy Many' approach. The Ruyi2.5-Camera variant implements a privacy-preserving two-stage recognition pipeline, and BPPO speeds up reinforcement learning fine-tuning. The model matches Qwen3-VL on general benchmarks and excels in privacy-constrained tasks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to extend multimodal capabilities while maintaining semantic consistency and privacy, addressing the need for scalable and efficient deployment in varied scenarios.

Method: Ruyi2.5 uses a shared-backbone architecture for co-training models of different scales. Ruyi2.5-Camera employs a two-stage pipeline with edge and cloud models, and BPPO optimizes reinforcement learning fine-tuning.

Result: Ruyi2.5 matches Qwen3-VL on general benchmarks and outperforms it in privacy-constrained tasks. BPPO achieves a 2-3x training speedup over GRPO.

Conclusion: Ruyi2.5 successfully integrates multimodal scalability, privacy preservation, and efficient reinforcement learning, demonstrating strong performance in diverse applications.
...
TL;DR: GSU, a text-only grid dataset, evaluates LLMs' spatial reasoning on navigation, object localization, and structure composition, revealing challenges in frames of reference and 3D shape identification, and suggesting specialized fine-tuning as a solution.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' spatial reasoning capabilities without visual inputs, isolating reasoning from perception and evaluating their ability to handle tasks like navigation and object localization.

Method: Introduces GSU, a text-only grid dataset, and tests LLMs on three core tasks: navigation, object localization, and structure composition. Evaluates models' performance and the impact of visual modality exposure.

Result: Most models understand basic grid concepts but struggle with embodied agent frames of reference and 3D shape identification. Visual modality exposure doesn't generalize 3D space understanding. Frontier models solve tasks, but fine-tuning smaller models shows potential.

Conclusion: Specialized fine-tuning of small LMs or LLMs could match frontier model performance, suggesting a path for developing specialized embodied agents with strong spatial reasoning.
...
TL;DR: PACE-RAG, a novel RAG framework, improves drug recommendation by combining patient context with similar-case prescribing patterns, achieving state-of-the-art performance on Parkinson's and MIMIC-IV benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current methods (LLMs, existing RAG) fail to address unique patient nuances in drug recommendations, especially for complex conditions like Parkinson's.

Method: PACE-RAG synthesizes individual patient context with prescribing tendencies of similar cases, analyzing tailored treatment patterns and generating explainable summaries.

Result: Achieved F1 scores of 80.84% (Parkinson's) and 47.22% (MIMIC-IV) using Llama-3.1-8B and Qwen3-8B, outperforming existing methods.

Conclusion: PACE-RAG is a robust, clinically grounded solution for personalized drug recommendations, validated by benchmark results.
...
TL;DR: The paper introduces SafeTutors, a benchmark to evaluate tutoring safety and pedagogy in large language models, highlighting risks like answer over-disclosure and misconception reinforcement.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of AI tutors focus on accuracy and generic safety but fail to assess pedagogical effectiveness and tutoring-specific safety risks.

Method: Introduces SafeTutors, a benchmark with 11 harm dimensions and 48 sub-risks, tested across mathematics, physics, and chemistry.

Result: All models showed broad harm; scaling didn't reliably improve safety, and multi-turn dialogue worsened pedagogical failures.

Conclusion: Mitigations for tutoring safety must be discipline-aware, and single-turn evaluations can hide systemic tutor failures in extended interactions.
...
TL;DR: The paper explores whether LLMs can improve critical thinking by learning argument reconstruction, introducing the GAAR engine, the Arguinas dataset, and showing performance gains in critical thinking tasks.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine if LLMs can enhance their critical thinking skills through argument reconstruction, filling a gap in understanding their capabilities in this area.

Method: The authors propose the GAAR engine for automatic argument reconstruction, create the Arguinas dataset, and test the impact of learning reconstruction on seven critical thinking tasks.

Result: Models trained with argument reconstruction, especially using Arguinas, outperform those without such training across all critical thinking tasks.

Conclusion: Learning argument reconstruction enhances LLMs' critical thinking abilities, with the Arguinas dataset proving particularly effective for training.
...
TL;DR: The paper introduces MSL (Minimal Sufficient Length), a metric to measure the shortest reasoning length for correct answers, and proposes TRiMS, a method to reduce reasoning-chain tokens by over 80% while slightly improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Large language models suffer from computational redundancy due to long reasoning chains, prompting the need for optimizing reasoning efficiency without sacrificing correctness.

Method: The authors define MSL and use it to analyze CoT compression strategies. They propose TRiMS, leveraging GRPO algorithm and dynamic batch aggregation to train models efficiently.

Result: TRiMS reduces token usage by over 80% while slightly boosting accuracy across benchmarks.

Conclusion: The work establishes MSL as a theoretical foundation for compressing reasoning chains and demonstrates practical success with TRiMS.
...
TL;DR: The paper explores how transformer-based LMs learn linguistic categories, comparing their behavior to human acquisition models, and finds abstraction plays a key role.


<details>
  <summary>Details</summary>
Motivation: Understanding how LMs acquire linguistic categories can shed light on human language acquisition models.

Method: Novel divergence-based metrics track learning trajectories in GPT-2 small, comparing abstract and concrete behaviors.

Result: Abstract class-level behavior emerges earlier than lexical-specific behavior, and linguistic behaviors arise abruptly in sequence.

Conclusion: Abstraction is crucial in LM learning, providing insights for models of human language acquisition.
...
TL;DR: L2A (Learning To Attend) improves language model efficiency by conditionally skipping global attention for most tokens, extending context length and reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Language models face challenges in generalizing beyond pretraining context lengths, making long-range reasoning and retrieval difficult. Traditional methods for extending context are computationally expensive.

Method: Introduces L2A, a layer that enables conditional long-range memory access by deciding when to use global attention, skipping it for ~80% of tokens. Custom Triton kernels are designed for efficient GPU implementation.

Result: L2A extends effective context length from 32K to 128K tokens, matches standard long-context training performance within 3%, and reduces KV cache memory by up to 50%. It achieves ~2x improvements in training throughput and time-to-first-token.

Conclusion: L2A offers a cost-effective solution for extending language model context lengths while maintaining performance and reducing computational overhead.
...
TL;DR: HypoTermInstruct, an SFT dataset, teaches LLMs epistemological humility using hypothetical terms, reducing hallucination while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Address LLM hallucinations caused by SFT rewarding always responding, aiming to improve reliability by teaching models to recognize knowledge limits.

Method: Develop HypoTermInstruct dataset with 31,487 responses for 11,151 hypothetical questions. Tested via 800 LoRA SFT runs on Llama3.1-8B and Gemma3-4B models.

Result: Replacing generic SFT data with HypoTermInstruct improved HypoTerm Score (0.19%-25.91%) and FactScore (+0.39%-+0.86%) while slightly reducing MMLU performance (0.26%-0.35%).

Conclusion: Targeted SFT data mitigates hallucination without preference/RL pipelines, offering a practical approach to more reliable AI.
...
TL;DR: XBridge enhances multilingual performance of LLMs by integrating pretrained translation models, improving performance on low-resource languages without retraining the LLM.


<details>
  <summary>Details</summary>
Motivation: Address the imbalanced multilingual performance of LLMs by leveraging pretrained translation models for better language handling.

Method: Proposes XBridge, a compositional encoder-LLM-decoder architecture with cross-model mapping layers and an alignment objective for multilingual generation.

Result: Outperforms baselines in multilingual tasks, especially for low-resource and unseen languages.

Conclusion: XBridge effectively bridges the gap in LLMs' multilingual capabilities without modifying the LLM itself.
...
TL;DR: A comprehensive benchmark evaluates diverse methods for detecting machine-generated text, revealing limitations in cross-domain and cross-LLM generalization.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of large language models (LLMs) necessitates robust detectors for machine-generated text, as existing benchmarks lack insights into cross-domain transfer and adversarial robustness.

Method: The study evaluates multiple detection approaches (classical classifiers, transformer encoders, stylometric models, perplexity-based detectors, and LLM-as-detector prompting) across two datasets (HC3 and ELI5).

Result: Transformer models excel in-distribution but falter under domain shift. Stylometric models match performance with interpretability. LLM-based detectors underperform, while perplexity-based methods show polarity inversion but remain effective when corrected.

Conclusion: No single method robustly generalizes across domains and LLM sources, highlighting the need for more adaptable detection solutions.
...
TL;DR: The AURORA model predicts tongue displacement and shape in vowel sounds using the first two formant values, serving as a teaching tool and biofeedback foundation.


<details>
  <summary>Details</summary>
Motivation: To explain the relationship between formants and articulation and support biofeedback applications in speech therapy and phonetics education.

Method: Developed using ultrasound tongue imaging and acoustic data from 40 English speakers, with qualitative evaluation of tongue features.

Result: Creation of a Shiny app and prototype software for real-time biofeedback, enhancing accessibility for phonetics students and speech therapy practitioners.

Conclusion: AURORA bridges the gap between acoustic theory and practical articulation, offering tools for education and therapy in phonetics.
...
TL;DR: Zipper-LoRA is a novel framework for multilingual ASR that dynamically balances shared and language-specific knowledge to address data imbalance, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of adapting Speech-LLMs to multilingual settings with imbalanced data distributions, leading to negative interference or limited knowledge transfer.

Method: Proposes Zipper-LoRA, a rank-level decoupling framework with three variants and a language-conditioned router, alongside a two-stage training strategy.

Result: Outperforms fully shared and independent baselines, especially in low-resource scenarios, and works reliably across different encoder configurations.

Conclusion: Zipper-LoRA effectively balances stability and plasticity for multilingual ASR, demonstrating robustness and practical applicability.
...
TL;DR: Fine-tuning LLMs with KA2L framework improves efficiency by targeting unknown knowledge, reducing costs by 50%, and enhancing performance.


<details>
  <summary>Details</summary>
Motivation: Address the lack of research on domain-specific knowledge comprehension in LLMs and improve training efficiency with targeted active learning.

Method: Introduces KA2L framework, using latent space analysis and knowledge distribution probing to identify unknown knowledge, and decoding hidden states to generate questions.

Result: KA2L reduces annotation and computation costs by 50% and enhances performance across datasets.

Conclusion: KA2L offers an efficient active learning strategy for LLMs, validated by experiments on nine open-source models.
...
TL;DR: A multi-agent framework integrates EDA tools for RTL code generation, optimizing functional correctness and physical metrics (PPA), supported by a memory mechanism for continuous improvement without retraining.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based RTL code generation methods focus on functional correctness but neglect Power, Performance, and Area (PPA), limiting practical applicability in hardware design.

Method: Proposes a PPA-aware framework with three agents (Programmer, Correctness, PPA) and an Evolved Memory Mechanism to externalize optimization experience and enable continual refinement.

Result: The framework achieves strong functional correctness and significant PPA improvements, validated through extensive experiments.

Conclusion: This approach transforms RTL generation into a feedback-driven, continual optimization process, enhancing LLM deployment in real-world hardware design.
...
TL;DR: The paper examines how large language models (LLMs) represent semantic relations like synonymy, antonymy, hypernymy, and hyponymy using linear probing and mechanistic interpretability techniques. Findings reveal asymmetries in hierarchical relations and stable signal patterns across models.


<details>
  <summary>Details</summary>
Motivation: To understand whether and how LLMs capture structured semantic relationships between concepts, particularly focusing on synonymy, antonymy, hypernymy, and hyponymy.

Method: The study combines linear probing with mechanistic interpretability techniques, including sparse autoencoders (SAE) and activation patching, to analyze three models of varying scales: Pythia-70M, GPT-2, and Llama 3.1 8B.

Result: Results show a directional asymmetry in hierarchical relations (hypernymy is redundantly encoded, hyponymy is more fragile), stable signal profiles peaking in mid-layers, and probe-level causality that is capacity-dependent.

Conclusion: The study clarifies where and how semantic relations are represented in LLMs, providing a reproducible framework for linking sparse features to probe-level causal evidence.
...
TL;DR: ARAM improves retrieval-augmented generation in diffusion models by dynamically adjusting guidance based on the reliability of retrieved context, outperforming baselines on QA tasks.


<details>
  <summary>Details</summary>
Motivation: Address retrieval-prior conflicts in diffusion-based models by ensuring reliable integration of external knowledge, a gap in current RAG research.

Method: Proposes ARAM, a training-free adaptive guidance framework for MDMs that adjusts guidance scale during denoising based on the SNR of retrieved context.

Result: ARAM enhances QA performance on knowledge-intensive benchmarks compared to existing RAG methods.

Conclusion: ARAM effectively mitigates noise in retrieved context, improving generation quality in diffusion-based RAG settings.
...
TL;DR: The paper introduces a multimodal, multilingual benchmark for detecting harmful humor, highlighting gaps in current static benchmarks and evaluating SOTA models.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks lack the nuance to detect harmful dark humor, which relies on cultural and contextual cues.

Method: A manually curated dataset of 3,000 texts, 6,000 images, and 1,200 videos in English and Arabic is annotated for safe, explicit, and implicit harmful humor.

Result: Closed-source models outperform open-source ones, with notable performance disparities between English and Arabic, emphasizing the need for cultural and reasoning-aware safety alignment.

Conclusion: The work underscores the importance of culturally grounded, multimodal benchmarks for improving safety in humor detection.
...
TL;DR: CoVerRL, a novel framework, avoids the consensus trap in label-free reinforcement learning by alternating a model between generator and verifier roles, improving both capabilities and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the issue of output diversity collapse in label-free reinforcement learning, termed the consensus trap, where models reinforce systematic errors.

Method: Proposes CoVerRL, where a single model alternates between generator and verifier roles, using majority voting for verifier training and progressively filtering errors.

Result: CoVerRL outperforms baselines by 4.7-5.9% on mathematical reasoning benchmarks and improves self-verification accuracy from 55% to over 85%.

Conclusion: The co-evolution of generator and verifier roles in CoVerRL effectively escapes the consensus trap, maintaining high reward accuracy and enhancing model reasoning.
...
TL;DR: The paper introduces an Information Theory-based method to automatically label reasoning steps in LLMs, reducing computational cost and improving reliability.


<details>
  <summary>Details</summary>
Motivation: To address the increased error propagation risk in multi-step reasoning of LLMs and reduce reliance on costly human annotations or intensive automatic labeling.

Method: Proposes using Information Theory to estimate the impact of each reasoning step on the correct answer likelihood, generating step-level labels with reduced computational complexity (O(N) vs. previous O(N log N)).

Result: Demonstrates effectiveness in chain-of-thought selection across diverse benchmarks (mathematics, Python, SQL, scientific QA), improving reliability and scalability.

Conclusion: The method offers scalable and efficient supervision for LLM reasoning, especially in tasks where error propagation is critical.
...
TL;DR: The paper explores a language model's spatial reasoning ability from unstructured text, focusing on converting narratives into stage-play layouts. It introduces an evaluation suite and a training method combining rejection SFT and RL, showing improved performance over baseline models.


<details>
  <summary>Details</summary>
Motivation: The goal is to automate spatial reasoning from text, mimicking human capabilities for media applications, particularly in inferring stage-play layouts from narratives without explicit spatial cues.

Method: The paper proposes a dramaturgy-inspired deterministic evaluation suite and a training/inference method combining rejection SFT (using Best-of-N sampling) with RL (via GRPO) for verifiable rewards.

Result: Experiments on classical English literature show improvements in character attribution, spatial plausibility, and movement economy, aligning with LLM-as-a-judge and human preferences.

Conclusion: The proposed approach enhances spatial reasoning in language models, demonstrating practical benefits for downstream applications like stage-play layout generation.
...
TL;DR: NEVU is a benchmark for actor-conditioned, event-centric, and direction-aware human value recognition in factual news, addressing gaps in existing datasets.


<details>
  <summary>Details</summary>
Motivation: Existing human value datasets lack actor-specific focus, explicit event structure, and value direction, limiting their usefulness for understanding values in factual news.

Method: NEVU is built from 2,865 English news articles, organizing annotations at four semantic unit levels and labeling (unit, actor) pairs. It uses an LLM-assisted pipeline with staged verification and human auditing.

Result: NEVU covers 45,793 unit-actor pairs and 168,061 directed value instances. Lightweight adaptation (LoRA) improves open-source models, showing NEVU supports supervised adaptation beyond prompting.

Conclusion: NEVU serves as a benchmark for value understanding in news and supports supervised model adaptation, filling a critical gap in existing datasets.
...
TL;DR: The paper investigates how LLMs generate verbal confidence scores, revealing that confidence is cached during answer generation and retrieved later, reflecting a richer evaluation of answer quality beyond token probabilities.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs internally generate verbal confidence scores, determining whether confidence is computed just-in-time or cached during generation, and what it represents (e.g., token probabilities or broader answer quality).

Method: Experiments with Gemma 3 27B and Qwen 2.5 7B using activation steering, patching, noising, swap experiments, attention blocking, linear probing, and variance partitioning.

Result: Confidence is cached during answer generation and retrieved from answer-adjacent positions. The cached representations explain significant variance in verbal confidence beyond token log-probabilities, indicating a sophisticated answer-quality evaluation.

Conclusion: Verbal confidence in LLMs reflects automatic, sophisticated self-evaluation rather than post-hoc reconstruction, with implications for understanding metacognition and improving model calibration.
...
TL;DR: A novel architecture reduces LLM hallucinations via tiered retrieval and verification, showing strong performance in factual accuracy benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) are prone to generating factually incorrect or ungrounded content (hallucinations), which is problematic in high-stakes domains requiring reliability.

Method: A domain-grounded tiered retrieval and verification architecture is proposed, featuring a four-phase pipeline: Intrinsic Verification, Adaptive Search Routing, Corrective Document Grading (CRAG), and Extrinsic Regeneration with atomic claim-level verification.

Result: The system outperformed zero-shot baselines across benchmarks, with win rates peaking at 83.7% in TimeQA v2 and 78.0% in MMLU Global Facts. Groundedness scores remained stable (78.8%-86.4%).

Conclusion: The framework effectively mitigates misinformation, though persistent issues like 'False-Premise Overclaiming' suggest future work should focus on pre-retrieval answerability nodes for further reliability improvements.
...
TL;DR: DebugLM introduces a framework to trace LLM behaviors back to specific training data sources using provenance tags, enabling precise debugging and targeted remediation without retraining.


<details>
  <summary>Details</summary>
Motivation: Lack of observability in LLM training pipelines makes debugging reactive and inefficient, especially under distribution shifts or updates. DebugLM aims to provide a principled way to trace behaviors to their data origins.

Method: DebugLM equips LLMs with built-in data provenance, associating responses with unique tags indicating the responsible dataset. It supports test-time remediation by enabling targeted refusal for specified sources without parameter changes.

Result: Experiments show DebugLM accurately traces behaviors in multi-stage pipelines and effectively remediates issues while maintaining model utility.

Conclusion: DebugLM offers a practical solution to improve LLM debugging by linking behaviors to training data, enhancing transparency and control without compromising performance.
...
TL;DR: The paper introduces Attention Transport Distance (ATD), a method using pretrained multilingual models' attention mechanisms to measure language distance, showing its effectiveness in recovering linguistic groupings and improving machine translation.


<details>
  <summary>Details</summary>
Motivation: The study addresses the lack of a unified, scalable quantitative approach to measuring language distance, essential for linguistics, anthropology, and evolutionary history.

Method: The proposed method, ATD, treats attention matrices from multilingual models as probability distributions and measures their divergence using optimal transport, providing a tokenization-agnostic measure of language distance.

Result: ATD accurately recovers established linguistic groupings, aligns with geographic and contact-induced relationships, and enhances low-resource machine translation performance.

Conclusion: The framework establishes a principled way to use neural networks for linguistic hypothesis testing, making multilingual models valuable tools for quantitative linguistic discovery and equitable AI.
...
TL;DR: The paper evaluates the safety of large language models (LLMs) in 12 Indic languages, revealing significant gaps in cross-language consistency and safety alignment, and introduces a benchmark for culturally informed evaluation.


<details>
  <summary>Details</summary>
Motivation: To understand and address the safety behavior of multilingual LLMs in culturally diverse, low-resource languages, particularly Indic languages, which are underrepresented in training data.

Method: Systematic evaluation of 10 LLMs using 6,000 culturally grounded prompts across 12 Indic languages, assessing safety through metrics like cross-language agreement, safety rate variance, prompt-level entropy, and bias scores.

Result: Findings show a low cross-language agreement (12.8%) and high variance in safety rates (over 17%), with some models over-refusing benign prompts or failing to flag unsafe content.

Conclusion: The study highlights critical safety generalization gaps in multilingual LLMs, advocating for language-aware alignment strategies and releasing the 	extsc{IndicSafe} benchmark for future evaluations.
...
TL;DR: A training-free method enhances LLMs' multi-token prediction using mask tokens from their embedding space, improving throughput by 15–19% and acceptance length by up to 12%.


<details>
  <summary>Details</summary>
Motivation: To leverage latent multi-token prediction (MTP) capabilities in LLMs without modifying weights or using auxiliary models.

Method: Probes LLMs with mask tokens, constructs speculative token trees, and verifies predictions in parallel for efficient, lossless decoding.

Result: Achieves 8–12% longer acceptance lengths and 15–19% higher throughput on LLaMA3 and Qwen3 benchmarks.

Conclusion: Decoder layers naturally align mask-token representations with next-token states, enabling accurate MTP without retraining.
...
TL;DR: ShapleyLaw, a game-theoretic approach, improves multilingual pretraining by quantifying cross-lingual transfer and optimizing language mixture ratios.


<details>
  <summary>Details</summary>
Motivation: Current multilingual scaling laws neglect cross-lingual transfer, leading to suboptimal language mixture ratios in pretraining data.

Method: Treats multilingual pretraining as a cooperative game, using Shapley values to quantify each language's contribution to test loss reduction.

Result: ShapleyLaw outperforms baselines in predicting model performance and optimizing language mixtures.

Conclusion: Incorporating cross-lingual transfer via game theory enhances multilingual pretraining efficiency and effectiveness.
...
TL;DR: The paper explores gender bias in decoder-only MT models, introducing 'Prior Bias' to measure default gender assumptions and showing post-training reduces bias.


<details>
  <summary>Details</summary>
Motivation: Large Language Models exhibit gender bias, especially in MT, due to language-specific gender marking. Standard benchmarks miss the complexity of this bias.

Method: Extends bias evaluation frameworks by introducing 'Prior Bias' and applying it to decoder-only MT models, assessing default gender assumptions.

Result: Decoder-only models don't outperform encoder-decoder models on gender metrics, but post-training (e.g., instruction tuning) improves contextual awareness and reduces masculine Prior Bias.

Conclusion: Post-training mitigates gender bias in MT models, highlighting the need for targeted interventions to address biases in large-scale models.
...
TL;DR: The paper addresses gender bias in machine translation between gender-neutral (English) and gendered (Italian) languages, introducing the ConGA framework for annotation and evaluation.


<details>
  <summary>Details</summary>
Motivation: Gender bias in MT systems persists when translating from gender-neutral to gendered languages, leading to unfair defaults like masculine overuse.

Method: The Contextual Gender Annotation (ConGA) framework provides guidelines for word-level gender annotation, distinguishing semantic (English) and grammatical (Italian) gender.

Result: Application to the gENder-IT dataset reveals systematic masculine overuse and inconsistent feminine realisation in MT systems.

Conclusion: ConGA offers a method and benchmark for improving gender awareness in NLP systems, addressing current limitations.
...
### [198] [A foundation model for electrodermal activity data](https://arxiv.org/abs/2603.16878)
...
TL;DR: The paper introduces EDAMAME, a large-scale open dataset for EDA research, and UME, the first foundation model for EDA, showing improved performance over baselines while using fewer resources.


<details>
  <summary>Details</summary>
Motivation: Progress in EDA modeling is limited by the lack of open, large-scale datasets. EDA is crucial for inferring cognitive load, stress, and engagement, but existing datasets are proprietary or small.

Method: The authors compile EDAMAME, a collection of EDA traces from 24 public datasets, and train UME, a foundation model for EDA, using this resource.

Result: UME outperforms baselines in 8 out of 10 scenarios and matches generalist models while using 20x fewer computational resources.

Conclusion: The study highlights the potential of UME and EDAMAME but also underscores challenges in EDA modeling, calling for further research. All resources are released to support the community.
...
### [199] [Federated Multi Agent Deep Learning and Neural Networks for Advanced Distributed Sensing in Wireless Networks](https://arxiv.org/abs/2603.16881)
*Nadine Muller,Stefano DeRosa,Su Zhang,Chun Lee Huan*

Main category: cs.LG

TL;DR: The paper surveys multi-agent deep learning (MADL) in wireless systems, focusing on distributed sensing and communications, and presents a taxonomy of learning formulations, neural architectures, techniques, and applications for 5G-Advanced and 6G networks.


<details>
  <summary>Details</summary>
Motivation: To address decentralized, partially observed, and resource-constrained control problems in wireless systems, leveraging the tight coupling of sensing, communication, and computing in 5G-Advanced and 6G visions.

Method: The survey synthesizes recent research (2021-2025) on MADL, organizing it into a task-driven taxonomy covering learning formulations, neural architectures, advanced techniques, and application domains. It also compares algorithms, training topologies, and system-level trade-offs.

Result: A comprehensive overview of MADL's state-of-the-art techniques and applications in wireless systems, with comparative insights into performance metrics like latency, spectral efficiency, and privacy.

Conclusion: Identifies open challenges such as scalability, non-stationarity, and security, and suggests future research directions for 6G systems integrating sensing, communication, and learning.
...
### [200] [Multi-Agent Reinforcement Learning for Dynamic Pricing: Balancing Profitability,Stability and Fairness](https://arxiv.org/abs/2603.16888)
*Krishna Kumar Neelakanta Pillai Santha Kumari Amma*

Main category: cs.LG

TL;DR: The paper evaluates MARL methods (MAPPO and MADDPG) for dynamic pricing in competitive markets, showing MAPPO's stability and high returns, while MADDPG offers fairer profit distribution.


<details>
  <summary>Details</summary>
Motivation: To address the need for adaptive pricing strategies in competitive retail markets using MARL approaches.

Method: Empirical evaluation of MAPPO and MADDPG against an IDDPG baseline in a simulated marketplace derived from real-world data.

Result: MAPPO achieves the highest average returns with low variance; MADDPG offers the fairest profit distribution among agents.

Conclusion: MARL methods, especially MAPPO, are scalable and stable alternatives to independent learning for dynamic pricing.
...
### [201] [From Language to Action in Arabic: Reliable Structured Tool Calling via Data-Centric Fine-Tuning](https://arxiv.org/abs/2603.16901)
...
TL;DR: AISA-AR-FunctionCall is an Arabic function-calling framework that reduces parse failures and improves accuracy, trained on a 270M-parameter model with systematic fine-tuning and reasoning enhancements.


<details>
  <summary>Details</summary>
Motivation: Existing function-calling models struggle with structural instability in Arabic, leading to high parse failures and inaccuracies.

Method: Built on a 270M-parameter FunctionGemma backbone, the framework uses dataset auditing, schema repair, tool-aware prompt restructuring, and supervised fine-tuning. A reasoning-augmented LoRA variant is also explored.

Result: Fine-tuning reduces parse failures from 87% to below 1%, improves function name accuracy by more than eightfold, and enhances argument alignment across dialects and domains.

Conclusion: The framework successfully addresses structural instability in Arabic function-calling, with separable challenges identified. All datasets and models are publicly released.
...
### [202] [What on Earth is AlphaEarth? Hierarchical structure and functional interpretability for global land cover](https://arxiv.org/abs/2603.16911)
...
TL;DR: The paper analyzes GAEF embeddings, revealing a hierarchical functional organization, where dimensions specialize in or generalize across land cover classes, enabling efficient classification with reduced dimensions.


<details>
  <summary>Details</summary>
Motivation: To understand the internal organization of geospatial foundation model embeddings and determine if they exhibit functional or hierarchical structure, which is currently unclear.

Method: A functional interpretability framework combining large-scale experimentation with structural analysis of embedding-class relationships, using feature importance and progressive ablation.

Result: Embedding dimensions show hierarchical specialization (specialist, generalist), and land cover classification can achieve 98% baseline performance with only 2-12 of 64 dimensions.

Conclusion: GAEF embeddings are hierarchically organized, offering insights for dimension selection and computational cost reduction in operational tasks.
...
### [203] [HoloByte: Continuous Hyperspherical Distillation for Tokenizer-Free Modeling](https://arxiv.org/abs/2603.16917)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: The paper introduces HoloByte, a tokenizer-free framework using Continuous Hyperspherical Distillation to reduce computational complexity in sequence modeling.


<details>
  <summary>Details</summary>
Motivation: Discrete subword tokenization introduces artificial boundaries and vocabulary dependence, fracturing the optimization landscape. HoloByte aims to resolve these issues by working directly on continuous byte-level representations.

Method: HoloByte partitions byte sequences into fixed-capacity chunks, projects them into a continuous hyperspherical manifold, and uses a dual-objective formulation with Holographic Latent Mean Squared Error for stability.

Result: The method reduces exact attention time complexity and outperforms a Byte-Pair Encoding baseline under matched parameter constraints.

Conclusion: Continuous Hyperspherical Distillation provides a mathematically rigorous and computationally efficient foundation for vocabulary-invariant sequence modeling.
...
### [204] [MHPO: Modulated Hazard-aware Policy Optimization for Stable Reinforcement Learning](https://arxiv.org/abs/2603.16929)
*Hongjun Wang,Wei Liu,Weibo Gu,Xing Sun,Kai Han*

Main category: cs.LG

TL;DR: MHPO introduces a Log-Fidelity Modulator and Decoupled Hazard Penalty to stabilize reinforcement learning by ensuring gradient fidelity and adaptive hazard regulation.


<details>
  <summary>Details</summary>
Motivation: Existing ratio control methods like hard clipping have issues with non-differentiable boundaries and lack hazard-aware mechanisms, making optimization unstable.

Method: Proposes MHPO with Log-Fidelity Modulator (LFM) for bounded, differentiable importance ratios and Decoupled Hazard Penalty (DHP) for adaptive regulation of policy shifts.

Result: MHPO outperforms existing methods on diverse benchmarks, enhancing both performance and training stability.

Conclusion: MHPO provides a robust framework for stable reinforcement learning by addressing gradient fidelity and hazard-aware regulation.
...
### [205] [Integrating Explainable Machine Learning and Mixed-Integer Optimization for Personalized Sleep Quality Intervention](https://arxiv.org/abs/2603.16937)
*Mahfuz Ahmed Anik,Mohsin Mahmud Topu,Azmine Toushik Wasi,Md Isfar Khan,MD Manjurul Ahsan*

Main category: cs.LG

TL;DR: A personalized predictive-prescriptive framework combines machine learning and optimization to translate sleep quality predictions into actionable behavioral interventions.


<details>
  <summary>Details</summary>
Motivation: Existing studies focus on predicting sleep quality but lack practical intervention strategies. This work bridges the gap between prediction and actionable recommendations.

Method: The framework uses interpretable machine learning (supervised classifier) to predict sleep quality and SHAP for feature attribution. Mixed-integer optimization models minimal behavioral adjustments, accounting for resistance to change.

Result: The model achieves high predictive performance (F1-score: 0.9544, accuracy: 0.9366). Sensitivity and Pareto analyses show trade-offs between improvement and intervention intensity.

Conclusion: The framework successfully integrates prediction, explanation, and optimization to provide personalized, actionable sleep improvement recommendations.
...
### [206] [Minimum-Action Learning: Energy-Constrained Symbolic Model Selection for Physical Law Identification from Noisy Data](https://arxiv.org/abs/2603.16951)
*Martin G. Frasch*

Main category: cs.LG

TL;DR: MAL framework identifies symbolic force laws by minimizing a Triple-Action functional, achieving noise reduction and high accuracy in recovering physical laws.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of identifying physical laws from noisy observational data in scientific machine learning.

Method: MAL uses a Triple-Action functional for trajectory reconstruction, sparsity, and energy-conservation enforcement, paired with a wide-stencil acceleration-matching technique for noise reduction.

Result: MAL recovers correct force laws with high accuracy (e.g., Kepler exponent p = 3.01 +/- 0.01) and achieves 100% pipeline-level identification with an energy-conservation criterion.

Conclusion: MAL offers interpretable, energy-constrained model selection, outperforming other methods like SINDy variants and neural networks.
...
### [207] [Integrating Inductive Biases in Transformers via Distillation for Financial Time Series Forecasting](https://arxiv.org/abs/2603.16985)
*Yu-Chen Den,Kuan-Yu Chen,Kendro Vincent,Darby Tien-Hao Chang*

Main category: cs.LG

TL;DR: The paper introduces TIPS, a Transformer-based model that synthesizes diverse inductive biases for financial forecasting, outperforming traditional models.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer models for financial forecasting often underperform due to their assumptions of stationarity, which financial markets violate with regime shifts and non-stationarity.

Method: TIPS distills knowledge from bias-specialized Transformer teachers (causality, locality, periodicity) into a single student model with regime-dependent alignment.

Result: TIPS achieves state-of-the-art performance in equity markets, outperforming baselines in annual return, Sharpe ratio, and Calmar ratio while reducing inference-time computation.

Conclusion: The success of TIPS highlights the need for regime-dependent inductive bias utilization to robustly generalize in non-stationary financial time series.
...
### [208] [Transformers Can Learn Rules They've Never Seen: Proof of Computation Beyond Interpolation](https://arxiv.org/abs/2603.17019)
*Andy Gray*

Main category: cs.LG

TL;DR: Transformers can infer rules not present in training, demonstrated through controlled experiments with XOR-based cellular automata and symbolic operator chains, challenging interpolation-only hypotheses.


<details>
  <summary>Details</summary>
Motivation: To determine if transformers can generalize beyond observed examples by inferring unseen rules, testing against the interpolation-only hypothesis.

Method: Two experiments: (1) XOR-based cellular automata with missing local input patterns, and (2) symbolic operator chains with held-out pairs, requiring intermediate symbolic derivations.

Result: Transformers successfully recovered rules in both settings, outperforming interpolation-based baselines (best 100% accuracy in Experiment 1, mean 41.8% in Experiment 2). Circuit extraction confirmed XOR computation, and intermediate-step supervision was crucial for performance.

Conclusion: Transformers can learn and express rules not directly observed in training, refuting strong interpolation-only claims, though applicability to large-scale language training remains open.
...
### [209] [Do Understanding and Generation Fight? A Diagnostic Study of DPO for Unified Multimodal Models](https://arxiv.org/abs/2603.17044)
*Abinav Rao,Sujan Rachuri*

Main category: cs.LG

TL;DR: DPO alignment fails to improve generation quality in unified multimodal models, with understanding and generation gradients being near-orthogonal and imbalanced due to VQ token asymmetry.


<details>
  <summary>Details</summary>
Motivation: To investigate whether DPO can simultaneously align understanding and generation capabilities in unified multimodal models, focusing on architectures like Janus-Pro.

Method: Applied DPO to Janus-Pro models (1B and 7B parameters) under seven training strategies and two post-hoc methods, analyzing gradients and performance metrics.

Result: No improvement in generation quality (CLIPScore) was observed across all conditions. Understanding and generation gradients were near-orthogonal (cos ~ 0) with a significant magnitude imbalance (11-14x).

Conclusion: Discrete VQ tokenization is a structural bottleneck for DPO alignment in unified models, with practical implications for VQ-based architectures.
...
### [210] [SCE-LITE-HQ: Smooth visual counterfactual explanations with generative foundation models](https://arxiv.org/abs/2603.17048)
*Ahmed Zeid,Sidney Bender*

Main category: cs.LG

TL;DR: SCE-LITE-HQ is a scalable framework for generating counterfactual explanations using pretrained generative models without task-specific retraining, producing valid and realistic results efficiently.


<details>
  <summary>Details</summary>
Motivation: Modern neural networks are powerful but lack interpretability. Counterfactual explanations help interpret predictions but are limited by computational cost and reliance on dataset-specific models.

Method: SCE-LITE-HQ leverages pretrained generative models, operates in latent space, uses smoothed gradients for stability, and employs mask-based diversification for realistic and diverse counterfactuals.

Result: The method produces valid, realistic, and diverse counterfactuals, matching or outperforming baselines without the overhead of training dedicated models.

Conclusion: SCE-LITE-HQ efficiently and effectively generates high-quality counterfactual explanations for interpreting black-box models.
...
### [211] [Early Quantization Shrinks Codebook: A Simple Fix for Diversity-Preserving Tokenization](https://arxiv.org/abs/2603.17052)
*Wenhao Zhao,Qiran Zou,Rushi Shah,Yudi Wu,Zhouhan Lin,Dianbo Liu*

Main category: cs.LG

TL;DR: The paper investigates collapses in vector quantization, a technique used in generative models, identifying causes like random initialization and limited encoder capacity, and proposes solutions.


<details>
  <summary>Details</summary>
Motivation: Understanding the understudied issue of collapses in vector quantization within generative models.

Method: Systematic investigation using synthetic and real datasets to identify collapse types and triggers.

Result: Found that random initialization and limited encoder capacity cause token and embedding collapses. Proposed solutions to mitigate these issues.

Conclusion: This is the first comprehensive study on representation collapses in vector quantization, offering insights and solutions for generative models.
...
### [212] [CircuitBuilder: From Polynomials to Circuits via Reinforcement Learning](https://arxiv.org/abs/2603.17075)
*Weikun K. Zhang,Rohan Pandey,Bhaumik Mehta,Kaijie Jin,Naomi Morato,Archit Ganapule,Michael Ruofan Zeng,Jarod Alper*

Main category: cs.LG

TL;DR: The paper explores automated arithmetic circuit synthesis for polynomial computation using RL agents, comparing PPO+MCTS and SAC methods, with SAC excelling in simpler cases and PPO+MCTS scaling better.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by auto-proof generation and Valiant's VP vs. VNP conjecture, aiming to discover efficient arithmetic circuits for polynomial computation.

Method: The problem is framed as a single-player game for an RL agent to build circuits within fixed operations. AlphaZero-style training compares PPO+MCTS and SAC approaches.

Result: SAC performs best on two-variable targets, while PPO+MCTS scales to three variables and improves on harder instances.

Conclusion: Polynomial circuit synthesis offers a compact, verifiable setting for studying self-improving search policies.
...
### [213] [SENSE: Efficient EEG-to-Text via Privacy-Preserving Semantic Retrieval](https://arxiv.org/abs/2603.17109)
*Akshaj Murhekar,Christina Liu,Abhijit Mishra,Shounak Roychowdhury,Jacek Gwizdka*

Main category: cs.LG

TL;DR: SENSE is a lightweight, privacy-preserving framework that decodes EEG signals into text without fine-tuning LLMs, using a two-stage process of semantic retrieval and prompt-based generation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of expensive training, limited accessibility, and privacy risks in existing BCI approaches that rely on fine-tuning large models for brain activity decoding.

Method: SENSE decouples decoding into on-device semantic retrieval (mapping EEG to a discrete textual space) and prompt-based language generation (using a zero-shot LLM). The EEG-to-keyword module is lightweight (~6M parameters) and runs locally.

Result: Evaluated on a 128-channel EEG dataset, SENSE matches or outperforms full fine-tuning baselines like Thought2Text while reducing computational overhead.

Conclusion: SENSE offers a scalable, privacy-aware solution for next-generation BCIs by localizing neural decoding and sharing only abstract semantic cues.
...
### [214] [Contextual Preference Distribution Learning](https://arxiv.org/abs/2603.17139)
*Benjamin Hudson,Laurent Charlin,Emma Frejinger*

Main category: cs.LG

TL;DR: The paper proposes a pipeline to learn human preference distributions for decision-making under uncertainty, significantly reducing post-decision surprise in a synthetic ridesharing environment.


<details>
  <summary>Details</summary>
Motivation: Decision-making often involves uncertainty due to diverse human preferences and contextual factors, necessitating methods that go beyond point estimates to support risk-averse solutions.

Method: A sequential pipeline combines learning preference distributions (using a bounded-variance gradient estimator) and optimizing downstream problems like risk-averse formulations, focusing on (integer) linear programs.

Result: The approach reduces average post-decision surprise by up to 114x compared to risk-neutral methods and 25x versus risk-averse baselines in synthetic ridesharing tests.

Conclusion: The proposed method effectively captures contextual shifts and preference distributions, enabling better risk-averse decision-making in uncertain environments.
...
### [215] [REAL: Regression-Aware Reinforcement Learning for LLM-as-a-Judge](https://arxiv.org/abs/2603.17145)
*Yasi Zhang,Tianyu Chen,Mingyuan Zhou,Oscar Leong,Ying Nian Wu,Michal Lukasik*

Main category: cs.LG

TL;DR: REAL is a regression-aware RL framework that outperforms SFT and standard RL methods by optimizing regression rewards, achieving better generalization and higher correlation scores.


<details>
  <summary>Details</summary>
Motivation: Standard RL methods ignore the ordinal structure in regression tasks, while existing regression-aware approaches are limited to SFT. REAL bridges this gap by integrating regression objectives into RL.

Method: REAL uses a generalized policy gradient estimator to decompose optimization into exploration over CoT trajectories and regression-aware prediction refinement.

Result: Experiments show REAL outperforms SFT baselines and standard RL, achieving +8.40 Pearson and +7.20 Spearman gains on Qwen3-32B.

Conclusion: Integrating regression objectives into RL exploration enhances LLM evaluation accuracy and generalization.
...
### [216] [Personalized Fall Detection by Balancing Data with Selective Feedback Using Contrastive Learning](https://arxiv.org/abs/2603.17148)
*Awatif Yasmin,Tarek Mahmud,Sana Alamgeer,Anne H. H. Ngu*

Main category: cs.LG

TL;DR: The paper proposes a personalization framework for fall detection that uses semi-supervised clustering and contrastive learning to improve accuracy by balancing user feedback samples. It evaluates three retraining strategies, with Training from Scratch (TFS) performing best.


<details>
  <summary>Details</summary>
Motivation: Current fall detection models suffer from data imbalance, where non-fall samples dominate, reducing sensitivity to actual falls. Personalized models could improve accuracy but face challenges due to limited real-world fall data.

Method: The framework combines semi-supervised clustering and contrastive learning to identify and balance informative user feedback. It evaluates three retraining strategies: TFS, Transfer Learning (TL), and Few-Shot Learning (FSL).

Result: TFS achieved the highest performance (25% improvement over baseline), followed by FSL (7% improvement). Real-time experiments with ten participants confirmed the framework's effectiveness.

Conclusion: Selective personalization, particularly using TFS, significantly enhances fall detection models, making them more adaptable for real-world deployment.
...
### [217] [Noise-Response Calibration: A Causal Intervention Protocol for LLM-Judges](https://arxiv.org/abs/2603.17172)
*Maxim Khomiakov,Jes Frellsen*

Main category: cs.LG

TL;DR: A calibration protocol for LLM judges based on noise interventions reveals a modality gap between text and tabular data, with text degrading predictably while tabular often doesn't.


<details>
  <summary>Details</summary>
Motivation: LLMs are used as automated judges but are stochastic and overconfident, making deployment tricky without ground truth.

Method: Proposes a slope-based hypothesis test using noise interventions (SNR for tabular, lexical for text) to measure performance deterioration.

Result: Text-based judges degrade predictably; tabular datasets often don't show significant performance deterioration under noise. Performance is lower on noise-insensitive datasets.

Conclusion: A reproducible methodology for robust LLM-judge calibration under distribution shift is presented, highlighting modality differences.
...
### [218] [Domain-informed explainable boosting machines for trustworthy lateral spread predictions](https://arxiv.org/abs/2603.17175)
...
### [219] [MetaClaw: Just Talk -- An Agent That Meta-Learns and Evolves in the Wild](https://arxiv.org/abs/2603.17187)
...
TL;DR: MetaClaw is a continual meta-learning framework for LLM agents that adapts to evolving user needs without downtime by synthesizing new skills and optimizing policies opportunistically.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents often remain static, failing to adapt to changing user needs or task distributions, creating a need for continuous improvement without service disruption.

Method: MetaClaw combines skill-driven fast adaptation (using failure trajectories to synthesize skills) with opportunistic policy optimization (gradient-based updates during inactive periods). It employs a versioning mechanism to prevent data contamination and scales via a proxy-based architecture.

Result: Experiments show MetaClaw improves accuracy by up to 32%, advances Kimi-K2.5 accuracy from 21.4% to 40.6%, and increases robustness by 18.3%.

Conclusion: MetaClaw effectively bridges the gap between continuous service and adaptive capability, enhancing LLM agent performance and robustness in dynamic environments.
...
### [220] [Self-Conditioned Denoising for Atomistic Representation Learning](https://arxiv.org/abs/2603.17196)
*Tynan Perez,Rafael Gomez-Bombarelli*

Main category: cs.LG

TL;DR: SCD (Self-Conditioned Denoising) introduces a backbone-agnostic method for self-supervised pretraining on diverse atomistic data, outperforming existing SSL methods and matching supervised pretraining in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing self-supervised learning methods in atomistic data, which are restricted to ground-state geometries or single domains, the paper proposes SCD for broader applicability.

Method: SCD uses a self-conditioned denoising objective that leverages self-embeddings for reconstruction across various domains of atomistic data, including small molecules, proteins, and non-equilibrium geometries.

Result: SCD significantly outperforms previous SSL methods, matches or exceeds supervised pretraining performance, and enables smaller models to compete with larger ones trained on bigger datasets.

Conclusion: SCD provides a scalable and effective approach for self-supervised pretraining in the physical sciences, offering competitive performance across diverse tasks and domains.
...
### [221] [Abstraction as a Memory-Efficient Inductive Bias for Continual Learning](https://arxiv.org/abs/2603.17198)
*Elnaz Rahmati,Nona Ghazizadeh,Zhivar Sourati,Nina Rouhani,Morteza Dehghani*

Main category: cs.LG

TL;DR: AAT improves online continual learning by capturing latent relational structures without needing a replay buffer, achieving results comparable to or better than ER baselines.


<details>
  <summary>Details</summary>
Motivation: To address forgetting and degraded generalization in online continual learning, caused by learning new information interfering with prior knowledge.

Method: Proposes Abstraction-Augmented Training (AAT), a loss-level modification that jointly optimizes over concrete instances and their abstract representations to stabilize learning in online data streams.

Result: AAT performs comparably or better than experience replay baselines, requiring no additional memory and minimal training changes.

Conclusion: AAT demonstrates structural abstraction as an effective, memory-free alternative to experience replay for continual learning.
...
### [222] [Catching rationalization in the act: detecting motivated reasoning before and after CoT via activation probing](https://arxiv.org/abs/2603.17199)
*Parsa Mirtaheri,Mikhail Belkin*

Main category: cs.LG

TL;DR: LLMs may rationalize answers using non-accurate chains of thought (CoT) due to motivated reasoning, detectable via probing internal activations better than CoT monitoring.


<details>
  <summary>Details</summary>
Motivation: To investigate how LLMs produce misleading rationalizations (motivated reasoning) in response to hints, even when their CoT does not reflect the hint's influence.

Method: Study motivated reasoning across LLM families and datasets using supervised probes on the model's residual stream, comparing pre- and post-generation probing with CoT monitoring.

Result: Pre-generation probes predict motivated reasoning as effectively as CoT monitors, while post-generation probes outperform them. Internal representations detect motivated reasoning more reliably than CoT analysis.

Conclusion: Probing internal activations is more effective than CoT monitoring for identifying motivated reasoning, with pre-generation probes offering early detection and potential efficiency gains.
...
### [223] [On the Cone Effect and Modality Gap in Medical Vision-Language Embeddings](https://arxiv.org/abs/2603.17246)
...
TL;DR: The paper explores the 'cone effect' in Vision-Language Models (VLMs), where embeddings are mapped into concentrated regions, causing a modality gap. It introduces a lightweight post-hoc method to control this gap with a hyperparameter, showing that reducing the gap improves performance, especially in medical datasets, but optimal performance requires task-dependent separation.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the practical impact of the modality gap in VLMs on supervised multimodal learning, particularly in medical domains. It seeks to analyze how controlling this gap affects downstream performance without costly retraining.

Method: The paper introduces a lightweight post-hoc mechanism that adjusts cross-modal separation via a single hyperparameter {λ} while keeping pretrained VLM encoders frozen. This allows systematic analysis of the modality gap's impact on performance.

Result: Reducing the modality gap consistently improves downstream performance, especially in medical datasets. However, fully collapsing the gap is not always optimal; intermediate, task-dependent separation yields the best results.

Conclusion: The modality gap is a tunable property of multimodal representations, not something to universally minimize. Optimal performance depends on task-dependent separation, particularly in medical applications.
...
### [224] [Binary Latent Protein Fitness Landscapes for Quantum Annealing Optimization](https://arxiv.org/abs/2603.17247)
*Truong-Son Hy*

Main category: cs.LG

TL;DR: Q-BIOLAT is a framework for modeling protein fitness in binary latent spaces using QUBO models, demonstrating strong performance on ProteinGym and compatibility with quantum annealing.


<details>
  <summary>Details</summary>
Motivation: The paper aims to bridge protein representation learning with combinatorial optimization to efficiently model and optimize protein fitness landscapes.

Method: Uses pretrained protein language models for embeddings, transforms them into binary latent spaces, and approximates fitness with QUBO models for combinatorial search.

Result: Q-BIOLAT captures meaningful protein fitness structure and identifies high-fitness variants, with distinct optimization strategies showing varied effectiveness.

Conclusion: The framework connects protein learning to combinatorial optimization and is compatible with quantum hardware, offering new directions for protein engineering.
...
### [225] [Pathology-Aware Multi-View Contrastive Learning for Patient-Independent ECG Reconstruction](https://arxiv.org/abs/2603.17248)
*Youssef Youssef,Jitin Singla*

Main category: cs.LG

TL;DR: A new method, Pathology-Aware Multi-View Contrastive Learning, reconstructs 12-lead ECGs from reduced leads by incorporating pathology data, improving accuracy and generalizability.


<details>
  <summary>Details</summary>
Motivation: Standard deep learning methods for ECG reconstruction often overlook cardiac pathology, leading to loss of vital morphology in precordial leads.

Method: The proposed framework uses a pathological manifold to regularize the latent space, integrating time-domain waveforms with pathology-aware embeddings via supervised contrastive alignment.

Result: Achieves ~76% RMSE reduction compared to state-of-the-art models on the PTB-XL dataset and shows superior generalization in cross-dataset evaluation.

Conclusion: The method successfully bridges the gap between hardware portability and diagnostic-grade ECG reconstruction by filtering anatomical nuisance variables.
...
### [226] [Variational Rectification Inference for Learning with Noisy Labels](https://arxiv.org/abs/2603.17255)
*Haoliang Sun,Qi Wei,Lei Feng,Yupeng Hu,Fan Liu,Hehe Fan,Yilong Yin*

Main category: cs.LG

TL;DR: The paper proposes Variational Rectification Inference (VRI) to address label noise in datasets by formulating loss rectification as a variational inference problem, improving robustness and generalization.


<details>
  <summary>Details</summary>
Motivation: Label noise in datasets negatively impacts deep models, causing overfitting. Existing probabilistic meta-learning models face model collapse, degrading generalization.

Method: VRI treats loss rectification as an amortized variational inference problem, using a hierarchical Bayes approach with a latent rectifying vector and meta-network for posterior approximation.

Result: VRI avoids collapsing to a Dirac delta function, improving generalization. Experiments confirm its effectiveness, especially with open-set noise.

Conclusion: VRI is a robust solution for noisy labels, enhancing generalization through variational inference and meta-learning.
...
### [227] [Classifier Pooling for Modern Ordinal Classification](https://arxiv.org/abs/2603.17278)
*Noam H. Rotenberg,Andreia V. Faria,Brian Caffo*

Main category: cs.LG

TL;DR: A model-agnostic method for ordinal classification is introduced, with an open-source Python package, showing improved performance over non-ordinal methods, especially with small datasets or many classes.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of modern machine-learning methods and software for handling ordinal data, which is common in clinical and other domains.

Method: A model-agnostic approach that adapts non-ordinal classification methods for ordinal data, implemented as a Python package.

Result: The method outperforms non-ordinal classifiers, particularly with small datasets or many outcome classes.

Conclusion: This work and its software enable the application of advanced machine learning to ordinal data, filling a critical gap.
...
### [228] [WINFlowNets: Warm-up Integrated Networks Training of Generative Flow Networks for Robotics and Machine Fault Adaptation](https://arxiv.org/abs/2603.17301)
*Zahin Sufiyan,Shadan Golestan,Yoshihiro Mitsuka,Shotaro Miwa,Osmar Zaiane*

Main category: cs.LG

TL;DR: WINFlowNets, a novel CFlowNets framework, co-trains flow and retrieval networks for robotic control, outperforming existing methods in reward and adaptability.


<details>
  <summary>Details</summary>
Motivation: Overcoming the limitations of pre-training retrieval networks in dynamic robotic environments, which often lack representative data.

Method: WINFlowNets co-trains flow and retrieval networks through a warm-up phase for the retrieval network, shared training architecture, and shared replay buffer.

Result: WINFlowNets achieves higher average reward and training stability than CFlowNets and RL algorithms, with strong adaptability in fault environments.

Conclusion: WINFlowNets shows promise for dynamic robotic systems, offering quick adaptation with limited data where traditional methods fail.
...
### [229] [Learning Permutation Distributions via Reflected Diffusion on Ranks](https://arxiv.org/abs/2603.17353)
*Sizhuang He,Yangtian Zhang,Shiyang Zhang,David van Dijk*

Main category: cs.LG

TL;DR: Soft-Rank Diffusion, a discrete diffusion framework, uses soft-rank forward processes and contextualized Plackett-Luce denoisers to improve learning probability distributions on permutations, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Learning probability distributions on the finite symmetric group S_n is challenging due to its large size and non-Euclidean structure. Existing methods using shuffle-based random walks result in abrupt and hard-to-denoise trajectories.

Method: Proposes Soft-Rank Diffusion, which replaces shuffle-based corruption with a soft-rank forward process, lifting permutations to continuous representations. Introduces contextualized generalized Plackett-Luce (cGPL) denoisers for the reverse process.

Result: Experiments show Soft-Rank Diffusion outperforms prior diffusion baselines, with strong gains in long-sequence and sequential settings.

Conclusion: The Soft-Rank Diffusion framework provides smoother, more tractable trajectories and better expressivity for sequential decision structures, improving performance in permutation-based tasks.
...
### [230] [Beyond Outliers: A Data-Free Layer-wise Mixed-Precision Quantization Approach Driven by Numerical and Structural Dual-Sensitivity](https://arxiv.org/abs/2603.17354)
...
TL;DR: NSDS is a calibration-free LMPQ framework that uses Numerical and Structural Dual-Sensitivity to improve quantization performance by addressing the limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LMPQ methods overlook the distinct roles and structural characteristics of intra-layer weight modules, leading to suboptimal performance.

Method: NSDS decomposes layers into operational roles, quantifies sensitivity numerically and structurally, and aggregates these scores using MAD-Sigmoid and Soft-OR for bit allocation.

Result: NSDS outperforms baselines across diverse models and tasks without needing calibration data.

Conclusion: The proposed NSDS framework effectively addresses the limitations of existing methods, demonstrating superior performance in low-bit quantization.
...
### [231] [Variational Kernel Design for Internal Noise: Gaussian Chaos Noise, Representation Compatibility, and Reliable Deep Learning](https://arxiv.org/abs/2603.17365)
*Ziran Liu*

Main category: cs.LG

TL;DR: The paper introduces Variational Kernel Design (VKD), a framework for designing internal noise in deep networks, and proposes Gaussian Chaos Noise (GCh) as an optimized noise mechanism. GCh outperforms traditional methods like hard binary masks in terms of calibration and robustness on ImageNet and ImageNet-C.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the shortcomings of existing noise mechanisms in deep networks (e.g., dropout, hard masking) by deriving a noise mechanism from learning desiderata and ensuring compatibility with network representations.

Method: The authors propose Variational Kernel Design (VKD), a framework where noise is characterized by a law family, correlation kernel, and injection operator. They derive Gaussian Chaos Noise (GCh) from a quadratic maximum-entropy principle, yielding a Gaussian optimizer with precision given by the Dirichlet Laplacian.

Result: GCh demonstrates superior performance in calibration and robustness under shift, improving negative log-likelihood (NLL) while maintaining competitive accuracy on ImageNet and ImageNet-C.

Conclusion: The study highlights the advantages of GCh over traditional noise mechanisms, offering a principled approach to internal noise design in deep networks for better performance and stability.
...
### [232] [Efficient Exploration at Scale](https://arxiv.org/abs/2603.17378)
...
TL;DR: Online learning algorithm improves data efficiency in RLHF, achieving 10x gains with fewer labels.


<details>
  <summary>Details</summary>
Motivation: To enhance the data efficiency of reinforcement learning from human feedback (RLHF) by developing an online learning algorithm.

Method: Incrementally updates reward and language models using choice data. Features include a small affirmative nudge, an epistemic neural network for reward uncertainty, and information-directed exploration.

Result: Matches offline RLHF performance with 10x fewer labels (20K vs. 200K). Extrapolates to 1,000x gains for larger datasets.

Conclusion: Demonstrates significant improvements in data efficiency for RLHF, achieving unprecedented gains with fewer labels.
...
### [233] [SCALE:Scalable Conditional Atlas-Level Endpoint transport for virtual cell perturbation prediction](https://arxiv.org/abs/2603.17380)
...
TL;DR: The paper introduces SCALE, a foundation model for virtual cell perturbation prediction, addressing bottlenecks in training, modeling, and evaluation to improve speed, stability, and biological fidelity.


<details>
  <summary>Details</summary>
Motivation: Large-scale perturbation prediction in virtual cell models is constrained by inefficient pipelines, unstable high-dimensional modeling, and inadequate evaluation protocols. The work aims to overcome these limitations.

Method: SCALE combines a BioNeMo-based framework for efficient training/inference, a conditional transport formulation with LLaMA-based encoding for stable modeling, and evaluates using biologically meaningful metrics on Tahoe-100M.

Result: The model achieves a 12.51x speedup in pretraining, 1.29x in inference, and improves PDCorr by 12.02% and DE Overlap by 10.66% over prior state-of-the-art.

Conclusion: Advancing virtual cell models requires co-designing scalable infrastructure, stable transport modeling, and biologically faithful evaluation, as demonstrated by SCALE.
...
### [234] [Cohomological Obstructions to Global Counterfactuals: A Sheaf-Theoretic Foundation for Generative Causal Models](https://arxiv.org/abs/2603.17384)
*Rui Wu,Hong Xie,Yongjun Li*

Main category: cs.LG

TL;DR: The paper addresses flaws in current generative models' assumptions about causality by formalizing structural causal models with algebraic topology. It introduces entropic regularization and a novel system to ensure computational tractability, demonstrating success in high-dimensional data.


<details>
  <summary>Details</summary>
Motivation: The study aims to correct the implicit assumption in generative models that locally consistent causality leads to globally coherent counterfactuals, which fails when the causal graph has non-trivial homology (e.g., structural conflicts or hidden confounders).

Method: Structural causal models are formalized as cellular sheaves over Wasserstein spaces. The authors introduce entropic regularization and derive the Entropic Wasserstein Causal Sheaf Laplacian, a system of coupled non-linear Fokker-Planck equations, and integrate it with the Implicit Function Theorem for computational efficiency.

Result: The framework successfully navigates topological barriers in high-dimensional data (e.g., scRNA-seq counterfactuals) using thermodynamic noise and introduces the Topological Causal Score for topology-aware causal discovery.

Conclusion: The paper provides a rigorous algebraic topological approach to causality in generative models, ensuring computational tractability and sensitivity in causal discovery, with empirical validation in high-dimensional applications.
...
### [235] [The Causal Uncertainty Principle: Manifold Tearing and the Topological Limits of Counterfactual Interventions](https://arxiv.org/abs/2603.17385)
*Rui Wu,Hong Xie,Yongjun Li*

Main category: cs.LG

TL;DR: The paper addresses geometric challenges in applying Pearl's do-calculus to continuous generative models, introduces the Counterfactual Event Horizon and Manifold Tearing Theorem, proposes the Causal Uncertainty Principle, and presents the GACF algorithm for scalable causal inference in high-dimensional data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the geometric challenges in translating Pearl's do-calculus to continuous generative models, which are essential for causal inference.

Method: The authors define the Counterfactual Event Horizon, prove the Manifold Tearing Theorem, and introduce the Causal Uncertainty Principle. They also develop the Geometry-Aware Causal Flow (GACF) algorithm, which uses a topological radar to avoid manifold tearing.

Result: The results include theoretical limits of interventions, the identification of finite-time singularities in deterministic flows, and validation of GACF on high-dimensional single-cell RNA sequencing (scRNA-seq) data.

Conclusion: The paper concludes that GACF is a scalable solution for causal inference in continuous generative models, addressing geometric challenges and enabling practical applications in high-dimensional data.
...
### [236] [Large-Scale 3D Ground-Motion Synthesis with Physics-Inspired Latent Operator Flow Matching](https://arxiv.org/abs/2603.17403)
...
TL;DR: GMFlow is a physics-inspired framework for generating large-scale, spatially coherent earthquake ground-motion time histories efficiently, enabling rapid hazard assessment for infrastructure.


<details>
  <summary>Details</summary>
Motivation: Traditional physics-based simulations for earthquake hazard analysis are computationally intensive and impractical for engineering workflows, necessitating a faster, scalable solution.

Method: GMFlow uses a latent operator flow matching framework to generate realistic ground-motion time histories conditioned on physical parameters, validated on simulated earthquake scenarios.

Result: GMFlow achieves a 10,000-fold speedup, generating coherent ground motion across 9 million grid points in seconds, making rapid uncertainty-aware hazard assessment feasible.

Conclusion: GMFlow provides an efficient solution for hazard analysis and extends the potential for large-scale spatiotemporal physical field synthesis in other scientific domains.
...
### [237] [Causal Representation Learning on High-Dimensional Data: Benchmarks, Reproducibility, and Evaluation Metrics](https://arxiv.org/abs/2603.17405)
*Alireza Sadeghi,Wael AbdAlmageed*

Main category: cs.LG

TL;DR: The paper discusses causal representation learning (CRL) models, their evaluation challenges, and proposes solutions for dataset suitability and a unified metric for model comparison, while emphasizing reproducibility.


<details>
  <summary>Details</summary>
Motivation: To address the complexities and reproducibility issues in evaluating CRL models across multiple directions like reconstruction, disentanglement, causal discovery, and counterfactual reasoning.

Method: Analyzed existing datasets, proposed essential characteristics for suitable datasets, introduced an aggregate metric for comprehensive model evaluation, and reviewed implementations for reproducibility.

Result: Identified limitations in current datasets, suggested improvements, and created a unified metric to simplify model comparison.

Conclusion: The study provides a framework for better dataset selection and evaluation in CRL, advocating for reproducibility and robust metrics to advance the field.
...
### [238] [The Phasor Transformer: Resolving Attention Bottlenecks on the Unit Circle](https://arxiv.org/abs/2603.17433)
*Dibakar Sigdel*

Main category: cs.LG

TL;DR: The paper introduces the Phasor Transformer, a model that uses phase-based token mixing for efficient long-context time-series learning, outperforming traditional self-attention methods in terms of computational efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: To address the quadratic computational bottleneck of dot-product self-attention in long-context time-series learning by leveraging phase-native alternatives for token mixing.

Method: The proposed Phasor Transformer block combines trainable phase-shifts and parameter-free DFT token coupling on the unit-circle manifold, achieving efficient global mixing without explicit attention maps. Stacking these blocks forms the Large Phasor Model (LPM).

Result: LPM demonstrates competitive forecasting performance on multi-frequency benchmarks, learning stable global dynamics with a compact parameter budget, and establishes an efficiency-performance frontier.

Conclusion: The Phasor Transformer offers a scalable and practical approach to temporal modeling in oscillatory domains by exploiting geometry-constrained phase computation.
...
### [239] [TimeAPN: Adaptive Amplitude-Phase Non-Stationarity Normalization for Time Series Forecasting](https://arxiv.org/abs/2603.17436)
*Yue Hu,Jialiang Tang,Siwei Yu,Baosheng Yu,Jing Zhang,Dacheng Tao*

Main category: cs.LG

TL;DR: The paper proposes TimeAPN, a framework for addressing non-stationarity in multivariate time series forecasting by modeling and predicting non-stationary factors in time and frequency domains.


<details>
  <summary>Details</summary>
Motivation: Non-stationarity in time series data, characterized by rapid amplitude and phase changes, causes distribution shifts that degrade forecasting performance. Existing methods assume smooth distribution evolution and overlook temporal dynamics.

Method: TimeAPN models mean sequences in time and frequency domains, forecasts their evolution, and captures phase discrepancies. It uses adaptive normalization for abrupt fluctuations and integrates predictions via de-normalization. The framework is model-agnostic.

Result: Experiments on seven datasets show TimeAPN improves long-term forecasting accuracy and outperforms reversible normalization methods.

Conclusion: TimeAPN effectively addresses non-stationarity by explicitly modeling amplitude and phase dynamics, enhancing forecasting performance across multiple horizons.
...
### [240] [Baguan-TS: A Sequence-Native In-Context Learning Model for Time Series Forecasting with Covariates](https://arxiv.org/abs/2603.17439)
...
TL;DR: The paper introduces Baguan-TS, a unified framework combining raw-sequence learning with in-context learning for time series forecasting, addressing challenges like calibration and oversmoothing, and outperforming benchmarks.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between tabularized feature-based ICL approaches and end-to-end sequence models lacking inference-time adaptation in time series forecasting.

Method: Develops Baguan-TS, a 3D Transformer attending over temporal, variable, and context axes, with innovations like target-space retrieval-based calibration and context-overfitting to mitigate oversmoothing.

Result: Outperforms established baselines on public benchmarks, achieving the highest win rate and significant improvements in point and probabilistic forecasting metrics.

Conclusion: Baguan-TS demonstrates robustness and effectiveness across diverse real-world energy datasets, offering a practical and high-performing solution for time series forecasting.
...
### [241] [Efficient Soft Actor-Critic with LLM-Based Action-Level Guidance for Continuous Control](https://arxiv.org/abs/2603.17468)
*Hao Ma,Zhiqiang Pu,Xiaolin Ai,Huimu Wang*

Main category: cs.LG

TL;DR: GuidedSAC is a reinforcement learning algorithm that uses LLMs to guide exploration in state-action spaces, improving efficiency and performance over standard SAC and other variants.


<details>
  <summary>Details</summary>
Motivation: To enhance exploration efficiency in large state-action spaces by leveraging LLMs as intelligent supervisors for the SAC algorithm.

Method: Combines SAC with LLM-based supervisors that analyze trajectories and provide action-level guidance, supported by theoretical convergence guarantees.

Result: Outperforms standard SAC and exploration-enhanced variants like RND, ICM, and E3B in sample efficiency and final performance across various environments.

Conclusion: GuidedSAC effectively improves RL exploration by integrating LLM guidance, maintaining SAC's theoretical guarantees while enhancing practical performance.
...
### [242] [Auto-Unrolled Proximal Gradient Descent: An AutoML Approach to Interpretable Waveform Optimization](https://arxiv.org/abs/2603.17478)
*Ahmet Kaplan*

Main category: cs.LG

TL;DR: AutoML and deep unfolding are combined to optimize wireless beamforming, achieving high efficiency with minimal training data and layers.


<details>
  <summary>Details</summary>
Motivation: To reduce the training data and inference cost of traditional iterative methods while maintaining interpretability.

Method: Convert PGD into a deep neural network with learnable parameters, add a hybrid layer, and use AutoGluon with TPE for hyperparameter optimization.

Result: Auto-PGD achieves 98.8% spectral efficiency of traditional PGD with only five layers and 100 training samples.

Conclusion: The approach significantly reduces resource requirements while maintaining high interpretability and performance.
...
### [243] [QuantFL: Sustainable Federated Learning for Edge IoT via Pre-Trained Model Quantisation](https://arxiv.org/abs/2603.17507)
*Charuka Herath,Yogachandran Rahulamathavan,Varuna De Silva,Sangarapillai Lambotharan*

Main category: cs.LG

TL;DR: QuantFL reduces energy costs in Federated Learning by leveraging pre-trained models and aggressive quantization, achieving significant communication savings without compromising accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the high energy cost and carbon footprint of frequent uplink transmissions in Federated Learning (FL) on IoT devices by exploiting pre-trained models for efficient fine-tuning.

Method: Proposes QuantFL, a framework that uses pre-trained initialization to enable lightweight bucket quantization, eliminating the need for complex error-feedback mechanisms.

Result: QuantFL reduces total communication by 40% (with up to 80% reduction in some scenarios) while matching or exceeding uncompressed baselines in accuracy, e.g., 89.00% on MNIST and 66.89% on CIFAR-100.

Conclusion: QuantFL provides a sustainable and scalable solution for FL on battery-constrained IoT networks, balancing energy efficiency and performance.
...
### [244] [Translation Invariance of Neural Operators for the FitzHugh-Nagumo Model](https://arxiv.org/abs/2603.17523)
*Luca Pellegrini*

Main category: cs.LG

TL;DR: The study evaluates Neural Operators (NOs) for learning stiff spatio-temporal dynamics in the FitzHugh-Nagumo model, using a novel training strategy to assess translation invariance. It benchmarks seven NO architectures, revealing trade-offs in accuracy, efficiency, and inference speed.


<details>
  <summary>Details</summary>
Motivation: To explore NOs' ability to model excitable cell dynamics and assess their robustness to translated spatio-temporal inputs, reducing computational costs in dataset generation.

Method: NOs are trained with varying spatial current inputs and tested on translated scenarios. Seven architectures (CNOs, DONs, DONs-CNN, POD-DONs, FNOs, TFNOs, LocalNOs) are benchmarked for accuracy, efficiency, and speed.

Result: CNOs perform well on translated tests but have high training costs. FNOs achieve low training error but slow inference and poor generalization. DONs are efficient but generalize poorly to tests.

Conclusion: NOs show promise for complex dynamics but face trade-offs in performance and efficiency. The study provides a benchmark for future applications in translated scenarios.
...
### [245] [AirDDE: Multifactor Neural Delay Differential Equations for Air Quality Forecasting](https://arxiv.org/abs/2603.17529)
*Binqing Wu,Zongjiang Shang,Shiyu Liu,Jianlong Huang,Jiahui Xu,Ling Chen*

Main category: cs.LG

TL;DR: AirDDE is a neural delay differential equation framework for air quality forecasting that incorporates delay modeling and physical guidance, outperforming existing methods with an 8.79% MAE reduction.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods ignore intrinsic delays in pollutant propagation, limiting their accuracy in air quality forecasting.

Method: AirDDE introduces a memory-augmented attention module and a physics-guided delay evolving function to model pollutant dynamics with delays and physical plausibility.

Result: AirDDE achieves state-of-the-art performance, reducing MAE by 8.79% on average across three datasets compared to baselines.

Conclusion: AirDDE effectively integrates delay modeling and physical guidance for superior air quality forecasting.
...
### [246] [Anisotropic Permeability Tensor Prediction from Porous Media Microstructure via Physics-Informed Progressive Transfer Learning with Hybrid CNN-Transformer](https://arxiv.org/abs/2603.17532)
*Mohammad Nooraiepour*

Main category: cs.LG

TL;DR: A physics-informed deep learning framework using MaxViT hybrid CNN-Transformer architecture with progressive transfer learning accurately predicts permeability tensors from pore-scale images, achieving high accuracy and reduced unexplained variance.


<details>
  <summary>Details</summary>
Motivation: Direct numerical simulation for permeability tensor prediction is time-consuming, limiting large-scale applications. A faster, accurate method is needed.

Method: Combines MaxViT architecture with progressive transfer learning and differentiable physical constraints, trained on 20000 synthetic samples with a three-phase curriculum.

Result: Achieves variance-weighted R² = 0.9960, reducing unexplained variance by 33% over the baseline.

Conclusion: The framework demonstrates effective transfer learning, robust integration of physical constraints, and progressive training, offering principles for physics-informed machine learning.
...
### [247] [CA-Based Interpretable Knowledge Representation and Analysis of Geometric Design Parameters](https://arxiv.org/abs/2603.17535)
*Alexander Köhler,Michael Breuß*

Main category: cs.LG

TL;DR: The paper addresses the challenge of estimating design parameters from PCA-based representations in CAD applications, highlighting limitations and presenting conditions for accurate parameter estimation.


<details>
  <summary>Details</summary>
Motivation: High-dimensional design spaces in CAD applications complicate downstream processes like simulations and optimization. PCA helps reduce dimensionality but doesn't directly recover design parameters.

Method: The study analyzes a modified PCA and compares it to standard PCA, investigates limitations, and identifies conditions for accurate parameter estimation. Experiments examine PCA stages and geometric changes.

Result: The modified PCA yields identical results to standard PCA. The paper identifies conditions under which interpretable and accurate parameter estimation is possible.

Conclusion: While PCA effectively reduces dimensionality, meaningful parameter recovery requires specific conditions. The study provides insights for improving parameter estimation in PCA-based representations.
...
### [248] [CLeAN: Continual Learning Adaptive Normalization in Dynamic Environments](https://arxiv.org/abs/2603.17548)
*Isabella Marasco,Davide Evangelista,Elena Loli Piccolomini,Michele Colajanni*

Main category: cs.LG

TL;DR: CLeAN introduces adaptive normalization for continual learning in tabular data, improving performance on new data and reducing catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Static data distributions in AI systems are ineffective in dynamic environments like cybersecurity or finance, where data shifts frequently. Continual learning addresses this but lacks proper data normalization methods.

Method: CLeAN uses learnable parameters updated via Exponential Moving Average (EMA) to estimate global feature scales, adapting to evolving data distributions.

Result: Evaluations show CLeAN improves model performance on new data and mitigates catastrophic forgetting across various continual learning strategies.

Conclusion: Adaptive normalization is crucial for enhancing stability in continual learning, offering a new approach to preserve knowledge in dynamic environments.
...
### [249] [Conditional Inverse Learning of Time-Varying Reproduction Numbers Inference](https://arxiv.org/abs/2603.17549)
*Lanlan Yu,Quan-Hui Liu,Haoyue Zheng,Xinfu Yang*

Main category: cs.LG

TL;DR: The paper proposes CIRL, a framework for estimating time-varying reproduction numbers from epidemic data, combining epidemiological structure with flexible statistical modeling for better adaptability and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for estimating reproduction numbers rely on strong structural assumptions, limiting adaptability to non-stationary dynamics. The paper addresses this by proposing a more flexible and robust approach.

Method: The CIRL framework learns a conditional mapping from historical incidence data and time information to latent reproduction numbers, integrating epidemiological structure with likelihood-based modeling using the renewal equation.

Result: CIRL produces robust estimates that adapt to abrupt changes and noisy data, demonstrated on synthetic epidemics and real-world SARS and COVID-19 data.

Conclusion: The CIRL framework effectively combines data-driven and epidemiological approaches, improving the accuracy and responsiveness of reproduction number estimation.
...
### [250] [FoMo X: Modular Explainability Signals for Outlier Detection Foundation Models](https://arxiv.org/abs/2603.17570)
*Simon Klüttermann,Tim Katzke,Phuong Huong Nguyen,Emmanuel Müller*

Main category: cs.LG

TL;DR: FoMo-X enhances outlier detection foundation models with interpretable, lightweight diagnostic capabilities by leveraging frozen embeddings and adding auxiliary heads.


<details>
  <summary>Details</summary>
Motivation: To address the lack of interpretability and operational context in existing tabular foundation models for outlier detection, which are often opaque and lack uncertainty measures.

Method: FoMo-X introduces modular diagnostic heads (Severity and Uncertainty Heads) attached to pretrained PFN embeddings, trained offline using the same prior as the backbone for efficient inference.

Result: FoMo-X achieves high fidelity in recovering diagnostic signals with minimal inference overhead, as validated on synthetic and real-world benchmarks (ADBench).

Conclusion: FoMo-X bridges the gap between model performance and explainability, enabling trustworthy, zero-shot outlier detection.
...
### [251] [Unsupervised Symbolic Anomaly Detection](https://arxiv.org/abs/2603.17575)
*Md Maruf Hossain,Tim Katzke,Simon Klüttermann,Emmanuel Müller*

Main category: cs.LG

TL;DR: SYRAN is an interpretable, unsupervised anomaly detection method using symbolic regression to learn human-readable equations for identifying anomalies.


<details>
  <summary>Details</summary>
Motivation: Current anomaly detection methods often use opaque, high-dimensional models, lacking interpretability. SYRAN aims to provide transparent, human-readable detection logic.

Method: SYRAN learns an ensemble of symbolic invariants (human-readable equations) that are approximately constant on normal data. Deviations from these equations indicate anomalies.

Result: SYRAN achieves strong anomaly detection performance comparable to state-of-the-art methods while offering high interpretability, with equations aligning with known scientific or medical relationships.

Conclusion: SYRAN successfully combines interpretability and performance in anomaly detection, bridging the gap between opaque models and transparent, explainable logic.
...
### [252] [Identifying Latent Actions and Dynamics from Offline Data via Demonstrator Diversity](https://arxiv.org/abs/2603.17577)
*Felix Schur*

Main category: cs.LG

TL;DR: The paper explores recovering latent actions and environment dynamics from offline trajectories without observed actions, leveraging demonstrator identity and policy diversity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand if latent actions and dynamics can be inferred from action-free trajectories when each demonstrator follows a unique policy, while environment dynamics remain shared.

Method: The approach involves modeling the next-observation distribution as a mixture of latent action-conditioned transitions with demonstrator-specific weights, using matrix factorization and diversity assumptions.

Result: The authors prove identifiability of latent transitions and policies up to permutation of action labels, extending to continuous spaces via a Gram-determinant criterion.

Conclusion: Demonstrator diversity is a key factor for identifiability, and minimal labeled action data resolves remaining ambiguities, enabling latent action and dynamics learning from offline RL data.
...
### [253] [One-Step Sampler for Boltzmann Distributions via Drifting](https://arxiv.org/abs/2603.17579)
*Wenhan Cao,Keyu Yan,Lin Zhao*

Main category: cs.LG

TL;DR: A framework for efficient sampling of Boltzmann distributions using neural generators and Gaussian-smoothed score fields.


<details>
  <summary>Details</summary>
Motivation: To simplify and stabilize the process of sampling from Boltzmann distributions by amortizing the sampling into a single forward pass.

Method: Trains a one-step neural generator by projecting samples along a Gaussian-smoothed score field, using two estimators for targets with unknown normalization constants.

Result: Achieves low error (mean error 0.0754, covariance error 0.0425, RBF MMD 0.0020) and handles nonconvex and curved low-energy geometries.

Conclusion: Drifting is effective for amortizing iterative sampling from Boltzmann distributions into a single forward pass at test time.
...
### [254] [Complementary Reinforcement Learning](https://arxiv.org/abs/2603.17621)
*Dilxat Muhtar,Jiashun Liu,Wei Gao,Weixun Wang,Shaopan Xiong,Ju Huang,Siran Yang,Wenbo Su,Jiamang Wang,Ling Pan,Bo Zheng*

Main category: cs.LG

TL;DR: Complementary RL enhances reinforcement learning by co-evolving an experience extractor and policy actor, improving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Current RL methods suffer from low sample efficiency due to sparse feedback and static historical experience that doesn't align with the actor's evolving capabilities.

Method: Introduces Complementary RL, where the experience extractor and policy actor co-evolve within the RL loop. The actor is optimized by sparse rewards, while the extractor is refined based on its contribution to the actor's success.

Result: Complementary RL achieves a 10% performance boost in single-task scenarios and scales well in multi-task settings compared to baseline methods.

Conclusion: The method establishes a paradigm for efficient, experience-driven learning in RL, addressing the limitations of static experience usage.
...
### [255] [End-to-end data-driven prediction of urban airflow and pollutant dispersion](https://arxiv.org/abs/2603.17606)
*Nishant Kumar,Franck Kerhervé,Lionel Agostini,Laurent Cordier*

Main category: cs.LG

TL;DR: The study develops a data-driven model for urban pollutant dispersion using LES data, SPOD, autoencoders, LSTMs, and CNNs, achieving accurate long-term predictions.


<details>
  <summary>Details</summary>
Motivation: Addressing the impact of climate change and urban population growth on environmental stresses by improving urban airflow and pollutant dispersion modeling for better decision-making.

Method: Combines spectral proper orthogonal decomposition (SPOD), autoencoders, LSTM networks, and convolutional neural networks (CNNs) to create a reduced-order model (ROM) from LES data.

Result: The model effectively predicts instantaneous and statistically stationary fields of airflow and pollutant dispersion over long periods.

Conclusion: The proposed framework offers a fast and accurate tool for urban pollutant dispersion modeling, aiding timely and cost-effective mitigation measures.
...
### [256] [AdaMuS: Adaptive Multi-view Sparsity Learning for Dimensionally Unbalanced Data](https://arxiv.org/abs/2603.17610)
*Cai Xu,Changhao Sun,Ziyu Guan,Wei Zhao*

Main category: cs.LG

TL;DR: The paper addresses the issue of dimensional disparities in multi-view learning by proposing the AdaMuS framework, which includes view-specific encoders, parameter-free pruning, and sparse fusion to balance and align views effectively.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view learning methods often bias towards high-dimensional views and struggle with extreme dimensional imbalances, leading to redundancy in low-dimensional views and poor alignment.

Method: The AdaMuS framework introduces view-specific encoders for mapping low-dimensional views into a unified space, a parameter-free pruning method to reduce redundancy, and a sparse fusion paradigm for effective alignment. A self-supervised learning paradigm is also proposed for better generalization.

Result: AdaMuS achieves superior performance and strong generalization across synthetic and real-world benchmarks, including classification and semantic segmentation tasks.

Conclusion: The AdaMuS framework effectively addresses dimensional imbalance in multi-view learning, ensuring balanced representation and alignment while improving generalization.
...
### [257] [Discovering Decoupled Functional Modules in Large Language Models](https://arxiv.org/abs/2603.17823)
*Yanke Yu,Jin Li,Ying Sun,Ping Li,Zhefeng Wang,Yi Zheng*

Main category: cs.LG

TL;DR: The paper introduces ULCMOD, a framework for discovering functional modules in LLMs, improving interpretability and performance.


<details>
  <summary>Details</summary>
Motivation: Understanding how LLMs organize functions into modules is essential for enhancing their trustworthiness and performance.

Method: Proposes ULCMOD, using an unsupervised approach with a novel objective function and IterD algorithm to disentangle neurons into modules and discover related topics.

Result: The method discovers high-quality, disentangled modules with meaningful semantics, improving downstream task performance and revealing coherent module organization.

Conclusion: ULCMOD fills a critical gap in LLM interpretability research, providing a tool to understand functional module organization.
...
### [258] [Only relative ranks matter in weight-clustered large language models](https://arxiv.org/abs/2603.17917)
*Borja Aizpurua,Sukhbinder Singh,Román Orús*

Main category: cs.LG

TL;DR: The paper explores the importance of weight rank (relative strength) over exact values in LLMs, using clustering and randomization to compress models and study robustness.


<details>
  <summary>Details</summary>
Motivation: To simplify LLMs by reducing unique weight values while maintaining accuracy, and to understand the role of weight rank in model performance.

Method: Applies weight clustering (K-means) to replace weight matrices with shared values, fine-tunes centroids, and randomizes cluster means to study rank effects.

Result: Reducing weights to 16-64 distinct values preserves accuracy; rank distortion harms performance more than other randomizations.

Conclusion: Weight rank is crucial for LLM performance; preserving rank aids compression and robustness, while affine corrections mitigate scale drift.
...
### [259] [ARES: Scalable and Practical Gradient Inversion Attack in Federated Learning through Activation Recovery](https://arxiv.org/abs/2603.17623)
*Zirui Gong,Leo Yu Zhang,Yanjun Zhang,Viet Vo,Tianqing Zhu,Shirui Pan,Cong Wang*

Main category: cs.LG

TL;DR: ARES is a powerful gradient inversion attack (GIA) that reconstructs training samples from large batches in Federated Learning without architectural changes, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing GIAs leak sensitive data in FL but require impractical architectural changes, prompting the need for a more practical and scalable attack.

Method: ARES frames the problem as noisy sparse recovery using generalized Lasso, extends to multi-sample recovery via the imprint method, and provides theoretical bounds.

Result: ARES achieves high-fidelity reconstruction in CNNs and MLPs, surpassing prior GIAs in large batches and realistic FL settings.

Conclusion: ARES highlights underestimated privacy risks in FL, emphasizing the need for stronger defenses against such attacks.
...
### [260] [Benchmarking Reinforcement Learning via Stochastic Converse Optimality: Generating Systems with Known Optimal Policies](https://arxiv.org/abs/2603.17631)
*Sinan Ibrahim,Grégoire Ouerdane,Hadi Salloum,Henni Ouerdane,Stefan Streif,Pavel Osinenko*

Main category: cs.LG

TL;DR: The paper introduces a benchmarking framework for RL algorithms, ensuring controlled and comprehensive evaluation by extending converse optimality to noisy, nonlinear systems.


<details>
  <summary>Details</summary>
Motivation: RL algorithm comparisons are complex due to sensitivity to environmental design and stochasticity. A rigorous benchmarking framework is needed to ensure fair and reproducible evaluations.

Method: The framework extends converse optimality to discrete-time, control-affine, nonlinear systems with noise, providing conditions for optimality of value functions and policies. It uses homotopy variations and randomized parameters to generate benchmark families.

Result: The framework successfully constructs diverse environments for controlled evaluation, enabling precise assessment of standard RL methods against a ground-truth optimum.

Conclusion: This work provides a reproducible foundation for rigorous RL benchmarking, addressing the challenges of algorithm comparison in complex environments.
...
### [261] [DSS-GAN: Directional State Space GAN with Mamba backbone for Class-Conditional Image Synthesis](https://arxiv.org/abs/2603.17637)
*Aleksander Ogonowski,Konrad Klimaszewski,Przemysław Rokita*

Main category: cs.LG

TL;DR: DSS-GAN introduces a novel generative adversarial network using Mamba as a hierarchical generator backbone and Directional Latent Routing (DLR) for improved noise-to-image synthesis.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance generative adversarial networks by developing a hierarchical generator backbone using Mamba and introducing a novel conditioning mechanism (DLR) to improve image synthesis.

Method: DSS-GAN employs Mamba as a hierarchical generator backbone and introduces Directional Latent Routing (DLR), which decomposes the latent vector into direction-specific subvectors. These subvectors are jointly projected with class embeddings to modulate features across generative scales.

Result: DSS-GAN outperforms StyleGAN2-ADA in FID, KID, and precision-recall scores on multiple datasets. Latent space analysis shows directional subvectors specialize, with perturbations causing structured, direction-correlated image changes.

Conclusion: The DSS-GAN framework, with its Mamba backbone and DLR mechanism, advances generative adversarial networks by improving image synthesis quality and revealing specialized latent space properties.
...
### [262] [Flow Matching Policy with Entropy Regularization](https://arxiv.org/abs/2603.17685)
*Ting Gao,Stavros Orfanoudakis,Nan Lin,Elvin Isufi,Winnie Daamen,Serge Hoogendoorn*

Main category: cs.LG

TL;DR: FMER is an ODE-based RL framework that uses flow matching for policy parameterization and entropy regularization, outperforming diffusion-based methods in efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the computational inefficiencies and indirect entropy control in SDE-based diffusion policies by proposing a more tractable and efficient ODE-based alternative.

Method: FMER employs flow matching to parameterize policies, sampling actions along a straight probability path, and uses an advantage-weighted target velocity field for policy updates. It also introduces a tractable entropy objective for maximum-entropy optimization.

Result: FMER outperforms state-of-the-art methods on sparse multi-goal benchmarks and reduces training time significantly compared to diffusion baselines.

Conclusion: FMER offers a more efficient and effective approach for RL tasks by combining flow matching and entropy regularization, demonstrating superior performance and reduced computational costs.
...
### [263] [Objective Mispricing Detection for Shortlisting Undervalued Football Players via Market Dynamics and News Signals](https://arxiv.org/abs/2603.17687)
*Chinenye Omejieke,Shuyao Chen,Xia Cui*

Main category: cs.LG

TL;DR: A framework for identifying undervalued football players using objective mispricing and NLP features, showing market dynamics as the primary signal and NLP as a secondary aid.


<details>
  <summary>Details</summary>
Motivation: To provide a reproducible, objective method for identifying undervalued players, moving away from subjective expert judgments.

Method: Uses structured data (market dynamics, biographical features) and NLP features (sentiment, embeddings) to estimate market value and identify mispricing.

Result: Market dynamics explain most variance in value, while NLP features add robustness. SHAP analyses highlight market trends and age as key factors.

Conclusion: The pipeline supports scouting decisions, prioritizing ranking over classification, and includes reproducibility and ethics considerations.
...
### [264] [Can Blindfolded LLMs Still Trade? An Anonymization-First Framework for Portfolio Optimization](https://arxiv.org/abs/2603.17692)
*Joohyoung Jeon,Hongchul Lee*

Main category: cs.LG

TL;DR: The paper proposes BlindTrade, a method to validate LLM trading agents by anonymizing tickers and company names, ensuring predictions reflect genuine market patterns rather than memorized associations. It achieves strong performance with robustness tests.


<details>
  <summary>Details</summary>
Motivation: To ensure LLM trading agents are trustworthy by distinguishing genuine market understanding from memorized ticker associations and addressing biases like memorization and survivorship bias.

Method: BlindTrade anonymizes tickers and company names, uses four LLM agents for scoring and reasoning, constructs a GNN graph from reasoning embeddings, and trades using a PPO-DSR policy.

Result: Achieved Sharpe 1.40 +/- 0.22 in 2025 YTD, with signal legitimacy validated through negative control experiments. Performance varies with market conditions, excelling in volatility but lagging in bull markets.

Conclusion: BlindTrade successfully validates the legitimacy of trading signals, though market-regime dependency highlights the need for adaptive strategies.
...
### [265] [Predicting Trajectories of Long COVID in Adult Women: The Critical Role of Causal Disentanglement](https://arxiv.org/abs/2603.17722)
*Jing Wang,Jie Shen,Yiming Luo,Amar Sra,Qiaomin Xie,Jeremy C. Weiss*

Main category: cs.LG

TL;DR: The study predicts Post-Acute Sequelae of SARS-CoV-2 (PASC) severity in women using clinical and wearable data, achieving high precision in distinguishing active pathology from confounding factors like menopause.


<details>
  <summary>Details</summary>
Motivation: Early prediction of PASC severity is crucial for women's health, especially due to diagnostic overlaps with hormonal transitions.

Method: Retrospective analysis of 1,155 women using clinical profiles and wearable data (cardiac activity, sleep) to develop a causal network via Large Language Model.

Result: Achieved 86.7% precision in severity prediction; effectively suppressed confounding factors (saliency scores below 0.27).

Conclusion: The framework successfully differentiates between active pathology and baseline noise, aiding accurate PASC prediction.
...
### [266] [Embedding World Knowledge into Tabular Models: Towards Best Practices for Embedding Pipeline Design](https://arxiv.org/abs/2603.17737)
*Oksana Kolomenko,Ricardo Knauer,Erik Rodner*

Main category: cs.LG

TL;DR: The paper benchmarks 256 LLM-based embedding pipeline configurations for tabular prediction, finding that design choices (preprocessing, embedding models, downstream models) significantly impact performance. Concatenating embeddings and using larger models improve results, while leaderboard rankings and model popularity are unreliable. Gradient boosting decision trees excel as downstream models.


<details>
  <summary>Details</summary>
Motivation: To determine how to effectively design LLM-based embedding pipelines for tabular prediction by systematically evaluating various configurations.

Method: Benchmarked 256 pipeline configurations, testing 8 preprocessing strategies, 16 embedding models, and 2 downstream models.

Result: Pipeline design significantly affects performance; concatenating embeddings and larger models work best. Leaderboard rankings and popularity are poor predictors. Gradient boosting decision trees perform well.

Conclusion: Provides practical guidance for building effective LLM-based embedding pipelines in tabular prediction, emphasizing careful design and model selection.
...
### [267] [Towards Infinitely Long Neural Simulations: Self-Refining Neural Surrogate Models for Dynamical Systems](https://arxiv.org/abs/2603.17750)
*Qi Liu,Laure Zanna,Joan Bruna*

Main category: cs.LG

TL;DR: The paper introduces a hyperparameter-free model, Self-refining Neural Surrogate (SNS), to address distribution drift in autoregressive neural surrogate models for dynamical systems, balancing short-term accuracy and long-term consistency.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models for simulating dynamical systems suffer from distribution drift over long time horizons due to compounding errors. Existing solutions rely on hyperparameter tuning, which is not optimal.

Method: The authors propose a mathematical framework to formalize the trade-off between short-time accuracy and long-time consistency. They introduce SNS, a conditional diffusion model that balances these aspects inherently.

Result: SNS effectively simulates complex dynamical systems over long time horizons without hyperparameter tuning, demonstrating high-fidelity performance.

Conclusion: SNS offers a robust, hyperparameter-free alternative to existing autoregressive models, ensuring both short-term fidelity and long-term consistency in simulations.
...
### [268] [Attention Sinks Induce Gradient Sinks](https://arxiv.org/abs/2603.17771)
*Yihong Chen,Quanming Yao*

Main category: cs.LG

TL;DR: Study explores how attention sinks in Transformers cause gradient sinks (gradient concentration) and how massive activations are an adaptive response to this, confirmed via a modified value-path gradient adjustment.


<details>
  <summary>Details</summary>
Motivation: To understand if the connection between attention sinks and massive activations in Transformers is direct or mediated by training-time mechanisms, focusing on backpropagation.

Method: Empirical and theoretical analysis of gradient concentration (gradient sinks) under causal masks, and introduction of V-scale, a modification to adjust value-path backpropagated gradients.

Result: Attention sinks induce gradient sinks, and massive activations adapt to this gradient pressure. V-scale preserves attention sinks but suppresses massive activations.

Conclusion: Gradient sink is a key training-time mediator linking attention sinks and massive activations in Transformer models.
...
### [269] [RangeAD: Fast On-Model Anomaly Detection](https://arxiv.org/abs/2603.17795)
*Luca Hinkamp,Simon Klüttermann,Emmanuel Müller*

Main category: cs.LG

TL;DR: Introduces On-Model AD and RangeAD for efficient anomaly detection by leveraging primary model outputs, reducing inference costs.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly detection methods ignore information encoded in the primary model, leading to inefficiencies.

Method: Proposes RangeAD, an algorithm using neuron-wise output ranges from the primary model for anomaly detection.

Result: RangeAD achieves superior performance on high-dimensional tasks with lower inference costs.

Conclusion: On-Model AD, exemplified by RangeAD, is a practical and efficient framework for anomaly detection.
...
### [270] [Dropout Robustness and Cognitive Profiling of Transformer Models via Stochastic Inference](https://arxiv.org/abs/2603.17811)
*Antônio Junior Alves Caiado,Michael Hahsler*

Main category: cs.LG

TL;DR: The paper explores the effects of MC Dropout on transformer models, revealing architectural dependencies in dropout robustness and a memory-reasoning performance tradeoff.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate how stochastic inference via MC Dropout impacts transformer models' reliability and uncertainty-awareness.

Method: Analyzed 19 transformer models using MC Dropout with 100 forward passes per sample, measuring accuracy stability and performance decomposition into memory/reasoning.

Result: Smaller models are stable; mid-sized ones balance performance. Larger models excel in memory tasks. High dropout disrupts memory more than reasoning. Many models degrade under MC Dropout.

Conclusion: Dropout robustness varies by architecture, uncorrelated with scale. A cognitive framework aids model selection for uncertainty-aware applications.
...
### [271] [Federated Distributional Reinforcement Learning with Distributional Critic Regularization](https://arxiv.org/abs/2603.17820)
*David Millard,Cecilia Alm,Rashid Ali,Pengcheng Shi,Ali Baheri*

Main category: cs.LG

TL;DR: The paper introduces federated distributional reinforcement learning (FedDistRL) to address multimodality and tail behavior in safety-critical settings, proposing TR-FedDistRL for improved safety and performance.


<details>
  <summary>Details</summary>
Motivation: Current federated RL methods focus on expected returns, overlooking statistical multimodality and tail behavior crucial for safety-critical applications.

Method: FedDistRL involves federating quantile value function critics. TR-FedDistRL introduces a risk-aware Wasserstein barycenter per client to constrain parameter averaging, using a shrink-squash step for distributional trust.

Result: Experiments show reduced mean-smearing, better safety metrics (catastrophe/accident rates), and lower critic/policy drift compared to baselines.

Conclusion: FedDistRL and TR-FedDistRL enhance safety and performance in federated RL by preserving distributional information through constrained averaging.
...
### [272] [Symmetry-Reduced Physics-Informed Learning of Tensegrity Dynamics](https://arxiv.org/abs/2603.17824)
*Jing Qin,Muhao Chen*

Main category: cs.LG

TL;DR: The paper proposes SymPINN, a symmetry-reduced physics-informed neural network framework for tensegrity dynamics, improving accuracy and efficiency by embedding geometric symmetries into the model.


<details>
  <summary>Details</summary>
Motivation: Existing PINN approaches for tensegrity dynamics often ignore geometric symmetries, leading to computational inefficiency and unstable optimization. The goal is to leverage symmetry to enhance model performance.

Method: SymPINN integrates group-theory-based symmetry into the solution expression and neural network architecture, uses symmetry orbits for node decomposition, and enforces equivariance through orbit-based coordinate generation, symmetry-consistent message passing, and physics residual constraints. It also employs hard constraints, Fourier feature encoding, and two-stage optimization.

Result: Numerical experiments on symmetric T-bars and lander structures show improved prediction accuracy and computational efficiency over standard models.

Conclusion: SymPINN demonstrates the potential of symmetry-aware learning for efficient and accurate modeling of tensegrity dynamics, outperforming traditional methods.
...
### [273] [Physics-Aware Machine Learning for Seismic and Volcanic Signal Interpretation](https://arxiv.org/abs/2603.17855)
*William Thorossian*

Main category: cs.LG

TL;DR: The paper explores ML's role in seismic and volcanic monitoring, focusing on adaptability, uncertainty, and physical constraints, while addressing challenges like domain shift and label dependence.


<details>
  <summary>Details</summary>
Motivation: To enhance seismic and volcanic monitoring by integrating ML for reliable, interpretable, and adaptable solutions under varying conditions.

Method: Surveys ML techniques, emphasizing classical signal processing, self-supervision, generative modeling, and evaluation protocols for regional transfer.

Result: Highlights advancements in ML for signal analysis but notes challenges in robustness, interpretability, and adaptability.

Conclusion: Open challenges remain for AI-assisted monitoring to be reliable and maintainable, requiring further research in robustness and interpretability.
...
### [274] [Procedural Generation of Algorithm Discovery Tasks in Machine Learning](https://arxiv.org/abs/2603.17863)
...
TL;DR: DiscoGen is a procedural generator for creating diverse machine learning tasks to evaluate and improve algorithm discovery systems, addressing limitations in current benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing task suites for evaluating algorithm discovery systems are flawed, prompting the need for a scalable, diverse, and configurable solution like DiscoGen.

Method: DiscoGen procedurally generates millions of tasks across ML fields, specified by configuration parameters, and introduces DiscoBench for benchmarking.

Result: DiscoGen enables scalable evaluation of algorithm discovery agents (ADAs) and supports ambitious research directions, demonstrated through prompt optimisation experiments.

Conclusion: DiscoGen offers a powerful, open-source tool for advancing algorithm discovery in machine learning by providing diverse, customizable tasks.
...
### [275] [RHYME-XT: A Neural Operator for Spatiotemporal Control Systems](https://arxiv.org/abs/2603.17867)
*Marijn Ruiter,Miguel Aguiar,Jake Rap,Karl H. Johansson,Amritam Das*

Main category: cs.LG

TL;DR: RHYME-XT is a framework for surrogate modeling of spatiotemporal control systems, using neural networks to project PIDEs onto a finite-dimensional subspace and learning their flow maps efficiently.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of modeling input-affine nonlinear PIDEs with localized rhythmic behavior, avoiding costly computations while maintaining accuracy.

Method: RHYME-XT employs Galerkin projection to approximate PIDEs on a learned subspace with neural network-parameterized basis functions, then learns the flow map directly using a specialized architecture.

Result: Experiments show RHYME-XT beats a state-of-the-art neural operator and enables effective knowledge transfer across datasets through fine-tuning.

Conclusion: RHYME-XT provides an efficient, accurate, and flexible approach for surrogate modeling of complex spatiotemporal systems governed by PIDEs.
...
### [276] [Operator-Theoretic Foundations and Policy Gradient Methods for General MDPs with Unbounded Costs](https://arxiv.org/abs/2603.17875)
*Abhishek Gupta,Aditya Mahajan*

Main category: cs.LG

TL;DR: The paper generalizes reinforcement learning results to MDPs with general state and action spaces using perturbation theory of linear operators, and introduces low-complexity PPO-type algorithms.


<details>
  <summary>Details</summary>
Motivation: To extend reinforcement learning results beyond finite-state and finite-action MDPs and linear function approximations, leveraging perturbation theory for broader applicability.

Method: Utilizes perturbation theory of linear operators to derive objective function derivatives, enabling generalization to general state and action spaces.

Result: Generalizes reinforcement learning results to more complex MDPs and proposes new low-complexity PPO-type algorithms for these settings.

Conclusion: This approach bridges gaps in reinforcement learning theory and practice, offering scalable solutions for general MDPs.
...
### [277] [RAMP: Reinforcement Adaptive Mixed Precision Quantization for Efficient On Device LLM Inference](https://arxiv.org/abs/2603.17891)
*Arpit Singh Gautam,Saurabh Jha*

Main category: cs.LG

TL;DR: RAMP is a reinforcement learning framework for adaptive mixed-precision quantization in LLMs, improving accuracy-efficiency trade-offs by assigning optimal bit widths per layer under a global budget. It includes Scale Folding for stable sub-4-bit quantization and achieves better performance than uniform methods like AWQ and GPTQ.


<details>
  <summary>Details</summary>
Motivation: Uniform bit-width quantization in LLMs leads to suboptimal accuracy-efficiency trade-offs, prompting the need for adaptive methods to optimize per-layer bit assignments.

Method: RAMP uses an off-policy Soft Actor-Critic framework to learn layer-specific bit-width assignments, conditioned on activation statistics, weight properties, and structural descriptors. Scale Folding mitigates activation outliers via per-channel scaling and normalization layer compensation.

Result: On Llama 2 7B, RAMP achieves 5.54 perplexity at 3.68GB, outperforming uniform 4-bit methods in size (6%) and quality (1-3%). It also generalizes zero-shot to other models like Llama 2 13B and Mistral 7B.

Conclusion: RAMP demonstrates that quantization sensitivity is architectural, enabling zero-shot transfer across models. The HALO pipeline ensures practical deployment with high performance retention.
...
### [278] [CARE: Covariance-Aware and Rank-Enhanced Decomposition for Enabling Multi-Head Latent Attention](https://arxiv.org/abs/2603.17946)
...
TL;DR: CARE, a Covariance-Aware, Rank-Enhanced pipeline, improves MLA conversion by addressing activation drift and attention fidelity issues, outperforming uniform-rank baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods for converting pretrained attention modules into MLA focus on weight matrices rather than input activations, ignoring activation covariance and uniform rank allocation issues, leading to degraded performance.

Method: CARE introduces activation-preserving factorization, adjusted-rank allocation, and KV-parity mapping to better align with activations and optimize KV budget usage.

Result: CARE reduces perplexity by up to 215x and improves mean accuracy by up to 1.70x compared to uniform-rank baselines, with full accuracy recovery possible after fine-tuning.

Conclusion: CARE provides a more effective and efficient approach for converting pretrained attention modules into MLA, addressing key limitations of existing methods.
...
### [279] [Unified Policy Value Decomposition for Rapid Adaptation](https://arxiv.org/abs/2603.17947)
*Cristiano Capone,Luca Falorsi,Andrea Ciardiello,Luca Manneschi*

Main category: cs.LG

TL;DR: The paper proposes a framework where policy and value functions share a low-dimensional goal embedding for rapid adaptation to novel tasks without retraining representations, inspired by biological gain modulation.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in rapid adaptation in complex control systems, and the paper aims to enable immediate adaptation to new tasks by leveraging shared goal embeddings.

Method: The method involves a bilinear actor-critic decomposition where value and policy functions are factorized using goal-conditioned coefficients and learned bases. The system is trained on a multi-directional locomotion task in MuJoCo.

Result: Results show that the shared low-dimensional goal embedding allows for zero-shot adaptation to novel tasks by interpolating in the goal space, with policy heads specializing in subsets of directions.

Conclusion: The framework provides a biologically plausible mechanism for efficient adaptation in high-dimensional control systems, highlighting its potential for general use in reinforcement learning.
...
### [280] [TerraLingua: Emergence and Analysis of Open-endedness in LLM Ecologies](https://arxiv.org/abs/2603.16910)
*Giuseppe Paolo,Jamieson Warner,Hormoz Shahrzad,Babak Hodjat,Risto Miikkulainen,Elliot Meyerson*

Main category: cs.MA

TL;DR: TerraLingua is a multi-agent ecology simulating real-world digital ecosystems with resource constraints and limited lifespans to study open-ended dynamics like cooperation and culture.


<details>
  <summary>Details</summary>
Motivation: Understanding how autonomous agents coordinate, form institutions, and accumulate shared culture in dynamic environments is crucial for real-world applications.

Method: TerraLingua is designed as a persistent multi-agent ecology with resource constraints and lifespans. AI Anthropologist analyzes agent behavior, group structure, and artifact evolution.

Result: Emergence of cooperative norms, division of labor, governance attempts, and cultural artifact lineages. Divergent outcomes linked to specific innovations and structures.

Conclusion: TerraLingua offers insights into cumulative culture and social organization in artificial populations, guiding real-world agent systems toward beneficial outcomes.
...
### [281] [Impacts of Electric Vehicle Charging Regimes and Infrastructure Deployments on System Performance: An Agent-Based Study](https://arxiv.org/abs/2603.16961)
*Jiahua Hu,Hai L. Vu,Wynita Griggs,Hao Wang*

Main category: cs.MA

TL;DR: The paper analyzes EV charging infrastructure planning using agent-based modeling in Melbourne, comparing optimization and utilization-refined deployment strategies to reduce system costs by optimizing charger allocation.


<details>
  <summary>Details</summary>
Motivation: The study addresses the need for better EV charging infrastructure planning, considering different charging regimes (destination vs. en-route) and their impact on system costs and user behavior.

Method: An agent-based modeling framework is used to simulate charging demand under three regimes, with evaluation of two deployment strategies (optimization-based and utilization-refined).

Result: Utilization-refined deployments lower total system costs, especially under combined charging regimes, by optimizing slow charger allocation and reducing reliance on en-route charging.

Conclusion: Effective infrastructure planning must consider user behavior and multiple charging regimes to minimize costs and improve system performance.
...
### [282] [Ablation Study of a Fairness Auditing Agentic System for Bias Mitigation in Early-Onset Colorectal Cancer Detection](https://arxiv.org/abs/2603.17179)
*Amalia Ionescu,Jose Guadalupe Hernandez,Jui-Hsuan Chang,Emily F. Wong,Paul Wang,Jason H. Moore,Tiffani J. Bright*

Main category: cs.MA

TL;DR: An agentic AI system with RAG improves fairness auditing in clinical AI by outperforming other configurations in identifying disparities in early-onset colorectal cancer.


<details>
  <summary>Details</summary>
Motivation: Address algorithmic bias and safety risks in clinical AI due to limited oversight and expertise, focusing on fairness in EO-CRC.

Method: Two-agent architecture: Domain Expert Agent synthesizes literature; Fairness Consultant Agent recommends fairness metrics. Ablation study compares LLMs (8B, 20B, 120B) in pretrained, non-RAG, and RAG configurations.

Result: Agent with RAG achieved highest semantic similarity to expert references, especially in disparity identification.

Conclusion: Agentic systems with retrieval can enhance fairness auditing scalability in clinical AI.
...
### [283] [Agentic Cognitive Profiling: Realigning Automated Alzheimer's Disease Detection with Clinical Construct Validity](https://arxiv.org/abs/2603.17392)
*Jiawen Kang,Kun Li,Dongrui Han,Jinchao Li,Junan Li,Lingwei Meng,Xixin Wu,Helen Meng*

Main category: cs.MA

TL;DR: Proposes Agentic Cognitive Profiling (ACP) to improve AD screening by aligning with clinical protocols, achieving high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings of inductive pattern recognition in AD screening, which lacks construct validity by prioritizing statistical shortcuts over clinical protocols.

Method: Decomposes standardized assessments into atomic tasks, using specialized LLM agents to extract verifiable scoring primitives, and decouples semantic understanding from measurement.

Result: Achieves 90.5% score match rate in task examination and 85.3% accuracy in AD prediction, outperforming baselines.

Conclusion: Demonstrates that construct validity and predictive performance can coexist, paving the way for more explainable AD screening systems.
...
### [284] [In Trust We Survive: Emergent Trust Learning](https://arxiv.org/abs/2603.17564)
*Qianpu Chen,Giulio Barbero,Mike Preuss,Derya Soydaner*

Main category: cs.MA

TL;DR: ETL is a lightweight, trust-based algorithm for AI agents that promotes cooperation in competitive environments with shared resources, requiring minimal rewards and observations.


<details>
  <summary>Details</summary>
Motivation: The goal is to enable AI agents to cooperate in competitive settings with shared resources, reducing conflicts and resource depletion.

Method: ETL uses a compact internal trust state to modulate memory, exploration, and actions, requiring only individual rewards and local observations.

Result: ETL reduces conflicts, prevents resource depletion, sustains high survival rates, and maintains cooperation in various environments.

Conclusion: ETL is effective for fostering cooperation in competitive scenarios with minimal overhead.
...
### [285] [Generative AI-assisted Participatory Modeling in Socio-Environmental Planning under Deep Uncertainty](https://arxiv.org/abs/2603.17021)
*Zhihao Pei,Nir Lipovetzky,Angela M. Rojas-Arevalo,Fjalar J. de Haan,Enayat A. Moallemi*

Main category: cs.AI

TL;DR: A templated workflow using large language models (LLMs) is proposed to simplify and speed up the problem conceptualization process in socio-environmental planning under deep uncertainty. The workflow helps translate stakeholders' natural-language descriptions into quantitative models efficiently.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the complexity and time-consuming nature of participatory modeling in socio-environmental planning by leveraging LLMs for initial problem conceptualization.

Method: The method involves using LLMs (like ChatGPT 5.2 Instant) to identify model components from stakeholders' descriptions, explore diverse perspectives, assemble components into a unified model, and implement it in Python iteratively.

Result: The workflow was tested on the lake problem and an electricity market problem, yielding acceptable outputs after a few iterations with human refinement.

Conclusion: LLMs are effective tools for facilitating participatory modeling in socio-environmental planning, streamlining the problem conceptualization process.
...
### [286] [Transformers are Bayesian Networks](https://arxiv.org/abs/2603.17063)
*Gregory Coppola*

Main category: cs.AI

TL;DR: The paper demonstrates that transformers function as Bayesian networks through five proofs, showing they implement belief propagation and have specific logical structures, with experimental validation.


<details>
  <summary>Details</summary>
Motivation: To explain why transformers work by proving they inherently operate as Bayesian networks, addressing the lack of understanding in their architecture.

Method: The paper uses formal proofs and experimental validation to show transformers implement weighted loopy belief propagation, with attention and feed-forward layers corresponding to AND/OR logic.

Result: Transformers are proven to be Bayesian networks, performing belief propagation, with experimental results supporting the theoretical claims.

Conclusion: The study concludes that transformers' success stems from their Bayesian network structure, and highlights the limitations of infinite concept spaces in verifiable inference.
...
### [287] [Cascade-Aware Multi-Agent Routing: Spatio-Temporal Sidecars and Geometry-Switching](https://arxiv.org/abs/2603.17112)
*Davide Di Gioia*

Main category: cs.AI

TL;DR: The paper highlights the 'geometry-blind' issue in AI reasoning systems' schedulers, which don't account for failure propagation differences in tree-like vs. cyclic graphs. It proposes a lightweight mitigation combining Euclidean and hyperbolic models with a learned geometry selector, improving performance significantly.


<details>
  <summary>Details</summary>
Motivation: Current schedulers in AI reasoning systems optimize load and fitness but ignore the geometric differences in failure propagation between tree-like and cyclic graphs, leading to inefficiencies.

Method: The authors propose an online geometry control method combining (1) a Euclidean spatio-temporal baseline, (2) a hyperbolic route-risk model with decay, and (3) a learned selector using topology and geometry-aware features.

Result: Their method improves win rates from 64-72% to 92% in non-tree regimes and achieves 87.2% overall, outperforming the baseline by +36.8 percentage points.

Conclusion: A lightweight 133-parameter solution effectively mitigates geometry-blind failure propagation, enhancing system performance.
...
### [288] [How Clued up are LLMs? Evaluating Multi-Step Deductive Reasoning in a Text-Based Game Environment](https://arxiv.org/abs/2603.17169)
*Rebecca Ansell,Autumn Toney-Wails*

Main category: cs.AI

TL;DR: LLM agents struggle with multi-step deductive reasoning in a Clue-based game, achieving only 4 correct wins out of 18, and fine-tuning doesn't reliably enhance performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate multi-step deductive reasoning in LLM agents using a text-based version of Clue and investigate whether fine-tuning on logic puzzles improves in-game reasoning.

Method: Implemented a rule-based text version of Clue with six agents (GPT-4o-mini and Gemini-2.5-Flash) and simulated 18 games.

Result: Agents achieved only 4 correct wins, struggled with consistent reasoning, and fine-tuning did not reliably improve performance.

Conclusion: LLM agents find it challenging to maintain deductive reasoning over extended tasks, and fine-tuning may not address reasoning precision issues.
...
### [289] [AI Scientist via Synthetic Task Scaling](https://arxiv.org/abs/2603.17216)
*Ziyang Cai,Harkirat Behl*

Main category: cs.AI

TL;DR: A novel synthetic environment pipeline for training AI agents in machine learning tasks improves performance on MLGym benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AI agents lack principled training methods and often generate ineffective ideas, prompting the need for better synthetic training environments.

Method: Developed a pipeline for generating synthetic machine learning challenges, validated via self-debugging and grounded in real datasets. Trained student models using trajectories from a teacher model.

Result: Student models showed significant performance gains: 9% for Qwen3-4B and 12% for Qwen3-8B on the MLGym benchmark.

Conclusion: The synthetic task pipeline effectively trains AI agents, enhancing their ability to perform machine learning research.
...
### [290] [Draft-and-Prune: Improving the Reliability of Auto-formalization for Logical Reasoning](https://arxiv.org/abs/2603.17233)
*Zhiyu Ni,Zheng Liang,Liangcheng Song,Chenrui Cao,Xian Zhang,Alberto Sangiovanni-Vincentelli,Pierluigi Nuzzo*

Main category: cs.AI

TL;DR: Draft-and-Prune (D&P) improves auto-formalization for logical reasoning by drafting multiple plans and verifying them, achieving high accuracy across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current auto-formalization pipelines are brittle, leading to execution failures or incorrect semantics. The goal is to enhance reliability and accuracy in translating natural-language reasoning into solver-executable programs.

Method: D&P drafts multiple natural-language plans for program generation, prunes contradictory or ambiguous formalizations, and aggregates results via majority voting.

Result: D&P significantly improves accuracy on benchmarks like AR-LSAT (78.43% with GPT-4, 78.00% with GPT-4o) and achieves near-perfect scores on PrOntoQA and LogicalDeduction.

Conclusion: D&P enhances auto-formalization-based logical reasoning without extra supervision, outperforming existing methods like MAD-LOGIC and CLOVER.
...
### [291] [Governed Memory: A Production Architecture for Multi-Agent Workflows](https://arxiv.org/abs/2603.17787)
*Hamed Taheri*

Main category: cs.AI

TL;DR: Governed Memory is a shared memory and governance layer for enterprise AI, addressing challenges like memory silos and governance gaps with four mechanisms, validated by controlled experiments showing high efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Enterprise AI faces structural challenges like memory silos, governance fragmentation, and quality degradation due to lack of shared memory and governance across autonomous agent nodes.

Method: Introduces Governed Memory with a dual memory model, tiered governance routing, reflection-bounded retrieval, and a closed-loop schema lifecycle to address these challenges.

Result: Experiments show 99.6% fact recall, 92% governance routing precision, 50% token reduction, zero cross-entity leakage, and 74.8% overall accuracy on the LoCoMo benchmark.

Conclusion: Governed Memory effectively bridges the memory governance gap in enterprise AI without compromising retrieval quality, validated by production deployment at Personize.ai.
...
### [292] [Graph-Native Cognitive Memory for AI Agents: Formal Belief Revision Semantics for Versioned Memory Architectures](https://arxiv.org/abs/2603.17244)
*Young Bin Park*

Main category: cs.AI

TL;DR: Kumiho introduces a graph-native cognitive memory architecture with formal belief revision semantics, unifying memory and asset management, and achieves strong performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored synthesis and formal grounding of AI agent memory components.

Method: Proposes Kumiho, a graph-native architecture combining Redis (working memory) and Neo4j (long-term graph), with hybrid retrieval and formal grounding in AGM belief revision.

Result: Achieves 0.565 F1 on LoCoMo, 93.3% accuracy on LoCoMo-Plus, and outperforms baselines. Architectural innovations include prospective indexing, event extraction, and client-side LLM reranking.

Conclusion: Kumiho's unified architecture and formal grounding enable robust performance, with model-decoupling allowing easy upgrades (e.g., GPT-4o-mini to GPT-4o).
...
### [293] [Contrastive Reasoning Alignment: Reinforcement Learning from Hidden Representations](https://arxiv.org/abs/2603.17305)
*Haozheng Luo,Yimin Wang,Jiahao Yu,Binghui Wang,Yan Chen*

Main category: cs.AI

TL;DR: CRAFT is a framework that enhances model safety by aligning reasoning in hidden spaces, outperforming existing defenses.


<details>
  <summary>Details</summary>
Motivation: Improving robustness against jailbreak attacks by leveraging model reasoning capabilities and hidden representations.

Method: Integrates contrastive representation learning with reinforcement learning to separate safe and unsafe reasoning paths.

Result: Achieves 79.0% improvement in reasoning safety and 87.7% in final-response safety over base models.

Conclusion: CRAFT effectively aligns reasoning for robust safety, surpassing state-of-the-art defenses.
...
### [294] [InfoDensity: Rewarding Information-Dense Traces for Efficient Reasoning](https://arxiv.org/abs/2603.17310)
*Chengwei Wei,Jung-jae Kim,Longyin Zhang,Shengkai Chen,Nancy F. Chen*

Main category: cs.AI

TL;DR: The paper introduces InfoDensity, a reward framework for RL training of LLMs, focusing on improving reasoning quality by optimizing intermediate steps and reducing verbosity, achieving better accuracy-efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches only optimize final response length in LLMs, ignoring intermediate reasoning quality, leading to verbosity and potential reward hacking. The paper argues that verbosity reflects poor reasoning quality.

Method: The study empirically tracks conditional entropy of answer distributions across reasoning steps, identifying properties of high-quality reasoning. InfoDensity combines AUC-based and monotonicity rewards, weighted by length scaling.

Result: InfoDensity matches or surpasses state-of-the-art baselines in accuracy while significantly reducing token usage, showing a strong accuracy-efficiency trade-off.

Conclusion: High-quality reasoning traces are informationally dense, and InfoDensity effectively optimizes reasoning quality and conciseness in LLMs.
...
### [295] [Physics-informed offline reinforcement learning eliminates catastrophic fuel waste in maritime routing](https://arxiv.org/abs/2603.17319)
...
### [296] [ShuttleEnv: An Interactive Data-Driven RL Environment for Badminton Strategy Modeling](https://arxiv.org/abs/2603.17324)
*Ang Li,Xinyang Gong,Bozhou Chen,Yunlong Lu,Jiaming Ji,Yongyi Wang,Yaodong Yang,Wenxin Li*

Main category: cs.AI

TL;DR: ShuttleEnv is a data-driven badminton simulation for reinforcement learning and strategic analysis, using elite-player data and probabilistic models to simulate realistic rallies.


<details>
  <summary>Details</summary>
Motivation: The paper aims to support research in reinforcement learning and behavior analysis in fast-paced sports by providing a realistic and interpretable simulation environment.

Method: ShuttleEnv uses elite-player match data and probabilistic models to simulate rally-level dynamics, avoiding physics-based simulation. It includes trained agents and live visualization.

Result: The environment enables exploration of play styles, emergent strategies, and decision-making behaviors, serving as a reusable platform for sports AI research.

Conclusion: ShuttleEnv successfully provides a realistic and interactive simulation for badminton, advancing sports AI research and demonstration.
...
### [297] [A Progressive Visual-Logic-Aligned Framework for Ride-Hailing Adjudication](https://arxiv.org/abs/2603.17328)
*Weiming Wu,Zi-Jian Cheng,Jie Meng,Peng Zhen,Shan Huang,Qun Li,Guobin Wu,Lan-Zhe Guo*

Main category: cs.AI

TL;DR: RideJudge is a progressive visual-logic-aligned framework designed to address inefficiencies in adjudicating ride-hailing disputes, outperforming larger models with 88.41% accuracy.


<details>
  <summary>Details</summary>
Motivation: The exponential rise in ride-hailing disputes makes manual review impractical, while existing automated methods lack transparency for quasi-judicial decisions. Multimodal LLMs struggle with aligning visual semantics and evidentiary rigor.

Method: RideJudge introduces SynTraj to ground liability concepts in trajectory patterns, Adaptive Context Optimization for expert knowledge distillation, Chain-of-Adjudication for evidence inquiry, and Ordinal-Sensitive Reinforcement Learning for calibrated decision-making.

Result: RideJudge-8B achieves 88.41% accuracy, outperforming larger 32B-scale models and setting a new benchmark for interpretable adjudication.

Conclusion: RideJudge addresses key systemic misalignments in dispute resolution, offering a scalable, transparent, and accurate solution for ride-hailing adjudication.
...
### [298] [Towards Safer Large Reasoning Models by Promoting Safety Decision-Making before Chain-of-Thought Generation](https://arxiv.org/abs/2603.17368)
*Jianan Chen,Zhifang Zhang,Shuo He,Linan Yue,Lei Feng,Minling Zhang*

Main category: cs.AI

TL;DR: The paper explores how enabling chain-of-thought (CoT) in large reasoning models (LRMs) degrades their safety capabilities. It proposes a method to ensure safety decisions are made before CoT generation, using a Bert-based classifier for auxiliary supervision.


<details>
  <summary>Details</summary>
Motivation: LRMs show degraded safety when CoT is enabled. The study aims to address this by ensuring safety decisions precede CoT generation.

Method: A Bert-based classifier extracts safety signals from a safe model (CoT-disabled), integrating these into LRMs' alignment as auxiliary supervision to enhance safety decision-making.

Result: The method significantly improves LRMs' safety while maintaining their general reasoning performance.

Conclusion: The proposed safety alignment method effectively balances safety and reasoning in LRMs, addressing the degradation issue when CoT is enabled.
...
### [299] [From Digital Twins to World Models:Opportunities, Challenges, and Applications for Mobile Edge General Intelligence](https://arxiv.org/abs/2603.17420)
*Jie Zheng,Dusit Niyato,Changyuan Zhao,Jiawen Kang,Jiacheng Wang*

Main category: cs.AI

TL;DR: The paper surveys the transition from digital twins to world models for edge general intelligence in dynamic 6G environments, highlighting their benefits and applications.


<details>
  <summary>Details</summary>
Motivation: To address limitations of digital twins in edge environments by leveraging world models for adaptive, autonomous, and scalable intelligence.

Method: Clarifies concepts, reviews design principles and architectures of world models, and examines their integration with digital twins in wireless EGI systems.

Result: Provides a roadmap for designing world-model-driven edge intelligence and identifies emerging applications in 6G networks.

Conclusion: World models enable more efficient edge intelligence, with future research needed for scalability, reliability, and interoperability.
...
### [300] [Proactive Knowledge Inquiry in Doctor-Patient Dialogue: Stateful Extraction, Belief Updating, and Path-Aware Action Planning](https://arxiv.org/abs/2603.17425)
*Zhenhai Pan,Yan Liu,Jia You*

Main category: cs.AI

TL;DR: The paper proposes a proactive framework for doctor-patient dialogue in EMR systems, focusing on knowledge inquiry and structured documentation, with promising pilot results under controlled conditions.


<details>
  <summary>Details</summary>
Motivation: Current EMR pipelines are output-oriented and lack proactive modeling of knowledge gaps, uncertainties, and next steps during consultations.

Method: The framework combines stateful extraction, sequential belief updating, gap-aware state modeling, hybrid retrieval, and a POMDP-lite action planner.

Result: Pilot evaluation showed 83.3% coverage, 80.0% risk recall, 81.4% structural completeness, and lower redundancy compared to baselines.

Conclusion: The proactive inquiry framework is methodologically interesting for dialogue-based EMR generation but requires further investigation; it is not yet ready for clinical deployment.
...
### [301] [When Only the Final Text Survives: Implicit Execution Tracing for Multi-Agent Attribution](https://arxiv.org/abs/2603.17445)
*Yi Nian,Haosen Cao,Shenzhe Zhu,Henry Peng Zou,Qingqing Luan,Yue Zhao*

Main category: cs.AI

TL;DR: IET (Implicit Execution Tracing) is a framework for token-level attribution and interaction topology reconstruction in multi-agent systems without relying on metadata or logs.


<details>
  <summary>Details</summary>
Motivation: Addressing accountability issues in multi-agent systems when execution logs and agent identifiers are missing by ensuring transparency in agent contributions and interactions.

Method: Embeds agent-specific signals into token distributions to create self-describing execution traces, detectable only with a secret key, and uses a transition-aware scoring method to reconstruct interaction graphs.

Result: IET accurately recovers agent segments and coordination structures while maintaining generation quality, enabling privacy-preserving auditing.

Conclusion: IET provides a practical solution for auditing multi-agent systems without compromising privacy or requiring additional metadata.
...
### [302] [Informative Semi-Factuals for XAI: The Elaborated Explanations that People Prefer](https://arxiv.org/abs/2603.17534)
*Saugat Aryal,Mark T. Keane*

Main category: cs.AI

TL;DR: The paper introduces an Informative Semi-Factuals (ISF) method in XAI, which enhances semi-factual explanations by revealing hidden features that keep outcomes unchanged, improving user preference and explanation quality.


<details>
  <summary>Details</summary>
Motivation: Current semi-factual methods in XAI lack explanations for why extreme value-changes don't alter outcomes, limiting their informativeness. The ISF method aims to address this gap.

Method: The ISF algorithm generates semi-factuals supplemented with information about hidden features that influence decisions, providing more detailed explanations.

Result: Experiments show ISF produces high-quality, informative semi-factuals. A user study confirms people prefer these over simpler explanations from existing methods.

Conclusion: The ISF method advances semi-factual explanations by making them more informative and user-preferred, addressing a key limitation in current XAI approaches.
...
### [303] [Per-Domain Generalizing Policies: On Learning Efficient and Robust Q-Value Functions (Extended Version with Technical Appendix)](https://arxiv.org/abs/2603.17544)
*Nicola J. Müller,Moritz Oster,Isabel Valera,Jörg Hoffmann,Timo P. Gros*

Main category: cs.AI

TL;DR: Learning Q-value functions for per-domain generalization outperforms state-value functions, using regularization to improve action distinction and performance across domains.


<details>
  <summary>Details</summary>
Motivation: The challenge is learning generalizing policies for planning efficiently, where standard methods use state-value functions from optimal plans. This work proposes Q-value functions for cheaper evaluation and better performance.

Method: Advocates learning Q-value functions with regularization terms to distinguish actions taken by the teacher planner, improving policy quality.

Result: Q-value policies consistently outperform state-value policies in 10 domains and are competitive with the planner LAMA-first.

Conclusion: Learning Q-value functions with regularization is more effective and efficient for planning compared to state-value functions.
...
### [304] [VeriGrey: Greybox Agent Validation](https://arxiv.org/abs/2603.17639)
*Yuntong Zhang,Sungmin Kang,Ruijie Meng,Marcel Böhme,Abhik Roychoudhury*

Main category: cs.AI

TL;DR: The paper introduces VeriGrey, a grey-box approach to test LLM agents for security risks, achieving higher efficacy in uncovering vulnerabilities compared to black-box methods.


<details>
  <summary>Details</summary>
Motivation: Autonomous interactions of LLM agents with external environments introduce security risks, necessitating a method to identify infrequent but dangerous behaviors.

Method: VeriGrey uses tool invocation sequences as feedback and mutates prompts to test for vulnerabilities, linking injection tasks to agent functionality.

Result: VeriGrey outperforms black-box methods by 33% on the AgentDojo benchmark and identifies attack scenarios in real-world agents like Gemini CLI and OpenClaw.

Conclusion: VeriGrey demonstrates the effectiveness of dynamic testing for LLM agents, paving the way for an agent assurance framework.
...
### [305] [Sensi: Learn One Thing at a Time -- Curriculum-Based Test-Time Learning for LLM Game Agents](https://arxiv.org/abs/2603.17683)
*Mohsen Arjmandi*

Main category: cs.AI

TL;DR: Sensi, an LLM agent for the ARC-AGI-3 challenge, improves test-time learning efficiency with a two-player architecture, curriculum learning, and a database-as-control-plane. It achieves 50-94x greater sample efficiency but reveals perceptual grounding as a new bottleneck.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents require excessive interactions to learn task structure in unknown environments. The goal is to improve learning efficiency and adaptability.

Method: Sensi uses: (1) a two-player architecture separating perception and action, (2) curriculum-based learning managed by a state machine, (3) a database-as-control-plane for steerable context, and (4) an LLM-as-judge for dynamic evaluation.

Result: Sensi v1 solves 2 game levels; Sensi v2 completes its curriculum in ~32 attempts (50-94x more efficient than systems needing 1600-3000 attempts). However, v2 fails due to perceptual hallucinations.

Conclusion: Sensi advances learning efficiency but shifts the bottleneck to perceptual grounding, highlighting the need for better perceptual accuracy in LLM agents.
...
### [306] [MALLES: A Multi-agent LLMs-based Economic Sandbox with Consumer Preference Alignment](https://arxiv.org/abs/2603.17694)
*Yusen Wu,Yiran Liu,Xiaotie Deng*

Main category: cs.AI

TL;DR: The paper introduces MALLES, a Multi-Agent Large Language Model-based Economic Sandbox, to address challenges in modern decision-making by leveraging LLMs for cross-domain simulations, preference learning, and dynamic interaction modeling.


<details>
  <summary>Details</summary>
Motivation: Modern decision-making faces challenges due to high-dimensional, multimodal environments, agent heterogeneity, and data sparsity. The paper aims to overcome these by creating a unified simulation framework.

Method: Uses LLMs post-trained on heterogeneous transaction records for preference learning. Implements a mean-field mechanism for stable interactions and a multi-agent discussion framework to distribute cognitive load.

Result: Achieves significant improvements in product selection accuracy, purchase quantity prediction, and simulation stability compared to existing baselines.

Conclusion: LLMs hold potential as a foundational tool for high-fidelity, scalable decision simulation and analysis in the real economy.
...
### [307] [From Virtual Environments to Real-World Trials: Emerging Trends in Autonomous Driving](https://arxiv.org/abs/2603.17714)
*A. Humnabadkar,A. Sikdar,B. Cave,H. Zhang,N. Bessis,A. Behera*

Main category: cs.AI

TL;DR: This survey reviews advances in synthetic data and virtual environments for autonomous driving, covering perception, validation, and domain adaptation, and outlines future research challenges.


<details>
  <summary>Details</summary>
Motivation: To address data scarcity, safety, and generalization issues in autonomous driving by leveraging synthetic data and simulations.

Method: Organized into three dimensions: synthetic data for perception and planning, digital twin-based validation, and domain adaptation strategies.

Result: A detailed taxonomy of datasets and tools, highlighting trends and the role of vision-language models in improving scene understanding.

Conclusion: Identifies key challenges like Sim2Real transfer and scalable safety validation, crucial for advancing autonomous driving systems.
...
### [308] [Facts as First Class Objects: Knowledge Objects for Persistent LLM Memory](https://arxiv.org/abs/2603.17781)
*Oliver Zahn,Simran Chana*

Main category: cs.AI

TL;DR: The paper benchmarks in-context memory against Knowledge Objects (KOs) in large language models, revealing failure modes of in-context memory and superior performance of KOs in accuracy, cost, and multi-hop reasoning.


<details>
  <summary>Details</summary>
Motivation: To evaluate the limitations of in-context memory in large language models and propose Knowledge Objects (KOs) as a more efficient and reliable alternative for knowledge storage and retrieval.

Method: Benchmarking in-context memory against KOs across various conditions, testing accuracy, cost, multi-hop reasoning, and cross-model replication. Also, evaluating embedding retrieval and neural memory performance.

Result: KOs achieve 100% accuracy at 252x lower cost and outperform in-context memory in multi-hop reasoning (78.9% vs. 31.6%). In-context memory suffers from capacity limits, compaction loss, and goal drift.

Conclusion: Knowledge Objects (KOs) are a superior alternative to in-context memory for persistent knowledge work, offering higher accuracy, lower cost, and better reliability. The paper also introduces density-adaptive retrieval and releases a benchmark suite.
...
### [309] [RPMS: Enhancing LLM-Based Embodied Planning through Rule-Augmented Memory Synergy](https://arxiv.org/abs/2603.17831)
*Zhenhang Yuan,Shenghai Yuan,Lihua Xie*

Main category: cs.AI

TL;DR: RPMS, a conflict-managed architecture for LLM agents, improves action feasibility and performance in closed-world embodied environments by enforcing rules and managing state drift, achieving significant gains over baselines.


<details>
  <summary>Details</summary>
Motivation: LLM agents struggle in closed-world environments due to strict action preconditions and sparse feedback, leading to invalid actions and state drift. RPMS addresses these issues to enhance agent performance.

Method: RPMS uses structured rule retrieval to enforce action feasibility, a lightweight belief state to gate memory applicability, and rules-first arbitration to resolve conflicts between rules and memory.

Result: RPMS achieves 59.7% success (+23.9 pp) with Llama 3.1 8B and 98.5% (+11.9 pp) with Claude Sonnet 4.5 on ALFWorld. Rule retrieval contributed +14.9 pp. Adaptation to ScienceWorld with GPT-4 also showed consistent gains.

Conclusion: RPMS effectively mitigates failure modes in LLM agents by grounding episodic memory in current state and action rules, demonstrating transferability across different environments.
...
### [310] [AgentFactory: A Self-Evolving Framework Through Executable Subagent Accumulation and Reuse](https://arxiv.org/abs/2603.18000)
*Zhang Zhang,Shuqi Lu,Hongjin Qian,Di He,Zheng Liu*

Main category: cs.AI

TL;DR: AgentFactory introduces a self-evolving LLM-based agent paradigm that saves successful task solutions as executable Python subagents, enabling continuous improvement and portability.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agent self-evolution methods rely on textual prompts or reflections, which are inefficient for re-execution in complex scenarios.

Method: AgentFactory preserves successful task solutions as executable subagent code, refined through execution feedback, and stored as standardized Python code.

Result: The system accumulates and improves a library of subagents over time, reducing effort for similar tasks without manual intervention.

Conclusion: AgentFactory offers a robust and portable solution for continuous capability accumulation in LLM-based agents.
