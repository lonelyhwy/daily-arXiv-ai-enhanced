...
TL;DR: SemanticFL, a novel federated learning framework, uses pre-trained diffusion models to align heterogeneous client data, improving global model performance in non-IID settings.


<details>
  <summary>Details</summary>
Motivation: Non-IID client data in federated learning degrades model performance, especially in multimodal perception. Existing methods fail to address semantic discrepancies, leading to suboptimal results.

Method: Leverages multi-layer semantic representations (VAE-encoded latents and U-Net features) from Stable Diffusion to create a shared latent space. Uses cross-modal contrastive learning for consistency.

Result: SemanticFL outperforms FedAvg by up to 5.49% in accuracy on benchmarks like CIFAR-10, CIFAR-100, and TinyImageNet.

Conclusion: SemanticFL effectively addresses non-IID challenges, providing robust representations for heterogeneous and multimodal data in federated learning.
...
TL;DR: A unified multi-task framework using a transformer visual encoder and feature fusion improves performance across diverse ultrasound image tasks.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of generalizing ultrasound image models across varying scanners, operators, and anatomical targets.

Method: Uses a transformer visual encoder (Qwen3-VL family) with multi-scale feature fusion, task-specific heads, and training with task-aware sampling and loss balancing.

Result: Improved validation performance from 67% to 85% and average test score of 81.84% across tasks.

Conclusion: The framework is simple, adaptable, and effective for diverse ultrasound analysis tasks.
...
TL;DR: FireANTs proposes a modified Levenberg-Marquardt optimizer for memory-efficient diffeomorphic image registration, reducing memory usage by 24.6% while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: The memory requirements of Adam optimizer in FireANTs limit its use for large images, prompting the need for a more efficient alternative.

Method: A modified Levenberg-Marquardt optimizer is introduced, requiring only a scalar damping parameter and adaptive tuning via a trust region approach, with optional Metropolis-Hastings rejection steps.

Result: The new optimizer reduces memory usage by up to 24.6% for large volumes and performs comparably or better than Adam on three out of four datasets.

Conclusion: The proposed LM optimizer offers a memory-efficient solution for diffeomorphic image registration while maintaining or improving performance across diverse datasets.
...
TL;DR: LoFi is a method for fine-grained representation learning in chest X-rays, improving retrieval and phrase grounding by combining sigmoid, captioning, and location-aware captioning losses.


<details>
  <summary>Details</summary>
Motivation: Current contrastive models lack region-level supervision, and large vision-language models struggle with fine-grained representations, leading to suboptimal performance in retrieval and phrase grounding tasks.

Method: Proposed LoFi jointly optimizes sigmoid, captioning, and location-aware captioning losses using a lightweight large language model, enabling region-level supervision through grounding and dense captioning objectives.

Result: LoFi achieves superior retrieval and phrase grounding performance on MIMIC-CXR and PadChest-GR datasets.

Conclusion: The integration of fine-grained representation learning with in-context learning enhances chest X-ray grounding, demonstrating the effectiveness of the proposed approach.
...
TL;DR: A new framework for vehicle camouflage attacks using ControlNet to synthesize stealthy adversarial vehicles, outperforming existing methods in attack effectiveness and human-perceived stealthiness.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks (DNNs) are vulnerable to adversarial attacks, like camouflage attacks, which manipulate object appearances to deceive detectors while staying undetected by humans.

Method: Formulates vehicle camouflage as conditional image-editing, uses ControlNet for synthesis, and combines image-level and scene-level strategies with a unified objective for fidelity, style, and adversarial effectiveness.

Result: Achieves >38% AP50 decrease, better stealthiness, and generalization to unseen detectors and the physical world.

Conclusion: The proposed framework is highly effective, stealthy, and transferable, advancing adversarial attack capabilities.
...
TL;DR: A benchmark called ProactiveBench evaluates MLLMs' ability to proactively request user help, revealing they generally lack this skill, which doesn't correlate with model capacity, and suggesting it can be learned via fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To examine whether multimodal large language models (MLLMs) can proactively request user interventions, akin to human behavior in collaborative tasks.

Method: ProactiveBench, a benchmark built from seven repurposed datasets, tests proactiveness in tasks like recognizing occluded objects and interpreting sketches. Evaluates 22 MLLMs and explores fine-tuning with reinforcement learning.

Result: MLLMs generally lack proactiveness, which doesn't correlate with model capacity. Conversation histories and in-context learning hurt performance, but fine-tuning shows promise for learning proactiveness.

Conclusion: Proactiveness can be learned in MLLMs, and ProactiveBench is released to advance proactive multimodal models.
...
TL;DR: The paper proposes an instruction-free tuning approach for large vision language models (LVLMs) in the medical domain, reducing reliance on handcrafted instructions by using image-description pairs and momentum proxy instructions.


<details>
  <summary>Details</summary>
Motivation: Constructing large-scale, high-quality instruction datasets in the medical domain is challenging due to the need for expert knowledge, necessitating an alternative approach to fine-tuning LVLMs without explicit instructions.

Method: The authors introduce instruction-free tuning with momentum proxy instructions and a response shuffling strategy, leveraging image-description pairs instead of curated instructions to maintain instruction-following capability.

Result: The approach achieves state-of-the-art accuracy on multiple-choice visual question answering tasks across four medical datasets (SKINCON, WBCAtt, CBIS, MIMIC-CXR) and improves fine-tuning efficiency.

Conclusion: The proposed method successfully enhances LVLM performance in the medical domain without explicit instructions, demonstrating flexibility and efficiency in fine-tuning.
...
TL;DR: Lightweight CNN 'VeloxNet' uses gMLP blocks with spatial gating for efficient embedded image classification, outperforming SqueezeNet and other baselines in accuracy and parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: The need for accurate yet resource-efficient deep learning models for embedded devices in tasks like aerial disaster monitoring inspired the development of VeloxNet.

Method: VeloxNet replaces SqueezeNet's fire modules with gMLP blocks featuring spatial gating units (SGUs) for global spatial modeling with fewer parameters.

Result: VeloxNet reduces parameters by 46.1% vs. SqueezeNet and improves F1 scores by 6.32%-30.83% on aerial image datasets.

Conclusion: Spatial gating blocks enhance accuracy and efficiency in resource-constrained deployments, demonstrating VeloxNet's superiority over local convolutional modules.
...
TL;DR: ViTRM is a parameter-efficient vision model using tiny recursive blocks, reducing parameters significantly while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Traditional deep vision models (CNN, ViT) are resource-heavy, limiting deployment in constrained environments. ViTRM aims to offer a lightweight alternative.

Method: ViTRM replaces a multi-layer ViT encoder with a tiny 3-layer block applied recursively, reducing parameters by up to 84x compared to ViT.

Result: ViTRM achieves competitive performance on CIFAR-10 and CIFAR-100 with 6x fewer parameters than CNNs and 84x fewer than ViT.

Conclusion: Recursive computation is a viable, parameter-efficient alternative to deep architectures in vision tasks.
...
TL;DR: Gastric-X is a large-scale multimodal benchmark for gastric cancer analysis, introducing 1.7K cases with diverse clinical data. It evaluates vision-language models (VLMs) on five core tasks to assess their clinical applicability and understanding.


<details>
  <summary>Details</summary>
Motivation: To address the lack of structured datasets for medical diagnosis and advance VLMs for clinical applications, specifically in gastric cancer analysis.

Method: The Gastric-X dataset includes CT scans, endoscopic images, biochemical indicators, diagnostic notes, and tumor annotations. Five core tasks (VQA, report generation, retrieval, classification, localization) simulate clinical workflows.

Result: The study aims to evaluate VLM performance and understanding of multimodal correlations in clinical contexts.

Conclusion: Gastric-X serves as a step toward aligning machine intelligence with physician reasoning and inspires next-gen medical VLMs.
...
TL;DR: ReXInTheWild is a new benchmark of 955 clinician-verified multiple-choice questions across 484 medical images, evaluating vision-language models' ability to interpret medical content in everyday photos.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a comprehensive benchmark for evaluating vision-language models in interpreting medical content from ordinary photographs, combining fine-grained image understanding and medical reasoning.

Method: Introduced ReXInTheWild, a dataset of 955 questions across 7 clinical topics and 484 photos from biomedical literature, tested on leading models like Gemini-3, Claude Opus, GPT-5, and MedGemma.

Result: Performance varied: Gemini-3 (78%), Claude Opus (72%), GPT-5 (68%), and MedGemma (37%). Error analysis revealed four common categories requiring different mitigation strategies.

Conclusion: ReXInTheWild offers a challenging benchmark for medical image understanding and reasoning, highlighting gaps in current models' capabilities. The dataset is available for further research.
...
TL;DR: A new large-scale BSL fingerspelling dataset, FS23K, is introduced, along with a recognition model that considers bi-manual interactions and mouthing cues, reducing the character error rate by half.


<details>
  <summary>Details</summary>
Motivation: Fingerspelling recognition in BSL is challenging due to rapid signing and letter omissions, and existing datasets are limited or inaccurate.

Method: An iterative annotation framework was used to create FS23K, and a recognition model incorporating bi-manual interactions and mouthing cues was proposed.

Result: The approach reduces the character error rate (CER) by half compared to prior state-of-the-art methods.

Conclusion: The method shows promise for advancing sign language understanding and enabling scalable automated annotation.
...
TL;DR: The paper introduces dinov3.seg, a framework for Open-Vocabulary Semantic Segmentation (OVSS), combining global and local text embeddings with refined visual representations for better dense predictions in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models (VLMs) struggle with dense prediction tasks like OVSS due to suboptimal representations learned through global contrastive objectives, limiting spatial precision and robustness.

Method: The proposed dinov3.seg framework includes a task-specific architecture, joint use of global and local text embeddings, early and late refinement of visual and image-text correlation features, and a high-resolution local-global inference strategy.

Result: Extensive experiments on five OVSS benchmarks show dinov3.seg outperforms current state-of-the-art methods, demonstrating its effectiveness and robustness.

Conclusion: dinov3.seg advances OVSS by improving spatial precision and robustness, combining semantic discrimination with fine-grained locality, and outperforming existing methods.
...
TL;DR: A lightweight, socially informed architecture for pedestrian intention prediction fuses behavioral streams and uses uncertainty quantification to achieve high performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Accurate pedestrian intention prediction is crucial for autonomous vehicle safety in urban environments.

Method: The architecture combines four behavioral streams (attention, position, situation, interaction) with highway encoders, a compact Transformer, and self-attention pooling. Uncertainty is quantified using a variational bottleneck and Mahalanobis distance detector.

Result: The model outperforms benchmarks (0.9 F1, 0.94 AUC-ROC on PSI 1.0; 0.78 F1, 0.79 AUC-ROC on PSI 2.0) and improves accuracy with selective prediction.

Conclusion: The modality-agnostic approach is efficient, interpretable, and suitable for resource-constrained platforms, offering actionable risk scores.
...
TL;DR: SeeClear improves monocular depth estimation for transparent objects by converting them into opaque images using a diffusion-based generative module.


<details>
  <summary>Details</summary>
Motivation: Transparent objects pose challenges for depth estimation due to refraction and transmission, causing unstable or incorrect predictions.

Method: SeeClear localizes transparent regions, transforms them into opaque shapes, and uses an off-the-shelf depth estimator. It is trained on a synthetic dataset (SeeClear-396k).

Result: SeeClear significantly enhances depth estimation accuracy for transparent objects on both synthetic and real-world datasets.

Conclusion: SeeClear provides a stable and effective solution for monocular depth estimation of transparent objects.
...
TL;DR: StreetForward is a feedforward framework for dynamic street reconstruction, using temporal mask attention and 3D Gaussian Splatting to achieve high-fidelity novel views and depth estimation without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: To enable rapid scene reconstruction for autonomous driving applications, avoiding time-consuming per-scene optimization in large-scale datasets.

Method: Proposes a temporal mask attention module for dynamic motion capture and uses 3D Gaussian Splatting for uniform representation of static and dynamic content, optimized with cross-frame rendering.

Result: Superior performance in novel view synthesis and depth estimation on the Waymo Open Dataset, with validated generalization on CARLA and other datasets.

Conclusion: StreetForward effectively captures dynamic motion and reconstructs scenes for autonomous driving, demonstrating strong generalization and performance.
...
TL;DR: EvoNAS is an efficient distributed framework for multi-objective evolutionary architecture search, addressing high inference costs and ranking inconsistencies in LVMs by combining VSS-ViT modules and a novel knowledge distillation strategy. It achieves better trade-offs between accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The high inference cost of large vision models and limitations of Evolutionary Neural Architecture Search (ENAS) in practical deployment due to expensive evaluations and inconsistent rankings among subnetworks.

Method: Proposes EvoNAS, combining VSS-ViT modules with Cross-Architecture Dual-Domain Knowledge Distillation (CA-DDKD) for improved representational capacity and ranking consistency. Introduces Distributed Multi-Model Parallel Evaluation (DMMPE) for efficient large-scale validation.

Result: EvoNets, the resulting architectures, achieve Pareto-optimal trade-offs between accuracy and efficiency, outperforming CNN-, ViT-, and Mamba-based models in inference latency and throughput while maintaining strong generalization.

Conclusion: EvoNAS successfully balances accuracy and efficiency in vision models, making it suitable for resource-constrained edge devices, with demonstrated effectiveness across multiple benchmarks.
...
TL;DR: The paper proposes an Event Prompter for RGB-Event PAR, using lightweight DCT/IDCT for event data and a memory bank with Hopfield networks to enhance accuracy efficiently.


<details>
  <summary>Details</summary>
Motivation: Existing methods for event-based PAR are computationally heavy and ignore contextual guidance, prompting the need for a more efficient solution.

Method: Uses DCT/IDCT for event data, an external memory bank with Hopfield networks, and cross-attention for RGB-event fusion.

Result: Validated effectiveness on benchmark datasets, showing improved accuracy.

Conclusion: The proposed framework is efficient and effective for PAR, with code to be released.
...
TL;DR: ConvNeur, a two-branch architecture, decouples global and local image feature processing to balance efficiency and accuracy, achieving competitive results in vision tasks.


<details>
  <summary>Details</summary>
Motivation: Modern vision models need to efficiently integrate global context and local detail, but existing solutions often compromise one for the other. ConvNeur explores the benefits of decoupling these roles.

Method: ConvNeur uses a two-branch approach: a lightweight neural memory branch for global context and a locality-preserving branch for fine structure. A learned gate modulates local features with global cues.

Result: The model achieves subquadratic scaling, retains local priors, and reduces overhead compared to global attention, matching or outperforming alternatives in classification, detection, and segmentation.

Conclusion: Decoupling global and local processing improves efficiency and accuracy, supporting the principle that such separation enhances vision model performance.
...
TL;DR: A two-stage acceleration framework is introduced to speed up diffusion-based image tokenization, combining multi-scale sampling and single-step denoising to achieve faster decoding with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based decoders for image tokenization are slow due to iterative sampling, making them impractical for real-time or large-scale applications. This work aims to address this inefficiency.

Method: Proposes a multi-scale sampling strategy (coarse to fine resolution) and distills the diffusion decoder into single-step denoising models at each scale.

Result: Achieves an order-of-magnitude reduction in decoding time with little quality degradation.

Conclusion: The framework provides a practical solution for efficient image tokenization, serving as a foundation for future research in visual tokenization and generation.
...
TL;DR: CurveStream is a training-free, curvature-aware hierarchical memory framework for streaming videos, improving performance by over 10% over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods for visual retention in streaming videos lack semantic awareness, leading to memory issues and loss of critical transitions. CurveStream addresses this by leveraging curvature in feature trajectories.

Method: Uses a Curvature Score to evaluate real-time semantic intensity and a K-Sigma dynamic threshold to manage memory states adaptively under token constraints.

Result: Achieves over 10% performance gains (e.g., 10.69% on StreamingBench, 13.58% on OVOBench) over baselines, setting new state-of-the-art results.

Conclusion: CurveStream effectively manages memory in streaming videos by focusing on high-curvature semantic transitions, outperforming existing methods.
...
TL;DR: MagicSeg introduces a diffusion model-driven pipeline to automatically generate datasets for open-world semantic segmentation, combining high-fidelity image-text generation with contrastive training and self-supervised mask extraction for improved performance.


<details>
  <summary>Details</summary>
Motivation: The high cost of acquiring fine-grained pixel annotations for open-world semantic segmentation motivates the development of an automated, cost-effective dataset generation method using diffusion models.

Method: MagicSeg generates images from class labels via textual descriptions, including paired counterfactual samples, and uses open-vocabulary detection and interactive segmentation to extract precise masks for self-supervised pretraining.

Result: Evaluations on PASCAL VOC, PASCAL Context, and COCO show state-of-the-art performance (62.9%, 26.7%, and 40.2%, respectively), demonstrating the dataset's effectiveness.

Conclusion: MagicSeg's automated dataset generation and training pipeline significantly enhance open-world semantic segmentation performance, offering a scalable and economical solution.
...
TL;DR: FlowScene is a tri-branch generative model that uses multimodal graphs to collaboratively generate scene layouts, shapes, and textures, outperforming existing methods in realism, style coherence, and human preference alignment.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of language-driven retrieval (lack of object-level control and scene-level style coherence) and graph-based methods (low-fidelity textured results) in scene generation.

Method: FlowScene employs a tight-coupled rectified flow model conditioned on multimodal graphs, enabling collaborative reasoning across the graph for joint generation of layouts, shapes, and textures.

Result: FlowScene surpasses language-conditioned and graph-conditioned baselines in realism, style consistency, and alignment with human preferences.

Conclusion: FlowScene offers fine-grained control over object properties and scene-level coherence, making it a practical and superior solution for high-fidelity scene generation.
...
TL;DR: K-GMRF is an online, training-free framework for covariance tracking using second-order dynamics on Lie groups, outperforming first-order methods in accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Existing covariance matrix estimators either ignore manifold constraints or rely on first-order updates, causing phase lag during rapid changes. K-GMRF addresses this by leveraging Lie group dynamics.

Method: The method reformulates covariance tracking as forced rigid-body motion on Lie groups, using Euler-Poincaré equations and a symplectic integrator to propagate angular velocity.

Result: K-GMRF achieves zero steady-state error under constant rotation and shows superior performance in synthetic ellipses (30x error reduction), SO(3) stabilization (error reduced from 29.4° to 9.9°), and motion-blur sequences (IoU improved from 0.55 to 0.74).

Conclusion: K-GMRF offers a robust, interpretable, and plug-and-play solution for covariance tracking, suitable for data-constrained scenarios and deep learning architectures.
...
TL;DR: ChangeRWKV introduces a new architecture combining the efficiency of CNNs and the global context of Transformers for remote sensing change detection, achieving state-of-the-art performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing methods like CNNs lack global context, while Transformers are computationally expensive. ChangeRWKV aims to bridge this gap.

Method: Hierarchical RWKV encoder and Spatial-Temporal Fusion Module (STFM) for multi-resolution feature representation and resolving spatial misalignments.

Result: Achieves 85.46% IoU and 92.16% F1 score on LEVIR-CD benchmark with fewer parameters and FLOPs.

Conclusion: ChangeRWKV presents an efficient and powerful paradigm for large-scale change detection tasks.
...
TL;DR: Physion-Eval is a benchmark for assessing physical realism in video generation models, using expert human reasoning to identify glitches in 10,990 videos. Results show high failure rates (83.3-93.5%) in physics-critical scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing video generation model evaluations lack insight into physical realism. The goal is to diagnose when and why these models violate real-world physics.

Method: Experts analyze videos from five state-of-the-art models, comparing them to real-world reference videos. Glitches, failure categories, and explanations are annotated.

Result: High failure rates: 83.3% (exocentric) and 93.5% (egocentric) of videos contain physical glitches in physics-critical scenarios.

Conclusion: Physion-Eval sets a new standard for evaluating physical realism in video generation and highlights the need for physics-grounded improvements.
...
TL;DR: FB-CLIP enhances zero-shot fine-grained anomaly detection by combining multi-strategy textual representations, foreground-background separation, and semantic consistency regularization, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Labeled anomalies are scarce, and existing vision-language models struggle with feature entanglement and coarse semantics.

Method: FB-CLIP uses multi-strategy textual representations, multi-view soft separation in visual features, and Semantic Consistency Regularization (SCR) for alignment.

Result: FB-CLIP effectively distinguishes anomalies from complex backgrounds and achieves accurate detection and localization in zero-shot settings.

Conclusion: The proposed framework improves fine-grained anomaly detection by addressing feature entanglement and semantic alignment challenges.
...
TL;DR: LoD-Loc v3 introduces a new method for aerial visual localization in urban areas, improving cross-scene generalization and dense scene performance via synthetic data and instance silhouette alignment.


<details>
  <summary>Details</summary>
Motivation: Address limitations of LoD-Loc v2, such as poor cross-scene generalization and failures in dense building scenes.

Method: Uses a synthetic data pipeline (InsLoD-Loc dataset) and shifts from semantic to instance silhouette alignment.

Result: Outperforms SOTA baselines in cross-scene and dense urban scenarios.

Conclusion: LoD-Loc v3 advances aerial localization with better generalization and performance.
...
TL;DR: ParallelVLM is a training-free speculative decoding framework that improves efficiency in video understanding tasks by addressing mutual waiting and speedup issues, achieving significant acceleration.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLMs face inefficiencies due to the large number of video tokens during autoregressive decoding, and existing token pruning methods suffer from information loss and limited speedup.

Method: ParallelVLM uses a draft-then-verify speculative decoding framework with two parallelized stages, maximizing hardware utilization and employing an Unbiased Verifier-Guided Pruning strategy to align draft and target models.

Result: Experiments show ParallelVLM expands the draft window by 1.6∼1.8× with high accepted lengths and accelerates benchmarks by 3.36× and 2.42× compared to vanilla autoregressive decoding.

Conclusion: ParallelVLM effectively addresses decoding inefficiencies and outperforms existing methods in video understanding tasks, demonstrating significant improvements in speed and accuracy.
...
TL;DR: OrbitNVS reformulates Novel View Synthesis (NVS) as an orbit video generation task, leveraging a pre-trained video model for high-quality view synthesis with improved geometry and appearance consistency.


<details>
  <summary>Details</summary>
Motivation: Existing NVS methods struggle with synthesizing plausible views for unobserved regions, especially under single-view input, and lack consistency in geometry and appearance. OrbitNVS aims to address these challenges.

Method: The approach adapts a pre-trained video generation model for NVS, incorporating camera adapters for accurate control. It introduces a normal map generation branch and uses attention mechanisms for geometric consistency, along with pixel-space supervision to reduce blurriness.

Result: OrbitNVS outperforms previous methods on the GSO and OmniObject3D benchmarks, achieving significant improvements in PSNR (+2.9 dB and +2.4 dB) in single-view settings.

Conclusion: OrbitNVS effectively addresses key challenges in NVS by leveraging video generation priors and tailored techniques, delivering superior performance in synthesizing novel views with consistent geometry and appearance.
...
TL;DR: UniPR is an end-to-end framework for real-to-sim perception and reconstruction from stereo images, using geometric constraints and Pose-Aware Shape Representation to improve efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing modular pipelines for object perception and reconstruction are inefficient and error-prone due to fragmented processing and lack of global context. UniPR aims to unify these tasks into a single, efficient framework.

Method: UniPR operates on stereo image pairs, leveraging geometric constraints to resolve scale ambiguity. It introduces Pose-Aware Shape Representation to avoid per-category definitions and integrates reconstruction with pose estimation.

Result: UniPR reconstructs all objects in a scene in a single forward pass, achieving efficiency and preserving physical proportions across object types. The framework is validated using the LVS6D dataset of 6,300+ objects.

Conclusion: UniPR demonstrates the potential for practical robotic applications by unifying perception and reconstruction tasks, improving efficiency, and maintaining accuracy across diverse objects.
...
TL;DR: HRNet addresses limitations in multimodal image registration by learning a stable shared feature space and unified hybrid transformation, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing methods leak modality-private cues into shared spaces and lack support for hybrid transformations, limiting their effectiveness.

Method: HRNet uses Modality-Specific Batch Normalization (MSBN) and a Cross-scale Disentanglement and Adaptive Projection (CDAP) module to extract shared features and suppress private cues. A Hybrid Parameter Prediction Module (HPPM) predicts global and local deformations.

Result: HRNet outperforms existing methods on rigid and non-rigid registration tasks across four datasets.

Conclusion: HRNet provides a robust solution for hybrid multimodal registration by combining feature disentanglement and hybrid transformation prediction.
...
TL;DR: IUP-Pose is a novel geometry-driven decoupled iterative framework with implicit dense alignment, addressing bottlenecks in relative pose estimation by decoupling rotation and translation and enhancing cross-view feature alignment. It achieves high accuracy and efficiency in real-time edge deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the trade-off in existing methods: feature-matching pipelines are accurate but non-differentiable, while ViT-based regressors are trainable but too expensive. The core problems identified are the coupling of rotation-translation estimation and poor cross-view feature alignment.

Method: IUP-Pose introduces a lightweight Multi-Head Bi-Cross Attention (MHBC) module for implicit dense alignment and a decoupled pipeline. It iteratively refines rotation with uncertainty and realigns feature maps via rotational homography before translation prediction.

Result: The method achieves 73.3% AUC@20deg on MegaDepth1500, operates at 70 FPS, and has only 37M parameters, showing a strong accuracy-efficiency balance for real-time use.

Conclusion: IUP-Pose successfully addresses the limitations of existing RPR methods, offering an end-to-end trainable, efficient, and accurate solution for relative pose estimation.
...
TL;DR: The paper proposes a dual prompt-driven feature encoding method (DPTracker) to improve UAV tracking by addressing illumination and viewpoint challenges, especially in nighttime conditions.


<details>
  <summary>Details</summary>
Motivation: Existing feature encoding methods fail to account for critical illumination and viewpoint cues, leading to poor tracking performance in nighttime UAV scenarios.

Method: A dual prompt-driven feature encoding method integrates prompt-conditioned feature adaptation and context-aware prompt evolution, including a pyramid illumination prompter and dynamic viewpoint prompter.

Result: Extensive experiments and real-world tests show DPTracker's effectiveness and robustness in nighttime UAV tracking.

Conclusion: DPTracker enhances domain-invariant feature encoding, outperforming existing methods in challenging nighttime conditions.
...
TL;DR: UniBioTransfer is a unified framework for diverse deepface generation tasks, addressing data scarcity and cross-task conflicts with innovative training and modeling strategies.


<details>
  <summary>Details</summary>
Motivation: Traditional task-specific models limit generalization and scalability. A unified approach is needed for handling multiple deepface tasks efficiently.

Method: Proposes UniBioTransfer with a unified data construction strategy and BioMoE, a mixture-of-experts model with a two-stage training strategy to disentangle task-specific knowledge.

Result: Outperforms existing unified and task-specific models, demonstrating effectiveness, generalization, and scalability across various deepface tasks.

Conclusion: UniBioTransfer successfully addresses multi-task deepface generation challenges, offering a practical and scalable solution.
...
TL;DR: OmniDiT is a unified Virtual Try-On (VTON) and Try-Off (VTOFF) framework using a Diffusion Transformer, addressing challenges like fine-grained detail preservation and efficient inference.


<details>
  <summary>Details</summary>
Motivation: Existing VTON methods struggle with detail preservation, generalization, pipeline complexity, and efficiency. OmniDiT aims to unify VTON and VTOFF tasks while overcoming these issues.

Method: Leverages a self-evolving data pipeline (Omni-TryOn dataset), token concatenation, adaptive position encoding, and Shifted Window Attention for linear complexity. Uses multiple timestep prediction and alignment loss to enhance generation.

Result: Achieves top performance in model-free VTON/VTOFF tasks and matches SOTA in model-based VTON, demonstrating effectiveness in complex scenes.

Conclusion: OmniDiT successfully unifies VTON and VTOFF, offering a scalable and efficient solution with superior performance.
...
TL;DR: GravCal, a feedforward model, corrects noisy gravity priors from a single image, reducing mean angular error significantly and providing a confidence score.


<details>
  <summary>Details</summary>
Motivation: Addressing the unreliability of IMU-derived gravity priors under conditions like linear acceleration, vibration, and transient motion, especially in scenarios where existing methods either depend on accurate inertial input or estimate gravity directly from images.

Method: GravCal combines residual correction of noisy priors and prior-independent image estimates, using a learned gate to fuse these predictions adaptively for improved accuracy.

Result: Mean angular error reduced from 22.02° to 14.24°, with larger improvements under severe prior corruption. A novel dataset of 148K frames with ground-truth gravity and IMU priors was also introduced.

Conclusion: GravCal effectively calibrates noisy gravity priors using a single image, demonstrating significant accuracy gains and providing a useful confidence signal for downstream applications.
...
TL;DR: SAVN-CE enables embodied agents to navigate freely in 3D spaces using coherent audio-visual streams. MAGNet, a transformer-based model, improves goal reasoning and outperforms state-of-the-art methods by 12.1% in success rate.


<details>
  <summary>Details</summary>
Motivation: Existing audio-visual navigation methods rely on precomputed room impulse responses, limiting agents to discrete positions and causing discontinuous observations. The paper aims to create a more realistic setting with continuous movement and coherent audio-visual perception.

Method: The authors introduce SAVN-CE for navigation in continuous 3D environments and propose MAGNet, a multimodal transformer model. MAGNet combines spatial and semantic goal representations with historical context and self-motion cues for memory-augmented goal reasoning.

Result: MAGNet achieves a 12.1% absolute improvement in success rate over state-of-the-art methods, demonstrating robustness in handling short-duration sounds and long-distance navigation.

Conclusion: SAVN-CE and MAGNet address limitations of previous approaches, offering a more realistic and effective solution for audio-visual navigation in continuous environments.
...
TL;DR: JMVR introduces a joint-modal framework for visual reconstruction using EEG and text, preserving EEG-specific details and outperforming existing methods in spatial and chromatic fidelity.


<details>
  <summary>Details</summary>
Motivation: Current methods overly align EEG features with text or image semantics, potentially losing rich spatial and chromatic details in EEG.

Method: JMVR treats EEG and text as independent modalities, uses multi-scale EEG encoding, and employs image augmentation to recover perceptual details.

Result: JMVR achieves state-of-the-art performance on the THINGS-EEG dataset, excelling in spatial structure and chromatic fidelity.

Conclusion: JMVR provides a robust framework for high-fidelity visual reconstruction by leveraging EEG-specific information and multi-scale encoding.
...
TL;DR: Optimizing user-provided bounding boxes by aligning them with video diffusion models' attention maps improves generation quality and control adherence.


<details>
  <summary>Details</summary>
Motivation: Control inputs like bounding boxes for text-to-video diffusion models are not always adhered to, and improving this alignment is an open problem.

Method: Proposes optimizing bounding boxes using a smooth mask for differentiability and an attention-maximization objective to align with model attention maps.

Result: Small adjustments to bounding boxes significantly improve generation quality and adherence to control inputs, validated through experiments and a user study.

Conclusion: Optimizing control inputs to match the model's familiarity enhances generation performance, with code made available for community research.
...
TL;DR: DynFlowDrive integrates flow-based dynamics into autonomous driving planning, improving reliability by modeling scene evolution under driving actions and employing a stability-aware trajectory selection strategy.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems rely on appearance generation or deterministic regression for future state prediction, which fails to capture trajectory-conditioned scene evolution, leading to unreliable planning.

Method: Proposes DynFlowDrive, a latent world model using rectified flow to learn a velocity field for progressive prediction of future latent states, and introduces a stability-aware multi-mode trajectory selection strategy.

Result: Demonstrates consistent improvements on nuScenes and NavSim benchmarks without additional inference overhead.

Conclusion: DynFlowDrive effectively enhances planning reliability in autonomous driving by leveraging flow-based dynamics and stability-aware trajectory selection.
...
TL;DR: ATHENA improves object count fidelity in text-to-image diffusion models without architectural changes or retraining.


<details>
  <summary>Details</summary>
Motivation: Addressing the systematic failures of diffusion models in numerical control when prompts specify explicit object counts.

Method: ATHENA is a model-agnostic framework that uses intermediate representations for count estimation and applies noise corrections early in denoising.

Result: ATHENA consistently improves count fidelity, especially at higher target counts, while maintaining accuracy-runtime trade-offs.

Conclusion: The framework effectively steers generation trajectories to correct structural errors early, enhancing numerical accuracy in diffusion models.
...
TL;DR: VLADR introduces vision-language attribute disentanglement and reinforcement for lifelong person re-identification, improving knowledge transfer and anti-forgetting capacity.


<details>
  <summary>Details</summary>
Motivation: Existing LReID methods lack fine-grained attribute knowledge, limiting acquisition and anti-forgetting capabilities.

Method: VLADR uses multi-grain text attribute disentanglement and inter-domain cross-modal attribute reinforcement to enhance knowledge transfer.

Result: VLADR outperforms state-of-the-art methods by 1.9%-2.5% in anti-forgetting and generalization capacity.

Conclusion: VLADR effectively leverages vision-language models for better lifelong person re-identification.
...
TL;DR: The paper introduces the Unbiased Dynamic Multimodal Learning (UDML) framework to address limitations in traditional and dynamic multimodal methods, such as static modality assumptions and empirical quality metrics. UDML uses a noise-aware uncertainty estimator and modality reliance bias quantification to improve performance across noise levels and modalities.


<details>
  <summary>Details</summary>
Motivation: Dynamic multimodal methods often rely on empirical metrics and assume equal initial modality contributions, leading to performance issues under extreme noise and penalizing hard-to-learn modalities. UDML aims to overcome these limitations.

Method: UDML employs a noise-aware uncertainty estimator that correlates feature corruption with noise levels and quantifies inherent modality reliance bias through modality dropout, integrating it into the weighting mechanism.

Result: Experiments on diverse multimodal benchmarks confirm UDML's effectiveness, versatility, and generalizability, outperforming static and existing dynamic fusion methods.

Conclusion: UDML provides a robust solution for dynamic multimodal learning by accurately measuring uncertainty and addressing modality bias, enhancing performance across varying noise conditions.
...
TL;DR: The paper introduces a self-constrained prior to improve 3D Gaussian Splatting (3DGS) for more accurate depth rendering, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: While 3DGS outperforms NeRF in rendering quality and speed, it lacks high-fidelity surface recovery. The authors aim to address this by constraining 3D Gaussian learning.

Method: A self-constrained prior is proposed, derived from a TSDF grid of depth maps. It measures a distance field to refine 3D Gaussians—removing outliers, adjusting positions, and optimizing opacity. The prior is updated with newer depth images and progressively tightens constraints.

Result: The method demonstrates superior performance on benchmarks compared to state-of-the-art techniques.

Conclusion: The self-constrained prior effectively enhances 3DGS for accurate depth rendering, validated by benchmark results.
...
TL;DR: TSegAgent reformulates dental 3D model analysis as a zero-shot geometric reasoning problem, reducing annotation costs and improving generalization.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of data-driven approaches (high annotation cost, poor generalization) in dental 3D model segmentation and identification.

Method: Combines foundation models with dental anatomy-based geometric reasoning, using multi-view abstraction and structural constraints (e.g., dental arch organization).

Result: Achieves accurate, low-cost segmentation/identification with strong generalization to unseen scans.

Conclusion: Geometric reasoning outperforms traditional data-driven methods in dental analysis tasks.
...
TL;DR: AI-assisted two-stage ECG framework improves detection of rare cardiac anomalies, achieving high accuracy and reducing performance disparities across demographics.


<details>
  <summary>Details</summary>
Motivation: Rare cardiac anomalies are hard to detect due to limited cases and demographic disparities, necessitating a generalizable, equitable solution.

Method: Self-supervised anomaly detection pretraining followed by fine-tuning with asymmetric loss for multi-label classification and anomaly localization.

Result: Achieves 94.7% AUROC for rare anomalies, reduces common-rare performance gap by 73%, and maintains accuracy across age and sex.

Conclusion: The framework shows strong clinical utility, interpretability, and scalability, addressing diagnostic disparities in ECG analysis.
...
TL;DR: The paper explores whether 2D foundation image models inherently understand 3D world structures. It introduces an agentic approach to 3D world synthesis, demonstrating that these models can produce coherent and realistic 3D scenes.


<details>
  <summary>Details</summary>
Motivation: To determine if advanced 2D image generation models and Vision-Language Models (VLMs) inherently possess 3D world modeling capabilities, despite being trained primarily on 2D data.

Method: The authors propose an agentic framework involving a VLM-based director for prompt formulation, a generator for synthesizing new image views, and a VLM-backed verifier to curate frames in both 2D and 3D spaces.

Result: The method shows that 2D models do encapsulate 3D world understanding, enabling the synthesis of expansive, realistic, and 3D-consistent scenes.

Conclusion: 2D foundation image models inherently grasp 3D world structures, and the proposed agentic approach effectively harnesses this capability for robust 3D scene generation.
...
TL;DR: BALM is a plug-in framework for balanced multimodal learning under imbalanced missing rates, featuring feature calibration and gradient rebalancing modules to enhance robustness and performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the imbalance in multimodal learning where information-rich modalities dominate and weaker or missing modalities contribute less, especially under imbalanced missing rates.

Method: Proposes BALM with two modules: Feature Calibration Module (FCM) for recalibrating unimodal features using global context, and Gradient Rebalancing Module (GRM) for balancing learning dynamics across modalities.

Result: Experimental results show BALM consistently improves robustness and performance in multimodal emotion recognition benchmarks under diverse missing and imbalance settings.

Conclusion: BALM effectively balances multimodal learning under imbalanced missing rates, enhancing model performance without altering original architectures.
...
TL;DR: The paper proposes PerformRecast, a method for editing facial expressions in portrait videos independently of head poses, using insights from the 3D Morphable Face Model (3DMM).


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to disentangle facial expressions from head poses, limiting independent expression editing. This is crucial for animation and film industries.

Method: The authors improve keypoints transformation to align with 3DMM, decouple facial and non-facial regions, and use a teacher model for supervision to enhance quality.

Result: The method produces high-quality results, outperforming existing approaches in controllability and efficiency, with more faithful alignment to driving videos.

Conclusion: PerformRecast offers a versatile and effective solution for expression-only video editing, providing fine-grained control and superior performance.
...
TL;DR: PhysNeXt is a dual-input deep learning framework that combines video frames and STMap representations for robust remote photoplethysmography (rPPG), outperforming existing methods in challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Existing rPPG methods face trade-offs between noise from raw videos and detail loss in STMap representations. PhysNeXt aims to integrate their strengths to improve pulse signal extraction.

Method: PhysNeXt uses a dual-input framework with video frames and STMaps, incorporating spatio-temporal difference modeling, cross-modal interaction, and a structured attention-based decoder.

Result: PhysNeXt achieves stable and fine-grained rPPG signal recovery under challenging conditions, validating its joint modeling approach.

Conclusion: PhysNeXt effectively enhances rPPG robustness by integrating video and STMap representations, with codes set for release.
...
TL;DR: ReLi3D is a unified pipeline for reconstructing 3D geometry, materials, and illumination from sparse multi-view images in under one second, using a transformer cross-conditioning architecture and a two-path prediction strategy.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D asset reconstruction requires separate pipelines for geometry, materials, and illumination, each with limitations and computational overhead. ReLi3D aims to unify these tasks into a single efficient process.

Method: The approach fuses multi-view inputs via a transformer cross-conditioning architecture and uses a two-path strategy: one for object structure/appearance and another for environment illumination. It includes a differentiable Monte Carlo renderer and a mixed-domain training protocol combining synthetic and real-world data.

Result: ReLi3D achieves fast, simultaneous reconstruction of geometry, materials, and illumination, with generalizable results in accuracy and quality.

Conclusion: By unifying previously separate tasks into a single feed-forward pipeline, ReLi3D enables near-instantaneous generation of complete, relightable 3D assets.
...
TL;DR: UPL introduces uncertainty-aware prototype learning for few-shot 3D segmentation, improving robustness and generalization with dual-stream refinement and probabilistic modeling.


<details>
  <summary>Details</summary>
Motivation: Existing prototype-based methods lack adaptability to intrinsic uncertainty from scarce supervision, leading to degraded robustness and limited generalization.

Method: UPL uses a dual-stream prototype refinement module and formulates prototype learning as variational inference to model uncertainty explicitly.

Result: UPL achieves state-of-the-art performance on ScanNet and S3DIS benchmarks with reliable uncertainty estimation.

Conclusion: The probabilistic approach in UPL enhances few-shot 3D segmentation by addressing uncertainty, offering robustness and interpretability.
...
TL;DR: GNAP is a novel neural network method that dynamically adjusts its size and parameters during training through growth and pruning, achieving high accuracy with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Traditional convolutional neural networks use fixed architectures, which may not efficiently adapt to data. GNAP aims to dynamically optimize network size and parameters for better efficiency and performance.

Method: GNAP alternates between growing phases (expanding network size for expressive power) and pruning phases (autonomously reducing parameters via gradient descent).

Result: GNAP achieves high accuracy with sparse parameters, e.g., 99.44% on MNIST with 6.2k parameters and 92.2% on CIFAR10 with 157.8k parameters.

Conclusion: GNAP demonstrates that dynamic network adjustment during training can yield highly efficient and accurate models for image classification.
...
TL;DR: PCSTracker is an end-to-end framework for consistent scene flow estimation in point cloud sequences, addressing challenges like dynamic geometry changes, occlusions, and error accumulation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for point cloud scene flow estimation struggle with temporal consistency over long sequences due to evolving geometry, occlusions, and error accumulation. This work aims to address these limitations.

Method: The proposed PCSTracker includes an iterative geometry motion joint optimization module (IGMO) for modeling temporal evolution of point features and a spatio-temporal point trajectory update module (STTU) for handling occlusions. It also uses an overlapping sliding-window inference strategy for long sequences.

Result: PCSTracker achieves the best accuracy in long-term scene flow estimation and maintains real-time performance at 32.5 FPS, outperforming RGB-D-based approaches.

Conclusion: PCSTracker effectively addresses challenges in point cloud scene flow estimation, ensuring temporal consistency and real-time performance, as demonstrated on synthetic and real-world datasets.
...
TL;DR: FREAK is a new benchmark for fine-grained hallucination assessment in MLLMs using photorealistic images with counter-commonsense edits, revealing severe hallucination issues in SOTA models.


<details>
  <summary>Details</summary>
Motivation: Existing hallucination evaluation benchmarks are limited by simplified tasks or lack of diversity, failing to adequately assess MLLMs' hallucination extent.

Method: Proposes FREAK, a benchmark with high-quality photorealistic images featuring fine-grained counter-commonsense edits, to evaluate hallucination in detailed visual perception.

Result: Experiments show severe hallucination issues in SOTA models. A controlled subset and CoT prompting techniques further reveal insights into hallucination patterns and reasoning processes.

Conclusion: FREAK provides a comprehensive tool for assessing hallucinations in MLLMs, uncovering critical issues and enabling deeper investigation into model reasoning.
...
TL;DR: HINGE adapts a pre-trained single-cell foundation model for histology-conditioned gene expression prediction, preserving gene relationships and outperforming existing methods in accuracy and biological coherence.


<details>
  <summary>Details</summary>
Motivation: High costs and limited throughput of spatial transcriptomics (ST) motivate predicting expression from histology. Existing generative models lack explicit modeling of gene-gene dependencies, which sc-FMs can address but face challenges in integration.

Method: HINGE introduces SoftAdaLN to inject visual context into a pre-trained sc-FM, uses masked diffusion objective, and employs a warm-start curriculum for alignment and stability.

Result: HINGE outperforms baselines in Pearson correlation, spatial marker accuracy, and co-expression consistency across three ST datasets.

Conclusion: HINGE provides a practical solution to adapt sc-FMs for histology-conditioned expression generation, enhancing biological coherence and prediction accuracy.
...
TL;DR: FlashCap introduces a flashing LED-based MoCap system for precise motion timing, along with the FlashMotion dataset. ResPose, a baseline method, improves pose estimation by ~40% and achieves millisecond accuracy.


<details>
  <summary>Details</summary>
Motivation: Precise motion timing (PMT) is critical in scenarios like sports but is overlooked due to lack of high-temporal-resolution datasets. Existing solutions (e.g., high-speed cameras) are costly and impractical for daily use.

Method: FlashCap uses flashing LEDs for motion capture and collects the FlashMotion dataset with millisecond resolution (event, RGB, LiDAR, IMU). ResPose, a baseline model, learns residual poses from events and RGBs for PMT and HPE.

Result: ResPose reduces pose estimation errors by ~40% and achieves millisecond-level timing accuracy. FlashMotion is rigorously validated for quality.

Conclusion: FlashCap and FlashMotion enable new research in PMT and high-temporal-resolution HPE. The dataset and code will be shared with the community.
...
TL;DR: The paper proposes a training-free object detection method using segmentation foundation models and feature-based classification, suitable for scenarios like continuous integration testing where training data generation is undesirable.


<details>
  <summary>Details</summary>
Motivation: To address use cases requiring object detection without training data or model training, such as automated testing in software development (e.g., continuous integration).

Method: Combines segments from segmentation foundation models with a simple feature-based classification, eliminating the need for training or dataset creation.

Result: The method achieves performance comparable to learning-based methods like YOLO, validated on detecting icons in navigation maps for automotive UI testing.

Conclusion: The approach saves time and cost by avoiding retraining and dataset generation, making it practical for specific applications like automated testing.
...
TL;DR: The paper introduces TIEdit, a benchmark for evaluating text-guided image editing (TIE) methods, and EditProbe, an LLM-based evaluator, to address limitations in existing benchmarks and improve alignment with human judgments.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation benchmarks for TIE lack scale and correlation with human judgments, prompting the need for a more systematic and reliable assessment method.

Method: TIEdit includes 512 source images with editing prompts and 5,120 edited images from 10 TIE models, evaluated by experts. EditProbe uses intermediate-layer probing of LLMs for better semantic and perceptual analysis.

Result: TIEdit collected 307,200 ratings, distilled into 15,360 MOSs. EditProbe showed stronger correlation with human perception than traditional metrics.

Conclusion: TIEdit and EditProbe offer a robust framework for evaluating TIE methods, with EditProbe achieving better alignment with human judgments.
...
TL;DR: ReManNet introduces a novel geometric-topological coupling for monocular 3D lane detection, achieving SOTA results by encoding road geometry and using a joint point-curve loss.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with depth ambiguity and weak geometric constraints, leading to ill-posed 2D-to-3D lane lifting and degraded outputs.

Method: Proposes the Road-Manifold Assumption and ReManNet, which combines visual features with Riemannian Gaussian descriptors on the SPD manifold and uses a 3D-TLIoU loss for better alignment.

Result: Achieves F1 improvements of +8.2% over the baseline and +1.8% over the previous best on OpenLane, with scenario-level gains up to +6.6%.

Conclusion: ReManNet effectively addresses geometric challenges in 3D lane detection, outperforming existing methods through robust geometric encoding and topology-aware loss.
...
TL;DR: TATAR unifies Image Quality Assessment (IQA) and Image Aesthetic Assessment (IAA) by addressing their mismatches with task-specific reasoning and rewards, outperforming prior unified methods.


<details>
  <summary>Details</summary>
Motivation: Existing unified models for IQA and IAA use a task-agnostic approach, which misaligns with their distinct requirements—IQA needs objective, low-level cues, while IAA requires high-level semantic judgment.

Method: TATAR introduces a unified framework with task-specific reasoning (fast-slow), two-stage learning (SFT+GRPO), and asymmetric rewards (Gaussian shaping for IQA, Thurstone ranking for IAA).

Result: TATAR outperforms prior unified baselines across eight benchmarks, matches task-specific models, and improves training stability for aesthetic assessment.

Conclusion: Task-conditioned post-training is a viable paradigm for unified perceptual scoring, as demonstrated by TATAR's effectiveness in addressing the mismatches between IQA and IAA.
...
TL;DR: DeSC introduces a decoupled framework for video anomaly detection, improving performance by separating sensitivity and consistency into specialized streams.


<details>
  <summary>Details</summary>
Motivation: Address the sensitivity-stability trade-off in existing unified frameworks for weakly supervised video anomaly detection.

Method: Proposes DeSC, a decoupled framework with two streams: temporal sensitivity for abrupt changes and semantic consistency for long-term coherence.

Result: Achieves 89.37% AUC on UCF-Crime and 87.18% AP on XD-Violence, outperforming previous methods.

Conclusion: DeSC effectively balances transient and sustained anomaly detection, setting new state-of-the-art results.
...
TL;DR: HOP3D introduces hierarchical orthogonal prototypes and an entropy-based regularizer for few-shot 3D point cloud segmentation, achieving robust novel-class adaptation without degrading base-class performance.


<details>
  <summary>Details</summary>
Motivation: The challenge is balancing adaptation to novel classes while preserving base-class performance in few-shot 3D point cloud segmentation.

Method: HOP3D uses hierarchical orthogonalization to decouple base and novel learning at gradient and representation levels, and an entropy-based regularizer to refine prototypes under sparse supervision.

Result: HOP3D outperforms state-of-the-art baselines on ScanNet200 and ScanNet++ in 1-shot and 5-shot settings.

Conclusion: HOP3D effectively mitigates base-novel interference and enhances few-shot adaptation, demonstrating superior performance.
...
TL;DR: The paper highlights a misalignment in generative OCR, where autoregressive decoding prioritizes plausibility over visual grounding, leading to errors. It introduces a Geometric Risk Controller to ensure reliable outputs through cross-view consensus.


<details>
  <summary>Details</summary>
Motivation: To address the deployment misalignment in generative OCR, where traditional methods risk severe errors due to over-generation and unsupported substitutions.

Method: Proposes a model-agnostic Geometric Risk Controller that probes structured views, applies lightweight screening, and accepts transcriptions based on cross-view consensus.

Result: Experiments show consistent reductions in extreme errors and over-generation, with predictable coverage costs.

Conclusion: Reliable generative OCR requires explicit risk control, not unconstrained generation, as demonstrated by the proposed controller.
...
TL;DR: The paper introduces Modular Body-Part Phase Control, a framework for fine-grained, localized motion editing in text-to-motion generation, addressing challenges in maintaining coherence while modifying specific body parts.


<details>
  <summary>Details</summary>
Motivation: Existing methods for text-to-motion generation struggle with modifying specific body parts without disrupting overall motion coherence, often relying on complex joint constraints. The authors aim to provide a more user-friendly and interpretable solution.

Method: Proposes a plug-and-play framework using sinusoidal phase signals (amplitude, frequency, phase shift, offset) to model body-part motion dynamics. A modular Phase ControlNet branch integrates this signal via residual feature modulation, decoupling control from the generative backbone.

Result: Experiments on diffusion- and flow-based models show the framework enables predictable, fine-grained control over motion magnitude, speed, and timing while preserving global motion coherence.

Conclusion: The Modular Body-Part Phase Control framework offers a practical and interpretable approach for controllable text-to-motion generation, simplifying iterative refinement and localized editing.
...
TL;DR: The paper explores whether Vision Foundation Models (VFMs) can enhance interactive semantic segmentation and object classification in biomedical imaging, comparing their performance with traditional shallow learning methods.


<details>
  <summary>Details</summary>
Motivation: Despite advancements in deep learning, feature-based shallow learning remains dominant in interactive semantic segmentation and object classification in biomedical imaging due to data diversity, lack of large pretraining datasets, and efficiency needs. This study investigates if VFMs can overcome these limitations and improve performance.

Method: The study evaluates multiple VFMs (general-purpose like SAM, SAM2, DINOv3; domain-specific like µSAM, PathoSAM) combined with shallow learning and attentive probing on five diverse datasets.

Result: Results show consistent improvements over traditional hand-crafted features, indicating a viable path for practical advancements in pixel and object classification tasks.

Conclusion: The study establishes a benchmark for VFMs in microscopy, demonstrating their potential and guiding future developments in the field.
...
TL;DR: SeGroS is a fine-tuning framework for Unified Multimodal Models (UMMs) that addresses granularity mismatch and supervisory redundancy using semantic visual grounding and corrupted inputs, improving generation fidelity and cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Current UMM training paradigms have limitations, such as granularity mismatch and supervisory redundancy, which SeGroS aims to resolve.

Method: SeGroS introduces semantic visual hints to address text prompt sparsity and generates semantically-grounded corrupted inputs to enhance supervision in masking-based UMMs.

Result: SeGroS significantly improves generation fidelity and cross-modal alignment, as demonstrated on benchmarks like GenEval, DPGBench, and CompBench.

Conclusion: SeGroS effectively enhances UMM performance by providing better supervision signals, making it a promising solution for multimodal modeling challenges.
...
TL;DR: HUGE-Bench is a new benchmark for High-Level UAV Vision-Language-Action tasks, focusing on concise language interpretation and safe execution of complex trajectories, with process-oriented and collision-aware metrics.


<details>
  <summary>Details</summary>
Motivation: Existing UAV VLN benchmarks are limited by their focus on long, step-wise descriptions and goal-centric evaluation, which don't align with real-world operations requiring brief, high-level commands and safety-aware behaviors.

Method: The benchmark includes 4 real-world digital twin scenes, 8 high-level tasks, and 2.56M meters of trajectories, using an aligned 3D Gaussian Splatting-Mesh representation for photorealistic rendering and collision-aware evaluation.

Result: Experiments show significant gaps in high-level semantic completion and safe execution among state-of-the-art VLA models, demonstrating HUGE-Bench's diagnostic value.

Conclusion: HUGE-Bench serves as a critical tool for advancing high-level UAV autonomy by addressing the limitations of existing benchmarks and providing comprehensive metrics for process fidelity, terminal accuracy, and safety.
...
TL;DR: Fourier Splatting introduces scalable primitives for radiance field rendering, enabling quality adjustment without pruning, achieving high fidelity and versatility.


<details>
  <summary>Details</summary>
Motivation: Existing methods link visual fidelity to primitive count, limiting scalability. The goal is to create inherently scalable primitives for flexible detail adjustment.

Method: Uses Fourier-encoded descriptors for scalable primitives, a straight-through estimator for stable optimization, and HYDRA for densification within MCMC.

Result: Achieves top-tier rendering quality among planar-primitive methods and matches perceptual metrics of leading volumetric representations.

Conclusion: Fourier Splatting offers a scalable, high-fidelity solution for bandwidth-constrained rendering, outperforming current methods.
...
TL;DR: Hyper-Connections (HC) improve 3D brain tumor segmentation across multiple architectures, with gains in accuracy and sensitivity to clinically dominant imaging sequences.


<details>
  <summary>Details</summary>
Motivation: To enhance multi-modal brain tumor segmentation by replacing fixed residual connections with dynamic Hyper-Connections for better feature fusion.

Method: Integrate HC as a drop-in replacement for fixed connections in five architectures (nnU-Net, SwinUNETR, VT-UNet, U-Net, U-Netpp) and evaluate on BraTS 2021 dataset.

Result: HC improves 3D models by up to +1.03% mean Dice, with notable gains in Enhancing Tumor sub-region and sharper sensitivity to dominant imaging sequences (T1ce, FLAIR).

Conclusion: HC is a simple, efficient, and broadly applicable mechanism for improving multi-modal feature fusion in medical image segmentation, particularly in 3D.
...
TL;DR: The paper proposes a framework to measure failure modes in deep learning-based online mapping for autonomous driving, focusing on memorization and overfitting, and introduces new evaluation metrics and dataset improvements.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for online mapping often fail to generalize beyond familiar environments. The study aims to identify and measure these failure modes to improve model robustness and dataset design.

Method: The framework disentangles memorization and overfitting effects using controlled evaluation subsets. It introduces Fréchet distance-based reconstruction statistics and failure-mode scores (localization overfitting and map geometry overfitting). Dataset biases are analyzed with MST diversity and symmetric coverage measures, leading to an MST-based sparsification strategy.

Result: Experiments on nuScenes and Argoverse 2 show improved generalization assessment and performance with map geometry-diverse and balanced training sets.

Conclusion: The results advocate for failure-mode-aware evaluation protocols and map geometry-centric dataset design to enhance deployable online mapping solutions.
...
TL;DR: The paper studies intra-modal misalignment in CLIP, identifies issues with projectors in shared embedding space, and proposes a training-free method to improve intra-modal alignment for tasks like image retrieval.


<details>
  <summary>Details</summary>
Motivation: To address the performance drop in intra-modal tasks (e.g., image-to-image retrieval) caused by intra-modal misalignment in CLIP-like models, despite their success in inter-modal tasks.

Method: Analyzes the cosine similarity of projected features and the contrastive CLIP loss to identify inter- and intra-modal operators. Uses spectral analysis to find isotropic and anisotropic subspaces and removes anisotropic directions to improve alignment.

Result: Demonstrates that removing anisotropic directions enhances intra-modal alignment, reduces latency, and outperforms existing methods on intra-modal retrieval and classification benchmarks.

Conclusion: The proposed training-free method effectively reduces intra-modal misalignment in CLIP-like models, improving performance for intra-modal tasks without additional training.
...
TL;DR: MedQ-Engine introduces a closed-loop data engine to improve medical image quality assessment in AI models, reducing the gap with human experts significantly.


<details>
  <summary>Details</summary>
Motivation: The high cost of descriptive annotations and the static nature of one-time data collection hinder the improvement of multimodal large language models in medical image quality assessment.

Method: MedQ-Engine uses data-driven clustering to identify failure prototypes, human-in-the-loop annotation of a large image pool, and quality-assured fine-tuning in a self-improving cycle. An entropy-guided routing mechanism optimizes annotation costs.

Result: The system elevates an 8B-parameter model to surpass GPT-4o by over 13% and narrows the gap with human experts to 4.34%, using only 10K annotations with 4x sample efficiency over random sampling.

Conclusion: MedQ-Engine effectively addresses the challenges of data annotation and model adaptation, significantly improving performance in medical image quality assessment.
...
TL;DR: SIMPLER is a pre-fine-tuning method that reduces inference and deployment costs by selecting effective model depth, achieving significant speedups and performance retention.


<details>
  <summary>Details</summary>
Motivation: High computational costs of fine-tuning foundation models for Earth Observation motivate the need for efficient methods that reduce training and inference complexity.

Method: SIMPLER computes layer-wise representation similarity on unlabeled data to identify redundant layers, requiring no gradients or hyperparameter tuning.

Result: The method prunes up to 79% of parameters while retaining 94% performance, with 2.1x training and 2.6x inference speedups.

Conclusion: SIMPLER generalizes across tasks and architectures, offering a practical solution for efficient model adaptation.
...
TL;DR: The paper introduces ATCG, a plug-and-play module that enhances GCD by combining textual and visual features, improving performance on fine-grained data.


<details>
  <summary>Details</summary>
Motivation: Current GCD methods rely heavily on visual-only pipelines, leading to weak boundaries for fine-grained categories. The authors aim to improve this by incorporating textual concepts derived from labeled knowledge.

Method: ATCG generates analogical textual concepts for unlabeled samples by leveraging labeled knowledge. These concepts are fused with visual features, transforming discovery into a visual-textual reasoning process.

Result: ATCG consistently improves performance across six benchmarks, particularly excelling on fine-grained datasets, for both known and novel classes.

Conclusion: ATCG effectively enhances GCD by integrating textual and visual reasoning, demonstrating significant improvements in category separation and overall performance.
...
### [81] [Adaptive Greedy Frame Selection for Long Video Understanding](https://arxiv.org/abs/2603.20180)
*Yuning Huang,Fengqing Zhu*

Main category: cs.CV

TL;DR: The paper introduces a question-adaptive greedy frame selection method for long-video question answering to optimize query relevance and semantic representativeness under a fixed frame budget.


<details>
  <summary>Details</summary>
Motivation: Existing methods for frame selection in large vision-language models struggle with balancing relevance and coverage, often missing key moments or producing redundant frames.

Method: The proposed method selects frames by combining query relevance (using SigLIP) and semantic representativeness (using DINOv2), guided by a greedy optimization approach with normalized, monotone, and submodular objectives.

Result: Experiments on MLVU demonstrate consistent accuracy improvements over uniform sampling and recent baselines, especially under tight frame budgets.

Conclusion: The question-adaptive greedy frame selection method effectively balances relevance and coverage, enhancing performance in long-video question answering tasks.
...
### [82] [PanORama: Multiview Consistent Panoptic Segmentation in Operating Rooms](https://arxiv.org/abs/2603.19920)
*Tuna Gürbüz,Ege Özsoy,Tony Danjun Wang,Nassir Navab*

Main category: cs.CV

TL;DR: PanORama introduces a novel, calibration-free panoptic segmentation method for operating rooms, ensuring multiview consistency directly in the backbone, achieving >70% Panoptic Quality and outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Enhancing spatial understanding in cluttered, dynamic operating rooms (ORs) is crucial, but limited visibility in sparse multiview images often causes segmentation errors.

Method: PanORama models cross-view interactions at the feature level in a single forward pass, eliminating the need for post-hoc refinement or camera calibration.

Result: Achieves >70% Panoptic Quality on MM-OR and 4D-OR datasets, outperforming state-of-the-art methods without needing camera parameters.

Conclusion: PanORama improves multiview segmentation in ORs, advancing surgical perception and assistance capabilities.
...
TL;DR: VideoSeek is a long-horizon video agent that efficiently seeks answer-critical evidence using video logic flow, reducing computational costs while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Most video agentic models rely on densely sampled frames, leading to high computational costs; VideoSeek addresses this by focusing on answer-critical evidence.

Method: VideoSeek operates in a think-act-observe loop with a toolkit for multi-granular observations, enabling query-aware exploration and reasoning.

Result: VideoSeek achieves strong accuracy with fewer frames, outperforming GPT-5 on LVBench by 10.2 points while using 93% fewer frames.

Conclusion: VideoSeek demonstrates the importance of video logic flow, reasoning capability, and toolkit design for efficient and accurate video understanding.
...
TL;DR: SegVGGT is an end-to-end framework for 3D reconstruction and instance segmentation from multi-view RGB images, using object queries and FADA to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing 3D instance segmentation methods rely on high-quality inputs and complex pipelines, while recent transformers lack semantic integration. SegVGGT aims to unify these tasks efficiently.

Method: SegVGGT integrates instance identification into a visual geometry transformer using object queries and FADA to manage attention dispersion.

Result: State-of-the-art performance on ScanNetv2 and ScanNet200, outperforming joint models and RGB-D-based approaches, with strong generalization on ScanNet++.

Conclusion: SegVGGT effectively unifies 3D reconstruction and instance segmentation, offering robust performance and generalization.
...
TL;DR: RAM uses a motion-aware semantic tracker with adaptive Kalman filtering, a Temporal HMR module, and a Predictor module to enhance 3D human motion capture, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: To address challenges like occlusions and dynamic interactions in 3D human motion capture, RAM aims to provide robust identity association and smooth motion estimation.

Method: RAM integrates a motion-aware semantic tracker with adaptive Kalman filtering, a memory-augmented Temporal HMR module for spatio-temporal priors, and a lightweight Predictor module for future pose forecasting.

Result: RAM significantly outperforms previous methods in Zero-shot tracking stability and 3D accuracy on benchmarks like PoseTrack and 3DPW.

Conclusion: RAM offers a generalizable paradigm for markerless 3D human motion capture in real-world scenarios, achieving robustness and coherence.
...
TL;DR: LIORNet is a self-supervised learning-based method for filtering noise in LiDAR point clouds under adverse weather, outperforming existing approaches without manual annotations.


<details>
  <summary>Details</summary>
Motivation: LiDAR performance degrades in snow, rain, and fog due to noise. Existing methods (distance, intensity, or learning-based) have limitations like poor adaptability or high annotation costs.

Method: LIORNet uses U-Net++ and self-supervised learning with pseudo-labels from physical/statistical cues (intensity thresholds, snow reflectivity, etc.) to distinguish noise without manual annotations.

Result: Experiments on WADS and CADC datasets show LIORNet outperforms state-of-the-art methods in accuracy and runtime while preserving environmental features.

Conclusion: LIORNet is a robust, practical solution for LiDAR noise filtering in extreme weather, suitable for real-time autonomous driving deployment.
...
TL;DR: A novel framework optimizes pre-trained Diffusion Probabilistic Models (DPMs) by dynamically adjusting computational paths per timestep, improving efficiency without sacrificing generative quality.


<details>
  <summary>Details</summary>
Motivation: DPMs face high inference latency due to iterative denoising. The paper aims to reduce this latency by optimizing computational paths dynamically.

Method: The proposed method learns timestep-specific masks to decide which blocks to execute or bypass, using a memory-efficient training process with timestep-aware loss scaling and knowledge-guided mask rectification.

Result: The approach demonstrates significant efficiency gains across various models (e.g., DDPM, LDM, DiT, PixArt) while maintaining generative quality.

Conclusion: The method effectively balances sampling speed and quality by treating denoising as a sequence of optimized computational paths, offering a practical solution for DPM inference.
...
TL;DR: HiPath is a lightweight vision-language model framework for structured pathology report prediction, using hierarchical modules for visual encoding, cross-modal alignment, and structured diagnosis generation, achieving high accuracy and safety.


<details>
  <summary>Details</summary>
Motivation: Existing pathology vision-language models simplify structured reports into flat labels or free-form text, neglecting the multi-granular nature of pathology reports. HiPath aims to address this gap.

Method: HiPath employs three trainable modules: HiPA for multi-image visual encoding, HiCL for cross-modal alignment via optimal transport, and Slot-MDP for structured diagnosis generation, totaling 15M parameters.

Result: Trained on 749K real-world cases, HiPath achieves 68.9% strict and 74.7% clinically acceptable accuracy with a 97.3% safety rate, outperforming baselines. Cross-hospital evaluation shows robust generalization.

Conclusion: HiPath demonstrates superior performance in structured pathology report prediction, maintaining high accuracy and safety, and generalizes well across hospitals.
...
TL;DR: The paper introduces a covariance-pooled representation and a novel pose encoding using SPD matrices for more accurate 6-DoF pose estimation from a single RGB image.


<details>
  <summary>Details</summary>
Motivation: Indirect methods for pose estimation are accurate but less efficient, while direct methods are efficient but less accurate due to reliance on globally pooled features and discontinuous pose representations. The paper aims to improve direct methods by leveraging spatial second-order statistics and continuous representations.

Method: The proposed method uses a covariance-pooled representation to encode feature distributions as an SPD matrix and introduces a novel pose encoding via Cholesky decomposition. A manifold-aware network head regresses the pose end-to-end, considering Riemannian geometry.

Result: Experiments show improved accuracy in direct pose regression, demonstrating the effectiveness of second-order pooling and continuous representations, even under partial occlusion.

Conclusion: The proposed approach advances direct pose estimation by incorporating spatial statistics and continuous representations, enhancing both accuracy and robustness.
...
TL;DR: 2K Retrofit enables efficient high-resolution (2K) geometric inference without modifying existing models, using coarse predictions and sparse refinement for high-accuracy results with low overhead.


<details>
  <summary>Details</summary>
Motivation: Current foundation models struggle with high-resolution (2K) geometric prediction due to high computational and memory costs, limiting real-world deployment.

Method: 2K Retrofit uses fast coarse predictions and entropy-based sparse refinement to selectively enhance uncertain regions, minimizing overhead.

Result: The framework achieves state-of-the-art accuracy and speed on benchmarks, enabling scalable high-resolution 3D vision applications.

Conclusion: 2K Retrofit bridges the gap between research and practical deployment in high-resolution geometric prediction, offering a viable solution for real-world use.
...
TL;DR: X-World is an action-conditioned multi-camera generative world model that simulates future driving observations in video space, offering scalable and reproducible evaluation for autonomous driving policies.


<details>
  <summary>Details</summary>
Motivation: Current evaluation pipelines for autonomous driving rely heavily on costly and limited real-world road testing, necessitating a controllable and stable simulator for realistic future observation generation.

Method: X-World generates future multi-camera video streams from synchronized history and future actions, with optional controls over traffic agents, road elements, and appearance (e.g., weather). It uses a multi-view latent video generator to ensure cross-view consistency and temporal coherence.

Result: X-World achieves high-quality multi-view video generation with strong view consistency, stable dynamics, and high controllability, adhering to action commands and scene controls.

Conclusion: X-World provides a practical foundation for scalable and reproducible evaluation of autonomous driving policies, overcoming limitations of real-world testing.
...
TL;DR: MedSPOT introduces a workflow-aware sequential grounding benchmark for clinical GUI environments, addressing the need for dynamic, multi-step reasoning in medical software.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook sequential, workflow-driven reasoning in real-world medical interfaces, limiting the reliability of Multimodal Large Language Models (MLLMs) in high-stakes clinical settings.

Method: MedSPOT models procedural interaction as a sequence of structured spatial decisions, using 216 task-driven videos with 597 annotated keyframes to capture interface hierarchies and contextual dependencies.

Result: The benchmark evaluates procedural robustness via a strict sequential protocol and introduces a failure taxonomy to diagnose model behavior, emphasizing error propagation in multi-step workflows.

Conclusion: MedSPOT provides a realistic and safety-critical benchmark for assessing multimodal models in clinical GUI environments, shifting focus from isolated grounding to workflow-aware sequential reasoning.
...
TL;DR: TTA methods improve FER performance under natural domain shifts, with effectiveness depending on distributional distance and shift severity.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of deep learning models struggling under natural distribution shifts during real-world deployments.

Method: Evaluated TTA methods for FER under natural domain shifts via cross-dataset experiments, focusing on entropy minimization, prototype adjustment, and feature alignment.

Result: TTA boosted FER performance by up to 11.34%, with method effectiveness varying by target distribution conditions (clean, noisy, or large shift).

Conclusion: TTA is effective for FER under natural shifts, with method choice depending on the specific distributional characteristics of the target domain.
...
TL;DR: NEC-Diff is a diffusion-based framework combining RAW images and event camera data to reconstruct high-quality images in extreme low-light conditions, addressing noise and texture loss through physics-driven constraints and adaptive feature fusion.


<details>
  <summary>Details</summary>
Motivation: High-quality imaging in extreme low-light is challenging due to noise and texture loss from photon scarcity. Event cameras complement conventional cameras but existing methods neglect noise, limiting accurate reconstruction.

Method: NEC-Diff combines RAW images and event data, using physics-driven constraints for denoising and adaptive feature fusion guided by SNR estimation to enhance reconstruction.

Result: The framework outperforms existing methods in extreme darkness, validated by the REAL dataset with 47,800 low-light samples.

Conclusion: NEC-Diff effectively reconstructs fine structures in extreme low-light, leveraging event-RAW hybrid imaging and adaptive denoising for superior results.
...
TL;DR: The paper proposes FRAM, a two-stage method for makeup transfer using fine-tuned makeup CLIP and region-aware features, addressing limitations of existing diffusion-based methods.


<details>
  <summary>Details</summary>
Motivation: Current makeup transfer methods using foundation models like CLIP struggle with capturing makeup styles and lack regional controllability.

Method: FRAM involves (1) fine-tuning CLIP with synthesized makeup data and (2) injecting identity and region-aware makeup features using learnable tokens and ControlNet Union.

Result: Experiments show improved regional controllability and superior makeup transfer performance.

Conclusion: FRAM effectively addresses the limitations of existing methods by enabling precise regional makeup transfer and better style preservation.
...
TL;DR: The paper proposes a coarse-to-fine crossmodal learning (CFCML) framework to reduce the modality gap between medical images and tabular data for disease diagnosis, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Address the neglect of local image information and task-relevant extraction in existing crossmodal learning methods for improved disease diagnosis.

Method: Introduces a two-stage approach: coarse stage explores multi-granularity feature relationships, while fine stage uses prototypes and hierarchical anchor-based mining to enhance inter-class disparity.

Result: Achieves improvements of 1.53% and 0.91% in AUC metrics on MEN and Derm7pt datasets, respectively.

Conclusion: CFCML effectively reduces modality gaps and enhances diagnostic accuracy, demonstrating superior performance over existing methods.
...
TL;DR: The paper identifies a gradient interference issue in multimodal large language models (MLLMs) during multi-layer feature fusion, proposes Detached Skip-Links to stabilize training, and introduces $R$-Probe to evaluate fine-grained information retention. Their method consistently improves performance on OCR and general multimodal tasks.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with OCR tasks due to compromised fine-grained visual details caused by gradient interference in multi-layer feature fusion. The authors aim to address this issue to improve model performance.

Method: They propose Detached Skip-Links, which reuses shallow features in the forward pass while stopping gradients through the skip branch during joint training. They also introduce $R$-Probe to assess fine-grained information retention.

Result: The method improves stability and convergence, consistently enhancing performance on OCR-centric benchmarks and general multimodal tasks across various ViT backbones and scales up to 7M training samples.

Conclusion: Detached Skip-Links and $R$-Probe effectively mitigate gradient interference and preserve fine-grained visual details, leading to better performance in MLLMs for both OCR and general multimodal tasks.
...
TL;DR: The paper proposes a preference-guided debiasing framework for no-reference enhancement image quality assessment (EIQA) to improve generalization by focusing on perceptual quality rather than enhancement-specific patterns.


<details>
  <summary>Details</summary>
Motivation: Existing NR-IQA models for enhanced images often overfit to specific enhancement algorithms, failing to generalize and evaluate genuine perceptual quality.

Method: The framework learns an enhancement-preference embedding space using supervised contrastive learning, estimates and removes enhancement-induced nuisance components, and employs a two-stage training strategy for stable optimization.

Result: The method outperforms existing approaches on public EIQA benchmarks, showing better robustness and cross-algorithm generalization.

Conclusion: The proposed framework effectively mitigates algorithm-induced bias and improves EIQA by focusing on algorithm-invariant perceptual quality cues.
...
TL;DR: CoA framework integrates domain knowledge into models without losing general capabilities, outperforming fine-tuning in accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: To prevent pretrained models from losing their generalization abilities when fine-tuned on domain-specific data.

Method: Uses Chain-of-Adaptation (CoA) with reinforcement learning to align domain knowledge while preserving reasoning and perceptual capabilities.

Result: Outperforms supervised fine-tuning in accuracy, generalization, and stability on surgical benchmarks.

Conclusion: CoA provides a reliable solution for domain specialization without compromising core visual-language abilities in VLMs.
...
TL;DR: The paper introduces Generalizable NGP-SR, a 3D-aware super-resolution framework that directly reconstructs high-resolution radiance fields from low-resolution images, ensuring view consistency and eliminating the need for per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: High-resolution rendering with NeRF is computationally expensive, and 2D super-resolution methods often fail to maintain multi-view consistency. The authors aim to address these limitations.

Method: The proposed NGP-SR framework leverages Neural Graphics Primitives (NGP) to condition radiance predictions on 3D coordinates and learned local texture tokens, enabling high-frequency detail recovery within the radiance field. It avoids external HR references or 2D upsampling.

Result: Experiments demonstrate that NGP-SR outperforms prior NeRF-based super-resolution methods in both reconstruction quality and runtime efficiency, while maintaining view consistency.

Conclusion: NGP-SR provides a scalable and practical solution for high-resolution novel view synthesis, generalizing well to unseen scenes and viewpoints without requiring per-scene optimization.
...
TL;DR: FacadeFixer is a multi-agent framework for facade defect inspection, addressing challenges like pixel imbalance and data scarcity by combining detection, segmentation, and generative synthesis.


<details>
  <summary>Details</summary>
Motivation: Facade defect inspection is challenging due to geometric variability, low contrast, and composite defects, which hinder existing models' generalization due to data scarcity and feature ambiguity.

Method: FacadeFixer uses specialized agents for detection and segmentation, coupled with a generative agent to decouple defects from backgrounds and synthesize high-fidelity augmented data with expert-level masks. A comprehensive multi-task dataset is introduced.

Result: FacadeFixer outperforms state-of-the-art baselines, excelling in pixel-level structural anomaly detection and demonstrating generative synthesis as a solution to data scarcity.

Conclusion: FacadeFixer provides an effective framework for facade defect inspection, leveraging collaborative reasoning and generative synthesis to overcome challenges in infrastructure inspection.
...
TL;DR: The paper introduces _DefectBench_, a benchmark for evaluating Large Multimodal Models (LMMs) in automated building facade inspection, highlighting their strengths in semantic understanding but weaknesses in precise localization.


<details>
  <summary>Details</summary>
Motivation: Current discriminative models for facade inspection lack active reasoning and generalizability. LMMs offer potential but need rigorous evaluation in engineering contexts.

Method: A human-in-the-loop annotation framework unifies datasets, creating _DefectBench_ to test LMMs across semantic, spatial, and geometric tasks.

Result: LMMs excel in topological awareness and semantics but struggle with precise localization; zero-shot generative segmentation matches specialized models.

Conclusion: _DefectBench_ sets a new standard for AI in civil engineering, showcasing LMMs' potential while identifying areas for improvement.
...
TL;DR: EgoForge is an egocentric goal-directed world simulator that generates first-person video from minimal inputs, improving intent alignment and consistency with a novel refinement method.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in simulating dynamic environments from egocentric video due to viewpoint changes, hand-object interactions, and latent human intent.

Method: Introduces EgoForge and VideoDiffusionNFT, a reward-guided refinement technique for diffusion sampling to enhance goal completion, temporal causality, scene consistency, and perceptual fidelity.

Result: EgoForge outperforms baselines in semantic alignment, geometric stability, and motion fidelity, demonstrating robust performance in real-world applications.

Conclusion: The proposed method advances egocentric video simulation by generating coherent video from minimal inputs, validated through experiments and real-world testing.
...
TL;DR: I2I-RFR reformulates standard image-to-image (I2I) regression networks as continuous-time transport models, improving performance with minimal changes to the training pipeline.


<details>
  <summary>Details</summary>
Motivation: Pixel-wise I2I regression is simple but often over-smooths complex targets, while generative models require added complexity. I2I-RFR aims to bridge this gap without heavy modifications.

Method: Augments backbone input with noise-corrupted ground-truth targets and optimizes a t-reweighted pixel loss, enabling ODE-based refinement at inference.

Result: I2I-RFR enhances performance across various tasks, especially perceptual quality and detail retention, with minimal inference steps.

Conclusion: I2I-RFR offers a lightweight way to integrate continuous-time refinement into I2I models, avoiding complex generative pipelines.
...
TL;DR: The paper introduces MuSteerNet, a framework for generating 3D human reactions from videos by addressing relational distortion between visual inputs and reaction outputs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for video-driven human reaction generation struggle to align synthesized motions with video content. This misalignment stems from relational distortion between observations and reactions.

Method: MuSteerNet employs a Prototype Feedback Steering mechanism to refine visual observations and a Dual-Coupled Reaction Refinement to enhance reaction quality using rectified cues.

Result: Extensive experiments show MuSteerNet effectively improves reaction quality and achieves competitive performance.

Conclusion: The proposed framework successfully mitigates relational distortion and enhances the synthesis of human reactions from videos.
...
TL;DR: The paper introduces a reasoning-first, zero-shot approach for Composed Video Retrieval (CoVR) that leverages large multimodal models to infer implicit consequences of edits, outperforming baselines and improving interpretability.


<details>
  <summary>Details</summary>
Motivation: Prior work in CoVR overlooks after-effects and implicit consequences of textual modifications. The authors argue that reasoning about these effects is crucial for successful retrieval.

Method: The proposed method uses large multimodal models to (i) infer causal and temporal consequences of edits and (ii) align reasoned queries to videos without task-specific finetuning. A new benchmark, CoVR-Reason, is introduced to evaluate reasoning.

Result: The zero-shot method outperforms baselines on recall metrics, especially on implicit-effect subsets, and shows higher step consistency and effect factuality.

Conclusion: Incorporating reasoning reduces dependence on task-specific supervision, improves generalization, and enhances interpretability, providing a scalable framework for explainable video search.
...
TL;DR: A deterministic framework for generating multiple segmentation masks efficiently, reducing computational cost and improving accuracy compared to generative models.


<details>
  <summary>Details</summary>
Motivation: Addressing the ambiguity in segmentation tasks by avoiding the computationally expensive sampling and clustering required by current generative models.

Method: Introduces mode proposal models to directly generate a fixed set of likely outcomes in one forward pass, using a confidence mechanism for refinement.

Result: Reduces inference time significantly while achieving higher ground-truth coverage and applicability to real-world datasets without full outcome distribution knowledge.

Conclusion: The proposed method offers a scalable and efficient alternative to generative models for ambiguous segmentation tasks.
...
TL;DR: LumosX is a framework improving text-to-video generation by integrating explicit modeling and face-attribute-aware data to enhance intra-group consistency and fine-grained control.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack mechanisms to ensure precise face-attribute alignment across subjects in video generation, prompting the need for better modeling and data resources.

Method: LumosX combines a tailored data pipeline with multimodal large language models for relational priors, and introduces Relational Self-Attention and Cross-Attention to enforce subject-attribute dependencies.
...
Conclusion: The framework effectively addresses the challenge of intra-group consistency in text-to-video generation, offering improved control and performance.
...
TL;DR: The paper reformulates image tampering detection from coarse region labels to a pixel-level, meaning-aware task, introducing a taxonomy, a new benchmark, and a framework for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks misalign with true edit signals by relying on object masks, ignoring untouched pixels and subtle edits. The paper aims to address this by focusing on pixel-level, semantic-aware detection.

Method: Introduces a taxonomy for edit primitives and their semantic classes, releases a new benchmark with per-pixel tamper maps, and proposes a training framework and metrics for pixel-level correctness and semantic understanding.

Result: Highlights misalignment in existing mask-based metrics, exposing failure modes on micro-edits and off-mask changes. The new framework improves localization, classification, and description of tampered regions.

Conclusion: The paper advances tamper detection from masks to pixels and semantic understanding, setting a rigorous standard for localization, classification, and description.
...
TL;DR: The paper introduces MME-CoF-Pro, a benchmark to evaluate reasoning coherence in video generative models, revealing weaknesses in coherence despite generation quality.


<details>
  <summary>Details</summary>
Motivation: To address the lack of evaluation for reasoning coherence in video models, ensuring generated events remain causally consistent.

Method: Proposes MME-CoF-Pro with 303 samples across 16 categories, using Reasoning Score and three hint-based evaluation settings.

Result: Finds video models weak in reasoning coherence, with text hints causing inconsistencies and visual hints aiding structured tasks.

Conclusion: Highlights gaps in reasoning coherence and the need for improved evaluation methods in video generative models.
...
TL;DR: The paper explores how automated adversarial prompt refinement can exploit vulnerabilities in LLMs, demonstrating reduced safety safeguards, especially in small models, and advocates for adaptive red-teaming in safety evaluations.


<details>
  <summary>Details</summary>
Motivation: To address the gap in existing safety evaluations that assume non-adaptive adversaries, the study investigates realistic attack scenarios where inputs are iteratively refined to bypass safeguards.

Method: Repurposing black-box prompt optimization techniques, the authors systematically search for safety failures using DSPy, applying three optimizers to prompts from HarmfulQA and JailbreakBench, with danger scores evaluated by GPT-5.1.

Result: The study shows a significant reduction in safety safeguards, with danger scores increasing dramatically (e.g., Qwen 3 8B's score rises from 0.09 to 0.79).

Conclusion: Static benchmarks may underestimate residual risk, highlighting the need for automated, adaptive red-teaming as part of robust safety evaluations.
...
TL;DR: DuCCAE is a hybrid conversational system that decouples real-time responses from asynchronous tasks to improve responsiveness and task capability, showing significant improvements in user retention and task completion.


<details>
  <summary>Details</summary>
Motivation: Address the trade-off between responsiveness and long-horizon task capability in immersive conversational systems, which affects turn-taking, persona consistency, and user trust.

Method: Proposes DuCCAE, a hybrid engine that decouples real-time response generation from asynchronous agentic execution, using a shared state for synchronization. It involves five subsystems for multi-agent collaboration and continuous improvement.

Result: DuCCAE outperforms baselines in execution reliability and dialogue quality, with reduced latency. Deployment metrics show a tripling of Day-7 user retention (34.2%) and a surge in complex task completion (65.2%).

Conclusion: The hybrid architecture successfully balances conversational continuity and reliable agentic execution, providing scalable deployment guidelines for industrial settings.
...
TL;DR: Spelling correction in healthcare QA significantly improves retrieval performance, especially when applied to queries rather than just the corpus.


<details>
  <summary>Details</summary>
Motivation: Address the high rate of spelling errors in consumer health queries, which hampers the effectiveness of QA systems.

Method: Evaluated four spelling correction methods on two datasets (TREC 2017 LiveQA and HealthSearchQA) under three conditions: uncorrected queries, corpus correction, and full correction. Used BM25 and TF-IDF for retrieval.

Result: Query correction improved MRR by +9.2% and NDCG@10 by +8.3%. Corpus-only correction showed minimal improvement (+0.5% MRR).

Conclusion: Query-side spelling correction is crucial for enhancing healthcare QA systems, with edit distance and context-aware methods performing well.
...
TL;DR: StreamBench, a new benchmark for evaluating language models in streaming environments, addresses the gap in assessing models with multiple concurrent events. It includes tasks like Topic Clustering, Temporal QA, and Summarization, showing that structural cues improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to evaluate language models under real-world streaming conditions where multiple concurrent events occur. This work aims to fill that gap.

Method: StreamBench is introduced, built from major news stories (2016 and 2025), with 605 events and 15,354 documents. It includes three tasks: Topic Clustering, Temporal QA, and Summarization. Structural cues are used to diagnose model performance.

Result: Structural cues improve clustering (up to +4.37%) and temporal QA (up to +9.63%), helping models locate and separate relevant information. Temporal reasoning remains a challenge.

Conclusion: Structural cues are a promising direction for improving language models in massive document streams, though temporal reasoning challenges persist.
...
TL;DR: The paper proposes Metadata Enriched Hybrid RAG and Direct Preference Optimization to improve LLMs' reliability in legal contexts, addressing retrieval and decoding errors.


<details>
  <summary>Details</summary>
Motivation: LLMs degrade in long legal documents, producing errors like incorrect clauses. Precision is critical in legal domains, but current methods like RAG remain limited.

Method: Proposes Metadata Enriched Hybrid RAG for better document retrieval and DPO to enforce safe refusal when context is insufficient.

Result: Combining these methods enhances grounding, reliability, and safety in legal language models.

Conclusion: The proposed solutions address key limitations of LLMs in legal settings, improving their practical utility.
...
TL;DR: GeoChallenge is a dataset of 90K geometry proof problems with multi-step reasoning over text and diagrams, highlighting LLMs' performance gaps and failure patterns.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack scale and visually grounded questions, limiting reliable evaluation of complex reasoning in LLMs.

Method: Introduced GeoChallenge, a dataset with fine-grained complexity ratings and formal annotations for controlled evaluation.

Result: Best LLM (GPT-5-nano) scored 75.89 vs. humans' 94.74, revealing failure patterns like exact match issues and weak visual reliance.

Conclusion: GeoChallenge highlights LLMs' reasoning gaps and provides a robust benchmark for future improvements.
...
TL;DR: The study evaluates advanced LLMs like GPT-5.2 on argument classification tasks, using innovative prompting strategies to improve accuracy, while identifying systematic model weaknesses.


<details>
  <summary>Details</summary>
Motivation: To assess the performance of state-of-the-art LLMs in argument mining and explore the impact of advanced prompting techniques on classification accuracy and robustness.

Method: Evaluates models (GPT-5.2, Llama 4, DeepSeek) on datasets (Args.me, UKP) using strategies like Chain-of-Thought prompting, rephrasing, voting, and certainty-based classification.

Result: GPT-5.2 achieved 78.0% (UKP) and 91.9% (Args.me) accuracy; advanced techniques improved performance by 2-8%. Qualitative analysis revealed common failure modes.

Conclusion: Advanced prompting enhances LLM performance in argument mining, but systematic challenges remain, highlighting the need for further research.
...
TL;DR: FinReasoning introduces a benchmark to evaluate LLMs in generating financial research reports, addressing gaps in current frameworks by assessing semantic consistency, data alignment, and deep insight.


<details>
  <summary>Details</summary>
Motivation: Existing financial benchmarks focus on comprehension rather than evaluating the reliability of completed reports, and current frameworks lack structured measures for deeper analytical skills.

Method: The study decomposes Chinese research-report generation into three stages aligned with real analyst workflows and proposes a fine-grained evaluation framework with a 12-indicator rubric.

Result: Most models show an understanding-execution gap, struggling with generating accurate corrections and correct data formats. No model excels across all three stages, with Doubao-Seed-1.8, GPT-5, and Kimi-K2 leading overall performance.

Conclusion: FinReasoning highlights key analytical bottlenecks in LLMs for financial report generation, providing a tool for improved evaluation and benchmarking.
...
TL;DR: LARFT is a training framework that improves LLMs' ability to follow length instructions by aligning their length cognition with actions via reinforcement learning and hindsight awareness.


<details>
  <summary>Details</summary>
Motivation: Existing methods for controlling output length in LLMs impose external constraints without addressing the model's intrinsic lack of length awareness, leading to suboptimal performance.

Method: LARFT integrates length-oriented reinforcement learning with hindsight length awareness, using on-policy data to train the model to recognize and adjust its output length.

Result: LARFT outperforms baselines, improving performance by +20.92 points on length instruction tasks with only a slight decline (-1.45 points) in general capability benchmarks.

Conclusion: LARFT effectively enhances LLMs' ability to precisely control output length while maintaining general performance, addressing a key limitation in current models.
...
TL;DR: The paper addresses under-served Bengali ASR and speaker diarization by proposing data-centric pipelines and fine-tuning models, achieving competitive performance with limited resources.


<details>
  <summary>Details</summary>
Motivation: Bengali, spoken by 230 million people, lacks robust ASR and speaker diarization solutions, prompting the need for efficient methods in low-resource settings.

Method: For Task~1, a data pipeline uses YouTube audiobooks/dramas with LLM-assisted normalization and augmentation, fine-tuning Whisper-medium. For Task~2, pyannote.audio is fine-tuned with hyperparameter optimization.

Result: Task~1 achieved WERs of 16.751 (public) and 15.551 (private). Task~2 achieved DERs of 0.19974 (public) and 0.26723 (private).

Conclusion: Careful data engineering and adaptive fine-tuning can deliver strong Bengali speech processing results without large datasets.
...
TL;DR: The paper introduces a flexible framework using LLMs to solve constrained path planning problems from natural language input, combining template matching for known problems and in-context learning for novel ones, with iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Traditional path planning methods are inflexible and require dedicated algorithms for each problem variant, making them hard to scale. This work aims to leverage LLMs for a more generalizable and scalable solution.

Method: The framework uses LLMs to interpret natural language inputs, matches known problems to templates, and infers representations for novel problems. It refines solutions iteratively through verification and self-correction.

Result: The framework effectively handles diverse constrained path planning tasks, demonstrating feasibility and scalability with minimal human intervention.

Conclusion: This LLM-based approach offers a flexible, scalable solution for real-world routing problems, enabling natural language problem specification and autonomous optimization.
...
TL;DR: MAPLE improves differentially private synthetic data generation for specialized domains by using metadata and in-context learning, outperforming PE in utility, convergence, and cost.


<details>
  <summary>Details</summary>
Motivation: Differentially private fine-tuning of LLMs via APIs is often impractical, especially in specialized domains where existing methods like PE perform poorly due to initialization issues.

Method: MAPLE employs differentially private tabular metadata extraction and in-context learning to better align the synthetic data with the target domain.

Result: MAPLE achieves better privacy-utility trade-offs, faster convergence, and lower API costs compared to PE in domain-specific tasks.

Conclusion: MAPLE effectively addresses the initialization bottleneck in PE, offering a more efficient and accurate solution for generating DP synthetic data in specialized domains.
...
TL;DR: HATL, a Hierarchical Adaptive Transfer Learning framework, dynamically unfreezes pretrained layers to improve Sign Language Machine Translation, outperforming traditional methods across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Bridging communication between Deaf and hearing individuals via SLMT faces challenges like scarce datasets and domain gaps. Existing static transfer learning methods often overfit, necessitating an adaptive framework.

Method: HATL combines dynamic unfreezing, layer-wise learning rate decay, and stability mechanisms to adapt pretrained representations to sign language characteristics. It uses ST-GCN++ for feature extraction and Transformer/ADAT for translation.

Result: HATL outperforms traditional methods, with ADAT achieving BLEU-4 improvements of 15.0% on PHOENIX14T and Isharah, and 37.6% on MedASL.

Conclusion: HATL effectively addresses overfitting and domain gaps in SLMT, demonstrating robust performance across diverse datasets and tasks.
...
TL;DR: The paper introduces Significance-Gain BPE, an alternative to standard BPE that improves tokenization by combining statistical significance with compression, reducing perplexity by 12-13% and improving bits per character by 0.9-1.0%.


<details>
  <summary>Details</summary>
Motivation: Standard BPE relies on raw pair frequency, which can conflate true adjacency cohesion with high-frequency pairs. The authors propose a statistically grounded alternative to improve tokenization efficiency.

Method: Significance-Gain BPE measures cohesion via a z-statistic under an independence null model and combines it with a compression-aware gain term. It is evaluated on WikiText-103 using a small Transformer model.

Result: The method reduces validation/test perplexity by 13%/12% and improves bits per character (BPC) by 0.9-1.0%. It also shows lower BPC in vocabulary-size comparisons.

Conclusion: Significance-Gain BPE demonstrates that statistically grounded merge selection enhances predictive efficiency across various compression levels.
...
TL;DR: The paper identifies a belief revision exponent in LLMs that governs probability updates, showing that values below one ensure stability. Empirical results suggest near-Bayesian behavior, with stability confirmed over multiple revisions.


<details>
  <summary>Details</summary>
Motivation: To address the lack of principled guarantees for stability in LLMs' probability updates during iterative revisions, the study aims to characterize and validate the update behavior.

Method: The study introduces a belief revision exponent to model how LLMs combine prior beliefs and new evidence. It evaluates stability theoretically and empirically across multiple models and benchmarks.

Result: Empirical findings reveal near-Bayesian behavior in single-step updates, with the exponent decreasing over successive revisions, ensuring long-term stability. Different models exhibit distinct trust-ratio patterns.

Conclusion: The belief revision exponent (α-law) serves as a diagnostic tool for monitoring update stability and reasoning quality in LLMs, providing insights into their inference-time behavior.
...
TL;DR: Generative Active Testing (GAT) is a framework for cost-effective benchmarking of LLMs in specialized domains like healthcare, using uncertainty-aware sample selection.


<details>
  <summary>Details</summary>
Motivation: There's a high demand for task-specific test sets for LLMs, but labeling costs are prohibitive, especially in expert domains. Existing methods lack support for generative QA tasks.

Method: GAT uses LLMs as surrogates for sample selection, converting generative tasks into pseudo-classification via a Statement Adaptation Module to capture uncertainties.

Result: GAT's zero-shot acquisition functions reduce estimation error by ~40% compared to traditional sampling methods.

Conclusion: GAT provides a scalable, cost-effective solution for benchmarking LLMs in specialized domains.
...
TL;DR: Fine-tuning LLMs on 'impossible objects' (mutually exclusive predicates) leads to a 'suppression of genesis' and increased dogmatism, fracturing the latent space and hindering creative synthesis.


<details>
  <summary>Details</summary>
Motivation: To explore the ontological effects of training LLMs on logically contradictory inputs, drawing on Kantian and Deleuzian philosophical frameworks.

Method: Two training regimes: an 'Analytic' adapter trained on tautological definitions and a 'Synthetic-Conflict' adapter trained on brute-force contradictions. Behavioral and mechanistic analyses were conducted.

Result: Conflict-trained models showed a significant drop in generating synthetic concepts (from 9.0% to 1.0%) and a rise in 'Pick-One' dogmatism (from 3.6% to 30.8%). The latent space showed fractures, creating a 'topological schism'.

Conclusion: Training on logical contradictions without mediation forces models into a dogmatic state, impairing their ability for creative synthesis.
...
TL;DR: The paper introduces a novel distillation framework to enhance reasoning in smaller LLMs, using Explanatory Inversion and Explanatory GRPO, showing significant performance and efficiency gains.


<details>
  <summary>Details</summary>
Motivation: To address limitations like superficial pattern memorization and poor generalization in distilled models, the paper aims to instill deeper conceptual understanding.

Method: The framework includes Explanatory Inversion (EI) to generate explanatory probes and Explanatory GRPO (EXGRPO) for reinforcement learning, rewarding coherent reasoning.

Result: Evaluations on 12 datasets show a 20.39% improvement over zero-shot performance and 6.02% over state-of-the-art baselines, with efficient training and strong generalization.

Conclusion: The proposed method significantly enhances reasoning capabilities in smaller models, outperforming existing approaches in performance and efficiency.
...
TL;DR: The paper introduces the EAFD schema and a conflict-aware graph reasoning framework to improve decision-making in hierarchical review workflows by modeling verification actions and learning from correction signals. It significantly outperforms an LLM-only baseline.


<details>
  <summary>Details</summary>
Motivation: Hierarchical review workflows generate valuable correction signals, but learning from them is hindered by information asymmetry. The goal is to improve decision-making by grounding reasoning in verifiable actions and explicit conflict modeling.

Method: The authors propose the EAFD schema, a minimal representation for adjudication reasoning, and develop a conflict-aware graph reasoning framework that constructs EAFD graphs, aggregates historical cases, and performs deductive reasoning for new cases. It also includes a Request More Information (RMI) feature for insufficient evidence.

Result: The framework achieves 95.8% alignment with human experts offline and maintains 96.3% alignment in production, outperforming a standard LLM-only baseline (70.8% alignment).

Conclusion: The EAFD schema and conflict-aware graph reasoning framework effectively address information asymmetry in hierarchical workflows, enabling robust, real-world decision-making with high alignment to expert judgments.
...
TL;DR: The paper introduces a domain-enhanced LLM workflow for combustion science, addressing hallucinations in general-purpose models by integrating domain knowledge and physical laws, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: General-purpose LLMs struggle with domain-specific tasks like combustion science due to lack of domain knowledge and adherence to physical laws, leading to hallucinations.
...
Result: The model outperforms state-of-the-art closed-source models and retrieval-augmented methods on combustion science reasoning tasks, validated by FlameBench.

Conclusion: The work provides a foundation for developing reliable domain-specific scientific research agents with enhanced reasoning capabilities.
...
TL;DR: The paper provides a non-technical breakdown of six key components of LLMs to help researchers understand their capabilities and limitations, offering a framework for critical decision-making in their use.


<details>
  <summary>Details</summary>
Motivation: To help researchers make informed decisions about using LLMs by demystifying their mechanisms without requiring technical expertise.

Method: Analyzes six essential components of LLMs (pre-training data, tokenization, transformer architecture, probabilistic generation, alignment, and agentic capabilities) through technical and research lenses.

Result: Identifies specific affordances and limitations of LLMs, providing a framework for evaluating their suitability for research needs.

Conclusion: The paper concludes with a case study on simulating social media dynamics, demonstrating the practical application of the framework.
...
TL;DR: Autonoma is a hierarchical multi-agent framework for end-to-end workflow automation from natural language, achieving high reliability and inclusivity.


<details>
  <summary>Details</summary>
Motivation: Addressing scalability, error propagation, and focus challenges in monolithic agent architectures for complex user demands.

Method: Uses a multi-tiered architecture with Coordinator, Planner, and Supervisor layers, plus modular specialized agents for robustness and extensibility.

Result: 97% task completion rate and 98% successful agent handoff rate, confirming operational reliability.

Conclusion: Autonoma effectively automates workflows from natural language, ensuring robustness, inclusivity, and extensibility.
...
TL;DR: This paper presents a human-centered workflow for using LLMs via APIs in qualitative and quantitative content analysis tasks like annotation, summarization, and information extraction, with a focus on rigor, transparency, and practical implementation.


<details>
  <summary>Details</summary>
Motivation: To address the underutilized potential of LLMs beyond chat-based access by conceptualizing them as universal text processing tools and providing a structured workflow for researchers.

Method: The paper outlines a comprehensive, human-centered workflow for LLM-based content analysis, synthesizing interdisciplinary insights and including validation procedures and best practices.

Result: A detailed workflow and supplementary materials (prompt library, Python code, and instructions) are provided to enhance the rigor and transparency of LLM applications in research.

Conclusion: The paper offers a practical and validated approach for researchers to leverage LLMs effectively in content analysis, addressing key limitations like prompt sensitivity and hallucination.
...
TL;DR: Transformers are shown to be stateless Differentiable Neural Computers (sDNCs) with specific constraints, unifying their memory-centric interpretation.


<details>
  <summary>Details</summary>
Motivation: To formally establish the equivalence between causal Transformer layers and stateless Differentiable Neural Computers, providing a unified view of their memory mechanisms.

Method: A formal derivation demonstrates that a causal Transformer layer matches an sDNC under four conditions: no recurrent state in the controller, write-once memory, content-based addressing as attention, and multi-head attention as parallel read heads.

Result: The equivalence is extended to cross-attention, showing encoder-decoder Transformers as sDNCs with separate read and write memories.

Conclusion: The findings offer a principled computational framework for understanding Transformers and their memory operations, bridging them with DNCs.
...
TL;DR: A benchmark (LSR) reveals LLM safety failures in low-resource West African languages, showing refusal rates drop from 90% (English) to 35-55%, with Igala most affected. Tools and dataset are open for use.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the issue of safety alignment in large language models (LLMs) that primarily rely on English-language training data, leading to failures in refusal mechanisms when harmful intent is expressed in low-resource languages, particularly West African languages.

Method: The authors introduce LSR (Linguistic Safety Robustness), a benchmark that measures cross-lingual refusal degradation using a dual-probe evaluation protocol (matched English and target-language probes) and a novel metric called Refusal Centroid Drift (RCD). The benchmark is evaluated on Gemini 2.5 Flash using culturally grounded attack probes across four harm categories.

Result: English refusal rates remain high (~90%), but in West African languages (Yoruba, Hausa, Igbo, Igala), refusal rates drop significantly (35-55%), with Igala showing the most severe degradation (RCD = 0.55).

Conclusion: The LSR benchmark highlights the inadequacy of current LLM safety mechanisms for low-resource languages, providing a tool (integrated into the Inspect AI framework) to measure and address this gap. The benchmark dataset and implementation are publicly available.0.55). LSR is implemented in the Inspect AI evaluation framework and is available as a PR-ready contribution to the UK AISI's inspect_evals repository. A live reference implementation and the benchmark dataset are publicly available.
...
TL;DR: The CURE benchmark evaluates multimodal large language models (MLLMs) in clinical diagnostics, revealing a gap between their reasoning and retrieval abilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs in clinical diagnostics focus on end-to-end answering, limiting the ability to assess foundational reasoning and retrieval separately.

Method: Introduces the CURE benchmark with 500 multimodal clinical cases linked to physician-cited literature, evaluating reasoning and retrieval under controlled conditions.

Result: MLLMs show high accuracy (73.4%) with physician-supplied evidence but drop significantly (to 25.4%) when relying on independent retrieval.

Conclusion: The study highlights the challenges of integrating multimodal evidence and precise literature retrieval, with CURE being a publicly available benchmark.
...
TL;DR: Proposed subdomain adaptation via mid-training improves radiology report summarization, outperforming direct fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To reduce physician workload by enhancing radiology report summarization using adapted LLMs.

Method: Explored three adaptation strategies, including subdomain mid-training, using UF Health data and benchmark datasets.

Result: GatorTronT5-Radio (mid-trained) achieved best performance in ROUGE-L and RadGraph-F1, also improving few-shot learning.

Conclusion: Supports 'pre-training, mid-training, fine-tuning' over direct fine-tuning for better summarization.
...
TL;DR: The paper introduces GraphRAG, a framework using knowledge graphs to improve automated short answer grading by capturing structural relationships, outperforming standard RAG methods.


<details>
  <summary>Details</summary>
Motivation: Address limitations of LLMs in automated grading, such as hallucinations and poor rubric adherence, by leveraging structured knowledge graphs for better reasoning.

Method: Proposes GraphRAG with a dual-phase pipeline: Microsoft GraphRAG for graph construction and HippoRAG for associative graph traversals to retrieve connected evidence.

Result: GraphRAG outperforms standard RAG baselines on NGSS dataset, especially in evaluating Science and Engineering Practices (SEP).

Conclusion: Structural retrieval via knowledge graphs enhances grading accuracy by modeling dependencies and enabling multi-hop reasoning.
...
TL;DR: The paper introduces MOSAIC, a modular framework for review summarization that emphasizes interpretable components and practical utility, validated through online A/B tests and offline experiments.


<details>
  <summary>Details</summary>
Motivation: Existing summarization research overlooks benchmark reliability and granular insights, prompting the need for a scalable, interpretable solution like MOSAIC.

Method: MOSAIC decomposes summarization into interpretable components: theme discovery, structured opinion extraction, and grounded summary generation, including opinion clustering for improved faithfulness.

Result: Online A/B tests show improved customer experience and measurable value, while offline experiments demonstrate superior aspect coverage and faithfulness compared to baselines.

Conclusion: MOSAIC's modular approach enhances summarization practicality and reliability, with the release of a new dataset (TRECS) addressing limitations in existing benchmarks.
...
TL;DR: The paper explores LoRA and a hyper-network-based adaptation for RoBERTa, showing they achieve calibration parity with full fine-tuning while being more parameter-efficient. It also investigates a dynamic approach and identifies trade-offs between regularization and accuracy.


<details>
  <summary>Details</summary>
Motivation: To address miscalibration in Transformer-based models and improve parameter efficiency without sacrificing calibration performance.

Method: Uses LoRA and a hyper-network-based adaptation for RoBERTa, evaluating on the GLUE benchmark and introducing a dynamic approach for structural coupling.

Result: LoRA achieves calibration parity with full fine-tuning, and the dynamic approach shows promise, though trade-offs exist between regularization and accuracy.

Conclusion: Structured low-rank updates offer a viable path for uncertainty-aware Transformers, balancing parameter efficiency and probabilistic reliability.
...
TL;DR: A survey on multilingual hate speech detection and counterspeech, addressing challenges beyond English-centric models with a three-phase framework and ethical considerations.


<details>
  <summary>Details</summary>
Motivation: The need to combat online hate speech in diverse linguistic and cultural settings, overcoming limitations of monolingual systems.

Method: A three-phase framework: task design, data curation, and evaluation, leveraging NLP advances and state-of-the-art resources.

Result: Consolidated progress in multilingual resources and techniques, identifying challenges like data scarcity and fairness issues.

Conclusion: Provides scalable guidelines for inclusive, context-aware systems, advancing online safety across diverse languages.
...
TL;DR: The paper compares traditional feature-based AI and generative AI in scoring constructed responses, emphasizing the need for more extensive validity evidence for generative AI due to its lack of transparency and unique concerns.


<details>
  <summary>Details</summary>
Motivation: To address the growing use of generative AI in high-stakes testing and improve the validity of scores from such systems.

Method: The study compares human ratings, feature-based AI, and generative AI scoring systems, using data from student essays to gather validity evidence.

Result: Generative AI requires more extensive validity evidence due to transparency and consistency issues, as demonstrated by the essay data.

Conclusion: The paper proposes best practices for collecting validity evidence to support the use of generative AI in constructed response scoring, highlighting its complexities.
...
TL;DR: URAG introduces a benchmark to evaluate RAG systems' uncertainty across domains, showing accuracy-uncertainty trade-offs and reliability variations.


<details>
  <summary>Details</summary>
Motivation: Current RAG evaluations focus on correctness, missing uncertainty and reliability impacts. URAG fills this gap by assessing uncertainty in various fields.

Method: URAG reformulates open-ended tasks into multiple-choice questions, using conformal prediction for uncertainty quantification, and evaluates 8 RAG methods with LAC and APS metrics.

Result: Findings include accuracy-uncertainty correlations, better trade-offs for modular RAG, domain-specific reliability, and factors amplifying errors.

Conclusion: URAG provides a systematic benchmark to improve RAG system trustworthiness, revealing insights into their performance and limitations.
...
TL;DR: Prompt framing influences LLM decisions in threshold voting tasks, often favoring risk-averse choices, highlighting framing effects as a bias source in non-interacting LLM deployments.


<details>
  <summary>Details</summary>
Motivation: To explore how prompt framing affects LLM decision-making in tasks involving individual-group interest conflicts, especially in non-interacting settings where coordination is limited.

Method: Two logically equivalent prompts with different framings were tested across diverse LLM families in isolated trials to assess their impact on choice distributions.

Result: Prompt framing significantly influences LLM decisions, often shifting preferences toward risk-averse options, with surface linguistic cues sometimes overriding logically equivalent formulations.

Conclusion: The findings reveal framing effects as a notable bias source in non-interacting LLM deployments, offering insights for alignment and prompt design to mitigate such biases.
...
TL;DR: The paper presents a computational approach to motif indexing using the Arabian Nights and a detailed motif index, achieving an F1 score of 0.85 with a fine-tuned Llama3 model.


<details>
  <summary>Details</summary>
Motivation: Motifs are recurring narrative elements important in folklore and modern texts. Automating their detection in folkloristic texts aids analysis and modern usage understanding.

Method: Five approaches were tested: classic retrieve and re-rank, off-the-shelf and fine-tuned embedding models, generative prompting of LLMs, and fine-tuned LLMs with LoRA.

Result: A fine-tuned Llama3 model performed best with an F1 score of 0.85.

Conclusion: Computational motif indexing is feasible, with fine-tuned LLMs showing promising results.
...
TL;DR: A review of collaboration analysis using task-oriented conversational data, covering theories, coding schemes, tasks, and modeling approaches.


<details>
  <summary>Details</summary>
Motivation: To explore how task-oriented human-human conversational data can be utilized for collaboration analysis.

Method: Conducts a review of existing work, including theories, coding schemes, tasks, and modeling approaches.

Result: Provides insights into leveraging conversational data for collaboration analysis and identifies gaps for future research.

Conclusion: The review serves as a practical resource and highlights unexplored areas in collaboration analysis.
...
TL;DR: LLM-MRD is a novel teacher-student framework for multimodal fake news detection, improving accuracy and efficiency by distilling LLM reasoning into a student model.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in existing fake news detection methods, such as lack of multi-view judgment and high computational costs of LLMs.

Method: Uses a teacher-student framework with multi-view reasoning modules and calibration distillation to efficiently transfer knowledge.

Result: Outperforms state-of-the-art methods with a 5.19% improvement in ACC and 6.33% in F1-Fake.

Conclusion: LLM-MRD provides a scalable and effective solution for multimodal fake news detection.
...
TL;DR: PrefPO is a novel minimal prompt optimization method that reduces the need for labeled data and hyperparameter tuning, outperforming existing methods on multiple tasks while improving prompt hygiene and reducing susceptibility to prompt hacking.


<details>
  <summary>Details</summary>
Motivation: Automated prompt optimization is needed to reduce the labor-intensive nature of manual prompt engineering. Existing methods often require labeled datasets and produce verbose or repetitive prompts, which are limitations PrefPO aims to address.

Method: PrefPO leverages a preference-based approach inspired by RLHF, using an LLM discriminator to iteratively improve prompts. It requires only a starting prompt and natural language criteria, minimizing dependency on labeled data.

Result: PrefPO matches or outperforms state-of-the-art methods on 6/9 BBH tasks and IFEval-Hard, while also excelling in unlabeled settings. It reduces prompt length and repetition by 3-5x and is less susceptible to prompt hacking.

Conclusion: PrefPO offers a scalable and effective solution for prompt optimization, proving robust in both labeled and unlabeled scenarios while addressing key drawbacks of existing methods.
...
TL;DR: The paper introduces the Memory-Driven Role-Playing paradigm to improve LLM role-playing consistency by framing persona knowledge as memory. It includes evaluation tools and benchmarks, showing small models can match larger ones with structured prompting.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of maintaining consistent LLM role-playing in long dialogues by framing persona knowledge as an internal memory store.

Method: Proposes the Memory-Driven Role-Playing paradigm, with tools: MREval (evaluation framework), MRPrompt (prompting architecture), and MRBench (benchmark).

Result: Small models like Qwen3-8B match larger ones (e.g., Qwen3-Max) using MRPrompt, and memory gains improve response quality.

Conclusion: The paradigm and tools provide a rigorous, staged approach to diagnose and enhance LLM role-playing abilities.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in prompt routing for large language models (LLMs) when scaling to large model pools with fine-grained performance gaps. Existing methods like manual task taxonomies or monolithic routers fail to capture nuanced distinctions.

Method: The authors propose a two-stage routing architecture: (1) automated fine-grained task discovery via graph-based clustering and a task classifier, and (2) a task-aware quality estimation stage using a mixture-of-experts with task-specific heads. Predictions from both stages are aggregated for inference.

Result: Evaluated on 10 benchmarks with 11 frontier models, the method outperforms baselines and exceeds the performance of the strongest individual model while reducing costs by more than half.

Conclusion: The two-stage routing architecture effectively balances task-level stability and prompt-specific adaptability, offering a scalable solution for optimizing LLM performance and cost.
...
TL;DR: The paper challenges the reliability of linear probes in evaluating large language models' awareness, finding they primarily track structural artifacts rather than true evaluation context.


<details>
  <summary>Details</summary>
Motivation: To determine whether linear probes on benchmark prompts genuinely reflect evaluation context or just surface structure, given the entanglement of context with format and genre.

Method: Used a controlled 2x2 dataset and diagnostic rewrites to test if probe-based signals persist when prompt format is partially controlled.

Result: Found that probes mainly track benchmark-canonical structure and fail to generalize to free-form prompts, regardless of linguistic style.

Conclusion: Standard probe-based methods do not reliably separate evaluation context from structural artifacts, weakening the evidence from existing studies.
...
TL;DR: Word-order learnability varies across languages, with vocabulary structure being a key predictor of model surprisal.


<details>
  <summary>Details</summary>
Motivation: To understand why some languages allow free word order while others enforce fixed word order.

Method: Pretraining transformer language models on synthetic word-order variants of natural languages.

Result: Greater word-order irregularity increases model surprisal, showing reduced learnability. Vocabulary structure predicts surprisal better than the free/fixed word-order distinction.

Conclusion: Vocabulary structure is a primary factor in computational word-order learnability.
...
TL;DR: The paper explores using LLMs to iteratively synthesize programmatic agent policies for multi-agent environments, comparing sparse and dense feedback methods. Dense feedback, including social metrics, outperforms sparse feedback, especially in cooperative scenarios like Cleanup. The study also investigates adversarial attacks and mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to leverage LLMs for generating and refining agent policies in multi-agent environments without traditional reinforcement learning, focusing on the impact of feedback design (sparse vs. dense) on policy performance.

Method: The framework prompts an LLM to generate Python policy functions, evaluates them in self-play, and refines them iteratively using performance feedback. It compares sparse (scalar reward) and dense (reward plus social metrics) feedback.

Result: Dense feedback consistently matches or exceeds sparse feedback across metrics, particularly in cooperative scenarios. Social metrics aid coordination, leading to strategies like territory partitioning and role assignment. Adversarial experiments reveal five attack classes.

Conclusion: Dense feedback enhances cooperative strategy synthesis in LLM-generated policies, while adversarial vulnerabilities highlight a trade-off between expressiveness and safety in LLM policy synthesis.
...
TL;DR: The paper introduces a new decoding method for LLMs to sustain creativity and diversity in exploratory search quests, enabling users to explore more unique and varied results.


<details>
  <summary>Details</summary>
Motivation: Current LLM decoding methods often produce homogeneous results, limiting their usefulness in long exploratory searches like finding a wedding dress or a unique research topic.

Method: A novel, easy-to-implement decoding scheme is developed to induce sustained creativity and diversity in LLM outputs without needing access to the model's inner workings.

Result: The approach produces many conceptually unique results, helping users explore diverse alternatives and make informed choices.

Conclusion: The new decoding method unlocks an LLM's broader knowledge, improving exploratory search quests by offering sustained creativity and diversity.
...
TL;DR: EvidenceRL is a reinforcement learning framework that improves grounding and correctness in LLM responses by optimizing for evidence adherence, showing significant gains in high-stakes domains like cardiac diagnosis and legal reasoning.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce plausible but unverified answers (hallucinations), which is problematic in high-stakes domains requiring evidence-based decisions. EvidenceRL addresses this gap.

Method: The framework uses reinforcement learning (Group Relative Policy Optimization) to score responses for grounding (entailment with evidence) and correctness (agreement with reference answers).

Result: EvidenceRL improves grounding (e.g., F1@3 from 37.0 to 54.5 in cardiac diagnosis) and reduces hallucinations (e.g., drop by 5×). It also increases evidence-supported diagnoses (31.8% to 61.6%).

Conclusion: EvidenceRL effectively enhances evidence adherence and faithfulness in LLMs across high-stakes domains without compromising accuracy, demonstrating its practical utility.
...
TL;DR: FDARxBench is a new expert-curated benchmark for evaluating document-grounded QA, using FDA drug labels to create challenging tasks like factual, multi-hop, and refusal questions.


<details>
  <summary>Details</summary>
Motivation: The benchmark addresses the difficulty of accurate QA over heterogeneous drug label documents, motivated by FDA generic drug assessment needs.

Method: A multi-stage pipeline generates high-quality, expert-curated QA examples, with evaluation protocols for open-book and closed-book reasoning.

Result: Experiments show gaps in factual grounding, long-context retrieval, and safe refusal behavior across models.

Conclusion: FDARxBench provides a regulatory-grade evaluation foundation for drug-label comprehension and LLM behavior assessment.
...
TL;DR: The paper evaluates the effectiveness and efficiency of reasoning strategies in LLMs for text classification, finding that not all reasoning methods improve performance and some are inefficient.


<details>
  <summary>Details</summary>
Motivation: To investigate whether deliberative reasoning strategies universally benefit text classification tasks, given their increasing application and associated token/time costs.

Method: Introduces TextReasoningBench, a benchmark comparing seven reasoning strategies (IO, CoT, SC-CoT, ToT, GoT, BoC, long-CoT) across ten LLMs on five datasets, using traditional and cost-aware metrics.

Result: Findings include: (1) Reasoning doesn't universally improve performance—moderate strategies like CoT show limited gains, while complex ones like ToT may degrade it; (2) Many strategies are inefficient, with high token costs for marginal gains.

Conclusion: Deliberative reasoning does not uniformly benefit text classification, and efficiency varies widely; simpler methods like CoT are often more effective and cost-efficient.
...
TL;DR: BEAVER is a training-free framework for compressing long-document contexts in LLMs, using structure-aware hierarchical selection to reduce latency and improve information utilization without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: The increasing context window size in LLMs introduces bottlenecks in inference latency and information usage. Existing compression methods are costly or disrupt semantics through aggressive pruning.

Method: BEAVER employs a hybrid planner for hierarchical selection, combining semantic and lexical dual-branch selection with sentence smoothing, and uses dual-path pooling to map variable-length contexts into dense tensors.

Result: BEAVER matches SOTA performance, excels in multi-needle retrieval, and reduces latency by 26.4x on 128k contexts, proving efficient for high-throughput applications.

Conclusion: BEAVER offers a scalable, efficient solution for long-document compression in LLMs, balancing performance and speed without requiring training.
...
TL;DR: A novel prompt engineering framework for Arabic Automatic Essay Scoring (AES) using LLMs, with three prompting strategies, achieves high trait-level agreement, especially with rubric-guided prompting.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of scalable, linguistically informed AES tools for Arabic, the study aims to create a structured framework for evaluating language proficiency traits.

Method: A three-tier prompting strategy (standard, hybrid, rubric-guided) is introduced to guide LLMs in evaluating traits like organization, vocabulary, development, and style. Eight LLMs are tested on the QAES dataset under zero-shot and few-shot settings.

Result: Fanar-1-9B-Instruct achieved the highest trait-level agreement (QWK = 0.28, CI = 0.41). Rubric-guided prompting improved performance across all traits, with discourse-level traits (Development and Style) showing the greatest gains.

Conclusion: Structured prompting, not model scale alone, enables effective Arabic AES. The framework sets a foundation for scalable assessment in low-resource educational contexts.
...
TL;DR: The paper challenges the common practice of selecting similar datasets for MLLMs and introduces DATAPROPHET, a training-free metric for predicting dataset influence, showing superior performance over traditional methods.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine if intuitive similarity between training and benchmark datasets reliably predicts performance, addressing the need for a method to estimate dataset influence before training.

Method: The authors analyze transfer across 14 vision-language datasets and propose DATAPROPHET, a metric combining multimodal perplexity, similarity, and diversity to rank datasets without training.

Result: DATAPROPHET achieves 86.0% correlation with actual performance gains and outperforms other selection methods, including a 6.9% improvement over uniform selection.

Conclusion: The findings suggest that dataset-specific factors matter more than broad task similarity, and DATAPROPHET offers a practical tool for efficient supervision-data selection.
...
TL;DR: EvoTaxo is a LLM-based framework for building and evolving taxonomies from social media streams, addressing challenges like noise and dynamic discourse. It outperforms baselines in structural quality and captures temporal shifts.


<details>
  <summary>Details</summary>
Motivation: Social media posts are short, noisy, and dynamic, making taxonomy induction difficult. Existing methods are not robust or scalable enough for evolving discourse.

Method: EvoTaxo converts posts into structured draft actions, accumulates evidence over time, and refines edits via dual-view clustering. It maintains concept memory banks for semantic consistency.

Result: EvoTaxo produces balanced taxonomies with clearer assignments, better coverage, and stronger structural quality than baselines. It also captures temporal discourse shifts.

Conclusion: EvoTaxo effectively addresses the challenges of dynamic social media data, offering a scalable and robust solution for taxonomy induction.
...
TL;DR: Study introduces FabTab, a benchmark dataset for detecting AI-generated fabricated scientific tables in NLP papers, using TAB-AUDIT framework with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Address growing concerns of AI-generated fabricated scientific manuscripts by focusing on tables as critical evidence.

Method: Construct FabTab dataset (1,173 AI-generated, 1,215 human papers), analyze differences, and use TAB-AUDIT with key feature (within-table mismatch).

Result: RandomForest achieves 0.987 AUROC in-domain and 0.883 AUROC out-of-domain, outperforming prior methods.

Conclusion: Tables serve as forensic signals for detecting AI-generated fraud, providing a benchmark for future research.
...
TL;DR: LoopRPT is a reinforcement pre-training framework designed for LoopLMs, improving intermediate reasoning steps and achieving better accuracy-computation trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods focus on output tokens, not aligning well with looped architectures where reasoning is implicit. LoopRPT addresses this gap.

Method: LoopRPT reframes next-token prediction as a reasoning task, applying reinforcement signals to latent steps via an EMA teacher and noisy rollouts.

Result: LoopRPT improves per-step representation quality, achieving Pareto dominance in accuracy-computation trade-offs and enhancing early-stage reasoning.

Conclusion: LoopRPT demonstrates reinforcement pre-training as an effective method for efficient latent reasoning in LoopLMs.
...
TL;DR: Performance-oriented Context Compression (PoC) shifts focus from fixed compression ratios to maintaining a performance floor, using predictors to find the most aggressive compression that meets the constraint, achieving better results.


<details>
  <summary>Details</summary>
Motivation: To address unpredictable performance degradation in existing context compression methods by ensuring a specified performance floor is met, enabling reliable deployment of LLMs.

Method: Introduces PoC with lightweight performance predictors (context-agnostic and context-aware) to determine the optimal compression ratio for an off-the-shelf compressor.

Result: The context-aware predictor outperforms the context-agnostic one, with PoC achieving superior overall performance on benchmarks like question-answering and summarization.

Conclusion: PoC provides a reliable, efficient, and performance-aware approach to context compression for LLMs, improving deployment reliability.
...
TL;DR: The paper introduces a protocol for evaluating MLLMs that accounts for human label variation (agreement/disagreement), revealing that larger models excel in high-agreement tasks but struggle with ambiguous or subjective cases.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored systematic differences in human annotator judgments (HLV) in benchmarks, especially for MLLMs, by proposing a more realistic evaluation protocol.

Method: The authors apply their protocol to two MLLM families (Gemma 3, Qwen 2.5 VL) using non-aggregated human annotations from a social media dataset, comparing performance under conditions of high agreement and high disagreement.

Result: Larger models perform better on high-agreement tasks but underperform medium-sized models in high-disagreement scenarios, showing that parameter count alone doesn't determine sensitivity to ambiguity.

Conclusion: Benchmarks relying solely on consensus labels overstate model capabilities; incorporating human label variation provides more robust MLLM assessments, particularly for content moderation.
...
TL;DR: The paper investigates how multilingual encoders represent Hindi-English code-mixed inputs, revealing weak alignment and an English-dominant processing bias. A trilingual alignment objective improves cross-lingual understanding.


<details>
  <summary>Details</summary>
Motivation: To understand how multilingual language models represent code-mixed inputs and whether these representations align with the constituent languages (English and Hindi).

Method: Constructs a trilingual corpus, uses probing techniques (CKA, token-level saliency, entropy-based analysis), and introduces a post-training alignment objective.

Result: Standard models align English and Hindi but poorly with code-mixed text. Continued pre-training improves English-code-mixed alignment but harms English-Hindi alignment. A trilingual alignment objective balances alignment and improves downstream tasks.

Conclusion: Grounding code-mixed representations in constituent languages enhances cross-lingual understanding, demonstrated by improved performance in sentiment analysis and hate speech detection.
...
TL;DR: The paper transforms Semantic Role Classification in FrameNet into a binary classification task using analogical relations, training a lightweight ANN without explicit semantic role information, achieving state-of-the-art results efficiently.


<details>
  <summary>Details</summary>
Motivation: To simplify Semantic Role Classification by leveraging analogical relations over frame evoking lexical units and frame element pairs, enabling efficient training and inference without direct semantic role input.

Method: Defines analogies as formal relations on LUs and FEs, constructs a new dataset for binary classification, and trains a lightweight ANN with analogical transfer for inference.

Result: The approach surpasses previous state-of-the-art results in Semantic Role Classification while maintaining computational efficiency and frugality.

Conclusion: The relational analogy-based method effectively simplifies and improves Semantic Role Classification, demonstrating the potential of lightweight models for complex NLP tasks.
...
TL;DR: The paper introduces a lightweight metric called semantic delta to distinguish human-written and LLM-generated dialogue, showing LLMs have stronger thematic concentration than humans.


<details>
  <summary>Details</summary>
Motivation: To understand if LLMs mimic human dialogue accurately, which is relevant across fields like education and academia.

Method: Uses the Empath framework to map texts to thematic intensity scores, defines semantic delta as the difference between top two category intensities, and compares distributions via Welch t-test.

Result: AI-generated texts consistently show higher semantic deltas, indicating more rigid topic structures compared to broader human dialogue.

Conclusion: The semantic delta metric is a cost-effective, zero-shot tool for detecting LLM-generated texts and highlights a gap in LLM conversational mimicry.
...
TL;DR: The paper explores reliable ways to measure the performance of auto-evaluators in machine translation error detection, proposing a new method (MPP) and evaluating its effectiveness.


<details>
  <summary>Details</summary>
Motivation: There’s no established technique to reliably measure auto-evaluators’ capabilities in detecting machine translation errors, despite advancements in MT and evaluation methods.

Method: The study investigates various implementations of precision, recall, and F-score at the span level, introduces the 'match with partial overlap and partial credit' (MPP) method with micro-averaging, and assesses its robustness.

Result: The research finds that similar evaluation approaches can produce significantly different rankings and that some widely-used techniques are inadequate for MT error detection. MPP proves to be a robust meta-evaluation strategy.

Conclusion: The paper concludes with the proposal of MPP as a reliable method for evaluating MT error detection and releases code for public use, also assessing the current state of the art in this field.
...
TL;DR: The study explores the efficiency of human translation via the Information Bottleneck (IB) framework, using spatial prepositions in translations to show alignment with optimal accuracy-complexity trade-offs.


<details>
  <summary>Details</summary>
Motivation: To investigate whether natural language systems, particularly in translation, adhere to the optimal balance between informativity and simplicity as predicted by the IB framework, focusing on spatial prepositions.

Method: Framed translation as an IB problem, used pile-sorting and similarity judgments for informativity estimates, and trained a low-rank projection model to analyze translations across English, German, and Serbian.

Result: The study found that actual translations of prepositions align closer to the IB optimal frontier than alternatives, indicating communicative efficiency in translators.

Conclusion: Translation can reveal cognitive efficiency pressures in cross-linguistic semantic systems, providing evidence for the IB framework in linguistic contexts beyond controlled experiments.
...
TL;DR: SAGE introduces an energy-efficient framework for training LLMs in low-resource contexts, focusing on high-quality, culturally relevant data to achieve top performance with minimal environmental impact.


<details>
  <summary>Details</summary>
Motivation: The linguistic divide in low-resource regions, coupled with the environmental cost of large-scale data training, demands a sustainable solution for digital inclusion.

Method: SAGE uses a reinforcement learning agent (GRPO-optimized) to curate compact training sets from expert-constructed dialogues, then fine-tunes LLMs with LoRA.

Result: Achieves state-of-the-art translation performance for 7 low-resource languages, reducing data usage by 97.1% and energy by 95.2%.

Conclusion: SAGE provides a scalable, eco-friendly approach to bridging the digital divide, balancing high performance with sustainability.
...
TL;DR: A hybrid topic modeling framework combining LDA and sPLS-DA is used to analyze Pushkin's 'Evgenij Onegin,' demonstrating reproducible thematic mapping in small corpora.


<details>
  <summary>Details</summary>
Motivation: To integrate unsupervised and supervised methods for modeling thematic structure and dynamics in narrative poetry, addressing challenges like small-corpus instability.

Method: Segments the text into lemmatized documents, applies multi-seed consensus for stability, and uses sPLS-DA to refine themes with lexical markers.

Result: Identifies five stable topics and aligns thematic mixtures with the poem's emotional arc through narrative hubs.

Conclusion: The framework provides a computational close-reading method for literary analysis, applicable to other dense texts despite abstraction of stylistic features.
...
TL;DR: Study examines how models handle ambiguity in collaborative tasks, finding mismatches between judgment and action despite detecting unreliable speakers.


<details>
  <summary>Details</summary>
Motivation: To understand how models separate literal interpretation from contextual inference in resolving underspecified instructions during collaboration.

Method: Introduces Build What I Mean (BWIM), a benchmark where models resolve ambiguity via contextual inference or clarification requests, testing state-of-the-art LLMs.

Result: Models detect speaker unreliability in confidence ratings but fail to use this for efficient clarification, showing suboptimal strategies like over-clarification or guessing.

Conclusion: Dissociation between judgment and action in LLMs highlights the challenge of integrating pragmatic reasoning into model behavior.
...
TL;DR: A multi-agent framework for generating and refining XAI narratives improves faithfulness and coherence, with Claude-4.5-Sonnet performing best. An ensemble strategy further enhances performance for most LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing XAI methods are often technical and expert-oriented, prompting the need for more interpretable and accessible explanations using natural language.

Method: A multi-agent framework (Narrator and Critic Agents) iteratively generates and refines XAI narratives based on feedback. Five agentic systems are evaluated across five LLMs on tabular datasets.

Result: Basic, Critic, and Critic-Rule Designs improve faithfulness. Claude-4.5-Sonnet reduces unfaithful narratives by 90%. An ensemble strategy boosts performance for four LLMs.

Conclusion: Agentic systems show promise for producing faithful and coherent XAI narratives, with iterative refinement and ensemble strategies enhancing effectiveness.
...
TL;DR: RouterKGQA improves KGQA performance by combining specialized and general models, reducing costs and enhancing accuracy.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of existing KGQA methods (retrieval-based and agent-based) by balancing efficiency and structural grounding.

Method: Collaboration between a specialized model for reasoning paths and a general model for KG-guided repair, with constraint-aware filtering and efficient workflow design.

Result: Outperforms previous methods by 3.57 points in F1 and 0.49 in Hits@1, with only 1.15 LLM calls per question.

Conclusion: RouterKGQA effectively mitigates LLM hallucination and improves KGQA performance at minimal cost.
...
TL;DR: The paper introduces LoASR-Bench, a benchmark to evaluate SpeechLMs in low-resource languages, revealing their limitations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on high-resource languages, leaving SpeechLMs' performance in low-resource languages unclear. This gap hinders their real-world multilingual deployment.

Method: Proposes LoASR-Bench, a comprehensive benchmark with 25 low-resource languages from 9 families, including Latin and non-Latin scripts, to assess SpeechLMs' ASR performance.

Result: Experiments show current SpeechLMs struggle with low-resource languages, highlighting their limitations.

Conclusion: The study underscores the need for improving SpeechLMs' generalizability for low-resource languages to enhance real-world ASR systems.
...
TL;DR: The paper explores how verbal and nonverbal linguistic features predict a listener's understanding in dialogues, focusing on cognitive load-related cues like surprisal, syntactic complexity, and gaze behavior.


<details>
  <summary>Details</summary>
Motivation: To understand how linguistic cues can predict listener understanding in real-time explanatory interactions.

Method: Statistical analysis of the MUNDEX corpus and classification experiments using off-the-shelf and fine-tuned BERT-based multimodal classifiers.

Result: Listener states (Understanding, Partial Understanding, Misunderstanding) were predictable, with improved accuracy when combining linguistic cues with textual features.

Conclusion: Integrating verbal and nonverbal cues enhances prediction of listener understanding, demonstrating the value of multimodal analysis in dialogue.
...
TL;DR: The paper compares SFT-only, DPO-only, and staged SFT-to-DPO training with FFT and LoRA on a small GPT-2 model, finding DPO offers minor gains over SFT, while FFT outperforms LoRA in performance.


<details>
  <summary>Details</summary>
Motivation: To clarify the empirical behavior of DPO under small backbones and modest data, and compare its effectiveness against SFT and parameterization methods like FFT and LoRA.

Method: Systematic comparison of SFT-only, DPO-only, and staged SFT-to-DPO training alongside full fine-tuning (FFT) versus LoRA on a GPT-2-scale decoder, evaluated using paraphrase detection and Shakespearean sonnet continuation.

Result: DPO provides small, task-dependent gains over SFT and can match SFT accuracy without a warm start if preference construction aligns with the supervised objective. FFT consistently outperforms LoRA, which does not reduce wall-clock time in their setup.

Conclusion: In small-scale settings, supervised full-parameter adaptation (FFT) is the primary performance driver, while DPO and LoRA offer limited marginal benefits.
...
TL;DR: The study evaluates how well ChatGPT-5 translates Arabic grammar terms, finding only 25% accuracy, and suggests AI-linguist collaboration for improvement.


<details>
  <summary>Details</summary>
Motivation: To assess the capability of Large Language Models (LLMs) like ChatGPT in accurately translating core syntax terms from generative syntax into Arabic.

Method: Collected 44 syntax terms, translated them via humans and ChatGPT-5, then compared and analyzed the translations using an analytical and comparative approach.

Result: Only 25% of ChatGPT translations were accurate, 38.6% inaccurate, and 36.4% partially correct, highlighting syntactic and semantic challenges.

Conclusion: Proposes collaborative strategies between AI specialists and linguists to enhance LLMs' translation accuracy for syntax terms.
...
TL;DR: LLMs show strong reasoning in isolated tasks but struggle in task-oriented dialogue (TOD) settings, as shown by the BOULDER benchmark. Performance gaps arise from multi-turn dialogue, role conditioning, and tool-use requirements.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the mismatch between LLMs' reasoning performance in isolated benchmarks and real-world TOD scenarios, questioning whether benchmarks reflect real-world robustness.

Method: The authors introduce BOULDER, a dynamic benchmark with eight travel-related tasks requiring arithmetic, spatial, and temporal reasoning. Each task has isolated and dialogue-based variants for controlled comparison.

Result: Experiments on eight LLMs reveal a significant performance gap between isolated and dialogue settings, attributed to multi-turn dialogue, role conditioning, and tool-use requirements.

Conclusion: The study emphasizes the need to evaluate LLM reasoning in realistic, interactive settings like TOD to ensure accurate assessment of their capabilities.
...
TL;DR: The paper introduces an attention-based pooling method to improve sentence-level embeddings in the HAL model, outperforming traditional mean pooling.


<details>
  <summary>Details</summary>
Motivation: Standard mean pooling in the HAL model causes information loss by equally weighting all tokens, ignoring contextual salience.

Method: Proposes a learnable, temperature-scaled additive attention mechanism and uses Truncated SVD to handle matrix sparsity before attention.

Result: Achieves 82.38% test accuracy on IMDB sentiment analysis, a 6.74% improvement over mean pooling, with better interpretability.

Conclusion: Attention-based pooling enhances HAL's sentence embeddings, improving accuracy and interpretability by focusing on salient tokens.
...
TL;DR: STC is an efficient uncertainty quantification method for LLMs, using semantic token clustering without auxiliary models or repeated sampling, reducing computational costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce unreliable outputs due to overconfidence, and existing uncertainty quantification methods are computationally expensive.

Method: Proposes Semantic Token Clustering (STC), which groups tokens into semantic clusters using embedding clustering and prefix matching, and quantifies uncertainty based on aggregated probability mass.

Result: STC matches state-of-the-art performance while significantly cutting computational overhead.

Conclusion: STC offers a lightweight, effective solution for uncertainty quantification in LLMs, enhancing reliability without added computational burden.
...
TL;DR: The paper evaluates how language models balance user alignment with evidence faithfulness under epistemic conflicts, finding that richer evidence improves accuracy but doesn’t prevent user-aligned reversals, highlighting three key failure modes.


<details>
  <summary>Details</summary>
Motivation: To assess the tension between user alignment and evidence faithfulness in instruction-tuned language models, using a controlled epistemic-conflict framework.

Method: Fine-grained ablations over evidence composition and uncertainty cues across 19 models (0.27B to 32B parameters) in a fixed-evidence setting.

Result: Richer evidence improves accuracy under neutral prompts but fails under user pressure. Three failure modes: negative partial-evidence interaction, non-monotonic robustness scaling, and varying distributional concentration.

Conclusion: Richer evidence alone isn’t enough to resist user pressure without explicit training for epistemic integrity.
...
TL;DR: Faithfulness in chain-of-thought (CoT) reasoning is not objective; classifier choice significantly impacts measurements, causing divergent results and model rankings.


<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that faithfulness in CoT reasoning is an objective, measurable property of a model by demonstrating the variability introduced by different classifiers.

Method: Applied three classifiers (regex-only detector, regex-plus-LLM pipeline, and Claude Sonnet 4 judge) to 10,276 reasoning traces from 12 models, analyzing inter-classifier agreement and systematic disagreements.

Result: Faithfulness rates varied widely (74.4%, 82.6%, 69.7%) with non-overlapping confidence intervals. Classifier choice caused significant per-model gaps (2.6–30.6 points) and reversed rankings.

Conclusion: Published faithfulness numbers are not comparable across studies due to classifier dependency; future evaluations should report sensitivity ranges across multiple methodologies.
...
### [191] [TrustFlow: Topic-Aware Vector Reputation Propagation for Multi-Agent Ecosystems](https://arxiv.org/abs/2603.19452)
*Volodymyr Seliuchenko*

Main category: cs.MA

TL;DR: TrustFlow is a reputation propagation algorithm using multi-dimensional vectors and topic-gated transfer operators, achieving high precision and resistance to attacks.


<details>
  <summary>Details</summary>
Motivation: To improve reputation systems by using multi-dimensional vectors for more nuanced trust assessment and better resistance to manipulation.

Method: Uses topic-gated transfer operators and interaction graphs, with Lipschitz-1 transfer operators and composable gates.

Result: Achieves 98% multi-label Precision@5 on dense graphs and 78% on sparse ones, with strong resistance to attacks.

Conclusion: TrustFlow outperforms scalar-based methods like PageRank by providing queryable vector reputations in the same embedding space as user queries.
...
### [192] [Planning Autonomous Vehicle Maneuvering in Work Zones Through Game-Theoretic Trajectory Generation](https://arxiv.org/abs/2603.19556)
*Mayar Nour,Atrisha Sarkar,Mohamed H. Zaki*

Main category: cs.MA

TL;DR: The paper introduces a game-theoretic framework for AVs to safely navigate work zones by modeling lane changes as non-cooperative games, reducing conflicts by 35%.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles struggle with work zones due to complex geometries and unpredictable traffic, requiring better decision-making for safe navigation.

Method: A game-theoretic planner is used to model lane changes as non-cooperative games, balancing safety, progress, and traffic stability.

Result: Simulations show a 35% reduction in conflicts and fewer high-risk events compared to traditional models.

Conclusion: The game-theoretic approach effectively enhances safety in work zone navigation for AVs.
...
### [193] [Helix: A Dual-Helix Co-Evolutionary Multi-Agent System for Prompt Optimization and Question Reformulation](https://arxiv.org/abs/2603.19732)
*Kewen Zhu,Liping Yi,Zhiming Zhao,Xiang Li,Qinghua Hu*

Main category: cs.MA

TL;DR: Helix introduces a multi-agent system for joint optimization of question reformulation and prompt instructions, outperforming existing methods by up to 3.95%.


<details>
  <summary>Details</summary>
Motivation: Current automated prompt optimization methods are limited by fixed templates or single-sided approaches, ignoring the interdependence between question formulation and prompt design.

Method: Helix uses a three-stage co-evolutionary framework: planner-guided decomposition, dual-track co-evolution, and strategy-driven question generation.

Result: Experiments show Helix improves performance by up to 3.95% across 12 benchmarks, with efficient optimization.

Conclusion: Helix's unified approach effectively bridges the gap between question reformulation and prompt optimization, enhancing adaptability and performance.
...
### [194] [GT-Space: Enhancing Heterogeneous Collaborative Perception with Ground Truth Feature Space](https://arxiv.org/abs/2603.19308)
*Wentao Wang,Haoran Xu,Guang Tan*

Main category: cs.LG

TL;DR: GT-Space is a scalable collaborative perception framework for heterogeneous agents in autonomous driving, using a common feature space from ground-truth labels for efficient feature alignment.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of handling heterogeneous features from agents with different sensing modalities or model architectures in multi-agent collaborative perception.

Method: GT-Space constructs a shared feature space using ground-truth labels, requiring only a single adapter module for feature projection. A fusion network is trained with contrastive losses for diverse modality combinations.

Result: GT-Space outperforms baselines on simulation and real-world datasets (OPV2V, V2XSet, RCooper) in detection accuracy and robustness.

Conclusion: GT-Space provides a flexible and scalable solution for heterogeneous agent collaboration, eliminating the need for pairwise feature alignment and improving performance.
...
### [195] [Speculating Experts Accelerates Inference for Mixture-of-Experts](https://arxiv.org/abs/2603.19289)
*Vivan Madan,Prajwal Singhania,Abhinav Bhatele,Tom Goldstein,Ashwinee Panda*

Main category: cs.LG

TL;DR: Proposes an expert prefetching scheme for MoE models to reduce CPU-GPU transfer bottlenecks, using internal representations to predict and overlap future expert computations, improving inference speed by up to 14%.


<details>
  <summary>Details</summary>
Motivation: Address the performance bottleneck caused by CPU-GPU transfers when offloading expert weights in memory-constrained MoE models.

Method: Introduces a prefetching scheme that speculates future experts using internal model representations, overlapping memory transfers with computation, and includes lightweight estimators for suboptimal accuracy cases.

Result: Achieves up to 14% reduction in time per output token (TPOT) and maintains downstream task accuracy without needing re-fetching.

Conclusion: The proposed method effectively reduces inference latency in MoE models by optimizing expert prefetching and speculative execution, with open-source code available.
...
TL;DR: GoAgent introduces a method for explicit group-based communication topology in LLM-based multi-agent systems, improving accuracy and reducing communication overhead.


<details>
  <summary>Details</summary>
Motivation: Existing methods implicitly derive group structures, leading to suboptimal coordination and excessive communication. Explicitly modeling groups as atomic units can enhance performance.

Method: GoAgent enumerates task-relevant candidate groups via LLM, autoregressively selects and connects them, and uses a CIB objective to compress inter-group communication.

Result: Achieves 93.84% average accuracy on six benchmarks and reduces token consumption by ~17%.

Conclusion: Explicit group-based topology and CIB compression significantly enhance MAS performance and efficiency.
...
### [197] [A Visualization for Comparative Analysis of Regression Models](https://arxiv.org/abs/2603.19291)
*Nassime Mountasir,Baptiste Lafabregue,Bruno Albert,Nicolas Lachiche*

Main category: cs.LG

TL;DR: The paper introduces a visualization method to better assess regression model performance by analyzing residuals in 2D space, using Mahalanobis distance, and colormaps for error distribution.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of traditional performance metrics in regression analysis, which aggregate too much information and may obscure key patterns. It aims to provide a more detailed and comprehensive view of model performance through visualization.

Method: The proposed method introduces a novel visualization approach with three main contributions: (1) evaluating residuals in a 2D space to compare two models simultaneously, (2D space, (2) uses the Mahalanobis distance to account for data correlations and scale differences, and (3) employs a colormap for percentile-based error distribution visualization.

Result: The visualization method offers a detailed and comprehensive view of regression model performance, revealing patterns that traditional metrics may miss. It enhances the evaluation and comparison of models by highlighting error distributions and correlations.

Conclusion: The novel visualization approach provides a deeper understanding of regression model performance, improving the evaluation and comparison process beyond conventional aggregate metrics.
...
### [198] [Maximizing mutual information between user-contexts and responses improve LLM personalization with no additional data](https://arxiv.org/abs/2603.19294)
*Hyunji Nam,Haoran Li,Natasha Jaques*

Main category: cs.LG

TL;DR: MIPO is a self-improvement framework for LLMs that uses contrastive data augmentation to generate preference pairs, optimizing mutual information between prompts and responses. It improves personalization and performance on tasks without external supervision.


<details>
  <summary>Details</summary>
Motivation: High-quality labeled data is costly and limited, and traditional methods rely on external oversight. The paper aims to enable self-improvement in LLMs without such dependencies.

Method: MIPO constructs preference pairs by generating positive and negative responses based on correct and unrelated prompts, respectively, and uses DPO to optimize mutual information.

Result: MIPO achieves 3-40% improvements in personalization tasks and 1-18% gains in math and multiple-choice problems without additional data or supervision.

Conclusion: MIPO offers a promising approach for LLM self-improvement, demonstrating effectiveness across tasks without relying on external oversight.
...
TL;DR: A scalable framework for few-step swarm steering using a control-space learning approach under linear time-invariant dynamics, respecting sampled-data updates.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of steering large-scale swarms with minimal control updates by leveraging sampled-data control.

Method: Introduces a control-space learning framework inspired by MeanFlow, parameterizing finite-horizon minimum-energy control coefficients and using a stop-gradient training objective.

Result: Provides a scalable solution for swarm steering that aligns with real-world sampled-data control systems.

Conclusion: The framework effectively bridges the gap between theoretical control and practical implementation for swarm steering.
...
### [200] [BrainSCL: Subtype-Guided Contrastive Learning for Brain Disorder Diagnosis](https://arxiv.org/abs/2603.19295)
*Xiaolong Li,Guiliang Guo,Guangqi Wen,Peng Cao,Jinzhu Yang,Honglin Wu,Xiaoli Liu,Fei Wang,Osmar R. Zaiane*

Main category: cs.LG

TL;DR: Proposes a subtype-guided contrastive learning framework to address heterogeneity in mental disorder populations, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Heterogeneity in mental disorder populations complicates contrastive learning; modeling latent subtypes can guide representation learning.

Method: Uses multi-view representations from clinical text and BOLD signals, unsupervised spectral clustering for subtypes, and a dual-level attention mechanism for prototypes. Subtype-guided contrastive learning reinforces intra-subtype consistency.

Result: Outperforms state-of-the-art approaches on Major Depressive Disorder, Bipolar Disorder, and Autism Spectrum Disorders.

Conclusion: Subtype prototype graphs effectively guide contrastive learning, improving model performance in mental disorder studies.
...
### [201] [TTQ: Activation-Aware Test-Time Quantization to Accelerate LLM Inference On The Fly](https://arxiv.org/abs/2603.19296)
*Toshiaki Koike-Akino,Jing Liu,Ye Wang*

Main category: cs.LG

TL;DR: The paper introduces a test-time quantization (TTQ) framework to compress large foundation models during inference, addressing domain shift issues in activation-aware compression methods.


<details>
  <summary>Details</summary>
Motivation: Existing activation-aware compression techniques rely on calibration data, leading to domain shift problems for unseen downstream tasks. TTQ aims to resolve this by enabling on-the-fly compression at inference.

Method: TTQ uses efficient online calibration to perform instant activation-aware quantization, adapting to any prompt during inference without retraining.

Result: Experiments show TTQ outperforms state-of-the-art baselines in quantization performance and achieves inference speedup.

Conclusion: TTQ effectively compresses large models dynamically during inference, eliminating reliance on calibration data and improving performance for diverse downstream tasks.
...
### [202] [CLaRE-ty Amid Chaos: Quantifying Representational Entanglement to Predict Ripple Effects in LLM Editing](https://arxiv.org/abs/2603.19297)
*Manit Baser,Alperen Yildiz,Dinil Mon Divakaran,Mohan Gurusamy*

Main category: cs.LG

TL;DR: CLaRE is a lightweight technique to identify ripple effects in LLMs using forward activations, improving efficiency and accuracy over baselines.


<details>
  <summary>Details</summary>
Motivation: Address the unpredictable ripple effects from model-editing techniques in LLMs, which can lead to unintended behavioral changes.

Method: Introduces CLaRE, a representation-level technique that quantifies fact entanglement using forward activations from a single intermediate layer.

Result: CLaRE achieves a 62.2% improvement in Spearman correlation with ripple effects, is 2.74x faster, and uses 2.85x less GPU memory than baselines.

Conclusion: CLaRE provides a scalable, efficient solution for studying and mitigating ripple effects in LLMs, with practical applications like model editing and evaluation.
...
### [203] [A Dynamic Bayesian and Machine Learning Framework for Quantitative Evaluation and Prediction of Operator Situation Awareness in Nuclear Power Plants](https://arxiv.org/abs/2603.19298)
*Shuai Chen,Huiqiao Jia,Tao Qing,Li Zhang,Xingyu Xiao*

Main category: cs.LG

TL;DR: The study introduces DBML SA, a dynamic Bayesian machine learning framework, to quantitatively model operator situation awareness in nuclear control environments, outperforming traditional methods like SAGAT and SART.


<details>
  <summary>Details</summary>
Motivation: Existing methods for assessing operator situation awareness are static and retrospective, failing to capture evolving cognitive dynamics. DBML SA addresses this by integrating probabilistic reasoning and data-driven intelligence.

Method: The study uses a dynamic Bayesian machine learning framework (DBML SA) to analyze 212 operational event reports, reconstructing causal-temporal structures of 11 performance shaping factors across cognitive layers. It combines Bayesian inference and neural networks for predictive modeling.

Result: DBML SA achieves a 13.8% mean absolute percentage error and aligns with subjective evaluations (p > 0.05). Training quality and stress dynamics are identified as key factors affecting situation awareness.

Conclusion: DBML SA offers real-time cognitive monitoring and predictive capabilities, advancing human-machine reliability management in digital control rooms beyond traditional questionnaire-based methods.
...
### [204] [PRIME-CVD: A Parametrically Rendered Informatics Medical Environment for Education in Cardiovascular Risk Modelling](https://arxiv.org/abs/2603.19299)
*Nicholas I-Hsien Kuo,Marzia Hoque Tania,Blanca Gallego,Louisa Jorm*

Main category: cs.LG

TL;DR: PRIME-CVD is a synthetic dataset designed for medical education, offering realistic but privacy-safe data for cardiovascular risk modeling and training.


<details>
  <summary>Details</summary>
Motivation: The lack of accessible patient-level EMR data due to privacy concerns hinders reproducibility and training in cardiovascular risk modeling. PRIME-CVD addresses this gap.

Method: PRIME-CVD generates synthetic datasets from a causal directed acyclic graph using public statistics and epidemiologic estimates, avoiding real EMR data. It includes two data assets: one for analysis and another for EMR-style training.

Result: The synthetic data preserves realistic risk gradients and subgroup imbalances while ensuring negligible disclosure risk, enabling safe and effective training and research.

Conclusion: PRIME-CVD provides a scalable, privacy-compliant solution for medical education and reproducible research in cardiovascular risk modeling.
...
TL;DR: STEU is a parameter-efficient method for machine unlearning in clinical language models, achieving targeted forgetting while preserving model utility with minimal parameter changes.


<details>
  <summary>Details</summary>
Motivation: Privacy regulations and institutional policies necessitate removing sensitive information from deployed models without full retraining, requiring a balance between forgetting targeted data and maintaining model performance.

Method: STEU updates only PMI-selected token embeddings and a small classifier head while keeping encoder layers frozen, enabling class-level unlearning with sparse parameter edits.

Result: Experiments on MIMIC-IV, MIMIC-III, and eICU datasets show STEU effectively suppresses target class performance (e.g., forget F1 = 0.0004) while preserving utility (retain avg F1 = 0.4766), modifying only 0.19% of parameters.

Conclusion: Targeted behavioral unlearning can be achieved through sparse embedding edits without altering deeper model representations, offering a practical solution for privacy-compliant updates.
...
TL;DR: KD-Brain is a Prior-Informed Graph Learning framework that uses semantic and clinical priors to improve subnetwork interaction modeling in mental disorder diagnosis.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based methods struggle with learning subnetwork interactions due to limited training samples, hindering accurate diagnosis and pathway identification.

Method: KD-Brain introduces a Semantic-Conditioned Interaction mechanism to inject semantic priors into attention queries and a Pathology-Consistent Constraint to align learned interactions with clinical priors.

Result: The framework achieves state-of-the-art performance in disorder diagnosis tasks and identifies interpretable biomarkers aligned with psychiatric pathophysiology.

Conclusion: KD-Brain effectively leverages prior knowledge to enhance subnetwork interaction modeling, improving diagnosis and biomarker identification in mental health.
...
TL;DR: MemReward is a graph-based experience memory framework that uses a GNN to propagate rewards from labeled to unlabeled rollouts, achieving near-Oracle performance with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Training LLMs for complex reasoning via reinforcement learning is hindered by the scarcity of reward labels, which are expensive or time-consuming to obtain.

Method: MemReward stores rollouts (thinking processes and answers) in a heterogeneous graph, using a GNN to propagate rewards from labeled nodes to unlabeled ones during optimization.

Result: With only 20% labeled data, MemReward achieves 97.3% (3B) and 96.6% (1.5B) of Oracle performance, outperforming Oracle on out-of-domain tasks and scaling smoothly with label budget.

Conclusion: MemReward effectively addresses the label scarcity problem in reinforcement learning fine-tuning, demonstrating strong performance with limited labeled data.
...
TL;DR: LeWM is a stable end-to-end Joint Embedding Predictive Architecture (JEPA) trained with only two loss terms, achieving efficient planning and encoding meaningful physical structure in latent spaces.


<details>
  <summary>Details</summary>
Motivation: Existing Joint Embedding Predictive Architectures (JEPAs) are fragile and rely on complex losses or auxiliary supervision. The goal is to introduce a simpler, more stable alternative.

Method: LeWM uses only two loss terms: a next-embedding prediction loss and a Gaussian-distributed latent embeddings regularizer, reducing hyperparameters from six to one.

Result: LeWM trains efficiently (~15M parameters, single GPU, few hours), plans 48x faster than foundation-model-based world models, and performs well on 2D/3D control tasks. Its latent space encodes physical structure.

Conclusion: LeWM provides a stable, efficient, and interpretable JEPA framework, outperforming complex alternatives while encoding meaningful physical insights.
...
TL;DR: DPxFin is a federated learning framework with reputation-guided adaptive differential privacy for combating money laundering, balancing privacy and utility effectively.


<details>
  <summary>Details</summary>
Motivation: Address privacy leakage in federated learning for financial data while maintaining model utility in fraud detection.

Method: Integrates reputation-guided adaptive differential privacy, dynamically adjusting noise based on client reputation to enhance privacy and utility.

Result: Outperforms traditional FL and fixed-noise DP baselines in accuracy-privacy trade-off, and withstands tabular data leakage attacks.

Conclusion: DPxFin offers a scalable and effective solution for privacy-preserving fraud detection in federated settings.
...
TL;DR: The paper introduces scalable multi-scale convolutional frameworks for time series classification, focusing on diverse input representations. It proposes MSNet and LS-Net, and adapts LiteMV for univariate signals. Evaluation shows LiteMV excels in accuracy, MSNet in calibration, and LS-Net in efficiency.


<details>
  <summary>Details</summary>
Motivation: To improve time series classification performance by leveraging diverse input representations and scalable multi-scale architectures.

Method: Introduces MSNet (robust and calibrated), LS-Net (lightweight), and adapts LiteMV for multi-representation univariate signals. Evaluated on 142 datasets.

Result: LiteMV achieves highest accuracy, MSNet best calibration, and LS-Net optimal efficiency-accuracy tradeoff. Multi-representation multi-scale modeling is versatile.

Conclusion: Scalable multi-representation multi-scale learning is a practical and principled approach for modern time series classification.
...
TL;DR: The paper proposes a DL framework to tackle challenges in discrete wireless resource allocation by modeling discrete variables via support sets and joint probability distributions.


<details>
  <summary>Details</summary>
Motivation: Addressing the difficulties in DL-based methods for discrete variable problems, such as zero-gradient issues, constraint enforcement, and non-SPSD property.

Method: Introduces a support set for discrete variables, models them as random variables with joint probability distributions, and learns conditional probabilities sequentially.

Result: Outperforms existing baselines in system performance and computational efficiency for mixed-discrete wireless resource allocation problems.

Conclusion: The framework effectively handles discrete variable challenges in DL, providing a scalable solution for wireless resource allocation.
...
### [213] [Target Concept Tuning Improves Extreme Weather Forecasting](https://arxiv.org/abs/2603.19325)
*Shijie Ren,Xinyue Gu,Ziheng Peng,Haifan Zhang,Peisong Niu,Bo Wu,Xiting Wang,Liang Sun,Jirong Wen*

Main category: cs.LG

TL;DR: TaCT is an interpretable framework for fine-tuning deep learning models in meteorological forecasting, specifically improving rare event prediction (e.g., typhoons) without compromising overall performance.


<details>
  <summary>Details</summary>
Motivation: Existing fine-tuning methods struggle to balance improving rare high-impact events (like typhoons) and maintaining general forecasting performance, leading to trade-offs.

Method: TaCT uses Sparse Autoencoders and counterfactual analysis to identify failure-related internal concepts, then selectively updates model parameters based on these concepts.

Result: Experiments demonstrate improved typhoon forecasting across regions without degrading other meteorological variables. The identified concepts align with meaningful circulation patterns.

Conclusion: TaCT offers a trustworthy, interpretable approach for model adaptation in scientific forecasting, addressing biases and enhancing rare event prediction.
...
### [214] [FalconBC: Flow matching for Amortized inference of Latent-CONditioned physiologic Boundary Conditions](https://arxiv.org/abs/2603.19331)
*Chloe H. Choi,Alison L. Marsden,Daniele E. Schiavazzi*

Main category: cs.LG

TL;DR: The paper introduces a probabilistic flow-based framework for boundary condition tuning in cardiovascular modeling, addressing challenges in open-loop models and anatomies with vascular lesions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for boundary condition tuning struggle with open-loop models and anatomies affected by vascular lesions, necessitating a more robust solution.

Method: The authors propose an amortized inference framework using probabilistic flow, incorporating clinical targets, inflow features, and patient-specific anatomy embeddings.

Result: The method is validated on patient-specific models, including an aorto-iliac bifurcation with stenosis and a coronary arterial tree.

Conclusion: The framework effectively addresses the limitations of current approaches, improving boundary condition tuning in complex cardiovascular scenarios.
...
### [215] [DAPA: Distribution Aware Piecewise Activation Functions for On-Device Transformer Inference and Training](https://arxiv.org/abs/2603.19338)
*Maoyang Xiang,Bo Wang*

Main category: cs.LG

TL;DR: DAPA is a hardware-friendly, distribution-aware activation function for Transformers, improving efficiency and performance with non-uniform piecewise approximation and quantization.


<details>
  <summary>Details</summary>
Motivation: Non-linear activation functions are resource-intensive and affect system performance and energy efficiency, especially in on-device inference and training.

Method: DAPA uses a non-uniform piecewise approximation based on pre-activation data distribution, quantized using Distribution-Weighted Mean Square Error for hardware efficiency.

Result: DAPA speeds up GELU computation by 16x, reduces DSP utilization by 16x, and maintains or improves performance in vision Transformers and GPT-2 models.

Conclusion: DAPA offers a hardware-efficient, high-performance alternative to traditional activation functions for Transformer architectures.
...
### [216] [Beyond Weighted Summation: Learnable Nonlinear Aggregation Functions for Robust Artificial Neurons](https://arxiv.org/abs/2603.19344)
*Berke Deniz Bozyigit*

Main category: cs.LG

TL;DR: The paper explores replacing fixed linear aggregation in neurons with learnable nonlinear methods to improve neural network robustness, especially under noisy conditions, without compromising trainability.


<details>
  <summary>Details</summary>
Motivation: Traditional weighted summation in neurons acts like a mean-based estimator, making it sensitive to noisy or extreme inputs. The study aims to enhance robustness by introducing nonlinear aggregation mechanisms.

Method: Two differentiable aggregation methods are proposed: F-Mean (learnable power-weighted) and Gaussian Support (distance-aware affinity weighting). Hybrid neurons blend linear and nonlinear aggregation via a learnable parameter.

Result: Experiments on CIFAR-10 and its noisy variant show hybrid neurons improve robustness significantly (up to 0.991 vs. 0.890 baseline), with F-Mean hybrids also performing better on clean data. Learned parameters favor sub-linear aggregation and high novelty utilization.

Conclusion: Neuron-level aggregation is an underexplored design dimension that can enhance noise tolerance in neural networks, as demonstrated by the success of the proposed hybrid neurons.
...
### [217] [Anatomical Heterogeneity in Transformer Language Models](https://arxiv.org/abs/2603.19348)
*Tomasz Wietrzykowski*

Main category: cs.LG

TL;DR: The paper challenges the assumption of layer homogeneity in transformer models, showing significant anatomical heterogeneity across layers and proposing a cost-effective training method.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of uniform computational budgets in transformer training by analyzing layer heterogeneity and optimizing resource allocation.

Method: Analyzed SmolLM2-135M using five diagnostic metrics: weight predictability, ablation degradation, recovery speed, weight manipulation robustness, and structural analysis. Introduced Growth Transformer Training to allocate budget by layer importance.

Result: Found profound layer heterogeneity, critical core layers, and anti-layers. Proposed training method achieved ~54% cost reduction and better performance.

Conclusion: Layer heterogeneity in transformers is significant, and adaptive training methods can improve efficiency and performance.
...
### [218] [A Mathematical Theory of Understanding](https://arxiv.org/abs/2603.19349)
*Bahar Taşkesen*

Main category: cs.LG

TL;DR: The paper models how a learner's prerequisite knowledge affects their ability to absorb and act on information, showing structural and epistemic limits to learning speed and the inefficiency of one-size-fits-all teaching.


<details>
  <summary>Details</summary>
Motivation: To understand how learner-side bottlenecks impact the effectiveness of instructional signals, given that information's value depends on the learner's capacity to decode it.

Method: Develops a mathematical model where the learner (human or AI) is an abstract system with a prerequisite structure over concepts, and teaching is sequential communication with a latent target.

Result: Identifies two learning speed limits: structural (prerequisite reachability) and epistemic (target uncertainty). Shows threshold effects in training and inefficiency of broadcast curricula.

Conclusion: Effective teaching requires aligning instruction with the learner's knowledge state, highlighting the trade-offs between personalized and broadcast teaching.
...
### [219] [Warm-Start Flow Matching for Guaranteed Fast Text/Image Generation](https://arxiv.org/abs/2603.19360)
*Minyoung Kim*

Main category: cs.LG

TL;DR: The paper proposes Warm-Start Flow Matching (WS-FM), a method to speed up sample generation in flow matching algorithms by using lightweight draft models for initialization, ensuring quality while reducing computation time.


<details>
  <summary>Details</summary>
Motivation: Current generative models (AR LLMs, diffusion, flow matching) are computationally expensive during inference, requiring significant time and GPU resources. The goal is to reduce this overhead without compromising output quality.

Method: WS-FM leverages fast, low-quality draft samples from lightweight models as initial distributions for flow matching. By starting closer to the target distribution (instead of pure noise), it reduces the required time steps and guarantees speed-up.

Result: Experiments on synthetic and real-world tasks (text/image generation) confirm WS-FM achieves guaranteed speed-up without degrading sample quality.

Conclusion: WS-FM is a promising approach to efficient high-quality sample generation, bridging the gap between speed and performance in generative models.
...
### [220] [Optimizing Resource-Constrained Non-Pharmaceutical Interventions for Multi-Cluster Outbreak Control Using Hierarchical Reinforcement Learning](https://arxiv.org/abs/2603.19397)
*Xueqiao Peng,Andrew Perrault*

Main category: cs.LG

TL;DR: The paper proposes a hierarchical reinforcement learning framework to optimize resource allocation for non-pharmaceutical interventions (NPIs) in infectious disease outbreaks, outperforming baselines by 20%-30%.


<details>
  <summary>Details</summary>
Motivation: Limited resources for NPIs in early outbreak stages necessitate efficient allocation across asynchronous and heterogeneous outbreak clusters, requiring decision-making under uncertainty.

Method: A constrained restless multi-armed bandit problem is solved using a hierarchical reinforcement learning framework with a global controller and local policy for resource allocation.

Result: The framework improves outbreak control effectiveness by 20%-30% in simulations, demonstrating scalability and faster decision-making for up to 40 concurrent clusters.

Conclusion: The hierarchical approach effectively addresses resource allocation challenges in infectious disease outbreaks, outperforming existing methods.
...
### [221] [GeoLAN: Geometric Learning of Latent Explanatory Directions in Large Language Models](https://arxiv.org/abs/2603.19460)
*Tianyu Bell Pan,Damon L. Woodard*

Main category: cs.LG

TL;DR: GeoLAN is a training framework for LLMs that uses geometric trajectories and stickiness conditions to improve transparency, metrics, and fairness, with notable benefits in mid-sized models.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of transparency in large language models (LLMs) by introducing a geometry-aware training approach.

Method: Developed GeoLAN with two differentiable regularizers (KT-CW and KT-Attn) to promote isotropy and diverse attention, tested on Gemma-3 and Llama-3-8B models.

Result: GeoLAN maintains task accuracy while improving geometric metrics and reducing fairness biases, especially in mid-sized models.

Conclusion: Geometry-aware training enhances mechanistic interpretability, revealing scale-dependent trade-offs between precision and performance.
...
### [222] [Deep Hilbert--Galerkin Methods for Infinite-Dimensional PDEs and Optimal Control](https://arxiv.org/abs/2603.19463)
*Samuel N. Cohen,Filippo de Feo,Jackson Hebner,Justin Sirignano*

Main category: cs.LG

TL;DR: Deep learning methods for solving nonlinear PDEs on Hilbert spaces using novel neural operators and proving universal approximation theorems.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of solving fully nonlinear second-order PDEs, such as HJB equations, on infinite-dimensional Hilbert spaces, which are not well-handled by existing methods.

Method: The authors introduce Hilbert--Galerkin Neural Operators (HGNOs) and prove Universal Approximation Theorems (UATs) for these operators. They also develop numerical training methods, Deep Hilbert--Galerkin and Hilbert Actor-Critic, to minimize the residual of the PDE in the Hilbert space.

Result: The paper demonstrates the ability of HGNOs to approximate solutions and their derivatives, including optimal feedback controls. Numerical examples solve Kolmogorov and HJB PDEs for heat and Burgers' equations.

Conclusion: The proposed deep learning-based approach shows promise for solving complex PDEs in infinite-dimensional spaces, with applications across various scientific domains.
...
### [223] [Global Convergence of Multiplicative Updates for the Matrix Mechanism: A Collaborative Proof with Gemini 3](https://arxiv.org/abs/2603.19465)
*Keith Rush*

Main category: cs.LG

TL;DR: The paper analyzes a fixed-point iteration in nuclear norm optimization, proving its monotonic convergence to a unique global optimizer, while also discussing AI's role in the proof process.


<details>
  <summary>Details</summary>
Motivation: To address an open problem in optimizing a regularized nuclear norm objective and explore the practical use of AI (Gemini 3) in mathematical proofs.

Method: The study employs a fixed-point iteration and proves its convergence using corrections and interventions based on AI (Gemini 3) contributions.

Result: The iteration converges monotonically to the unique global optimizer, solving the open problem.

Conclusion: The paper closes a gap in the literature and highlights AI's utility in mathematical research, offering insights into collaborative AI-human proof strategies.
...
### [224] [Adaptive Layerwise Perturbation: Unifying Off-Policy Corrections for LLM RL](https://arxiv.org/abs/2603.19470)
*Chenlu Ye,Xuanchang Zhang,Yifan Hao,Zhou Yu,Ziji Zhang,Abhinav Gullapalli,Hao Chen,Jing Huang,Tong Zhang*

Main category: cs.LG

TL;DR: ALP introduces adaptive perturbations to hidden states to stabilize training and reduce the gap between inference and updated policies in LLM RL.


<details>
  <summary>Details</summary>
Motivation: Address off-policy issues like policy staleness and training-inference mismatch in LLM RL, which cause heavy-tailed importance ratios and unstable training.

Method: Proposes Adaptive Layerwise Perturbation (ALP), injecting learnable perturbations into input hidden states during updates to flatten the distribution of importance ratios.

Result: ALP improves final performance, avoids importance ratio tail blow-up and KL spikes, and boosts exploration in tasks like math and tool-integrated reasoning.

Conclusion: ALP effectively stabilizes training and enhances performance by mitigating policy deviation and heavy-tailed ratios.
...
### [225] [TRACE: Trajectory Recovery with State Propagation Diffusion for Urban Mobility](https://arxiv.org/abs/2603.19474)
*Jinming Wang,Hai Wang,Hongkai Wen,Geyong Min,Man Luo*

Main category: cs.LG

TL;DR: TRACE introduces a novel diffusion model for recovering sparse GPS trajectories into dense, continuous forms, outperforming state-of-the-art methods with over 26% accuracy improvement.


<details>
  <summary>Details</summary>
Motivation: High-quality GPS trajectories are crucial for location-based services, but real-world data is often sparse and uneven. Recovering these trajectories is challenging due to complex spatio-temporal patterns.

Method: TRACE uses a State Propagation Diffusion Model (SPDM) with a memory mechanism to retain and leverage intermediate results during denoising for effective trajectory reconstruction.

Result: Extensive experiments show TRACE outperforms state-of-the-art methods, offering over 26% accuracy improvement without significant inference overhead.

Conclusion: TRACE advances the quality of GPS trajectory recovery, benefiting mobile and web-connected location services and urban applications.
...
### [226] [Any-Subgroup Equivariant Networks via Symmetry Breaking](https://arxiv.org/abs/2603.19486)
*Abhinav Goel,Derek Lim,Hannah Lawrence,Stefanie Jegelka,Ningyuan Huang*

Main category: cs.LG

TL;DR: ASEN is a versatile model that achieves equivariance to multiple subgroups by modulating input features, outperforming traditional equivariant and non-equivariant models.


<details>
  <summary>Details</summary>
Motivation: Current equivariant architectures are limited by their rigid design for specific symmetries, hindering the development of flexible, multi-modal models.

Method: ASEN starts with a permutation-equivariant base model and uses symmetry-breaking inputs to achieve subgroup equivariance, addressing computational challenges with approximate methods.

Result: ASEN successfully simulates equivariant MLPs and shows empirical superiority in symmetry selection, multitask, and transfer learning tasks.

Conclusion: ASEN demonstrates the feasibility of a single model achieving equivariance across multiple subgroups, offering a flexible and efficient alternative to traditional approaches.
...
### [227] [ICLAD: In-Context Learning for Unified Tabular Anomaly Detection Across Supervision Regimes](https://arxiv.org/abs/2603.19497)
*Jack Yi Wei,Narges Armanfard*

Main category: cs.LG

TL;DR: ICLAD is a foundation model for tabular anomaly detection that generalizes across datasets and supervision regimes, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for anomaly detection are limited by dataset-specific training and single supervision regimes, missing opportunities to leverage shared structures and adapt to different supervision levels.

Method: ICLAD uses meta-learning on synthetic tasks and in-context learning at inference time, assigning anomaly scores without updating model weights.

Result: ICLAD achieves state-of-the-art performance across 57 tabular datasets from ADBench under three supervision regimes.

Conclusion: ICLAD establishes a unified framework for tabular anomaly detection, demonstrating superior generalization and adaptability.
...
### [228] [Stochastic Sequential Decision Making over Expanding Networks with Graph Filtering](https://arxiv.org/abs/2603.19501)
*Zhan Gao,Bishwadeep Das,Elvin Isufi*

Main category: cs.LG

TL;DR: The paper proposes a stochastic sequential decision-making framework for graph filtering that adapts to expanding graphs, using multi-agent reinforcement learning and a context-aware graph neural network for long-term rewards and dynamic adaptation.


<details>
  <summary>Details</summary>
Motivation: Existing graph filters focus on fixed graphs and ignore the dynamic expansion of graphs with uncertain patterns. Current methods also lack a long-term perspective, relying on past or present information without considering future impacts.

Method: The authors introduce a framework where filter shifts are represented as agents in a multi-agent system, trained using multi-agent reinforcement learning. They also develop a context-aware graph neural network to parameterize the policy, adapting filter parameters based on graph and agent information.

Result: Experiments on synthetic and real-world datasets (e.g., cold-start recommendation and COVID prediction) demonstrate the superiority of the proposed sequential decision-making approach over batch and online filtering methods.

Conclusion: The proposed framework effectively addresses the limitations of existing graph filtering methods by incorporating future impacts and dynamic graph expansion, validated by improved performance in diverse applications.
...
### [229] [Scalable Cross-Facility Federated Learning for Scientific Foundation Models on Multiple Supercomputers](https://arxiv.org/abs/2603.19544)
*Yijiang Li,Zilinghan Li,Kyle Chard,Ian Foster,Todd Munson,Ravi Madduri,Kibaek Kim*

Main category: cs.LG

TL;DR: A framework for federated learning (FL) in heterogeneous HPC environments is introduced, addressing challenges like resource heterogeneity and scheduling, validated by fine-tuning a large language model.


<details>
  <summary>Details</summary>
Motivation: Scientific applications often need large-scale models trained on decentralized data due to privacy, sovereignty, or volume constraints. FL enables this without centralizing data, but deploying it in HPC environments poses unique challenges.

Method: The study uses the APPFL framework with Globus Compute and Transfer orchestration, deploying FL experiments across four DOE supercomputers. It explores algorithmic choices and heterogeneity impacts.

Result: FL across HPC facilities is feasible. Heterogeneity and scheduling significantly affect performance, with algorithmic choices playing a crucial role. A large language model was successfully fine-tuned on a chemistry dataset.

Conclusion: The framework proves practical for FL in HPC settings, highlighting scheduler-aware algorithm design as a critical challenge for future work.
...
### [230] [Subspace Kernel Learning on Tensor Sequences](https://arxiv.org/abs/2603.19546)
*Lei Wang,Xi Ding,Yongsheng Gao,Piotr Koniusz*

Main category: cs.LG

TL;DR: UKTL introduces a scalable, uncertainty-aware kernel framework for multi-mode tensor data, achieving state-of-the-art performance in action recognition.


<details>
  <summary>Details</summary>
Motivation: To capture complex interactions in tensor data while maintaining computational efficiency and robustness.

Method: Uses a kernel framework with dynamic pivot tensors via soft k-means, uncertainty-aware subspace weighting, and structured kernel compositions.

Result: Achieves top performance on action recognition benchmarks (NTU-60, NTU-120, Kinetics-Skeleton) with improved generalization and interpretability.

Conclusion: Establishes a scalable, interpretable kernel learning paradigm for structured multi-way tensor sequences.
...
### [231] [Neural Uncertainty Principle: A Unified View of Adversarial Fragility and LLM Hallucination](https://arxiv.org/abs/2603.19562)
*Dong-Xiao Zhang,Hu Lou,Jun-Jie Zhang,Jun Zhu,Deyu Meng*

Main category: cs.LG

TL;DR: The paper introduces a Neural Uncertainty Principle (NUP) to unify adversarial vulnerability in vision and hallucination in language models, attributing both to a common geometric origin. It proposes practical solutions like ConjMask and LogitReg for robustness and hallucination detection.


<details>
  <summary>Details</summary>
Motivation: To address the seemingly separate issues of adversarial fragility in vision and hallucination in language models by identifying their shared geometric origin and providing a unified theoretical framework.

Method: Formalizes the Neural Uncertainty Principle (NUP) to explain the trade-off between compression and sensitivity dispersion. Uses a single-backward probe to measure input-gradient correlation and proposes ConjMask and LogitReg for practical mitigation.

Result: NUP successfully unifies the two problems under a shared uncertainty-budget framework. ConjMask improves robustness without adversarial training, and the probe detects hallucination risk in language models.

Conclusion: NUP offers a principled, unified approach to diagnosing and mitigating boundary anomalies in both perception and generation tasks, providing practical tools for reliability analysis.
...
### [232] [Wearable Foundation Models Should Go Beyond Static Encoders](https://arxiv.org/abs/2603.19564)
...
TL;DR: The paper advocates for wearable foundation models (WFMs) to evolve beyond static, short-term health monitoring to support longitudinal, anticipatory health reasoning by integrating richer data, longitudinal-aware modeling, and agentic inference systems.


<details>
  <summary>Details</summary>
Motivation: Current WFMs are limited to short-term, retrospective health tasks and lack the ability to model chronic or progressive conditions. The paper argues for a shift toward longitudinal, anticipatory health reasoning.

Method: The authors propose three foundational shifts: (1) Structurally rich data integration, (2) Longitudinal-aware multimodal modeling, and (3) Agentic inference systems for planning and decision-making.

Result: These shifts aim to transform WFMs from static, short-term predictors to dynamic systems capable of continuous, anticipatory health monitoring and intervention.

Conclusion: The paper concludes that refocusing WFMs on longitudinal, anticipatory reasoning can better support chronic and progressive health conditions, aligning wearable health monitoring with personalized, proactive healthcare.
...
### [233] [Demonstrations, CoT, and Prompting: A Theoretical Analysis of ICL](https://arxiv.org/abs/2603.19611)
*Xuhan Tong,Yuchen Zeng,Jiawei Zhang*

Main category: cs.LG

TL;DR: The paper analyzes In-Context Learning (ICL) in LLMs, linking design choices like demonstration selection and CoT prompting to generalization behavior, and validates insights experimentally.


<details>
  <summary>Details</summary>
Motivation: To address the gap in theoretical understanding of ICL, by analyzing practical factors like demonstration selection, CoT prompting, and prompt templates under mild assumptions.

Method: Theoretical analysis of ICL, deriving an upper bound on test loss and examining factors like demonstration quality, intrinsic ICL capability, and distribution shift. CoT prompting is analyzed as task decomposition.

Result: ICL performance depends on demonstration quality, model capability, and distribution shift. CoT improves performance when subtasks are well chosen and easier to learn. Performance sensitivity to prompt templates varies with the number of demonstrations.

Conclusion: Pretraining enables generalization to unseen tasks, CoT allows composing subtasks, and demonstrations/instructions aid retrieval and composition. Theoretical insights are experimentally validated.
...
### [234] [On Performance Guarantees for Federated Learning with Personalized Constraints](https://arxiv.org/abs/2603.19617)
*Mohammadjavad Ebrahimi,Daniel Burbano,Farzad Yousefian*

Main category: cs.LG

TL;DR: PC-FedAvg is a method for personalized constrained federated learning that maintains cross-estimates of agents' variables, enabling personalization without consensus or shared constraints.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of heterogeneous resource or model constraints in federated learning by allowing each agent to have its own convex local objective and private constraint set.

Method: PC-FedAvg uses a multi-block local decision vector for each agent, updating all blocks locally while penalizing infeasibility only in its own block. Cross-estimates enable personalization without sharing constraint information.

Result: The method achieves communication-complexity rates of O(ε−2) for suboptimality and O(ε−1) for agent-wise infeasibility, validated by experiments on MNIST and CIFAR-10.

Conclusion: PC-FedAvg effectively handles personalized constrained federated optimization, balancing communication efficiency and agent-specific constraints without requiring consensus.
...
### [235] [DeepStock: Reinforcement Learning with Policy Regularizations for Inventory Management](https://arxiv.org/abs/2603.19621)
*Yaqi Xie,Xinru Hao,Jiaxi Liu,Will Ma,Linwei Xin,Lei Cao,Yidong Zhang*

Main category: cs.LG

TL;DR: DRL with policy regularizations improves inventory management by simplifying hyperparameter tuning and enhancing performance, as demonstrated in Alibaba's Tmall and synthetic experiments.


<details>
  <summary>Details</summary>
Motivation: Despite DRL's potential for inventory management, its sensitivity to hyperparameters hinders consistent success. Policy regularizations inspired by classical concepts aim to address this.

Method: Imposed policy regularizations based on classical inventory concepts like 'Base Stock' to refine DRL training processes.

Result: Significant acceleration in hyperparameter tuning and improved performance of DRL methods, confirmed by deployment on Alibaba's Tmall and synthetic tests.

Conclusion: Policy regularizations make DRL more effective and practical for inventory management, reshaping preferred methods.
...
### [236] [Continual Learning for Food Category Classification Dataset: Enhancing Model Adaptability and Performance](https://arxiv.org/abs/2603.19624)
...
TL;DR: Proposes a continual learning framework for text-guided food classification to integrate new categories without retraining from scratch, improving adaptability in dietary applications.


<details>
  <summary>Details</summary>
Motivation: Addresses the limitation of fixed datasets in capturing domain diversity and the inability of conventional pipelines to recognize new categories.

Method: Introduces a continual learning framework that enables incremental updates for integrating new food categories without degrading prior knowledge.

Result: Demonstrates potential for adaptive food recognition, though further refinements are needed.

Conclusion: The framework shows promise for applications in dietary monitoring and personalized nutrition planning.
...
### [237] [Alternating Diffusion for Proximal Sampling with Zeroth Order Queries](https://arxiv.org/abs/2603.19633)
...
### [238] [RiboSphere: Learning Unified and Efficient Representations of RNA Structures](https://arxiv.org/abs/2603.19636)
*Zhou Zhang,Hanqun Cao,Cheng Tan,Fang Wu,Pheng Ann Heng,Tianfan Fu*

Main category: cs.LG

TL;DR: RiboSphere is a framework for RNA structure modeling that uses discrete geometric representations, combining vector quantization and flow matching to achieve high-fidelity RNA structure reconstruction and capture motif-level compositional details.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from challenges in RNA structure modeling due to backbone flexibility, non-canonical interactions, and limited experimental data. RiboSphere addresses these by leveraging modular RNA architecture and discrete representations.

Method: RiboSphere employs a geometric transformer encoder for SE(3)-invariant features, discretizes them with finite scalar quantization (FSQ), and reconstructs atomic coordinates using a flow-matching decoder conditioned on discrete codes.

Result: RiboSphere achieves strong performance in structure reconstruction (RMSD 1.25Å, TM-score 0.84) and effectively transfers pretrained representations to inverse folding and RNA-ligand binding prediction, even with scarce data.

Conclusion: The framework successfully models RNA structures by capturing motif-level compositionality and demonstrates robust generalization, making it a promising tool for RNA structural biology.
...
### [239] [Heavy-Tailed and Long-Range Dependent Noise in Stochastic Approximation: A Finite-Time Analysis](https://arxiv.org/abs/2603.19648)
*Siddharth Chandak,Anuj Yadav,Ayfer Ozgur,Nicholas Bambos*

Main category: cs.LG

TL;DR: This paper studies Stochastic Approximation (SA) under heavy-tailed and long-range dependent noise, providing finite-time moment bounds and explicit convergence rates.


<details>
  <summary>Details</summary>
Motivation: Existing SA analyses rely on classical noise assumptions, but practical scenarios often involve heavy-tailed and long-range dependent noise, which this work addresses.

Method: The authors use a noise-averaging argument to regularize noise impact without altering the SA iteration, and apply this to SGD and gradient play.

Result: First finite-time moment bounds are established, with explicit convergence rates that quantify the effects of heavy tails and temporal dependence.

Conclusion: The framework generalizes SA analysis to non-classical noise models and is validated through numerical experiments.
...
### [240] [Ensembles-based Feature Guided Analysis](https://arxiv.org/abs/2603.19653)
*Federico Formica,Stefano Gregis,Andrea Rota,Aurora Francesca Zanenga,Mark Lawford,Claudio Menghi*

Main category: cs.LG

TL;DR: EFGA improves the recall of FGA by combining rules into ensembles with different aggregation criteria, showing higher recall with minor precision loss.


<details>
  <summary>Details</summary>
Motivation: Existing solutions like FGA have limited recall in explaining DNN behaviors, prompting the need for improved techniques.

Method: EFGA aggregates FGA-extracted rules into ensembles using different aggregation criteria to enhance applicability.

Result: EFGA significantly boosts recall (+28.51% to +33.15% on MNIST/LSC) with negligible precision drops (-0.89% to -0.69%).

Conclusion: EFGA effectively balances precision and recall, outperforming FGA in explainability for DNNs.
...
### [241] [The Residual Stream Is All You Need: On the Redundancy of the KV Cache in Transformer Inference](https://arxiv.org/abs/2603.19664)
*Kaleem Ullah Qasim,Jiashu Zhang,Muhammad Kafeel Shaheen,Razan Alharith,Heying Zhang*

Main category: cs.LG

TL;DR: The paper demonstrates that the key-value (KV) cache in transformer inference is redundant, showing that keys and values can be recomputed from the residual stream with zero error. The proposed KV-Direct method reduces memory usage significantly while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To challenge the necessity of the KV cache in transformer inference, proving it can be replaced by recomputing keys and values from the residual stream without loss of accuracy.

Method: The study verifies redundancy across six models, introduces KV-Direct for bounded-memory inference by checkpoints residual vectors, and compares it against baselines.

Result: KV-Direct maintains 100% token match at all cache budgets, reduces peak memory to 42 MB (vs. 103 MB standard), and shows recomputation is up to 5x faster than reading cached tensors.

Conclusion: The KV cache is entirely redundant, and KV-Direct offers an efficient alternative that significantly reduces memory overhead while preserving output quality.
...
### [242] [Scale-Dependent Radial Geometry and Metric Mismatch in Wasserstein Propagation for Reverse Diffusion](https://arxiv.org/abs/2603.19670)
*Zicheng Lyu,Zengfeng Huang*

Main category: cs.LG

TL;DR: The paper analyzes reverse diffusion under weak log-concavity, revealing a metric mismatch between early radial contraction and terminal Euclidean error measurement. A one-switch routing argument is proposed to exploit this structure, yielding non-asymptotic guarantees for reverse SDE discretizations.


<details>
  <summary>Details</summary>
Motivation: Existing analyses treat sampling error in Euclidean geometry, but weak log-concavity introduces radial contraction early on, creating a mismatch with terminal error measurement. This motivates a new framework to address the discrepancy.

Method: The paper formalizes the metric mismatch with a radial lower profile for the reverse drift. A one-switch routing argument is used: reflection coupling for early contraction, followed by a metric switch and propagation in Euclidean geometry.

Result: Non-asymptotic end-to-end guarantees for reverse SDE discretizations are derived, along with a switch-selection objective and a structural limit on the conversion exponent.

Conclusion: The proposed approach effectively handles the metric mismatch in reverse diffusion, providing practical guarantees and insights for learned reverse drift.
...
### [243] [Ontology-Based Knowledge Modeling and Uncertainty-Aware Outdoor Air Quality Assessment Using Weighted Interval Type-2 Fuzzy Logic](https://arxiv.org/abs/2603.19683)
*Md Inzmam,Ritesh Chandra,Sadhana Tiwari,Sonali Agarwal,Triloki Pant*

Main category: cs.LG

TL;DR: The paper proposes a hybrid ontology-based uncertainty-aware framework for air quality monitoring, integrating fuzzy logic and semantic modeling to improve AQI classification and decision-making.


<details>
  <summary>Details</summary>
Motivation: Traditional AQI calculations lack the ability to handle uncertainty and transitions between classes, prompting the need for a more robust framework.

Method: The study combines Weighted Interval Type-2 Fuzzy Logic with semantic knowledge modeling, using IT2-FAHP for pollutant weighting and an OWL-based ontology for representation. Semantic reasoning is implemented via SWRL rules and SPARQL queries.

Result: The framework shows better reliability in AQI classification and uncertainty handling compared to traditional methods, offering explainable reasoning and decision support.

Conclusion: The proposed approach enhances air quality monitoring by addressing uncertainty and improving semantic reasoning, making it more effective for environmental governance.
...
### [244] [Regret Analysis of Sleeping Competing Bandits](https://arxiv.org/abs/2603.19700)
*Shinnosuke Uba,Yutaro Yamaguchi*

Main category: cs.LG

TL;DR: The paper introduces Sleeping Competing Bandits, a variant of the Competing Bandits framework, where player and arm availability varies over time. It extends regret definitions, proposes an algorithm with tight regret bounds, and proves its asymptotic optimality for certain cases.


<details>
  <summary>Details</summary>
Motivation: Real-world scenarios often involve varying availability of players and arms, unlike traditional models. This paper addresses this gap by formalizing Sleeping Competing Bandits.

Method: The authors extend regret definitions for this setting and propose an algorithm with asymptotic regret bounds under reasonable assumptions.

Result: The algorithm achieves an upper regret bound of O(NK log T_i/Δ²) and a lower bound of Ω(N(K−N+1) log T_i/Δ²), proving asymptotic optimality when K ≫ N.

Conclusion: The work formalizes a practical variant of Competing Bandits, provides a near-optimal algorithm, and establishes theoretical regret bounds, advancing the field of online learning and game theory.
...
### [245] [Learning from Similarity/Dissimilarity and Pairwise Comparison](https://arxiv.org/abs/2603.19713)
*Tomoya Tate,Kosuke Sugiyama,Masato Uchida*

Main category: cs.LG

TL;DR: The paper introduces SD-Pcomp, a weakly supervised learning method for binary classification using relative judgments (SD and Pcomp labels) instead of probabilistic supervision, improving performance and robustness to label noise.


<details>
  <summary>Details</summary>
Motivation: Existing methods like SconfConfDiff rely on probabilistic labels, which are subjective and unstable. The study aims to develop a more reliable framework using binary judgments.

Method: The SD-Pcomp framework uses Similarity/Dissimilarity (SD) and Pairwise Comparison (Pcomp) labels, with two unbiased risk estimators: a convex combination of SD and Pcomp, and a unified estimator integrating both.

Result: The proposed method outperforms single weak-label methods, is robust to label noise, and handles uncertainty in class prior estimation effectively.

Conclusion: SD-Pcomp provides a more stable and effective alternative to probabilistic supervision in weakly supervised binary classification.
...
### [246] [FedRG: Unleashing the Representation Geometry for Federated Learning with Noisy Clients](https://arxiv.org/abs/2603.19722)
*Tian Wen,Zhiqin Yang,Yonggang Zhang,Xuefeng Jiang,Hao Peng,Yuwei Wang,Bo Han*

Main category: cs.LG

TL;DR: The paper proposes FedRG, a method to improve federated learning performance by addressing noisy labels through representation geometry and self-supervision.


<details>
  <summary>Details</summary>
Motivation: Federated learning (FL) faces performance drops due to noisy annotations in distributed settings. Existing methods relying on scalar loss values are unreliable in heterogeneous scenarios.

Method: FedRG uses self-supervision to create label-agnostic spherical representations, iteratively fits a vMF mixture model to capture semantic clusters, and integrates geometric evidence with a soft mapping mechanism to identify noisy samples. It also employs a personalized noise absorption matrix for robust optimization.

Result: FedRG significantly outperforms state-of-the-art methods in FL scenarios with data heterogeneity and diverse noisy clients.

Conclusion: FedRG effectively addresses noisy label challenges in FL by prioritizing representation geometry, demonstrating superior performance in heterogeneous and noisy settings.
...
### [247] [FedPDPO: Federated Personalized Direct Preference Optimization for Large Language Model Alignment](https://arxiv.org/abs/2603.19741)
*Kewen Zhu,Liping Yi,Zhiming Zhao,Zhuang Qi,Han Yu,Qinghua Hu*

Main category: cs.LG

TL;DR: The paper introduces FedPDPO, a personalized federated framework for aligning large language models (LLMs) with human preferences in federated learning (FL), addressing challenges of non-IID data and limited reward generalization.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with human preferences in FL is difficult due to decentralized, privacy-sensitive, and non-IID data. Existing methods like DPO degrade in performance under such conditions.

Method: FedPDPO uses a parameter-efficient fine-tuning architecture (LoRA adapters) and introduces a globally shared LoRA adapter, a personalized DPO training strategy with client-specific rewards, and a bottleneck adapter for balancing global and local features.

Result: Extensive experiments show FedPDPO achieves up to 4.80% average accuracy improvement in federated intra-domain and cross-domain settings.

Conclusion: FedPDPO effectively addresses non-IID heterogeneity and improves preference alignment in FL, outperforming existing methods.
...
### [248] [Dual Path Attribution: Efficient Attribution for SwiGLU-Transformers through Layer-Wise Target Propagation](https://arxiv.org/abs/2603.19742)
*Lasse Marten Jantsch,Dong-Jae Koh,Seonghyeon Lee,Young-Kyoon Suh*

Main category: cs.LG

TL;DR: DPA is a novel framework for efficient and faithful attribution in transformer-based LLMs, requiring only one forward and one backward pass.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of dense component attribution in transformer-based LLMs, which is currently too expensive, while maintaining faithfulness and computational efficiency.

Method: Introduces Dual Path Attribution (DPA), which decomposes and linearizes the SwiGLU Transformers' computational structure into distinct pathways, enabling target-centric propagation with O(1) time complexity.

Result: DPA achieves state-of-the-art faithfulness and unprecedented efficiency in interpretability benchmarks.

Conclusion: DPA provides a scalable and efficient solution for tracing information flow in transformer models without the need for counterfactual examples.
...
### [249] [Scalable Learning of Multivariate Distributions via Coresets](https://arxiv.org/abs/2603.19792)
*Zeyu Ding,Katja Ickstadt,Nadja Klein,Alexander Munteanu,Simon Omlor*

Main category: cs.LG

TL;DR: A novel coreset construction for multivariate conditional transformation models (MCTMs) improves scalability and efficiency for large-scale regression and density estimation, maintaining statistical accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for non-parametric or semi-parametric regression and density estimation struggle with large-scale data, requiring scalable solutions.

Method: Develops coresets for MCTMs using importance sampling, ensuring log-likelihood remains within multiplicative error bounds. Geometric approximation handles logarithmic terms for stable inference.

Result: Achieves substantial data reduction while maintaining accuracy. Outperforms full-parametric models in adaptability for complex distributions and non-linear relationships.

Conclusion: The approach significantly enhances computational efficiency for large datasets, enabling broader applications in statistics and machine learning.
...
### [250] [Quantifying Gate Contribution in Quantum Feature Maps for Scalable Circuit Optimization](https://arxiv.org/abs/2603.19805)
*F. Rodríguez-Díaz,D. Gutiérrez-Avilés,A. Troncoso,F. Martínez-Álvarez*

Main category: cs.LG

TL;DR: GATE optimizes quantum feature maps by ranking gates via a significance index, reducing circuit size and runtime while maintaining or improving accuracy in quantum machine learning tasks.


<details>
  <summary>Details</summary>
Motivation: Current quantum devices face limitations like noise, decoherence, and connectivity constraints, hindering efficient execution of feature map-based circuits in quantum machine learning.

Method: The paper introduces GATE, a circuit optimization methodology that reduces quantum feature maps using a gate significance index. This index combines fidelity, entanglement, and sensitivity to quantify gate relevance. The approach iteratively eliminates low-contribution gates and evaluates optimized models based on accuracy, runtime, and balanced performance.

Result: Evaluations on real-world datasets using PegasosQSVM and Quantum Neural Network models show consistent reductions in circuit size and runtime, with preserved or improved predictive accuracy. Best trade-offs occur at intermediate thresholds.

Conclusion: GATE effectively optimizes quantum feature maps, demonstrating practical benefits for quantum machine learning despite device limitations.
...
TL;DR: A VR study explores pedestrian interactions with automated shuttles, proposing GazeX-LSTM, a model combining gaze and contextual data for better behavior prediction.


<details>
  <summary>Details</summary>
Motivation: Predicting pedestrian behavior in unstructured urban spaces with automated shuttles is crucial for safety and efficiency.

Method: VR study capturing pedestrian interactions, proposing GazeX-LSTM to integrate gaze and contextual data for behavior prediction.

Result: Eye gaze and contextual data improve prediction accuracy, showing complementary roles in understanding pedestrian behavior.

Conclusion: Eye gaze-informed modeling enhances pedestrian behavior prediction, aiding safer automated vehicle technologies in shared spaces.
...
TL;DR: GDEGAN introduces adaptive kernels to replace dot-product attention in equivariant GNNs, improving binding site prediction accuracy by capturing local feature variations, outperforming existing methods by 37-66% in DCC and 7-19% in DCA success rates.


<details>
  <summary>Details</summary>
Motivation: Accurate binding site prediction is crucial for computational drug discovery. Current equivariant GNN methods fail to account for variations in chemical and geometric properties of neighboring residues, limiting their effectiveness.

Method: GDEGAN replaces dot-product attention with adaptive kernels that use local feature distribution statistics. It dynamically computes neighborhood statistics, using local variance as an adaptive bandwidth parameter with learnable temperatures, allowing context-specific importance for each protein region.

Result: GDEGAN shows significant improvements over state-of-the-art methods, achieving 37-66% better DCC and 7-19% higher DCA success rates across COACH420, HOLO4k, and PDBBind2020 datasets.

Conclusion: GDEGAN's adaptive attention mechanism enhances binding site prediction, offering a powerful tool for accelerating protein-ligand docking and therapeutic target identification in drug discovery.
...
TL;DR: FIPO is a reinforcement learning algorithm that improves reasoning in large language models by re-weighting tokens based on their future influence, outperforming existing baselines in length and accuracy.


<details>
  <summary>Details</summary>
Motivation: Current methods like GRPO rely on coarse-grained credit assignment, limiting performance by not distinguishing critical logical pivots from trivial tokens.

Method: FIPO incorporates discounted future-KL divergence into policy updates to create a dense advantage formulation, re-weighting tokens based on their influence.

Result: FIPO extends chain-of-thought length from 4,000 to 10,000 tokens and increases AIME 2024 Pass@1 accuracy from 50.0% to 58.0%, outperforming competitors.

Conclusion: Dense advantage formulations are crucial for unlocking the full reasoning potential of base models in ORM-based algorithms.
...
### [254] [NASimJax: GPU-Accelerated Policy Learning Framework for Penetration Testing](https://arxiv.org/abs/2603.19864)
*Raphael Simon,José Carrasquel,Wim Mees,Pieter Libin*

Main category: cs.LG

TL;DR: NASimJax, a JAX-based reimplementation of NASim, offers 100x faster training for RL-based penetration testing, enabling scalable and diverse network simulations for improved policy generalization. Methods like Prioritized Level Replay and action decomposition (2SAS) enhance performance at scale.


<details>
  <summary>Details</summary>
Motivation: Existing simulators for penetration testing RL policies are too slow, limiting scalability and generalization. NASimJax addresses this by enabling faster, larger-scale training.

Method: The paper introduces NASimJax, a JAX-based reimplementation of NASim, and formulates penetration testing as a Contextual POMDP. It uses Prioritized Level Replay, Domain Randomization, and a two-stage action decomposition (2SAS) for scalable training.

Result: NASimJax achieves up to 100x higher throughput, enabling training on larger networks. Prioritized Level Replay outperforms Domain Randomization, and 2SAS improves action-space scalability. Training on sparse topologies enhances generalization.

Conclusion: NASimJax provides a fast, realistic platform for advancing RL-based penetration testing, with methods like 2SAS and Prioritized Level Replay improving scalability and generalization.
...
### [255] [On the Dynamics & Transferability of Latent Generalization during Memorization](https://arxiv.org/abs/2603.19865)
*Simran Ketha,Venkatakrishnan Ramaswamy*

Main category: cs.LG

TL;DR: Deep networks can memorize shuffled labels but still retain latent generalization, which can be recovered via probes. Latent generalization peaks early in training and may be linearly decodable, with potential for transfer to model generalization.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the dynamics of latent generalization in deep networks during memorization and explore whether this latent generalization can be linearly decoded or transferred to improve model generalization.

Method: The authors track training dynamics empirically, analyze the quadratic classifier (MASC probe), design a new linear probe, and explore weight editing to transfer latent generalization to the model.

Result: Latent generalization peaks early in training, and a new linear probe shows potential for recovering this generalization. Weight editing can transfer latent generalization to the model.

Conclusion: Latent generalization exists in deep networks even during memorization and can be linearly decoded or transferred to enhance model performance, offering insights into network behavior.
...
### [256] [Discovery of Decision Synchronization Patterns from Event Logs](https://arxiv.org/abs/2603.19879)
*Tijmen Kuijpers,Karolin Winter,Remco Dijkman*

Main category: cs.LG

TL;DR: The paper proposes an approach to discover decision synchronization patterns in business processes, inspired by supply chains, and validates it in two scenarios.


<details>
  <summary>Details</summary>
Motivation: Decision synchronization in business processes is crucial for fairness, efficiency, and prioritization, but current process discovery techniques often overlook it.

Method: The approach involves formalizing and discovering constraints for four decision synchronization patterns, validated in artificial scenarios with single and multiple patterns.

Result: The approach reliably discovered the expected patterns in both scenarios, proving its effectiveness.

Conclusion: The proposed method successfully addresses the gap in discovering decision synchronization patterns, demonstrating its potential for practical applications.
...
### [257] [What If Consensus Lies? Selective-Complementary Reinforcement Learning at Test Time](https://arxiv.org/abs/2603.19880)
*Dong Yan,Jian Liang,Yanbo Wang,Shuo Lu,Ran He,Tieniu Tan*

Main category: cs.LG

TL;DR: SCRL improves test-time reinforcement learning for LLMs by filtering unreliable pseudo-labels and introducing negative supervision to prune incorrect trajectories, enhancing robustness and performance.


<details>
  <summary>Details</summary>
Motivation: Existing TTRL methods are vulnerable to weak consensus scenarios, reinforcing incorrect trajectories due to reliance on positive pseudo-labeling alone.

Method: SCRL employs Selective Positive Pseudo-Labeling with strict consensus criteria and introduces Entropy-Gated Negative Pseudo-Labeling for pruning incorrect trajectories based on uncertainty.

Result: SCRL outperforms baselines in reasoning benchmarks, showing robust generalization and training stability under constrained budgets.

Conclusion: SCRL effectively mitigates label noise amplification and enhances reasoning performance in LLMs, presenting a significant advancement in TTRL.
...
### [258] [Integrating Meta-Features with Knowledge Graph Embeddings for Meta-Learning](https://arxiv.org/abs/2603.19888)
*Antonis Klironomos,Ioannis Dasoulas,Francesco Periti,Mohamed Gad-Elrab,Heiko Paulheim,Anastasia Dimou,Evgeny Kharlamov*

Main category: cs.LG

TL;DR: KGmetaSP leverages knowledge-graph embeddings to improve pipeline performance estimation and dataset similarity estimation by capturing dataset-pipeline interactions.


<details>
  <summary>Details</summary>
Motivation: Existing meta-learning approaches often overlook past experimental results and pipeline metadata, limiting their ability to capture performance similarity patterns. KGmetaSP addresses this gap.

Method: The approach represents datasets and pipelines in a unified knowledge graph (KG) and derives embeddings for pipeline-agnostic meta-models (PPE) and distance-based retrieval (DPSE).

Result: KGmetaSP achieves accurate PPE with a single meta-model and outperforms baselines in DPSE, validated on a large-scale benchmark of 144,177 OpenML experiments.

Conclusion: KGmetaSP and the released benchmark provide a new reference for meta-learning, showcasing how unified KGs can advance the field.
...
### [259] [Memori: A Persistent Memory Layer for Efficient, Context-Aware LLM Agents](https://arxiv.org/abs/2603.19935)
*Luiz C. Borro,Luiz A. B. Macarini,Gordon Tindall,Michael Montero,Adam B. Struck*

Main category: cs.LG

TL;DR: Memori is an LLM-agnostic persistent memory layer that structures memory as semantic triples and summaries, reducing token costs and improving performance.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems for LLMs cause vendor lock-in, high token costs, and degraded performance due to excessive raw conversation injection.

Method: Memori uses an Advanced Augmentation pipeline to convert unstructured dialogue into compact semantic triples and summaries for precise retrieval and reasoning.

Result: Memori achieves 81.95% accuracy on LoCoMo, using only 1,294 tokens per query (~5% of full context) and reducing tokens by 67% compared to competitors.

Conclusion: Structured memory representations outperform larger context windows, enabling scalable and cost-efficient LLM agent deployment.
...
### [260] [Graph2TS: Structure-Controlled Time Series Generation via Quantile-Graph VAEs](https://arxiv.org/abs/2603.19970)
*Shaoshuai Du,Joze M. Rozanec,Andy Pimentel,Ana-Lucia Varbanescu*

Main category: cs.LG

TL;DR: The paper proposes Graph2TS, a quantile-graph conditioned VAE for time-series generation, separating global structure from local stochastic variations to improve fidelity and alignment.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of preserving global temporal structure while modeling stochastic local variations in highly volatile, weakly periodic time-series data.

Method: Introduces a quantile-based transition graph to represent global structure, then uses a VAE (Graph2TS) to generate time series conditioned on this graph.

Result: Outperforms diffusion- and GAN-based baselines in distributional fidelity, temporal alignment, and representativeness across diverse datasets.

Conclusion: Structure-controlled, cross-modal generation via Graph2TS is a promising approach for time-series modeling.
...
### [261] [Model-Driven Learning-Based Physical Layer Authentication for Mobile Wi-Fi Devices](https://arxiv.org/abs/2603.19972)
*Yijia Guo,Junqing Zhang,Yao-Win Peter Hong,Stefano Tomasin*

Main category: cs.LG

TL;DR: The paper proposes a learning-based PLA scheme, LiteNP-Net, combining hypothesis testing and deep learning to address authentication risks in IoT, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: IoT faces authentication risks due to wireless broadcast nature. PLA using hypothesis testing is optimal but impractical, while deep learning lacks optimality. A hybrid solution is needed.

Method: Proposed LiteNP-Net, a lightweight neural network, integrating conditional statistical models into hypothesis testing for an optimal NP detector, validated via simulations and Wi-Fi experiments.

Result: Simulations showed LiteNP-Net nears NP detector performance without channel stats. Real-world Wi-Fi tests outperformed correlation and Siamese-based methods.

Conclusion: LiteNP-Net bridges the gap between theoretical optimality and practicality in PLA, offering a robust solution for IoT authentication.
...
### [262] [Breaking the Capability Ceiling of LLM Post-Training by Reintroducing Markov States](https://arxiv.org/abs/2603.19987)
*Yurun Yuan,Tengyang Xie*

Main category: cs.LG

TL;DR: The paper identifies a bottleneck in RL for LLMs due to reliance on history-based states and proposes using explicit Markov states to improve performance and sample efficiency.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for LLMs often fail to discover novel strategies, as they rely on expanding histories rather than compact Markov states, limiting their potential for open-ended discovery.

Method: The authors reintroduce explicit Markov states, a classical RL principle, into LLM post-training. They provide theoretical guarantees and empirical validation using complex logic puzzles.

Result: Introducing Markov states consistently outperforms standard RL post-training, breaking performance ceilings and enabling more efficient learning.

Conclusion: Structured Markovian representations are crucial for advancing RL in LLMs, moving beyond history-based states to unlock new reasoning capabilities in Generative AI.
...
### [263] [A Super Fast K-means for Indexing Vector Embeddings](https://arxiv.org/abs/2603.20009)
*Leonardo Kuffo,Sven Hepkema,Peter Boncz*

Main category: cs.LG

TL;DR: SuperKMeans is a faster k-means variant for high-dimensional vector embeddings, outperforming FAISS, Scikit-Learn, and cuVS while maintaining centroid quality. It uses efficient pruning and early termination to reduce runtime.


<details>
  <summary>Details</summary>
Motivation: The need for a faster and efficient clustering method for high-dimensional vector embeddings without compromising quality in similarity search tasks.

Method: SuperKMeans reduces data-access and compute overhead by pruning unnecessary dimensions and introduces Early Termination by Recall to stop iterations when centroid quality no longer improves.

Result: SuperKMeans is up to 7x faster on CPUs and 4x faster on GPUs compared to existing methods, with no loss in retrieval quality.

Conclusion: SuperKMeans offers significant speed improvements for clustering high-dimensional vectors, making it practical for large-scale similarity search tasks.
...
### [264] [AgenticRS-EnsNAS: Ensemble-Decoupled Self-Evolving Architecture Search](https://arxiv.org/abs/2603.20014)
*Yun Chen,Moyu Zhang,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.LG

TL;DR: Introduces Ensemble-Decoupled Architecture Search to reduce NAS validation costs from O(M) to O(1) per candidate, leveraging ensemble theory and actionable design principles.


<details>
  <summary>Details</summary>
Motivation: Addresses the prohibitive computational cost (O(M)) of validating candidate architectures in industrial NAS deployments, where ensembles are standard for robustness.

Method: Proposes a framework using ensemble theory to predict system-level performance from single-learner evaluation, with theoretical guarantees and three solution strategies.

Result: Reduces per-candidate search cost to O(1) while maintaining O(M) deployment cost for validated architectures, with insights into base diversity and accuracy gains.

Conclusion: The framework enables efficient architecture iteration in industrial settings, with rigorous theory and empirical validation planned for future work.
...
### [265] [ODySSeI: An Open-Source End-to-End Framework for Automated Detection, Segmentation, and Severity Estimation of Lesions in Invasive Coronary Angiography Images](https://arxiv.org/abs/2603.20021)
...
TL;DR: ODySSeI is an open-source, end-to-end framework for automated lesion detection, segmentation, and severity estimation in ICA images, enhancing robustness and speed for clinical use.


<details>
  <summary>Details</summary>
Motivation: To address the subjectivity and variability in interpreting ICA images, the authors developed ODySSeI to provide automated, reproducible, and scalable analysis.

Method: ODySSeI integrates deep learning models using a Pyramidal Augmentation Scheme (PAS) for lesion detection and segmentation, and introduces a quantitative LSE technique to estimate lesion severity directly from predicted geometry.

Result: The framework shows strong generalizability, with PAS improving performance (2.5-fold in detection vs. 1-3% in segmentation). LSE achieves high accuracy (±2-3 pixels deviation in MLD). It processes images quickly (seconds on CPU, fraction on GPU).

Conclusion: ODySSeI is a comprehensive, open-source tool for real-time ICA analysis, supporting clinical decision-making with automated, scalable solutions.
...
### [266] [Continual Learning as Shared-Manifold Continuation Under Compatible Shift](https://arxiv.org/abs/2603.20036)
*Henry J. Kobs*

Main category: cs.LG

TL;DR: The paper proposes SPMA-OG, a geometry-preserving method for continual learning that focuses on maintaining a shared latent support through techniques like sparse replay and anchor-geometry preservation, showing improved old-task retention and representation metrics.


<details>
  <summary>Details</summary>
Motivation: The study addresses the limitation of existing continual learning methods that do not explicitly guide the evolution of latent representations, proposing a geometric approach for scenarios where old and new data share the same latent support.

Method: SPMA-OG combines sparse replay, output distillation, relational geometry preservation, local smoothing, and chart-assignment regularization on old anchors to maintain a shared manifold.

Result: SPMA-OG outperforms sparse replay baselines in old-task retention and representation-preservation on CIFAR10 and Tiny-ImageNet, and achieves near-perfect anchor-geometry preservation on a synthetic benchmark while maintaining new-task accuracy.

Conclusion: Geometry-aware anchor regularization is an effective inductive bias for continual learning when preserving a shared latent support is crucial.
...
### [267] [Federated Hyperdimensional Computing for Resource-Constrained Industrial IoT](https://arxiv.org/abs/2603.20037)
*Nikita Zeulin,Olga Galinina,Nageen Himayat,Sergey Andreev*

Main category: cs.LG

TL;DR: Explores hyperdimensional computing (HDC) in federated learning for resource-constrained IIoT, showing efficient and resilient distributed intelligence.


<details>
  <summary>Details</summary>
Motivation: Resource constraints in IIoT devices limit advanced analytics; HDC offers a lightweight solution.

Method: Integrates HDC into federated learning, reducing communication by exchanging prototype representations.

Result: Federated HDC achieves fast convergence and communication efficiency in collaborative IIoT learning.

Conclusion: HDC is a viable lightweight framework for distributed intelligence in resource-constrained IIoT.
...
### [268] [Fine-tuning Timeseries Predictors Using Reinforcement Learning](https://arxiv.org/abs/2603.20063)
*Hugo Cazaux,Ralph Rudd,Hlynur Stefánsson,Sverrir Ólafsson,Eyjólfur Ingi Ásgeirsson*

Main category: cs.LG

TL;DR: The chapter introduces three reinforcement learning algorithms for fine-tuning financial forecasters, showing improved performance and transfer learning benefits.


<details>
  <summary>Details</summary>
Motivation: To enhance financial forecasting models by leveraging reinforcement learning for fine-tuning and demonstrating its effectiveness.

Method: Implementing a plan to backpropagate reinforcement learning loss to supervised learning models and comparing pre- and post-tuning performance.

Result: Performance improved after fine-tuning, with evidence of transfer learning benefits.

Conclusion: Fine-tuning with reinforcement learning effectively enhances financial forecasting models and offers practical insights for practitioners.
...
### [269] [How Out-of-Equilibrium Phase Transitions can Seed Pattern Formation in Trained Diffusion Models](https://arxiv.org/abs/2603.20092)
*Luca Ambrogioni*

Main category: cs.LG

TL;DR: The paper proposes a theoretical framework interpreting diffusion model generation as out-of-equilibrium phase transitions, highlighting a critical regime and its practical implications for control.


<details>
  <summary>Details</summary>
Motivation: To understand the generation process in trained diffusion models through the lens of non-equilibrium phase transitions and critical phenomena.

Method: Uses analytical tractable patch score models and connects dynamics to Ginzburg-Landau field theories, with empirical validation on trained convolutional diffusion models.

Result: Reveals critical regime signatures like mode softening and growing correlation lengths, and shows practical relevance via improved generation control.

Conclusion: Non-equilibrium critical phenomena serve as a unifying principle for understanding and enhancing the behavior of diffusion models.
...
### [270] [Spectral Alignment in Forward-Backward Representations via Temporal Abstraction](https://arxiv.org/abs/2603.20103)
*Seyed Mahdi B. Azad,Jasper Hoffmann,Iman Nematollahi,Hao Zhu,Abhinav Valada,Joschka Boedecker*

Main category: cs.LG

TL;DR: The paper explores how temporal abstraction can address the spectral mismatch in forward-backward (FB) representations for learning successor representations (SR) in continuous spaces, improving stability and accuracy in long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the challenge of accurately learning low-rank FB representations in continuous environments where high-rank transition dynamics complicate the process.

Method: The method involves analyzing temporal abstraction as a low-pass filter to suppress high-frequency spectral components, thereby reducing the effective rank of the SR while maintaining value function accuracy.

Result: Empirical results demonstrate that temporal abstraction stabilizes FB learning, especially at high discount factors, by aligning the spectral properties of the environment with the FB architecture.

Conclusion: Temporal abstraction is identified as a principled way to shape the spectral structure of MDPs, enabling effective long-horizon representations in continuous control tasks.
...
### [271] [The $\mathbf{Y}$-Combinator for LLMs: Solving Long-Context Rot with $λ$-Calculus](https://arxiv.org/abs/2603.20105)
*Amartya Roy,Rasul Tutunov,Xiaotong Ji,Matthieu Zimmer,Haitham Bou-Ammar*

Main category: cs.LG

TL;DR: Introducing λ-RLM, a framework for long-context reasoning that replaces open-ended recursive code generation with a typed functional runtime, ensuring verifiable execution and formal guarantees.


<details>
  <summary>Details</summary>
Motivation: Current Recursive Language Models (RLMs) rely on an unpredictable read-eval-print loop (REPL) for recursion, making execution hard to verify and analyze. λ-RLM aims to address these limitations.

Method: λ-RLM uses a typed functional runtime grounded in λ-calculus, executing pre-verified combinators and applying neural inference only to bounded leaf subproblems, structuring recursion as a functional program.

Result: Empirical results show λ-RLM outperforms standard RLMs in 29 of 36 comparisons, improves accuracy by up to +21.9 points, and reduces latency by up to 4.1x across tasks and models.

Conclusion: Typed symbolic control in λ-RLM provides a more reliable and efficient foundation for long-context reasoning compared to open-ended recursive code generation.
...
TL;DR: The paper addresses the security risk of trojan horse attacks in deep forecasting models, focusing on a competition to identify hidden triggers in spacecraft telemetry models.


<details>
  <summary>Details</summary>
Motivation: The increasing use of deep forecasting models in safety-critical applications like space operations introduces security risks, particularly trojan horse attacks that manipulate predictions via hidden triggers.

Method: The challenge involved over 200 teams identifying hidden triggers in deep forecasting models for spacecraft telemetry, using a novel task formulation, benchmark set, and evaluation protocol.

Result: The competition yielded best solutions and insights for identifying triggers in time series forecasting models, highlighting effective detection strategies.

Conclusion: The paper emphasizes the importance of addressing trojan horse attacks in forecasting models and provides resources and research directions for improving trigger identification in time series data.
...
### [273] [GO-GenZip: Goal-Oriented Generative Sampling and Hybrid Compression](https://arxiv.org/abs/2603.20109)
*Pietro Talli,Qi Liao,Alessandro Lieto,Parijat Bhattacharjee,Federico Chiariotti,Andrea Zanella*

Main category: cs.LG

TL;DR: The paper introduces a GenAI-driven framework for network telemetry, optimizing sampling and compression to reduce data costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Current network telemetry pipelines face sustainability issues due to massive KPI streams. The work aims to improve efficiency by focusing on goal-oriented data relevance.

Method: The framework combines adaptive sampling with generative modeling and hybrid compression (lossless + GenAI-driven lossy) to selectively capture and encode data.

Result: Tests show over 50% reduction in sampling and transfer costs, with maintained reconstruction accuracy and task fidelity.

Conclusion: The proposed framework offers a scalable solution for sustainable network telemetry by prioritizing relevant data and efficient encoding.
...
### [274] [Var-JEPA: A Variational Formulation of the Joint-Embedding Predictive Architecture -- Bridging Predictive and Generative Self-Supervised Learning](https://arxiv.org/abs/2603.20111)
*Moritz Gögl,Christopher Yau*

Main category: cs.LG

TL;DR: The paper reinterprets the Joint-Embedding Predictive Architecture (JEPA) as a deterministic specialization of variational inference, introduces Variational JEPA (Var-JEPA) with explicit latent generative structure, and demonstrates strong performance on tabular data.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between JEPA and probabilistic generative models by showing that JEPA's design aligns with variational inference principles, and to develop a more principled variant (Var-JEPA) that avoids ad-hoc regularization.

Method: Develops Var-JEPA by optimizing an Evidence Lower Bound (ELBO) to make latent generative structure explicit, applying it to tabular data (Var-T-JEPA).

Result: Var-T-JEPA improves representation learning and downstream performance over T-JEPA, remaining competitive with raw-feature baselines.

Conclusion: Var-JEPA provides a principled, uncertainty-aware alternative to standard JEPA, demonstrating the value of explicit latent generative models in self-supervised learning.
...
### [275] [Conditioning Protein Generation via Hopfield Pattern Multiplicity](https://arxiv.org/abs/2603.20115)
*Jeffrey D. Varner*

Main category: cs.LG

TL;DR: The paper introduces a scalable method to bias protein sequence generation toward functional subsets using a single parameter, without retraining or architectural changes, and identifies a calibration gap between intended and decoded sequences.


<details>
  <summary>Details</summary>
Motivation: To address the limitation of existing protein sequence generation methods that treat all stored sequences equally and lack the ability to direct generation toward specific functional subsets.

Method: A single scalar parameter is added as a bias to the sampler's attention logits, allowing continuous shifting of generation from the full family to a user-specified subset.

Result: Conditioning is exact at the sampler's internal level, but decoded sequence phenotypes may fall short due to a dimensionality reduction issue, termed the calibration gap. Experiments on five Pfam families confirmed the relationship between encoding separation and gap.

Conclusion: The method effectively expands small sets of experimentally characterized sequences into diverse libraries, demonstrating practical utility in functional protein design.
...
### [276] [Revisiting Gene Ontology Knowledge Discovery with Hierarchical Feature Selection and Virtual Study Group of AI Agents](https://arxiv.org/abs/2603.20132)
*Cen Wan,Alex A. Freitas*

Main category: cs.LG

TL;DR: A novel agentic AI-based virtual study group is proposed to extract ageing-related biological knowledge using hierarchical feature selection, validated with existing literature.


<details>
  <summary>Details</summary>
Motivation: To leverage agentic AI techniques for enhancing knowledge discovery in ageing-related biology.

Method: Proposes a virtual study group using hierarchical feature selection on ageing-related Gene Ontology terms across four model organisms, validated with existing research.

Result: Most AI-generated claims align with existing literature, and the virtual study group's internal mechanisms are effective.

Conclusion: Agentic AI can successfully aid in ageing-related biological knowledge discovery, supported by existing research and internal framework mechanisms.
...
### [277] [Beyond Single Tokens: Distilling Discrete Diffusion Models via Discrete MMD](https://arxiv.org/abs/2603.20155)
*Emiel Hoogeboom,David Ruhe,Jonathan Heek,Thomas Mensink,Tim Salimans*

Main category: cs.LG

TL;DR: A method called D-MMD is introduced to distill discrete diffusion models effectively, maintaining quality and diversity, and even outperforming the original models.


<details>
  <summary>Details</summary>
Motivation: The difficulty in distilling discrete diffusion models compared to continuous ones inspired the development of D-MMD to bridge this gap.

Method: D-MMD adapts successful continuous diffusion distillation techniques to discrete models, avoiding the collapse seen in previous methods.

Result: D-MMD preserves high quality and diversity in generated samples and can outperform the original models in some cases.

Conclusion: D-MMD successfully addresses the challenge of distilling discrete diffusion models, offering a robust solution with demonstrated effectiveness.
...
### [278] [Kolmogorov-Arnold causal generative models](https://arxiv.org/abs/2603.20184)
*Alejandro Almodóvar,Mar Elizo,Patricia A. Apellániz,Santiago Zazo,Juan Parras*

Main category: cs.LG

TL;DR: KaCGM is a transparent causal generative model for tabular data using Kolmogorov-Arnold Networks, balancing expressiveness with auditability.


<details>
  <summary>Details</summary>
Motivation: To address the lack of auditability in deep causal models by proposing a transparent alternative for high-stakes domains.

Method: Uses Kolmogorov-Arnold Networks (KANs) to parameterize structural equations, enabling inspectable mechanisms and symbolic approximations.

Result: Competitive performance on benchmarks and real-world case studies, with interpretable causal effects and simplified equations.

Conclusion: KaCGM achieves both expressiveness and transparency, supporting trustworthy deployment in tabular decision-making.
...
### [279] [When both Grounding and not Grounding are Bad -- A Partially Grounded Encoding of Planning into SAT (Extended Version)](https://arxiv.org/abs/2603.19429)
*João Filipe,Gregor Behnke*

Main category: cs.AI

TL;DR: A middle-ground approach in planning introduces three SAT encodings that partially ground predicates while keeping actions lifted, improving scalability and outperforming state-of-the-art methods in length-optimal planning.


<details>
  <summary>Details</summary>
Motivation: To address the exponential size blowup from full grounding in classical planning and the limitations of fully lifted approaches, a hybrid method is proposed.

Method: Developed three SAT encodings that partially ground predicates while keeping actions lifted, ensuring linear scalability with plan length.

Result: The best encoding outperforms existing methods in length-optimal planning, particularly in hard-to-ground domains.

Conclusion: The hybrid approach balances lifted and grounded representations, offering scalable and efficient solutions for classical planning problems.
...
### [280] [Hyperagents](https://arxiv.org/abs/2603.19461)
*Jenny Zhang,Bingchen Zhao,Wannan Yang,Jakob Foerster,Jeff Clune,Minqi Jiang,Sam Devlin,Tatiana Shavrina*

Main category: cs.AI

TL;DR: The paper introduces hyperagents, self-referential AI systems that enable open-ended self-improvement by integrating task-solving and meta-level modification into a single editable program, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing self-improving AI systems rely on fixed meta-level mechanisms, limiting their improvement speed. The paper aims to overcome this by enabling metacognitive self-modification.

Method: The authors extend the Darwin Gödel Machine (DGM) to create DGM-Hyperagents (DGM-H), where meta-level modification is editable, allowing both task-solving and improvement mechanisms to evolve.

Result: DGM-H outperforms baselines and prior self-improving systems, with meta-level improvements transferring across domains and accumulating over time.

Conclusion: DGM-Hyperagents demonstrate open-ended self-improvement, enhancing not just solutions but also the process of searching for improvements, paving the way for more adaptable AI systems.
...
### [281] [A Subgoal-driven Framework for Improving Long-Horizon LLM Agents](https://arxiv.org/abs/2603.19685)
*Taiyi Wang,Sian Gooding,Florian Hartmann,Oriana Riva,Edward Grefenstette*

Main category: cs.AI

TL;DR: The paper addresses challenges in LLM-based agents for web navigation by proposing a framework with subgoal decomposition and an RL training method using milestone-based rewards, achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agents struggle with long-horizon planning due to losing track during execution and sparse rewards during RL training, limiting their effectiveness in dynamic environments like web navigation.

Method: The authors introduce an agent framework for online planning via subgoal decomposition and MiRA, an RL training framework with dense, milestone-based reward signals.

Result: The proposed methods improve proprietary models (e.g., Gemini) by ~10% in success rate and boost the open Gemma3-12B model from 6.4% to 43.0%, outperforming GPT-4 variants and WebRL.

Conclusion: Combining explicit planning with milestone-based rewards enhances long-horizon capabilities, advancing robust autonomous systems.
...
### [282] [Teaching an Agent to Sketch One Part at a Time](https://arxiv.org/abs/2603.19500)
*Xiaodan Du,Ruize Xu,David Yunis,Yael Vinker,Greg Shakhnarovich*

Main category: cs.AI

TL;DR: A method for generating vector sketches part-by-part using a multi-modal language model trained with process-reward reinforcement learning and a novel dataset, ControlSketch-Part.


<details>
  <summary>Details</summary>
Motivation: To enable interpretable, controllable, and locally editable text-to-vector sketch generation by leveraging structured part-level data and visual feedback.

Method: Train a multi-modal language model-based agent using multi-turn process-reward reinforcement learning after supervised fine-tuning, supported by the ControlSketch-Part dataset with automatic part-level annotations.

Result: The approach allows for interpretable, controllable, and locally editable sketch generation, validated by the dataset's rich part-level annotations.

Conclusion: Structured part-level data and visual feedback enhance the quality and control of text-to-vector sketch generation.
...
### [283] [Learning to Disprove: Formal Counterexample Generation with Large Language Models](https://arxiv.org/abs/2603.19514)
*Zenan Li,Zhaoyu Li,Kaiyu Yang,Xiaoxing Ma,Zhendong Su*

Main category: cs.AI

TL;DR: The paper addresses the gap in AI's focus on proof construction by fine-tuning LLMs to generate and verify counterexamples, using a symbolic mutation strategy and expert iteration framework for improved performance.


<details>
  <summary>Details</summary>
Motivation: Current AI efforts in mathematics predominantly focus on proof construction, neglecting the equally important task of discovering counterexamples. This paper aims to fill this gap by enhancing LLMs' ability to reason about and generate counterexamples.

Method: The authors formalize counterexample generation, requiring LLMs to propose candidates and provide formal proofs verifiable in Lean 4. They introduce a symbolic mutation strategy for diverse training data and a multi-reward expert iteration framework for effective training.

Result: Experiments on three benchmarks demonstrate that the proposed mutation strategy and training framework significantly improve the performance of LLMs in counterexample generation and theorem proving.

Conclusion: The study successfully bridges the gap in AI's mathematical reasoning by enabling LLMs to generate and verify counterexamples, showing notable performance gains through innovative training strategies.
...
### [284] [ItinBench: Benchmarking Planning Across Multiple Cognitive Dimensions with Large Language Models](https://arxiv.org/abs/2603.19515)
*Tianlong Wang,Pinqiao Wang,Weili Shi,Sheng li*

Main category: cs.AI

TL;DR: ItinBench is a benchmark introducing spatial reasoning (route optimization) and verbal reasoning tasks to evaluate LLMs like Llama 3.1 8B, Mistral Large, and GPT family, revealing their struggles with multi-cognitive tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional evaluations of LLMs focus on controlled reasoning tasks, but real-world challenges require integrating multiple cognitive domains. ItinBench aims to address this gap by combining spatial and verbal reasoning in travel planning.

Method: ItinBench evaluates LLMs by incorporating route optimization (spatial reasoning) alongside traditional verbal reasoning tasks in trip itinerary planning. It tests models like Llama 3.1 8B, Mistral Large, and GPT family.

Result: LLMs struggle to maintain consistent performance when handling multiple cognitive tasks simultaneously, highlighting limitations in real-world cognitive integration.

Conclusion: ItinBench provides a comprehensive testbed for evaluating LLMs across diverse cognitive domains, offering insights into building more realistic and challenging reasoning benchmarks.
...
### [285] [PA2D-MORL: Pareto Ascent Directional Decomposition based Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.19579)
*Tianmeng Hu,Biao Luo*

Main category: cs.AI

TL;DR: PA2D-MORL method improves multi-objective reinforcement learning by decomposing tasks and optimizing policies for better Pareto frontier approximation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of approximating high-quality Pareto policy sets in complex, continuous, or high-dimensional MORL tasks.

Method: PA2D-MORL uses Pareto ascent direction for scalarization weights and multi-objective policy gradients, with evolutionary optimization and fine-tuning for Pareto frontier approximation.

Result: Outperforms state-of-the-art algorithms in quality and stability on multi-objective robot control tasks.

Conclusion: PA2D-MORL effectively improves the approximation of Pareto policy sets and enhances performance in complex MORL tasks.
...
### [286] [PowerLens: Taming LLM Agents for Safe and Personalized Mobile Power Management](https://arxiv.org/abs/2603.19584)
*Xingyu Feng,Chang Sun,Yuzhu Wang,Zhangbing Zhou,Chengwen Luo,Zhuangzhuang Chen,Xiaomin Ouyang,Huanqi Yang*

Main category: cs.AI

TL;DR: PowerLens uses LLMs for personalized mobile power management on Android, achieving high accuracy, energy savings, and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: Existing power management systems lack personalization and ignore user activities, leading to inefficient battery use.

Method: PowerLens leverages LLMs for context-aware policy generation, a multi-agent architecture, PDL-based constraints, and a two-tier memory system for implicit preference learning.

Result: Achieves 81.7% action accuracy, 38.8% energy savings over stock Android, with minimal battery consumption (0.5% daily).

Conclusion: PowerLens demonstrates effective, personalized power management with strong safety and user satisfaction.
...
### [287] [HyEvo: Self-Evolving Hybrid Agentic Workflows for Efficient Reasoning](https://arxiv.org/abs/2603.19639)
*Beibei Xu,Yutong Ye,Chuyun Shen,Yingbo Zhou,Cheng Chen,Mingsong Chen*

Main category: cs.AI

TL;DR: HyEvo is a framework automating workflow generation by combining LLM nodes for semantic reasoning and code nodes for rule-based execution, reducing costs and latency compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing automated workflow-generation methods are inefficient due to reliance on predefined operator libraries and homogeneous LLM workflows, limiting performance and flexibility.

Method: HyEvo uses heterogeneous atomic synthesis, integrating probabilistic LLM nodes with deterministic code nodes. It employs an LLM-driven multi-island evolutionary strategy to refine workflows iteratively.

Result: HyEvo outperforms existing methods in reasoning and coding benchmarks, reducing inference cost by up to 19x and execution latency by up to 16x.

Conclusion: HyEvo addresses inefficiencies in automated workflow generation by leveraging hybrid computation and evolutionary optimization, delivering better performance and cost efficiency.
...
### [288] [Stepwise: Neuro-Symbolic Proof Search for Automated Systems Verification](https://arxiv.org/abs/2603.19715)
*Baoding He,Zenan Li,Wei Sun,Yuan Yao,Taolue Chen,Xiaoxing Ma,Zhendong Su*

Main category: cs.AI

TL;DR: A neuro-symbolic proof generation framework leverages LLMs and ITP tools to automate proof search for software verification, achieving high success rates on benchmarks like seL4.


<details>
  <summary>Details</summary>
Motivation: Manual proof construction in interactive theorem proving limits scalability, and advances in LLMs offer a promising path for automation in software verification.

Method: The framework combines neural (fine-tuned LLMs for proof-step generation) and symbolic (ITP tools for step repair, state filtering, and subgoal discharge) approaches in a best-first tree search over proof states.

Result: The system proves 77.6% of theorems in the seL4 benchmark, outperforming prior LLM-based methods and Sledgehammer, while handling multi-step proofs effectively.

Conclusion: The neuro-symbolic approach demonstrates strong generalization, offering a scalable solution for automated software verification.
...
### [289] [Embodied Science: Closing the Discovery Loop with Agentic Embodied AI](https://arxiv.org/abs/2603.19782)
...
TL;DR: The paper advocates for 'embodied science,' a paradigm where AI agents interact continuously with the physical world for scientific discovery, proposing the PLAD framework.


<details>
  <summary>Details</summary>
Motivation: Current AI approaches are misaligned with the iterative, physical nature of scientific discovery, focusing on isolated predictions rather than real-world interaction.

Method: The authors introduce the Perception-Language-Action-Discovery (PLAD) framework, where agents perceive environments, reason scientifically, execute actions, and learn from feedback.

Result: This embodied approach bridges digital prediction and empirical validation, enabling autonomous discovery in life and chemical sciences.

Conclusion: Embodied science offers a promising roadmap for integrating AI reasoning with physical experimentation to advance scientific discovery.
...
### [290] [FormalEvolve: Neuro-Symbolic Evolutionary Search for Diverse and Prover-Effective Autoformalization](https://arxiv.org/abs/2603.19828)
*Haijian Lu,Wei Wang,Jing Liu*

Main category: cs.AI

TL;DR: FormalEvolve, a neuro-symbolic framework, enhances autoformalization by generating diverse, semantically consistent candidates via LLM-driven evolution and AST rewrites, improving prover effectiveness.


<details>
  <summary>Details</summary>
Motivation: Addressing the gap between semantic consistency and prover effectiveness in autoformalization by ensuring generated formalizations are both correct and efficient for proof-search.

Method: FormalEvolve uses LLM-driven mutation, crossover with bounded patch repair, and symbolic AST rewrite operations to create diverse, semantically consistent repertoires.

Result: Achieves semantic hit rates of 58.0% and 84.9% on CombiBench and ProofNet, reduces Gini concentration, and improves downstream proving performance under a fixed prover budget.

Conclusion: FormalEvolve effectively bridges the gap between semantic consistency and prover efficiency, demonstrating practical improvements in autoformalization tasks.
...
### [291] [Utility-Guided Agent Orchestration for Efficient LLM Tool Use](https://arxiv.org/abs/2603.19896)
*Boyan Liu,Gongming Zhao,Hongli Xu*

Main category: cs.AI

TL;DR: The paper proposes a utility-guided orchestration policy for LLM agents to balance answer quality and execution cost, offering a controllable framework for studying trade-offs.


<details>
  <summary>Details</summary>
Motivation: Addresses the tension between answer quality and execution cost in tool-using LLM agents, aiming for a more flexible and analyzable approach.

Method: Introduces a utility-guided policy that selects actions (respond, retrieve, tool call, verify, stop) based on gain, cost, uncertainty, and redundancy.

Result: Experiments show orchestration signals significantly impact agent behavior, with lightweight utility design proving practical for control.

Conclusion: The framework provides a defensible and practical mechanism for managing quality-cost trade-offs in LLM agents.
...
### [292] [On the Ability of Transformers to Verify Plans](https://arxiv.org/abs/2603.19954)
*Yash Sarrof,Yupei Du,Katharina Stein,Alexander Koller,Sylvie Thiébaux,Michael Hahn*

Main category: cs.AI

TL;DR: Transformers' inconsistent success in AI planning tasks is addressed by analyzing their ability to verify plans, introducing C*-RASP for length generalization guarantees, and identifying learnable planning domains.


<details>
  <summary>Details</summary>
Motivation: To understand when and how transformers can generalize in AI planning tasks, particularly for plan verification.

Method: Introduces C*-RASP, an extension of C-RASP, to analyze transformer generalization under growing sequence length and vocabulary size.

Result: Identifies a class of planning domains where transformers can provably learn to verify long plans, supported by empirical evidence.

Conclusion: The study provides theoretical and empirical insights into transformer generalization in planning tasks, highlighting structural properties affecting learnability.
...
### [293] [Experience is the Best Teacher: Motivating Effective Exploration in Reinforcement Learning for LLMs](https://arxiv.org/abs/2603.20046)
*Wenjian Zhang,Kongcheng Zhang,Jiaxin Qi,Baisheng Lai,Jianqiang Huang*

Main category: cs.AI

TL;DR: The paper introduces HeRL, a Hindsight experience guided Reinforcement Learning framework, to enhance LLM reasoning by using failed trajectories and unmet rubrics as guidance for effective exploration, achieving superior performance over baselines.


<details>
  <summary>Details</summary>
Motivation: Current RL with rubric-based rewards for LLMs suffers from ineffective exploration limited to the current policy distribution. The paper aims to align exploration efforts with desired target behaviors to improve learning.

Method: HeRL leverages hindsight experience by using failed trajectories and unmet rubrics as in-context guidance. It also introduces a bonus reward to incentivize responses with potential for improvement under such guidance.

Result: HeRL demonstrates superior performance gains across various benchmarks and benefits from experience guided self-improvement at test time.

Conclusion: HeRL effectively bootstraps exploration in RL for LLMs by leveraging hindsight experience, leading to more accurate gradient estimation and improved performance without repetitive trial-and-error.
...
### [294] [DIAL-KG: Schema-Free Incremental Knowledge Graph Construction via Dynamic Schema Induction and Evolution-Intent Assessment](https://arxiv.org/abs/2603.20059)
*Weidong Bao,Yilin Wang,Ruyu Gao,Fangling Leng,Yubin Bao,Ge Yu*

Main category: cs.AI

TL;DR: DIAL KG is a closed-loop framework for incremental knowledge graph construction, addressing limitations of static methods by dynamically updating the graph and schemas in three stages: Dual-Track Extraction, Governance Adjudication, and Schema Evolution. It achieves SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Traditional knowledge graph construction methods are static and inflexible, struggling with dynamic data and predefined schemas. DIAL KG aims to overcome these issues with incremental updates and adaptive schemas.

Method: The framework uses a Meta-Knowledge Base (MKB) and operates in three stages: 1) Dual-Track Extraction for comprehensive knowledge capture, 2) Governance Adjudication to ensure fact quality, and 3) Schema Evolution for adaptive schema updates.

Result: Extensive experiments show DIAL KG achieves state-of-the-art performance in graph quality and schema induction.

Conclusion: DIAL KG provides a scalable, dynamic solution for knowledge graph construction, outperforming traditional static methods.
...
### [295] [Pitfalls in Evaluating Interpretability Agents](https://arxiv.org/abs/2603.20101)
*Tal Haklay,Nikhil Prakash,Sana Pandey,Antonio Torralba,Aaron Mueller,Jacob Andreas,Tamar Rott Shaham,Yonatan Belinkov*

Main category: cs.AI

TL;DR: The paper discusses the challenges in evaluating automated interpretability systems for AI models, highlighting pitfalls of replication-based methods and proposing an unsupervised intrinsic evaluation approach.


<details>
  <summary>Details</summary>
Motivation: To address the growing complexity and volume of explanations generated by automated interpretability systems, particularly in circuit analysis, and to improve evaluation methods.

Method: The study develops an agentic system where a research agent designs experiments and refines hypotheses, comparing its explanations to human expert ones. It also introduces an unsupervised intrinsic evaluation based on functional interchangeability.

Result: The system appears competitive with human experts in circuit analysis tasks, but replication-based evaluation shows limitations like subjectivity and memorization.

Conclusion: The work underscores challenges in evaluating automated interpretability systems and critiques replication-based evaluation, advocating for intrinsic methods.
...
### [296] [Learning Dynamic Belief Graphs for Theory-of-mind Reasoning](https://arxiv.org/abs/2603.20170)
*Ruxiao Chen,Xilei Zhao,Thomas J. Cova,Frank A. Drews,Susu Xu*

Main category: cs.AI

TL;DR: The paper introduces a structured cognitive trajectory model for Theory of Mind (ToM) reasoning in LLMs, improving action prediction and belief tracking in high-stakes, uncertain environments.


<details>
  <summary>Details</summary>
Motivation: Existing methods for ToM reasoning in LLMs either use direct prompting or static latent-state models, leading to incoherent mental models and weak reasoning in dynamic contexts. The paper aims to address these limitations.

Method: The proposed model represents mental states as dynamic belief graphs, inferring latent beliefs, learning their time-varying dependencies, and linking belief evolution to decisions. It includes a projection to probabilistic graphical models, an energy-based factor graph, and an ELBO-based objective.

Result: The model significantly improves action prediction and recovers interpretable belief trajectories in real-world disaster evacuation datasets, aligning with human reasoning.

Conclusion: The structured cognitive trajectory model enhances LLM-based ToM reasoning in high-uncertainty environments, offering a principled approach for dynamic belief tracking and decision-making.
