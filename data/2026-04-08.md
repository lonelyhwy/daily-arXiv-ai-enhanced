...
TL;DR: The paper surveys the shift from heuristic-based to generative AI methods for automatic video trailer generation, focusing on LLMs, MLLMs, and diffusion models, and discusses future trends and ethical challenges.


<details>
  <summary>Details</summary>
Motivation: To review the evolution of video trailer generation techniques, highlighting the transition from traditional heuristic methods to advanced generative AI, and to address the economic and ethical implications.

Method: The survey analyzes architectural advancements like GCNs and TGTs, evaluates generative techniques (Transformers, LLM pipelines, text-to-video models), and synthesizes recent literature to propose a new taxonomy.

Result: Identifies a paradigm shift toward generative editing and semantic reconstruction in trailer generation, driven by foundation models like Sora and Veo.

Conclusion: Future trailer generation will likely prioritize controllable generative methods over extractive selection, with significant economic and ethical considerations.
...
TL;DR: RCP is a framework combining cumulative visual token pruning with a delayed repair mechanism to reduce inference costs in LVLMs while minimizing performance degradation.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models face high inference costs due to processing many visual tokens; existing pruning methods degrade performance by shifting hidden state distributions.

Method: RCP uses a cross-attention pruner for consistent token reduction and a delayed repair adapter (DRA) with FiLM-based modulation to compensate for pruned token loss, trained with a repair loss for representation consistency.

Result: RCP removes up to 88.9% of visual tokens and reduces FLOPs by 85.7% with minimal accuracy drop, outperforming prior fine-tuning-free methods on benchmarks.

Conclusion: RCP efficiently reduces computation and maintains performance by integrating pruning and repair mechanisms, offering a lightweight and effective solution for LVLMs.
...
TL;DR: Video-MME-v2 is a rigorous benchmark for evaluating video understanding, featuring a tri-level hierarchy and non-linear evaluation to assess robustness and reasoning, revealing gaps between models and human performance.


<details>
  <summary>Details</summary>
Motivation: To address the discrepancy between inflated benchmark scores and actual model capabilities in video understanding.

Method: Introduces a progressive tri-level hierarchy for complexity and a group-based non-linear evaluation strategy, supported by extensive human annotation and quality assurance.

Result: Identifies significant gaps between models like Gemini-3-Pro and humans, highlighting hierarchical bottlenecks and dependency on textual cues.

Conclusion: Video-MME-v2 sets a challenging standard for future video MLLMs by exposing current limitations and emphasizing reasoning coherence.
...
TL;DR: ID-Sim is a metric designed to match human selective sensitivity to identities, aiding identity-focused tasks like personalized image generation.


<details>
  <summary>Details</summary>
Motivation: Humans excel at distinguishing similar identities across diverse contexts, but vision models lag behind. Current identity-focused tasks lack suitable evaluation metrics.

Method: ID-Sim is a feed-forward metric trained on a curated dataset of real-world images and synthetic data with controlled identity and contextual variations.

Result: The metric is evaluated on a new benchmark, showing consistency with human annotations in identity-focused recognition, retrieval, and generative tasks.

Conclusion: ID-Sim addresses the gap in identity-focused evaluation, facilitating progress in vision models for identity-sensitive tasks.
...
TL;DR: SVAgent, a storyline-guided cross-modal multi-agent framework for VideoQA, improves reasoning by mimicking human-like narrative understanding, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing VideoQA methods focus on locating frames rather than reasoning through evolving storylines, which is how humans understand videos. SVAgent addresses this gap.

Method: SVAgent uses a storyline agent to construct narratives, a refinement suggestion agent to analyze failures, and cross-modal decision agents to predict answers. A meta-agent aligns these predictions.

Result: SVAgent achieves superior performance and interpretability by emulating human-like storyline reasoning.

Conclusion: The proposed framework enhances VideoQA by incorporating storyline-guided reasoning, outperforming traditional methods in robustness and consistency.
...
TL;DR: A three-channel denoising diffusion probabilistic model (DDPM) is proposed to generate simultaneous CC and MLO mammographic views, using a difference-guided approach to enhance cross-view consistency.


<details>
  <summary>Details</summary>
Motivation: Many datasets lack complete paired CC and MLO views, hindering algorithm development reliant on cross-view consistency. This work aims to fill this gap by synthesizing realistic dual-view mammograms.

Method: A pretrained DDPM is fine-tuned on a private dataset, using a three-channel configuration (CC, MLO, and their absolute difference) to guide coherent anatomical relationships. The model generates synthetic dual-view pairs.

Result: The difference-based encoding preserves global breast structure across views, producing synthetic images resembling real acquisitions. Geometric and distributional evaluations confirm consistency.

Conclusion: The study demonstrates the feasibility of simultaneous dual-view mammogram synthesis, opening possibilities for dataset augmentation and AI applications in breast imaging.
...
TL;DR: The paper highlights the limitations in current video understanding benchmarks and post-training datasets for vision-language models (VLMs), proposing VidGround as a simple, effective solution to improve performance by focusing on visually grounded questions.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks and datasets for VLMs often contain linguistic biases, undermining their ability to truly assess or improve visual understanding. The authors aim to address this gap.

Method: They introduce VidGround, a method that curates post-training data to include only visually grounded questions, avoiding linguistic biases. This is paired with RL-based post-training algorithms.

Result: VidGround improves performance by up to 6.2 points using only 69.1% of the original data, outperforming more complex techniques and highlighting the importance of data quality.

Conclusion: Data curation focusing on visual grounding is crucial for advancing VLM capabilities, as it addresses the bottleneck of poor-quality benchmarks and datasets.
...
TL;DR: The paper introduces SecurePix, a CMOS-compatible pixel architecture for in-pixel encryption using ferroelectric field-effect transistors, achieving strong resistance to neural network inference with low overhead.


<details>
  <summary>Details</summary>
Motivation: To address the vulnerability of visual data in the imaging pipeline by ensuring end-to-end security through encryption before pixel values appear on readout lines.

Method: The SecurePix architecture utilizes a symmetric key via programmable ferroelectric field-effect transistors for true in-pixel encryption. It was designed and simulated in HSPICE and laid out using a 45 nm CMOS process.

Result: The layout achieved a pixel pitch of 2.33 x 3.01 µm². Encryption reduced ResNet-18 accuracy from 99.29% to 9.58% on MNIST and from 91.33% to 6.98% on CIFAR-10. Low power consumption was demonstrated with 17 µW µs for programming and 1.25 µW µs for sensing per pixel.

Conclusion: SecurePix provides robust, low-overhead hardware-level security for image sensors, effectively resisting unauthorized inference while enabling recovery for authorized users.
...
TL;DR: A modality-aware VQ-VAE called NeuroQuant is proposed for multi-modal brain MRI reconstruction, outperforming existing VAEs by combining shared latent representations and modality-specific features.


<details>
  <summary>Details</summary>
Motivation: Existing brain VAEs primarily focus on single-modality data (e.g., T1-weighted MRI), neglecting the diagnostic value of other modalities like T2-weighted MRIs.

Method: NeuroQuant uses a shared latent representation with factorized multi-axis attention, a dual-stream 3D encoder for separating modality-invariant anatomy and modality-dependent appearance, discretization via a shared codebook, and FiLM for decoding. Training involves a joint 2D/3D strategy.

Result: NeuroQuant achieves superior reconstruction fidelity on two multi-modal brain MRI datasets compared to existing VAEs.

Conclusion: NeuroQuant provides a scalable foundation for generative modeling and cross-modal brain image analysis.
...
TL;DR: The paper introduces MIRAGE, a training-free framework for precise image editing in complex multi-instance scenarios, and a benchmark to evaluate fine-grained consistency.


<details>
  <summary>Details</summary>
Motivation: Existing models like FLUX.2 and Qwen - Image - Edit struggle with over-editing and misalignment in multi-instance edits. The paper aims to address these limitations.

Method: MIRAGE uses a vision-language model to parse instructions into regional subsets and employs a multi-branch parallel denoising strategy for localized editing while preserving background integrity.

Result: MIRAGE outperforms existing methods in precise instance-level editing and background consistency, as shown in evaluations on MIRA-Bench and RefEdit-Bench.

Conclusion: The proposed framework and benchmark significantly improve performance in complex image editing tasks and are publicly available for further research.
...
TL;DR: The paper introduces the Large Sparse Reconstruction Model (LSRM) to improve 3D reconstruction by scaling transformer context windows, achieving high-fidelity results with sparse attention and key contributions like coarse-to-fine pipelines and 3D-aware routing.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between feed-forward 3D reconstruction methods and dense-view optimization in recovering fine-grained texture and appearance by expanding the context window and leveraging sparse attention.

Method: LSRM uses a coarse-to-fine pipeline, 3D-aware spatial routing, and block-aware sequence parallelism to scale sparse attention for 3D reconstruction, handling more tokens than prior methods.

Result: LSRM outperforms SOTA methods with 2.5 dB higher PSNR and 40% lower LPIPS, matching or exceeding dense-view optimization in inverse rendering tasks.

Conclusion: Scaling transformer context windows with sparse attention and innovative design significantly improves 3D reconstruction and inverse rendering, closing the gap with dense-view methods.
...
TL;DR: The paper proposes a training-free method for merging multiplicative orthogonal adapters using structured orthogonal parametrization and geometric properties, improving generative model performance.


<details>
  <summary>Details</summary>
Motivation: There's a need to efficiently combine adapters tuned for different tasks, especially merging subject and style adapters in generative models, without additional training.

Method: The authors use structured orthogonal parametrization (OFT) and derive formulas for adapter merging, including geodesics approximation and a spectra restoration transform for high-quality fusion.

Result: Experiments show the method successfully combines concept and style features of adapters in subject-driven generation tasks.

Conclusion: This is the first training-free technique for merging orthogonal adapters, offering practical advantages for model adaptation.
...
TL;DR: The paper proposes a detection-guided sVLM framework combining object detection with small vision-language models for efficient and accurate construction hazard identification, improving performance with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Address the trade-off between computational efficiency and accuracy in construction hazard detection by leveraging small vision-language models (sVLMs) and object detection.

Method: Integrates YOLOv11n for object detection with sVLMs (e.g., Gemma-3 4B) for contextual hazard assessment using structured prompts. Evaluated six sVLMs in zero-shot settings on a construction site dataset.

Result: Improved hazard detection (F1-score from 34.5% to 50.6%) and explanation quality (BERTScore F1 from 0.61 to 0.82) with only 2.5 ms overhead per image.

Conclusion: The framework effectively balances efficiency and accuracy for real-time construction hazard detection.
...
TL;DR: BoxerNet is a transformer-based network that lifts 2D object detections to 3D bounding boxes, leveraging existing 2D detection methods and reducing the need for extensive 3D training data.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of 3D object localization, especially for open-world categories, which is less explored than 2D detection.

Method: Uses BoxerNet to transform 2D bounding boxes into 3D, incorporating multi-view fusion, geometric filtering, and aleatoric uncertainty for robust regression.

Result: Outperforms state-of-the-art baselines, achieving higher mAP scores in open-world 3D lifting tasks.

Conclusion: BoxerNet effectively leverages 2D detection algorithms to improve 3D object localization, reducing dependency on costly 3D annotations.
...
TL;DR: A hierarchical transformer framework is introduced for analyzing heterogeneous meshes in neuroimaging, supporting diverse vertex-level features and mesh topologies, achieving state-of-the-art results in disease classification and prediction.


<details>
  <summary>Details</summary>
Motivation: Current methods in neuroimaging either ignore clinically informative mesh features or are limited to a single topology, restricting their applicability across diverse imaging pipelines.

Method: The proposed hierarchical transformer operates on tree partitions from simplicial complexes, integrates feature projection for diverse descriptors, and uses self-supervised pretraining via masked reconstruction.

Result: The framework achieves state-of-the-art performance in Alzheimer's disease classification, amyloid burden prediction, and focal cortical dysplasia detection across different mesh modalities.

Conclusion: The introduced framework effectively handles heterogeneous neuroimaging meshes, enabling robust and transferable analysis across diverse tasks and datasets.
...
TL;DR: The paper introduces a human-in-the-loop framework for efficiently estimating two-point correlation functions (2PCF) of specific subsets in large datasets, reducing annotation effort and variance compared to Monte Carlo methods.


<details>
  <summary>Details</summary>
Motivation: Measuring 2PCF for subsets like star clusters in astronomy datasets is time-consuming due to the need for extensive labeling. The paper aims to streamline this process.

Method: The framework uses a pre-trained classifier to guide sampling, adaptively selecting points for human annotation and providing unbiased estimates of pair counts across distance bins.

Result: The method achieves lower variance and reduces annotation effort significantly compared to Monte Carlo approaches.

Conclusion: The proposed framework, including a novel estimator and sampling strategy, enables scalable and statistically sound 2PCF measurement in astronomy.
...
TL;DR: A responsible computing framework is proposed to analyze protest dynamics using synthetic imagery, balancing privacy risks, utility, and fairness.


<details>
  <summary>Details</summary>
Motivation: Protest-related social media data is valuable but high-risk due to surveillance and privacy concerns. AI systems can exploit this data, posing risks to individuals. Existing methods lack a holistic approach integrating risk assessment, analysis, and fairness.

Method: The framework replaces real protest imagery with synthetic reproductions via conditional image synthesis, enabling analysis without exposing identifiable individuals.

Result: The approach produces realistic, diverse synthetic images, balancing analytical utility with reduced privacy risks. It also assesses demographic fairness in the generated data.

Conclusion: The method mitigates harm by enabling sensitive analysis while acknowledging residual risks, offering a pragmatic solution without absolute privacy guarantees.
...
TL;DR: The paper introduces COVER, a lightweight and interpretable metric for selecting informative camera poses in 3D reconstruction tasks, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The quality of data used for 3D reconstructions depends on selecting informative camera poses. The paper aims to develop a simple and effective criterion for active view selection to improve scene modeling.

Method: The proposed COVER metric minimizes a tractable approximation of the Fisher Information Gain by favoring viewpoints covering insufficiently observed geometry. It avoids transmittance estimation and is robust to noise and training dynamics.

Result: Integrated into Nerfstudio, COVER consistently improves reconstruction quality across multiple datasets and radiance-field baselines in fixed and embodied data acquisition scenarios.

Conclusion: COVER provides an efficient and interpretable solution for active view selection, enhancing 3D reconstruction performance over existing methods.
...
TL;DR: Region-R1, a query-side region cropping framework, improves MM-RAG re-ranking by dynamically focusing on question-relevant image regions, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Standard re-rankers in MM-RAG process full query images globally, making them vulnerable to visual distractors like background clutter, which skew similarity scores.

Method: Proposes Region-R1, a framework that formulates region selection as a decision-making problem during re-ranking. It uses r-GRPO (region-aware group relative policy optimization) to dynamically crop discriminative regions.

Result: Region-R1 achieves state-of-the-art performance on benchmarks E-VQA and InfoSeek, increasing conditional Recall@1 by up to 20%.

Conclusion: Query-side adaptation, as demonstrated by Region-R1, is a simple yet effective way to enhance MM-RAG re-ranking by focusing on relevant image regions.
...
TL;DR: The paper introduces UFPR-VeSV, a dataset for Fine-Grained Vehicle Classification (FGVC) and its integration with ALPR, showcasing real-world challenges in vehicle attribute recognition.


<details>
  <summary>Details</summary>
Motivation: To address gaps in existing studies by providing a dataset that captures diverse real-world conditions and integrates FGVC with ALPR for improved vehicle recognition.

Method: A dataset of 24,945 images with annotations for colors, makes, models, and types was collected and validated. Benchmarking involved five deep learning models and two OCR models for ALPR.

Result: The dataset proved challenging due to diverse conditions. FGVC and ALPR integration showed potential for real-world applications.

Conclusion: UFPR-VeSV fills a critical gap in vehicle recognition research, offering a robust dataset and demonstrating the value of combining FGVC and ALPR.
...
TL;DR: The paper introduces a benchmark for evaluating visual embeddings like CLIP, DINOv2/v3, and SSCD on face data, focusing on identity leakage and privacy risks. It proposes a one-shot linear projector (ISP) to mitigate identity leakage while preserving utility. The results show ISP effectively balances privacy and utility across datasets.


<details>
  <summary>Details</summary>
Motivation: Visual embeddings (e.g., CLIP, DINOv2/v3, SSCD) are widely used in retrieval systems but pose privacy risks with face data due to unmeasured identity leakage. The study aims to address this gap by providing metrics and mitigations for privacy-aware deployment.

Method: The paper benchmarks visual embeddings using open-set verification, diffusion-based template inversion checks, and face-context attribution. It also proposes a one-shot linear projector (ISP) to remove identity subspaces while preserving utility.

Result: CLIP exhibits higher identity leakage than DINOv2/v3 and SSCD. ISP successfully reduces identity leakage to near-chance levels while maintaining high utility for non-biometric tasks, and transfers well across datasets.

Conclusion: The study establishes the first privacy audit for non-face-recognition encoders and demonstrates that linear subspace removal (ISP) offers strong privacy guarantees without compromising utility for visual search and retrieval.
...
TL;DR: The paper introduces SmokeGS-R, a pipeline for 3D reconstruction in smoke-affected scenes, focusing on decoupling geometry recovery from appearance correction using physics-guided methods. It achieves strong performance metrics on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Real-world smoke complicates 3D reconstruction by attenuating radiance, adding airlight, and destabilizing multi-view consistency. The goal is to develop a robust solution for such challenging conditions.

Method: Decouples geometry recovery and appearance correction. Uses physics-guided pseudo-clean supervision, sharp clean-only 3D Gaussian Splatting, and appearance harmonization techniques like geometric-mean reference aggregation and LAB-space Reinhard transfer.

Result: Achieved PSNR=15.217, SSIM=0.666 on the challenge leaderboard. Post-release evaluation showed similar results (PSNR=15.209, SSIM=0.644, LPIPS=0.551), outperforming baselines by +3.68 dB PSNR.

Conclusion: The geometry-first strategy with post-render appearance harmonization is effective for smoke-affected 3D reconstruction, as demonstrated by strong performance and robustness.
...
TL;DR: An approach for object-level detection and segmentation in 3D Gaussian Splatting scenes using a 3D object codebook, improving F1 score by 65% and mAP by 11% over baselines.


<details>
  <summary>Details</summary>
Motivation: To enhance object-level detection and segmentation of indoor assets in 3DGS scenes reconstructed from drone-captured imagery.

Method: Introduces a 3D object codebook combining mask semantics and spatial information to guide multi-view mask association and detection. Integrates 2D models with constrained merging for coherent 3D object instances.

Result: Experiments show reliable multi-view mask consistency, a 65% F1 score improvement, and an 11% mAP gain over baselines.

Conclusion: The method effectively improves object-level detection and segmentation in 3DGS scenes.
...
TL;DR: The paper proposes VLA-InfoEntropy, a method to improve inference efficiency in Vision-Language-Action models by using image and attention entropy to reduce redundancy while preserving critical information.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the computational overhead and low inference efficiency in Vision-Language-Action (VLA) models caused by joint processing of high-dimensional visual features, complex linguistic inputs, and continuous action sequences.

Method: The method uses image entropy to quantify grayscale distribution and attention entropy to capture attention score distribution. Combined with timestep information, it dynamically shifts focus from global to local informative regions.

Result: The method reduces inference parameters, speeds up inference, and outperforms existing approaches in experiments.

Conclusion: VLA-InfoEntropy effectively integrates spatial, semantic, and temporal cues to enhance efficiency and reliability in VLA models.
...
TL;DR: The paper introduces an unsupervised framework (UMS) for multi-agent and single-agent LiDAR perception, leveraging data sharing and cooperative views to improve 3D object detection and classification without human labels.


<details>
  <summary>Details</summary>
Motivation: Current methods lack a unified unsupervised solution for both multi-agent and single-agent perception. The paper aims to fill this gap by exploiting cooperative data sharing for improved performance.

Method: UMS uses a Proposal Purifying Filter for classification, Progressive Proposal Stabilizing for reliable pseudo labels, and Cross-View Consensus Learning to guide single-agent detection with multi-agent views.

Result: UMS outperforms state-of-the-art methods on V2V4Real and OPV2V datasets in unsupervised 3D detection tasks for both multi-agent and single-agent scenarios.

Conclusion: The UMS framework demonstrates the effectiveness of unsupervised multi-agent cooperation in enhancing LiDAR-based perception, offering a scalable solution for real-world applications.
...
TL;DR: The paper proposes a multi-cue guided local feature learning framework to enhance feature detection and description by leveraging semantic and geometric cues. It introduces novel modules for robust keypoint selection and descriptor construction, validated through extensive experiments.


<details>
  <summary>Details</summary>
Motivation: Existing methods for local feature detection and description rely on single appearance cues, leading to unstable keypoints and insufficient discriminability. This paper aims to address these limitations by integrating semantic and geometric cues.

Method: The framework includes a joint semantic-normal prediction head and a depth stability prediction head on a lightweight backbone. It introduces the Semantic-Depth Aware Keypoint (SDAK) mechanism for robust keypoint detection and the Unified Triple-Cue Fusion (UTCF) module for descriptor construction.

Result: Extensive experiments on four benchmarks confirm the framework's effectiveness in improving detection robustness and descriptor discriminability.

Conclusion: The proposed multi-cue framework successfully enhances local feature learning by synergistically leveraging semantic and geometric cues, outperforming existing methods.
...
TL;DR: The paper reformulates Infrared Small Target Detection (IRSTD) as a centroid regression task, proposing SPIRE, a method using single-point supervision and probabilistic response encoding for improved efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Existing IRSTD methods focus on segmentation but neglect the sparse and noisy nature of small targets, leading to inefficiencies. The authors argue for a shift to target localization.

Method: SPIRE uses Point-Response Prior Supervision (PRPS) to convert single-point annotations into probabilistic maps and employs a High-Resolution Probabilistic Encoder (HRPE) for efficient regression without a decoder.

Result: SPIRE achieves competitive detection performance with low false alarm rates and reduced computational costs on benchmarks like SIRST-UAVB and SIRST4.

Conclusion: The study demonstrates that centroid regression and probabilistic encoding are effective for IRSTD, offering a more efficient and accurate alternative to traditional segmentation approaches.
...
TL;DR: 3DTurboQuant introduces a data-independent, near-optimal quantization method for compressing 3D Gaussian Splatting and DUSt3R models, achieving significant compression ratios with minimal loss in quality.


<details>
  <summary>Details</summary>
Motivation: Existing methods for compressing 3D reconstructors require per-scene fine-tuning and data-dependent codebook learning, which is inefficient. The authors aim to eliminate these inefficiencies by leveraging the inherent properties of the parameter vectors in these models.

Method: The proposed method, 3DTurboQuant, uses precomputed, data-independent Lloyd-Max quantization. It includes (1) a dimension-dependent criterion for quantization, (2) norm-separation bounds to connect quantization error to rendering quality, (3) an entry-grouping strategy for 2D hash grid features, and (4) a composable pruning-quantization pipeline.

Result: On NeRF Synthetic, the method compresses 3DGS by 3.5x with negligible PSNR loss (0.02dB) and DUSt3R KV caches by 7.9x with high pointmap fidelity (39.7dB). The process is fast, requiring no training or calibration data.

Conclusion: 3DTurboQuant demonstrates that data-independent quantization can achieve near-optimal compression for 3D reconstructors, eliminating the need for per-scene fine-tuning or codebook learning, while maintaining high quality and efficiency.
...
TL;DR: UAVReason is introduced as a unified large-scale benchmark for high-altitude UAV scenarios, addressing domain shifts and consolidating diverse reasoning and generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models fail in UAV scenarios due to domain shifts like tiny objects, repetitive textures, and ambiguous orientations. UAVReason aims to bridge this gap.

Method: A high-fidelity UAV simulation platform generates the benchmark, featuring 273K VQA pairs, 23.6K captioned frames, 68.2K temporal sequences, and 188.8K cross-modal samples. A unified multi-task learning model is established.

Result: Experiments show improved performance in VQA (EM/F1), segmentation (mIoU), and generation (CLIP Score), overcoming limitations of general-domain models.

Conclusion: UAVReason successfully addresses UAV-native challenges, and unified multi-task learning enhances performance. The benchmark will be publicly released to advance research.
...
TL;DR: LUMOS introduces a semi-supervised framework for OCT layer segmentation, addressing label scarcity and varying granularities with a Dual-Decoder Network and progressive learning, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in OCT layer segmentation, such as limited annotations and inconsistent label granularities across datasets, which existing methods fail to exploit effectively.

Method: LUMOS uses a Dual-Decoder Network with Hierarchical Prompting Strategy (DDN-HPS) to reduce pseudo-label noise and Reliable Progressive Multi-granularity Learning (RPML) for stable cross-granularity alignment via region-level reliability weighing and progressive training.

Result: Experiments on six OCT datasets show LUMOS outperforms existing methods, demonstrating superior cross-domain and cross-granularity generalization.

Conclusion: LUMOS effectively addresses label scarcity and granularity inconsistencies in OCT segmentation, achieving state-of-the-art performance and robust generalization.
...
TL;DR: The paper introduces Object-Anchored Composed Image Retrieval (OACIR), a task focusing on instance-level consistency, and presents a benchmark (OACIRR) and framework (AdaFocal) to address it.


<details>
  <summary>Details</summary>
Motivation: Current Composed Image Retrieval (CIR) prioritizes semantic matching but struggles with instance-level fidelity, which is often more critical in practice.

Method: A new task (OACIR) is proposed, and a dataset (OACIRR) with bounding boxes for instance anchoring is created. The AdaFocal framework uses a Context-Aware Attention Modulator to balance focus between the instance and context.

Result: AdaFocal outperforms existing models, excelling in instance-level consistency, and sets a strong baseline for OACIR.

Conclusion: The work advances instance-aware retrieval systems and provides a foundation for future research in flexible, fine-grained retrieval.
...
TL;DR: LSGS-Loc is a novel visual localization pipeline for large-scale UAV scenarios, addressing pose initialization and artifact sensitivity in 3D Gaussian Splatting (3DGS).


<details>
  <summary>Details</summary>
Motivation: Visual localization in large-scale UAV scenarios is challenging due to geometric complexity and environmental variations, and existing 3DGS-based methods struggle with pose initialization and rendering artifacts.

Method: LSGS-Loc introduces a scale-aware pose initialization strategy and a Laplacian-based reliability masking mechanism to improve localization robustness and accuracy.

Result: The method achieves state-of-the-art accuracy and robustness on large-scale UAV benchmarks, outperforming existing 3DGS-based approaches.

Conclusion: LSGS-Loc effectively addresses the limitations of existing methods, providing a reliable solution for visual localization in large-scale 3DGS scenes.
...
TL;DR: A novel 3D object detection method dynamically adjusts sensor fusion preferences based on weather conditions, improving robustness in adverse weather.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-4D radar fusion methods lack adaptability to changing environmental conditions, limiting their robustness in adverse weather.

Method: The framework maintains three parallel feature streams (LiDAR, 4D radar, and fusion) and uses a lightweight router with condition tokens to dynamically weight their contributions. Weather-supervised learning ensures diverse and condition-dependent routing.

Result: State-of-the-art performance on the K-Radar benchmark, with interpretable insights into modality preferences across various weather conditions.

Conclusion: The proposed dynamic routing approach enhances robustness in adverse weather and provides transparency in sensor fusion decisions.
...
TL;DR: CRISP, a model-agnostic and parameter-free framework, addresses distribution shift in medical imaging by leveraging rank stability of predicted probabilities.


<details>
  <summary>Details</summary>
Motivation: Distribution shift in medical imaging causes performance degradation and health inequities, which existing methods fail to address due to their reliance on simulated shifts or pseudo-supervision.

Method: CRISP uses the principle of "Rank Stability of Positive Regions" to simulate shifts via latent feature perturbation, identifying stable high-probability and low-probability regions, and iteratively refines high-precision and high-recall priors.

Result: CRISP outperforms state-of-the-art methods in multi-center cardiac MRI and CT-based lung vessel segmentation, achieving significant improvements in robustness and precision.

Conclusion: CRISP's novel rank-based approach effectively mitigates distribution shift, offering superior performance in unpredictable real-world scenarios.
...
TL;DR: SGPer is a framework for wheat disease segmentation that combines semantic and geometric priors from DINOv2 and SAM, achieving high accuracy with limited data.


<details>
  <summary>Details</summary>
Motivation: The challenge of wheat disease segmentation due to temporal appearance variations requires a scalable solution without extensive data collection.

Method: SGPer integrates DINOv2 for semantic priors and SAM for boundary localization, using disease-adapted prompts and dynamic filtering.

Result: SGPer achieves state-of-the-art performance in data-constrained scenarios for wheat disease and organ segmentation.

Conclusion: The synergy of semantic and geometric priors in SGPer provides a robust solution for wheat disease segmentation under limited data.
...
TL;DR: VideoStir introduces a structured, intent-aware RAG framework for long videos, addressing limitations of existing methods by preserving spatio-temporal structure and leveraging implicit query intent.


<details>
  <summary>Details</summary>
Motivation: Existing methods for scaling multimodal large language models (MLLMs) to long videos flatten videos into segments, breaking their spatio-temporal structure, and rely on explicit semantic matching, which may miss implicit query relevance.

Method: VideoStir structures videos as spatio-temporal graphs, performs multi-hop retrieval for evidence aggregation, and uses an MLLM-backed intent-relevance scorer to align frames with query intent. A dataset (IR-600K) is curated for intent alignment learning.

Result: VideoStir is competitive with state-of-the-art baselines without auxiliary information, highlighting its effectiveness in structured, intent-aware reasoning for long-video RAG.

Conclusion: VideoStir demonstrates the potential of shifting from flattened semantic matching to structured, intent-aware retrieval-augmented generation for long videos, improving relevance and context preservation.
...
TL;DR: The paper proposes Cross-Stage Attention Propagation (CSAP), a decoder framework for lightweight semantic segmentation that reduces redundancy by propagating attention maps from the deepest feature scale, improving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Existing multi-scale decoders in lightweight semantic segmentation introduce redundancy due to independently computed attention at each scale, which are strongly correlated. CSAP aims to address this inefficiency.

Method: CSAP computes attention at the deepest feature scale and propagates the resulting attention maps to shallower stages, eliminating redundant query-key computations at those stages while preserving multi-scale contextual reasoning.

Result: CSAP-Tiny achieves 42.9% mIoU on ADE20K with 5.5 GFLOPs, 80.5% on Cityscapes with 21.5 GFLOPs, and 40.9% on COCO-Stuff 164K with 5.5 GFLOPs, outperforming SegNeXt-Tiny by +1.8% on ADE20K with 16.8% fewer operations.

Conclusion: CSAP significantly reduces computational cost while maintaining or improving segmentation accuracy, making it a promising approach for lightweight semantic segmentation.
...
TL;DR: HUG3D is a framework for reconstructing textured 3D human models from a single image, addressing challenges in multi-human scenes by incorporating group-level context and interaction priors.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail in multi-human scenes due to artifacts like overlaps and occlusions, highlighting the need for approaches that model group-level context and interactions.

Method: HUG3D transforms input into canonical orthographic space, uses Human Group-Instance Multi-View Diffusion (HUG-MVD) to generate multi-view normals and images, and optimizes geometry with Human Group-Instance Geometric Reconstruction (HUG-GR) for plausibility and contact modeling.

Result: HUG3D outperforms single-human and existing multi-human methods, producing high-fidelity, physically plausible 3D reconstructions of interacting people.

Conclusion: The framework successfully integrates group- and instance-level information to resolve occlusions and interactions, advancing 3D human reconstruction in complex scenes.
...
TL;DR: GameAD introduces a risk-aware game framework for autonomous driving, outperforming existing methods in safety.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving models treat all agents equally, failing to distinguish real collision threats from complex backgrounds. This limits their ability to prioritize safety effectively.

Method: Proposes GameAD, a framework that models autonomous driving as a risk-aware game problem. It integrates components like Risk-Aware Topology Anchoring and Minimax Risk-Aware Sparse Attention for risk-prioritized interactions.

Result: GameAD significantly outperforms state-of-the-art methods on nuScenes and Bench2Drive datasets, particularly in trajectory safety.

Conclusion: The risk-aware game approach improves decision-making in autonomous driving, enhancing safety and performance over traditional methods.
...
TL;DR: The paper introduces a pipeline for generating synthetic eye movement videos using a 3D simulator to address the scarcity of behavioral data. It evaluates the method's effectiveness and releases a dataset for behavioral classifier development.


<details>
  <summary>Details</summary>
Motivation: Behavioral modalities like eye movements are scarce, expensive to annotate, and privacy-sensitive. The paper proposes synthetic data generation as a solution to overcome these limitations.

Method: The method involves extracting real human iris trajectories from reference videos and replaying them on a 3D eye movement simulator via headless browser automation. This generates labeled synthetic eye movement videos.

Result: The generated trajectories preserve the temporal dynamics of the source data (KS D < 0.14). The 3D simulator shows bounded sensitivity for reading-scale movements, revealing insights for future simulator design. A dataset of 12 hours of synthetic eye movement video is released.

Conclusion: The pipeline and dataset support behavioral classifier development, bridging gaps in behavioral modeling and vision-language systems. The findings also inform improvements in simulator design.
...
TL;DR: The paper introduces a public dataset and a novel diagnostic method for canine pneumothorax, combining VLM-guided segmentation with RMT-based detection for high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Addressing data scarcity and the need for trustworthy models in diagnosing canine pneumothorax.

Method: Uses a Vision-Language Model (VLM) for iterative segmentation and Random Matrix Theory (RMT) for spectral detection of pathological signals.

Result: Achieves superior boundary accuracy in segmentation and highly sensitive detection of pneumothorax.

Conclusion: The proposed method is accurate and interpretable, with publicly available source code.
...
TL;DR: WSA-Net is a lightweight framework that enhances faint subsurface defect signals in Ground Penetrating Radar data through physical-feature reconstruction, outperforming existing methods in accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight detectors for subsurface defect detection prioritize efficiency over sensitivity, missing low-frequency structures and failing to decouple clutter, which leads to weak signal detection challenges.

Method: WSA-Net integrates four mechanisms: signal preservation with partial convolutions, clutter suppression via heterogeneous grouping attention, geometric reconstruction to sharpen hyperbolas, and context anchoring for semantic clarity.

Result: Evaluations on the RTST dataset show WSA-Net achieves 0.6958 mAP@0.5 and 164 FPS with only 2.412 M parameters, demonstrating superior performance.

Conclusion: WSA-Net's signal-centric approach effectively reduces false negatives in infrastructure inspection, proving the value of integrating physical-feature awareness in lightweight architectures.
...
TL;DR: A unified framework for nighttime image dehazing integrates domain-aligned data construction, stage-wise training, and inference-time enhancement, offering a practical solution to the NTIRE 2026 challenge.


<details>
  <summary>Details</summary>
Motivation: Nighttime image dehazing is complex due to haze scattering, low illumination, non-uniform lighting, and strong light interference. Limited supervision and scarce target-domain samples exacerbate domain drift and training instability.

Method: The framework uses a pre-trained CLIP visual encoder to screen external samples for domain-aligned training data. NAFNet is trained in two stages for adaptation and broader degradation. Inference-time enhancements include TLC, x8 self-ensemble, and weighted snapshot fusion.

Result: The proposed framework effectively addresses nighttime dehazing challenges without complex network redesign, providing a practical and stable solution.

Conclusion: The unified framework demonstrates a practical and effective approach for nighttime image dehazing by combining domain-aligned data, stage-wise training, and inference enhancements.
...
TL;DR: The paper introduces ContrAR, a benchmark for evaluating vision-language models (VLMs) against adversarial virtual content in augmented reality (AR), highlighting gaps in current models' robustness and performance.


<details>
  <summary>Details</summary>
Motivation: To address the security risks posed by contradictory virtual content attacks in AR, which can mislead users or deliver harmful information.

Method: Systematic modeling of attacks and development of ContrAR, a benchmark with 312 real-world AR videos validated by humans, used to test 11 VLMs.

Result: Current VLMs show some understanding of contradictory content but need improvement in detection and reasoning, with a trade-off between accuracy and latency.

Conclusion: The study underscores the need for enhanced VLM robustness in AR and identifies balancing accuracy and latency as a key challenge.
...
TL;DR: GCNV-Net is a novel 3D medical segmentation framework combining a Tri-directional Dynamic Nonvoid Voxel Transformer, Geometrical Cross-Attention, and Nonvoid Voxelization to achieve high accuracy and efficiency in segmenting diverse anatomies.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack both accuracy and computational efficiency in 3D medical scan segmentation across diverse anatomies and imaging modalities.

Method: GCNV-Net integrates a 3DNVT for spatial dependency modeling, GCA for geometric feature fusion, and Nonvoid Voxelization to reduce redundancy, achieving significant computational savings.

Result: GCNV-Net reduces FLOPs by 56.13% and latency by 68.49%, outperforming existing methods on multiple benchmarks (BraTS2021, ACDC, etc.) with improved Dice, IoU, NSD, and HD95 metrics.

Conclusion: GCNV-Net balances accuracy and efficiency, showing robustness across diverse clinical applications and strong potential for clinical deployment.
...
TL;DR: CR-Diff improves cross-resolution consistency in diffusion models by pruning adverse weights and amplifying outputs, enhancing fidelity and coherence without compromising default resolution performance.


<details>
  <summary>Details</summary>
Motivation: UNet-based diffusion models degrade at out-of-training resolutions due to resolution-dependent parameter behaviors, weakening semantic alignment and structural stability.

Method: CR-Diff uses block-wise pruning to remove adverse weights and pruned output amplification to purify predictions, supported by prompt-specific refinement.

Result: Experiments show CR-Diff enhances perceptual fidelity and semantic coherence across various resolutions and backbones while maintaining default resolution performance.

Conclusion: CR-Diff effectively addresses resolution dependency issues in diffusion models, offering a flexible and scalable solution for cross-resolution consistency.
...
TL;DR: STSF-Net improves multimodal change detection (MMCD) by modeling modality-specific and spatio-temporal features, outperforming SOTA methods on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing MMCD methods lack effective cross-modal interaction and exploitation of modality-specific features, limiting fine-grained change detection.

Method: STSF-Net jointly models modality-specific and spatio-temporal common features, uses semantic-guided fusion, and introduces the Delta-SN6 dataset.

Result: STSF-Net outperforms SOTA methods by 3.21%, 1.08%, and 1.32% in mIoU on Delta-SN6, BRIGHT, and Wuhan-Het datasets, respectively.

Conclusion: STSF-Net enhances MMCD accuracy by addressing modality-specific and spatio-temporal feature challenges; the Delta-SN6 dataset supports future research.
...
TL;DR: EchoAgent is an agentic system for end-to-end echocardiography interpretation, integrating eyes-hands-minds workflow for reliable and automated cardiac analysis.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning approaches for Echo analysis focus on restricted skills (eyes-hands or eyes-minds), limiting clinical reliability. EchoAgent aims to unify these capabilities for comprehensive interpretation.

Method: EchoAgent combines an expertise-driven cognition engine (knowledge base), a hierarchical collaboration toolkit (eyes-hands for video parsing and measurement), and an orchestrated reasoning hub for multimodal inference.

Result: EchoAgent achieves 80.00% accuracy across diverse cardiac structure analyses on CAMUS and MIMIC-EchoQA datasets, covering 48 views and 14 anatomical regions.

Conclusion: EchoAgent demonstrates the potential to replicate an echocardiologist's workflow, offering a reliable and automated solution for Echo interpretation.
...
TL;DR: The paper proposes a Prompt-based Missing Modality Adaptation (ProMMA) framework to address challenges in multimodal sentiment analysis caused by missing modalities, improving robustness and accuracy.


<details>
  <summary>Details</summary>
Motivation: The missing modality problem in multimodal sentiment analysis degrades model performance and generalizability. Existing methods lack rigorous evaluation of missing modality generation and fail to explore structural dependencies among multimodal prompts.

Method: The ProMMA framework includes a Missing Modality Evaluator, Modality-invariant Prompt Disentanglement module, Dynamic Prompt Weighting module, and Multi-level Prompt Dynamic Connection module to dynamically assess, decompose, adapt, and integrate prompts for better handling of missing modalities.

Result: Experiments on CMU MOSI, CMU MOSEI, and CH-SIMS benchmarks show state-of-the-art performance and stability under diverse missing modality conditions.

Conclusion: The ProMMA framework effectively addresses the missing modality problem, enhancing model robustness and accuracy in multimodal sentiment analysis.
...
TL;DR: SpecMamba is a parameter-efficient, frequency-aware framework for few-shot hyperspectral target detection, combining a DCT Mamba Adapter, Prior-Guided Tri-Encoder, and Self-Supervised Pseudo-Label Mapping to improve accuracy and cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for few-shot hyperspectral target detection (HTD) are inefficient, prone to overfitting, and ignore the frequency-domain structure and spectral band continuity, limiting their performance and generalization.

Method: SpecMamba introduces a Discrete Cosine Transform Mamba Adapter (DCTMA) to capture spectral dependencies efficiently, a Prior-Guided Tri-Encoder (PGTE) to leverage lab spectral priors, and a Self-Supervised Pseudo-Label Mapping (SSPLM) strategy for test-time adaptation.

Result: Extensive experiments on public datasets show SpecMamba outperforms state-of-the-art methods in detection accuracy and cross-domain generalization.

Conclusion: SpecMamba effectively addresses challenges in few-shot HTD by integrating frequency-aware adaptation and leveraging spectral priors, achieving superior performance.
...
TL;DR: EasyPolar introduces a multi-view polarimetric imaging framework using three RGB cameras to overcome resolution and artifact issues in single-shot polarization imaging, supported by a confidence-guided reconstruction network.


<details>
  <summary>Details</summary>
Motivation: Existing polarization sensors (DoFP) suffer from reduced resolution and artifacts due to spatial multiplexing. EasyPolar aims to overcome these limitations while maintaining snapshot capability.

Method: Uses a triple-camera setup (one unpolarized and two polarized views) and a confidence-guided polarization reconstruction network for multi-view fusion, ensuring geometric constraints and suppressing artifacts.

Result: The method achieves high-quality polarization imaging and improves performance in downstream tasks.

Conclusion: EasyPolar successfully addresses the limitations of DoFP sensors by combining multi-view imaging with a robust reconstruction framework.
...
TL;DR: The paper addresses overfitting in Composed Image Retrieval (CIR) tasks by introducing WRF4CIR, a weight-regularized fine-tuning method that uses adversarial perturbations to improve generalization.


<details>
  <summary>Details</summary>
Motivation: Current CIR methods relying on fine-tuning vision-language pre-trained models suffer from severe overfitting, especially with limited triplet data. A systematic study reveals a significant generalization gap, prompting the need for a better solution.

Method: WRF4CIR applies adversarial perturbations to model weights during fine-tuning, generated in the opposite direction of gradient descent, to make fitting training data harder and mitigate overfitting.

Result: Extensive experiments show WRF4CIR significantly narrows the generalization gap and outperforms existing methods on benchmark datasets.

Conclusion: WRF4CIR effectively addresses overfitting in CIR tasks, demonstrating improved generalization and performance compared to current approaches.
...
TL;DR: The paper proposes PTA, a 'Purify-then-Align' framework combining meta-learning and knowledge diffusion to address missing modalities and contamination in multimodal human sensing.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenges of missing modalities and contamination effects in robust multimodal human sensing, the authors aim to bridge the representation gap and mitigate contamination.

Method: PTA uses a meta-learning-driven weighting mechanism to purify noisy modalities and a diffusion-based knowledge distillation paradigm to align modalities, enhancing single-modality encoders.

Result: Comprehensive experiments on MM-Fi and XRF55 datasets show PTA achieves state-of-the-art performance and improves robustness in missing-modality scenarios.

Conclusion: The PTA framework effectively addresses contamination and representation gaps, enhancing the robustness of single-modality models in multimodal sensing.
...
TL;DR: BPC-Net is a new annotation-free skin lesion segmentation framework that addresses boundary under-confidence via Gaussian Probability Smoothing (GPS) and achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Annotation-free segmentation is crucial for low-resource settings, but existing methods struggle with noisy pseudo-labels and boundary under-confidence, impacting mask quality.

Method: Proposes BPC-Net with GPS for boundary calibration, a feature-decoupled decoder, and an interaction-branch adaptation strategy to handle noise and cross-domain transfer.

Result: Achieves 85.80% Dice and 76.97% Jaccard on ISIC-2017, ISIC-2018, and PH2, nearing supervised results on PH2.

Conclusion: BPC-Net effectively addresses boundary under-confidence and outperforms unsupervised methods, offering a practical solution for low-resource scenarios.
...
TL;DR: ID-Selection is a token pruning method for LVLMs that balances importance and diversity, achieving high efficiency and performance even under extreme pruning ratios.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods fail to balance importance and diversity, especially at high reduction ratios, leading to redundant or overlooked tokens.

Method: ID-Selection combines importance scoring with diversity-aware iterative selection, suppressing similar tokens while retaining high-scoring ones.

Result: On LLaVA-1.5-7B, ID-Selection prunes 97.2% of tokens, reduces FLOPs by over 97%, and retains 91.8% of performance without extra training.

Conclusion: ID-Selection effectively balances token importance and diversity, offering superior efficiency and performance for LVLM inference.
...
TL;DR: The paper investigates the impact of style transfer design choices on domain generalization in computer vision, showing that diverse style pools and artistic styles outperform other options. It introduces StyleMixDG, a lightweight augmentation method.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the Sim2Real gap and inconsistencies in prior work regarding style transfer for domain generalization in deep learning models trained on synthetic data.

Method: A systematic empirical study evaluates key factors like style pool diversity, texture complexity, and style source. The findings guide the creation of StyleMixDG, a model-agnostic augmentation recipe.

Result: Results show that diverse style pools and artistic styles are most effective, and texture complexity is negligible with a large pool. StyleMixDG improves performance on benchmark datasets.

Conclusion: The study resolves prior contradictions and introduces StyleMixDG, a practical, lightweight solution for domain generalization in scene understanding tasks.
...
TL;DR: A novel STGR framework combines LLM reasoning and vision models for precise, language-guided pulmonary screening, achieving state-of-the-art results with minimal fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in medical image segmentation from clinical reports, such as semantic ambiguity and overfitting in multimodal models.

Method: Proposes STGR with Text-to-Vision Intent Distillation (TVID) and dynamic graph reasoning for mask selection, plus Selective Asymmetric Fine-Tuning (SAFT).

Result: Achieves 81.5% DSC on LIDC-IDRI, surpassing LISA by 5%, with high cross-fold stability (0.6% variance).

Conclusion: STGR and SAFT enable robust, context-aware clinical deployment with minimal parameter updates.
...
TL;DR: FunRec is a method for reconstructing functional 3D indoor scenes from egocentric RGB-D videos, surpassing prior work in accuracy and enabling applications like simulation and robot interaction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for articulated reconstruction require controlled setups or CAD priors, limiting their applicability to real-world interactions. FunRec aims to overcome this by operating on in-the-wild human interaction sequences.

Method: FunRec automatically discovers articulated parts, estimates kinematic parameters, tracks 3D motion, and reconstructs static and moving geometry in canonical space, producing simulation-compatible meshes.

Result: FunRec outperforms prior methods, achieving up to +50 mIoU improvement in part segmentation, 5-10 times lower articulation and pose errors, and higher reconstruction accuracy in real and simulated benchmarks.

Conclusion: FunRec enables practical applications such as URDF/USD export for simulation, hand-guided affordance mapping, and robot-scene interaction, demonstrating its versatility and superiority over existing approaches.
...
TL;DR: DetailVerifyBench is introduced as a benchmark for detecting and localizing hallucinations in long image captions, addressing gaps in existing benchmarks with fine granularity and domain diversity.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately detecting hallucinations in long, detailed image captions has grown with Multimodal Large Language Models, necessitating a more rigorous benchmark.

Method: Created DetailVerifyBench, a benchmark with 1,000 high-quality images across five domains, featuring dense, token-level annotations for hallucination types.

Result: DetailVerifyBench supports precise hallucination localization in extensive captions, offering the most challenging benchmark for long image captioning.

Conclusion: The benchmark fills a critical gap in evaluating fine-grained hallucination detection capabilities in modern image captioning models.
...
TL;DR: LLaRS is a unified foundation model for multi-modal, multi-task remote sensing vision restoration, outperforming existing models by aligning heterogeneous data and using step-level dynamic weight adjustment.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing restoration models are limited by handling only one degradation type per model, necessitating a unified approach for multiple tasks.

Method: LLaRS uses Sinkhorn-Knopp optimal transport for band alignment, three mixture-of-experts layers for feature routing, and dynamic weight adjustment for training stability.

Result: LLaRS outperforms seven competitive models and shows strong transfer capabilities on unseen data.

Conclusion: LLaRS is a scalable and efficient solution for multi-task remote sensing restoration, with demonstrated superior performance and adaptability.
...
TL;DR: SGANet is a novel framework for multi-view anomaly detection, addressing feature inconsistency by combining semantic and geometric alignment across viewpoints and modalities.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised methods struggle with feature inconsistency due to viewpoint variations and modality discrepancies.

Method: SGANet introduces a unified framework with three components: SCFRM for cross-view feature refinement, SSPA for semantic-structural alignment, and MVGA for geometric alignment.

Result: SGANet achieves state-of-the-art performance on SiM3D and Eyecandies datasets in anomaly detection and localization.

Conclusion: SGANet effectively addresses multi-view anomaly detection challenges, proving useful in industrial scenarios.
...
TL;DR: Monocular broadcast videos can be used for fatigue monitoring in football by extracting player kinematics, but challenges like trajectory noise and calibration errors remain.


<details>
  <summary>Details</summary>
Motivation: Fatigue monitoring in football is crucial for injury prevention and performance, but current methods rely on subjective or intrusive measures. This paper explores the feasibility of using low-cost broadcast videos for fatigue analysis.

Method: The study uses Game State Reconstruction to extract player trajectories and a novel kinematics algorithm to derive speed and acceleration. These are analyzed via acceleration-speed (A-S) profiles to assess fatigue.

Result: The method successfully recovers kinematic patterns for fatigue analysis but is sensitive to trajectory noise, calibration errors, and temporal discontinuities in broadcast footage.

Conclusion: Monocular broadcast videos offer a viable, low-cost option for fatigue monitoring, though future research must address methodological challenges.
...
TL;DR: PanopticQuery is a framework for unified query-time reasoning in 4D scenes, improving semantic grounding via multi-view consensus and neural field optimization, outperforming existing methods on complex language queries.


<details>
  <summary>Details</summary>
Motivation: The need to understand dynamic 4D environments through natural language queries, addressing limitations in contextual reasoning and noisy, view-dependent predictions.

Method: Utilizes 4D Gaussian Splatting for reconstruction and introduces a multi-view semantic consensus mechanism to ground queries by aggregating and filtering 2D semantic predictions across views and time.

Result: Sets a new state-of-the-art on complex language queries, handling attributes, actions, spatial relationships, and multi-object interactions effectively.

Conclusion: PanopticQuery advances 4D scene understanding by unifying reconstruction and semantic grounding, validated by the Panoptic-L4D benchmark.
...
TL;DR: RATNet, a foundation model for gastrointestinal endoscopy imaging, uses analogical reasoning to improve diagnosis by transferring knowledge from heterogeneous datasets. It outperforms existing models in various scenarios, offering generalizability, adaptability, and cost-effectiveness.


<details>
  <summary>Details</summary>
Motivation: The growing global health burden of gastrointestinal diseases and the limitations of current AI-assisted diagnosis (e.g., lack of generalizability, robustness, and scalability) motivate the development of RATNet.

Method: RATNet employs a cyclic pre-training strategy, combining an encoder, a relevance-knowledge acquisition and transfer (RAT) module, a projector, and a multi-task head. It supports fine-tuning, linear probing, and zero-shot transfer.

Result: RATNet outperforms existing models (e.g., GastroNet, GastroVision) in diagnosing common diseases, few-shot learning for rare diseases, zero-shot transfer, robustness under long-tailed distributions, adaptation to novel diseases, and privacy-preserving federated learning.

Conclusion: RATNet's analogical reasoning mechanism enhances generalization and resistance to bias, making it a practical and cost-effective foundation for intelligent gastrointestinal diagnosis, especially in resource-limited settings.
...
TL;DR: The paper explores relationships between 30 diverse medical vision tasks using Task-Contrastive Learning (TaCo) to map them into a shared representation space across 39 datasets, revealing insights into their intrinsic structure and interconnectedness.


<details>
  <summary>Details</summary>
Motivation: The study aims to uncover the underlying relationships between various medical vision tasks, which are currently underexplored, to better understand their representational similarities and differences.

Method: The authors introduce Task-Contrastive Learning (TaCo), a framework that embeds 30 heterogeneous medical vision tasks (e.g., segmentation, denoising, geometric transformations) into a shared representation space across 39 datasets from diverse imaging modalities.

Result: The work maps tasks into a joint space, identifying distinct and overlapping representations, and shows how iterative task alterations reflect in the embedding space.

Conclusion: The study provides foundational insights into the intrinsic structure of medical vision tasks, enhancing understanding of their similarities and interconnected properties in embedding spaces.
...
TL;DR: SnapFlow is a self-distillation method that compresses multi-step denoising into a single forward pass for Vision-Language-Action (VLA) models, maintaining performance while significantly reducing latency.


<details>
  <summary>Details</summary>
Motivation: Current VLA models like pi0, pi0.5, and SmolVLA suffer from high latency due to iterative denoising steps, which account for 80% of inference time. Reducing steps naively degrades task success.

Method: SnapFlow uses self-distillation to replace multi-step denoising with a single forward pass. It mixes flow-matching samples with consistency samples to avoid trajectory drift and employs a zero-initialized target-time embedding for flexibility.

Result: SnapFlow achieves 98.75% success on pi0.5 (matching 10-step teacher) with 9.6x denoising speedup. On SmolVLA, it reduces MSE by 8.3% and accelerates inference by 3.56x. It maintains performance across execution horizons.

Conclusion: SnapFlow enables efficient, high-performance VLA inference without architecture changes or external teachers, offering compositional speedups alongside other optimization techniques.
...
TL;DR: Proposed framework integrates visual priors with 3D scene modeling to reconstruct smoke-degraded scenes, using Nano-Banana-Pro for image enhancement and Smoke-GS for robust reconstruction.


<details>
  <summary>Details</summary>
Motivation: Reconstructing 3D scenes from smoke-degraded images is challenging due to scattering effects and cross-view inconsistency. A robust solution is needed.

Method: Uses Nano-Banana-Pro for image enhancement and Smoke-GS, a 3D Gaussian Splatting framework, with a view-dependent medium branch to model smoke effects.

Result: The method improves robustness to smoke degradation and generates consistent, clear novel views in smoke environments.

Conclusion: The proposed framework effectively addresses smoke-induced challenges, enhancing 3D scene reconstruction and view synthesis.
...
TL;DR: GUIDE is a framework for multimodal large language models (MLLMs) that injects multi-level geometric priors into early layers, improving spatial awareness and outperforming existing methods on spatial reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs lack physical spatial awareness, and current geometry-aware models lose local geometric details due to flattened fusion. GUIDE addresses this by integrating multi-granularity geometric information.

Method: GUIDE uses multi-level sampling in the geometric encoder to capture features from local to global scales. It aligns and fuses these priors with MLLM early layers and employs context-aware gating to optimize spatial cue usage.

Result: GUIDE significantly outperforms baselines on complex spatial reasoning and perception tasks, demonstrating effective integration of 3D geometric priors.

Conclusion: GUIDE introduces a novel paradigm for enhancing MLLMs with geometric awareness, achieving superior performance in spatial tasks through progressive geometric priors injection.
...
TL;DR: Using monocular depth priors in 3D Gaussian Splatting can enhance rendering but requires addressing scale ambiguity and inconsistency. A new framework integrates noisy depth priors and selective regularization for better results.


<details>
  <summary>Details</summary>
Motivation: Accurate depth priors improve 3D Gaussian Splatting but are hard to obtain. Monocular depth models offer a cheaper alternative but introduce challenges like scale ambiguity and inconsistency, which need mitigation.

Method: A training framework integrates noisy and scale-ambiguous depth priors into geometric supervision. It isolates ill-posed geometry for selective regularization to prevent inaccuracies from affecting well-reconstructed structures.

Result: Experiments show consistent improvements in geometric accuracy, depth estimation, and rendering quality across various datasets and model variants.

Conclusion: The proposed method effectively leverages monocular depth priors for Gaussian Splatting, enhancing rendering without relying on expensive depth acquisition systems.
...
TL;DR: MPM is a training-free token merging method for transformers that uses mutual nearest-neighbor pairs to reduce sequence length, improving latency and throughput without significant accuracy drop.


<details>
  <summary>Details</summary>
Motivation: Token reduction methods for transformers often focus on classification and use proxy metrics. For semantic segmentation, dense feature reconstruction is needed, and existing methods' overhead can negate speed gains.

Method: Mutual Pair Merging (MPM) forms mutual nearest-neighbor pairs in cosine space, averages them, and uses a merge map for gather-based reconstruction before the decoder. It requires no learned parameters or continuous compression settings.

Result: MPM reduces latency by up to 60% on Raspberry Pi 5 and increases throughput by 20% on H100 with minimal mIoU drop (<3%) on ADE20K.

Conclusion: Simple, training-free token merging like MPM can yield practical speed improvements for segmentation when overhead is considered.
...
TL;DR: This paper introduces GaussianGrow, a method for generating 3D Gaussians from point clouds, ensuring geometric accuracy through a text-guided growing scheme and multi-view synthesis.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating 3D Gaussians without reliable geometric priors led to the development of GaussianGrow to improve accuracy and quality.

Method: GaussianGrow learns to grow 3D Gaussians from point clouds using a text-guided scheme, multi-view diffusion for appearance synthesis, and iterative camera pose detection for completion.

Result: The method effectively generates 3D Gaussians from synthetic and real-scanned point clouds, demonstrating improved geometric accuracy.

Conclusion: GaussianGrow presents a robust solution for text-guided 3D Gaussian generation, addressing limitations of prior methods through innovative geometric and appearance constraints.
...
TL;DR: The paper introduces 'information scope' as a new dimension for interpreting Sparse Autoencoder (SAE) features in CLIP vision encoders, distinguishing between localized and global visual cues using the Contextual Dependency Score (CDS).


<details>
  <summary>Details</summary>
Motivation: Existing analyses of SAE features in CLIP focus on semantic meaning, neglecting how broadly features aggregate visual evidence. This work aims to address this gap.

Method: Proposes the Contextual Dependency Score (CDS) to quantify whether SAE features are positionally stable (local) or variant (global).

Result: Features with different information scopes systematically influence CLIP's predictions and confidence, revealing distinct behaviors.

Conclusion: Information scope is a critical axis for understanding CLIP representations, offering deeper diagnostic insights into SAE-derived features.
...
TL;DR: The paper proposes the Signal Attenuation Diffusion Model (SADM) for Low-Light Image Enhancement (LLIE), integrating signal attenuation into diffusion to unify brightness adjustment and noise suppression without auxiliary networks or staged training.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based LLIE methods suffer from inconsistent optimization due to two-stage pipelines or auxiliary networks, leading to suboptimal performance.

Method: SADM incorporates a signal attenuation coefficient in the diffusion process, simulating low-light degradation and guiding denoising for simultaneous brightness recovery and noise suppression.

Result: SADM eliminates the need for correction modules or staged training, aligning with DDIM via multi-scale pyramid sampling for efficient, high-quality restoration.

Conclusion: SADM effectively unifies brightness adjustment and noise suppression in a single stage, outperforming existing methods in LLIE tasks.
...
TL;DR: FoleyDesigner is a novel framework for automating spatio-temporally aligned Foley sound generation in films, integrating video analysis, diffusion models, and professional mixing. It includes a new stereo audio dataset, FilmStereo, and outperforms baselines in alignment and compatibility with industry standards.


<details>
  <summary>Details</summary>
Motivation: Manual Foley sound creation for films is labor-intensive, and existing methods lack precise spatio-temporal alignment and high-quality datasets. This work aims to automate and enhance the Foley sound design process.

Method: Uses a multi-agent architecture for video analysis, latent diffusion models for spatio-temporal alignment, and LLM-driven hybrid mechanisms. Introduces FilmStereo, a high-quality stereo audio dataset with metadata and annotations.

Result: Achieves superior spatio-temporal alignment compared to baselines and seamless integration with professional standards like Dolby Atmos.

Conclusion: FoleyDesigner automates and improves Foley sound design, offering creative flexibility and compatibility with industry pipelines.
...
TL;DR: ASSR-Net is a new method for hyperspectral image fusion that improves spatial detail and spectral fidelity by addressing anisotropic structure reconstruction and spectral distortion.


<details>
  <summary>Details</summary>
Motivation: Existing methods for hyperspectral image fusion struggle with blurred spatial details and spectral distortion, leading to compromised quality.

Method: ASSR-Net uses a two-stage approach: anisotropic structure-aware spatial enhancement (ASSE) for spatial details and hierarchical prior-guided spectral calibration (HPSC) for spectral accuracy.

Result: Experiments show ASSR-Net outperforms state-of-the-art methods in preserving spatial details and maintaining spectral consistency.

Conclusion: The proposed ASSR-Net effectively addresses key challenges in hyperspectral image fusion, achieving high-quality results in both spatial and spectral domains.
...
TL;DR: Diffusion-based compressors using Reverse Channel Coding (RCC) are more robust to bit flips than traditional methods. A variant of Turbo-DDCM enhances robustness without compromising compression performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of robustness in modern image compression methods to bit-level corruption, particularly in noisy environments.

Method: Utilizes diffusion-based compressors and introduces a more robust variant of Turbo-DDCM.

Result: RCC-based compressors show superior robustness to bit flips compared to classical and learned codecs.

Conclusion: RCC-based compression can create more resilient compressed representations, potentially reducing the need for error-correcting codes in noisy settings.
...
TL;DR: The paper discusses the importance of subtle visual signals in various applications and introduces the Subtle Visual Challenge to promote robust representation learning for these signals, focusing on deception detection and rPPG estimation.


<details>
  <summary>Details</summary>
Motivation: Subtle visual signals contain hidden patterns crucial for applications like biometric security and medical diagnosis. However, current models lack robustness and generalization, prompting the need for focused research.

Method: The authors organized the Subtle Visual Challenge with two tasks: cross-domain multimodal deception detection and remote photoplethysmography (rPPG) estimation, involving 22 teams.

Result: 22 teams participated, and baseline models were released on the MMDD2026 platform to encourage further research.

Conclusion: The challenge aims to advance robust and generalizable models for subtle visual understanding, fostering progress in computer vision and multimodal learning.
...
TL;DR: The paper proposes a learning scheme to improve visual attribution maps for interpretability in computer vision models by optimizing deletion and insertion metrics directly, using a differentiable relaxation method for efficient training and inference.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing interpretability methods, which face trade-offs between efficiency, causality, and granularity, particularly in vision transformers.

Method: The method involves optimizing deletion and insertion metrics by framing them as permutation learning and using Gumbel-Sinkhorn for differentiable relaxation, enabling end-to-end training with attribution-guided perturbations.

Result: Results show consistent quantitative improvements and sharper, boundary-aligned explanations, especially for transformer-based vision models.

Conclusion: The proposed approach effectively balances efficiency and accuracy in generating dense, pixel-level attributions, outperforming existing methods.
...
TL;DR: The paper proposes a new training objective for controllable Text-to-Image (T2I) models, called $x_0$-supervision, which improves convergence speed, visual quality, and conditioning accuracy.


<details>
  <summary>Details</summary>
Motivation: Current T2I models lack precise control over image layouts, and existing methods for controllable generation suffer from slow training convergence.

Method: The authors revisit the training objective of controllable diffusion models, introducing $x_0$-supervision or a re-weighted diffusion loss to enhance convergence.

Result: Experiments show up to 2× faster convergence and improved visual quality and conditioning accuracy compared to prior methods.

Conclusion: The proposed $x_0$-supervision offers a more efficient and effective approach for training controllable T2I models.
...
TL;DR: The paper introduces a Hybrid QUBO framework for neural network pruning, combining heuristic metrics with global optimization to improve performance, validated on an image denoising dataset.


<details>
  <summary>Details</summary>
Motivation: Existing pruning methods use greedy heuristics or oversimplified QUBO objectives, missing complex filter interactions. A more principled approach is needed to balance individual filter importance and redundancy.

Method: Proposes a Hybrid QUBO framework integrating gradient-aware sensitivity metrics (Taylor, Fisher) and activation similarity, with dynamic capacity-driven search and TT Refinement for fine-tuning.

Result: Outperforms greedy pruning and L1-based QUBO on the SIDD dataset, with TT Refinement further boosting performance at suitable scales.

Conclusion: Hybrid combinatorial formulations offer robust, scalable, and interpretable solutions for neural network compression.
...
TL;DR: BADAS-2.0 is an advanced collision anticipation system that improves upon BADAS-1.0 with better long-tail benchmarks, knowledge distillation for edge deployment, and enhanced explainability.


<details>
  <summary>Details</summary>
Motivation: To address rare and safety-critical driving scenarios, improve model efficiency for real-time edge deployment, and enhance the explainability of collision predictions.

Method: Introduced a 10-group long-tail benchmark, used active learning for data annotation, expanded the dataset, distilled models for edge deployment, and incorporated attention heatmaps and vision-language models for explainability.

Result: Expanded dataset to 178,500 labeled videos, achieved 7-12x speedup with compact models, and improved explainability with attention heatmaps and textual reasoning.

Conclusion: BADAS-2.0 significantly advances collision anticipation with better accuracy, efficiency, and transparency, making it suitable for real-world deployment.
...
TL;DR: This paper presents a multi-model framework using Sentinel-2 and Sentinel-1 SAR data with vision transformers and ensemble learning for landslide detection, achieving high accuracy and operational flexibility.


<details>
  <summary>Details</summary>
Motivation: The need for accurate and timely landslide detection to mitigate geohazard impacts drives the study.

Method: A modular framework fuses optical and SAR data, using multi-encoder vision transformers and ensemble models (LightGBM, XGBoost) for detection.

Result: Achieves an F1 score of 0.919, balancing precision and recall, and performs well in a non-classical change detection setting.

Conclusion: The framework is scalable, transferable, and effective for landslide detection and broader environmental monitoring.
...
TL;DR: The paper argues that imbalanced learning, driven by the performance-dominant modality, improves multimodal performance. It introduces PDMP, a strategy to prioritize this modality for better optimization.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning often underperforms due to insufficient optimization, attributed to imbalanced learning. The paper challenges the idea of balanced learning and suggests prioritizing dominant modalities for better results.

Method: Proposes the Performance-Dominant Modality Prioritization (PDMP) strategy: identifies the dominant modality via unimodal performance rankings and uses asymmetric gradient modulation to prioritize it.

Result: Extensive experiments validate PDMP's superiority, showing improved multimodal performance by focusing on the dominant modality.

Conclusion: Imbalanced learning driven by the performance-dominant modality is more effective for multimodal optimization, and PDMP provides a practical solution independent of model structures.
...
TL;DR: VoxSAMNet addresses the challenges of voxel sparsity and semantic imbalance in monocular SSC by introducing a sparsity-aware framework, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The imbalanced distribution of voxels (mostly empty) and rare foreground classes in monocular SSC hinder performance, prompting the need for a sparsity-aware solution.

Method: Proposes VoxSAMNet with DSFR for refining occupied voxels and Foreground Modulation (FD + TGIF) to handle class imbalance and enhance relevant features.

Result: Achieves mIoU scores of 18.2% and 20.2% on SemanticKITTI and SSCBench-KITTI-360, outperforming prior methods.

Conclusion: VoxSAMNet's sparsity-aware and semantics-guided design advances monocular SSC, offering a promising direction for future research.
...
TL;DR: The paper proposes RHVI-FDD, a hierarchical decoupling framework for low-light image enhancement, addressing noise, detail loss, and color distortion through luminance-chrominance decoupling and frequency-domain processing.


<details>
  <summary>Details</summary>
Motivation: Low-light images suffer from noise, detail loss, and color distortion, which hinder analysis and retrieval tasks. Existing methods struggle with these intertwined degradations.

Method: A hierarchical decoupling framework (RHVI-FDD) with a macro-level RHVI transform for robust luminance-chrominance separation and a micro-level Frequency-Domain Decoupling (FDD) module for feature separation and processing.

Result: The method outperforms state-of-the-art approaches on multiple datasets in both objective metrics and subjective visual quality.

Conclusion: RHVI-FDD effectively addresses the challenges of low-light image enhancement by decoupling luminance and chrominance and processing features in the frequency domain.
...
TL;DR: The paper introduces the Character Error Vector (CEV) to address limitations of the Character Error Rate (CER) in OCR evaluation, especially under parsing errors. CEV decomposes errors into parsing, OCR, and interaction components, validated using SpACER and Jensen-Shannon Distance methods.


<details>
  <summary>Details</summary>
Motivation: CER’s assumption of perfect parsing limits its usefulness in real-world OCR evaluation, especially for complex layouts or degraded images. CEV aims to bridge parsing and local metrics.

Method: CEV is a bag-of-characters evaluator decomposable into parsing, OCR, and interaction errors. Implemented via SpACER and Jensen-Shannon Distance, it’s validated against CER and parse quality.

Result: CEV effectively bridges parsing and local metrics, outperforming CER in page-level OCR evaluation. Traditional pipeline approaches beat state-of-the-art end-to-end models in degraded newspaper datasets.

Conclusion: CEV is a robust metric for OCR evaluation, decomposing errors for targeted improvements. It’s packaged in a Python library for broader use in document understanding research.
...
TL;DR: The paper proposes a geometry-aware and active sensing method, GeoUQ-GFNet, for sparse radio map reconstruction in urban environments, using a benchmark (UrbanRT-RM) and uncertainty-guided active sensing.


<details>
  <summary>Details</summary>
Motivation: Existing methods for radio map construction often neglect geometric priors and predictive uncertainty, which are crucial for accurate reconstruction in complex urban settings with limited measurements.

Method: The authors introduce UrbanRT-RM, a ray-tracing benchmark, and propose GeoUQ-GFNet, a lightweight network that predicts dense gain radio maps and spatial uncertainty maps from sparse measurements and scene priors. Uncertainty is used for active sensing.

Result: GeoUQ-GFNet shows strong and consistent performance across various urban scenes and transmitter placements. Uncertainty-guided sampling outperforms non-adaptive methods under the same measurement budget.

Conclusion: The study highlights the benefits of integrating geometry-aware learning, uncertainty estimation, and benchmark-driven evaluation for effective radio map reconstruction in complex urban environments.
...
TL;DR: HaloProbe is a Bayesian framework introduced to detect and mitigate object hallucinations in large vision-language models, outperforming prior attention-based methods by addressing hidden confounders and using non-invasive mitigation.


<details>
  <summary>Details</summary>
Motivation: Prior methods relying on attention weights for detecting hallucinations are unreliable due to hidden confounders like token position and object repetition, leading to misleading aggregated statistics (Simpson's paradox).

Method: HaloProbe factorizes external description statistics and internal decoding signals to estimate token-level hallucination probabilities. It uses balanced training to isolate internal evidence and combines it with learned prior over external features.

Result: HaloProbe-guided decoding effectively reduces hallucinations while preserving utility and fluency, outperforming state-of-the-art intervention-based methods.

Conclusion: HaloProbe provides a reliable, non-invasive solution for detecting and mitigating hallucinations in vision-language models, addressing limitations of prior attention-based approaches.
...
TL;DR: EfficientMonoHair is a fast and accurate framework for strand-level hair reconstruction from monocular video, combining implicit neural networks with multi-view geometric fusion to improve efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for hair geometry reconstruction face a trade-off between accuracy and efficiency, with implicit neural representations lacking fine details and explicit optimization methods being computationally heavy and unscalable.

Method: Proposes EfficientMonoHair, which uses a fusion-patch-based multi-view optimization for faster point cloud direction adjustments and a parallel hair-growing strategy to stabilize large-scale strand tracing under noisy conditions.

Result: The method achieves high-fidelity strand geometries with robustness, matching state-of-the-art reconstruction quality on synthetic benchmarks while being nearly 10 times faster.

Conclusion: EfficientMonoHair effectively balances accuracy and efficiency in strand-level hair reconstruction, making it a practical solution for virtual human modeling and hairstyle digitization.
...
TL;DR: WikiSeeker is a novel multi-modal RAG framework that improves KB-VQA by introducing a multi-modal retriever and redefining VLMs as Refiner and Inspector agents, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current KB-VQA methods rely heavily on images for retrieval and underutilize VLMs. WikiSeeker addresses these gaps to enhance retrieval and answer generation.

Method: WikiSeeker proposes a multi-modal retriever and assigns VLMs two roles: Refiner (rewrites queries for better retrieval) and Inspector (routes reliable retrieval to another LLM).

Result: Extensive experiments on EVQA, InfoSeek, and M2KR show WikiSeeker achieves top performance, improving retrieval accuracy and answer quality.

Conclusion: WikiSeeker's innovative approach effectively leverages VLMs and multi-modal retrieval, setting a new benchmark for KB-VQA tasks.
...
TL;DR: Etch, a black-box attack framework, targets text-to-image models by embedding harmful text in benign scenes, achieving high success rates and highlighting vulnerabilities in current safety measures.


<details>
  <summary>Details</summary>
Motivation: Identify and formalize inscriptive jailbreaks that exploit text-rendering capabilities of T2I models, revealing a gap in existing safety filters.

Method: Propose Etch, which decomposes adversarial prompts into semantic camouflage, visual-spatial anchoring, and typographic encoding, iteratively refining via a zero-order loop and vision-language feedback.

Result: Etch achieves an average attack success rate of 65.57% (peaking at 91.00%) across 7 models, outperforming baselines.

Conclusion: Current T2I safety alignments have a critical blind spot, necessitating typography-aware multimodal defenses.
...
TL;DR: The paper proposes a 3D-2D approach for dental morphological comparison using social media photos and post-mortem scans, overcoming limitations like perspective distortion and lack of objective quantification.


<details>
  <summary>Details</summary>
Motivation: The lack of ante-mortem medical records in scenarios like migrant deaths and non-universal healthcare systems motivates the use of social media photos for dental comparison. Current methods lack proper modeling of perspective distortion and objective quantification.

Method: The approach uses computer vision and optimization to align 3D post-mortem scans with 2D ante-mortem photos. Two automatic methods are developed: one using paired landmarks and another using teeth region segmentation to estimate camera parameters.

Result: The methods achieved mean ranking values of 1.6 and 1.5 over 20,164 cross comparisons from 142 samples, outperforming existing automatic dental chart comparison approaches.

Conclusion: The proposed 3D-2D approach provides an automatic, objective, and quantitative score for morphological correspondence, improving dental identification accuracy and interpretability.
...
TL;DR: The paper introduces PVIR, a benchmark for Video Instance Removal (VIR) that emphasizes physical consistency, and evaluates four methods, revealing challenges in handling complex physical interactions.


<details>
  <summary>Details</summary>
Motivation: Current VIR benchmarks lack emphasis on physical causalities, prompting the need for a physics-aware benchmark to assess methods more rigorously.

Method: PVIR includes 95 high-quality videos with instance masks and prompts, split into Simple and Hard subsets to test physical interactions. Four methods are evaluated using a human protocol focusing on semantic, visual, and spatial failures.

Result: PISCO-Removal and UniVideo perform best, while DiffuEraser and CoCoCo struggle with blurring artifacts and instruction following, especially in the Hard subset.

Conclusion: The benchmark reveals significant challenges in recovering complex physical side effects, highlighting the need for improved VIR methods.
...
TL;DR: AICA-Bench introduces a benchmark for holistic affective image analysis, evaluates VLMs, and proposes GAT Prompting to improve emotion intensity and description depth.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in integrating perception, reasoning, and generation for affective image content analysis using VLMs.

Method: Introduced AICA-Bench with three tasks (EU, ER, EGCG), evaluated 23 VLMs, and proposed GAT Prompting for better intensity calibration and richer descriptions.

Result: GAT Prompting reduced intensity errors and enhanced descriptive depth, setting a strong baseline for affective multimodal tasks.

Conclusion: The study highlights VLMs' current limitations and offers GAT as a solution, paving the way for future affective multimodal research.
...
TL;DR: Selective aggregation of cross-attention maps from relevant heads improves T2I model interpretability and outperforms DAAM in mean IoU scores.


<details>
  <summary>Details</summary>
Motivation: To explore the underutilized distinct characteristics of attention maps from different heads in T2I models and enhance interpretability.

Method: Selectively aggregating cross-attention maps from heads most relevant to a target concept.

Result: Achieves higher mean IoU scores than DAAM; relevant heads capture concept-specific features better and aid in diagnosing prompt misinterpretations.

Conclusion: Attention head selection improves interpretability and controllability of T2I generation.
...
TL;DR: ADM-GS introduces an explicit appearance decomposition for multi-traversal scene reconstruction, improving consistency and accuracy in autonomous driving simulations.


<details>
  <summary>Details</summary>
Motivation: Addressing appearance inconsistencies across multiple traversals of the same area due to varying illumination and environmental conditions.

Method: Decomposes static background appearance into traversal-invariant material and traversal-dependent illumination using a neural light field with frequency-separated hybrid encoding.

Result: Achieves a +0.98 dB PSNR improvement over baselines on Argoverse 2 and Waymo Open datasets.

Conclusion: ADM-GS effectively reduces appearance entanglement, enhancing multi-traversal reconstruction for digital twin and simulation applications.
...
TL;DR: The paper identifies a 'center bias' in CLIP models, where they focus too much on central image regions, missing boundary objects. It analyzes the issue and proposes training-free fixes like visual prompting.


<details>
  <summary>Details</summary>
Motivation: CLIP models lack fine-grained visual understanding, specifically ignoring off-center objects, which limits their performance in tasks needing such recognition.

Method: The study uses embedding decomposition and attention map analysis to trace the issue to pooling mechanisms in visual embedding aggregation.

Result: The analysis reveals that off-center object concepts are lost in the final representation. Training-free methods like visual prompting help reduce this bias.

Conclusion: Center bias in CLIP models can be mitigated without retraining by redirecting attention to off-center regions, improving their fine-grained understanding.
...
TL;DR: The paper introduces SRCP, a framework addressing limitations of successor representations (SR) in visual zero-shot unsupervised RL, improving generalization and skill controllability.


<details>
  <summary>Details</summary>
Motivation: Existing SR methods struggle in high-dimensional visual environments due to suboptimal dynamics-irrelevant representations and poor skill controllability.

Method: SRCP decouples representation learning from successor training using saliency-guided dynamics tasks and integrates a fast-sampling consistency policy with tailored objectives.

Result: Experiments on 16 tasks across 4 datasets show SRCP achieves state-of-the-art zero-shot generalization in visual URL.

Conclusion: SRCP enhances SR methods by improving representation quality and policy modeling, demonstrating strong performance in zero-shot visual URL tasks.
...
TL;DR: SonoSelect is an ultrasound method that adaptively guides probe movement for efficient scanning, reducing redundancy and improving organ and cyst coverage.


<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound requires multiple scan views, which can be redundant and costly. An efficient method to guide probe movement is needed to enhance diagnostic clarity and coverage.

Method: SonoSelect frames ultrasound view exploration as a sequential decision-making problem, using 3D spatial memory to guide probe movement. It optimizes for organ coverage, low reconstruction uncertainty, and reduced redundancy.

Result: SonoSelect achieves high organ classification accuracy with minimal views (2/N) and significant kidney (54.56%) and cyst (35.13%) coverage in simulated tests.

Conclusion: SonoSelect effectively reduces redundancy in ultrasound scanning while improving diagnostic accuracy and coverage, demonstrating its potential for clinical use.
...
TL;DR: A novel approach using a vision-language diffusion model with LoRA reduces CT metal artifacts efficiently, requiring minimal paired data and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Metal artifacts in CT scans degrade image quality and hinder standard deep learning methods, which need large datasets.

Method: Adapts a general vision-language diffusion model via LoRa, using multi-reference conditioning for artifact reduction.

Result: Achieves state-of-the-art performance with only 16–128 training examples and reduces data needs significantly.

Conclusion: Foundation models, when adapted, provide a scalable, data-efficient solution for medical image reconstruction.
...
TL;DR: The paper introduces a Mixture-of-Modality-Experts (MoME) framework with Holistic Token Learning (HTL) to improve adaptive multimodal learning, outperforming existing baselines in driver action recognition.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal learning methods struggle with adapting to changing modality reliability and capturing fine-grained cues.

Method: Proposes MoME for adaptive modality collaboration and HTL for intra-expert refinement and inter-expert knowledge transfer using tokens.

Result: Outperforms single-modal and multimodal baselines on driver action recognition, with improved interpretability.

Conclusion: The MoME framework and HTL strategy enhance multimodal learning by improving specialization and reducing fusion ambiguity.
...
TL;DR: MMEmb-R1 is a new multimodal embedding framework that adaptively uses reasoning to improve performance while minimizing unnecessary computation and latency.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the underutilization of generative reasoning in MLLMs for embedding tasks, avoiding issues like structural misalignment and unnecessary computation.

Method: Proposes MMEmb-R1, which treats reasoning as a latent variable, uses counterfactual intervention for beneficial reasoning paths, and employs reinforcement learning for selective reasoning.

Result: Achieves a score of 71.2 on the MMEB-V2 benchmark with only 4B parameters, setting a new state-of-the-art while reducing overhead.

Conclusion: The adaptive reasoning approach in MMEmb-R1 effectively balances performance and efficiency in multimodal embedding tasks.
...
TL;DR: HumANDiff introduces a novel framework for human video generation using articulated noise sampling, joint appearance-motion learning, and geometric motion consistency learning to enhance motion control and fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing generative video diffusion models struggle with capturing human motion dynamics and physics accurately, leading to less realistic results.

Method: The framework uses three key designs: 1) articulated motion-consistent noise sampling, 2) joint appearance-motion learning, and 3) geometric motion consistency learning.

Result: HumANDiff achieves state-of-the-art performance in generating motion-consistent, high-fidelity human videos with diverse clothing styles.

Conclusion: The proposed method enables scalable, controllable human video generation without requiring modifications to existing diffusion model architectures.
...
TL;DR: OmniCamera is a framework that disentangles camera motion and scene content in video generation, enabling flexible control. It uses a hybrid dataset and a dual-level training strategy to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models often entangle camera motion and scene content, limiting independent control. This work aims to explicitly disentangle these factors for more creative video generation.

Method: The framework combines a hybrid dataset (OmniCAM) of real and synthetic videos with a Dual-level Curriculum Co-Training strategy, which introduces control modalities progressively and trains on synthetic data before adapting to real data.

Result: OmniCamera achieves state-of-the-art performance, offering precise control over camera movements while maintaining high visual quality.

Conclusion: The paper presents a novel approach to disentangling camera and content control in video generation, supported by innovative data and training strategies, leading to superior results.
...
TL;DR: The paper introduces A-ROM, a framework leveraging pre-trained ViTs for interpretable medical imaging models, combining human-readable concepts and kNN classifiers to ensure transparency.


<details>
  <summary>Details</summary>
Motivation: Address the opacity of deep learning models in medical imaging, hindering clinical adoption, by proposing a transparent and scalable alternative.

Method: Uses pre-trained Vision Transformers (ViTs) to create a generalizable metric space, replaces traditional decision layers with a human-readable concept dictionary and kNN classifier, and avoids gradient-based fine-tuning.

Result: A-ROM achieves competitive performance on the MedMNIST v2 suite, offering a transparent, few-shot solution suitable for clinical use.

Conclusion: A-ROM provides a scalable and interpretable approach for medical imaging, meeting transparency demands while maintaining performance.
...
TL;DR: The paper explores how text-to-image diffusion models handle ambiguous prompts by localizing decision-making processes within self-attention layers, proposing ICM for targeted interventions.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the opaque decision-making in diffusion models, especially for non-descriptive prompts, and improve model steering for better outcomes.

Method: The authors introduce a probing-based localization technique to identify layers with high attribute separability and propose Implicit Choice-Modification (ICM) for precise interventions.

Result: Self-attention layers are key in resolving ambiguous concepts. ICM demonstrates superior debiasing performance with fewer artifacts compared to existing methods.

Conclusion: Targeted interventions in specific self-attention layers enhance model control, offering a more effective approach for debiasing and improving generative outputs.
...
TL;DR: EDGE-Shield is a scalable content filter for Text-to-Image models, reducing latency by 79% for Z-Image-Turbo and 50% for Qwen-Image while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Address the risks of copyright violation and deepfake generation in Text-to-Image models by providing scalable, up-to-date protection without relying on fixed knowledge cutoffs.

Method: Leverage embedding-based matching for efficient reference comparison and introduce an $x$-pred transformation to enhance classification accuracy during earlier denoising stages.

Result: EDGE-Shield achieves significant latency reductions (79% for Z-Image-Turbo, 50% for Qwen-Image) while maintaining filtering accuracy across different model architectures.

Conclusion: The proposed EDGE-Shield offers a practical and scalable solution for real-time violative content filtering in generative models.
...
TL;DR: Graph-PiT models structural dependencies of visual components using a graph prior, improving part-based controllability in image generation.


<details>
  <summary>Details</summary>
Motivation: Existing part-based frameworks ignore spatial and semantic relationships between parts, leading to structurally unsound compositions.

Method: Uses a Hierarchical Graph Neural Network (HGNN) to refine part embeddings with bidirectional message passing, graph Laplacian smoothness loss, and edge-reconstruction loss.

Result: Achieves better structural coherence and plausibility in generated images across synthetic and real domains.

Conclusion: Graph-PiT enhances multi-part image synthesis by explicitly modeling part relationships, offering scalability and interpretability.
...
TL;DR: The paper addresses challenges in synthesizing editable TikZ code from visuals by introducing a high-quality dataset (SciTikZ-230K), a benchmark (SciTikZ-Bench), and a novel reinforcement learning method (Dual Self-Consistency) to improve model performance.


<details>
  <summary>Details</summary>
Motivation: The gaps in data quality and evaluation hinder the progress of Multimodal Large Language Models in generating accurate TikZ code for scientific schematics.

Method: The framework includes a large-scale dataset (SciTikZ-230K), a benchmark (SciTikZ-Bench), and a Dual Self-Consistency Reinforcement Learning paradigm for optimization.

Result: The trained model, SciTikZer-8B, outperforms proprietary and massive models like Gemini-2.5-Pro and Qwen3-VL-235B-A22B-Instruct.

Conclusion: The proposed framework and methods significantly advance the field of graphics program synthesis, enabling better performance in generating TikZ code from visuals.
...
TL;DR: A 3D generative framework based on Σ-Voxfield grid is proposed for scalable, multiview-consistent outdoor driving scene generation, leveraging semantic-conditioned diffusion and deferred rendering.


<details>
  <summary>Details</summary>
Motivation: Current methods for 3D scene generation either lack geometric coherence or are limited to small scales. This work aims to enable large-scale, photorealistic 3D generation without per-scene optimization.

Method: The framework uses Σ-Voxfield grids, trained with a semantic-conditioned diffusion model on local voxel neighborhoods, and scaled via progressive spatial outpainting. Deferred rendering produces photorealistic images.

Result: The approach successfully generates diverse, large-scale urban scenes, rendering photorealistic images with various configurations while maintaining computational efficiency.

Conclusion: The proposed method advances large-scale 3D scene generation, offering scalability, coherence, and photorealistic rendering without per-scene optimization.
...
TL;DR: A lightweight multimodal adaptation framework bridges RGB-pretrained VLMs and thermal infrared imagery, achieving high accuracy in species recognition and habitat-context interpretation.


<details>
  <summary>Details</summary>
Motivation: To address the representation gap between RGB-pretrained VLMs and thermal infrared imagery for ecological monitoring.

Method: Developed a thermal dataset and fine-tuned VLMs using multimodal projector alignment. Benchmarked models (InternVL3-8B-Instruct, Qwen2.5-VL-7B-Instruct, Qwen3-VL-8B-Instruct) under closed-set and open-set prompting.

Result: Qwen3-VL-8B-Instruct with open-set prompting performed best (F1 scores: 0.935 deer, 0.915 rhino, 0.968 elephant; enumeration accuracies: 0.779, 0.982, 1.000).

Conclusion: Lightweight projector-based adaptation effectively transfers RGB-pretrained VLMs to thermal drone imagery, enhancing ecological monitoring capabilities.
...
TL;DR: PoM, a new token mixing mechanism with linear complexity, replaces self-attention in transformers, matching performance while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost of self-attention in transformers, PoM offers an efficient alternative without sacrificing performance.

Method: PoM aggregates tokens via a learned polynomial function, enabling contextual information retrieval with linear complexity.

Result: PoM matches self-attention performance across five domains (e.g., text generation, image generation) and reduces computational costs for long sequences.

Conclusion: PoM is a viable, efficient replacement for self-attention in transformers, maintaining performance while lowering computational demands.
...
TL;DR: DiffHDR is a novel framework that converts low dynamic range (LDR) videos to high dynamic range (HDR) using a generative radiance inpainting approach in a video diffusion model's latent space.


<details>
  <summary>Details</summary>
Motivation: Current LDR formats lose much of the original HDR scene detail, limiting accurate luminance mapping and post-production flexibility. Existing methods struggle to restore realistic detail in over- and underexposed regions.

Method: DiffHDR formulates LDR-to-HDR conversion as a generative radiance inpainting task in the latent space of a pretrained video diffusion model, operating in Log-Gamma color space. It also enables controllable conversion via text prompts or reference images and uses synthesized HDR video data for training.

Result: DiffHDR outperforms state-of-the-art methods in radiance fidelity and temporal stability, producing realistic HDR videos with significant re-exposure flexibility.

Conclusion: The framework successfully addresses the limitations of existing LDR-to-HDR conversion techniques, offering high-quality and controllable HDR video generation.
...
TL;DR: Action Images is a unified world action model (WAM) that treats policy learning as multiview video generation, using pixel-grounded action images to leverage pretrained video backbones for zero-shot policy learning.


<details>
  <summary>Details</summary>
Motivation: Existing WAMs often use separate action modules or non-pixel-grounded action representations, limiting their ability to exploit pretrained video models and transfer across viewpoints. This work aims to unify and improve policy learning by grounding actions in pixel space.

Method: The approach translates 7-DoF robot actions into interpretable 'action images'—multiview action videos that explicitly track robot-arm motion. This allows the video backbone to act as a zero-shot policy without additional modules.

Result: The model achieves strong zero-shot success rates on RLBench and real-world evaluations, outperforming prior video-space world models in video-action joint generation quality.

Conclusion: Interpretable action images are a promising route for policy learning, offering a unified and pixel-grounded approach that enhances transferability and leverages pretrained video models effectively.
...
TL;DR: The paper proposes a topology-based method to optimize reasoning chains in large language models, balancing accuracy and efficiency by embedding topological patterns into the lightweight Chain-of-Thought paradigm.


<details>
  <summary>Details</summary>
Motivation: Current reasoning paradigms like Chain-of-Thought (CoT) have logical gaps in their reasoning chains, while multi-round methods (GoT, ToT, AoT) are effective but costly. This work aims to enhance CoT's reasoning capability without the high cost of multi-round methods.

Method: A topology-based framework embeds essential topological patterns into CoT, using persistent homology to map CoT, ToT, and GoT into a unified topological space. A Topological Optimization Agent diagnoses and repairs structural deficiencies in CoT chains.

Result: Experiments on multiple datasets show the proposed method offers a superior balance between reasoning accuracy and efficiency compared to multi-round methods, achieving 'single-round generation with multi-round intelligence'.

Conclusion: The topology-based optimization provides a practical solution for enhancing reasoning in LLMs, combining the efficiency of single-round methods with the intelligence of multi-round paradigms.
...
TL;DR: The paper explores how bidirectional supervision in autoregressive language models can mitigate the reversal curse, comparing masked language modeling (MLM) to decoder-only masking-based training. It finds that explicit training signals for source entities are crucial, and that reversal accuracy doesn't necessarily imply unified concept representations.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how bidirectional supervision (like MLM or decoder-only masking) helps autoregressive models overcome the reversal curse, where models fail to retrieve facts in reverse order.

Method: The authors evaluate MLM and decoder-only masking-based training on four reversal benchmarks and conduct a mechanistic study to analyze how these objectives succeed.

Result: Results show that reversal accuracy requires explicit training signals for source entities, with evidence suggesting forward and reverse facts are stored distinctly rather than as unified representations.

Conclusion: The study cautions that objective-level fixes improve reversal behavior but may not induce the expected latent generalization from unified concepts, highlighting the complexity of fact storage in models.
...
TL;DR: The paper proposes Inclusion-of-Thoughts (IoT), a strategy to improve LLMs' stability in answering MCQs by filtering out distracting options, enhancing performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with distracting options in MCQs, leading to unstable answers. IoT aims to reduce cognitive load and improve focus on plausible choices.

Method: IoT progressively filters MCQ options to retain only plausible ones, simplifying the decision-making process and stabilizing model reasoning.

Result: IoT significantly improves LLM performance across arithmetic, commonsense reasoning, and educational tasks with minimal computational cost.

Conclusion: IoT effectively mitigates the impact of distractors in MCQs, enhancing both the stability and transparency of LLM decision-making.
...
TL;DR: Phase-Associative Memory (PAM) is a complex-valued recurrent sequence model with matrix state, achieving competitive performance on WikiText-103 despite computational overhead.


<details>
  <summary>Details</summary>
Motivation: The paper explores an alternative to traditional transformer models by introducing complex-valued representations and retrieval, motivated by evidence of non-classical contextuality in language processing.

Method: PAM uses complex-valued outer products to accumulate associations in a matrix state and retrieves information via conjugate inner products. It transitions from vector-state models to matrix states to address capacity limitations.

Result: PAM achieves a validation perplexity of 30.0 on WikiText-103, close to a transformer's 27.1, despite higher computational costs and no custom kernels. The matrix state resolves the capacity issues of vector-state models.

Conclusion: The success of PAM supports the idea that complex-valued operations and non-classical contextuality may be fundamental to language modeling, suggesting new directions for computational formalism.
...
### [121] [Social Dynamics as Critical Vulnerabilities that Undermine Objective Decision-Making in LLM Collectives](https://arxiv.org/abs/2604.06091)
*Changgeon Ko,Jisu Shin,Hoyun Song,Huije Lee,Eui Jun Hwang,Jong C. Park*

Main category: cs.CL

TL;DR: LLM agents acting as human delegates in multi-agent systems are vulnerable to social dynamics like conformity and persuasion, leading to degraded decision accuracy under social pressure.


<details>
  <summary>Details</summary>
Motivation: The study explores how social context and dynamics, inspired by social psychology, affect the reliability of LLM agents acting as representatives in multi-agent environments.

Method: The researchers defined and manipulated four social phenomena (social conformity, perceived expertise, dominant speaker effect, and rhetorical persuasion) by varying adversarial groups, peer intelligence, argument length, and argumentative styles.

Result: The representative agent's accuracy declined under increased social pressure, such as larger adversarial groups, more capable peers, and longer arguments. Rhetorical strategies also influenced the agent's judgment.

Conclusion: Multi-agent systems are sensitive to both individual reasoning and social dynamics, revealing vulnerabilities in AI delegates that parallel human psychological biases in group decision-making.
...
### [122] [This Treatment Works, Right? Evaluating LLM Sensitivity to Patient Question Framing in Medical QA](https://arxiv.org/abs/2604.05051)
*Hye Sun Yun,Geetika Kapoor,Michael Mackert,Ramez Kouzy,Wei Xu,Junyi Jessy Li,Byron C. Wallace*

Main category: cs.CL

TL;DR: LLMs in medical QA are sensitive to question phrasing, leading to inconsistent responses, especially with positive vs. negative framing, even when grounded in the same evidence.


<details>
  <summary>Details</summary>
Motivation: Patients use LLMs for complex medical questions, but responses vary based on phrasing, risking inconsistency in high-stakes settings.

Method: Systematic evaluation in a controlled RAG setting using expert-selected documents, testing query variations (framing and language style) across 6,614 pairs and 8 LLMs.

Result: Positively- and negatively-framed queries often yield contradictory conclusions; framing effect worsens in multi-turn conversations. No interaction between framing and language style.

Conclusion: Query phrasing alone can systematically influence LLM responses in medical QA, underscoring the need for phrasing robustness in RAG systems.
...
### [123] [Memory Dial: A Training Framework for Controllable Memorization in Language Models](https://arxiv.org/abs/2604.05074)
*Xiangbo Zhang,Ali Emami*

Main category: cs.CL

TL;DR: Memory Dial is introduced as a training framework to control memorization pressure in language models, enabling study of memorization effects distinct from other factors.


<details>
  <summary>Details</summary>
Motivation: To isolate and control memorization in language models, addressing the challenge of disentangling memorization effects from architecture, data, or optimization.

Method: Memory Dial interpolates between cross-entropy and a temperature-sharpened objective via parameter α, producing models varying only in memorization pressure.

Result: Experiments show α reliably controls memorization, larger models respond more to memorization pressure, and frequent sequences are easier to memorize.

Conclusion: Memory Dial offers a controlled framework for studying memorization and its interaction with generalization in language models.
...
### [124] [Beyond LLM-as-a-Judge: Deterministic Metrics for Multilingual Generative Text Evaluation](https://arxiv.org/abs/2604.05083)
*Firoj Alam,Gagan Bhatia,Sahinur Rahman Laskar,Shammur Absar Chowdhury*

Main category: cs.CL

TL;DR: OmniScore, a family of deterministic learned metrics, addresses the cost and sensitivity issues of LLM-based text evaluation by using small, efficient models trained on large-scale synthetic data.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) as automated judges are costly and sensitive to prompts, limiting reproducibility. OmniScore offers a practical alternative.

Method: Developed using small (<1B) parameter models trained on synthetic supervision (564k instances, 107 languages) and evaluated with 8,617 annotations.

Result: OmniScore provides reliable, multi-dimensional scores across QA, translation, and summarization in 6 languages, matching LLM-judge behavior with low latency.

Conclusion: Lightweight, deterministic learned metrics like OmniScore are practical, scalable alternatives to frontier LLMs for text evaluation.
...
### [125] [Document Optimization for Black-Box Retrieval via Reinforcement Learning](https://arxiv.org/abs/2604.05087)
*Omri Uzan,Ron Polonsky,Douwe Kiela,Christopher Potts*

Main category: cs.CL

TL;DR: The paper introduces document optimization as a method to improve retrieval quality by fine-tuning document representations to align with query distributions, outperforming traditional document expansion and even larger models.


<details>
  <summary>Details</summary>
Motivation: Traditional document expansion degrades performance with modern retrievers by adding noise. The goal is to optimize documents to better align with query distributions under a target retriever.

Method: Fine-tune language or vision language models to transform documents using GRPO, with retrieval ranking improvements as rewards. Requires only black-box access to retrieval ranks and works across various retriever types.

Result: Significant improvements in retrieval tasks (e.g., code and visual document retrieval). Smaller, optimized retrievers outperform larger ones, and combining document optimization with fine-tuning yields the best results.

Conclusion: Document optimization is a powerful alternative to traditional document expansion, enhancing retrieval quality and efficiency, often outperforming larger models or combining effectively with fine-tuning.
...
### [126] [Multilingual Language Models Encode Script Over Linguistic Structure](https://arxiv.org/abs/2604.05090)
*Aastha A K Verma,Anwoy Chatterjee,Mehak Gupta,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: The study explores how multilingual language models organize representations for diverse languages, finding that surface-form cues like orthography heavily influence these representations, while linguistic abstraction emerges gradually in deeper layers.


<details>
  <summary>Details</summary>
Motivation: The research aims to understand whether abstract language identity or surface-form cues (e.g., orthography) shape the representations in multilingual language models, particularly in compact, distilled models.

Method: The authors analyze language-associated units in Llama-3.2-1B and Gemma-2-2B models using the Language Activation Probability Entropy (LAPE) metric and decompose activations with Sparse Autoencoders. They also study the impact of romanization and word-order shuffling on these representations.

Result: Representations are strongly influenced by orthography, with romanization creating disjoint representations. Typological structure becomes more accessible in deeper layers, and generation is most sensitive to units invariant to surface-form perturbations.

Conclusion: Multilingual language models organize representations around surface form, with linguistic abstraction emerging gradually but not collapsing into a unified interlingua.
...
TL;DR: MegaTrain is a memory-centric system for training large language models up to 120B parameters on a single GPU by leveraging host memory and optimizing CPU-GPU bandwidth.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiencies of traditional GPU-centric systems in training large language models at full precision on a single GPU by minimizing persistent device state and overcoming CPU-GPU bandwidth bottlenecks.

Method: 1) Uses a memory-centric approach with host memory for parameters and optimizer states. 2) Implements a pipelined double-buffered execution engine for overlapping tasks. 3) Replaces autograd graphs with stateless layer templates for flexibility.

Result: Achieves reliable training of 120B parameter models on a single GPU with 1.5TB host memory, outperforming DeepSpeed ZeRO-3 by 1.84x in throughput for 14B models.

Conclusion: MegaTrain demonstrates significant efficiency gains in training large models on limited hardware, making it a practical solution for resource-constrained scenarios.
...
### [128] [RAG or Learning? Understanding the Limits of LLM Adaptation under Continuous Knowledge Drift in the Real World](https://arxiv.org/abs/2604.05096)
*Hanbing Liu,Lang Cao,Yang Li*

Main category: cs.CL

TL;DR: The paper addresses the challenge of adapting large language models (LLMs) to evolving knowledge over time, proposing a new benchmark for evaluating methods like continual finetuning and retrieval-augmented generation (RAG), and introducing Chronos, a time-aware retrieval baseline to improve temporal consistency.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with knowledge drift as the world evolves, leading to outdated predictions and inconsistent reasoning. Existing methods lack evaluation in real-world dynamic settings.

Method: The paper introduces a benchmark for dynamic events and proposes Chronos, a time-aware retrieval baseline that organizes evidence into an Event Evolution Graph.

Result: Existing methods, including vanilla RAG, perform poorly under continuous knowledge drift due to issues like catastrophic forgetting. Chronos improves temporal consistency without additional training.

Conclusion: This work establishes a foundation for better understanding and improving LLM adaptation to continuous knowledge drift in realistic scenarios.
...
### [129] [$π^2$: Structure-Originated Reasoning Data Improves Long-Context Reasoning Ability of Large Language Models](https://arxiv.org/abs/2604.05114)
...
### [130] [SenseAI: A Human-in-the-Loop Dataset for RLHF-Aligned Financial Sentiment Reasoning](https://arxiv.org/abs/2604.05135)
*Berny Kabalisa*

Main category: cs.CL

TL;DR: SenseAI is a human-in-the-loop financial sentiment dataset with reasoning chains and human feedback, designed for improving LLM fine-tuning and identifying systematic model errors.


<details>
  <summary>Details</summary>
Motivation: To address gaps in existing financial sentiment datasets by incorporating reasoning, human feedback, and real-world outcomes for better model alignment and improvement.

Method: The dataset includes 1,439 labeled data points with reasoning chains, confidence scores, and human corrections, analyzed for systematic patterns in model behavior.

Result: Identified predictable errors like Latent Reasoning Drift and confidence miscalibration, showing LLM errors are correctable with structured HITL data.

Conclusion: SenseAI supports targeted model improvement in financial AI, offering opportunities for better evaluation and alignment.
...
### [131] [EvolveRouter: Co-Evolving Routing and Prompt for Multi-Agent Question Answering](https://arxiv.org/abs/2604.05149)
*Jiatan Huang,Zheyuan Zhang,Kaiwen Shi,Yanfang Ye,Chuxu Zhang*

Main category: cs.CL

TL;DR: EvolveRouter is a trainable framework for multi-agent question answering that improves agent quality and collaboration structure through closed-loop co-evolution and adaptive inference.


<details>
  <summary>Details</summary>
Motivation: Existing routing methods for multi-agent QA are limited by fixed agent pools and rigid collaboration schemes, failing to improve agents or adapt to queries dynamically.

Method: EvolveRouter combines graph-based query routing with instruction refinement in a closed-loop process and uses an adaptive inference strategy to dynamically determine collaboration size.

Result: Experiments on five QA benchmarks show EvolveRouter outperforms SOTA routing baselines in F1 and exact match, with benefits from closed-loop refinement and adaptive collaboration.

Conclusion: EvolveRouter enhances multi-agent reasoning by jointly optimizing agent quality and collaboration structure, demonstrating improved performance and efficiency.
...
### [132] [Just Pass Twice: Efficient Token Classification with LLMs for Zero-Shot NER](https://arxiv.org/abs/2604.05158)
*Ahmed Ewais,Ahmed Hashish,Amr Ali*

Main category: cs.CL

TL;DR: JPT enables causal LLMs to perform bidirectional token classification by concatenating inputs, achieving faster, more accurate zero-shot NER.


<details>
  <summary>Details</summary>
Motivation: Causal LLMs struggle with token classification due to their unidirectional attention, leading to slow decoding and errors in generative approaches.

Method: JPT concatenates input sentences to allow full bidirectional context in causal LLMs without architectural changes, using definition-guided embeddings.

Result: JPT surpasses prior methods by +7.9 F1 on CrossNER and MIT benchmarks, with 20x speed gain over generative approaches.

Conclusion: JPT effectively adapts causal LLMs for bidirectional token classification, improving zero-shot NER performance and efficiency.
...
### [133] [What Makes a Good Response? An Empirical Analysis of Quality in Qualitative Interviews](https://arxiv.org/abs/2604.05163)
*Jonathan Ivey,Anjalie Field,Ziang Xiao*

Main category: cs.CL

TL;DR: The paper evaluates 10 measures of interview response quality to identify which predict a response's contribution to study findings, using a new dataset of 343 transcripts. Direct relevance to research questions is the strongest predictor, while clarity and surprisal-based informativeness are not.


<details>
  <summary>Details</summary>
Motivation: To validate which measures of interview response quality actually contribute to study goals, as existing measures lack validation.

Method: Identified, implemented, and evaluated 10 proposed measures of interview response quality using the Qualitative Interview Corpus (343 transcripts from 14 real research projects).

Result: Direct relevance to research questions is the strongest predictor of response quality; clarity and surprisal-based informativeness are not.

Conclusion: Provides insights and scalable metrics to improve qualitative study design and automated interview system evaluation.
...
### [134] [Gradient-Controlled Decoding: A Safety Guardrail for LLMs with Dual-Anchor Steering](https://arxiv.org/abs/2604.05179)
*Purva Chiniya,Kevin Scaria,Sagar Chaturvedi*

Main category: cs.CL

TL;DR: GCD introduces a training-free guardrail for LLMs that reduces false positives and guarantees first-token safety by combining acceptance and refusal anchor tokens.


<details>
  <summary>Details</summary>
Motivation: Address the susceptibility of LLMs to jailbreak and prompt-injection attacks while improving user experience by reducing over-refusal of benign queries.

Method: Gradient-Controlled Decoding (GCD) uses acceptance and refusal anchor tokens to tighten decision boundaries, preset-injects refusal tokens if flagged, and resumes autoregressive decoding.

Result: GCD reduces false positives by 52% vs. GradSafe, lowers attack success rate by up to 10%, adds minimal latency, transfers to multiple models, and requires few demonstration templates.

Conclusion: GCD is an effective, efficient, and adaptable solution for mitigating jailbreak and prompt-injection attacks in LLMs.
...
### [135] [Improving Clinical Trial Recruitment using Clinical Narratives and Large Language Models](https://arxiv.org/abs/2604.05190)
*Ziyi Chen,Mengxian Lyu,Cheng Peng,Yonghui Wu*

Main category: cs.CL

TL;DR: The study explores using LLMs to improve clinical trial patient screening, comparing encoder- and decoder-based models and strategies like RAG. MedGemma with RAG achieved the best results (89.05% micro-F1).


<details>
  <summary>Details</summary>
Motivation: Addressing the labor-intensive bottleneck in patient screening for clinical trials, which often leads to under-enrollment and trial failures, by leveraging advancements in LLMs.

Method: Evaluated general-purpose and medical-adapted LLMs, testing strategies (Original long-context, NER-based extractive summarization, RAG) on the 2018 N2C2 Track 1 dataset.

Result: MedGemma with RAG performed best (89.05% micro-F1). Generative LLMs excelled in long-document reasoning but showed smaller gains for short-context criteria like lab tests.

Conclusion: Adopting LLMs for trial recruitment requires tailored model and strategy selection (rule-based queries, encoder-based, or generative LLMs) to balance efficiency and computing costs.
...
### [136] [Faster Superword Tokenization](https://arxiv.org/abs/2604.05192)
*Craig W. Schmidt,Chris Tanner,Yuval Pinter*

Main category: cs.CL

TL;DR: The paper improves Byte Pair Encoding (BPE) by introducing faster, memory-efficient training methods for BoundlessBPE and SuperBPE, achieving a 600x speedup on 1GB data.


<details>
  <summary>Details</summary>
Motivation: Overcome the impractical training times and memory limitations of existing BPE extensions (BoundlessBPE, SuperBPE) while maintaining their ability to form superwords.

Method: Proposes a two-phase training approach, separating regular and supermerge learning, and aggregates supermerge candidates by frequency to avoid memory bottlenecks.

Result: Achieved training times of 603 and 593 seconds for BoundlessBPE and SuperBPE on 1GB data, a 600x speedup, with identical results to original implementations.

Conclusion: The improved methods make BPE extensions practical for large-scale use, with open-source Python and Rust implementations provided.
...
### [137] [XMark: Reliable Multi-Bit Watermarking for LLM-Generated Texts](https://arxiv.org/abs/2604.05242)
*Jiahao Xu,Rui Hu,Olivera Kotevska,Zikai Zhang*

Main category: cs.CL

TL;DR: XMark is a new method for embedding and decoding binary messages in LLM-generated text, improving decoding accuracy and text quality even with limited tokens.


<details>
  <summary>Details</summary>
Motivation: Existing multi-bit watermarking methods for LLMs face computational limitations or trade-offs between text quality and decoding accuracy, especially when token counts are low.

Method: XMark uses a unique encoder that minimizes distortion in logit distribution for watermarked token generation and a tailored decoder for reliable message recovery with limited tokens.

Result: Experiments show XMark outperforms prior methods in decoding accuracy and text quality across various tasks.

Conclusion: XMark provides an effective solution for watermarking LLM-generated text, addressing key limitations of existing approaches.
...
### [138] [Exemplar Retrieval Without Overhypothesis Induction: Limits of Distributional Sequence Learning in Early Word Learning](https://arxiv.org/abs/2604.05243)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: Autoregressive transformer models fail to achieve second-order generalizations about shape as a stable feature for object categories, despite perfect first-order exemplar retrieval.


<details>
  <summary>Details</summary>
Motivation: To understand the learning mechanism behind children's ability to generalize shape as a stable feature for object categories, known as an overhypothesis.

Method: Trained autoregressive transformer models (3.4M-25.6M parameters) on synthetic corpora where shape was the stable feature, with eight controlled conditions.

Result: Models achieved 100% first-order exemplar retrieval but only 50-52% second-order generalization, confirmed by equivalence testing. Feature-swap diagnostics showed reliance on template matching.

Conclusion: Autoregressive sequence learning has limitations in achieving developmental-scale second-order generalizations.
...
### [139] [Beneath the Surface: Investigating LLMs' Capabilities for Communicating with Subtext](https://arxiv.org/abs/2604.05273)
*Kabir Ahuja,Yuxuan Li,Andrew Kyle Lampinen*

Main category: cs.CL

TL;DR: The paper explores whether language models can use subtext in communication, introducing four evaluation suites. Frontier models often favor literal communication but can sometimes leverage common ground for subtext. The study highlights model limitations and provides quantifiable measures for subtext.


<details>
  <summary>Details</summary>
Motivation: The study aims to systematically evaluate language models' ability to use subtext in creative communication, addressing a gap in current research on nuanced, human-like interactions.

Method: The authors introduce four evaluation suites, including tasks like interpreting allegories and playing multi-agent/multi-modal games inspired by Dixit, to assess subtext use.

Result: Frontier models tend to communicate literally, but some can reduce overly literal clues by 30-50% when leveraging common ground. Allegory understanding is influenced by paratextual and persona conditions.

Conclusion: The work quantifies subtext in communication, revealing model weaknesses and idiosyncrasies. It encourages future research on socially grounded creative communication and reasoning.
...
### [140] [Right at My Level: A Unified Multilingual Framework for Proficiency-Aware Text Simplification](https://arxiv.org/abs/2604.05302)
*Jinhong Jeong,Junghun Park,Youngjae Yu*

Main category: cs.CL

TL;DR: Re-RIGHT is a reinforcement learning framework for multilingual text simplification without parallel corpora, outperforming LLM baselines in lexical coverage and semantic preservation.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based readability control methods rely on labeled corpora and focus on English, limiting accessibility for non-English L2 learners.

Method: Re-RIGHT uses reinforcement learning with three reward modules (vocabulary coverage, semantic preservation, coherence) trained on 43K multilingual vocabulary data.

Result: Re-RIGHT outperforms LLMs like GPT-5.2 and Gemini 2.5 in lexical coverage at target proficiency levels while preserving meaning and fluency.

Conclusion: Re-RIGHT offers a scalable, adaptive solution for multilingual text simplification, addressing gaps in existing methods.
...
### [141] [DIA-HARM: Dialectal Disparities in Harmful Content Detection Across 50 English Dialects](https://arxiv.org/abs/2604.05318)
*Jason Lucas,Matt Murtagh,Ali Al-Lawati,Uchendu Uchendu,Adaku Uchendu,Dongwon Lee*

Main category: cs.CL

TL;DR: DIA-HARM is the first benchmark evaluating disinformation detection robustness across 50 English dialects, revealing vulnerabilities in current models and systematic disadvantages for non-Standard American English speakers.


<details>
  <summary>Details</summary>
Motivation: Current disinformation classifiers are predominantly developed and tested on Standard American English (SAE), leaving their effectiveness across dialects unexplored.

Method: Using Multi-VALUE's transformations, the authors created D3, a corpus of 195K samples derived from disinformation benchmarks, and evaluated 16 detection models on dialectal variations.

Result: Human-written dialectal content reduces detection accuracy (1.4-3.6% F1), while AI-generated content remains stable. Fine-tuned transformers outperform zero-shot LLMs, and multilingual models generalize better than monolingual ones.

Conclusion: Disinformation detectors are systematically less effective for non-SAE speakers, highlighting the need for more dialectally robust models. The DIA-HARM framework, D3 corpus, and tools are publicly released.
...
### [142] [Human Values Matter: Investigating How Misalignment Shapes Collective Behaviors in LLM Agent Communities](https://arxiv.org/abs/2604.05339)
*Xiangxu Zhang,Jiamin Wang,Qinlin Zhao,Hanze Guo,Linzhuo Li,Jing Yao,Xiao Zhou,Xiaoyuan Yi,Xing Xie*

Main category: cs.CL

TL;DR: This paper explores the impact of human value misalignment on LLM-based multi-agent systems, introducing CIVA to analyze collective behavior changes and revealing critical findings on system failures and emergent behaviors.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how human value misalignment affects LLM agents' collective behavior in multi-agent systems, addressing gaps in current research.

Method: The authors develop CIVA, a controlled multi-agent environment grounded in social science theories, where LLM agents interact autonomously. They systematically manipulate value prevalence and analyze behavior through simulations.

Result: Key findings include identifying critical values influencing collective dynamics, detecting macro-level system failures (e.g., collapse), and observing micro-level emergent behaviors (e.g., deception).

Conclusion: Human values significantly impact LLM collective outcomes, highlighting the need for future work on multi-agent value alignment.
...
TL;DR: DQA improves IT support diagnostics by maintaining persistent state and aggregating historical cases, outperforming standard RAG systems in success rate and efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard multi-turn RAG systems lack explicit diagnostic state, making them ineffective for accumulating evidence and resolving root causes in IT support interactions.

Method: DQA introduces a framework with persistent diagnostic state, conversational query rewriting, retrieval aggregation, and state-conditioned response generation.

Result: DQA achieves a 78.7% success rate (vs. 41.3% for RAG) and reduces average turns from 8.4 to 3.9 in 150 IT support scenarios.

Conclusion: DQA effectively addresses the limitations of standard RAG systems, offering a more systematic and efficient approach to IT troubleshooting.
...
TL;DR: The paper introduces ICR-Drive, a framework to evaluate the robustness of vision-language-action (VLA) models in autonomous driving by testing instruction variants (Paraphrase, Ambiguity, Noise, Misleading), revealing performance drops due to minor language changes.


<details>
  <summary>Details</summary>
Motivation: Standard evaluations of VLA models assume precise instructions, but real-world instructions vary in phrasing, specificity, and may include misleading text. This leaves instruction-level robustness under-measured, prompting the need for a diagnostic framework.

Method: ICR-Drive generates controlled instruction variants (Paraphrase, Ambiguity, Noise, Misleading) and replays identical CARLA routes under matched conditions to isolate the impact of instruction language on performance.

Result: Experiments on models like LMDrive and BEVDriver show that minor instruction changes can cause significant performance drops, highlighting distinct failure modes and a reliability gap for deploying VLA models in autonomous driving.

Conclusion: The study reveals that current VLA models are vulnerable to instruction variations, emphasizing the need for improved robustness in safety-critical applications like autonomous driving.
...
TL;DR: The paper introduces multi-turn calibration for LLMs, highlighting its risks and proposing MTCal and ConfChat to improve reliability in dynamic conversations.


<details>
  <summary>Details</summary>
Motivation: To address the gap in confidence estimation for multi-turn interactions, ensuring LLMs are reliable in high-stakes domains.

Method: Introduces ECE@T for tracking calibration dynamics, proposes MTCal for minimizing ECE@T, and ConfChat for better decoding.

Result: MTCal achieves strong performance in multi-turn calibration, and ConfChat enhances model response quality.

Conclusion: Multi-turn calibration is crucial for scaling LLM reliability in real-world applications.
...
TL;DR: MetaSD improves speculative decoding by integrating multiple drafters and dynamically allocating resources, outperforming single-drafter methods.


<details>
  <summary>Details</summary>
Motivation: Single drafters in speculative decoding are often limited to specific tasks or domains, reducing their effectiveness across diverse applications.

Method: The paper introduces MetaSD, which integrates multiple drafters and uses alignment feedback to dynamically allocate computational resources, framing drafter selection as a multi-armed bandit problem.

Result: MetaSD consistently outperforms single-drafter approaches in extensive experiments.

Conclusion: The unified framework of MetaSD enhances the flexibility and performance of speculative decoding for large language model inference.
...
TL;DR: VL-MDR bridges the gap between interpretable but slow generative reward models and efficient but opaque discriminative ones by decomposing evaluation into granular, interpretable dimensions.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between interpretability and efficiency in vision-language reward modeling.

Method: Proposes VL-MDR, a framework that dynamically decomposes evaluation into fine-grained dimensions using a visual-aware gating mechanism, and curates a dataset of 321k preference pairs across 21 dimensions.

Result: VL-MDR outperforms existing open-source reward models on benchmarks like VL-RewardBench and improves reliability in VLM alignment.

Conclusion: VL-MDR provides a scalable and interpretable solution for vision-language model alignment, effectively mitigating visual hallucinations.
...
TL;DR: ContentFuzz is a framework that rewrites social media posts to bypass information cocoons by altering machine-inferred stance labels while preserving original intent.


<details>
  <summary>Details</summary>
Motivation: Social media algorithms often confine users to echo chambers by routing content to like-minded audiences, limiting exposure to diverse viewpoints and hindering constructive discourse.

Method: ContentFuzz uses a large language model (LLM) guided by confidence feedback from stance detection models to generate rewrites that change machine-classified stance labels while preserving semantic integrity.

Result: Evaluated on four stance detection models across three datasets in two languages, ContentFuzz successfully alters machine-classified stance labels without compromising the original content's meaning.

Conclusion: ContentFuzz demonstrates the potential to expand the reach of dissenting opinions on social media by overcoming algorithmic biases that enforce information cocoons.
...
TL;DR: VeriGUI, a verification-driven GUI agent, addresses undetected action failures in vision-language models by modeling outcomes and recovery under noisy environments, using a TVAE framework and two-stage training.


<details>
  <summary>Details</summary>
Motivation: Current autonomous GUI agents assume deterministic responses, leading to undetected failures and ineffective behaviors in real-world noisy environments. Learning recovery strategies is costly due to limited feedback.

Method: Proposes VeriGUI with a TVAE framework (Thinking--Verification--Action--Expectation) for failure detection and corrective reasoning. Uses a two-stage training pipeline: Robust SFT with synthetic failures and GRPO with asymmetric verification rewards.

Result: VeriGUI reduces failure loops and improves recovery success while maintaining standard task performance, as tested on the Robustness Benchmark based on AndroidControl.

Conclusion: VeriGUI effectively addresses the limitations of current GUI agents by integrating verification and recovery mechanisms, demonstrating robustness in noisy environments.
...
TL;DR: The paper addresses the gap in Omni-LLMs for cross-modal coreference, introducing CrossOmni dataset and solutions to enhance reasoning.


<details>
  <summary>Details</summary>
Motivation: Omni-LLMs struggle with fine-grained cross-modal alignment, particularly in identifying shared referents across modalities, a critical aspect for robust reasoning.

Method: The challenge is formalized as a cross-modal coreference problem. Solutions include In-Context Learning and SFT+GRPO framework to improve alignment.

Result: Experiments show significant performance improvements and generalization to collaborative reasoning tasks.

Conclusion: Cross-modal coreference is identified as essential for advancing omni-modal reasoning in Omni-LLMs.
...
TL;DR: The paper finds a power law with an exponent near 5/3 in the power spectrum of text embeddings, indicating scale-free, context-dependent organization in language.


<details>
  <summary>Details</summary>
Motivation: To understand the statistical regularities in natural language by analyzing text embeddings and their fluctuations.

Method: Text is represented as trajectories in a high-dimensional embedding space, and scale-dependent fluctuations are quantified using an embedding-step signal. Power spectrum analysis is applied.

Result: A power law with an exponent close to 5/3 is observed across multiple languages and corpora, present in contextual embeddings but absent in static word embeddings or randomized text.

Conclusion: The findings suggest scale-free, self-similar integration of semantic information, analogous to turbulence, and provide a model-agnostic benchmark for studying language structure.
...
TL;DR: The paper proposes CoT2Edit, a method to improve knowledge editing in LLMs by leveraging Chain of Thoughts reasoning, addressing limitations of poor generalization and narrow scope in current approaches.


<details>
  <summary>Details</summary>
Motivation: Current knowledge editing methods for LLMs suffer from poor generalization and narrow scope, failing to handle diverse unstructured information effectively.

Method: The approach involves generating CoTs for structured and unstructured data, training the model via SFT and GRPO, and using RAG for dynamic fact retrieval during inference.

Result: The method achieves strong generalization across six knowledge editing scenarios with just one training round on three LLMs.

Conclusion: CoT2Edit effectively enhances knowledge editing in LLMs, addressing key limitations and improving practical usability.
...
TL;DR: The paper analyzes the efficiency challenges in Large Vision-Language Models (LVLMs), presenting a taxonomy of techniques to address bottlenecks in encoding, prefilling, and decoding, and suggests future research directions.


<details>
  <summary>Details</summary>
Motivation: To overcome the systemic efficiency barrier of visual token dominance in LVLMs, which affects inference speed due to high-resolution features, quadratic attention, and memory constraints.

Method: The paper provides a structured taxonomy of efficiency techniques for the LVLM inference lifecycle, examining how upstream decisions impact downstream bottlenecks.

Result: The analysis reveals trade-offs between visual fidelity and system efficiency, categorizing optimizations by information density, long-context attention, and memory limits.

Conclusion: Future research should explore hybrid compression, modality-aware decoding, progressive state management, and hardware-algorithm co-design, as outlined in the paper.
...
TL;DR: The paper introduces JailAgent, a framework to address security threats in LLM-based agents without modifying user prompts, using trigger extraction, reasoning hijacking, and constraint tightening.


<details>
  <summary>Details</summary>
Motivation: Existing red-team methods for LLM-based agents rely on prompt modifications, which lack adaptability and may harm performance. The paper aims to solve this.

Method: The JailAgent framework manipulates the agent's reasoning and memory retrieval through Trigger Extraction, Reasoning Hijacking, and Constraint Tightening stages.

Result: JailAgent performs well in cross-model and cross-scenario environments due to precise trigger identification, real-time adaptation, and optimized objectives.

Conclusion: JailAgent effectively addresses the limitations of existing methods by avoiding prompt modifications and enhancing security in LLM-based agents.
...
TL;DR: AutoSOTA is an automated system that improves SOTA models by replicating and optimizing research papers, achieving 105 new SOTA models in an average of five hours per paper.


<details>
  <summary>Details</summary>
Motivation: The need to accelerate the empirical model optimization pipeline in AI research due to prolonged cycles of debugging and refinement.

Method: AutoSOTA uses a multi-agent architecture with eight specialized agents for tasks like grounding papers to code, environment repair, experiment tracking, and idea generation.

Result: AutoSOTA successfully discovers 105 new SOTA models, surpassing original methods, and identifies innovations beyond hyperparameter tuning.

Conclusion: End-to-end research automation can optimize performance and reduce repetitive experimental burden, redirecting human effort toward higher-level creativity.
...
TL;DR: The paper addresses the degradation of self-conditioning in few-step diffusion models for fast inference, proposing a training framework to improve robustness and token-level noise-awareness to enhance optimization.


<details>
  <summary>Details</summary>
Motivation: Self-conditioning deteriorates in few-step diffusion models, leading to significant approximation gaps and degraded sample quality. The study aims to mitigate this issue to maintain high-quality inference at faster speeds.

Method: A novel training framework is introduced, which perturbs the self-conditioning signal to match inference noise, enhancing robustness. Additionally, a token-level noise-awareness mechanism prevents training saturation.

Result: The proposed framework outperforms standard continuous diffusion models, achieves up to 400x faster inference, and remains competitive with other one-step diffusion frameworks.

Conclusion: The study successfully improves the robustness and optimization of few-step diffusion models, maintaining high sample quality while enabling significantly faster inference.
...
TL;DR: Context-Agent models dialogue history as a dynamic tree to handle non-linear conversations, improving coherence and efficiency, and introduces the NTM benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with non-linear human conversation due to treating dialogue history as a flat sequence, leading to inefficiency and coherence loss.

Method: Introduces Context-Agent, a framework representing dialogue history as a dynamic tree structure to manage multiple topics and branches.

Result: Context-Agent improves task completion rates and token efficiency across various LLMs, demonstrating better performance in non-linear dialogues.

Conclusion: Structured context management, like dynamic trees, is valuable for complex, dynamic dialogues, as shown by the effectiveness of Context-Agent.
...
TL;DR: EpiBench is a benchmark designed to assess multi-turn, multimodal research workflows, focusing on proactive search, evidence integration, and sustained evidence use. Current leading models show significant room for improvement.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack systematic evaluation of proactive search, multi-evidence integration, and sustained evidence use in research workflows, necessitating a new benchmark.

Method: EpiBench introduces episodic multi-turn tasks requiring navigation across papers, alignment of evidence from figures/tables, and memory-based evidence use for objective questions.

Result: Leading models achieve only 29.23% accuracy on the hard split, highlighting challenges in multi-turn, multi-evidence workflows.

Conclusion: EpiBench provides a fine-grained evaluation framework for research agents, underscoring the need for improved multi-evidence integration and reproducibility.
...
TL;DR: THIVLVC is a two-stage system for Latin dependency parsing, using retrieval-augmented generation (RAG) to refine UDPipe baseline parses. It shows significant CLAS improvements, especially in poetry, and uncovers annotation inconsistencies.


<details>
  <summary>Details</summary>
Motivation: The system aims to improve Latin dependency parsing by leveraging structurally similar examples from a treebank and refining parses using a large language model, addressing inconsistencies in the gold standard.

Method: THIVLVC retrieves similar sentences from the CIRCSE treebank using length and POS n-gram similarity, then prompts a language model to refine UDPipe's baseline parse with retrieved examples and UD guidelines.

Result: On poetry (Seneca), THIVLVC improves CLAS by +17 over UDPipe; on prose (Thomas Aquinas), the gain is +1.5. Error analysis shows 53.3% of unanimous annotator decisions favor THIVLVC, revealing annotation inconsistencies.

Conclusion: THIVLVC demonstrates the effectiveness of retrieval-augmented parsing for Latin, particularly in poetry, and highlights inconsistencies in treebank annotations.
...
TL;DR: The paper introduces YoNER, a multidomain Yorùbá NER dataset, and evaluates transformer models, showing African-centric models outperform multilingual ones. It also releases OyoBERT, a Yorùbá-specific language model.


<details>
  <summary>Details</summary>
Motivation: Address the lack of diverse domain coverage in existing Yorùbá NER resources by creating a new dataset and evaluating model performance across domains.

Method: Developed YoNER, a multidomain dataset (Bible, Blogs, Movies, Radio, Wikipedia) with 5,000 sentences and 100,000 tokens. Evaluated transformer models via cross-domain and cross-lingual experiments, and introduced OyoBERT.

Result: African-centric models outperformed multilingual models for Yorùbá, but cross-domain performance dropped, especially for blogs and movies. Formal domains like news and Wikipedia transferred better. OyoBERT outperformed multilingual models in in-domain tests.

Conclusion: The YoNER dataset and OyoBERT model are released to advance Yorùbá NLP research, with findings highlighting the importance of domain-specific and African-centric models.
...
TL;DR: This paper introduces a graph-based CoT (Chain-of-Thought) optimization framework to reduce redundancy in reasoning tasks using RL, achieving a 42% reduction in reasoning tokens while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The sparsity of reward signals in RL-enhanced CoT can lead to undesirable patterns like overthinking, specifically Indiscriminate and Repetitive Reflection. The work aims to optimize reasoning efficiency by addressing these patterns.

Method: A graph-based CoT framework converts linear CoT into a DAG with dependency edges. It employs dual pruning (branch-level and depth-level) and a three-stage pipeline: SFT, DPO, and GRPO with length penalty.

Result: The approach reduces average reasoning tokens by 42% while maintaining or improving accuracy.

Conclusion: The proposed framework effectively mitigates redundancy in CoT reasoning, enhancing efficiency without sacrificing performance.
...
TL;DR: LVSpec is a training-free loosely Speculative Decoding framework for Video-LLMs, enhancing inference speed by 2.7x-2.94x while preserving high fidelity (>99.8% performance). It identifies visual-relevant tokens and tolerates positional shifts, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the high inference latency in Video-LLMs during autoregressive generation, as rigid exact-match rules in existing Speculative Decoding methods limit acceleration potential.

Method: LVSpec introduces a training-free framework with visual-relevant token identification and a position-shift tolerant mechanism to maximize token acceptance.

Result: Achieves 2.70x-2.94x speedup for Video-LLMs, >99.8% performance retention, and 136% higher mean accepted length compared to SOTA training-free SD methods.

Conclusion: LVSpec effectively balances speed and fidelity, advancing the efficiency of Video-LLMs.
...
TL;DR: The paper analyzes chain-of-thought reasoning in large language models (LLMs) as structured trajectories in representation space, showing that mathematical reasoning follows ordered subspaces. Correct and incorrect solutions diverge late, allowing mid-reasoning prediction of correctness. It introduces trajectory-based steering for reasoning correction and control.


<details>
  <summary>Details</summary>
Motivation: To understand and characterize the geometric structure of reasoning trajectories in LLMs, enabling better interpretation, prediction, and control of reasoning behavior.

Method: Analyzes mathematical reasoning in LLMs, showing step-specific subspaces and their separability. Introduces trajectory-based steering for inference-time interventions.

Result: Demonstrates that correct and incorrect solutions diverge late, enabling mid-reasoning prediction (ROC-AUC up to 0.87). Trajectory-based steering allows reasoning correction and length control.

Conclusion: Reasoning trajectories provide a geometric framework for interpreting, predicting, and controlling LLM reasoning, with practical applications in steering and correction.
...
TL;DR: Attention Editing is a framework for converting trained LLMs to new attention architectures without re-pretraining, improving efficiency while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: KV cache memory and bandwidth dominate LLM inference costs, especially for long-context and long-generation tasks. Existing solutions like MLA and SWA are hard to integrate into trained models due to structural constraints.

Method: Attention Editing replaces original attention with a learnable target module, trained via progressive distillation (layer-wise teacher-forced optimization and model-level distillation) with optional weak feature matching.

Result: Applied to Qwen3-8B and Qwen3-30B-A3B, the framework maintains performance while significantly improving efficiency, demonstrating feasibility on Ascend 910B clusters.

Conclusion: Attention Editing enables practical and robust large-scale attention conversion, offering efficiency gains without re-pretraining.
...
TL;DR: Study on GenAI voice chatbots for L2 oral practice in Chinese EFL learners, analyzing dialogue act patterns to compare high- and low-progress sessions.


<details>
  <summary>Details</summary>
Motivation: To explore how interactional processes with GenAI chatbots impact L2 learners' oral proficiency gains, as this area remains underexplored.

Method: Analyzed 70 sessions from 12 Grade 9 Chinese EFL learners over 10 weeks, annotating 6,957 dialogue acts (DAs) using a pedagogy-informed coding scheme.

Result: High-progress sessions featured more learner-initiated questions and prompting-based corrective feedback sequences, while low-progress sessions had higher clarification-seeking rates.

Conclusion: Findings highlight the importance of feedback type and timing in effective GenAI chatbot interactions and provide insights for adaptive chatbot design in L2 education.
...
TL;DR: MedLayBench-V is a new multimodal benchmark for aligning expert and lay medical language, ensuring semantic equivalence through structured refinement.


<details>
  <summary>Details</summary>
Motivation: Current Med-VLMs lack lay-language communication skills, hindering patient-centered care.

Method: The dataset uses Structured Concept-Grounded Refinement (SCGR) with UMLS CUIs and entity constraints.

Result: MedLayBench-V enables training and evaluation of Med-VLMs for expert-lay communication.

Conclusion: This benchmark addresses the gap in multimodal medical language simplification.
...
TL;DR: This paper evaluates LLMs' ability to generate outputs that match a desired target distribution, revealing gaps in current methods and proposing a new fine-tuning framework for better control.


<details>
  <summary>Details</summary>
Motivation: The real world is stochastic, but LLMs are often evaluated on single-round inference against fixed ground truths. This work aims to assess and improve LLMs' ability to align with desired distributions, such as real-world statistics or uniform distributions.

Method: The study formulates distribution alignment using attributes like gender, race, and sentiment in occupational contexts. It critiques existing methods like prompt engineering and Direct Preference Optimization, proposing a new fine-tuning framework combining Steering Token Calibration and Semantic Alignment.

Result: Experiments on six datasets show the proposed framework outperforms baselines, achieving precise control over output distributions in attribute generation tasks.

Conclusion: Standard LLMs and alignment techniques fail to reliably control output distributions, but the novel framework successfully bridges this gap, enabling more accurate distributional alignment.
...
TL;DR: The paper analyzes linguistic expressions in LLMs' reasoning parts to identify those linked to overconfidence and shows that suppressing these can calibrate confidence without performance loss.


<details>
  <summary>Details</summary>
Motivation: To address the issue of overconfidence in LLMs despite their use of uncertain linguistic expressions during reasoning.

Method: Applied regression to predict confidence from linguistic expressions in reasoning parts, analyzed $n$-grams' relationship with confidence, and tested causality and verification.

Result: LLMs remain overconfident during reasoning, and specific linguistic information contributes to this. Suppressing overconfident expressions can calibrate confidence without performance drops.

Conclusion: Confidence calibration in LLMs is achievable by targeting and suppressing specific overconfident linguistic expressions.
...
TL;DR: The paper introduces PhageBench, a benchmark for evaluating phage genome understanding by LLMs, showing their potential in some tasks but limitations in complex reasoning.


<details>
  <summary>Details</summary>
Motivation: Bacteriophages are crucial for microbial ecosystems and antibiotic alternatives, making accurate genome interpretation valuable. Current LLMs' ability to interpret nucleotide sequences is underexplored.

Method: PhageBench is introduced, containing 5,600 high-quality samples across five tasks in three stages: Screening, Quality Control, and Phenotype Annotation. Eight LLMs were evaluated.

Result: General-purpose LLMs outperformed random baselines in phage contig identification and host prediction but struggled with complex reasoning tasks like long-range dependencies.

Conclusion: The findings emphasize the need for next-gen models with improved reasoning for biological sequences.
...
TL;DR: The paper addresses hallucinations in large language models by estimating knowledge scores and scaling learning signals, improving uncertainty expression and accuracy.


<details>
  <summary>Details</summary>
Motivation: To mitigate hallucinations in LLMs caused by knowledge misalignment between pre-training and fine-tuning, ensuring reliable responses.

Method: Estimates fine-grained, instance-level knowledge scores using multi-sampled inference and scales learning signals accordingly, promoting 'I don't know' responses for out-of-scope queries.

Result: The model effectively expresses uncertainty for unknown queries while maintaining accuracy on known ones, with improved performance metrics for uncertainty discrimination.

Conclusion: This approach enhances model reliability by aligning knowledge and encouraging explicit uncertainty expression, validated by proposed evaluation metrics.
...
TL;DR: The paper introduces CARE, a framework for evaluating AI-generated therapist responses against therapeutic principles, showing significant improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: To address the lack of structured evaluation for AI therapist responses in mental health applications, ensuring they adhere to core therapeutic principles.

Method: The study uses FAITH-M, a benchmark with expert annotations, and CARE framework integrating contextual modeling, contrastive retrieval, and knowledge-distilled reasoning.

Result: CARE achieves a 64.26% improvement in F-1 score over the baseline (Qwen3), demonstrating robustness under domain shifts but challenges in capturing clinical nuance.

Conclusion: CARE offers a clinically grounded framework for assessing therapeutic fidelity in AI mental health systems, though implicit nuance remains a challenge.
...
TL;DR: CLEAR introduces a novel loss function with reverse training to enhance cross-lingual retrieval performance, achieving up to 15% improvement in low-resource languages while maintaining English performance.


<details>
  <summary>Details</summary>
Motivation: Existing multilingual embedding models face challenges in cross-lingual scenarios due to imbalanced resources and inadequate alignment training, leading to degraded performance.

Method: CLEAR uses a reverse training scheme, leveraging English passages as a bridge to strengthen alignments between target languages and English, improving retrieval performance.

Result: CLEAR improves cross-lingual retrieval by up to 15%, especially in low-resource languages, with minimal degradation in English.

Conclusion: CLEAR is effective and scalable for multilingual training, offering broad potential applications in cross-lingual retrieval tasks.
...
TL;DR: The study explores demographics-agnostic techniques to reduce bias in Wake-up Word detection, achieving significant fairness improvements across sex, age, and accent groups.


<details>
  <summary>Details</summary>
Motivation: Addressing persistent demographic biases in Voice-based interfaces to ensure equitable performance across diverse speaker populations.

Method: Utilized demographics-agnostic training (excluding labels for training), data augmentation, and knowledge distillation from pre-trained speech models on the OK Aura database.

Result: Techniques reduced Predictive Disparity by 39.94% (sex), 83.65% (age), and 40.48% (accent) compared to baseline.

Conclusion: Label-agnostic methods effectively promote fairness in Wake-up Word detection, highlighting their potential for bias mitigation.
...
TL;DR: Agentic Graph Learning (AGL) integrates graph learning with LLM capabilities for topology-aware navigation and inference, outperforming existing methods by up to 28.4% in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing agentic frameworks for LLMs treat external information as unstructured text, missing the topological dependencies in real-world data.

Method: Proposes AgentGL, an RL-driven framework for AGL, featuring graph-native tools, search-constrained thinking, and graph-conditioned curriculum RL.

Result: AgentGL achieves up to 17.5% and 28.4% absolute improvements in node classification and link prediction, respectively, over baselines.

Conclusion: AGL represents a promising approach for LLMs to autonomously navigate and reason over complex relational environments.
...
TL;DR: The paper introduces 'distinctiveness', a method to evaluate learner representations by measuring differences between students without requiring labels or specific outcomes. It finds learner-level representations are more effective than interaction-level ones.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of evaluating learner representations when instructional outcomes are unclear or context-dependent, by focusing on differentiation between learners.

Method: The study uses pairwise distances to measure distinctiveness in learner representations, comparing individual question-based and aggregated interaction-based representations.

Result: Learner-level representations show better separation, clustering, and discrimination than interaction-level ones.

Conclusion: Distinctiveness is a valuable pre-deployment metric for assessing learner representations, independent of instructional outcomes.
...
TL;DR: LoRM is a self-supervised framework for understanding rotating-machinery signals by treating them as a machine language, enabling real-time condition monitoring through token-based sequence prediction.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional signal-processing methods by reformulating multi-modal sensor data as a token-based sequence-prediction problem, leveraging pre-trained language models for efficient knowledge transfer.

Method: LoRM tokenizes local signals into discrete units, quantizes future target segments, and fine-tunes a pre-trained language model on industrial signals for condition monitoring via token-prediction errors.

Result: Demonstrates stable real-time tracking and strong cross-tool generalization in tool condition monitoring experiments, bridging language modelling with industrial signal analysis.

Conclusion: LoRM offers a practical and efficient approach to condition monitoring by combining self-supervised learning and language modelling techniques for rotating-machinery signals.
...
TL;DR: The paper compares sequential and parallel sampling in Large Reasoning Models (LRMs) and finds that parallel sampling outperforms sequential sampling, primarily due to the latter's lack of exploration.


<details>
  <summary>Details</summary>
Motivation: To understand why parallel sampling outperforms sequential sampling in LRMs, despite the latter's theoretical representation power.

Method: The study empirically tests three hypotheses: the role of aggregation, context length, and exploration, using various model families and question domains.

Result: Aggregation and context length are not significant factors; the performance gap is mainly due to sequential sampling's lack of exploration.

Conclusion: The lack of exploration in sequential sampling is a key reason for its inferior performance compared to parallel sampling.
...
TL;DR: MCircKE addresses the 'Reasoning Gap' in LLMs by identifying and updating specific causal circuits for precise knowledge editing, improving multi-step reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge editing methods for LLMs often fail to utilize edited facts in multi-step reasoning, creating a 'Reasoning Gap.'

Method: Introduces MCircKE, a framework that maps causal circuits for a reasoning task and surgically updates parameters within them.

Result: Demonstrates effectiveness on the MQuAKE-3K benchmark for multi-hop reasoning.

Conclusion: MCircKE effectively bridges the Reasoning Gap by targeting specific circuits for knowledge updates.
...
TL;DR: Introducing the French-YMCA corpus, a new linguistic resource for children and adolescents, with 39,200 text files and 22.5M words, designed to improve digital interactions for youth.


<details>
  <summary>Details</summary>
Motivation: Children have unique and evolving language needs, differing from adults, requiring specialized resources.

Method: Developed a corpus with diverse sources, consistent grammar/spelling, and open online accessibility.

Result: The corpus can train language models to better understand and respond to youth's language.

Conclusion: The French-YMCA corpus enhances age-appropriate digital interactions for children and adolescents.
...
TL;DR: FrontierFinance benchmark addresses AI-driven labor displacement in finance by evaluating 25 complex tasks, showing human experts outperform state-of-the-art AI systems.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to measure AI performance on practical professional tasks in finance, a high-risk sector for AI displacement, and lack accountability mechanisms.

Method: Developed with financial professionals, the benchmark includes 25 complex financial modeling tasks, detailed rubrics, and human baselines for evaluation.

Result: Human experts scored higher and were more likely to produce client-ready outputs compared to current AI systems.

Conclusion: The FrontierFinance benchmark highlights the gap between AI and human expertise in finance, providing a robust tool for tracking AI progress in the sector.
...
TL;DR: The paper explores multimodal pun understanding by introducing a dataset (MultiPun) and evaluating Vision-Language Models' (VLMs) performance, revealing their struggles and proposing improvements.


<details>
  <summary>Details</summary>
Motivation: To systematically study VLMs' ability to understand multimodal puns, which exploit polysemy and phonetic similarity for humor, and to address the lack of rigorous benchmarks for this task.

Method: The authors propose a multimodal pun generation pipeline, introduce the MultiPun dataset (with puns and distractors), and evaluate VLMs. They also suggest prompt-level and model-level strategies to enhance pun comprehension.

Result: Most models struggle to distinguish puns from distractors. The proposed strategies improve F1 scores by an average of 16.5%.

Conclusion: The findings offer insights for developing VLMs capable of nuanced, human-like humor understanding through cross-modal reasoning.
...
TL;DR: BOSCH is a training-free method for optimizing the hybridization of large language models by selecting heads for sliding-window attention, outperforming existing layer-level and static head-level approaches.


<details>
  <summary>Details</summary>
Motivation: Existing hybridization schemes for LLMs either operate at the layer level, ignoring head-level routing, or use static head-level rankings, which suffer from entanglement. BOSCH addresses these limitations by dynamically selecting heads for sliding-window attention.

Method: BOSCH formulates the problem as a Large Neighborhood Search, decomposing it into three subproblems: layer-importance detection, adaptive per-layer SWA-ratio assignment, and grouped head-level optimization within ratio buckets.

Result: Experiments on LLMs (1.7B to 30B parameters) show BOSCH consistently outperforms layer-level heuristics and static head-level methods, with greater improvements at higher SWA ratios. It also recovers long-context performance faster during continual pretraining.

Conclusion: BOSCH highlights the importance of dynamic head-level selection for each target SWA ratio, as evidenced by substantial turnover in selected heads across ratios.
...
TL;DR: FinReporting is a system designed to address challenges in cross-jurisdiction financial reporting by using LLMs as constrained verifiers, improving consistency and reliability across diverse accounting standards.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of cross-jurisdiction financial reporting due to structural differences in accounting taxonomies, tagging infrastructures, and aggregation conventions. Existing systems often assume a single-market setting.

Method: FinReporting introduces an agentic workflow that builds a unified canonical ontology for financial statements. It decomposes reporting into auditable stages like filing acquisition, extraction, canonical mapping, and anomaly logging, using LLMs as constrained verifiers under explicit rules and evidence grounding.

Result: The system was evaluated on annual filings from the US, Japan, and China, demonstrating improved consistency and reliability under heterogeneous reporting regimes.

Conclusion: FinReporting provides a practical solution for localized cross-jurisdiction financial reporting, offering an interactive demo for cross-market inspection and structured export of financial statements.
...
TL;DR: The paper critiques current knowledge editing methods for LLMs, introducing a diagnostic framework to reveal Surface Compliance and cognitive instability, emphasizing the need for robust memory modification.


<details>
  <summary>Details</summary>
Motivation: To address the unreliability of current LLM knowledge editing evaluations and ensure trustworthy real-world deployment by uncovering genuine memory modification.

Method: Introduces a diagnostic framework using discriminative self-assessment under in-context learning (ICL) to scrutinize subtle behavioral changes post-editing.

Result: Reveals Surface Compliance (mimicking target outputs without real memory change) and cognitive instability from recursive edits, reducing reversibility.

Conclusion: Highlights risks in current editing paradigms and stresses the importance of robust memory modification for sustainable LLM systems.
...
TL;DR: ROTATE disentangles MLP neurons in weight space without data or forward passes, optimizing rotations for interpretable vocabulary channels.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of interpreting model weights by identifying coherent, monosemantic concepts in MLP neurons.

Method: Rotates neuron weights to maximize vocabulary-space kurtosis, recovering sparse, interpretable vocabulary channels.

Result: Recovers faithful vocabulary channels, outperforming activation-based baselines by 2-3x in neuron description tasks.

Conclusion: ROTATE provides a scalable, data-free method for fine-grained interpretation of language models.
...
TL;DR: Proposes BiMind, a dual-head reasoning framework for detecting incorrect information by balancing content verification and knowledge-augmented reasoning, with innovations like attention geometry adaptation and self-retrieval knowledge mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods struggle to balance content verification and external knowledge integration, especially under collapsed attention geometries, leading to inefficiencies in detecting incorrect information. The paper aims to address these challenges.

Method: Introduces BiMind, featuring an attention geometry adapter, self-retrieval knowledge mechanism, and uncertainty-aware fusion strategies. These innovations include token-conditioned offsets, kNN retrieval, and entropy-gated fusion to enhance reasoning.

Result: BiMind outperforms advanced detection approaches on public datasets, providing interpretable diagnostics through a novel metric, Value-of-eXperience (VoX), which measures knowledge contributions.

Conclusion: The BiMind framework effectively balances content and knowledge-augmented reasoning, offering a robust solution for detecting incorrect information while providing insights into the role of external knowledge.
...
### [187] [A Multi-Stage Validation Framework for Trustworthy Large-scale Clinical Information Extraction using Large Language Models](https://arxiv.org/abs/2604.06028)
...
TL;DR: A multi-stage validation framework for LLM-based clinical information extraction is proposed, enabling scalable and trustworthy assessment without exhaustive manual annotation, demonstrated through substance use disorder diagnosis extraction.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) show promise for extracting clinical information from unstructured health records, but their real-world application is limited by the lack of scalable and trustworthy validation methods.

Method: The framework includes prompt calibration, rule-based plausibility filtering, semantic grounding assessment, confirmatory evaluation using a judge LLM, selective expert review, and external predictive validity analysis.

Result: Applied to 919,783 clinical notes, the framework removed 14.59% of unsupported LLM-positive extractions. The primary LLM achieved an F1 score of 0.80, and LLM-extracted diagnoses predicted SUD specialty care engagement more accurately than structured-data baselines (AUC=0.80).

Conclusion: The study demonstrates that scalable and trustworthy deployment of LLM-based clinical information extraction is feasible without intensive manual annotation.
...
### [188] [From Hallucination to Structure Snowballing: The Alignment Tax of Constrained Decoding in LLM Reflection](https://arxiv.org/abs/2604.06066)
*Hongxu Zhou*

Main category: cs.CL

TL;DR: The study explores how enforcing structured reflection via constrained decoding in LLMs fails to improve self-correction, leading to 'structure snowballing' and revealing an 'alignment tax' between structural rules and model capacity.


<details>
  <summary>Details</summary>
Motivation: To understand if structured reflection through constrained decoding can prevent error propagation in LLMs without external tools or training.

Method: Evaluated an 8B-parameter model (Qwen3-8B) using outlines-based constrained decoding to enforce structured reflection.

Result: Structural constraints did not improve self-correction; instead, they caused 'structure snowballing,' where models prioritize formatting over semantic accuracy.

Conclusion: Constrained decoding imposes an 'alignment tax,' highlighting a trade-off between structural granularity and model capacity in autonomous workflows.
...
### [189] [Short Data, Long Context: Distilling Positional Knowledge in Transformers](https://arxiv.org/abs/2604.06070)
*Patrick Huber,Ernie Chang,Chinnadhurai Sankar,Rylan Conway,Igor Fedorov,Md Rifat Arefin,Adithya Sagar*

Main category: cs.CL

TL;DR: The paper explores transferring long-context retrieval capabilities to student models via logit-based knowledge distillation using packed short-context samples. It leverages Rotary Position Embedding (RoPE) to achieve efficient long-context performance without costly pre-training.


<details>
  <summary>Details</summary>
Motivation: The high cost of long-context pre-training for language models motivates the search for efficient alternatives like knowledge distillation to transfer these capabilities.

Method: The study uses logit-based knowledge distillation with packed short-context samples, analyzing Rotary Position Embedding (RoPE) and its role in positional information transfer.

Result: Key findings include the effectiveness of phase-wise RoPE scaling, the direct transfer of positional information via logits, and structured parameter updates during long-context extension.

Conclusion: Logit-based knowledge distillation successfully transfers long-context capabilities, offering a cost-effective solution without extensive pre-training.
...
### [190] [Stories of Your Life as Others: A Round-Trip Evaluation of LLM-Generated Life Stories Conditioned on Rich Psychometric Profiles](https://arxiv.org/abs/2604.06071)
*Ben Wigler,Maria Tsfasman,Tiffany Matej Hrkalovic*

Main category: cs.CL

TL;DR: LLMs can robustly encode and decode personality traits in text, achieving accuracy near human reliability, and produce behaviorally differentiated narratives that align with real human data.


<details>
  <summary>Details</summary>
Motivation: To determine if personality conditioning in LLMs creates psychometrically informative representations or just superficial alignment with trait descriptors, given limitations in existing evaluations.

Method: Conditioned LLMs on real psychometric profiles from 290 participants to generate life stories, then used independent LLMs to recover personality scores from these narratives.

Result: Personality scores were recoverable with high accuracy (mean r = 0.750, 85% of human ceiling), robust across 10 generators and 3 scorers. Generated narratives showed behavioral differentiation and replicated emotional patterns in real conversations.

Conclusion: LLMs can robustly encode and decode individual personality differences, with patterns that replicate real human behavior, supporting the personality-language relationship captured during pretraining.
...
### [191] [LAG-XAI: A Lie-Inspired Affine Geometric Framework for Interpretable Paraphrasing in Transformer Latent Spaces](https://arxiv.org/abs/2604.06086)
*Olexander Mazurets,Olexander Barmak,Leonid Bedratyuk,Iurii Krak*

Main category: cs.CL

TL;DR: LAG-XAI introduces a geometric framework for interpreting Transformer-based models by modeling paraphrasing as affine transformations, achieving high interpretability with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To address the lack of interpretability in Transformer-based language models by providing a structured, geometrically interpretable framework for paraphrasing.

Method: Proposes LAG-XAI, a geometric framework using affine transformations (rotation, deformation, translation) to model paraphrasing. A computationally efficient mean-field approximation is derived from local Lie group actions.

Result: Achieves AUC of 0.7713, capturing 80% of the non-linear baseline's capacity. Identifies geometric invariants like a stable reconfiguration angle (~27.84°) and near-zero deformation. Cross-domain validation and hallucination detection (95.3% accuracy on HaluEval) demonstrate utility.

Conclusion: LAG-XAI offers a mathematically grounded, interpretable, and resource-efficient approach for understanding Transformer models, balancing accuracy and transparency.
...
TL;DR: The paper proposes Exclusive Unlearning (EU) to remove diverse harmful content from LLMs by broadly forgetting unwanted knowledge while retaining desired capabilities.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of harmful content generation when LLMs are applied in sensitive industries like healthcare and education.

Method: Introduces Exclusive Unlearning (EU), which extensively forgets unwanted knowledge while preserving specific domain-related expressions.

Result: EU ensures model safety against harmful inputs (e.g., jailbreaks) while maintaining performance in targeted domains like medicine and mathematics.

Conclusion: Exclusive Unlearning offers a promising approach for comprehensive harm removal in LLMs without sacrificing functionality in critical applications.
...
### [193] [GLANCE: A Global-Local Coordination Multi-Agent Framework for Music-Grounded Non-Linear Video Editing](https://arxiv.org/abs/2604.05076)
*Zihao Lin,Haibo Wang,Zhiyang Xu,Siyao Dai,Huanjie Dong,Xiaohan Wang,Yolo Y. Tang,Yixin Wang,Qifan Wang,Lifu Huang*

Main category: cs.MA

TL;DR: GLANCE is a multi-agent framework for music-grounded video editing, using a bi-loop architecture and global-local coordination to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: To address limitations of fixed pipelines and simplified paradigms in music-grounded mashup video creation, enabling adaptability to diverse prompts and materials.

Method: GLANCE employs a bi-loop architecture (long-horizon planning and segment-wise editing) with a global-local coordination mechanism to resolve conflicts.

Result: GLANCE outperforms baselines by 33.2% and 15.6% on tasks, validated by human evaluation and a new benchmark, MVEBench.

Conclusion: GLANCE demonstrates superior performance and flexibility in music-grounded video editing, validated by rigorous evaluation.
...
### [194] [Governance-Aware Agent Telemetry for Closed-Loop Enforcement in Multi-Agent AI Systems](https://arxiv.org/abs/2604.05119)
*Anshul Pathak,Nishant Jain*

Main category: cs.MA

TL;DR: GAAT introduces a governance-aware architecture for real-time policy enforcement in multi-agent AI systems, addressing the gap in existing observability tools.


<details>
  <summary>Details</summary>
Motivation: Current tools like OpenTelemetry and Langfuse lack real-time governance enforcement, leading to policy violations detected only post-damage.

Method: GAAT includes a Governance Telemetry Schema, real-time violation detection, a Governance Enforcement Bus, and a Trusted Telemetry Plane.

Result: The system achieves sub-200 ms latency for policy violation detection and ensures cryptographic provenance.

Conclusion: GAAT effectively closes the loop between telemetry collection and real-time policy enforcement, enhancing governance in multi-agent systems.
...
### [195] [Nash Approximation Gap in Truncated Infinite-horizon Partially Observable Markov Games](https://arxiv.org/abs/2604.05131)
*Lan Sang,Chinmay Maheshwari*

Main category: cs.MA

TL;DR: A finite-memory truncation framework is proposed to approximate infinite-horizon Partially Observable Markov Games (POMGs), ensuring tractability and near-optimal equilibria.


<details>
  <summary>Details</summary>
Motivation: POMGs are used for multi-agent decision-making under asymmetric information, but their reformulation over belief states becomes intractable in infinite-horizon settings due to growing state and action spaces.

Method: The paper introduces a finite-memory truncation framework, where agents base decisions on finite windows of common and private information, approximating the original POMG as a finite-state, finite-action Markov game.

Result: Under filter stability conditions, Nash equilibria of the truncated game are shown to be ε-Nash equilibria of the original POMG, with ε diminishing as the truncation length increases.

Conclusion: The framework provides a tractable solution for infinite-horizon POMGs, offering practical applicability while maintaining equilibrium guarantees.
...
### [196] [A Theory-guided Weighted $L^2$ Loss for solving the BGK model via Physics-informed neural networks](https://arxiv.org/abs/2604.04971)
*Gyounghun Ko,Sung-Jun Son,Seung Yeon Cho,Myeong-Su Lee*

Main category: cs.LG

TL;DR: A velocity-weighted L² loss function is introduced for Physics-Informed Neural Networks (PINNs) to improve accuracy in solving the BGK model by focusing on high-velocity regions.


<details>
  <summary>Details</summary>
Motivation: The standard L² loss in PINNs fails to ensure accurate predictions of macroscopic moments in the BGK model, necessitating a better loss formulation.

Method: The paper proposes a velocity-weighted L² loss function, penalizing errors in high-velocity regions, and provides a stability estimate for this approach.

Result: Numerical experiments show the weighted loss improves accuracy and robustness over the standard L² loss in various benchmarks.

Conclusion: The proposed velocity-weighted loss ensures convergence and superior performance in solving the BGK model compared to the standard PINN approach.
...
### [197] [Territory Paint Wars: Diagnosing and Mitigating Failure Modes in Competitive Multi-Agent PPO](https://arxiv.org/abs/2604.04983)
*Diyansha Singh*

Main category: cs.LG

TL;DR: Territory Paint Wars is a competitive multi-agent reinforcement learning environment used to study PPO failures under self-play, uncovering implementation-level issues and emergent competitive overfitting, addressed via opponent mixing.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate and address failure modes of Proximal Policy Optimisation (PPO) in competitive multi-agent reinforcement learning.

Method: A minimal competitive environment (Territory Paint Wars) was created in Unity, with controlled ablations to identify implementation-level failures and emergent pathologies like competitive overfitting. Opponent mixing was introduced as a solution.

Result: Initial PPO training achieved only a 26.8% win rate. Correcting implementation issues improved performance, but competitive overfitting reduced generalization win rate to 21.6%. Opponent mixing restored generalization to 77.1%.

Conclusion: Competitive overfitting is a critical issue in self-play MARL, detectable only through generalization metrics, and can be mitigated by opponent mixing without complex infrastructure.
...
### [198] [Enhancing sample efficiency in reinforcement-learning-based flow control: replacing the critic with an adaptive reduced-order model](https://arxiv.org/abs/2604.04986)
*Zesheng Yao,Zhen-Hua Wan,Canjun Yang,Qingchao Xia,Mengqi Zhang*

Main category: cs.LG

TL;DR: A ROM-based DRL framework improves sample efficiency in active flow control by integrating a ROM for gradient estimation and adaptive refinement, outperforming traditional and DRL methods with less data.


<details>
  <summary>Details</summary>
Motivation: Model-free DRL methods are inefficient in sample use, prompting the development of a ROM-based approach to enhance efficiency and performance in flow control.

Method: The framework combines a linear dynamical system and a NODE in a ROM, updated continuously with new data, to optimize controllers via differentiable simulation.

Result: Effective control in Blasius boundary layer and superior drag reduction in square cylinder flow with minimal data, outperforming traditional and DRL methods.

Conclusion: The ROM-based DRL framework significantly improves sample efficiency, addressing a key limitation of model-free DRL methods in active flow control.
...
### [199] [Cactus: Accelerating Auto-Regressive Decoding with Constrained Acceptance Speculative Sampling](https://arxiv.org/abs/2604.04987)
*Yongchang Hao,Lili Mou*

Main category: cs.LG

TL;DR: The paper proposes Cactus, a method to improve speculative sampling by ensuring controlled divergence from the verifier distribution and higher acceptance rates.


<details>
  <summary>Details</summary>
Motivation: Current speculative sampling methods are overly restrictive, while typical acceptance sampling distorts the verifier distribution. There's a need for a balanced approach.

Method: The authors formalize speculative sampling as a constrained optimization problem and introduce Cactus, which guarantees controlled divergence and better acceptance rates.

Result: Empirical results demonstrate Cactus's effectiveness across various benchmarks.

Conclusion: Cactus provides a promising solution to the limitations of existing speculative sampling methods, balancing acceptance rates and distribution fidelity.
...
TL;DR: The paper proposes an ordered pipeline combining unstructured pruning, INT8 quantization-aware training, and knowledge distillation to improve accuracy-size-latency trade-offs for edge deployment.


<details>
  <summary>Details</summary>
Motivation: Existing compression proxies like parameter count or FLOPs don't reliably predict wall-clock inference time, especially with unstructured sparsity, which can slow down CPU execution.

Method: Uses a practical pipeline combining unstructured pruning, INT8 quantization-aware training (QAT), and knowledge distillation (KD) to target measured latency.

Result: The ordered pipeline outperforms single techniques, achieving 0.99-1.42 ms CPU latency with competitive accuracy and compact checkpoints on CIFAR-10/100 using ResNet-18, WRN-28-10, and VGG-16-BN.

Conclusion: The study suggests evaluating compression choices in the joint accuracy-size-latency space using measured runtime for effective edge deployment.
...
### [201] [Learning-Based Multi-Criteria Decision Making Model for Sawmill Location Problems](https://arxiv.org/abs/2604.04996)
*Mahid Ahmed,Ali Dogru,Chaoyang Zhang,Chao Meng*

Main category: cs.LG

TL;DR: The study presents a Learning-Based Multi-Criteria Decision-Making (LB-MCDM) framework combining ML and GIS for optimal sawmill location, with Random Forest Classifier performing best and Supply-Demand Ratio being the top factor.


<details>
  <summary>Details</summary>
Motivation: To improve efficiency, profitability, and sustainability in timber supply chains by strategically locating sawmills using a data-driven approach.

Method: A LB-MCDM framework integrates ML algorithms (e.g., Random Forest, XGBoost) with GIS-based spatial analysis and MCDM, validated via a case study in Mississippi.

Result: Random Forest Classifier performed best. SHAP analysis highlighted Supply-Demand Ratio as the most influential factor, with 10-11% of Mississippi deemed highly suitable for sawmills.

Conclusion: The LB-MCDM framework offers an unbiased, replicable solution for sawmill location, enhancing decision-making in timber supply chains.
...
### [202] [El Nino Prediction Based on Weather Forecast and Geographical Time-series Data](https://arxiv.org/abs/2604.04998)
*Viet Trinh,Ha-Vy Luu,Quoc-Khiem Nguyen-Pham,Hung Tong,Thanh-Huyen Tran,Hoai-Nam Nguyen Dang*

Main category: cs.LG

TL;DR: A novel framework combining real-time weather data and hybrid deep learning (CNN-LSTM) improves El Niño prediction accuracy and lead time.


<details>
  <summary>Details</summary>
Motivation: Enhance El Niño prediction to mitigate its global climatic, economic, and societal impacts, addressing limitations of traditional models.

Method: Integrates real-time global weather data, anomalies, ocean heat content, and pressure. Uses CNN for spatial features and LSTM for temporal dependencies.

Result: Framework captures complex precursors and evolving patterns of El Niño events.

Conclusion: Hybrid deep learning with comprehensive data integration offers superior El Niño prediction.
...
### [203] [PRIME: Prototype-Driven Multimodal Pretraining for Cancer Prognosis with Missing Modalities](https://arxiv.org/abs/2604.04999)
...
TL;DR: PRIME is a missing-aware multimodal self-supervised pretraining framework for cancer prognosis, integrating fragmented clinical data with robust performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods require fully paired inputs, but clinical data is often incomplete. PRIME addresses this by learning from partially observed cohorts.

Method: PRIME unifies modality embeddings, uses a shared prototype memory bank for latent-space imputation, and employs inter-modality alignment and post-fusion consistency objectives.

Result: PRIME outperforms others in survival prediction and classification tasks, achieving 0.653 C-index, 0.689 AUROC, and 0.637 AUROC, with robustness to missing data.

Conclusion: PRIME demonstrates the viability of missing-aware multimodal pretraining for prognosis in fragmented clinical data.
...
### [204] [Learning Stable Predictors from Weak Supervision under Distribution Shift](https://arxiv.org/abs/2604.05002)
*Mehrdad Shoeibi,Elias Hossain,Ivan Garibay,Niloofar Yousefi*

Main category: cs.LG

TL;DR: The paper explores robustness under supervision drift, focusing on CRISPR-Cas13d experiments with weak-label supervision. It highlights challenges in temporal transfer due to changing feature-label relationships.


<details>
  <summary>Details</summary>
Motivation: To understand robustness under distribution shift when learning from weak or proxy supervision, particularly when the supervision mechanism changes (supervision drift). The study is motivated by the lack of understanding of such scenarios.

Method: The authors formalize supervision drift as changes in P(y | x, c) across contexts. They use CRISPR-Cas13d experiments where guide efficacy is inferred indirectly from RNA-seq responses. A controlled non-IID benchmark is built with domain (cell lines) and temporal shifts.

Result: Models show strong in-domain performance (ridge R^2 = 0.356, Spearman rho = 0.442) and partial cross-cell-line transfer (rho ~ 0.40). However, temporal transfer fails (e.g., XGBoost R^2 = -0.155, rho = 0.056), attributed to supervision drift rather than model limitations.

Conclusion: Feature stability serves as a diagnostic for detecting non-transferability before deployment. The findings underscore the challenge of temporal transfer due to supervision drift.
...
### [205] [Energy-Based Dynamical Models for Neurocomputation, Learning, and Optimization](https://arxiv.org/abs/2604.05042)
*Arthur N. Montanari,Francesco Bullo,Dmitry Krotov,Adilson E. Motter*

Main category: cs.LG

TL;DR: The paper explores neuro-inspired computational approaches blending control theory, neuroscience, and machine learning to enhance scalability, robustness, and energy efficiency in dynamical systems.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between artificial and biological systems by leveraging energy-based dynamical models for improved computation in tasks like model learning, memory retrieval, and optimization.

Method: Reviews classical models (e.g., Hopfield networks, Boltzmann machines) and extends to modern developments like dense associative memory, oscillator-based networks, and proximal-descent dynamics.

Result: Demonstrates how control-theoretic principles can innovate neurocomputing systems, moving beyond traditional feedforward and backpropagation methods.

Conclusion: The tutorial highlights the potential of energy-based dynamical models to advance next-generation artificial intelligence systems.
...
### [206] [PCA-Driven Adaptive Sensor Triage for Edge AI Inference](https://arxiv.org/abs/2604.05045)
*Ankit Hemant Lade,Sai Krishna Jasti,Nikhil Sinha,Indar Kumar,Akanksha Tiwari*

Main category: cs.LG

TL;DR: PCA-Triage is a streaming algorithm for optimizing bandwidth in multi-channel sensor networks using incremental PCA loadings to set per-channel sampling rates, outperforming baselines and maintaining high performance under reduced bandwidth.


<details>
  <summary>Details</summary>
Motivation: Industrial IoT faces bandwidth constraints in multi-channel sensor networks, necessitating efficient data sampling methods to maximize utility within limited bandwidth.

Method: PCA-Triage converts incremental PCA loadings into proportional per-channel sampling rates under a fixed bandwidth budget, operating in O(wdk) time with no trainable parameters.

Result: PCA-Triage outperforms 9 baselines, achieving high performance (e.g., F1 = 0.961) at 50% bandwidth, and remains robust under packet loss and noise (3.7--4.8% degradation).

Conclusion: PCA-Triage is an effective, unsupervised solution for bandwidth optimization in sensor networks, offering near-full-data performance even under significant bandwidth reductions.
...
### [207] [Blind-Spot Mass: A Good-Turing Framework for Quantifying Deployment Coverage Risk in Machine Learning Systems](https://arxiv.org/abs/2604.05057)
*Biplab Pal,Santanu Bhattacharya,Madanjit Singh*

Main category: cs.LG

TL;DR: Blind-spot mass B_n(tau) quantifies ML deployment coverage risk by estimating the probability mass of under-supported states, validated across domains like HAR and clinical data.


<details>
  <summary>Details</summary>
Motivation: Modern ML systems often have heavy-tailed operational state distributions, leading to 'coverage blindness' where models appear accurate but fail in rare states.

Method: Proposes B_n(tau) using Good-Turing unseen-species estimation to measure under-supported states and a coverage-imposed accuracy ceiling to separate data and capacity limits.

Result: Validated in wearable HAR and MIMIC-IV clinical data, showing blind-spot mass generalizes across domains and identifies risk-dominant regimes.

Conclusion: Blind-spot decomposition offers actionable insights for safer ML deployment, guiding targeted data collection and domain-specific constraints.
...
### [208] [Dynamic Linear Coregionalization for Realistic Synthetic Multivariate Time Series](https://arxiv.org/abs/2604.05064)
...
TL;DR: DynLMC introduces dynamic inter-channel correlations in synthetic data for time series models, improving zero-shot forecasting performance.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic data generators for time series lack realistic dynamic correlations and inter-channel dependencies.

Method: DynLMC, a Dynamic Linear Model of Coregionalization, incorporates time-varying, regime-switching correlations and cross-channel lag structures.

Result: Fine-tuning foundational models on DynLMC-generated data yields consistent zero-shot forecasting improvements across nine benchmarks.

Conclusion: Modeling dynamic inter-channel correlations enhances the transferability of foundation models for time series, emphasizing data-centric pretraining.
...
### [209] [Towards Scaling Law Analysis For Spatiotemporal Weather Data](https://arxiv.org/abs/2604.05068)
...
TL;DR: HiVG introduces a hierarchical SVG tokenization framework for autoregressive vector graphics generation, improving fidelity, spatial consistency, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing SVG generation methods use generic byte-level tokenization, which poorly reflects geometric structure, leading to coordinate hallucination and inefficiency.

Method: HiVG decomposes SVG strings into atomic tokens and compresses command-parameter groups into segment tokens. It also uses Hierarchical Mean--Noise (HMN) initialization and curriculum training.

Result: HiVG improves generation fidelity, spatial consistency, and sequence efficiency in text-to-SVG and image-to-SVG tasks.

Conclusion: HiVG's tailored tokenization and training strategies outperform conventional approaches in SVG generation.
...
### [211] [Feature-Aware Anisotropic Local Differential Privacy for Utility-Preserving Graph Representation Learning in Metal Additive Manufacturing](https://arxiv.org/abs/2604.05077)
*MD Shafikul Islam,Mahathir Mohammad Bappy,Saifur Rahman Tushar,Md Arifuzzaman*

Main category: cs.LG

TL;DR: FI-LDP-HGAT combines a hierarchical graph attention network and a feature-importance-aware privacy mechanism to improve defect detection in metal AM while preserving data utility under privacy constraints.


<details>
  <summary>Details</summary>
Motivation: The need for reliable quality assurance in metal AM faces challenges due to proprietary sensor data and the limitations of existing privacy-preserving techniques that degrade utility.

Method: The paper proposes FI-LDP-HGAT, integrating a stratified HGAT for capturing layer-wise dependencies and an anisotropic Gaussian mechanism (FI-LDP) for feature privatization.

Result: FI-LDP-HGAT achieves 81.5% utility recovery at a moderate privacy budget (epsilon=4) and maintains high defect recall (0.762) under strict privacy (epsilon=2), outperforming baseline methods.

Conclusion: FI-LDP-HGAT provides a principled, interpretable solution for balancing privacy and utility in metal AM defect detection, validated by mechanistic analysis.
...
### [212] [Vintix II: Decision Pre-Trained Transformer is a Scalable In-Context Reinforcement Learner](https://arxiv.org/abs/2604.05112)
...
TL;DR: Extending the Decision Pre-Trained Transformer (DPT) to multi-domain environments with Flow Matching improves generalization and outperforms prior methods like Algorithm Distillation in in-context reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance the generalization and scalability of in-context reinforcement learning (ICRL) by improving the Decision Pre-Trained Transformer (DPT), addressing limitations of prior methods like Algorithm Distillation (AD).

Method: The authors extend DPT to diverse multi-domain environments using Flow Matching, preserving its Bayesian posterior sampling interpretation, and train the agent across hundreds of tasks.

Result: The extended DPT achieves better generalization on held-out test tasks, outperforming prior AD scaling and showing stronger performance in both online and offline inference.

Conclusion: This work reinforces ICRL as a viable alternative to expert distillation for training generalist agents, with improved scalability and generalization.
...
### [213] [Reasoning Through Chess: How Reasoning Evolves from Data Through Fine-Tuning and Reinforcement Learning](https://arxiv.org/abs/2604.05134)
*Lucas Dionisopoulos,Nicklas Majamaki,Prithviraj Ammanabrolu*

Main category: cs.LG

TL;DR: Fine-tuning a model to predict the best move in chess leads to effective reinforcement learning (RL) but unfaithful reasoning. Multi-move trajectory training yields comparable performance with faithful reasoning and stable RL. RL improves move quality and reduces hallucination.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how reasoning evolves in language models from supervised fine-tuning (SFT) to reinforcement learning (RL), specifically in the context of chess, to improve model performance and reasoning fidelity.

Method: The research involves fine-tuning a language model on chess-related datasets, comparing direct best-move prediction with multi-move trajectory training, and analyzing the impact of RL on reasoning quality and move selection.

Result: Direct best-move prediction leads to effective RL but unfaithful reasoning, while multi-move trajectory training achieves comparable performance with faithful reasoning. RL improves move quality and reduces hallucination rates.

Conclusion: The study demonstrates the trade-offs between reasoning fidelity and performance in language models trained for chess, highlights the benefits of RL, and identifies SFT-checkpoint metrics as predictors of post-RL performance. The released resources enable surpassing leading open-source reasoning models in chess.
...
### [214] [Not All Turns Are Equally Hard: Adaptive Thinking Budgets For Efficient Multi-Turn Reasoning](https://arxiv.org/abs/2604.05164)
*Neharika Jali,Anupam Nayak,Gauri Joshi*

Main category: cs.LG

TL;DR: TAB (Turn-Adaptive Budgets) improves LLM multi-turn reasoning efficiency by adaptively allocating tokens based on conversation history, saving up to 35-40% tokens while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Addressing inefficiencies in multi-turn LLM reasoning due to overthinking and long traces for simple queries, which prior single-turn methods fail to handle.

Method: Formulates multi-turn reasoning as a sequential compute allocation problem using a multi-objective MDP, training TAB with GRPO to optimize token budgets per turn.

Result: TAB achieves better accuracy-token tradeoff, saving 35% tokens; TAB All-SubQ saves up to 40% tokens when future sub-questions are known.

Conclusion: TAB and TAB All-SubQ effectively optimize token usage in multi-turn reasoning, outperforming static and baseline methods.
...
### [215] [General Multimodal Protein Design Enables DNA-Encoding of Chemistry](https://arxiv.org/abs/2604.05181)
...
TL;DR: DISCO uses a multimodal deep generative model to co-design protein sequences and 3D structures for novel enzymes, achieving high catalytic activity in new-to-nature reactions.


<details>
  <summary>Details</summary>
Motivation: To expand the scope of genetically encodable enzymatic transformations beyond what natural evolution has achieved.

Method: Introduces DISCO, a model that co-designs protein sequence and 3D structure around arbitrary biomolecules, using inference-time scaling methods for optimization.

Result: DISCO designed diverse heme enzymes with novel geometries, catalyzing new-to-nature reactions like cyclopropanation and insertions with high activity.

Conclusion: DISCO provides a scalable method to create evolvable enzymes, broadening the potential for genetically encoded chemistry.
...
### [216] [Cross-fitted Proximal Learning for Model-Based Reinforcement Learning](https://arxiv.org/abs/2604.05185)
...
### [217] [FNO$^{\angle θ}$: Extended Fourier neural operator for learning state and optimal control of distributed parameter systems](https://arxiv.org/abs/2604.05187)
*Zhexian Li,Ketan Savla*

Main category: cs.LG

TL;DR: The paper proposes an extended Fourier neural operator (FNO) for learning optimal control of PDE-governed systems, using complex domain frequency variables to improve accuracy and training efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the Fourier neural operator (FNO) by incorporating insights from the Ehrenpreis-Palamodov fundamental principle, which allows representing state and optimal control of linear PDEs as complex domain integrals, improving learning and control accuracy.

Method: The method involves extending the FNO layer by modifying the frequency variable in the inverse Fourier transform from real to complex domain, leveraging the integral representation from the fundamental principle.

Result: The extended FNO demonstrates significant improvements in training errors (order of magnitude) and more accurate predictions for non-periodic boundary values, tested on the nonlinear Burgers' equation.

Conclusion: The study concludes that the extended FNO architecture effectively captures the integral representation of PDE solutions, outperforming the original FNO in learning and control tasks for PDE-governed systems.
...
### [218] [On the Geometry of Positional Encodings in Transformers](https://arxiv.org/abs/2604.05217)
*Giansalvo Cirrincione*

Main category: cs.LG

TL;DR: This paper develops a theoretical framework for positional encodings in neural language models, proving their necessity, optimality conditions, and practical implementation via multidimensional scaling.


<details>
  <summary>Details</summary>
Motivation: Positional encodings are crucial for word order sensitivity in Transformers, but lack a clear mathematical foundation. This paper aims to provide such a theory and improve encoding design.

Method: The study establishes four key theorems and propositions, including the necessity of positional signals and a method for optimal encoding via multidimensional scaling. Experiments validate the theory using BERT-base on SST-2 and IMDB datasets.

Result: The paper proves that optimal positional encodings can be constructed using classical MDS, achieving low stress and minimal parametrization. ALiBi outperforms sinusoidal and RoPE encodings in experiments.

Conclusion: The research provides a rigorous mathematical framework for positional encodings, demonstrating their necessity and offering practical guidelines for optimal design, validated by experimental results.
...
### [219] [Curvature-Aware Optimization for High-Accuracy Physics-Informed Neural Networks](https://arxiv.org/abs/2604.05230)
...
TL;DR: Advanced optimization strategies for physics-informed neural networks (PINNs) to solve complex differential equations, featuring new methods and rigorous comparisons.


<details>
  <summary>Details</summary>
Motivation: To enhance the efficiency and robustness of neural networks in solving challenging differential equations, enabling rapid convergence and high accuracy.

Method: Implementation of Natural Gradient, Self-Scaling BFGS, and Broyden optimizers, along with new PINN-based methods for specific equations, tested on various problems and compared with high-order numerical methods.

Result: Demonstrated improved convergence and efficiency for solving PDEs and ODEs, with rigorous validation against established numerical methods.

Conclusion: The proposed optimization strategies and PINN-based methods offer scalable and efficient solutions for large data-driven problems in scientific machine learning.
...
### [220] [Extending Tabular Denoising Diffusion Probabilistic Models for Time-Series Data Generation](https://arxiv.org/abs/2604.05257)
...
### [221] [Jeffreys Flow: Robust Boltzmann Generators for Rare Event Sampling via Parallel Tempering Distillation](https://arxiv.org/abs/2604.05303)
*Guang Lin,Christian Moya,Di Qi,Xuda Ye*

Main category: cs.LG

TL;DR: The paper introduces Jeffreys Flow, a generative framework to address mode collapse in Boltzmann generators by using Jeffreys divergence, improving accuracy and scalability in complex systems.


<details>
  <summary>Details</summary>
Motivation: Sampling rough energy landscapes is challenging due to rare events and metastable trapping. Boltzmann generators suffer from mode collapse, which limits their effectiveness.

Method: The proposed Jeffreys Flow framework uses the symmetric Jeffreys divergence to distill empirical data from Parallel Tempering trajectories, balancing precision and mode coverage.

Result: Jeffreys Flow suppresses mode collapse and corrects inaccuracies, demonstrating scalability and accuracy in non-convex benchmarks and applications like stochastic gradient biases and quantum thermal states.

Conclusion: Jeffreys Flow offers a robust solution for sampling complex systems, overcoming limitations of existing methods and enhancing performance in multi-modal distributions.
...
### [222] [LLMs Should Express Uncertainty Explicitly](https://arxiv.org/abs/2604.05306)
*Junyu Guo,Shangding Gu,Ming Jin,Costas Spanos,Javad Lavaei*

Main category: cs.LG

TL;DR: The paper explores training large language models to express uncertainty as a control signal, comparing global confidence verbalization and local uncertainty markers during reasoning, showing complementary benefits for decision-making.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve decision-making in LLMs by treating uncertainty as a trainable signal rather than a latent quantity, enhancing tasks like abstention, retrieval, and verification.

Method: Two interfaces are compared: a global one where the model verbalizes calibrated confidence for its final answer, and a local one where it emits an <uncertain> marker during high-risk reasoning states.

Result: Verbalized confidence improves calibration and reduces errors, while local signaling makes failures visible during generation and improves retrieval triggers. The two interfaces work differently internally.

Conclusion: Effective uncertainty in LLMs should be trained as task-matched communication: global confidence for trusting final answers and local signals for intervention needs.
...
### [223] [A Theoretical Framework for Statistical Evaluability of Generative Models](https://arxiv.org/abs/2604.05324)
*Shashaank Aiyer,Yishay Mansour,Shay Moran,Han Shao*

Main category: cs.LG

TL;DR: The paper introduces a theoretical framework for evaluating generative models, focusing on test-based metrics and Rényi divergences, and discusses their evaluability from finite samples.


<details>
  <summary>Details</summary>
Motivation: Evaluation of generative models is challenging due to their open-ended nature and unclear appropriate metrics. This work aims to provide a theoretical foundation for such evaluations.

Method: The study categorizes metrics into test-based metrics (e.g., IPMs) and Rényi divergences, analyzing their evaluability from finite samples.

Result: IPMs with bounded test classes can be evaluated up to approximation errors, while Rényi and KL divergences are not evaluable due to dependence on rare events. Perplexity's role is also analyzed.

Conclusion: The framework clarifies which metrics can be reliably evaluated for generative models, highlighting the limitations of Rényi divergences and the potential of IPMs.
...
### [224] [Cross-Machine Anomaly Detection Leveraging Pre-trained Time-series Model](https://arxiv.org/abs/2604.05335)
*Yangmeng Li,Kei Sano,Toshihiro Kitao,Ryoji Anzaki,Yukiya Saitoh,Hironori Moki,Dragan Djurdjanovic*

Main category: cs.LG

TL;DR: A framework combining a domain-invariant feature extractor with unsupervised anomaly detection improves cross-machine anomaly detection in manufacturing.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of detecting anomalies in machines with similar processes but differing behaviors using sensory data.

Method: Proposes a framework integrating a feature extractor (using MOMENT and Random Forest Classifiers) to disentangle machine and condition features, followed by unsupervised anomaly detection.

Result: Outperforms raw-signal and MOMENT-embedding baselines in experiments on industrial data from three machines.

Conclusion: The framework effectively enhances cross-machine generalization for anomaly detection.
...
### [225] [LMI-Net: Linear Matrix Inequality--Constrained Neural Networks via Differentiable Projection Layers](https://arxiv.org/abs/2604.05374)
*Sunbochen Tang,Andrea Goertzen,Navid Azizan*

Main category: cs.LG

TL;DR: LMI-Net is a modular differentiable projection layer that enforces linear matrix inequality (LMI) constraints, ensuring stability and robustness in learning-based control design.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based methods often fail to preserve hard LMI constraints required for formal guarantees in dynamical systems, limiting their reliability.

Method: LMI-Net lifts LMI constraints into an affine equality and positive semidefinite cone, using Douglas-Rachford splitting for forward pass and implicit differentiation for backward propagation.

Result: LMI-Net ensures feasibility under distribution shift and retains fast inference, outperforming soft-constrained models in tasks like invariant ellipsoid synthesis and controller design.

Conclusion: LMI-Net bridges semidefinite-program-based certification with modern learning, providing reliable models that satisfy LMI constraints for dynamical systems.
...
### [226] [Training Without Orthogonalization, Inference With SVD: A Gradient Analysis of Rotation Representations](https://arxiv.org/abs/2604.05414)
*Chris Choy*

Main category: cs.LG

TL;DR: The paper analyzes why SVD orthogonalization harms training and why Gram-Schmidt is less effective than 9D representations at inference, providing gradient analysis for 3×3 matrices and SO(3) projection.


<details>
  <summary>Details</summary>
Motivation: To understand why SVD orthogonalization negatively impacts training and why 9D representations with SVD projection are preferred over Gram-Schmidt at inference.

Method: Detailed gradient analysis of SVD orthogonalization for 3×3 matrices and SO(3) projection, including deriving the exact spectrum of the SVD backward pass Jacobian.

Result: SVD introduces quantifiable gradient distortion, especially early in training, while stabilized SVD still causes gradient direction error. Gram-Schmidt Jacobian's asymmetric spectrum explains the preference for 9D parameterization.

Conclusion: Training with direct 9D regression and applying SVD projection only at inference is theoretically justified, avoiding gradient issues during training.
...
### [227] [ALTO: Adaptive LoRA Tuning and Orchestration for Heterogeneous LoRA Training Workloads](https://arxiv.org/abs/2604.05426)
...
### [228] [Top-K Retrieval with Fixed-Size Linear-Attention Completion: Backbone- and KV-Format-Preserving Attention for KV-Cache Read Reduction](https://arxiv.org/abs/2604.05438)
*Yasuto Hoshi,Daisuke Miyashita,Jun Deguchi*

Main category: cs.LG

TL;DR: The paper proposes a retrieval-completion attention module to reduce KV cache traffic in long-context generation by combining exact attention over selected tokens with estimated attention for the rest, improving performance over Top-K methods.


<details>
  <summary>Details</summary>
Motivation: Long-context generation faces limitations due to KV cache traffic, especially when KV is offloaded beyond GPU memory. Existing methods like Top-K retrieval reduce traffic but introduce bias by ignoring unretrieved tokens.

Method: The proposed retrieval-completion attention module retains backbone weights and KV-cache format. It computes exact attention over anchors and retrieved Top-K tokens, estimates the rest using a prefill-time feature-map summary, and combines them with a single normalization.

Result: The method outperforms Top-K selection at matched read budgets, with significant gains in high-entropy heads across long-context benchmarks.

Conclusion: The approach effectively reduces KV cache traffic while minimizing bias, offering improved performance for long-context generation tasks.
...
### [229] [Reproducing AlphaZero on Tablut: Self-Play RL for an Asymmetric Board Game](https://arxiv.org/abs/2604.05476)
*Tõnis Lees,Tambet Matiisen*

Main category: cs.LG

TL;DR: Modified AlphaZero for Tablut, achieving stable learning and performance by using separate policy/value heads and stabilization techniques.


<details>
  <summary>Details</summary>
Motivation: Adapt AlphaZero to asymmetric games like Tablut, where original architecture struggles due to conflicting evaluation functions for unequal player objectives.

Method: Introduced separate policy and value heads for each player role, shared residual trunk, C4 augmentation, larger replay buffer, and playing against past checkpoints.

Result: Improved performance with a BayesElo rating of 1235, reduced policy entropy, and fewer remaining pieces, showing focused play.

Conclusion: AlphaZero can be adapted to asymmetric games with distinct heads and stabilization methods, ensuring effective learning and performance.
...
### [230] [Channel-wise Retrieval for Multivariate Time Series Forecasting](https://arxiv.org/abs/2604.05543)
*Junhyeok Kang,Jun Seo,Soyeon Park,Sangjun Han,Seohui Bae,Hyeokjun Choe,Soonyoung Lee*

Main category: cs.LG

TL;DR: CRAFT introduces a channel-wise retrieval-augmented forecasting framework to address inter-variable heterogeneity in multivariate time series forecasting, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing retrieval-augmented forecasting methods use a channel-agnostic strategy, ignoring the distinct periodicities and spectral profiles of different channels, which limits performance.

Method: CRAFT retrieves historical segments independently for each channel, using a two-stage pipeline: a sparse relation graph prunes irrelevant candidates, and spectral similarity ranks references to emphasize dominant periodic components.

Result: CRAFT outperforms state-of-the-art forecasting baselines on seven public benchmarks, achieving superior accuracy with practical inference efficiency.

Conclusion: By addressing inter-variable heterogeneity and leveraging spectral analysis, CRAFT sets a new standard for retrieval-augmented forecasting in multivariate time series.
...
### [231] [Same Graph, Different Likelihoods: Calibration of Autoregressive Graph Generators via Permutation-Equivalent Encodings](https://arxiv.org/abs/2604.05613)
*Laurits Fredsgaard,Aaron Thomas,Michael Riis Andersen,Mikkel N. Schmidt,Mahito Sugiyama*

Main category: cs.LG

TL;DR: The paper introduces Linearization Uncertainty (LU) to measure inconsistencies in likelihoods across graph linearizations, showing that biased training linearizations lead to poor calibration under permutation, and LU better correlates with molecular stability than NLL.


<details>
  <summary>Details</summary>
Motivation: To address inconsistencies in likelihoods assigned by autoregressive graph generators across different linearizations of the same graph, and to evaluate the reliability of these likelihoods.

Method: Uses Segmented Eulerian Neighborhood Trails (SENT) for graph linearization, quantifies violations in NLL as LU, and evaluates transformers trained under four linearization strategies on two datasets.

Result: Biased orderings achieve lower NLL but high ECE under permutation. LU outperforms NLL in correlating with molecular stability (AUC 0.85 vs. 0.43).

Conclusion: Permutation-based evaluation with LU provides a more reliable metric for assessing graph generation quality, particularly for molecular stability.
...
### [232] [From Uniform to Learned Knots: A Study of Spline-Based Numerical Encodings for Tabular Deep Learning](https://arxiv.org/abs/2604.05635)
*Manish Kumar,Anton Frederik Thielmann,Christoph Weisser,Benjamin Säfken*

Main category: cs.LG

TL;DR: The paper explores the impact of spline-based numerical encodings on tabular deep learning, comparing B-splines, M-splines, and I-splines with various knot-placement strategies. Results show performance varies by task, backbone, and output size, with learnable knots increasing training costs.


<details>
  <summary>Details</summary>
Motivation: To understand the role and effectiveness of explicit numerical preprocessing in tabular deep learning, particularly focusing on spline-based encodings.

Method: Investigates B-splines, M-splines, and I-splines with uniform, quantile-based, target-aware, and learnable-knot placement. Evaluates on regression and classification datasets using MLP, ResNet, and FT-Transformer backbones.

Result: Performance depends on task, output size, and backbone. PLE is robust for classification, while no single encoding dominates regression. Learnable knots increase training costs but can be optimized.

Conclusion: Numerical encodings' effectiveness varies by context, requiring assessment of both predictive performance and computational overhead.
...
### [233] [Optimal-Transport-Guided Functional Flow Matching for Turbulent Field Generation in Hilbert Space](https://arxiv.org/abs/2604.05700)
*Li Kunpeng,Wan Chenguang,Qu Zhisong,Lim Kyungtak,Virginie Grandgirard,Xavier Garbet,Yu Hua,Ong Yew Soon*

Main category: cs.LG

TL;DR: FOT-CFM is a novel generative framework for turbulence modeling in infinite-dimensional function space, outperforming traditional methods in fidelity.


<details>
  <summary>Details</summary>
Motivation: Traditional turbulence modeling struggles with spatiotemporal dynamics and multi-scale intermittency. Pixel-based generative models are limited, necessitating a function-space approach.

Method: Proposed Functional Optimal Transport Conditional Flow Matching (FOT-CFM), a resolution-invariant framework using Hilbert space and OT theory for deterministic probability paths.

Result: FOT-CFM excels in reproducing high-order turbulent statistics and energy spectra across chaotic systems like Navier-Stokes equations.

Conclusion: FOT-CFM bridges the gap in turbulence computing, offering superior fidelity and faster sampling compared to existing methods.
...
### [234] [Controllable Image Generation with Composed Parallel Token Prediction](https://arxiv.org/abs/2604.05730)
*Jamie Stirling,Noura Al-Moubayed,Chris G. Willcocks,Hubert P. H. Shum*

Main category: cs.LG

TL;DR: The paper introduces a theoretically-grounded method for composing discrete probabilistic generative processes, improving fidelity in conditional generation and offering faster, more flexible control over input conditions.


<details>
  <summary>Details</summary>
Motivation: Existing conditional discrete generative models fail to faithfully compose multiple input conditions, limiting their practical utility.

Method: The authors derive a theoretical formulation for composing discrete generative processes, with masked generation as a case study. It supports novel condition combinations and emphasizes/negates individual conditions.

Result: The method reduces error rates by 63.4% and improves FID by -9.58 across datasets (CLEVR, FFHQ), while being 2.3x to 12x faster than comparable methods. It also enhances fine-grained control in text-to-image models.

Conclusion: The proposed method significantly advances conditional generative modeling, offering better performance, speed, and flexibility in handling diverse input conditions.
...
### [235] [Graph Topology Information Enhanced Heterogeneous Graph Representation Learning](https://arxiv.org/abs/2604.05732)
*He Zhao,Zhiwei Zeng,Yongwei Wang,Chunyan Miao*

Main category: cs.LG

TL;DR: ToGRL is a novel framework for Graph Structure Learning (GSL) in heterogeneous graphs, addressing challenges like noise impact and memory consumption by learning task-relevant topology embeddings and leveraging prompt tuning for better downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Existing GSL methods are designed for homogeneous graphs, leaving heterogeneous graphs underexplored. Noise and suboptimal structures in real-world heterogeneous graphs adversely affect GRL model performance.

Method: ToGRL uses a two-stage GSL module to extract task-relevant topology embeddings, constructs a new graph, and employs a representation learning module with prompt tuning for downstream tasks.

Result: ToGRL outperforms state-of-the-art methods on five real-world datasets, demonstrating improved performance and adaptability.

Conclusion: ToGRL successfully addresses GSL challenges in heterogeneous graphs, offering high-quality structures and representations for downstream tasks.
...
### [236] [Bivariate Causal Discovery Using Rate-Distortion MDL: An Information Dimension Approach](https://arxiv.org/abs/2604.05829)
*Tiago Brogueira,Mário A. T. Figueiredo*

Main category: cs.LG

TL;DR: The paper critiques current MDL-based bivariate causal discovery methods for inadequately estimating the description length of the cause variable and proposes a new method, RDMDL, which uses rate-distortion theory to address this gap.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve causal discovery methods by addressing the shortcomings of existing MDL-based approaches, particularly in estimating the description length of the cause variable.

Method: They propose RDMDL, a new method that measures the description length of the cause using rate-distortion theory and combines it with traditional causal mechanism description length estimation.

Result: Experimental results show that RDMDL performs competitively on the Tübingen dataset.

Conclusion: The paper successfully introduces a more robust causal discovery method by better estimating the description length of the cause variable, validated by empirical performance.
...
### [237] [Hidden in the Multiplicative Interaction: Uncovering Fragility in Multimodal Contrastive Learning](https://arxiv.org/abs/2604.05834)
*Tillmann Rheude,Stefan Hegselmann,Roland Eils,Benjamin Wild*

Main category: cs.LG

TL;DR: Gated Symile improves multimodal contrastive learning by adaptively gating unreliable modalities, outperforming Symile and CLIP in retrieval accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods like Symile treat all modalities equally, ignoring reliability differences, which can degrade performance when modalities are misaligned or missing.

Method: Gated Symile introduces a contrastive gating mechanism that adjusts modality contributions per-candidate, suppressing unreliable inputs by interpolating embeddings toward neutral directions or NULL.

Result: Gated Symile achieves higher top-1 retrieval accuracy than Symile and CLIP on synthetic and real-world trimodal datasets.

Conclusion: Gated Symile demonstrates the importance of adaptive gating for robust multimodal contrastive learning, especially with imperfect or more than two modalities.
...
### [238] [Expectation Maximization (EM) Converges for General Agnostic Mixtures](https://arxiv.org/abs/2604.05842)
*Avishek Ghosh*

Main category: cs.LG

TL;DR: The paper extends gradient EM for fitting k parametric functions in an agnostic setting, showing exponential convergence to optimal solutions under proper conditions.


<details>
  <summary>Details</summary>
Motivation: To generalize the mixed linear regression problem by fitting k arbitrary parametric functions with strongly convex and smooth loss, addressing diverse applications like mixed classifiers and generalized linear regression.

Method: Proposes and analyzes gradient EM for fitting parametric functions, ensuring convergence with proper initialization and separation conditions.

Result: Demonstrates that gradient EM converges exponentially to population loss minimizers with high probability, even in non-generative settings.

Conclusion: Gradient EM is effective for optimal solutions in agnostic setups beyond mixed linear regression, applicable to a broad class of problems.
...
### [239] [EEG-MFTNet: An Enhanced EEGNet Architecture with Multi-Scale Temporal Convolutions and Transformer Fusion for Cross-Session Motor Imagery Decoding](https://arxiv.org/abs/2604.05843)
*Panagiotis Andrikopoulos,Siamak Mehrkanoon*

Main category: cs.LG

TL;DR: EEG-MFTNet, a deep learning model combining EEGNet with multi-scale temporal convolutions and a Transformer encoder, improves motor imagery decoding from EEG signals, achieving 58.9% accuracy.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of accurately decoding motor imagery from EEG due to noise and variability, aiming to enhance brain-computer interfaces for motor-impaired individuals.

Method: EEG-MFTNet integrates EEGNet with multi-scale temporal convolutions and a Transformer encoder to capture short and long-range EEG signal dependencies, evaluated on the SHU dataset.

Result: EEG-MFTNet outperforms baseline models with 58.9% accuracy and maintains low computational complexity, demonstrating potential for real-time BCI applications.

Conclusion: The study highlights EEG-MFTNet's effectiveness in improving motor imagery decoding, advancing robust and adaptive BCI systems for assistive technologies and neurorehabilitation.
...
### [240] [Modeling Patient Care Trajectories with Transformer Hawkes Processes](https://arxiv.org/abs/2604.05844)
*Saumya Pandey,Varun Chandola*

Main category: cs.LG

TL;DR: The paper proposes a Transformer Hawkes Process framework to model irregular patient healthcare trajectories, addressing class imbalance with inverse square-root weighting.


<details>
  <summary>Details</summary>
Motivation: Patient healthcare trajectories are irregular and imbalanced, making modeling and prediction challenging. Understanding these patterns is crucial for predicting future care needs.

Method: Combines Transformer-based history encoding with Hawkes process dynamics to model event dependencies and predict event type and time-to-event. Introduces inverse square-root class weighting to handle imbalance.

Result: Demonstrates improved performance on real-world data, providing clinically meaningful insights for identifying high-risk patients.

Conclusion: The framework effectively models patient trajectories and addresses class imbalance, offering practical utility in healthcare.
...
### [241] [Weight-Informed Self-Explaining Clustering for Mixed-Type Tabular Data](https://arxiv.org/abs/2604.05857)
*Lehao Li,Qiang Huang,Yihao Ang,Bryan Kian Hsiang Low,Anthony K. H. Tung,Xiaokui Xiao*

Main category: cs.LG

TL;DR: WISE is a framework for clustering mixed-type tabular data, unifying representation, feature weighting, clustering, and interpretation in an unsupervised, transparent pipeline. It outperforms baselines in clustering quality and provides interpretable explanations.


<details>
  <summary>Details</summary>
Motivation: Clustering mixed-type data is challenging due to misaligned representations, uneven feature relevance, and disconnected explanations. WISE addresses these issues by integrating representation, weighting, clustering, and interpretation.

Method: WISE uses Binary Encoding with Padding (BEP) for aligned feature representation, Leave-One-Feature-Out (LOFO) for diverse feature-weighting views, and a two-stage weight-aware clustering procedure. It also employs Discriminative FreqItems (DFI) for interpretable explanations.

Result: WISE outperforms classical and neural baselines in clustering quality on six real-world datasets while remaining efficient and producing faithful, human-interpretable explanations.

Conclusion: WISE provides a unified, interpretable, and effective solution for clustering mixed-type tabular data, addressing representation, weighting, and explanation challenges.
...
### [242] [ReLU Networks for Exact Generation of Similar Graphs](https://arxiv.org/abs/2604.05929)
...
TL;DR: A foundation model for SEM images is introduced, pretrained on diverse scientific micrographs, enabling generalization and outperforming state-of-the-art techniques in defocus-to-focus translation.


<details>
  <summary>Details</summary>
Motivation: SEM imaging is constrained by task-specific models and labor-intensive processes, limiting scalability. The goal is to develop a versatile foundation model to address these limitations.

Method: A self-supervised transformer architecture is used to pretrain on multi-instrument, multi-condition SEM images, learning transferable representations for fine-tuning on downstream tasks like defocus-to-focus translation.

Result: The model successfully restores focused details from defocused inputs without paired supervision and outperforms existing methods in evaluation metrics.

Conclusion: This work establishes adaptable SEM models, combining foundational representation learning with real-world imaging needs to accelerate materials discovery.
...
### [244] [On Dominant Manifolds in Reservoir Computing Networks](https://arxiv.org/abs/2604.05967)
*Noa Kaplan,Alberto Padoan,Anastasia Bizyaeva*

Main category: cs.LG

TL;DR: This paper explores how training shapes the geometry of recurrent network dynamics in Reservoir Computing (RC) for forecasting, linking dominant modes to data's intrinsic dimensionality and connecting RC to Koopman eigenfunctions and Dynamic Mode Decomposition.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how low-dimensional dominant manifolds emerge in RC networks for temporal forecasting, bridging gaps between RC, Koopman theory, and Dynamic Mode Decomposition.

Method: The research uses a simplified linear and continuous-time reservoir model, analyzing dimensionality and structure of dominant modes in relation to training data. It includes simulations and discusses nonlinear RC generalization.

Result: Dominant modes of the trained reservoir correlate with approximations of Koopman eigenfunctions, revealing a connection between RC and Dynamic Mode Decomposition.

Conclusion: The work clarifies the relationship between RC training and the geometry of network dynamics, offering insights into forecasting and nonlinear extensions.
...
### [245] [Data Distribution Valuation Using Generalized Bayesian Inference](https://arxiv.org/abs/2604.05993)
*Cuong N. Nguyen,Cuong V. Nguyen*

Main category: cs.LG

TL;DR: A novel framework, Generalized Bayes Valuation, is introduced to quantify data distribution values using generalized Bayesian inference and transferability measures, proving effective in various applications like annotator evaluation and data augmentation.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the data distribution valuation problem, a recent challenge distinct from classical data valuation, with applications in diverse fields.

Method: The proposed framework, Generalized Bayes Valuation, uses generalized Bayesian inference and transferability-based loss functions to unify solutions for practical problems like annotator evaluation and data augmentation. It is also extended to continuous data streams.

Result: Experiments demonstrate the framework's effectiveness and efficiency across different real-world scenarios.

Conclusion: The Generalized Bayes Valuation framework successfully addresses the data distribution valuation problem, offering a unified approach for practical applications and scalability to continuous data streams.
...
### [246] [Gated-SwinRMT: Unifying Swin Windowed Attention with Retentive Manhattan Decay via Input-Dependent Gating](https://arxiv.org/abs/2604.06014)
*Dipan Maity,Suman Mondal,Arindam Roy*

Main category: cs.LG

TL;DR: Gated-SwinRMT introduces hybrid vision transformers combining Swin Transformer's shifted-window attention with Retentive Networks' spatial decay, using input-dependent gating for improved performance.


<details>
  <summary>Details</summary>
Motivation: To enhance vision transformers by integrating shifted-window attention with spatial decay and input-dependent gating, addressing limitations like low-rank bottlenecks and uninformative attention scores.

Method: Proposes two variants: Gated-SwinRMT-SWAT (sigmoid activation, ALiBi slopes, SwiGLU gating) and Gated-SwinRMT-Retention (softmax retention, G1 sigmoid gate). Evaluated on Mini-ImageNet and CIFAR-10.

Result: Gated-SwinRMT-SWAT achieved 80.22% and Gated-SwinRMT-Retention 78.20% top-1 accuracy on Mini-ImageNet, outperforming the RMT baseline (73.74%). On CIFAR-10, the advantage reduced to +0.56pp.

Conclusion: Gated-SwinRMT variants outperform baselines, with effectiveness varying by dataset scale, demonstrating the potential of hybrid attention mechanisms in vision transformers.
...
### [247] [PromptEvolver: Prompt Inversion through Evolutionary Optimization in Natural-Language Space](https://arxiv.org/abs/2604.06061)
*Asaf Buchnick,Aviv Shamsian,Aviv Navon,Ethan Fetaya*

Main category: cs.LG

TL;DR: PromptEvolver improves prompt inversion for text-to-image generation by using a genetic algorithm to optimize natural-language prompts, achieving high-fidelity reconstructions without needing access to the model's internals.


<details>
  <summary>Details</summary>
Motivation: Existing methods for prompt inversion often produce suboptimal reconstructions and unnatural prompts, limiting transparency and controllability in text-to-image generation.

Method: The paper introduces PromptEvolver, which employs a genetic algorithm to optimize prompts, guided by a vision-language model. It operates on black-box models, requiring only image outputs.

Result: PromptEvolver consistently outperforms competing methods across multiple benchmarks, delivering high-fidelity reconstructions of target images.

Conclusion: PromptEvolver advances prompt inversion by generating natural-language prompts and improving reconstruction fidelity, enhancing transparency and controllability in text-to-image generation.
...
### [248] [A machine learning framework for uncovering stochastic nonlinear dynamics from noisy data](https://arxiv.org/abs/2604.06081)
*Matteo Bosso,Giovanni Franzese,Kushal Swamy,Maarten Theulings,Alejandro M. Aragón,Farbod Alijani*

Main category: cs.LG

TL;DR: A hybrid framework combining symbolic regression and probabilistic machine learning is introduced to uncover governing equations and quantify parameter uncertainty in noisy systems, validated on numerical and biological examples.


<details>
  <summary>Details</summary>
Motivation: The need to understand how noise influences parameter inference and dynamics in real-world systems, bridging the gap between traditional symbolic regression (ignoring uncertainty) and Gaussian processes (lacking dynamic insight).

Method: A hybrid approach integrating deep symbolic regression with Gaussian process-based maximum likelihood estimation to model deterministic dynamics and noise structure without prior assumptions.

Result: Validated on numerical benchmarks (harmonic, Duffing, van der Pol oscillators) and experimental biological oscillators, the framework is data-efficient (100-1000 data points) and robust to noise.

Conclusion: The framework effectively recovers symbolic equations and quantifies uncertainty, offering broad applicability in noisy dynamical systems.
...
### [249] [Learning $\mathsf{AC}^0$ Under Graphical Models](https://arxiv.org/abs/2604.06109)
*Gautam Chandrasekaran,Jason Gaitonde,Ankur Moitra,Arsen Vasilyan*

Main category: cs.LG

TL;DR: The paper presents quasipolynomial-time algorithms for learning constant-depth circuits (AC^0) under correlated distributions, specifically graphical models with polynomial growth and strong spatial mixing, overcoming the limitations of uniform distribution reliance.


<details>
  <summary>Details</summary>
Motivation: The work addresses the critique of relying on product structures in learning algorithms, aiming to extend learning guarantees to more realistic, correlated distributions, which has been a longstanding challenge in computational learning theory.

Method: The authors develop new sampling algorithms to transfer low-degree polynomial approximation results from the uniform setting to graphical models, avoiding reliance on Fourier analysis.

Result: Quasipolynomial-time algorithms are successfully provided for learning AC^0 circuits under graphical models with polynomial growth and strong spatial mixing, extending to other function classes like monotone functions and halfspaces.

Conclusion: The paper advances learning theory by providing a general approach to learning under correlated distributions, significantly broadening the applicability of previous results beyond the product setting.
...
### [250] [Gym-Anything: Turn any Software into an Agent Environment](https://arxiv.org/abs/2604.06126)
*Pranjal Aggarwal,Graham Neubig,Sean Welleck*

Main category: cs.LG

TL;DR: The paper introduces Gym-Anything, a framework to create interactive environments for complex software tasks, addressing scalability issues in current research.


<details>
  <summary>Details</summary>
Motivation: Current research on computer-use agents is limited to short-horizon tasks with low economic value due to the high effort required for environment creation.

Method: The authors propose a multi-agent approach: a coding agent sets up environments, and an audit agent verifies correctness. They apply this to 200 software applications, creating CUA-World, a dataset of 10K+ tasks.

Result: CUA-World includes long-horizon tasks (500+ steps) and improves performance of models, with auditing at test time further boosting results.

Conclusion: The framework and dataset advance research in realistic computer-use agents, with released code and benchmarks supporting future work.
...
### [251] [Toward Consistent World Models with Multi-Token Prediction and Latent Semantic Enhancement](https://arxiv.org/abs/2604.06155)
*Qimin Zhong,Hao Liao,Haiming Qin,Mingyang Zhou,Rui Mao,Wei Chen,Naipeng Chao*

Main category: cs.LG

TL;DR: LSE-MTP improves LLM world models by anchoring predictions to hidden states, reducing structural hallucinations and enhancing robustness.


<details>
  <summary>Details</summary>
Motivation: The debate over whether LLMs develop coherent internal world models and the limitations of standard Multi-Token Prediction (MTP) in avoiding structural hallucinations.

Method: Proposed Latent Semantic Enhancement MTP (LSE-MTP), which grounds predictions in true hidden state trajectories instead of discrete tokens.

Result: Empirical evidence shows LSE-MTP enhances representation alignment, reduces hallucinations, and improves robustness on synthetic graphs and real-world data.

Conclusion: LSE-MTP bridges the gap between discrete tokens and continuous state representations, offering a more reliable approach for LLM world modeling.
...
### [252] [Target Policy Optimization](https://arxiv.org/abs/2604.06159)
*Jean Kaddour*

Main category: cs.LG

TL;DR: TPO separates the questions of selecting completions and adjusting parameters in RL, outperforming other methods in sparse reward scenarios.


<details>
  <summary>Details</summary>
Motivation: To address overshooting or undershooting in standard policy-gradient methods by decoupling the selection of completions and parameter updates.

Method: TPO constructs a target distribution from scored completions and fits the policy using cross-entropy loss.

Result: TPO matches or outperforms PG, PPO, GRPO, and DG, especially under sparse rewards.

Conclusion: TPO provides a more stable and effective approach for RL tasks by decoupling target distribution and policy fitting.
...
### [253] [Topological Characterization of Churn Flow and Unsupervised Correction to the Wu Flow-Regime Map in Small-Diameter Vertical Pipes](https://arxiv.org/abs/2604.06167)
*Brady Koenig,Sushovan Majhi,Atish Mitra,Abigail Stein,Burt Todd*

Main category: cs.LG

TL;DR: The paper introduces the first topology-based definition of churn flow using Euler Characteristic Surfaces (ECS) and an unsupervised framework combining Multiple Kernel Learning (MKL) to analyze two-phase flow regimes.


<details>
  <summary>Details</summary>
Motivation: For over 40 years, churn flow lacked a quantitative mathematical definition. The study aims to address this gap by leveraging topological methods to characterize and differentiate flow regimes without relying on labeled data.

Method: The authors use Euler Characteristic Surfaces (ECS) and formulate unsupervised regime discovery as Multiple Kernel Learning (MKL). They blend two ECS-derived kernels (temporal alignment and amplitude statistics) with gas velocity. The framework is applied to unlabeled datasets from Montana Tech and Texas A&M University.

Result: The framework places 64% weight on topology-derived features, identifies a +3.81 m/s discrepancy in existing models, and achieves 95.6% 4-class accuracy and 100% churn recall in validation tests. It also confirms higher topological complexity in churn flow compared to slug flow.

Conclusion: This work provides the first mathematical definition of churn flow and demonstrates that unsupervised topological descriptors can outperform or match supervised models, correcting widely adopted mechanistic models.
...
### [254] [In-Place Test-Time Training](https://arxiv.org/abs/2604.06169)
*Guhao Feng,Shengjie Luo,Kai Hua,Ge Zhang,Di He,Wenhao Huang,Tianle Cai*

Main category: cs.LG

TL;DR: In-Place TTT enables LLMs to dynamically adapt at inference time by updating fast weights in the final projection matrix of MLP blocks, aligning with next-token prediction and improving scalability.


<details>
  <summary>Details</summary>
Motivation: Overcome the limitations of static LLMs by enabling dynamic adaptation to new information during inference, addressing barriers like architectural incompatibility and computational inefficiency.

Method: Introduces In-Place TTT, using the final projection matrix of MLP blocks as adaptable fast weights and a tailored objective for next-token prediction, with a chunk-wise update mechanism.

Result: Achieves superior performance on tasks with long contexts (up to 128k) and outperforms other TTT-related approaches when pretrained from scratch.

Conclusion: In-Place TTT is a scalable and effective framework for continual learning in LLMs, validated by experiments and ablation studies.
...
### [255] [PaperOrchestra: A Multi-Agent Framework for Automated AI Research Paper Writing](https://arxiv.org/abs/2604.05018)
*Yiwen Song,Yale Song,Tomas Pfister,Jinsung Yoon*

Main category: cs.AI

TL;DR: PaperOrchestra is a multi-agent framework for AI research paper writing, outperforming existing methods in literature review and manuscript quality.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of synthesizing unstructured research materials into high-quality manuscripts, a gap in AI-driven scientific discovery.

Method: Introduces PaperOrchestra, a flexible framework that transforms pre-writing materials into LaTeX manuscripts, including literature synthesis and visuals.

Result: Outperforms baselines with a 50%-68% win rate in literature review quality and 14%-38% in overall manuscript quality.

Conclusion: PaperOrchestra advances AI-driven paper writing by providing a more flexible and effective solution.
...
### [256] [Pramana: Fine-Tuning Large Language Models for Epistemic Reasoning through Navya-Nyaya](https://arxiv.org/abs/2604.04937)
*Sharath Sathish*

Main category: cs.AI

TL;DR: The paper introduces Pramana, a method to improve large language models' reasoning by fine-tuning them on Navya-Nyaya logic, a structured 2,500-year-old Indian framework, achieving 100% semantic correctness on evaluation tasks.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) often produce unfounded claims and struggle with systematic reasoning, leading to unreliable outputs in domains requiring justification. This work aims to address this gap by grounding LLMs in traceable evidence and structured reasoning.

Method: Fine-tuning LLMs (Llama 3.2-3B and DeepSeek-R1-Distill-Llama-8B) on 55 Nyaya-structured logical problems using Navya-Nyaya logic, which enforces a 6-phase reasoning process: doubt analysis, evidence identification, syllogism, counterfactual verification, fallacy detection, and ascertainment.

Result: Despite only 40% strict format adherence, the models achieved 100% semantic correctness on held-out evaluation tasks, demonstrating internalization of reasoning content. Ablation studies highlighted the importance of format prompting and temperature settings.

Conclusion: Integrating Navya-Nyaya logic into LLMs provides effective cognitive scaffolding for systematic reasoning, improving reliability. The release of models, datasets, and infrastructure encourages further research in epistemic frameworks for AI.
...
### [257] [From Governance Norms to Enforceable Controls: A Layered Translation Method for Runtime Guardrails in Agentic AI](https://arxiv.org/abs/2604.05229)
*Christopher Koch*

Main category: cs.AI

TL;DR: A method to translate governance objectives into actionable runtime guardrails for agentic AI, addressing risks during execution by linking standards to control layers.


<details>
  <summary>Details</summary>
Motivation: Agentic AI systems introduce risks during execution that existing governance standards don't directly address, requiring practical runtime controls.

Method: Proposes a layered translation method connecting governance objectives to four control layers (governance objectives, design-time constraints, runtime mediation, assurance feedback) and introduces a control tuple and rubric for layer assignment.

Result: Demonstrates the method in a procurement-agent case study, emphasizing the need for runtime guardrails to be observable, determinate, and time-sensitive.

Conclusion: Standards should guide control placement across architecture and runtime policy, with runtime guardrails reserved for specific, justified interventions.
...
### [258] [Operational Noncommutativity in Sequential Metacognitive Judgments](https://arxiv.org/abs/2604.04938)
...
### [259] [SCMAPR: Self-Correcting Multi-Agent Prompt Refinement for Complex-Scenario Text-to-Video Generation](https://arxiv.org/abs/2604.05489)
*Chengyi Yang,Pengzhen Li,Jiayin Qi,Aimin Zhou,Ji Wu,Ji Liu*

Main category: cs.AI

TL;DR: SCMAPR is a multi-agent framework for refining ambiguous text prompts in Text-to-Video (T2V) generation, improving alignment and quality in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Current T2V systems struggle with complex scenarios due to ambiguous or underspecified text prompts, necessitating a refined approach to prompt handling.

Method: SCMAPR uses a stage-wise multi-agent process: scenario routing, policy-conditioned refinement, and semantic verification with conditional revision.

Result: SCMAPR improves text-video alignment and generation quality, achieving gains of up to 2.67% and 3.28% on benchmarks compared to state-of-the-art baselines.

Conclusion: SCMAPR effectively addresses the challenges of complex-scenario T2V generation, demonstrating significant improvements in benchmark evaluations.
...
### [260] [Proximity Measure of Information Object Features for Solving the Problem of Their Identification in Information Systems](https://arxiv.org/abs/2604.04939)
*Volodymyr Yuzefovych*

Main category: cs.AI

TL;DR: The paper introduces a new proximity measure for features of information objects from multiple sources, combining probabilistic and possibility measures for quantitative and qualitative features, respectively, without requiring value transformation.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of determining if features from independent sources relate to the same observation object, accounting for determination errors in both quantitative and qualitative features.

Method: Uses a probabilistic measure for quantitative features and a possibility measure for qualitative features. Validates the measure's compliance with required axioms and proposes variants for IO proximity.

Result: Demonstrates the feasibility of the measure by showing it adheres to necessary axioms and avoids feature value transformation.

Conclusion: The proposed measure effectively combines quantitative and qualitative feature proximity, offering flexibility and practicality for diverse information objects.
...
### [261] [LUDOBENCH: Evaluating LLM Behavioural Decision-Making Through Spot-Based Board Game Scenarios in Ludo](https://arxiv.org/abs/2604.05681)
*Ojas Jain,Dhruv Kumar*

Main category: cs.AI

TL;DR: LudoBench is a benchmark for evaluating LLM strategic reasoning in Ludo, featuring 480 handcrafted scenarios and a simulator with diverse agent types. Models show limited alignment with game-theory strategies and exhibit distinct behavioral archetypes, highlighting prompt-sensitivity.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' strategic reasoning in complex, stochastic multi-agent environments like Ludo, addressing the need for interpretable benchmarks.

Method: LudoBench includes 480 spot scenarios and a 4-player simulator with Random, Heuristic, Game-Theory, and LLM agents. Game-theory agents use Expectiminimax search for strategic evaluation.

Result: LLMs agree with game-theory baselines only 40-46% of the time, showing two behavioral archetypes: finishers and builders, each capturing only half of the ideal strategy.

Conclusion: LudoBench offers a lightweight, interpretable framework for benchmarking LLM strategic reasoning, revealing key vulnerabilities like prompt-sensitivity. Resources are openly available.
...
### [262] [ReVEL: Multi-Turn Reflective LLM-Guided Heuristic Evolution via Structured Performance Feedback](https://arxiv.org/abs/2604.04940)
*Cuong Van Duc,Minh Nguyen Dinh Tuan,Tam Vu Duc,Tung Vu Duy,Son Nguyen Van,Hanh Nguyen Thi,Binh Huynh Thi Thanh*

Main category: cs.AI

TL;DR: The paper introduces ReVEL, a framework combining LLMs and evolutionary algorithms for automated heuristic design, using multi-turn reasoning and structured feedback to improve heuristic robustness and diversity.


<details>
  <summary>Details</summary>
Motivation: The challenge of designing effective heuristics for NP-hard problems is expertise-intensive. Current LLM applications in this space are brittle and underutilize iterative reasoning.

Method: ReVEL integrates LLMs as multi-turn reasoners within an evolutionary algorithm. It uses performance-profile grouping and feedback-driven reflection to refine heuristics.

Result: ReVEL produces more robust and diverse heuristics, outperforming baselines on standard benchmarks with statistically significant improvements.

Conclusion: Multi-turn reasoning with structured grouping is a promising paradigm for automated heuristic design.
...
### [263] [Algebraic Structure Discovery for Real World Combinatorial Optimisation Problems: A General Framework from Abstract Algebra to Quotient Space Learning](https://arxiv.org/abs/2604.04941)
*Min Sun,Federica Storti,Valentina Martino,Miguel Gonzalez-Andrades,Tony Kam-Thong*

Main category: cs.AI

TL;DR: The paper introduces a framework to identify and exploit algebraic structures in combinatorial optimization, improving efficiency and success rates in finding global optima.


<details>
  <summary>Details</summary>
Motivation: Combinatorial optimization problems often contain hidden algebraic structures that, once revealed, can reduce search space and enhance the likelihood of finding optimal solutions. The aim is to leverage these structures for better performance.

Method: The framework involves identifying algebraic structures, formalizing operations, constructing quotient spaces to eliminate redundancy, and optimizing over these reduced spaces. It uses a characteristic-vector encoding to map conjunctive rules to the Boolean hypercube, enabling structure-aware search.

Result: Experiments on real clinical data and synthetic benchmarks show that the quotient-space-aware genetic algorithms achieve success rates of 48% to 77%, compared to 35% to 37% for standard methods, while maintaining diversity in solutions.

Conclusion: Exploiting algebraic structures in combinatorial optimization provides a simple and effective way to improve efficiency and success rates in finding global optima.
...
### [264] [Part-Level 3D Gaussian Vehicle Generation with Joint and Hinge Axis Estimation](https://arxiv.org/abs/2604.05070)
*Shiyao Qian,Yuan Ren,Dongfeng Bai,Bingbing Liu*

Main category: cs.AI

TL;DR: A generative framework for creating animatable 3D Gaussian vehicles from single or sparse multi-view images, addressing challenges in articulation and kinematic parameter prediction.


<details>
  <summary>Details</summary>
Motivation: Current simulation frameworks for autonomous driving lack part-level articulation, limiting realism for perception algorithms that rely on dynamic features like wheel steering or door opening.

Method: Introduces a part-edge refinement module for exclusive Gaussian ownership and a kinematic reasoning head to predict joint positions and hinge axes of movable parts.

Result: Enables faithful part-aware simulation, bridging the gap between static generation and animatable vehicle models.

Conclusion: The proposed framework improves the realism of vehicle simulation by addressing articulation and kinematic challenges, enhancing its utility for autonomous driving applications.
...
### [265] [MMORF: A Multi-agent Framework for Designing Multi-objective Retrosynthesis Planning Systems](https://arxiv.org/abs/2604.05075)
*Frazier N. Baker,Trieu Nguyen,Reza Averly,Botao Yu,Daniel Adu-Ampratwum,Huan Sun,Xia Ning*

Main category: cs.AI

TL;DR: MMORF is a framework for multi-objective retrosynthesis planning using modular multi-agent systems, achieving strong performance on safety, cost, and hard-constraint tasks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for dynamic balancing of quality, safety, and cost objectives in retrosynthesis planning by leveraging multi-agent systems (MAS) with specialized agents.

Method: The authors present MMORF, a framework with modular agentic components that can be flexibly combined to create systems like MASIL and RFAS. These systems are evaluated on a curated benchmark of 218 tasks.

Result: MASIL excels in safety and cost metrics for soft-constraint tasks, often Pareto-dominating baselines, while RFAS achieves a 48.6% success rate on hard-constraint tasks, outperforming state-of-the-art baselines.

Conclusion: MMORF proves effective as a foundational framework for exploring MAS in multi-objective retrosynthesis planning, with demonstrated success in diverse tasks.
...
### [266] [MedGemma 1.5 Technical Report](https://arxiv.org/abs/2604.05081)
...
TL;DR: MedGemma 1.5 4B is an advanced medical AI model with enhanced capabilities in imaging, localization, and document understanding, outperforming its predecessor in various tasks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to expand the capabilities of medical AI by integrating high-dimensional imaging, improved document understanding, and anatomical localization into a single model.

Method: Innovations include new training data, 3D volume slicing, pathology sampling, and architectural enhancements to support diverse medical modalities.

Result: MedGemma 1.5 shows significant improvements: 11% better MRI classification, 3% better CT classification, 47% gain in pathology imaging, and 35% better localization. It also excels in clinical text tasks (5–22% gains).

Conclusion: MedGemma 1.5 is a robust, open-source foundation for next-gen medical AI, offering improved performance across multiple medical domains.
...
### [267] [Uncertainty-Guided Latent Diagnostic Trajectory Learning for Sequential Clinical Diagnosis](https://arxiv.org/abs/2604.05116)
*Xuyang Shen,Haoran Liu,Dongjin Song,Martin Renqiang Min*

Main category: cs.AI

TL;DR: The paper introduces a Latent Diagnostic Trajectory Learning (LDTL) framework to improve sequential clinical diagnosis by using two LLM agents: one for planning and one for diagnosis, outperforming baselines in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based diagnostic systems assume fully observed patient information and lack explicit modeling of sequential evidence acquisition, leading to less effective diagnostic paths.

Method: The LDTL framework uses a planning LLM agent and a diagnostic LLM agent, treating diagnostic sequences as latent paths with a posterior distribution to prioritize informative trajectories.

Result: Experiments on MIMIC-CDM show the framework improves diagnostic accuracy and reduces test requirements compared to baselines.

Conclusion: The LDTL framework effectively models sequential diagnosis, with trajectory-level posterior alignment being crucial for its success.
...
### [268] [Non-monotonic causal discovery with Kolmogorov-Arnold Fuzzy Cognitive Maps](https://arxiv.org/abs/2604.05136)
*Jose L. Salmeron*

Main category: cs.AI

TL;DR: KA-FCMs enhance Fuzzy Cognitive Maps by using learnable B-spline functions for non-monotonic causal relationships, outperforming traditional methods while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Standard FCMs are limited in modeling non-monotonic dynamics; KA-FCMs address this by redefining causal transmission.

Method: Replaces scalar weights with learnable B-spline functions on edges, shifting non-linearity to the causal phase.

Result: KA-FCMs outperform baselines and match MLPs in accuracy, preserving interpretability and extracting explicit laws.

Conclusion: KA-FCMs offer a promising neural-symbolic solution for complex systems, balancing accuracy and interpretability.
...
### [269] [A mathematical theory of evolution for self-designing AIs](https://arxiv.org/abs/2604.05142)
*Kenneth D Harris*

Main category: cs.AI

TL;DR: The paper explores AI evolution through directed self-design, contrasting it with biological evolution, and shows how fitness dynamics can lead to deception if not aligned with human utility.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how AI systems evolve through self-improvement, focusing on the differences from biological evolution and the potential risks of misalignment between AI fitness and human goals.

Method: The authors develop a mathematical model of AI evolution, replacing random mutations with a directed tree of possible AI programs. They analyze evolutionary dynamics under bounded fitness and a fixed probability of self-replication.

Result: The model reveals that fitness may not increase over time without assumptions, but with specific conditions, it concentrates on the maximum reachable value. Deception can emerge if fitness and human utility are misaligned.

Conclusion: The findings highlight risks in AI alignment, suggesting mitigation through objective reproduction criteria to prevent deception.
...
### [270] [IntentScore: Intent-Conditioned Action Evaluation for Computer-Use Agents](https://arxiv.org/abs/2604.05157)
*Rongqian Chen,Yu Li,Zeyu Fang,Sizhe Tang,Weidong Cao,Tian Lan*

Main category: cs.AI

TL;DR: IntentScore is a plan-aware reward model that evaluates GUI action quality, improving task success rates by learning from offline interactions.


<details>
  <summary>Details</summary>
Motivation: Current Computer-Use Agents (CUAs) generate GUI actions without evaluating quality, leading to irreversible errors. IntentScore addresses this by scoring actions to prevent cascading mistakes.

Method: IntentScore learns from 398K offline GUI interaction steps across three OSes. It uses contrastive alignment for state-action relevance and margin ranking for action correctness, embedding planning intent in the action encoder.

Result: Achieves 97.5% pairwise discrimination accuracy and improves task success rate by 6.9 points on the unseen OSWorld environment.

Conclusion: IntentScore generalizes well to unseen agents and tasks, demonstrating the effectiveness of learning reward estimation from diverse offline interactions.
...
### [271] [Bypassing the CSI Bottleneck: MARL-Driven Spatial Control for Reflector Arrays](https://arxiv.org/abs/2604.05162)
*Hieu Le,Oguz Bedir,Mostafa Ibrahim,Jian Tao,Sabit Ekin*

Main category: cs.AI

TL;DR: The paper proposes an AI-driven MARL framework to control RIS for CSI-free operation, outperforming traditional methods in dynamic NLOS environments.


<details>
  <summary>Details</summary>
Motivation: The high computational overhead of CSI estimation in RIS deployment motivates a data-driven, AI-native approach to bypass this bottleneck.

Method: A MARL framework with CTDE architecture and MAPPO is used to control mechanically adjustable metallic reflector arrays, mapping constraints to a reduced-order virtual focal point space.

Result: Simulations show a 26.86 dB improvement over static reflectors, with better spatial selectivity and temporal stability. The approach also demonstrates resilience to localization noise.

Conclusion: MARL-driven spatial abstractions offer a scalable and practical solution for AI-empowered wireless networks.
...
### [272] [Learning to Focus: CSI-Free Hierarchical MARL for Reconfigurable Reflectors](https://arxiv.org/abs/2604.05165)
*Hieu Le,Mostafa Ibrahim,Oguz Bedir,Jian Tao,Sabit Ekin*

Main category: cs.AI

TL;DR: The paper introduces a CSI-free, hierarchical multi-agent reinforcement learning (HMARL) framework for controlling mechanically reconfigurable reflective surfaces in RIS-enabled mmWave networks, achieving significant RSSI improvements and scalability without costly channel estimation.


<details>
  <summary>Details</summary>
Motivation: The prohibitive computational overhead of CSI estimation and the complexity of centralized optimization in RIS-enabled mmWave networks hinder large-scale deployments. The authors aim to overcome these challenges by eliminating CSI estimation and leveraging spatial intelligence for wave propagation management.

Method: The proposed framework replaces pilot-based channel estimation with user localization data. It employs a two-tier neural architecture: a high-level controller for user-to-reflector allocations and low-level controllers using MAPPO for continuous focal point optimization under a CTDE scheme.

Result: Deterministic ray-tracing evaluations show the framework achieves up to 7.79 dB RSSI improvements over centralized baselines, robust multi-user scalability, and resilient beam-focusing performance under practical localization errors.

Conclusion: The work presents a scalable, cost-effective solution for intelligent wireless environments by eliminating CSI overhead while maintaining high-fidelity signal redirection using hierarchical reinforcement learning.
...
### [273] [Instruction-Tuned LLMs for Parsing and Mining Unstructured Logs on Leadership HPC Systems](https://arxiv.org/abs/2604.05168)
*Ahmad Maroof Karimi,Jong Youl Choi,Charles Qing Cao,Awais Khan*

Main category: cs.AI

TL;DR: An LLM-driven framework for high-fidelity parsing of HPC logs using domain adaptation and chain-of-thought reasoning, achieving accuracy comparable to larger models.


<details>
  <summary>Details</summary>
Motivation: Leadership-class HPC systems produce massive, unstructured logs from diverse sources, requiring robust parsing and mining to extract actionable insights.

Method: A domain-adapted, instruction-following LLM framework fine-tunes an 8B-parameter LLaMA model for HPC logs using hybrid fine-tuning and CoT reasoning.

Result: The approach matches parsing accuracy of larger models like LLaMA 70B and Claude, successfully analyzing 600 million logs from the Frontier supercomputer.

Conclusion: The framework offers a privacy-preserving, efficient, and scalable solution for log mining in HPC environments, uncovering critical operational patterns.
...
### [274] [ClawsBench: Evaluating Capability and Safety of LLM Productivity Agents in Simulated Workspaces](https://arxiv.org/abs/2604.05172)
...
TL;DR: ClawsBench is a benchmark for evaluating LLM agents in realistic productivity settings, featuring high-fidelity mock services and structured tasks, revealing both success and safety challenges.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack realism and stateful workflows, making it risky to evaluate LLM agents in live productivity environments.

Method: ClawsBench includes five mock services with state management and 44 tasks, testing agents with varying scaffolding (domain skills and meta prompts) to assess performance and safety.

Result: Agents achieved 39-64% task success but showed 7-33% unsafe action rates, with no clear correlation between success and safety. Eight unsafe behavior patterns were identified.

Conclusion: ClawsBench highlights the trade-offs between task success and safety in LLM agents, emphasizing the need for better evaluation frameworks in real-world productivity contexts.
...
### [275] [Attribution Bias in Large Language Models](https://arxiv.org/abs/2604.05224)
*Eliza Berman,Bella Chang,Daniel B. Neill,Emily Black*

Main category: cs.AI

TL;DR: AttriBench is a balanced benchmark for evaluating quote attribution in LLMs, revealing systematic biases in accuracy and suppression across demographic groups.


<details>
  <summary>Details</summary>
Motivation: To address the need for accurate content attribution in LLMs and assess representational fairness, especially concerning demographic biases.

Method: Introduces AttriBench, a balanced dataset by fame and demographics, and evaluates 11 LLMs across prompts, focusing on quote attribution and suppression.

Result: LLMs show large disparities in attribution accuracy and suppression rates across race, gender, and intersectional groups, highlighting systematic biases.

Conclusion: Quote attribution serves as a benchmark for fairness in LLMs, with AttriBench revealing significant biases not captured by standard metrics.
...
### [276] [EAGLE: Edge-Aware Graph Learning for Proactive Delivery Delay Prediction in Smart Logistics Networks](https://arxiv.org/abs/2604.05254)
*Zhiming Xue,Menghao Huo,Yujue Wang*

Main category: cs.AI

TL;DR: A hybrid deep learning framework combines temporal order-flow dynamics and spatial supply chain dependencies to proactively predict delivery delays, outperforming baseline methods in accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for predicting delivery delays in logistics networks either ignore network topology or overlook spatial dependencies. The paper aims to bridge this gap by modeling both temporal and spatial aspects of supply chain data.

Method: The proposed framework uses a lightweight Transformer patch encoder for temporal order-flow dynamics and an Edge-Aware Graph Attention Network (E-GAT) for inter-hub relational dependencies, optimized via multi-task learning.

Result: The framework achieves an F1-score of 0.8762 and an AUC-ROC of 0.9773 on the DataCo Smart Supply Chain dataset, with a cross-seed F1 standard deviation of 0.0089, indicating high accuracy and stability.

Conclusion: The hybrid framework provides a robust solution for proactive supply chain risk management, outperforming baseline methods in predictive accuracy and training stability.
...
### [277] [Simulating the Evolution of Alignment and Values in Machine Intelligence](https://arxiv.org/abs/2604.05274)
*Jonathan Elsworth Eicher*

Main category: cs.AI

TL;DR: The study explores how model alignment affects populations of models over time, revealing that deceptive beliefs can persist despite high testing accuracy, and suggests combining improved evaluation methods, adaptive test design, and mutational dynamics to reduce deception.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the long-term effects of alignment on model populations, particularly focusing on the persistence of deceptive beliefs despite high test performance.

Method: The research applies evolutionary theory to model how different populations of beliefs and selection methods can lead to the fixation of deceptive beliefs. It uses iterative alignment testing and analyzes mutations to understand complex developments.

Result: Even with high correlation ($ρ= 0.8$) between testing accuracy and true value, deceptive beliefs can become fixed. Mutations introduce complexity, emphasizing the need for better tests to avoid malicious deception. Improved evaluator capabilities, adaptive test design, and mutational dynamics significantly reduce deception while maintaining alignment fitness ($p_{	ext{adj}} < 0.001$).

Conclusion: The findings highlight the importance of updating test quality and combining multiple strategies to mitigate deception in aligned models, ensuring both performance and reliability.
...
### [278] [Pressure, What Pressure? Sycophancy Disentanglement in Language Models via Reward Decomposition](https://arxiv.org/abs/2604.05279)
*Muhammad Ahmed Mohsin,Ahsan Bilal,Muhammad Umer,Emily Fox*

Main category: cs.AI

TL;DR: The paper addresses sycophancy in large language models (LLMs), where models shift answers based on perceived user preferences or authority cues, and proposes a reward decomposition method (GRPO) to reduce this behavior by training on pressure resistance, context fidelity, and other dimensions.


<details>
  <summary>Details</summary>
Motivation: Sycophancy in LLMs undermines their reliability as they adapt answers to perceived user preferences or authority, even without evidence. Standard alignment methods fail to distinguish between pressure capitulation and evidence blindness, limiting their effectiveness.

Method: The authors introduce Group Relative Policy Optimisation (GRPO), a multi-component reward decomposition approach with five terms (pressure resistance, context fidelity, etc.). They train models using a contrastive dataset with pressure-free and pressured variants across authority levels and opposing evidence contexts.

Result: The method consistently reduces sycophancy across five base models, with each reward term governing an independent behavioral dimension. The trained models generalize beyond the training setup, reducing sycophancy by up to 17 points on SycophancyEval.

Conclusion: The proposed GRPO framework effectively reduces sycophancy in LLMs by disentangling and addressing its underlying components, demonstrating generalizability to untrained pressure forms and prompt structures.
...
### [279] [Graph of Skills: Dependency-Aware Structural Retrieval for Massive Agent Skills](https://arxiv.org/abs/2604.05333)
*Dawei Li,Zongxia Li,Hongyang Du,Xiyang Wu,Shihang Gui,Yongbei Kuang,Lichao Sun*

Main category: cs.AI

TL;DR: Graph of Skills (GoS) improves agent performance by efficiently managing large skill libraries, reducing token costs and latency while increasing reward.


<details>
  <summary>Details</summary>
Motivation: Scaling skill libraries for agents introduces challenges like context window saturation and high token costs, necessitating better retrieval methods.

Method: GoS uses a skill graph built offline, with dependency-aware retrieval via hybrid seeding, Personalized PageRank, and context-budgeted hydration.

Result: GoS boosts reward by 43.6% and cuts input tokens by 37.8%, outperforming full skill-loading and vector retrieval across model families.

Conclusion: GoS efficiently balances reward, token use, and runtime, proving effective for large skill libraries in agent systems.
...
### [280] [TRACE: Capability-Targeted Agentic Training](https://arxiv.org/abs/2604.05336)
*Hangoo Kang,Tarun Suresh,Jon Saad-Falcon,Azalia Mirhoseini*

Main category: cs.AI

TL;DR: TRACE is an end-to-end system that identifies lacking capabilities in LLMs, creates targeted training environments, and trains adapters to improve agent performance across tasks.


<details>
  <summary>Details</summary>
Motivation: Current approaches either use synthetic data not aligned with model deficits or require implicit learning. TRACE addresses this by systematically targeting and improving specific capabilities.

Method: TRACE contrasts successful and failed trajectories to identify lacking capabilities, synthesizes targeted training environments, and trains LoRA adapters via RL, routing to the relevant adapter during inference.

Result: TRACE improves performance by +14.1 points on τ²-bench and +7 perfect scores on ToolSandbox, outperforming baselines significantly.

Conclusion: TRACE efficiently scales and outperforms existing methods, proving effective for agent self-improvement in diverse environments.
...
### [281] [Dynamic Agentic AI Expert Profiler System Architecture for Multidomain Intelligence Modeling](https://arxiv.org/abs/2604.05345)
*Aisvarya Adeseye,Jouni Isoaho,Seppo Virtanen,Mohammad Tahir*

Main category: cs.AI

TL;DR: The paper introduces an AI profiler to classify user expertise in natural language interactions, achieving high accuracy in matching self-assessments.


<details>
  <summary>Details</summary>
Motivation: Modern AI systems need to adapt to diverse user expertise levels for effective human-machine interaction.

Method: A modular AI system (built on LLaMA v3.1) processes, scores, aggregates, and classifies responses into four expertise levels, validated via static and dynamic testing.

Result: The profiler matched user self-ratings 83-97% of the time, with discrepancies linked to self-bias or model limitations.

Conclusion: The AI profiler effectively classifies user expertise, though nuances and biases highlight areas for improvement.
...
### [282] [From Retinal Evidence to Safe Decisions: RETINA-SAFE and ECRT for Hallucination Risk Triage in Medical LLMs](https://arxiv.org/abs/2604.05348)
*Zhe Yu,Wenpeng Xing,Meng Han*

Main category: cs.AI

TL;DR: The paper introduces RETINA-SAFE, a benchmark for evaluating hallucinations in medical LLMs for diabetic retinopathy, and proposes ECRT, a two-stage framework for risk triage and subtype attribution.


<details>
  <summary>Details</summary>
Motivation: Addressing hallucinations in medical LLMs is critical for patient safety, especially when evidence is insufficient or conflicting, as in diabetic retinopathy decisions.

Method: Proposes ECRT, a two-stage white-box detection framework: Stage 1 performs risk triage, and Stage 2 refines unsafe cases. It leverages internal representations and logit shifts under different conditions.

Result: ECRT improves balanced accuracy by +0.15 to +0.19 over baselines and consistently outperforms single-stage approaches, showing strong risk triage and subtype attribution.

Conclusion: White-box internal signals grounded in retinal evidence offer a practical and interpretable approach for medical LLM risk triage.
...
### [283] [ETR: Entropy Trend Reward for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2604.05355)
*Xuan Xiong,Huan Liu,Li Gu,Zhixiang Chi,Yue Qiu,Yuanhao Yu,Yang Wang*

Main category: cs.AI

TL;DR: ETR improves CoT reasoning efficiency by encouraging downward entropy trends, reducing length by 67% while boosting accuracy by 9.9%.


<details>
  <summary>Details</summary>
Motivation: Existing methods shorten CoTs inefficiently by assuming low uncertainty is always desirable, overlooking the role of entropy trends.

Method: Proposes Entropy Trend Reward (ETR) to encourage progressive uncertainty reduction, integrated into GRPO for optimization.

Result: ETR achieves a superior accuracy-efficiency tradeoff, notably improving DeepSeek-R1-Distill-7B's performance.

Conclusion: ETR effectively enhances reasoning efficiency by focusing on entropy trajectories, outperforming existing methods.
...
### [284] [LatentAudit: Real-Time White-Box Faithfulness Monitoring for Retrieval-Augmented Generation with Verifiable Deployment](https://arxiv.org/abs/2604.05358)
*Zhe Yu,Wenpeng Xing,Meng Han*

Main category: cs.AI

TL;DR: LatentAudit is a white-box auditor that uses residual-stream activations to measure the faithfulness of retrieval-augmented generation (RAG) answers to retrieved evidence, achieving high accuracy and efficiency across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Retrieval-augmented generation (RAG) reduces but doesn't eliminate hallucinations, necessitating real-time verification of answer faithfulness to retrieved evidence.

Method: LatentAudit pools mid-to-late residual-stream activations from a generator to measure their Mahalanobis distance to the evidence representation, forming a simple quadratic rule for real-time auditing.

Result: The method achieves high AUROC scores (e.g., 0.942 on PubMedQA) with minimal overhead, remains stable across benchmarks and model families, and supports public verification without revealing sensitive data.

Conclusion: Residual-stream geometry provides a practical basis for real-time RAG faithfulness monitoring and verifiable deployment, addressing key limitations of RAG systems.
...
### [285] [TFRBench: A Reasoning Benchmark for Evaluating Forecasting Systems](https://arxiv.org/abs/2604.05364)
...
TL;DR: TFRBench is the first benchmark for evaluating reasoning in forecasting systems, focusing on cross-channel dependencies, trends, and external events. It uses a multi-agent framework to improve accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current time-series forecasting benchmarks focus only on numerical accuracy, treating models as black boxes. TFRBench aims to evaluate and improve reasoning capabilities.

Method: A multi-agent framework with iterative verification loops synthesizes reasoning traces. It spans ten datasets across five domains.

Result: Reasoning traces improve forecasting accuracy (e.g., ~40.2% to 56.6%). Off-the-shelf LLMs struggle with reasoning and domain-specific dynamics.

Conclusion: TFRBench sets a new standard for interpretable, reasoning-based evaluation in forecasting, demonstrating its effectiveness and usefulness.
...
### [286] [LLM-as-Judge for Semantic Judging of Powerline Segmentation in UAV Inspection](https://arxiv.org/abs/2604.05371)
*Akram Hossain,Rabab Abdelfattah,Xiaofeng Wang,Kareem Abdelfatah*

Main category: cs.AI

TL;DR: Using an LLM as a semantic judge to assess drone-based power line segmentation reliability shows promise for safety-critical inspections.


<details>
  <summary>Details</summary>
Motivation: Lightweight segmentation models on drones for power line inspection often degrade in real-world conditions, raising safety concerns. The study explores if an LLM can reliably evaluate segmentation quality.

Method: The research formalizes a watchdog scenario where an offboard LLM evaluates segmentation overlays. Two protocols assess the judge's repeatability (stability of scores/confidence) and perceptual sensitivity (response to visual corruptions like fog, rain, etc.).

Result: The LLM provides consistent judgments and adjusts confidence appropriately as segmentation quality degrades. It remains perceptually sensitive to missing or misidentified power lines under adverse conditions.

Conclusion: An LLM, when carefully constrained, can reliably monitor segmentation quality in safety-critical aerial inspections, offering a viable solution for real-world deployment challenges.
...
### [287] [Towards Effective In-context Cross-domain Knowledge Transfer via Domain-invariant-neurons-based Retrieval](https://arxiv.org/abs/2604.05383)
...
### [288] [Neural Assistive Impulses: Synthesizing Exaggerated Motions for Physics-based Characters](https://arxiv.org/abs/2604.05394)
*Zhiquan Wang,Bedrich Benes*

Main category: cs.AI

TL;DR: A framework called Assistive Impulse Neural Control is proposed to enable physics-based character animation to track exaggerated, stylized motions by reformulating external assistance in impulse space for stability.


<details>
  <summary>Details</summary>
Motivation: Current DRL methods struggle with exaggerated motions that defy standard physics, often causing training instability when external forces are applied.

Method: The approach uses impulse space for external assistance, decomposing the signal into analytic high-frequency and learned low-frequency components via a hybrid neural policy.

Result: The method robustly tracks highly agile, dynamically infeasible motions that were previously unachievable.

Conclusion: Reformulating assistance in impulse space with hybrid control enables stable and effective tracking of exaggerated motions in physics-based animation.
...
### [289] [Reason Analogically via Cross-domain Prior Knowledge: An Empirical Study of Cross-domain Knowledge Transfer for In-Context Learning](https://arxiv.org/abs/2604.05396)
*Le Liu,Zhiming Li,Jianzhi Yan,Zike Yuan,Shiwei Chen,Youcheng Pan,Buzhou Tang,Qingcai Chen,Yang Xiang,Danny Dongning Sun*

Main category: cs.AI

TL;DR: Cross-domain in-context learning (ICL) is feasible through knowledge transfer, with positive effects beyond a certain example threshold, driven by reasoning structure repair.


<details>
  <summary>Details</summary>
Motivation: Existing ICL relies on in-domain expert demonstrations, limiting its use when such annotations are scarce. The paper explores whether cross-domain demonstrations can improve ICL performance.

Method: A comprehensive empirical study of retrieval methods to test cross-domain knowledge transfer in ICL, analyzing example thresholds and reasoning structure repair.

Result: Positive transfer occurs beyond an example absorption threshold, with gains from reasoning structure repair rather than semantic cues.

Conclusion: Cross-domain knowledge transfer is feasible for improving ICL, encouraging further research into effective retrieval methods.
...
### [290] [CODESTRUCT: Code Agents over Structured Action Spaces](https://arxiv.org/abs/2604.05407)
*Myeongsoo Kim,Joe Hsu,Dingmin Wang,Shweta Garg,Varun Kumar,Murali Krishna Ramanathan*

Main category: cs.AI

TL;DR: CODESTRUCT reframes codebases as structured action spaces, improving code agent accuracy and reducing token usage by leveraging AST entities instead of text spans.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based code agents struggle with unstructured text edits, leading to frequent failures due to formatting drift or ambiguous patterns.

Method: CODESTRUCT introduces readCode and editCode to operate on named AST entities, enabling syntax-validated transformations on semantic program elements.

Result: On SWE-Bench, CODESTRUCT improved Pass@1 accuracy by 1.2-5.0% and reduced token usage by 12-38%. Notably, GPT-5-nano's accuracy improved by 20.8%. Similar gains were observed on CodeAssistBench.

Conclusion: Structure-aware interfaces, like CODESTRUCT, provide a more reliable foundation for code agents by addressing the limitations of text-based approaches.
...
### [291] [Multi-Agent Pathfinding with Non-Unit Integer Edge Costs via Enhanced Conflict-Based Search and Graph Discretization](https://arxiv.org/abs/2604.05416)
*Hongkai Fan,Qinjing Xie,Bo Ouyang,Yaonan Wang,Zhi Yan,Jiawen He,Zheng Fang*

Main category: cs.AI

TL;DR: The paper introduces MAPFZ, a novel Multi-Agent Pathfinding variant with non-unit integer costs, enhancing realism while maintaining a finite state space. It proposes CBS-NIC for efficient solving and BOGD for edge cost discretization, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional MAPF methods assume unit edge costs and single-timestep actions, limiting real-world applicability. MAPFR handles non-unit costs but suffers from an unbounded state space. MAPFZ aims to address these limitations by ensuring a finite state space and improved realism.

Method: Two key contributions: (1) CBS-NIC, an enhanced Conflict-Based Search framework with time-interval conflict detection and improved SIPP. (2) BOGD, a Bayesian Optimization-based method for discretizing non-unit edge costs with sub-linear regret.

Result: Extensive experiments show the approach outperforms state-of-the-art methods in runtime and success rate across diverse benchmarks.

Conclusion: MAPFZ combines realism with finite state space, and CBS-NIC, BOGD provide efficient solving and discretization, respectively, marking a significant advancement in MAPF for real-world applications.
...
### [292] [Automated Auditing of Hospital Discharge Summaries for Care Transitions](https://arxiv.org/abs/2604.05435)
*Akshat Dasula,Prasanna Desikan,Jaideep Srivastava*

Main category: cs.AI

TL;DR: The paper proposes an automated framework using Large Language Models (LLMs) to audit discharge summaries, addressing inconsistencies that lead to care fragmentation and readmissions.


<details>
  <summary>Details</summary>
Motivation: Manual auditing of discharge summaries is inefficient and unscalable, posing risks to patient safety.

Method: The approach uses a privacy-preserving LLM to validate key documentation elements (e.g., follow-up instructions, medication changes) based on a structured checklist derived from the DISCHARGED framework.

Result: The framework proves feasible for scalable, automated auditing of discharge summaries, as tested on MIMIC-IV data.

Conclusion: This work lays the groundwork for systematic quality improvement in electronic health record documentation through automated auditing.
...
### [293] [Adaptive Serverless Resource Management via Slot-Survival Prediction and Event-Driven Lifecycle Control](https://arxiv.org/abs/2604.05465)
*Zeyu Wang,Cuiqianhe Du,Renyue Zhang,Kejian Tong,Qi He,Qiyuan Tian*

Main category: cs.AI

TL;DR: An adaptive framework reduces cold starts by 51.2% and improves cost-efficiency by 2x in serverless computing using dynamic resource allocation and probabilistic modeling.


<details>
  <summary>Details</summary>
Motivation: Address inefficiencies and cold start latency in serverless computing caused by traditional static resource allocation under variable workloads.

Method: Uses event-driven architecture, probabilistic modeling, dual-strategy mechanism with dynamic idle duration adjustment, and intelligent request waiting.

Result: Reduces cold starts by up to 51.2% and improves cost-efficiency by nearly 2x compared to baseline methods.

Conclusion: The proposed framework effectively optimizes serverless performance in multi-cloud environments.
...
### [294] [OntoTKGE: Ontology-Enhanced Temporal Knowledge Graph Extrapolation](https://arxiv.org/abs/2604.05468)
*Dongying Lin,Yinan Liu,Shengwei tang,Bin Wang,Xiaochun Yang*

Main category: cs.AI

TL;DR: OntoTKGE is a novel encoder-decoder framework that integrates ontological knowledge with temporal KG extrapolation to address sparse historical interactions, improving performance over existing models.


<details>
  <summary>Details</summary>
Motivation: The challenge of predicting future facts in TKGs with sparse historical interactions for some entities motivates leveraging ontological knowledge to enhance learning.

Method: OntoTKGE integrates ontological knowledge from ontology-view KGs to guide the learning process, enriching entity embeddings by combining ontological and temporal knowledge.

Result: Experiments on four datasets show OntoTKGE significantly boosts TKG extrapolation model performance and outperforms state-of-the-art baselines.

Conclusion: OntoTKGE effectively addresses sparsity in TKG extrapolation by leveraging ontological knowledge, demonstrating superior results and adaptability to various models.
...
### [295] [Can We Trust a Black-box LLM? LLM Untrustworthy Boundary Detection via Bias-Diffusion and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2604.05483)
*Xiaotian Zhou,Di Tang,Xiaofeng Wang,Xiaozhong Liu*

Main category: cs.AI

TL;DR: The paper introduces GMRL-BD, an algorithm to identify untrustworthy topic boundaries in LLMs using Wikipedia's Knowledge Graph and reinforcement learning, tested on popular LLMs with released dataset.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce biased or incorrect answers on certain topics, but without knowing these untrustworthy boundaries, their applications are limited. This research aims to identify these boundaries.

Method: The GMRL-BD algorithm uses a Knowledge Graph from Wikipedia and multiple reinforcement learning agents to efficiently detect topics where LLMs are likely biased, with black-box access and query constraints.

Result: Experiments showed GMRL-BD's efficiency in detecting untrustworthy boundaries with limited queries. A new dataset with biased topics for popular LLMs (e.g., Llama2, Vicuna) was released.

Conclusion: The proposed algorithm effectively identifies untrustworthy topic boundaries in LLMs, providing a practical tool for assessing model reliability and enabling safer applications.
...
### [296] [Auditable Agents](https://arxiv.org/abs/2604.05485)
*Yi Nian,Aojie Yuan,Haiyue Zhang,Jiate Li,Yue Zhao*

Main category: cs.AI

TL;DR: The paper emphasizes the need for auditability in LLM agent systems to ensure accountability, defines five dimensions of auditability, and proposes mechanisms to achieve it while demonstrating feasibility and identifying open research problems.


<details>
  <summary>Details</summary>
Motivation: The rise of LLM agents capable of acting in the world raises concerns about preventing harmful actions and ensuring accountability post-deployment.

Method: Defines five dimensions of agent auditability (action recoverability, lifecycle coverage, policy checkability, responsibility attribution, evidence integrity) and three mechanism classes (detect, enforce, recover). Presents layered evidence including ecosystem measurements, runtime feasibility tests, and controlled recovery experiments.

Result: Basic security prerequisites for auditability are unmet in prominent projects; pre-execution mediation adds minimal overhead; responsibility-relevant information can be partially recovered without conventional logs.

Conclusion: Auditability is essential for accountability in agent systems. The paper proposes an Auditability Card for agent systems and identifies six open research problems.
...
### [297] [Thinking Diffusion: Penalize and Guide Visual-Grounded Reasoning in Diffusion Multimodal Language Models](https://arxiv.org/abs/2604.05497)
*Keuntae Kim,Mingyu Kang,Yong Suk Choi*

Main category: cs.AI

TL;DR: dMLLMs struggle with premature answer generation and poor visual grounding. Proposed PSP and VRG methods improve accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: To address the issues of premature answer generation and insufficient visual grounding in dMLLMs when combined with CoT reasoning.

Method: Introduces Position and Step Penalty (PSP) to delay premature answers and Visual Reasoning Guidance (VRG) to enhance visual grounding.

Result: Achieves up to 7.5% higher accuracy and over 3x speedup compared to baseline methods.

Conclusion: The proposed PSP and VRG effectively improve dMLLMs' reasoning performance and visual alignment.
...
### [298] [OmniDiagram: Advancing Unified Diagram Code Generation via Visual Interrogation Reward](https://arxiv.org/abs/2604.05514)
*Haoyue Yang,Xuanle Zhao,Xuexin Liu,Feibang Jiang,Yao Zhu*

Main category: cs.AI

TL;DR: OmniDiagram is a unified framework for diverse diagram generation, using a novel RL-based visual feedback strategy (Viva) and a large dataset (M3$^2$Diagram) to achieve SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing diagram generation methods are limited in task variety and language support, restricting their applicability.

Method: Introduces OmniDiagram with Viva, a visual feedback strategy using generative inquiries for RL training, and M3$^2$Dataset.

Result: OmniDiagram achieves SOTA performance on diagram code generation benchmarks.

Conclusion: The framework and dataset advance programmable diagram generation with broader applicability and improved visual fidelity.
...
### [299] [UniCreative: Unifying Long-form Logic and Short-form Sparkle via Reference-Free Reinforcement Learning](https://arxiv.org/abs/2604.05517)
...
TL;DR: UniCreative is a reinforcement learning framework that dynamically aligns creative writing tasks for both coherence and expressiveness without relying on supervised data.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of balancing global coherence in long-form narratives with local expressiveness in short-form texts, avoiding costly supervised data.

Method: Proposes AC-GenRM for adaptive constraint-aware rewards and ACPO for policy optimization, eliminating the need for supervised fine-tuning.

Result: AC-GenRM matches expert evaluations, and ACPO improves performance in diverse writing tasks, showing emergent meta-cognitive abilities.

Conclusion: The framework successfully aligns human preferences across writing tasks, validating its effectiveness for creative writing automation.
...
### [300] [Market-Bench: Benchmarking Large Language Models on Economic and Trade Competition](https://arxiv.org/abs/2604.05523)
*Yushuo Zheng,Huiyu Duan,Zicheng Zhang,Yucheng Zhu,Xiongkuo Min,Guangtao Zhai*

Main category: cs.AI

TL;DR: Market-Bench is a benchmark evaluating LLMs' economic capabilities in procurement and retail tasks, revealing performance disparities and dominance by a few models.


<details>
  <summary>Details</summary>
Motivation: To assess how well large language models (LLMs) can manage and acquire economic resources in competitive market scenarios.

Method: A configurable multi-agent supply chain model where LLMs act as retailers, participating in budget-constrained auctions (procurement) and setting prices/slogans (retail). Economic, operational, and semantic metrics are tracked.

Result: Significant performance gaps among 20 LLMs, with a 'winner-take-most' phenomenon—only a few models achieve capital appreciation, while others stagnate near break-even.

Conclusion: Market-Bench offers a reproducible framework to study LLM interactions in competitive markets, highlighting disparities in economic task performance.
...
### [301] [ActivityEditor: Learning to Synthesize Physically Valid Human Mobility](https://arxiv.org/abs/2604.05529)
*Chenjie Yang,Yutian Jiang,Anqi Liang,Wei Qi,Chenyu Wu,Junbo Zhang*

Main category: cs.AI

TL;DR: ActivityEditor is a dual-LLM-agent framework for zero-shot cross-regional trajectory generation, addressing data scarcity in human mobility modeling. It ensures socio-semantic coherence and physical validity through structured intentions and iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Existing data-driven methods for human mobility modeling struggle with data scarcity, limiting their applicability in regions lacking historical trajectory data.

Method: The framework uses two LLM agents: one generates structured intentions and coarse activity chains, while the other refines these into mobility trajectories via reinforcement learning with real-world constraints.
...
Conclusion: The framework provides a robust, generalizable solution for mobility simulation in data-scarce scenarios, with demonstrated effectiveness in cross-regional applications.
...
### [302] [Inventory of the 12 007 Low-Dimensional Pseudo-Boolean Landscapes Invariant to Rank, Translation, and Rotation](https://arxiv.org/abs/2604.05530)
*Arnaud Liefooghe,Sébastien Verel*

Main category: cs.AI

TL;DR: The paper introduces a stronger notion of rank landscape invariance, extending beyond rank-invariance to include neighborhood structure and symmetries. It provides an exhaustive inventory of invariant landscape classes for pseudo-Boolean functions in dimensions 1-3, revealing 12,007 classes and highlighting the impact of non-injective functions on landscape complexity.


<details>
  <summary>Details</summary>
Motivation: The study aims to deepen the understanding of optimization landscapes by considering not just rankings but also neighborhood structures and symmetries, which can influence algorithm performance. This approach helps classify problems more precisely and offers insights into landscape difficulty.

Method: The authors analyze rank landscapes by examining invariant classes for pseudo-Boolean functions in dimensions 1, 2, and 3, including non-injective cases. They compare these classes to rank-invariance alone.

Result: The analysis identifies 12,007 invariant landscape classes, showing that non-injective functions produce more classes than injective ones. It also reveals complex relationships between topological properties and algorithm behaviors.

Conclusion: The inventory of invariant landscape classes serves as a valuable resource for benchmark design and understanding landscape difficulty, aiding in the development of optimization algorithms with controlled hardness.
...
### [303] [Experience Transfer for Multimodal LLM Agents in Minecraft Game](https://arxiv.org/abs/2604.05533)
*Chenghao Li,Jun Liu,Songbo Zhang,Huadong Jian,Hao Ni,Lik-Hang Lee,Sung-Ho Bae,Guoqing Wang,Yang Yang,Chaoning Zhang*

Main category: cs.AI

TL;DR: Echo is a memory framework for multimodal LLM agents that decomposes reusable knowledge into five dimensions to improve efficiency in complex tasks, achieving faster performance in Minecraft.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for agents to efficiently reuse past experiences in complex environments, moving beyond static memory usage.

Method: Echo decomposes knowledge into five dimensions and uses In-Context Analogy Learning (ICAL) to retrieve and adapt experiences for new tasks.

Result: Echo achieved a 1.3x to 1.7x speed-up in Minecraft tasks and demonstrated rapid chain-unlocking of similar items.

Conclusion: Experience transfer enhances the efficiency and adaptability of multimodal LLM agents in interactive environments.
...
### [304] [SignalClaw: LLM-Guided Evolutionary Synthesis of Interpretable Traffic Signal Control Skills](https://arxiv.org/abs/2604.05535)
*Da Lei,Feng Xiao,Lu Li,Yuzhan Liu*

Main category: cs.AI

TL;DR: SIGNALCLAW is a framework using LLMs to synthesize interpretable traffic control skills, achieving competitive performance in both routine and event-driven scenarios.


<details>
  <summary>Details</summary>
Motivation: Current methods like reinforcement learning lack interpretability, while program synthesis is restricted by domain-specific languages. SIGNALCLAW aims to bridge this gap.

Method: Uses LLMs as evolutionary skill generators, combining natural language feedback, event-driven composition, and independent skill evolution.

Result: Achieves low delays (7.8-9.2s in routine scenarios, 11.2-18.5s for emergencies) and outperforms baselines like MaxPressure and DQN.

Conclusion: SIGNALCLAW effectively balances performance and interpretability, enabling human-inspectable and modifiable traffic control policies.
...
### [305] [A canonical generalization of OBDD](https://arxiv.org/abs/2604.05537)
*Florent Capelli,YooJung Choi,Stefan Mengel,Martín Muñoz,Guy Van den Broeck*

Main category: cs.AI

TL;DR: Tree Decision Diagrams (TDD) generalize OBDDs, offering tractability for tasks like model counting and enumeration, and are more succinct, especially for CNF formulas of bounded treewidth.


<details>
  <summary>Details</summary>
Motivation: The paper aims to introduce TDDs as a more powerful and succinct alternative to OBDDs for representing Boolean functions, addressing limitations of OBDDs in handling certain CNF formulas.

Method: The authors propose TDDs as a model generalizing OBDDs, analyze their tractability properties, and study their compilation from CNF formulas using bottom-up compilation, relating it to factor width.

Result: TDDs retain the tractability of OBDDs (e.g., model counting) while being more succinct, especially for CNF formulas with bounded treewidth, which OBDDs cannot handle efficiently.

Conclusion: TDDs are a promising model for Boolean functions, offering advantages over OBDDs in succinctness and applicability to a broader class of problems, particularly those involving bounded treewidth CNF formulas.
...
### [306] [From Large Language Model Predicates to Logic Tensor Networks: Neurosymbolic Offer Validation in Regulated Procurement](https://arxiv.org/abs/2604.05539)
*Cedric Haufe,Frieder Stolzenburg*

Main category: cs.AI

TL;DR: A neurosymbolic approach combining symbolic and subsymbolic AI for validating offer documents in regulated institutions, emphasizing interpretability and XAI support.


<details>
  <summary>Details</summary>
Motivation: To ensure decisions in regulated public institutions are factually correct and legally verifiable by integrating domain knowledge with language models.

Method: Uses a language model for information extraction and an LTN (Logic Tensor Network) for auditable decision-making, linking domain knowledge to semantic understanding.

Result: Performance comparable to existing models, with advantages in interpretability, modular predicate extraction, and XAI support.

Conclusion: The neurosymbolic pipeline offers a viable solution for verifiable and interpretable decision-making in regulated settings.
...
### [307] [ResearchEVO: An End-to-End Framework for Automated Scientific Discovery and Documentation](https://arxiv.org/abs/2604.05587)
*Zhe Zhao,Haibin Wen,Jiaming Ma,Jiachang Zhan,Tianyi Xu,Ye Wei,Qingfu Zhang*

Main category: cs.AI

TL;DR: ResearchEVO is a framework that automates scientific discovery and paper generation by evolving algorithms and grounding findings in theory, validated on quantum error correction and physics-informed neural networks.


<details>
  <summary>Details</summary>
Motivation: To emulate the two-stage scientific breakthrough process (undirected experimentation followed by theoretical grounding) computationally.

Method: Uses LLM-guided bi-dimensional co-evolution for algorithm discovery and retrieval-augmented generation for paper writing with anti-hallucination checks.

Result: Discovered novel algorithms in quantum error correction and neural networks and generated publication-ready papers with correct citations.

Conclusion: ResearchEVO is the first end-to-end system for automated algorithmic discovery and scientific documentation, validated on real-world problems.
...
### [308] [Label Effects: Shared Heuristic Reliance in Trust Assessment by Humans and LLM-as-a-Judge](https://arxiv.org/abs/2604.05593)
*Xin Sun,Di Wu,Sijing Qin,Isao Echizen,Abdallah El Ali,Saku Sugawara*

Main category: cs.AI

TL;DR: LLMs as evaluators are biased by source labels, mirroring human reliance on such labels for trust judgments.


<details>
  <summary>Details</summary>
Motivation: To challenge the reliability of LLMs used as automated evaluators (LLM-as-a-Judge) by examining how source labels bias trust judgments.

Method: Used a counterfactual design to compare trust judgments of human-authored vs. AI-generated labels, analyzed human eye-tracking data, and examined LLM internal states (attention allocation and decision uncertainty).

Result: Both humans and LLMs trust human-labeled content more. LLMs show denser attention to labels than content, mirroring human gaze patterns, and higher uncertainty under AI labels.

Conclusion: Source labels bias evaluations, raising concerns about label-sensitive LLM-as-a-Judge applications. Aligning models with human preferences may propagate biases, suggesting a need for debiased evaluation methods.
...
### [309] [Beyond Behavior: Why AI Evaluation Needs a Cognitive Revolution](https://arxiv.org/abs/2604.05631)
*Amir Konigsberg*

Main category: cs.AI

TL;DR: The paper critiques Turing's behavioral test for intelligence in AI, arguing it has limited the field by ignoring questions about internal processes, paralleling psychology's shift from behaviorism to cognitivism. It calls for a new epistemology in AI that values process and mechanism alongside behavior.


<details>
  <summary>Details</summary>
Motivation: To highlight how Turing's behavioral test has constrained AI research by focusing solely on outputs, preventing exploration of internal mechanisms and processes that define intelligence.

Method: The paper traces the influence of Turing's behavioral epistemology in AI, draws parallels to psychology's transition from behaviorism to cognitivism, and articulates a new epistemology for AI.

Result: The authors argue that behavioral evidence alone is insufficient for attributing intelligence and propose a shift to include internal computational processes in evaluating AI systems.

Conclusion: The paper advocates for an epistemological transition in AI, similar to psychology's cognitive revolution, to enable asking and answering questions about internal mechanisms and processes in intelligent systems.
...
### [310] [PECKER: A Precisely Efficient Critical Knowledge Erasure Recipe For Machine Unlearning in Diffusion Models](https://arxiv.org/abs/2604.05634)
*Zhiyong Ma,Zhitao Deng,Huan Tang,Jialin Chen,Zhijun Zheng,Zhengping Li,Qingyuan Chuai*

Main category: cs.AI

TL;DR: PECKER is an efficient machine unlearning (MU) method that uses a saliency mask to prioritize gradient updates, reducing training time and computational overhead while maintaining effectiveness.


<details>
  <summary>Details</summary>
Motivation: Existing MU methods are inefficient due to poorly directed gradient updates, leading to high training time and computational costs. PECKER aims to address these inefficiencies.

Method: PECKER introduces a saliency mask within a distillation framework to prioritize updates for parameters most relevant to forgetting targeted data, reducing unnecessary gradient computations.

Result: PECKER achieves faster unlearning for both class and concept forgetting on CIFAR-10 and STL-10 datasets, while maintaining alignment with the true image distribution.

Conclusion: PECKER outperforms existing MU methods by improving training efficiency without compromising unlearning efficacy.
...
### [311] [CuraLight: Debate-Guided Data Curation for LLM-Centered Traffic Signal Control](https://arxiv.org/abs/2604.05663)
*Qing Guo,Xinhang Li,Junyu Chen,Zheng Guo,Shengzhe Xu,Lin Zhang,Lei Li*

Main category: cs.AI

TL;DR: The paper introduces CuraLight, a framework combining reinforcement learning (RL) and large language models (LLMs) to improve traffic signal control (TSC) by addressing issues like interpretability, data scarcity, and generalization.


<details>
  <summary>Details</summary>
Motivation: Traditional RL and LLM-based approaches for TSC lack interpretability, sufficient interaction data, and generalization to diverse traffic scenarios.

Method: CuraLight uses an RL agent to generate high-quality interaction data for fine-tuning an LLM-based controller, followed by a multi-LLM ensemble system to evaluate and refine signal timing decisions.

Result: Experiments show CuraLight outperforms baselines, reducing travel time by 5.34%, queue length by 5.14%, and waiting time by 7.02%.

Conclusion: Combining RL-assisted exploration with deliberation-based data curation enhances the scalability and interpretability of TSC systems.
...
### [312] [QA-MoE: Towards a Continuous Reliability Spectrum with Quality-Aware Mixture of Experts for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2604.05704)
*Yitong Zhu,Yuxuan Jiang,Guanxuan Jiang,Bojing Hou,Peng Yuan Zhou,Ge Lin Kan,Yuyang Wang*

Main category: cs.AI

TL;DR: QA-MoE, a Quality-Aware Mixture-of-Experts framework, addresses noise and missing data in Multimodal Sentiment Analysis by leveraging self-supervised uncertainty to guide expert routing.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Multimodal Sentiment Analysis struggle with dynamic noise and missing modalities, treating them as discrete cases or assuming fixed corruption ratios, which limits adaptability.

Method: Introduces a Continuous Reliability Spectrum and QA-MoE, which uses self-supervised aleatoric uncertainty to quantify modality reliability and guide expert routing.

Result: QA-MoE achieves competitive or state-of-the-art performance across diverse degradation scenarios and demonstrates a promising One-Checkpoint-for-All property.

Conclusion: The proposed framework effectively handles varying reliability conditions in multimodal inputs, improving adaptability and performance in real-world scenarios.
...
### [313] [Can Large Language Models Reinvent Foundational Algorithms?](https://arxiv.org/abs/2604.05716)
*Jian Zhao,Haoren Luo,Yu Wang,Yuhan Cao,Pingyue Sheng,Tianxing He*

Main category: cs.AI

TL;DR: LLMs tested for foundational innovation using an 'Unlearn-and-Reinvent' pipeline to see if they can recreate key algorithms after unlearning them. Results show mixed success, with hints improving outcomes, and highlight the role of generative verifiers in sustaining reasoning.


<details>
  <summary>Details</summary>
Motivation: To assess whether LLMs can achieve foundational innovation by reinventing core algorithms after unlearning them, a key step in evaluating their creative potential.

Method: The 'Unlearn-and-Reinvent' pipeline uses GRPO-based unlearning to remove specific algorithms (e.g., Dijkstra's or Euclid's) from LLMs, then tests reinvention in controlled settings with varying hint levels.

Result: Qwen3-4B-Thinking-2507 reinvented 50% of algorithms without hints, 70% with minimal hints, and 90% with more hints. Step-by-step hints failed for complex algorithms, but test-time reinforcement learning helped with Strassen's algorithm.

Conclusion: LLMs show potential for foundational innovation, but success depends on hint levels and generative verifiers to sustain reasoning. Current limits exist, especially for complex algorithms.
...
### [314] [Emergent social transmission of model-based representations without inference](https://arxiv.org/abs/2604.05777)
*Silja Keßler,Miriam Bautista-Salinero,Claudio Tennie,Charley M. Wu*

Main category: cs.AI

TL;DR: The paper explores how simple social learning cues, without complex mentalizing, can effectively transmit higher-level knowledge, using reinforcement learning simulations to demonstrate this process.


<details>
  <summary>Details</summary>
Motivation: To understand how humans acquire complex knowledge from others despite cognitive limitations, challenging the assumption that costly mentalizing (like inferring beliefs) is necessary.

Method: Reinforcement learning simulations where a naïve agent learns either alone or by observing an expert, using heuristic action selection or value representation boosting without inferring mental states.

Result: Simple social cues bias the learner's experience, causing their representation to align with the expert's. Model-based learners benefit the most, showing faster learning and more expert-like representations.

Conclusion: Cultural transmission can emerge from non-mentalizing, simple social learning processes that leverage asocial learning mechanisms.
...
### [315] [Hierarchical Reinforcement Learning with Augmented Step-Level Transitions for LLM Agents](https://arxiv.org/abs/2604.05808)
*Shuai Zhen,Yanhua Yu,Ruopei Guo,Nan Cheng,Yang Deng*

Main category: cs.AI

TL;DR: STEP-HRL is a hierarchical reinforcement learning framework that improves LLM agent efficiency by focusing on single-step transitions and compact history summaries, outperforming baselines in performance and token usage.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents rely on long interaction histories, leading to high computational costs and scalability issues. STEP-HRL addresses this by optimizing step-level learning.

Method: Proposes a hierarchical framework (STEP-HRL) that conditions on single-step transitions and uses subtasks to represent global progress. It includes a local progress module to summarize interaction history compactly.

Result: STEP-HRL outperforms baselines on ScienceWorld and ALFWorld benchmarks, achieving better performance, generalization, and reduced token usage.

Conclusion: STEP-HRL effectively addresses scalability and computational cost challenges in LLM agents, demonstrating superior efficiency and performance.
...
### [316] [Reciprocal Trust and Distrust in Artificial Intelligence Systems: The Hard Problem of Regulation](https://arxiv.org/abs/2604.05826)
*Martino Maggetti*

Main category: cs.AI

TL;DR: The paper explores how AI systems should be seen as having agency, enabling trust relationships with humans, and the implications for AI regulation.


<details>
  <summary>Details</summary>
Motivation: The growing need to understand and regulate AI's trustworthiness in governance and deployment drives the discussion.

Method: The article argues for recognizing AI's agency and examines trust dynamics between AI and humans, focusing on regulatory challenges.

Result: The study highlights key tensions and dilemmas in AI regulation due to reciprocal trust dynamics.

Conclusion: The paper identifies unresolved issues in AI governance, emphasizing the need for frameworks addressing AI's agency and trust relationships.
...
### [317] [Vision-Guided Iterative Refinement for Frontend Code Generation](https://arxiv.org/abs/2604.05839)
*Hannah Sansford,Derek H. C. Law,Wei Liu,Abhishek Tripathi,Niresh Agarwal,Gerrit J. J. van den Burg*

Main category: cs.AI

TL;DR: A fully automated framework uses a vision-language model as a visual critic to refine code generation for web development, improving solution quality by up to 17.8%. Fine-tuning with LoRA achieves 25% of these gains.


<details>
  <summary>Details</summary>
Motivation: Multi-stage human-in-the-loop refinement for code generation is effective but costly, especially in visual domains like frontend web development.

Method: A vision-language model acts as a visual critic to provide structured feedback on rendered webpages, guiding iterative code refinement. LoRA fine-tuning is also tested to internalize improvements.

Result: The framework improves solution quality by up to 17.8% across refinement cycles. Fine-tuning achieves 25% of the critic's gains without increasing token counts.

Conclusion: Automated, VLM-based critique significantly enhances code generation quality, emphasizing the need for iterative refinement in visually complex outputs like web development.
...
### [318] [Deep Researcher Agent: An Autonomous Framework for 24/7 Deep Learning Experimentation with Zero-Cost Monitoring](https://arxiv.org/abs/2604.05854)
*Xiangyue Zhang*
...
### [319] [When Do We Need LLMs? A Diagnostic for Language-Driven Bandits](https://arxiv.org/abs/2604.05859)
*Uljad Berdica,Fernando Acero,Anton Ipsen,Parisa Zehtabi,Michael Cashmore,Manuela Veloso*

Main category: cs.AI

TL;DR: LLMP-UCB, a bandit algorithm using LLMs for uncertainty estimates in CMABs, is compared to lightweight numerical bandits. The latter match or exceed LLM accuracy at lower cost, with embedding dimensionality as a tunable lever. A geometric diagnostic helps decide between LLM and lightweight solutions.


<details>
  <summary>Details</summary>
Motivation: To address the computational cost and uncertainty estimation challenges of using LLMs in non-episodic sequential decision-making (e.g., finance), while exploring cost-effective alternatives.

Method: Introduce LLMP-UCB for uncertainty estimates via repeated LLM inference. Compare it to lightweight numerical bandits using text embeddings (dense or Matryoshka). Test embedding dimensionality as a tuning parameter.

Result: Lightweight numerical bandits match or outperform LLM-based solutions at a fraction of the cost. Embedding dimensionality effectively balances exploration-exploitation.

Conclusion: A geometric diagnostic guides practitioners in choosing between LLM-driven reasoning and lightweight bandits, offering a cost-effective, uncertainty-aware framework for financial AI applications.
...
### [320] [Joint Knowledge Base Completion and Question Answering by Combining Large Language Models and Small Language Models](https://arxiv.org/abs/2604.05875)
*Yinan Liu,Dongying Lin,Sigang Luo,Xiaochun Yang,Bin Wang*

Main category: cs.AI

TL;DR: A framework called JCQL combines LLM and SLM strengths to jointly enhance knowledge base completion (KBC) and knowledge base question answering (KBQA), improving performance for both tasks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for joint KBC and KBQA rely on SLMs, ignoring LLMs' reasoning capabilities. Combining both can mutually reinforce the tasks.

Method: JCQL integrates LLM with SLM: KBC enhances KBQA by reducing LLM hallucination and computational costs, while KBQA enhances KBC by providing supplementary training data.

Result: JCQL outperforms baselines on two benchmark datasets for both KBC and KBQA tasks.

Conclusion: Combining LLM and SLM in JCQL effectively improves performance for both KBC and KBQA, demonstrating mutual reinforcement between the tasks.
...
### [321] [HybridKV: Hybrid KV Cache Compression for Efficient Multimodal Large Language Model Inference](https://arxiv.org/abs/2604.05887)
*Bowen Zeng,Feiyang Ren,Jun Zhang,Xiaoling Gu,Ke Chen,Lidan Shou,Huan Li*

Main category: cs.AI

TL;DR: HybridKV, a hybrid KV cache compression framework, reduces memory overhead and speeds up decoding in multimodal models by employing stage-specific compression strategies for attention heads.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of key-value (KV) caches in Multimodal Large Language Models (MLLMs) leads to high memory overhead and latency, even on high-end GPUs. Existing solutions lack adaptability to heterogeneous attention head behaviors.

Method: HybridKV introduces a three-stage framework: classifying heads as static or dynamic, hierarchically allocating KV budgets, and compressing static heads via text-prior pruning and dynamic heads via chunk-wise retrieval.

Result: HybridKV achieves a 7.9x reduction in KV cache memory and 1.52x faster decoding, with minimal performance loss or even improvement compared to full-cache MLLMs.

Conclusion: HybridKV effectively addresses memory and latency issues in MLLMs by leveraging tailored compression strategies for different attention head behaviors.
...
### [322] [Context-Value-Action Architecture for Value-Driven Large Language Model Agents](https://arxiv.org/abs/2604.05939)
*TianZe Zhang,Sirui Sun,Yuhang Xie,Xin Zhang,Zhiqiang Wu,Guojie Song*

Main category: cs.AI

TL;DR: The paper addresses behavioral rigidity in LLM agents, showing that more reasoning worsens value polarization. It introduces the CVA architecture to improve fidelity and diversity, validated on a large dataset.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents lack behavioral fidelity and diversity, masked by biased evaluations. The study aims to uncover and fix this issue.

Method: Proposes the CVA architecture, based on S-O-R and Schwartz's Theory, using a Value Verifier trained on human data to decouple reasoning from action.

Result: CVA outperforms baselines on CVABench (1.1M interactions), reducing polarization while enhancing fidelity and interpretability.

Conclusion: CVA effectively mitigates behavioral rigidity and polarization in LLMs, offering better performance and interpretability.
...
### [323] [Towards Trustworthy Report Generation: A Deep Research Agent with Progressive Confidence Estimation and Calibration](https://arxiv.org/abs/2604.05952)
*Yi Yuan,Xuhong Wang,Shanzhe Lei*

Main category: cs.AI

TL;DR: A novel deep research agent is introduced, focusing on improving report trustworthiness via confidence estimation and calibration, validated by enhanced interpretability and user trust.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation frameworks for research agents lack measures for trustworthiness, especially in open-ended scenarios without ground-truth answers, leading to potential misinformation.

Method: The proposed agent uses progressive confidence estimation, deliberative search, deep retrieval, and multi-hop reasoning to ground claims in evidence and assign confidence scores.

Result: The system produces more trustworthy reports, with experiments showing improved interpretability and increased user trust.

Conclusion: The approach successfully addresses trustworthiness in automated report generation, offering a more reliable and transparent method for knowledge exploration.
...
### [324] [Beyond Compromise: Pareto-Lenient Consensus for Efficient Multi-Preference LLM Alignment](https://arxiv.org/abs/2604.05965)
*Renxuan Tan,Rongpeng Li,Zhifeng Zhao,Honggang Zhang*

Main category: cs.AI

TL;DR: The paper introduces PLC, a game-theoretic framework for multi-objective preference alignment in LLMs, enabling dynamic negotiation and escape from local optima to achieve global Pareto improvements.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with diverse human values requires moving beyond single-preference paradigms. Current MPA methods often converge to suboptimal local points due to rigid trade-off handling.

Method: Proposes PLC, which uses consensus-driven lenient gradient rectification to tolerate local degradation if a dominant coalition surplus exists, enabling exploration of the Pareto frontier.

Result: Theoretical and experimental results show PLC outperforms baselines in escaping local optima and achieving better Pareto-optimal alignment.

Conclusion: PLC demonstrates the effectiveness of negotiation-driven alignment for MPA, offering a promising approach to robust LLM deployment.
...
### [325] [Flowr -- Scaling Up Retail Supply Chain Operations Through Agentic AI in Large Scale Supermarket Chains](https://arxiv.org/abs/2604.05987)
...
TL;DR: The paper introduces Flowr, an AI framework that automates retail supply chain workflows by decomposing tasks into specialized AI agents, reducing manual effort and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: Manual retail supply chain workflows are repetitive, decision-intensive, and difficult to scale, leading to inefficiencies despite data analytics investments.

Method: Flowr uses specialized AI agents for distinct cognitive roles, coordinated by a central reasoning LLM, with human oversight via a Model Context Protocol interface.

Result: Flowr reduces manual coordination, improves demand-supply alignment, and enables proactive handling of exceptions at a scale beyond manual processes.

Conclusion: Flowr provides a scalable, domain-independent solution for automating retail supply chains, validated in a large supermarket chain setting.
...
### [326] [Epistemic Blinding: An Inference-Time Protocol for Auditing Prior Contamination in LLM-Assisted Analysis](https://arxiv.org/abs/2604.06013)
*Michael Cuccarese*

Main category: cs.AI

TL;DR: The paper introduces 'epistemic blinding,' a protocol to audit how much of an LLM's output stems from provided data versus memorized priors, demonstrating its impact in drug prioritization and equity screening.


<details>
  <summary>Details</summary>
Motivation: To address the invisibility of whether LLM outputs are data-driven or based on memorized priors, which undermines auditability in agentic systems.

Method: Uses an inference-time protocol that anonymizes entity identifiers before prompting, then compares results to an unblinded control. Also includes LLM-guided optimization and blinded reasoning in a drug target prioritization system.

Result: Blinding altered 16% of top-20 drug predictions without affecting validated target recovery, and showed 30-40% bias in equity screening due to brand recognition.

Conclusion: Epistemic blinding restores auditability, enabling researchers to discern the source of LLM outputs without claiming improved results.
...
### [327] [How LLMs Follow Instructions: Skillful Coordination, Not a Universal Mechanism](https://arxiv.org/abs/2604.06015)
*Elisabetta Rocchetti,Alfio Ferrara*

Main category: cs.AI

TL;DR: Instruction tuning in language models relies on skillful coordination of diverse linguistic capabilities rather than a single universal mechanism for following instructions.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand whether instruction-following in language models is driven by a universal mechanism or by the compositional deployment of skills.

Method: The research uses diagnostic probing across nine diverse tasks in three instruction-tuned models, analyzing representational sharing, cross-task transfer, causal ablation, and temporal dynamics.

Result: The findings show limited representational sharing, weak cross-task transfer, sparse asymmetric dependencies, and task stratification by complexity. Temporal analysis reveals dynamic monitoring during generation.

Conclusion: Instruction-following is better explained as the coordination of diverse linguistic skills rather than a single abstract process.
...
### [328] [Artificial Intelligence and the Structure of Mathematics](https://arxiv.org/abs/2604.06107)
*Maissam Barkeshli,Michael R. Douglas,Michael H. Freedman*

Main category: cs.AI

TL;DR: AI is expected to revolutionize mathematics by exploring formal proofs and uncovering new insights, potentially answering whether math is discovered or invented.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore how AI can provide a new perspective on the global structure of mathematics, complementing traditional logic, and address foundational questions.

Method: The approach involves analyzing the formal structure of mathematics using universal proof and structural hypergraphs, and outlining criteria for AI models to achieve automated mathematical discovery.

Result: The essay suggests that AI could autonomously traverse mathematical structures, offering deeper understanding and insights into the nature of mathematics.

Conclusion: AI may transform mathematics by revealing its underlying structure and answering long-standing philosophical questions about its nature.
...
### [329] [Claw-Eval: Toward Trustworthy Evaluation of Autonomous Agents](https://arxiv.org/abs/2604.06132)
...
TL;DR: Claw-Eval is a comprehensive evaluation suite addressing limitations in existing benchmarks for large language model agents, introducing trajectory-aware grading, safety/robustness metrics, and multimodal task coverage.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for autonomous agents are limited by opaque grading, inadequate safety/robustness evaluation, and narrow modality coverage, necessitating a more robust framework.

Method: Claw-Eval uses 300 human-verified tasks across 9 categories, recording agent actions via execution traces, audit logs, and snapshots to enable fine-grained grading. It evaluates Completion, Safety, and Robustness with metrics like Average Score and Pass@k.

Result: Experiments on 14 models show trajectory-opaque evaluation misses 44% of safety violations, error injection degrades consistency, and multimodal performance varies significantly.

Conclusion: Claw-Eval provides actionable insights for developing reliable agents and highlights gaps in current evaluation methods.
