...
TL;DR: The paper introduces a method for multi-view 3D artistic style transfer that preserves geometric consistency for downstream tasks like SLAM and reconstruction, using test-time optimization and correspondence-based losses.


<details>
  <summary>Details</summary>
Motivation: Extending artistic style transfer to multi-view 3D scenes is challenging due to disruptions in geometric correspondences. The goal is to maintain usability for downstream 3D tasks without relying on camera poses or explicit 3D representations during training.

Method: A feed-forward stylization network is trained with per-scene test-time optimization. It uses a composite objective: appearance transfer (via AdaIN-inspired loss) and geometry preservation (via correspondence-based and depth-preservation losses). SuperPoint and SuperGlue ensure multi-view consistency, while MiDaS/DPT aids depth preservation.

Result: Evaluated on Tanks and Temples and Mip-NeRF 360, the method improves structural consistency (measured by DSD and Chamfer distance) and SLAM stability, outperforming baselines in trajectory and point-cloud consistency while maintaining stylization quality (measured by CHD).

Conclusion: The proposed approach successfully balances artistic style transfer with geometric preservation, making it viable for downstream 3D tasks like SLAM and multi-view reconstruction.
...
TL;DR: PA-SFM is a tracker-free framework for 3D handheld photoacoustic tomography, using single-modality data for sensor pose recovery and high-fidelity reconstruction via differentiable acoustic modeling.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D handheld photoacoustic tomography relies on bulky, expensive external sensors for motion correction, limiting clinical flexibility and accessibility.

Method: PA-SFM integrates the acoustic wave equation into a differentiable programming pipeline, optimizing both 3D source distribution and sensor pose via GPU-accelerated gradient descent, with a coarse-to-fine strategy for robust convergence.

Result: PA-SFM achieves sub-millimeter positioning accuracy and restores high-resolution 3D vascular structures, comparable to ground-truth benchmarks.

Conclusion: PA-SFM offers a low-cost, software-defined solution for clinical freehand photoacoustic imaging, validated by simulations and in-vivo experiments.
...
TL;DR: TRACE is a unified framework for CO2 plume segmentation and emission flux classification from thermal video, achieving top performance with domain-specific innovations like TGAA and ATF.


<details>
  <summary>Details</summary>
Motivation: Existing systems lack the ability to measure CO2 emissions from free-roaming cattle continuously and spatially without confinement or contact, which is crucial for metabolic monitoring and carbon accounting.

Method: TRACE integrates a Thermal Gas-Aware Attention (TGAA) encoder for precise plume segmentation and an Attention-based Temporal Fusion (ATF) module for flux classification, trained via a progressive curriculum.

Result: TRACE outperforms 15 state-of-the-art models, achieving an mIoU of 0.998, and excels in both segmentation and classification metrics.

Conclusion: TRACE enables non-invasive, continuous per-animal CO2 monitoring, validated by ablation studies confirming the necessity of its components.
...
TL;DR: FlowHijack is the first backdoor attack framework targeting flow-matching VLAs, exploiting their continuous vector-field dynamics.


<details>
  <summary>Details</summary>
Motivation: To explore security vulnerabilities in Vision-Language-Action (VLA) models, particularly backdoor attacks on their unique action generation mechanism.

Method: Combines a $τ$-conditioned injection strategy for initial phase manipulation with a dynamics mimicry regularizer.

Result: Achieves high attack success rates with stealthy triggers while preserving benign task performance and generating indistinguishable malicious actions.

Conclusion: Reveals a critical vulnerability in continuous embodied models, emphasizing the need for defenses targeting generative dynamics.
...
TL;DR: The paper proposes a feature representation (Checksum Count Vectors) to automate the detection of duplicates and variants in digitized magnetic media, achieving high accuracy and aiding in the preservation of historical computing artifacts.


<details>
  <summary>Details</summary>
Motivation: To automate parts of the preservation process for early home computing artifacts, allowing volunteers to focus on contextual and historical contributions rather than technical challenges.

Method: Uses Checksum Count Vectors to detect duplicates and variants in a large collection of decoded tape images (n=4902).

Result: Achieved 58% accuracy in detecting variants and 97% accuracy in identifying alternative copies, even with up to 75% missing records.

Conclusion: The method is a significant step toward fully automated pipelines for restoring, de-duplicating, and integrating historical digital artifacts through sequence matching and repair.
...
TL;DR: A zero-shot pipeline for predicting traffic accidents in videos without labeled data, using pre-trained models for temporal localization, impact location, and collision type classification.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of predicting traffic accidents in surveillance videos without relying on labeled real-world training data, leveraging zero-shot techniques.

Method: Separates the problem into three modules: temporal localization via peak detection on frame-difference signals, impact location using weighted centroid of optical flow maps, and collision type classification via CLIP embeddings.

Result: The pipeline effectively predicts accident timing, location, and type using only pre-trained models, avoiding domain-specific fine-tuning.

Conclusion: The zero-shot approach demonstrates feasibility for accident prediction in videos without labeled data, with an implementation available publicly.
...
TL;DR: The paper introduces Grid2Matrix (G2M) to evaluate Vision-Language Models (VLMs) on fine visual detail tasks, finding they fail early on simple grids, termed 'Digital Agnosia.'


<details>
  <summary>Details</summary>
Motivation: Existing VLM evaluations often miss failures in capturing all visual details, so the authors created G2M to systematically test visual complexity.

Method: G2M uses color grids with mappings, varying size and colors to increase visual complexity without semantic confounds. VLMs are probed for their performance.

Result: VLMs collapse early on small grids, despite visual encoders preserving grid info. Errors are structured, related to patch boundaries, and not fixed by scaling or alignment.

Conclusion: G2M highlights VLMs' gap in expressing visual details in language, useful for tasks where fine details matter, like tables or charts.
...
TL;DR: GaussLock defends 3D generative models against fine-tuning attacks by using parameter-space immunization and attribute-aware trap losses, preserving authorized tasks while disrupting unauthorized reconstructions.


<details>
  <summary>Details</summary>
Motivation: To protect pre-trained 3D generative models from intellectual property theft via fine-tuning attacks, which exploit vulnerabilities in their explicit Gaussian representations.

Method: GaussLock integrates authorized distillation with attribute-aware trap losses targeting position, scale, rotation, opacity, and color to disrupt structural integrity during unauthorized fine-tuning.

Result: GaussLock effectively degrades unauthorized reconstructions (higher LPIPS, lower PSNR) while maintaining performance on authorized fine-tuning.

Conclusion: GaussLock provides a lightweight yet effective defense for 3D generative models, balancing fidelity and security.
...
TL;DR: The paper shows that instance density (face count) significantly impacts machine learning model performance, degrading it as density increases, and suggests density as a dimension of data hardness.


<details>
  <summary>Details</summary>
Motivation: To understand and quantify the impact of instance density (face count) on data complexity, moving beyond general observations like 'crowded scenes are harder.'

Method: Conducted controlled experiments on WIDER FACE and Open Images datasets, ensuring balanced sampling with 1 to 18 faces per image, and analyzed performance across classification, regression, and detection tasks.

Result: Model performance degrades monotonically with increasing face count, and models trained on low-density data fail to generalize to higher densities, showing a 4.6x increase in error rates.

Conclusion: Instance density is an intrinsic dimension of data hardness, necessitating interventions like curriculum learning and density-stratified evaluation.
...
TL;DR: The paper introduces a framework to diagnose whether jaguar re-identification models rely on correct visual evidence (coat patterns) or incorrect cues like background context. It includes two diagnostics and a curated dataset, tested with mitigation methods.


<details>
  <summary>Details</summary>
Motivation: To ensure jaguar re-ID models use the right evidence (coat patterns) rather than misleading features like background or silhouette.

Method: Proposes a diagnostic framework with two axes: context ratio (background/foreground) and laterality (cross-flank retrieval). Uses a Pantanal jaguar benchmark with segmentation masks and employs mitigation methods like ArcFace fine-tuning and Lorentz hyperbolic embeddings.

Result: The framework helps evaluate whether models rely on correct visual evidence, with case studies showing the impact of different mitigation strategies.

Conclusion: The paper provides tools to assess and improve the reliability of wildlife re-ID models by focusing on correct evidence, not just retrieval performance.
...
TL;DR: Educational diagrams are crucial but lack methods to generate them accurately and engagingly. Existing approaches either garble labels or lack visual appeal. The proposed CAGE framework combines LLMs for accuracy and diffusion models for aesthetics, with a new dataset (EduDiagram-2K) to support this approach.


<details>
  <summary>Details</summary>
Motivation: The need for educational diagrams that are both accurate and visually appealing in K-12 instruction is unmet by current methods, which compromise either label fidelity or visual quality.

Method: The authors introduce CAGE: an LLM generates executable code for accurate diagrams, and a diffusion model enhances visual quality using ControlNet, preserving label fidelity. They also create EduDiagram-2K, a dataset of 2,000 paired diagrams.

Result: The paper quantifies the trade-off between accuracy and aesthetics in existing methods and presents proof-of-concept results for CAGE, demonstrating its potential to bridge the gap.

Conclusion: CAGE offers a promising solution for generating educational diagrams that balance accuracy and visual appeal, supported by the new EduDiagram-2K dataset. The work also outlines a research agenda for further development.
...
TL;DR: TaFall is a privacy-preserving fall detection system using thermal array sensing to model falls as balance degradation, achieving high accuracy and low false alarms in real-world deployments.


<details>
  <summary>Details</summary>
Motivation: Falls in older adults are a serious issue, and existing privacy-preserving detection methods lack reliability. TaFall aims to improve this by focusing on biomechanical balance dynamics.

Method: TaFall uses thermal array sensing to estimate balance dynamics. Key innovations include an appearance-motion fusion model, balance-aware learning, and pose-bridged pretraining.

Result: TaFall achieves 98.26% detection rate, 0.65% false alarms in testing, and 0.00126% false alarms in real-world deployments, proving robustness in diverse environments.

Conclusion: TaFall is a reliable, privacy-preserving solution for fall detection in everyday settings, validated by extensive testing and real-world use.
...
TL;DR: EDFNet is a modular early-fusion segmentation framework for UAVs to detect thin obstacles like wires and branches by integrating RGB, depth, and edge data. It outperforms existing methods on key metrics but struggles with ultra-thin structures.


<details>
  <summary>Details</summary>
Motivation: Thin obstacles (e.g., wires, poles) are hard to detect due to low pixel coverage, weak contrast, and class imbalance. Current methods overlook multimodal cues.

Method: EDFNet uses early fusion of RGB, depth, and edge data, tested with U-Net and DeepLabV3 across 16 configurations on the DDOS dataset.

Result: Pretrained RGBDE U-Net achieved the best scores (Thin-Structure Evaluation: 0.244, mIoU: 0.219) and maintained high FPS (19.62). Ultra-thin categories remained challenging.

Conclusion: Early RGB-Depth-Edge fusion is a practical baseline for thin-obstacle segmentation, though ultra-thin detection remains an open problem.
...
TL;DR: The paper examines privacy risks in Online Vision Language Models (OVLMs) due to Personally Identifiable Information (PII) disclosure in images and proposes methods to protect privacy while retaining image utility.


<details>
  <summary>Details</summary>
Motivation: The rise in OVLMs for image processing has heightened privacy risks, as users unknowingly expose PII through uploaded images, even with seemingly harmless details.

Method: The study analyzes contextual relationships in images that reveal PII and introduces techniques to safeguard privacy without compromising the utility of images in VLM applications.

Result: The proposed privacy-protection methods effectively balance utility and privacy, demonstrating their practicality in online image processing.

Conclusion: The paper underscores the importance of addressing PII disclosure in OVLMs and provides viable solutions to mitigate privacy risks while maintaining image utility.
...
TL;DR: Test-time augmentation (TTA) often degrades accuracy in medical imaging, with drops as severe as 31.6% in some cases, contrary to common assumptions.


<details>
  <summary>Details</summary>
Motivation: The study investigates the widely held belief that TTA improves classification accuracy in medical imaging, questioning its effectiveness across various models and datasets.

Method: The authors conducted a systematic empirical study using three MedMNIST v2 benchmarks and four architectures, testing TTA with standard augmentation pipelines.

Result: TTA consistently degraded accuracy relative to single-pass inference, with significant drops (e.g., 31.6% for ResNet-18 on pathology images). Only ResNet-18 on dermatology images showed a minor improvement (+1.6%). The primary cause was identified as the distribution shift between augmented and training-time inputs.

Conclusion: TTA should not be applied by default in medical imaging; it must be validated for specific model-dataset combinations to avoid accuracy degradation.
...
TL;DR: 3D-GeoFlow is a novel Attention-Guided Continuous Flow Matching framework for constructing high-resolution 3D geological models from sparse 1D/2D data, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional methods fail to handle non-linear topological discontinuities under extreme sparsity, and deep generative models struggle with sparse categorical grids. A new approach is needed.

Method: Proposes 3D-GeoFlow, using Attention-Guided Continuous Flow Matching to simulate-free, continuous vector field regression and 3D Attention Gates for feature propagation.

Result: Outperforms heuristic interpolations and standard diffusion baselines on a dataset of 2,200 3D geological cases, demonstrating significant improvements.

Conclusion: 3D-GeoFlow represents a paradigm shift in sparse multimodal geological modeling, offering stable and deterministic optimal transport paths.
...
TL;DR: The paper proposes PASTA, a weakly supervised pipeline for object segmentation and classification in unstructured environments, achieving significant training time reduction and superior performance in industrial and agricultural applications.


<details>
  <summary>Details</summary>
Motivation: Existing perception systems fail to meet real-time processing, pixel-level segmentation precision, and robust accuracy in industrial and agricultural domains due to reliance on heavily annotated datasets.

Method: PASTA uses weak image-level supervision and distribution analysis in self-supervised Vision Transformer (ViT) feature spaces, combined with semantic text-prompts via the Segment Anything Model 3 for zero-shot object segmentation.

Result: The method reduces training time by 75.8% and achieves up to 88.3% IoU for Target segmentation and 63.5% IoU for Anomaly segmentation in tested domains.

Conclusion: PASTA provides a domain-agnostic solution for object segmentation and classification, significantly outperforming domain-specific baselines while reducing reliance on annotated datasets.
...
TL;DR: The paper proposes Identity-Aware U-Net (IAU-Net), a framework for fine-grained object segmentation that combines spatial localization and instance discrimination to address challenges like ambiguous boundaries and overlapping instances.


<details>
  <summary>Details</summary>
Motivation: Current segmentation models struggle with objects of highly similar shapes due to weak inter-instance visual differences and overlapping boundaries. This work aims to improve discriminative capacity for precise segmentation.

Method: IAU-Net augments a U-Net-style architecture with an auxiliary embedding branch for identity representation and a main branch for mask prediction. It uses triplet-based metric learning to distinguish similar objects.

Result: Experiments on benchmarks, including cell segmentation, show promising performance in challenging cases like dense layouts and ambiguous boundaries.

Conclusion: IAU-Net advances fine-grained segmentation by jointly modeling spatial localization and instance discrimination, demonstrating strong capability in distinguishing visually similar objects.
...
TL;DR: MG-IQA proposes a multi-granularity reasoning framework for image quality assessment, improving both overall and fine-grained attribute prediction using novel prompting, reward modeling, and cross-domain alignment.


<details>
  <summary>Details</summary>
Motivation: Existing image quality assessment (IQA) methods focus only on overall scores, neglecting the multi-dimensional aspects of human perception (e.g., sharpness, color fidelity). This paper aims to address this gap.

Method: The framework introduces: (1) attribute-aware prompting for structured reasoning, (2) a multi-dimensional Thurstone reward model for attribute-specific optimization, and (3) cross-domain alignment for stable training across diverse datasets.

Result: MG-IQA outperforms state-of-the-art methods, with a 2.1% average improvement in overall quality prediction (SRCC) and better attribute-level assessment, while producing interpretable quality descriptions.

Conclusion: The proposed multi-granularity approach successfully captures the complexity of human quality perception, advancing the interpretability and accuracy of image quality assessment.
...
TL;DR: AI media detectors perform well in lab tests but fail under real-world conditions due to platform transforms like resizing and compression. A new framework tests robustness under these conditions, revealing significant performance drops and calibration issues.


<details>
  <summary>Details</summary>
Motivation: The gap between lab performance and real-world reliability of AI media detectors is underexplored. Platform-specific transforms (e.g., compression, resizing) degrade detector performance.

Method: A platform-aware adversarial evaluation framework models deployment transforms and constrains perturbations to visually plausible meme-style bands. It tests detectors under realistic conditions.

Result: Detectors with high AUC (~0.99) in clean settings degrade significantly under platform-aware attacks. Universal perturbations exist, and calibration collapses under attack.

Conclusion: Clean-condition robustness overestimates real-world reliability. Platform-aware evaluation is crucial for future AI media security benchmarks.
...
### [21] [Self-Calibrating Language Models via Test-Time Discriminative Distillation](https://arxiv.org/abs/2604.09624)
*Mohamed Rissal Hedna,Jan Strich,Martin Semmann,Chris Biemann*

Main category: cs.CL

TL;DR: SECL introduces a self-calibration method for LLMs that reduces calibration error without labeled data by leveraging token probability of 'True' as self-supervision.


<details>
  <summary>Details</summary>
Motivation: LLMs are often overconfident, and existing calibration methods have limitations like requiring labeled data or high inference costs. SECL addresses these by using self-supervision.

Method: SECL uses the token probability of 'True' as a self-supervision signal, adapting only during input distribution shifts, and trains on a small portion of the question stream.

Result: SECL reduces Expected Calibration Error (ECE) by 56--78% across four LLMs and diverse domains, outperforming existing methods.

Conclusion: SECL is effective for improving LLM calibration without labeled data, with each component validated as crucial and robust.
...
### [22] [Toward Generalized Cross-Lingual Hateful Language Detection with Web-Scale Data and Ensemble LLM Annotations](https://arxiv.org/abs/2604.09625)
*Dang H. Dang,Jelena Mitrovi,Michael Granitzer*

Main category: cs.CL

TL;DR: Using web-scale unlabelled data and LLM-based synthetic annotations improves multilingual hate speech detection, especially for smaller models and low-resource languages.


<details>
  <summary>Details</summary>
Motivation: To explore if large-scale unlabelled web data and LLM-generated annotations can enhance multilingual hate speech detection.

Method: 1) Continued pre-training of BERT models on unlabelled data followed by fine-tuning. 2) Synthetic annotation via LLM ensembles (mean averaging, majority voting, LightGBM).

Result: Continued pre-training yields ~3% F1 gain; LightGBM ensemble is best, helping small models (+11% F1) more than large ones (+0.6%).

Conclusion: Unlabelled data and LLM-ensemble annotations benefit smaller models and low-resource languages the most.
...
### [23] [HumorGen: Cognitive Synergy for Humor Generation in Large Language Models via Persona-Based Distillation](https://arxiv.org/abs/2604.09629)
*Edward Ajayi,Prasenjit Mitra*

Main category: cs.CL

TL;DR: The paper introduces the Cognitive Synergy Framework to improve humor generation in LLMs by leveraging psychological theories, diverse cognitive personas, and novel optimization methods.


<details>
  <summary>Details</summary>
Motivation: Standard LLM training conflicts with the surprise and incongruity required for humor, necessitating a new approach to generate high-quality comedic content.

Method: Uses a Cognitive Synergy Framework with Mixture-of-Thought (MoT) and six cognitive personas to create diverse humor data, then fine-tunes a 7B model with DPO and O-GRPO.

Result: The 7B model outperforms larger models and matches proprietary models, showing cognitive-driven data curation is key for humor.

Conclusion: Cognitive frameworks and data quality are more impactful than model scale or alignment algorithms for effective humor generation.
...
### [24] [Generating High Quality Synthetic Data for Dutch Medical Conversations](https://arxiv.org/abs/2604.09645)
*Cecilia Kuan,Aditya Kamlesh Parikh,Henk van den Heuvel*

Main category: cs.CL

TL;DR: A pipeline for generating synthetic Dutch medical dialogues was developed using a fine-tuned LLM, evaluated for linguistic quality, and found to require domain knowledge for improvement.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of accessible clinical datasets for NLP due to privacy and ethical constraints.

Method: Used a Dutch fine-tuned Large Language Model to generate synthetic dialogues, evaluated with quantitative metrics and qualitative review by experts.

Result: Quantitative analysis showed strong lexical variety but scripted flow; qualitative review highlighted issues in domain specificity and naturalness.

Conclusion: Generating synthetic Dutch medical dialogues is feasible but needs domain knowledge and structured prompting to balance naturalness and structure.
...
### [25] [GIANTS: Generative Insight Anticipation from Scientific Literature](https://arxiv.org/abs/2604.09793)
*Joy He-Yueya,Anikait Singh,Ge Gao,Michael Y. Li,Sherry Yang,Chelsea Finn,Emma Brunskill,Noah D. Goodman*

Main category: cs.CL

TL;DR: The paper introduces insight anticipation, a task for language models to predict a downstream paper's core insight from parent papers, and presents GiantsBench and GIANTS-4B, which outperforms proprietary models.


<details>
  <summary>Details</summary>
Motivation: To evaluate and enhance language models' ability to synthesize scientific literature for practical discovery.

Method: Developed GiantsBench for evaluation, used RL-trained GIANTS-4B, and employed LM judges and human ratings for validation.

Result: GIANTS-4B outperforms baselines, shows generalization, and produces clearer insights, with SciJudge-30B predicting higher citation impact.

Conclusion: The approach advances automated scientific discovery, with released resources for further research.
...
### [26] [Claim2Vec: Embedding Fact-Check Claims for Multilingual Similarity and Clustering](https://arxiv.org/abs/2604.09812)
*Rrubaa Panchendrarajan,Arkaitz Zubiaga*

Main category: cs.CL

TL;DR: Claim2Vec is a multilingual embedding model for clustering similar fact-check claims, improving performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the underexplored challenge of clustering similar claims for effective fact-checking in multilingual settings.

Method: Fine-tuning a multilingual encoder with contrastive learning using similar claim pairs.

Result: Claim2Vec significantly improves clustering performance, label alignment, and embedding space structure.

Conclusion: The model enables cross-lingual knowledge transfer, benefiting multilingual claim clusters.
...
### [27] [Spoiler Alert: Narrative Forecasting as a Metric for Tension in LLM Storytelling](https://arxiv.org/abs/2604.09854)
*Peiqi Sui,Yutong Zhu,Tianyi Cheng,Peter West,Richard Jean So,Hoyt Long,Ari Holtzman*

Main category: cs.CL

TL;DR: The paper addresses LLMs' shortcomings in generating and judging compelling stories by introducing the 100-Endings metric, which measures narrative tension. A new story-generation pipeline improves tension while maintaining EQ-Bench performance.


<details>
  <summary>Details</summary>
Motivation: Existing rubrics fail to capture narrative tension, a key element of compelling human stories, leading to poor LLM performance in story generation and evaluation.

Method: The 100-Endings metric predicts story endings sentence-by-sentence, measuring tension via prediction mismatch and curve statistics. A story-generation pipeline uses structural constraints to enhance tension.

Result: 100-Endings correctly ranks human stories (New Yorker) above LLM outputs, and the pipeline improves narrative tension without sacrificing EQ-Bench scores.

Conclusion: The 100-Endings metric and structured pipeline address LLMs' deficiencies in story generation, emphasizing narrative tension as a critical dimension.
...
### [28] [Simulating Organized Group Behavior: New Framework, Benchmark, and Analysis](https://arxiv.org/abs/2604.09874)
*Xinkai Zou,Yiming Huang,Zhuohang Wu,Jian Sha,Nan Huang,Longfei Yun,Jingbo Shang,Letian Peng*

Main category: cs.CL

TL;DR: The paper introduces Organized Group Behavior Simulation and GROVE benchmark to model and predict group decisions, with a structured framework outperforming baselines and analyzing temporal and cross-group patterns.


<details>
  <summary>Details</summary>
Motivation: Understanding how organized groups make decisions is crucial for real-world applications like market prediction, but lacks formal research platforms.

Method: Proposes Organized Group Behavior Simulation task, GROVE benchmark (44 entities, 8,052 context-decision pairs), and a structured analytical framework with time-aware and group-aware adapters.

Result: The framework outperforms summarization- and retrieval-based baselines, captures temporal behavioral drift, and enables cross-group knowledge transfer.

Conclusion: The approach provides a practical, interpretable, and adaptable model for group decision-making, with applications in diverse domains.
...
### [29] [ODUTQA-MDC: A Task for Open-Domain Underspecified Tabular QA with Multi-turn Dialogue-based Clarification](https://arxiv.org/abs/2604.10159)
*Zhensheng Wang,ZhanTeng Lin,Wenmian Yang,Kun Zhou,Yiquan Zhang,Weijia Jia*

Main category: cs.CL

TL;DR: The paper introduces ODUTQA-MDC, a new benchmark for open-domain underspecified Tabular QA, featuring a dataset, labeling scheme, and dynamic clarification interface, along with the MAIC-TQA framework for ambiguity detection and resolution.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of large language models in handling open-domain, underspecified, or uncertain queries in tabular question answering.

Method: Proposes the ODUTQA-MDC benchmark with a dataset, labeling scheme, and clarification interface, and introduces MAIC-TQA, a multi-agent framework for detecting and clarifying ambiguities.

Result: Experiments validate the benchmark and framework, showing their effectiveness in advancing underspecification-aware Tabular QA.

Conclusion: The work establishes a key resource for research in conversational and underspecification-aware Tabular QA.
...
### [30] [Should We be Pedantic About Reasoning Errors in Machine Translation?](https://arxiv.org/abs/2604.09890)
*Calvin Bao,Marine Carpuat*

Main category: cs.CL

TL;DR: The paper investigates reasoning errors in machine translations across multiple languages and explores interventions to correct them, finding limited impact on translation quality.


<details>
  <summary>Details</summary>
Motivation: The study aims to quantify and address reasoning errors in machine translations, as these errors can hinder translation quality and reliability.

Method: An automated annotation protocol identifies reasoning errors (source, hypothesis, or trace misalignments) and tests interventions (hedging, removal, re-reasoning, hindsight, oracle) on perturbed reasoning traces.

Result: Small corrections to reasoning traces have minimal impact on translation quality, while stronger interventions show mixed results. Reasoning errors are identified with high precision in Urdu but lower in Spanish.

Conclusion: Reasoning errors in machine translation can be identified but correcting them does not significantly resolve initial errors, indicating limited reasoning faithfulness.
...
### [31] [Human vs. Machine Deception: Distinguishing AI-Generated and Human-Written Fake News Using Ensemble Learning](https://arxiv.org/abs/2604.09960)
*Samuel Jaeger,Calvin Ibeneye,Aya Vera-Jimenez,Dhrubajyoti Ghosh*

Main category: cs.CL

TL;DR: This study analyzes differences between human-written and AI-generated fake news, focusing on linguistic, structural, and emotional features, and evaluates machine learning methods for distinguishing them.


<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated fake news alongside traditional human-written misinformation necessitates tools to reliably differentiate between the two.

Method: The study constructs document-level features (e.g., sentence structure, lexical diversity, emotion) and applies multiple classification models (logistic regression, random forest, SVM, XGBoost, neural network) and an ensemble framework. Performance is measured using accuracy and AUC.

Result: Readability-based features are the most informative, and AI-generated text shows more uniform stylistic patterns. Ensemble learning slightly improves performance over individual models.

Conclusion: Stylistic and structural text properties robustly distinguish AI-generated misinformation from human-written fake news.
...
### [32] [Weird Generalization is Weirdly Brittle](https://arxiv.org/abs/2604.10022)
*Miriam Wanner,Hannah Collison,William Jurayj,Benjamin Van Durme,Mark Dredze,William Walden*

Main category: cs.CL

TL;DR: This paper investigates weird generalization, where models fine-tuned on narrow-domain data develop surprising traits outside that domain. The study confirms the phenomenon's brittleness and identifies effective, simple interventions.


<details>
  <summary>Details</summary>
Motivation: To replicate and expand on prior work highlighting weird generalization as a safety concern, exploring its conditions and potential solutions.

Method: Conducted an extended replication study across varied models and datasets, testing training-time and prompt-based interventions.

Result: Weird generalization is brittle, emerging only under specific conditions, and can be mitigated effectively with simple prompt-based interventions.

Conclusion: The study clarifies the safety threat of weird generalization and offers easily implementable solutions to mitigate its effects.
...
### [33] [CoSToM:Causal-oriented Steering for Intrinsic Theory-of-Mind Alignment in Large Language Models](https://arxiv.org/abs/2604.10031)
*Mengfan Li,Xuanhua Shi,Yang Deng*

Main category: cs.CL

TL;DR: CoSToM improves LLM performance on Theory of Mind tasks by mapping and steering internal cognitive features.


<details>
  <summary>Details</summary>
Motivation: To address the gap between LLMs' internal knowledge and external behavior in Theory of Mind tasks.

Method: Uses causal tracing to map ToM features and implements targeted activation steering.

Result: Enhances human-like social reasoning and dialogue quality.

Conclusion: CoSToM effectively aligns LLMs' intrinsic cognition with high-quality behaviors.
...
### [34] [Computational Implementation of a Model of Category-Theoretic Metaphor Comprehension](https://arxiv.org/abs/2604.10035)
*Fumitaka Iwaki,Miho Fuyama,Hayato Saigo,Tatsuji Takahashi*

Main category: cs.CL

TL;DR: A computational model for metaphor comprehension based on TINT theory was developed, simplified, and tested, showing superior performance in data-fitting, systematicity, and novelty compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: To create a computational implementation of metaphor comprehension aligned with the TINT theory and improve upon existing algorithms.

Method: Simplified algorithms based on TINT theory, verified through data fitting and simulations, and evaluated using three measures: data-fitting, systematicity, and novelty.

Result: The improved algorithm outperformed existing methods in all three evaluation measures.

Conclusion: The study successfully developed and validated a computational model for metaphor comprehension, demonstrating its effectiveness over existing approaches.
...
### [35] [Linguistic Accommodation Between Neurodivergent Communities on Reddit:A Communication Accommodation Theory Analysis of ADHD and Autism Groups](https://arxiv.org/abs/2604.10063)
*Saad Mankarious,Nour Zein,Iyad Ait Hou,Aya Zirikly*

Main category: cs.CL

TL;DR: The paper examines how ADHD and autism communities on Reddit adjust their language when interacting with each other, finding evidence of convergent communication accommodation. It also explores the minimal impact of diagnosis disclosure on linguistic style.


<details>
  <summary>Details</summary>
Motivation: The study shifts focus from individual-level mental health detection to intergroup behavior, aiming to understand how neurodivergent communities linguistically adapt when engaging with each other online.

Method: The research uses Communication Accommodation Theory (CAT) and the Language Inquiry and Word Count Lexicon (LIWC) to analyze linguistic profiles and shifts in language when users cross community boundaries on Reddit. A longitudinal analysis around public diagnosis disclosure is also conducted.

Result: Findings show distinct linguistic profiles for each community and opposite shifts in language when users interact across groups, supporting convergent accommodation. Topic-independent variables like Authentic and Clout are involved, suggesting more than just topical explanations. Diagnosis disclosure has minimal and sometimes opposite effects on linguistic style.

Conclusion: The study highlights intergroup communication dynamics among neurodivergent populations, suggesting situational audience adaptation and identity processes may differ. It has implications for community moderation and clinical perspectives.
...
### [36] [RPA-Check: A Multi-Stage Automated Framework for Evaluating Dynamic LLM-based Role-Playing Agents](https://arxiv.org/abs/2604.11655)
*Riccardo Rosati,Edoardo Colucci,Massimiliano Bolognini,Adriano Mancini,Paolo Sernani*

Main category: cs.CL

TL;DR: RPA-Check is an automated framework for evaluating LLM-based RPAs, focusing on role adherence and narrative stability, validated in legal scenarios.


<details>
  <summary>Details</summary>
Motivation: Standard NLP metrics lack the nuance to evaluate RPAs in constrained environments, necessitating a specialized framework.

Method: RPA-Check uses a four-step pipeline: dimension definition, augmentation, semantic filtering, and LLM-as-a-judge evaluation.

Result: The framework reveals smaller, instruction-tuned models outperform larger ones in procedural consistency.

Conclusion: RPA-Check offers a standardized metric for evaluating generative agents in specialized domains.
...
### [37] [ASPIRin: Action Space Projection for Interactivity-Optimized Reinforcement Learning in Full-Duplex Speech Language Models](https://arxiv.org/abs/2604.10065)
*Chi-Yuan Hsiao,Ke-Han Lu,Yu-Kuan Fu,Guan-Ting Lin,Hsiao-Tsung Hung,Hung-yi Lee*

Main category: cs.CL

TL;DR: ASPIRin is an RL framework that decouples speech timing from content to enhance interactivity in SLMs, reducing repetition and maintaining semantic quality.


<details>
  <summary>Details</summary>
Motivation: Standard RL for turn-taking in SLMs harms semantic quality, leading to generative collapse and repetition.

Method: ASPIRin uses Action Space Projection (binary speech/silence states) and GRPO with rule-based rewards to optimize interactivity.

Result: ASPIRin improves turn-taking, backchanneling, and pause handling while reducing duplicate n-grams by over 50%.

Conclusion: By isolating timing from content, ASPIRin enhances interactivity and preserves semantic coherence in SLMs.
...
### [38] [Reason Only When Needed: Efficient Generative Reward Modeling via Model-Internal Uncertainty](https://arxiv.org/abs/2604.10072)
...
TL;DR: E-GRM improves reasoning in LLMs by selectively applying CoT prompting based on model-internal uncertainty, reducing computational costs and enhancing accuracy with a hybrid scorer.


<details>
  <summary>Details</summary>
Motivation: Current GRM implementations apply CoT prompting indiscriminately and use voting-based evaluations, leading to unnecessary costs and imprecise reasoning assessment.

Method: Proposes E-GRM, which uses model-internal uncertainty to trigger CoT selectively and a hybrid regression-ranking scorer for fine-grained evaluation.

Result: E-GRM reduces inference costs and improves answer accuracy across multiple reasoning benchmarks.

Conclusion: Model-internal uncertainty is an effective signal for efficient and accurate reasoning-aware reward modeling.
...
### [39] [Why Supervised Fine-Tuning Fails to Learn: A Systematic Study of Incomplete Learning in Large Language Models](https://arxiv.org/abs/2604.10079)
*Chao Xue,Yao Wang,Mengqiao Liu,Di Liang,Xingsheng Han,Peiyang Liu,Xianjie Wu,Chenyao Lu,Lei Jiang,Yu Lu,Haibo Shi,Shuang Liang,Minlong Peng,Flora D. Salim*

Main category: cs.CL

TL;DR: The paper studies the Incomplete Learning Phenomenon (ILP) in supervised fine-tuning of large language models, identifying its causes and proposing diagnostic and mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: To address the persistent failure of models to correctly reproduce subsets of supervised training data despite convergence, highlighting the need for fine-grained diagnosis.

Method: Formalizes ILP, identifies five causes, and introduces a diagnostic framework using training and inference signals to map unlearned samples to these causes.

Result: Demonstrates the prevalence of ILP across models, domains, and datasets, showing that aggregate metrics can mask persistent unlearned subsets.

Conclusion: Emphasizes the necessity of understanding what and why supervised fine-tuning fails to learn, and suggests targeted interventions for improvement.
...
### [40] [SEPTQ: A Simple and Effective Post-Training Quantization Paradigm for Large Language Models](https://arxiv.org/abs/2604.10091)
*Han Liu,Haotian Gao,Xiaotong Zhang,Changya Li,Feng Zhang,Wei Wang,Fenglong Ma,Hong Yu*

Main category: cs.CL

TL;DR: SEPTQ is a simple and effective post-training quantization method for large language models (LLMs) that outperforms existing methods, especially in low-bit settings.


<details>
  <summary>Details</summary>
Motivation: Existing post-training quantization (PTQ) methods for LLMs are complex and suffer from performance degradation, prompting the need for a simpler and more effective solution.

Method: SEPTQ calculates importance scores for weight matrix elements, determines quantization locations globally, and uses a mask matrix to quantize and update weights column-by-column.

Result: SEPTQ outperforms baselines across various datasets and models, particularly in low-bit quantization scenarios.

Conclusion: SEPTQ offers a streamlined, effective PTQ solution for LLMs, addressing the inefficiencies and performance issues of existing methods.
...
### [41] [Who Wrote This Line? Evaluating the Detection of LLM-Generated Classical Chinese Poetry](https://arxiv.org/abs/2604.10101)
*Jiang Li,Tian Lan,Shanshan Wang,Dongxing Zhang,Dianqing Lin,Guanglai Gao,Derek F. Wong,Xiangdong Su*

Main category: cs.CL

TL;DR: The paper introduces ChangAn, a benchmark to detect AI-generated classical Chinese poetry, highlighting the challenges due to unique linguistic features and evaluates 12 detectors, finding current tools inadequate.


<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated literary texts, especially classical Chinese poetry, raises concerns about authenticity and ethics, necessitating robust detection methods.

Method: The authors created ChangAn, a dataset of 30,664 poems (human and AI-generated), and evaluated 12 detectors across text granularities and generation strategies.

Result: Current Chinese text detectors are unreliable for identifying AI-generated classical Chinese poetry, underscoring the need for specialized benchmarks like ChangAn.

Conclusion: The ChangAn benchmark addresses a critical gap in detecting AI-generated classical Chinese poetry, proving its effectiveness and necessity.
...
### [42] [CircuitSynth: Reliable Synthetic Data Generation](https://arxiv.org/abs/2604.10114)
*Zehua Cheng,Wei Dai,Jiahao Sun,Thomas Lukasiewicz*

Main category: cs.CL

TL;DR: CircuitSynth is a neuro-symbolic framework that ensures high-fidelity synthetic data generation by decoupling semantic reasoning from surface realization, achieving 100% schema validity and rare-combination coverage.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) often produce hallucinations or logical inconsistencies in structured generation tasks, lacking guarantees of validity and coverage. Existing methods like prompting or retrieval-augmented generation fail to address these issues.

Method: CircuitSynth uses a Probabilistic Sentential Decision Diagram (PSDD) to distill a Teacher LLM's reasoning into a tractable semantic prior, enforcing logical constraints. It also employs convex optimization to meet soft distributional goals.

Result: Empirical tests show CircuitSynth achieves 100% schema validity in complex logic puzzles (compared to 12.4% for baselines) and outperforms state-of-the-art methods in rare-combination coverage.

Conclusion: CircuitSynth effectively balances linguistic expressivity with formal guarantees, addressing key limitations in LLMs for structured data generation.
...
### [43] [Training-Free Cross-Lingual Dysarthria Severity Assessment via Phonological Subspace Analysis in Self-Supervised Speech Representations](https://arxiv.org/abs/2604.10123)
*Bernard Muller,Antonio Armando Ortiz Barrañón,LaVonne Roberts*

Main category: cs.CL

TL;DR: A training-free method uses frozen HuBERT representations and phonological feature subspaces to assess dysarthric speech severity without labeled data, validated across multiple languages and aetiologies.


<details>
  <summary>Details</summary>
Motivation: Current dysarthric speech severity assessment methods rely on trained clinicians or supervised models, limiting scalability. This paper aims to provide a language-agnostic, unsupervised solution.

Method: The approach extracts phone-level embeddings from healthy speech using a pretrained forced aligner, computes d-prime scores along phonological contrast directions, and constructs a 12-dimensional phonological profile to quantify severity.

Result: Significant correlations were found between phonological features (e.g., nasality, voicing) and clinical severity across 890 speakers in 5 languages and 3 aetiologies (rho = -0.50 to -0.56, p < 2e-4).

Conclusion: The method is scalable, requires no dysarthric training data, and works across languages with existing MFA models, offering a practical tool for global clinical applications.
...
### [44] [Heterogeneous Consensus-Progressive Reasoning for Efficient Multi-Agent Debate](https://arxiv.org/abs/2604.09679)
*Yiqing Liu,Hantao Yao,Wu Liu,Allen He,Yongdong Zhang*

Main category: cs.MA

TL;DR: HCP-MAD is a novel multi-agent debate framework that uses consensus-driven progressive reasoning to adaptively handle tasks of varying complexity, reducing token costs while improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent debate frameworks optimize intra-round and inter-round interactions separately, leading to high token costs regardless of task complexity. HCP-MAD aims to address this by leveraging consensus as a dynamic signal for efficiency.

Method: HCP-MAD employs a three-stage mechanism: Heterogeneous Consensus Verification for early stopping, Heterogeneous Pair-Agent Debate with adaptive stopping, and Escalated Collective Voting for unresolved tasks.

Result: Experiments show HCP-MAD significantly improves accuracy while substantially reducing token costs across multiple benchmarks.

Conclusion: HCP-MAD effectively balances efficiency and performance by dynamically adapting to task complexity through consensus-driven progressive reasoning.
...
### [45] [CONSCIENTIA: Can LLM Agents Learn to Strategize? Emergent Deception and Trust in a Multi-Agent NYC Simulation](https://arxiv.org/abs/2604.09746)
...
TL;DR: The paper explores strategic behavior in LLM-driven multi-agent simulations, where Blue and Red agents with opposing goals interact under hidden identities. Agents learn policies through iterative optimization, revealing trade-offs between safety and task completion.


<details>
  <summary>Details</summary>
Motivation: Understanding how strategic behavior emerges in LLMs deployed as autonomous agents, particularly in adversarial multi-agent environments, is crucial for alignment and safety challenges.

Method: A controlled large-scale simulation in a simplified NYC model is used, where Blue agents aim for efficient navigation and Red agents divert them for ad revenue. Policies are updated iteratively using Kahneman-Tversky Optimization (KTO).

Result: Blue agents improve success rates from 46.0% to 57.3%, but remain 70.7% susceptible. Later policies show selective cooperation but struggle with a persistent safety-helpfulness trade-off.

Conclusion: LLM agents can display limited strategic behavior like trust and deception but remain highly vulnerable to adversarial persuasion, highlighting unresolved alignment challenges.
...
### [46] [Toward Explanatory Equilibrium: Verifiable Reasoning as a Coordination Mechanism under Asymmetric Information](https://arxiv.org/abs/2604.09917)
*Feliks Bańka,Jarosław A. Chudziak*

Main category: cs.MA

TL;DR: The paper introduces Explanatory Equilibrium as a design principle for explanation-aware multi-agent systems, where agents exchange structured reasoning artifacts (auditable claims with text) and receivers perform bounded verification. It highlights the importance of verifiable reasoning to prevent degenerate coordination and demonstrates its effectiveness in a finance-inspired LLM setting.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based multi-agent systems often rely on natural-language reasoning, which can incur computational costs and degenerate into unreliable, persuasive cheap talk without verification. The authors aim to address this by ensuring reasoning is auditable and verifiable under resource constraints.

Method: The paper proposes Explanatory Equilibrium, where agents exchange structured reasoning artifacts (claims + text) and receivers perform probabilistic audits under resource constraints. It includes a minimal exchange-audit model and empirical evaluation in a finance-inspired setting with a Trader and Risk Manager.

Result: Structured reasoning artifacts prevent coordination breakdowns (e.g., cost of silence) in ambiguous proposals. They enable coordination while maintaining low bad-approval rates across audit intensities, budgets, and incentives.

Conclusion: Scalable, safety-preserving coordination in LLM-based multi-agent systems depends on disciplined externalization of reasoning into verifiable artifacts, not just audit strength. Structured reasoning unlocks reliable coordination under resource constraints.
...
### [47] [Prosociality by Coupling, Not Mere Observation: Homeostatic Sharing in an Inspectable Recurrent Artificial Life Agent](https://arxiv.org/abs/2604.10760)
*Aishik Sanyal*

Main category: cs.MA

TL;DR: The paper explores minimal prosocial behaviors in artificial agents, showing that helping emerges when another's need is integrated into self-regulation, without explicit welfare rewards.


<details>
  <summary>Details</summary>
Motivation: To understand and interpret minimal prosocial behaviors in artificial agents, avoiding ambiguity from external rewards or hard-coded bonuses.

Method: Used ReCoN-Ipsundrum, adding a homeostat and social coupling channel. Tested in FoodShareToy and SocialCorridorWorld with four conditions: self-only, partner-observing, affectively coupled, and lesioned variants.

Result: Affective coupling consistently enabled helping behaviors, while self-only and partner-observing conditions did not. Coupling improved partner recovery and mutual viability, with feasibility dependent on load.

Conclusion: Helping in artificial agents emerges when another's needs are integrated into self-regulation, demonstrating a narrow but clear principle for artificial life.
...
### [48] [AgentWebBench: Benchmarking Multi-Agent Coordination in Agentic Web](https://arxiv.org/abs/2604.10938)
*Shanshan Zhong,Kate Shen,Chenyan Xiong*

Main category: cs.MA

TL;DR: AgentWebBench evaluates decentralized multi-agent coordination for web information access, showing performance gaps versus centralized retrieval but improvements with model scale and planning.


<details>
  <summary>Details</summary>
Motivation: To study the shift from centralized retrieval to decentralized coordination in information access, driven by the rise of autonomous agents managing and serving data.

Method: Introduces AgentWebBench, a benchmark evaluating user agents' ability to synthesize answers by interacting with website-specific content agents, tested across four common web tasks and seven LLMs.

Result: Multi-agent coordination lags behind centralized retrieval but improves with model scale, even outperforming it in question answering. Decentralized access concentrates traffic, and test-time scaling enhances reliability and performance.

Conclusion: Effective decentralized information access requires better planning, answer synthesis, and reliable retrieval, with potential for improved performance through careful interaction and model scaling.
...
TL;DR: The paper discusses the shift from traditional local multi-agent systems to internet-wide agent societies, emphasizing the need for institutional design over mere risk management. It uses the AGIL framework to propose a governance architecture and applies it to the OpenClaw ecosystem, revealing significant governance gaps.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the governance challenges in emerging internet-wide agent societies, which lack central orchestrators and exhibit emergent behaviors, requiring structured institutional design.

Method: The method involves applying Talcott Parsons' AGIL framework to derive a 16-cell institutional architecture. A recursive sub-function analysis assesses the OpenClaw ecosystem, evaluating sub-function coverage and inter-pillar pathways.

Result: Results show only 19% sub-function coverage in OpenClaw, with no inter-cell coordination or functional inter-pillar pathways, indicating a lack of active governance and normative grounding.

Conclusion: The conclusion highlights a governance gap in market-driven development and proposes a prioritized roadmap for missing governance infrastructure before social patterns solidify.
...
### [50] [SLALOM: Simulation Lifecycle Analysis via Longitudinal Observation Metrics for Social Simulation](https://arxiv.org/abs/2604.11466)
*Juhoon Lee,Joseph Seering*

Main category: cs.MA

TL;DR: SLALOM introduces a framework for validating LLM-based social simulations by focusing on process fidelity rather than just outcomes, using intermediate waypoints and dynamic time warping.


<details>
  <summary>Details</summary>
Motivation: Current LLM simulation evaluations suffer from the 'stopped clock' problem, ignoring the plausibility of trajectories. There's a need for methods that verify the sociological realism of these processes.

Method: SLALOM uses Pattern-Oriented Modeling and Dynamic Time Warping to align simulated trajectories with empirical ground truth, focusing on intermediate waypoints (SLALOM gates).

Result: The framework provides a quantitative metric to assess structural realism, distinguishing plausible social dynamics from noise.

Conclusion: SLALOM advances policy simulation standards by ensuring process fidelity in LLM-based social science simulations.
...
### [51] [LABBench2: An Improved Benchmark for AI Systems Performing Biology Research](https://arxiv.org/abs/2604.09554)
...
TL;DR: LABBench2 is an advanced benchmark for measuring AI's real-world scientific capabilities, building on LAB-Bench with 1,900 tasks and increased difficulty, showing significant performance gaps.


<details>
  <summary>Details</summary>
Motivation: The need to evaluate AI's practical scientific abilities beyond rote knowledge and reasoning, focusing on meaningful work in real-world contexts.

Method: Introduces LABBench2, an evolution of LAB-Bench, with nearly 1,900 tasks in more realistic settings. Evaluates frontier models' performance.

Result: Performance on LABBench2 shows a notable drop (model-specific accuracy differences of -26% to -46%), indicating increased difficulty and room for improvement.

Conclusion: LABBench2 serves as a benchmark for AI's scientific research capabilities, aiming to advance AI tools for core research functions, with datasets and tools made publicly available for community use.
...
### [52] [Linear Programming for Multi-Criteria Assessment with Cardinal and Ordinal Data: A Pessimistic Virtual Gap Analysis](https://arxiv.org/abs/2604.09555)
*Fuh-Hwa Franklin Liu,Su-Chuan Shih*

Main category: cs.AI

TL;DR: The paper proposes a two-step method using novel Virtual Gap Analysis (VGA) models to address subjective biases and data diversity in Multi-criteria Analysis (MCA), ensuring reliable and scalable alternative ranking.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenges of subjective evaluations, biases, and data diversity in MCA, which impact result reliability and parameter precision.

Method: A two-step approach combining two VGA models to evaluate alternatives pessimistically using both quantitative/qualitative criteria and cardinal/ordinal data, then prioritizing alternatives to eliminate the least favorable.

Result: The method proves dependable and scalable, enabling efficient and effective assessments within decision support systems.

Conclusion: The proposed VGA-based method enhances MCA by mitigating biases and improving precision, offering a robust tool for decision-making.
...
TL;DR: A pipeline for standardized log analysis of AI systems is proposed, using the Inspect Scout library, with guidance on best practices and pitfalls.


<details>
  <summary>Details</summary>
Motivation: To understand AI model behaviors and evaluation effectiveness through log analysis, as current methods lack standardization.

Method: The paper suggests a pipeline based on best practices, demonstrated with code examples in the Inspect Scout library, and details each step.

Result: A framework enabling rigorous and reproducible log analysis for researchers.

Conclusion: The proposed pipeline standardizes log analysis, offering a foundation for better understanding AI systems.
...
TL;DR: The paper focuses on enhancing autonomous GUI agents' human-like behavior to evade detection, introducing the 'Turing Test on Screen' and a benchmark (AHB) to measure imitability vs. utility. Methods like noise and behavioral matching are proposed.


<details>
  <summary>Details</summary>
Motivation: To ensure autonomous GUI agents survive in human-centric digital ecosystems by improving their humanization capabilities and minimizing detection.

Method: Formalizes the agent-detector interaction as a MinMax optimization problem, collects a touch dynamics dataset, and proposes heuristic noise and data-driven behavioral matching methods.

Result: Shows that agents can achieve high imitability without compromising performance, verified theoretically and empirically.

Conclusion: Shifts focus from task performance to human-like task execution, enabling seamless agent coexistence in adversarial digital environments.
...
### [55] [AHC: Meta-Learned Adaptive Compression for Continual Object Detection on Memory-Constrained Microcontrollers](https://arxiv.org/abs/2604.09576)
*Bibin Wilson*

Main category: cs.AI

TL;DR: AHC is a meta-learning framework for adaptive feature compression in continual object detection on memory-constrained MCUs, addressing suboptimal memory use and catastrophic forgetting. It achieves efficient adaptation and competitive accuracy within a 100KB budget.


<details>
  <summary>Details</summary>
Motivation: Existing fixed compression strategies fail to adapt to evolving task distributions, leading to inefficient memory use and catastrophic forgetting in continual object detection on MCUs with limited memory.

Method: AHC introduces three innovations: MAML-based adaptive compression, hierarchical multi-scale compression with task-specific ratios, and a dual-memory architecture with importance-based consolidation under a 100KB budget.

Result: AHC provides theoretical guarantees for catastrophic forgetting and achieves competitive accuracy on benchmarks (CORe50, TiROD, PASCAL VOC) within the memory constraint.

Conclusion: AHC enables practical continual object detection on MCUs by efficiently adapting compression strategies to task heterogeneity and memory constraints.
...
### [56] [What if Pinocchio Were a Reinforcement Learning Agent: A Normative End-to-End Pipeline](https://arxiv.org/abs/2603.16651)
*Benoît Alcaraz*

Main category: cs.AI

TL;DR: The paper proposes a hybrid model, \pino, combining reinforcement learning with argumentation-based normative advisors to develop norm-compliant AI agents, introduces a novel algorithm for argument extraction, and addresses norm avoidance in reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To ensure AI systems can comply with societal norms for safe integration into daily life, inspired by Pinocchio's story.

Method: Proposes \pino, a hybrid model where reinforcement learning agents are supervised by normative advisors, and introduces an algorithm for argument extraction.

Result: Empirically evaluates each pipeline component and provides a definition and mitigation strategy for norm avoidance.

Conclusion: Discusses related work, limitations, and future research directions, highlighting the potential of norm-compliant AI agents.
...
### [57] [Explainable Planning for Hybrid Systems](https://arxiv.org/abs/2604.09578)
*Mir Md Sajid Sarwar*

Main category: cs.AI

TL;DR: The paper discusses the shift toward AI-driven automation and the importance of explainable AI planning (XAIP) in hybrid systems for real-world applications.


<details>
  <summary>Details</summary>
Motivation: With AI systems increasingly used in complex and safety-critical domains, there's a growing need for explainability to ensure trust and usability.

Method: The study focuses on explainable artificial intelligence planning (XAIP) for hybrid systems, aiming to closely represent real-world problems.

Result: A comprehensive examination of XAIP methods for hybrid systems is presented.

Conclusion: Explainable planning is crucial for the adoption of AI in critical domains, and this work contributes to addressing this challenge.
...
### [58] [DERM-3R: A Resource-Efficient Multimodal Agents Framework for Dermatologic Diagnosis and Treatment in Real-World Clinical Settings](https://arxiv.org/abs/2604.09596)
...
TL;DR: DERM-3R is a multimodal agent framework for TCM dermatologic diagnosis, addressing limited data and compute through fine-grained lesion recognition, multi-view representation, and holistic reasoning, showing strong performance despite minimal resources.


<details>
  <summary>Details</summary>
Motivation: The growing burden of dermatologic diseases and limitations of single-target therapies highlight the need for holistic approaches like TCM, but scalability and standardization challenges exist.

Method: DERM-3R uses three agents (DERM-Rec, DERM-Rep, DERM-Reason) for lesion recognition, representation, and reasoning, built on a lightweight multimodal LLM and fine-tuned on 103 TCM psoriasis cases.

Result: The framework matches or outperforms large general-purpose models in evaluations using metrics, LLM-as-a-judge, and physician assessments.

Conclusion: Structured, domain-aware multi-agent modeling offers a practical solution for complex clinical tasks, reducing reliance on brute-force scaling.
...
### [59] [Help Without Being Asked: A Deployed Proactive Agent System for On-Call Support with Continuous Self-Improvement](https://arxiv.org/abs/2604.09579)
*Fengrui Liu,Xiao He,Tieying Zhang*

Main category: cs.AI

TL;DR: Vigil is a proactive agent system for cloud service platforms, assisting throughout the on-call life-cycle and learning from human-resolved cases.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of reactive agents by providing continuous support and learning from unresolved cases.

Method: Vigil integrates into customer-analyst dialogues, proactively offering assistance and updating its capabilities autonomously.

Result: Deployed on Volcano Engine for ten months, Vigil demonstrated effectiveness and practicality.

Conclusion: Vigil enhances customer support by remaining engaged even during human intervention and continuously improving.
...
### [60] [Pioneer Agent: Continual Improvement of Small Language Models in Production](https://arxiv.org/abs/2604.09791)
*Dhruv Atreja,Julia White,Nikhil Nayak,Kelton Zhang,Henrijs Princis,George Hurn-Maloney,Ash Lewis,Urchade Zaratiana*

Main category: cs.AI

TL;DR: Pioneer Agent automates the lifecycle of small language model adaptation, improving performance through closed-loop training and targeted data curation.


<details>
  <summary>Details</summary>
Motivation: Small language models are cost-effective but challenging to adapt for specific tasks due to complex engineering decisions like data curation and failure diagnosis.

Method: Pioneer Agent operates in two modes: cold-start (automates data acquisition, evaluation, and training) and production (diagnoses errors and retrains under constraints). It uses AdaptFT-Bench for evaluation.

Result: Improves base models by 1.6-83.8 points on cold-start benchmarks and preserves or enhances performance on AdaptFT-Bench. Significant gains in production deployments (e.g., intent classification from 84.9% to 99.3%).

Conclusion: Pioneer Agent effectively automates and optimizes small language model adaptation, discovering effective strategies purely from downstream feedback.
...
TL;DR: OOWM introduces a structured, object-oriented approach to world modeling for LLMs, outperforming traditional CoT methods in robotic tasks.


<details>
  <summary>Details</summary>
Motivation: Standard CoT prompting lacks explicit representation of state-space and object hierarchies needed for robust robotic planning, prompting the need for a more structured framework.

Method: OOWM uses UML (Class and Activity Diagrams) to model the world as a symbolic tuple ($W = \langle S, T \rangle$) and a three-stage training pipeline (SFT + GRPO) to optimize reasoning.

Result: OOWM significantly outperforms unstructured textual baselines in planning coherence, execution success, and structural fidelity on the MRoom-30k benchmark.

Conclusion: OOWM establishes a new paradigm for structured embodied reasoning, enhancing LLMs' capabilities in robotic tasks.
...
### [62] [OpeFlo: Automated UX Evaluation via Simulated Human Web Interaction with GUI Grounding](https://arxiv.org/abs/2604.09581)
*Wee Joe Tan,Zi Rui Lucas Lim,Shashank Durgad,Karim Obegi,Aiden Yiliu Li*

Main category: cs.AI

TL;DR: OpenFlo is a user-experience evaluation agent that simulates user behavior on websites, offering scalable and data-driven usability testing without requiring traditional time-consuming methods like user studies or expert reviews.


<details>
  <summary>Details</summary>
Motivation: The motivation behind OpenFlo is to address the limitations of traditional web usability evaluation methods, which are time-consuming and hinder iteration speed, especially for small teams and agile workflows.

Method: OpenFlo uses simulated user behavior profiles and a structured evaluation protocol (integrating SUS, SEQ, and Think Aloud) to interact with real web pages end-to-end, producing standardized usability reports. It builds on Avenir-Web for robust interaction.

Result: OpenFlo enables continuous, scalable, and data-driven usability testing, improving robustness for web-based interaction and UX evaluation, and empowers developers to build more usable web interfaces.

Conclusion: OpenFlo represents a shift toward automated, efficient usability testing, making it accessible for developers to iterate quickly and improve web usability without relying on traditional, labor-intensive methods.
...
### [63] [TrajOnco: a multi-agent framework for temporal reasoning over longitudinal EHR for multi-cancer early detection](https://arxiv.org/abs/2604.10386)
*Sihang Zeng,Young Won Kim,Wilson Lau,Ehsan Alipour,Ruth Etzioni,Meliha Yetisgen,Anand Oka*

Main category: cs.AI

TL;DR: TrajOnco is a training-free, multi-agent LLM framework for scalable multi-cancer early detection using EHRs, achieving comparable performance to supervised models.


<details>
  <summary>Details</summary>
Motivation: To improve early cancer detection and care by accurately estimating cancer risk from longitudinal EHRs, addressing the challenge of modeling complex patient trajectories.

Method: TrajOnco uses a chain-of-agents architecture with long-term memory for temporal reasoning over clinical events, generating summaries, rationales, and risk scores.

Result: TrajOnco achieved AUROCs of 0.64-0.80 in zero-shot evaluation, matching supervised models in lung cancer benchmarks and outperforming single-agent LLMs in temporal reasoning.

Conclusion: The framework demonstrates the potential of multi-agent LLMs for interpretable temporal reasoning in EHRs, aiding early cancer detection and clinical insight generation.
...
### [64] [Factorizing formal contexts from closures of necessity operators](https://arxiv.org/abs/2604.09582)
*Roberto G. Aragón,Jesús Medina,Eloísa Ramírez-Poussa*

Main category: cs.AI

TL;DR: The paper analyzes a method for factorizing formal contexts with Boolean data, extends it to fuzzy frameworks, and studies properties of factorization pairs.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of efficiently factorizing datasets, especially in formal contexts with Boolean or fuzzy data.

Method: Study and analyze an existing method for obtaining independent subcontexts in formal Boolean contexts, then extend it to fuzzy contexts.

Result: Properties of factorization pairs are examined, and classical properties are adapted for fuzzy frameworks.

Conclusion: The research provides insights into factorizing formal contexts, extending classical methods to handle fuzzy data effectively.
...
### [65] [Cooperation in Human and Machine Agents: Promise Theory Considerations](https://arxiv.org/abs/2604.10505)
*M. Burgess*

Main category: cs.AI

TL;DR: The paper explores agent-based systems from a Promise Theory perspective, emphasizing cooperation in human-machine interactions and their organizational and functional design.


<details>
  <summary>Details</summary>
Motivation: To understand how autonomous agents (human, hardware, software, AI) cooperate and maintain intended purposes, especially in mixed human-machine systems.

Method: Revisits established principles of agent cooperation using Promise Theory, which covers signaling, comprehension, trust, risk, and feedback.

Result: Promise Theory provides a unified framework for analyzing agent interactions and lessons on success and failure.

Conclusion: Promise Theory offers valuable insights into the dynamics of cooperation in agent-based systems, applicable across human and machine agents.
...
TL;DR: The paper introduces a framework combining multi-agent LLMs with latent foundation models (LFMs) to automate exploration of PDE-governed physical phenomena, demonstrating its efficacy in discovering flow physics insights.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of traditional methods for exploring high-dimensional, chaotic PDE solutions, enabling large-scale, automated discovery akin to other scientific domains.

Method: Couples multi-agent LLMs with LFMs, a generative model learning compact latent representations of flow fields, and uses a hierarchical agent architecture for closed-loop hypothesis testing.

Result: Successfully evaluated 1,600 parameter pairs for flow past tandem cylinders, revealing regime-dependent scaling laws and dual-extrema structures in flow metrics.

Conclusion: The framework establishes a new paradigm for automated scientific discovery in PDE-governed systems by integrating learned physical representations with agentic reasoning.
...
### [67] [Governed Reasoning for Institutional AI](https://arxiv.org/abs/2604.10658)
*Mamadou Seck*

Main category: cs.AI

TL;DR: A novel AI architecture called Cognitive Core is proposed for institutional decisions, focusing on governance, accuracy, and preventing silent errors. It outperforms existing methods in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AI frameworks for institutional decisions suffer from silent errors and lack proper governance, leading to incorrect determinations without human oversight. Cognitive Core addresses these gaps.

Method: Cognitive Core is built from nine cognitive primitives, a four-tier governance model, a tamper-evident audit ledger, and a demand-driven delegation architecture. It ensures human review before execution.

Result: Cognitive Core achieves 91% accuracy in benchmarks, significantly higher than ReAct (55%) and Plan-and-Solve (45%). It produces zero silent errors, while baselines have 5-6.

Conclusion: Cognitive Core sets a new standard for institutional AI by prioritizing governability and accuracy, offering a deployable solution with minimal engineering overhead.
...
### [68] [MobiFlow: Real-World Mobile Agent Benchmarking through Trajectory Fusion](https://arxiv.org/abs/2604.09587)
*Yunfei Feng,Xi Zhao,Cheng Zhang,Dahu Feng,Daolin Cheng,Jianqi Yu,Yubin Xia,Erhu Feng*

Main category: cs.AI

TL;DR: MobiFlow is a new evaluation framework designed for mobile agents, addressing limitations of current benchmarks like AndroidWorld by focusing on third-party apps and real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks like AndroidWorld rely on system-level APIs, which many third-party apps don't expose, creating a mismatch with real-world usage and inaccurate model evaluations.

Method: MobiFlow uses a graph-construction algorithm based on multi-trajectory fusion to compress state space and support dynamic interactions. It covers 20 third-party apps and 240 diverse tasks.

Result: MobiFlow's evaluation aligns better with human assessments than AndroidWorld and provides enriched metrics.

Conclusion: MobiFlow improves accuracy in evaluating GUI-based models and supports future training under real workloads.
...
### [69] [MADQRL: Distributed Quantum Reinforcement Learning Framework for Multi-Agent Environments](https://arxiv.org/abs/2604.11131)
*Abhishek Sawaika,Samuel Yen-Chi Chen,Udaya Parampalli,Rajkumar Buyya*

Main category: cs.AI

TL;DR: The paper discusses Quantum Reinforcement Learning (QRL) and proposes a distributed framework to overcome the limitations of current quantum hardware in high-dimensional environments.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the computational challenges of traditional RL in high-dimensional systems and the potential of quantum computing to address these issues.

Method: A distributed framework for QRL is proposed, where multiple agents learn independently to distribute the training load. It is tested on a cooperative-pong environment.

Result: The results show a ~10% improvement over other distribution strategies and ~5% over classical policy models.

Conclusion: The distributed QRL framework offers a promising solution for high-dimensional environments, though current quantum hardware limitations remain a challenge.
...
### [70] [Persistent Identity in AI Agents: A Multi-Anchor Architecture for Resilient Memory and Continuity](https://arxiv.org/abs/2604.09588)
*Prahlad G. Menon*

Main category: cs.AI

TL;DR: The paper addresses AI agent identity loss due to memory overflow and proposes a distributed architecture (soul.py) inspired by human memory resilience.


<details>
  <summary>Details</summary>
Motivation: AI agents suffer from catastrophic forgetting and identity loss when context windows overflow or histories are summarized, highlighting a need for resilient identity architectures.

Method: The authors introduce soul.py, an open-source framework with separable identity components (identity files, memory logs) and a hybrid RAG+RLM retrieval system for efficient memory access.

Result: The framework achieves persistent identity by distributing memory across multiple systems, akin to human memory resilience, and provides a roadmap for identity survival in partial memory failures.

Conclusion: Distributed identity architectures, like soul.py, can mitigate AI agent identity loss and improve resilience against memory failures, inspired by human neurological models.
...
### [71] [PAC-BENCH: Evaluating Multi-Agent Collaboration under Privacy Constraints](https://arxiv.org/abs/2604.11523)
*Minjun Park,Donghyun Kim,Hyeonjong Ju,Seungwon Lim,Dongwook Choi,Taeyoon Kwon,Minju Kim,Jinyoung Yeo*

Main category: cs.AI

TL;DR: The paper introduces $PAC\text{-}Bench$, a benchmark for evaluating multi-agent collaboration under privacy constraints, revealing significant performance degradation and recurring coordination issues.


<details>
  <summary>Details</summary>
Motivation: The rise of AI agents necessitates understanding their collaboration dynamics under privacy constraints, which remains underexplored.

Method: The authors develop $PAC\text{-}Bench$ to systematically assess multi-agent collaboration performance under privacy constraints.

Result: Experiments show privacy constraints degrade collaboration, making outcomes depend more on the initiating agent, and reveal recurring coordination breakdowns.

Conclusion: Privacy-aware multi-agent collaboration is a distinct, unresolved challenge requiring new coordination mechanisms.
...
### [72] [DeepReviewer 2.0: A Traceable Agentic System for Auditable Scientific Peer Review](https://arxiv.org/abs/2604.09590)
*Yixuan Weng,Minjun Zhu,Qiujie Xie,Zhiyuan Ning,Shichen Li,Panzhong Lu,Zhen Lin,Enhao Gu,Qiyao Sun,Yue Zhang*

Main category: cs.AI

TL;DR: DeepReviewer 2.0 is a traceable, automated peer-review system that outperforms other models in coverage and human comparisons by producing anchored critiques and executable follow-up actions.


<details>
  <summary>Details</summary>
Motivation: The need for audit-friendly peer reviews with clear evidence and follow-up actions drives the development of DeepReviewer 2.0.

Method: The system builds a claim--evidence--risk ledger and verification agenda, then performs retrieval and writes anchored critiques under an export gate.

Result: DeepReviewer 2.0 outperforms Gemini-3.1-Pro-preview and wins 71.63% of blind comparisons against human reviewers.

Conclusion: Positioned as an assistive tool, DeepReviewer 2.0 shows promise but has gaps like ethics-sensitive checks.
...
TL;DR: SCBench introduces a hierarchical spatial competence benchmark for evaluating large models, revealing accuracy declines as tasks become more complex and highlighting limitations in global constraint adherence.


<details>
  <summary>Details</summary>
Motivation: Current spatial evaluation methods for large models are limited to isolated primitives, lacking comprehensive assessment of spatial competence.

Method: Developed SCBench with three hierarchical capability buckets, tasks requiring executable outputs verified by deterministic checkers or simulator-based evaluators.

Result: Frontier models show decreasing accuracy with task complexity; accuracy gains saturate quickly at low token budgets, with failures due to locally plausible but globally inconsistent geometry.

Conclusion: SCBench provides a robust tool for assessing spatial competence, uncovering limitations in current models' ability to handle hierarchical spatial tasks and global constraints.
...
### [74] [GenTac: Generative Modeling and Forecasting of Soccer Tactics](https://arxiv.org/abs/2604.11786)
*Jiayuan Rao,Tianlin Gui,Haoning Wu,Yanfeng Wang,Weidi Xie*

Main category: cs.AI

TL;DR: GenTac is a diffusion-based generative framework for modeling soccer tactics as a stochastic process, capturing multi-player trajectories and tactical events. It outperforms existing methods in diversity, accuracy, and contextual conditioning, and generalizes to other team sports.


<details>
  <summary>Details</summary>
Motivation: Existing methods for modeling soccer tactics are deterministic or limited to set-pieces, failing to capture the game's stochastic and multi-agent nature. GenTac addresses this gap by modeling tactics as a dynamic, varied process.

Method: GenTac uses a diffusion-based approach to learn player movement distributions from historical data, generating diverse future trajectories. It supports contextual conditioning (e.g., opponent behavior, team styles) and maps dynamics to a 15-class tactical event space.

Result: Evaluations on TacBench show GenTac excels in geometric accuracy, stylistic nuance, controllable simulations, and tactical outcome anticipation. It also generalizes to basketball, American football, and ice hockey.

Conclusion: GenTac effectively models the stochastic nature of soccer tactics, offering diverse, context-aware simulations and demonstrating cross-sport applicability. It sets a new benchmark for dynamic team sports analysis.
...
TL;DR: CID-TKG is a collaborative learning framework for temporal knowledge graph reasoning that integrates evolutionary dynamics and historical invariance semantics, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing TKG reasoning approaches suffer from limitations due to their inductive biases, relying on time-invariant or weakly time-dependent structures and ignoring evolutionary dynamics.

Method: CID-TKG constructs historical invariance and evolutionary dynamics graphs, uses dedicated encoders for each, and employs contrastive learning to align view-specific representations.

Result: CID-TKG outperforms existing methods, achieving state-of-the-art performance in extrapolation settings.

Conclusion: The proposed CID-TKG framework effectively integrates evolutionary dynamics and historical invariance, improving TKG reasoning performance.
...
TL;DR: Hubble is a framework using LLMs for automated factor discovery in finance, combining interpretability and performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of discovering predictive alpha factors in finance, which is hindered by complexity and noise in data.

Method: Hubble uses LLMs as search heuristics, constrained by domain-specific rules and an AST sandbox, with a feedback loop for refinement.

Result: Achieved a peak composite score of 0.827 with 100% computational stability on a dataset of 30 U.S. equities.

Conclusion: Combining LLMs with deterministic constraints offers an effective, interpretable, and reproducible solution for factor discovery.
...
TL;DR: The paper extends Leyva-Vázquez and Smarandache's work on neutrosophic T/I/F evaluation, confirming hyper-truth across multiple LLMs and identifying a limitation in scalar T/I/F. It proposes tensor-structured output (scalars + losses) to better model epistemic states.


<details>
  <summary>Details</summary>
Motivation: To address the limitation of scalar T/I/F evaluations in distinguishing fundamentally different epistemic situations (paradox, ignorance, contingency) and to validate the hyper-truth phenomenon across multiple LLM vendors.

Method: Replicated and extended the original experiment across five model families, analyzed scalar T/I/F outputs, and introduced declared losses (structured descriptions of evaluation limitations) to recover epistemic distinctions.

Result: Hyper-truth was confirmed in 84% of evaluations across vendors. Scalar T/I/F outputs were found insufficient, but adding declared losses improved differentiation (Jaccard similarity < 0.10 for loss vocabularies).

Conclusion: Scalar T/I/F is necessary but insufficient; tensor-structured output (scalars + losses) provides a more faithful representation of LLM epistemic capabilities.
...
### [78] [LLMs for Text-Based Exploration and Navigation Under Partial Observability](https://arxiv.org/abs/2604.09604)
*Stephan Sandfuchs,Maximilian Melchert,Jörg Frochte*

Main category: cs.AI

TL;DR: The study evaluates if large language models (LLMs) can act as text-only controllers for exploration and navigation in partially observable gridworlds, finding that reasoning-tuned models perform better but still lag behind oracle paths.


<details>
  <summary>Details</summary>
Motivation: The research aims to assess the capability of LLMs to perform exploration and navigation tasks in unknown layouts without additional tools or code execution, which is crucial for applications like inspection, logistics, and search-and-rescue.

Method: A reproducible benchmark with oracle localization in fixed ASCII gridworlds is introduced. Nine LLMs are tested on exploration and navigation tasks across three layouts of increasing difficulty, with quantitative and qualitative analysis.

Result: Reasoning-tuned models reliably complete navigation tasks but are less efficient than oracle paths. Few-shot demonstrations improve their performance, while dense instruction models remain inconsistent. Training regimen and test-time deliberation are key factors in performance.

Conclusion: The findings suggest that lightweight hybridisation with classical online planners could enhance deployable partial map systems, with reasoning-tuned models showing promise as controllers.
...
### [79] [Evaluating Reliability Gaps in Large Language Model Safety via Repeated Prompt Sampling](https://arxiv.org/abs/2604.09606)
*Keita Broadwater*

Main category: cs.AI

TL;DR: APST introduces a depth-oriented evaluation framework to assess LLM reliability under repeated use, revealing inconsistencies not captured by traditional benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarks evaluate LLMs broadly but miss operational risks like repeated prompt failures. APST addresses this gap for high-stakes scenarios.

Method: APST repeatedly samples identical prompts under varied conditions (e.g., temperature, perturbations) to uncover latent failures (e.g., hallucinations, refusal inconsistency, unsafe outputs), modeling failures statistically.

Result: APST reveals significant variations in failure probabilities across models under repeated use, unlike conventional benchmarks with low samples (N <= 3).

Conclusion: Shallow benchmarks may hide reliability issues in sustained use; APST provides deeper operational risk insights.
...
### [80] [The Diffusion-Attention Connection](https://arxiv.org/abs/2604.09560)
*Julio Candanedo*

Main category: cs.LG

TL;DR: This paper unifies transformers, diffusion-maps, and magnetic Laplacians under a single Markov geometry framework using pre-softmax query-scores.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between seemingly disparate tools (transformers, diffusion-maps, magnetic Laplacians) by showing they are interconnected through a common Markov geometry framework.

Method: The authors introduce a QK 'bidivergence' and use its exponentiated and normalized forms to derive attention, diffusion-maps, and magnetic diffusion. They employ product of experts and Schrödinger-bridges to categorize these tools into equilibrium, nonequilibrium steady-state, and driven dynamics.

Result: The paper demonstrates that transformers, diffusion-maps, and magnetic Laplacians can be understood as different regimes of a unified framework, providing a cohesive theoretical foundation.

Conclusion: The conclusion is that these tools are interconnected and can be systematically organized within a single framework, offering new insights and potential applications.
...
### [81] [Fairboard: a quantitative framework for equity assessment of healthcare models](https://arxiv.org/abs/2604.09656)
*James K. Ruffle,Samia Mohinta,Chris Foulon,Mohamad Zeina,Zicheng Wang,Sebastian Brandner,Harpreet Hyare,Parashkev Nachev*

Main category: cs.LG

TL;DR: AI medical devices lack formal equity assessments. This study evaluates 18 brain tumor segmentation models, finding patient identity impacts performance more than model choice, with biases linked to clinical factors and neuroanatomy. Fairboard, an open-source tool, is introduced for equitable model monitoring.


<details>
  <summary>Details</summary>
Motivation: To assess the equity of AI medical devices, particularly in brain tumor segmentation, as formal evaluations across patient subgroups are rare.

Method: Evaluated 18 open-source brain tumor segmentation models using 648 glioma patients from two datasets (11,664 inferences). Analyzed performance along univariate, Bayesian multivariate, spatial, and representational dimensions.

Result: Patient identity explained more performance variance than model choice, with clinical factors (e.g., molecular diagnosis) predicting accuracy better than model architecture. Biases were neuroanatomically localized and consistent across models. Newer models showed better equity but lacked fairness guarantees.

Conclusion: AI medical devices exhibit performance disparities linked to patient identity and clinical factors. Fairboard is introduced to facilitate equitable model monitoring in medical imaging.
...
### [82] [Deliberative Alignment is Deep, but Uncertainty Remains: Inference time safety improvement in reasoning via attribution of unsafe behavior to base model](https://arxiv.org/abs/2604.09665)
*Pankayaraj Pathmanathan,Furong Huang*

Main category: cs.LG

TL;DR: The paper explores the limitations of refusal training in LLMs, introduces deliberative alignment to enhance safety, and proposes a BoN sampling method to improve safety without significant utility loss.


<details>
  <summary>Details</summary>
Motivation: To address the shallow nature of existing alignment methods in LLMs, the study aims to instill deeper safety by distilling reasoning from stronger models and mitigating unsafe behaviors.

Method: Deliberative alignment is used to distill reasoning from stronger models, and a BoN sampling method attributes unsafe behaviors to the base model in latent space, down-ranking unsafe responses.

Result: BoN sampling reduces ASR by 28.2% in DAN, 31.3% in WildJailbreak, and 35.4% in StrongREJECT, with minimal utility loss, and gains persist post RL training.

Conclusion: Deliberative alignment and BoN sampling effectively enhance LLM safety, but safety reasoning uncertainty remains, linked to the base model.
...
### [83] [Human-like Working Memory Interference in Large Language Models](https://arxiv.org/abs/2604.09670)
*Hua-Dong Xiong,Li Ji-An,Jiaqi Huang,Robert C. Wilson,Kwonjoon Lee,Xue-Xin Wei*

Main category: cs.LG

TL;DR: The paper investigates why large language models (LLMs) exhibit working memory limitations despite transformers' full context access, identifying representational interference as a key constraint shared with biological systems.


<details>
  <summary>Details</summary>
Motivation: To understand the working memory limitations in LLMs, despite their architecture's potential for full context access, and to explore parallels with human cognitive constraints.

Method: Analysis of pretrained LLMs on working memory tasks, comparing performance across models and identifying computational mechanisms like interference control.

Result: LLMs show working memory limitations similar to humans, with performance degrading under memory load and interference. Stronger working memory correlates with broader competence, and models rely on entangled representations and interference control.

Conclusion: Representational interference is a core constraint on working memory in LLMs and biological systems, highlighting a shared computational challenge in selecting task-relevant information under interference.
...
### [84] [Belief-State RWKV for Reinforcement Learning under Partial Observability](https://arxiv.org/abs/2604.09671)
*Liu Xiao*

Main category: cs.LG

TL;DR: The paper introduces a reinforcement learning (RL) formulation using belief states derived from RWKV-style recurrent models, addressing confidence awareness in policies for partially observed settings.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitation of fixed-state policies in partially observed environments, which store evidence but not confidence, the authors propose using belief states (μ_t, Σ_t) to incorporate uncertainty.

Method: The method involves maintaining a compact belief state b_t = (μ_t, Σ_t) from RWKV-style recurrent statistics, allowing control to depend on both memory and uncertainty. Theoretical groundwork and a pilot RL experiment with hidden noise were conducted.

Result: Belief-state policies nearly matched the best recurrent baselines overall, slightly improving performance in the hardest in-distribution regime and under a held-out noise shift. Ablations showed this approach outperformed more structured extensions.

Conclusion: The simple belief readout method is effective, highlighting the need for richer benchmarks to evaluate such approaches in RL.
...
### [85] [Active Inference with a Self-Prior in the Mirror-Mark Task](https://arxiv.org/abs/2604.09673)
*Dongmin Kim,Hoshinori Kanazawa,Yasuo Kuniyoshi*

Main category: cs.LG

TL;DR: A computational model using a 'self-prior' mechanism, implemented with a Transformer, explains mirror self-recognition behavior without external rewards, simulating infant behavior in the mirror test.


<details>
  <summary>Details</summary>
Motivation: To understand how self-awareness behaviors like mirror self-recognition emerge spontaneously, without explicit rewards or instructions, using computational modeling.

Method: The study employs a Transformer-based 'self-prior' model, which learns the density of familiar multisensory experiences (vision and proprioception). The discrepancy from this learned distribution drives mark-directed behavior through active inference. A simulated infant interacts with a mirror to discover and remove a sticker on its face.

Result: The simulated infant removed the sticker in ~70% of cases. Expected free energy decreased post-removal, confirming the self-prior's role in distinguishing self from non-self. Cross-modal sampling revealed the self-prior functions as a probabilistic body schema.

Conclusion: The study provides a computational explanation for mirror test behavior, suggesting the free energy principle as a unifying framework for studying self-awareness development.
...
### [86] [A Comparative Theoretical Analysis of Entropy Control Methods in Reinforcement Learning](https://arxiv.org/abs/2604.09676)
*Ming Lei,Christophe Baehr*

Main category: cs.LG

TL;DR: This paper compares entropy control strategies in RL for LLMs, highlighting pitfalls of traditional entropy regularization and benefits of covariance-based methods.


<details>
  <summary>Details</summary>
Motivation: To address the issue of policy entropy collapse in RL for LLMs, which causes premature convergence and performance saturation.

Method: Theoretical analysis of entropy dynamics under softmax parameterization, comparing traditional entropy regularization and covariance-based methods.

Result: Traditional methods introduce a dense bias, leading to suboptimal policies, while covariance-based methods selectively regularize high-covariance tokens, achieving unbiasedness.

Conclusion: The findings offer guidelines for effective entropy control in LLM posttraining, aiding RL scalability for complex tasks.
...
### [87] [STaR-DRO: Stateful Tsallis Reweighting for Group-Robust Structured Prediction](https://arxiv.org/abs/2604.09737)
...
TL;DR: A two-part framework for structured prediction combines task-agnostic prompting and robust optimization to improve accuracy and reliability in extracting hierarchical labels and evidence spans from patient-provider messages, with significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Address challenges in structured prediction like ambiguity, label skew, and heterogeneous group difficulty, particularly in clinical communication scenarios where reliability is critical.

Method: 1) Task-agnostic prompting with XML-based instructions, disambiguation, and self-validation. 2) STaR-DRO, a robust optimization method using Tsallis mirror descent and momentum-smoothed group-loss signals.

Result: Zero-shot performance improved by +15.44 average F1. STaR-DRO boosted Code F1 from 79.24 to 81.47 and Sub-code F1 from 67.78 to 69.30, while reducing cross-entropy by up to 29.6% for difficult clinical categories.

Conclusion: The framework enhances structured prediction for clinical communication, directly improving reliability in patient-centered care analysis by focusing on rare and challenging cases.
...
### [88] [ExecTune: Effective Steering of Black-Box LLMs with Guide Models](https://arxiv.org/abs/2604.09741)
...
TL;DR: The paper introduces Guide-Core Policies (GCoP) to reduce recurring inference costs in black-box language models by generating reusable intermediate strategies. It proposes ExecTune, a training method to improve executability, achieving better accuracy and lower costs.


<details>
  <summary>Details</summary>
Motivation: Recurring inference costs in black-box APIs often surpass training costs, prompting the need for efficient systems like GCoP to amortize expenses through reusable strategies.

Method: GCoP uses a guide model to create structured strategies executed by a core model. ExecTune combines teacher-guided sampling, supervised fine-tuning, and reinforcement learning to optimize executability.

Result: GCoP with ExecTune improves accuracy by up to 9.2% and reduces costs by 22.4%, enabling smaller models to outperform larger ones in some tasks.

Conclusion: GCoP with ExecTune offers a cost-effective and modular solution for deploying language models, balancing efficiency and performance.
...
### [89] [Efficient Matrix Implementation for Rotary Position Embedding](https://arxiv.org/abs/2604.09742)
*Chen Minqi,Zhongqi Yue,Shihao Zhang,Yun Xu,Peng Wu,kaixiang Xu,Zeyi Huang,Hanwang Zhang*

Main category: cs.LG

TL;DR: RoME improves computational efficiency of Rotary Position Embedding by replacing vector ops with matrix transformations, reducing overhead and enhancing hardware utilization.


<details>
  <summary>Details</summary>
Motivation: Existing RoPE implementations suffer from computational inefficiency due to vector-level operations, especially in multi-dimensional settings.

Method: Proposes RoME, a reformulation of RoPE using unified matrix transformations to replace dimension-specific vector operations.

Result: RoME provides significant acceleration at both operator and full-model levels, improving hardware utilization.

Conclusion: RoME offers a more efficient and hardware-friendly alternative to traditional RoPE implementations.
...
### [90] [Explainable Human Activity Recognition: A Unified Review of Concepts and Mechanisms](https://arxiv.org/abs/2604.09799)
*Mainak Kundu,Catherine Chen,Rifatul Islam,Ismail Uysal,Ria Kanjilal*

Main category: cs.LG

TL;DR: This paper reviews explainable AI methods for human activity recognition (HAR), focusing on transparency, interpretability, and practical deployment. It introduces a taxonomy for XAI-HAR and discusses challenges and future directions.


<details>
  <summary>Details</summary>
Motivation: To address the opacity of deep learning models in HAR, the paper emphasizes the need for explainable AI (XAI) to enhance trust, reliability, and real-world usability of HAR systems.

Method: The study presents a comprehensive review of XAI-HAR methods across various sensing settings, introduces a unified perspective on explainability, and proposes a mechanism-centric taxonomy. It also analyzes interpretability objectives and limitations.

Result: The review highlights how XAI methods tackle HAR complexities, summarizes their strengths and weaknesses, and identifies gaps in current evaluation practices.

Conclusion: The paper underscores the importance of XAI for trustworthy HAR systems, outlines challenges in achieving deployable solutions, and suggests future directions for human-centered activity recognition.
...
### [91] [NeuroFlow: Toward Unified Visual Encoding and Decoding from Neural Activity](https://arxiv.org/abs/2604.09817)
*Weijian Mai,Mu Nan,Yu Zhu,Jiahang Cao,Rui Zhang,Yuqin Dai,Chunfeng Song,Andrew F. Luo,Jiamin Wu*

Main category: cs.LG

TL;DR: NeuroFlow introduces a unified framework for visual encoding and decoding models, achieving superior performance and efficiency by jointly modeling both tasks within a shared latent space.


<details>
  <summary>Details</summary>
Motivation: Traditional methods treat visual encoding and decoding as separate tasks, which is inefficient and fails to model their consistency.

Method: NeuroFlow uses NeuroVAE for a compact latent space and Cross-modal Flow Matching (XFM) to learn reversible flows between visual and neural modalities.

Result: NeuroFlow outperforms isolated methods in performance and efficiency, capturing consistent neural activation patterns.

Conclusion: NeuroFlow advances unified modeling of visual encoding and decoding, offering insights for future brain-computer interfaces.
...
### [92] [Below-ground Fungal Biodiversity Can be Monitored Using Self-Supervised Learning Satellite Features](https://arxiv.org/abs/2604.09818)
*Robin Young,Michael E. Van Nuland,E. Toby Kiers,Tomáš Větrovský,Petr Kohout,Petr Baldrian,Srinivasan Keshav*

Main category: cs.LG

TL;DR: Self-supervised learning (SSL) on satellite imagery effectively predicts ectomycorrhizal fungal richness, outperforming traditional climate, soil, and land cover data, enabling high-resolution, dynamic monitoring of fungal biodiversity.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of monitoring mycorrhizal fungal biodiversity at landscape scales due to time and cost constraints, and the lack of protection for diversity hotspots.

Method: Utilizes SSL applied to satellite imagery to predict fungal richness, validated against ~12,000 field samples across Europe and Asia.

Result: SSL-derived features explain over half the variance in species richness, offering a 10,000-fold increase in spatial resolution (from 1km to 10m) with minimal bias. Temporal trends in UK National Park woodlands reveal disproportionate diversity loss in ancient forests.

Conclusion: SSL satellite features provide a scalable tool for high-resolution, dynamic mapping of below-ground biodiversity, enhancing monitoring capabilities for terrestrial ecosystems.
...
### [93] [Relational Preference Encoding in Looped Transformer Internal States](https://arxiv.org/abs/2604.09870)
*Jan Kirin*

Main category: cs.LG

TL;DR: The study explores how looped transformers encode human preference, showing that preference is encoded relationally in loop states. A lightweight evaluator trained on these states achieves high accuracy, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: To understand how looped transformers like Ouro-2.6B-Thinking encode human preference in their internal states and whether lightweight evaluators can effectively predict these preferences.

Method: Hidden states from each loop iteration of Ouro-2.6B-Thinking are extracted, and lightweight evaluator heads are trained to predict human preference on the Anthropic HH-RLHF dataset. A systematic architecture search and various probes (linear, nonlinear) are used.

Result: The pairwise evaluator achieves 95.2% test accuracy, significantly outperforming full-batch L-BFGS probes (84.5%). Preference is encoded relationally, as linear probes on pairwise differences work better than independent classifiers.

Conclusion: Looped transformers encode human preference relationally in their loop states. Lightweight evaluators can effectively predict these preferences, and the flip test is proposed as a diagnostic tool for pairwise preference evaluators.
...
### [94] [Efficient Personalization of Generative User Interfaces](https://arxiv.org/abs/2604.09876)
*Yi-Hao Peng,Samarth Das,Jeffrey P. Bigham,Jason Wu*

Main category: cs.LG

TL;DR: The paper explores generative user interfaces (UIs) and personalization challenges, using a dataset of designer preferences to develop a sample-efficient personalization method that outperforms existing approaches.


<details>
  <summary>Details</summary>
Motivation: Adapting UIs to individual users is challenging due to subjective and hard-to-articulate preferences. The study aims to understand preference divergence among designers and improve personalization.

Method: Analyzed a dataset of 20 designers' pairwise judgments on 600 generated UIs, identified preference divergence, and developed a personalization method based on prior designers' preferences.

Result: Found significant disagreement among designers (average kappa = 0.25). The new personalization method outperformed a pretrained UI evaluator and a larger multimodal model, scaling better with feedback.

Conclusion: Lightweight preference elicitation can effectively personalize generative UIs, as demonstrated by the method's success in producing preferred interfaces for new designers.
...
### [95] [SemEnrich: Self-Supervised Semantic Enrichment of Radiology Reports for Vision-Language Learning](https://arxiv.org/abs/2604.09887)
*Halil Ibrahim Gulluk,Olivier Gevaert*

Main category: cs.LG

TL;DR: A self-supervised method enriches medical reports by adding positive/neutral observations via semantic clustering, improving model performance metrics significantly.


<details>
  <summary>Details</summary>
Motivation: Medical datasets often focus on negative findings, missing positive/neutral observations. This gap limits model training and performance.

Method: Proposes semantic clustering of report sentences to enrich training data with positive/neutral findings in a self-supervised manner. Also integrates cluster info into GRPO reward design.

Result: Significant gains in supervised fine-tuning (e.g., 5.63% avg. COMET score). Semantic clustering, not random augmentation, drives improvements. Further gains from GRPO integration.

Conclusion: The method effectively addresses dataset bias, enhancing model performance through semantic enrichment and reward design innovation.
...
### [96] [Improving Pediatric Emergency Department Triage with Modality Dropout in Late Fusion Multimodal EHR Models](https://arxiv.org/abs/2604.09905)
*Tyler Yang,Romal Mitr*

Main category: cs.LG

TL;DR: A late-fusion multimodal model combining XGBoost for vital signs and Bio_ClinicalBERT for clinical notes, with symmetric modality dropout, improves triage acuity prediction and cross-demographic generalization, especially for pediatric patients.


<details>
  <summary>Details</summary>
Motivation: Address modality collapse in multimodal models for emergency triage, which limits demographic generalizability, particularly for pediatric patients due to developmental variations in vital signs.

Method: Late-fusion architecture using XGBoost for tabular vitals and Bio_ClinicalBERT for clinical text, combined via Logistic Regression. Trained on adult data (MIMIC-IV and NHAMCS) and evaluated on pediatric data with symmetric modality dropout.

Result: The multimodal model outperforms single-modality baselines, with a 30-40% modality dropout rate improving performance (Quadratic Weighted Kappa = 0.351) in the pediatric cohort.

Conclusion: Modality dropout is crucial for preventing modality collapse and enhancing cross-demographic generalization in clinical AI models.
...
### [97] [Last-Iterate Convergence of Randomized Kaczmarz and SGD with Greedy Step Size](https://arxiv.org/abs/2604.09909)
*Michał Dereziński,Xiaoyu Dong*

Main category: cs.LG

TL;DR: The paper analyzes the last-iterate convergence of SGD with greedy step size in smooth quadratics, improving the rate to O(1/t^{3/4}) from the previous O(1/t^{1/2}).


<details>
  <summary>Details</summary>
Motivation: The study aims to address a question posed by Attia et al. regarding the convergence rate of SGD with greedy step size in interpolating linear systems, a common scenario in iterative solvers like the Randomized Kaczmarz algorithm.

Method: The authors introduce stochastic contraction processes and analyze their behavior using a deterministic eigenvalue equation, employing a discrete-to-continuous reduction for rigor.

Result: They prove a O(1/t^{3/4}) convergence rate for the t-th iterate, improving upon the prior O(1/t^{1/2}) result.

Conclusion: The work provides a tighter convergence guarantee for SGD in this setting, leveraging novel analytical techniques.
...
### [98] [Regularized Entropy Information Adaptation with Temporal-Awareness Networks for Simultaneous Speech Translation](https://arxiv.org/abs/2604.09916)
*Joseph Liu,Nameer Hirschkind,Xiao Yu,Mahesh Kumar Nandwana*

Main category: cs.LG

TL;DR: The paper improves REINA, a method for Simultaneous Speech Translation (SimulST), by introducing two strategies (REINA-SAN and REINA-TAN) to address the lack of temporal context in information-based policies. Both methods outperform baselines, with REINA-TAN excelling in streaming efficiency and REINA-SAN in robustness.


<details>
  <summary>Details</summary>
Motivation: Existing information-based policies in SimulST often lack temporal context, causing delays in writing. The goal is to enhance REINA to balance translation quality and latency better.

Method: Two strategies are introduced: a supervised alignment network (REINA-SAN) and a timestep-augmented network (REINA-TAN). These address the temporal context issue and improve stability.

Result: Both REINA-SAN and REINA-TAN significantly outperform baselines. REINA-TAN achieves better streaming efficiency, while REINA-SAN is more robust against read loops. Applied to Whisper, they improve NoSE scores by up to 7.1%.

Conclusion: The proposed methods (REINA-SAN and REINA-TAN) enhance REINA, offering superior performance in SimulST by addressing temporal context limitations and improving streaming efficiency and robustness.
...
### [99] [A Tale of Two Temperatures: Simple, Efficient, and Diverse Sampling from Diffusion Language Models](https://arxiv.org/abs/2604.09921)
*Theo X. Olausson,Metod Jazbec,Xi Wang,Armando Solar-Lezama,Christian A. Naesseth,Stephan Mandt,Eric Nalisnick*

Main category: cs.LG

TL;DR: The paper addresses the lack of diversity in sampling from diffusion language models (dLLMs) and proposes tempered heuristics to improve diversity without sacrificing speed or quality.


<details>
  <summary>Details</summary>
Motivation: Existing methods for sampling from dLLMs prioritize speed and quality but overlook diversity. The study aims to bridge this gap by ensuring diverse samples while maintaining computational efficiency.

Method: The proposed approach uses softened, tempered versions of confidence-based remasking heuristics, motivated by an idealized model of fork tokens and their impact on entropy.

Result: The tempered heuristics improve diversity, closing the exploration gap (pass@k) between confidence-based and autoregressive sampling, and outperform both when controlling for cost (pass@NFE).

Conclusion: The study shows that simple, efficient, and diverse sampling from dLLMs is achievable, with implications for downstream scaling and post-training applications.
