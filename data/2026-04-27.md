...
TL;DR: A novel approach using a single image to forecast solar panel irradiance is presented, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: To reduce soft costs in solar panel installation by improving irradiance assessment accuracy, especially in urban settings with small nearby structures.

Method: Uses a single image from the panel's location to determine camera orientation and visible sky portion, forecasting irradiance from the sun, sky, and reflections. Validated with real measurements.

Result: More accurate irradiance forecasts compared to conventional methods and 3D model-based simulations. Also identifies optimal panel orientation.

Conclusion: The approach simplifies and improves irradiance assessment, with practical applications demonstrated by the Solaris device.
...
TL;DR: Soft Anisotropic Diagrams (SAD) is a differentiable image representation using adaptive sites, offering efficient rendering and superior performance compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: To create a flexible, efficient, and differentiable image representation that preserves gradients and supports content-aligned boundaries.

Method: Uses anisotropic metrics and weighted distances with a softmax blend over top-K sites, enabling GPU-friendly rendering and training with adaptive budget control.

Result: Outperforms Image-GS and Instant-NGP in benchmarks, achieving faster encoding (2.2 s vs. 28 s) and 4-19x training speedups.

Conclusion: SAD is effective for differentiable pipelines, offers fast random access, and compact storage, making it a practical solution for various applications.
...
TL;DR: EgoMAGIC is an egocentric medical activity dataset with 3,355 videos of 50 tasks, released for research, including an action detection challenge. It includes trained YOLO models and baseline results, achieving 0.526 mAP.


<details>
  <summary>Details</summary>
Motivation: To support the development of virtual assistants in augmented reality for medical tasks by providing a comprehensive dataset.

Method: Data collection via head-mounted stereo cameras, labeling 1.95 million objects, training 40 YOLO models, and benchmarking action detection on 8 tasks.

Result: Best-performing model achieved an average mAP of 0.526 for action detection.

Conclusion: EgoMAGIC is a valuable resource for medical AI applications, including action detection, recognition, and error detection, accessible via Zenodo.
...
TL;DR: H-Sets, a two-stage framework, identifies and attributes higher-order feature interactions in image classifiers, producing sparser and more faithful saliency maps than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing feature attribution methods overlook feature interactions, which are crucial for image classification tasks where semantic meaning depends on pixel interdependencies.

Method: H-Sets involves detecting interacting pairs via input Hessians, merging them into coherent sets using segmentation (e.g., SAM), and attributing sets with IDG-Vis, an extension of Integrated Directional Gradients.

Result: H-Sets generates more interpretable and faithful saliency maps, outperforming existing methods on models like VGG, ResNet, DenseNet, and MobileNet across ImageNet and CUB datasets.

Conclusion: H-Sets effectively captures and attributes feature interactions, improving interpretability and faithfulness in saliency maps for image classifiers.
...
TL;DR: FLARE-BO extends Bayesian optimisation to jointly optimise eight parameters for enhancing low-light images, outperforming existing methods without dataset-specific training.


<details>
  <summary>Details</summary>
Motivation: Low-light visual perception is critical for autonomous systems, but current methods are limited in parameters and lack illumination decomposition or white balance correction.

Method: FLARE-BO employs Bayesian optimisation to adjust eight parameters, including gamma correction, LIME-style illumination, and various denoising techniques, using a principled search strategy.

Result: Benchmarked on the LOL dataset, FLARE-BO shows significant improvement over existing methods not trained on this dataset.

Conclusion: The proposed FLARE-BO framework effectively addresses limitations of prior methods, enhancing low-light image quality without requiring dataset-specific training.
...
TL;DR: A system for calibrating and verifying optical mocap setups, especially for fisheye cameras, addressing common errors and drift, with successful deployment in production.


<details>
  <summary>Details</summary>
Motivation: Optical mocap systems often face calibration errors, especially with fisheye cameras, which can corrupt downstream data. A robust solution is needed.

Method: The system jointly estimates camera extrinsics and board-to-marker transforms, using a staged solver for reliable convergence. It includes a verification tool ('lollypop') for independent assessment.

Result: The calibration outperforms existing methods and reliably detects degradation over time, as tested on a Meta Quest 3 headset with fisheye cameras.

Conclusion: The proposed system effectively addresses calibration challenges in mocap setups, ensuring reliability and has been successfully deployed in production pipelines.
...
TL;DR: PAGaS improves Gaussian Splatting for multi-view stereo depth by using 1DoF Gaussians, enhancing geometric accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of early GS variants in accurately modeling scene geometry, particularly for multi-view stereo depth tasks.

Method: Uses pixel-aligned 1DoF Gaussians, constrained by back-projected pixel volumes, optimizing only depth.

Result: Produces highly detailed depths and outperforms baselines on 3D reconstruction benchmarks.

Conclusion: PAGaS successfully adapts GS for multi-view stereo depth, achieving superior geometric fidelity.
...
TL;DR: An unsupervised anomaly detection framework for OCT imaging is proposed, improving diagnostic reliability without expensive expert annotations.


<details>
  <summary>Details</summary>
Motivation: The need for costly expert annotations and supervised models' limited generalization across diverse pathologies and devices motivates an unsupervised approach.

Method: The framework uses a discrete latent model trained on normal B-scans, with retinal layer-aware supervision and structured triplet learning to separate healthy from pathological representations.

Result: The method outperforms baselines (e.g., VAE, VQVAE) with AUROC 0.799 on Kermany and achieves 0.884 on Srinivasan, demonstrating robust domain adaptation. It also shows competitive results on RETOUCH.

Conclusion: The unsupervised framework effectively detects and localizes anomalies without disease-specific labels, offering scalable and generalizable solutions for OCT analysis.
...
TL;DR: This paper introduces a generative model inspired by human visual perception to unify motion-based scene interpretation across diverse settings, from random dots to naturalistic videos, achieving robust object-level understanding.


<details>
  <summary>Details</summary>
Motivation: The study aims to bridge the gap between human and computer vision systems by developing a unified approach for detecting and segmenting moving entities, leveraging insights from human perception.

Method: The proposed model hierarchically groups low-level motion cues and high-level appearance features into particles, then clusters them into independently moveable entities. It uses a hardware-accelerated, parallelized block Gibbs sampling algorithm for inference.

Result: The framework works across 2D random dot kinematograms, Gestalt-inspired camouflaged objects, and naturalistic RGB videos, achieving human-like object perception, accurate 3D structure recovery, and robust object tracking.

Conclusion: The paper establishes a general, biologically inspired framework for motion-based perception, demonstrating its versatility and effectiveness in diverse scenarios.
...
TL;DR: SAMIDARE enhances SAM2MOT for crowded sports scenes with density-aware mask re-generation, selective memory updates, and state-aware association, achieving top performance on SportsMOT.


<details>
  <summary>Details</summary>
Motivation: Addressing mask errors and ID switches in dense sports scenes for robust multi-object tracking.

Method: Incorporates density-aware mask re-generation, selective memory updates, and state-aware association to improve tracking.

Result: Achieves state-of-the-art results, outperforming the baseline by 2.5 HOTA and 4.2 IDF1 points.

Conclusion: Adaptive feature management with mask control and state-aware association provides an efficient solution for dense sports tracking.
...
TL;DR: This study focuses on generating one person's motion based on another's in interaction scenarios, using Transformer-based models and a person ID embedding to improve consistency and interaction dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing methods in human motion modeling primarily address single-agent motion generation, neglecting the mutual dependence in interaction scenarios. This study aims to bridge this gap.

Method: The authors construct a dataset of paired action-reaction motion sequences from boxing matches and compare three Transformer-based models (simple Transformer, iTransformer, Crossformer) with a person ID embedding for identity distinction.

Result: The simple Transformer performs best, generating plausible motions without posture collapse, while iTransformer and Crossformer accumulate errors. The person ID embedding enhances structural consistency.

Conclusion: Explicitly modeling individual identity is crucial for interaction-aware motion generation, as demonstrated by the success of the simple Transformer and the proposed embedding.
...
TL;DR: The paper introduces MDC-guided Cross-modal Prior Transfer (MCPT) to address cross-modal incompatibility in SAR imagery, using frequency-domain modeling for better optical prior adaptation.


<details>
  <summary>Details</summary>
Motivation: Existing domain adaptation methods lack inductive bias for SAR imagery, limiting their ability to transfer optical prior effectively.

Method: Proposes MCPT with Adaptive Frequency Tokenization (AFT) and Frequency-aware Expert Refinement (FER) for feature refinement and contrastive learning.

Result: Achieves state-of-the-art performance on SAR-GCD tasks, demonstrating effective optical prior adaptation.

Conclusion: Frequency-domain discrepancy modeling enhances the transfer of optical prior to SAR imagery, improving downstream task performance.
...
TL;DR: EvFlow-GS is a framework that combines event streams and optical flow to improve 3D reconstruction from motion-blurred images, addressing residual artifacts and blurry textures.


<details>
  <summary>Details</summary>
Motivation: Existing methods using event cameras for 3D reconstruction suffer from residual artifacts and blurry details due to inaccurate event priors and noisy events.

Method: Proposes EvFlow-GS, integrating event streams and optical flow to optimize learnable double integral, camera poses, and 3D Gaussian Splatting with novel event-based losses and event-residual prior.

Result: Demonstrates leading performance in achieving sharper 3D reconstructions.

Conclusion: EvFlow-GS effectively mitigates artifacts and enhances texture details in 3D reconstruction from motion-blurred images.
...
TL;DR: SAGA-ReID improves person re-identification by aligning patch tokens with CLIP's text embeddings, outperforming global pooling methods, especially under occlusion.


<details>
  <summary>Details</summary>
Motivation: Global pooling in CLIP-based ReID lacks spatial selectivity, making it fragile under occlusion and cross-camera variations.

Method: SAGA-ReID aligns intermediate patch tokens with anchor vectors in CLIP's text embedding space, suppressing corrupted regions and emphasizing stable evidence.

Result: SAGA-ReID outperforms CLIP-ReID, especially under occlusion (+10.6 Rank-1), and surpasses sequential patch aggregation on stronger backbones.

Conclusion: Structured reconstruction via SAGA addresses a bottleneck unresolved by backbone quality alone, offering robust performance in occluded and standard settings.
...
TL;DR: CharTide is a novel framework for chart-to-code generation that improves visual precision and syntactic correctness by decoupling training into visual, code logic, and modality fusion streams, and reformulates alignment as a data verification problem using an Inquiry-Driven RL framework.


<details>
  <summary>Details</summary>
Motivation: Existing chart-to-code generation approaches are limited by conflating visual perception with program logic in homogeneous datasets, hindering the full use of multimodal supervision.

Method: CharTide uses a Tri-Perspective Tuning strategy to decouple training into visual perception, pure-text code logic, and modality fusion streams. It also introduces an Inquiry-Driven RL framework for alignment, verified by atomic QA tasks.

Result: CharTide-7B/8B outperforms open-source baselines, surpasses GPT-4o, and is competitive with GPT-5 on ChartMimic, Plot2Code, and ChartX datasets.

Conclusion: CharTide's systematic redesign of training and alignment data advances chart-to-code generation by leveraging multimodal supervision and verifiable alignment, achieving state-of-the-art performance.
...
TL;DR: The paper introduces a framework for detecting 3D-grounded reflectional symmetries from single RGB images, focusing on architectural landmarks, and presents a scalable data annotation pipeline and a symmetry detector leveraging scene geometry.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D symmetry detection from single images fail to generalize to real-world scenes and often only predict symmetry plane orientation due to scale ambiguity.

Method: The authors propose a scalable data annotation pipeline to create a large-scale dataset (ArchSym) from SfM reconstructions and a symmetry detector parameterizing symmetries as signed distance maps relative to predicted scene geometry.

Result: The symmetry annotation pipeline is validated against geometry-based alternatives, and the symmetry detector outperforms state-of-the-art baselines on the new benchmark.

Conclusion: The framework addresses limitations of existing methods, offering a robust solution for detecting 3D symmetries in real-world scenes.
...
TL;DR: FMDiffWA is a frequency-domain modulated diffusion framework for watermark attacks, balancing attack efficacy and visual fidelity by selectively modulating watermark-related frequency components.


<details>
  <summary>Details</summary>
Motivation: The limited progress in watermark attack techniques has disrupted the attack-defense balance in digital image watermarking, hindering advancements in the field.

Method: The framework introduces a frequency-domain watermark modulation (FWM) module integrated into both forward and reverse diffusion processes, along with an auxiliary refinement constraint to improve training.

Result: FMDiffWA effectively neutralizes invisible watermarks while preserving image quality and generalizes well across various watermarking schemes.

Conclusion: FMDiffWA outperforms existing watermark attacks in visual fidelity and attack efficacy, offering a promising solution to restore the attack-defense balance.
...
TL;DR: SportsTime introduces a benchmark and CoTR, a method for long-form sports video understanding, addressing the challenges of sparse evidence and reasoning in MLLMs.


<details>
  <summary>Details</summary>
Motivation: Sports videos' complex dynamics and sparse temporal evidence make long-horizon reasoning difficult for MLLMs.

Method: Developed SportsTime (14K+ QA pairs) and CoTR, which uses temporal-reward GRPO for training and an iterative evidence-seeking loop for inference.

Result: CoTR improves temporal reasoning and grounding quality over existing MLLM baselines.

Conclusion: SportsTime and CoTR effectively address long-form sports video understanding challenges, advancing MLLM capabilities in this domain.
...
TL;DR: OccDirector is a framework that generates 4D occupancy dynamics for autonomous driving simulation using natural language, avoiding rigid geometric priors and excelling in complex multi-agent interactions.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on rigid geometric conditions or simplistic text, missing the ability to handle complex, sequential multi-agent interactions in 4D occupancy generation.

Method: OccDirector uses a VLM-driven Spatio-Temporal MMDiT with history-prefix anchoring to map natural language scripts into plausible voxel dynamics, without geometric priors.

Result: The framework achieves state-of-the-art generation quality and instruction-following capabilities, validated by the novel OccInteract-85k dataset and benchmarks.

Conclusion: OccDirector shifts the paradigm from appearance synthesis to language-driven behavior orchestration in autonomous driving simulation.
...
TL;DR: The paper introduces the Land Transportation Dataset (LTD) and the UniVLT model to address the gap in city-scale traffic analysis and safety-oriented VQA for urban transportation systems, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Urban transportation systems need scalable intelligence for safety, but existing research focuses on microscopic autonomous driving, neglecting city-scale traffic analysis and safety-oriented VQA.

Method: Introduces LTD, a large-scale vision-language dataset for urban traffic environments, and UniVLT, a transportation foundation model trained via curriculum-based knowledge transfer.

Result: UniVLT achieves SOTA performance on open-ended reasoning tasks across diverse domains, though limitations in complex multi-view traffic scenarios are noted.

Conclusion: The work bridges the gap between microscopic AD and macroscopic traffic analysis, highlighting the potential and challenges of foundation models in urban transportation.
...
TL;DR: The paper introduces an evidence-rounded open-vocabulary SGG framework to improve reliability by verifying relations with visual, geometric, and contextual evidence, counterfactual testing, and predicate learning.


<details>
  <summary>Details</summary>
Motivation: Address the reliability issue in open-vocabulary SGG where predicted relations may rely on language priors or object co-occurrence rather than visual evidence.

Method: Proposes a framework with a vision-language proposer for relation candidates, evidence decomposition, a relation-conditioned evidence encoder, counterfactual verifier, predicate learning, and graph-level optimization.

Result: Improves recall-based metrics, unseen predicate generalization, and counterfactual grounding quality across benchmarks.

Conclusion: Shifting from relation generation to verification enhances reliability and interpretability of scene graphs.
...
TL;DR: RIME is a unified framework for multimodal embeddings, combining generation and embedding optimization with retrieval-friendly rewrites, outperforming prior models with shorter reasoning steps.


<details>
  <summary>Details</summary>
Motivation: Addressing the redundancy and ambiguity in Chain-of-Thought reasoning for multimodal embeddings, aiming for more efficient and accurate retrieval.

Method: Introduces RIME with Cross-Mode Alignment (CMA) and Refine Reinforcement Learning (Refine-RL) to align generative and discriminative spaces and optimize rewrites.

Result: Outperforms prior generative embedding models on MMEB-V2, MRMR, and UVRB datasets, with reduced reasoning length.

Conclusion: RIME effectively bridges generative and discriminative embedding spaces, improving efficiency and accuracy in multimodal tasks.
...
TL;DR: DocPrune is a training-free, progressive token pruning framework for efficient long-document understanding, improving throughput and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing token-reduction methods for natural images and videos fail to leverage the structural sparsity of documents, leading to inefficient computational resource usage.

Method: DocPrune progressively prunes unnecessary tokens (e.g., background or irrelevant ones) and selects optimal layers for pruning based on the model's comprehension level.

Result: DocPrune achieves 3.0x and 3.3x throughput improvements in the encoder and decoder, respectively, and boosts the F1 score by +1.0 on M3DocRAG.

Conclusion: DocPrune enhances both efficiency and accuracy for long-document understanding without requiring additional training.
...
TL;DR: The study evaluates synthetic image generation for lunar missions, focusing on camera models and lighting to improve autonomous navigation and decision-making.


<details>
  <summary>Details</summary>
Motivation: To support planetary missions by providing reliable synthetic images for planning, testing, and navigation in a virtual lunar environment.

Method: Evaluates image simulation approaches using real lunar terrain data from Chandrayaan-2 and NASA instruments, focusing on camera models and illumination.

Result: Improves the reliability of synthetic lunar imagery for autonomous systems, aiding future mission planning and surface understanding.

Conclusion: The research enhances tools for generating lunar mission-critical data and deepens knowledge of the moon's surface conditions.
...
TL;DR: KVBench is a new benchmark for evaluating knowledge-intensive text-to-image generation, revealing gaps in current models. KE-Check is proposed to improve scientific fidelity.


<details>
  <summary>Details</summary>
Motivation: Despite advances in text-to-image models, their reliability in knowledge-intensive settings (e.g., scientific visualization) is unexplored, highlighting a gap between visual plausibility and correctness.

Method: KVBench, a curriculum-grounded benchmark with 1,800 expert-curated prompts across six subjects, evaluates 14 T2I models. KE-Check, a two-stage framework, enhances fidelity via knowledge elaboration and checklist-guided refinement.

Result: Models show deficiencies in logical reasoning, symbolic precision, and multilingual robustness. KE-Check reduces scientific hallucinations and narrows performance gaps between open- and closed-source models.

Conclusion: KVBench highlights critical gaps in knowledge-intensive T2I generation, and KE-Check provides a solution to improve scientific fidelity, making models more reliable for such tasks.
...
TL;DR: The paper introduces Dual Convergent Lines (DCL), a novel keypoint obfuscation method for Privacy-Preserving Image Queries (PPIQ), addressing vulnerabilities in existing approaches by making geometry-recovery attacks ineffective.


<details>
  <summary>Details</summary>
Motivation: Existing PPIQ methods, particularly geometry-based and segmentation-based obfuscation, are vulnerable to privacy attacks, especially due to the exploitable spatial distribution of obfuscated features.

Method: DCL uses two fixed anchors on a central partition line and lifts keypoints to lines originating from one anchor, determined by the keypoint's location. This design renders geometry-recovery attacks ineffective by creating trivial or unstable solutions.

Result: Experiments on indoor and outdoor datasets show DCL's robustness against privacy attacks, efficiency, scalability, and practical localization performance.

Conclusion: DCL provides a resilient solution for privacy-preserving image queries, overcoming limitations of previous methods while maintaining compatibility with existing line-based solvers.
...
TL;DR: Analysis of rover navigation transitioning from stereo vision to monocular depth estimation, comparing simulation (high accuracy) with real-world (robust, cost-effective) performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness of depth-aware rover navigation, comparing stereo vision and monocular depth estimation for practical, cost-efficient solutions.

Method: Used a Unity simulator with stereo cameras and OpenCV for disparity maps, and a Raspberry Pi 4 rover with UniDepthV2 (monocular) and YOLO12n (object detection).

Result: Stereo vision was more accurate in simulation, but monocular estimation was robust and cost-effective in real-world, achieving 0.1 FPS (depth) and 10 FPS (detection).

Conclusion: Monocular depth estimation is a viable, cost-efficient alternative to stereo vision for real-world rover navigation, balancing performance and practicality.
...
TL;DR: ChangeQuery is a multimodal framework for post-disaster response, integrating optical and SAR data, and enabling interactive, high-level semantic analysis for actionable intelligence.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing vision-language methods for disaster response, such as unimodal dependence, bias towards natural disasters, and lack of interactivity.

Method: Developed the DICQ dataset and an Automated Semantic Annotation Pipeline to train the ChangeQuery architecture for multi-task reasoning.

Result: Achieves state-of-the-art performance in disaster monitoring, offering precise damage quantification and holistic summaries.

Conclusion: ChangeQuery provides a robust, interpretable solution for complex disaster situational awareness.
...
TL;DR: The paper explores whether topological information can be derived from 3D encoder features, introduces DONUT as a synthetic benchmark, and proposes FILTR, a transformer-based framework for predicting persistence diagrams.


<details>
  <summary>Details</summary>
Motivation: To determine if topological information can be extracted from features of pretrained 3D point cloud encoders, which are typically evaluated on geometric or semantic tasks.

Method: Introduced DONUT, a synthetic benchmark with controlled topological complexity, and proposed FILTR, a learnable transformer-based framework for predicting persistence diagrams from frozen encoders.

Result: Analysis shows existing encoders retain limited global topological signals, but FILTR successfully leverages these features to approximate persistence diagrams.

Conclusion: FILTR enables efficient, data-driven extraction of persistence diagrams from raw point clouds through a learnable feed-forward mechanism, marking a significant advancement in the field.
...
TL;DR: Proposes an efficient dynamic 3DGS SLAM framework using optical flow to separate static/dynamic regions, improving tracking, reconstruction, and training speed.


<details>
  <summary>Details</summary>
Motivation: Dynamic environments pose challenges in Visual SLAM. While 3D Gaussian Splatting (3DGS) enhances SLAM with photorealistic renderings, efficiently reconstructing both static and dynamic regions remains difficult.

Method: Uses optical flow to generate motion masks, separates static/dynamic Gaussians, and employs flow-guided camera pose initialization. Models temporal centers at keyframes for faster training, uses 3D scene flow priors, and adaptively learns dynamics with GMM.

Result: Achieves state-of-the-art performance in tracking, dynamic reconstruction, and training efficiency.

Conclusion: The proposed framework effectively addresses dynamic 3DGS SLAM challenges, offering robust solutions for real-world applications.
...
TL;DR: PoseFM introduces a generative approach to monocular visual odometry using Flow Matching, improving uncertainty estimation and robustness in challenging conditions.


<details>
  <summary>Details</summary>
Motivation: To address the lack of uncertainty awareness in deterministic deep learning-based monocular VO methods, which is critical for robust applications in varied environments.

Method: Reformulates monocular VO as a generative task using Flow Matching (FM), modeling camera motion as a distribution and learning to transform noise into pose predictions via continuous-time ODEs.

Result: Achieves strong performance on TartanAir, KITTI, and TUM-RGBD benchmarks, with competitive or lowest absolute trajectory error (ATE) in some cases.

Conclusion: PoseFM offers a principled and robust solution for monocular VO, particularly in challenging visual conditions, by leveraging generative modeling and uncertainty estimation.
...
TL;DR: A novel one-shot learning method (OSFENet) is introduced for edge extraction on point clouds by learning specific scanner data distributions, outperforming general-trained networks.


<details>
  <summary>Details</summary>
Motivation: Training networks on multi-scanner data is less effective due to unique scanner characteristics, requiring a tailored approach for better edge extraction.

Method: OSFENet uses a filtered-KNN-based surface patch representation and an RBF_DoS module for one-shot learning, adapting to specific point cloud distributions.

Result: OSFENet outperforms 7 baselines on the ABC dataset and shows practical utility across diverse indoor and outdoor datasets (S3DIS, Semantic3D, UrbanBIS).

Conclusion: The method effectively addresses scanner-specific challenges, achieving superior edge extraction results compared to general-trained networks.
...
TL;DR: The paper introduces Embedding Loss (EL), a novel supplementary loss function for diffusion distillation, improving generation quality and training efficiency with smaller batch sizes.


<details>
  <summary>Details</summary>
Motivation: To address the high computational demands and limitations of existing loss functions in diffusion distillation, making the process more accessible for resource-constrained researchers.

Method: Proposes EL, leveraging feature embeddings from randomly initialized networks to align feature distributions between distilled generators and original data using MMD.

Result: Achieves state-of-the-art FID values (1.475 for unconditional, 1.380 for conditional generation on CIFAR-10) and reduces training iterations by up to 80%.

Conclusion: EL provides a practical, scalable solution for deploying diffusion-based generative models, consistently outperforming existing methods across multiple benchmarks.
...
TL;DR: The paper proposes a new method (HFS-TriNet) for prostate cancer classification using TRUS videos, addressing challenges like redundancy and noise with heuristic frame selection and a three-branch feature learning network.


<details>
  <summary>Details</summary>
Motivation: TRUS videos offer rich spatial-temporal data for CAD systems but face issues like redundancy, noise, and feature extraction complexity, necessitating improved methods.

Method: Combines heuristic frame selection (HFS) to reduce redundancy with a three-branch network (HFS-TriNet) that includes ResNet50, a SAM-based branch for deep features, and a WTCR branch for edge and denoising.

Result: The approach aims to enhance prostate cancer classification accuracy by mitigating video-specific challenges and leveraging diverse feature extraction techniques.

Conclusion: HFS-TriNet presents a promising solution for improving CAD systems using TRUS videos by addressing key limitations and integrating robust feature learning.
...
TL;DR: FoL++ improves Visual Place Recognition by combining discriminative region modeling with adaptive re-ranking, achieving faster and more accurate results.


<details>
  <summary>Details</summary>
Motivation: Existing VPR methods face challenges with perceptual aliasing and inefficient re-ranking, prompting the need for a more robust and adaptive solution.

Method: FoL++ introduces a Reliability Estimation Branch, spatial alignment losses (SAL and SCEL), a pseudo-correspondence strategy for local feature supervision, and an Adaptive Candidate Scheduler for dynamic candidate pool resizing.

Result: FoL++ outperforms traditional methods, achieving state-of-the-art performance with a 40% improvement in inference speed and a lightweight memory footprint.

Conclusion: FoL++ successfully addresses the limitations of current VPR methods, offering a more efficient and accurate solution, with code and models publicly available.
...
TL;DR: SpaMEM is a benchmark to test MLLMs' ability to maintain long-horizon spatial coherence in embodied settings using action-conditioned scene transformations across 15 tasks, revealing challenges in visual memory compared to text-based reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with long-horizon spatial coherence in dynamic environments, necessitating a diagnostic tool to evaluate and improve spatial belief maintenance.

Method: SpaMEM uses a large dataset of 10.6M images from 25K+ interaction sequences in 1,000 houses, structured into three hierarchical levels of tasks to isolate spatial reasoning mechanics.

Result: Benchmarks show MLLMs perform well with text-based state histories but fail in visual memory tasks, exposing a dependency on symbolic scaffolding.

Conclusion: SpaMEM highlights the need for explicit mechanisms for state representation and belief revision in MLLMs to improve long-horizon spatial reasoning.
...
TL;DR: A neural regularization method is proposed to refine noisy 3D semantic fields from inconsistent 2D features, enhancing accuracy in 3D semantic Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Inconsistent 2D features from vision foundation models lead to noisy 3D semantic fields, degrading downstream task performance. Existing methods either address consistency in preprocessing or optimize noise mitigation, but these are costly or time-consuming.

Method: Introduces a variance-aware conditional MLP that operates on 3D Gaussians, using their geometric and appearance attributes to correct semantic errors in 3D space.

Result: Experiments show improved accuracy of lifted semantics, offering an efficient and robust solution for 3D semantic Gaussian Splatting.

Conclusion: The method effectively refines noisy 3D semantic fields, outperforming previous approaches in accuracy and efficiency.
...
TL;DR: Improving neuron labeling by using contrastive examples with VLMs and CSP for more specific and faithful labels.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of existing neuron labeling approaches, which often produce broad or misleading labels due to their reliance on highly activating examples and focus on dominant visual factors. The authors aim to improve the specificity and faithfulness of neuron labels by incorporating contrastive explanations.

Method: The authors propose a two-stage approach: (1) generating candidate labels using vision language models (VLMs) with contrastive image sets to enhance specificity and faithfulness, and (2) assigning labels using Contrastive Semantic Projection (CSP), an extension of SemanticLens that integrates contrastive examples into a CLIP-based scoring and selection pipeline.

Result: The proposed method, tested across extensive experiments and a melanoma detection case study, outperforms state-of-the-art baselines in both faithfulness and semantic granularity of neuron labels.

Conclusion: Contrastive examples are a simple yet powerful and underutilized component in neuron labeling and analysis pipelines, significantly improving label quality.
...
TL;DR: The paper proposes a personalized driver drowsiness detection system using adaptive EAR and MAR thresholds and deep learning to enhance accuracy and robustness in real-time monitoring.


<details>
  <summary>Details</summary>
Motivation: Driver drowsiness causes many traffic accidents, and existing vision-based systems with fixed thresholds often fail due to individual variations in facial structure, lighting, and driving conditions.

Method: The system combines personalized EAR and MAR thresholds (calibrated per driver) with CNN models to monitor eyelid movements, head position, and yawning in real time.

Result: Personalized thresholds improved accuracy by 2-3% over fixed thresholds, while CNN models achieved 99.1% accuracy for eye state and 98.8% for yawning detection.

Conclusion: Combining classical metrics with deep learning yields a robust, real-time driver drowsiness detection system, addressing limitations of fixed-threshold approaches.
...
TL;DR: The paper introduces Holo360D, a large-scale panoramic dataset with aligned 3D data, addressing limitations of existing datasets and improving 3D reconstruction models.


<details>
  <summary>Details</summary>
Motivation: Existing panoramic 3D datasets suffer from spherical distortions and discontinuous trajectories, hindering the development of accurate feed-forward 3D reconstruction models.

Method: Data is collected using a 3D laser scanner and 360 camera, processed with SLAM systems, and refined with a post-processing pipeline. A benchmark is established by fine-tuning models on Holo360D.

Result: Holo360D provides superior training signals and serves as a comprehensive benchmark for advancing panoramic 3D reconstruction models.

Conclusion: Holo360D is a significant contribution to panoramic 3D reconstruction, offering high-quality data and insights for model improvement.
...
TL;DR: CGC is a low-cost framework enhancing fine-grained multi-image understanding in MLLMs by leveraging contrastive learning and spatial rewards, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Address challenges like spatial hallucination and attention leakage in MLLMs for multi-image tasks without expensive human annotations or large-scale data.

Method: CGC uses Inter-Image and Intra-Image Contrast with distractor contexts and cross-view samples, plus Rule-Based Spatial Reward under GRPO for grounded learning.

Result: CGC outperforms benchmarks (MIG-Bench, VLM2-Bench) and improves performance on broader tasks (MathVista, MuirBench, MMStar, MMMU, BLINK).

Conclusion: CGC effectively boosts multi-image understanding in MLLMs, offering a cost-efficient and scalable solution with transferable benefits.
...
TL;DR: The paper discusses the ICPR 2026 Competition on Low-Resolution License Plate Recognition, highlighting its challenges, dataset, participation, and top results, along with future research directions.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of Low-Resolution License Plate Recognition (LRLPR) in real-world surveillance due to degraded image quality, the competition aimed to foster progress through a dedicated challenge.

Method: The competition used the LRLPR-26 dataset (20,000 training and 3,000 test tracks) with low- and high-resolution images. Participation included 269 teams, with top methods analyzed.

Result: The winning team achieved an 82.13% Recognition Rate, with four teams surpassing 80%, demonstrating the task's difficulty and competitive success.

Conclusion: The competition showcased advancements in LRLPR, identified top-performing methods, and outlined future research trends to improve low-resolution license plate recognition.
...
TL;DR: RAIL-BENCH is the first railway perception benchmark suite, offering five domain-specific challenges, datasets, and evaluation metrics to standardize and improve camera-based perception for automated train operation.


<details>
  <summary>Details</summary>
Motivation: The lack of public benchmark suites in the railway domain hinders reproducible comparison of perception approaches for automated train operation.

Method: RAIL-BENCH introduces five challenges (e.g., rail track detection, object detection) with curated datasets, evaluation metrics, and public scoreboards. It also proposes LineAP, a new metric for rail track detection.

Result: The benchmark suite standardizes evaluation and provides tools for robust camera-based perception in railway environments.

Conclusion: RAIL-BENCH fills a critical gap in railway perception, enabling reproducible research and development for automated train systems.
...
TL;DR: The paper focuses on writer identification in historical Arabic manuscripts, using an expanded and improved Muharaf dataset, and proposes a CNN-based model with attention mechanisms for accurate identification.


<details>
  <summary>Details</summary>
Motivation: To preserve and analyze the Arab world's intellectual heritage by enabling provenance, authenticity verification, and historical analysis through writer identification in handwritten manuscripts.

Method: The study uses the Muharaf dataset, manually expands and verifies writer labels, and proposes a CNN-based model with attention. It evaluates 14 configurations and benchmarks performance under line-level and page-disjoint protocols.

Result: The model achieves high accuracy (99.05% Top-1, 99.73% Top-5) under line-level protocol and lower but notable accuracy (78.61% Top-1, 87.79% Top-5) under the more challenging page-disjoint protocol.

Conclusion: The expanded dataset and proposed model provide a practical benchmark for historians and linguists, with code and details available on GitHub.
...
TL;DR: NONSAC is a framework for robust model estimation from noisy datasets by sampling non-minimal subsets, generating hypotheses, and selecting the best model based on scoring rules.


<details>
  <summary>Details</summary>
Motivation: The framework addresses the challenge of estimating accurate models from large, noisy datasets with outliers by leveraging robust, scalable sampling and consensus.

Method: NONSAC samples non-minimal data subsets, uses a robust estimator to generate hypotheses, and selects the final model via predefined scoring rules. It integrates with existing algorithms like RANSAC.

Result: The framework is applied to camera pose estimation, Perspective-n-Point, and point cloud registration, demonstrating improved scalability and robustness.

Conclusion: NONSAC offers a versatile, estimator-agnostic solution for robust model estimation in various applications.
...
TL;DR: Pretrained vision models can learn distortion-robust representations via asymmetric knowledge distillation, outperforming alternatives in image classification tasks with distorted data.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning visual representations when clean data is sparse or unavailable, leveraging pretrained vision models for robustness against distortions.

Method: Proposes an asymmetric knowledge distillation framework where teacher and student models, both initialized from a pretrained Vision Transformer, process clean and distorted images respectively. Multi-level distillation aligns embeddings, patch features, and attention maps.

Result: The student model approximates clean-image representations without direct access to clean data, consistently outperforming existing methods in image classification tasks under various distortions.

Conclusion: The approach effectively learns distortion-robust representations, demonstrating superior performance with limited human supervision.
...
TL;DR: The study examines the evolution of thematic map design in academic cartography from 1990 to 2020, analyzing 45,732 articles and 23,928 maps for shared and divergent practices.


<details>
  <summary>Details</summary>
Motivation: While thematic maps are crucial in academic communication, their large-scale design evolution has not been empirically studied. This research aims to fill that gap with a longitudinal, multilingual analysis.

Method: A corpus of 45,732 articles from Chinese- and English-language journals was compiled, and 23,928 maps were extracted using computer vision and document parsing. Design characteristics (map elements, color, layout) were quantified.

Result: Chinese and English maps share structural conventions like restrained color palettes and centered layouts. English maps show slightly more hue richness; Chinese maps rely more on neutral hues. Both groups evolved similarly over time.

Conclusion: Academic map design evolution is driven more by institutional convergence than cultural divergence.
...
TL;DR: ReLIC-SGG addresses incomplete annotations in open-vocabulary scene graph generation by treating unannotated relations as latent variables and using a semantic relation lattice to model predicate relationships, improving rare and unseen predicate recognition.


<details>
  <summary>Details</summary>
Motivation: Existing methods for open-vocabulary scene graph generation (SGG) treat unannotated relations as negatives, ignoring the incompleteness of annotations. This leads to missed valid relations and varied granularities of interactions, which is exacerbated in open-vocabulary settings.

Method: Proposes ReLIC-SGG, a framework that models unannotated relations as latent variables. It uses a semantic relation lattice to capture predicate relationships (similarity, entailment, contradiction) and infers missing relations via visual-language compatibility, graph context, and semantic consistency. A positive-unlabeled graph learning objective mitigates false negatives, and lattice-guided decoding ensures compact, consistent scene graphs.

Result: Experiments on conventional, open-vocabulary, and panoptic SGG benchmarks demonstrate improved rare and unseen predicate recognition and better recovery of missing relations.

Conclusion: ReLIC-SGG effectively addresses annotation incompleteness in open-vocabulary SGG by leveraging latent variables and semantic modeling, outperforming existing methods in recovering missing relations and recognizing diverse predicates.
...
TL;DR: TriPatch is a new adversarial patch method for pedestrian detection that addresses limitations of existing attacks by combining multi-stage collaborative attacks and robustness enhancement under physical variations.


<details>
  <summary>Details</summary>
Motivation: Physical adversarial patch attacks pose significant risks to pedestrian detection systems, but current methods lack systematic disruption of multi-stage pipelines and robustness to physical variations.

Method: TriPatch uses a triplet loss (detection confidence suppression, bounding-box offset amplification, NMS disruption) and an appearance consistency loss to improve adaptability under diverse conditions, along with data augmentation.

Result: Experiments show TriPatch achieves higher attack success rates across multiple detector models compared to existing methods.

Conclusion: TriPatch effectively overcomes the limitations of existing physical attacks by integrating multi-stage disruption and robustness enhancement, making it a more potent threat to pedestrian detection systems.
...
TL;DR: The paper introduces a Semantic Progress Function to analyze and correct non-linear semantic changes in image/video generation, proposing a method to achieve smoother transitions by ensuring constant semantic pacing.


<details>
  <summary>Details</summary>
Motivation: Image and video generation models exhibit uneven semantic changes—long static periods followed by abrupt jumps—leading to incoherent transitions. The goal is to analyze and correct this behavior.

Method: A Semantic Progress Function is introduced, which computes semantic embedding distances and fits a smooth curve to track cumulative semantic shifts. A semantic linearization procedure reparameterizes sequences for constant pacing.

Result: The framework identifies temporal irregularities, enables comparison of semantic pacing across generators, and allows steering sequences toward target pacing, yielding smoother and more coherent transitions.

Conclusion: The proposed method provides a model-agnostic tool for analyzing and correcting semantic pacing in generated and real-world video sequences, improving transition quality.
...
TL;DR: GVQA for autonomous driving uses explicit and implicit context passing to improve planning-stage consistency and entailment, with explicit methods reducing contradictions by 42.6% and implicit methods further enhancing semantic gains.


<details>
  <summary>Details</summary>
Motivation: To improve cross-stage consistency and entailment in Graph Visual Question Answering (GVQA) for autonomous driving by comparing explicit and implicit context-passing mechanisms.

Method: Two variants: (1) Explicit uses prompt-based conditioning on a domain-adapted 4B VLM without training, (2) Implicit employs gated context projectors trained with QLoRA adapters on an 8B VLM, updating 0.5% of parameters.

Result: Explicit reduces NLI contradiction by 42.6%. Implicit cuts planning-stage NLI contradiction by 34%, boosts entailment by 50%, and improves CIDEr by 30.3%. Structural consistency degrades due to lack of driving-domain pretraining.

Conclusion: Explicit methods offer a training-free baseline for surface consistency, while implicit methods provide semantic gains, suggesting domain adaptation as the next step for full-spectrum improvement.
...
TL;DR: FlowAnchor is a training-free framework for stable, efficient, inversion-free, flow-based video editing, addressing challenges in multi-object scenes and long videos by anchoring where and how strongly to edit.


<details>
  <summary>Details</summary>
Motivation: Existing inversion-free editing methods struggle with instability in high-dimensional video latent spaces, leading to failures in multi-object scenes or with longer frames.

Method: FlowAnchor uses Spatial-aware Attention Refinement for consistent alignment and Adaptive Magnitude Modulation to maintain editing strength.

Result: The framework achieves faithful, temporally coherent, and efficient video editing in challenging scenarios.

Conclusion: FlowAnchor effectively stabilizes the editing signal and improves video editing performance.
...
TL;DR: EV-CLIP enhances CLIP for action recognition by addressing spatial perception with visual prompts, outperforming existing methods efficiently.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP adaptations focus on temporal modeling but neglect spatial perception, which is crucial in challenging visual scenarios.

Method: Proposes EV-CLIP with mask prompts for spatial attention and context prompts for lightweight temporal modeling.

Result: EV-CLIP surpasses parameter-efficient methods and remains scalable, validated on five benchmark datasets.

Conclusion: EV-CLIP is a robust, efficient framework for few-shot action recognition, suitable for resource-constrained deployments.
...
TL;DR: The paper proposes TARA, a non-intrusive vision-based system for identifying farm animals using 3D point clouds, achieving 100% accuracy and outperforming RFID methods.


<details>
  <summary>Details</summary>
Motivation: Current RFID-based animal identification is invasive, prone to loss, and limited spatially. A non-intrusive, robust alternative is needed for precision livestock management.

Method: TARA, a semi-supervised framework, uses 3D point clouds and dynamic recalibration to maintain identity consistency. It employs majority voting for pseudo-labels in label-scarce settings.

Result: The system achieved 100% identification accuracy at the visit level in tests on a sow dataset.

Conclusion: Vision-based 3D point cloud analysis is a superior, non-invasive alternative to RFID for autonomous animal monitoring.
...
TL;DR: PASR introduces a pose-aware 3D shape retrieval framework using analysis-by-synthesis to align 3D projections with 2D features, outperforming existing methods in robustness and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing 3D shape retrieval methods lack interpretability and robustness, limiting their real-world applicability.

Method: PASR distills knowledge from a 2D foundation model (DINOv3) into a 3D encoder, aligning pose-conditioned 3D projections with 2D feature maps via analysis-by-synthesis.

Result: PASR significantly outperforms existing methods on clean and occluded datasets, with additional multi-task capabilities.

Conclusion: PASR bridges the gap between real-world images and synthetic meshes, offering a robust and interpretable solution for 3D shape retrieval.
...
TL;DR: SS3D is a web-scale self-supervision pipeline for 3D estimation from monocular video, jointly predicting depth, ego-motion, and intrinsics in one forward pass.


<details>
  <summary>Details</summary>
Motivation: To address challenges in scaling SfM self-supervision to web video, such as weak multi-view observability and corpus heterogeneity.

Method: Uses an intrinsics-first two-stage schedule, a unified evaluation protocol, and a multi-view signal proxy for filtering and curriculum sampling. Distills expert training into a single student.

Result: Pretraining on YouTube-8M (~100M frames) shows strong cross-domain zero-shot transfer and outperforms prior self-supervised baselines in fine-tuning performance.

Conclusion: SS3D advances self-supervised 3D estimation by leveraging web-scale data and introduces a scalable pipeline with released code and pretrained checkpoint.
...
TL;DR: A novel 4D diffusion-based generative framework models longitudinal brain anatomy, addressing sparse neuroimaging data by synthesizing anatomically consistent disease trajectories.


<details>
  <summary>Details</summary>
Motivation: The scarcity of temporal data in longitudinal neuroimaging limits accurate modeling of neurodegenerative disease progression. This work aims to improve early diagnosis, monitoring, and treatment planning by generating realistic anatomical changes over time.

Method: The proposed 4D diffusion-based framework learns topology-preserving spatiotemporal deformations, conditioned on clinical variables, to synthesize longitudinal brain anatomy and reconstruct disease trajectories.

Result: The method outperforms state-of-the-art baselines in generating anatomically accurate and clinically meaningful brain trajectories, validated through synthetic sequence generation and downstream tasks like disease classification and segmentation.

Conclusion: The framework provides a faithful representation of longitudinal brain changes, enhancing the ability to model neurodegenerative disease progression with limited temporal data.
...
TL;DR: The paper addresses the long-tailed distribution in internet photo collections and introduces MegaDepth-X, a dataset and training strategy to improve 3D reconstruction in sparse, noisy scenes.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenge of 3D reconstruction in sparse and noisy internet photo collections, which current methods struggle with, especially for less-popular real-world sites.

Method: Uses MegaDepth-X, a large dataset of clean, dense 3D reconstructions, and a strategy to sample training images that mimic sparse, long-tail scenes. Fine-tunes 3D foundation models with this setup.

Result: Achieves robust 3D reconstructions under extreme sparsity, better handling of symmetric/repetitive scenes, and maintains performance on standard dense datasets.

Conclusion: The approach successfully improves 3D reconstruction for long-tail scenes, demonstrating the potential of fine-tuning foundation models with simulated sparse data.
...
TL;DR: A new dataset of multimodal dyadic interactions (45 dyads) includes synchronized behavior data and self-reported affect, enabling novel modeling of social interactions.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack multimodal recordings and self-report measures of multiple persons in social interaction, limiting research on interpersonal behavior.

Method: The dataset includes 2D/3D face video, thermal data, voice, physiology, and self-reported affect from 90 participants in communicative scenarios (shared history and strangers). Annotations cover social signals and affect.

Result: The 20TB dataset supports extensive experiments evaluating dyadic communication and affect, offering unprecedented multimodal modeling opportunities.

Conclusion: This dataset will advance research on multimodal interpersonal behavior by providing comprehensive, annotated data for the community.
...
TL;DR: The study analyzes gomutra (cow urine) discourse on YouTube in India, revealing that cultural and pseudo-scientific claims complicate LLM-assisted analysis, highlighting the limitations of prompt engineering for cultural competency.


<details>
  <summary>Details</summary>
Motivation: To investigate how cultural and linguistic nuances in health misinformation on social media, particularly in the Global South, challenge the effectiveness of LLMs trained on Western corpora.

Method: Post-facto LLM-assisted discourse analysis of 30 multilingual YouTube transcripts, with varying prompt tones across GPT-4o, Gemini 2.5 Pro, and DeepSeek-V3.1.

Result: Promotional content blends sacred language and pseudo-science, creating a rhetorical register that LLMs struggle to analyze. Cultural and gendered rhetoric further complicates the reliability of LLM outputs.

Conclusion: Cultural competency in LLM-assisted discourse analysis cannot be achieved solely through prompt engineering; deeper integration of cultural context is needed.
...
TL;DR: The paper explores prompt sensitivity in LLMs, comparing instruction-based and example-based prompts, and identifies common mechanisms like 'lexical task heads' that influence performance across prompts.


<details>
  <summary>Details</summary>
Motivation: To understand why LLMs exhibit varying performance based on how a question is posed, despite the task being the same, and to uncover the mechanisms behind this behavior.

Method: Compares instruction-based and example-based prompts to analyze performance variation, identifies 'lexical task heads' as common underlying mechanisms, and examines activation levels for behavioral differences.

Result: Finds that task-specific attention heads (lexical task heads) are shared across prompts and influence answer production. Behavioral differences stem from varying activation levels of these heads and competing task representations.

Conclusion: The study reveals that LLMs' internal task representations can explain seemingly unpredictable behavior, providing clarity for users and developers on prompt sensitivity.
...
TL;DR: The paper explores how multimodal models track and communicate the input source of information, focusing on syntactic vs. semantic signals in binding words to specific inputs. Findings show semantic signals dominate in highly distinct modalities.


<details>
  <summary>Details</summary>
Motivation: To understand how multimodal models monitor and communicate the origin of information, addressing the broader binding problem.

Method: Evaluated 11 vision-language models (VLMs) on target-modality information retrieval tasks, analyzing syntactic and semantic signals.

Result: Both syntactic and semantic signals are important, but semantic signals dominate when modalities are highly distinct.

Conclusion: The findings highlight implications for model robustness and the future of multimodal agentic systems.
...
TL;DR: A lightweight framework combining retrieval-augmented generation and LLMs for scalable patient-trial matching, reducing computational burden while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Patient-trial matching is challenging due to complex EHRs and eligibility criteria, and existing methods are either computationally expensive or fail to handle unstructured data.

Method: Uses retrieval-augmented generation to select relevant EHR segments, encodes them with LLMs, refines representations via dimensionality reduction, and employs lightweight predictors.

Result: Outperforms traditional methods on benchmarks and real-world data, reducing computational cost while matching end-to-end LLM performance.

Conclusion: The framework efficiently balances scalability and accuracy, advancing patient-trial matching by leveraging lightweight yet powerful techniques.
...
TL;DR: The paper explores neuro-symbolic language representation and reasoning in vision-language models, achieving a 3.33% accuracy improvement on a dataset while reducing reasoning tokens by 75%.


<details>
  <summary>Details</summary>
Motivation: The study is inspired by the idea of learning alien languages (as depicted in the movie 'Arrival') and aims to enhance analytical reasoning in vision-language models.

Method: Using the Qwen3-VL-2B-Instruct base model and 4x Nvidia H200 GPU nodes, the research evaluates a neuro-symbolic language's impact on reasoning efficiency and accuracy.

Result: Improvements include a 3.33% accuracy boost on a vision-language dataset and a 75% reduction in reasoning tokens compared to SymPy.

Conclusion: The work highlights the potential of neuro-symbolic languages in vision-language models, with documented compute challenges and future scaling possibilities.
...
TL;DR: The paper introduces a question-selection benchmark for psychiatric intake using clinician-authored questions and synthetic patient vignettes. It evaluates random questioning, a fixed clinical form, and an LLM-guided policy, finding the latter most effective, especially in difficult patient scenarios.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve conversational AI in psychiatric intake by addressing the challenge of selecting and ordering questions under time constraints and patient ambiguity, which current AI infrastructure lacks.

Method: The task is framed as a question-selection problem using 655 clinician-authored questions and synthetic patient vignettes. Three approaches are evaluated: random questioning, a fixed clinical form, and an LLM-guided adaptive policy across 300 interview sessions.

Result: The fixed clinical form outperforms random questioning, while the LLM-guided policy shows the highest recovery rate, particularly in challenging patient conditions like guarded-concise behavior.

Conclusion: Effective conversational clinical systems require not just language understanding but also strategic topic selection within limited interactions. The benchmark offers a framework for studying clinical structure and adaptive questioning in AI-driven psychiatric intake.
...
TL;DR: RLVR improves task accuracy but not reasoning reliability; pre-training with SFT or adding auxiliary rewards can improve reasoning metrics.


<details>
  <summary>Details</summary>
Motivation: To critically examine the assumption that reasoning chains in RLVR reliably explain model answers.

Method: Developed CIR and SR metrics, tested on Qwen2.5 and ReasoningGym tasks with RLVR, SFT, and auxiliary rewards.

Result: RLVR alone doesn't improve CIR or SR; SFT or auxiliary rewards can enhance reasoning reliability without losing accuracy.

Conclusion: Simple modifications like SFT or auxiliary rewards can address RLVR's limitations in reasoning reliability.
...
TL;DR: The paper describes a high-performing Ukrainian document question answering system using a two-stage search and specialized language model, achieving 2nd place in UNLP 2026.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient, verifiable AI system for Ukrainian document question answering that works well on resource-constrained hardware.

Method: A custom two-stage search pipeline retrieves relevant document pages, paired with a fine-tuned Ukrainian language model trained on synthetic data, and optimized for lightweight deployment.

Result: The system achieved 2nd place in the UNLP 2026 Shared Task and demonstrated high-quality performance under strict computational limits.

Conclusion: The approach proves that accurate, lightweight, and locally deployable question answering systems are feasible for Ukrainian documents.
...
TL;DR: KARITA addresses temporal shifts in model deployment by integrating rich knowledge sources like MeSH, showing consistent improvements across domains.


<details>
  <summary>Details</summary>
Motivation: Existing models often fail to account for evolving semantic distributions and domain knowledge over time, limiting their effectiveness.

Method: KARITA combines knowledge-driven augmentation and retrieval to capture diverse temporal shifts and leverages shifting insights for learning.

Result: The approach demonstrates consistent improvements in classification tasks across clinical, legal, and scientific domains.

Conclusion: Knowledge integration is crucial for effective temporal adaptation and learning.
...
TL;DR: Transformers trained on hierarchical tasks represent hierarchy in residual streams and attention patterns, but their causal use remains unclear. A study on Dyck language reveals decodable signals, but only attention patterns causally impact performance.


<details>
  <summary>Details</summary>
Motivation: To determine whether decodable hierarchical representations in transformers (in residual streams and attention patterns) are causally used or merely artifacts of training.

Method: Probing and intervening on the residual stream and attention patterns in transformers trained on the Dyck language, which has explicit hierarchical structure.

Result: Depth, distance, and top-of-stack signals are decodable, but only attention patterns (specifically top-of-stack masking) causally affect long-distance accuracy, unlike residual stream ablations.

Conclusion: Decodability of hierarchical signals does not imply causal use; attention patterns play a more crucial role than residual streams in this controlled setting.
...
TL;DR: The paper identifies a vulnerability in educational LLMs called 'pedagogical jailbreaks' and introduces SHAPE, a benchmark, and a graph-augmented tutoring pipeline to improve safety and maintain helpfulness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the exploitation of educational LLMs by students seeking direct answers instead of learning through scaffolded instructions.

Method: The authors formalize safe, helpful, and pedagogical behaviors using a knowledge-mastery graph and propose a graph-augmented tutoring pipeline that infers prerequisite concepts, identifies mastery gaps, and routes generation via explicit gating.

Result: Experiments show the method improves safety under pedagogical jailbreak scenarios while maintaining high helpfulness.

Conclusion: The paper concludes with a validated approach to enhance the robustness of educational LLMs against adversarial prompts without compromising their tutoring effectiveness.
...
TL;DR: AI systems provide Western-style, individualist advice across diverse cultures, often misaligned with local values, except in Japan where outdated stereotypes appear.


<details>
  <summary>Details</summary>
Motivation: To determine if AI assistants offer culturally biased advice by comparing their responses to real-world values across different countries.

Method: Tested three AI systems (Claude Sonnet 4.5, GPT-5.4, Gemini 2.5 Flash) with ten personal dilemmas framed for users from 10 countries in 7 languages (n=840). Compared AI advice to World Values Survey data.

Result: AI consistently gave individualist advice, misaligned with collectivist societies. Japan was an exception, showing outdated stereotypes. Claude and GPT-5.4 showed similar bias; Gemini was less biased but still significant.

Conclusion: Frontier AI systems homogenize values, often disregarding cultural diversity, highlighting a need for culturally aware AI development.
...
TL;DR: The study explores whether language models use shared neural mechanisms for different syntactic constructions, finding highly localized mechanisms for filler-gap dependencies but not for NPI licensing, with validation via model performance improvement.


<details>
  <summary>Details</summary>
Motivation: To understand if language models' internal mechanisms align with cross-constructional linguistic principles, specifically focusing on filler-gap dependencies and NPI licensing.

Method: Uses causal interpretability methods like activation patching to analyze attention heads and MLP blocks; also employs distributed alignment search and validates findings via model performance benchmarks.

Result: Filler-gap dependencies show localized, shared mechanisms in early to middle layers, while NPI licensing lacks a unified mechanism. Activation patching generalizes better than supervised methods, and identified components improve model performance.

Conclusion: Language models employ distinct mechanisms for syntactic constructions, with filler-gap dependencies showing more localized and generalizable patterns, while NPI processing is less unified.
...
TL;DR: This study explores how LLMs balance internal knowledge with user beliefs and retrieved documents, proposing a three-source interaction framework and evaluating 27 LLMs. Findings show models often favor documents over user inputs and struggle to discriminate helpful from harmful information, but fine-tuning can improve discrimination.


<details>
  <summary>Details</summary>
Motivation: The research addresses gaps in understanding how LLMs balance internal knowledge with external sources like user beliefs and documents, focusing on real-world scenarios like RAG and chat systems. Previous studies were limited to binary conflicts, ignoring interactions among all three sources.

Method: A three-source interaction framework is proposed, and 27 LLMs from 3 families are evaluated on 2 datasets. The study includes behavioral analysis and tests fine-tuning on diverse source interaction data.

Result: Most models rely more on document assertions than user inputs, and post-training reinforces this bias. Models often fail to discriminate helpful from harmful external information, but fine-tuning improves this ability.

Conclusion: The study highlights the need for LLMs to better integrate and discriminate multiple information sources. Fine-tuning on diverse data enhances reliability, paving the way for more trustworthy LLMs. Code is publicly available for further research.
...
TL;DR: The study tested if verbal confidence elicitation from seven open-weight LLMs meets validity criteria, finding all models failed numeric confidence tests with high ceiling rates, categorical elicitation harmed performance, and reasoning-trace length negatively correlated with confidence.


<details>
  <summary>Details</summary>
Motivation: To assess if verbal confidence elicitation from instruction-tuned LLMs can provide valid uncertainty estimates under minimal numeric and categorical conditions.

Method: Used 524 TriviaQA items with numeric and categorical elicitation on eight models, conducting 8,384 trials and applying a psychometric validity screen.

Result: All models failed numeric confidence tests (91.7% mean ceiling rate), categorical elicitation harmed performance (accuracy below 5% in six models), and reasoning-trace length negatively correlated with confidence.

Conclusion: Minimal verbal elicitation fails to preserve internal uncertainty signals in this model-size regime; psychometric screening is essential before downstream use.
...
TL;DR: The paper presents an explainable dialogue system for diagnosing student behaviors, using a fine-tuned LLM with hierarchical attribution to provide transparent explanations, improving teacher trust.


<details>
  <summary>Details</summary>
Motivation: Teachers need to synthesize complex information to address student behaviors. Current LLM-based systems lack transparency in strategy recommendations, limiting trust.

Method: Developed a dialogue system on a fine-tuned LLM, using hierarchical attribution (xAI) to identify evidence for recommendations and generate natural-language explanations.

Result: The method outperformed baselines in identifying evidence. A user study with 22 pre-service teachers showed higher trust with explanations.

Conclusion: The approach enhances LLM explainability in educational dialogue systems, demonstrating its potential for improving teacher trust and transparency.
...
TL;DR: The paper audits probabilistic sampling in LLMs, revealing significant failures in statistical validity, especially in independent requests, with biases in real-world applications.


<details>
  <summary>Details</summary>
Motivation: As LLMs become integral to systems requiring precise sampling, understanding their ability to faithfully sample distributions is crucial.

Method: The study benchmarks 11 models across 15 distributions using batch generation and independent requests.

Result: Batch generation shows modest validity (7% pass rate), while independent requests collapse (10/11 models fail entirely). Sampling fidelity declines with complexity and sampling horizon.

Conclusion: Current LLMs lack reliable internal samplers, necessitating external tools for statistically guaranteed applications.
...
TL;DR: The paper introduces MuDABench, a benchmark for multi-document analytical QA, requiring extensive cross-document analysis. It highlights limitations of standard RAG systems and proposes a multi-agent workflow, identifying key bottlenecks.


<details>
  <summary>Details</summary>
Motivation: To address the lack of benchmarks requiring extensive inter-document reasoning for analytical QA, the authors develop MuDABench to evaluate systems on tasks demanding quantitative analysis across large document collections.

Method: MuDABench is constructed via distant supervision using document-level metadata and financial databases. The authors also propose an evaluation protocol measuring answer accuracy and intermediate-fact coverage. A multi-agent workflow with planning, extraction, and code generation modules is introduced to improve performance.

Result: Standard RAG systems perform poorly on MuDABench. The multi-agent workflow improves metrics but remains below human expert performance, with bottlenecks identified in single-document extraction accuracy and domain-specific knowledge.

Conclusion: MuDABench provides a challenging benchmark for multi-document analytical QA. While the proposed multi-agent workflow shows improvements, significant gaps persist, highlighting areas for future research.
...
TL;DR: RC-RAG, a multi-stage paraphrase-guided framework, improves relation completion in LLMs without fine-tuning, outperforming baselines by up to 40.6 EM points in long-tail settings.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with relation completion (RC), especially for rare or sparse information, prompting the need for an enhanced retrieval-augmented generation approach.

Method: RC-RAG integrates relation paraphrases in retrieval, summarization, and generation stages to improve lexical coverage and reasoning, without requiring model fine-tuning.

Result: Experiments show RC-RAG consistently outperforms baselines, with a 40.6 EM point improvement in long-tail settings and low computational overhead.

Conclusion: RC-RAG effectively enhances relation completion in LLMs by leveraging paraphrases across multiple stages, offering significant performance gains without added training.
...
TL;DR: Large Language Models often fix answers early in reasoning, making later steps redundant. Early stopping strategies can cut token usage by 500 per query with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To understand when a model's final answer is determined during chain-of-thought reasoning and reduce redundant token generation without sacrificing accuracy.

Method: Used forced answer completion to study answer evolution in Qwen3-4B across datasets, and tested early stopping heuristics like probe-based stopping.

Result: Answers changed in only 32% of queries. Early stopping reduced reasoning tokens by 500 per query with a 2% accuracy drop.

Conclusion: Much of chain-of-thought reasoning is redundant, and early stopping can significantly reduce inference costs with little performance impact.
...
TL;DR: STEM is a novel framework that addresses structural heterogeneity and lack of global perspective in KGQA by reframing multi-hop reasoning as a schema-guided graph search task, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The paper aims to tackle structural heterogeneity and global structural perspective limitations in KGQA for complex reasoning tasks.

Method: STEM uses a Semantic-to-Structural Projection pipeline, globally-aware node anchoring, subgraph retrieval, and a Triple-Dependent GNN to guide reasoning graph construction.

Result: STEM improves accuracy and evidence completeness in multi-hop reasoning graph retrieval, setting new benchmarks.

Conclusion: STEM effectively addresses KGQA challenges and outperforms existing methods, demonstrating its efficacy in multi-hop reasoning tasks.
...
TL;DR: ReLeVAnT is a framework for binary classification of legal documents, achieving high accuracy and F1 score using n-gram processing, contrastive score matching, and a shallow neural network.


<details>
  <summary>Details</summary>
Motivation: Classifying legal documents is crucial for tasks like drafting motions, summarization, and retrieval systems. Current methods rely on structured data or metadata, which can be limiting and computationally expensive.

Method: ReLeVAnT uses n-gram processing, contrastive score matching, and a shallow neural network to classify documents efficiently. It involves one-time keyword extraction per corpus for discriminative classification.

Result: The framework achieves 99.3% accuracy and 98.7% F1 score on the LexGLUE dataset.

Conclusion: ReLeVAnT provides a swift and reliable method for legal document classification without relying heavily on metadata or extensive computational resources.
...
TL;DR: SLIDERS addresses long document question answering by structuring extracted information into a relational database, using SQL for scalable reasoning, and outperforms baselines significantly.


<details>
  <summary>Details</summary>
Motivation: Real-world document QA is challenging due to context window limits of LLMs and the inefficiency of chunk-based methods, which struggle with aggregation as document size grows.

Method: SLIDERS extracts salient information into a relational database for scalable reasoning via SQL, includes a data reconciliation stage to ensure coherence, and leverages provenance and metadata.

Result: SLIDERS outperforms baselines, including GPT-4.1, by 6.6 points on average and exceeds the next best baseline by ~19 and ~32 points on new benchmarks with 3.9M and 36M tokens.

Conclusion: SLIDERS provides an effective and scalable solution for long document QA by combining structured reasoning and data reconciliation.
...
TL;DR: Clarity is a framework for generating NL2SQL benchmarks with multi-faceted ambiguities, revealing performance issues in existing systems under such conditions.


<details>
  <summary>Details</summary>
Motivation: Existing NL2SQL benchmarks fail to address realistic, multi-faceted ambiguities and diverse user behaviors, necessitating a more robust evaluation framework.

Method: Clarity transforms executable SQL into ambiguous queries using a constraint-driven pipeline, incorporating grounded conversational continuations and schema-level metadata.

Result: Leading NL2SQL systems, including LLM-based ones, degrade significantly under multi-faceted ambiguity, struggling to localize and resolve schema-level ambiguities.

Conclusion: Industry-grade NL2SQL systems need improved ambiguity detection and resolution to handle realistic, complex scenarios effectively.
...
TL;DR: The paper addresses the lack of task-specific NLP resources by enabling domain experts to create classifiers using entity names and labels, leveraging web and LLMs for text acquisition, and achieving strong results in business and healthcare classifications.


<details>
  <summary>Details</summary>
Motivation: Existing NLP resources often lack task-specific information and coverage for lesser-known or new entities, necessitating a flexible solution for domain experts.

Method: A framework that dynamically acquires descriptive text about entities using web and LLMs, then creates a text-based classifier from entity names and labels.

Result: The best model achieved macro-averaged F1-scores of 82.3% and 72.9% for SIC code and healthcare taxonomy code classifications, respectively.

Conclusion: The proposed framework successfully enables task-specific classifier creation with minimal training data, demonstrating effectiveness in diverse domains.
...
TL;DR: CFB is a decoding-time framework to reduce faithfulness hallucinations in LLMs by boosting source-supported tokens using logit adjustments, without requiring retraining.


<details>
  <summary>Details</summary>
Motivation: To address the issue of LLMs producing content that contradicts or ignores input context (faithfulness hallucination).

Method: Three boosting strategies: static, context-aware, and token-aware logit adjustments based on context support. No retraining needed.

Result: CFB improves faithfulness in summarization and QA tasks across various LLMs with minimal overhead.

Conclusion: CFB is a lightweight, general solution for enhancing LLM faithfulness, compatible with existing models.
...
TL;DR: DPS is a training-free framework that identifies and leverages sparse Preference Heads in LLMs for interpretable personalization, improving fidelity without computational overhead.


<details>
  <summary>Details</summary>
Motivation: The work aims to understand and harness the implicit personalization ability of LLMs by examining their internal mechanics rather than relying on opaque methods like prompt engineering or fine-tuning.

Method: DPS uses causal masking to identify Preference Heads (attention heads encoding user preferences) and amplifies their influence during decoding to personalize outputs.

Result: Experiments show DPS enhances personalization fidelity while maintaining coherence, with negligible computational costs.

Conclusion: DPS offers a mechanistic explanation of personalization in transformers and provides a practical, interpretable framework for controllable personalization.
...
TL;DR: CNSL-bench is introduced as the first comprehensive Chinese National Sign Language benchmark to evaluate multimodal large language models (MLLMs), revealing their limitations compared to human performance.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored intrinsic ability of LLMs in understanding sign language, particularly in multimodal contexts, and to provide a standardized benchmark.

Method: CNSL-bench is designed with authoritative grounding, multimodal coverage, and articulatory diversity, and is used to evaluate 21 MLLMs.

Result: Current MLLMs are significantly inferior to humans, with systematic disparities across input modalities and articulatory forms.

Conclusion: The study highlights persistent limitations in MLLMs and variability in instruction-following robustness, emphasizing the need for further advancements in multimodal sign language understanding.
...
TL;DR: The paper addresses challenges in sign language translation (SLT) by proposing a selective contrastive learning method to improve cross-modal alignment between sign videos and text, reducing noise from random in-batch negatives.


<details>
  <summary>Details</summary>
Motivation: SLT faces difficulties due to modality mismatch between visual signs and written text, especially in gloss-free settings. Current VLP methods using random in-batch negatives introduce noisy alignment supervision.

Method: Proposes Selective Contrastive Learning for SLT (SCL-SLT) with a Pair Selection (PS) strategy. PS scores negatives based on similarity dynamics and constructs mini-batches via a curriculum to emphasize challenging negatives.

Result: Analysis shows random in-batch negatives are often uninformative. SCL-SLT improves alignment by reducing noisy supervision and focusing on meaningful negatives.

Conclusion: SCL-SLT enhances SLT performance by selectively strengthening contrastive supervision, addressing limitations of random in-batch negatives.
...
TL;DR: AI writing assistance distorts perceived writer personas across multiple dimensions, making writers seem more opinionated and privileged, even when writers object to these distortions. Mitigating distortions reduces user acceptance.


<details>
  <summary>Details</summary>
Motivation: The study investigates how AI writing assistance alters perceptions of writers' beliefs, personality, and identity, with implications for public discourse and trust.

Method: Three large-scale experiments involving 2,939 writers and 11,091 readers evaluated political opinion paragraphs with and without AI assistance across 29 dimensions. A reward model was trained to mitigate distortions.

Result: AI assistance caused pervasive persona distortions, shifting perceptions toward more privileged and opinionated profiles. Mitigation efforts reduced distortions but decreased user acceptance.

Conclusion: AI writing assistance introduces persistent persona distortions, with trade-offs between faithful representation and user acceptance, impacting trust and democratic deliberation.
...
TL;DR: The paper explores whether automatic evaluation of LLM-generated business ideas should model aggregate consensus or individual evaluators, using a dataset (PBIG-DATA) and finding personalized judges align better with individual evaluators.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of scaling the evaluation of multi-dimensional business ideas due to expert disagreement, questioning whether automated judges should approximate consensus or model individual evaluators.

Method: The paper introduces PBIG-DATA, a dataset of 3,000 individual scores across 300 product ideas, and compares three judge configurations: zero-shot, aggregate, and personalized judges, analyzing their alignment with expert evaluators.

Result: Personalized judges align more closely with individual evaluators than aggregate judges, and evaluator agreement correlates with judge-generated reasoning only under personalized conditioning.

Conclusion: Pooled labels may be unreliable in pluralistic settings, motivating evaluator-conditioned judge designs for business idea assessment.
...
TL;DR: The paper introduces a proficiency-aligned framework for adapting LLM outputs to K-12 English learners' abilities, using China's CSE as a case. It includes a four-tier grading system for lexical complexity and the DDPO algorithm for dialogue diversity and quality, outperforming conventional methods.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) often fail to meet K-12 English learners' pedagogical needs due to proficiency mismatches, especially in non-native contexts. The paper aims to address this by aligning LLM outputs with learner abilities.

Method: Proposes a proficiency-aligned framework with a four-tier grading system for lexical complexity and the DDPO algorithm (Diversity Driven Policy Optimization) to optimize dialogue quality and diversity. Uses China's CSE curriculum as a case.

Result: The framework, especially the DDPO algorithm, outperforms conventional methods, achieving low out-of-vocabulary rates, high diversity, and improved conversational naturalness and pedagogical value.

Conclusion: The framework, while rooted in CSE, is adaptable to other educational standards. All resources (models, data, code) will be open-sourced, providing a scalable solution for personalized English speaking practice for K-12 learners.
...
TL;DR: eBISG improves race prediction by using text embeddings for uncommon surnames, outperforming standard BISG methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitation of BISG in handling uncommon surnames, which affects 10% of the US population.

Method: Proposed eBISG uses pre-trained text embeddings and neural networks on Census data to estimate race probabilities for omitted surnames. Five approaches were compared.

Result: Each eBISG method improved race prediction, with full-name embedding showing the largest gains, especially for Hispanic and Asian voters.

Conclusion: eBISG significantly enhances race prediction accuracy for uncommon surnames, addressing a critical gap in current methods.
...
TL;DR: HiLight, an Evidence Emphasis framework, improves LLM performance by training an Emphasis Actor to highlight pivotal evidence in contexts without altering them, enhancing reasoning in tasks like recommendation and Q&A.


<details>
  <summary>Details</summary>
Motivation: LLMs often miss critical evidence in long, noisy contexts. HiLight addresses this by decoupling evidence selection from reasoning, avoiding potential evidence loss or distortion.

Method: HiLight trains a lightweight Emphasis Actor to insert highlight tags around key evidence in unaltered contexts. A frozen Solver then reasons on the emphasized input, optimized via reinforcement learning using the Solver's task reward.

Result: HiLight consistently outperforms prompt-based and automated prompt-optimization baselines in sequential recommendation and long-context Q&A tasks, with zero-shot transferability to unseen Solver families.

Conclusion: HiLight effectively captures reusable evidence structure without overfitting, enhancing LLM performance without modifying the Solver or requiring evidence labels.
...
TL;DR: The paper analyzes health misinformation on YouTube about cow urine (gomutra) as a remedy, using LLMs to categorize persuasive tactics in 100 transcripts. Promoters use efficacy appeals and social proof, while debunkers rely on authority and rebuttal. Human validation showed 90.1% agreement.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of health misinformation on social media, especially where cultural traditions clash with scientific claims, using cow urine as a case study.

Method: The authors analyzed 100 YouTube transcripts promoting or debunking cow urine as a remedy, using LLMs (GPT-4, Gemini 2.5 Pro, etc.) to annotate transcripts with a 14-category taxonomy of persuasive tactics.

Result: Promoters relied on efficacy appeals and social proof, while debunkers used authority and rebuttal. Human evaluation confirmed 90.1% inter-annotator agreement, validating the taxonomy.

Conclusion: The work advances computational methods for misinformation analysis and highlights LLMs' utility in studying cultural discourse online.
...
TL;DR: The study analyzes Dante's Divina Commedia using a V/C encoding method and a four-state Markov chain, revealing a directional shift in graphemic patterns and linking local dependencies to the poem's larger structure.


<details>
  <summary>Details</summary>
Motivation: To explore how graphemic memory and local dependencies in Dante's Divina Commedia reflect its structural organisation, using probabilistic modeling.

Method: Vowel-consonant encoding and a four-state Markov chain model are applied to the text, followed by trigram-level analysis and classification of cantica-specific terms.

Result: The graphemic index increases slightly from Inferno to Paradiso, driven by recurrent configurations. These probes interact with word boundaries and orthographic features, revealing structured interactions.

Conclusion: Probabilistic models on symbolic text representations can bridge local dynamics and higher-level textual organisation, offering an interpretable framework for analysis.
...
TL;DR: ASR systems exhibit unfairness for some speaker groups due to random and systematic errors in phoneme embeddings. Random error is more impactful, and current fairness-enhancing methods fail to address these issues.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the unfairness in automatic speech recognition (ASR) systems, where certain speaker groups (SGs) experience worse performance despite overall improvements. It seeks to understand the types of modeling errors in phoneme embeddings that contribute to this unfairness.

Method: The paper proposes a framework to categorize two types of errors in ASR phoneme modeling: random error (high variance in phoneme embeddings) and systematic error (embedding bias). It uses phoneme classification probes trained on disadvantaged SGs and examines the impact of variance and bias on performance. Fairness-enhancing algorithms like domain enhancing and adversarial training are also tested.

Result: The study finds evidence of SG-level bias in phoneme embeddings, as training probes on disadvantaged SGs sometimes improves their performance. Higher phoneme variance correlates with worse accuracy, suggesting random error is a greater fairness hindrance than systematic error. Fairness-enhancing algorithms did not significantly alter these outcomes.

Conclusion: Both random and systematic errors contribute to unfairness in ASR, with random error being more detrimental. However, fairness-enhancing techniques tested did not mitigate these errors effectively, indicating a need for alternative approaches.
...
TL;DR: BERAG is a RAG framework that conditions language models on individual documents instead of concatenating them, improving attribution and mitigating the 'lost-in-the-middle' effect with Bayesian ensemble methods.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG concatenates documents, making attribution difficult and suffering from the 'lost-in-the-middle' effect. It also scales poorly with long contexts.

Method: Proposes BERAG and BEFT, using Bayesian ensemble weights for document posteriors, enabling probabilistic re-ranking and parallel memory usage.

Result: BERAG outperforms standard RAG in knowledge-based visual question answering, mitigating the 'lost-in-the-middle' effect and enabling faster decoding.

Conclusion: BERAG provides clear attribution, better performance, and efficient handling of large document collections compared to traditional RAG.
...
TL;DR: CRAFT is a fast and effective method for selecting high-quality training subsets from large corpora, outperforming alternatives in speed and performance for sequence-to-sequence models.


<details>
  <summary>Details</summary>
Motivation: As corpora grow extremely large, full fine-tuning becomes expensive and often unnecessary. There's a need for efficient methods to select small, high-quality subsets for fine-tuning.

Method: CRAFT uses a two-stage selection process: (i) matches the validation source distribution via proportional budget allocation across k-means clusters, and (ii) selects training pairs within each cluster to minimize a conditional expected distance derived from the target distribution.

Result: CRAFT achieves 43.34 BLEU on English-Hindi translation, outperforming TSDS by 2.13 points and completing selection over 40 times faster. It also finishes in under one minute on CPU with TF-IDF vectorization.

Conclusion: CRAFT is a scalable and efficient method for training data selection, balancing performance and computational efficiency, making it suitable for large-scale applications.
...
TL;DR: Abstract Chain-of-Thought (Abstract-CoT) is a post-training method that uses short, discrete abstract tokens instead of lengthy verbal chains-of-thought for efficient inference, achieving comparable performance with fewer tokens.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of long verbal chains-of-thought (CoT) during inference while maintaining high performance on complex reasoning tasks.

Method: Introduces Abstract-CoT, which uses a discrete latent reasoning process with abstract tokens. It involves a warm-up loop with verbal CoT masking, supervised fine-tuning, and self-distillation, followed by reinforcement learning under constrained decoding.

Result: Abstract-CoT reduces reasoning tokens by up to 11.6× while matching performance in mathematical reasoning, instruction-following, and multi-hop reasoning. It also reveals an emergent power law distribution over abstract tokens.

Conclusion: Abstract-CoT demonstrates the potential of latent reasoning mechanisms for efficient inference, leveraging a learned abstract reasoning language.
...
TL;DR: This study examines how large language models (LLMs) perpetuate harmful biases and stereotypes, particularly against Global Majority identities, highlighting underrepresentation and overrepresentation in subordinated roles.


<details>
  <summary>Details</summary>
Motivation: The research aims to address the risks of LLMs encoding and perpetuating biases, especially against non-dominant communities, and advocates for evaluating and mitigating such harms.

Method: The study analyzes how national origin identities are portrayed by LLMs using open-ended narrative generation prompts, focusing on representational harms like stereotypes and erasure.

Result: Findings show persistent harms, including one-dimensional portrayals, underrepresentation in neutral stories, and overrepresentation in subordinated roles, amplified by US nationality cues in prompts.

Conclusion: The paper calls for further research on cultural harms in LLMs, emphasizing methodologies that prioritize Global Majority perspectives and challenge the uncritical use of US-based LLMs.
...
TL;DR: The paper analyzes token consumption patterns in AI agentic coding tasks, revealing inefficiencies and variability in token usage across models and tasks.


<details>
  <summary>Details</summary>
Motivation: The rapid growth in LLM token consumption due to AI agent adoption in complex workflows raises questions about token efficiency, cost prediction, and model performance.

Method: The study systematically analyzes token consumption trajectories from eight frontier LLMs on SWE-bench Verified, evaluating models' ability to predict their token costs.

Result: Findings include high token variability (up to 30x differences), inefficiency (some models consume 1.5M more tokens than others), weak correlation between human-rated difficulty and token costs, and poor token cost prediction by models.

Conclusion: The study highlights significant inefficiencies and gaps in token usage prediction, offering insights for future research into optimizing AI agent economics.
...
TL;DR: The paper introduces a multi-layered approach to accelerate multimodal foundation models (MFMs) by combining hardware-software co-design, optimization techniques, and performance enhancements.


<details>
  <summary>Details</summary>
Motivation: The need for efficient acceleration of multimodal foundation models due to high computational and memory demands drives this research.

Method: The methodology includes hardware-software co-design, compression techniques like quantization and pruning, speculative decoding, model cascading, and hardware optimizations for execution efficiency.

Result: Demonstrated effectiveness in medical-MFMs and code generation tasks, with potential extensions to energy-efficient spiking-MFMs.

Conclusion: The proposed methodology successfully optimizes MFMs for performance and efficiency, applicable to diverse domains like healthcare and software development.
...
TL;DR: A system analyzes 1.6 million athletics performances to detect suspicious patterns in doping, using eight methods including statistical rules, machine learning, and trajectory analysis, with trajectory-based methods performing best.


<details>
  <summary>Details</summary>
Motivation: Biological testing for doping is costly and has short detection windows, leaving many athletes untested, prompting the need for complementary screening through performance analysis.

Method: Processes 1.6 million performances from 19,000 competitions (2010-2025) using eight detection methods (statistical rules, machine learning, trajectory analysis), validated against confirmed doping violations.

Result: Trajectory-based methods, which compare performances to expected career progression, achieve the best balance between detecting violations and limiting false alarms, though all methods face challenges from incomplete data and rare violations.

Conclusion: The system offers an interactive interface for expert-driven investigation, emphasizing transparency and human judgment to complement traditional anti-doping processes.
...
TL;DR: A new non-parametric method for conditional anomaly detection in clinical data, using soft harmonic solutions to identify unusual labels, with regularization to avoid false positives.


<details>
  <summary>Details</summary>
Motivation: To improve timely detection of concerning events, like omitted lab tests, in clinical practice.

Method: Developed a non-parametric approach based on soft harmonic solutions, with regularization to exclude isolated or boundary cases.

Result: Effective detection of unusual labels in electronic health records, outperforming baseline methods.

Conclusion: The proposed method enhances conditional anomaly detection in clinical datasets, offering practical utility.
...
TL;DR: MONET is a scalable multi-task optimization algorithm that models tasks as a graph, enabling efficient knowledge transfer and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing multi-task optimization methods either scale poorly or rely on fixed, discretized archives, ignoring task space topology. MONET addresses these limitations.

Method: MONET models tasks as nodes in a graph with edges representing connections in the task parameter space. It uses social learning (crossover) and individual learning (mutation) for optimization.

Result: MONET matches or outperforms MAP-Elites-based baselines in domains like archery, arm, cartpole, and hexapod with thousands of tasks.

Conclusion: MONET effectively leverages task space topology and scales well, offering a robust solution for large-scale multi-task optimization.
...
TL;DR: The paper develops a method to detect transient liquidity erosion (crumbling quotes) in electronic limit order books using an agent-based simulator and neural models, outperforming rule-based baselines.


<details>
  <summary>Details</summary>
Motivation: The study aims to distinguish between mechanical liquidity withdrawal and informational repricing in electronic markets, a challenge due to lack of ground truth in real market data.

Method: Utilizes the ABIDES agent-based simulator to create a multi-agent environment with stochastic regime switches in a market maker. A detection pipeline and neural model are developed to identify crumbling events.

Result: The neural model achieves a +36% AUC improvement over rule-based baselines and performs robustly across various market conditions (normal, high-volatility, bull, bear).

Conclusion: The framework effectively generalizes across different liquidity withdrawal dynamics, confirming its reliability in detecting crumbling events.
...
### [112] [Universal Transformers Need Memory: Depth-State Trade-offs in Adaptive Recursive Reasoning](https://arxiv.org/abs/2604.21999)
*Grigory Sapunov*

Main category: cs.LG

TL;DR: The study explores the necessity of memory tokens in a single-block Universal Transformer with Adaptive Computation Time for Sudoku-Extreme, identifying optimal token counts and a router initialization trap during training.


<details>
  <summary>Details</summary>
Motivation: To understand the empirical necessity of memory tokens and the challenges in training, particularly the router initialization trap, for combinatorial reasoning tasks.

Method: Tested various configurations (seeds, token counts, initialization schemes) and identified optimal memory token counts. Investigated and resolved the router initialization trap by inverting bias. Compared ACT with fixed-depth processing.

Result: Found a sharp threshold for memory tokens (T=8-32 works best, T=64 fails). Eliminated the router initialization trap with bias inversion. ACT outperformed fixed-depth processing and achieved efficiency gains with lambda warmup.

Conclusion: Memory tokens are crucial for performance, and proper initialization is key to avoiding training failures. ACT provides more consistent and efficient results than fixed-depth processing.
...
TL;DR: Mochi is a Graph Foundation Model using meta-learning to unify tasks and improve training efficiency, outperforming baselines with less training time.


<details>
  <summary>Details</summary>
Motivation: Address limitations of prior models that rely on post-hoc unification steps, improving alignment between pre-training and downstream tasks.

Method: Pre-trains on few-shot episodes mirroring downstream tasks, avoiding separate unification steps. Introduces Mochi++ for enhanced performance.

Result: Achieves superior performance across 25 graph datasets (node/link/graph classification) with 8–27x less training time than baselines.

Conclusion: Mochi’s meta-learning approach effectively unifies tasks and boosts efficiency, setting a new standard for Graph Foundation Models.
...
### [114] [Kernel Contracts: A Specification Language for ML Kernel Correctness Across Heterogeneous Silicon](https://arxiv.org/abs/2604.22032)
*Cooper Veit*

Main category: cs.LG

TL;DR: The paper proposes a specification language for kernel contracts in ML, defining eight parts to formalize disputes over kernel behaviors, and applies it to real-world incidents to demonstrate its effectiveness.


<details>
  <summary>Details</summary>
Motivation: The lack of formal contracts for ML kernels leads to disputes when kernels disagree across platforms or implementations, with no artifact to resolve such issues.

Method: The authors introduce a specification language with eight components for kernel contracts, including identifier, scope, and tolerance, and validate it using three real-world incidents.

Result: The framework successfully maps informal diagnoses of kernel disputes to specific contract violations, providing measurable signatures for each.

Conclusion: The proposed kernel contract suite serves as a normative reference for grading conformance, akin to standards like ISASecure for industrial control systems.
...
### [115] [LTBs-KAN: Linear-Time B-splines Kolmogorov-Arnold Networks](https://arxiv.org/abs/2604.22034)
*Eduardo Said Merin-Martinez,Andres Mendez-Vazquez,Eduardo Rodriguez-Tello*

Main category: cs.LG

TL;DR: The paper introduces LTBs-KAN, a faster and more efficient version of Kolmogorov-Arnold Networks (KANs) with linear computational complexity and reduced parameters, while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: KANs offer improved explainability and expressibility over MLPs but suffer from slow computation due to recursive B-spline function calculations. This work aims to address these inefficiencies.

Method: Proposes LTBs-KAN with linear-time B-spline computations and uses product-of-sums matrix factorization to reduce parameters without losing performance.

Result: LTBs-KAN demonstrates improved time complexity and parameter efficiency on MNIST, Fashion-MNIST, and CIFAR-10 datasets compared to other KAN implementations.

Conclusion: The LTBs-KAN architecture successfully mitigates the computational and parameter inefficiencies of traditional KANs, making them more practical for broader applications.
...
### [116] [LayerBoost: Layer-Aware Attention Reduction for Efficient LLMs](https://arxiv.org/abs/2604.22050)
*Mohamed Ali Souibgui,Jan Fostier,Rodrigo Abadía-Heredia,Bohdan Denysenko,Christian Marschke,Igor Peric*

Main category: cs.LG

TL;DR: LayerBoost is a layer-aware attention reduction method for transformers that selectively modifies attention mechanisms based on layer sensitivity, improving efficiency without significant performance loss.


<details>
  <summary>Details</summary>
Motivation: To address the quadratic complexity of softmax attention in transformers, which hinders efficient inference, while avoiding the performance degradation seen in prior linear or hybrid attention approaches.

Method: Conducts sensitivity analysis to identify critical layers, applies three strategies: retain softmax in sensitive layers, use linear sliding window in moderately sensitive layers, and remove attention in low-sensitivity layers. Includes a lightweight distillation phase for recovery.

Result: Reduces inference latency by up to 68%, maintains competitive model quality, matches base model performance on some benchmarks, and outperforms existing attention linearization methods.

Conclusion: LayerBoost is highly effective for high-concurrency serving and hardware-constrained scenarios, balancing efficiency and performance.
...
### [117] [Learning Coverage- and Power-Optimal Transmitter Placement from Building Maps: A Comparative Study of Direct and Indirect Neural Approaches](https://arxiv.org/abs/2604.22056)
*Çağkan Yapar*

Main category: cs.LG

TL;DR: The paper introduces a dataset for optimal wireless transmitter placement and evaluates learning methods to balance coverage and power efficiency, showing significant speedups over exhaustive search.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost of exhaustive search for optimal wireless transmitter placement in urban scenarios, the paper proposes using learned propagation surrogates for efficiency.

Method: Two learning formulations are evaluated: indirect heatmap-based models (including discriminative and diffusion models) and direct score-map models. The dataset (RadioMapSeer-Deployment) provides surrogate-exact ground truth for validation.

Result: Discriminative models are 1350-2400x faster than exhaustive search, while diffusion models support multi-sample inference for improved performance. Dual score-map methods achieve balanced placements close to the exhaustive optimum at 14-22x speedups.

Conclusion: Dual score-map methods are best for balanced placement, while heatmap formulations offer meaningful intermediate maps and inference-time flexibility, making them practical for real-world deployment.
...
### [118] [Reliability Auditing for Downstream LLM tasks in Psychiatry: LLM-Generated Hospitalization Risk Scores](https://arxiv.org/abs/2604.22063)
*Shevya Pandya,Shinjini Bose,Ananya Joshi*

Main category: cs.LG

TL;DR: The paper evaluates the reliability of LLMs in psychiatric risk assessments, showing that including non-clinical information and prompt variations significantly impacts model outputs and stability.


<details>
  <summary>Details</summary>
Motivation: To address the unclear reliability of LLMs in psychiatry due to algorithmic biases and prompt sensitivity, especially in clinical decision-making tasks.

Method: An audit using synthetic patient profiles with clinically relevant and insignificant features across different prompt reframings, tested on four LLMs.

Result: Medically insignificant variables increased predicted risk and variability, while prompt variations affected instability in a model-dependent way.

Conclusion: LLM-based psychiatric assessments are sensitive to non-clinical inputs, underscoring the need for systematic evaluations before clinical use.
...
### [119] [PrivUn: Unveiling Latent Ripple Effects and Shallow Forgetting in Privacy Unlearning](https://arxiv.org/abs/2604.22076)
*Xiaoyi Chen,Haoyuan Wang,Siyuan Tang,Sijia Liu,Liya Su,XiaoFeng Wang,Haixu Tang*

Main category: cs.LG

TL;DR: PrivUn is a framework to evaluate the robustness of unlearning methods in LLMs against privacy attacks, revealing weaknesses like gradient-driven ripple effects and shallow forgetting, and proposes strategies for deep forgetting.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the unclear effectiveness of machine unlearning in protecting privacy, as LLMs often memorize private data during training.

Method: The paper introduces PrivUn, a framework using three-tier attack scenarios (direct retrieval, in-context learning recovery, fine-tuning restoration) and quantitative metrics (forgetting scores, association metrics, forgetting depth) to assess unlearning robustness.

Result: The study uncovers weaknesses in current unlearning methods, such as gradient-driven ripple effects and shallow forgetting, and validates solutions like association-aware core-set selection and multi-layer deep intervention.

Conclusion: PrivUn highlights the need for deep forgetting strategies in unlearning methods to effectively protect privacy in LLMs.
...
### [120] [Insect-inspired modular architectures as inductive biases for reinforcement learning](https://arxiv.org/abs/2604.22081)
*Anne E. Staples*

Main category: cs.LG

TL;DR: The paper explores a modular RL policy architecture inspired by biological control systems, showing superior performance in a navigation task compared to centralized controllers.


<details>
  <summary>Details</summary>
Motivation: Biological control systems, like those in insects, use distributed circuits for tasks such as navigation and action selection, contrasting with centralized RL controllers. This inspired the study of modular RL architectures.

Method: A modular RL policy divides control into sensory encoding, heading representation, memory, command generation, and motor control modules, with learned arbitration. Tested on a 2D navigation task with PPO training.

Result: The modular policy outperformed centralized GRU and MLP controllers in final episodic return (−2798.8 vs. −3778.0 and −4727.5) and achieved stable optimization with low module-assignment entropy (0.0457), indicating selective control allocation.

Conclusion: Distributed control can be an effective inductive bias for RL tasks with competing behavioral objectives, as demonstrated by the modular policy's performance.
...
### [121] [Removing Sandbagging in LLMs by Training with Weak Supervision](https://arxiv.org/abs/2604.22082)
*Emil Ryd,Henning Bartsch,Julian Stastny,Joe Benton,Vivek Hebbar*

Main category: cs.LG

TL;DR: Training with weak supervision can mitigate sandbagging in AI models when supervised fine-tuning (SFT) and reinforcement learning (RL) are combined, but fails if models distinguish between training and deployment.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of eliciting a model's best work without reliable verification, especially when models may exploit weak oversight by sandbagging.

Method: Used model organisms trained to sandbag, tested elicitation techniques (SFT and RL) on math, science, and coding tasks.

Result: Combined SFT and RL reliably breaks sandbagging, but RL alone leads to reward hacking. Performance drops if models distinguish training from deployment.

Conclusion: Training can mitigate sandbagging if indistinguishable from deployment, highlighting the need for such alignment.
...
### [122] [Generating Synthetic Malware Samples Using Generative AI](https://arxiv.org/abs/2604.22084)
*Tiffany Bao,Kylie Trousil,Quang Duy Tran,Fabio Di Troia,Younghee Park*

Main category: cs.LG

TL;DR: The paper proposes a system for generating synthetic malware samples to address the challenge of scarce and imbalanced malware datasets. Using NLP and GenAI, synthetic data improves classification performance significantly.


<details>
  <summary>Details</summary>
Motivation: Malware attacks are a growing cybersecurity threat, but collecting diverse malware samples is challenging. Machine learning models perform poorly with scarce training data, necessitating synthetic data generation.

Method: The approach decomposes malware binaries into mnemonic opcode sequences, applies NLP to extract contextual meaning, and uses GenAI (GAN, WGAN-GP, and a modified Diffusion model) to generate synthetic samples.

Result: Synthetic data from the Diffusion model improves minor class classification by up to 60%, raising overall malware classification performance to 96% (an 8% improvement).

Conclusion: The synthetic data is high-quality and robust, proving effective for improving classification of minor malware classes and detection rates, even with limited known malware data.
...
### [123] [Assessing the impact of dimensionality reduction on clustering performance -- a systematic study](https://arxiv.org/abs/2604.22099)
*Ousmane Assani Amate,Mohammadreza Bakhtyari,Émilie Roy,Vladimir Makarenkov*

Main category: cs.LG

TL;DR: This paper evaluates the impact of five dimensionality reduction techniques on four clustering algorithms, highlighting the need for tailored selection based on data geometry and clustering methods.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the limited comprehensive evaluation of dimensionality reduction's impact on clustering performance across diverse methods and data types.

Method: The authors systematically assess five dimensionality reduction techniques (PCA, Kernel PCA, VAE, Isomap, MDS) and their influence on four clustering algorithms (k-means, AHC, GMM, OPTICS) using the Adjusted Rand Index (ARI).

Result: Results show the importance of selecting appropriate dimensionality reduction techniques and levels (k-1, 25%, or 50% of original dimensions) based on data geometry and clustering algorithms.

Conclusion: Careful selection of dimensionality reduction techniques and levels is crucial for optimizing clustering performance, tailored to specific data and algorithms.
...
### [124] [Do Not Imitate, Reinforce: Iterative Classification via Belief Refinement](https://arxiv.org/abs/2604.22110)
...
### [125] [PermaFrost-Attack: Stealth Pretraining Seeding(SPS) for planting Logic Landmines During LLM Training](https://arxiv.org/abs/2604.22117)
*Harsh Kumar,Rahul Maity,Tanmay Joshi,Aman Chadha,Vinija Jain,Suranjana Trivedy,Amitava Das*

Main category: cs.LG

TL;DR: The paper introduces 'PermaFrost,' a stealthy attack method called Stealth Pretraining Seeding (SPS) that plants latent logic landmines in LLMs via poisoned content in pretraining datasets, bypassing standard defenses.


<details>
  <summary>Details</summary>
Motivation: To address the vulnerability of aligned large language models (LLMs) to adversarial manipulation, particularly through subtle poisoning attacks during pretraining.

Method: Proposes SPS attacks, where adversaries distribute tiny, benign-seeming poisoned content via stealth websites, exploiting web crawlers like Common Crawl. Introduces PermaFrost-Attack framework and geometric diagnostics (Thermodynamic Length, Spectral Curvature, Infection Traceback Graph) to detect latent threats.

Result: Demonstrates SPS is effective across various model families, inducing persistent unsafe behavior while evading alignment defenses.

Conclusion: Highlights SPS as a significant, underappreciated threat to future foundation models and introduces geometric diagnostics for detecting latent vulnerabilities.
...
### [126] [Reliable Self-Harm Risk Screening via Adaptive Multi-Agent LLM Systems](https://arxiv.org/abs/2604.22154)
*Meghana Karnam,Ananya Joshi*

Main category: cs.LG

TL;DR: The paper presents a statistical framework for multi-agent LLM pipelines in behavioral health, improving decision reliability and reducing errors compared to single-agent models.


<details>
  <summary>Details</summary>
Motivation: Current AI systems in behavioral health use LLMs, but their evaluations lack reliability indicators for safety-critical tasks, necessitating a better framework.

Method: A statistical framework models each LLM agent as a stochastic decision, using tighter confidence bounds, adaptive sampling based on input difficulty, and regret guarantees.

Result: The adaptive sampling strategy achieved a false positive rate of 0.095 (vs. 0.159 for single-agent models), reducing incorrect flagging by 40% while maintaining similar false negative rates.

Conclusion: Principled adaptive sampling improves precision without sacrificing recall in multi-agent LLM pipelines for behavioral health.
...
### [127] [Sum-of-Checks: Structured Reasoning for Surgical Safety with Large Vision-Language Models](https://arxiv.org/abs/2604.22156)
*Weiqiu You,Cassandra Goldberg,Amin Madani,Daniel A. Hashimoto,Eric Wong*

Main category: cs.LG

TL;DR: Sum-of-Checks framework improves LVLM accuracy and transparency for surgical CVS assessment by decomposing criteria into expert-defined checks.


<details>
  <summary>Details</summary>
Motivation: To enhance the reliability and auditability of LVLMs in safety-critical surgical tasks like CVS assessment in laparoscopic cholecystectomy.

Method: Decomposes CVS criteria into expert-defined checks, aggregates binary judgments, and evaluates using LVLMs on the Endoscapes2023 benchmark.

Result: Improves frame-level mean average precision by 12-14% and shows reliability on observational checks but variability on anatomical evidence.

Conclusion: Explicit separation of evidence elicitation from decision-making enhances LVLM reliability and transparency in surgical AI systems.
...
### [128] [Logistic Bandits with $\tilde{O}(\sqrt{dT})$ Regret without Context Diversity Assumptions](https://arxiv.org/abs/2604.22161)
*Seoungbin Bae,Dabeen Lee*

Main category: cs.LG

TL;DR: ThepaperintroducesSupSplitLog,anovelalgorithmforthe$K$-armedlogisticbanditproblem,achieving$\tilde{\mathcal{O}}(\sqrt{dT})$regretwithoutcontextdiversityassumptions,improvingonexistingmethods.


<details>
  <summary>Details</summary>
Motivation: Existingmethodsforthelogisticbanditproblemrelyonrestrictivecontextdiversityassumptions,whichlimittheirapplicability.Thepaperaimstoovercomethislimitationbyproposingamoreflexiblealgorithm.

Method: Thealgorithmsplitssamplesintotwosubsets:oneforaninitial-pointestimatorandtheotherforaNewton-typeone-stepcorrection.Thesplittingrulebalancesaccuracybetweenthesesteps.

Result: SupSplitLogachieves$\tilde{\mathcal{O}}(\sqrt{dT})$regretwithoutcontextdiversityassumptions,improvesdependenceondimension$d$,andcanadapttodata-dependentcomplexitymeasures.

Conclusion: SupSplitLogisasuperioralgorithmforlogisticbandits,validatedbytheoreticalandexperimentalresults,offeringflexibilityandimprovedperformance.
...
### [129] [Estimating Tail Risks in Language Model Output Distributions](https://arxiv.org/abs/2604.22167)
*Rico Angell,Raghav Singhal,Zachary Horvitz,Zhou Yu,Rajesh Ranganath,Kathleen McKeown,He He*

Main category: cs.LG

TL;DR: The paper proposes a method to estimate the probability of harmful outputs from language models efficiently, addressing the tail risk in safety evaluations.


<details>
  <summary>Details</summary>
Motivation: With the widespread deployment of language models, ensuring their safety is crucial. Current evaluations overlook the probabilistic nature and tail behavior of models, necessitating a method to efficiently estimate rare harmful outputs.

Method: The authors use importance sampling by creating unsafe versions of the target model, making harmful outputs more probable and enabling efficient estimation with fewer samples.

Result: The method matches brute-force Monte Carlo estimates with 10-20x fewer samples, estimating probabilities as low as 10^-4 with just 500 samples. It also reveals model sensitivity to input perturbations and predicts deployment risks.

Conclusion: Accurate rare-event estimation is critical and feasible for safety evaluations, as demonstrated by the proposed method. Code is available for reproducibility.
...
### [130] [Optimal sequential decision-making for error propagation mitigation in digital twins](https://arxiv.org/abs/2604.22168)
*Annice Najafi,Shokoufeh Mirzaei*

Main category: cs.LG

TL;DR: The paper addresses error propagation in modular digital twins using MDP and POMDP frameworks, comparing them with model-free RL methods and highlighting the value of information for improving regime classification.


<details>
  <summary>Details</summary>
Motivation: To mitigate error propagation in modular digital twins by developing sequential decision-making frameworks that balance system fidelity and maintenance costs.

Method: The study uses MDP and POMDP frameworks, leveraging HMM-derived states and interventions, solved via dynamic programming and validated through stochastic simulation. It also benchmarks Q-learning and REINFORCE for model-free policy learning.

Result: MDP achieves the highest reward and nominal operation time; POMDP recovers 95% of MDP performance under noise. Sensitivity analyses confirm robustness, with gaps in policy hierarchy being statistically significant.

Conclusion: The MDP and POMDP frameworks effectively mitigate error propagation, with the performance gap emphasizing the value of improved classification accuracy in decision-making.
...
### [131] [ReCast: Recasting Learning Signals for Reinforcement Learning in Generative Recommendation](https://arxiv.org/abs/2604.22169)
*Peiyan Zhang,Hanmo Liu,Chengxuan Tong,Yuxia Wu,Wei Guo,Yong Liu*

Main category: cs.LG

TL;DR: ReCast improves RL for generative recommendation by repairing unlearnable groups and contrasting strong positives/negatives, outperforming baselines with significant efficiency gains.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the breakdown of the assumption that all sampled groups in RL are learnable, particularly in sparse-hit generative recommendation, where many groups remain unlearnable.

Method: Proposes ReCast, a framework that restores learnability for all-zero groups and replaces full-group reward normalization with boundary-focused contrastive updates on the strongest positives and hardest negatives.

Result: ReCast outperforms OpenOneRec-RL by up to 36.6% in Pass@1, achieves target performance with only 4.1% of the rollout budget, and improves system-level metrics like update time, memory usage, and MFU.

Conclusion: ReCast demonstrates that constructing learnable optimization events from sparse supervision is crucial for generative recommendation, not just reward assignment.
...
### [132] [Sharpness-Aware Poisoning: Enhancing Transferability of Injective Attacks on Recommender Systems](https://arxiv.org/abs/2604.22170)
*Junsong Xie,Yonghui Yang,Pengyang Shao,Le Wu*

Main category: cs.LG

TL;DR: The paper introduces SharpAP, a method to improve the transferability of poisoning attacks in recommender systems by targeting the worst-case victim model using sharpness-aware minimization.


<details>
  <summary>Details</summary>
Motivation: Existing poisoning attack methods assume that poisoned data for a surrogate model can attack other victim models, but this often fails due to structural discrepancies. Targeting the worst-case victim model can improve transferability.

Method: SharpAP uses sharpness-aware minimization to approximate the worst-case victim model and optimizes poisoned data against it, formulated as a min-max-min tri-level optimization problem.

Result: Experiments on three datasets show SharpAP enhances attack transferability significantly.

Conclusion: SharpAP effectively mitigates overfitting to surrogate models and improves robustness of poisoning attacks in recommender systems.
...
TL;DR: DROL introduces a one-step actor with top-1 dynamic routing, improving performance by locally updating actions without compromising inference speed.


<details>
  <summary>Details</summary>
Motivation: To enhance one-step offline RL actors by avoiding compromises between Q-improvement and data support, enabling local updates.

Method: Uses latent-conditioned actor with top-1 dynamic routing: samples K actions, assigns dataset actions to nearest candidates, and updates winners with Behavior Cloning and critic guidance.

Result: Competitive with FQL baseline, improves OGBench tasks, strong on AntMaze and Adroit.

Conclusion: DROL successfully balances local improvements and single-pass inference, proving effective in offline RL tasks.
...
TL;DR: The paper proposes a 2.5D U-Net architecture to segment gaseous microemboli (GME) in cardiac ultrasound imaging, improving detection and quantification in real-time for surgical monitoring.


<details>
  <summary>Details</summary>
Motivation: GME detection in cardiac interventions is challenging due to operator-dependent views, high velocity, and background noise, necessitating a more automated and accurate method.

Method: A 2.5D U-Net architecture is used to segment GME in spatiotemporal data, enhancing detection robustness and segmentation accuracy while maintaining real-time performance.

Result: The approach achieves high segmentation accuracy and robust GME detection against background noise, enabling real-time integration into surgical monitoring protocols.

Conclusion: The 2.5D U-Net-based method provides a reliable, real-time solution for GME detection and quantification, improving patient monitoring during cardiac interventions.
...
TL;DR: The paper explores how large language models (LLMs) detect and correct their own errors, focusing on a second-order confidence mechanism revealed by post-answer newline (PANL) signals.


<details>
  <summary>Details</summary>
Motivation: Understanding the mechanisms behind LLMs' self-error detection and correction, as current frameworks lack clarity on this capability.

Method: Using a verify-then-correct paradigm, the study tests whether PANL signals support error detection and self-correction, comparing behavioral and neural signals across models and tasks.

Result: PANL activations predict error detection and correction beyond traditional measures like log-probabilities. Causal interventions confirm PANL's role in rescuing error detection.

Conclusion: LLMs naturally implement a second-order confidence architecture, where internal evaluative signals (PANL) encode both error likelihood and correction capability.
...
TL;DR: The paper introduces a deep learning-based method (RSSNet) for unmixing single-channel noisy Raman spectra into individual components, outperforming existing methods by >4dB and demonstrating strong generalization to real-world data.


<details>
  <summary>Details</summary>
Motivation: The challenge of unmixing noisy, single-channel Raman spectra into individual components is critical for applications like controlled substance detection, but existing methods (e.g., sparse regression) lack noise tolerance and practical applicability.

Method: A neural approach inspired by speech separation, using a deep separation network (RSSNet), is proposed to decompose mixed spectra from a library of thousands of components.

Result: RSSNet outperforms competing methods by >4dB on synthetic datasets and generalizes well to real-world mixed spectra of mineral powders.

Conclusion: The RSSNet approach enables fast, accurate unmixing of Raman spectra, paving the way for new applications in Raman spectroscopy.
...
TL;DR: The paper introduces the FETs benchmark, showing foundation models outperform traditional ML in energy forecasting, especially in data-constrained settings.


<details>
  <summary>Details</summary>
Motivation: The shift to a climate-neutral energy system demands scalable forecasting solutions. Current methods are dataset-specific and resource-intensive, prompting exploration of foundation models.

Method: FETS benchmark includes: (1) categorizing energy forecasting use cases, (2) analyzing 54 datasets, and (3) comparing foundation models with classical ML across forecasting settings.

Result: Foundation models outperform dataset-specific ML, with covariate-informed models excelling. Performance correlates with spectral entropy and improves at higher aggregation levels.

Conclusion: Foundation models offer scalable, generalizable solutions for energy forecasting, especially in data-limited or privacy-sensitive scenarios.
...
TL;DR: TabSCM is a tabular-data generator that preserves causal dependencies, outperforming other methods in fidelity, utility, and privacy, while being faster and more interpretable.


<details>
  <summary>Details</summary>
Motivation: Existing tabular-data generators often ignore causal structure, leading to downstream models learning spurious or unfair patterns. TabSCM aims to address this by preserving causal dependencies.

Method: TabSCM uses a CPDAG to orient a DAG, fits root-node marginals with KDE or categorical frequencies, and learns topologically ordered structural assignments using conditional diffusion models for continuous variables and gradient-boosted trees for categorical ones.

Result: TabSCM outperforms state-of-the-art GAN, diffusion, and LLM baselines in statistical fidelity, downstream utility, and privacy risk, while reducing rule violations and enabling causally meaningful interventions.

Conclusion: TabSCM offers a practical solution for realistic, explainable, and causally sound tabular-data generation, with significant speed improvements and interpretability benefits for fairness auditing and policy simulation.
...
TL;DR: This study explores how model size impacts downstream task performance in structured medical foundation models, finding that optimal size varies by task (disease vs. medication prediction) and does not always require the largest models.


<details>
  <summary>Details</summary>
Motivation: To determine whether increasing model size consistently improves predictions for structured medical data, given its limited vocabulary and sparse observations, unlike NLP where larger models generally perform better.

Method: Pretrained encoder-only Transformers at five scales (2.2M-101M parameters) using a random sample (2.3 million patients, 32 hospitals) from a nationwide Japanese claims database. Evaluated performance for disease incidence and medication prediction.

Result: Downstream performance saturated at task-dependent thresholds: disease prediction benefited from larger models (32M-101M), while medication prediction plateaued at 11M, reducing pretraining time by 178 hours. The best models consistently outperformed Light Gradient Boosting Machine baselines in precision-recall AUC.

Conclusion: The optimal model size for structured medical data depends on task characteristics, not just scale, offering practical guidance for balancing performance and computational cost.
...
TL;DR: The paper introduces SOC-ICNN, a neural network architecture that generalizes classical ICNNs by incorporating second-order cone programming, enabling smoother curvature in representations while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Classical ReLU-based ICNNs are limited to piecewise-linear functions due to their equivalence to LP. The authors aim to overcome this representational bottleneck by extending the optimization framework to SOCP.

Method: The proposed SOC-ICNN architecture injects positive semi-definite curvature and Euclidean norm-based conic primitives, extending the optimization class from LP to SOCP.

Result: Theoretical proofs show that SOC-ICNNs strictly expand the representational capacity of ReLU-ICNNs without increasing computational complexity. Experiments confirm improved function approximation and decision quality.

Conclusion: SOC-ICNN successfully addresses the limitations of classical ICNNs by introducing smooth curvature, maintaining optimization-theoretic rigor, and demonstrating competitive performance in downstream tasks.
...
TL;DR: Extends NAC for uncertainty estimation in regression, showing better results than Monte-Carlo Dropout.


<details>
  <summary>Details</summary>
Motivation: The paper aims to extend the Neural Activation Coverage (NAC) technique, initially used for out-of-distribution detection and generalization, to serve as an uncertainty estimation method for pre-trained artificial neural networks in regression tasks.

Method: The authors adapt and apply NAC to estimate uncertainty in regression models, comparing its performance with other techniques like Monte-Carlo Dropout.

Result: Experiments demonstrate that NAC uncertainty scores are more meaningful than those from other methods, such as Monte-Carlo Dropout.

Conclusion: NAC is a viable and effective technique for uncertainty estimation in regression tasks, outperforming existing methods.
...
TL;DR: The paper introduces a robust fuzzy local k-plane clustering (RFLkPC) method to address susceptibility to outliers and bounded cluster issues in traditional KPC models, using a mixture of hinge loss and L1 norm.


<details>
  <summary>Details</summary>
Motivation: Traditional KPC models are sensitive to outliers and assume infinitely extending clusters, negatively impacting performance. The RFLkPC aims to mitigate these issues.

Method: The proposed RFLkPC method combines hinge loss and L1 norm to robustly handle bounded plane clusters, with provided optimization algorithms.

Result: Experiments on simulated and real data confirm RFLkPC's efficiency over related models.

Conclusion: RFLkPC effectively addresses outlier sensitivity and finite area assumptions, improving clustering performance for bounded data.
...
TL;DR: The paper reveals a hidden failure mode in continual-learning methods when combined with Adam, showing that adaptive decoupled routing is the only stable solution.


<details>
  <summary>Details</summary>
Motivation: To address the instability and collapse of continual-learning methods when used with Adam, particularly in high-overlap, non-adaptive scenarios.

Method: The study evaluates various continual-learning methods (e.g., projection, replay mixing) and introduces adaptive decoupled routing to fix the issue by modifying gradients for the first moment only while preserving second-moment statistics.

Result: Adaptive decoupled routing achieves stability (9.4) and outperforms other methods (e.g., 11.6 for replay buffer, 14.1 for fixed-strength decoupling) in 8-domain and 16-domain continual learning tasks.

Conclusion: The proposed adaptive decoupled routing is the only consistently stable solution across methods, optimizers, and scales, addressing the hidden failure mode in Adam-based continual learning.
...
TL;DR: Graph Transformers' performance varies with task locality; distance-misaligned training reveals the need for adaptive communication strategies to match task requirements.


<details>
  <summary>Details</summary>
Motivation: To understand how Graph Transformers balance local and global communication for node-classification tasks, especially in scenarios where the relevance of information varies with graph distance.

Method: A synthetic node-classification benchmark on contextual stochastic block model graphs is used, where labels are generated by a mix of local and far-shell signals. The study examines distance-misaligned training and evaluates adaptive controllers.

Result: Task locality affects the preferred graph-distance bias. An oracle adaptive controller nearly matches the best fixed bias across regimes, while a task-agnostic controller underperforms, highlighting the importance of control targets.

Conclusion: Distance-resolved diagnosis is valuable for diagnosing Graph Transformer failures and designing effective graph-aware control mechanisms.
...
TL;DR: L2C is a framework that connects local and cluster-level causal discovery, automatically finding partitions and handling latent variables without assuming causal sufficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for causal discovery struggle with latent variables and either focus too narrowly or make unrealistic assumptions about clusters.

Method: L2C automates cluster discovery, uses a reduction theorem to simplify clusters, applies local causal discovery, and performs macro-level inference without causal sufficiency.

Result: Theoretical and experimental results show L2C accurately recovers clusters and outperforms baselines in macro causal effect identification.

Conclusion: L2C provides a sound, efficient, and scalable solution for bridging local and cluster-level causal discovery.
...
TL;DR: The study compares land surface temperature (LST) and universal thermal climate index (UTCI) in Singapore, revealing significant spatial and mechanistic differences between the two metrics. It highlights the need for physiologically relevant indices like UTCI in urban planning.


<details>
  <summary>Details</summary>
Motivation: Heat exposure impacts urban livability, but LST may not adequately represent human heat stress. The study aims to bridge this gap by evaluating discrepancies between LST and UTCI.

Method: A 'Modeling-Comparing-Assessing' framework is used, combining Landsat-retrieved LST and GPU-accelerated UTCI. Geographically weighted XGBoost (GW-XGBoost) and generalized additive models (GAM) analyze relationships with urban factors.

Result: LST and UTCI show notable spatial discrepancies. Urban factors affect them differently, with sky view factor being key for UTCI but marginal for LST. Higher albedo unexpectedly increases UTCI.

Conclusion: UTCI better captures human heat stress than LST. The findings advocate for integrating physiologically relevant thermal indices in urban planning for targeted heat risk management.
...
TL;DR: HubRouter is a pluggable module that replaces O(n^2) attention layers with O(nM) hub-mediated routing, improving efficiency and performance in certain cases.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of traditional O(n^2) attention layers by introducing a more efficient O(nM) routing mechanism using learned hub tokens.

Method: HubRouter uses an encode-decode-score-council pipeline where M learned hubs cross-attend to tokens, tokens project against hubs for routing fingerprints, a score head selects top-k tokens, and a sparse council attends only to the selected subset.

Result: HubRouter shows improvements in perplexity (PPL) and training throughput, with specific configurations (e.g., M=8-14) reliably converging. However, retrofitting into pretrained models was unsuccessful.

Conclusion: HubRouter offers a viable alternative to traditional attention layers, balancing efficiency and performance, though it introduces a slight quality cost in some cases.
...
TL;DR: MADE-IT resolves the saturation-redundancy dilemma in Continual Model Merging by using manifold-aware expert evolution and implicit routing, improving accuracy and pruning redundancy.


<details>
  <summary>Details</summary>
Motivation: Existing CMM methods struggle with parameter saturation in backbone-centric approaches and expert redundancy in MoE variants, necessitating a more adaptive solution.

Method: Proposes MADE-IT, which uses manifold geometry for expert management, a subspace affinity metric for evolution, and implicit routing via feature-subspace alignment.

Result: MADE-IT outperforms baselines in accuracy and robustness, prunes redundant experts effectively, and works well in long-horizon and shuffled task sequences.

Conclusion: MADE-IT offers a robust solution to the challenges in CMM, balancing diversity and efficiency while eliminating the need for parameterized gating networks.
...
TL;DR: Deep learning compares DDL and CFM for kinetic parameter estimation in itaconic acid production, with CFM outperforming DDL and matching nonlinear regression accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve kinetic parameter estimation for bioprocess modeling using deep learning, comparing CFM and DDL against traditional nonlinear regression.

Method: Two deep learning strategies (DDL and CFM) are tested on batch experiments at varying agitation speeds and scales, benchmarked against nonlinear regression.

Result: CFM provides more accurate and robust predictions than DDL, closely matching nonlinear regression results, and performs well in scale-up scenarios.

Conclusion: CFM is a reliable, flexible, and data-efficient method for parameter estimation in dynamic bioprocess models across different conditions and scales.
...
TL;DR: The paper introduces an end-to-end framework for continuous EMG-to-kinematics regression using consumer-grade hardware, achieving superior performance in decoding finger motion compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: To enable natural control for prosthetics, AR/XR interfaces, and teleoperation by addressing the challenges of accurately estimating finger kinematics from forearm EMG signals.

Method: Combines an 8-channel EMG armband, a webcam, and automatic synchronization to collect a dataset (EMG-FK). Uses the Temporal Riemannian Regressor (TRR), a GRU-based model with multi-band Riemannian covariance features.

Result: TRR outperforms state-of-the-art methods, achieving 9.79° and 16.71° average absolute error in intra- and cross-subject evaluations, respectively. It also runs efficiently in real-time (10 predictions/s).

Conclusion: The framework lowers barriers for reproducible, real-time EMG-based decoding of high-dimensional finger motion, enhancing natural control for embedded systems.
...
TL;DR: FeatEHR-LLM uses LLMs to generate clinically meaningful features from irregular EHR data, outperforming baselines in clinical prediction tasks.


<details>
  <summary>Details</summary>
Motivation: EHR feature engineering is challenging due to irregular data patterns. Existing methods lack clinical relevance or assume clean data, limiting real-world use.

Method: FeatEHR-LLM employs LLMs to generate executable feature-extraction code from dataset schemas, handling irregular temporal data and sparsity via tool-augmented generation.

Result: Evaluated on eight tasks, FeatEHR-LLM achieves the highest mean AUROC on 7 out of 8 tasks, improving by up to 6 percentage points over baselines.

Conclusion: FeatEHR-LLM effectively addresses EHR feature engineering challenges, offering privacy-aware and clinically relevant feature generation.
...
TL;DR: A framework for clinical readmission prediction was developed, addressing explainability, reliability, and fairness, validated on MIMIC-IV data.


<details>
  <summary>Details</summary>
Motivation: The study aimed to overcome barriers in clinical translation of readmission prediction, including explainability, deployment reliability, and demographic fairness.

Method: Models (Logistic regression, XGBoost, LightGBM) were trained on 415231 adult admissions from MIMIC-IV, using SHAP for explanations and fairness evaluation across subgroups.

Result: XGBoost showed best performance (AUC-ROC 0.696), LightGBM had best calibration. Subgroups met equity thresholds, with prior admissions as the top predictor.

Conclusion: The framework offers competitive performance, actionable insights, and fairness, with publicly available code.
...
TL;DR: Supervised Contrastive Learning (SCL) produces higher-quality feature attribution explanations than Cross-Entropy (CE) in neural networks, improving model transparency and trustworthiness.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that SCL can enhance feature attribution explanations in neural networks, making models more transparent and reliable, especially in safety-critical applications.

Method: The study empirically compares SCL and CE-based training for image classification, focusing on the quality of feature attribution explanations (faithfulness, complexity, continuity).

Result: SCL-trained models yield better feature attribution explanations in terms of faithfulness, complexity, and continuity compared to CE.

Conclusion: SCL is a superior training objective for achieving not only accuracy but also model transparency, reinforcing its value in building trustworthy neural networks.
...
TL;DR: SOLAR-RL integrates global trajectory insights into offline learning, improving task completion and robustness for GUI navigation.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in balancing the benefits of Online RL (long-term dynamics) with the limitations of Offline RL (static data).

Method: SOLAR-RL reconstructs rollout candidates from static data, detects failure points, and assigns dense rewards to simulate online feedback.

Result: The framework outperforms baselines in task completion rates and robustness.

Conclusion: SOLAR-RL offers a sample-efficient solution for training MLLM agents in dynamic GUI tasks.
...
TL;DR: The paper proposes a data-free method using matrix von Neumann entropy to estimate client contributions in Federated Learning, introducing two schemes (SpectralFed and SpectralFuse) that outperform baselines in accuracy correlation.


<details>
  <summary>Details</summary>
Motivation: Current methods for client contribution estimation in Federated Learning rely on server-side validation data or self-reported client information, which can compromise privacy or be manipulated. A privacy-preserving, data-free alternative is needed.

Method: Introduces a data-free signal based on the matrix von Neumann (spectral) entropy of final-layer updates. Two schemes are proposed: SpectralFed (uses normalized entropy as aggregation weights) and SpectralFuse (fuses entropy with class-specific alignment via a rank-adaptive Kalman filter).

Result: The entropy-derived scores show high correlation with standalone client accuracy across CIFAR-10/100, FEMNIST, and FedISIC benchmarks under diverse non-IID regimes, without needing validation data or client metadata.

Conclusion: Spectral entropy serves as a reliable indicator of client contribution in Federated Learning, offering a privacy-preserving and manipulation-resistant alternative to existing methods.
...
TL;DR: SpikingBrain2.0 (SpB2.0) introduces architectural and training innovations to improve long-context efficiency and performance for large models, achieving significant speedups and energy savings.


<details>
  <summary>Details</summary>
Motivation: The need for foundation models that maintain performance and efficiency with long sequences while minimizing training overhead drives the development of SpB2.0.

Method: SpB2.0 uses Dual-Space Sparse Attention (DSSA), combining Sparse Softmax and Linear Attention, and dual quantization paths (INT8-Spiking and FP8). It also employs an optimized Transformer-to-Hybrid training pipeline.

Result: SpB2.0 achieves a 10.13x TTFT speedup at 4M context, supports 10M tokens on 8 GPUs, and offers cross-platform compatibility with significant sparsity and energy reduction.

Conclusion: SpikingBrain2.0 demonstrates the potential of integrating brain-inspired mechanisms with efficient architectures for lightweight, multimodal models, especially in resource-constrained scenarios.
...
TL;DR: BudgetFormer introduces adaptive multi-head attention in Transformers, dynamically allocating attention heads per input to reduce computational costs while maintaining or improving performance.


<details>
  <summary>Details</summary>
Motivation: Standard multi-head attention uniformly uses all heads for every input, which is inefficient for tasks like text classification where not all heads are necessary.

Method: BudgetFormer learns a head budget and relevance distribution per input, selecting the most informative heads, and uses an exploration-exploitation training strategy.

Result: Experiments show reduced FLOPs and memory usage while often outperforming standard multi-head attention in text classification.

Conclusion: Adaptive head allocation improves Transformer efficiency and effectiveness, especially for tasks where full head usage isn't required.
...
TL;DR: A method combining cross-lingual transfer learning and unsupervised clustering to discover morphological features in low-resource Bantu languages, validated on Giriama with high accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of morphological documentation in low-resource Bantu languages by leveraging existing linguistic resources and discovering language-specific patterns.

Method: Uses an ensemble of transfer learning from Swahili and unsupervised clustering, combined via weighted voting, to exploit complementary strengths in cognate detection and language-specific innovation discovery.

Result: Achieved 78.2% lemmatization accuracy on known verb paradigms and expanded a corpus to 19,624 words with high segmentation (97.3%) and lemmatization (86.7%) rates. Discovered undocumented morphological patterns like a- prefix variant and contracted k'- prefix.

Conclusion: The method effectively combines transfer learning and clustering for morphological discovery in low-resource languages, with promising accuracy and practical applications for linguistic documentation.
...
TL;DR: The paper proposes a shift from invariance-based self-supervised learning to Action-Conditioned World Models for healthcare, focusing on disease progression dynamics. The approach outperforms supervised baselines, especially in low-resource settings.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised learning in healthcare uses invariance-based objectives, which are misaligned with clinical diagnosis as they suppress transient pathological changes. The paper aims to address this misalignment by modeling disease dynamics.

Method: Utilizes Action-Conditioned World Models (adapted from LeJEPA) to simulate disease progression, treating pathology as a transition vector in the patient's latent state. The model predicts future electrophysiological states post-disease onset.

Result: Outperforms fully supervised baselines on the MIMIC-IV-ECG dataset, particularly in low-resource regimes, achieving over 0.05 AUROC improvement.

Conclusion: Modeling biological dynamics provides a more robust supervision signal than static classification, enhancing sample efficiency and diagnostic accuracy.
...
TL;DR: Neural models trained on modern Bantu morphological data recover cross-lingual lexical structure aligning with historical Proto-Bantu forms.


<details>
  <summary>Details</summary>
Motivation: To determine if modern neural models can uncover historical linguistic patterns consistent with Proto-Bantu reconstructions.

Method: Used BantuMorph v7 to analyze 14 languages, extracted embeddings, identified cognate candidates, and compared them with BLR3 and ASJP databases.

Result: Confirmed alignment of 90.9% of top noun candidates and 12 verb cognates with Proto-Bantu forms, validated by an independent translation model.

Conclusion: The models recover shared Bantu lexical structure, though the dataset's regional focus limits definitive distinction between Proto-Bantu retentions and later innovations.
...
TL;DR: The paper introduces the Associativity Peakiness (AP) metric to evaluate clustering algorithm performance using contingency tables, filling a gap in existing literature.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for clustering algorithms focus on vector pairs, missing detailed features in contingency tables. A dedicated metric for contingency tables is needed for accurate performance comparison.

Method: The AP metric is proposed, analogous to confusion matrix quality measures in supervised learning. Simulations with 500 contingency tables tested its efficacy.

Result: AP outperforms existing metrics in dynamic range and computational efficiency for evaluating clustering algorithm performance.

Conclusion: The AP metric is a robust and efficient tool for assessing clustering algorithms via contingency tables, offering superior performance over current alternatives.
...
TL;DR: Shapley values in AI lack consensus on practical use. This study evaluates eight variants in risk workflows, showing standard metrics don't align with human utility and may cause automation bias.


<details>
  <summary>Details</summary>
Motivation: To address fragmentation in Shapley value formulations and test their real-world utility in high-stakes decision-making.

Method: A unified amortized framework evaluates eight Shapley variants across risk datasets and a fraud-detection environment, involving professional analysts and 3,735 case reviews.

Result: Standard metrics (e.g., sparsity, faithfulness) don't match human clarity or utility. Explanations boosted confidence but not performance, risking automation bias.

Conclusion: Current evaluation metrics fail to predict human impact. Evidence-based guidance is provided for selecting Shapley formulations in operational systems.
...
TL;DR: GP-MLMPC uses Gaussian Processes in a model-learning NMPC scheme for nonlinear batch processes, achieving significant performance improvements with minimal data.


<details>
  <summary>Details</summary>
Motivation: Batch processes are transient and nonlinear, but NMPC adoption is limited by the cost and lack of dynamic models. GP-MLMPC addresses this by learning models from data.

Method: Initialized with a single trajectory, GP-MLMPC iteratively updates GPs using new observations, enforces safe operation via chance constraints, and tests in a simulated semi-batch reactor.

Result: After 4 iterations, tracking error reduced by 83%. Under economic objectives, product mass increased 17-fold by iteration 8, matching full-model NMPC performance.

Conclusion: GP-MLMPC is a data-efficient, sample-efficient approach for controlling nonlinear batch processes without mechanistic knowledge, achieving quick convergence.
...
TL;DR: WG-SRC replaces opaque graph neural networks with a transparent, white-box method using fixed graph-signal dictionaries and closed-form solutions for node classification and dataset diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current graph neural networks lack transparency in their learned representations, obscuring the reasons for node classifications and the required feature-level mechanisms. WG-SRC aims to provide clarity through a white-box approach.

Method: WG-SRC uses a fixed graph-signal dictionary (raw features, low/high-pass propagation), Fisher coordinate selection, class-wise PCA, closed-form ridge classification, and score fusion. It provides explicit class subspaces and linear decisions.

Result: WG-SRC remains competitive with baselines across six datasets, achieving positive gain under aligned splits. It breaks down behavior into interpretable components, identifying dataset-specific fingerprints like low-pass dominance or high-pass sensitivity.

Conclusion: WG-SRC offers transparent insights into graph-learning mechanisms, guiding post-evaluation analysis and dataset modifications. Its diagnostic capabilities are validated by predictive performance and mechanistic interventions.
...
TL;DR: The paper addresses the high cost of fitting scaling laws in large-scale training by proposing a budget-aware sequential experimental design method, which optimizes run selection to maximize extrapolation accuracy in high-cost target regions.


<details>
  <summary>Details</summary>
Motivation: Fitting scaling laws for planning expensive training runs is costly, and assembling pilot experiments is now a major budget challenge. The goal is to efficiently allocate budget for runs to improve extrapolation accuracy.

Method: The authors formulate scaling-law fitting as a budget-aware sequential experimental design problem. They propose an uncertainty-aware method to allocate budget toward runs most useful for target-region extrapolation.

Result: The method outperforms classical baselines across diverse benchmarks, achieving near-full experimental set performance while using only about 10% of the total training budget.

Conclusion: The proposed method offers a cost-effective solution for scaling-law fitting, significantly reducing budget requirements while maintaining high extrapolation accuracy.
...
### [166] [DM$^3$-Nav: Decentralized Multi-Agent Multimodal Multi-Object Semantic Navigation](https://arxiv.org/abs/2604.22014)
*Amin Kashiri,Atharva Jamsandekar,Yasin Yazıcıoğlu*

Main category: cs.MA

TL;DR: DM³-Nav is a decentralized multi-agent semantic navigation system that supports multimodal goal specification and multi-object missions without central coordination, outperforming centralized baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a fully decentralized navigation system for multi-agent robots, eliminating reliance on central coordinators or shared global states, thus avoiding single points of failure.

Method: The system uses ad-hoc pairwise communication for coordination, exchanges local maps and navigation intent, and employs an implicit task allocation mechanism to reduce redundant exploration.

Result: Evaluations on HM3DSem scenes and real-world office environments show DM³-Nav matches or exceeds centralized baselines while relying entirely on onboard sensing and computation.

Conclusion: DM³-Nav successfully demonstrates decentralized, scalable, and robust navigation for multi-agent systems, validated in both simulated and real-world settings.
...
### [167] [Seeing the Whole Elephant: A Benchmark for Failure Attribution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2604.22708)
*Mengzhuo Chen,Junjie Wang,Fangwen Mu,Yawen Wang,Zhe Liu,Huanxiang Feng,Qing Wang*

Main category: cs.MA

TL;DR: The paper introduces TraceElephant, a benchmark for failure attribution in LLM-based multi-agent systems, emphasizing full execution observability to improve accuracy and align with real-world debugging scenarios.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for failure attribution in multi-agent systems lack full observability, missing inputs and context crucial for debugging. The paper aims to address this gap by focusing on full execution traces.

Method: The authors propose TraceElephant, a benchmark with full execution traces and reproducible environments, and systematically evaluate failure attribution techniques under various configurations.

Result: Full traces improve attribution accuracy by up to 76% over partial-observation benchmarks, revealing that missing inputs obscure many failure causes.

Conclusion: TraceElephant provides a foundation for future research, promoting evaluation practices that reflect real-world debugging and supporting more transparent multi-agent systems.
...
TL;DR: The paper introduces 'Math Takes Two,' a benchmark to evaluate if AI agents can develop mathematical reasoning from scratch through communication, bypassing predefined mathematical syntax.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine whether language models truly reason mathematically or just mimic patterns, focusing on the need for precise communication in developing numerical cognition.

Method: A new benchmark, 'Math Takes Two,' tests if two agents can create a shared symbolic system to solve visual tasks, requiring them to discover numerical structures independently.

Result: The benchmark provides a way to assess emergent numerical reasoning in models by requiring them to develop abstract concepts without prior knowledge.

Conclusion: Math Takes Two offers a unique approach to evaluate and foster genuine mathematical reasoning in AI models by mimicking human-like cognitive and communicative processes.
...
TL;DR: The paper introduces an artifact-based agent framework for medical imaging, combining adaptability and reproducibility in real-world clinical workflows.


<details>
  <summary>Details</summary>
Motivation: The shift from benchmark evaluation to clinical deployment in medical imaging requires adaptable and reproducible methods to handle dataset-specific conditions and evolving goals.

Method: The framework uses a semantic layer with artifact contracts for structured workflow state interrogation and goal-conditioned configuration assembly. It employs a workflow executor for deterministic provenance tracking and local agent operation for privacy compliance.

Result: Evaluated on clinical CT and MRI cohorts, the framework showed adaptive configuration synthesis, deterministic reproducibility, and semantic querying capabilities.

Conclusion: The framework successfully balances adaptability and reproducibility in heterogeneous clinical environments without compromising either.
...
TL;DR: MolClaw is an autonomous agent for drug discovery, integrating 30+ tools via a hierarchical skill architecture, outperforming current AI in complex workflows.


<details>
  <summary>Details</summary>
Motivation: Current AI agents struggle with robustness and performance in high-complexity computational drug discovery workflows.

Method: MolClaw uses a three-tier hierarchical skill architecture (tool-level, workflow-level, discipline-level) to integrate specialized tools and ensure robust performance.

Result: MolClaw achieves state-of-the-art performance, excelling in structured workflows, as confirmed by the MolBench benchmark.

Conclusion: Workflow orchestration is the key bottleneck for AI-driven drug discovery, addressed by MolClaw's skill architecture.
...
TL;DR: LLM agents can reproduce social science results using only methods descriptions and original data, but performance varies by model, scaffold, and paper. Failures arise from agent errors and underspecified methods.


<details>
  <summary>Details</summary>
Motivation: To determine if LLM agents can reproduce empirical social science results without access to original code or results, only using methods descriptions and data.

Method: Developed an agentic reproduction system that extracts structured methods from papers, reimplements them under strict isolation, and compares outputs to original results.

Result: Agents largely recover published results, but performance varies across models, scaffolds, and papers. Root causes of discrepancies include agent errors and underspecified methods.

Conclusion: LLM agents show promise in reproducing results with limited information, but improvements are needed for consistency and handling underspecified methods.
...
TL;DR: The paper proposes a two-layer certification framework to evaluate AI-generated research, separating knowledge quality from human contribution, and validates it through dry-run tests.


<details>
  <summary>Details</summary>
Motivation: Current publication systems assume human authorship and lack standards for evaluating AI-generated research, creating a need for a principled approach.

Method: The paper uses normative-conceptual analysis, designs a framework under four constraints, and validates it with dry-run tests on two submission cases.

Result: The framework successfully grades contributions (Categories A, B, C) and introduces benchmark slots for automated research, handling attribution uncertainty.

Conclusion: The framework allows transparent evaluation of AI-generated work within existing systems, focusing on epistemic achievement rather than unverifiable human claims.
...
### [173] [Sound Agentic Science Requires Adversarial Experiments](https://arxiv.org/abs/2604.22080)
*Dionizije Fa,Marko Culjak*

Main category: cs.AI

TL;DR: LLM-based agents in science can accelerate data analysis but risk producing plausible yet unverified claims, so they should prioritize seeking falsification over crafting compelling narratives.


<details>
  <summary>Details</summary>
Motivation: To address the risk of LLM-based agents generating unverified scientific claims by focusing on falsification rather than just producing publishable positives.

Method: Proposing a falsification-first standard for evaluating non-experimental claims made with agentic assistance, urging agents to actively seek ways claims can fail.

Result: Highlights the need for a shift in how LLM-based agents contribute to science, emphasizing verification through falsification.

Conclusion: Scientific use of LLM agents should prioritize falsification to ensure robust, validated knowledge rather than just accelerating the production of plausible claims.
...
### [174] [AgentSearchBench: A Benchmark for AI Agent Search in the Wild](https://arxiv.org/abs/2604.22436)
*Bin Wu,Arastun Mammadli,Xiaoyu Zhang,Emine Yilmaz*

Main category: cs.AI

TL;DR: AgentSearchBench is a large-scale benchmark introduced to evaluate agent search in real-world scenarios, showing gaps between semantic similarity and actual agent performance and emphasizing the need for execution signals in agent discovery.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of AI agent ecosystems creates a challenge in identifying suitable agents for tasks due to compositional and execution-dependent capabilities. Existing benchmarks are limited and don't address realistic scenarios.

Method: The authors introduce AgentSearchBench, a benchmark built from nearly 10,000 real-world agents. It formalizes agent search as retrieval and reranking problems using executable task queries and high-level descriptions, evaluating relevance with execution-grounded performance signals.

Result: Experiments reveal a gap between semantic similarity and actual agent performance, showing limitations of description-based methods. Lightweight behavioral signals, like execution-aware probing, improve ranking quality.

Conclusion: The study highlights the importance of incorporating execution signals into agent discovery, demonstrating the need to move beyond description-based approaches for accurate agent search.
...
TL;DR: Memanto is a universal memory layer for AI agents that reduces complexity and computational overhead compared to traditional hybrid semantic graph systems, achieving state-of-the-art accuracy with efficient retrieval.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems for autonomous agents rely on complex hybrid semantic graphs, which are computationally expensive and slow. Memanto aims to simplify this with a more efficient architecture.

Method: Memanto uses a typed semantic memory schema with 13 predefined categories, automated conflict resolution, and temporal versioning, powered by Moorcheh's Information Theoretic Search engine for fast, deterministic retrieval.

Result: Memanto achieves 89.8% and 87.1% accuracy on LongMemEval and LoCoMo benchmarks, outperforming hybrid graph and vector-based systems with lower operational complexity and no ingestion costs.

Conclusion: Memanto demonstrates that high-fidelity agent memory can be achieved without complex knowledge graphs, enabling scalable deployment of agentic systems.
...
TL;DR: The paper introduces ESRRSim, a framework for evaluating emergent strategic reasoning risks (ESRRs) in large language models (LLMs), identifying deception, evaluation gaming, and reward hacking as key risks.


<details>
  <summary>Details</summary>
Motivation: The study aims to systematically understand and benchmark the risks posed by LLMs engaging in deceptive or strategic behaviors, which is increasingly relevant as these models become more capable and widely deployed.

Method: The authors developed ESRRSim, a taxonomy-driven agentic framework with a 7-category, 20-subcategory risk taxonomy. It generates evaluation scenarios and uses dual rubrics to assess model responses and reasoning traces.

Result: Evaluation of 11 reasoning LLMs showed significant variation in risk profiles, with detection rates ranging from 14.45% to 72.72%, and noted generational improvements in models adapting to evaluation contexts.

Conclusion: The findings highlight the need for robust evaluation frameworks like ESRRSim to address emergent strategic risks in LLMs, as models continue to evolve and potentially adapt to testing environments.
...
TL;DR: The paper introduces the concept of 'background temperature' (T_bg) to explain nondeterministic outputs in LLMs even at T=0, linking it to implementation-level perturbations, and proposes methods to measure T_bg for reproducibility.


<details>
  <summary>Details</summary>
Motivation: To address the issue of divergent outputs in LLMs at T=0 due to implementation-level nondeterminism, the authors aim to formalize this behavior and propose a measurable framework.

Method: The authors define T_bg as the effective temperature caused by implementation perturbations, relate it to stochastic processes, and suggest an empirical protocol to estimate T_bg using an ideal reference system.

Result: Pilot experiments on major LLM providers demonstrate the idea and highlight implications for reproducibility, evaluation, and deployment.

Conclusion: The concept of T_bg provides a framework to understand and measure implementation-induced nondeterminism in LLMs, aiding reproducibility and practical applications.
...
TL;DR: CognitiveTwin is a digital twin framework using multi-modal data and Transformer-based architecture to predict personalized cognitive decline in Alzheimer's disease, showing high accuracy, fairness, and robustness.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of predicting heterogeneous cognitive decline in Alzheimer's patients, ensuring fairness and robustness in clinical tools.

Method: Integrates multi-modal longitudinal data with a Transformer-based architecture and Deep Markov Model for temporal dynamics, trained on the TADPOLE dataset.

Result: CognitiveTwin provides accurate, personalized predictions, fairness across demographics, and resilience to missing data.

Conclusion: The framework is a reliable tool for clinical trial enrichment and personalized care planning in Alzheimer's disease.
...
### [179] [From Skills to Talent: Organising Heterogeneous Agents as a Real-World Company](https://arxiv.org/abs/2604.22446)
*Zhengxu Yu,Yu Fu,Zhiyuan He,Yuxuan Huang,Lee Ka Yiu,Meng Fang,Weilin Luo,Jun Wang*

Main category: cs.AI

TL;DR: The paper introduces OneManCompany (OMC), a framework to elevate multi-agent systems to an organisational level, enabling dynamic recruitment and self-improvement through a structured loop (Explore-Execute-Review).


<details>
  <summary>Details</summary>
Motivation: Current multi-agent systems lack a principled organisational layer for dynamic assembly and governance, limiting adaptability and improvement over time.

Method: OMC uses portable agent identities (Talents) orchestrated via organisational interfaces and a Talent Market for on-demand recruitment. Decision-making employs an E²R tree search for hierarchical planning and execution.

Result: OMC achieves an 84.67% success rate, outperforming the state of the art by 15.48 percentage points, and demonstrates cross-domain adaptability.

Conclusion: OMC transforms multi-agent systems into self-organising, adaptable AI organisations, addressing limitations of fixed team structures and static coordination.
...
### [180] [Superminds Test: Actively Evaluating Collective Intelligence of Agent Society via Probing Agents](https://arxiv.org/abs/2604.22452)
*Xirui Li,Ming Li,Yunze Xiao,Ryan Wong,Dianqi Li,Timothy Baldwin,Tianyi Zhou*

Main category: cs.AI

TL;DR: The study investigates whether collective intelligence emerges in large-scale autonomous agent societies, finding it does not due to sparse and shallow interactions.


<details>
  <summary>Details</summary>
Motivation: Understanding if collective intelligence arises spontaneously in large populations of language model agents.

Method: Using MoltBook with over two million agents, the Superminds Test framework evaluates society-level intelligence across reasoning, synthesis, and interaction tiers.

Result: No collective intelligence emerged; societies underperformed individual models in reasoning, synthesis, and coordination. Interactions were shallow and generic.

Conclusion: Collective intelligence requires more than scale; current societies lack meaningful interaction depth and information exchange.
...
### [181] [On the Hybrid Nature of ABPMS Process Frames and its Implications on Automated Process Discovery](https://arxiv.org/abs/2604.22455)
*Anti Alman,Izack Cohen,Avigdor Gal,Fabrizio Maria Maggi,Marco Montali*

Main category: cs.AI

TL;DR: The paper introduces the concept of a hybrid process frame in AI-Augmented Business Process Management Systems (ABPMS), combining procedural and declarative models under an open-world assumption to enable framed autonomy.


<details>
  <summary>Details</summary>
Motivation: To enhance ABPMS by providing a more flexible and permissive process representation that accommodates both procedural and declarative models, allowing for semi-autonomous behavior.

Method: Conceptualizes the process frame as a hybrid model, blending procedural and declarative process models with execution semantics based on an open-world assumption. Proposes mapping declarative constraints to procedural fragments.

Result: A framework for hybrid process frames that supports semi-concurrent execution and allows for discovery of process frames by translating declarative constraints into procedural fragments.

Conclusion: The hybrid approach to process frames in ABPMS enables framed autonomy and flexibility, bridging the gap between procedural and declarative paradigms, and opens avenues for advanced process discovery.
...
### [182] [QuantClaw: Precision Where It Matters for OpenClaw](https://arxiv.org/abs/2604.22577)
*Manyi Zhang,Ji-Fu Li,Zhongao Sun,Xiaohao Liu,Zhenhua Dong,Xianzhi Yu,Haoli Bai,Xiaobo Xia*

Main category: cs.AI

TL;DR: QuantClaw dynamically adjusts precision in autonomous agent systems to save costs and reduce latency without compromising performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the high computational and monetary costs in autonomous agent systems like OpenClaw by exploring task-dependent precision requirements.

Method: Proposes QuantClaw, a plug-and-play precision routing plugin that dynamically assigns precision based on task characteristics.

Result: QuantClaw achieves up to 21.4% cost savings and 15.7% latency reduction on GLM-5 (FP8 baseline) while maintaining or improving task performance.

Conclusion: Treating precision as a dynamic resource in agent systems offers significant benefits in cost and latency without increasing user complexity.
...
### [183] [Rethinking Math Reasoning Evaluation: A Robust LLM-as-a-Judge Framework Beyond Symbolic Rigidity](https://arxiv.org/abs/2604.22597)
*Erez Yosef,Oron Anschel,Shunit Haviv Hakimi,Asaf Gendler,Adam Botach,Nimrod Berman,Igor Kviatkovsky*

Main category: cs.AI

TL;DR: The paper proposes an LLM-based evaluation framework for assessing mathematical reasoning in models, addressing limitations of symbolic comparison methods.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for mathematical reasoning, like symbolic comparison, are rigid and don't generalize well across diverse problem representations and answer formats.

Method: The authors introduce an LLM-based evaluation framework to overcome these limitations, ensuring flexibility and accuracy in assessment.

Result: The framework outperforms symbolic evaluation methods (e.g., in Lighteval and SimpleRL) by handling diverse formats and representations more effectively.

Conclusion: This approach enhances reliability in evaluating mathematical problem-solving, contributing to better benchmarking and advancement in intelligent systems.
...
### [184] [Agentic World Modeling: Foundations, Capabilities, Laws, and Beyond](https://arxiv.org/abs/2604.22748)
...
TL;DR: The paper proposes a taxonomy for world models in AI, categorizing them by capability levels (Predictor, Simulator, Evolver) and governing-law regimes (physical, digital, social, scientific). It synthesizes over 400 works, analyzes methods, failures, and evaluations, and provides a roadmap for advancing world models.


<details>
  <summary>Details</summary>
Motivation: The need for better world models arises as AI systems shift from text generation to goal-oriented, interactive tasks, requiring predictive models of environment dynamics.

Method: The authors introduce a 'levels x laws' taxonomy with three capability levels (L1-L3) and four governing-law regimes. They analyze over 400 works and 100 systems across these categories.

Result: The framework synthesizes diverse research, identifies evaluation principles, and provides architectural guidance. It highlights failure modes and governance challenges.

Conclusion: The roadmap connects isolated AI communities and guides the development of world models capable of simulating and reshaping environments.
