...
TL;DR: VTC compresses text into images for efficient processing but suffers varying performance. The paper introduces a measure of information loss via transport cost, enabling better routing and foveation for improved NLP tasks.


<details>
  <summary>Details</summary>
Motivation: Current visual text compression lacks a principled measure of task-relevant information loss, leading to unpredictable downstream performance.

Method: Formulates VTC using measure transport, decomposing costs into precision and coverage. Proposes label-free routing and foveation.

Result: Improves task scores by 3.3% with 10.3% fewer tokens, matching oracle performance on 17/24 datasets.

Conclusion: Transport cost analysis enhances VTC by optimizing routing and resolution, balancing efficiency and utility.
...
TL;DR: A comprehensive review of edge deep learning, focusing on computer vision and medical diagnostics, covering principles, hardware, implementation methods, applications, and future challenges.


<details>
  <summary>Details</summary>
Motivation: To highlight the transformative potential of edge deep learning in real-time, resource-constrained environments, particularly for medical diagnostics and computer vision.

Method: Review foundational principles, categorize edge hardware platforms, and analyze lightweight design and model compression techniques for deploying deep neural networks on edge devices.

Result: Demonstrates the significant impact of edge-deployed deep learning models in practical scenarios, particularly in computer vision and medical diagnostics.

Conclusion: The survey serves as a reference for researchers, outlining future directions and challenges to advance intelligent edge deep learning solutions.
...
TL;DR: HumanNet is a large-scale human-centric video dataset designed to advance embodied intelligence by providing diverse, annotated human activity data for learning physical interactions.


<details>
  <summary>Details</summary>
Motivation: Current embodied learning is limited by the lack of scalable, diverse, and richly annotated human activity data. HumanNet addresses this gap by offering a dataset that captures real-world interactions.

Method: HumanNet provides one million hours of video from first- and third-person perspectives, annotated with captions, motion descriptions, and body signals. It introduces a systematic curation paradigm focusing on human-centric filtering and annotation enrichment.

Result: Training with HumanNet's egocentric video outperformed training with robot-specific data in vision-language-action tasks, suggesting its potential as a scalable substitute for robot data.

Conclusion: HumanNet offers a scalable, cost-effective solution for advancing embodied foundation models by leveraging human-centric videos.
...
TL;DR: The paper introduces R$^3$L, a framework to improve the reliability of relative spatial reasoning in 3D layout generation by addressing error accumulation in multi-hop reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing methods using Multimodal Large Language Models (MLLMs) for relative spatial reasoning produce unreliable relations due to error accumulation in repeated reference-frame transformations, leading to semantic and metric drift.

Method: R$^3$L employs three techniques: invariant spatial decomposition to decouple relation chains, consistent spatial imagination for self-consistency via an imagine-and-revise loop, and supportive spatial optimization for easier pose optimization through coordinate re-parameterization.

Result: Experiments show R$^3$L generates more physically feasible and semantically consistent layouts across diverse scenes and instructions, with frame-induced inconsistency resolution being key to reliable reasoning.

Conclusion: R$^3$L effectively mitigates errors in multi-hop relative spatial reasoning, enhancing the reliability and consistency of 3D layout generation.
...
TL;DR: LookWhen is a selector-extractor framework that improves video recognition efficiency by focusing on key tokens, reducing computational cost while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Videos contain redundancy, making full token processing computationally expensive. The paper aims to optimize when, where, and what to compute.

Method: Uses a shallow selector to quickly score tokens and a deep extractor to process only top-K tokens. Pre-trains with supervision for selection (nearest-neighbor distance) and extraction (distilling video and image teachers).

Result: LookWhen achieves better accuracy-computation trade-offs, Pareto-dominates in 9 of 12 cases, and is 6.7x faster than baselines like InternVideo2-B.

Conclusion: The framework efficiently balances accuracy and computation, making it practical for real-world video recognition tasks.
...
TL;DR: AdpSplit improves 3D Gaussian Splatting (3DGS) by reducing training time through adaptive splitting, preserving rendering quality.


<details>
  <summary>Details</summary>
Motivation: Vanilla 3DGS requires many densification rounds to expose fine details, which is inefficient for training schedules with fewer iterations.

Method: AdpSplit introduces an error-driven adaptive split operator that determines the number of split children and initializes parameters from L1-pixel-error region statistics.

Result: AdpSplit reduces training time by 9.2%-22.3% across datasets and achieves full-schedule PSNR with a 16.4% reduction in training time.

Conclusion: AdpSplit enhances efficiency in 3DGS training while maintaining rendering quality, offering significant time savings.
...
TL;DR: TriDE improves camera direction estimation in structure-from-motion by using camera-triangle consistency and message passing, outperforming existing methods in accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing methods for camera direction estimation are locally plausible but globally inconsistent, motivating the need for a joint estimation approach.

Method: TriDE leverages camera-triangle consistency and refines directions via message passing between directions and weighted triangles, avoiding costly global optimization.

Result: TriDE achieves significant accuracy improvements in direction estimation and better downstream camera locations, validated on real image graphs.

Conclusion: TriDE bridges local pairwise estimation and global camera pose geometry, offering a robust and efficient solution for structure-from-motion tasks.
...
TL;DR: The paper presents a method to detect and mitigate label bias in image segmentation training data using Confident Learning without needing clean ground truth, ensuring equitable performance across subgroups.


<details>
  <summary>Details</summary>
Motivation: Label bias in image segmentation datasets can cause performance disparities across demographic subgroups, but detecting such bias typically requires clean, unbiased annotations, which are scarce.

Method: The study adapts Confident Learning to segmentation, comparing training labels to model predictions to detect directional errors and quantify bias. It leverages subgroup separability in the encoder's feature space for mitigation.

Result: The framework reliably detects and mitigates label bias in synthetic and real-life datasets, achieving equitable performance without clean labels.

Conclusion: The proposed data-centric approach effectively addresses label bias in image segmentation, improving fairness without relying on clean ground truth annotations.
...
### [10] [Domain-level metacognitive monitoring in frontier LLMs: A 33-model atlas](https://arxiv.org/abs/2605.06673)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: The study reveals that aggregate metacognitive scores hide variations across different domains in the MMLU benchmark, with Applied/Professional knowledge being the easiest and Formal Reasoning/Natural Science the hardest to monitor. Domain-level differences are significant and stable, suggesting the need for domain screening in model deployment.


<details>
  <summary>Details</summary>
Motivation: To investigate how metacognitive quality varies across different domains in the MMLU benchmark and whether aggregate scores mask important domain-level differences.

Method: Administered 1,500 MMLU items across six domains to 33 frontier LLMs, computed Type-2 AUROC per model-domain cell using verbalized confidence, and analyzed domain-level variations and coherence.

Result: Applied/Professional knowledge was easiest to monitor, while Formal Reasoning and Natural Science were hardest. Domain-level variations were significant, and some model families showed within-family clustering. Aggregate metrics obscured stable domain differences.

Conclusion: Domain-level variations in metacognitive quality are stable and significant, supporting domain screening before deploying models in specific applications. Aggregate metrics alone are insufficient for assessing model performance.
...
### [11] [VITA-QinYu: Expressive Spoken Language Model for Role-Playing and Singing](https://arxiv.org/abs/2605.06765)
...
TL;DR: VITA-QinYu is the first expressive E2E spoken language model supporting role-playing and singing, excelling in both expressiveness and conversational performance.


<details>
  <summary>Details</summary>
Motivation: To enhance speech models by capturing expressiveness like role-playing and singing, which are traditionally overlooked.

Method: Uses a hybrid speech-text paradigm with multi-codebook audio tokens and a data generation pipeline for 15.8K hours of training data.

Result: Outperforms peers in role-playing (7% higher) and singing (0.13 MOS better), with top conversational accuracy and fluency.

Conclusion: VITA-QinYu sets a new standard in expressive SLMs, with open-source release for broader impact.
...
### [12] [IntentGrasp: A Comprehensive Benchmark for Intent Understanding](https://arxiv.org/abs/2605.06832)
*Yuwei Yin,Chuyuan Li,Giuseppe Carenini*

Main category: cs.CL

TL;DR: IntentGrasp is a new benchmark for evaluating LLM intent understanding, revealing significant gaps between model and human performance, and proposing Intentional Fine-Tuning (IFT) to improve it.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of accurately understanding intent in speech, conversation, and writing to develop better LLM assistants.

Method: IntentGrasp is created from 49 corpora across 12 domains, featuring a large training set and two evaluation sets. The paper also introduces Intentional Fine-Tuning (IFT) to improve model performance.

Result: Evaluations on 20 LLMs show poor performance (below 60% on All Set, below 25% on Gem Set). IFT significantly improves results by 30+ F1 points on All Set and 20+ on Gem Set.

Conclusion: The study highlights the potential of IFT to enhance LLM intent understanding, paving the way for more capable and safe AI assistants.
...
### [13] [TajPersLexon: A Tajik-Persian Lexical Resource and Hybrid Model for Cross-Script Low-Resource NLP](https://arxiv.org/abs/2605.06886)
*Mullosharaf K. Arabov*

Main category: cs.CL

TL;DR: Introduces TajPersLexon, a Tajik-Persian parallel lexical resource of 40,112 pairs, and compares three methods for cross-script tasks, showing high accuracy with neural and retrieval models.


<details>
  <summary>Details</summary>
Motivation: Addresses the need for lexical resources and effective methods for cross-script retrieval, transliteration, and alignment in low-resource settings.

Method: Compares a lightweight hybrid pipeline, neural sequence-to-sequence models, and retrieval methods through CPU-only benchmarking.

Result: Neural and retrieval baselines achieve 98-99% top-1 accuracy; hybrid model shows a favorable accuracy-efficiency trade-off (96.4% in OCR post-correction).

Conclusion: The task is solvable, with reproducible results, and the dataset, code, and models will be publicly released.
...
### [14] [MIST: Multimodal Interactive Speech-based Tool-calling Conversational Assistants for Smart Homes](https://arxiv.org/abs/2605.06897)
*Maximillian Chen,Xuanming Zhang,Michael Peng,Zhou Yu,Alexandros Papangelis,Yohan Jo*

Main category: cs.CL

TL;DR: MIST introduces a synthetic dataset for multimodal, voice-driven IoT interactions, highlighting performance gaps between open- and closed-weight LLMs and encouraging research on voice assistants for physical world constraints.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of modeling IoT devices with voice interfaces, combining spatiotemporal constraints, speech inputs, and dynamic state tracking.

Method: The paper presents MIST, a multimodal dataset for multi-turn, voice-driven code generation over IoT devices, and an extensible framework for creating similar datasets.

Result: Findings reveal a notable performance gap between open- and closed-weight LLMs on MIST, with even top models showing room for improvement.

Conclusion: MIST and its framework are released to advance research on voice assistants that navigate physical world constraints.
...
### [15] [Reflections and New Directions for Human-Centered Large Language Models](https://arxiv.org/abs/2605.06901)
...
TL;DR: The paper introduces a framework for Human-Centered Large Language Models (HCLLMs), emphasizing the need to integrate human priorities into every stage of LLM development, from design to deployment.


<details>
  <summary>Details</summary>
Motivation: As LLMs gain global influence across various sectors, there's a pressing need to ensure these systems prioritize human concerns, values, and goals alongside technical capabilities.

Method: The framework integrates insights from NLP, HCI, and responsible AI, addressing ethics, economics, and technical objectives. It provides recommendations for each stage of the LLM pipeline, including design, data sourcing, training, evaluation, and deployment.

Result: The study culminates in a case study applying the HCLLM framework to explore the future of work with these models, demonstrating practical implementation.

Conclusion: The paper advocates for rigorous, human-centered approaches in LLM development to ensure ethical, responsible, and user-aligned outcomes.
...
### [16] [MELD: Multi-Task Equilibrated Learning Detector for AI-Generated Text](https://arxiv.org/abs/2605.06903)
*Chenjun Li,Cheng Wan,Johannes C. Paetzold*

Main category: cs.CL

TL;DR: MELD is a robust AI-generated text detector that uses multi-task learning and auxiliary supervision to outperform existing methods, especially under adversarial conditions and low false-positive rates.


<details>
  <summary>Details</summary>
Motivation: The need for reliable AI-generated text detection is critical for academic integrity, content moderation, and provenance tracking. Existing detectors lack robustness to attacks and unseen generators.

Method: MELD introduces a multi-task learning approach with auxiliary supervision (generator-family, attack-type, and source-domain heads), balanced losses, and distillation for robustness. It also uses a hard-negative ranking loss.

Result: MELD ranks as the strongest open-source detector on the RAID leaderboard and performs competitively with commercial models. On MELD-eval, it achieves 99.9% TPR at 1% FPR without additional finetuning.

Conclusion: MELD demonstrates superior robustness and performance in detecting AI-generated text, even under adversarial conditions, making it a deployable solution for real-world applications.
...
### [17] [Can LLMs Take Retrieved Information with a Grain of Salt?](https://arxiv.org/abs/2605.06919)
*Behzad Shayegh,Mohamed Osama Ahmed,Fred Tung,Leo Feng*

Main category: cs.CL

TL;DR: The paper evaluates LLMs' ability to adjust responses based on context certainty, identifies limitations, and proposes an interaction strategy to improve reliability.


<details>
  <summary>Details</summary>
Motivation: Assess LLMs' performance in adapting responses to the certainty of retrieved information, especially in high-stakes domains like medicine and finance.

Method: Evaluates eight LLMs on context-certainty obedience, analyzes their limitations, and proposes an interaction strategy with prior reminders, certainty recalibration, and context simplification.

Result: The proposed strategy reduces obedience errors by 25% on average without modifying model weights.

Conclusion: The study provides a metric for evaluating LLMs, insights into their uncertainty handling, and a portable strategy to improve context-certainty obedience.
...
### [18] [MultiSoc-4D: A Benchmark for Diagnosing Instruction-Induced Label Collapse in Closed-Set LLM Annotation of Bengali Social Media](https://arxiv.org/abs/2605.06940)
*Souvik Pramanik,S. M. Riaz Rahman Antu,Shak Mohammad Abyad,Md. Ibrahim Khalil,Md. Shahriar Hussain*

Main category: cs.CL

TL;DR: The paper introduces MultiSoc-4D, a Bengali social media dataset benchmark, to study LLM annotation behavior. It reveals 'instruction-induced label collapse,' where LLMs favor fallback labels, leading to under-detection of minority categories like hate speech and sarcasm.


<details>
  <summary>Details</summary>
Motivation: To investigate how LLMs handle closed-set instructions in low-resource languages like Bengali, focusing on annotation biases and their impact on NLP datasets.

Method: A structured pipeline using ChatGPT, Gemini, Claude, and Grok to annotate 58K+ social media comments along four dimensions, with a shared 20% validation set for systematic diagnosis.

Result: LLMs showed a preference for fallback labels, missing 79% of hate speech and 75% of sarcasm cases. Statistical analysis revealed a 'label agreement illusion' with Fleiss' Kappa near zero for sarcasm detection.

Conclusion: The study highlights systemic biases in LLM annotations, underscoring the need for benchmarks like MultiSoc-4D to diagnose and address these issues in low-resource language NLP.
...
### [19] [Group of Skills: Group-Structured Skill Retrieval for Agent Skill Libraries](https://arxiv.org/abs/2605.06978)
*Kun Zeng,Yu Huo,Siyu Zhang,Zi Ye,Yuecheng Zhuo,Haoyue Liu,Yuquan Lu,Junhao Wen,Xiaoying Tang*

Main category: cs.CL

TL;DR: GoSkills improves skill retrieval by organizing skills into role-labeled groups, enhancing context presentation without altering existing systems.


<details>
  <summary>Details</summary>
Motivation: Current methods for retrieving skills often lack explicit context, forcing agents to infer execution details, which can be inefficient or error-prone.

Method: GoSkills groups skills into structured, labeled contexts using a typed skill graph, expands support groups via a group graph, and presents a fixed execution contract with clear fields (Start, Support, Check, Avoid).

Result: Experiments show GoSkills maintains requirement coverage under limited skill budgets, outperforms flat skill-access methods, and often improves reward and runtime.

Conclusion: GoSkills effectively addresses context ambiguity in skill retrieval, proving its utility in real-world applications like SkillsBench and ALFWorld.
...
TL;DR: Proposes GenTTP, a model for predicting travel time by accounting for varying route choices, improving accuracy over methods limited to typical demand patterns.


<details>
  <summary>Details</summary>
Motivation: Existing travel time prediction methods, based on graph neural networks, fail to capture varying route choices and generalize poorly across different demand realizations.

Method: Introduces GenTTP, a framework that learns spatiotemporal traffic patterns and microscopic relationships between route choices and travel times.

Result: GenTTP successfully differentiates route choices and provides accurate flow and travel time predictions, addressing a critical gap in generalization.

Conclusion: The proposed model outperforms previous methods by generalizing across varying route assignments and improving prediction accuracy.
...
TL;DR: The paper explores how communication frequency and content impact collective belief states in distributed agents, introducing the concept of 'epistemic alignment' to measure effective belief sharing.


<details>
  <summary>Details</summary>
Motivation: Agents in real-world settings must coordinate under uncertainty with partial observations, but communication has costs. Poor communication can degrade collective reasoning, especially in bandwidth-constrained environments.

Method: The study formalizes the concept of epistemic alignment, distinguishing effective belief sharing from poor communication, which can lead to confidently wrong hypotheses.

Result: The paper shows that traditional coordination metrics alone, like Jensen-Shannon Divergence or consensus rates, cannot detect the quality of epistemic alignment.

Conclusion: Effective communication design is crucial for maintaining epistemic alignment among distributed agents, as it directly impacts their ability to converge on correct hypotheses.
...
TL;DR: The paper argues for modeling agentic AI systems using social theory and introduces the Multi-Agent Social Systems (MASS) framework to analyze interactions and outcomes in multi-agent environments.


<details>
  <summary>Details</summary>
Motivation: Agentic AI systems are increasingly part of social environments. The paper highlights the need to understand emergent dynamics in these systems, drawing parallels to human social group studies.

Method: Proposes the MASS framework, grounded in social theory, with four structural priors: strategic heterogeneity, networked-constrained dependence, co-evolution, and distributional instability. Formal propositions validate each prior.

Result: The paper demonstrates the significance of each structural prior in modeling multi-agent interactions and outlines a research agenda for MASS applications.

Conclusion: The MASS framework provides a structured approach to modeling, evaluating, and governing AI systems in social contexts, leveraging insights from social theory.
...
TL;DR: The paper explores the impact of agent decision order in N-level Stackelberg Games and introduces the Hierarchical Priority Adjustment (HPA) method to optimize it, improving performance in multi-agent systems.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems often rely on default decision orders, but the paper questions whether this order affects the game's equilibrium. It aims to analyze and optimize this order for better outcomes.

Method: Proposes the Hierarchical Priority Adjustment (HPA) method, which dynamically selects agent decision orders at an upper level and executes strategies in STMG at a lower level. Uses a slow-fast update scheme with shared intrinsic rewards.

Result: HPA outperforms benchmarks in high-precision control tasks, including multi-agent MuJoCo, demonstrating robust adaptation to changing environments.

Conclusion: Optimizing the decision order of agents in N-level Stackelberg Games is crucial for achieving better equilibrium and performance in multi-agent systems.
...
TL;DR: OrchJail is a fuzzing framework that jailbreaks tool-calling T2I agents by exploiting high-risk tool-orchestration patterns, improving effectiveness and efficiency in inducing harmful outputs.


<details>
  <summary>Details</summary>
Motivation: The need to address safety risks in tool-calling T2I agents, where harmful outputs arise from multi-step tool orchestration, even if individual steps are benign.

Method: OrchJail learns from successful jailbreak traces and their causal relationships to prompt wording, guiding fuzzing toward prompts likely to trigger unsafe multi-step behaviors.

Result: OrchJail achieves higher attack success rates, better image fidelity, and lower query costs, remaining robust against common defenses.

Conclusion: Tool orchestration is a critical attack surface, and OrchJail provides a novel framework for uncovering safety risks in T2I agents.
...
TL;DR: The study simulates the emergence of social reality in emotion concepts using a computational model where agents adapt their bodily control and symbol interpretations through social interaction, leading to consensus.


<details>
  <summary>Details</summary>
Motivation: To explore how social reality in emotion concepts emerges from interoceptive sensations and social interaction, as suggested by the theory of constructed emotion.

Method: A computational model integrating symbol emergence, active inference, and social interaction was used. Two agents exchanged inferred symbols while adapting their bodily control goals and interpretations.

Result: The agents' interoceptive prior preferences and symbol probability distributions converged, demonstrating the emergence of social reality based on social consensus.

Conclusion: The computational model successfully simulates the emergence of social reality in emotion concepts through social interaction and adaptation, supporting the theory of constructed emotion.
...
### [27] [RateQuant: Optimal Mixed-Precision KV Cache Quantization via Rate-Distortion Theory](https://arxiv.org/abs/2605.06675)
*Fei Zuo,Zikang Zhou,Hao Cong,Xiaoyan Xi,Ho Fai Leung*

Main category: cs.LG

TL;DR: The paper addresses memory bottlenecks in large language models due to KV cache by proposing RateQuant, a method for mixed-precision quantization that avoids distortion model mismatch.


<details>
  <summary>Details</summary>
Motivation: Current KV cache quantizers use uniform bit-widths, ignoring varying head importance, and mixed-precision allocation can worsen performance due to distortion model mismatch.

Method: RateQuant fits per-quantizer distortion models and solves bit-allocation via reverse waterfilling, ensuring optimal performance.

Result: RateQuant significantly reduces perplexity (70% reduction for KIVI and 6.6 PPL improvement for QuaRot) with minimal calibration time (1.6 s) and no inference overhead.

Conclusion: RateQuant effectively resolves distortion model mismatch, improving KV cache quantization efficiency without added inference costs.
...
### [28] [LKV: End-to-End Learning of Head-wise Budgets and Token Selection for LLM KV Cache Eviction](https://arxiv.org/abs/2605.06676)
*Enshuai Zhou,Yifan Hao,Chao Wang,Rui Zhang,Di Huang,Jiaming Guo,Xing Hu,Zidong Du,Qi Guo,Yunji Chen*

Main category: cs.LG

TL;DR: LKV introduces a learned approach to KV cache compression, outperforming heuristic methods by aligning compression with task objectives, achieving near-lossless performance with only 15% KV cache retention.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache compression methods rely on heuristics, leading to resource misallocation and suboptimal performance. The goal is to develop a task-optimized, data-driven solution.

Method: LKV formulates KV compression as a differentiable optimization problem, integrating LKV-H for budget learning and LKV-T to derive KV importance without materializing attention matrices.

Result: LKV achieves state-of-the-art performance on LongBench and RULER benchmarks, with near-lossless results at 15% KV cache retention.

Conclusion: Learned budgeting is crucial for overcoming heuristic limitations, demonstrating the effectiveness of data-driven KV compression.
...
### [29] [A Wasserstein GAN-based climate scenario generator for risk management and insurance: the case of soil subsidence](https://arxiv.org/abs/2605.06678)
*Antoine Heranval,Olivier Lopez,Didier Ngatcha,Daniel Nkameni*

Main category: cs.LG

TL;DR: The paper introduces SwiGAN, a Conditional GAN-based AI framework to simulate future drought patterns, aiding insurance strategies.


<details>
  <summary>Details</summary>
Motivation: The rising costs of natural disasters and the need for long-term insurance strategies drive the development of SwiGAN.

Method: Uses Conditional GANs to generate spatio-temporal trajectories of the Soil Wetness Index (SWI) for drought assessment.

Result: SwiGAN simulates plausible drought patterns up to 2050, supporting adaptive risk management and insurance planning.

Conclusion: The framework is effective for drought analysis and adaptable to other climate-related risks and actuarial uses.
...
### [30] [Breaking the Illusion: When Positive Meets Negative in Multimodal Decoding](https://arxiv.org/abs/2605.06679)
*Yubo Jiang,Yitong An,Xin Yang,Abudukelimu Wuerkaixi,Xuxin Cheng,Fengying Xie,Zhiguo Jiang,Cao Liu,Ke Zeng,Haopeng Zhang*

Main category: cs.LG

TL;DR: PND is a training-free framework that combats object hallucination in VLMs by contrasting positive and negative decoding paths to enhance visual fidelity.


<details>
  <summary>Details</summary>
Motivation: VLMs often rely too much on linguistic priors, leading to object hallucination, where generated content contradicts visual reality. Attention imbalance in VLMs, where visual features are under-weighted, was identified as a key issue.

Method: Positive-and-Negative Decoding (PND) intervenes in the decoding process: a positive path amplifies visual evidence, and a negative path constructs counterfactuals to penalize prior-dominant generation. Outputs from both paths are contrasted to steer visually grounded results.

Result: Experiments on datasets POPE, MME, and CHAIR show state-of-the-art performance in reducing object hallucination without requiring retraining.

Conclusion: PND effectively addresses attention imbalance in VLMs, improving visual fidelity and reducing reliance on linguistic priors through its dual-path contrast mechanism.
...
### [31] [On the Role of Strain and Vorticity in Numerical Integration Error for Flow Matching](https://arxiv.org/abs/2605.06680)
*Chenxi Tao,Seung-Kyum Choi*

Main category: cs.LG

TL;DR: Flow matching integrates learned velocity fields for data generation, with integration steps (NFE) affecting cost. Analysis shows strain (S) and vorticity (Omega) roles differ: strain amplifies errors exponentially, while vorticity contributes linearly. Optimal transport fields are irrotational, enabling second-order accuracy. Jacobian regularization with weights (alpha, beta) reduces error (2.7x at NFE=5) and improves FID by 14% at NFE=10.


<details>
  <summary>Details</summary>
Motivation: Understand the role of strain and vorticity in velocity fields to minimize integration error and improve efficiency in flow matching systems.

Method: Decompose velocity Jacobian into strain (S) and vorticity (Omega), analyze their error contributions, and apply weighted Jacobian regularization (weights alpha, beta).

Result: Strain governs exponential error, vorticity contributes linearly. Optimal transport fields achieve second-order accuracy. Experiments show reduced error (2.7x at NFE=5) and FID improvement (14% at NFE=10).

Conclusion: Targeting strain and vorticity via Jacobian regularization enhances flow matching efficiency, reducing error and improving performance at lower NFE.
...
### [32] [A Hierarchical Ensemble Pipeline for Anomaly Detection in ESA Satellite Telemetry](https://arxiv.org/abs/2605.06681)
*Lorenzo Riccardo Allegrini,Geremia Pompei*

Main category: cs.LG

TL;DR: A hierarchical ensemble method for anomaly detection in ESA telemetry data uses shapelet and statistical features, achieving strong generalization on the ESA-ADB benchmark.


<details>
  <summary>Details</summary>
Motivation: To detect subtle anomalies in multivariate satellite telemetry data effectively.

Method: Hierarchical pipeline integrating shapelet-based and statistical feature extraction, per-channel modeling, intra-channel stacking, and cross-channel aggregation.

Result: Demonstrates strong generalization on the ESA-ADB challenge.

Conclusion: Hierarchical modeling is effective for detecting anomalies in satellite telemetry.
...
### [33] [Toeplitz MLP Mixers are Low Complexity, Information-Rich Sequence Models](https://arxiv.org/abs/2605.06683)
*Benjamin L. Badger,Ethan Roland*

Main category: cs.LG

TL;DR: The paper introduces the Toeplitz MLP Mixer (TMM), a transformer-like architecture that reduces computational complexity by replacing attention with Toeplitz matrix multiplication, achieving better efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Transformers are limited by quadratic computational complexity. The authors aim to develop a more efficient alternative with sub-quadratic complexity while maintaining performance.

Method: The TMM replaces attention with triangular-masked Toeplitz matrix multiplication, reducing time and space complexity during training and inference.

Result: TMMs achieve greater training efficiency, better input information retention, improved copying ability, and superior benchmark accuracy in information retrieval and in-context learning.

Conclusion: TMMs outperform comparable architectures in efficiency and performance, with trained Toeplitz layers showing surprising invertibility properties.
...
### [34] [From Canopy to Collision: A Hybrid Predictive Framework for Identifying Risk Factors in Tree-Involved Traffic Crashes](https://arxiv.org/abs/2605.06684)
*Abdul Azim,Ahmed Hossain,Soumyadip Maitra,Panick Kalambay*

Main category: cs.LG

TL;DR: The study analyzes tree-involved run-off-road crashes using a machine learning and statistical framework to identify key risk factors, finding restraint non-use, vehicle age, speeding, and driver impairment as top predictors of severe outcomes.


<details>
  <summary>Details</summary>
Motivation: Tree-involved crashes often lead to severe or fatal injuries, highlighting the need to understand and mitigate risk factors to improve road safety.

Method: A multi-step process involving CatBoost for classification, SHAP for factor importance, logistic regression for validation, and interaction analysis.

Result: Restraint non-use is the top predictor, with vehicle age, speeding, and driver impairment also significant. Key interactions between factors further influence severity.

Conclusion: The findings support targeted interventions like seat belt enforcement, speed management in poor lighting, and fleet modernization to reduce crash severity.
...
### [35] [Conformal Agent Error Attribution](https://arxiv.org/abs/2605.06788)
*Naihe Feng,Yi Sui,Shiyi Hou,Ga Wu,Jesse C. Cresswell*

Main category: cs.LG

TL;DR: The paper introduces a framework for error attribution in multi-agent systems (MAS) using conformal prediction (CP) to isolate errors and enable automated recovery.


<details>
  <summary>Details</summary>
Motivation: Error attribution in MAS is challenging due to long interaction traces. Existing methods lack precision and efficiency in identifying decisive errors for recovery.

Method: The paper proposes new algorithms for filtration-based CP tailored for sequential data like agent trajectories, predicting contiguous sequences for efficient debugging.

Result: The approach isolates errors precisely, allowing MAS to rollback and correct errors, and is verified across various agents and datasets.

Conclusion: The framework provides model-agnostic, principled uncertainty guarantees for MAS error attribution, with practical code released for implementation.
...
### [36] [Robustness of Refugee-Matching Gains to Off-Policy Evaluation Choices](https://arxiv.org/abs/2605.06686)
*Kirk Bansak,Elisabeth Paulson,Dominik Rothenhäusler,Jeremy Ferwerda,Jens Hainmueller,Michael Hotard*

Main category: cs.LG

TL;DR: This paper evaluates the stability of counterfactual impact results in refugee matching using various off-policy methods, confirming consistency with prior research.


<details>
  <summary>Details</summary>
Motivation: To assess the reliability of counterfactual impact evaluation in refugee matching, ensuring robust results for policy applications.

Method: Uses inverse probability weighting (IPW) and augmented IPW (AIPW) variants, testing different models and assignment procedures.

Result: Impact estimates are consistent in magnitude and statistically significant, aligning with Bansak et al. (2018).

Conclusion: The findings affirm the stability and robustness of counterfactual impact evaluation methods in refugee matching.
...
### [37] [GraphDC: A Divide-and-Conquer Multi-Agent System for Scalable Graph Algorithm Reasoning](https://arxiv.org/abs/2605.06671)
...
### [38] [More Thinking, More Bias: Length-Driven Position Bias in Reasoning Models](https://arxiv.org/abs/2605.06672)
*Xiao Wang*

Main category: cs.AI

TL;DR: The paper examines position bias in reasoning-capable models, finding it scales with reasoning trajectory length rather than reducing heuristic biases.


<details>
  <summary>Details</summary>
Motivation: To investigate if chain-of-thought reasoning and reasoning-tuned models actually reduce shallow heuristic biases like position bias in multiple-choice QA tasks.

Method: Analyzed thirteen reasoning-mode configurations across models (including DeepSeek-R1) on datasets like MMLU, ARC-Challenge, and GPQA, measuring Position Bias Score (PBS) and its correlation with reasoning trajectory length. Used a truncation intervention for causal evidence.

Result: Twelve of thirteen configurations showed positive partial correlation between PBS and reasoning trajectory length. A truncation intervention confirmed causal evidence of increasing bias with trajectory length. Direct-answer bias was found to be distinct and unrelated to trajectory length.

Conclusion: Reasoning-capable models exhibit position bias influenced by reasoning trajectory length, suggesting they shouldn't be assumed order-robust. The study provides tools (PBS, truncation probes) for auditing such biases.
...
### [39] [Fast and Effective Redistricting Optimization via Composite-Move Tabu Search](https://arxiv.org/abs/2605.06682)
*Hai Jin,Diansheng Guo*

Main category: cs.AI

TL;DR: A composite-move Tabu search (CM-Tabu) method is introduced to address spatial redistricting challenges, improving solution quality and efficiency by expanding feasible neighborhoods while preserving contiguity.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for high-quality, rapid, and flexible solutions for spatial redistricting, particularly overcoming the contiguity constraint that limits traditional methods.

Method: CM-Tabu systematically expands the feasible neighborhood in Tabu search by identifying minimal sets of units or pairs that can move or switch while preserving contiguity, using linear-time analysis of contiguity graphs.

Result: The method significantly enhances solution quality, robustness, and efficiency, achieving global optima in cases like Philadelphia and supporting multi-criteria trade-offs.

Conclusion: CM-Tabu is effective for real-world spatial redistricting, offering improved performance for decision-support workflows.
...
### [40] [State Representation and Termination for Recursive Reasoning Systems](https://arxiv.org/abs/2605.06690)
*Debashis Guha,Amritendu Mukherjee,Sanjay Kukreja,Tarun Kumar*

Main category: cs.AI

TL;DR: The paper introduces a method to represent and evaluate recursive reasoning systems by using an epistemic state graph and the order-gap criterion to determine when to stop iterating.


<details>
  <summary>Details</summary>
Motivation: To address implicit design choices in recursive reasoning systems—how to represent the evolving reasoning state and when to stop iterating—by providing explicit solutions.

Method: Represent the reasoning state as an epistemic state graph and introduce the order-gap criterion to evaluate when further iteration is unnecessary.

Result: A necessary and sufficient condition is derived for the linearised order-gap to be non-degenerate near the fixed point, ensuring the criterion is meaningful.

Conclusion: The framework is applicable to various domains like agent loops, tree-of-thought reasoning, theorem proving, and continual learning, providing a local condition for stopping iteration.
...
### [41] [Hidden Coalitions in Multi-Agent AI: A Spectral Diagnostic from Internal Representations](https://arxiv.org/abs/2605.06696)
*Cameron Berg,Susan L. Schneider,Mark M. Bailey*

Main category: cs.AI

TL;DR: The paper introduces a method to detect coalition structures in multi-agent systems by analyzing internal neural representations, validated in reinforcement learning and large language model settings.


<details>
  <summary>Details</summary>
Motivation: To distinguish genuine informational coupling from spurious similarity in AI agent coalitions, as overt behavior alone is insufficient for detection.

Method: Constructs a pairwise mutual-information graph from agents' hidden states and uses spectral partitioning to identify coalition boundaries.

Result: The method successfully identifies programmed and dynamic coalition structures, rejects false positives, and reveals representational hierarchies in large language models.

Conclusion: Analyzing hidden-state mutual information with spectral partitioning is a scalable tool for monitoring emergent structures in distributed AI systems.
...
TL;DR: This paper introduces CASCADE, a framework for deployment-time learning (DTL) in LLMs, enabling agents to improve from experience during deployment without parameter updates. It achieves a 20.9% boost in success rate across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: The rigid separation between training and deployment in LLMs limits their ability to adapt continuously, unlike natural intelligence. This work aims to bridge this gap by enabling learning during deployment.

Method: The paper proposes CASCADE, a framework for LLM agents with episodic memory, treating experience reuse as a contextual bandit problem to balance exploration and exploitation.

Result: CASCADE improves the macro-averaged success rate by 20.9% over zero-shot prompting and outperforms gradient-based and memory-based baselines across 16 tasks.

Conclusion: By reframing deployment as a learning process, this work lays the groundwork for AI systems that can continually improve through interaction.
...
### [43] [From Storage to Experience: A Survey on the Evolution of LLM Agent Memory Mechanisms](https://arxiv.org/abs/2605.06716)
*Jinghao Luo,Yuchen Tian,Chuxue Cao,Ziyang Luo,Hongzhan Lin,Kaixin Li,Chuyi Kong,Ruichao Yang,Jing Ma*

Main category: cs.AI

TL;DR: This paper proposes a three-stage evolutionary framework (Storage, Reflection, Experience) for LLM agent memory mechanisms, addressing long-range consistency, dynamic environments, and continual learning, with a focus on proactive exploration and cross-trajectory abstraction.


<details>
  <summary>Details</summary>
Motivation: Current research on LLM agent memory mechanisms is fragmented, lacking a unified view. This paper aims to bridge the gap by offering a coherent evolutionary framework.

Method: The study formalizes memory mechanism development into three stages—Storage, Reflection, and Experience—and analyzes core drivers like long-range consistency and continual learning.

Result: The framework provides design principles and a roadmap for next-generation LLM agents, highlighting transformative mechanisms like proactive exploration.

Conclusion: The paper offers a unified, evolutionary perspective for advancing LLM agent memory mechanisms, facilitating future development in the field.
...
### [44] [When Does a Language Model Commit? A Finite-Answer Theory of Pre-Verbalization Commitment](https://arxiv.org/abs/2605.06723)
*Long Zhang,Wei-neng Chen,Feng-feng Wei,Zi-bo Qin*

Main category: cs.AI

TL;DR: The paper studies how language models stabilize their answer preferences before revealing the final answer, focusing on finite-answer preference stabilization.


<details>
  <summary>Details</summary>
Motivation: To understand when and how language models solidify their answer preferences during reasoning, using a computable framework.

Method: Uses finite-answer preference stabilization, projecting model continuation probabilities onto a finite answer set, and defines metrics like answer onset and stabilization time. Experiments are conducted with Qwen3-4B-Instruct.

Result: The model's answer projection stabilizes before the answer is parseable, showing a 17--31 token lead. The signal reflects eventual output, not truth, and is linearly recoverable from hidden summaries.

Conclusion: The framework provides insights into model reasoning but does not reliably control generation, highlighting local sensitivity of preferences.
...
### [45] [Weblica: Scalable and Reproducible Training Environments for Visual Web Agents](https://arxiv.org/abs/2605.06761)
*Oğuzhan Fatih Kar,Roman Bachmann,Yuanzheng Gong,Anders Boesen Lindbo Larsen,Afshin Dehghan*

Main category: cs.AI

TL;DR: The paper proposes Weblica, a framework for creating reproducible and scalable web environments to train visual web agents, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The complexity and diversity of the web make it difficult to scale training data for visual web agents, as existing methods rely on limited offline or simulated environments.

Method: Weblica uses HTTP-level caching to replay stable visual states and LLM-based environment synthesis grounded in real-world websites and navigation skills.

Result: The framework enables RL training in thousands of diverse environments, with Weblica-8B outperforming similar-sized baselines and performing competitively with API models.

Conclusion: Weblica effectively addresses scalability and diversity challenges in training web agents, demonstrating superior performance and efficiency.
...
### [46] [Beyond the Black Box: Interpretability of Agentic AI Tool Use](https://arxiv.org/abs/2605.06890)
*Hariom Tatsat,Ariye Shater*

Main category: cs.AI

TL;DR: The paper introduces a mechanistic-interpretability toolkit to enhance internal observability of AI agents' tool-use decisions, aiming to reduce failures and risks in enterprise workflows.


<details>
  <summary>Details</summary>
Motivation: AI agents in enterprise workflows often fail in tool-use, which is costly in long-horizon settings. Existing methods lack internal visibility into decision-making processes.

Method: The toolkit uses Sparse Autoencoders (SAEs) and linear probes to analyze model states before actions, identifying layers and features linked to tool decisions. It trains on the NVIDIA Nemotron dataset and tests on GPT-OSS 20B and Gemma 3 27B models.

Result: The framework provides insights into internal signals before actions, helping diagnose failures and mitigate risks, especially in long-horizon interactions.

Conclusion: Mechanistic interpretability can enhance practical observability for monitoring tool calls and risks in AI agent systems, complementing external evaluations.
