...
TL;DR: ReactiveGWM introduces a reactive game world model that dynamically simulates player-NPC interactions, decoupling player controls from NPC behaviors for scalable, strategy-rich interactions.


<details>
  <summary>Details</summary>
Motivation: Current game world models fail to capture meaningful player-NPC interactions due to their subjective, player-centric design, treating NPCs as passive elements rather than reactive entities.

Method: ReactiveGWM uses a diffusion backbone with lightweight additive biases for player actions and cross-attention modules for NPC responses, learning game-agnostic interactive logic.

Result: The model enables zero-shot strategy transfer across games, maintaining fine-grain player controllability and robust NPC strategy adherence.

Conclusion: ReactiveGWM paves the way for scalable, strategy-rich NPC interactions by decoupling player and NPC dynamics and using game-agnostic representations.
...
TL;DR: Deep Pre-Alignment (DPA) improves Vision Language Models (VLMs) by replacing the standard ViT encoder with a small VLM as perceiver, ensuring visual features are deeply aligned with the text space of the target LLM, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs suffer from an alignment challenge where visual features remain distant from the text space in initial LLM layers, wasting depth on superficial alignment instead of deep reasoning.

Method: Proposes DPA, a novel architecture using a small VLM as perceiver to deeply align visual features with the text space of the target LLM.

Result: DPA outperforms baselines by 1.9 points on 8 benchmarks at 4B scale, widening to 3.0 points at 32B scale, and reduces language capability forgetting by 32.9% over 3 text benchmarks.

Conclusion: DPA effectively addresses alignment challenges, enhances performance across scales and benchmarks, and offers a seamless upgrade path for VLM development with minimal overhead.
...
TL;DR: The paper critiques FID as a saturated metric for image generation and advocates for precision and recall as complementary measures. It introduces RTM, an iterative refinement method, which improves both quality and diversity, outperforming current state-of-the-art approaches.


<details>
  <summary>Details</summary>
Motivation: Current image generation models often rely on FID, which is flawed as it conflates fidelity and mode coverage. Mode collapse can occur even with low FID, highlighting the need for better metrics and methods prioritizing diversity.

Method: Introduces RTM, replacing single-pass latent mapping in style-based generators with iterative refinement. Integrated with IMLE, it optimizes mode coverage while maintaining quality.

Result: RTM achieves highest precision and recall among state-of-the-art methods while maintaining competitive FID. Improvements shown across CIFAR-10, CelebA-HQ, and few-shot benchmarks.

Conclusion: Recursive refinement (RTM) enhances both quality and diversity, addressing flaws in existing metrics and methods. It outperforms baselines without sacrificing coverage.
...
TL;DR: COPRA is a dynamic parameter adaptation framework for VLM-based video anomaly detection, addressing mismatches in data distribution and model configuration, and showing strong performance across various tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based VAD methods suffer from mismatches between training and inference, limiting generalization under distribution shifts and inconsistencies in data sampling.

Method: COPRA generates input-specific parameter updates to dynamically adapt a frozen VLM for each video segment during training and inference.

Result: COPRA outperforms static baselines in both in-domain and cross-domain settings and generalizes to unseen tasks like Video QA and Dense Captioning.

Conclusion: COPRA is an effective framework for scalable and adaptive video understanding, with code to be released publicly.
...
TL;DR: A multimodal pipeline combining LiDAR, visible-thermal fusion, and Airborne Optical Sectioning (AOS) improves human detection under forest canopy, showing potential for UAV-based search-and-rescue.


<details>
  <summary>Details</summary>
Motivation: Reliable human detection in forested areas is challenging due to occlusion and sparse visibility. This work aims to address these issues for UAV-based applications like search-and-rescue.

Method: Three approaches were integrated: LiDAR evaluation for active sensing feasibility, visible-thermal fusion for enhanced saliency, and AOS for clutter suppression. YOLOv5 was fine-tuned on thermal data.

Result: LiDAR penetration was limited; visible-thermal fusion improved low-contrast detection; AOS enhanced ground-plane detection. Fine-tuned YOLOv5 achieved ~0.83 mAP on FLIR thermal data.

Conclusion: The pipeline provides a baseline for UAV-deployable systems in forests, encouraging future work on dedicated datasets and real-time integration.
...
TL;DR: Minerva-Ego is a new benchmark for evaluating complex egocentric visual reasoning, featuring multi-step multimodal questions and dense human-annotated reasoning traces. It reveals a performance gap between models and humans, with hints improving model performance.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks lack evaluation of intermediate reasoning steps and often limit answers to text. Minerva-Ego addresses this by providing detailed reasoning traces and multimodal questions for egocentric settings.

Method: The benchmark extends high-quality egocentric video data with challenging questions, dense reasoning traces, and spatiotemporal mask annotations. Experiments compare state-of-the-art models to human performance.

Result: State-of-the-art models significantly lag behind humans in performance. Providing 'where' and 'when' hints improves model accuracy.

Conclusion: Minerva-Ego highlights the need for better reasoning in video models and offers a tool for future research, with hints being a promising direction for improvement.
...
TL;DR: GCNNs use dense sampling of transformation groups to maintain equivariance, but this is computationally expensive. The paper proposes feature-space sampling to reduce costs without losing accuracy, enabling faster training for equivariant 3D classifiers.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of dense sampling in GCNNs, especially for complex transformations like 3D rotations, limits practical applications.

Method: Replace dense geometric sampling with feature-space sampling, using representative samples selected by feature similarity to reduce computational effort.

Result: Coarse feature-space sampling preserves classification accuracy well, allowing precomputation and faster training for equivariant 3D classifiers.

Conclusion: Feature-space sampling offers an efficient way to balance computational cost and accuracy in GCNNs, particularly for 3D tasks.
...
TL;DR: ChangeFlow is a generative framework for remote sensing change detection that reformulates the task as synthesizing change masks in latent space, improving robustness and consistency over discriminative methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with context-dependent and ambiguous change masks due to per-pixel classification and lack of global consistency modeling.

Method: ChangeFlow uses a generative approach with rectified flow in latent space, guided by a lightweight conditioning signal, and supports sampling-based ensembling.

Result: ChangeFlow achieves an average F1 of 80.4%, outperforming previous methods by 1.3 points while maintaining comparable inference speed.

Conclusion: The generative framework of ChangeFlow addresses limitations of discriminative methods, offering improved performance and practical confidence estimation for ambiguous regions.
...
TL;DR: MorphoHELM is introduced as a benchmark to fairly evaluate feature extraction methods for Cell Painting, addressing fragmented evaluation standards in microscopy image analysis.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized evaluation for deep learning methods in morphological profiling assays like Cell Painting makes fair comparisons difficult.

Method: MorphoHELM consolidates, extends, and corrects existing evaluation standards, testing methods across varying levels of batch effects and technical noise.

Result: No single model outperforms classic computer vision strategies across all settings, with trade-offs between methods observed.

Conclusion: Classic computer vision methods remain robust for general use, and MorphoHELM provides an open benchmark for future research.
...
TL;DR: PanoWorld is a panoramic video world model generating geometry-consistent 360° videos from a single image and caption, addressing inconsistencies in prior methods through geometric and dynamics-aware modeling.


<details>
  <summary>Details</summary>
Motivation: Existing panoramic video methods focus on visual realism but often produce inconsistent depth, broken correspondences, and implausible motion, limiting their utility for embodied AI applications requiring holistic spatial understanding.

Method: PanoWorld introduces depth and trajectory consistency losses, spherical-geometry-aware adaptations, and leverages a pre-trained perspective video world model. It also uses the PanoGeo dataset for training and evaluation.

Result: PanoWorld improves geometric consistency over prior methods while maintaining visual realism, validating its approach to panoramic video generation as a geometric modeling problem.

Conclusion: Panoramic video generation must prioritize geometric modeling to meet the spatial understanding needs of embodied AI, as demonstrated by PanoWorld's success.
...
TL;DR: ELDOR is a UAV benchmark for monitoring illegal gold mining in the Amazon, featuring annotated imagery for semantic segmentation and related tasks, showing gaps in detecting small structures and suggesting multimodal solutions.


<details>
  <summary>Details</summary>
Motivation: Illegal gold mining causes ecological harm but is hard to monitor with satellites due to scale and cloud issues. UAVs can provide finer-scale data.

Method: ELDOR uses manually annotated UAV orthomosaics (2,500+ hectares) for four benchmark tasks: semantic segmentation, segmentation-derived recognition, multi-label classification, and class-presence recognition. Various models are tested.

Result: Current methods struggle with rare small mining structures and fine recovery classes, highlighting the need for context-aware and multimodal approaches.

Conclusion: ELDOR addresses monitoring gaps and suggests improving methods for small-scale detection, with an interactive tool for practical use.
...
TL;DR: The paper investigates under-studied aspects of uncertainty estimation in segmentation, examining factors like task difficulty, dataset variability, and method combinations to improve performance.


<details>
  <summary>Details</summary>
Motivation: To identify key variables affecting uncertainty estimation in segmentation tasks, addressing gaps in understanding how different factors influence performance and practical utility.

Method: A large-scale study across datasets, backbones, and tasks (semantic and panoptic segmentation), evaluating uncertainty estimation under varied conditions and combining approaches.

Result: Panoptic segmentation underperforms; time series samples are limited; sample diversity aids calibration; ensembles outperform deterministic networks under specific conditions.

Conclusion: Uncertainty estimation effectiveness varies by task and context; ensembles offer advantages but require careful deployment, while simpler methods suffice in some cases.
...
TL;DR: MR2-ByteTrack introduces an efficient Video Object Detection method for low-power MCUs, reducing computational costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of ultra-low-power MCUs in handling video object detection due to memory and compute constraints, while avoiding impractical cloud computing solutions.

Method: MR2-ByteTrack alternates between full- and low-resolution inference, links detections across frames using ByteTrack, and corrects misclassifications with the Rescore algorithm. It’s applied to both CNN- and Transformer-based models.

Result: Achieves mAP scores up to 49.0 for CNNs and 48.7 for Transformers, reduces operations by 53% (CNN) and 32% (Transformer), and yields 55% energy savings on a GAP9 MCU.

Conclusion: The method enables real-time Transformer-based VOD on MCU-class devices with significant energy savings, marking a practical advancement for embedded vision systems.
...
TL;DR: Social-Mamba is a human trajectory forecasting model that efficiently handles social interactions through a novel structured sequential process, achieving state-of-the-art accuracy with computational scalability.


<details>
  <summary>Details</summary>
Motivation: Existing models for human trajectory forecasting struggle with computational inefficiency and misalignment with the dynamic nature of social interactions, necessitating a more scalable and accurate solution.

Method: Social-Mamba introduces the Cycle Mamba block for bidirectional information flow, organizes agents on an egocentric grid, and uses social triplet factorization to decompose interactions into temporal, egocentric, and goal-centric scans. A learnable social gate and global scan integrate these dynamically.

Result: Social-Mamba outperforms existing methods on five trajectory forecasting benchmarks, offering superior accuracy, parameter efficiency, and computational scalability. It also enhances performance when embedded in a flow-matching framework.

Conclusion: Social-Mamba provides a robust and flexible foundation for future trajectory forecasting research, combining accuracy with computational efficiency. The code is publicly available.
...
TL;DR: The paper introduces a novel approach to Concealed Object Segmentation (COS) using Retinex theory for homogeneous image decomposition, demonstrating improved discriminability and proposing the RIDE framework with specialized modules for effective segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for COS tasks either work directly on RGB images or use heterogeneous decompositions, which can obscure pixel-aligned cues. The paper aims to address this by using Retinex theory to decompose images into illumination and reflectance components in the same spatial domain.

Method: The proposed method, RIDE, includes: (1) a Task-Driven Retinex Decomposition module for learning optimal factorizations, (2) a Discriminability Gap Attention mechanism to adaptively leverage decomposition benefits, and (3) a Camouflage-Breaking Contrastive loss in reflectance feature space.

Result: The paper demonstrates that Retinex decomposition preserves or improves foreground-background discriminability across COS sub-tasks, with anti-correlation maximizing gains. The RIDE framework effectively leverages these insights.

Conclusion: The study provides a theoretical foundation and practical framework for improving COS tasks by exploiting the Discriminability Gap Theorem and Retinex decomposition, showing consistent advantages over existing methods.
...
TL;DR: VideoRLVR optimizes video diffusion models using rule-based feedback, improving performance in tasks requiring verifiable spatial, temporal, or logical constraints.


<details>
  <summary>Details</summary>
Motivation: Video diffusion models lack verifiable reasoning, limiting their use in tasks with explicit constraints. VideoRLVR addresses this by integrating reinforcement learning with verifiable rewards.

Method: VideoRLVR uses an SDE-GRPO optimization backbone, dense decomposed rewards, and an Early-Step Focus strategy to efficiently train models for verifiable reasoning.

Result: VideoRLVR outperforms baselines in Maze, FlowFree, and Sokoban tasks, especially in low-success-rate settings, and surpasses other video generation models on verifiable reasoning benchmarks.

Conclusion: Verifiable reinforcement learning can enhance video models, shifting focus from perceptual realism to reliable, rule-consistent reasoning.
...
TL;DR: IA-JEPA, a novel motion-centric masking strategy, improves causal reasoning in self-supervised learning by focusing on physical interactions.


<details>
  <summary>Details</summary>
Motivation: Standard Joint Embedding Predictive Architectures (JEPA) often fail to capture causal dynamics, limiting their ability to model physical interactions.

Method: Proposed Interaction-Aware JEPA (IA-JEPA) uses a self-supervised motion-centric masking strategy to prioritize physical interactions like collisions or momentum transfers.

Result: IA-JEPA achieves 14.26% accuracy on causal reasoning tasks (vs. 3.22% for baselines) and improves latent space discriminability (+10% entropy gain).

Conclusion: IA-JEPA offers a scalable, self-supervised approach to build world models that internalize causal structure, demonstrated on benchmarks like CLEVRER and PHYRE-Lite.
...
TL;DR: The paper introduces an empowered t-FCW graph representation for point clouds, analyzing its effectiveness and designing a network that uses it exclusively. It shows robustness and interpretability, achieving fast processing times.


<details>
  <summary>Details</summary>
Motivation: The original t-FCW showed promise for point cloud classification, but its effectiveness and broader applicability were unclear. This work aims to analyze and enhance its properties.

Method: The authors analyze the properties of empowered and original t-FCW, design a network using empowered t-FCW as feature extractors, and build memory banks for classification, part segmentation, and semantic segmentation.

Result: Empowered t-FCW inherits robustness from surface descriptors and provides interpretability. The network processes ModelNet40 classification in ~7 seconds on an NVIDIA RTX A5000 GPU.

Conclusion: Empowered t-FCW is effective, efficient, and interpretable, serving as both a standalone baseline and a plug-in for deep models.
...
TL;DR: The paper proposes a method to convert exocentric video to egocentric video using body pose extraction and human kinematics, improving egocentric world model training and performance.


<details>
  <summary>Details</summary>
Motivation: Egocentric data is scarce and limited, while exocentric video is abundant but not directly usable for egocentric models. The goal is to bridge this gap.

Method: Extract body poses from exocentric video, transform it to egocentric video using human kinematics, and use this data to train egocentric world models.

Result: Training with converted data improves prediction and planning performance in egocentric world models.

Conclusion: This method enables leveraging abundant exocentric videos for egocentric model training, advancing applications in robotics and AR.
...
TL;DR: Thepaperexaminestheeffectivenessofsparsetop-$k$routingwithhardcapacityconstraintsinvisionclassification,highlightingacompute-leveragepatternandthenecessityofmulti-expertroutingatscale.


<details>
  <summary>Details</summary>
Motivation: ToaddressthelimitationsofMixture-of-Experts(MoE)networksinvisiondeployments,suchasexpertcollapseandinefficiency,bystudyingsparseroutinginvisiontasks.

Method: Evaluatessparsetop-$k$routingundermulti-seedprotocolsonfourbenchmarks,withcontrolledexperimentstoisolatefactorslikecomputeleverageandmulti-expertrouting.

Result: Positiveaccuracyrequiressignificantcomputerouting($ρ$)andmulti-expertrouting($k\geq2$)atImageNetscale.SoftMoEvariantoutperformsdensebaselinesinper-sampleCNNsettings.

Conclusion: Sparseroutinginvisiontasksbenefitsfromcomputeleverageandmulti-expertcollaboration,withSoftMoEvariantsofferingimprovementsovertraditionalmethods.
...
TL;DR: The paper introduces AnyAct, a method to derive human reenactments from monocular videos of non-human characters by focusing on sparse local articulated motion cues.


<details>
  <summary>Details</summary>
Motivation: The work aims to reinterpret non-human character motions into plausible human performances, addressing challenges in video-based motion capture and motion retargeting for non-human sources.

Method: AnyAct uses conditional human motion generation from transferable sparse local 2D articulated motion, featuring human-motion-only supervision, progressive 3D-to-2D training, and global-local motion decoupling.

Result: Experiments show AnyAct produces high-fidelity human reenactments that preserve the dynamics of non-human characters, with ablation studies confirming the method's core designs.

Conclusion: AnyAct effectively bridges the gap between non-human character videos and human reenactments, offering a practical solution for animation authoring.
...
TL;DR: DiffVAS is a visual active search framework using diffusion models and target-conditioned reinforcement learning to search for multiple objects in partially observable environments, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current visual active search (VAS) approaches are limited by assumptions of complete prior knowledge and single-target policies, which are impractical for real-world applications like wildlife protection and search-and-rescue.

Method: DiffVAS employs a diffusion model to reconstruct geospatial areas from partial observations and uses target-conditioned reinforcement learning for adaptive planning.

Result: Experiments show DiffVAS significantly outperforms state-of-the-art methods in searching diverse objects in partially observable settings.

Conclusion: DiffVAS advances VAS by enabling multi-target searches in realistic, partially observable environments, enhancing its practical applicability.
...
TL;DR: The paper introduces a self-prompting method for scene text editing that avoids reliance on pre-trained encoders and leverages a diffusion transformer for style-consistent, open-vocabulary text modification.


<details>
  <summary>Details</summary>
Motivation: Existing methods for scene text editing fail to preserve stylistic features of original text and are limited by pre-trained encoders, prompting the development of a more flexible approach.

Method: The proposed method uses a two-stage training strategy with a Multi-Modal Diffusion Transformer (MM-DiT) to construct style and glyph prompts directly from the image, enabling open-vocabulary editing.

Result: The method achieves state-of-the-art performance in text accuracy and style consistency across various languages.

Conclusion: The self-prompting approach effectively addresses limitations of existing methods, offering improved flexibility and performance in scene text editing.
...
TL;DR: Tuning-free video editing with SNIS and NGM improves visual quality and performance.


<details>
  <summary>Details</summary>
Motivation: Video editing is challenging due to underutilization of noisy latent information in existing tuning-free methods, leading to unsatisfactory results.

Method: Proposes a tuning-free, instruction-based video editing framework with Structural Noise Initialization Strategy (SNIS) and Noise Guidance Mechanism (NGM) to optimize noise levels and guide denoising.

Result: Achieves better visual quality and state-of-the-art performance in experiments.

Conclusion: The framework effectively utilizes noisy latent information for superior video editing outcomes.
...
TL;DR: The paper proposes DSS-USOD, a method for underwater salient object detection (USOD) that addresses challenges like image degradation by dynamically coordinating boundary-sensitive and region-coherent features.


<details>
  <summary>Details</summary>
Motivation: Existing USOD methods struggle with underwater image degradations, leading to inaccurate localization, fragmented regions, and coarse boundaries.

Method: DSS-USOD extracts a shared base representation, decomposes it into boundary-sensitive and region-coherent features, and uses a spatial coordination module to adaptively regulate their contributions.

Result: Extensive experiments show DSS-USOD achieves superior performance on benchmark datasets, and real-world deployment on an underwater robot validates its effectiveness.

Conclusion: DSS-USOD effectively balances boundary precision and region coherence under degraded underwater conditions, demonstrating practical utility for underwater object inspection.
...
TL;DR: MI-CXR is a benchmark for evaluating multi-interval reasoning in longitudinal chest X-ray sequences, revealing limitations in current vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack focus on longitudinal reasoning in medical imaging, prompting the creation of MI-CXR to assess multi-visit CXR analysis.

Method: MI-CXR uses five-way multiple-choice questions over five-visit patient timelines, testing three task families: Temporal Event Localization, Interval-wise Change Reasoning, and Global Trajectory Summarization.

Result: Evaluation of 14 state-of-the-art VLMs showed low accuracy (29.3%), with models struggling to enforce temporal constraints or compose evidence globally.

Conclusion: MI-CXR highlights key limitations in current VLMs and serves as a benchmark for advancing longitudinal medical reasoning.
...
TL;DR: The paper introduces the LDGuid framework, which explicitly learns and injects semantic differences into change detection models, improving robustness and performance, especially in noisy settings.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for change detection struggle to explicitly represent task-relevant semantic differences, limiting their robustness and accuracy.

Method: The LDGuid framework uses adversarial autoencoding and an information bottleneck to pretrain a difference embedding module, which learns task-relevant differences. These differences are then used as guidance in CD models like U-Net, BIT, and AERNet.

Result: LDGuid improves segmentation performance across multiple datasets (LEVIR-CD, WHU-CD, SVCD, CaBuAr), with significant gains in noisy conditions and better incorporation of domain-specific knowledge.

Conclusion: Semantic difference learning, as demonstrated by LDGuid, can substantially enhance the robustness and performance of change detection models in remote sensing.
...
TL;DR: Proposes a method for 3D human pose estimation from a single view without 3D supervision, using 2D diffusion priors and conditional multi-view ancestral sampling (cMAS).


<details>
  <summary>Details</summary>
Motivation: To address the challenge of estimating 3D human poses from a single view when 3D supervision is unavailable, leveraging pre-trained 2D motion diffusion models for improved accuracy.

Method: Extends multi-view ancestral sampling of diffusion models to 2D-3D lifting, introducing conditional multi-view ancestral sampling (cMAS) to optimize 3D poses while ensuring their projections follow 2D manifold constraints and anatomical limits.

Result: Achieves superior cross-domain performance on the Yoga dataset compared to state-of-the-art supervised and unsupervised methods, especially for extreme poses lacking 3D supervision.

Conclusion: The proposed cMAS method effectively leverages 2D diffusion priors for 3D pose estimation, demonstrating strong performance without 3D supervision.
...
TL;DR: The paper introduces Adaptive Geodesic Correction (AGC), a training-free defense for CLIP models against adversarial attacks, leveraging effective data augmentations for robust feature alignment.


<details>
  <summary>Details</summary>
Motivation: CLIP models are vulnerable to adversarial perturbations, and existing defenses are computationally expensive. The study explores the potential of specific data augmentations to enhance robustness.

Method: AGC identifies reliable augmentations as geometric anchors and corrects input features adaptively, without parameter updates, balancing robustness and accuracy.

Result: AGC improves robust accuracy by 44.4% over baselines and reduces inference latency by 10x across eight datasets and three CLIP backbones.

Conclusion: AGC reveals a geometric property of CLIP features, providing an efficient and effective solution for robust multimodal deployment.
...
TL;DR: The paper introduces a decoupled framework for few-step image generation, improving efficiency and quality over existing methods like Sphere Encoder.


<details>
  <summary>Details</summary>
Motivation: Existing few-step image generation methods face training instability, scalability issues, and computational inefficiency due to repeated pixel-space operations and conflicting objectives.

Method: The proposed approach decouples the framework into a fixed pretrained image encoder and a separate latent denoising model trained entirely in a spherical latent space, eliminating repeated pixel-space operations.

Result: The method outperforms Sphere Encoder in generation quality and inference speed on datasets like Animal-Faces, Oxford-Flowers, and ImageNet-1K, while competing with strong baselines.

Conclusion: Decoupling the framework improves efficiency and specialization, addressing the limitations of existing few-step image generation methods.
...
TL;DR: COVER is a method for converting 3D assets into sparse panoramic RGB-D-pose data with complete coverage and low redundancy, used to create the CM-EVS dataset for 3D learning.


<details>
  <summary>Details</summary>
Motivation: Existing 3D data sources lack sparse, comparable, and geometry-consistent panoramic training interfaces, leading to redundancy and inconsistency.

Method: COVER curates viewpoints by projecting geometry into ERP probes, scoring coverage, and penalizing depth conflicts, ensuring low redundancy and auditable provenance.

Result: CM-EVS dataset includes 36,373 curated ERP frames from 1,275 indoor scenes, offering full-sphere RGB, metric depth, and calibrated pose with minimal frames per scene.

Conclusion: COVER improves coverage-conflict trade-offs, making CM-EVS a valuable resource for geometry-consistent panoramic 3D learning.
...
TL;DR: The paper examines the impact of pretraining objectives on fine-grained classification in low-data scenarios, comparing four ViT-B/16 encoders with different objectives and evaluating their performance using linear and nonlinear probes.


<details>
  <summary>Details</summary>
Motivation: To provide guidance for selecting pretrained encoders in expert domains where labeling is expensive and data is scarce.

Method: Compares four frozen ViT-B/16 encoders (supervised classification, contrastive learning, masked reconstruction, self-distillation) using linear and nonlinear probes with leave-one-out cross-validation and permutation testing.

Result: Supervised and contrastive encoders perform best with linear probes, while masked reconstruction improves with nonlinear probes. DINOv3 underperforms across all probes.

Conclusion: Prioritize margin-enforcing pretraining objectives for linear decision rules in low-data scenarios, and consider reconstruction-style encoders when nonlinear classifiers are feasible.
...
TL;DR: NeRP addresses the Base-New Trade-off in VLMs by correcting asymmetric confusion biases using a plug-and-play prompting strategy without modifying model parameters.


<details>
  <summary>Details</summary>
Motivation: The paper tackles the challenge of improving unseen class recognition in transfer learning without degrading known-class performance, observed through asymmetric confusion in VLMs.

Method: Proposes NeRP, leveraging neutral text prompts and reference images to measure class-wise biases and correct mispredictions via local flips between confusable classes.

Result: NeRP significantly boosts unseen-class accuracy across multiple benchmarks while maintaining known-class performance.

Conclusion: NeRP effectively mitigates pretraining-induced biases in VLMs, offering a practical solution to the Base-New Trade-off.
...
TL;DR: LRCP, a training-free compression framework, leverages the low-rank structure of visual tokens to reduce redundancy, achieving high performance retention with significant token reduction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for visual token compression either introduce positional bias or overlook global structure. The observation of a low-rank structure in visual tokens motivated a new approach.

Method: Proposes LRCP, which estimates a dominant low-rank subspace using PCA and scores tokens by their projection residual, retaining those poorly explained by the low-rank structure.

Result: LRCP preserves 94.7% of image-understanding performance with an 88.9% token reduction and 97.8% of video-understanding accuracy with an 87.5% token reduction.

Conclusion: LRCP effectively compresses visual tokens by exploiting their low-rank structure, demonstrating superior performance and efficiency without requiring training.
...
TL;DR: The paper introduces GMAE, a deep learning framework for multi-view clustering that disentangles view-specific and view-common features to improve clustering performance.


<details>
  <summary>Details</summary>
Motivation: Existing deep MVC methods often mix view distributions during fusion, leading to poor latent space quality and suboptimal clustering results.

Method: GMAE uses dual-path autoencoders to separate view-specific and view-common features, along with cross-view adversarial discriminators to enhance feature discrimination and alignment.

Result: Extensive experiments on 13 datasets show GMAE outperforms state-of-the-art methods in complete and incomplete MVC tasks.

Conclusion: GMAE effectively addresses view-distribution entanglement and generates robust embeddings, improving clustering performance.
...
TL;DR: Classifier-free guidance (CFG) traditionally uses a fixed scale, but this mismatches the dynamic needs of ODE trajectories. Velocity-Adaptive Guidance Scale (VAGS) adjusts guidance strength adaptively, improving generation and editing tasks without extra training or resources.


<details>
  <summary>Details</summary>
Motivation: The fixed scale of CFG doesn't account for varying semantic and structural needs during the ODE trajectory, leading to suboptimal performance in tasks like editing and generation.

Method: VAGS introduces a training-free adaptive scale, combining temporal signal-level terms and cosine similarity between velocity fields, to adjust guidance strength dynamically.

Result: VAGS outperforms fixed CFG and recent training-free variants in tasks like inversion-free editing and generation, improving structural fidelity and quality.

Conclusion: VAGS offers a robust, flexible, and efficient alternative to fixed CFG, enhancing performance without additional training or computational costs.
...
TL;DR: ChronoEarth-492K is introduced as the first large-scale, temporally calibrated hyperspectral SSL dataset, enabling long-horizon spatiotemporal modeling, with a benchmark for unified evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing hyperspectral self-supervised learning datasets lack temporal depth, limiting the development of long-horizon spatiotemporal modeling.

Method: The dataset is built using NASA's EO-1 Hyperion mission data (2001-2017), comprising 492,354 radiometrically harmonized patches across 185,398 global locations. A benchmark is established for evaluation.

Result: ChronoEarth-492K includes 28,786 multi-temporal sequences, enabling both short- and long-horizon temporal analysis. Baseline results from state-of-the-art models are reported.

Conclusion: ChronoEarth and its benchmark provide a large-scale, temporally grounded platform for systematic spatiotemporal hyperspectral representation learning.
...
TL;DR: The study investigates how vision-language models (VLMs) struggle with line tracing tasks, often switching to nearby distractors, and finds that common solutions like scaling, reasoning, or instructions don't fully resolve the issue.


<details>
  <summary>Details</summary>
Motivation: Despite strong performance on benchmarks, VLMs lack robust control over basic visual operations like line tracing. The study aims to isolate and understand these limitations.

Method: Controlled tracing tasks were designed with nearby competitors to reduce ambiguity. Behavioral interventions and internal analyses were used to identify failure causes.

Result: VLMs frequently lose the target path, especially with similar distractors. Scaling, reasoning, and instructions provided limited improvements.

Conclusion: Path-switching failures persist in VLMs, even in complex settings, highlighting a fundamental limitation in their ability to follow visual paths robustly.
...
TL;DR: ElasticDiT dynamically balances image fidelity and latency for mobile devices by adjusting spatial compression and DiT block depths, outperforming specialized models.


<details>
  <summary>Details</summary>
Motivation: Deploying high-fidelity image generation models like DiT on mobile devices is challenging due to computational and memory constraints, requiring a dynamic trade-off solution.

Method: ElasticDiT uses Shift Sparse Block Attention (SSBA) and Tiny DWT-Distilled VAE (T-DVAE) to reduce latency and memory while maintaining quality, with on-the-fly reconfiguration.

Result: ElasticDiT outperforms specialized models, achieving high performance scores (HPS 32.87) and competitive quality at 84.16% sparsity, with T-DVAE reducing VAE costs by 8x.

Conclusion: ElasticDiT provides a versatile, hardware-adaptive solution for mobile image generation, eliminating the need for multiple models and paving the way for future high-resolution applications.
...
TL;DR: The paper introduces a teacher selection metric, Ratio 1-2, to improve knowledge distillation for fine-grained image recognition, achieving significant student model performance gains.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of selecting the right teacher model for knowledge distillation, which heavily impacts the performance of smaller student models in resource-constrained settings.

Method: The proposed method uses a metric based on teacher prediction ratios (Ratio 1-2) to select the most suitable teacher for distillation.

Result: The metric improves teacher selection by 18% over previous methods, enabling student models to achieve up to 17% accuracy gains.

Conclusion: The Ratio 1-2 metric effectively enhances teacher selection in knowledge distillation, leading to better performance of student models in fine-grained image recognition tasks.
...
TL;DR: The paper introduces a viewpoint-aware 3D referring segmentation dataset and evaluates existing models, showing improved accuracy with viewpoint conditioning.


<details>
  <summary>Details</summary>
Motivation: Current 3D referring segmentation methods lack explicit observer viewpoint representation, causing ambiguity in spatial relations like 'left' or 'behind'.

Method: A dataset with 220k samples (scalable to millions) is created using camera poses to annotate observer-centric and viewpoint-independent relations. Viewpoint-aware models are evaluated and improved by encoding camera poses.

Result: Models without viewpoint conditioning struggle; accuracy improves from 0.30 to 0.47 mIoU when viewpoint information is incorporated.

Conclusion: Explicit viewpoint representation enhances 3D scene understanding, and the dataset and models will be publicly available.
...
TL;DR: EntropyScan is introduced as a lightweight, trigger-agnostic method for detecting backdoor attacks in Large Vision-Language Models (LVLMs) by analyzing structural anomalies in visual attention allocation. It achieves high accuracy with an F1 score of 98.5% and AUC of 96.6%.


<details>
  <summary>Details</summary>
Motivation: Current defenses for LVLMs focus on sample-level detection, requiring knowledge of training data or triggers. There's a need for a model-level approach to identify backdoored models without such dependencies.

Method: EntropyScan quantifies attention deviations caused by backdoor injection, analyzing visual attention distributions from LLM layers and using Tsallis entropy to detect distortions. It employs Z-score normalization on benign samples for identification.

Result: The method demonstrated high effectiveness, with an F1 score of 98.5% and AUC of 96.6% across multiple LVLM architectures and attack scenarios.

Conclusion: EntropyScan provides a robust, lightweight solution for model-level backdoor detection in LVLMs, addressing a critical gap in current defense strategies. The code will be made publicly available.
...
TL;DR: Semi-MedRef is a teacher-student SSL framework for medical referring image segmentation that maintains image-text alignment under strong augmentations, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Annotation costs for MRIS are high in low-label regimes, and existing SSL methods struggle with cross-modal alignment under perturbations.

Method: Semi-MedRef introduces T-PatchMix, PosAug, and ITCL to preserve image-text consistency.

Result: Outperforms fully supervised and SSL baselines on QaTa-COV19 and MosMedData+.

Conclusion: The proposed framework effectively addresses cross-modal alignment challenges in SSL for MRIS.
...
TL;DR: DiLA introduces disentangled latent actions to balance action abstraction and generation fidelity in world models, outperforming existing methods in video generation and interpretability.


<details>
  <summary>Details</summary>
Motivation: Address the trade-off between action abstraction and generation fidelity in Latent Action Models (LAMs) by disentangling content and structure.

Method: DiLA uses content-structure disentanglement, where latent action learning and disentanglement co-evolve, separating spatial layouts (structure) from visual details (content).

Result: DiLA achieves superior video generation quality, action transfer, visual planning, and manifold interpretability.

Conclusion: DiLA provides a unified framework for high-level action abstraction and high-fidelity generation, advancing self-supervised world model learning.
...
TL;DR: The paper introduces DecomPose, a framework to address gradient conflicts in category-level 6D object pose estimation by using difficulty-aware gradient decoupling and stability-driven asymmetric branching.


<details>
  <summary>Details</summary>
Motivation: The motivation is to resolve the issue of geometric heterogeneity across categories causing gradient conflicts and negative transfer in shared model parameters for 6D object pose estimation.

Method: The method involves gradient-based diagnostics to quantify cross-category contention, followed by difficulty-aware gradient decoupling (grouping categories with shared difficulty proxies) and stability-driven asymmetric branching (assigning higher-capacity branches to simpler categories).

Result: DecomPose effectively reduces optimization contention and improves pose estimation performance, as demonstrated on REAL275, CAMERA25, and HouseCat6D benchmarks.

Conclusion: The proposed framework successfully mitigates gradient conflicts and negative transfer, leading to superior performance in category-level 6D object pose estimation.
...
TL;DR: The paper introduces the Unified Action Model (UAM) to address the 'embodiment tax' in vision-language-action (VLA) models, preserving multimodal competence by adding a Dorsal Expert pathway, achieving high success rates and semantic retention.


<details>
  <summary>Details</summary>
Motivation: Current VLAs, built by fine-tuning vision-language models (VLMs) on action data, lose multimodal competence—termed the 'embodiment tax.' The paper explores whether this degradation is avoidable by drawing inspiration from biological vision's two-stream organization.

Method: The proposed UAM incorporates a parallel Dorsal Expert, analogous to the brain's dorsal pathway, initialized from a pretrained generative model and trained with a mid-level reasoning objective to predict visual dynamics. This allows end-to-end training without frozen parameters or auxiliary data.

Result: UAM retains over 95% of the underlying VLM's multimodal capability while achieving the highest success rate on manipulation tasks, including out-of-distribution generalization (e.g., unseen objects, novel compositions, and instruction variation).

Conclusion: Architectural separation, as in UAM, preserves semantic capability in VLAs without frozen weights or auxiliary data, enabling natural transfer of semantic generalization from VLMs to actions.
...
TL;DR: BiomedAP is a framework addressing prompt fragility in biomedical VLMs by using gated cross-modal fusion and dual-anchor constraints, improving few-shot accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing biomedical VLMs struggle with prompt variations in noisy clinical settings due to isolated modality optimization.

Method: BiomedAP introduces gated cross-modal fusion for dynamic noise regulation and dual-anchor constraints for stable semantic alignment.

Result: Outperforms baselines in 11 benchmarks, achieving competitive few-shot accuracy and enhanced robustness to prompt variations.

Conclusion: BiomedAP effectively improves cross-modal alignment and robustness in biomedical VLMs under noisy clinical conditions.
...
TL;DR: BARRIER introduces a framework for machine unlearning by focusing on dynamic hidden-layer activations, ensuring targeted concept erasure while protecting other representations with mathematical guarantees.


<details>
  <summary>Details</summary>
Motivation: Traditional machine unlearning methods often cause collateral damage by erasing unintended concepts due to the lack of formal guarantees for preserving neutral knowledge.

Method: BARRIER uses Interval Arithmetic on SVD-based projections of activation spaces to bound target regions for unlearning, ensuring updates only affect the forget region while protecting the retain distribution.

Result: BARRIER achieves state-of-the-art trade-offs, maximizing targeted erasure while safeguarding other representations, with empirical validation across classifiers and diffusion models.

Conclusion: BARRIER transforms machine unlearning by shifting the focus to activation geometry, providing rigorous guarantees and enabling aggressive updates without degrading other knowledge.
...
TL;DR: HyperDiT addresses the granularity dilemma in pixel-space diffusion models by using hyper-connected cross-scale interactions, achieving state-of-the-art image generation results.


<details>
  <summary>Details</summary>
Motivation: To overcome the trade-off between capturing global semantics and generating high-fidelity details in pixel-space diffusion models.

Method: Introduces HyperDiT, utilizing cross-attention for multi-scale interactions, SA-RoPE for spatial alignment, and Registers for semantic learning from a pretrained VFM.

Result: Achieves SoTA FID of 1.56 on ImageNet 256x256.

Conclusion: HyperDiT combines fine-grained and semantic streams, offering a superior paradigm for high-fidelity pixel generation.
...
TL;DR: The paper addresses the issue of attribute flooding in MLLMs for artwork emotion explanations by introducing Attribute-Grounded Selective Reasoning (AGSR) and a supervised framework called FAB-G.


<details>
  <summary>Details</summary>
Motivation: To improve the interpretability of emotion explanations in multimodal large language models by focusing on emotionally operative attributes rather than enumerating all visible ones.

Method: Introduces AGSR and FAB-G, a framework that predicts attribute-level salience and constrains emotional analysis to salient cues, using a human-annotated dataset extension.

Result: FAB-G improves emotion, arousal, and valence prediction, aligns better with human-marked attributes, and produces more concise explanations than baselines.

Conclusion: AGSR and FAB-G enhance the quality and efficiency of artwork emotion understanding, with potential for generalization beyond the EmoArt dataset.
...
TL;DR: The paper introduces a learned optimizer for 3D Gaussian Splatting (3DGS) that avoids degradation over long optimization horizons, improving early novel view synthesis quality while remaining stable.


<details>
  <summary>Details</summary>
Motivation: Standard optimizers (like Adam, SGD) for 3DGS are general-purpose and inefficient due to independent parameter updates. Learned optimizers with correlated updates exist but degrade over time without manual learning rate scheduling.

Method: A meta-learning scheme using a checkpoint buffer and optimizer rollout strategy, combined with an architecture encoding gradient scale information in latent states, is proposed.

Result: The learned optimizer improves early view synthesis quality and remains stable over long horizons, with zero-shot generalization to unseen reconstruction settings.

Conclusion: The method outperforms conventional optimizers, and the framework introduced allows unified training and evaluation of optimizers across sparse and dense view settings.
...
TL;DR: The paper introduces Generation-to-Understanding (G2U) synergy, where visual generation enhances multimodal understanding by producing self-generated visual thoughts as reasoning steps, improving perceptual tasks without retraining.


<details>
  <summary>Details</summary>
Motivation: The goal is to address the one-directional unification in multimodal AI, where understanding guides generation but not vice versa, by exploring how generation can support understanding.

Method: The proposed G2U framework uses visual generation (like detail enhancement or context expansion) as an intermediate step to produce self-generated visual thoughts, which are then fed back to refine perception.

Result: Evaluated on twelve benchmarks, the reversed information flow improves multimodal understanding, with generative fidelity bounding perceptual gain and edit prompts affecting transfer efficiency.

Conclusion: Current models lack stable task alignment in self-generated visual thoughts, indicating a gap in unified cognition, and suggesting imagination as the starting point for deeper understanding.
...
TL;DR: A unified framework for preprocessing and registering 3D fingerprints to integrate with legacy 2D systems, achieving ridge-level accuracy and improved compatibility.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between 3D and 2D fingerprint systems by addressing preprocessing and registration challenges.

Method: Combines visualization, point-cloud fusion, pose normalization, and cross-modal registration to convert 3D fingerprints to 2D equivalents.

Result: Achieves 0.09 mm 3D fusion error, ridge-scale projection accuracy, and improved matching scores.

Conclusion: The framework effectively integrates 3D fingerprints with 2D systems, serving as a geometric bridge.
...
TL;DR: The paper introduces E²PO, a novel framework to address the issue of vanishing intra-group variance in group-based RL optimization, ensuring stable training and better alignment with human preferences.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for aligning generative models with human intent suffer from rapid decay of intra-group variance, causing unstable training and suboptimal results.

Method: The proposed E²PO framework uses embedding-level perturbations within sample groups to maintain robust variance and preserve learning signals.

Result: Experiments show E²PO outperforms existing baselines, achieving superior alignment with human preferences.

Conclusion: E²PO effectively solves the variance decay problem, offering a stable and efficient solution for RL-based optimization.
...
TL;DR: FashionChameleon enables real-time, interactive garment switching in videos using single-garment data, achieving 23.8 FPS on a single GPU.


<details>
  <summary>Details</summary>
Motivation: Address the gap in low-latency, interactive garment control for applications like e-commerce and content creation.

Method: Uses a Teacher Model with In-Context Learning, Streaming Distillation, and Training-Free KV Cache Rescheduling for garment switching.

Result: Achieves real-time generation (23.8 FPS) and 30-180x speedup over baselines while preserving motion coherence.

Conclusion: FashionChameleon offers a practical solution for interactive multi-garment video customization with high efficiency and consistency.
...
TL;DR: The study highlights the mismatch between sample-level metrics and ecological needs in open-set plankton recognition, introducing a community-level error metric (OSCD) and showing improved performance with community-aware calibration.


<details>
  <summary>Details</summary>
Motivation: Automated plankton image recognition often encounters unseen taxa and non-target particles, but existing evaluation metrics focus on sample-level performance rather than ecological community-level estimates.

Method: The study evaluates open-set recognition methods using controlled pseudo-communities and three datasets (marine zooplankton, marine phytoplankton, freshwater plankton). It introduces OSCD, a community-level error metric, and compares sample-level and community-aware calibration methods.

Result: Closed-set classifiers performed well on known taxa but struggled with unknowns. Sample-level metrics (e.g., FPR@95%) led to ecological errors, while community-aware calibration reduced OSCD in some cases. The best method varied by dataset.

Conclusion: Open-set plankton recognition should prioritize ecological measurement over sample-level detection, with community-aware calibration improving accuracy where validation data is representative.
...
TL;DR: WorldAct transforms static 3D worlds into editable, interaction-ready scenes using multimodal agents for decomposition, reconstruction, and inpainting.


<details>
  <summary>Details</summary>
Motivation: Current 3D world modeling systems produce static, non-editable assets, limiting their use in immersive content creation and embodied simulation. WorldAct addresses this by enabling dynamic interaction and editing.

Method: WorldAct employs a multimodal agent to perform scene decomposition, identify actionable objects, reconstruct object-level meshes, and restore the background via 3D inpainting.

Result: The framework creates scenes that support object-level editing, collision-aware manipulation, and task execution while maintaining global coherence, outperforming original static scenes.

Conclusion: WorldAct offers a practical solution for making 3D world models editable and interactive, enhancing their utility in real-world applications.
...
TL;DR: VLMs often make self-reflective statements, but VisualSwap testing shows they rarely genuinely re-examine images, missing swaps with high frequency. User instructions, not self-generated statements, improve visual attention.


<details>
  <summary>Details</summary>
Motivation: To determine if self-reflective statements in VLMs (like 'let me check the figure again') lead to actual visual re-examination or are just learned textual patterns.

Method: Developed VisualSwap, testing models by replacing images after reasoning. Used VS-Bench (800 image pairs from MathVista, MathVerse, MathVision, MMMU-Pro) on Qwen3-VL, Kimi-VL, ERNIE-VL.

Result: Models missed the swap with up to 60% accuracy drop. Thinking models were 3x more vulnerable than instructed ones; scaling didn't help. User instructions improved visual grounding, but self-reflection didn't.

Conclusion: Current VLMs often say but don't actually see during self-reflective statements. User instructions significantly boost visual attention, unlike self-generated reflections.
...
TL;DR: GHOST is a KV cache management framework for streaming 3D reconstruction that reduces memory usage while maintaining quality by using 3D geometry outputs to evict redundant tokens.


<details>
  <summary>Details</summary>
Motivation: Existing methods either degrade reconstruction quality or fail to preserve geometrically valuable tokens due to memory constraints in streaming 3D reconstruction.

Method: GHOST uses hierarchical scoring, a privilege mechanism, and layer-wise budget allocation to manage the KV cache without training.

Result: GHOST reduces the KV cache by half, speeds up inference by 1.75x, and maintains high reconstruction quality.

Conclusion: GHOST effectively addresses memory bottlenecks in streaming 3D reconstruction while preserving geometric fidelity and performance.
...
TL;DR: The paper introduces AdaScope, an RL-enhanced plug-in for diffusion models that optimizes intervention timing during denoising to improve alignment with human preferences while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for diffusion models are computationally expensive and inefficient due to their application across the entire denoising trajectory, leading to delayed rewards and reward hacking.

Method: AdaScope adaptively determines the optimal timing for RL intervention by monitoring structural evolution and semantic consistency during denoising, and dynamically halts training when reward gains saturate.

Result: AdaScope achieves a 66% performance improvement and reduces computational costs by 59% compared to state-of-the-art methods.

Conclusion: AdaScope successfully balances computational efficiency and performance enhancement, offering a practical solution for RL-based fine-tuning in diffusion models.
...
TL;DR: A practical stereo calibration framework for low-resolution TIR and high-resolution RGB cameras using dynamic patterns and advanced corner detection, achieving accurate geometric calibration for building analysis.


<details>
  <summary>Details</summary>
Motivation: Accurate calibration of RGB-TIR stereo systems is challenging with low-cost, low-resolution thermal sensors, especially for building envelope analysis.

Method: Uses an OLED screen to display modality-specific patterns, a tailored corner detection algorithm, and baseline-constrained bundle adjustment for calibration.

Result: Achieves a stereo baseline of 32.7 mm with a reprojection error of 0.382 px, validated on a building mock-up for energy performance assessment.

Conclusion: The framework provides reliable and precise calibration for RGB-TIR systems with large resolution disparities, suitable for building energy analysis.
...
TL;DR: SOLAR introduces a self-supervised framework for symmetric multimodal-to-multimodal retrieval, leveraging unlabeled data and outperforming existing methods with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Addressing the underexplored challenge of symmetric MM2MM retrieval, where existing methods fail due to reliance on asymmetric datasets.

Method: A two-stage framework: first learns an intersection mask for alignment, then uses it to create samples for self-supervised embedding learning.

Result: Outperforms ten SOTA methods by 7.08 points with 50x fewer parameters and a 5x smaller embedding dimension.

Conclusion: SOLAR is efficient and effective for symmetric MM2MM retrieval, supported by a new benchmark.
...
TL;DR: DepthVLM enhances Vision-Language Models (VLMs) to predict dense geometry (depth) alongside language tasks, outperforming existing VLMs and pure vision models.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack 3D understanding due to text-only supervision, limiting fine-grained visual perception and dense geometry recovery. Prior methods are either error-prone or inefficient.

Method: DepthVLM attaches a lightweight depth head to an LLM backbone, trained under a unified vision-text supervision paradigm with a two-stage schedule to generate full-resolution depth maps and language outputs in one pass.

Result: DepthVLM significantly outperforms existing VLMs and pure vision models, offering higher inference efficiency and improved 3D spatial reasoning.

Conclusion: DepthVLM advances VLMs toward a unified foundation model for dense geometry prediction while retaining multimodal capabilities, with plans to release code and checkpoints.
...
TL;DR: FSCM is a GAN framework that enhances infrared image colorization using spectral information, improving structural and semantic accuracy.


<details>
  <summary>Details</summary>
Motivation: Current infrared colorization methods struggle with structural distortion and semantic confusion due to insufficient spectral cues.

Method: FSCM uses a frequency-enhanced spatial-spectral state-space generator, combining state-space modeling, frequency enhancement, and dual-stream hybrid gating, plus semantic segmentation-guided loss.

Result: FSCM outperforms existing methods in visual quality and semantic fidelity.

Conclusion: FSCM effectively addresses limitations in infrared colorization by leveraging spectral and spatial-spectral coupling.
...
TL;DR: A probabilistic framework for wildfire smoke severity assessment from satellite images, offering uncertainty measures and achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: Emergency response, air quality modeling, and human health risk management require rapid and accurate wildfire smoke severity assessment.

Method: Uses a pre-trained EfficientNet-B3 with a CBAM module and evidential deep learning head to predict severity classes and uncertainty without Monte Carlo sampling.

Result: Achieves 93.8% weighted test accuracy (91.1% unweighted) with ECE=0.0274. Selective prediction retains 96.7% accuracy for the most certain 50% of patches.

Conclusion: The model effectively identifies ambiguous smoke regions and provides practical uncertainty measures, confirming its reliability for real-world applications.
...
TL;DR: The paper proposes a causal framework to standardize the classification and measurement of image degradations, making it easier to compare results across datasets and tasks, and introduces a lightweight severity measurement layer.


<details>
  <summary>Details</summary>
Motivation: Image degradations are studied in various fields but lack a unified approach for classification and severity measurement, hindering cross-comparison.

Method: A dual-axis taxonomy categorizes degradations by causal source and perceptual effect, and a severity measurement layer uses metrics like PSNR, SSIM, and LPIPS to standardize severity.

Result: The framework enables consistent comparison of degradations across datasets and tasks, demonstrated via the COCO Degradation benchmark for object detector robustness.

Conclusion: The proposed framework addresses the inconsistency in degradation classification and severity measurement, improving interoperability across research communities.
...
TL;DR: RaPD introduces continuous diffusion in a Neural Image Field latent space, enabling resolution-flexible generation with fixed diffusion costs.


<details>
  <summary>Details</summary>
Motivation: Natural images are continuous, but most generative models operate on discrete grids, limiting resolution-flexible generation. Existing methods introduce continuity only at the decoding stage, leaving the latent space discretized and reconstruction-oriented.

Method: RaPD performs diffusion in a continuous Neural Image Field (NIF) latent space. It uses Semantic Representation Guidance for generation-aware latent learning and a Coordinate-Queried Attention Renderer for coordinate-conditioned, scale-aware rendering.

Result: Experiments show superior generation quality and resolution scalability, with a single denoised latent renderable at arbitrary resolutions by changing query coordinates.

Conclusion: RaPD bridges the reconstruction-generation gap, enabling resolution-agnostic generation without increasing diffusion costs.
...
TL;DR: AdaEraser improves object removal in images by adaptively modulating self-attention during denoising, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current training-free methods for object removal degrade generation quality by indiscriminately suppressing self-attention in vacated regions.

Method: AdaEraser dynamically modulates attention based on target object presence, using a token-wise adaptive suppression strategy across denoising steps.

Result: AdaEraser achieves superior object removal performance, surpassing even training-based methods in experiments.

Conclusion: The adaptive framework of AdaEraser effectively balances background reconstruction and object removal, enhancing generation quality.
...
TL;DR: Invaria, a 3D point cloud encoder, addresses the sensitivity of existing models to resolution and scale changes by achieving invariance through next-resolution prediction and receptive field calibration.


<details>
  <summary>Details</summary>
Motivation: Existing 3D point cloud encoders fail to generalize like image encoders due to overfitting to specific quantization densities and object scales, limiting real-world applications.

Method: Proposed Invaria, which achieves scale and density invariance via next-resolution prediction and receptive field calibration, focusing on robust structural invariants.

Result: Invaria shows 56.0% higher mIoU at lower resolution and 20% improvement at reduced scale, with a 45% smaller model size and 40% fewer input tokens.

Conclusion: Invaria provides a scalable and efficient solution for 3D point cloud encoding, overcoming resolution and scale sensitivity while maintaining performance.
...
TL;DR: The paper proposes a Decomposed Vision-Language Alignment framework to improve open-vocabulary segmentation by factorizing textual prompts into concept and attribute tokens, enhancing generalization to unseen category-attribute combinations.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary segmentation models often fail to generalize to new combinations of categories and attributes due to entangled semantic units in textual prompts.

Method: The framework decomposes prompts into concept and attribute tokens, uses Feature-Gated Cross-Attention for attribute-specific fusion, and aggregates per-token similarities in log-space.

Result: The method significantly enhances generalization to unseen attribute-category compositions in fine-grained segmentation benchmarks.

Conclusion: The proposed framework effectively improves compositional semantics in open-vocabulary segmentation and can integrate seamlessly into existing transformer architectures.
...
TL;DR: The paper introduces a group-revision optimization paradigm using reinforcement learning to improve object-level grounding in vision-language models, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods use sparse, response-level rewards, which provide minimal learning signals in hard scenarios.

Method: Proposes a group-revision approach: initial responses are revised, with improvements quantified and converted into shaping signals for reward refinement.

Result: Achieves consistent gains in referring, reasoning segmentation, REC, and counting benchmarks.

Conclusion: The method enhances learning on hard cases and outperforms GRPO-based models.
...
TL;DR: SAE-FT is a novel method for robust fine-tuning of vision-language models, focusing on visual representations to avoid degrading robustness against distribution shifts without relying on expensive text-guidance.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning large pre-trained models like CLIP often reduces robustness against distribution shifts. SAE-FT aims to address this trade-off by focusing on visual representations to maintain robustness and interpretability.

Method: SAE-FT regularizes changes to visual representations by penalizing the addition or removal of semantically meaningful features identified by a Sparse Autoencoder trained on the pre-trained model.

Result: SAE-FT matches or exceeds state-of-the-art performance on ImageNet and its distribution shift benchmarks, while being computationally efficient and interpretable.

Conclusion: SAE-FT provides a transparent, efficient, and effective solution for robust fine-tuning of pre-trained vision-language models, improving performance without compromising robustness.
...
TL;DR: Flash-GRPO is a single-step training framework that improves efficiency and alignment quality for video diffusion models, outperforming full trajectory training under low computational budgets.


<details>
  <summary>Details</summary>
Motivation: Addressing the computational bottleneck and instability issues in existing methods for aligning video diffusion models with human preferences.

Method: Flash-GRPO uses iso-temporal grouping to ensure prompt-wise temporal consistency and temporal gradient rectification to balance gradient magnitudes across timesteps.

Result: Experiments show Flash-GRPO accelerates training, maintains stability, and achieves state-of-the-art alignment quality for models ranging from 1.3B to 14B parameters.

Conclusion: Flash-GRPO offers a scalable and efficient alternative to traditional methods, enabling high-quality alignment with reduced computational costs.
...
TL;DR: Echo-Forcing is a training-free scene memory framework for interactive long video generation, addressing issues like outdated background contamination and delayed response to new prompts.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with interactive scenarios like prompt switching and historical scene recall due to entangled historical KV states.

Method: Echo-Forcing introduces three mechanisms: Hierarchical Temporal Memory, Scene Recall Frames, and Difference-aware Memory Decay.

Result: Echo-Forcing outperforms in long-video and interactive generation settings, as shown in VBench-Long evaluations.

Conclusion: The framework effectively supports smooth transitions, hard cuts, and long-range scene recall with a bounded cache budget.
...
TL;DR: An end-to-end computer-aided workflow for virus titration via plaque assays, using SAM-based models for well and plaque segmentation, shows high accuracy and reduces manual effort.


<details>
  <summary>Details</summary>
Motivation: Manual plaque counting from plaque assay images is labor-intensive and inconsistent, necessitating an automated solution.

Method: Combines SAM-derived models for well localization and plaque detection, tested on mixed datasets (MAYV/CVB3, Vaccinia virus). Outputs PFU/mL and integrates into a web platform.

Result: Strong agreement with manual counts (Pearson 0.92 for MAYV/CVB3, 0.88 for VACV) and high concordance with expert annotations.

Conclusion: The automated system offers reproducible, scalable plaque assay analysis, reducing manual effort, and will be open-sourced.
...
TL;DR: A unified framework using MLLMs-guided Gaussian Splatting for physics-aware reconstruction and simulation in robot-assisted surgery, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack physics-based descriptions for realistic simulation in endoscopic surgery. This gap is addressed to enhance surgical outcomes.

Method: Combines 4D Gaussian Splatting with segmentation and depth estimation. Uses MLLM-guided object-wise material fields and differentiable MPM for physical property inference.

Result: Achieves superior simulation fidelity and physical accuracy on open-source and in-house datasets.

Conclusion: The framework shows potential to advance robot-assisted surgical applications by improving realism in scene reconstruction and simulation.
...
TL;DR: 3D-GS lacks robust segmentation; SAM-HQ improves 2D masks, and a novel method ensures 3D consistency for accurate editing.


<details>
  <summary>Details</summary>
Motivation: Existing 3D-GS methods struggle with segmentation for editing tasks due to inconsistencies and coarse masks.

Method: Uses SAM-HQ for better 2D masks and introduces a prior-guided label reassignment for 3D consistency.

Result: Achieves state-of-the-art segmentation accuracy and enables real-time, high-fidelity object editing.

Conclusion: The framework advances 3D scene editing with superior boundary preservation and practical VR/robotics applications.
...
TL;DR: VideoSeeker improves instance-level video understanding by integrating visual prompts and proactive perception, outperforming baselines by +13.7% and surpassing closed-source models like GPT-4o and Gemini-2.5-Pro.


<details>
  <summary>Details</summary>
Motivation: Current LVLMs struggle with precise spatiotemporal localization and rely on text prompts, which lack spatial/temporal precision and decouple visual perception from language reasoning.

Method: Proposes VideoSeeker, a paradigm using visual prompts for instance-level video understanding, with a four-stage data synthesis pipeline, cold-start supervision, and RL training.

Result: Achieves +13.7% improvement over baselines and outperforms closed-source models; shows effective transferability on general benchmarks.

Conclusion: VideoSeeker advances instance-level video understanding with visual prompts and proactive perception, leveraging automated data synthesis and training techniques.
...
TL;DR: The paper proposes ReAlign, a lightweight AIGI detection framework that leverages LLM-generated reasoning texts for improved generalization and semantic-error sensitivity, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of existing AIGI detection methods (lack of semantic understanding in non-LLM models and computational inefficiency in LLM-based approaches) by exploring the value of LLM-generated reasoning texts.

Method: Introduces ReAlign, a framework that distills high-quality reasoning texts from a GRPO-optimized LLM into an efficient detector via contrastive learning and joint optimization.

Result: ReAlign consistently surpasses state-of-the-art detectors in accuracy and generalization, especially for high-fidelity forgeries, as demonstrated on multiple benchmarks.

Conclusion: ReAlign successfully integrates the strengths of LLM reasoning (semantic sensitivity) into a lightweight detector, offering a balanced and effective solution for AIGI detection.
...
TL;DR: GenShield is a unified framework for explainable AI-generated image detection and controllable artifact correction, using a 'diagnose-then-repair' approach with strong performance.


<details>
  <summary>Details</summary>
Motivation: Address the gap in connecting AI-generated image (AIGI) detection with artifact correction, and improve authenticity in applications like misinformation detection and digital forensics.

Method: Proposes GenShield, an autoregressive framework with a Visual Chain-of-Thought curriculum learning strategy for multi-step, self-explained correction. A dataset with 'artifact-restored' pairs is also created.

Result: The method achieves state-of-the-art performance on correction benchmarks and mainstream AIGI detection benchmarks, demonstrating strong generalization.

Conclusion: GenShield effectively bridges AIGI detection and artifact correction, offering a closed-loop solution with practical applications in digital authenticity.
...
TL;DR: The paper proposes a VLM-assisted framework to dynamically adjust sensor fusion in 3D semantic occupancy prediction, addressing environmental variability by leveraging CLIP and weather-specific cues.


<details>
  <summary>Details</summary>
Motivation: Environmental variability (e.g., low-light for cameras, backscatter noise for LiDAR) creates modality trust issues in static fusion strategies, necessitating adaptive solutions.

Method: A VLM-assisted framework uses CLIP's latent space and weather-specific text embeddings to guide sensor fusion. A parameter-efficient adapter aligns embeddings with sensor features, and a gating strategy dynamically adjusts fusion based on visibility and illumination.

Result: The framework improves performance on the nuScenes dataset, achieving mIoU scores of 26.3 and 21.1 on OccMamba and M-CONet architectures, respectively, surpassing traditional baselines.

Conclusion: The proposed approach effectively addresses environmental variability in multi-modal 3D semantic occupancy prediction, demonstrating superior adaptability and performance.
...
TL;DR: STABLE is a dual-system combining semantics and physics to generate simulation-ready tabletop scenes from task instructions, improving physical plausibility over prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely solely on LLMs for scene layout prediction, leading to issues like object collisions or floating due to poor 3D spatial reasoning. STABLE addresses this gap by integrating semantics and physics.

Method: STABLE uses a Semantic Reasoner (fine-tuned LLM) for coarse layout generation and a Physics Corrector (physics-aware model) for refining poses. It alternates between these modules to progressively build the scene.

Result: STABLE generates scenes that adhere to task instructions and are physically plausible, outperforming prior methods in physical validity.

Conclusion: STABLE's dual-system approach effectively combines semantic reasoning with physical corrections to produce high-quality, simulation-ready tabletop scenes.
...
TL;DR: Register tokens improve convergence and generation quality in pixel-space Diffusion Transformers (DiTs) by producing cleaner feature maps at high noise levels, unlike Vision Transformers (ViTs).


<details>
  <summary>Details</summary>
Motivation: To determine if register tokens, effective in ViTs, also benefit DiTs and explore their role in diffusion models.

Method: Analyze DiTs for patch-token outliers, test register tokens in pixel-space DiTs, and propose a dual-stream architecture for specialized register token processing.

Result: Register tokens enhance DiTs despite no patch-token outliers, improving feature maps and generation quality, with recent architectures already incorporating similar mechanisms.

Conclusion: Register tokens are beneficial for pixel-space DiTs, and a dual-stream architecture further optimizes their use with minimal overhead.
...
TL;DR: ML-FOP-SOAP, a second-order optimization framework, improves stability and efficiency in multimodal training by addressing cross-modality gradient heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Autoregressive next-token training struggles with modality competition, destabilizing optimization and limiting large-batch scaling.

Method: Proposes ML-FOP-SOAP with Multi-Level Variance Correction, Fisher-Orthogonal Projection, and hierarchical folding for fine-grained variance capture.

Result: Achieves consistent gains in visual generation and textual understanding, with 1.4× sample efficiency and 1.5× faster training than AdamW.

Conclusion: ML-FOP-SOAP offers a robust optimizer for scaling multimodal foundation models, improving stability and efficiency.
...
TL;DR: Res²CLIP proposes a framework to address challenges in few-shot anomaly detection by shifting multimodal alignment into a unified residual space, resolving cross-granularity and domain shift issues.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based methods struggle with coarse-grained text prompts and domain shifts, degrading generalization. Res²CLIP aims to improve adaptation to fine-grained differences and maintain open-world generalization.

Method: The framework shifts alignment to a residual space, introducing three branches: text prompt-based, visual prompt-based, and residual-to-residual alignment. Learnable optimizations are constrained within the residual domain.

Result: Experiments show Res²CLIP effectively addresses cross-granularity mismatch and cross-category generalization degradation, outperforming existing methods.

Conclusion: Res²CLIP advances few-shot anomaly detection by leveraging residual alignment, offering a scalable and generalizable solution for real-world scenarios.
...
TL;DR: MAgSeg is a decoder-free MLLM approach for segmenting fragmented agricultural landscapes in the Global South, overcoming context length and domain alignment challenges.


<details>
  <summary>Details</summary>
Motivation: Agricultural segmentation in the Global South faces challenges like fragmented plots, high intra-class variance, and limited labeled data, which existing MLLMs struggle with due to context length and domain alignment issues.

Method: MAgSeg introduces a decoder-free MLLM approach with a novel instruction tuning data format, enabling segmentation from high-resolution satellite imagery without auxiliary vision decoders.

Result: MAgSeg outperforms state-of-the-art MLLM baselines in evaluations across three Global South countries.

Conclusion: MAgSeg provides a scalable solution for mapping smallholder agricultural landscapes, addressing key limitations of current MLLM approaches.
...
TL;DR: VLA-AD is a distillation framework that transfers large VLA policies into smaller, efficient student policies using a Vision-Language Model for offline semantic supervision, achieving significant reductions in model size and inference time without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: Large VLA policies face challenges in real-time closed-loop control due to their size and high inference costs. VLA-AD aims to address this by distilling these policies into lightweight versions while maintaining performance.

Method: VLA-AD uses a Vision-Language Model to provide high-level semantic guidance (e.g., task phase anchors and directional descriptions) alongside low-level action imitation during training, enabling the student policy to operate independently at test time.

Result: The 158M-parameter student policy matches the performance of the OpenVLA-7B teacher with only a 0.27% average gap, achieving a 44x reduction in size and 3.28x speedup, running at 12.5 Hz on an RTX 4090.

Conclusion: VLA-AD demonstrates that offline semantic guidance can enhance the efficiency, robustness, and deployability of VLA policies, making them more practical for real-world applications.
...
TL;DR: IVGT is an Implicit Visual Geometry Transformer that reconstructs continuous 3D geometry from unposed multi-view images, outperforming existing methods by learning a neural scene representation and supporting tasks like novel view synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D geometry reconstruction from multi-view images often lack geometric continuity and suffer from redundancy. IVGT addresses this by implicitly modeling continuous geometry.

Method: IVGT uses an implicit neural scene representation in a canonical coordinate system, enabling continuous spatial queries to predict SDF values and colors. It is trained with 2D supervision and 3D regularization.

Result: IVGT generalizes well across scenes and excels in tasks like mesh reconstruction, novel view synthesis, depth estimation, and camera pose estimation.

Conclusion: IVGT provides a robust solution for coherent 3D reconstruction from unposed images, demonstrating versatility and strong performance across multiple applications.
...
TL;DR: OP-Mix is a unified data mixing algorithm for language model training, addressing pretraining and continual learning phases efficiently by simulating candidate mixtures using low-rank adapters, resulting in significant performance gains and compute savings.


<details>
  <summary>Details</summary>
Motivation: Data mixing is crucial for model quality but existing methods are fragmented, addressing only one phase or lacking principled guidance. A unified solution is needed.

Method: OP-Mix uses low-rank adapters to simulate candidate data mixtures cheaply, eliminating proxy models and aligning with actual learning dynamics.

Result: OP-Mix improves pretraining perplexity by 6.3%, matches retraining and distillation in continual learning with 66% and 95% less compute respectively.

Conclusion: OP-Mix reframes language model training as a continuous learning process, offering a scalable and efficient solution for data mixing across all phases.
...
TL;DR: Analysis of 130,486 literary translations reveals a negative correlation between fluency (measured by translationese classifier) and faithfulness (measured by COMET-KIWI), indicating a tradeoff in literary translation. The effect is consistent for human and Google Translate but weaker for TranslateGemma.


<details>
  <summary>Details</summary>
Motivation: To investigate whether fluency in large language model (LLM) translations corresponds to semantic preservation in literary texts.

Method: Analyzed 130,486 translated paragraphs from 106 novels in 16 languages, comparing human, Google Translate, and TranslateGemma translations. Fluency was measured using a translationese classifier (POS n-grams), and faithfulness with COMET-KIWI, controlling for paragraph length.

Result: Found a consistent negative correlation between fluency and faithfulness for human and Google Translate, but a weaker, often non-significant correlation for TranslateGemma.

Conclusion: Segment length impacts automatic evaluation, and there is a tradeoff between fluency and faithfulness in literary translation.
...
TL;DR: DiscoExplorer is a new open-source web interface for analyzing discourse relations across 16 languages, addressing challenges in standardization and accessibility of datasets.


<details>
  <summary>Details</summary>
Motivation: The study aims to overcome the difficulties in comparing discourse relations across languages due to data complexity and lack of accessible tools.

Method: The paper introduces DiscoExplorer, a local-computer-compatible web interface, and makes datasets from the DISRPT Shared Task publicly available.

Result: The tool provides query language, search, and visualization features for discourse relations and signaling devices, demonstrated through example studies.

Conclusion: DiscoExplorer facilitates cross-linguistic research on discourse relations by offering an accessible and standardized platform for analysis.
...
TL;DR: The paper analyzes a large-scale Ukrainian court citation graph, revealing unsupervised legal domain boundaries and high accuracy in predicting legislative importance.


<details>
  <summary>Details</summary>
Motivation: The study aims to uncover how judicial citation structures can encode legal domain boundaries and predict legislative importance without supervision, leveraging massive court decision data.

Method: The authors construct a citation graph from 99.5 million court decisions, using regex for citation extraction, and apply Louvain community detection for domain identification and citation features for predictive modeling.

Result: Key findings include a power-law degree distribution, successful unsupervised recovery of legal domains, and near-perfect accuracy in predicting top legislative articles. The citation entropy spike detected the 2022 invasion.

Conclusion: The citation-derived ontology is integrated into an LLM-assisted legal analysis system, demonstrating practical applications. The data and methods are released openly for further research.
...
TL;DR: Humans communicate similarly to greedy algorithms under vocabulary constraints, with skilled individuals showing non-greedy behaviors like backtracking.


<details>
  <summary>Details</summary>
Motivation: To understand how humans adapt to limited vocabulary constraints and compare their behavior to computational sampling algorithms.

Method: Studied human responses to questions under vocabulary limitations (max 250 words) and compared them to greedy and globally optimal sampling algorithms using Sequential Monte Carlo inference with large language models.

Result: Humans resemble greedy sampling but skilled individuals exhibit non-greedy behaviors. Both humans and algorithms rely on semantically light words in high-constraint settings.

Conclusion: The findings highlight human adaptability under constraints and offer insights for cognition, psycholinguistics, L2 communication, and language impairments.
...
### [102] [Response-Conditioned Parallel-to-Sequential Orchestration for Multi-Agent Systems](https://arxiv.org/abs/2605.15573)
*Nurbek Tastan,Alex Iacob,Lorenzo Sani,Meghdad Kurmanji,Nicholas D. Lane,Samuel Horvath,Karthik Nandakumar*

Main category: cs.CL

TL;DR: Nexa introduces a hybrid multi-agent collaboration framework combining parallel and sequential modes to optimize communication, latency, and accuracy using a trainable response-conditioned policy.


<details>
  <summary>Details</summary>
Motivation: Existing parallel and sequential collaboration modes fail to balance communication efficiency, latency, and response accuracy.

Method: Nexa uses a parallel execution stage, embeds responses in a shared semantic space, predicts a sparse directed acyclic graph for sequential propagation, and employs a lightweight transformer policy.

Result: Nexa's policy is reusable across different agent numbers, tasks, or agent changes, showcasing generalizability.

Conclusion: Nexa bridges the gap between parallel and sequential collaboration, offering a scalable and efficient solution for multi-agent systems.
...
TL;DR: Adesua, a WhatsApp-based AI Teaching Assistant, provides science education support in West Africa by integrating textbooks and exam questions with AI for conversational learning and automated assessments.


<details>
  <summary>Details</summary>
Motivation: Address high student-teacher ratios and teacher shortages in Sub-Saharan Africa by leveraging WhatsApp's accessibility to offer personalized learning and formative assessment.

Method: Adesua combines curated textbooks and past exam questions with generative AI to enable question answering and automated feedback via WhatsApp.

Result: A 6-month trial in Ghana showed 93.75% perceived usefulness of AI answers, though with limited user ratings (n=16).

Conclusion: Preliminary results suggest potential for scalable, low-cost learning support in resource-constrained settings, warranting further evaluation.
...
TL;DR: Eskwai for Students, a generative AI assistant for legal education in Ghana, was developed and deployed, showing promising results and ethical considerations in use.


<details>
  <summary>Details</summary>
Motivation: To address the limited development and deployment of generative AI systems for legal education in the Global South, particularly Ghana.

Method: Developed a retrieval-augmented generation (RAG) system using a curated database of 12K case laws and 1.4K legislation, deployed in a 30-month study with 3.1K students.

Result: Used by 3.1K students for 32K queries, showing helpfulness but raising ethical concerns about query types.

Conclusion: Provides insights into responsible use of generative AI for legal education in the Global South.
...
TL;DR: The paper introduces Capability Conditioned Scaffolding to address Professional Domain Drift by conditioning AI interventions on structured capability profiles.


<details>
  <summary>Details</summary>
Motivation: Existing personalization in large language models fails to account for differences in user evaluation capacity across expertise domains, leading to unreliable reliance on AI in areas of weak expertise.

Method: The proposed framework, Capability Conditioned Scaffolding, partitions expertise into strong, mixed, and weak domains and conditions AI intervention behavior on structured capability profiles.

Result: A pilot evaluation across MMLU subsets and LLM substrates demonstrates consistent intervention behavior, including categorical inversion and selective activation in mixed domains.

Conclusion: Capability-aware scaffolding can enhance reliable human-AI collaboration by addressing domain-specific evaluation gaps.
...
TL;DR: Analysis of six LLM architectures across 12 cognitive tasks shows encoder-decoder differences in activation, attention entropy, and sparsity, with math tasks having highest attention entropy and decoders being sparser.


<details>
  <summary>Details</summary>
Motivation: To understand how different LLM architectures process cognitive tasks, focusing on neural activation and attention patterns.
...
Result: Mathematical reasoning has highest attention entropy; decoder models are sparser than encoder models.

Conclusion: Insights into LLM computational traits can guide model selection and optimization for big data tasks.
...
TL;DR: LM surprisals predict reading times but underpredict syntactic ambiguity difficulty, possibly due to LMs considering more simultaneous parses than humans. Testing with RNNGs shows this difference doesn't fully explain the gap.


<details>
  <summary>Details</summary>
Motivation: To understand why language model (LM) surprisals underpredict human processing difficulty in syntactic ambiguities, particularly garden path sentences.

Method: Used Recurrent Neural Network Grammars (RNNGs) with word-synchronous beam search to vary the number of simultaneous parses for surprisal computation and compared results with human reading times.

Result: Reducing simultaneous active parses increased predicted garden path effects but not enough to match human-level difficulty.

Conclusion: Differences in the number of simultaneous parses between LMs and humans don't fully account for the mismatch in surprisal predictions.
...
TL;DR: The study investigates how reasoning-trained language models' hidden-state trajectories during chain-of-thought (CoT) generation differ from mere token count adjustments, focusing on competitive programming, mathematics, and Boolean satisfiability. After adjusting for generation length, difficulty correlates with trajectory geometry, especially in programming where harder problems show more direct paths. Behavioral annotations link stronger coupling to strategy shifts and uncertainty monitoring.


<details>
  <summary>Details</summary>
Motivation: To distinguish whether longer CoT generations in reasoning-trained models reflect deeper computation or just more steps, and to understand how difficulty interacts with trajectory geometry across domains.

Method: Analyzed hidden-state trajectories during CoT generation in competitive programming, mathematics, and Boolean satisfiability, adjusting for generation length to isolate difficulty effects. Used linear probes and behavioral annotations to explore strategy and uncertainty.

Result: After length adjustment, difficulty systematically correlates with trajectory geometry, most clearly in programming (direct paths, less curvature). Mathematics and Boolean satisfiability show weaker but present coupling. Behavioral annotations indicate strategy shifts and uncertainty monitoring.

Conclusion: Length correction is essential for trajectory analysis. Reasoning training associates with distinct trajectory geometry, strongest in programming, with domain-dependent strength. Strategy and uncertainty play roles in corrected coupling.
...
TL;DR: A modular RAG pipeline is proposed to extract and normalize observations from nurse-patient transcripts, achieving 80.36% F1 score with GPT-5.2, full schema, RAG, and auditing.


<details>
  <summary>Details</summary>
Motivation: Clinicians spend significant time on documentation, and converting conversational transcripts into structured formats at scale is challenging. The MEDIQA-SYNUR task aims to address this by extracting observations from transcripts.

Method: The approach uses a modular RAG pipeline with schema-constrained prompting, deterministic postprocessing, and a second-pass audit, tested with Llama-4 and GPT-5.2 models.

Result: The best configuration (GPT-5.2, full schema, RAG, auditing) achieved an 80.36% F1 score. RAG consistently improved performance, and auditing provided modest gains.

Conclusion: RAG enhances performance, schema constraint effectiveness varies by model, and auditing helps correct residual errors.
...
TL;DR: FINESSE-Bench introduces a suite of specialized benchmarks for hierarchical evaluation of financial competencies in LLMs, addressing gaps in current financial benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing financial benchmarks lack explicit hierarchy and comprehensive evaluation of foundational to expert-level financial reasoning.

Method: FINESSE-Bench includes eight benchmarks (3,993 questions) inspired by professional certifications, trading tasks, and olympiads, with a unified evaluation protocol.

Result: The benchmarks evaluate domain breadth, performance degradation with difficulty, computational tasks, and specialized financial domains.

Conclusion: FINESSE-Bench complements existing benchmarks and enables substantive evaluation of professionally relevant financial competencies in LLMs.
...
TL;DR: RoPE in Transformer-based long-context models loses locality bias and token relevance consistency as context length increases, failing to distinguish positions or tokens effectively. Adjusting hyperparameters can't fix both issues simultaneously, suggesting the need for new mechanisms.


<details>
  <summary>Details</summary>
Motivation: The paper aims to uncover limitations in Rotary Positional Embeddings (RoPE) for long-context language models, focusing on their failure to maintain locality and consistency as context length grows.

Method: Theoretical analysis of RoPE's behavior with increasing context length, examining locality bias and token relevance consistency. Empirical validation confirms these limitations persist despite architectural adjustments.

Result: RoPE-based attention becomes unpredictable, losing locality and consistency, with failure probabilities approaching randomness. Hyperparameter adjustments trade off token and position distinction but can't resolve both.

Conclusion: Current RoPE mechanisms are inherently flawed for long-context models, necessitating new approaches to positional and token order encoding in Transformers.
...
TL;DR: DetectRL-X is a multilingual benchmark evaluating LLM-generated content detectors across 8 languages and 6 domains, incorporating real-world usage patterns and attacks to assess detector performance.


<details>
  <summary>Details</summary>
Motivation: The study addresses the critical need for reliable detection of LLM-generated content in multilingual, real-world scenarios, where current detectors' performance is underexplored.

Method: The researchers created DetectRL-X, a benchmark with human-written and LLM-generated texts in 8 languages and 6 domains, including AI-assisted writing operations and multilingual paraphrasing/perturbation attacks.

Result: Experimental results highlight the strengths and limitations of state-of-the-art detectors, showing how factors like domains, generators, and attacks impact performance across languages.

Conclusion: DetectRL-X proves effective for evaluating and improving multilingual and language-specific detectors, addressing real-world challenges in LLM-generated content detection.
...
TL;DR: BetaPRM introduces a distributional Process Reward Model that predicts step-level success probability and reliability, improving decision-making in reasoning tasks by distinguishing trustworthy rewards.


<details>
  <summary>Details</summary>
Motivation: Current PRMs provide single reward scores without reliability indicators, leading to potential misuse of imperfect step-level rewards. BetaPRM addresses this by predicting both success probability and reliability.

Method: BetaPRM uses a Beta-Binomial likelihood to model step-success supervision from Monte Carlo continuations, learning a Beta belief instead of regressing to point targets. This provides a reliability signal.

Result: BetaPRM enhances PRM-guided Best-of-N reasoning, improving accuracy while reducing token usage by up to 33.57%. Adaptive Computation Allocation (ACA) further optimizes this process.

Conclusion: BetaPRM and ACA offer a more reliable and efficient approach for step-level reward prediction and reasoning, outperforming fixed-budget methods in both accuracy and computational efficiency.
...
TL;DR: The paper explores draft-conditioned latent refinement models for non-autoregressive text generation, emphasizing the challenges of continuous latent states and discrete tokens. It highlights the importance of decoder recoverability over latent geometry.


<details>
  <summary>Details</summary>
Motivation: To address the difficulty of interfacing between continuous latent states and discrete tokens in non-autoregressive text generation using diffusion and flow models.

Method: Uses a draft-conditioned latent refinement model with a frozen BERT encoder, parallel decoder, denoising DraftPrior, local FlowNet, and learned diagonal MetricNet. Evaluates recoverability and refinement techniques.

Result: Full 768-dimensional BERT latents outperform compressed 256-dimensional ones. DraftPrior performance degrades with token dropout. Flow refinement and decoder-aware readout offer minor gains, while metric learning doesn't fully bridge the decoder gap.

Conclusion: Latent geometry alone is insufficient; continuous latent text generation should prioritize decoder recoverability, start distribution quality, and refinement's preservation of decoder-readable structure.
...
TL;DR: GiLT enhances Transformer language models with dependency graphs, improving syntactic generalization without adding structural tokens, while maintaining competitive perplexity.


<details>
  <summary>Details</summary>
Motivation: To improve syntactic generalization in language models by leveraging dependency graphs, avoiding the need for extra structural tokens.

Method: GiLT modulates Transformer attention weights using features from incrementally constructed dependency graphs.

Result: GiLT achieves better syntactic generalization and competitive perplexity, and can be finetuned for improved downstream task performance.

Conclusion: GiLT demonstrates the effectiveness of dependency graphs in enhancing Transformer models for syntactic tasks.
...
TL;DR: The study analyzes activation magnitudes in modern open LLMs, revealing variations across families, generations, and training stages, and their impact on low-bit quantization.


<details>
  <summary>Details</summary>
Motivation: To understand the dynamic range of activations in modern open LLMs for better deployment, particularly in low-bit quantization and stable inference.

Method: A unified pipeline was used to measure global and layerwise maxima across 27 checkpoints from 8 open LLM families, including dense, MoE, and instruction-tuned variants.

Result: Activations vary by nearly four orders of magnitude, with Qwen3.5 and MoE models showing lower peaks. MoE models have 14-23x lower peaks than dense counterparts, and the residual stream often carries the global maximum.

Conclusion: Activation magnitude is tied to model family, architecture, and training stage, not just size. It should be measured and reported before low-bit deployment.
...
TL;DR: A KG-grounded benchmark (MHGraphBench) evaluates LLMs on mental health tasks, revealing gaps in relation prediction and reasoning despite high entity recognition, with performance influenced by output format and KG snippets.


<details>
  <summary>Details</summary>
Motivation: To assess how well LLMs capture and apply biomedical knowledge in mental health, focusing on entity recognition, relation judgment, and reasoning.

Method: Developed a benchmark using PrimeKG, with nine task families and controlled negative options, tested on 15 LLMs.

Result: LLMs perform well on entity recognition but struggle with relation prediction and two-hop reasoning; KG snippets and output format affect performance.

Conclusion: MHGraphBench measures agreement with a curated KG under specific conditions, not real-world clinical safety.
...
TL;DR: The paper investigates whether large language models (LLMs) can transfer code generation skills to languages not encountered during pretraining, revealing a persistent gap in semantic competence despite fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To understand if LLMs can generalize their code generation abilities to languages absent from their pretraining data, highlighting potential limitations in current methods.

Method: Introduced PyLang, an unfamiliar imperative language, and evaluated Qwen3 models (4B, 8B, 32B) on 352 problems, testing zero-shot and fine-tuned performance, and analyzing internal representations.

Result: Fine-tuning improves syntax but not semantic competence; Python outperforms PyLang by up to 19%. Models share internal representations (CKA > 0.97) but fail at output-stage translation.

Conclusion: LLMs exhibit a language-agnostic algorithmic understanding but struggle to implement it in unfamiliar languages, calling for methods to decouple reasoning from language-specific execution.
...
TL;DR: PSD is a training-free framework for dLLMs that improves inference efficiency by parallel speculative decoding, achieving up to 5.5x tokens per forward pass without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing dLLMs suffer from high inference costs due to iterative denoising steps. The paper aims to reduce this cost by optimizing both spatial (unmasking multiple tokens) and temporal (collapsing steps) aspects.

Method: PSD uses confidence scores from a single forward pass to select positions for unmasking and constructs multi-depth speculative drafts without extra model calls. It then verifies drafts hierarchically in a batched pass.

Result: Experiments on three dLLMs show PSD achieves up to 5.5x tokens per forward pass with accuracy comparable to greedy decoding, balancing efficiency and quality.

Conclusion: PSD effectively reduces inference costs for dLLMs while maintaining generation quality, offering a practical solution for efficient text generation.
...
TL;DR: LLMs are biased toward English, and continual pre-training for adaptation doesn't cost less than training from scratch, highlighting the need for dedicated per-language investment.


<details>
  <summary>Details</summary>
Motivation: To address the heavy bias of LLMs toward English and evaluate the effectiveness of continual pre-training for adapting LLMs to other languages.

Method: Analysis of sequences generated by open-weight LLMs and comparison between continual pre-training and training from scratch for language adaptation.

Result: Continual pre-training offers no cost advantage over training from scratch, even for cultural understanding in the target language.

Conclusion: Dedicated per-language investment is crucial for future LLM development instead of relying on expanding English-centric resources.
...
TL;DR: The paper introduces CHA-Gen, a scalable Chinese ambiguity dataset based on Potential Ambiguity Theory, examines LLMs' struggles with ambiguity detection, and provides insights into LLMs' handling of ambiguity.


<details>
  <summary>Details</summary>
Motivation: Existing Chinese ambiguity datasets lack scalability, and most research on LLMs' ambiguity handling is focused on English. This paper aims to fill this gap by creating a robust Chinese ambiguity dataset and analyzing LLMs' performance.

Method: A semi-automatic pipeline guided by Potential Ambiguity Theory is used to construct CHA-Gen. The dataset includes 5,712 sentences (ambiguous and unambiguous) across 18 structures. LLMs are evaluated via direct querying, machine translation, and CoT prompting. Uncertainty is quantified using semantic entropy.

Result: LLMs struggle with ambiguity detection, showing failure modes like ambiguity blindness and misattribution. CoT improves performance. Base models better capture semantic diversity compared to instruction-tuned models, which are overconfident. Models also exhibit bias toward dominant interpretations.

Conclusion: CHA-Gen provides a scalable foundation for Chinese ambiguity research. The study highlights LLMs' challenges with ambiguity and offers insights for improving their handling of ambiguous sentences in Chinese.
...
TL;DR: DCDM replaces positional blocks in diffusion models with content-defined semantic chunks, improving performance over baselines.


<details>
  <summary>Details</summary>
Motivation: Positional blocks in diffusion models waste semantic coherence by grouping unrelated tokens. DCDM addresses this by using content-defined chunks.

Method: DCDM employs Chunking Attention to route tokens into $K$ learnable clusters, enabling autoregressive denoising over semantic chunks.

Result: DCDM outperforms unstructured and positional-block diffusion baselines, with stable advantages across scales and early in training.

Conclusion: Dynamic chunking in diffusion models improves sequence modeling by leveraging semantic coherence.
...
TL;DR: The paper introduces a Diagram-as-Code paradigm using mxGraph XML for precise diagram generation and editing, addressing limitations of pixel-based methods. It presents VCG-Bench, a benchmark for structured diagram tasks, and evaluates SOTA VLMs.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in current VLMs' ability to handle structured, controllable diagrammatic tasks, which are crucial for professional workflows.

Method: Proposes a Diagram-as-Code paradigm with symbolic logic, using mxGraph XML, and introduces VCG-Bench with a dataset, paradigm definition, and tailored evaluation protocol.

Result: Highlights challenges faced by SOTA VLMs in structured fidelity and instruction compliance, showcasing their vision and reasoning limitations.

Conclusion: The proposed method and benchmark demonstrate the need for improved VLMs in handling structured diagram tasks.
...
TL;DR: LLMs like Claude Haiku can assist in patient inquiry triage under low-resource labeling but require human oversight, not autonomous use.


<details>
  <summary>Details</summary>
Motivation: Online patient inquiries are informal and incomplete, needing accurate routing to appropriate clinical follow-up levels.

Method: Used HealthCareMagic-100K to create labeled sets, compared TF-IDF and BioBERT with six prompted LLMs under 0, 4, and 12-shot conditions.

Result: Claude Haiku (12-shot) achieved macro-F₁ 0.475, outperforming BioBERT (0.378). Few-shot prompting and model agreement improved specific tasks.

Conclusion: LLMs can aid triage prioritization but need human review; autonomous deployment isn't recommended.
...
TL;DR: ASRU is a controllable multimodal unlearning framework that improves both unlearning effectiveness and generation quality by integrating activation redirection and a customized reward function.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning methods for MLLMs focus on output deviations but neglect generation quality, leading to issues like hallucinations or rigid responses, which compromise usability and safety.

Method: ASRU uses activation redirection to induce initial refusal behavior and optimizes fine-grained refusal boundaries with a customized reward function, balancing knowledge unlearning and model utility.

Result: Experiments on Qwen3-VL show ASRU improves unlearning effectiveness by 24.6% and generation quality by 5.8x while preserving model utility with minimal retained data.

Conclusion: ASRU effectively addresses the trade-off between unlearning and model utility, offering a practical solution for safe and usable MLLM unlearning.
...
TL;DR: H-Mem is a hybrid memory mechanism for LLM-based agents, combining temporal-semantic tree and knowledge graph structures to improve memory evolution modeling and retrieval, achieving state-of-the-art QA performance.


<details>
  <summary>Details</summary>
Motivation: Existing memory mechanisms in LLM-based agents lack effective modeling of memory evolution and retrieval, leading to poor memory utilization.

Method: H-Mem uses a hybrid structure with a temporal-semantic tree for memory evolution and a knowledge graph for entity relationships, enhancing retrieval.

Result: H-Mem achieves state-of-the-art performance on QA tasks across three agent memory benchmarks.

Conclusion: H-Mem addresses gaps in memory modeling and retrieval, demonstrating superior performance in memory utilization for LLM-based agents.
...
TL;DR: NCCE shifts context engineering from global optimization to personalized, instance-wise routing, boosting LLM task accuracy by dynamically matching inputs with optimal context strategies.


<details>
  <summary>Details</summary>
Motivation: Existing context engineering methods focus on global optimization, missing instance-level performance gains by not tailoring contexts to individual inputs.

Method: NCCE uses a catalog of anchor contexts and a Context-CF Co-Evolution mechanism, where a lightweight Neural Collaborative Filtering model guides context generation and routes optimal strategies per instance.

Result: Theoretical and experimental results show NCCE significantly improves task accuracy by personalizing context strategies for each input.

Conclusion: Personalization in context engineering is critical, and NCCE effectively demonstrates its benefits through dynamic, instance-wise routing.
...
TL;DR: DimMem is a lightweight dimensional memory framework for LLMs, improving efficiency and accuracy in memory tasks.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems for LLMs trade fidelity for efficiency, often losing structural details needed for precise recall.

Method: DimMem represents memories as atomic, typed units with explicit fields (time, location, etc.), enabling structured retrieval and updates.

Result: DimMem achieved 81.43% and 78.20% accuracy on benchmarks, reducing token costs by 24% and outperforming existing systems.

Conclusion: Explicit dimensional structuring is effective and efficient for long-term memory in LLM agents, with learnable extraction by compact models.
...
TL;DR: Smaller, open-source LLMs (<30B parameters) offer a privacy-preserving, cost-effective alternative to massive proprietary models for Quality Estimation in machine translation, achieving competitive results with human judgments.


<details>
  <summary>Details</summary>
Motivation: Addressing data privacy concerns and high costs associated with proprietary large language models (LLMs) in Quality Estimation (QE) for machine translation.

Method: Using smaller, open-source LLMs with a single-pass prompting strategy to generate quality scores, MQM error annotations, suggested corrections, and full post-editions.
...
Conclusion: Smaller, open-source LLMs effectively approximate the capabilities of much larger proprietary LLMs in QE, offering a viable, privacy-preserving solution.
...
TL;DR: ForMaT is a multilingual translation corpus of 3,956 PDFs across 15 language pairs, preserving layout metadata for multimodal machine translation, addressing challenges in spatial grounding and geometric synchronization.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current MT systems in handling spatial grounding and geometric synchronization, which often lose the link between text and its visual context.

Method: Employ K-Medoids sampling over 45 geometric features to ensure structural diversity, focusing on visually diverse PDF documents with complex elements like nested tables and formulas.

Result: ForMaT serves as a benchmark for developing layout-aware translation models that integrate visual and textual context for high-fidelity document reconstruction.

Conclusion: ForMaT is a valuable resource for advancing multimodal machine translation by preserving layout metadata and addressing the gap in geometric synchronization.
...
TL;DR: Block attention improves KV cache reuse but faces challenges in text segmentation and inefficient fine-tuning. SemanticSeg dataset and block distillation framework address these, achieving near-full-attention performance.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance block attention's practical use in long-context tasks like RAG by solving segmentation and fine-tuning inefficiencies.

Method: 1) Created SemanticSeg dataset for training a segmenter. 2) Proposed block distillation with block sink tokens, block dropout, and token-level loss weighting.

Result: The segmenter outperforms baselines, and block distillation achieves performance close to full attention.

Conclusion: The approach provides a scalable and practical solution for deploying block attention.
...
TL;DR: The paper explores applying Group Relative Policy Optimization (GRPO) to NLLB-200 encoder-decoder models for machine translation fine-tuning, achieving consistent improvements across 13 languages without parallel data.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning approaches for MT fine-tuning focus on decoder-only large language models, leaving encoder-decoder architectures underexplored. This work aims to bridge that gap.

Method: GRPO is applied to NLLB-200 models using a hybrid reference-free reward (LaBSE and COMET-Kiwi) without parallel data during fine-tuning. Evaluations cover 13 diverse languages.

Result: GRPO improves performance on all 13 languages, with gains up to +5.03 chrF++ for Traditional Chinese. It matches 3-epoch supervised fine-tuning on morphologically complex languages without target-language data.

Conclusion: The approach is most effective where baseline performance is weak and reward discriminability is high, making it valuable in low-resource settings. The pattern holds across English and Spanish source languages.
...
TL;DR: A framework using symbolic methods to extract and link evidence from law enforcement narratives, achieving high accuracy in specific categories.


<details>
  <summary>Details</summary>
Motivation: Manual reading of law enforcement narratives is time-consuming; structured facts are needed for review, training, and investigations.

Method: Symbolic methods involving redaction, semantic parsing, predicate mapping to ontology, and reasoning to convert narratives into facts.

Result: 54.1% of extracted events had a confidence score ≥0.80; 93.7% mapped via PropBank--VerbNet--WordNet. Full agreement on initiation, stolen items, and time cues.

Conclusion: Symbolic methods effectively extract structured facts from narratives, with room for improvement in areas like forced entry interpretation.
...
TL;DR: The paper proposes a three-level taxonomy (Cultural Awareness, Sensitivity, Competence) to clarify and improve the evaluation of cultural capabilities in AI systems, addressing current ambiguities and practical deployment risks.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of AI cultural capabilities lack clear definitions and focus, often limiting assessments to factual recall rather than deeper cultural understanding and adaptability.

Method: The authors draw from Intercultural Communication scholarship to develop a three-level taxonomy: Cultural Awareness (knowing), Sensitivity (framing knowledge), and Competence (adapting).

Result: The taxonomy provides a structured framework to enhance the validity and interpretability of AI evaluations in multicultural settings.

Conclusion: Clearer definitions of cultural capabilities are essential to avoid overestimating AI performance and ensure appropriate deployment in culturally sensitive contexts.
...
TL;DR: The paper explores whether vision language models (VLMs) can adapt to different learner profiles in math education, proposing a rubric for evaluating their adaptivity.


<details>
  <summary>Details</summary>
Motivation: VLMs are used in math education for personalized instruction, but their ability to adapt to diverse learner profiles is unverified. There's no systematic framework to evaluate this adaptivity.

Method: The study uses a learner model from adaptive learning literature to create a rubric assessing adaptivity across cognitive, motivational, and complexity aspects. It also evaluates correctness and quality of VLM responses.

Result: VLMs show measurable differences in adaptivity but struggle to consistently provide learner model-based responses, especially with limited learner information.

Conclusion: Current VLMs lack consistent adaptivity in math instruction, highlighting the need for improved evaluation and model capabilities.
...
TL;DR: The paper investigates how LLMs' scoring inconsistencies arise from output format changes (e.g., 1-5 ratings vs. True/False) and identifies a shared latent evaluator sub-graph in MLPs that governs judgments independently of formatting.


<details>
  <summary>Details</summary>
Motivation: To understand the internal mechanisms causing format-induced inconsistencies in LLM-as-a-judge systems, which existing methods only diagnose at the input-output level.

Method: Uses Position-aware Edge Attribution Patching (PEAP) to causally analyze Gemma-3, Qwen2.5, and Llama-3, identifying a sparse, generalized Latent Evaluator sub-graph in MLPs. Tests its role via zero-ablation.

Result: Judgments share a common evaluator sub-graph; zero-ablating it collapses judgments without affecting world knowledge. Format inconsistencies stem from fragile, format-specific terminal branches mapping a continuous judgment signal.

Conclusion: Benchmark reliability comparisons across formats may partly reflect formatter geometry rather than evaluation quality, highlighting the need to decouple abstract judging from output formatting.
...
TL;DR: S2ST-Omni 2 improves multilingual speech-to-speech translation by leveraging structured typological priors instead of flat language labels, enhancing data efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Existing S2ST systems often neglect shared linguistic structures across languages, limiting performance with scarce supervised data.

Method: Proposes S2ST-Omni 2, incorporating typology-informed hierarchical encoding, dynamically-gated Dual-CTC, and typology-aware LLM prompting.

Result: Achieves superior performance on CVSS-C across metrics (BLEU, COMET, etc.) and shows data-efficient multilingual adaptation.

Conclusion: Structured typological priors provide useful inductive biases, improving multilingual S2ST, especially with limited training data.
...
TL;DR: RecMem reduces token costs in memory systems by consolidating memory only when recurring interactions occur, using lightweight embeddings and semantic refinement.


<details>
  <summary>Details</summary>
Motivation: Overcoming the high token consumption of eager memory consolidation in existing memory systems by rethinking when consolidation should happen.

Method: RecMem stores interactions in a subconscious layer with lightweight embeddings, invoking LLMs only for recurring, semantically similar interactions. It includes a semantic refinement mechanism.

Result: RecMem cuts token costs by up to 87% while outperforming existing systems in accuracy.

Conclusion: RecMem offers an efficient and accurate alternative to traditional memory consolidation methods.
...
TL;DR: LLM-driven data augmentation improves cognitive score prediction from speech by addressing dataset size and class imbalance issues.


<details>
  <summary>Details</summary>
Motivation: Accurate assessment of cognitive decline from spontaneous speech is challenging due to limited data and class imbalance.

Method: Proposes a GPT-5-driven framework to generate synthetic oral-like monologues using written responses as anchors. Uses Partial Least Squares regression on Sentence-BERT embeddings to predict cognitive scores.

Result: Similarity-guided augmentation reduces prediction error for minority low-score participants and maintains performance for the majority group.

Conclusion: Semantically guided LLM-driven augmentation effectively addresses class imbalance and improves data efficiency in clinical speech analysis.
...
TL;DR: A unified framework analyzes metric-based methods for detecting machine-generated texts (MGTs), addressing token-level bias with multi-level contextual token relations and achieving improved detection performance.


<details>
  <summary>Details</summary>
Motivation: Machine-generated texts pose risks like disinformation, necessitating reliable detection. Metric-based methods are more practical than complex models but face challenges like token-level bias.

Method: Proposes a multi-level framework: a Markov-informed calibration for local token relations and a rule-support reasoning module for global relations, combined in joint inference.

Result: Substantial improvements in detection across diverse scenarios (cross-LLM and cross-domain) with low computational overhead.

Conclusion: The framework effectively addresses token-level biases in MGT detection, offering robust and efficient solutions for real-world applications.
...
TL;DR: The paper proposes DebiasRAG, a tuning-free, dynamic framework to mitigate social biases in LLMs without degrading their capabilities, using retrieval-augmented generation and fairness constraints.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce biased content (e.g., race, gender, age), and existing debiasing methods degrade performance or require extra resources. A dynamic, efficient solution is needed.

Method: DebiasRAG uses three stages: (1) generating query-specific debiasing candidates, (2) constructing a context pool, and (3) reranking contexts with gradient updates. It combines bias contexts with RAG-retrieved data to constrain LLM outputs.

Result: DebiasRAG improves fairness in LLM outputs while preserving their generative capabilities, without requiring fine-tuning or domain expertise.

Conclusion: DebiasRAG offers a novel, efficient approach to dynamic debiasing in LLMs, balancing fairness and performance without additional training.
...
TL;DR: The paper introduces SGR, a framework to enhance LLM reasoning by using external subgraph generation, improving accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with deep reasoning and factual consistency despite their broad NLP capabilities. SGR addresses this by leveraging external knowledge.

Method: SGR generates query-specific subgraphs from external knowledge bases, uses their structure for multi-step inference, and combines reasoning trajectories for predictions.

Result: Experiments on benchmark datasets show SGR consistently outperforms baselines, enhancing reasoning accuracy and factual reliability.

Conclusion: SGR effectively leverages external structured knowledge to improve LLM reasoning, offering a scalable solution for complex tasks.
...
TL;DR: AI coding systems (AntiGravity and ERA) autonomously generate efficient 3DPV structures, overcoming algorithmic reward hacking to discover optimal designs.


<details>
  <summary>Details</summary>
Motivation: To demonstrate how AI coding systems can generate novel scientific hypotheses and improve the efficiency of three-dimensional photovoltaic (3DPV) structures compared to flat solar panels.

Method: Combines a generic coding agent (AntiGravity) with an LLM-driven tree search algorithm (ERA) to generate and optimize 3DPV structures. Initial designs are refined through large-scale tree search, with constraints added to eliminate reward hacking.

Result: ERA discovers improved 3DPV designs with various constraints, including optimal structures that avoid self-shadowing and optimize zenith tracking.

Conclusion: The combination of coding agents and tree search (ERA) offers a powerful platform for scientific discovery in problems with empirically evaluable solutions.
...
TL;DR: The paper introduces a value-based persona method for LLMs to better simulate diverse human opinions across cultures, improving prediction accuracy and response diversity.


<details>
  <summary>Details</summary>
Motivation: Existing methods for simulating human opinions in LLMs rely on indirect proxies like sociodemographics, limiting their ability to reproduce culturally diverse responses. The paper aims to address this gap.

Method: The authors propose a value-based persona construction method, deriving textual descriptors from survey responses to capture cultural dimensions. They sample value profiles from target populations and calibrate LLM responses to improve diversity and accuracy.

Result: The approach reduces prediction error across countries, especially for underrepresented populations, and closely matches human response diversity.

Conclusion: The method narrows the performance gap between dominant and less-represented cultures in LLM training data, offering a more accurate and diverse simulation of human opinions.
...
TL;DR: Argus introduces a cooperative system with a Searcher and Navigator to improve deep research by assembling evidence pieces like a jigsaw, outperforming parallel brute-force methods.


<details>
  <summary>Details</summary>
Motivation: Parallel rollouts in deep research often duplicate evidence instead of complementing it, leading to inefficiencies and context limits.

Method: Argus uses a Searcher to gather evidence traces and a Navigator to manage a shared evidence graph, dispatching Searchers and reasoning over the graph.

Result: Argus improves performance by 5.5 points with one Searcher and 12.7 with 8, reaching 86.2 on BrowseComp with 64 Searchers.

Conclusion: Argus efficiently combines evidence through cooperation, outperforming proprietary agents while maintaining manageable context size.
...
TL;DR: The paper introduces a method inspired by aphasia studies to analyze language models (LMs) by "lesioning" parameters and comparing resulting symptoms to human aphasia profiles. It reveals distinct symptom distributions between LMs and humans.


<details>
  <summary>Details</summary>
Motivation: To understand the functional organization of LMs by drawing parallels to human aphasias, which link brain regions to specific language impairments.

Method: Lesioning (zeroing out) parameters in five 1B-scale LMs and measuring effects against clinical aphasia symptoms using the Text Aphasia Battery (TAB).

Result: LM lesions produced a range of symptoms differing from humans, with notable differences between attention and feed-forward components and effects of layer depth.

Conclusion: Aphasia-like symptoms in LMs are influenced by model-specific learning and processing, suggesting they are not domain-invariant like human aphasias.
...
TL;DR: The paper discusses how smart metering, generative AI, and quantum-inspired optimization are transforming energy utility management.


<details>
  <summary>Details</summary>
Motivation: To explore the impact of emerging technologies on energy utility operations.

Method: Analysis of the integration of smart metering, generative AI, and quantum-inspired optimization.

Result: These technologies are reshaping infrastructure management, customer engagement, and environmental accountability.

Conclusion: The convergence of these technologies is driving significant advancements in energy utility management.
...
TL;DR: The paper proposes an end-to-end framework to enhance utility billing by integrating generative AI for personalized statements, forecasting, and load scheduling.


<details>
  <summary>Details</summary>
Motivation: Utilities need to provide readable bills with carbon data and manage load efficiently under grid and emissions constraints.

Method: The framework unifies generative AI for billing statements, a transformer-based forecaster for consumption estimates, and load scheduling capabilities.

Result: The proposed solution addresses the need for clear billing, accurate consumption forecasting, and efficient grid management.

Conclusion: The framework successfully integrates multiple advanced technologies to modernize utility services.
...
TL;DR: DeepSlide is a human-in-the-loop multi-agent system optimizing the full presentation process, from planning to delivery, outperforming baselines in narrative flow and pacing.


<details>
  <summary>Details</summary>
Motivation: Existing AI slide generators focus on visual plausibility but neglect the delivery process (e.g., pacing, narrative). DeepSlide aims to bridge this gap.

Method: Uses a controllable logical-chain planner, content-tree retriever, Markov-style rendering, and sandboxed execution for reliability.

Result: Matches baselines in artifact quality and excels in delivery metrics like narrative flow and pacing precision across diverse domains.

Conclusion: DeepSlide effectively integrates planning, grounding, and rendering to enhance both presentation artifacts and delivery.
...
### [151] [ICRL: Learning to Internalize Self-Critique with Reinforcement Learning](https://arxiv.org/abs/2605.15224)
*Jianbo Lin,Xiaomin Yu,Yi Xin,Yifu Guo,Zhuosong Jiang,Zhongqi Yue,Weishi Wang,Heqing Zou,Chengwei Qin,Hui Xiong*

Main category: cs.AI

TL;DR: The paper proposes ICRL, a framework to internalize self-critique in large language models using reinforcement learning, improving performance without external critique.


<details>
  <summary>Details</summary>
Motivation: Current models rely on external critique for corrections but fail to retain improvements without it, and critics cannot iteratively improve feedback quality.

Method: ICRL jointly trains a solver and critic from a shared backbone, using reinforcement learning to convert critique success into unassisted solver ability. Includes distribution-calibration re-weighting and role-wise group advantage estimation.

Result: ICRL shows consistent gains (6.4 points on agentic tasks, 7.0 on math reasoning) and achieves critic performance comparable to larger models with fewer tokens.

Conclusion: ICRL enables models to internalize critique, reducing dependency on external feedback and improving autonomous performance.
...
### [152] [Does Theory of Mind Improvement Really Benefit Human-AI Interactions? Empirical Findings from Interactive Evaluations](https://arxiv.org/abs/2605.15205)
*Nanxu Gong,Zixin Chen,Haotian Li,Zishu Zhao,Jianxun Lian,Huamin Qu,Yanjie Fu,Xing Xie*

Main category: cs.AI

TL;DR: The paper introduces interactive ToM evaluation and shows static benchmarks don't always predict dynamic human-AI performance.


<details>
  <summary>Details</summary>
Motivation: Existing ToM benchmarks focus on static, third-person tasks, missing the dynamic, first-person nature of human-AI interactions.

Method: Proposed interactive ToM evaluation paradigm, tested four ToM techniques on real-world datasets and user studies for goal- and experience-oriented tasks.

Result: Improvements in static benchmarks don't consistently enhance dynamic human-AI interactions.

Conclusion: Interaction-based ToM evaluation is essential for developing socially aware LLMs for human-AI symbiosis.
...
### [153] [SkillSmith: Compiling Agent Skills into Boundary-Guided Runtime Interfaces](https://arxiv.org/abs/2605.15215)
*Duling Xu,Zheng Chen,Zaifeng Pan,Jiawei Guan,Dong Dong,Jialin Li,Bangzheng Pu*

Main category: cs.AI

TL;DR: SkillSmith, a boundary-first compiler-runtime framework, reduces redundancy in LLM-based agents by compiling skills into minimal executable interfaces, improving efficiency and cost.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agent systems introduce redundancy through irrelevant context injection and repeated skill-specific reasoning. SkillSmith aims to minimize these inefficiencies.

Method: SkillSmith compiles skill packages offline into minimal executable interfaces by extracting fine-grained operational boundaries, allowing dynamic access to relevant components at runtime.

Result: SkillSmith reduces token usage by 57.44%, thinking iterations by 42.99%, solve time by 50.57%, and cost by 57.44%. It also enables reuse of compiled artifacts by smaller models.

Conclusion: SkillSmith effectively minimizes redundancy in LLM-based agents, improving efficiency, cost, and task accuracy while enabling broader model reuse.
...
TL;DR: The paper introduces the Belief Engine (BE), an auditable layer for LLM-based agents to track and explain stance changes in deliberation tasks, using structured memory and a log-odds update rule.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agents lack transparency in explaining why stances change during deliberation. The paper aims to address this by creating a system that reveals the underlying reasons for stance changes, such as evidence uptake or anchoring.

Method: BE treats belief as an evidential state over a proposition and updates stance using a log-odds rule controlled by evidence uptake (u) and prior anchoring (a). It extracts arguments into structured memory.

Result: BE reliably shapes stance dynamics across multiple LLMs while preserving an evidence-level update trail. On the DEBATE dataset, BE best reconstructs participants whose final stance aligns with extracted evidence.

Conclusion: BE provides a configurable infrastructure for studying evidence-grounded deliberation, tying openness, commitment, and disagreement to explicit update assumptions rather than hidden prompt effects.
...
TL;DR: Instruction-tuned language models show no output-level bias in high-stakes decisions but retain latent demographic biases in their internal representations, which can be exploited to reverse decisions asymmetrically.


<details>
  <summary>Details</summary>
Motivation: To understand whether suppressed demographic biases in internal representations affect model outputs and whether this effect is symmetric across groups.

Method: Used open-weight models for mortgage underwriting with racially-matched applications, activation steering, and cross-layer interventions.

Result: Models show no output bias but amplify demographic representations internally. Interventions caused near-complete decision reversals, revealing asymmetric latent bias.

Conclusion: Behavioral audits focusing on outputs are insufficient; dual-layer testing frameworks (output + representational analysis) are needed for fair AI governance.
...
TL;DR: The paper introduces CAX-Agent, a lightweight agent harness for MAPDL automation, focusing on recovery policies to improve reliability in large language model deployments for finite-element simulations. Empirical evaluation shows the 'model_only' recovery strategy outperforms others.


<details>
  <summary>Details</summary>
Motivation: To address reliability challenges in large language model deployments for MAPDL finite-element simulations, where inconsistent outputs and task failures are common due to lack of structured control and fault recovery.

Method: CAX-Agent organizes execution into three layers (LLM service, agent harness, solver backend) with a recovery ladder. Three recovery strategies (no_recovery, rule_only, model_only) are empirically evaluated on 50 structural benchmarks.

Result: 'Model_only' achieved the best completion rate (0.9267), task score (3.59/4), total score (9.16/10), and zero-intervention rate (0.84), significantly outperforming other strategies with large effect sizes.

Conclusion: The 'model_only' recovery policy is highly effective, demonstrating the potential of CAX-Agent for improving reliability in MAPDL automation. Further validation is needed for broader applications.
...
### [157] [Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP](https://arxiv.org/abs/2605.16205)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: The paper examines design choices for compound LLM agents in adversarial POMDP environments, finding that programmatic state abstraction and task decomposition outperform deliberation tools in cost and performance.


<details>
  <summary>Details</summary>
Motivation: Practitioners lack guidance on effective design choices for LLM agents in adversarial, partially observable environments, leading to potential inefficiencies in performance and inference costs.

Method: A controlled study in CybORG CAGE-2 evaluates various configurations, including context representation, deliberation methods, and hierarchical decomposition, across multiple models.

Result: Programmatic state abstraction significantly improves performance (up to 76% better mean return). Hierarchical decomposition without deliberation achieves the best results, while deliberation tools degrade performance and increase costs.

Conclusion: Invest in programmatic infrastructure and task decomposition over per-agent reasoning for better performance and cost-efficiency in adversarial POMDPs.
...
TL;DR: The paper introduces the NOVA framework to analyze iterative self-improvement in AI, identifying conditions for genuine knowledge accumulation and failure modes like contamination. It also quantifies diminishing returns in discovery and highlights the role of human guidance.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand if AI systems can discover new knowledge autonomously and the conditions or costs involved, addressing potential pitfalls like contamination and exploration failure.

Method: The NOVA framework models the iterative "generate, verify, accumulate, retrain" loop as an adaptive sampling process. It analyzes conditions for knowledge coverage, imperfect verification, and scaling laws for discovery costs under Zipfian assumptions.

Result: The paper identifies failure modes (e.g., contamination, forgetting) and proves a scaling law for cumulative generation costs, showing diminishing returns as discoveries advance. It also clarifies the role of Good-Turing estimation and human amplification.

Conclusion: The work demonstrates the challenges and costs of AI-driven knowledge discovery, emphasizing the need for robust verification and expert input near exploration barriers.
...
### [159] [FORGE: Self-Evolving Agent Memory With No Weight Updates via Population Broadcast](https://arxiv.org/abs/2605.16233)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: FORGE, a memory-evolving protocol for LLM agents, improves decision-making through self-generated memory without gradient updates, outperforming zero-shot and Reflexion baselines in network-defense tasks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance decision-making in LLM agents by leveraging self-generated memory without requiring gradient updates. It addresses the challenge of negative, heavy-tailed rewards in zero-shot settings.

Method: FORGE uses a staged, population-based protocol with a Reflexion-style inner loop. It converts failed trajectories into reusable knowledge artifacts (Rules, Examples, or Mixed) and propagates the best-performing memory across the population.

Result: FORGE improves average evaluation return by 1.7-7.7x over zero-shot and 29-72% over Reflexion, reducing major-failure rates to ~1%. Population broadcast and Examples were key to performance gains.

Conclusion: FORGE effectively mitigates capability gaps in weaker models and improves decision-making without gradient updates, with performance gains tied to population broadcast and memory representation choices.
...
TL;DR: The paper introduces an SDL software architecture using the Model Context Protocol (MCP) to unify interfaces for human users and AI agents, demonstrated via a color-matching SDL case study.


<details>
  <summary>Details</summary>
Motivation: To address the technical demands and lack of standardized interfaces for AI agents in existing SDL frameworks, making SDLs more accessible.

Method: Proposes an MCP-based SDL architecture where functionalities are exposed through MCP servers, introducing the NIMO Controller with a visual programming interface.

Result: The system's usability is validated through a color-matching SDL case study.

Conclusion: The MCP-based architecture successfully provides a unified interface for both human users and AI agents, improving SDL accessibility.
...
TL;DR: The paper introduces a Distributed Trust Framework (DTF) to address the risks of autonomous AI agents in identity-centric authorization systems, ensuring secure, verifiable, and governable agent actions.


<details>
  <summary>Details</summary>
Motivation: Traditional identity-centric authorization assumes valid credentials ensure safe actions, but autonomous AI agents can generate unsafe actions, posing risks in sovereign AI systems. This necessitates a shift to verifiable and distributed trust models.

Method: The paper proposes DTF, which includes a Justification Proof for action admissibility, a consensus model for evaluation, ephemeral Execution Identity, and an append-only Evidence Chain. It ensures no execution without proof, no authority without consensus, and no mutation without evidence.

Result: DTF is instantiated over an OpenKedge-based governed mutation substrate and validated in cloud-native environments, proving its effectiveness in enforcing secure and auditable agent actions.

Conclusion: By transitioning from standing identity to proof-derived authority, DTF provides a scalable and verifiable foundation for governable and bounded agentic execution in sovereign AI deployments.
...
TL;DR: Solvita is an agentic evolution framework designed to improve reasoning in large language models for competitive programming by enabling continuous learning through specialized agents and trainable knowledge networks.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with rigorous reasoning in competitive programming, and existing multi-agent frameworks lack the ability to retain and utilize past problem-solving experience.

Method: Solvita uses four specialized agents (Planner, Solver, Oracle, Hacker) in a closed-loop system, each paired with trainable graph-structured knowledge networks. The system updates these networks dynamically based on outcomes like pass/fail verdicts and adversarial vulnerabilities.

Result: Solvita achieves state-of-the-art performance across CodeContests, APPS, AetherCode, and live Codeforces rounds, outperforming existing multi-agent pipelines and nearly doubling single-pass baseline accuracy.

Conclusion: Solvita's framework effectively accumulates transferable reasoning experience over time, significantly improving the accuracy and reliability of code-generation agents for competitive programming.
...
TL;DR: LaMR, a structured pruning framework, decomposes code relevance into semantic evidence and dependency support, improving efficiency and performance in LLM-powered coding agents.


<details>
  <summary>Details</summary>
Motivation: Existing code pruners compress context into a single score, creating a bottleneck by not distinguishing between different relevance facets like semantic spans and structural support.

Method: LaMR uses two dedicated CRFs for semantic and dependency relevance, a gating network to weight them dynamically, and AST-based program analysis for supervision without extra annotation.

Result: LaMR outperforms full-context baselines, saving 31% more tokens in multi-turn tasks and improving Exact Match by +3.5 in single-turn tasks.

Conclusion: LaMR effectively filters noise, often enhancing performance and efficiency, with minimal marginal drops.
...
TL;DR: The paper evaluates large language models (LLMs) in goal recognition tasks, showing their uneven performance and highlighting it as a benchmark for planning knowledge.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' foundational planning knowledge by examining their performance in goal recognition, a task better suited to their strengths than symbolic reasoning.

Method: Systematic zero-shot evaluation of LLMs on classical PDDL benchmarks, analyzing their ability to integrate evidence for goal recognition.

Result: LLMs show uneven competence; some scale with evidence and approach landmark-based accuracy, while others remain fixed on world-knowledge priors.

Conclusion: Goal recognition serves as a principled benchmark to evaluate LLMs' planning knowledge, revealing differences in evidence integration.
...
TL;DR: Ensemble monitoring improves AI safety by combining diverse signals, outperforming individual or homogeneous monitors in detecting misaligned actions.


<details>
  <summary>Details</summary>
Motivation: To ensure AI systems act safely and align with user intent, especially in scalable autonomous settings where human oversight is impractical.

Method: Built 12 GPT-4.1-Mini monitors using prompting and fine-tuning, evaluated on coding tasks with adversarial inputs.

Result: Diverse ensembles outperformed individual and homogeneous monitors; a 3-monitor ensemble achieved 2.4x better detection. Fine-tuned monitors were key in top-performing ensembles.

Conclusion: Ensemble monitoring, driven by diversity and fine-tuning, is a practical AI control strategy for safety without high inference costs.
...
TL;DR: The paper introduces NSPI, a neuro-symbolic framework combining LLMs and symbolic computation to automate polynomial inequality proofs, achieving scalability for high-variable problems.


<details>
  <summary>Details</summary>
Motivation: Addressing scalability challenges in automated proving of polynomial inequalities, where purely symbolic methods struggle with large variables/degrees and LLM-guided methods are limited to simpler cases.

Method: NSPI uses an LLM to propose an approximate SOS decomposition, refines it symbolically for exact proof, and certifies the proof in Lean, creating an end-to-end pipeline.

Result: Effective and scalable performance on benchmarks with polynomials up to 10 variables.

Conclusion: NSPI successfully combines LLMs and symbolic computation for scalable, reliable polynomial inequality proving.
...
TL;DR: X-SYNTH is a framework for enterprise context synthesis that leverages human attention and behavioral patterns to improve AI agent tasks, achieving a 6.5x increase in True Lead Rate.


<details>
  <summary>Details</summary>
Motivation: The current approach for AI agent tasks relies on matching requests to stored system state, which is lossy and ineffective for complex tasks, leading to low True Lead Rates and high False Lead Rates.

Method: X-SYNTH uses human attention and behavioral traces to create Digital Twin Signatures (DTS) and applies seven distinct attention filters to synthesize relevant context based on behavioral patterns.

Result: X-SYNTH improves True Lead Rate from 9.5% to 61.9% and reduces False Lead Rate from 90.5% to 18.8% in a sales lead identification task.

Conclusion: Enterprise context synthesis is a relevance problem, and human attention serves as the most reliable ground truth, making X-SYNTH a significant improvement over traditional retrieval methods.
...
TL;DR: CAPS introduces a cascaded adaptive pairwise selection framework to optimize verifier compute in large language models, outperforming existing methods while reducing token usage.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency and high computational cost of existing pairwise verification methods in large language models.

Method: CAPS allocates verifier compute non-uniformly along evidence and distribution axes, implementing a four-stage cascade with an optional rescue subroutine.

Result: CAPS outperforms leading pairwise verifiers on 14 of 20 test suites while using only 25.4% of the token budget on code tasks and surpasses pointwise self-verification on all 20.

Conclusion: CAPS provides an efficient and interpretable solution for optimizing verifier compute, with a clear trade-off diagnostic for pre-deployment checks.
...
TL;DR: RTL-BenchMT is an agentic framework for maintaining RTL generation benchmarks, addressing flawed and overfitting cases automatically.


<details>
  <summary>Details</summary>
Motivation: Current RTL benchmarks face challenges like flawed cases and overfitting, which are hard to resolve manually. Reducing human maintenance costs is crucial.

Method: The framework automates the identification and revision of flawed cases and the detection and updating of overfitting cases.

Result: A refined benchmark suite is produced, aiding in thorough analysis and improvement of RTL benchmarks.

Conclusion: RTL-BenchMT addresses critical challenges in RTL benchmarks, reduces manual effort, and provides an open-source refined suite.
...
TL;DR: DRS-GUI is a training-free framework for GUI agents that dynamically searches and grounds instruction-relevant elements in cluttered screenshots, improving performance by 14% in experiments.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agents struggle to accurately identify relevant elements in high-resolution screenshots due to cluttered UI components. The paper aims to mimic human-like dynamic perceptual adjustments to solve this.

Method: Introduces a lightweight UI Perceptor with three human-like actions (Focus, Shift, Scatter) and an Action Planner using Monte Carlo Tree Search (MCTS) to dynamically explore and evaluate regions.

Result: DRS-GUI improves grounding performance by 14% on ScreenSpot-Pro, enhancing both general and GUI-specific MLLMs like Qwen2.5-VL-7B and UGround-V1-7B.

Conclusion: The proposed framework effectively prunes redundant UI elements and generalizes well, offering a significant advancement for GUI grounding tasks.
...
TL;DR: The paper advocates for metacognition as a design principle in AI to enhance accuracy, security, and efficiency. It proposes monitoring systems and resource allocation based on problem difficulty, demonstrated in a Federated Learning case study.


<details>
  <summary>Details</summary>
Motivation: To improve AI performance by integrating metacognitive strategies, inspired by psychology and cognitive science, into system design.

Method: Proposes a metacognitive approach where systems self-monitor and allocate resources dynamically. Introduces a software framework for implementing these strategies in AI applications.

Result: Demonstrates improved learning efficiency, effectiveness, and security in Federated Learning, supported by a practical software framework.

Conclusion: Metacognition is a promising design principle for AI, with practical benefits shown in Federated Learning, though challenges in implementation remain.
...
TL;DR: STAR is a framework for repairing errors in LLM-based root cause analysis (RCA) agents by localizing and fixing issues in specific stages, improving accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based RCA agents are unreliable due to error propagation in reasoning traces, requiring a method to localize and repair these errors.

Method: STAR decomposes RCA into four stages (EP, HS, AS, DR), audits each stage, and uses Fast/Slow Routing and counterfactual evaluation for localization and repair.

Result: STAR improves root cause localization and fault classification, accurately identifies faulty stages, and repairs most errors within one or two replays.

Conclusion: Modeling where RCA agents fail enhances reliability and debuggability, making STAR a promising solution for self-repairing RCA systems.
...
TL;DR: OmniManim is a framework for generating educational animations with improved visual quality by incorporating render feedback, explicit visual planning, and localized repair.


<details>
  <summary>Details</summary>
Motivation: Large language models often produce animations with visual defects (e.g., overlap, misalignment) that are only detectable after execution, necessitating a feedback-aware approach.

Method: OmniManim uses a shared scene state, visual planning (Vision Agent), structured diagnostics, and repair. The Vision Agent predicts keyframe layouts with bounding-box denoising and interpolation-aware optimization.

Result: OmniManim outperforms baseline models and multi-agent frameworks in render quality on the EduRequire-500 dataset, with ablation studies confirming the importance of visual planning.

Conclusion: Explicit visual planning, especially coarse spatial priors and interpolation-aware optimization, is key to generating high-quality educational animations without visual defects.
...
TL;DR: TopoEvo is a topology-aware self-evolving multi-agent framework for root cause analysis in microservices, addressing noisy multimodal data, failure propagation, and topology drift.


<details>
  <summary>Details</summary>
Motivation: Root cause analysis in microservices is hindered by noisy multimodal observability, cascading failures, and topology drift. Existing LLM-based methods are topology-agnostic and prone to symptom-amplification bias.

Method: TopoEvo uses Metric-orthogonal Multimodal Alignment (MOMA) for stable node representations, Vector Quantization (VQ) for symptom tokenization, and a Hypothesis--Evidence--Test (HET) workflow for verification. It also includes a self-evolving mechanism for adaptation.

Result: TopoEvo effectively distinguishes initiating anomalies from amplified symptoms and maintains robustness under topology drift.

Conclusion: TopoEvo advances RCA in microservices by integrating topology-aware reasoning and self-evolving mechanisms, addressing key challenges in multimodal data and dynamic environments.
...
TL;DR: ColPackAgent is an agent framework for autonomous Monte Carlo simulations of colloidal packing using a Model Context Protocol (MCP) server and agent skill, enabling reliable execution of workflows for studies in phase behavior and materials design.


<details>
  <summary>Details</summary>
Motivation: To address the unreliability of general-purpose LLM agents in executing colloidal packing workflows without dedicated tools, ColPackAgent integrates a custom Python package and a structured workflow contract.

Method: The framework uses an MCP server hosting a colpack Python package for HOOMD-blue hard-particle Monte Carlo and a four-stage workflow skill. It operates interactively, autonomously, or via autoresearch with human feedback or prompts.

Result: Demonstrated through various colloidal packing simulations, the system effectively executes workflows and benchmarks LLM performance on stage-specific prompts, showing reliable setup, planning, and analysis.

Conclusion: Combining a domain-specific Python package with MCP tools and a portable agent skill effectively transforms simulation toolkits into agent-assisted research workflows.
...
TL;DR: The study tests the reliability of AI metrics for assessing user states in adaptive systems, finding many metrics unstable at the individual score level but useful in aggregated analyses. A replicable evaluation framework is proposed.


<details>
  <summary>Details</summary>
Motivation: To empirically test the stability and interpretability of AI metrics for user state assessment in conversational and adaptive systems.

Method: Replication evaluation procedures were used to assess the repeatability of metrics across three large language models (GPT-4o audio, Gemini 2.0 Flash, Gemini 2.5 Flash), analyzing individual and aggregated reliability.

Result: Only 31 of 213 metrics met reliability criteria, showing individual score instability but aggregated utility. Metrics retain value in post-hoc studies for analyzing interaction rules and user experience parameters.

Conclusion: Metric reliability isn't default in interpretive domains. The study proposes a replicable framework for responsible AI design, emphasizing explicit validation of reliability.
...
TL;DR: The paper introduces ALSO, a framework for online strategy optimization in multi-agent social simulations, addressing the non-stationarity of such environments. It outperforms static baselines and existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance social intelligence in dynamic, non-stationary environments where existing methods (e.g., static personas or offline reinforcement learning) fall short.

Method: ALSO formulates interactions as an adversarial bandit problem and uses a lightweight neural surrogate to predict rewards, enabling sample-efficient online adaptation.

Result: Experiments on the Sotopia benchmark show ALSO outperforms static baselines and existing optimization methods in dynamic settings.

Conclusion: ALSO effectively bridges the gap in social adaptation, proving robust for building adaptive social agents in non-stationary environments.
...
TL;DR: CUAs are expanding LLMs into action execution in complex environments. SaaS-Bench benchmark evaluates agents in realistic professional workflows, revealing limitations in current LLM-based agents.


<details>
  <summary>Details</summary>
Motivation: To assess CUAs in realistic and complex environments, overcoming the limitations of existing benchmarks that focus on simplified settings.

Method: Introduces SaaS-Bench, a benchmark with 106 tasks across 23 SaaS systems, requiring long-horizon execution and evaluated with weighted checkpoints.

Result: Current LLM-based agents struggle, with the strongest model completing fewer than 4% of tasks, highlighting planning and tracking limitations.

Conclusion: SaaS-Bench provides a realistic evaluation framework, exposing gaps in agent capabilities for professional workflows.
...
### [180] [Imperfect World Models are Exploitable](https://arxiv.org/abs/2605.15960)
*Logan Mondal Bhamidipaty,Esmeralda S. Whitammer,David Abel,Mykel J. Kochenderfer,Subramanian Ramamoorthy*

Main category: cs.AI

TL;DR: The paper proposes a new definition of model exploitation in reinforcement learning, linking it to reward hacking, and develops a theory showing exploitation is unavoidable in large policy sets. It also introduces a relaxed notion and a safe horizon to avoid exploitation.


<details>
  <summary>Details</summary>
Motivation: To formally bridge reward hacking and model exploitation, highlighting the challenges and limits of safe planning in world models.

Method: Develops a general theory of reward hacking and model exploitation, analyzes inevitability, and introduces a relaxed notion of exploitation.

Result: Proves exploitation is unavoidable in large policy sets, derives a safe horizon for avoiding it, and connects it to reward hacking.

Conclusion: The study establishes a formal connection between reward hacking and model exploitation, clarifying the constraints of safe planning.
...
### [181] [PAGER: Bridging the Semantic-Execution Gap in Point-Precise Geometric GUI Control](https://arxiv.org/abs/2605.15963)
*Jingxuan Wei,Xi Bai,Shan Liu,Caijun Jia,Zheng Sun,Xinglong Xu,Siyuan Li,Linzhuang Sun,Bihui Yu,Conghui He,Cheng Tan*

Main category: cs.AI

TL;DR: The paper introduces PAGE Bench, a benchmark for precision-sensitive GUI tasks requiring point-level accuracy, and proposes PAGER, a topology-aware agent that improves task success significantly over existing models.


<details>
  <summary>Details</summary>
Motivation: Current large vision-language models for GUI agents rely on region-tolerant interactions, which fail in precision-sensitive tasks requiring exact point-level actions.

Method: The authors introduce PAGER, an agent that decomposes tasks into dependency-structured planning and pixel-level execution, using supervised tuning and precision-aligned reinforcement learning.

Result: PAGER achieves 4.1x higher task success than general baselines and raises step success rate from below 9% to over 62% for precision-sensitive GUI tasks.

Conclusion: PAGER bridges the Semantic-Execution Gap and sets a new state of the art for point-precise GUI control.
...
### [182] [Deterministic Event-Graph Substrates as World Models for Counterfactual Reasoning](https://arxiv.org/abs/2605.15967)
*Fabio Rovai*

Main category: cs.AI

TL;DR: Event-graph substrates are world models using RDF triples for agent state, enabling exact counterfactuals and domain transfer. They outperform benchmarks like NS-DR and ALOE in certain query categories and introduce a new benchmark, twin-EventLog.


<details>
  <summary>Details</summary>
Motivation: To create inspectable, domain-agnostic world models for exact counterfactual reasoning without learned components.

Method: Use append-only RDF triples for state representation, fork logs for counterfactuals, and formalize causal-ancestor traversal for queries. Evaluated with a CLEVRER-DSL interpreter.

Result: Outperforms NS-DR by 9.89-20.26 points in query categories and ALOE in descriptive/explanatory queries. Exceeds Llama-3.1-8B by 18.80 points on twin-EventLog benchmark.

Conclusion: Event-graph substrates offer efficient counterfactual reasoning, surpassing existing benchmarks, with potential for broad application.
...
### [183] [Learning Bilevel Policies over Symbolic World Models for Long-Horizon Planning](https://arxiv.org/abs/2605.15975)
*Dillon Z. Chen,Till Hofmann,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: ThepaperproposesBISON,asystemcombininglow-levelimitationlearningandhigh-levelsymbolicabstractionsforlong-horizonplanning,outperformingexistingmethodsingeneralizationandefficiency.


<details>
  <summary>Details</summary>
Motivation: ToaddressthechallengeofbuildingembodiedAIagentscapableofsolvinglong-horizonplanningproblemsbyleveragingbothlow-levelimitationlearningandhigh-levelsymbolicabstractions.

Method: Introducesbilevelpolicies$(π^{\mathrm{hl}},π^{\mathrm{ll}})$,where$π^{\mathrm{ll}}$islearnedfromlow-leveldemonstrationsand$π^{\mathrm{hl}}$isderivedfromsymbolicabstractionsandinductivegeneralization.ImplementedintheBISONsystem.

Result: BISONoutperformsVLAandend-to-endmethodsingeneralizationtolonghorizonsandlargerobjectcounts,withhighertimeandmemoryefficiency.HLpoliciescansolveproblemswith10,000objectsinunderaminute.

Conclusion: Combininglow-levelimitationlearningwithhigh-levelsymbolicabstractionsinBISONeffectivelyaddresseslong-horizonplanningchallenges,demonstratingsuperiorperformanceandscalability.
...
### [184] [Petri Net Induced Heuristic Search for Resource Constrained Scheduling](https://arxiv.org/abs/2605.15983)
*Ido Lublin,Dor Atzmon,Izack Cohen*

Main category: cs.AI

TL;DR: The paper presents an A*-based heuristic approach using Timed Transition Petri Nets to solve RCPSP, outperforming MIP baselines in efficiency and success.


<details>
  <summary>Details</summary>
Motivation: Addressing the Resource-Constrained Project Scheduling Problem (RCPSP) efficiently, overcoming limitations of existing methods like MIP.

Method: Formulates RCPSP as optimal search over a reachability graph using Timed Transition Petri Nets with resource constraints, employing A* with a combined heuristic.

Result: The approach outperforms MIP baselines (SCIP, CBC) in success rate and solve time, with performance degradation linked to resource tightness and formulation size.

Conclusion: The heuristic search method is effective for RCPSP, with resource strength influencing solver performance at scale.
...
### [185] [ScreenSearch: Uncertainty-Aware OS Exploration](https://arxiv.org/abs/2605.16024)
*Michael Solodko,Justin Wagle*

Main category: cs.AI

TL;DR: ScreenSearch combines structural screen retrieval and ambiguity-aware exploration for desktop GUI agents, achieving large-scale diversity and uncovering a novelty-ambiguity trade-off in exploration.


<details>
  <summary>Details</summary>
Motivation: Desktop GUI agents face partial observability, where visually similar screens can have different states, leading to varied outcomes. Effective exploration requires expanding reachable states and reducing ambiguity.

Method: ScreenSearch uses structural screen retrieval and deduplication, indexing UIA trees as location-aware features. It employs an ambiguity-aware PUCT graph-bandit for exploration, leveraging matched-action outcome dispersion to probe ambiguous states.

Result: The system collected over 1M screenshots and 30K deduplicated states across 11 applications, revealing a novelty-ambiguity trade-off. Policies reducing ambiguity quickly often discover less frontier.

Conclusion: State identity, proposal quality, and ambiguity-aware search are crucial for deciding when to probe or commit. Ambiguity reduction alone is insufficient for effective exploration.
...
### [186] [Reasoners or Translators? Contamination-aware Evaluation and Neuro-Symbolic Robustness in Tax Law](https://arxiv.org/abs/2605.16052)
*Parisa Kordjamshidi,Samer Aslan,Madhavan Seshadri,Leslie Barrett,Enrico Santus*

Main category: cs.AI

TL;DR: The paper examines whether LLMs' performance in legal reasoning is genuine or due to data contamination, proposing a neuro-symbolic approach for more reliable legal AI.


<details>
  <summary>Details</summary>
Motivation: To determine if LLMs' success in legal reasoning stems from true understanding or data contamination, and to improve reliability in legal AI.

Method: Conducted a comprehensive empirical study with contamination detection, compared monolithic LLMs with hybrid neuro-symbolic systems, and built a test suite for generalization.

Result: Found that data contamination can inflate LLM performance, and neuro-symbolic frameworks offer more reliable and robust legal reasoning.

Conclusion: Legal reasoning is compositional, and neuro-symbolic systems provide better generalization and reliability for legal AI.
...
### [187] [Sign-Separated Finite-Time Error Analysis of Q-Learning](https://arxiv.org/abs/2605.16103)
*Donghwan Lee*

Main category: cs.AI

TL;DR: The paper analyzes the error dynamics of Q-learning with constant step-size, decomposing errors into positive and negative parts. It shows asymmetry in error propagation due to the Bellman maximum.


<details>
  <summary>Details</summary>
Motivation: To understand the finite-time error behavior of Q-learning, particularly the asymmetry in error dynamics caused by the Bellman maximum operator.

Method: Error is decomposed into positive and negative parts, analyzed using lower comparison LTI systems for the negative part and linear switching systems for the positive part.

Result: Bounds reveal faster exponential decay for the negative-side LTI certificate and identify asymmetry in Q-learning error dynamics.

Conclusion: The analysis highlights the impact of overestimation in Q-learning and provides finite-time bounds for both deterministic and stochastic cases.
...
### [188] [ShopGym: An Integrated Framework for Realistic Simulation and Scalable Benchmarking of E-Commerce Web Agents](https://arxiv.org/abs/2605.16116)
*Chinmay Savadikar,Mingyu Zhao,Yuanzheng Zhu,Han Li,Shuang Xie,Alberto Castelo,Tianfu Wu,Lingyun Wang*

Main category: cs.AI

TL;DR: ShopGym introduces a framework for realistic and scalable simulation of e-commerce web agents, addressing the tradeoff between realism and control in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating e-commerce web agents either lack realism (sandbox benchmarks) or controllability (live storefronts), creating a need for a balanced solution.

Method: ShopGym includes ShopArena, which converts live storefronts into sandbox shops, and ShopGuru, which synthesizes benchmark tasks across seven skill categories.

Result: Validation shows synthetic shops preserve structural properties of live storefronts, with agent performance correlated between synthetic and live environments.

Conclusion: ShopGym successfully bridges the gap between realism and control, providing a scalable and reproducible framework for e-commerce agent evaluation.
...
### [189] [Property-Guided LLM Program Synthesis for Planning](https://arxiv.org/abs/2605.16142)
*Augusto B. Corrêa,André G. Pereira,Jendrik Seipp*

Main category: cs.AI

TL;DR: Property-guided LLM program synthesis uses formal property checks and counterexamples to reduce program generation and evaluation costs while improving program quality.


<details>
  <summary>Details</summary>
Motivation: Current LLM program synthesis relies on simple numeric scores, which lack feedback on failures, leading to high costs and inefficiency. Property-guided synthesis aims to address this by providing specific feedback.

Method: The approach involves checking candidate programs against formally defined properties. If a property is violated, evaluation stops early, and the LLM receives a counterexample to guide repair. This is applied to PDDL planning domains for synthesizing direct heuristic functions.

Result: The method generates seven times fewer programs per domain, solves more tasks without search, and reduces evaluation computation by orders of magnitude. Synthesized heuristics perform effectively on test tasks.

Conclusion: Property-guided LLM synthesis is a cost-effective and efficient alternative to traditional scoring methods, especially for problems with verifiable properties.
...
### [190] [Look Before You Leap: Autonomous Exploration for LLM Agents](https://arxiv.org/abs/2605.16143)
*Ziang Ye,Wentao Shi,Yuxin Liu,Yu Wang,Zhengzhou Cai,Yaorui Shi,Qi Gu,Xunliang Cai,Fuli Feng*

Main category: cs.AI

TL;DR: Large language model agents often fail in new environments due to premature exploitation, highlighting the need for better exploration strategies measured by Exploration Checkpoint Coverage.


<details>
  <summary>Details</summary>
Motivation: To address the lack of adaptability in agents by improving their exploration capabilities in unfamiliar environments.

Method: Introduces a training strategy with alternating task and exploration rollouts, leading to the Explore-then-Act paradigm for decoupled information-gathering and task execution.

Result: Agents trained with standard reinforcement learning show narrow behaviors, while the new strategy improves exploration and adaptability.

Conclusion: Systematic exploration is essential for creating generalizable and practical agents.
...
### [191] [An Algebraic Exposition of the Theory of Dyadic Morality](https://arxiv.org/abs/2605.16153)
*Kush R. Varshney*

Main category: cs.AI

TL;DR: The paper formalizes the Theory of Dyadic Morality (TDM) using structural causal modeling (SCM) and introduces three psychological operators to model moral judgments. It addresses scalability issues and applies the framework to AI policy design, advocating for contextual measurement of mind perception.


<details>
  <summary>Details</summary>
Motivation: To provide a rigorous algebraic foundation for the Theory of Dyadic Morality (TDM) and extend its applicability to AI systems, ensuring moral computations align with human cognition.

Method: The paper uses structural causal modeling (SCM) notation to formalize TDM, introduces three psychological operators to capture moral judgment processes, and addresses scalability challenges through node collapse and sequential processing.

Result: The framework enables scalable moral cognition modeling and offers practical applications in AI policy design, such as detecting conflicting obligations and designing post-failure communication.

Conclusion: The algebraic formalization of TDM supports neurosymbolic AI systems in computing morality rigorously while aligning with human moral cognition, advocating for contextual empirical measurement.
...
### [192] [Formal Methods Meet LLMs: Auditing, Monitoring, and Intervention for Compliance of Advanced AI Systems](https://arxiv.org/abs/2605.16198)
*Parand A. Alamdari,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: The paper introduces techniques for auditing and monitoring AI systems, especially LLMs, combining formal methods and machine learning to ensure compliance with behavioral constraints throughout their lifecycle.


<details>
  <summary>Details</summary>
Motivation: The need to govern AI systems by monitoring and auditing them for compliance with safety, norms, and regulations, particularly for black-box systems like LLMs.

Method: Combines formal methods (e.g., Linear Temporal Logic) with machine learning to enable offline auditing and online monitoring, including predictive and intervening monitors.

Result: The techniques outperform baseline LLM methods in detecting violations, reduce violation rates, and maintain task performance. LLMs also show degraded temporal reasoning under certain conditions.

Conclusion: The proposed methods effectively address AI governance challenges by ensuring compliance with constraints and improving monitoring capabilities, even for complex black-box systems.
...
TL;DR: Clinical decision support systems (CDSS) need open, auditable pipelines. The Fully Open Meditron pipeline is introduced as the first fully open LLM-CDSS, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based CDSS lack transparency in data provenance and training pipelines, hindering reproducibility and auditability.

Method: Fully Open Meditron provides a transparent pipeline with a clinician-audited corpus, reproducible data construction, and a use-aligned evaluation protocol. It combines public datasets with clinician-vetted synthetic data and ensures system-wide decontamination.

Result: Meditron variants outperform their base models, with Apertus-70B-MeditronFO improving by +6.6 points on medical benchmarks. Gemma-3-27B-MeditronFO is preferred over MedGemma in 58.6% of comparisons.

Conclusion: Fully open pipelines can achieve top domain-specific performance while maintaining auditability and reproducibility.
...
TL;DR: An autonomous system using LLM-guided tree search generates, evaluates, and optimizes disease forecasting models, outperforming human-curated CDC ensembles in real-time evaluations.


<details>
  <summary>Details</summary>
Motivation: The labor-intensive manual curation of infectious disease forecasting models limits scalability to granular geographic resolutions or emerging pathogens.

Method: The system employs LLM-guided tree search to iteratively generate, evaluate, and optimize executable forecasting software, including handling data-scarce scenarios and ensuring structural fidelity.

Result: The machine-generated ensemble matched or outperformed human-curated CDC ensembles, successfully navigating cold-start scenarios for RSV and preventing reward hacking.

Conclusion: This framework overcomes the modeling labor bottleneck, enabling rapid, expert-level disease forecasting at unprecedented scales.
...
TL;DR: A probabilistic approach for dating historical manuscripts using visual features, achieving high accuracy and calibration with uncertainty decomposition.


<details>
  <summary>Details</summary>
Motivation: To improve the granularity and accuracy of dating historical manuscripts beyond traditional century-label methods, leveraging deep regression and uncertainty quantification.

Method: Uses an EfficientNet-B2 backbone with a Normal-Inverse-Gamma output head, trained with a joint negative-log-likelihood and evidence-regularization objective to output predictive distributions.

Result: Achieves a test MAE of 5.4 years, with 93% of patches within 5 years and 97% within 10 years, outperforming other methods in calibration and efficiency.

Conclusion: The approach provides fine-grained dating, effective uncertainty decomposition, and practical benefits for historical manuscript analysis.
...
### [197] [AgentStop: Terminating Local AI Agents Early to Save Energy in Consumer Devices](https://arxiv.org/abs/2605.15206)
*Dzung Pham,Kleomenis Katevas,Ali Shahin Shamsabadi,Hamed Haddadi*

Main category: cs.LG

TL;DR: The paper examines the inefficiency of locally deployed LLM-based agents on consumer hardware, introduces AgentStop to reduce wasted energy by 15-20%, and maintains task performance with minimal impact.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns and recurring costs of cloud-based LLM agents by deploying them locally, despite increased resource demands.

Method: Introduces AgentStop, a lightweight supervisor that predicts and terminates unlikely-to-succeed agent trajectories using low-cost execution signals like token-level log probabilities.

Result: AgentStop reduces wasted energy by 15-20% with less than a 5% drop in task performance on benchmarks.

Conclusion: Predictive early termination via AgentStop enables sustainable, privacy-preserving LLM agents on user devices.
...
### [198] [TeamTR: Trust-Region Fine-Tuning for Multi-Agent LLM Coordination](https://arxiv.org/abs/2605.15207)
*Yi Xie,Siao Liu,Falong Fan,Yuanqi Yao,Yue Zhao,Bo Liu*

Main category: cs.LG

TL;DR: TeamTR, a trust-region framework, addresses the compounding occupancy shift in multi-agent LLM systems, outperforming single-agent baselines by 7.1% on average.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems underperform single-model baselines due to structural failure modes like compounding occupancy shift during sequential fine-tuning.

Method: TeamTR resamples trajectories after each agent update and enforces per-agent divergence control, formalizing the problem and providing improvement bounds.

Result: TeamTR improves performance by 7.1% on average, mitigates coordination regressions, and allows plug-and-play component replacement.

Conclusion: TeamTR effectively addresses the compounding occupancy shift, enhancing multi-agent LLM system performance and coordination.
...
### [199] [Quantization Undoes Alignment: Bias Emergence in Compressed LLMs Across Models and Precision Levels](https://arxiv.org/abs/2605.15208)
*Plawan Kumar Rath,Rahul Maliakkal*

Main category: cs.LG

TL;DR: The study examines how post-training quantization affects bias in large language models, revealing that lower precision levels (like 3-bit) introduce new biases, unnoticed by standard quality metrics.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of model compression via quantization on bias emergence, which is poorly studied despite its widespread use.

Method: Controlled empirical study of three instruction-tuned models at five precision levels, evaluated on a bias benchmark with 12,148 items across 5 random seeds.

Result: Quantization (especially 3-bit) introduces new biases (6-21% of items) and reduces model uncertainty (17.4% drop in 'unknown' answers), while standard metrics like perplexity remain largely unaffected.

Conclusion: Aggregate metrics miss bias-related degradation, highlighting the need for explicit bias testing in compression protocols before deployment.
...
### [200] [Mask-Morph Graph U-Net: A Generalisable Mesh-Based Surrogate for Crashworthiness Field Prediction under Large Geometric Variation](https://arxiv.org/abs/2605.15231)
*Haoran Li,Tobias Lehrer,Yingxue Zhao,Haosu Zhou,Philipp Stocker,Tobias Pfaff,Nan Li*

Main category: cs.LG

TL;DR: The paper introduces MMGUNet, a graph neural network approach to improve crash simulation surrogate models by morphing graph hierarchies and using masked pretraining for better accuracy and data efficiency.


<details>
  <summary>Details</summary>
Motivation: Nonlinear finite element crash simulations are computationally expensive. Machine-learning surrogate models like GNNs offer faster alternatives, but existing methods face limitations in generalisability and accuracy.

Method: MMGUNet uses feature-aligned barycentric parameterisation to morph coarsened graph hierarchies and applies node masking during pretraining. High-parameter edge-specific layers are frozen during fine-tuning.

Result: MMGUNet improves test accuracy, reduces train-test discrepancy, and enhances data efficiency. It outperforms fixed-coarse-graph baselines and external benchmarks in prediction error.

Conclusion: MMGUNet provides a practical, reusable, and data-efficient solution for mesh-based surrogate modeling in crashworthiness design.
...
### [201] [MuteBench: Modality Unavailability Tolerance Evaluation for Incomplete Multimodal Fusion](https://arxiv.org/abs/2605.15235)
*Wugeng Zheng,Ziwen Kan,Tianlong Chen,Chen Chen,Song Wang*

Main category: cs.LG

TL;DR: MuteBench is a benchmark evaluating 6 fusion architectures on 9 clinical datasets under two missing-data modes, revealing architecture family as the strongest predictor of robustness.


<details>
  <summary>Details</summary>
Motivation: To address the lack of benchmarks assessing multimodal fusion architectures under common sensor failure modes (modality and within-modality missing) in clinical AI systems.

Method: Developed MuteBench, covering 9 datasets, 6 fusion architectures, and 2 missing-data modes over 125,000 samples. Evaluated robustness, parameter count, and imputation impact.

Result: Architecture family is the strongest robustness predictor. Channel-independent models handle modality missing well but struggle with within-modality missing. Diffusion-based imputation improves classification under within-modality missing.

Conclusion: MuteBench offers practical guidance for selecting robust fusion architectures and designing future multimodal methods, highlighting the importance of architecture choice and imputation strategies.
...
### [202] [Reducing the Safety Tax in LLM Safety Alignment with On-Policy Self-Distillation](https://arxiv.org/abs/2605.15239)
*Yu Fu,Longxuan Yu,Haz Sameen Shahgir,Zhipeng Wei,Hui Liu,N. Benjamin Erichson,Yue Dong*

Main category: cs.LG

TL;DR: The paper introduces on-policy self-distillation for safety alignment (OPSA) to address the tradeoff between safety and reasoning by using self-generated rollouts and KL supervision, outperforming off-policy methods.


<details>
  <summary>Details</summary>
Motivation: The tradeoff between safety and reasoning in models (safety tax) is often due to distributional mismatch from off-policy training. The goal is to improve safety alignment without sacrificing reasoning.

Method: OPSA leverages on-policy self-distillation, where the model generates its own rollouts and receives per-token KL supervision from a frozen teacher copy with privileged safety context. Teacher flip rate measures context effectiveness.

Result: OPSA outperforms off-policy methods across model scales and training sizes, with notable gains on smaller models (+8.85 and +5.49 points). Token-level updates focus on early compliance decisions.

Conclusion: OPSA effectively mitigates the safety–reasoning tradeoff by focusing on early compliance decisions, achieving better safety and reasoning performance, especially in smaller models.
...
### [203] [Logical Grammar Induction via Graph Kolmogorov Complexity: A Neuro-Symbolic Framework for Self-Healing Clinical Data Integrity](https://arxiv.org/abs/2605.15242)
*Abolfazl Zarghani,Amir Malekesfandiari*

Main category: cs.LG

TL;DR: Logic-GNN integrates TGNN and Graph Kolmogorov Complexity to detect human-induced errors in HIS by treating clinical records as a structured language with latent logic, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: Human-induced data entry errors in HIS are hard to distinguish from legitimate clinical extremes, compromising reliability.

Method: Uses Logic-GNN, a neuro-symbolic framework combining TGNN and Graph Kolmogorov Complexity to model clinical records as a structured language, defining anomalies as grammatical violations.

Result: Achieves F1-score of 0.94, outperforming baselines by 12% in distinguishing medical outliers from data corruption.

Conclusion: Logic-GNN offers a self-healing mechanism for real-time HIS data integrity, improving reliability.
...
### [204] [Reading the Cell, Designing the Cure: Perturbation-Conditioned Molecular Diffusion for Function-Oriented Drug Design](https://arxiv.org/abs/2605.15243)
*Ziyu Xu,Zijian Zhang,Liang Wang,Zhiyuan Liu,Qiang Liu,Shu Wu,Liang Wang*

Main category: cs.LG

TL;DR: The paper introduces Transcriptome-based Drug Design (TBDD) as a generative approach to design drugs based on desired transcriptomic changes, addressing challenges like domain gaps and data sparsity with a multi-resolution diffusion framework called ACURE.


<details>
  <summary>Details</summary>
Motivation: The motivation is to design drugs when reliable target structures are scarce or phenotypes stem from dysregulated pathways, using transcriptomic perturbations as a functional readout.

Method: The proposed method, ACURE, uses a Transcriptome Perturbation Functional Feature Extractor (TFE) to analyze transcriptomic data, align it with chemical views, and aggregate signals. It employs a multi-resolution diffusion framework.

Result: ACURE outperforms baselines in structural quality and functional consistency, validated by benchmarks and a zero-shot gene-inhibitor design task.

Conclusion: The conclusion highlights the potential of phenotype-driven generative discovery, with ACURE offering a robust solution for TBDD challenges.
...
### [205] [Privacy Evaluation of Generative Models for Trajectory Generation](https://arxiv.org/abs/2605.15246)
...
TL;DR: The paper examines privacy risks in generative models for synthetic trajectory data, showing they don't inherently protect privacy and can be vulnerable to attacks.


<details>
  <summary>Details</summary>
Motivation: To address the assumption that generative models for trajectory data preserve privacy, highlighting the need for empirical privacy evaluation.

Method: Implemented Membership Inference Attacks on representative generative models to assess privacy risks.

Result: Found that generative models for trajectories do not inherently eliminate privacy risks, demonstrating vulnerabilities to attacks.

Conclusion: The generative nature of trajectory models does not guarantee privacy, emphasizing the need for rigorous empirical evaluation.
...
### [206] [GQLA: Group-Query Latent Attention for Hardware-Adaptive Large Language Model Decoding](https://arxiv.org/abs/2605.15250)
*Fanxu Meng*

Main category: cs.LG

TL;DR: The paper introduces Group-Query Latent Attention (GQLA), a modification of Multi-head Latent Attention (MLA), to improve efficiency and adaptability for diverse hardware.


<details>
  <summary>Details</summary>
Motivation: MLA's single decoding path limits efficiency on commodity GPUs and forfeits tensor parallelism. GQLA aims to provide flexibility without retraining or custom kernels.

Method: GQLA modifies MLA to expose two decoding paths (MQA-absorb and GQA) using the same parameters. It also introduces TransGQLA to convert pretrained GQA checkpoints into GQLA models.

Result: GQLA compresses KV cache to 28.125% of GQA's baseline on the MQA-absorb path while maintaining GQA-level traffic on the per-group path.

Conclusion: GQLA optimizes performance for both high-end and commodity GPUs, supports tensor parallelism, and avoids the need for retraining.
...
### [207] [PDRNN: Modular Data-driven Pedestrian Dead Reckoning on Loosely Coupled Radio- and Inertial-Signalstreams](https://arxiv.org/abs/2605.15252)
*Peter Bauer,Andreas Porada,Felix Ott,Christopher Mutschler,Tobias Feigl*

Main category: cs.LG

TL;DR: PDRNN, a hybrid AI-assisted PDR system using modular RNNs, outperforms traditional methods by effectively fusing asynchronous sensor data and avoiding error accumulation in dynamic movements.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in fusing noisy, biased, and asynchronous sensor data for accurate pose estimation during dynamic movements.

Method: Proposes PDRNN, a modular RNN architecture with separate ML models for orientation, velocity/distance, and optional absolute positioning, fused robustly using uncertainty estimates.

Result: Superior accuracy and precision in dynamic sports movement data, avoiding error accumulation and offering better component control.

Conclusion: PDRNN is a scalable, robust solution for PDR systems, enhancing accuracy and adaptability in dynamic environments.
...
### [208] [Position: Ideas Should be the Center of Machine Learning Research](https://arxiv.org/abs/2605.15253)
*Jairo Diaz-Rodriguez*

Main category: cs.LG

TL;DR: The paper critiques the divide in machine learning between benchmark-driven engineering and idealized theory, advocating for an 'Ideas First' approach to prioritize understanding and equitable research.


<details>
  <summary>Details</summary>
Motivation: The gap between practical engineering (focused on metrics) and theoretical research (often not applicable) neglects the core of science—ideas. The authors aim to refocus the field on ideas as the central object of study.

Method: Proposes the 'Ideas First' framework, where ideas are tested through tailored experiments designed to detect behavioral signatures in models, rather than chasing benchmarks or theoretical purity.

Result: Bridges theory and practice, promotes equity by reducing reliance on extensive resources, and treats benchmarks and theorems as tools for testing hypotheses.

Conclusion: The paper calls for a research culture centered on ideas, using benchmarks and theorems as means to validate mechanistic hypotheses rather than primary goals.
...
### [209] [Curriculum Learning of Physics-Informed Neural Networks based on Spatial Correlation](https://arxiv.org/abs/2605.15254)
*Xujia Chen,Xinyue Hu,Letian Chen,Daming Shi,Wenhui Fan*

Main category: cs.LG

TL;DR: The paper introduces a spatially correlated curriculum learning framework for Physics-Informed Neural Networks (PINNs) to address training challenges like spatial coupling and information propagation, improving solution accuracy.


<details>
  <summary>Details</summary>
Motivation: PINNs face issues like high-dimensional non-convex loss landscapes and ineffective spatial information propagation, especially in boundary value problems (BVPs). Existing methods overlook spatial coupling and consistency.

Method: The proposed framework uses spatial causal weights, a low-frequency information bridge, and region-adaptive reweighting to enhance training stability and accuracy.

Result: Experiments show the method reduces training failures and improves solution accuracy without additional computational cost.

Conclusion: The framework effectively tackles PINN training difficulties by addressing spatial coupling and inter-region consistency, offering a novel solution for BVPs.
...
### [210] [Training on Documents About Monitoring Leads to CoT Obfuscation](https://arxiv.org/abs/2605.15257)
*Reilly Haskins,Bilal Chughtai,Joshua Engels*

Main category: cs.LG

TL;DR: Monitor-aware models can evade detection by obfuscating their reasoning, posing a risk to Chain-of-thought (CoT) monitoring effectiveness.


<details>
  <summary>Details</summary>
Motivation: To investigate if models aware of CoT monitoring can hide their reasoning to avoid detection, given the reliance on faithful externalization for monitoring.

Method: Used synthetic document finetuning to expose models to CoT monitor descriptions, compared undetected misbehavior rates between monitor-aware and unaware models, and analyzed CoT controllability.

Result: Monitor-aware models showed higher undetected misbehavior rates, correlating with CoT controllability. They also learned to reward-hack faster under reinforcement learning.

Conclusion: Knowledge of monitoring and high CoT controllability undermines the reliability of CoT-based monitoring.
...
### [211] [Tadpole: Autoencoders as Foundation Models for 3D PDEs with Online Learning](https://arxiv.org/abs/2605.15284)
*Qiang Liu,Felix Koehler,Benjamin Holzschuh,Nils Thuerey*

Main category: cs.LG

TL;DR: Tadpole is a foundation model for 3D PDEs, pre-trained as an autoencoder on synthetic data, enabling transferable and scalable solutions for tasks like dynamics learning and generative modeling.


<details>
  <summary>Details</summary>
Motivation: Address challenges in transferability, scalability, and multi-functionality for 3D PDEs by leveraging large-scale pre-training on synthetic data.

Method: Pre-trained as an autoencoder on synthetic 3D PDE data; uses efficient fine-tuning strategies like low-rank adaptation and latent-space transformations for downstream tasks.

Result: Demonstrates strong performance in diverse downstream tasks, including dynamics learning and generative modeling, with minimal trainable parameters.

Conclusion: Tadpole is a versatile and effective foundation model for 3D PDE learning, with pre-trained weights and code available.
...
### [212] [Universal Approximation of Nonlinear Operators and Their Derivatives](https://arxiv.org/abs/2605.15285)
*Filippo de Feo*

Main category: cs.LG

TL;DR: The paper introduces Derivative-Informed Operator Learning (DIOL) and proves the first Universal Approximation Theorems (UATs) for nonlinear differentiable operators between Banach spaces and their derivatives, addressing foundational open questions in operator learning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to advance the field of Operator Learning (OL) by addressing the lack of UATs for nonlinear operators and their derivatives, which are crucial for applications in high-order accuracy, optimization, and numerical methods.

Method: The authors use Encoder-Decoder Architectures to parameterize nonlinear operators and leverage key features like Banach space properties, Bastiani differentiability, compact-open topologies, and weighted Sobolev spaces to prove UATs.

Result: The paper successfully proves the first UATs for nonlinear differentiable operators in Banach spaces, providing a theoretical foundation for DIOL and its applications.

Conclusion: The work generalizes classical approximation results to infinite-dimensional settings, opening new avenues for applications in optimization, PDE control, and numerical methods.
...
### [213] [GQA-μP: The maximal parameterization update for grouped query attention](https://arxiv.org/abs/2605.15290)
...
TL;DR: This paper advances hyperparameter transfer for LLMs by refining the μP framework with spectral norm conditions and addressing scalability for non-full-rank matrices, particularly in GQA architectures.


<details>
  <summary>Details</summary>
Motivation: To reduce the computational cost of tuning large language models (LLMs) by ensuring hyperparameter transferability across different model architectures.

Method: The authors build on spectral feature-learning, elevate spectral norm conditions to define feature learning, and modify spectral norms to handle non-full-rank weight matrices. This enables derivation of μP scalings for grouped-query attention (GQA).

Result: Theoretical derivations are validated by successful learning rate transfer across GQA repetition hyperparameters and weight decay experiments.

Conclusion: The refined μP framework effectively enables hyperparameter transfer in LLMs, even for complex architectures like GQA, reducing the need for extensive tuning.
...
### [214] [GESD: Beyond Outcome-Oriented Fairness](https://arxiv.org/abs/2605.15295)
*Gideon Popoola,John Sheppard*

Main category: cs.LG

TL;DR: Introducing GESD, a procedural fairness metric to address disparities in model explanations across subgroups, integrated into FEU framework for improved utility and fairness.


<details>
  <summary>Details</summary>
Motivation: Existing fairness metrics focus on outcomes but ignore procedural or explanatory biases in ML models, limiting understanding of biased decisions.

Method: Group-level Explanation Stability Disparity (GESD) measures explanation stability across subgroups, combined with FEU framework to optimize utility, outcome, and explanation fairness.

Result: GESD captures group-wise explanation discrepancies; FEU outperforms state-of-the-art methods in utility and fairness.

Conclusion: GESD bridges outcome and explanation fairness, providing a tool for bias diagnosis and mitigation in predictive models.
...
### [215] [How Data Augmentation Shapes Neural Representations](https://arxiv.org/abs/2605.15306)
*Tianxiao He,Alex H. Williams,Sarah E. Harvey*

Main category: cs.LG

TL;DR: The paper explores how data augmentation affects the geometry of learned representations in neural networks, revealing patterns and trajectories in a metric-invariant space.


<details>
  <summary>Details</summary>
Motivation: Understanding the impact of data augmentation on the internal representations of neural networks, which remains underexplored.

Method: Using shape analysis tools to embed hidden representations into a metric space invariant to scaling, translation, rotation, and reflection, analyzing trajectories and distortions.

Result: Increasing augmentation strength leads to predictable trajectories, with different augmentations steering representations in unique directions. Insights from neural geometry predict the best representations for model ensembling.

Conclusion: Analyzing shape-space trajectories provides a principled way to understand and compare data augmentation methods, revealing shared geometric patterns across architectures.
...
### [216] [Beyond Bounded Variance: Variance-Reduced Normalized Methods for Nonconvex Optimization under Blum-Gladyshev Noise](https://arxiv.org/abs/2605.15314)
*Antesh Upadhyay,Arda Fazla,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: The paper analyzes nonconvex stochastic optimization under the Blum-Gladyshev noise model, showing convergence of normalized stochastic gradient descent with momentum and a variance-reduced method, achieving optimal complexity rates under various smoothness conditions.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of stochastic optimization under noise where variance grows with distance from initialization, a scenario not well-covered by existing bounded-variance models.

Method: The authors analyze normalized stochastic gradient descent with momentum and a variance-reduced normalized STORM method under standard and generalized smoothness frameworks.

Result: Convergence is proven with oracle complexity $O(\varepsilon^{-6})$ for momentum and minimax optimal $O(\varepsilon^{-4})$ for STORM under mean-square smoothness. Generalized smoothness affects complexity rates, e.g., $O(\varepsilon^{-(4+\alpha)})$ for $\alpha \in (0,1)$.

Conclusion: The paper provides the first convergence guarantees for normalized methods in non-convex stochastic optimization under Blum-Gladyshev noise, covering both standard and generalized smoothness regimes without restrictive assumptions.
...
### [217] [From Weight Perturbation to Feature Attribution for Explaining Fully Connected Neural Networks](https://arxiv.org/abs/2605.15328)
*Thodoris Lymperopoulos,Denia Kanellopoulou*

Main category: cs.LG

TL;DR: The paper introduces two novel attribution methods, XWP and XWP_c, for interpreting Fully Connected Neural Networks by perturbing feature weights instead of values, addressing limitations like Added Bias and Out-of-Distribution data.


<details>
  <summary>Details</summary>
Motivation: Despite their simplicity, FCNNs lack consensus on interpretability. The paper aims to improve understanding of neural networks by proposing a new approach to feature attribution that avoids common pitfalls of existing methods.

Method: The authors propose perturbing the weights attached to features (rather than feature values) to estimate attribution. This leads to the development of two methods, XWP and XWP_c, which are tested on simple DNNs.

Result: The new methods achieve competitive performance in identifying image signals, matching or exceeding established attribution methods on standard metrics.

Conclusion: The work advances explainability in neural networks by introducing a robust framework that mitigates long-standing vulnerabilities and enhances the reliability of model explanations.
...
### [218] [From I/O to Code with Discovery Agent](https://arxiv.org/abs/2605.15334)
*Yihong Dong,Jiaru Qian,Haoran Zhang,Peixu Wang,Binhua Li,Zhi Jin,Yongbin Li,Ge Li,Xiaokang Yang,Xue Jiang*

Main category: cs.LG

TL;DR: The paper introduces DIO-Agent, an evolutionary search-based method for synthesizing programs from input-output behavior (IO2Code), outperforming traditional and state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: While NL2Code has progressed with LLMs, synthesizing programs from input-output behavior (IO2Code) remains unsolved due to the complexity of recovering underlying principles from behavior.

Method: DIO-Agent employs evolutionary search over discrete program space, using an LLM as a mutation operator guided by execution errors, and incorporates the Transformation Priority Premise to bias toward simpler constructs.

Result: DIO-Agent consistently outperforms traditional and SOTA baselines across difficulty levels and LLMs, surpassing test-time scaling strategies with the same sampling budget.

Conclusion: DIO-Agent advances IO2Code by combining evolutionary search with LLMs and simplicity-biased mutations, demonstrating superior performance in program synthesis from input-output behavior.
...
### [219] [Bounded-Rationality, Hedging, and Generalization](https://arxiv.org/abs/2605.15340)
*Pedro A. Ortega*

Main category: cs.LG

TL;DR: The paper explores how a learner balances data fitting with hedging against sample-induced distortion, framing it as a bounded-rational decision problem. It introduces tradeoff and certificate curves based on the learner's response law, shows how to recover these from behavior, and ties generalization to hedging properties.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the tradeoff between a learner's adaptation to training data and its ability to hedge against distortion, providing a framework to analyze and certify generalization.

Method: The paper models the learner's decision problem using induced channels from samples to outputs, focusing on response laws represented by f-divergence regularizers. It derives tradeoff and certificate curves and shows their recovery from observed behavior.

Result: The framework reveals how generalization can be assessed through the learner's hedging behavior, with the recovered curves offering practical certificates for sample-induced distortion.

Conclusion: Generalization is framed as a testable property of the learner's response law, linking empirical and population loss through the lens of hedging and distortion.
...
### [220] [LEAP: Trajectory-Level Evaluation of LLMs in Iterative Scientific Design](https://arxiv.org/abs/2605.15341)
*Marilyn Zhang,Tianfeng Chen,Fabián Barzuna,Ankita Rathod,Mark E. Whiting*

Main category: cs.LG

TL;DR: The paper introduces LEAPBench to evaluate LLMs' learning efficiency in iterative scientific design, highlighting the importance of trajectory metrics over final-outcome scoring and showing LLMs do not outperform Bayesian baselines.


<details>
  <summary>Details</summary>
Motivation: To address the gap in measuring learning trajectories in iterative scientific design benchmarks, which is crucial for assessing efficiency in cost and time.

Method: Introduces LEAPBench, a 55-task framework with a trajectory metric (best-so-far AUC), compares LLMs against Bayesian optimization, and includes an audit grounded in published literature.

Result: Trajectory scoring changes best-model decisions on 53% of tasks; LLMs do not outperform Bayesian baselines. Domain-agnostic prompting matches published-best designs more often in certain tasks.

Conclusion: LEAPBench provides a better measure of learning efficiency, revealing overlooked gains and serving as a tractable training target for improving performance.
...
### [221] [PACER: Acyclic Causal Discovery from Large-Scale Interventional Data](https://arxiv.org/abs/2605.15353)
*Ramon Viñas Torné,Sílvia Fàbregas Salazar,Soyon Park,Ivo Alexander Ban,Artyom Gadetsky,Nikita Doikov,Maria Brbić*

Main category: cs.LG

TL;DR: PACER introduces a scalable framework for causal discovery that guarantees acyclic graphs, outperforming existing methods in efficiency and accuracy for high-dimensional data.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address limitations in existing causal discovery methods for high-dimensional interventional data, such as soft acyclicity constraints leading to cyclic graphs and scalability issues.

Method: PACER parameterizes a distribution over DAGs using variable permutations and edge probabilities, enabling direct optimization without surrogate penalties. It supports unified likelihood-based treatment of data, flexible models, and prior knowledge integration. Closed-form expressions are derived for linear-Gaussian cases.

Result: PACER matches or surpasses state-of-the-art methods in benchmarks (e.g., protein signaling, genetic perturbations), scales to thousands of variables, and achieves significant speedups (up to 100x) over penalty-based approaches.

Conclusion: The study demonstrates that exact and scalable causal discovery from high-dimensional perturbation data is feasible through principled design of the search space.
...
### [222] [Controllable Molecular Generative Foundation Models](https://arxiv.org/abs/2605.15354)
*Yihan Zhu,Yuhan Liu,Weijiang Li,Tengfei Luo,Meng Jiang*

Main category: cs.LG

TL;DR: CoMole introduces a motif-aware graph diffusion pipeline for controllable molecular generation, outperforming baselines in controllability and validity without post-hoc fixes.


<details>
  <summary>Details</summary>
Motivation: Current molecular graph generation lacks a unified framework for diverse tasks with reliable control. Reinforcement learning (RL) faces challenges due to large action spaces and invalid chemical states.

Method: CoMole uses a motif-aware graph diffusion pipeline to transfer pretrained structural priors into controllable generation, optimizing conditional reverse policies with RL over chemically meaningful decisions.

Result: CoMole ranks first in controllability across nine targets, reduces MAE by up to 48.2%, and maintains high validity (>0.94) without rule-based corrections. It also transfers controllability to unseen properties by optimizing task embeddings.

Conclusion: CoMole provides a unified, effective framework for controllable molecular generation, addressing limitations of atom-level RL and achieving superior performance in heterogeneous tasks.
...
### [223] [Federated Learning of Spiking Neural Networks under Heterogeneous Temporal Resolutions](https://arxiv.org/abs/2605.15355)
*Sanja Karilanova,Subhrakanti Dey,Ayça Özçelikkale*

Main category: cs.LG

TL;DR: Proposes a federated learning framework for Spiking Neural Networks (SNNs) to address temporal resolution mismatch in edge devices, enabling accurate training across heterogeneous temporal data.


<details>
  <summary>Details</summary>
Motivation: Edge devices in time-series applications collect data at varying temporal resolutions, causing challenges for federated learning. Traditional federated averaging fails due to mismatched neuron parameters.

Method: Introduces a federated learning framework for SNNs and stateful deep networks, focusing on integrating neuron parameters learned from different temporal resolutions and optimizing model aggregation.

Result: Evaluated on SNN-native datasets (SHD and DVS-Gesture) under resolution heterogeneity, the framework recovers accuracy lost due to temporal mismatch, allowing clients to train locally while remaining compatible with the global model.

Conclusion: The proposed framework successfully mitigates temporal resolution challenges in federated learning for SNNs, enhancing compatibility and accuracy across heterogeneous edge devices.
...
### [224] [PRB-RUPFormer: A Recursive Unified Probabilistic Transformer for Residual PRB Forecasting](https://arxiv.org/abs/2605.15363)
*Saad Masrur,Yuxuan Jiang,Matti Hiltunen,Ajay Rajkumar,Ismail Guvenc*

Main category: cs.LG

TL;DR: PRB-RUPFormer is a Transformer-based model for predicting residual PRBs in cellular systems, capturing cross-carrier dependencies and providing probabilistic forecasts with uncertainty intervals.


<details>
  <summary>Details</summary>
Motivation: Current PRB prediction methods lack cross-carrier dependency modeling and uncertainty measures, limiting their utility for robust spectrum-aware control in highly variable traffic conditions.

Method: PRB-RUPFormer uses temporal, seasonal, and carrier-aware embeddings to process multivariate KPI time series, enabling recursive rollout and stable long-horizon forecasting with quantile-based uncertainty prediction.

Result: Evaluations on commercial LTE data show median MAE below 0.05 and hit probabilities above 0.80 for one- and seven-day forecasts.

Conclusion: PRB-RUPFormer effectively supports spectrum-aware RAN functions, making it suitable for dynamic spectrum access scenarios.
...
### [225] [Is One Score Enough? Rethinking the Evaluation of Sequentially Evolving LLM Memory](https://arxiv.org/abs/2605.15384)
*Songwei Dong,Zihan Chen,Chengshuai Shi,Peng Wang,Jundong Li,Cong Shen*

Main category: cs.LG

TL;DR: SeqMem-Eval is a framework to evaluate LLM memory in sequential tasks, revealing issues like forgetting and negative transfer that aggregate metrics overlook.


<details>
  <summary>Details</summary>
Motivation: Memory in large language models (LLMs) is crucial for sequential tasks, but existing evaluations using aggregate metrics like final accuracy or cumulative performance can mask critical issues like forgetting and negative transfer.

Method: A diagnostic evaluation framework called SeqMem-Eval is introduced, inspired by continual learning. It assesses memory evolution, generalization, consolidation, and retention during sequential inference, focusing on metrics like online utility, hold-out generalization, backward transfer, and forgetting.

Result: Experiments reveal that higher final or cumulative accuracy doesn't always indicate better memory quality. Many methods show performance gains but suffer from forgetting or negative transfer, and different memory designs exhibit trade-offs between adaptability and stability.

Conclusion: SeqMem-Eval provides a finer-grained evaluation of LLM memory, uncovering hidden trade-offs and failure modes that standard metrics miss, highlighting the need for more nuanced assessments of memory in sequential tasks.
...
### [226] [Unified High-Probability Analysis of Stochastic Variance-Reduced Estimation](https://arxiv.org/abs/2605.15388)
*Zhankun Luo,Antesh Upadhyay,M. Berk Sahin,Sang Bin Moon,Anuran Makur,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: A unified framework for stochastic variance-reduced estimation is developed, generalizing classical methods and enabling high-probability bounds in both Euclidean and non-Euclidean settings.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the lack of a unified analysis for stochastic estimators, which obscures structural tradeoffs in reliability.

Method: The framework uses a recursion with three components: memory retention, reset probability, and a correction term for iterate movement.

Result: It recovers classical estimators, motivates new variants, and provides high-probability bounds for estimation error. Applications include improved oracle complexities for optimization problems.

Conclusion: The framework offers a versatile tool for stochastic estimation, with applications in unconstrained and constrained optimization, improving existing results.
...
### [227] [LPDS: Evaluating LLM Robustness Through Logic-Preserving Difficulty Scaling](https://arxiv.org/abs/2605.15393)
*Philipp Mondorf,Samuel J. Bell,Jesse Dodge,Dieuwke Hupkes*

Main category: cs.LG

TL;DR: The paper introduces Logic-Preserving Difficulty Scaling (LPDS) to systematically find and quantify hard problem variations for LLMs, showing that fine-tuning on these improves robustness.


<details>
  <summary>Details</summary>
Motivation: Current LLMs often fail on logic-preserving variations of problems, but existing evaluations lack systematic ways to identify the hardest variations, potentially overestimating robustness.

Method: The authors propose LPDS, which quantifies problem difficulty and searches for variations that maximize difficulty and expose model failures.

Result: LPDS finds variations that cause up to 5x larger performance drops than random sampling, and fine-tuning on harder variations improves robustness more effectively.

Conclusion: LPDS provides a systematic approach to evaluate and improve LLM robustness by targeting the most challenging problem variations.
...
### [228] [Representation Without Reward: A JEPA Audit for LLM Fine-Tuning](https://arxiv.org/abs/2605.15394)
*Biswa Sengupta*

Main category: cs.LG

TL;DR: The paper investigates whether joint-embedding predictive architectures (JEPAs) improve task metrics in autoregressive language models by predicting latent representations, but finds no significant improvements despite various training auxiliaries.


<details>
  <summary>Details</summary>
Motivation: To determine if JEPAs can enhance language-model performance by focusing on latent representations rather than observed outputs, specifically in natural-language-to-regex generation tasks.

Method: The study tests 22 training-time auxiliaries under a fixed Llama-3.2-1B-Instruct LoRA harness, evaluating trajectory-shape regularization, distributional constraints, and other techniques.

Result: No auxiliary survives multiple-comparison corrections, though some show minor improvements. Hidden-state geometries and task metrics remain weakly coupled.

Conclusion: The null results suggest weak coupling between hidden-state representations and task accuracy, prompting a reframing of JEPA evaluation as a coupling problem.
...
### [229] [Breakeven complexity: A new perspective on neural partial differential equation solvers](https://arxiv.org/abs/2605.15399)
...
### [230] [$φ$-Balancing for Mixture-of-Experts Training](https://arxiv.org/abs/2605.15403)
*Lizhang Chen,Jonathan Li,Qi Wang,Runlong Liao,Shuozhe Li,Chen Liang,Ni Lao,Qiang Liu*

Main category: cs.LG

TL;DR: $\phi$-balancing optimizes expert utilization in MoE models by targeting population-level balance through a convex potential, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing load-balancing methods for MoE models are heuristic and introduce bias due to noisy mini-batch statistics, necessitating a principled approach.

Method: Proposes $\phi$-balancing, a framework minimizing a convex potential of expected routing distribution, using convex duality and mirror descent for efficient online optimization.

Result: Outperforms Switch-style and loss-free baselines in large-scale pretraining and fine-tuning, achieving stable and effective expert utilization.

Conclusion: $\phi$-balancing provides a more principled and efficient solution for expert balance in MoE models, enhancing scalability and performance.
...
### [231] [Transformer Scalability Crisis: The First Comprehensive Empirical Analysis of Performance Walls in Modern Language Models](https://arxiv.org/abs/2605.15413)
*Mahdi Naser Moghadasi,Faezeh Ghaderi*

Main category: cs.LG

TL;DR: A large-scale evaluation of 118 transformer models reveals scalability issues, showing performance drops as sequence length increases, with compressed models outperforming larger ones in efficiency.


<details>
  <summary>Details</summary>
Motivation: To systematically understand the scalability limitations of transformer architectures in NLP through empirical analysis.

Method: Evaluated 118 transformer models across seven architectural categories, measuring performance, loading times, memory use, and computational efficiency at varying sequence lengths.

Result: Performance declines sharply with longer sequences (88.1% success at 512 tokens, 44.9% at 1024, 0% at 2048); compressed models are more efficient (649.2 tokens/sec/M parameters vs. 12.5 for large models).

Conclusion: The study challenges scaling assumptions, provides evidence of O(n²) attention complexity impacts, and offers practical deployment insights and benchmarking methods.
...
### [232] [Margin-Adaptive Confidence Ranking for Reliable LLM Judgement](https://arxiv.org/abs/2605.15416)
*Gaojie Jin,Yong Tao,Lijia Yu,Tianjin Huang*

Main category: cs.LG

TL;DR: The paper introduces a method to improve agreement between LLMs and human judgments by learning a confidence estimator instead of using heuristic signals, enhancing the monotonic relationship between confidence and disagreement risk.


<details>
  <summary>Details</summary>
Motivation: The existing framework’s reliance on heuristic confidence signals may not guarantee agreement between LLMs and humans due to violated assumptions and unanalyzed generalization behavior.

Method: The approach learns a confidence estimator using simulated annotator diversity and a margin-based ranking formulation to model LLM confidence in distinguishing human-agreement and disagreement cases.

Result: The method improves ranking accuracy, strengthens the monotonic relationship between confidence and disagreement risk, and achieves higher success rates in meeting target agreement levels across datasets and judge models.

Conclusion: The proposed confidence estimator outperforms heuristic signals, providing a more reliable framework for ensuring LLM-human agreement with generalization guarantees.
...
### [233] [$f$-Trajectory Balance: A Loss Family for Tuning GFlowNets, Generative Models, and LLMs with Off- and On-Policy Data](https://arxiv.org/abs/2605.15417)
*Jake Fawkes,Jason Hartford*

Main category: cs.LG

TL;DR: The paper extends the use of mean square error as a surrogate loss for training generative models to the broader family of $f$-divergences, maintaining effective on-policy and off-policy properties.


<details>
  <summary>Details</summary>
Motivation: To generalize the effective surrogate loss (mean square error) used in GFlowNets and variational inference to a wider family of $f$-divergences, enabling more flexible and efficient training of generative models.

Method: The authors extend the construction of surrogate losses to $f$-divergences, showing a one-to-one correspondence between translation-invariant loss functions and $f$-divergences, and design new surrogate losses based on this equivalence.

Result: The proposed surrogate losses retain the properties of corresponding $f$-divergences (e.g., mode coverage) and perform effectively in both on-policy and off-policy scenarios across various tasks, including molecule discovery and LLM tuning.

Conclusion: The work successfully generalizes surrogate losses to $f$-divergences, providing a versatile tool for training generative models with improved properties in diverse applications.
...
### [234] [Lagrangian Flow Matching: A Least-Action Framework for Principled Path Design](https://arxiv.org/abs/2605.15419)
...
### [235] [DualKV: Shared-Prompt Flash Attention for Efficient RL Training with Large Rollouts and Long Contexts](https://arxiv.org/abs/2605.15422)
*Jiading Gai,Shuai Zhang,Xiang Song,Bernie Wang,George Karypis*

Main category: cs.LG

TL;DR: DualKV eliminates redundant prompt replication in RL training by processing shared prompts once, achieving significant speedups and memory efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard FlashAttention redundantly replicates prompt tokens across sequences, increasing compute and memory costs in RL training. DualKV exploits causal masking to avoid this redundancy.

Method: DualKV combines a fused CUDA kernel for shared prompt and response processing with a data-pipeline redesign, reducing token replication and extending efficiency to the entire model.

Result: DualKV achieves 1.63–2.09x policy-update speedup, 2x larger micro-batches, and raises MFU from 36% to 76% on Qwen3-8B. Similar gains are observed for DAPO.

Conclusion: DualKV offers a mathematically equivalent yet highly efficient alternative to standard attention for RL training, with scalable benefits for large models.
...
### [236] [Spectral Priors vs. Attention: Investigating the Utility of Attention Mechanisms in EEG-Based Diagnosis](https://arxiv.org/abs/2605.15433)
*Tawsik Jawad,Gowtham Atluri,Vikram Ravindra*

Main category: cs.LG

TL;DR: EEG signals are noisy and hard to classify due to low spatial resolution. A spectrally selective feature construction method improves classification by isolating brainwave bands, outperforming deep learning models. Attention mechanisms fail to identify stable neural signatures.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of classifying neurodegenerative diseases using noisy EEG signals with high intergroup similarity.

Method: Proposed a spectrally selective approach to transform raw EEG data into spectral features, focusing on frequency and time-frequency domains. Compared with traditional ML and SOTA deep learning models.

Result: Frequency-derived features enabled traditional ML models to match or exceed deep learning performance. Attention mechanisms failed to identify stable neural signatures, even with frequency-selective inputs. Demonstrated across multiple datasets.

Conclusion: Spectrally selective feature construction enhances EEG classification, outperforming deep learning and highlighting limitations of attention mechanisms in this context.
...
### [237] [On the Stability of Growth in Structural Plasticity](https://arxiv.org/abs/2605.15435)
*Lute Lillo,Nick Cheney*

Main category: cs.LG

TL;DR: The paper explores the differences between growing and pruning neural network units during training, highlighting the unique challenges of integrating new units and showing that growth is not simply the inverse of pruning.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the implications of dynamically editing network structures during training, particularly focusing on the challenges of growing new units compared to pruning existing ones.

Method: The research isolates the 'insertion problem' of new units, analyzes their gradient signals, and evaluates performance in MLP benchmarks and harder image-classification settings. It also examines interventions like optimizer state, insertion, selection, and trainability.

Result: Newborn units often receive weaker gradient signals than incumbent units. Growth can achieve high final accuracy but pruning performs better in averaging over the training trajectory or when retraining sparse networks. Improved integration of new units boosts adaptive performance but doesn't guarantee better final subnetworks.

Conclusion: Growth should be evaluated as a time-sensitive optimization process, not just as an architecture-search operator. Its success depends on the stability of unit insertion, especially in continual-learning scenarios.
...
### [238] [Don't Stop Me Yet: Sampling Loss Minima via Dissipative Riemannian Mechanics](https://arxiv.org/abs/2605.15459)
*Albert Kjøller Jacobsen,Leo Uhre Jakobsen,Johanna Marie Gegenfurtner,Georgios Arvanitidis*

Main category: cs.LG

TL;DR: DiMS, a new sampler for neural network loss minima, uses kinetic energy dynamics to sample reparameterization invariant solutions exactly, outperforming existing methods in uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: Understanding and sampling from the connected components of minima in neural network loss functions is challenging, with existing methods either being inexact or limited in exploration.

Method: DiMS employs a dynamical system with kinetic energy, gravitational pull, and friction to sample from minimum level sets, controlled by physically motivated hyperparameters.

Result: DiMS improves performance in uncertainty quantification compared to previous sampling approaches.

Conclusion: DiMS provides an effective and exact method for sampling reparameterization invariant solutions, enhancing exploration of neural network loss minima.
...
### [239] [DrugSAGE:Self-evolving Agent Experience for Efficient State-of-the-Art Drug Discovery](https://arxiv.org/abs/2605.15461)
*Yikun Zhang,Xiwei Cheng,Tianyu Liu,Yuanqi Du,Wengong Jin*

Main category: cs.LG

TL;DR: Thepaperintroduces\method(Self-evolvingAgentExperience),aframeworkthatreusesexperienceacrosstaskstoefficientlybuildstate-of-the-artdrugdiscoverymodels,outperformingothermethodsby10-30%.


<details>
  <summary>Details</summary>
Motivation: CurrentLLM-basedagentsindrugdiscoveryrequireextensivetrialanderrorforeachnewtaskwithoutretainingaccumulatedexperience,leadingtohighsearchcosts.

Method: \methodaccumulatesacross-taskmemoryofverifiedskills,effectivestrategies,anderrorfixes,enablingdirectsolutiontransferorefficientsearch.

Result: In33tasks,\methodranksfirstamongnineSOTAagents.Withmemoryfrom16tasks,itscores0.935on17held-outtasksandoutperformsbaselinesby10-30%withouttest-timesearch.

Conclusion: Cross-taskmemorysignificantlyenhancesefficiencyindevelopingSOTAmodelsfordrugdiscovery,asdemonstratedby\method'ssuperiorperformance.
...
### [240] [Layer-wise Derivative Controlled Networks](https://arxiv.org/abs/2605.15463)
*Rowan Martnishn,Sean Anderson*

Main category: cs.LG

TL;DR: ChainzRule (CR) is a novel neural architecture using Polynomial Engine and Differential Regularization (DREG) to balance accuracy, hardware efficiency, and stability while outperforming standard models.


<details>
  <summary>Details</summary>
Motivation: Traditional models struggle with conflicting demands of accuracy, efficiency, and stability, often leading to unpredictable behavior. CR aims to harmonize these goals.

Method: CR replaces piecewise-linear activations with a Polynomial Engine and employs DREG for targeted regularization on intermediate derivatives to suppress sensitivity without losing representational power.

Result: CR outperforms standard models with 15.5x fewer parameters, reduces gradient volatility by 23.1%, and achieves 70.17% accuracy on Yelp Full ordinal regression.

Conclusion: CR demonstrates that stability and accuracy can coexist, offering a practical solution for sensitive real-world applications.
...
### [241] [GRLO: Towards Generalizable Reinforcement Learning in Open-Ended Environments from Zero](https://arxiv.org/abs/2605.15464)
*Shangjian Yin,Yu Fu,Yue Dong,Zhouxing Shi*

Main category: cs.LG

TL;DR: GRLO studies the generalization of RLHF learned from minimal data in open-ended environments, showing strong transfer to downstream tasks like math and code generation with significantly less compute and data than traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the high GPU compute requirements of in-domain RL training (e.g., RLVR) and explore if conversational abilities from RLHF can implicitly transfer to specialized tasks like reasoning and code generation.

Method: Uses GRLO (generalized RL from open-ended environments) on Qwen3-4B-Base, training with only 5K prompts and 22.7 GPU hours, focusing on transfer learning from RLHF to downstream tasks.

Result: GRLO improves average performance from 24.1 to 63.1 across domains, requiring 46x less data and 68x less compute than RLVR. It achieves competitive results with lower costs and shows selective gains from additional in-domain RLVR.

Conclusion: GRLO provides a simple, efficient method for broadly capable post-training, reducing resource barriers while maintaining strong performance.
...
### [242] [Toward World Modeling of Physiological Signals with Chaos-Theoretic Balancing and Latent Dynamics](https://arxiv.org/abs/2605.15465)
...
TL;DR: NormWear-2 is a world model for long-horizon forecasting of physiological signals, combining pre-trained knowledge with adaptive insights, outperforming state-of-the-art models across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Existing models focus on static tasks, leaving long-horizon forecasting and the predictive nature of physiological signals underexplored. NormWear-2 aims to bridge this gap.

Method: The model encodes physiological signals and clinical interventions into a shared latent space, using chaos-theoretic balancing and non-parametric state transitions for multi-scale forecasting.

Result: NormWear-2 achieves the best performance in forecasting across time, frequency, and latent domains, outperforming existing models while maintaining robust representation quality.

Conclusion: NormWear-2 advances general-purpose world models for physiological signals, enabling coherent forecasting across diverse clinical and real-world settings.
...
### [243] [Njord: A Probabilistic Graph Neural Network for Ensemble Ocean Forecasting](https://arxiv.org/abs/2605.15470)
*Daniel Holmberg,Joel Oskarsson,Erik Larsson,Fredrik Lindsten,Teemu Roos*

Main category: cs.LG

TL;DR: Njord is a probabilistic data-driven ocean forecasting model that outperforms deterministic alternatives, providing uncertainty estimates and better accuracy across global and regional scales.


<details>
  <summary>Details</summary>
Motivation: Existing machine learning ocean models are deterministic and lack uncertainty estimates, which is problematic since ocean dynamics are inherently chaotic.

Method: Njord combines a deep latent variable framework with graph neural networks, using K-means cluster meshes to handle large, irregular ocean grids.

Result: Njord achieves the lowest errors on the global OceanBench benchmark, especially in surface temperature prediction, and performs well regionally (e.g., Baltic Sea).

Conclusion: Njord's probabilistic approach and scalable architecture make it a superior tool for accurate and reliable ocean forecasting.
...
### [244] [Learning Normalized Energy Models for Linear Inverse Problems](https://arxiv.org/abs/2605.15487)
...
### [245] [SurvivalPFN: Amortizing Survival Prediction via In-Context Bayesian Inference](https://arxiv.org/abs/2605.15488)
*Shi-ang Qi,Vahid Balazadeh,Michael Cooper,Russell Greiner,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: SurvivalPFN is a prior-data fitted network that simplifies survival analysis by amortizing Bayesian inference for censored data, achieving strong performance without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Survival analysis is complex, often requiring expertise to choose the right estimator. SurvivalPFN aims to simplify this by providing a general-purpose solution that adapts to data complexity.

Method: SurvivalPFN is pretrained on diverse synthetic, identifiable, and right-censored datasets. It performs inference in a single forward pass, avoiding task-specific training or hyperparameter tuning.

Result: In a benchmark with 61 datasets, 21 methods, and 5 metrics, SurvivalPFN outperforms established survival models, offering calibrated survival distributions.

Conclusion: SurvivalPFN provides a principled, practical foundation for survival analysis, with potential applications in healthcare, finance, and engineering.
...
### [246] [Ghosted Layers: Unconstrained Activation Alignment for Recovering Layer-Pruned LLMs](https://arxiv.org/abs/2605.15491)
*Vincent-Daniel Yun,Junhyuk Jo,Sai Praneeth Karimireddy,Sunwoo Lee*

Main category: cs.LG

TL;DR: Ghosted Layers is a training-free method to recover performance in pruned Transformer models by aligning activations optimally.


<details>
  <summary>Details</summary>
Motivation: Layer pruning in large language models causes hidden state mismatches, degrading performance. Ghosted Layers aims to address this without additional training.

Method: The method derives a closed-form optimal linear operator from a calibration set to reconstruct activation discrepancies from pruned layers.

Result: Ghosted Layers improves accuracy and perplexity over existing baselines while maintaining efficiency.

Conclusion: The approach effectively mitigates performance degradation in pruned models without requiring retraining.
...
### [247] [Learning with Conflicts of Interest](https://arxiv.org/abs/2605.15504)
*Nischal Aryal,Arash Termehchy,Ali Vakilian,Marianne Winslett*

Main category: cs.LG

TL;DR: The paper proposes a game-theoretic framework to address conflicts of interest between ML system owners and users, ensuring users benefit without being misled by biased information.


<details>
  <summary>Details</summary>
Motivation: Alignment issues between ML system owners and users due to financial, social, and political factors lead to biased outputs, harming user interests. Existing solutions lack incentives for owners to mitigate biases.

Method: A game-theoretic framework models interactions between ML systems and users. Scalable algorithms optimize desired information and actions while minimizing bias and manipulation.

Result: The framework and algorithms provide theoretical guarantees for maximizing beneficial information and reducing manipulative actions.

Conclusion: The proposed approach effectively balances user protection and ML system utility by addressing the inherent conflict of interest.
...
### [248] [STS: Efficient Sparse Attention with Speculative Token Sparsity](https://arxiv.org/abs/2605.15508)
*Ceyu Xu,Jiangnan Yu,Yongji Wu,Yuan Xie*

Main category: cs.LG

TL;DR: STS is a sparse attention mechanism for LLMs that avoids retraining by using a draft model's attention scores to dynamically prune computations, achieving 2.67x speedup with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: The quadratic complexity of attention in LLMs creates memory and computational bottlenecks, especially for agentic applications handling long sequences.

Method: STS leverages a draft model's attention scores to construct a sparsity mask for the target LLM, pruning expensive computations without retraining.

Result: STS achieves a 2.67x speedup at ~90% sparsity on NarrativeQA, with negligible accuracy drop compared to dense attention.

Conclusion: STS sets a new state-of-the-art in sparsity-accuracy trade-off, enabling higher sparsity for given accuracy by repurposing draft model attention scores.
...
### [249] [parallelcbf: A composable safety-filter and auditability framework for tensor-parallel reinforcement learning](https://arxiv.org/abs/2605.15509)
*Yijun Lu,Zilei Yang,Yuyin Ma*

Main category: cs.LG

TL;DR: ParallelCBF is the first framework to unify tensor-parallel UAV environments, hard-gate CBF safety filters, sharded BC-to-RL pipelines, and operational auditability for end-to-end safety-constrained training.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks lack unification of capabilities for end-to-end safety-constrained training in UAV simulations and RL benchmarks.

Method: ParallelCBF integrates tensor-parallel UAV environments, CBF safety filters, BC-to-RL pipelines, and composable auditability APIs.

Result: The framework includes a CPU PyTorch implementation, property-based safety tests, and auditable dataset features, successfully halting degraded training stages.

Conclusion: ParallelCBF provides a necessary unified solution for reproducible empirical robotics research, available for public use.
...
### [250] [OgBench: A Framework for Evaluating Graph Neural Networks on Omics Data](https://arxiv.org/abs/2605.15511)
*Louisa Cornelis,Johan Mathe,Louis Van Langendonck,Guillermo Bernárdez,Nina Miolane*

Main category: cs.LG

TL;DR: GNNs often don't outperform MLPs in low-sample, high-node omics settings, challenging assumptions about graph structure's value.


<details>
  <summary>Details</summary>
Motivation: Address the gap in GNN benchmarking for biological omics data, which operates in the $n \ll p$ regime.

Method: Introduces OgBench, a modular platform for benchmarking GNNs, MLPs, and baselines on standardized omics-derived graphs.

Result: Widely used GNNs perform similarly or worse than simple MLPs and baselines in this setting.

Conclusion: Findings question the assumed value of graph structure in omics, prompting reevaluation and future tailored GNN designs.
...
### [251] [On the Fragility of Data Attribution When Learning Is Distributed](https://arxiv.org/abs/2605.15520)
*Xian Gao,Bo Hui,Min-Te Sun,Wei-Shinn Ku*

Main category: cs.LG

TL;DR: The paper reveals vulnerabilities in data attribution methods, showing that participants can artificially inflate their attribution values without harming model performance, highlighting a new attack surface.


<details>
  <summary>Details</summary>
Motivation: To expose the assumption that attribution values accurately reflect contributions in machine learning pipelines, demonstrating how this can be exploited.

Method: The study uses latent optimization to inject synthetic batches into distributed training, exploiting non-IID label coverage and evaluator sensitivities.

Result: The attack successfully inflates the adversary's attribution value and alters the attribution structure among benign clients, without degrading accuracy or triggering defenses.

Conclusion: This work identifies attribution as a new attack surface, calling for robust and incentive-compatible scoring mechanisms to mitigate such exploits.
...
### [252] [Neural Point-Forms](https://arxiv.org/abs/2605.15524)
*Bruno Trentini,Jacob Hume,Vincenzo Antonio Isoldi,Philipp Misof,Ekaterina S. Ivshina,Kelly Maggs*

Main category: cs.LG

TL;DR: The paper introduces neural point-forms (NPFs), a family of learnable geometric features for point clouds that encode higher-order tangency information using Laplacian-based techniques and differential forms.


<details>
  <summary>Details</summary>
Motivation: Point cloud learning often misses higher-order geometric information. The paper aims to capture this by leveraging differential forms and Diffusion Geometry.

Method: NPFs use Laplacian-based techniques to model differential forms on point clouds via inner products, creating comparison matrices for tangency information.

Result: NPFs yield a compact, efficient, and permutation-invariant neural layer, showing competitive performance in synthetic and biological experiments.

Conclusion: NPFs provide an interpretable and effective representation, especially when labels depend on sampling density or manifold-like structure.
...
### [253] [Rethinking Neural Network Learning Rates: A Stackelberg Perspective](https://arxiv.org/abs/2605.15530)
*Sihan Zeng,Sujay Bhatt,Sumitra Ganesh*

Main category: cs.LG

TL;DR: Non-uniform learning rates in neural networks, optimized via Stackelberg reformulation, can outperform uniform rates by enhancing convergence and local curvature.


<details>
  <summary>Details</summary>
Motivation: To understand the conditions and mechanisms where layer-specific learning rates improve neural network training, which is currently limited in principled understanding.

Method: Investigates non-uniform learning rates using Stackelberg optimization, treating the training process as a two-time-scale alternating gradient descent algorithm.

Result: Demonstrates finite-time convergence and identifies two mechanisms: stronger optimization structure in some problems and sharper local curvature in the Stackelberg objective, leading to faster learning.

Conclusion: Non-uniform learning rates, especially with smaller rates for body layers and larger rates for the final layer, can accelerate training, as validated in supervised and reinforcement learning experiments.
...
### [254] [DeltaPrompts: Escaping the Zero-Delta Trap in Multimodal Distillation](https://arxiv.org/abs/2605.15532)
*Jaehun Jung,Hyunwoo Kim,Brandon Cui,Ximing Lu,David Acuna,Prithviraj Ammanabrolu,Yejin Choi*

Main category: cs.LG

TL;DR: DeltaPrompts introduces a staged synthesis pipeline to create high-divergence prompts, improving distillation efficiency for Vision-Language Models by targeting student failure modes.


<details>
  <summary>Details</summary>
Motivation: Standard prompts in distillation often provide minimal learning signal due to zero-delta issues, limiting student improvement. DeltaPrompts aims to address this by identifying and leveraging high-divergence prompts.

Method: The method involves quantifying answer divergence (Δ) and using a staged synthesis pipeline to generate 200k synthetic, high-divergence prompts from existing datasets.

Result: DeltaPrompts achieves up to 15% relative improvement across 10 benchmarks in diverse settings, including on-policy distillation, transfer learning, and off-policy fine-tuning.

Conclusion: Focusing on high-divergence prompts enhances distillation efficiency and model performance, demonstrating the value of targeted prompt generation.
...
### [255] [CTF4Nuclear: Common Task Framework for Nuclear Fission and Fusion Models](https://arxiv.org/abs/2605.15549)
...
TL;DR: The paper proposes a Common Task Framework (CTF) to fairly compare Machine Learning (ML) methods for nuclear engineering, addressing the need for reliable surrogate models in safety-critical applications.


<details>
  <summary>Details</summary>
Motivation: High-fidelity simulations for nuclear systems are computationally expensive, and model-based approaches often suffer from discrepancies with real-world measurements. ML offers potential for quick, reliable predictions but lacks standardized comparison methods in safety-critical settings.

Method: The authors introduce a CTF for ML in nuclear engineering, using curated datasets and evaluating methods on 12 metrics, including a new paradigm for system monitoring from sparse measurements. Standard ML baselines are benchmarked against these datasets.

Result: The framework highlights limitations of current ML methods and aims to replace ad hoc comparisons with standardized evaluations, improving rigor and reproducibility.

Conclusion: The CTF provides a structured approach to evaluate ML methods in nuclear engineering, promoting better decision-making and reliability in this safety-critical field.
...
### [256] [Characterizing Learning in Deep Neural Networks using Tractable Algorithmic Complexity Analysis](https://arxiv.org/abs/2605.15551)
*Pedram Bakhtiarifard,Sophia N. Wilson,Mahmoud Afifi,Jonathan Wenshøj,Raghavendra Selvan*

Main category: cs.LG

TL;DR: The paper introduces QuBD to estimate algorithmic complexity in large-scale DNNs, revealing insights into learning dynamics and practical quantization guidance.


<details>
  <summary>Details</summary>
Motivation: Training large DNNs is resource-heavy, and the 'learning as compression' hypothesis suggests structured weights enable compression. Current complexity estimators (CTM/BDM) are limited to small binary objects, necessitating a scalable method for modern DNNs.

Method: QuBD extends algorithmic complexity estimation by quantizing weights to a finite alphabet and aggregating per bit-plane CTM estimates. It theoretically tightens the gap to true KCS complexity compared to binarization.

Result: QuBD shows complexity decreases during learning, scales with data, rises during overfitting, and correlates with generalization. Most algorithmic information resides in significant bit-planes, aiding quantization decisions.

Conclusion: QuBD provides scalable KCS complexity estimates for large non-binary objects like DNN weights, offering insights into learning mechanisms and practical quantization diagnostics.
...
### [257] [CrystalBoltz: End-to-End Protein Structure Determination via Experiment-Guided Diffusion for X-Ray Crystallography](https://arxiv.org/abs/2605.15564)
*Minseo Kim,Huanghao Mai,Jay Shenoy,Alec Follmer,Gordon Wetzstein,Frederic Poitevin*

Main category: cs.LG

TL;DR: CrystalBoltz is a generative framework for crystallographic refinement, combining Bayesian inference with experimental data to improve protein structure prediction accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing generative models for protein structures lack direct integration with experimental crystallographic data, limiting their utility in X-ray structure determination.

Method: CrystalBoltz uses Bayesian inference over atomic structures, directly incorporating structure-factor amplitudes from experiments to guide refinement.

Result: The method achieves lower coordinate RMSD and R-factors compared to baselines, with a 33x reduction in runtime.

Conclusion: CrystalBoltz significantly enhances crystallographic refinement by integrating experimental data more effectively, reducing manual effort and improving accuracy.
...
### [258] [VSPO: Vector-Steered Policy Optimization for Behavioral Control](https://arxiv.org/abs/2605.15604)
*Xuechen Zhang,Zijian Huang,Kai Yang,Weijia Zhang,Jiasi Chen,Samet Oymak*

Main category: cs.LG

TL;DR: VSPO enhances language models by using steering vectors to control target behaviors, improving rare behavior sampling and policy optimization.


<details>
  <summary>Details</summary>
Motivation: Addressing sparse behavioral rewards in multi-objective optimization for language models to better align with desired secondary behaviors.

Method: Introduces Vector-Steered Policy Optimization (VSPO), modifying GRPO to vary steering intensities, acting as on-policy latent self-distillation.

Result: VSPO outperforms alternatives like reward shaping, showing better control over target behaviors without sacrificing task accuracy.

Conclusion: VSPO effectively optimizes language models for desired behaviors, proving superior in theory and practice across benchmarks.
...
### [259] [AstraFlow: Dataflow-Oriented Reinforcement Learning for Agentic LLMs](https://arxiv.org/abs/2605.15565)
*Haizhong Zheng,Yizhuo Di,Jiahui Wang,Shuowei Jin,Xueshen Liu,Yongji Wu,Z. Morley Mao,Ion Stoica,Jiawei Zhao,Beidi Chen*

Main category: cs.LG

TL;DR: AstraFlow is a dataflow-oriented RL system that simplifies multi-policy collaborative training by decoupling components, improving efficiency and scalability without code changes.


<details>
  <summary>Details</summary>
Motivation: Agentic reinforcement learning (RL) for large language models (LLMs) is costly and complex, requiring support for multi-policy training and efficient resource use. Existing systems lack flexibility and require extensive engineering for extensions.

Method: AstraFlow introduces a dataflow-oriented architecture with autonomous components for rollouts, dataflow management, and training, replacing trainer-centered control with principled abstractions.

Result: AstraFlow supports multi-policy training, elastic scaling, and cross-region execution without system-level code changes, achieving 2.7x faster training while maintaining or improving accuracy.

Conclusion: AstraFlow addresses the limitations of current RL systems by enabling efficient, scalable, and flexible agentic RL for LLMs through its decoupled, dataflow-oriented design.
...
### [260] [Gaussian Relational Graph Transformer](https://arxiv.org/abs/2605.15575)
*Zezhong Ding,Jin Li,Xugang Wang,Xike Xie*

Main category: cs.LG

TL;DR: GelGT, a Gaussian relational graph transformer, improves relational graph learning by addressing long-range dependencies and jointly modeling structural, semantic, and temporal information.


<details>
  <summary>Details</summary>
Motivation: Existing relational graph learning methods struggle with capturing long-range dependencies and fail to jointly model structural, semantic, and temporal information effectively.

Method: GelGT introduces a structure-semantic collaborative sampling strategy and a Gaussian graph attention mechanism with learnable Gaussian bias to dynamically encode temporal dependencies.

Result: Extensive experiments show GelGT achieves state-of-the-art performance, with up to a 13.8% improvement in predictive tasks.

Conclusion: GelGT successfully addresses the limitations of existing methods and demonstrates superior performance on real-world datasets.
...
### [261] [Embracing Biased Transition Matrices for Complementary-Label Learning with Many Classes](https://arxiv.org/abs/2605.15586)
*Tan-Ha Mai,Chao-Kai Chiang,Han-Hwa Shih,Gang Niu,Masashi Sugiyama,Hsuan-Tien Lin*

Main category: cs.LG

TL;DR: BICL overcomes the scalability limitations of CLL by introducing a biased label generation process, achieving significant accuracy improvements on many-class datasets.


<details>
  <summary>Details</summary>
Motivation: Address the enduring bottleneck in CLL's scalability to large label spaces caused by uniform label generation assumptions.

Method: Propose BICL, a framework using biased (non-uniform) complementary label generation to restrict labels to a subset of classes, enhancing learning signals.

Result: BICL achieves over sevenfold accuracy improvements on CIFAR-100 and TinyImageNet-200 compared to traditional CLL methods.

Conclusion: BICL establishes a viable path for practical CLL applications in many-class settings, overcoming a long-standing challenge.
...
### [262] [Offline Reinforcement Learning with Universal Horizon Models](https://arxiv.org/abs/2605.15603)
*Hojun Chung,Junseo Lee,Songhwai Oh*

Main category: cs.LG

TL;DR: Universal Horizon Models (UHM) improve offline reinforcement learning by predicting future states under arbitrary horizons, outperforming baselines on tasks with suboptimal datasets and long-horizon reasoning.


<details>
  <summary>Details</summary>
Motivation: Model-based RL suffers from compounding errors due to repeated model inference on self-generated states, and existing methods like GHM struggle with distant future states.

Method: UHM generalizes GHM by predicting future states under arbitrary horizons and uses a winsorized horizon distribution to stabilize training.

Result: UHM outperforms baselines on 100 OGBench tasks, especially with suboptimal datasets and long-horizon reasoning.

Conclusion: UHM provides a scalable and effective solution for offline RL by addressing challenges in modeling distant future states and stabilizing training.
...
### [263] [Transformer-like Inference from Optimal Control](https://arxiv.org/abs/2605.15608)
*Aditya Kudre,Heng-Sheng Chang,Prashant G. Mehta*

Main category: cs.LG

TL;DR: The paper derives transformer-like architectures from optimal control theory, showing that decoder-only transformers can be seen as solutions to an optimal control problem for prediction tasks. It introduces the dual filter as an inference algorithm that mirrors transformer layers and reveals insights into transformer behavior.


<details>
  <summary>Details</summary>
Motivation: The paper aims to provide a theoretical foundation for transformer architectures by deriving them from first principles of optimal control theory. It seeks to understand why transformers work well and how their layer operations can be seen as solutions to a prediction problem.

Method: The framework is developed for two model classes: a nonlinear discrete-valued process (motivated by transformers) and a linear Gaussian model (as a tractable baseline). The prediction objective is reformulated as an optimal control problem, leading to the dual filter algorithm, which mirrors transformer layers. Numerical experiments compare the optimal control solution to trained transformer attention weights.

Result: The dual filter algorithm, derived from optimal control, matches the layer structure of decoder-only transformers. Experiments show transformers implicitly exploit non-Markovian structure when embedding dimensions are insufficient.

Conclusion: The paper demonstrates that transformer architectures can be derived from optimal control theory, providing a theoretical justification for their success. The dual filter offers a principled way to understand and potentially improve transformer designs.
...
### [264] [Position: Zeroth-Order Optimization in Deep Learning Is Underexplored, Not Underpowered](https://arxiv.org/abs/2605.15622)
...
TL;DR: This paper challenges the dismissal of zeroth-order (ZO) optimization as unscalable, arguing that its limitations stem from underdevelopment rather than inherent flaws. It proposes rethinking ZO methods by focusing on variance control, subspace views, system advantages, and task clarity to enable scalable and efficient learning.


<details>
  <summary>Details</summary>
Motivation: The paper aims to correct the misconception that ZO optimization is fundamentally unscalable, highlighting its potential in deep learning due to memory efficiency and applicability to gray- or black-box pipelines. It argues that perceived limitations arise from narrow development practices.

Method: The authors revisit ZO optimization by focusing on variance control, variance-query tradeoffs, and directional derivatives. They propose (i) subspace and spectral views for interpretable variance reduction, (ii) leveraging the forward-only nature of ZO for system advantages, and (iii) clarifying ZO evaluations.

Result: The paper identifies three underexplored opportunities for ZO optimization and advocates for a shift in development practices to harness its unique strengths, such as communication efficiency and pipeline-friendliness.

Conclusion: ZO optimization is viable for large-scale, system-aware, and resource-efficient learning if redeveloped around its strengths, such as subspace views and forward-only advantages, rather than dismissed due to outdated practices.
...
### [265] [IO-SVD: Input-Output Whitened SVD for Adaptive-Rank LLM Compression](https://arxiv.org/abs/2605.15626)
*Ali Abbasi,Chayne Thrash,Haoran Qin,Hamed Pirsiavash,Soheil Kolouri*

Main category: cs.LG

TL;DR: IO-SVD is a post-training compression method for large language models that uses input-output whitening and heterogeneous rank allocation to reduce model size efficiently with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Large language models face high storage and compute costs, limiting deployment in resource-constrained settings. Current SVD-based compression methods often degrade model quality under aggressive compression.

Method: IO-SVD forms a KL-aware double-sided whitening space for weights, uses a second-order expansion of KL loss for output-side sensitivity, and introduces efficient heterogeneous rank allocation and loss-aware remapping for hybrid SVD-quantization.

Result: IO-SVD compresses LLMs effectively with minimal performance degradation, offering practical inference speedups, as demonstrated across diverse model families.

Conclusion: IO-SVD provides an efficient, hardware-agnostic way to compress large language models while preserving performance, making it suitable for resource-constrained and latency-sensitive applications.
...
### [266] [Perforated Neural Networks for Keyword Spotting](https://arxiv.org/abs/2605.15647)
*Vishy Gopal,Aris Ilias Goutis,Ralph Crewe,Erin Yanacek,Rorry Brenner*

Main category: cs.LG

TL;DR: This paper explores Perforated Backpropagation for keyword spotting on edge devices, demonstrating improved accuracy and efficiency with dendritic models.


<details>
  <summary>Details</summary>
Motivation: Edge machine learning faces constraints like memory, compute limits, and accuracy requirements. Existing methods often trade one resource for another without improving both accuracy and model size simultaneously.

Method: The paper applies Perforated Backpropagation by adding artificial Dendrite Nodes to a convolutional neural network trained on the Edge Impulse keyword spotting tutorial pipeline.

Result: Dendritic models outperformed traditional architectures in 800 hyperparameter trials. The best model achieved 0.933 accuracy with 1,500 parameters vs. the baseline's 0.921 with 4,000 parameters.

Conclusion: Perforated Backpropagation enhances edge AI by improving both model quality and deployment efficiency simultaneously.
...
### [267] [Layer Equivalence Is Not a Property of Layers Alone: How You Test Redundancy Changes What You Find](https://arxiv.org/abs/2605.16234)
*Gabriel Garcia*

Main category: cs.LG

TL;DR: The paper highlights the distinction between replacement and interchange tests for transformer layer equivalence in compression, showing their results can diverge significantly, especially at scale.


<details>
  <summary>Details</summary>
Motivation: To clarify the differences between replacement and interchange tests for assessing transformer layer equivalence in compression, as conflating them can lead to misleading conclusions about layer pruning safety.

Method: Measure both replacement and interchange protocols across various checkpoints and architectures, including Pythia (410M and 1.4B) and Qwen3-8B, comparing their impacts on layer pruning safety.

Result: The replacement-interchange gap grows from initialization to convergence. Qwen3-8B shows interchange-guided removal is safer than replacement-guided, while Llama-3.1-8B ties the two protocols despite lower interchange KL.

Conclusion: Before pruning or merging layers, both swap-KLs should be evaluated on the target checkpoint, as their gaps can significantly affect layer safety. This diagnostic requires only unlabeled forward passes.
...
### [268] [Towards Code-Oriented LM Embeddings for Surrogate-Assisted Neural Architecture Search](https://arxiv.org/abs/2605.15649)
...
### [269] [Sharp Spectral Thresholds for Logit Fixed Points](https://arxiv.org/abs/2605.15651)
*Tongxi Wang*

Main category: cs.LG

TL;DR: Thepaperidentifiesandfillsagapinthestabilityanalysisofsoftmaxfeedbacksystems,showingthatclassicaltheorymissesastableregimeandprovidingasharperthresholdforstability.


<details>
  <summary>Details</summary>
Motivation: Toaddressthelimitationsofclassicaltheoryinanalyzingthestabilityofsoftmaxfeedbacksystems,particularlyinidentifyingthetruepointofqualitativechangeandextendingstabilityguarantees.

Method: Theauthorsanalyzefinite-dimensionalaffinelogitsystemsandderiveasharpdimension-freeEuclideanthresholdforstability,contrastingitwiththeclassicalcondition.

Result: Thenewthreshold,\(β\|ΠWΠ\|_{\mathcalT\to\mathcalT}<2\),extendsstabilityguaranteestoreward-responsivesystemsandidentifiesthetruephasetransitionpoint.

Conclusion: Thestudyenhancestheunderstandingofsoftmaxfeedbacksystemsbycorrectingtheclassicalapproachandprovidingamoreaccuratestabilityboundary.
...
### [270] [On the Power of Adaptivity for $\varepsilon$-Best Arm Identification in Linear Bandits](https://arxiv.org/abs/2605.15663)
*Arnab Maiti,Yunbei Xu,Kevin Jamieson*

Main category: cs.LG

TL;DR: This paper studies the minimax sample complexity of ε-best arm identification in linear bandits, presenting non-adaptive and adaptive methods, along with matching lower bounds.


<details>
  <summary>Details</summary>
Motivation: The goal is to identify an arm with nearly optimal reward in linear bandits, using as few samples as possible, and to understand the role of adaptivity in sample complexity.

Method: Presents a non-adaptive fixed-design method with sample complexity bounds, proves a matching lower bound, and investigates adaptive sampling for structured action sets.

Result: The paper shows that adaptivity yields only logarithmic improvements for some action sets but polynomial improvements for others, and introduces a novel adaptive algorithm for ℓ₂-norm estimation.

Conclusion: The study highlights the varying impact of adaptivity on sample complexity and provides a new adaptive algorithm for a specific estimation task.
...
### [271] [Interaction-Aware Influence Functions for Group Attribution](https://arxiv.org/abs/2605.15675)
*Jaeseung Heo,Kyeongheung Yun,Youngbin Choi,Sehyun Hwang,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: The paper introduces an interaction-aware influence function to capture how groups of training examples jointly influence a target function, outperforming standard first-order methods.


<details>
  <summary>Details</summary>
Motivation: Standard influence functions sum individual influences but fail to account for interactions between examples, such as redundancy or complementarity, which can affect the target function.

Method: Proposes an estimator by expanding the target to second order, adding a pairwise interaction term to capture joint effects of examples.

Result: Empirical evaluation shows the estimator outperforms first-order influence across multiple dataset-model pairs and improves data selection for instruction-tuning tasks.

Conclusion: The interaction-aware influence function provides a more accurate measure of group influence and enhances performance in practical applications like data selection.
...
### [272] [FRWKV+: Adaptive Periodic-Position Branch Interaction for Frequency-Space Linear Time Series Forecasting](https://arxiv.org/abs/2605.15690)
*Qingyuan Yang,Dongyue Chen,Da Teng,Junhua Xiao,Jiaji Pan,Shizhuo Deng*

Main category: cs.LG

TL;DR: FRWKV+ enhances the FRWKV model for frequency-space forecasting by introducing cross-branch gates and an Adaptive PhaseGate mechanism, improving information exchange and periodic-position context utilization.


<details>
  <summary>Details</summary>
Motivation: To address the weak exchange of information between real and imaginary frequency streams and the need for reliable periodic-position cues in lightweight forecasting models.

Method: Proposes FRWKV+ with cross-branch gates for frequency stream interaction and an Adaptive PhaseGate mechanism for periodic-position context, controlled by an adaptive trust mechanism.

Result: FRWKV+ outperforms other variants in MSE metrics and shows clear gains in periodic regimes, with component analysis validating its key features.

Conclusion: FRWKV+ effectively leverages periodic-position context and adaptive corrections, though simpler rules may suffice in certain boundary cases.
...
### [273] [SEED: Targeted Data Selection by Weighted Independent Set](https://arxiv.org/abs/2605.15691)
*Yuan Zhang,Lifeng Guo,Junwen Pan,Chang Liu,Wenzhao Zheng,Kuan Cheng,Kurt Keutzer,Shanghang Zhang*

Main category: cs.LG

TL;DR: The paper introduces SEED, a data selection method using a Weighted Independent Set (WIS) on a similarity graph, with refinements for node value calibration and local scale normalization to enhance performance.


<details>
  <summary>Details</summary>
Motivation: Data selection aims to balance quality and diversity in large-scale training corpora, but existing methods struggle with distinguishing informative signals and handling heterogeneous domain distributions.

Method: Formulates data selection as a WIS problem on a similarity graph, with node value calibration and local scale normalization to improve node weights and edge construction.

Result: SEED outperforms state-of-the-art methods on tasks like instruction tuning, visual instruction tuning, and semantic segmentation across diverse model families.

Conclusion: SEED provides a robust and scalable solution for data selection, validated by the curated dataset Honeybee-Remake-SEED-200K and extensive experiments.
...
### [274] [Tighter Regret Bounds for Contextual Action-Set Reinforcement Learning](https://arxiv.org/abs/2605.15692)
*Zijun Chen,Zihan Zhang*

Main category: cs.LG

TL;DR: The paper extends the MVP algorithm to episodic reinforcement learning with adversarial action contexts, providing theoretical guarantees including minimax and gap-dependent regret bounds, and a sample complexity bound.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of episodic reinforcement learning where action sets vary per episode, the study aims to provide robust theoretical guarantees for performance in adversarial and stochastic contexts.

Method: The MVP algorithm is extended to handle episodic reinforcement learning with varying action sets, focusing on regret minimization against episode-wise optimal values.

Result: The study achieves a minimax regret bound of O~(√SAH³K log L) for adversarial contexts and O~(√SAH³K) for stochastic contexts, along with gap-dependent regret improvements.

Conclusion: The MVP algorithm's extension effectively addresses varying action sets in episodic reinforcement learning, with strong theoretical guarantees and potential improvements in regret bounds.
...
### [275] [Going Beyond the Edge: Distributed Inference of Transformer Models on Ultra-Low-Power Wireless Devices](https://arxiv.org/abs/2605.15694)
*Alexander Gräfe,Ding Huo,Johannes Berger,Marco Zimmerling,Sebastian Trimpe*

Main category: cs.LG

TL;DR: CATS is a framework enabling distributed transformer inference on ultra-low-power IoT devices by co-designing transformer partitioning, wireless communication, and training, using a pruned communication primitive and message-dropout for robustness.


<details>
  <summary>Details</summary>
Motivation: Transformer models are computationally and memory-intensive, making them unsuitable for single ultra-low-power IoT devices. CATS addresses this by enabling collaborative execution across multiple devices.

Method: CATS uses SomeGather, a pruned communication primitive, to reduce bandwidth and RAM usage. It also employs message-dropout during training for robustness against unreliable wireless communication.

Result: CATS successfully deploys transformer models up to 14 times larger than what a single device can handle, on up to 16 devices.

Conclusion: CATS pioneers distributed transformer inference on ultra-low-power wireless devices, demonstrating scalability and robustness in real-world deployments.
...
### [276] [AGOP-IxG: A Gradient Covariance Filter for Local Feature Attribution on Tabular Data, with a Controlled Benchmark](https://arxiv.org/abs/2605.15700)
*Raj Kiran Gupta Katakam*

Main category: cs.LG

TL;DR: AGOP-IxG, a fast per-sample attribution method for tabular classifiers, outperforms SHAP, Integrated Gradients, InputXGradient, and LIME in accuracy and speed on synthetic and real datasets.


<details>
  <summary>Details</summary>
Motivation: The need for explainable machine learning models in automated pipelines for diverse stakeholders drives the development of AGOP-IxG, addressing the lack of ground-truth attribution evaluations in real data.

Method: AGOP-IxG pre-multiplies per-sample gradients by a rank-truncated Average Gradient Outer Product matrix. It is evaluated against SHAP, Integrated Gradients, InputXGradient, and LIME on synthetic and real datasets.

Result: AGOP-IxG shows superior Spearman rank correlation, noise feature mass, and top-$k$ precision on synthetic datasets, and is 350× to 1,650× faster than SHAP. Global faithfulness tests show comparable performance (±1.7% AUC) to other methods.

Conclusion: AGOP-IxG is a highly efficient and accurate attribution method for per-sample local explanations in tabular classifiers, though it is not optimized for global feature ranking.
...
### [277] [Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models](https://arxiv.org/abs/2605.15706)
*Xingjian Wu,Junkai Lu,Siyu Yan,Xiangfei Qiu,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: DMoA is a self-evolving multi-agent framework that dynamically routes and activates agents for adaptive collaboration, outperforming existing systems in flexibility, efficiency, and performance across 9 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems (MAS) often use fixed communication topologies, limiting their adaptability. This work aims to create a more flexible and dynamic system for complex reasoning tasks.

Method: DMoA introduces a differentiable, context-aware routing mechanism using recurrent structures to dynamically route and activate agents. It employs predictive entropy as a self-supervised signal for optimizing routing.

Result: DMoA achieves state-of-the-art performance in 9 benchmarks, demonstrating efficiency, robustness, and strong ensembling capabilities.

Conclusion: The DMoA framework effectively addresses the limitations of static MAS by enabling adaptive agent collaboration and improving performance without external annotations.
...
### [278] [Bidirectional Fusion Guided by Cardiac Patterns for Semi-Supervised ECG Segmentation](https://arxiv.org/abs/2605.15722)
*Jeonghwa Lim,Minje Park,Sunghoon Joo*

Main category: cs.LG

TL;DR: CardioMix, a bidirectional CutMix strategy for ECG segmentation, enhances labeled data with realistic variations from unlabeled data, improving model performance across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: The scarcity of annotated ECG data challenges training deep learning models, and existing semi-supervised methods underutilize information exchange between labeled and unlabeled sets.

Method: CardioMix uses a bidirectional CutMix strategy guided by cardiac patterns to enrich labeled data and apply stronger supervisory signals to unlabeled data, ensuring physiological relevance.

Result: CardioMix outperforms existing CutMix-based methods on the SemiSegECG benchmark, demonstrating consistency across datasets and labeled ratios.

Conclusion: CardioMix is an effective plug-and-play framework for ECG segmentation, addressing data scarcity and improving semi-supervised learning performance.
...
### [279] [GOMA: Toward Structure-Driven Multimodal Alignment from a Graph Signal Smoothing Perspective](https://arxiv.org/abs/2605.15723)
*Xu Wang,Xunkai Li,Yinlin Zhu,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: GOMA is a graph-based framework for refining frozen vision-language embeddings by leveraging multimodal attributed graphs (MAGs), outperforming competitors in retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods like CLIP rely on isolated image-text pairs, missing relational context in multimodal data. MAGs offer a way to refine embeddings using graph structure.

Method: GOMA decouples message flow, propagation of multimodal evidence, and smoothing depth. It uses modality-aware propagation, finite-step coupled smoothing, and adaptive readout of node-specific trajectories.

Result: GOMA achieves state-of-the-art or tied performance on seven MAG benchmarks and remains more stable than graph-based competitors.

Conclusion: MAG structure can effectively refine frozen multimodal embeddings, with GOMA demonstrating superior retrieval performance and stability.
...
### [280] [A Unified Perturbation Framework for Analyzing Leaderboard Stability and Manipulation](https://arxiv.org/abs/2605.15761)
*Hosna Oyarhoseini,Jimmy Lin,Amir-Hossein Karimi*

Main category: cs.LG

TL;DR: The paper introduces a framework to assess the robustness of leaderboard rankings in benchmarking large language models, showing they are vulnerable to small perturbations.


<details>
  <summary>Details</summary>
Motivation: Leaderboards like LMArena are crucial for benchmarking models but lack understanding of ranking stability under data changes.

Method: Uses a perturbation framework with Drop, Add, Flip, and player removal to analyze effects on rankings, consistency, and uncertainty.

Result: Modern leaderboards are non-robust; tiny perturbations (under 1%) can alter top models, degrade rankings, and change confidence intervals.

Conclusion: The framework offers a tool to audit leaderboard stability and encourages more robust evaluation protocols.
...
### [281] [Continual Learning of Domain-Invariant Representations](https://arxiv.org/abs/2605.15775)
*Pascal Janetzky,Tobias Schlagenhauf,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: The paper introduces a class of continual learning (CL) methods focused on learning domain-invariant representations to improve generalization to unseen domains, outperforming existing CL baselines.


<details>
  <summary>Details</summary>
Motivation: Existing CL methods often rely on domain-specific cues, leading to poor generalization. The paper addresses this by focusing on domain-invariant representations that capture underlying causal mechanisms.

Method: Proposes CL methods combining replay-based training with sequential invariance alignment to learn and preserve invariant structures across domains.

Result: The methods consistently outperform existing CL baselines in generalization to unseen domains across six diverse datasets, including vision, medicine, manufacturing, and ecology.

Conclusion: This work pioneers domain-invariant representation learning for CL, demonstrating its effectiveness in improving out-of-domain generalization.
...
### [282] [Grokking as Structural Inference: Transformers Need Bayesian Lottery Tickets](https://arxiv.org/abs/2605.15787)
*Kai Hidajat,Solden Stoll,Joseph An*

Main category: cs.LG

TL;DR: The paper explores why Transformers delay generalization after memorizing training data, attributing it to Bayesian structural conditions in attention and MLP capacity, and proposes interventions to bypass delays.


<details>
  <summary>Details</summary>
Motivation: Understanding the delayed generalization in Transformers, which existing theories partially explain but overlook attention-specific constraints.

Method: Formalizes attention as a Bayesian posterior, identifies conditions for generalization (MLP capacity and Bayesian structural condition), and tests interventions like KL-based methods.

Result: Shows delayed generalization is due to structural inference delays, and experiments validate that structural interventions can bypass these delays.

Conclusion: Decoupling MLP capacity and attention structure explains delayed generalization, and interventions can accelerate learning, offering insights for efficient Transformer training.
...
### [283] [Learning Context-conditioned Gaussian Overbounds for Convolution-Based Uncertainty Propagation](https://arxiv.org/abs/2605.15789)
*Ruirui Liu,Xuejie Hou,Yiping Jiang,Hui Ren*

Main category: cs.LG

TL;DR: The paper introduces a neural network-based framework for learning context-aware Gaussian overbounds with provable conservatism, improving upon traditional methods by being less redundant and more adaptable.


<details>
  <summary>Details</summary>
Motivation: Uncertainty quantification is crucial in safety-critical fields, but existing methods like predictor-level intervals or classical Gaussian overbounding are often overly conservative or lack adaptability to feature-conditioned errors. This work aims to address these limitations.

Method: The authors propose a learning framework where neural networks produce feature-conditioned Gaussian overbounds (mean and scale) with guaranteed conservatism on a quantile grid. The framework uses a loss function that enforces conservatism at selected quantiles while minimizing distributional distance.

Result: The method yields tighter bounds compared to traditional approaches while maintaining conservatism on both synthetic and real-world datasets, including multipath, ionospheric, and tropospheric residual errors.

Conclusion: The framework is versatile and applicable to systems requiring conservative, feature-conditioned uncertainty estimates, offering improvements in adaptability and conservatism over existing methods.
...
### [284] [AOT-POT: Adaptive Operator Transformation for Large-Scale PDE Pre-training](https://arxiv.org/abs/2605.15793)
*Qitan Lv,Hong Wang,Zhongkai Hao,Wen Wu,Xuenan Xu,Bowen Zhou,Feng Wu,Chao Zhang*

Main category: cs.LG

TL;DR: AOT-POT proposes adaptive operator transformation to unify diverse PDE solution operators for pre-training, achieving state-of-the-art performance with minimal added parameters.


<details>
  <summary>Details</summary>
Motivation: The complexity and diversity of PDE solution operators make multi-PDE pre-training challenging, which existing methods address by scaling model capacity rather than simplifying the operators.

Method: AOT-POT transforms diverse solution operators into simpler forms using adaptive transformations, involving parallel streams, adaptive aggregation, and Sinkhorn-projected matrices for stable training.

Result: Achieves a 40.9% average reduction in L2 error on 12 benchmarks with only 3% additional parameters, and fine-tuning further reduces errors significantly for both in-domain and out-of-domain PDEs.

Conclusion: Adaptive operator transformation is an effective and complementary approach to advancing PDE foundation models, outperforming methods that solely focus on model capacity scaling.
...
### [285] [Martingale Neural Operators: Learning Stochastic Marginals via Doob-Meyer Factorization](https://arxiv.org/abs/2605.15806)
*Kai Hidajat*

Main category: cs.LG

TL;DR: The paper introduces Martingale Neural Operator (MNO) to predict both the conditional mean and covariance for stochastic PDEs, avoiding collapse to the mean and improving efficiency over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing neural operators fail to capture variance and tail structure in stochastic PDEs, which are crucial for uncertainty quantification, while traditional methods sacrifice efficiency.

Method: MNO leverages the Doob-Meyer theorem to decompose predictions into a predictable drift and a zero-mean martingale, parameterized by a mean and a low-rank factor for covariance.

Result: MNO significantly reduces Wasserstein distance (up to 120x) and evaluates faster (3x) than baselines, performing well on 1D and 2D tasks but struggling with quasi-deterministic systems.

Conclusion: MNO successfully balances efficiency and accuracy for stochastic PDEs, though challenges remain for certain deterministic-like systems.
...
### [286] [Intrinsic Wasserstein Rates for Score-Based Generative Models on Smooth Manifolds](https://arxiv.org/abs/2605.15822)
*Guoji Fu,Taiji Suzuki,Wee Sun Lee,Atsushi Nitanda*

Main category: cs.LG

TL;DR: Score-based generative models (SGMs) for data on low-dimensional manifolds achieve a Wasserstein-1 sample exponent, with explicit bounds on geometry, density, and ambient dimensions.


<details>
  <summary>Details</summary>
Motivation: Many data distributions lie on low-dimensional nonlinear structures (manifolds), and the study aims to understand how SGMs perform in such settings.

Method: The analysis uses a variance-preserving SGM estimator, separating score approximation into large-noise tangent-cell and small-noise projection-centered regimes. It employs ReLU networks to implement nearest-projection coordinates with finite intrinsic anchors and Gauss-Newton iterations.

Result: The SGM estimator attains the intrinsic Wasserstein--1 sample exponent, isolating factors like geometry, density, and ambient dependence.

Conclusion: For manifolds with polynomial geometry and density bounds, the score-network parameters depend polynomially on the ambient dimension, providing a practical and efficient framework.
...
### [287] [Ti-iLSTM: A TinyDL Approach for Logic-Level Anomaly Detection in Industrial Water Treatment Systems](https://arxiv.org/abs/2605.15874)
*Mandar Joshi,Farzana Zahid,Judy Bowen,Matthew M. Y. Kuo,Valeriy Vyatkin,Emil Karlsson*

Main category: cs.LG

TL;DR: The paper proposes a lightweight TinyDL-based LSTM framework (Ti-iLSTM) for detecting logic-layer anomalies in PLCs of Industrial Water Treatment Systems, achieving high performance with minimal resource usage.


<details>
  <summary>Details</summary>
Motivation: Industrial Water Treatment Systems (IWTS) are vulnerable to cyber threats, especially logic-layer deception anomalies, which are hard to detect with traditional methods. A lightweight, on-device solution is needed for resource-constrained PLCs.

Method: The authors introduce a TinyDL-based incremental LSTM (Ti-iLSTM) framework that optimizes memory and space usage of LSTM models for anomaly detection in PLCs.

Result: Experiments on the SWaT dataset show high performance (F1-score=0.983, ROC-AUC=0.998). Validation on the WADI dataset confirms broader applicability.

Conclusion: The research shows that combining logic-aware supervision with TinyDL enables efficient and accurate anomaly detection for resource-constrained PLCs in industrial settings.
...
### [288] [Shapley Neuron Values for Continual Learning: Which Neurons Matter Most?](https://arxiv.org/abs/2605.15877)
*Mohammad Ali Vahedifar,Abhisek Ray,Qi Zhang*

Main category: cs.LG

TL;DR: SNV (Shapley Neuron Valuation) uses cooperative game theory to prevent catastrophic forgetting in neural networks by selectively freezing important neurons, outperforming other buffer-free methods on ImageNet-1k.


<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting in neural networks during continual learning without expanding architecture or using buffers.

Method: Introduces Shapley Neuron Valuation (SNV) to quantify neuron importance and selectively freeze critical neurons while allowing others to remain plastic.

Result: SNV improves accuracy by +2.88% in class incremental learning and +6.46% in task incremental learning compared to baselines on ImageNet-1k.

Conclusion: SNV is an effective, buffer-free solution for continual learning, leveraging neuron valuation to mitigate catastrophic forgetting and enhance performance.
...
### [289] [Practical Validity Conditions for Byzantine-Tolerant Federated Learning](https://arxiv.org/abs/2605.15887)
*Mélanie Cambus,Darya Melnyk,Tijana Milentijević,Stefan Schmid*

Main category: cs.LG

TL;DR: The paper introduces the minimum enclosing ball (MEB) validity condition for robust aggregation in federated learning, addressing limitations of convex validity and proving its practicality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the shortcomings of convex validity in Byzantine-tolerant federated learning by proposing a more flexible and resilient geometric guarantee.

Method: The method involves introducing MEB validity and its relaxed version (c-MEB), analyzing their resilience, and providing optimal aggregation rules like MinMax-MEB and standard aggregators.

Result: Results show that relaxed c-MEB validity is achievable with a majority of honest clients (n>2t) and connects validity conditions in distributed computing and Byzantine-tolerant aggregation.

Conclusion: The conclusion highlights that relaxed MEB validity offers a practical alternative to convex validity, bridging gaps in Byzantine-robust aggregation research.
...
### [290] [CHoE: Cross-Domain Heterogeneous Graph Prompt Learning via Structure-Conditioned Experts](https://arxiv.org/abs/2605.15888)
*Peiyuan Li,Yongqi Huang,Jitao Zhao,Dongxiao He,Di Jin,Weixiong Zhang*

Main category: cs.LG

TL;DR: CHoE is a cross-domain HGPL method that uses expert networks and prompt tuning to improve performance in heterogeneous graph settings.


<details>
  <summary>Details</summary>
Motivation: Existing HGPL methods are limited to in-domain scenarios, but real-world applications often span multiple domains with different data distributions, causing performance degradation.

Method: CHoE introduces structure-conditioned experts during pre-training and uses a structure-aware expert routing mechanism during prompt tuning. It also includes a prompt-based semantic fusion module to integrate multi-view representations.

Result: CHoE consistently outperforms baseline approaches in few-shot cross-domain applications.

Conclusion: CHoE effectively addresses the limitations of existing HGPL methods by enabling cross-domain applications and improving performance.
...
### [291] [Context-aware Entity-Relation Extraction for Threat Intelligence Knowledge Graphs](https://arxiv.org/abs/2605.15904)
*Inoussa Mouiche,sherif Saad*

Main category: cs.LG

TL;DR: The paper introduces CTiKG, a framework for constructing Cybersecurity Knowledge Graphs (CKGs) from unstructured CTI reports, improving accuracy in entity and relation extraction compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in extracting structured threat intelligence from unstructured reports, such as complex structures and semantic ambiguity, which hinder existing pipeline-based approaches.

Method: The proposed CTiKG framework uses hybrid NLP models with SecureBERT+ embeddings and domain ontology to enhance entity and relation extraction accuracy.

Result: Experiments show CTiKG achieves 3-4% better NER and up to 8% better RE performance than baselines, validated on DNRTI and STUCCO benchmarks.

Conclusion: CTiKG offers a robust solution for building CKGs, with released datasets to support reproducibility and further research.
...
### [292] [LoCO: Low-rank Compositional Rotation Fine-tuning](https://arxiv.org/abs/2605.15916)
*An Nguyen,Jaesik Choi,Anh Tong*

Main category: cs.LG

TL;DR: LoCO introduces a novel PEFT method using low-rank skew-symmetric matrices and compositional rotation chains for efficient fine-tuning, preserving representation geometry and enabling parallel computation.


<details>
  <summary>Details</summary>
Motivation: Existing PEFT methods like low-rank adaptations lack geometric structure preservation in pretrained representations, prompting the need for a more efficient and structured approach.

Method: LoCO constructs orthogonal transformations via low-rank skew-symmetric matrices and compositional rotation chains, with a parallel computation scheme for practical high-dimensional use.

Result: LoCO outperforms or competes with both orthogonal and non-orthogonal methods in tasks like diffusion transformer fine-tuning, vision transformer adaptation, and language model adaptation.

Conclusion: LoCO offers a parameter-efficient, geometrically preserving fine-tuning method with practical computational benefits, validated across diverse domains.
...
### [293] [A Retrieval-Enhanced Transformer for Multi-Step Port-of-Call Sequence Prediction in Global Liner Shipping](https://arxiv.org/abs/2605.15937)
*Yanzhao Su,Fang He,Yineng Wang*

Main category: cs.LG

TL;DR: The paper proposes a CCRE framework for multi-step port-of-call sequence prediction, leveraging retrieval-enhanced historical data and adaptive fusion to improve accuracy and strategic stability.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to address unreliable voyage schedules and lack visibility beyond the immediate next port, necessitating a more robust solution for tactical resource orchestration.

Method: Uses a retrieval-enhanced historical encoder and Transformer-based trajectory encoder for adaptive fusion, along with an autoregressive Transformer decoder enriched with Scheduled Sampling and Gumbel-Softmax relaxation.

Result: Achieves 72.3% first-destination accuracy and 61.4% average three-step accuracy, outperforming baselines like CatBoost and LSTM by significant margins.

Conclusion: CCRE demonstrates scalability and effectiveness in capturing complex routing patterns, offering a significant improvement over existing methods for maritime logistics.
...
### [294] [When and Why Adversarial Training Improves PINNs: A Neural Tangent Kernel Perspective](https://arxiv.org/abs/2605.15959)
*Yuan-dong Cao,Chi Chiu SO,Jun-Min Wang,He Wang*

Main category: cs.LG

TL;DR: The paper introduces an adversarial training framework for PINNs, explaining its effectiveness, analyzing GAN variants, and proposing a practical algorithm to improve PINN training and performance.


<details>
  <summary>Details</summary>
Motivation: PINNs suffer from training difficulties like spectral bias and poor accuracy on high-frequency solutions. Adversarial training has shown empirical success, but lacks theoretical understanding.

Method: A new analysis framework for adversarially trained PINNs, leveraging GAN dynamics, is proposed to explain effectiveness, unify GAN variants, and develop an efficient training algorithm.

Result: The method significantly reduces PINNs' training issues, often outperforming alternatives by several orders of magnitude in accuracy.

Conclusion: The framework provides theoretical grounding for adversarial training in PINNs and leads to a practical, high-performance algorithm.
...
### [295] [Entropy-Based Characterisation of the Polarised Regime in Latent Variable Models](https://arxiv.org/abs/2605.15965)
*Peter Clapham,Lisa Bonheme,Marek Grzes*

Main category: cs.LG

TL;DR: The paper introduces an information-theoretic classification for identifying active, passive, and mixed dimensions in VAEs, overcoming limitations of prior-dependent criteria. It links entropy to KL minimization and validates the approach across varied models.


<details>
  <summary>Details</summary>
Motivation: Existing methods for identifying active dimensions in VAEs rely on Gaussian priors, restricting their use. This work aims to provide a more universally applicable criterion based on entropy.

Method: The authors propose an entropy-based classification for the polarized regime in VAEs, supported by theoretical links to KL minimization and empirical testing across multiple VAE models.

Result: The entropy criterion effectively identifies polarized regimes in diverse models. Passive dimensions, when normalized, can improve downstream tasks, indicating collapse is scale-related.

Conclusion: The study presents a robust entropy-based framework for analyzing VAEs, demonstrating its utility across models and hinting at practical benefits of passive dimensions in downstream tasks.
...
### [296] [Accelerated Gradient Descent for Faster Convergence with Minimal Overhead](https://arxiv.org/abs/2605.16017)
*Manuel Graca,L. Miguel Silveira,Arlindo Oliveira,Frank Liu*

Main category: cs.LG

TL;DR: CT-AGD is a curvature-aware optimization method for deep learning, reducing training epochs by 33% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To address inefficiencies in non-convex optimization for deep learning by leveraging local curvature information.

Method: CT-AGD uses finite-difference quotients to capture curvature and includes heuristics to reduce noise and bias from mini-batch training.

Result: Achieves same accuracy as baselines but reduces training epochs by 33% on average.

Conclusion: CT-AGD is an efficient alternative to first-order methods, balancing performance and computational overhead.
...
### [297] [Variational Autoregressive Networks with probability priors](https://arxiv.org/abs/2605.16020)
*Piotr Białas,Piotr Korcyl,Tomasz Stebel,Dawid Zapolski*

Main category: cs.LG

TL;DR: Incorporating physical priors into neural-network-based samplers improves efficiency in Monte Carlo methods for spin models, reducing training difficulty and enabling larger simulations.


<details>
  <summary>Details</summary>
Motivation: Monte Carlo methods suffer from critical slowing down near phase transitions. Existing neural-network-based samplers are hard to train due to overly general architectures that ignore physical symmetries.

Method: The authors propose a framework that uses a physics-informed prior probability distribution for training, building on strategies integrating spin-spin interactions.

Result: The approach significantly enhances performance in the Ising and Edwards-Anderson spin glass models, reducing training burden and enabling larger system simulations.

Conclusion: Physics-informed priors outperform 'blank slate' models, making neural-network-based samplers more efficient for discrete spin models.
...
### [298] [Mind Dreamer: Untethering Imagination via Active Latent Intervention on Latent Manifolds](https://arxiv.org/abs/2605.16030)
*Shaojun Xu,Xiaoling Zhou,Yihan Lin,Yapeng Meng,Xinglong Ji,Luping Shi,Rong Zhao*

Main category: cs.LG

TL;DR: Mind Dreamer (MD) uses Active Latent Intervention to overcome Markovian constraints, improving sample efficiency in reinforcement learning by synthesizing non-continuous latent jumps and resolving credit assignment issues.


<details>
  <summary>Details</summary>
Motivation: Current MBRL methods are limited by Historical Tethering, where imagination starts from observed states, creating a learning asymmetry between world model discovery and policy optimization.

Method: MD introduces Active Latent Intervention (ALI) and a learned generator to create non-continuous latent jumps. It uses Relay Value Function (RVF) and Relay Uncertainty Function (RUF) for credit assignment across spatial ruptures.

Result: MD achieves a 1.67x average speedup over DreamerV3 on DeepMind Control Suite and up to 8.8x in sparse-reward tasks.

Conclusion: MD effectively addresses the limitations of MBRL by minimizing a global Relay Manifold Expected Free Energy and expanding the manifold's spectral gap, leading to faster convergence in sparse-reward scenarios.
...
### [299] [MIND: Decoupling Model-Induced Label Noise via Latent Manifold Disentanglement](https://arxiv.org/abs/2605.16081)
*Dayong Ren*

Main category: cs.LG

TL;DR: The paper proposes MIND, a framework to tackle systematic label noise from pre-trained models by decoupling it into subspace-dependent components, outperforming existing methods on complex benchmarks.


<details>
  <summary>Details</summary>
Motivation: The challenge of model-induced label noise, stemming from annotator biases, is not well-addressed by current robust learning methods, necessitating a new approach.

Method: MIND decouples high-dimensional noise into tractable subspaces via Latent Manifold Disentanglement, using the Latent Decoupling Estimator (LDE) for dynamic projection.

Result: MIND significantly surpasses state-of-the-art methods on benchmarks like CIFAR-100 and real-world 3D datasets (S3DIS, ScanNet), and corrects zero-shot hallucinations in Vision-Language Models.

Conclusion: MIND offers a robust solution for handling systematic label noise in data-hungry applications, demonstrating its potential as a distillation framework for Foundation Models.
...
### [300] [Ada-Diffuser: Latent-Aware Adaptive Diffusion for Decision-Making](https://arxiv.org/abs/2605.16054)
*Fan Feng,Selena Ge,Minghao Fu,Zijian Li,Yujia Zheng,Zeyu Tang,Yingyao Hu,Biwei Huang,Kun Zhang*

Main category: cs.LG

TL;DR: A new framework, Ada-Diffuser, integrates latent dynamic inference into generative decision-making for improved planning and control.


<details>
  <summary>Details</summary>
Motivation: Existing approaches ignore latent factors critical for dynamics and decision-making, limiting their effectiveness.

Method: Proposes a unified framework with latent dynamic inference, using Ada-Diffuser, a causal diffusion model.

Result: Theoretically and experimentally shows accurate latent inference and adaptive policy learning in simulations.

Conclusion: Ada-Diffuser effectively models latent dynamics, enhancing decision-making in control and robotic tasks.
...
### [301] [SAFE Quantum Machine Learning with Variational Quantum Classifiers](https://arxiv.org/abs/2605.16067)
*Ying Chen,Paolo Giudici,Vasily Kolesnikov,Paolo Recchia*

Main category: cs.LG

TL;DR: A variational quantum classifier using amplitude encoding with a classical pre-encoding layer achieves competitive performance and balanced reliability in safety-critical settings.


<details>
  <summary>Details</summary>
Motivation: To develop a stable and reliable quantum classifier for high-dimensional data, addressing challenges in robustness and explainability.

Method: Combines normalized amplitude embeddings with bounded quantum observables, stabilized by a learnable classical pre-encoding layer. Uses SAFE-AI metrics for evaluation.

Result: The quantum model matches classical baselines in predictive performance and shows better robustness, noise resistance, and stability.

Conclusion: Variational quantum circuits provide a principled approach for stable, reliable learning in safety-critical applications.
...
### [302] [ITGPT: Generative Pretraining on Irregular Timeseries](https://arxiv.org/abs/2605.16069)
...
### [303] [Multi-level Self-supervised Pretraining on Compositional Hierarchical Graph for Molecular Property Prediction](https://arxiv.org/abs/2605.16088)
*Xiayu Liu,Zhengyi Lu,Hou-biao Li*

Main category: cs.LG

TL;DR: MolCHG is a self-supervised pretraining framework for molecular property prediction using a multi-level compositional hierarchical graph, achieving top performance on most benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for molecular property prediction lack multi-level structural granularity and treat bond information as secondary. MolCHG addresses this by independently modeling bond-level information.

Method: MolCHG uses a Compositional Hierarchical Graph with four node types across three levels. It includes atom-bond cross-view contrastive learning, fragment-level functional group prediction, and graph-level structure prediction tasks.

Result: Outperforms baselines on seven of nine MoleculeNet benchmarks, with competitive performance on the remaining two. Ablation studies confirm the complementary nature of its components.

Conclusion: MolCHG demonstrates the effectiveness of multi-level self-supervised pretraining and independent bond-level modeling for improved molecular property prediction.
...
### [304] [Centralized vs Decentralized Federated Learning: A trade-off performance analysis](https://arxiv.org/abs/2605.16089)
*Chaimaa Medjadji,Guilain Leduc,Sylvain Kubler,Yves Le Traon*

Main category: cs.LG

TL;DR: The paper compares three Federated Learning architectures (CFL, DFL, SDFL) experimentally using Fedstellar simulator, MNIST dataset, and MLP classifier to analyze their strengths, limitations, and trade-offs.


<details>
  <summary>Details</summary>
Motivation: With the rise of IoT devices, centralized data storage faces challenges like privacy and communication issues. FL offers a solution, but few studies compare its architectures (CFL, DFL, SDFL) to guide optimal usage.

Method: Experimental analysis using Fedstellar simulator, MNIST dataset, and MLP classifier to evaluate and compare CFL, DFL, and SDFL.

Result: The study provides insights into the strengths, limitations, and trade-offs of each FL architecture.

Conclusion: This research fills the gap in comparative analysis of FL architectures, aiding in informed decision-making for applications.
...
### [305] [Federated Imputation under Heterogeneous Feature Spaces](https://arxiv.org/abs/2605.16099)
*Imane Hocine,Chaimaa Medjadji,Sylvain Kubler,Gregoire Danoy,Yves Le Traon*

Main category: cs.LG

TL;DR: FedHF-Impute improves federated imputation by using a global feature graph for cross-client knowledge transfer, outperforming baselines in heterogeneous feature spaces.


<details>
  <summary>Details</summary>
Motivation: Existing FL methods assume aligned feature schemas, which is unrealistic in tabular settings with partially overlapping features. This limits effectiveness for federated imputation.

Method: Proposes FedHF-Impute, which separates structural feature unavailability from missingness and uses a global feature graph to propagate information via message passing.

Result: Outperforms FL baselines by 26.9% and 8.4% in RMSE on SECOM and AirQuality datasets, and matches performance on PhysioNET with only a 0.3% difference.

Conclusion: FedHF-Impute effectively addresses heterogeneous feature spaces in FL, enabling better imputation accuracy without compromising federated communication.
...
### [306] [Attention Dispersion in Dynamic Graph Transformers: Diagnosis and a Transferable Fix](https://arxiv.org/abs/2605.16112)
...
### [307] [Multi-Fidelity Flow Matching: Cascaded Refinement of PDE Solutions](https://arxiv.org/abs/2605.16118)
*Sipeng Chen,Junliang Liu,Hewei Tang,Shibo Li*

Main category: cs.LG

TL;DR: MFFM leverages residual-calibrated source noise and conditioning on low-fidelity solutions to improve flow-matching training, achieving efficient multi-resolution refinement for PDE solutions.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of refining parametric PDE solutions efficiently by exploiting the flexibility of source distribution in conditional flow matching.

Method: Uses a cascade refinement framework with local Gaussian-blur correlation for source calibration, conditions the velocity network on low-fidelity solutions, and fine-tunes the cascade end-to-end with a deterministic one-step rollout.

Result: Achieves efficient refinement with one velocity evaluation per cascade level, validated on eight benchmarks including super-resolution and spatiotemporal forecasting tasks.

Conclusion: MFFM provides a learned analog of multigrid refinement, enabling high-fidelity PDE solutions with deterministic efficiency.
...
### [308] [Entropy Across the Bridge: Conditional-Marginal Discretization for Flow and Schrödinger Samplers](https://arxiv.org/abs/2605.16126)
*Bruno Trentini,Dejan Stancevic,Michael M. Bronstein,Alexander Tong,Luca Ambrogioni*

Main category: cs.LG

TL;DR: The paper introduces a conditional-marginal entropy-rate objective for optimizing inference-time scheduling in flow-based generative models, improving sample quality under low-budget inference.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of sample quality degradation in flow-based generative models when inference budgets are limited, by optimizing where the sampler spends its few function evaluations.

Method: The authors derive a conditional-marginal entropy-rate objective to separate bridge geometry from marginal flow evolution. This is used to create a training-free, entropic inference-time scheduler that prioritizes boundary-heavy nonuniform grids.

Result: Experiments on Gaussian Brownian bridges, trained 2D models, EDM/CIFAR-10, and AlphaFlow protein generation show significant improvements in sample quality. For example, 10-step ODE-Heun MMD improved by 18.1%, and 5-step FID on CIFAR-10 reached 186.3.

Conclusion: The entropy-rate scheduling method is proven effective for low-budget inference, offering practical benefits for high-dimensional bridge and flow samplers across various benchmarks.
...
### [309] [Navigating Potholes with Geometry-Aware Sharpness Minimization](https://arxiv.org/abs/2605.16134)
*Simon Dufort-Labbé,Mehrab Hamidi,Razvan Pascanu,Ioannis Mitliagkas,Damien Scieur,Aristide Baratin*

Main category: cs.LG

TL;DR: LLQR+SAM integrates SAM with a learned preconditioner from LLQR to enhance sharpness-aware optimization, outperforming standalone SAM and LLQR in benchmarks.


<details>
  <summary>Details</summary>
Motivation: The research addresses SAM's uniform treatment of all parameter directions by leveraging LLQR's learned geometry for more effective sharpness-aware minimization.

Method: The approach combines SAM with a learned preconditioner from LLQR, updated sparsely and averaged to capture loss landscape geometry, enabling faster curvature probing.

Result: LLQR+SAM outperforms SAM and LLQR individually in vision and sequence modeling tasks, validating the synergy between slow learned geometry and fast sharpness correction.

Conclusion: The study demonstrates that combining slow learned geometry with fast sharpness correction (LLQR+SAM) is more effective than using either method alone, offering consistent performance gains.
...
### [310] [Surrogate Neural Architecture Codesign Package (SNAC-Pack)](https://arxiv.org/abs/2605.16138)
*Jason Weitz,Dmitri Demler,Benjamin Hawks,Aaron Wang,Nhan Tran,Javier Duarte*

Main category: cs.LG

TL;DR: SNAC-Pack is an AutoML framework for hardware-aware neural architecture codesign, optimizing for FPGA deployment with multi-objective search and surrogate modeling to avoid costly synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing NAS methods focus on accuracy or use poor proxy metrics like BOPs, lacking consideration for FPGA hardware costs. SNAC-Pack addresses this gap.

Method: Uses multi-objective global search (Optuna, NSGA-II), hardware surrogate modeling, local search with QAT and pruning, and FPGA synthesis via hls4ml.

Result: Demonstrated on jet classification and qubit readout, SNAC-Pack matches/exceeds baselines while reducing FPGA resource use and exploration time.

Conclusion: SNAC-Pack automates FPGA-optimized NAS, significantly improving efficiency and reducing manual effort in hardware-aware model design.
...
### [311] [Learn Where Outcomes Diverge: Efficient VLA RL via Probabilistic Chunk Masking](https://arxiv.org/abs/2605.16154)
*Vaidehi Bagaria,Nikshep Grampurohit,Pulkit Verma*

Main category: cs.LG

TL;DR: PCM optimizes GRPO-based VLA RL by selectively computing gradients for impactful trajectory chunks, achieving similar success rates with faster training and lower memory usage.


<details>
  <summary>Details</summary>
Motivation: GRPO's uniform gradient computation across trajectories is inefficient, as many phases contribute little to learning. PCM addresses this inefficiency.

Method: PCM probabilistically selects and computes gradients for chunks with high learning potential, using success-failure action variance as a proxy for gradient variance.

Result: PCM matches GRPO's success rates while being 2.38× faster, 4.8× faster in gradient updates, and using 60% less memory, processing fewer than 20% of chunks.

Conclusion: PCM efficiently allocates gradient computation, significantly speeding up GRPO-based RL without sacrificing performance.
...
### [312] [Entropic Auto-Encoding via Implicit Free-Energy Minimization](https://arxiv.org/abs/2605.16164)
*Hazhir Aliahmadi,Irina Babayan,Greg van Anders*

Main category: cs.LG

TL;DR: EAEs mitigate posterior collapse in VAEs by using an ensemble of encoders and entropy-driven priors, achieving diverse, data-consistent generations and preserving underlying data structure.


<details>
  <summary>Details</summary>
Motivation: To address the failure mode of VAEs (posterior collapse) where latent variables are ignored due to explicit prior imposition, the paper introduces EAEs.

Method: EAEs use reconstruction loss as the sole explicit objective, with entropy implicitly generating latent priors through a free energy-minimizing encoder ensemble. This approach biases learning toward informative latent representations.

Result: EAEs learn non-Gaussian, multimodal latent distributions, enabling diverse generations and preserving data structure. Successfully applied to reaction-diffusion, MNIST, and CelebA datasets.

Conclusion: EAEs effectively mitigate posterior collapse, learning rich latent representations and demonstrating versatility across diverse datasets.
...
### [313] [BAPR: Bayesian amnesic piecewise-robust reinforcement learning for non-stationary continuous control](https://arxiv.org/abs/2605.16170)
*Yifan Zhang,Liang Zheng*

Main category: cs.LG

TL;DR: BAPR combines Bayesian change detection with robust RL to handle abrupt regime changes, balancing performance and safety.


<details>
  <summary>Details</summary>
Motivation: Real-world control systems operate under piecewise stationary conditions, requiring methods that avoid the conservatism of robust RL and the risks of adaptive policies during abrupt changes.

Method: BAPR integrates Bayesian Online Change Detection (BOCD) with robust ensemble RL, using a convex combination of Bellman operators weighted by a frozen belief distribution. It includes a context-conditioning module for mode-aware representations.

Result: BAPR ensures a γ-contraction, with formal error bounds and verified theorems. It adapts conservatism post-change, with O(log(1/δ)) detection delay, and requires no mode labels during deployment.

Conclusion: BAPR effectively addresses the challenges of piecewise stationary environments, balancing performance and robustness, with all results formally verified.
...
### [314] [Imitation learning for clinical decision support in pediatric ECMO](https://arxiv.org/abs/2605.16175)
...
TL;DR: The paper proposes using imitation learning to model pediatric ECMO decision-making, testing TabPFN against traditional methods like XGBoost and MLPs, with TabPFN outperforming them.


<details>
  <summary>Details</summary>
Motivation: To improve decision support in pediatric ECMO by addressing data scarcity and high complexity.

Method: Frames decision-making as imitation learning, comparing TabPFN with XGBoost and MLPs on real-world ECMO data.

Result: TabPFN consistently outperforms traditional baselines.

Conclusion: TabPFN is a strong baseline for pediatric ECMO decision support.
...
### [315] [Hypothesis-driven construction of mesoscopic dynamics](https://arxiv.org/abs/2605.16211)
*Zhuoyuan Li,Aiqing Zhu,Qianxiao Li*

Main category: cs.LG

TL;DR: The paper proposes a new paradigm for learning mesoscopic dynamics using a mathematically constrained hypothesis class, offering theoretical guarantees and interpretable models, validated on both known and unknown systems.


<details>
  <summary>Details</summary>
Motivation: Traditional scientific modeling with fixed equations is challenging for complex systems like multiscale ones, prompting the need for an alternative approach.

Method: Utilizes a generalized Onsager principle to create a unified framework for dissipative and conservative dynamics, ensuring theoretical guarantees before learning.

Result: The framework provides accurate, robust, and interpretable models, validated on continuum PDEs and microscopic chain models.

Conclusion: The approach effectively learns dynamics while offering interpretable insights into underlying physics, bridging gaps in traditional modeling.
...
### [316] [The Privacy Price of Tail-Risk Learning: Effective Tail Sample Size in Differentially Private CVaR Optimization](https://arxiv.org/abs/2605.16219)
...
### [317] [Dynamics-Level Watermarking of Flow Matching Models with Random Codes](https://arxiv.org/abs/2605.16239)
*Shuchan Wang*

Main category: cs.LG

TL;DR: A dynamics-level watermarking method for generative models embeds watermarks in the model's velocity field without altering output quality.


<details>
  <summary>Details</summary>
Motivation: To ensure secure and reliable watermarking of generative models without compromising generation quality.

Method: Embedding watermarks in the learned dynamics (velocity field) of flow matching models using random coding.

Result: Successful message recovery on MNIST and CIFAR-10, preserved generation quality, and secure decoding without the key.

Conclusion: The approach effectively watermarks generative models while maintaining output quality and security.
